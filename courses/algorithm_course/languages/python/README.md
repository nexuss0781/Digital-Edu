...
8. **@functools.cache** — Automatic Memoization (Saving results to avoid repeating work) for dynamic programming
...
# Memoization (Saving results to avoid repeating work) for DP
@cache
def dp(i, j):
...
### Dynamic Programming
- Practice with `@cache` decorator for Memoization (Saving results to avoid repeating work)
