...
def fibonacci(n: int, cache: dict[int, int] = {}) -> int:
    """Fibonacci with Memoization (Saving results to avoid repeating work) using mutable default."""
    if n in cache:
        return cache[n]
    if n < 2:
        return n
...
def fibonacci(n: int) -> int:
    """Fibonacci with proper Memoization (Saving results to avoid repeating work)."""
    if n < 2:
        return n
...
```python
# BAD: O(n²) Time Complexity (Speed of the algorithm) due to string immutability
...
String concatenation with `+=` creates a new string object each iteration because strings are immutable. This is O(n²) Time Complexity (Speed of the algorithm):
