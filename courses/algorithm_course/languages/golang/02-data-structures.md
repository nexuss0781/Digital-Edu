...
// ---------------------------------
// 5. Memoization (Saving results to avoid repeating work) (Caching Results)
...
**Definition**: A Stack is a **LIFO (Last-In-First-Out, like a stack of plates)** (Last-In, First-Out) data structure.
...
**Definition**: A Queue is a **FIFO (First-In-First-Out, like a line at a store)** (First-In, First-Out) data structure.
...
* **Data Structure**: Uses a **Queue** (FIFO (First-In-First-Out, like a line at a store)) to keep track of what to visit next.
...
func bfs(start int, graph map[int][]int) {
    ...
    queue := []int{start} // Slice acting as a FIFO (First-In-First-Out, like a line at a store) queue
...
**Definition**: A Linked List is a chain of nodes. Each node holds **Data** and a **Pointer** (clue) to the next node. Unlike Arrays, they are NOT stored in one Connected sequence of memory blocks.
