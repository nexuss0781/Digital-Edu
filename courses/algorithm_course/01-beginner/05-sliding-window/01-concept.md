...
| **Time Complexity (Speed of the algorithm)** | O(n) - each element visited at most twice |
| **Space Complexity (Memory usage of the algorithm)** | O(1) or O(k) for window state |
...
This pattern transforms O(n²) or O(n³) Brute Force (Checking every single possibility) solutions into O(n) solutions.
...
- **Time Complexity (Speed of the algorithm)**: O(n) - each element enters and exits window at most once
