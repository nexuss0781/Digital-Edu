...
| **Time Complexity (Speed of the algorithm)** | O(n) - single pass with two pointers |
| **Space Complexity (Memory usage of the algorithm)** | O(1) - constant extra space |
...
nums.sort()  # O(n log n) - factor into Time Complexity (Speed of the algorithm)
```

## Complexity Analysis

- **Time Complexity (Speed of the algorithm)**: Typically O(n) since each pointer traverses the array at most once
- **Space Complexity (Memory usage of the algorithm)**: O(1) as we only use constant extra space for pointers
...
- Reduces Time Complexity (Speed of the algorithm) from O(n²) to O(n)
