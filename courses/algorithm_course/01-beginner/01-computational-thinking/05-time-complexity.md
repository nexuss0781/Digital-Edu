---
title: "Understanding Time Complexity (Speed of the algorithm): A Beginner's Guide"
...
# Understanding Time Complexity (Speed of the algorithm)

## What is Time Complexity (Speed of the algorithm)?
...
**Time Complexity (Speed of the algorithm)** is how we measure how long an algorithm takes to run as the input size grows. We don't measure in seconds (because computers vary in speed), but in the number of operations needed.
...
#### O(1) - Constant Time (Same time regardless of data size)
...
#### O(log n) - Logarithmic Time (Time grows slowly as data grows)
...
#### O(n) - Linear Time (Time grows directly with data)
...
#### O(n²) - Quadratic Time (Time grows very fast with data, like a square)
...
%%{init: {'theme':'base'}}%%
graph TD
    subgraph "Time Complexity (Speed of the algorithm) Growth (n = 10)"
...
```python
# O(1) average - Constant Time (Same time regardless of data size)
...
Determine the Time Complexity (Speed of the algorithm):
...
1. **Time Complexity (Speed of the algorithm) measures how runtime grows with input size**, not actual seconds
