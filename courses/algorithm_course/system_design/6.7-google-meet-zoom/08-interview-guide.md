...
"Single SFU becomes a Slowest part of the process beyond 50-100 participants.
...
|---|---|
| **AV1 SVC replacing Simulcast** | Single encoded stream with spatial/temporal layers; SFU drops enhancement layers per subscriber; eliminates 1.5-2x upstream bandwidth overhead; discuss hardware decode requirements (M1+, Intel 12th gen+) |
| **WebTransport for signaling** | QUIC-based multiplexed transport; each control stream gets independent congestion control; connection migration survives WiFi↔cellular without ICE restart; potential WebSocket replacement |
| **MLS protocol for E2EE** | RFC 9420; O(log N) group key update via ratchet tree; discuss scalability limits (tree update latency at 100+ participants) and forward/post-compromise security properties |
| **AI meeting intelligence** | Real-time transcription → summarization → action items pipeline; discuss where each stage runs (client-side vs server-side) and E2EE compatibility (server-side requires cleartext) |
| **Neural audio codecs** | Lyra/SoundStream achieve intelligible speech at 3-6 Kbps vs 32 Kbps Opus; discuss trade-off: requires ML inference on every audio frame vs traditional codec |

### Distinguishing "Great" Answers

| Question Area | Good Answer | Great Answer |
|---|---|---|
| **SFU architecture** | "SFU forwards packets without transcoding" | "SFU rewrites RTP headers (SSRC, sequence numbers) to hide layer switches from decoders; caches keyframes to prevent PLI storms; runs per-subscriber bandwidth estimation via TWCC at 10 Hz" |
| **NAT traversal** | "Use STUN and TURN for connectivity" | "ICE candidates gathered in priority order (host > srflx > relay); ~15% require TURN at 2x bandwidth cost; periodic ICE restart attempts to find direct path after initial TURN fallback; TURN over TCP/443 as last resort" |
| **Recording** | "Record all streams on the server" | "Recording bot joins as hidden SFU subscriber; captures individual tracks in 5-minute segments uploaded to object storage; post-meeting MCU compositing with active-speaker layout; recording is fundamentally MCU-like, not SFU-like" |
| **E2EE** | "Encrypt media end-to-end" | "Insertable Streams API encrypts frame payload before RTP packetization; SFU reads headers for routing but cannot access content; MLS key schedule provides forward secrecy with O(log N) update cost; disables all server-side features by design" |
| **Congestion control** | "Reduce quality when bandwidth drops" | "GCC uses Kalman filter on inter-arrival time deltas (TWCC feedback) to estimate available bandwidth; AIMD pattern (additive increase, multiplicative decrease); loss-based correction overlaid on delay-based estimation; per-subscriber, not per-room" |
