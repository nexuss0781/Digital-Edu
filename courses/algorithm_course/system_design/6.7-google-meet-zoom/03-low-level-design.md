...
PROCEDURE:
    ...
    FOR EACH packetGroup IN twccFeedback.groups:
        ...
        IF estimatedGradient > OVERUSE_THRESHOLD:
            state = OVERUSE            // Queue building up at Slowest part of the process
...
**Complexity**: O(1) per feedback packet. The Kalman filter maintains constant state (2x2 matrix) regardless of history length. TWCC feedback arrives roughly every 100ms, so this runs at ~10 Hz per connection. The AIMD (Additive Increase Multiplicative Decrease) pattern converges to fair bandwidth share when multiple streams compete on the same Slowest part of the process link.
...
| Algorithm | Time Complexity (Speed of the algorithm) | Space Complexity (Memory usage of the algorithm) | Execution Frequency |
|-----------|----------------|------------------|---------------------|
