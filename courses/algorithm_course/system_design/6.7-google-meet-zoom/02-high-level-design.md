...
%%{init: {'theme': 'neutral'}}%%
stateDiagram-v2
    [*] --> Scheduled: Create/schedule meeting

    Scheduled --> Provisioning: First participant clicks join
    Provisioning --> WaitingRoom: Waiting room enabled
    Provisioning --> Active: No waiting room

    WaitingRoom --> Active: Host admits participants
    WaitingRoom --> Abandoned: Host never joins (15 min timeout)

    Active --> Active: Participants join/leave
    Active --> TopologyUpgrade: 3rd participant joins P2P call
    TopologyUpgrade --> Active: SFU established, media renegotiated

    Active --> GracePeriod: Last participant disconnects
    GracePeriod --> Active: Participant rejoins within 60s
    GracePeriod --> Ended: Grace period expires

    Active --> Ended: Host ends meeting for all

    Ended --> PostProcessing: Recording enabled
    PostProcessing --> Archived: Compositing + storage complete
...
| Meeting State | SFU Resources | Signaling Resources | AI Resources |
|---|---|---|---|
| Scheduled | None allocated | Meeting metadata in DB | None |
| Provisioning | SFU node selected, ports allocated | WebSocket connection established | Model warm-up if captions enabled |
| Active (< 3 users) | P2P preferred; SFU standby | Roster management | Per-participant AI pipelines |
| Active (3-50 users) | Single SFU node, full simulcast | Full roster sync | Shared AI inference pool |
| Active (50-500 users) | Cascaded SFU (2-4 nodes) | Batched roster updates | Dedicated AI workers per meeting |
| Active (500+ users) | Multi-level cascade, Last-N video | Paginated roster, delta-only updates | Active-speaker-only AI processing |
| PostProcessing | Released | Released | Transcription workers if needed |

---

## AI-Assisted Meeting Features

| Feature | Processing Location | Latency Budget | Architecture Impact |
|---|---|---|---|
| **Noise Cancellation** | Client-side (WASM/GPU) | < 10ms per audio frame | Runs in AudioWorklet; no server dependency; model size ~2 MB; operates on raw PCM before Opus encoding |
| **Background Segmentation** | Client-side (GPU/WebGL) | < 16ms per video frame | Person segmentation model runs per frame; composites with virtual background before encoding; falls back to blur on low-end devices |
| **Live Captions** | Server-side (GPU cluster) | < 2 seconds end-to-end | Audio stream tapped at SFU, sent to ASR service; text returned via data channel; incompatible with E2EE |
| **Meeting Summarization** | Server-side (async) | Post-meeting (5-15 min) | Full transcript processed by LLM for key points, action items, decisions; stored with recording |
| **Real-Time Translation** | Server-side (GPU) | < 3 seconds | Transcription → translation → text-to-speech pipeline; each step adds latency; displayed as subtitle track per language |

---

## Emerging Technology Integration (2025–2026)

| Technology | Status | Architectural Impact |
|---|---|---|
| **AV1 SVC** | Production-ready; hardware decode on M1+, Intel 12th gen+ | Single encoded stream with spatial/temporal layers replaces simulcast; eliminates 1.5-2x upstream overhead; SFU drops enhancement layers per subscriber |
| **WebTransport** | Experimental; QUIC-based multiplexed transport | Potential WebSocket replacement for signaling; each meeting control stream gets independent congestion control; connection migration survives WiFi↔cellular handoff |
| **WebCodecs API** | Stable in Chrome/Edge | Direct access to video encoder/decoder from JavaScript; enables custom video processing pipelines (filters, watermarks) without full MediaStream pipeline overhead |
| **WHIP/WHEP** | IETF draft advancing | WebRTC-native broadcast ingest (WHIP) and playback (WHEP); replaces RTMP for low-latency live streaming; unifies conferencing and broadcasting stacks |
| **Neural Audio Codecs** | Research/early adoption | Lyra/SoundStream achieve intelligible speech at 3-6 Kbps; 5-10x more efficient than Opus minimum; requires on-device ML inference |
| **Per-Frame ML QoE** | Emerging | ML models estimate per-frame quality (blur, artifact, freeze) at the SFU without decoding; enables quality-aware routing decisions |

---

## Cross-Cutting Concerns

| Concern | How It Manifests | Design Response |
|---|---|---|
| **Clock synchronization** | Media timestamps must be coherent across participants for lip-sync; NTP-based sync has ~10ms accuracy | RTP timestamps use 90kHz clock derived from sender's wall clock; receiver uses jitter buffer to compensate for clock drift between sender and receiver |
| **Codec negotiation** | Different browsers support different codecs (Safari: H.264, Chrome: VP9/AV1); mandatory to agree during SDP exchange | SDP offer lists all supported codecs in preference order; SFU selects the highest-priority codec supported by both sides; transcoding avoided entirely |
| **Network migration** | Mobile users switch between WiFi and cellular; IP address changes mid-call | ICE restart re-gathers candidates without dropping the meeting; 1-3 second freeze during re-establishment; SRTP keys are rederived via new DTLS handshake |
| **Multi-device join** | Same user joins from laptop and phone simultaneously | Meeting service detects duplicate user_id; offers to transfer session or allows both devices with independent media subscriptions |
