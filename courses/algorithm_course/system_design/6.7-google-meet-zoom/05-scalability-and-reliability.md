...
## Operational Patterns (2025–2026)

### Diurnal Scaling Strategy

Video conferencing traffic follows predictable business-hour patterns across time zones. The scaling strategy exploits this predictability:

```
Time-Zone Cascade (UTC):
    00:00-06:00  AP-South peak (India, Southeast Asia)
    06:00-08:00  EU ramp-up overlapping AP tail
    08:00-12:00  EU + US-East simultaneous peak (highest global load)
    12:00-16:00  US-East peak + US-West ramp-up
    16:00-20:00  US-West peak, EU wind-down
    20:00-00:00  Global trough (maintenance window)

Pre-scaling rules:
    T-30 min before predicted regional peak: provision 20% additional SFU capacity
    T-0 at peak: autoscaler maintains 30% headroom
    T+2 hours after peak: begin draining excess nodes (no new rooms)
    T+3 hours: terminate drained nodes
```

### Capacity Planning Under Uncertainty

| Planning Scenario | Strategy | Buffer |
|---|---|---|
| **Normal weekday** | Time-zone-based pre-scaling from historical patterns | 30% headroom |
| **Major product launch / company all-hands** | Pre-provisioned dedicated SFU cluster; capacity reserved via API | 100% dedicated |
| **Viral event (unexpected 10x spike)** | Autoscaler burst capacity from warm pool; emergency shedding of free-tier meetings | 50% burst pool |
| **Holiday / off-peak** | Minimum fleet (2 nodes per region for redundancy) | No excess |
| **Regional incident** | Cross-region overflow routing; accept 50-100ms additional latency | Peer region absorbs |

### SFU Fleet Management

The SFU fleet is managed as a stateful service with special operational constraints:

1. **Rolling upgrades**: SFU nodes cannot be restarted mid-meeting. Upgrades mark nodes as "draining" (accept no new rooms), wait for all active rooms to complete naturally (median meeting: 45 min, p99: 4 hours), then upgrade the node. A fleet-wide upgrade takes 6-12 hours.
2. **Canary deployment**: New SFU versions are deployed to 1% of the fleet first. Media quality metrics (MOS, packet loss, join time) are compared against the baseline fleet. If any metric degrades beyond threshold, the canary is rolled back.
3. **Warm pool**: 10% of the fleet is kept as provisioned-but-unassigned "warm" nodes that can absorb traffic within 30 seconds (no cold-start provisioning delay).
4. **Bin-packing optimization**: Rooms are consolidated onto fewer nodes during off-peak hours to maximize drain efficiency and minimize cost. The orchestrator preferentially assigns new rooms to already-busy nodes (up to 80% utilization) rather than spreading evenly.

### Real-World Capacity Benchmarks

| Benchmark | Value | Context |
|---|---|---|
| SFU node capacity (commodity 32-core) | 150-250 rooms, ~2,000 concurrent streams | Limited by outbound NIC bandwidth (10 Gbps), not CPU |
| TURN server capacity (10 Gbps) | ~5,000 concurrent relay sessions | Bandwidth-bound, not compute-bound |
| Signaling server capacity | ~10,000 WebSocket connections per node | Memory-bound (connection state per socket) |
| Recording worker throughput (GPU) | 50 concurrent composite sessions | GPU memory and encode pipeline bound |
| Meeting orchestrator throughput | ~5,000 room assignments per second | Decision latency < 10ms per assignment |

### Cross-Region Cascade Latency Budget

```
Participant in Tokyo → SFU-Tokyo → Private Backbone → SFU-Virginia → Participant in New York

Segment breakdown:
    Tokyo participant → SFU-Tokyo:        ~15ms (first hop, edge PoP)
    SFU-Tokyo internal processing:        ~1ms  (packet forwarding)
    Tokyo → Virginia backbone:            ~80ms (physical distance, private fiber)
    SFU-Virginia internal processing:     ~1ms  (packet forwarding)
    SFU-Virginia → New York participant:  ~10ms (last hop, edge PoP)
    ─────────────────────────────────────────────
    Total one-way latency:                ~107ms

    Round-trip (mouth-to-ear):            ~214ms
    Assessment: Slightly above 200ms target. Acceptable for inter-continental
    meetings. Intra-region meetings (same SFU) achieve ~30-50ms round-trip.
```

### Cost-Driven Architecture Decisions

Infrastructure cost at scale drives several non-obvious architectural choices:

| Decision | Cost Driver | Annual Savings at Scale |
|---|---|---|
| **Simulcast → AV1 SVC migration** | 30-40% reduction in upstream bandwidth per participant; fewer TURN relay bytes | Significant egress savings across 31K SFU nodes |
| **Last-N video for large meetings** | 50+ participant meetings forward only top 5-10 speakers' video; remaining participants receive audio only | Reduces per-meeting SFU bandwidth by 80%+ for large rooms |
| **P2P for 1-on-1 calls** | Zero SFU cost for ~60% of all meetings (1-on-1 calls are the majority by count) | Eliminates SFU overhead for the largest meeting category |
| **Post-meeting recording** | GPU compositing runs as batch workload during off-peak hours | GPU utilization smoothed; no real-time GPU reservation per meeting |
| **TURN connection recycling** | Periodic ICE restart attempts to find direct path after initial TURN fallback | ~30% of initially-relayed connections eventually find a direct path |
| **Client-side AI processing** | Noise cancellation and background blur run on participant's device GPU | Eliminates server-side GPU allocation for the two most common AI features |

### Incident Response: SFU Region Failure Playbook

```
SEVERITY: P1 -- Region-wide SFU outage affecting active meetings

DETECTION (T+0 to T+30s):
    - SFU heartbeat monitors detect simultaneous failure of multiple nodes
    - Region health score drops below threshold (< 50% nodes healthy)
    - Global orchestrator receives cascading node-dead notifications

AUTOMATIC RESPONSE (T+30s to T+60s):
    - GeoDNS withdraws affected region from rotation for new meetings
    - All new meeting creation routes to nearest healthy region
    - Signaling servers push RECONNECT signal to all affected participants
    - Participants' clients resolve DNS to next-nearest SFU region

CLIENT RECOVERY (T+60s to T+90s):
    - Clients perform fresh ICE negotiation to new region SFU
    - New DTLS handshake and SRTP session established
    - Audio resumes first; video follows after bandwidth stabilization
    - Users experience 30-60 second interruption, then meeting continues

POST-INCIDENT (T+5min to T+1hr):
    - Verify all affected meetings recovered or gracefully ended
    - Check recording pipeline for gap detection and segment recovery
    - Publish incident summary to status page
