...
| Incident Type | Detection | Immediate Response | Recovery | Post-Incident |
|---|---|---|---|---|
| **Settlement account depletion** | Real-time balance monitoring; predicted depletion alert | Queue outbound PIX; continue inbound; emergency liquidity injection from treasury | Resume outbound payments once balance restored; process queued transfers in priority order | Root cause analysis of prediction model miss; adjust safety threshold |
| **DICT cache poisoning** | Cache entry validation against live DICT lookups (sampled); anomalous routing pattern detection | Flush suspect cache partition; route all lookups to live DICT; performance degrades but correctness preserved | Incremental cache rebuild from BCB sync feed; validate rebuilt entries against live DICT | Investigate poisoning vector; tighten sync protocol validation |
| **Fraud model degradation** | Daily precision/recall monitoring; sudden FPR spike or fraud volume increase | Switch to rule-based scoring (pre-calibrated conservative thresholds); alert ML engineering team | Deploy retrained model via canary; validate on shadow traffic before full rollout | Investigate feature drift; add monitoring for root cause features |
| **SPI connectivity loss** | RSFN link health monitoring; SPI heartbeat failure | Activate secondary RSFN link; if both down, queue outbound messages; continue accepting inbound via secondary routing | Reconnect primary link; drain outbound queue in sequence-number order; reconcile any gap | Network root cause analysis; validate dual-link independence |
| **Mass credential compromise** | Anomalous API key usage patterns (geographic spread, request volume); merchant reports | Revoke compromised API keys; issue replacement keys; block suspicious transactions from affected merchants | Merchant re-onboarding for affected accounts; audit all transactions during compromise window | Strengthen key issuance process; implement key binding to IP ranges |
| **SEFAZ integration failure (nationwide)** | All 27 state circuit breakers tripping simultaneously | Activate DPEC contingency mode for all states; NF-e generation continues in offline mode; payments unaffected | Queue retroactive SEFAZ authorization; process in batches as states come online | Coordinate with SEFAZ national infrastructure team; validate all contingency NF-e authorized |
...
|---|---|---|---|---|
| **Critical** | SPI signing keys, HSM master keys, RSFN credentials | HSM only; never in application memory beyond operation | Named operators only; dual-control for access | Indefinite (rotated, not deleted) |
| **Restricted** | PIX keys (CPF/CNPJ), payer personal data, fraud scores | Field-level encryption + at-rest encryption | Role-based; per-query audit logging; purpose limitation | Transaction data: 5 years; fraud features: 30 days post-deletion request |
| **Confidential** | Transaction amounts, merchant business data, settlement reports | At-rest encryption (AES-256-GCM) | Team-based access; audit log sampling | 10 years (financial records regulation) |
| **Internal** | System metrics, internal API schemas, architecture docs | Standard encryption | Employee access; NDA-covered | Duration of relevance |
| **Public** | API documentation, QR code specifications, developer guides | Unencrypted (integrity-signed) | Open access | Indefinite |

---

## Supply Chain Security

| Risk Area | Threat | Control |
|---|---|---|
| **Digital certificate procurement** | Compromised CA issues fraudulent ICP-Brasil certificates | Use only BCB-approved CAs; validate certificate chain to ICP-Brasil root; monitor Certificate Transparency logs for unauthorized issuances |
| **HSM firmware** | Tampered firmware exfiltrates signing keys | Procure HSMs only from FIPS 140-3 certified vendors; verify firmware signatures before deployment; maintain air-gapped HSM management workstations |
| **Tax rule data providers** | Incorrect or manipulated tax rate feeds cause systematic mis-taxation | Cross-validate tax rates against official Diário Oficial publications; dual-provider verification for ICMS rate changes; human approval for rate changes >2% |
| **DICT sync feed** | Manipulated sync messages redirect payments to attacker accounts | Validate DICT sync message signatures against BCB's public key; sequence number gap detection; reconcile cache against live DICT queries (sampled) |
| **Third-party ML model dependencies** | Compromised pre-trained model components introduce backdoors into fraud detection | Train all fraud models in-house on proprietary data; no pre-trained model import without full audit; model binary signing and integrity verification |

---

## Deployment Security for Financial-Critical Services

### SPI Gateway Deployment Protocol

1. **Pre-deployment validation:** Verify RSFN connectivity from canary node; validate SPI message format compatibility; confirm settlement account balance sufficient for post-deployment transaction processing
2. **Canary deployment:** Route 1% of SPI traffic to new version; monitor settlement success rate, message format validation errors, and latency percentiles for 30 minutes
3. **Progressive rollout:** 1% → 10% → 50% → 100% over 4 hours; each stage requires explicit operator approval based on SLI dashboards
4. **Rollback trigger:** Automatic rollback if settlement failure rate exceeds 0.01% or p99 latency exceeds 5 seconds during any rollout stage
5. **Post-deployment verification:** Reconciliation check comparing settlement confirmations received vs. expected; validate no orphaned transactions during deployment

**NEVER during SPI Gateway deployment:**
- Deploy during BCB-announced SPI maintenance windows (risk of confounding failures)
- Deploy both RSFN links simultaneously (always maintain one known-good link)
- Deploy on month-end days (peak transaction volume + merchant payout processing)
- Skip canary phase for "minor" changes (even config changes go through canary)
