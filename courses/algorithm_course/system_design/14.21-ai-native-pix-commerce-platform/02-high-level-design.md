...
|--------------|---------|----------|-----|
| Merchant → API Gateway | Request-response | HTTPS REST (mTLS for PSP) | Standard merchant API; mutual TLS for regulated communication |
| API Gateway → Transaction Orchestrator | Async queue | Internal message broker | Decouples gateway from processing; enables backpressure |
| Transaction Orchestrator → SPI | Request-response | ISO 20022 XML over RSFN | BCB-mandated protocol; synchronous settlement confirmation |
| Transaction Orchestrator → Fraud Engine | Request-response | gRPC (internal) | Low-latency synchronous call on critical path; 200ms budget |
| Transaction Orchestrator → NF Generator | Async event | Event stream | Decoupled from settlement; NF generation is not blocking |
| DICT Client → BCB DICT | Request-response + sync feed | RSFN protocol | Lookup (sync) + incremental cache update (async feed) |
| Settlement confirmed → Merchant | Webhook push | HTTPS POST with HMAC signature | Merchant notification; retried with exponential backoff |
| NF Generator → SEFAZ | Request-response | SOAP/XML web service | Government-mandated protocol; per-state endpoint |
| Mandate Manager → Billing Scheduler | Scheduled trigger | Internal cron + distributed lock | Time-based mandate processing with exactly-once guarantee |
| MED Handler → Fund Tracing | Event-driven | Internal event stream | Async graph traversal triggered by MED notification |
...
|-------------|-----------|-------|--------|----------------|
| **Pre-settlement fraud scoring** | Gradient-boosted ensemble + GNN | Device fingerprint, DICT metadata, velocity counters, amount patterns | Risk score (0-1), decision (approve/decline/step-up) | < 200ms |
| **Social engineering detection** | Behavioral sequence model | Interaction timing, session duration, call activity correlation | Social engineering probability | < 100ms (part of fraud pipeline) |
| **Mule account detection** | Graph neural network | Incoming transaction patterns, account age, payer diversity | Mule account probability | < 200ms |
| **PIX Automático churn prediction** | Time-series classifier | Mandate age, payment history, cancellation patterns | Churn probability (30-day) | Batch (daily) |
| **Tax classification recommendation** | Multi-label classifier | Product description, merchant category, historical classifications | Suggested NCM codes with confidence | < 500ms |
| **Dynamic pricing optimization** | Reinforcement learning | Time of day, demand signals, inventory, customer segment | Optimal price adjustment | < 100ms |
| **Reconciliation anomaly detection** | Isolation forest + rules | Settlement amounts, timing patterns, merchant baseline | Anomaly score per transaction | < 1s (post-settlement) |
| **Merchant risk scoring** | Gradient boosting | Transaction volume, fraud rate, chargeback/MED rate, business category | Risk tier (low/medium/high) | Batch (weekly) |
...
|----------|------|---------|--------|--------------|
| **ADR-001: Direct SPI connectivity** | Day 0 | Platform processes >500K accounts; direct vs. indirect PSP participation | Direct RSFN connection | Lower latency; full DICT access; higher operational overhead |
| **ADR-002: Async Nota Fiscal** | Day 0 | NF generation coupled vs. decoupled from settlement | Async post-settlement trigger | Payment SLO independent of SEFAZ availability; NF delivery delayed 2-10s |
| **ADR-003: Pre-settlement fraud scoring** | Day 0 | PIX irrevocability eliminates post-transaction recovery | Mandatory pre-settlement scoring with 200ms budget | Adds latency to every transaction; dramatically reduces fraud loss |
| **ADR-004: DICT local cache** | Month 1 | 5M+ daily DICT lookups at 30-50ms remote is too slow | Full local cache with incremental sync | 2ms lookups; 50 GB RAM cost; cache consistency lag during portability |
| **ADR-005: Event-sourced transactions** | Day 0 | Reconciliation and audit require complete transaction history | Event sourcing with CQRS | Complete audit trail; enables replay; increased storage; consumer idempotency required |
| **ADR-006: PIX Automático partitioned scheduler** | Month 6 | 500K+ mandates need reliable scheduling across timezones | Partitioned by mandate_id with leader election | Exactly-once billing; partition rebalancing on scale changes; 60s max failover gap |

---

## Flow 4: Split Payment Settlement
...
sequenceDiagram
    ...
    participant PA as Payer's PSP
    participant SPI as BCB SPI
    participant OUR as Our SPI Gateway
    participant TXN as Transaction Orchestrator
    participant SPLIT as Split Payment Engine
    participant LED as Settlement Ledger
    participant WH as Webhook Service

    C->>PA: Confirm PIX payment (R$100.00)
    PA->>SPI: PIX payment message
    SPI->>OUR: Incoming settlement (endToEndId, R$100.00)
    OUR->>TXN: Settlement received

    TXN->>SPLIT: Apply split rules (charge metadata)
    SPLIT->>SPLIT: Compute: Seller R$82.00, Platform R$10.00, Logistics R$7.50, PSP R$0.50
    SPLIT->>SPLIT: Validate: sum = R$100.00 ✓

    SPLIT->>LED: Credit seller sub-account R$82.00 (locked 7d)
    SPLIT->>LED: Credit platform account R$10.00
    SPLIT->>LED: Credit logistics account R$7.50
    SPLIT->>LED: Credit PSP fee account R$0.50

    LED-->>SPLIT: All credits applied atomically
    SPLIT-->>TXN: Split settled

    TXN->>WH: Notify seller (R$82.00 received, available in 7d)
    TXN->>WH: Notify marketplace (split complete, R$10.00 commission)
```

---

## Flow 5: PIX por Aproximação (NFC Payment)
...
sequenceDiagram
    ...
    participant MPOS as Merchant POS
    participant API as API Gateway
    participant TXN as Transaction Orchestrator
    participant FR as Fraud Engine
    participant SPI as SPI Gateway
    ...
    MPOS->>NFC: Display: R$45.00 payment ready
    PHONE->>NFC: NFC tap detected
    NFC->>WALLET: Payment request (R$45.00, merchant_id)
    WALLET->>WALLET: Biometric authentication (fingerprint/face)
    WALLET->>WALLET: Check R$500 limit compliance

    WALLET->>BCB: PIX payment initiation via payer's PSP
    BCB->>SPI: Settlement message to our PSP
    SPI->>TXN: Incoming NFC payment
    TXN->>FR: Fraud scoring (<200ms)
    FR-->>TXN: Score: 0.01 (APPROVE)
    TXN->>SPI: Accept settlement
    SPI->>BCB: Confirmed
    BCB-->>WALLET: Settlement finalized

    NFC->>MPOS: Payment confirmed
    MPOS->>MPOS: Print/display receipt

    Note over PHONE,MPOS: Total time: < 3 seconds tap-to-confirmation
```
...
| Layer | Deployment Model | Rationale |
|-------|-----------------|-----------|
| **API Gateway** | Multi-AZ stateless cluster, auto-scaling | Absorbs burst traffic; no state to lose |
| **Transaction Orchestrator** | Multi-AZ with event-sourced state | Saga state reconstructible from event log; no sticky sessions |
| **Fraud Engine** | Dedicated compute with ML accelerators | Isolated from payment path resource contention; GPU/TPU for model inference |
| **SPI Gateway** | Active-passive with dual RSFN links | RSFN connectivity is limited (2 links); active link handles traffic, passive ready for failover |
| **DICT Cache** | Multi-AZ in-memory cluster with warm replicas | Cold start takes 6-8 hours; warm replicas eliminate recovery delay |
| **Billing Scheduler** | Partitioned with leader election | Exactly-once processing guaranteed; 60s max failover gap |
| **Nota Fiscal Pipeline** | Async worker pool, auto-scaling | Decoupled from payment path; scales independently with SEFAZ response times |
| **Event Stream** | Multi-AZ replicated cluster | Partitioned by merchant_id; no single partition hotspot |
| **Transaction Database** | Distributed SQL, synchronous replication | RPO=0 (zero data loss); 6-node cluster across 2 AZs |
| **NF Archive** | Tiered storage (hot → warm → cold) | 5-year retention at 135 TB; cold tier for >12-month NF-e |
