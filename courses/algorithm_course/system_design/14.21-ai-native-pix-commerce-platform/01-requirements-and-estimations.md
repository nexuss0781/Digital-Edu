# Requirements & Estimations — AI-Native PIX Commerce Platform

## Functional Requirements

| ID | Requirement | Description |
|---|---|---|
| FR-01 | **PIX QR Code Payments** | Generate static and dynamic (COB/COBV) QR codes following the BR Code/EMVCo specification; host dynamic charge endpoints with JWT-encoded payment details; support expiration, single-use, and amount-locked QR variants; embed merchant order metadata for automated reconciliation |
| FR-02 | **PIX Key Management** | Register and manage merchant PIX keys (CPF, CNPJ, phone, email, random/EVP) in the DICT; handle key portability and ownership claims; support multiple keys per merchant account with routing rules; AI-assisted key selection recommending the optimal key type for each merchant's use case |
| FR-03 | **PIX Automático (Recurring)** | Implement mandate-based recurring billing: customer authorization flow, mandate registration with amount/frequency/start-date, automated charge submission 2-10 days before billing date per BCB rules, mandate lifecycle management (creation, modification, cancellation), retry logic for failed debits, and AI-predicted churn scoring |
| FR-04 | **PIX por Aproximação (NFC)** | Support contactless PIX payments via NFC-enabled terminals; integrate with digital wallet providers; enforce R$500 per-transaction limit; sub-3-second terminal-to-settlement latency; device compatibility management across Android NFC variants |
| FR-05 | **Split Payment Orchestration** | Configure split rules per marketplace seller (percentage or fixed amount splits); embed split instructions in PIX initiation; atomic multi-party settlement via SPI; support commission deduction, logistics fee allocation, and platform fee extraction; real-time split tracking dashboards |
| FR-06 | **AI Fraud Detection** | Pre-transaction fraud scoring completing in <200ms: device fingerprinting, behavioral biometrics, transaction graph analysis against DICT data, velocity anomaly detection, social engineering pattern recognition, geolocation consistency checks; configurable risk thresholds per merchant segment |
| FR-07 | **Instant Settlement & Reconciliation** | Real-time matching of SPI settlement confirmations (endToEndId) against merchant orders; automated discrepancy detection (amount mismatch, missing settlement, duplicates); settlement reporting with merchant-level and transaction-level granularity; AI-powered anomaly detection for systematic reconciliation gaps |
| FR-08 | **Nota Fiscal Integration** | Auto-generate NF-e (goods), NFS-e (services), or NFC-e (consumer retail) for each PIX payment; compute correct tax rates (ICMS, ISS, PIS, COFINS) based on merchant location, product category, and applicable tax regime; submit XML to SEFAZ via web services; handle rejection, correction, and cancellation flows |
| FR-09 | **MED (Special Return Mechanism)** | Implement BCB's Mecanismo Especial de Devolução workflow: receive fraud reports from payer PSPs, block funds in receiving account within mandated timeframe, trace fund movement across accounts (MED 2.0 multi-hop tracing), execute return transfers, and maintain audit trail for BCB compliance |
| FR-10 | **Merchant Onboarding & KYC** | AI-assisted merchant registration: CNPJ validation, Receita Federal data verification, beneficial ownership analysis, risk scoring based on business category and transaction projections, automated PSP enrollment with SPI connectivity testing, and ongoing due diligence monitoring |
| FR-11 | **Dynamic Pricing QR** | AI-generated dynamic pricing embedded in QR payloads: time-of-day pricing adjustments, demand-based surge pricing for high-traffic periods, loyalty discount application, bundle pricing for multi-item orders, and A/B testing of pricing strategies via different QR endpoint configurations |
| FR-12 | **PIX Copia e Cola** | Generate and validate text-based PIX payment strings for digital commerce (e-commerce checkout, in-app payments, messaging-based sales); payload validation following BR Code specification; deep-link integration for mobile banking app redirection |

---

## Out of Scope

| Item | Rationale |
|---|---|
| **Card payment processing** | Platform is PIX-native; card acquiring is handled by separate payment processors |
| **International remittances** | PIX is a domestic-only system; cross-border transfers use different BCB infrastructure |
| **Cryptocurrency/Drex integration** | Brazil's CBDC (Drex) is still in pilot phase and operates on separate rails |
| **Lending/credit products** | PIX Garantias and credit lines are separate financial products requiring distinct licensing |
| **Physical terminal hardware** | Platform provides software APIs; terminal hardware manufacturing/distribution is out of scope |

---

## Non-Functional Requirements

### Performance SLOs

| Metric | Target | Rationale |
|---|---|---|
| **QR Code Generation Latency** | p50 < 50ms, p99 < 200ms | Dynamic QR must be displayed instantly in merchant POS or e-commerce checkout; includes charge endpoint creation and payload signing |
| **DICT Key Lookup** | p50 < 5ms, p99 < 20ms | Key resolution is on the critical path of every PIX transaction; cached locally with DICT sync protocol for updates |
| **Fraud Scoring Latency** | p50 < 50ms, p99 < 200ms | Must complete before SPI submission; exceeding 200ms risks transaction timeout on the payer's PSP side |
| **End-to-End PIX Settlement** | p95 < 10 seconds | BCB target; includes payer authentication, DICT lookup, SPI message routing, and settlement confirmation |
| **Nota Fiscal Generation** | p50 < 2s, p99 < 8s | SEFAZ web service response time varies by state; generation should not block payment confirmation to merchant |
| **PIX Automático Billing** | 100% of mandates processed by scheduled time | Billing requests must be submitted 2-10 days before due date; zero tolerance for missed billing windows |
| **System Availability** | 99.99% (52 min downtime/year) | PIX operates 24/7/365; merchant payment acceptance cannot have scheduled downtime windows |
| **Settlement Reconciliation** | 99.95% auto-matched within 30 seconds | Less than 0.05% of transactions should require manual reconciliation investigation |

### Reliability Requirements

| Requirement | Target |
|---|---|
| **Zero data loss** | Every PIX transaction, settlement confirmation, and Nota Fiscal must be durably persisted before acknowledgment; write-ahead logging with synchronous replication |
| **Exactly-once settlement** | No duplicate settlements; idempotency enforced via endToEndId deduplication at the SPI integration layer |
| **Disaster recovery** | RPO = 0 (no data loss), RTO < 5 minutes; multi-region active-passive with synchronous replication for transaction data |
| **Graceful degradation** | If Nota Fiscal generation fails, payment processing continues; tax document generation retried asynchronously; if AI fraud scoring times out, fall back to rule-based scoring |

---

## Capacity Estimations

### Transaction Volume

| Metric | Value | Derivation |
|---|---|---|
| **Total PIX daily volume (Brazil)** | ~250 million transactions | BCB data: 7.9 billion monthly transactions as of late 2025 |
| **P2B (commerce) share** | ~44% of total | P2B surpassed P2P in September 2025 |
| **Daily commerce transactions** | ~110 million | 250M × 44% |
| **Platform target market share** | 2-5% of P2B | Mid-tier PSP serving SME merchants |
| **Platform daily transactions** | 2.2-5.5 million | 110M × 2-5% |
| **Peak-to-average ratio** | 3:1 | Lunch hour (11:00-13:00) and month-end spikes |
| **Peak transactions per second** | ~190 TPS (at 5% share) | 5.5M / 86,400 × 3 peak factor |

### Storage Estimates

| Data Category | Per-Transaction Size | Daily Volume (5M txns) | Monthly Storage |
|---|---|---|---|
| **Transaction records** | ~2 KB | 10 GB | 300 GB |
| **QR code payloads** | ~1 KB (dynamic) | 5 GB | 150 GB |
| **Nota Fiscal XML** | ~15 KB (authorized) | 75 GB | 2.25 TB |
| **Fraud scoring logs** | ~3 KB | 15 GB | 450 GB |
| **Settlement confirmations** | ~1 KB | 5 GB | 150 GB |
| **Audit/compliance logs** | ~2 KB | 10 GB | 300 GB |
| **Total** | ~24 KB | 120 GB/day | ~3.6 TB/month |

### Nota Fiscal Volume

| Metric | Value |
|---|---|
| **Daily NF-e/NFS-e/NFC-e** | ~5 million (one per transaction) |
| **SEFAZ API calls/day** | ~10 million (submission + status check) |
| **XML storage (5-year retention)** | ~135 TB (legally mandated retention period) |
| **Digital certificate operations** | ~5 million/day (XML signing) |

### DICT Synchronization

| Metric | Value |
|---|---|
| **Total DICT keys (Brazil)** | ~800 million registered keys |
| **Local DICT cache size** | ~50 GB (key + account mapping data) |
| **Cache refresh rate** | ~500K updates/day (new registrations, portability, deletions) |
| **Lookup rate (platform)** | ~5 million/day (one per transaction + retries) |

### Network Bandwidth

| Flow | Bandwidth |
|---|---|
| **SPI messages (inbound + outbound)** | ~50 Mbps sustained, 150 Mbps peak |
| **SEFAZ API traffic** | ~100 Mbps sustained (XML payloads are large) |
| **DICT sync traffic** | ~10 Mbps sustained |
| **Merchant API traffic** | ~200 Mbps sustained (QR generation, webhooks, dashboards) |
| **Total** | ~360 Mbps sustained, ~1 Gbps peak |

### Infrastructure Sizing

| Component | Specification |
|---|---|
| **Payment processing cluster** | 16-24 nodes, 32 vCPU / 64 GB RAM each |
| **Fraud scoring cluster** | 8-12 nodes with ML inference accelerators |
| **Database cluster** | 6-node distributed SQL cluster, 10 TB SSD per node |
| **DICT cache** | 4-node in-memory cluster, 64 GB RAM each |
| **Nota Fiscal processing** | 8-12 nodes (XML generation and SEFAZ API calls are CPU-intensive) |
| **Message queue cluster** | 6-node cluster for event streaming |

---

## Edge Cases and Extreme Scenarios

| Scenario | Scale | Design Impact |
|----------|-------|---------------|
| **PIX Automático mass cancellation** | 10% of mandates cancelled on billing eve due to viral social media post | Scheduler must handle mass cancellation before next-day billing; reject charges for cancelled mandates; notify affected merchants |
| **SEFAZ nationwide outage** | All 27 state SEFAZ systems down simultaneously (infrastructure attack) | Enter contingency mode (DPEC) for all NF-e; queue for retroactive authorization; payments continue unaffected |
| **Black Friday payment surge** | 8× normal TPS sustained for 12 hours | Pre-scale fraud engine, QR service, and NF pipeline; SEFAZ rate limiting becomes the Slowest part of the process |
| **Social engineering campaign** | Coordinated fraud ring targets 1000+ merchants simultaneously | Fraud engine detects cluster pattern; automatic threshold tightening; MED pre-positioning for known mule accounts |
| **PIX key portability storm** | Major bank migrates 10M customers to new PSP | DICT cache invalidation storm; fallback to direct DICT queries for affected keys; cache rebuild over 24h |
| **Settlement account liquidity crisis** | Pre-funded SPI settlement account depleted during high-volume period | Auto-alert treasury; queue outbound payments; accept inbound payments; emergency liquidity injection |
| **Duplicate SPI messages** | Network partition causes SPI to redeliver settlement confirmations | endToEndId deduplication in transaction store; idempotent settlement processing |
| **Timezone Edge Case (Unusual or extreme situation)** | Merchant in Acre (UTC-5) with PIX Automático customer in Fernando de Noronha (UTC-2) | Billing window and cancellation cutoff computed per customer timezone, not merchant timezone |

---

## Cost Estimation Framework

| Component | Unit Cost Basis | Monthly Estimate | Notes |
|-----------|----------------|------------------|-------|
| **Payment processing cluster** | 16-24 nodes, 32 vCPU each | $15-25K | Stateless; auto-scales with TPS |
| **Fraud scoring cluster** | 8-12 nodes with ML accelerators | $20-35K | Model inference is GPU-intensive |
| **Database cluster** | 6-node distributed SQL, 10 TB SSD each | $30-50K | High IOPS for transactional writes |
| **DICT cache** | 4 nodes, 64 GB RAM each | $5-8K | Memory-optimized; data reconstructible |
| **Nota Fiscal pipeline** | 8-12 nodes (CPU-intensive XML generation) | $10-15K | SEFAZ API calls are I/O-bound |
| **Event streaming** | 6-node cluster | $8-12K | Partitioned by merchant_id |
| **Digital certificate HSM** | 2 HSM devices for NF-e signing | $5-8K | A3 certificate storage |
| **RSFN connectivity** | 2 dedicated links to BCB network | $10-15K | Dual links for redundancy |
| **Bandwidth** | ~1 Gbps peak (SEFAZ XML dominates) | $3-5K | XML payloads are 15KB each |
| **Total estimated** | — | **$110-180K/month** | Excludes PIX transaction fees |

---

## SLO / SLA Table

| Service | Metric | SLO | SLA | Measurement |
|---------|--------|-----|-----|-------------|
| QR Generation | Latency p99 | < 200ms | < 500ms | COB payload creation + BR Code rendering |
| Fraud Scoring | Latency p99 | < 200ms | < 500ms (fallback to rules) | Feature extraction + model inference |
| End-to-End Settlement | Latency p95 | < 10s | < 30s | QR scan to SPI settlement confirmation |
| Nota Fiscal | Latency p99 | < 8s | < 30s | XML generation + SEFAZ authorization |
| Reconciliation | Auto-match rate | > 99.95% | > 99.5% | SPI confirmation matched to merchant order |
| System Availability | Uptime | 99.99% | 99.95% | All critical-path services |
| PIX Automático | Billing success rate | 100% of due mandates processed | > 99.9% | No missed billing windows |
| Settlement Accuracy | Discrepancy rate | < 0.01% | < 0.1% | Amount mismatches per total transactions |
| Webhook Delivery | Delivery p99 | < 5s post-settlement | < 30s | Settlement confirmation to merchant webhook |
| MED Response | Block request processing | < 30 min | < 2h | MED notification to fund block execution |

---

## Key Estimation Insights

1. **Nota Fiscal storage dominates**: At 15 KB per authorized XML × 5M daily transactions × 5-year retention, NF storage reaches 135 TB. This is 10× the size of transaction records themselves. Tiered storage (hot → warm → cold) with legally compliant archival is essential.

2. **SEFAZ is the slowest external dependency**: SEFAZ response times range from 500ms (São Paulo) to 8s (smaller states). Coupling NF generation to payment settlement would degrade the payment SLO to the slowest SEFAZ. Async decoupling is architecturally mandatory.

3. **Fraud scoring costs scale with model complexity**: A simple rule-based engine costs ~$5K/month; a full ML pipeline with graph neural networks costs ~$35K/month. The ROI depends on fraud volume—at 0.007% fraud rate on 5M daily transactions, that's 350 fraudulent transactions/day averaging R$1,000 each = R$350K/day potential loss. The ML investment pays for itself if it prevents even 10% more fraud than rules alone.

4. **DICT cache is cheap but critical**: 50 GB of RAM across 4 nodes costs $5-8K/month but saves 20-50ms per transaction. At 5M daily transactions, that's 25,000-69,000 seconds of cumulative latency saved per day.

5. **Peak-to-average ratio is moderate (3:1)**: Unlike e-commerce platforms with 10-15× peak multipliers, PIX commerce peaks are predictable (lunch rush, month-end). This allows efficient capacity planning without massive over-provisioning.

---

## Estimation Cheat Sheet

Quick-reference formulas for back-of-envelope calculations during system design:

| What | Formula | Example |
|------|---------|---------|
| **Daily transactions** | Brazil_P2B × market_share | 110M × 5% = 5.5M |
| **Average TPS** | daily_txns ÷ 86,400 | 5.5M ÷ 86,400 ≈ 64 TPS |
| **Peak TPS** | avg_TPS × peak_factor | 64 × 3 ≈ 190 TPS |
| **DICT cache size** | total_keys × bytes_per_entry | 800M × 65B ≈ 50 GB |
| **Daily storage** | daily_txns × bytes_per_txn_full | 5.5M × 24 KB ≈ 120 GB |
| **Monthly NF-e storage** | daily_txns × 15 KB × 30 | 5.5M × 15 KB × 30 ≈ 2.25 TB |
| **5-year NF-e archive** | monthly_NF × 60 | 2.25 TB × 60 ≈ 135 TB |
| **Fraud engine daily load** | daily_txns × models_in_ensemble | 5.5M × 3 ≈ 16.5M inferences |
| **Settlement account turnover** | daily_txns × avg_amount | 5.5M × R$80 ≈ R$440M/day |
| **SEFAZ API calls** | daily_txns × 2 (submit + status) | 5.5M × 2 = 11M calls/day |
| **False positives/day** | daily_txns × FPR | 5.5M × 0.05% ≈ 2,750 blocked |
| **MED claims/day** | daily_txns × fraud_rate | 5.5M × 0.007% ≈ 385 claims |
| **DICT sync updates/day** | ~500K (BCB feed) | Independent of platform volume |
| **Digital cert signing ops** | daily_txns (one per NF-e) | ~5.5M/day |
| **Webhook delivery/day** | daily_txns × 2 (payment + NF) | ~11M webhook calls |

---

## Design Constraints and Regulatory Non-Negotiables

These constraints are not configurable or negotiable—they are hard requirements from BCB regulations and Brazilian tax law:

| Constraint | Source | Impact |
|-----------|--------|--------|
| **Settlement is irrevocable** | PIX regulation (BCB) | No chargebacks; fraud detection must be pre-settlement |
| **End-to-end settlement < 10 seconds** | BCB SLO | Total time budget from payer confirmation to settlement finality |
| **PIX Automático: 2-10 day advance billing** | BCB Resolution | Billing scheduler must submit within this window, no earlier, no later |
| **Customer can cancel mandate until 23:59 day before** | BCB Resolution | Race condition with billing submission; check before processing |
| **NF-e mandatory for every commercial transaction** | Brazilian tax law | Cannot skip or defer indefinitely; contingency mode (DPEC) for SEFAZ outages |
| **NF-e 5-year retention** | Tax law (CTN Art. 195) | 135 TB archive with legally compliant storage |
| **MED fund block within BCB-mandated timeframe** | BCB Resolution 493/2025 | Automated MED handling; cannot be manual-only |
| **LGPD data subject rights (15-day response)** | Lei 13.709/2018 | Access, correction, deletion requests processed within deadline |
| **24/7/365 operation** | PIX operating rules | No maintenance windows; zero-downtime deployments mandatory |
| **Direct SPI participation above 500K accounts** | BCB requirement | Large PSPs cannot use indirect connectivity |
| **PIX por Aproximação R$500 limit** | BCB Resolution 541/2025 | Enforced at wallet level; cannot override |
| **endToEndId uniqueness and deduplication** | SPI protocol | Must prevent exactly-once settlement violations |

---

## Comparison with Other Instant Payment Systems

| Dimension | PIX (Brazil) | UPI (India) | FedNow (US) | SEPA Instant (EU) |
|-----------|-------------|-------------|-------------|-------------------|
| **Daily volume** | ~250M | ~500M | <1M (early) | ~20M |
| **Settlement finality** | Irrevocable, seconds | Near-instant, dispute mechanism | Irrevocable, seconds | Irrevocable with 10s recall |
| **Key directory** | DICT (800M keys) | UPI handles (VPA) | N/A (uses account numbers) | IBAN-based |
| **Recurring billing** | PIX Automático (mandates) | UPI Autopay | Planned | SEPA Direct Debit |
| **Tax integration** | Mandatory NF-e per transaction | GST e-invoicing (separate) | Not required | VAT reporting (separate) |
| **Fraud recovery** | MED (limited) | Dispute resolution | Recall (limited) | Recall (10s window) |
| **NFC support** | PIX por Aproximação (2025-2026) | UPI Lite (NFC) | N/A | Planned |
| **Population coverage** | 95% adult population | 67% adult population | <5% (early adoption) | ~60% of banks |
