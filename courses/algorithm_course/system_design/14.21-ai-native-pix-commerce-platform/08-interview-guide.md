...
| "Why not use event sourcing for every service?" | Some candidates apply event sourcing uniformly | Event sourcing adds complexity and is justified for the settlement ledger and fraud evidence trail where auditability and replay are critical; QR code generation and NF submission are stateless pipelines where simple request-response is sufficient |
| "How would you implement PIX Garantias (payment guarantees) on top of this platform?" | Tests whether candidates can extend the architecture without redesigning it | PIX Garantias requires holding funds in escrow until a condition is met (delivery confirmation, service completion). This adds a conditional settlement state to the transaction lifecycle—the settlement is initiated but completion is gated by an external event. The key insight is that this doesn't change the core payment flow; it adds a post-settlement hold and conditional release. |
| "What happens when your settlement account runs dry mid-transaction?" | Candidates may not consider liquidity as a runtime constraint | Outbound PIX transactions (refunds, split distributions) require pre-funded settlement accounts at BCB. If the balance is insufficient, the SPI rejects the transaction. The system must monitor the balance in real-time, predict depletion based on pending outflows, and trigger emergency liquidity injection from treasury before hitting zero. |

---

## Comparison with Related Systems

| Dimension | PIX Commerce Platform | Card Payment Gateway | UPI (India) | SEPA Instant (EU) | Mobile Money (Africa) |
|---|---|---|---|---|---|
| **Settlement finality** | Instant, irrevocable (seconds) | Batch settlement (T+1 to T+3) | Instant, with dispute mechanism | Instant, limited recall window (10s) | Real-time between agents |
| **Fraud recovery** | MED (limited, adversarial) | Chargebacks (systematic) | Dispute resolution (multi-step) | Recall request (time-limited) | Agent mediation |
| **Tax integration** | Mandatory real-time NF-e | Not platform responsibility | GST e-invoicing (separate) | VAT reporting (separate) | Not applicable |
| **Recurring billing** | PIX Automático (mandate-based) | Card-on-file (token-based) | UPI Autopay (mandate-based) | SEPA Direct Debit | Mobile money recurring |
| **Key infrastructure** | DICT (800M keys) | Card networks (BIN routing) | UPI handles (VPA) | IBAN routing | Phone number routing |
| **Primary fraud vector** | Social engineering | Card-not-present | SIM swap + social engineering | Account takeover | Agent fraud |
| **Regulatory complexity** | High (BCB + SEFAZ + LGPD) | Moderate (PCI-DSS + card scheme rules) | High (RBI + NPCI + GST) | Moderate (ECB + GDPR) | Varies by country |

---

## System Design Variants

### 15-Minute Version (Phone Screen)

**Focus:** QR code payment flow only—dynamic QR generation, SPI settlement, merchant notification.

**Expected depth:** Component identification, basic data flow, understanding of irrevocability. Skip fraud engine, NF pipeline, and recurring billing. The candidate should demonstrate that they understand PIX is not card payments.

### 30-Minute Version (First Round)

**Focus:** Full payment flow + one deep dive (fraud detection OR Nota Fiscal integration).

**Expected depth:** Component architecture, data flow, one detailed subsystem. Candidate should identify the async decoupling of NF from payment, discuss fraud latency budget, and address at least one failure mode (SEFAZ outage or SPI connectivity loss).

### 60-Minute Version (Onsite)

**Focus:** Full platform including PIX Automático, split payments, multi-region resilience.

**Expected depth:** Architecture decision records (why CQRS for the ledger, why saga for the payment lifecycle), detailed capacity estimation, operational maturity (chaos engineering, settlement account management), and regulatory awareness (MED, LGPD, BACEN Resolution 403).

---

## Architecture Walkthrough Script

A structured 7-step narrative for whiteboard discussions:

1. **Start with the merchant:** "A merchant displays a QR code at their checkout counter. This QR encodes a BR Code payload pointing to our charge endpoint, with amount, merchant ID, and order reference."

2. **Customer scans:** "The customer scans with their banking app. Their PSP reads the BR Code, resolves the payee's PIX key via DICT, and displays the payment confirmation screen."

3. **Payment initiation:** "The customer confirms. Their PSP submits a PIX payment via SPI. We receive the incoming SPI message—this is the critical moment."

4. **Fraud gate:** "Before accepting settlement, we run fraud scoring. We have 200ms. The feature store provides pre-computed velocity counters, DICT metadata gives us payee key characteristics, and the model ensemble renders a score. If the score exceeds the threshold, we reject the payment—this is our only chance, because settlement is irrevocable."

5. **Settlement:** "We accept. SPI settles in central bank money. We receive the settlement confirmation with the endToEndId. This is the point of no return."

6. **Post-settlement fan-out:** "Three things happen in parallel: (a) merchant webhook notification, (b) Nota Fiscal generation pipeline triggered, (c) reconciliation engine matches the settlement to the merchant's order."

7. **Nota Fiscal:** "The NF pipeline computes taxes based on merchant location and product category, generates signed XML, and submits to the merchant's state SEFAZ. If SEFAZ is down, we enter contingency mode (DPEC)—the payment is already confirmed, the NF will be authorized retroactively."

---

## Follow-Up Questions by Experience Level

### For Junior Candidates (Probe Fundamentals)

| Question | What You're Testing |
|----------|-------------------|
| "Walk me through what happens to the data when a QR code is scanned" | Basic understanding of request-response, data flow, and client-server interaction |
| "Why can't we just store everything in one big database?" | Awareness of horizontal scaling, read/write separation, and data locality |
| "What's the difference between encrypting data at rest vs. in transit?" | Security fundamentals relevant to financial systems |

### For Senior Candidates (Probe Trade-offs)

| Question | What You're Testing |
|----------|-------------------|
| "Your fraud model is producing 0.2% false positives on small merchants but 0.01% on large merchants. Why, and what do you do?" | Understanding of data distribution bias; small merchants have less training data; calibration per merchant segment |
| "How do you deploy a new fraud model without increasing risk?" | Shadow scoring, A/B testing on fraud decisions, canary deployment with automatic rollback on FPR increase |
| "Your DICT cache is 6 hours stale for one key range after a sync failure. What's your blast radius?" | Reasoning about cache consistency impact, fallback strategies, and risk quantification |

### For Staff+ Candidates (Probe System Thinking)

| Question | What You're Testing |
|----------|-------------------|
| "PIX transaction volume is growing 40% year-over-year. What breaks first in your architecture?" | Ability to identify Slowest part of the process progression: DICT cache memory → fraud engine compute → settlement account liquidity → database IOPS |
| "BCB announces that PIX settlement will be extended to cross-border transactions with Argentina. What changes?" | Forward-thinking: multi-currency settlement, FX rate management, cross-border fraud patterns, bilateral regulatory compliance |
| "Design the migration path from indirect PSP (via sponsor) to direct SPI connectivity" | Operational maturity: dual-running period, traffic migration, settlement account setup, RSFN certification process |
