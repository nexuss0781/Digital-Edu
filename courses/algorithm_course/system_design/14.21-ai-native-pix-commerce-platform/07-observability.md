...
| Service | SLI | SLO | Monthly Budget | Measurement |
|---------|-----|-----|----------------|-------------|
| QR Code Generation | p99 latency | < 200ms | 43 min above threshold | COB creation + BR Code rendering |
| Fraud Scoring | p99 latency | < 200ms | 43 min above threshold | Feature extraction + model inference |
| End-to-End Settlement | p95 latency | < 10s | 99.95% within target | QR scan to SPI confirmation |
| System Availability | Uptime | 99.99% | 4.3 min downtime/month | All critical-path services |
| Settlement Reconciliation | Auto-match rate | > 99.95% | 2,500 unmatched/month (at 5M/day) | endToEndId correlation |
| Webhook Delivery | p99 latency | < 5s post-settlement | 99.9% within target | Settlement event to merchant webhook |
| Nota Fiscal Authorization | p99 latency | < 8s | 99.5% within target | XML generation to SEFAZ response |
| MED Fund Blocking | Time to block | < 5 min | 100% within target | MED notification to fund freeze |

### Error Budget Policy

- **Budget remaining > 50%:** Normal development velocity; new feature deployments permitted
- **Budget remaining 25-50%:** Increased monitoring; canary deployments mandatory; no experimental changes
- **Budget remaining < 25%:** Feature freeze; only reliability improvements and bug fixes deployed
- **Budget exhausted:** Incident review mandatory; all changes require senior engineer approval; no deploys without rollback plan verified
...
|---|---|---|
| **SEFAZ API calls** | Calls per NF-e (submit + status poll) | Reduce status polling with webhook-based SEFAZ response where supported (SP, MG) |
| **Fraud engine inference** | ML inferences per transaction | Cascading evaluation: 70% resolved by rules (free), only 30% hit ML pipeline |
| **DICT cache memory** | GB of RAM across cache cluster | Compress key metadata; evict expired/deleted keys proactively |
| **Event stream storage** | GB per month of event log retention | Tiered retention: 7 days hot, 30 days warm, 5 years cold archive |
| **NF-e archive storage** | TB of XML documents (5-year retention) | Compress authorized XML; tiered storage (hot 30d → warm 1y → cold 5y) |
| **Digital certificate HSM ops** | Signing operations per day | Batch NF-e signing where possible (up to 100 XML documents per batch call) |
| **Bandwidth (SEFAZ XML)** | Mbps sustained for XML submission | XML compression for inter-service transport; expand only at SEFAZ submission |

---

## Business Health Observability

Metrics that bridge technical observability and business outcomes:

### Leading Indicators

| Indicator | Metric | Why It Matters |
|-----------|--------|---------------|
| **Merchant activation rate** | New merchants completing first transaction / registered merchants | Low rate indicates onboarding friction or KYC Slowest part of the process |
| **QR scan-to-payment conversion** | Payments completed / QR codes scanned (from charge endpoint hits) | Low rate indicates QR display issues, amount errors, or payer PSP problems |
| **PIX Automático mandate churn** | Monthly mandate cancellations / active mandates | Rising churn indicates customer dissatisfaction or insufficient retention actions |
| **Fraud false positive merchant impact** | Merchants with >1% blocked transactions / total active merchants | High rate indicates model calibration issues affecting merchant experience |
| **NF-e rejection rate** | Rejected NF-e / total submitted, grouped by rejection reason | Systematic rejections indicate tax computation errors or stale tax rules |
| **Settlement account health** | Current balance / average hourly outflow (runway in hours) | Below 4-hour runway triggers treasury alert; below 1-hour triggers emergency |
| **Reconciliation lag** | Time between SPI settlement and order matching | Growing lag indicates reconciliation engine falling behind volume |
| **MED claim rate** | MED claims received / total transactions | Rising rate indicates fraud detection model degradation or new fraud vector |
