...
## Settlement Account Management Algorithm

```
FUNCTION manage_settlement_account(account):
    // PSPs must pre-fund their SPI settlement account at BCB.
    // Insufficient funds = rejected outbound PIX transactions.

    current_balance = get_spi_balance(account)
    projected_outflow = predict_outflow(next_4_hours)
    projected_inflow = predict_inflow(next_4_hours)
    net_projected = current_balance + projected_inflow - projected_outflow

    // Maintain minimum buffer for peak scenarios
    minimum_buffer = average_hourly_outflow * 2

    IF net_projected < minimum_buffer:
        deficit = minimum_buffer - net_projected
        INITIATE liquidity_injection(deficit)
        ALERT treasury_team("Settlement account approaching minimum")

    IF net_projected < 0:
        ALERT_P0("Settlement account may become negative within 4 hours")
        // Enable priority mode: process only high-value/VIP merchant payments
        ENABLE payment_triage_mode()

    // Daily reconciliation
    IF is_reconciliation_hour():
        spi_reported_balance = query_spi_balance()
        internal_computed_balance = sum_all_settled_transactions()
        IF ABS(spi_reported_balance - internal_computed_balance) > THRESHOLD:
            ALERT_P1("Settlement balance discrepancy detected")
            INITIATE investigation(spi_reported_balance, internal_computed_balance)

FUNCTION predict_outflow(window_hours):
    // ML model: historical patterns + current queue
    pending_refunds = count_pending_refunds()
    pending_med_returns = count_pending_med_returns()
    historical_average = get_average_outflow(day_of_week, hour_of_day)
    queue_depth = get_outbound_queue_depth()
    RETURN ml_model.predict(pending_refunds, pending_med_returns,
                            historical_average, queue_depth)
```

---

## SEFAZ Contingency Mode Algorithm

```
FUNCTION submit_nota_fiscal(nf_document, state):
    sefaz_endpoint = get_sefaz_endpoint(state)

    TRY:
        response = call_sefaz(sefaz_endpoint, nf_document, timeout=8s)
        IF response.status == AUTHORIZED:
            STORE nf_document with authorization_protocol
            RETURN AUTHORIZED
        ELSE IF response.status == REJECTED:
            error_code = response.rejection_code
            IF is_auto_correctable(error_code):
                corrected_nf = auto_correct(nf_document, error_code)
                RETURN submit_nota_fiscal(corrected_nf, state)  // retry
            ELSE:
                FLAG for merchant review
                RETURN REJECTED(error_code)

    CATCH timeout OR connection_error:
        // Enter contingency mode (DPEC)
        contingency_auth = generate_dpec_authorization(nf_document)
        STORE nf_document with contingency_auth
        ENQUEUE for_retroactive_submission(nf_document, state)
        RETURN CONTINGENCY

FUNCTION process_contingency_queue():
    // Runs every 5 minutes for each state with pending contingency NFs
    FOR EACH state IN states_with_contingency_queue:
        IF sefaz_health_check(state) == HEALTHY:
            batch = dequeue_contingency_nfs(state, limit=100)
            FOR EACH nf IN batch:
                response = call_sefaz(state, nf, timeout=15s)
                IF response.status == AUTHORIZED:
                    UPDATE nf SET status = AUTHORIZED,
                                  contingency_resolved_at = NOW()
                ELSE:
                    RETRY later OR flag for manual review
```

---

## Split Payment Rounding Algorithm

```
FUNCTION compute_split(total_cents, split_rules):
    // split_rules: [{participant_id, percentage, type: "percentage"|"fixed"}]
    // total_cents: integer amount in centavos (BRL × 100)

    // Sort participants deterministically for reproducible rounding
    sorted_rules = SORT split_rules BY participant_id ASC

    allocated = 0
    results = []

    FOR i = 0 TO LEN(sorted_rules) - 2:  // all except last
        rule = sorted_rules[i]
        IF rule.type == "percentage":
            amount = FLOOR(total_cents * rule.percentage / 100)
        ELSE:
            amount = rule.fixed_amount_cents
        results.APPEND({participant: rule.participant_id, amount: amount})
        allocated += amount

    // Last participant gets the remainder (absorbs rounding)
    last_rule = sorted_rules[LAST]
    last_amount = total_cents - allocated
    results.APPEND({participant: last_rule.participant_id, amount: last_amount})

    // Validation
    ASSERT SUM(results.amount) == total_cents
    ASSERT ALL(results.amount >= 0)

    // Audit trail: store the computation inputs for reproducibility
    STORE split_audit{
        total_cents, split_rules, sorted_order, results,
        rounding_method: "floor_with_remainder_to_last_by_uuid"
    }
...
## Charge Endpoint Lifecycle Management

The dynamic QR's charge endpoint is a high-traffic API that must handle the full lifecycle of a payment charge:

```
FUNCTION handle_charge_request(charge_id, request):
    // Called when payer's PSP fetches charge details after scanning QR
    charge = lookup_charge(charge_id)

    IF charge IS NULL:
        RETURN response(status: 404, error: "CHARGE_NOT_FOUND")

    SWITCH charge.status:
        CASE ACTIVE:
            IF charge.expiration < NOW():
                charge.status = EXPIRED
                STORE charge
                RETURN response(status: 410, error: "CHARGE_EXPIRED",
                    message: "This payment has expired")

            // Return charge details for payer confirmation
            RETURN response(status: 200, {
                value: charge.amount_cents,
                merchant_name: charge.merchant_name,
                merchant_city: charge.merchant_city,
                txn_id: charge.txn_id,
                calendar: {expiration: charge.expiration},
                additional_info: charge.order_metadata
            })

        CASE COMPLETED:
            RETURN response(status: 410, error: "CHARGE_ALREADY_PAID",
                message: "This charge has already been settled",
                end_to_end_id: charge.settlement_e2e_id)

        CASE EXPIRED:
            RETURN response(status: 410, error: "CHARGE_EXPIRED")

        CASE CANCELLED:
            RETURN response(status: 410, error: "CHARGE_CANCELLED",
                message: "This charge was cancelled by the merchant")

FUNCTION create_charge(merchant_id, amount, items, split_rules, expiration_seconds):
    charge_id = GENERATE_UUID()
    txn_id = GENERATE_TXN_ID()  // 26-character alphanumeric
    ...
    charge = {
        ...
        amount_cents: amount,
        items: items,
        split_rules_snapshot: DEEP_COPY(split_rules),  // Snapshot at creation time
        status: ACTIVE,
    ...
    br_code_payload = build_br_code(
        merchant_pix_key: lookup_merchant_key(merchant_id),
        amount: amount,
        txn_id: txn_id,
        url: "pix.platform.com/v2/cob/" + charge_id
    )

    RETURN {
        charge_id: charge_id,
        qr_code_base64: render_qr(br_code_payload),
        copia_e_cola: br_code_payload,
...
## endToEndId Deduplication Strategy

The endToEndId is the SPI-assigned unique identifier for every PIX transaction. Duplicate SPI messages (caused by network retries, partition recovery, or SPI redelivery) must be detected and rejected to ensure exactly-once settlement.

```
FUNCTION process_spi_settlement(spi_message):
    e2e_id = spi_message.end_to_end_id

    // Atomic check-and-insert using database unique constraint
    TRY:
        INSERT INTO settlement_dedup (
            end_to_end_id: e2e_id,
            received_at: NOW(),
            spi_message_hash: HASH(spi_message)
        )
    CATCH UniqueConstraintViolation:
        // Already processed this endToEndId
        existing = LOOKUP settlement_dedup WHERE end_to_end_id = e2e_id

        IF existing.spi_message_hash == HASH(spi_message):
            // Exact duplicate — safe to acknowledge without reprocessing
            LOG "Duplicate SPI message for e2e_id={e2e_id}, acknowledging"
            RETURN ACK
        ELSE:
            // Same e2e_id but different content — should never happen
            // This indicates a serious SPI protocol violation
            ALERT "e2e_id collision with different content: {e2e_id}"
            RETURN REJECT(reason: "CONFLICTING_E2E_ID")

    // First time seeing this endToEndId — process normally
    txn = match_charge_by_txn_id(spi_message.txn_id)

    IF txn IS NULL:
        // Unexpected settlement — no matching charge in our system
        LOG_ALERT "Orphan settlement: e2e_id={e2e_id}, txn_id={spi_message.txn_id}"
        STORE orphan_settlement(spi_message)
        RETURN ACK  // Accept settlement (funds already moved), investigate async

    // Normal settlement processing
    process_settlement(txn, spi_message)
    RETURN ACK
```
