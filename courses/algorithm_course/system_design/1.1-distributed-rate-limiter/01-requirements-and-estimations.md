...
| Edge Case (Unusual or extreme situation) | Scenario | Expected Behavior |
|-----------|----------|-------------------|
| **Window boundary burst** | 100 requests at :59, 100 at :00 of next window | Sliding window catches; fixed window allows 2x |
| **Clock skew across nodes** | Node A is 3 seconds ahead of Node B | Use Redis TIME as authoritative; boundary buffer |
| **Negative token count** | Concurrent requests drain tokens below zero | Lua script prevents; floor at zero |
| **User tier upgrade mid-window** | User upgrades from free to premium mid-minute | Apply new limit immediately; do not reset counter |
| **Zero-cost request** | Health check or preflight with cost=0 | Allow without incrementing; log but don't count |
| **Key collision** | Two different users hash to same rate limit key | Include dimension + identifier in key; collision-free |
| **Redis key eviction** | Memory pressure evicts active rate limit key | Behaves as new window (allows full limit); monitor eviction rate |
| **Idempotent retry storm** | Client retries same request 50 times in 1 second | Dedup via idempotency key; count once |

### Key Assumptions

| Assumption | Value | Impact if Wrong |
|------------|-------|-----------------|
| Average key size | 75 bytes | Storage estimates 2-3x off if composite keys are longer |
| Peak-to-average ratio | 3x | Under-provisioning if real ratio is 5-10x (flash sales, viral events) |
| Redis single-thread throughput | 100K ops/sec | Need more shards earlier if Lua scripts reduce effective throughput |
