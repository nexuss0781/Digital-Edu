...
| **Algorithm Details** | Step-by-step plan in plain English, time/Space Complexity (Memory usage of the algorithm), trade-offs |
...
> "The main challenge here is race conditions. If two requests arrive simultaneously, both might read the same count, both pass the check, and both increment - exceeding our limit. The solution is to use Redis Lua scripts which execute atomically. Let me write the Step-by-step plan in plain English..."
...
3. **Bottlenecks**
   - "The main Slowest part of the process is Redis. Hot keys can overwhelm a single shard. We can mitigate with local aggregation..."
...
| Forgetting latency impact | Rate limiter becomes Slowest part of the process | Target <5ms added latency |
...
### Deep Dive Option A: Cost-Based Rate Limiting

**When to bring up:** Interviewer asks about fairness for APIs with mixed-cost operations.

**Key points:**
1. Not all requests are equal — a bulk import should cost more than a simple GET
2. Token bucket naturally supports cost: deduct `operation_cost` instead of 1
3. Cost table maintained per endpoint (exact match > pattern > method default > global)
4. Users see remaining tokens, not remaining requests — requires clear documentation
5. Challenge: estimating cost before execution (some queries are data-dependent)

**Trade-off to highlight:** Predictability vs precision. Static cost table is predictable but may mismatch real resource usage. Dynamic cost (measure actual CPU/IO) is precise but adds latency and complexity.

### Deep Dive Option B: Adaptive Rate Limiting

**When to bring up:** Interviewer asks about handling traffic spikes or backend overload.

**Key points:**
1. Static limits are set for peak capacity — wasteful at low load, insufficient under unexpected spikes
2. Monitor backend health: p99 latency, error rate, queue depth
3. Multiplicative decrease (fast), additive increase (slow) — same principle as TCP congestion control
4. Guard rails: never below 10% (starvation) or above 150% (runaway) of base limit
5. Minimum 30s adjustment interval to prevent oscillation

**Trade-off to highlight:** Responsiveness vs stability. Fast adjustments respond to real problems but risk oscillation. Slow adjustments are stable but may not react fast enough to protect the backend.
...
|-------------|----------------|-----------------|
| **Distributed locks for counting** | 5-50ms overhead per request; lock-holder crash blocks all keys | Lua scripts for atomic check-and-increment |
| **Single algorithm for all endpoints** | Burst-tolerant algo on constant-rate backend = overload | Per-endpoint algorithm selection based on traffic pattern |
| **Fail-closed as default** | Rate limiter outage = total API outage (self-inflicted DDoS) | Fail-open with local in-memory fallback |
| **Trusting X-Forwarded-For blindly** | Trivial to spoof; bypasses IP-based limits | Validate at edge; use authenticated identity as primary dimension |
| **Shared Redis for rate limiting and application** | Application queries compete with rate limit operations on the hot path | Dedicated Redis cluster for rate limiting |
| **Resetting all tokens at window boundary** | Causes thundering herd when throttled clients all retry at :00 | Gradual refill (token bucket) or jittered Retry-After headers |

---

## Advanced Trade-Off Discussions

### Embedded SDK vs Sidecar vs Service

| Deployment | Latency | Consistency | Operational Complexity |
|-----------|---------|-------------|----------------------|
| **Embedded SDK** (in-process) | ~0.1ms (local), ~1ms (Redis) | Per-process state; no coordination without Redis | Low — no extra infra |
| **Sidecar** (service mesh) | ~1-2ms (local proxy hop) | Mesh-wide policy; eventual consistency | Medium — mesh required |
| **Separate service** (gRPC) | ~2-5ms (network round-trip) | Global state; centralized enforcement | High — dedicated service to operate |

**When to choose:** Embedded for single-language shops with < 10 services. Sidecar for Kubernetes-native orgs with service mesh. Separate service for multi-language, high-scale platforms needing independent scaling.

### Global Limit Split: Static vs Dynamic Quota Allocation

| Approach | Pros | Cons |
|----------|------|------|
| **Static split** (40/30/30 per region) | Simple, predictable | Wastes quota in low-traffic regions |
| **Dynamic rebalance** (usage-proportional) | Efficient, adapts to traffic shifts | Rebalance lag, thundering herd on rebalance |
| **Token lease** (nodes request quota from central pool) | Fine-grained, minimal waste | Central pool is SPOF; lease acquisition adds latency |
...
| Area | Expected Depth |
|------|---------------|
| Requirements | Identifies scale, consistency, latency trade-offs |
| Algorithm | Explains 2-3 algorithms with trade-offs, picks one with justification |
| Distributed challenges | Race conditions, Lua scripts, eventual consistency |
| Failure modes | Fail-open vs fail-closed, circuit breaker, local fallback |
| Scalability | Sharding, hot key mitigation, caching layers |

### Staff+ Engineer Expectations

| Area | Expected Depth |
|------|---------------|
| Cost-based limiting | Weighted operations, fairness models |
| Adaptive limiting | Dynamic adjustment based on backend health, AIMD |
| Multi-window enforcement | Atomic multi-window checks, Lua script design |
| Organizational impact | Rate limiting as product feature (tier differentiation, usage monetization) |
| Edge-to-origin architecture | Hierarchical enforcement, quota allocation strategies |
| Production war stories | Hot key incidents, thundering herd, split-brain scenarios |

---

## Discussion Starters

- "How would you rate-limit a GraphQL API where query cost varies 1000x depending on the query?"
- "Your rate limiter is adding 15ms p99 to every request. Walk me through how you'd diagnose and fix this."
- "A customer reports they're being rate-limited below their contracted limit. How do you investigate?"
