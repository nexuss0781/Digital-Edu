...
## Insight 12: The Dependency Graph Is the Single Most Underinvested Component — And It Determines the Accuracy of Every Safety Decision

**Category:** External Dependencies

**One-liner:** Every safety guarantee the platform makes — blast radius limits, concurrent experiment isolation, indirect impact estimation — is only as accurate as the service dependency graph, yet most organizations treat the graph as an afterthought maintained by a different team.

**Why it matters:** The blast radius controller computes indirect impact by traversing the dependency graph. If the graph says Service A depends on Service B but misses the dependency on Service C, then an experiment targeting Service C will underestimate its blast radius — the BRC approves an experiment that actually affects more services than it calculated. The dependency graph has multiple freshness problems: new services deployed after the last graph refresh are invisible; traffic pattern shifts (Service A now sends 80% of requests to Service B instead of 50%) change dependency weights; canary deployments create transient dependencies that don't appear in the long-term graph. The graph is typically owned by the service mesh or observability team, not the chaos engineering team, creating an organizational gap where the chaos platform's safety depends on another team's data quality. The architectural response is threefold: (1) the BRC should use pessimistic estimation — assume unknown dependencies exist and add a safety margin to computed blast radius, (2) the platform should validate the dependency graph's completeness by cross-referencing with actual traffic data during experiments, and (3) experiments that reveal undocumented dependencies should automatically update the graph (chaos as a dependency discovery mechanism).

---

## Insight 13: Chaos Engineering Is Fundamentally a Confidence-Building Exercise — The Metric Is Not "Did It Pass?" but "How Much Do We Trust Our System?"

**Category:** Scaling

**One-liner:** The primary output of chaos engineering is not a pass/fail result but a calibrated confidence level in the system's resilience — and this confidence degrades over time as the system changes, requiring continuous re-validation.

**Why it matters:** Organizations often treat chaos experiments like unit tests: run them once, get a pass, and move on. But chaos experiment results are perishable — a service that tolerated 25% pod failure last month may fail today because of a configuration change, a new dependency, or a traffic pattern shift. The valuable output is not any single experiment result but the trend: is the service's resilience improving, stable, or degrading over time? This shifts the platform's design from optimizing single-experiment throughput to optimizing trend visibility. The resilience scoring engine becomes the most user-facing component — not the fault injector. The platform should surface: "Payment-api's resilience score has dropped from 87 to 72 over the past 30 days; the regression correlates with the v3.2 deployment that changed retry configuration." This trend-based framing also changes how the organization values chaos engineering: the ROI is not "we found a bug" (which sounds like testing) but "we maintain calibrated confidence in our system's behavior under failure" (which sounds like risk management). The architectural consequence is that the platform must store and index every experiment result for trend analysis, support regression detection across deployment boundaries, and integrate with incident management to correlate chaos discoveries with actual incident prevention.
