...
### Slowest part of the process: Dependency Graph Freshness
...
### Slowest part of the process: Observability System Under Chaos
...
### Slowest part of the process: Concurrent Fault Application
...
## Critical Component 4: GameDay Multi-Experiment Coordinator

### Why It Is Critical

A GameDay simultaneously orchestrates 10-20 experiments across multiple services, managed by multiple teams. The coordinator must handle: phased experiment execution (briefing → injection → observation → debrief), inter-experiment dependencies (Experiment B starts only after Experiment A achieves steady state), team communication synchronization, and emergency abort that cleanly reverts all experiments within 30 seconds.

### The Coordination Problem

```mermaid
flowchart TB
    subgraph Phase1["Phase 1: Infrastructure Faults"]
        E1["Exp 1: Network partition<br/>DB primary ↔ replica"]
        E2["Exp 2: DNS failure<br/>service-discovery"]
    end

    subgraph Gate1["Gate: Steady-State Check"]
        G1{"All Phase 1<br/>hypotheses pass?"}
    end

    subgraph Phase2["Phase 2: Application Faults"]
        E3["Exp 3: 500ms latency<br/>payment-api"]
        E4["Exp 4: 10% error injection<br/>checkout-service"]
        E5["Exp 5: CPU pressure<br/>recommendation-engine"]
    end

    subgraph Gate2["Gate: Combined Assessment"]
        G2{"System handles<br/>combined faults?"}
    end

    subgraph Phase3["Phase 3: Cascading Failure"]
        E6["Exp 6: Kill 25% pods<br/>payment-api + checkout"]
    end

    Phase1 --> Gate1
    G1 -->|Pass| Phase2
    G1 -->|Fail| ABORT["Abort GameDay<br/>Revert All"]
    Phase2 --> Gate2
    G2 -->|Pass| Phase3
    G2 -->|Fail| ABORT
    ...
    class E1,E2,E3,E4,E5,E6 phase
    class G1,G2 gate
    class ABORT abort
```

### Edge Case (Unusual or extreme situation): Phase Rollback During Transition

A subtle failure mode: Phase 1 completes and the coordinator starts Phase 2, but the steady-state check for Phase 1's metrics arrives late and shows a violation. Should Phase 2 be aborted?

**Solution:** Each phase's steady-state hypothesis continues to be evaluated even after the phase transitions. If a Phase 1 metric violates its hypothesis during Phase 2 execution, the coordinator aborts the entire GameDay — because the Phase 1 fault is still active (it hasn't been reverted), and the combined impact of Phase 1 + Phase 2 faults may exceed safety limits.
...
|-----------|----------------|-----------------|----------------|
| Blast Radius Calculation | O(V + E) where V = services, E = dependency edges | O(V) for visited set | Yes — blocks experiment start |
| Steady-State Evaluation | O(H × Q) where H = hypotheses, Q = query latency | O(H) for state tracking | Yes — on critical monitoring loop |
| Agent Safety Loop | O(F) where F = active faults on this agent | O(F) for fault registry | Yes — runs continuously |
| Progressive Escalation | O(S × D × H) where S = steps, D = duration, H = hypotheses | O(S) for step results | Yes — on experiment execution path |
| Experiment Conflict Detection | O(A × T) where A = active experiments, T = targets | O(T) for overlap sets | Yes — blocks new experiments |
| GameDay Coordination | O(P × E × H) where P = phases, E = experiments, H = hypotheses | O(E) for experiment states | Yes — on GameDay execution path |
| Split-Brain Reconciliation | O(F) per agent reconnection | O(F) for state diff | No — runs on reconnection |
| Orphan Reconciliation Sweep | O(N × F) where N = agents, F = avg faults | O(N × F) for global state | No — runs periodically |

---

## Edge Cases and Failure Modes

### Edge Case (Unusual or extreme situation) 1: Cascading Autonomous Reverts

When a network partition affects multiple agents simultaneously, they all start their partition timers independently. If they timeout at similar times, all agents revert simultaneously — potentially causing a "thundering herd" of revert operations that creates its own transient failures (e.g., mass connection re-establishment, cache invalidation storms).

**Mitigation:** Jitter the partition safety timeout per agent (±15% random). This spreads reverts over a time window rather than creating a synchronous revert spike.

### Edge Case (Unusual or extreme situation) 2: Circular Dependency in Blast Radius Graph

If the dependency graph contains cycles (Service A depends on B, B depends on C, C depends on A), the blast radius traversal can loop infinitely.

**Mitigation:** The traversal algorithm maintains a visited set and terminates cycle detection. The indirect impact for circular dependencies is calculated using the maximum impact across the cycle, not the sum (which would inflate to infinity).

### Edge Case (Unusual or extreme situation) 3: Time-Zone-Aware Experiment Scheduling

An experiment scheduled for "2 PM business hours" by a team in New York should not execute at 2 PM UTC if the intent was Eastern time. Scheduling must be time-zone-aware with explicit zone declaration.

### Edge Case (Unusual or extreme situation) 4: Agent Binary Rollback During Active Experiment

An agent binary upgrade introduces a bug. The upgrade is rolled back, but the new binary had different fault module implementations. On rollback to the old binary, the old binary may not know how to revert faults applied by the new binary's modules.

**Mitigation:** Fault revert commands are stored as generic shell commands (not binary-version-specific APIs). Any binary version can execute stored revert commands. Version-independent revert format is a safety requirement for the fault registry.

---

## Locking Strategy

| Resource | Lock Type | Granularity | Timeout | Held By |
|----------|-----------|-------------|---------|---------|
| Blast radius reservation | Distributed mutex | Global (all experiments) | 5 seconds | BRC during validation |
| Experiment state | Optimistic (version counter) | Per experiment | N/A (retry on conflict) | Orchestrator during transitions |
| Agent fault registry | Local file lock | Per agent | 1 second | Agent during inject/revert |
| GameDay phase | CAS (compare-and-swap) | Per GameDay | N/A | Coordinator during phase transition |
| Guardrail modification | Write lock | Per guardrail scope | 10 seconds | Admin during guardrail updates |
| Dependency graph cache | Read-write lock | Global | 30 seconds | BRC on read; graph updater on write |
| Audit log | Lock-free (append-only) | Per partition | N/A | All components |

---

## Case Studies

### Case Study 1: How a Streaming Platform Discovered DNS Single Points of Failure

A large streaming platform ran a chaos experiment that injected DNS resolution failures for their service discovery layer. Expected blast radius: 5% of microservices would fail to discover peers for 30 seconds. Actual result: 100% of services lost service discovery because every service used the same DNS resolver pool, creating an undocumented single point of failure. The blast radius controller had estimated 5% impact based on the service dependency graph, but the graph did not model shared infrastructure dependencies (DNS, NTP, certificate authorities). **Lesson:** The dependency graph must include infrastructure dependencies, not just service-to-service API calls.

### Case Study 2: Grace Period Miscalibration in a Payment System

A fintech company set a 60-second grace period for their payment service's error rate hypothesis. During a chaos experiment (latency injection on the database), the payment service genuinely failed at second 10. The grace period meant the SSHE waited until second 70 to trigger rollback. During those 60 seconds, ~18,000 payment transactions failed with timeouts. The company then implemented adaptive grace periods: 5 seconds for payment-critical services, 30 seconds for internal analytics. **Lesson:** Grace period is not one-size-fits-all; it must be calibrated based on the service's customer-impact sensitivity.

### Case Study 3: GameDay Reveals Monitoring Blind Spot

During a GameDay at a logistics company, one experiment injected 500ms latency on the metrics pipeline itself. The SSHE could not evaluate any hypothesis because all metric queries timed out. But the system was actually healthy — the only thing broken was the monitoring. The GameDay revealed that the company had no independent health check channel separate from their primary observability stack. **Lesson:** Critical safety mechanisms (like the SSHE) need an independent monitoring path, not just the primary observability pipeline.
...
## Deep Dive 5: Reconciliation Engine — The Orphan Hunter

### Why It Is Critical

An "orphaned fault" is the worst operational state the platform can produce: a production system is degraded by an injected fault, but no experiment claims ownership. No SSHE is monitoring it. No rollback will ever fire. The fault persists until a human notices unexplained degradation in production metrics.

### How Orphans Are Created

| Cause | Mechanism |
|-------|-----------|
| Agent crash after inject, before ACK | Control plane doesn't know the fault was applied; agent restart reverts (if revert-before-inject implemented correctly) |
| Control plane crash during experiment | Experiment state lost or corrupted; agents still have active faults |
| Network partition during rollback | Rollback command never reaches agent; control plane marks experiment as "rolled back" |
| Queue message loss | Rare but possible; command enqueued but never delivered |
| Bug in fault module | Fault applied via iptables rule, but the revert command has incorrect parameters — revert executes but doesn't actually remove the rule |

### Reconciliation Algorithm

```
FUNCTION reconciliation_sweep():
    // Runs every 5 minutes on the control plane

    // Step 1: Collect all active faults reported by agents
    agent_faults = MAP()
    FOR EACH agent IN healthy_agents():
        faults = query_agent_fault_state(agent)
        FOR EACH fault IN faults:
            agent_faults[fault.experiment_id].add(fault)

    // Step 2: Compare against active experiments
    active_experiments = load_active_experiments()
    active_experiment_ids = SET(active_experiments.map(e => e.id))

    // Step 3: Identify orphans
    orphaned_faults = []
    FOR EACH experiment_id, faults IN agent_faults:
        IF experiment_id NOT IN active_experiment_ids:
            orphaned_faults.extend(faults)

    // Step 4: Revert orphans
    FOR EACH orphan IN orphaned_faults:
        send_revert_command(orphan.agent_id, orphan.fault_id)
        log_audit_event("orphaned_fault_reverted", orphan)
        alert("ORPHANED_FAULT_DETECTED", severity = P1, details = orphan)

    RETURN ReconciliationResult(
        agents_checked = len(healthy_agents()),
        orphans_found = len(orphaned_faults),
        orphans_reverted = len(orphaned_faults)
    )
```

### Reconciliation Guarantees

The sweep provides **eventual liveness** — any orphaned fault will be detected and reverted within at most one sweep interval (5 minutes). Combined with the agent-side safety timer (which provides independent revert within 60 seconds of the experiment's max duration), the system has two independent paths to orphan resolution. The sweep catches orphans that the safety timer misses (e.g., when the timer itself has a bug or when the fault was applied with no associated experiment metadata).

---

## Slowest part of the process Deep Dive: BRC Lock Contention During GameDays

### Problem

During a GameDay, 10–20 experiments start within a 60-second window. Each experiment must acquire the global blast radius lock for validation. With lock hold time of ~100ms per validation, serialized throughput is limited to ~10 experiments/second — sufficient for normal operations but a Slowest part of the process during GameDay burst.

### Solution: Partitioned Locking with Service-Group Affinity

```
FUNCTION reserve_blast_radius_partitioned(experiment):
    // Determine which service groups this experiment affects
    affected_groups = get_service_groups(experiment.targets)

    // Acquire locks only for affected groups (fine-grained)
    locks = []
    FOR EACH group IN sorted(affected_groups):  // Sorted to prevent deadlock
        lock = ACQUIRE LOCK("blast_radius:" + group, timeout = 2s)
        IF lock IS NULL:
            RELEASE ALL locks
            RETURN REJECTED("lock_timeout")
        locks.append(lock)

    TRY:
        // Validate against active reservations in these groups only
        active = load_active_reservations(affected_groups)
        combined = merge(active, experiment.blast_radius)
        IF combined violates any guardrail:
            RETURN REJECTED(violation_detail)
        store_reservation(experiment.id, experiment.blast_radius, ttl = experiment.duration + buffer)
        RETURN APPROVED
    FINALLY:
        RELEASE ALL locks
...
This allows experiments targeting non-overlapping service groups to proceed in parallel. During a typical GameDay, experiments target different services, so contention drops from O(N) serialized to O(K) where K is the maximum number of experiments targeting the same service group simultaneously (usually 2–3).
