...
## Critical Component 5: AI-Powered Features (2025-2026)

### AI DJ Architecture

Spotify's AI DJ combines recommendation models with generative AI for a personalized radio-like experience:
...
│  │  • Current mood signals (tempo, energy of recent plays)             │   │
│  │  • Skip rate patterns                                               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                     │
│       ▼                                                                     │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Segment Planner (Reinforcement Learning)                            │   │
│  │  • Decide segment theme: "old favorites", "new discovery",          │   │
│  │    "genre deep dive", "artist spotlight"                            │   │
│  │  • 3-5 tracks per segment                                          │   │
...
│       ▼                     ▼                                              │
│  ┌──────────────┐   ┌──────────────────────────────────────────────┐      │
│  │ BaRT Scoring │   │ Commentary Generator (LLM)                    │      │
│  │ Select tracks│   │ • Generate DJ script for segment intro        │      │
│  │ per segment  │   │ • Include artist facts, genre context          │      │
│  └──────┬───────┘   │ • TTS synthesis with DJ voice                  │      │
│         │           └──────────────────────┬───────────────────────┘      │
│         │                                  │                              │
│         └──────────────┬───────────────────┘                              │
│                        ▼                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Playback Sequencer                                                  │   │
│  │  DJ intro (TTS) → Track 1 → Track 2 → ... → DJ transition → ...    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  Latency Budget:                                                           │
│  • Segment planning: <100ms (pre-computed)                                 │
│  • LLM commentary: <500ms (cached templates + infill)                     │
│  • TTS synthesis: <300ms (streaming, pre-buffered)                        │
│  • Total: <1s ahead of playback (pipelined)                               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Prompted Playlists (LLM-Powered)

Users describe what they want in natural language, and an LLM generates a playlist:

| Component | Function | Architecture |
|-----------|----------|-------------|
| **Intent Parser** | Extract mood, genre, tempo, era from natural language | Fine-tuned LLM with audio domain knowledge |
| **Constraint Mapper** | Convert parsed intent to BaRT-compatible filters | Rule engine + learned mappings |
| **Candidate Generator** | Retrieve matching tracks from catalog index | Elasticsearch with audio feature vectors |
| **Ranker** | Score and order tracks for coherent flow | BaRT model with playlist-flow objectives |
| **Explainer** | Generate playlist title and description | LLM with track metadata context |

```
User: "Something chill for a rainy Sunday morning with coffee"

Intent Parser Output:
  mood: relaxed, contemplative
  tempo: 60-90 BPM
  energy: low (0.1-0.4)
  genre_affinity: acoustic, indie folk, jazz, lo-fi
  era: no preference
  context: morning, weekend

Candidate Generation: 8,000 tracks matching filters
BaRT Scoring: Top 30 ranked by user preference + flow coherence
Output: 30-track playlist with generated title and description
```

### Podcast AI Features

| Feature | Architecture | Scale Impact |
|---------|-------------|-------------|
| **Episode Summaries** | LLM summarization of transcript | Batch processing on publish |
| **Chapter Auto-Detection** | Topic segmentation model on audio | Async pipeline per episode |
| **Transcript Search** | ASR → index → search API | Elasticsearch with phonetic matching |
| **Clip Sharing** | Start/end detection from transcript highlights | Client-side with server validation |
...
| Rank | Slowest part of the process | Severity | Impact |
|------|------------|----------|--------|
...
### Slowest part of the process 1: CDN Cache Miss Storms
...
### Slowest part of the process 2: Playlist Sync Conflicts
...
### Slowest part of the process 3: Offline Key Refresh Storms
...
### Slowest part of the process 4: Search at Scale
