...
## Case Study: AI-First Transformation (2024-2026)

**Challenge:** Spotify's traditional collaborative filtering recommendations were effective but struggled with three problems: (1) filter bubbles that limited discovery, (2) inability to understand user intent beyond listening history, and (3) no natural language interface for playlist creation.

**Architecture Decisions:**
- **AI DJ (2024):** Combined reinforcement learning segment planner with LLM-generated commentary and TTS synthesis, requiring a sub-1-second pipeline that pre-buffers DJ segments ahead of playback
- **Prompted Playlists (2025):** Integrated a fine-tuned LLM with the existing BaRT recommendation engine, translating natural language ("chill rainy Sunday") into structured audio feature queries across 100M+ track catalog
- **Podcast AI (2025-2026):** Deployed batch LLM summarization on podcast transcripts at publish time, with on-demand chapter auto-detection using topic segmentation models

**Results:** AI DJ became the fastest-adopted Spotify feature, reaching 50M+ users within 6 months. Prompted playlists increased playlist creation by 25% among users who tried the feature. Podcast summaries reduced browse-to-play time by 30%.

**Key Lesson:** AI features in a streaming platform succeed when they augment existing recommendation infrastructure (BaRT, collaborative filtering) rather than replacing it. The LLM handles natural language understanding and content generation, but track selection still flows through proven scoring pipelines optimized over a decade.
...
| Recommendation Engine | [3.12-recommendation-engine](../3.12-recommendation-engine/00-index.md) | ML personalization (BaRT comparison) |
...
| AI Voice Assistant | [3.19-ai-voice-assistant](../3.19-ai-voice-assistant/00-index.md) | TTS/audio generation (AI DJ voice) |
| LLM Inference Engine | [3.23-llm-inference-engine](../3.23-llm-inference-engine/00-index.md) | LLM serving for prompted playlists |
