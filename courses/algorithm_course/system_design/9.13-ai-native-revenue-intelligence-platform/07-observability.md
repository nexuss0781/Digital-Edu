...
|----------|---------------|-------------|
| Call-to-insight pipeline | Full path from audio capture to CRM writeback | End-to-end latency analysis; Slowest part of the process identification |
...
## 6. Service Level Indicators (SLIs) and Objectives (SLOs)

### Core SLOs

| SLO | SLI Definition | Target | Error Budget (monthly) |
|-----|---------------|--------|----------------------|
| **Call recording availability** | Successful recordings / total calls attempted | 99.99% | 4.3 min downtime |
| **Transcript availability** | Transcripts available within 5 min / total calls | 99.5% | 3.6 hours of delayed transcripts |
| **Deal score freshness** | Scores updated within 5 min of new signal / total updates | 99.0% | 7.2 hours of stale scores |
| **Dashboard API latency** | P99 latency < 2 seconds | 99.0% | 7.2 hours of elevated latency |
| **CRM sync completeness** | CRM writebacks completed within 2 min / total writes | 98.0% | 14.4 hours of delayed syncs |
| **Forecast accuracy** | Forecasts within 10% of actual (closed periods) | 85.0% | N/A (accuracy, not availability) |

### Error Budget Policy

```
Step-by-step plan in plain English: Error Budget Enforcement

FUNCTION evaluate_error_budget(slo_name, current_period):
    slo = get_slo_definition(slo_name)
    budget = slo.error_budget_monthly
    consumed = get_error_budget_consumed(slo_name, current_period)
    remaining_pct = (budget - consumed) / budget * 100

    IF remaining_pct < 10%:
        // Less than 10% budget remaining: freeze all deployments
        set_deployment_freeze(slo.affected_services)
        alert_oncall("Error budget nearly exhausted", slo_name, remaining_pct)
        RETURN "FROZEN"

    ELSE IF remaining_pct < 30%:
        // Yellow zone: slow rollouts only; no risky changes
        set_deployment_policy(slo.affected_services, "canary_only")
        alert_engineering_lead(slo_name, remaining_pct)
        RETURN "CAUTION"
...
### Runbook: ASR Queue Overflow (P1)

| Step | Action | Expected Result |
|------|--------|----------------|
| 1 | Check `asr.batch.queue_depth_hours` metric | Confirm queue depth > 30 min |
| 2 | Check `asr.gpu.utilization` across cluster | Identify if GPUs are saturated or if workers are unhealthy |
| 3 | Scale ASR worker pool by 50% | Queue depth should begin decreasing within 5 min |
| 4 | If stuck jobs: identify jobs with processing time > 3x average | Kill stuck jobs; they will reprocess from audio source |
| 5 | If GPU OOM: reduce batch size from 32 to 16 segments | Lower throughput but prevents OOM kills |
| 6 | Monitor queue drain rate; scale down when queue < 5 min | Return to normal capacity |

### Runbook: Deal Score Calibration Drift (P2)

| Step | Action | Expected Result |
|------|--------|----------------|
| 1 | Identify affected tenant segments (check `scoring.calibration.segment_bias`) | Pinpoint which deal segments have drifted |
| 2 | Review recent model deployments or data distribution changes | Determine if drift is from model change or data shift |
| 3 | Trigger per-segment recalibration using last 30 days of closed deals | Brier score should improve within 1 recalibration cycle |
| 4 | If recalibration insufficient: trigger full model retraining with expanded training window | New model version ready in <4 hours |
| 5 | A/B test new model against current on 10% shadow traffic | Validate improvement before full rollout |
| 6 | Promote new model if Brier score improves by >0.02 | Calibration restored |

### Runbook: CRM Sync Rate Limit Sustained (P2)

| Step | Action | Expected Result |
|------|--------|----------------|
| 1 | Identify affected tenant and CRM provider (check `crm.sync.rate_limit_hits`) | Confirm which tenant and CRM endpoint is rate-limited |
| 2 | Review outbound write volume; check for write storms from bulk deal score updates | Identify if volume spike is legitimate or pathological |
| 3 | Enable aggressive write batching for affected tenant | Reduce API call count by 3-5x |
| 4 | Deprioritize low-value writes (activity logs) in favor of high-value (deal scores) | Critical data flows within rate limits |
| 5 | If sustained: contact CRM provider for rate limit increase or use bulk API | Long-term fix for high-volume tenants |

---

## 8. Observability for ML Model Health

### Model Performance Monitoring

| Signal | Detection Method | Response |
|--------|-----------------|----------|
| **Accuracy drift** | Compare model predictions against actual outcomes (closed deals) | Trigger recalibration; if persistent, retrain |
| **Feature drift** | Statistical tests (KS test, PSI) on input feature distributions | Investigate data pipeline; retrain if distribution shift is real |
| **Prediction distribution shift** | Monitor mean/variance of model outputs over time | May indicate model degradation or legitimate market change |
| **Latency degradation** | P99 inference latency exceeds baseline by >50% | Check GPU health; batch size; model version |
| **Confidence collapse** | Average confidence drops below 0.5 | Model may be encountering out-of-distribution inputs; investigate |

### A/B Testing Framework for Models

```
Step-by-step plan in plain English: Model A/B Test Evaluation

FUNCTION evaluate_model_ab_test(model_a, model_b, test_config):
    // Split traffic by tenant (not by individual prediction)
    // to avoid confounding from within-tenant correlation
    control_tenants = test_config.control_tenant_ids
    treatment_tenants = test_config.treatment_tenant_ids

    // Collect predictions for closed deals during test period
    control_results = get_predictions_and_outcomes(model_a, control_tenants, test_config.period)
    treatment_results = get_predictions_and_outcomes(model_b, treatment_tenants, test_config.period)

    // Compare Brier scores
    control_brier = compute_brier_score(control_results)
    treatment_brier = compute_brier_score(treatment_results)

    // Statistical significance test
    p_value = permutation_test(control_brier, treatment_brier, n_permutations=10000)

    RETURN {
        control_brier: control_brier,
        treatment_brier: treatment_brier,
        improvement: control_brier - treatment_brier,
        p_value: p_value,
        significant: p_value < 0.05,
        recommendation: "promote" IF treatment_brier < control_brier AND p_value < 0.05 ELSE "keep_current"
    }
```
