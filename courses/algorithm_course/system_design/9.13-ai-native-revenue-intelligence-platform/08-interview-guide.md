...
### V1: Post-Call Analytics (Day 1)
- Batch recording upload → overnight transcription → basic keyword search
- Manual deal scoring by sales managers using spreadsheets
- No CRM integration; standalone dashboard
- Single-model NLP: sentiment only
- **Value**: Searchable call recordings with basic analytics

### V2: Near-Real-Time Intelligence (Growth Stage)
- Streaming audio capture with 5-minute transcription SLA
- Multi-model NLP pipeline: sentiment, topics, objections, action items
- AI-powered deal scoring with per-tenant calibration
- Bi-directional CRM sync for deal scores and activity logging
- Revenue graph for relationship intelligence
- **Value**: Actionable deal intelligence visible in CRM within minutes

### V3: AI-Native Platform (Current Target)
- Real-time coaching overlays during live calls
- LLM-powered call summarization and follow-up generation
- Monte Carlo forecast simulation with scenario analysis
- Agentic AI: autonomous CRM updates, email drafting, meeting prep
- Win/loss analysis with competitive intelligence extraction
- AI roleplay for sales training
- **Value**: AI copilot for every revenue team member
...
|--------|-------|---------|
| Calls recorded per day | 7M | Across all tenants |
| Audio hours per day | 7M | Average call ~60 min |
| Audio storage per day | ~100 TB | Before compression |
| Transcript segments per day | 2B | ~285 segments per call |
| NLP inferences per day | 60B | 2B segments × ~30 models/signals |
| Active deals tracked | 50M | Across all tenants |
| Deal score updates per day | 200M | Multiple signals per deal per day |
| CRM sync writes per day | 500M | Scores + activity + coaching notes |
| Concurrent live calls | 1M | Peak during business hours |
| ASR GPU cost target | <$0.002/minute | After optimization |
| Transcription SLA | <5 min post-call | Batch high-accuracy transcription |
| Deal score freshness | <5 min from new signal | Near-real-time update |
| Forecast accuracy target | Within 10% of actual | Measured at period close |

---

## 8. Trap Questions and How to Handle Them

| Trap | What They Want | How to Answer |
|------|---------------|---------------|
| "Why not use one big LLM for everything?" | Test ML architecture depth | "A single LLM for 60B daily inferences would be prohibitively expensive. Specialized models (sentiment in 5ms, LLM summary in 2s) match latency and cost to the task. The LLM is reserved for high-value generative tasks." |
| "Can't you just use real-time ASR for everything?" | Test latency/accuracy trade-off understanding | "Real-time ASR has 15-20% WER; batch ASR achieves 8-10% WER. The platform runs both: real-time for live coaching, batch for permanent record. Insights from batch ASR override real-time results." |
| "Why is forecasting hard? Just sum up the deal scores." | Test forecasting depth | "Summing probabilities gives expected value but no confidence interval. Monte Carlo simulation captures variance, correlation between deals, and pipeline coverage. The manager needs to know not just 'likely $5M' but '80% chance between $4.2M and $5.8M'." |
| "This is just a CRM with audio" | Test domain understanding | "CRM stores structured data entered by reps. This platform generates intelligence from unstructured conversation data---audio, transcripts, NLP signals---that reps never manually enter. The CRM is the *consumer* of insights, not the *source*." |
...
|----------|----------|----------|----------------|
| **ASR strategy** | Real-time only (lower latency) | Dual-path: real-time + batch (higher accuracy) | Dual-path: real-time for coaching, batch for permanent record |
| **Model architecture** | Single LLM for all NLP | Specialized ensemble + LLM for generative | Ensemble: 30x cheaper for classification tasks; LLM for summaries only |
| **Deal scoring** | Batch (hourly) | Event-driven (near-real-time) | Event-driven with hourly recalibration |
| **CRM sync** | Push (immediate writeback) | Pull (CRM polls our API) | Push with retry queue; pull as fallback for rate-limited tenants |
| **Graph database** | Dedicated per tenant | Shared with partition isolation | Shared for standard; dedicated for enterprise tenants |
| **Audio storage** | Keep all at full quality | Tiered with compression | Tiered: 7x cost reduction with minimal quality loss |
| **Forecast method** | Single point estimate | Monte Carlo simulation | Monte Carlo: managers need confidence intervals, not just expected values |
| **Model serving** | Self-hosted GPU | Managed inference API | Self-hosted for ASR/NLP (volume economics); managed API for LLM (burst flexibility) |
...
|-----------|-------------|
| **Data pipeline thinking** | Trace the full path: audio → transcript → NLP → signals → scores → forecasts → CRM |
| **Cost awareness** | Cite GPU costs, storage tiering savings, model distillation benefits |
| **ML system design** | Explain calibration, ensemble architecture, feedback loops, A/B testing for models |
| **Integration complexity** | Discuss CRM rate limits, conflict resolution, bi-directional sync challenges |
| **Compliance first** | Mention consent handling and data residency early, not as an afterthought |
| **Scale reasoning** | Use specific numbers (7M calls/day, 60B NLP inferences) to justify architecture choices |
