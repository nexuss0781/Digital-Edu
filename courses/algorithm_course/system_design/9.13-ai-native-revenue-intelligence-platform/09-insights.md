...
**Latency**: Specialized models complete inference in 1--10ms. LLMs require 500ms--5s. For the NLP pipeline to complete within its 10-minute SLA, models must run in parallel with minimal per-model latency. LLM latency would be the Slowest part of the process even with massive parallelism.
