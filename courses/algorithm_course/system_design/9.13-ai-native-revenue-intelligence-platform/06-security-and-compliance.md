...
## 7. Data Residency and Cross-Border Compliance

### Regional Data Flow Control

```
Step-by-step plan in plain English: Data Residency Enforcement

FUNCTION validate_data_flow(data, source_region, target_region, tenant_id):
    tenant_policy = get_data_residency_policy(tenant_id)

    // Check if cross-region transfer is allowed
    IF source_region != target_region:
        IF tenant_policy.restrict_cross_region:
            RAISE DataResidencyViolation(
                "Cross-region transfer blocked for tenant",
                source=source_region, target=target_region
            )

    // Check if target region is in allowed list
    IF target_region NOT IN tenant_policy.allowed_regions:
        RAISE DataResidencyViolation(
            "Target region not in allowed list",
            allowed=tenant_policy.allowed_regions
        )

    // Log data flow for compliance audit
    log_data_flow_event(data.type, source_region, target_region, tenant_id)
    RETURN ALLOW
```
...
|-----------|-----------------|------------------------|
| **GDPR** | Data minimization, right to erasure, consent, DPIAs | Consent engine, erasure pipeline, PII detection, regional storage |
| **CCPA** | Right to know, right to delete, opt-out of sale | Data export API, deletion workflow, no data sharing without consent |
| **SOC 2 Type II** | Security controls, availability, confidentiality | Audit logs, access controls, encryption, monitoring |
| **HIPAA** (healthcare tenants) | PHI protection, access controls, audit trails | PHI detection in transcripts, field-level encryption, BAA support |
| **FedRAMP** (government tenants) | Federal security standards | Dedicated government cloud deployment, FIPS 140-2 encryption |

### Call Recording Consent Jurisdictions

| Jurisdiction Type | Consent Requirement | Platform Handling |
|-------------------|-------------------|-------------------|
| One-party consent (most US states) | Only one participant must consent | Recording notification at call start; rep consent assumed |
| Two-party/all-party consent (CA, IL, etc.) | All participants must consent | Active consent prompt; recording paused if any party declines |
| EU (GDPR) | Explicit informed consent with legitimate interest | Pre-call consent, recording purpose stated, opt-out available |
| No-consent recording (internal calls) | Internal training calls may have blanket consent | Policy-based: internal calls follow org policy; external calls follow jurisdiction |

### API Security Hardening

| Layer | Protection | Implementation |
|-------|-----------|----------------|
| **Edge** | DDoS mitigation, bot detection | CDN-level rate limiting; challenge-response for suspicious traffic |
| **API Gateway** | Authentication, tenant isolation | JWT validation; API key rotation; tenant_id propagation |
| **Service mesh** | mTLS between services | Certificate-based service identity; no plaintext internal traffic |
| **Data layer** | Query isolation, encryption | Row-level security by tenant_id; TDE + field-level encryption for PII |
| **Egress** | Data exfiltration prevention | Outbound allowlists; no raw transcript export without admin approval |
