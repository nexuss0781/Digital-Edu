...
A single inventory counter would create a serialization Slowest part of the process where all 50,000 requests/sec contend for one row.
...
4. Queue processes FIFO (First-In-First-Out, like a line at a store), granting inventory access tokens
...
## 8. Recommendation Engine at Scale

### Challenge: Real-Time Personalization at 230K Pages/Sec

Every product page must show personalized recommendations ("Frequently Bought Together," "Customers Also Viewed," "Inspired by Your Browsing"). At 230K page views per second, computing recommendations on-the-fly is impractical.

### Architecture: Precomputed + Real-Time Hybrid

```
Precomputed Layer (batch, hourly):
├── Item-to-item collaborative filtering:
│   "Product A → Products B, C, D" (based on co-purchase patterns)
│   Stored in key-value store: key="recs:item:{product_id}", value=[product_ids]
│   Coverage: top 10M products (by purchase volume)
│
├── User-to-item personalized recommendations:
│   "User X → Products P, Q, R" (based on purchase + browse history)
│   Stored in key-value store: key="recs:user:{user_id}", value=[product_ids]
│   Coverage: top 50M active users
│
└── Category-level trending products:
    "Category Electronics>Audio → Products T, U, V"
    Fallback for cold-start users and long-tail products

Real-Time Layer (on request, < 20ms):
├── Session-based signals:
│   Current session clicks → adjust recommendation weights
│   "User just browsed 3 running shoes → boost athletic footwear"
│
├── Contextual filters:
│   Remove already-purchased products
│   Remove out-of-stock products
│   Filter by user's price sensitivity band
│
└── Re-ranking:
    Lightweight model merges precomputed scores with session signals
    Output: 6-12 recommended products per widget
```
...
|----------|------------------|
| New user, no history | Category-level trending products + new arrivals |
| New product, no co-purchase data | Content-based similarity (attribute matching) |
| Long-tail product (< 100 views) | Category fallback + brand similarity |
| Anonymous user | Session-based recommendations only (clicks in current session) |

---

## 9. Cart Price Staleness and Re-Pricing

### The Problem

Carts persist for days or weeks. During that time, seller prices change, promotions expire, and items go in/out of stock. If the cart shows a stale price and checkout charges a different amount, it erodes customer trust.

### Solution: Lazy Re-Pricing with Change Indicators

```
On cart view (every time user opens cart):
1. FOR each item IN cart:
    current_price = PricingEngine.getPrice(item.product_id, item.offer_id)

    IF current_price != item.price_at_add:
        item.current_price = current_price
        item.price_changed = TRUE
        item.price_direction = IF current_price > item.price_at_add THEN "UP" ELSE "DOWN"

    IF NOT InventoryService.isAvailable(item.product_id):
        item.available = FALSE
        item.unavailable_reason = "OUT_OF_STOCK"

2. Display price-change badges:
   "Price dropped $10 since you added this!" (green badge)
   "Price increased $5 since you added this" (yellow badge)
   "This item is no longer available" (red badge → suggest alternatives)

3. At checkout initiation:
   Re-verify ALL prices and availability
   Show summary of all changes since items were added
   Require explicit confirmation if any price increased
```

---

## 10. Catalog Deduplication at Ingestion Scale

### The Problem

With 2M sellers uploading 5M listings daily, the same product gets listed under slightly different titles:
- "Sony WH-1000XM5 Wireless Headphones - Black"
- "Sony WH1000XM5 Bluetooth Noise Cancelling Headphone Black"
- "SONY WH 1000 XM5 wireless over-ear headphones (black)"

All three are the same product. If treated as separate listings, customers see three product pages instead of one, and buy box competition breaks.

### Multi-Signal Matching Pipeline

```
Input: Seller submits new listing

Stage 1: Exact Match (< 10ms)
├── UPC/EAN/ISBN barcode lookup → exact product match
├── If UPC found → 99.9% confidence → link offer to existing product
└── If no UPC → proceed to fuzzy matching

Stage 2: Fuzzy Title Match (< 50ms)
├── Normalize: lowercase, remove punctuation, expand abbreviations
├── Tokenize: "sony wh 1000 xm5 wireless bluetooth headphones black"
├── TF-IDF similarity against catalog titles
├── Match score > 0.85 → candidate match
└── Multiple candidates → proceed to attribute matching

Stage 3: Attribute Matching (< 30ms)
├── Compare: brand, model number, color, size
├── Weight: model number match = 50%, brand = 25%, attributes = 25%
├── Score > 0.90 → high-confidence match
└── Score 0.70-0.90 → candidate for manual review

Stage 4: Image Matching (< 100ms, async)
├── Compute perceptual hash of listing images
├── ANN search against existing product image embeddings
├── If visual match + title match → confirm dedup
└── If visual mismatch + title match → flag as potential variant

Result Actions:
├── Confidence > 95% → auto-link to existing product
├── Confidence 70-95% → queue for catalog team review
├── Confidence < 70% → create new product listing
└── Exact duplicate (same seller, same product) → reject with message
```
...
| Slowest part of the process | Impact | Solution |
|------------|--------|----------|
...
| Cart price staleness | Customer surprise at checkout from changed prices | Lazy re-pricing with price-change indicators |
| Recommendation cold start | No signals for new users/products | Hybrid precomputed + session-based with category fallbacks |
| Catalog deduplication | Same product as multiple listings | Multi-stage fuzzy matching pipeline (UPC → title → attributes → images) |
