...
- "Inventory is the consistency Slowest part of the process—I'll use optimistic locking to prevent overselling without creating lock contention"
...
| **Flash sale inventory** | Single atomic counter | Pre-sharded counters | Pre-sharded | Single counter = 50K writes/sec on one key → Slowest part of the process; 64 shards = 800 writes/sec per shard |
...
| Dimension | Amazon E-Commerce | Ride-Hailing (7.1) | Food Delivery (7.4) | Flight Booking (7.6) |
|-----------|-------------------|-------------------|---------------------|---------------------|
| **Inventory model** | Distributed across warehouses; pre-reserved | Real-time driver pool; dynamic | Restaurant menu stock; limited | External GDS; seat maps |
| **Consistency need** | Strong for reservation; eventual for display | Eventual (driver matching) | Strong (kitchen capacity) | Strong (seat assignment) |
| **Cart pattern** | Long-lived, multi-seller, merge-on-login | No cart (instant match) | Single-vendor, 30-min expiry | No cart (single booking) |
| **Saga complexity** | Inventory + payment + order + fulfillment routing | Match + dispatch + payment | Order + prep + pickup + delivery | Reserve + pay + ticket |
| **Traffic spike** | Prime Day: 10-15× for 48 hours | Rush hour: 2-3× daily | Meal times: 3× daily | Holidays: 3-5× seasonal |
| **Price model** | Seller competition + buy box + promotions | Dynamic surge pricing | Menu price + delivery fee | Fare classes + load factor |

---

## System Design Variants

### 15-Minute Version (Quick Sketch)
- Draw: Client → API Gateway → Search/Cart/Checkout → DB/Cache/Events
- Mention: CQRS for catalog, key-value for cart, saga for checkout
- Key insight: "1000:1 read-write ratio drives every architectural decision"

### 30-Minute Version (Standard Interview)
- Cover: Architecture + inventory reservation + checkout saga + buy box
- Skip: Flash sales, cell-based architecture, caching details
- Prioritize: One deep dive into inventory or checkout

### 60-Minute Version (Senior/Staff Level)
- Full architecture + two deep dives
- Add: Cell-based deployment, chaos engineering, multi-region replication
- Discuss: Progressive load shedding, error budget philosophy, team structure (20+ services owned by different teams → API contracts and SLOs between teams)

---

## "What Would You Do Differently?" Scenarios

| Scenario | Alternative Approach | Trade-Off |
|----------|---------------------|-----------|
| "What if the catalog was 1M products, not 500M?" | Skip search index; query catalog DB directly with full-text index | Simpler architecture, but loses ML reranking and faceted search performance |
| "What if there were no third-party sellers?" | Remove buy box, seller management, listing service | Simpler but limits catalog growth; all inventory must be self-sourced |
| "What if flash sales weren't a requirement?" | Standard inventory reservation without pre-sharding | Eliminates shard management complexity; standard optimistic locking suffices |
| "What if checkout latency didn't matter (< 5s is fine)?" | Synchronous two-phase commit instead of saga | Simpler error handling but higher latency; acceptable for low-volume systems |
| "What if the system only served one country?" | Single-region deployment; no data residency concerns | Removes multi-region complexity and cross-region replication |
...
### "How do you handle returns affecting promotions?"

**Weak answer:** "We just refund the item price."

**Strong answer:** "Returns are tricky when promotions are involved. If a customer ordered 3 items qualifying for '10% off orders over $100' and returns one item, the order may drop below $100. The system must recalculate: was the promotion threshold broken? If yes, the customer owes the difference (the promotion discount on remaining items is clawed back). The refund amount for the returned item is: `item_price - proportional_clawback`. This requires the order to store both the pre-promotion and post-promotion prices for each item, and a promotion recalculation engine that runs on every return event. Edge Case (Unusual or extreme situation): if the returned item was the most expensive, the clawback might exceed the refund amount—in practice, most platforms eat the loss as a customer goodwill decision."

### "How would you migrate from a monolith to this microservices architecture?"

**Strong answer:** "Strangler fig pattern. Start with the highest-value, most independent service: search. Extract search into its own service behind the same API contract. Route 1% of traffic to the new service, compare results, and gradually increase. Next, extract cart (it already has a clean key-value access pattern). Then checkout (the saga pattern naturally decomposes into independent steps). Keep the monolith running for everything else until each service is extracted. The migration takes years, not months—and that's by design. Each extraction must be safe to roll back."

### "What metrics would tell you the system is unhealthy before customers notice?"

**Strong answer:** "Leading indicators, not lagging. Cache hit rate dropping below 85% (means DB is about to get hammered). Event bus consumer lag growing (means fulfillment is falling behind). Inventory reservation retry rate above 5% (means hot SKU contention is spiking). Search zero-result rate above 5% (means index is stale or corrupt). These fire 5-15 minutes before customer-facing metrics like error rates or latency degradation, giving us time to react."

---

## Whiteboard Notation Guide

When drawing the architecture, use these conventions:

```
Component Notation:
├── [Rectangle]        → Service or application component
├── [(Cylinder)]       → Database or persistent store
├── [/Parallelogram/]  → Cache or index
├── [[Double bracket]] → External service (payment gateway)
├── {Diamond}          → Decision point
└── Arrows:
    ├── ─── solid      → Synchronous call
    ├── ··· dotted     → Asynchronous / event-driven
    └── ═══ double     → Data replication

Labeling:
├── Write QPS and read QPS on data stores
├── Latency budgets on critical paths (e.g., "< 300ms")
├── Consistency model next to each database (strong/eventual)
└── Cache hit rates on cache connections (e.g., "95% hit")
```

### Architecture Walkthrough Script

```
"I'd draw it in four layers:

1. CLIENT → API Gateway → BFF: Standard edge layer with rate limiting and auth.

2. DISCOVERY: Search Service backed by a search index (500M docs, 58K QPS).
   Catalog Service for writes, CQRS to separate read/write paths.
   Why: 1000:1 read-write ratio makes single-DB search impossible.

3. COMMERCE: Cart (key-value store—not relational, because the access pattern
   is always by user_id). Checkout Orchestrator as a saga.
   Inventory Service with optimistic locking and sharded DB.
   Why: Strong consistency only at the reservation boundary.

4. FULFILLMENT: Event-driven from OrderPlaced event.
   Router selects warehouse(s), carrier, generates labels.
   Each shipment has independent tracking lifecycle.
   Why: Async decouples customer-facing latency from fulfillment complexity."
```
