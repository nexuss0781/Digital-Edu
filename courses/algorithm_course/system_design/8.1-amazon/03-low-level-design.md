...
Deal {
    deal_id:            UUID
    product_id:         UUID
    deal_type:          enum                -- LIGHTNING, DEAL_OF_DAY, PRIME_EXCLUSIVE, COUPON
    deal_price:         decimal             -- discounted price during deal
    original_price:     decimal             -- reference price for strikethrough
    discount_percent:   float               -- computed: (1 - deal_price/original_price) × 100
    total_quantity:     int                 -- total units available at deal price
    claimed_quantity:   int                 -- units claimed so far
    shard_count:        int                 -- number of inventory shards (e.g., 64)
    max_per_customer:   int                 -- limit per customer (typically 1-3)
    start_time:         timestamp
    end_time:           timestamp
    status:             enum                -- SCHEDULED, ACTIVE, SOLD_OUT, EXPIRED, CANCELLED
    eligibility:        enum                -- ALL, PRIME_ONLY, INVITE_ONLY
    created_at:         timestamp
}
...
DealClaim {
    ...
    expires_at:         timestamp           -- 15-min TTL for deal reservation
    order_id:           UUID                -- populated on purchase
}

-- Index: (deal_id, status), (user_id, deal_id) UNIQUE for max_per_customer enforcement
```
...
Category {
    category_id:        UUID
    name:               string              -- "Over-Ear Headphones"
    slug:               string              -- "over-ear-headphones"
    parent_id:          UUID                -- null for root categories
    path:               string[]            -- ["Electronics", "Audio", "Headphones", "Over-Ear"]
    depth:              int                 -- 0 = root, 4 = leaf
    is_leaf:            boolean             -- only leaf categories have products
    product_count:      int                 -- cached count of active products
    attribute_template: string[]            -- required attributes for products in this category
    display_order:      int                 -- sort position within parent
    status:             enum                -- ACTIVE, HIDDEN, DEPRECATED
}

-- Index: (parent_id, display_order), (slug UNIQUE)
-- Note: ~40,000 leaf categories, ~5,000 internal nodes
-- Tree cached in memory with 1-hour refresh; ~2 MB serialized
```
...
Shipment {
    ...
    order_id:           UUID
    warehouse_id:       UUID
    items:              ShipmentItem[]      -- subset of order items
    carrier:            string              -- carrier identifier
    tracking_number:    string
    shipping_method:    enum                -- STANDARD, EXPEDITED, SAME_DAY, NEXT_DAY
    label_url:          string              -- shipping label (object storage)
...
-- Index: (order_id), (tracking_number UNIQUE), (status, estimated_delivery)
```
...
```
FUNCTION estimateDelivery(product_id, shipping_address, shipping_method):
    -- Step 1: Find warehouses with stock
    warehouses = getWarehousesWithStock(product_id)
    IF warehouses.isEmpty():
        RETURN { available: FALSE }

    -- Step 2: For each warehouse, compute delivery estimate
    estimates = []
    FOR each wh IN warehouses:
        -- Processing time: picking + packing (varies by warehouse load)
        processing_hours = getWarehouseProcessingTime(wh.id, shipping_method)

        -- Carrier transit time: based on origin-destination zip pair
        transit_days = getCarrierTransitTime(
            origin_zip: wh.zip_code,
            dest_zip: shipping_address.zip_code,
            method: shipping_method
        )

        -- Cut-off time: orders after 2 PM ship next business day
        cutoff = getWarehouseCutoffTime(wh.id, shipping_method)
        IF now() > cutoff:
            processing_hours += 24  -- shifts to next business day

        -- Skip weekends and holidays for delivery estimate
        ship_date = addBusinessHours(now(), processing_hours)
        delivery_date = addBusinessDays(ship_date, transit_days)

        -- Apply historical accuracy adjustment
        -- If carrier has been delivering 1 day late to this zip recently, add buffer
        accuracy_buffer = getHistoricalDeliveryBuffer(wh.zip_code, shipping_address.zip_code)
        delivery_date = addDays(delivery_date, accuracy_buffer)

        estimates.append({
            warehouse_id: wh.id,
            delivery_date: delivery_date,
            shipping_cost: calculateShippingCost(wh, shipping_address, shipping_method),
            confidence: getDeliveryConfidence(wh.zip_code, shipping_address.zip_code)
        })

    -- Step 3: Return fastest delivery option
    best = estimates.minBy(delivery_date)
    ...
    RETURN {
        ...
        confidence: best.confidence,  -- "guaranteed" | "estimated"
        all_options: estimates.sortBy(delivery_date)
    }
```
...
```
FUNCTION autocomplete(prefix, user_context):
    -- Stage 1: Prefix matching from trie index (< 5ms)
    candidates = trieIndex.prefixSearch(prefix, limit=20)

    -- Stage 2: Augment with recent user searches
    recent = getUserRecentSearches(user_context.user_id)
    user_matches = recent.filter(q => q.startsWith(prefix))

    -- Stage 3: Score and rank suggestions
    scored = []
    FOR each candidate IN candidates:
        score = candidate.global_frequency * 0.5           -- popular queries rank higher
             + candidate.category_relevance * 0.2          -- relevant to user's browsing
             + (1.0 IF candidate IN user_matches ELSE 0) * 0.3  -- personal history boost

        scored.append({ query: candidate.text, score, category: candidate.top_category })

    -- Stage 4: Deduplicate and diversify
    scored.sortBy(score, DESC)
    results = diversify(scored, max_per_category=3)

    RETURN results.take(10)  -- return top 10 suggestions
```
