...
### Product Authenticity Pipeline

```
For high-value categories (electronics, luxury, pharmaceuticals):

1. Transparency Program (serialization):
   ├── Each unit gets unique serial code at manufacturing
   ├── Code registered on platform's authenticity ledger
   ├── Customer scans code to verify authenticity
   └── Duplicate code detection → flag as counterfeit

2. Brand Registry:
   ├── Brand owners register trademarks + authorized sellers
   ├── Automated monitoring: flag listings from non-authorized sellers
   ├── IP violation reporting: brand owners submit takedown requests
   └── Response SLA: 24 hours for review, 48 hours for removal

3. Test Buy Program:
   ├── Platform periodically purchases from high-risk sellers
   ├── Physical inspection at authentication center
   ├── Failed tests → seller suspension + inventory quarantine
   └── Results feed into seller risk scoring model
```
...
|---------------|----------|---------|--------|------------|
| **Restricted** | Payment tokens, tax IDs, bank accounts | Isolated encrypted store | Named individuals only; audit-logged | Field-level + at-rest + in-transit |
| **Confidential** | Customer PII (name, address, email) | Regional encrypted DB | Service-level auth + purpose limitation | At-rest + in-transit |
| **Internal** | Order details, seller performance scores | Standard DB | Authenticated services | At-rest + in-transit |
| **Public** | Product catalog, reviews, categories | Cache + CDN | Public APIs with rate limiting | In-transit only |

---

## Incident Response for E-Commerce

| Incident Type | Detection | Response Time | Immediate Action |
|--------------|-----------|---------------|------------------|
| **Payment data breach** | Anomalous access patterns to payment store | < 15 min | Isolate affected systems; notify payment processor; begin forensic analysis |
| **Account takeover wave** | Spike in password resets + login failures from varied IPs | < 30 min | Enable global CAPTCHA; force MFA for affected accounts; block credential-stuffing IPs |
| **Counterfeit product ring** | Cluster of brand complaints + test buy failures | < 4 hours | Suspend seller accounts; quarantine inventory; remove listings |
| **Price manipulation attack** | Seller drops price to $0.01 then raises after buy box win | < 1 hour | Rollback buy box assignment; flag seller for review; apply price velocity limits |
| **Inventory oversell** | Reserved > total for any SKU-warehouse pair | Immediate (automated) | Halt orders for affected SKU; audit reservation logs; compensate affected customers |
