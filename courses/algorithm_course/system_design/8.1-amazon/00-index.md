...
|---------------|-------------|
| **Read/Write Pattern** | Extremely read-heavy: 1000:1 browse-to-buy ratio; product pages served from multi-layer cache (CDN + distributed cache), search queries at 50K+ QPS |
...
| **Peak Event Strategy** | Cell-based deployment + progressive load shedding + pre-sharded flash sale counters |
...
flowchart TB
    subgraph Core["Core Commerce"]
        CATALOG[Product Catalog<br/>500M+ SKUs]
        SEARCH[Search & Discovery<br/>58K QPS Peak]
        CART[Shopping Cart<br/>Distributed KV]
        CHECKOUT[Checkout Saga<br/>Inventory + Payment + Order]
    end

    subgraph Marketplace["Marketplace Layer"]
        BUYBOX[Buy Box Engine<br/>80% of Sales]
        SELLER[Seller Platform<br/>2M+ Sellers]
    end

    subgraph Fulfillment["Fulfillment Engine"]
        INV[Inventory<br/>200+ Warehouses]
        ROUTE[Fulfillment Router<br/>Split Shipments]
        SHIP[Shipping & Tracking]
    end

    subgraph Scale["Scale Challenges"]
        PRIME[Prime Day<br/>10-15× Traffic]
        FLASH[Flash Sales<br/>50K+ req/sec per deal]
        CELL[Cell-Based<br/>Blast Radius Isolation]
    end
    ...
    classDef market fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef fulfill fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef scale fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class CATALOG,SEARCH,CART,CHECKOUT core
    class BUYBOX,SELLER market
    class INV,ROUTE,SHIP fulfill
    class PRIME,FLASH,CELL scale
```
...
| [1.19 - CQRS Implementation](../1.19-cqrs-implementation/) | Core pattern for catalog write-path / search read-path separation |
| [12.18 - Marketplace Platform](../12.18-marketplace-platform/) | Seller onboarding, multi-vendor orchestration, commission models |
| [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Embedded financing at checkout, credit decisioning integration |
...
|-------|----------------------|
| **Vector Search + Semantic Understanding** | Hybrid retrieval (BM25 + dense vectors) replaces keyword-only search; enables natural language queries like "something cozy to read on the beach" |
| **Conversational Commerce** | LLM-powered shopping assistants guide product discovery via multi-turn dialogue; requires session-aware recommendation pipelines |
| **Visual Search & AR Try-On** | Image embedding pipelines for "search by photo"; augmented reality product previews for furniture, fashion, cosmetics |
| **AI-Powered Dynamic Pricing** | Real-time price optimization using demand signals, competitor monitoring, and inventory velocity; buy box algorithm incorporates AI repricing |
| **Composable Commerce (MACH)** | Microservices + API-first + Cloud-native + Headless architecture enables rapid feature composition and channel-agnostic commerce |
| **Green Fulfillment Routing** | Carbon-aware warehouse selection adds sustainability scoring to the fulfillment router; consolidated shipment options reduce packaging waste |
| **Predictive Inventory** | ML models forecast demand per SKU per warehouse; pre-position stock before demand materializes; reduce stockout rates by 20-30% |
| **Edge-Based Personalization** | Personalization models deployed to CDN edge nodes; sub-10ms recommendation inference without origin round-trips |
...
- DeepMind / Google Research — Vector Search and Hybrid Retrieval for E-Commerce (2025)
- Composable Commerce Alliance — MACH Architecture Reference Implementations
- ACM Queue — Cell-Based Architecture at Scale: Lessons from Production
- IEEE — Hybrid Search Architectures for Large-Scale E-Commerce Catalogs
- Stanford CS246 — Mining Massive Datasets: Recommendation Systems at Scale
- O'Reilly — Designing Data-Intensive Applications: CQRS, Event Sourcing, and Saga Patterns
