...
|----------|---------|-----------|
| **P0 — Must Have** | Product Search, Cart, Checkout, Inventory Reservation, Order Placement | Core purchase path; without these, no revenue |
| **P0 — Must Have** | Payment Processing, Inventory Management | Financial correctness and stock accuracy |
| **P1 — Should Have** | Marketplace (3P sellers), Buy Box, Seller Dashboard | 60%+ of GMV from 3P sellers |
| **P1 — Should Have** | Reviews & Ratings, Delivery Estimation | Critical for conversion rate |
| **P2 — Nice to Have** | Recommendations, Wishlists, Subscribe & Save | Engagement and retention features |
| **P2 — Nice to Have** | Promotions & Lightning Deals, Gift Wrapping | Revenue optimization |
| **P3 — Future** | Visual Search, AR Try-On, Conversational Commerce | AI-driven next-generation features |
...
| Scenario | Constraint | Design Impact |
|----------|-----------|---------------|
| **Seller bulk upload** | 2M sellers × 100 updates/day avg = 200M catalog updates/day | Async ingestion pipeline with rate limiting per seller; batch API for bulk operations |
| **Product variation explosion** | Some products have 50+ variation combinations (size × color × material) | Variation group model with lazy loading; only render visible combinations |
| **Cross-border orders** | Import duties, customs declarations, currency conversion | Separate checkout flow for international orders; duty calculator service |
| **Gift wrapping + messaging** | 15% of holiday orders include gift options | Additional order metadata without changing core order model |
| **Subscribe & Save** | Recurring orders with dynamic pricing and quantity adjustments | Subscription service with cron-based order generation; price lock windows |
| **Digital products** | No physical fulfillment; instant delivery (e-books, music, software) | Separate digital fulfillment pipeline; license key generation and delivery |
| **Pre-orders** | Inventory doesn't exist yet; estimated delivery dates shift | Pre-order reservation model with soft delivery commitments; notification on availability |
| **Back-ordered items** | Demand exceeds supply; customers willing to wait | Backorder queue with FIFO (First-In-First-Out, like a line at a store) allocation on restock events |
...
## Prime Day Capacity Planning

```
--- Prime Day Traffic Multipliers ---
Component              Normal Peak    Prime Day Peak    Multiplier
Page views/sec:        231,500        3,472,500         15×
Search QPS:            57,900         579,000           10×
Add-to-cart/sec:       5,800          145,000           25× (flash deals)
Orders/sec:            1,150          3,450             3×
Cart operations/sec:   580            14,500            25×
Inventory updates/sec: 50,000         250,000           5×

--- Prime Day Infrastructure Requirements ---
Search cluster:        100 → 800 nodes (pre-provisioned)
Checkout instances:    50 → 400 instances
Cache capacity:        2 TB → 8 TB (deal data pre-warming)
Event bus throughput:  500K msg/sec → 2.5M msg/sec
CDN bandwidth:         500 GB/s → 2 TB/s

--- Prime Day Duration ---
Event duration:        48 hours
Peak window:           First 2 hours (initial deal rush)
Sustained high load:   12-16 hours/day
```
...
|-----------|----------------|------------------|-------|
| **Compute (services)** | ~2,000 instances avg | $2-4M | Auto-scales 3-10× at peak |
| **Search cluster** | 100 nodes, 1 TB index | $300-500K | Dominated by memory for index |
| **Relational DBs** | 40 shards (order + inventory) | $400-600K | High-IOPS storage for writes |
| **Key-value store** | 50 GB cart + 2 TB cache | $100-200K | Memory-optimized instances |
| **Object storage** | 2 PB images | $40-60K | Low per-GB cost; dominated by retrieval |
| **CDN bandwidth** | ~500 GB/s peak | $5-10M | Largest cost component |
| **Event bus** | 500K msg/sec sustained | $200-400K | Scales with order volume |
| **Total estimated** | — | **$10-20M/month** | Excludes Prime Day surge provisioning |
...
3. **Inventory is the consistency Slowest part of the process**: 2.5B SKU-warehouse records with 50K updates/sec requires careful sharding. Strong consistency is mandatory only for the reservation path (checkout); eventual consistency is acceptable for display availability.
...
7. **Seller bulk uploads create ingestion spikes**: 2M sellers updating listings creates 200M catalog changes/day (~2,300/sec average), but bulk upload windows (early morning) can spike to 10× this rate. The catalog ingestion pipeline must handle 23K updates/sec without impacting search index freshness.

8. **Order history grows indefinitely**: At 10M orders/day with 7-year retention, the order database reaches ~100 TB of hot data. Partition by year; archive orders older than 2 years to cold storage with on-demand retrieval.
...
| What | Formula | Result |
|------|---------|--------|
| Page views/sec (avg) | 2B / 86,400 | ~23,150 |
| Page views/sec (peak) | 23,150 × 10 | ~231,500 |
| Orders/sec (avg) | 10M / 86,400 | ~115 |
| Orders/sec (Prime Day) | 115 × 30 | ~3,450 |
| Search QPS (avg) | 500M / 86,400 | ~5,790 |
| Search QPS (peak) | 5,790 × 10 | ~57,900 |
| Cart storage | 100M × 500 B | ~50 GB |
| Catalog storage | 500M × 5 KB | ~2.5 TB |
| Image storage | 500M × 8 × 500 KB | ~2 PB |
| Daily order growth | 10M × 2 KB + 35M × 500 B | ~37.5 GB/day |
| Inventory records | 500M × 5 WH avg | ~2.5B rows |
| CDN bandwidth (peak) | 231.5K × 200 KB | ~46 GB/s |
| Review storage | 1B × 1 KB | ~1 TB |
