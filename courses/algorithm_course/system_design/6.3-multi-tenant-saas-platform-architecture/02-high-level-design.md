...
### 7. Tenant-Aware Search Architecture

| Aspect | Design Decision | Rationale |
|--------|----------------|-----------|
| **Index isolation** | Per-org index shards within a cell-level search cluster | Org data never mixed in the same index segment; enables per-org re-indexing |
| **Schema mapping** | Dynamic field mappings derived from tenant metadata | Custom fields auto-mapped to search index fields on creation |
| **Query security** | OrgID mandatory in every search query (enforced by search proxy) | Same pattern as database queries; search proxy rejects unscoped queries |
| **Ranking** | Per-org relevance tuning (customizable boost factors) | Enterprise tenants can weight fields differently |
| **Near-real-time** | Async indexing from CDC stream (< 2s latency) | Write path not blocked by indexing; eventual consistency acceptable for search |

### 8. Bulk API Architecture

The bulk API handles large-scale data operations (import, export, delete) asynchronously:

```mermaid
stateDiagram-v2
    [*] --> Open: POST /jobs/ingest
    Open --> UploadComplete: PATCH (close uploads)
    Open --> Aborted: PATCH (abort)
    UploadComplete --> InProgress: Worker picks up job
    InProgress --> JobComplete: All batches processed
    InProgress --> Failed: Unrecoverable error
    JobComplete --> [*]: Results available
    Failed --> [*]: Error details available
    Aborted --> [*]: Partial results purged

    note right of InProgress
        Processing: 10K records/batch
        Governor limits: async profile (2x sync)
        Throttle: 2s inter-batch delay
        Progress: queryable via GET /jobs/{id}
    end note
```

**Key design decisions:**
- **Job-level idempotency:** Job ID is the idempotency key; re-submitting the same job ID returns existing results
- **Partial success:** Each batch within a job succeeds or fails independently; the job reports per-batch results
- **Back-pressure:** If a cell's bulk queue depth > 10K jobs, new submissions are throttled (429 with Retry-After)
- **Priority:** Interactive API traffic always has priority over bulk jobs via separate thread pools and connection pools
...
- [x] **Change data capture** -- CDC streams for per-tenant event subscriptions
- [x] **Idempotency** -- Idempotency keys for POST, job IDs for bulk, ETags for PATCH
...
| Event Category | Examples | Delivery Guarantee | Consumers |
|---------------|---------|-------------------|-----------|
| **Data events** | Record created, updated, deleted | At-least-once | Search indexer, audit log, webhooks, CDC stream |
| **Metadata events** | Field added, object created, rule changed | Exactly-once (cache invalidation) | All app servers (pub/sub), metadata cache |
| **Lifecycle events** | Org provisioned, suspended, migrated | At-least-once | Billing system, monitoring, compliance audit |
| **Governor events** | Limit approached (80%+), limit exceeded | At-most-once (best effort) | Tenant admin notifications, platform monitoring |
| **System events** | Cell failover, deploy started, backup completed | At-least-once | Global monitoring, operations dashboard |

### Change Data Capture (CDC) for Tenant Integrations

```
Tenant Data Write → WAL → CDC Extractor → Per-Org Event Stream → Tenant's Consumer
...
CDC Event Format:
{
    ...
    "changes": {
        "Revenue": { "old": 5000000, "new": 6000000 },
        "Modified_By": { "old": "user_A", "new": "user_B" }
    },
    "sequence_number": 1742555400123
}
```

**Isolation:** Each tenant's CDC stream is logically isolated. Tenants subscribe only to their own org's events. The CDC extractor filters by org_id at the WAL level, ensuring no cross-tenant event leakage.

**Delivery guarantees:**
- Data events: At-least-once with sequence numbers for consumer deduplication
- Metadata events: Exactly-once via transactional outbox pattern (event written in same DB transaction as metadata change)
- Retention: 7 days in stream; consumers track their own offset per org
- Back-pressure: If consumer falls > 1 hour behind, CDC stream pauses and alerts the org admin
- Filtering: Consumers can subscribe to specific objects/events (e.g., only Account.updated)
