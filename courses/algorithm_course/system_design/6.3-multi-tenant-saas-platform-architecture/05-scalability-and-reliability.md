...
3. **Partition Cutting off unnecessary steps** -- All queries include `org_id`, eliminating 127/128 partitions
...
sequenceDiagram
    ...
    participant SC as Source Cell
    participant TC as Target Cell
    participant TR as Tenant Registry
    participant GW as API Gateway

    Note over CP: Phase 1: Preparation
    CP->>TC: Provision resources for migrating org
    CP->>SC: Begin async data copy (MT_Data, MT_Indexes, metadata)
    SC-->>TC: Streaming data copy (CDC-based)
    CP->>TR: Mark org status = MIGRATING

    Note over CP: Phase 2: Dual-Write
    CP->>GW: Enable write-forwarding for this org
    GW->>SC: Writes go to source (primary)
    SC->>TC: Forward writes to target (secondary)
    CP->>CP: Wait for target to catch up (delta < 100 records)

    Note over CP: Phase 3: Cutover (2-5 seconds)
    CP->>GW: Pause requests for this org (queue briefly)
    SC->>TC: Flush final writes
    CP->>CP: Verify record count match (source == target)
    CP->>TR: Update routing: org → target cell
    CP->>GW: Resume requests (now routed to target)

    Note over CP: Phase 4: Cleanup
    CP->>TC: Warm metadata cache for org
    CP->>SC: Keep source data for 7 days (rollback)
    CP->>SC: Delete source data after grace period
    CP->>TR: Mark org status = ACTIVE
```

### Migration Triggers and Strategies

| Trigger | Migration Type | Downtime | Data Transfer |
|---------|---------------|----------|---------------|
| **Cell capacity exceeded** | Rebalance (move smallest orgs to new cell) | < 5s per org | CDC-based streaming |
| **Enterprise customer onboarding** | Dedicated cell provisioning | Zero (fresh setup) | N/A (new org) |
| **Region change request** | Cross-region migration | < 30s (longer DNS propagation) | Full data copy + dual-write |
| **Hot tenant isolation** | Emergency extraction to dedicated cell | < 10s | Fast CDC with priority |
| **Cell decommission** | Bulk migration of all orgs | Staggered (1 org at a time) | Sequential CDC |
...
```
max_orgs_per_cell = MIN(
    connection_pool_size / avg_connections_per_org,          // Connection bound
    iops_capacity / avg_iops_per_org,                        // I/O bound
    storage_capacity / avg_storage_per_org,                   // Storage bound
    cpu_capacity / avg_cpu_per_org_query                      // CPU bound
)

// Example calculation:
// Connections: 500 pool / 0.5 avg connections = 1,000 orgs
// IOPS: 50,000 IOPS / 25 avg IOPS = 2,000 orgs
// Storage: 10 TB / 5 GB avg = 2,000 orgs
// CPU: 32 cores / 0.02 avg cores = 1,600 orgs
// Slowest part of the process: connections → max 1,000 orgs (add connection pooling proxy to unlock)
```

### Scaling Decision Matrix

| Metric | Scale Up (Vertical) | Scale Out (Horizontal) | Split Cell |
|--------|--------------------|-----------------------|-----------|
| CPU > 70% sustained | Larger instance class | Add app servers | N/A |
| Memory > 85% | Larger instance | Add cache nodes | N/A |
| IOPS > 80% capacity | Higher IOPS storage tier | Add read replicas | Consider |
| Connection pool > 80% | N/A (pooling proxy) | Add proxies | N/A |
| Org count > 80% of max | N/A | N/A | **Yes** |
| Single org > 20% of cell load | N/A | N/A | **Extract to dedicated cell** |
| Storage > 70% capacity | Expand volume | N/A | Consider |

---

## Data Archival and Lifecycle Management

### Record Archival Strategy

Long-lived tenants accumulate billions of records over years. Without archival, query performance degrades and storage costs grow linearly.

```mermaid
flowchart LR
    subgraph Hot["Hot Tier (SSD, Primary DB)"]
        ACTIVE["Active Records<br/>(Last 12 months,<br/>frequently accessed)"]
    end

    subgraph Warm["Warm Tier (HDD, Read Replica)"]
        ARCHIVED["Archived Records<br/>(1-3 years old,<br/>occasionally accessed)"]
    end

    subgraph Cold["Cold Tier (Object Storage)"]
        DEEP["Deep Archive<br/>(3+ years,<br/>compliance retention)"]
    end

    ACTIVE -->|"Auto-archive policy<br/>(age > 12 months, unmodified)"| ARCHIVED
    ARCHIVED -->|"Deep archive policy<br/>(age > 3 years)"| DEEP
    DEEP -->|"Restore request<br/>(async, < 4 hours)"| ARCHIVED
...
| Tier | Storage Type | Access Latency | Cost Ratio | Records |
|------|-------------|---------------|------------|---------|
| **Hot** | SSD on primary database | < 10ms | 1x (baseline) | Active, recent, frequently queried |
| **Warm** | HDD on dedicated read replica | < 100ms | 0.3x | Aged-out, compliance-accessible |
| **Cold** | Object storage (compressed, encrypted) | Minutes-hours (restore required) | 0.05x | Deep compliance archive |

**Archival is per-tenant:** Each org's admin configures archival policies (which objects, age threshold, exemption criteria). The archival engine runs as a background job per cell, processing one org at a time to avoid noisy-neighbor effects.

### Deployment Strategy: Blue-Green Per Cell

```
Cell Deployment Flow:
1. Provision "green" app server fleet (same size as "blue")
2. Deploy new code version to green fleet
3. Run smoke tests against green (synthetic tenant traffic)
4. Shift 10% of cell traffic to green (canary)
5. Monitor for 15 minutes (error rate, latency, governor violations)
6. IF healthy: shift 50% → 100% of traffic
7. IF unhealthy: route 100% back to blue; investigate
8. Drain blue fleet after 30 minutes (allow long-running requests to complete)
9. Terminate blue fleet

Rollback time: < 30 seconds (re-route traffic to blue fleet)
Full deployment time: ~45 minutes per cell
```
