...
- Queries always include `org_id` → partition Cutting off unnecessary steps eliminates 127/128 partitions
...
|------------|------------|---------|--------|
| 400 | `INVALID_FIELD` | Custom field name not found in org's metadata | No |
| 400 | `VALIDATION_FAILED` | Record failed custom validation rule | No (fix data) |
| 401 | `INVALID_TOKEN` | JWT expired or signature invalid | Re-authenticate |
| 403 | `INSUFFICIENT_ACCESS` | User lacks object/field/record permission | No |
| 404 | `NOT_FOUND` | Record or object does not exist for this org | No |
| 409 | `DUPLICATE_VALUE` | Unique field constraint violated | No (fix data) |
| 429 | `GOVERNOR_LIMIT` | Per-transaction governor limit exceeded | No (optimize code) |
| 429 | `RATE_LIMIT` | Per-org API rate limit exceeded | Yes (after Retry-After) |
| 429 | `DAILY_LIMIT` | Per-org daily API call limit reached | Yes (next day) |
| 500 | `INTERNAL_ERROR` | Platform error (query compiler, metadata engine) | Yes (exponential backoff) |
| 503 | `MAINTENANCE` | Cell or org under maintenance | Yes (after Retry-After) |

### Webhook / Outbound Events API

```
# Register a webhook for an object
POST /api/v1/webhooks
Body: {
    "object": "Account",
    "events": ["created", "updated", "deleted"],
    "targetUrl": "https://customer-system.example.com/hooks/account",
    "secret": "whsec_...",
...
# Webhook payload (signed with HMAC-SHA256)
{
    ...
    "data": {
        "record_id": "001xx00000ABC123",
        "changed_fields": ["Revenue"],
        "old_values": { "Revenue": 5000000 },
        "new_values": { "Revenue": 6000000 }
    },
    "signature": "sha256=..."
}
```

**Delivery guarantees:** At-least-once with exponential retry (5 attempts over 24h). Failed deliveries logged in org's event log. Webhook disabled after 7 consecutive days of failure.
...
// Time Complexity (Speed of the algorithm): O(F) where F = number of fields/conditions referenced
// Space Complexity (Memory usage of the algorithm): O(F) for mappings and join clauses
```
...
// Time Complexity (Speed of the algorithm): O(1) per check
// Space Complexity (Memory usage of the algorithm): O(1) per transaction context
```
...
// Time Complexity (Speed of the algorithm): O(1) - fixed-length operations
...
// Time Complexity (Speed of the algorithm): O(1) for routing decision
// Space Complexity (Memory usage of the algorithm): O(T) where T = number of tenants in cache
```

---

### 6. Sharing Rule Evaluation (Record-Level Access)

The sharing model determines which users can access which records, layered on top of the object and field security.

**Step-by-step plan in plain English:**

```
FUNCTION evaluate_record_access(user_id, org_id, record_id, operation):
    // Step 1: Get org-wide default for this object
    owd = metadata_cache.get_owd(org_id, record.obj_id)
    // OWD options: PUBLIC_READ_WRITE, PUBLIC_READ, PRIVATE

    IF owd == PUBLIC_READ_WRITE AND operation IN (READ, UPDATE):
        RETURN GRANTED  // Everyone in the org can read and edit

    IF owd == PUBLIC_READ AND operation == READ:
        RETURN GRANTED  // Everyone can read

    // Step 2: Check ownership
    IF record.owner_id == user_id:
        RETURN GRANTED  // Owner always has full access

    // Step 3: Check role hierarchy (managers see subordinates' records)
    user_role = get_role(user_id)
    owner_role = get_role(record.owner_id)
    IF is_ancestor_role(user_role, owner_role):
        RETURN GRANTED  // Manager role inherits access

    // Step 4: Check sharing rules (criteria-based)
    sharing_rules = metadata_cache.get_sharing_rules(org_id, record.obj_id)
    FOR EACH rule IN sharing_rules:
        IF evaluate_criteria(record, rule.criteria):
            IF user_in_group(user_id, rule.shared_with_group):
                IF operation == READ OR rule.access_level == READ_WRITE:
                    RETURN GRANTED

    // Step 5: Check manual shares (explicit grants)
    manual_shares = query_manual_shares(org_id, record_id)
    FOR EACH share IN manual_shares:
        IF share.shared_with == user_id OR user_in_group(user_id, share.shared_with):
            IF operation == READ OR share.access_level == READ_WRITE:
                RETURN GRANTED

    RETURN DENIED

// Complexity: O(R + S + M) where R = role hierarchy depth,
//             S = sharing rules count, M = manual shares count
```
...
### Provisioning Steps (< 500ms for metadata-only setup)

```
FUNCTION provision_tenant(org_name, edition, region, admin_email):
    // Step 1: Generate org identity (20ms)
    org_id = generate_uuid()
    instance_key = assign_cell(region, edition)  // Cell selection algorithm

    // Step 2: Create org record in tenant registry (50ms)
    INSERT INTO tenant_orgs (org_id, org_name, edition, cell_id, region, ...) VALUES (...)

    // Step 3: Initialize default metadata (200ms)
    // Standard objects: Account, Contact, Lead, Opportunity, Case, Task
    FOR EACH standard_object IN STANDARD_OBJECT_TEMPLATES:
        INSERT INTO MT_Objects (org_id, obj_name, ...) VALUES (org_id, standard_object.name, ...)
        FOR EACH field IN standard_object.fields:
            INSERT INTO MT_Fields (org_id, obj_id, field_name, field_num, ...) VALUES (...)

    // Step 4: Create admin user and default security (100ms)
    admin_user_id = generate_record_id(org_id, USER_OBJ_ID)
    INSERT INTO MT_Data (guid, org_id, obj_id, ...) VALUES (admin_user_id, org_id, USER_OBJ_ID, ...)
    create_default_profiles(org_id)   // System Admin, Standard User, Read Only
    create_default_roles(org_id)      // CEO, VP, Manager, Rep
    set_org_wide_defaults(org_id)     // Account: Private, Contact: Controlled by Parent

    // Step 5: Set governor limits based on edition (10ms)
    apply_edition_limits(org_id, edition)

    // Step 6: Send welcome email with login link (async)
    queue_email(admin_email, WELCOME_TEMPLATE, {org_id, instance_url, temp_password})

    // Step 7: Warm metadata cache for new org (async, background)
    queue_cache_warm(org_id)

    RETURN {org_id, instance_url, admin_user_id}

// Total synchronous time: ~400ms
// Tenant is usable immediately after Step 3 completes
```

### Cell Assignment Algorithm

```
FUNCTION assign_cell(region, edition):
    // Enterprise edition: may qualify for dedicated cell
    IF edition == "ENTERPRISE" AND org_size > DEDICATED_THRESHOLD:
        RETURN provision_dedicated_cell(region)

    // Find eligible cells in the requested region
    candidates = query_cells(region=region, status=ACTIVE)

    // Filter by capacity
    eligible = candidates.filter(cell =>
        cell.org_count < cell.max_orgs * 0.8   AND   // 80% org capacity
        cell.qps < cell.max_qps * 0.7          AND   // 70% QPS capacity
        cell.storage < cell.max_storage * 0.7         // 70% storage capacity
    )

    IF eligible IS EMPTY:
        // No capacity → provision new cell (async, takes minutes)
        RETURN provision_new_cell(region)

    // Select cell with most headroom (load-balanced assignment)
    RETURN eligible.sort_by(headroom_score, DESC).first()
```
