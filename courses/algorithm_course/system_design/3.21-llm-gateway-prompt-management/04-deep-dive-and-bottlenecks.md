...
| Edge Case (Unusual or extreme situation) | Scenario | Handling |
|-----------|----------|----------|
...
| **Agent infinite loop** | Agent A calls Agent B which calls Agent A | Depth-limited call chains, cycle detection via trace ID |
| **Cascading model failure** | Quality evaluator disagrees with all tiers | Return best-effort response with quality warning |
| **Prompt canary regression** | New prompt version degrades metrics | Automatic rollback within minutes, alert team |

---

## Critical Component 5: Model Cascading Engine (2025+)

### The Cost Optimization Problem

Most enterprise workloads have a power-law distribution: 70-85% of requests are simple enough for small models, but organizations default to their most capable (and expensive) model for everything.

```
REQUEST COMPLEXITY DISTRIBUTION (Typical Enterprise)
═══════════════════════════════════════════════════════════════════

Simple (70-85%)         Moderate (10-20%)      Complex (5-10%)
┌─────────────────────┐ ┌──────────────┐       ┌──────────┐
│                     │ │              │       │          │
│  Factual Q&A        │ │  Multi-step  │       │ Analysis │
│  Summarization      │ │  Reasoning   │       │ Creative │
│  Classification     │ │  Code gen    │       │ Strategy │
│  Extraction         │ │  Comparison  │       │ Research │
│                     │ │              │       │          │
│  Cost: $0.25/M tok  │ │  Cost: $3/M  │       │$15/M tok │
└─────────────────────┘ └──────────────┘       └──────────┘
```
...
```python
class ModelCascadeEngine:
    """
    Progressive model escalation for cost optimization.

    Start with cheapest model, escalate only when quality
    checks fail. Achieves up to 87% cost reduction on
    workloads where most queries are straightforward.

    Cascade order:
    1. Tier 1: Small model (fast, cheap)
    2. Tier 2: Medium model (balanced)
    3. Tier 3: Large model (highest quality)
    """

    def __init__(self, tiers: List[ModelTier]):
        self.tiers = sorted(tiers, key=lambda t: t.cost_per_token)
        self.quality_evaluator = QualityEvaluator()

    async def execute(self, request: LLMRequest) -> CascadeResult:
        """Execute request through cascade with quality gates."""
        attempts = []

        for tier in self.tiers:
            response = await tier.model.complete(request)
            attempts.append(CascadeAttempt(
                tier=tier.name,
                response=response,
                cost=calculate_cost(response, tier)
            ))

            # Quality evaluation (must be faster than inference)
            ...
            if quality.passes_threshold:
                return CascadeResult(
                    response=response,
                    resolved_at_tier=tier.name,
                    total_cost=sum(a.cost for a in attempts),
                    attempts=attempts
                )

            # Log escalation for training data
            metrics.increment("cascade.escalation",
                tags={"from_tier": tier.name, "reason": quality.failure_reason})

        # All tiers attempted -- return best effort from highest tier
        return CascadeResult(
            response=attempts[-1].response,
            resolved_at_tier=self.tiers[-1].name,
            total_cost=sum(a.cost for a in attempts),
            attempts=attempts,
...
The quality evaluator must be **faster and cheaper** than the models it evaluates:

| Check Type | Latency | What It Catches |
|-----------|---------|-----------------|
| **Rule-based** (JSON validity, response length, keyword presence) | <1ms | Format violations, empty responses |
| **Confidence scoring** (model's own logprobs) | 0ms (from response metadata) | Low-confidence answers |
| **Tiny classifier** (fine-tuned on gateway's accept/reject data) | 5-10ms | Quality patterns learned from production data |
| **Reference comparison** (when ground truth available) | 1-2ms | Factual correctness for known-answer queries |

### Cascade Cost Analysis

```
COST COMPARISON: FLAT vs CASCADING (1M requests/day)
═══════════════════════════════════════════════════════════════════

FLAT (always use large model):
  1,000,000 × $0.015 avg cost = $15,000/day

CASCADING (87% tier 1, 10% tier 2, 3% tier 3):
  870,000 × $0.001 (Tier 1)  = $870
  100,000 × $0.005 (Tier 2)  = $500
   30,000 × $0.015 (Tier 3)  = $450
  Escalation overhead (~15%)  = $273
  Total                       = $2,093/day

  Savings: $12,907/day = 86% cost reduction
```

---

## Critical Component 6: Content-Addressable Prompt Versioning (2025+)

### The Prompt Deployment Problem

Sequential versioning (v1, v2, v3) creates merge conflicts, makes rollback fragile, and provides no protection against untested prompts reaching production.
...
class PromptVersionManager:
    """
    Content-addressable prompt versioning with staged rollout.

    Key properties:
    - Immutable: same content always produces same ID
    - Deduplication: identical prompts never stored twice
    - Rollback: point to any previous content hash
    - Canary: traffic splitting between versions
    """

    def compute_version_id(self, template: PromptTemplate) -> str:
        """
        Generate content-addressable ID from prompt content.

        Uses SHA256 of normalized content, producing deterministic
        IDs regardless of whitespace or metadata differences.
        """
        normalized = normalize_whitespace(template.content)
        content_bytes = f"{template.model_family}:{normalized}".encode()
        return f"pv_{hashlib.sha256(content_bytes).hexdigest()[:16]}"
    ...
    async def deploy_canary(
        ...
        """
        Deploy new prompt version as canary with automated rollback.

        Traffic split: (100 - canary_percentage)% to stable,
        canary_percentage% to canary. If canary's error rate
        or quality degradation exceeds threshold, auto-rollback.
        """
        deployment = DeploymentConfig(
            prompt_id=prompt_id,
            splits=[
                TrafficSplit(version_hash=stable_hash,
                           weight=100 - canary_percentage),
                TrafficSplit(version_hash=canary_hash,
                           weight=canary_percentage)
            ],
            rollback_config=RollbackConfig(
                quality_threshold=auto_rollback_threshold,
                min_sample_size=100,
                evaluation_window_minutes=30
            )
        )

        await self.config_store.save(deployment)
        return deployment
```
...
Author → Commit → Test → Canary → Promote → Monitor
  │        │        │       │         │         │
  │        │        │       │         │         └─ Quality drift
  │        │        │       │         │            detection
  │        │        │       │         └─ 100% traffic
