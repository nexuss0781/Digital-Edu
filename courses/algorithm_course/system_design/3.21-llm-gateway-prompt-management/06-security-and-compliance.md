...
## Dual-Layer Guardrails Architecture (2025+)

Modern LLM gateways implement two independent guard layers -- one before inference, one after -- each with different threat models and detection strategies:
...
INPUT GUARDS (Pre-Inference)                OUTPUT GUARDS (Post-Inference)
┌─────────────────────────────┐            ┌──────────────────────────────┐
│ 1. Prompt Injection Detection│            │ 1. Hallucination Detection   │
│    - Instruction override    │            │    - Factual grounding check │
│    - Jailbreak patterns      │            │    - Source attribution       │
│    - Indirect injection      │            │                              │
│                              │            │ 2. Toxicity/Safety Filter    │
│ 2. PII Detection & Masking  │            │    - Content policy          │
│    - SSN, credit card, email │            │    - Harmful content         │
│    - Custom entity patterns  │            │    - Bias detection          │
│    - Mask before LLM sees it │            │                              │
│                              │            │ 3. PII Leakage Prevention    │
│ 3. Topic Restriction         │            │    - Scan output for PII     │
│    - Off-topic detection     │            │    - Training data leakage   │
│    - Forbidden topic list    │            │                              │
│                              │            │ 4. Format Compliance         │
│ 4. Cost Estimation           │            │    - JSON schema validation  │
│    - Token count check       │            │    - Response length limits  │
│    - Budget pre-validation   │            │    - Language detection       │
└─────────────────────────────┘            └──────────────────────────────┘
       │ Block: 2-5% of requests                  │ Block: 0.5-1% of responses
       │ Latency: 5-15ms                          │ Latency: 10-30ms
```

### LLM-as-a-Judge for Output Evaluation

For cases where rule-based output guards are insufficient, a small evaluation LLM can assess response quality:

| Evaluation Dimension | Method | Latency | When to Use |
|---------------------|--------|---------|-------------|
| **Factual accuracy** | Ground-truth comparison | <5ms | Knowledge-base Q&A |
| **Instruction following** | Regex + Practical rule of thumb | <1ms | Structured output tasks |
| **Safety/toxicity** | Classifier model | 10-20ms | User-facing applications |
| **Hallucination risk** | Small LLM judge | 50-100ms | High-stakes domains (medical, legal) |
| **Relevance** | Embedding similarity to query | 5-10ms | Open-ended conversations |

**Trade-off:** LLM-as-a-Judge adds 50-100ms latency but catches semantic violations (e.g., medical misinformation) that pattern-matching cannot detect. Use selectively for high-stakes domains.

### Agent-Specific Security (2025+)

Multi-agent workflows introduce new attack surfaces:

| Threat | Description | Mitigation |
|--------|-------------|------------|
| **Confused deputy** | Agent tricked into using tools beyond its authorization | Per-agent tool allowlists, least-privilege MCP scopes |
| **Prompt injection via tools** | Malicious content in tool responses manipulates agent behavior | Sanitize tool outputs before injecting into agent context |
| **Cross-agent data leakage** | Agent A's sensitive context accessible to Agent B | Session isolation, encrypted inter-agent channels |
| **Recursive escalation** | Agent requests increasingly powerful models/tools | Call depth limits, cost ceiling per workflow |
| **Shadow API keys** | Agents create or cache API keys without governance | All provider access must flow through gateway virtual keys |
