...
### 2025-2026 Features (P2/P3 - Emerging)

| Category | Requirement | Description |
|----------|------------|-------------|
| **Agent Support** | MCP protocol bridging | Translate Model Context Protocol requests for tool routing |
| **Agent Support** | A2A protocol support | Route Agent-to-Agent communication through gateway |
| **Agent Support** | Multi-agent cost attribution | Track costs across multi-agent workflows where one user request triggers many LLM calls |
| **Smart Routing** | Semantic router | BERT-based task classification routes to optimal model by complexity |
| **Smart Routing** | Model cascading | Start with cheapest model, escalate on quality failure -- up to 87% cost reduction |
| **Prompt Management** | Content-addressable versioning | Hash-based prompt IDs for deterministic rollback |
| **Prompt Management** | Canary deployment | Traffic splitting between prompt versions with automated quality comparison |
| **Guardrails** | Dual-layer guards | Separate input guards (prompt injection, PII) and output guards (hallucination, toxicity) |
| **Guardrails** | LLM-as-a-Judge | Use small LLM to evaluate output quality before returning to client |
| **Observability** | LLM-specific spans | OpenTelemetry GenAI semantic conventions for LLM tracing |
