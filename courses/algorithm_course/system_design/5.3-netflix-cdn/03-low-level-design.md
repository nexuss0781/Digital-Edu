...
```
Step-by-step plan in plain English: NGINX Content Serving Flow
...
```
Step-by-step plan in plain English: BGP Session Configuration
...
```
Step-by-step plan in plain English: OCA Selection Algorithm
...
```
Step-by-step plan in plain English: File-Level Popularity Prediction
...
```
Step-by-step plan in plain English: Zero-Copy Data Transfer
...
```
Step-by-step plan in plain English: Kernel TLS with NIC Offload
...
### QUIC Advantages for Video Delivery

```
Traditional TCP (HTTP/2):
  - Head-of-line blocking: one lost packet stalls ALL multiplexed streams
  - Connection setup: TCP handshake + TLS handshake = 2-3 RTTs
  - Migration: IP change = new connection (mobile users)

QUIC (HTTP/3):
  - Independent streams: packet loss affects only the impacted stream
  - 0-RTT resumption: reconnect without handshake (repeat visitors)
  - Connection migration: session survives IP/network changes
  - Built-in TLS 1.3: no separate handshake

Impact for Netflix:
  - Reduced rebuffer on lossy mobile networks (independent streams)
  - Faster playback start for returning users (0-RTT)
  - Seamless wifi-to-cellular handoff during mobile viewing
  - kTLS offload for QUIC being developed for FreeBSD kernel
```

### OCA Protocol Stack Evolution

```
Current Stack:
  FreeBSD → kTLS → NGINX → HTTP/1.1 + HTTP/2 → sendfile()
  TLS 1.3 with hardware offload (AES-NI)

Emerging Stack:
  FreeBSD → kernel QUIC → NGINX-QUIC → HTTP/3 → zero-copy
  0-RTT session resumption, connection migration support
  Challenge: QUIC uses UDP, so kTLS offload path differs from TCP

800 Gbps Prototype:
  - 18x NVMe 2TB drives (432 Gbps aggregate disk I/O)
  - 8x 100GbE NICs
  - AMD EPYC Genoa (96 cores, PCIe Gen5)
  - Network becomes sole Slowest part of the process (disk > network)
  - In testing for high-demand metro areas
```

---

## Time/Space Complexity (Memory usage of the algorithm)
