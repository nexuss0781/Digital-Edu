...
```
Step-by-step plan in plain English: Proximity Rank Computation
...
```
Step-by-step plan in plain English: Cache Miss Classification
...
### Slowest part of the process 1: Fill Window Constraints

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    FILL WINDOW Slowest part of the process ANALYSIS                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
### Slowest part of the process 2: NVMe I/O Limits

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      NVMe I/O Slowest part of the process ANALYSIS                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
│  • Disk is the Slowest part of the process, not network                                       │
│                                                                              │
...
│  • Network becomes Slowest part of the process (good - drives have headroom)                 │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Slowest part of the process 3: BGP Convergence Time

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    BGP CONVERGENCE Slowest part of the process ANALYSIS                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
### Scenario 4: Live Event at Scale (65M Concurrent Viewers)

```
CHALLENGE: Live events (boxing, NFL, WWE RAW) cannot use proactive caching
           because content does not exist until broadcast time

ARCHITECTURAL SHIFT:
- VOD caching model inverted: content generated in real-time
- Origin servers produce live segments (2-6 second chunks)
- OCAs pull segments rather than receiving nightly fills
- Segments expire rapidly (minutes, not months)

LIVE CDN OVERLAY:
1. Live encoder produces multi-bitrate segments at origin
2. IXP Storage OCAs pull segments from origin (near real-time)
3. Edge OCAs pull from IXP Storage (adds 2-4 seconds latency)
4. Clients request segments as they are produced
5. Total glass-to-glass latency target: < 10 seconds

SCALE CHALLENGE (65M concurrent):
- Each viewer requests segment every 2-6 seconds
- At 65M concurrent: 10-30M segment requests/second globally
- Single origin cannot serve all OCAs directly
- Solution: tiered pull-through with IXP Storage as first-level fan-out

MITIGATION:
- Pre-warm OCA clusters in expected high-demand regions
- Dynamic ABR shifts to lower bitrate under congestion
- Different steering weights for live vs VOD traffic
- Dedicated live origin servers separate from VOD infrastructure
```
