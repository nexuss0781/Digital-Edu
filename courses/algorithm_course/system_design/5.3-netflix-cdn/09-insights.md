...
## Insight 9: NVMe I/O as the True Slowest part of the process, Not Network
...
**Why it matters:** System architects often assume network is the Slowest part of the process for content delivery, but Netflix's analysis shows that NVMe sequential read throughput limits serving capacity. Optimizations like zero-copy sendfile(), hot content in page cache, sequential read patterns, and NUMA-aware memory allocation all target disk I/O efficiency. The latest 400 Gbps OCA flips this relationship with 18 NVMe drives (432 Gbps aggregate), making the 4x100 GbE network the new Slowest part of the process -- a desirable state since it means drives have headroom for page cache misses and concurrent fill operations.
