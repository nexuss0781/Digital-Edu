...
|-----------|--------|------------------------------|
| **Architecture** | 25% | Two-tier L4/L7 with clear justification; Anycast for HA |
| **Algorithm Depth** | 20% | Maglev internals, P2C math, WLC for heterogeneous pools |
| **Reliability** | 20% | LB HA before backend HA; health check state machine; connection draining |
| **Scaling** | 15% | DSR for bandwidth; kernel bypass for throughput; zone-aware routing |
| **Trade-offs** | 10% | Clear reasoning for L4 vs L7, active vs passive HC, consistent hash vs LC |
| **Production Awareness** | 10% | TLS cost, HTTP/2 impact, deployment drain, observability |

### Score Levels

| Level | Score | Characteristics |
|-------|-------|-----------------|
| **Expert** | 9-10 | Explains Maglev permutation tables, quantifies TLS CPU, discusses DSR bandwidth savings with numbers, identifies HTTP/2 breaking L4 LB |
| **Strong** | 7-8 | Two-tier architecture, consistent hashing (not just ring), health check tuning, connection draining, mentions Anycast |
| **Competent** | 5-6 | L4 vs L7 distinction, round-robin + least connections, basic health checks, mentions HA |
| **Developing** | 3-4 | Single-tier LB, one algorithm, no HA discussion, missing health checks |
| **Insufficient** | 1-2 | Confuses LB with reverse proxy, no algorithm discussion, ignores failure modes |

### Red Flags (Immediate Score Reduction)

| Red Flag | Why It Matters |
|----------|----------------|
| No mention of LB availability | Shows candidate doesn't understand blast radius |
| Only knows round-robin | Insufficient algorithm knowledge for distributed systems |
| Deep health checks for routing | Would cause cascade failures in production |
| No TLS discussion at L7 | Ignores dominant CPU cost at scale |
| "Just add more servers" for 100x | Missing architectural thinking (DSR, kernel bypass, CDN offload) |

### Follow-Up Depth Probes

| Probe Question | What It Tests |
|----------------|---------------|
| "Walk me through what happens when a backend takes 30 seconds to respond" | Cascading failure understanding, circuit breakers, timeouts |
| "How does your consistent hashing handle a backend that keeps flapping?" | Debounce logic, dampening, operational maturity |
| "Your L4 LB is at 90% CPU -- what do you investigate first?" | Diagnostic thinking: connection table? health checks? DDoS? |
| "A client reports intermittent 502 errors but your backends are healthy" | Connection draining gaps, race conditions, retry storms |
