...
├── Slowest part of the process: NIC capacity, connection table
...
├── Slowest part of the process: CPU for TLS/HTTP processing
...
In multi-zone deployments, cross-zone traffic incurs latency (1-2ms per zone hop) and bandwidth costs. Zone-aware load balancing prefers local backends, falling back to remote zones only when the local pool is insufficient.

### Zone Preference Algorithm

```
FUNCTION select_backend_zone_aware(request, local_zone, all_zones):
    local_backends = get_healthy_backends(local_zone)
    local_utilization = avg_utilization(local_backends)

    // Prefer local zone if it has capacity
    IF local_utilization < ZONE_OVERFLOW_THRESHOLD:  // typically 80%
        RETURN select_backend(local_backends, algorithm)

    // Local zone overloaded -- spill to other zones proportionally
    all_healthy = get_all_healthy_backends(all_zones)
    RETURN select_backend(all_healthy, algorithm)
```

### Zone Failover Matrix

| Local Zone Health | Action | Latency Impact |
|-------------------|--------|----------------|
| > 80% backends healthy | Route 100% locally | None |
| 50-80% healthy | Route locally, monitor | Slight increase |
| 20-50% healthy | Spill 50% to nearest zone | +1-2ms |
| < 20% healthy | Full failover to other zones | +2-5ms |
| 0% healthy | All traffic to remote zones | +2-5ms |

### Subsetting for Large Backend Pools

When a service has thousands of backends, each LB node maintaining connections to all backends creates O(LB × Backend) connections. Subsetting limits each LB to a fixed subset:

```
FUNCTION compute_subset(lb_id, backends, subset_size):
    // Deterministic subset based on LB identity
    // Ensures different LBs get different subsets for coverage
    shuffled = deterministic_shuffle(backends, seed=lb_id)
    RETURN shuffled[0 : subset_size]

Example:
  1000 backends, 10 LB nodes, subset_size = 200
  Each LB connects to 200 backends (20% of pool)
  Total connections: 10 × 200 = 2,000 (vs 10 × 1000 = 10,000 without subsetting)
  Coverage: Every backend in at least 2 subsets (statistically)
```

**Trade-off:** Subsetting reduces connection count but can create load imbalance if subsets overlap unevenly. Google's approach uses deterministic subsetting with periodic rotation to maintain balance.
