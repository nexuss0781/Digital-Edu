...
|---|---|---|
| [1.1 Distributed Cache](../1.1-distributed-cache/) | Consistent hashing distributes cache keys the same way LB distributes connections | Maglev/ring hashing, minimal disruption on membership change |
| [2.1 API Gateway](../2.1-api-gateway/) | L7 LB functions overlap with API gateway routing, TLS, rate limiting | Content-based routing, TLS termination, request transformation |
| [5.6 Video Streaming Platform](../5.6-video-streaming-platform/) | DSR pattern is critical for asymmetric streaming traffic (small request, large response) | Direct Server Return, bandwidth optimization |
| [12.5 CDN Design](../12.5-cdn-design/) | CDN PoPs use LB concepts for origin selection and edge cache routing | Anycast, geo-routing, health-based failover |
| [12.8 WebRTC Infrastructure](../12.8-webrtc-infrastructure/) | SFU node selection mirrors LB algorithms with capacity-aware assignment | Weighted assignment, region-affinity routing |
| [15.1 Distributed Tracing](../15.1-distributed-tracing/) | Trace propagation through LB requires header injection and span creation | Request correlation, latency breakdown attribution |
| [1.5 Service Mesh](../1.5-service-mesh/) | Service mesh distributes LB logic to sidecar proxies at application layer | Per-service routing, mTLS, circuit breaking |
