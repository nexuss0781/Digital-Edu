...
### 6. DSR vs Full Proxy vs Tunnel Mode

**Three fundamental modes for how the LB forwards traffic to backends:**

```
Mode 1: Full Proxy (L7 default)
  Client → [LB terminates TCP + TLS] → [New TCP to backend] → Backend
  LB sees: Full request and response
  Overhead: Two TCP connections per request
  Use: Content routing, request modification, response caching

Mode 2: Direct Server Return (DSR)
  Client → [LB rewrites MAC, forwards packet] → Backend → Client (directly)
  LB sees: Only inbound traffic (requests)
  Overhead: Minimal (MAC rewrite only)
  Use: High-bandwidth asymmetric workloads (video, large downloads)
  Constraint: Backend must have VIP configured on loopback

Mode 3: IP Tunneling (IPIP/GRE)
  Client → [LB encapsulates in tunnel] → Backend → Client (directly)
  LB sees: Only inbound traffic
  Overhead: Encapsulation header (~20 bytes)
  Use: Cross-subnet DSR (when LB and backend are in different networks)
  Constraint: Backend must support decapsulation
```

**DSR Bandwidth Savings (Quantified):**

```
Typical API workload:
  Request: 2 KB average
  Response: 20 KB average
  Ratio: 1:10 (asymmetric)

Full Proxy LB bandwidth: request + response = 22 KB per request
DSR LB bandwidth: request only = 2 KB per request

Bandwidth reduction: 10x on the LB
At 290K QPS: Full Proxy = 6.4 Gbps, DSR = 580 Mbps on LB path
```

### 7. gRPC and HTTP/2 Load Balancing

```
Challenge: HTTP/2 multiplexes many requests on one TCP connection.
  L4 LB sees: One connection → routes ALL requests to ONE backend
  Result: Severe load imbalance (one backend gets all requests)

Solution: L7 LB that understands HTTP/2 framing
  L7 LB demultiplexes streams
  Each stream can route to a different backend
  Enables per-request load balancing on multiplexed connections

gRPC-specific considerations:
  - gRPC uses HTTP/2 exclusively → L7 mandatory for proper balancing
  - Long-lived gRPC streams → least-connections naturally adapts
  - gRPC health checking protocol (grpc.health.v1.Health) → native support
  - Header-based routing: route by gRPC service name and method
```
