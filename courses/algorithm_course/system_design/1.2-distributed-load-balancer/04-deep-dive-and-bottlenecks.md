...
## Critical Component #4: Load Balancing Algorithm Selection Engine

### Why This Is Critical

The algorithm selection engine determines how requests are distributed across backends. The wrong algorithm for a given workload creates hot spots, wastes capacity, or breaks session affinity. Production systems often need multiple algorithms simultaneously -- different services behind the same LB may require different distribution strategies.

### Power of Two Choices (P2C) Deep Dive

P2C achieves near-optimal load distribution with O(1) state by exploiting a mathematical insight: choosing the better of two random options is exponentially better than choosing one at random.

```
FUNCTION select_backend_p2c(backends, load_metric):
    // Pick two backends uniformly at random
    idx_a = random(0, LEN(backends) - 1)
    idx_b = random(0, LEN(backends) - 1)
    ...
    // Compare load and pick the lighter one
    IF load_metric(a) <= load_metric(b):
        RETURN a
    ELSE:
        RETURN b
```
...
```
Single random choice:  Max load = O(log N / log log N)
Power of two choices:  Max load = O(log log N)

For N = 1000 backends:
  Random:     Max load ≈ 4.5x average (some backends heavily loaded)
  P2C:        Max load ≈ 1.6x average (near-uniform)
  Least-Conn: Max load ≈ 1.0x average (optimal, but O(N) per decision)
```
...
| Metric | Pros | Cons |
|--------|------|------|
| Active connections | Simple, real-time | Doesn't account for request cost |
| CPU utilization | Capacity-aware | Requires backend reporting, stale data |
| Response latency (EWMA) | Quality-aware | Lag in detection, noisy signal |
| Weighted composite | Comprehensive | Complex calibration |

### Weighted Least Connections (WLC) Internals

WLC accounts for heterogeneous backends by normalizing connection count by weight:

```
FUNCTION select_backend_wlc(backends):
    best = NULL
    best_score = INFINITY
    ...
    FOR EACH backend IN backends:
        ...
        score = backend.active_connections / backend.weight
        IF score < best_score:
            best_score = score
            best = backend
...
Example:
  Backend A: weight=10, connections=50 → score = 5.0
  Backend B: weight=5,  connections=20 → score = 4.0 ← selected
  Backend C: weight=10, connections=60 → score = 6.0

  Despite B having fewer absolute connections, score normalizes by capacity
```

### Algorithm Hot-Swap

Production LBs must change algorithms without dropping connections:

```
FUNCTION hot_swap_algorithm(new_algorithm):
    // Existing connections continue using current routing
    // Only NEW connections use the new algorithm
    old_algorithm = atomic_load(current_algorithm)
    atomic_store(current_algorithm, new_algorithm)

    // Log the transition for debugging
    log("Algorithm changed from {} to {}", old_algorithm, new_algorithm)

    // Existing connections drain naturally via keep-alive timeout
    // No active migration needed -- connections complete on old routing
```
...
| Failure | Impact | Mitigation |
|---------|--------|------------|
| Load metric stale | P2C picks loaded backend | Use EWMA with short half-life (10s) |
| All backends equal load | P2C degenerates to random | Acceptable -- uniform is the goal |
| Weight misconfiguration | Traffic skew | Validate weights sum > 0, alert on extremes |
| Algorithm mismatch | Session breaks after swap | Use consistent hashing for stateful services |

---

## Slowest part of the process Analysis

### Slowest part of the process #1: TLS Termination CPU

**Problem:** TLS handshakes are CPU-intensive. At high scale, TLS termination becomes the Slowest part of the process.
...
### Slowest part of the process #2: Single Point of Failure at VIP
...
### Slowest part of the process #3: Connection Table Memory
...
### Slowest part of the process #4: Health Check Scalability
