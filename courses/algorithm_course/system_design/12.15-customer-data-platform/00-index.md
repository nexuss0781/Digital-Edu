...
### Privacy as an Architectural Rule that never changes

Unlike most platforms where privacy is a compliance add-on, CDPs must treat consent as an architectural Rule that never changes that flows through every data processing path. Consent state must be checked at event ingestion (is this source permitted to collect this event type?), at profile enrichment (is this trait derivable for this purpose?), at segment evaluation (is this user consented for this segment's purpose?), and at destination delivery (does the destination's declared purpose match the user's consent?). Erasure requests must cascade through a pipeline that identifies and deletes all copies of a user's data across dozens of storage systems — including immutable append-only event logs — within legally mandated timeframes, with cryptographic proof of deletion for auditors.

---

## What Makes This System Architecturally Unique

1. **The profile is a derived view, not a primary store.** The append-only event log is the source of truth; the unified profile is a materialized view that can always be rebuilt by replaying events. This inversion means disaster recovery is "restore checkpoint + replay," not "restore N independent components and hope they converge."

2. **Consent is enforced at three independent checkpoints.** Unlike systems where access control is checked once at the gateway, a CDP enforces consent at ingest (should this event be collected?), at enrichment (should this trait be computed for this purpose?), and at delivery (does the user consent for this destination's purpose?). Any single enforcement point can fail; three-point enforcement makes consent violations require simultaneous failures.

3. **The fan-out multiplier dominates infrastructure cost.** Ingest is the visible entry point, but delivery is where resources are consumed. With an average 5× fan-out per event (and up to 500× for large workspaces), the delivery tier requires 10–50× the infrastructure footprint of the ingest tier. Most CDP designs underinvest in delivery because the math is counterintuitive.

4. **Identity resolution is a consensus problem, not a matching problem.** When two concurrent event streams carry overlapping identifiers pointing to different profiles, the merge operation must be serialized. This is not pattern matching — it is distributed consensus over a mutable shared graph, and treating it otherwise produces identity data corruption at scale.

5. **The computed trait dependency DAG creates hidden cascades.** A single event can trigger a chain reaction through dependent computed traits, each of which may change audience memberships, each of which fans out to multiple destinations. Understanding and controlling this amplification is essential for capacity planning.
...
|---|---|---|
| **V1: Event Pipe** | Basic event collection and forwarding | SDK → edge → queue → destination; no identity resolution; no profiles; one-to-one event forwarding |
| **V2: Identity + Profile** | Customer recognition | Deterministic identity matching; unified profile with traits; schema validation; basic batch segmentation |
| **V3: Real-Time Audiences** | Streaming segment evaluation | Dual-path segment evaluation; inverted segment index; streaming membership updates; computed traits |
| **V4: Privacy-Native** | Regulatory compliance as architecture | Purpose-based consent enforcement; crypto-shredding erasure; audit trail; data classification; data residency |
| **V5: Composable / Warehouse-Native** | Customer data ownership | Warehouse sync; reverse ETL; composable activation layer; customer-controlled storage |
| **V6: AI-Augmented** | Intelligent automation | ML-powered identity resolution confidence; automated audience discovery; predictive traits; anomaly detection on identity graph |
