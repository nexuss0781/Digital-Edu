...
|-----|--------|---------------|-----------------|
| Event ingestion success rate | ≥ 99.95% | ~21.6M dropped events/month (at 43B/day avg) | >0.1% drop rate in 1h → page |
| Event-to-profile propagation p99 | < 500ms | 43.8 min/month of violations | >1% violations in 15 min → alert |
| Profile lookup API p99 | < 50ms | 43.8 min/month of violations (99.9%) | >0.5% violations in 5 min → alert |
| Streaming segment update p99 | < 1,000ms | 43.8 min/month of violations | >1% violations in 15 min → alert |
| Destination delivery success rate | ≥ 99.9% | ~215M failed deliveries/month (at 215B/day) | >0.5% failure rate in 1h → page |
| Platform availability (ingest) | ≥ 99.99% | 4.32 min/month | Any downtime > 1 min → page |
| Erasure completion | 100% within 30 days | Zero tolerance | Any overdue request → P0 |

### Error Budget Policy

| Budget Status | Remaining | Action |
|---------------|-----------|--------|
| **Green** | >75% | Normal operations; all feature work proceeds |
| **Yellow** | 50–75% | Review burn rate; schedule reliability improvements |
| **Orange** | 25–50% | Freeze non-critical feature deploys; focus on reliability |
| **Red** | <25% | Freeze all deploys except reliability fixes; incident review |
| **Exhausted** | 0% | All deploys frozen; postmortem required; executive review |

### Budget Attribution

| Category | Description | Example |
|----------|-------------|---------|
| **Planned** | Maintenance windows, upgrades | Profile store shard rebalancing |
| **Infrastructure** | Cloud provider issues, network | AZ connectivity flap |
| **Software** | Bugs in CDP code | Identity resolution regression |
| **Load** | Unexpected traffic spike | Viral campaign causing 5× ingest burst |
| **External** | Destination outages, client SDK bugs | Destination API returning 500s |

---

## Latency Budget Breakdown

### Event-to-Profile Propagation Path (target: < 500ms p99)

| Step | p50 | p99 | Budget Share |
|------|-----|-----|-------------|
| Edge collector → ingest queue | 5ms | 20ms | 4% |
| Queue → pipeline worker dequeue | 10ms | 50ms | 10% |
| Schema validation | 2ms | 10ms | 2% |
| Identity resolution (graph lookup + lock) | 20ms | 100ms | 20% |
| Profile read-modify-write | 15ms | 80ms | 16% |
| Segment evaluation dispatch | 5ms | 30ms | 6% |
| **Total** | **~57ms** | **~290ms** | — |
| **Headroom for retries + contention** | — | 210ms | 42% |

### Profile Lookup API Path (target: < 50ms p99)

| Step | p50 | p99 | Budget Share |
|------|-----|-----|-------------|
| API gateway + auth | 2ms | 5ms | 10% |
| Profile store read | 3ms | 15ms | 30% |
| Audience membership hydration | 2ms | 10ms | 20% |
| Response serialization | 1ms | 5ms | 10% |
| **Total** | **~8ms** | **~35ms** | — |
| **Headroom** | — | 15ms | 30% |
...
|--------|--------|--------|--------|---------------|
| Profiles | 100M | 500M | 1B | New workspace onboarding + organic user growth |
| Events/day (avg) | 5B | 20B | 43B | More event sources + richer instrumentation |
| Active destinations | 5,000 | 20,000 | 50,000 | Workspace growth + destination catalog expansion |
| Segment definitions | 500K | 2M | 5M | Self-serve audience building adoption |
| Computed traits/workspace | 50 | 200 | 500 | Increased data-driven decisioning |
| Identity graph nodes | 500M | 2.5B | 5B | Profile growth × avg identifiers per profile |

### Cost Scaling Drivers

| Driver | Scaling Behavior | Mitigation |
|--------|-----------------|------------|
| Raw event storage | Linear with event volume; dominates at PB scale | Aggressive compression (3×); tiered storage (hot→warm→cold) |
| Fan-out delivery | Superlinear (events × destinations) | Batch coalescing; conditional fan-out (only deliver on membership change) |
| Identity graph | Sublinear (most operations are local to small clusters) | Cluster-level sharding; prune inactive identifiers |
| CEP evaluation | Linear with events × active streaming segments | Inverted index; compile-time segment routing |
| Profile store I/O | Proportional to event-triggered profile updates | Write batching; CRDT merge to reduce read-modify-write cycles |
