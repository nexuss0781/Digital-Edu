...
4. **Garbage collection complexity** - Cutting off unnecessary steps tombstones requires coordination.
...
### Cell-Based Architecture Extension

**Scenario:** "Your 3-region system has a bad deployment that takes down US-East. How do you reduce the blast radius?"

**Strong Answer:** "Cell-based architecture — subdivide each region into independent cells, each serving 1-5% of users. A bad deployment rolls out cell-by-cell with automated rollback. A cell failure affects only its users, not the entire region. Slack uses this pattern: their worst-case incident affects ~2% of users. Cross-cell replication within a region uses synchronous writes; cross-region replication between corresponding cells uses async. The trade-off is operational complexity — you're managing 3 regions × N cells instead of 3 regions."

### Edge Databases and Tunable Consistency

**Scenario:** "How would you extend this to serve users from 50+ edge locations?"

**Strong Answer:** "Edge databases like Turso (libSQL), Cloudflare D1, and Durable Objects enable read replicas at the edge with configurable consistency. Reads serve from the nearest edge location with bounded staleness (e.g., 'data no older than 5 seconds'). Writes still route to the owning region but use optimistic responses with local queueing. The tunable consistency model extends naturally: edge reads use LOCAL_ONE, regional writes use LOCAL_QUORUM, critical operations upgrade to GLOBAL_QUORUM. The key insight is that tunable consistency per request replaces the false binary of 'eventual' vs 'strong' systems."

### Data Sovereignty as Architecture Driver

**Scenario:** "How do you handle India's DPDP Act requiring certain data to stay in India, while still providing active-active globally?"

**Strong Answer:** "Split each record into a PII envelope (stays in India cell) and a non-PII operational payload (globally replicated). The PII envelope is referenced by opaque token. Cross-region services fetch non-PII locally and make targeted cross-region calls only for PII. This 'data residency cell' pattern treats each jurisdiction as a cell with restricted replication scope — some data is global, some is jurisdictional, and the boundary is at the field level, not the record level."
