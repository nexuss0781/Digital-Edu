...
| **CCPA/CPRA** | California users | Data access, deletion, opt-out, sensitive data |
| **LGPD** | Brazil user data | Data localization, DPO requirement, consent |
| **DPDP Act** | India user data | Data processing consent, cross-border transfer rules |
| **PDPL** | Saudi Arabia | Data classification, localization for sensitive data |
...
### Data Sovereignty Cells (2025-2026 Pattern)

The proliferation of data localization laws (GDPR, LGPD, DPDP Act, PDPL) has driven the emergence of **data sovereignty cells** — jurisdictional boundaries within the active-active architecture where PII replication is constrained by law.

| Jurisdictional Zone | Data That Must Stay | Data That Can Replicate Globally |
|---|---|---|
| **EU Cell** | Names, emails, addresses, health data, biometrics | Anonymized analytics, preferences (non-PII), counters |
| **India Cell** | Personal data classified as "significant" or "critical" | Aggregated metrics, de-identified behavioral data |
| **Brazil Cell** | PII as defined by LGPD (broad scope) | Non-personal operational data |
| **Global Cell** | None — only non-PII by design | All non-PII data, globally replicated |

Implementation: Each record is split into a PII envelope (stays in jurisdictional cell) and a non-PII operational payload (globally replicated with standard CRDTs). The PII envelope is referenced by opaque token — cross-region services fetch non-PII locally and make targeted cross-region calls only for the PII portion.
