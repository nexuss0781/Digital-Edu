...
### Decision 6: Cell-Based vs Region-Based Blast Radius

| Option | Blast Radius | Complexity | User Impact on Failure |
|--------|-------------|------------|----------------------|
| **Region-level** | Entire region (33% of users) | Lower | All users in region affected |
| **Cell-based** | Single cell (1-5% of users) | Higher | Only cell's users affected |
| **Hybrid (cells within regions)** | Cell within region | Medium-High | Contained, with regional fallback |

**Recommendation: Hybrid (Cells Within Regions)**

Rationale: Each region is subdivided into cells — self-contained deployment units with dedicated compute, data partitions, and dependencies. Users are assigned to cells via consistent hashing of user ID. A cell failure affects only its assigned users (typically 1-5%), while region-level routing continues to work for cross-region failover. This pattern, adopted by Slack and Shopify, combines the latency benefits of regional proximity with the blast radius containment of cell isolation. Cell-to-cell replication within a region uses synchronous writes; cross-region replication between corresponding cells uses the standard async pipeline.
...
| **Conflict Resolution** | CRDT Library + Custom | Automerge 2.0, Loro, Yjs, Custom |
