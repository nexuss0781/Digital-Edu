...
| [04 - Deep Dive & Bottlenecks](./04-deep-dive-and-bottlenecks.md) | CRDT engine, replication transport, Slowest part of the process analysis |
...
|---|---|
| [Distributed Consensus (2.7)](../2.7-distributed-consensus/00-index.md) | Active-active deliberately avoids consensus for writes (AP over CP); consensus protocols like Raft/Paxos are used within regions for local quorum but not across regions for normal operations |
| [Distributed Hash Table (1.9)](../1.9-distributed-hash-table/00-index.md) | Consistent hashing from DHT designs underpins per-region data partitioning; DHT ring membership protocols inform node failure detection |
| [Container Orchestration (2.2)](../2.2-container-orchestration-system/00-index.md) | Multi-region container orchestration provides the deployment substrate; federation patterns mirror active-active regional independence |
| [Smart Home Platform (10.3)](../10.3-smart-home-platform/00-index.md) | Device shadow/digital twin patterns use eventual consistency and CRDT-like merge semantics similar to active-active conflict resolution |
| [Figma (6.10)](../6.10-figma/00-index.md) | Real-time collaborative editing uses CRDTs (Automerge/Yjs) for conflict-free document merging — same mathematical foundation applied to user-facing collaboration |
| [WebRTC Collaborative Canvas (6.11)](../6.11-webrtc-collaborative-canvas/00-index.md) | Multi-user canvas synchronization faces identical concurrent-edit challenges; operational transform vs CRDT is the canvas equivalent of LWW vs CRDT in active-active |
| [Distributed Caching (1.7)](../1.7-distributed-caching/00-index.md) | Cache invalidation across regions is a subset of the replication consistency problem; cache-aside patterns must account for replication lag |

> **Cell-based architecture trend (2025-2026):** The industry is converging on cell-based architecture as the dominant pattern for blast radius containment in multi-region systems. Each cell is a fully independent deployment unit (compute + data + dependencies) serving a fixed subset of users, with cross-cell replication for disaster recovery only. This pattern — adopted by Slack, Shopify, and major cloud providers — reduces the blast radius of any failure from "entire region" to "single cell" (typically 1-5% of users), enabling active-active at finer granularity than region-level. Tunable consistency (per-request consistency level selection) and edge databases (Turso, D1, Durable Objects) are extending active-active patterns to the edge, reducing the latency gap between local and cross-region operations.
