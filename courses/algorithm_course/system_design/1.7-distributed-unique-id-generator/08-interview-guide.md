...
│  │ 15-30 min │ Deep Dive       │ Bit layout, Step-by-step plan in plain English, clock handling │   │
...
| Is the database single-node or distributed? | Range-partitioned DBs create write hotspots with time-ordered IDs | Varies |
| Are IDs exposed to JavaScript clients? | 64-bit integers lose precision in JS; need string serialization | Usually yes |
...
### Step-by-step plan in plain English (Be Ready to Write)
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  What they're testing: Do you understand the hardest Edge Case (Unusual or extreme situation)?             │
│                                                                              │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  What they're testing: Edge Case (Unusual or extreme situation) handling, scaling understanding            │
│                                                                              │
...
### Trap 5: "Should we use UUID v7 instead of Snowflake now that it's a standard?"

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Question: "UUID v7 is now an RFC standard. Should we use it over          │
│             Snowflake?"                                                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  What they're testing: Nuanced understanding of format trade-offs          │
│                                                                              │
│  BAD answer: "Yes, it's the new standard so we should migrate."            │
│                                                                              │
│  GOOD answer:                                                                │
│  "UUID v7 is excellent and solves the sortability problem of UUID v4,      │
│   but the decision depends on the use case:                                │
│                                                                              │
│   UUID v7 is BETTER when:                                                   │
│   • You need UUID format compatibility (existing APIs, interop)            │
│   • You want zero coordination (no machine ID management)                  │
│   • You're using PostgreSQL 18+ (native uuidv7() function)                │
│   • Team doesn't want to manage Snowflake infrastructure                   │
│                                                                              │
│   Snowflake is STILL BETTER when:                                           │
│   • You have billions of rows and index size matters (64-bit vs 128-bit)   │
│   • You need maximum throughput (4M IDs/sec vs slower CSPRNG)              │
│   • You need to extract machine ID from the ID (debugging, routing)        │
│   • You need deterministic capacity planning (known machine count)         │
│                                                                              │
│   IMPORTANT CAVEAT:                                                         │
│   Neither is suitable as primary keys in range-partitioned distributed     │
│   databases like Spanner — time-ordered IDs create write hotspots.         │
│   Those systems should use random UUIDs or bit-reversed sequences.         │
│                                                                              │
│   For most new systems in 2025+, UUID v7 is the safer default unless       │
│   you have specific reasons to need 64-bit IDs."                           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
## 2025-2026 Interview Context

### Why This Topic Remains Highly Relevant

| Factor | 2025-2026 Relevance |
|--------|---------------------|
| **UUID v7 now standard** | RFC 9562 (2024) and PostgreSQL 18 native support (2025) make this a current-events question |
| **Format selection depth** | Snowflake vs UUID v7 vs ULID vs TSID — more options than ever, testing nuanced trade-off reasoning |
| **Distributed database anti-pattern** | Spanner and CockroachDB explicitly warn against time-ordered IDs — tests awareness of partitioning strategy interaction |
| **Clock sync evolution** | PTP replacing NTP in data centers improves timestamp reliability — demonstrates awareness of infrastructure trends |
| **Cross-platform serialization** | JavaScript BigInt adoption and string serialization of 64-bit IDs remains a practical concern |

### Modern Interview Questions (2025-2026)

| Question | Expected Depth |
|----------|---------------|
| "Your system uses Spanner. What ID format would you choose and why?" | Must discuss write hotspot problem — UUID v7/Snowflake are wrong answers |
| "How would you migrate from UUID v4 to UUID v7 in a running system?" | Schema evolution, dual-write period, foreign key considerations |
| "Compare TSID vs Snowflake vs UUID v7 for a new microservices platform" | Bit layout configurability, coordination overhead, database compatibility |
| "Your Snowflake generator hit the 69-year lifetime limit. What now?" | Custom epoch review, format migration strategy, backward compatibility |
