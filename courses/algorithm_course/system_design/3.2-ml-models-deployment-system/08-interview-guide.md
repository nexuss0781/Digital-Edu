...
| Related System | Connection Point | Interview Leverage |
|----------------|------------------|--------------------|
| [LLM Gateway](../3.21-llm-gateway-prompt-management/00-index.md) | Token-level rate limiting, routing | "The gateway layer uses token bucket algorithms similar to API rate limiting, but counted in GPU tokens not API calls" |
| [Feature Store](../3.16-feature-store/00-index.md) | Real-time feature serving for inference | "Feature retrieval latency must be within the inference SLO budget -- a 50ms feature fetch eats half of a 100ms p99 target" |
| [Netflix Runway](../3.7-netflix-runway-model-lifecycle/00-index.md) | Drift detection, retraining triggers | "Model monitoring connects to the lifecycle system -- PSI drift alerts trigger the retraining pipeline" |
| [A/B Testing Platform](../12.14-ab-testing-platform/00-index.md) | Statistical rigor, experiment management | "ML A/B tests add guardrail metrics beyond standard experiments -- latency ceilings and revenue floors trigger rollback independently" |
| [Distributed Rate Limiter](../1.1-distributed-rate-limiter/00-index.md) | GPU-bound admission control | "GPU admission differs from CPU rate limiting: the cost per request varies 100x depending on prompt length" |
| [Edge AI/ML Inference](../2.13-edge-ai-ml-inference/00-index.md) | Quantization, on-device serving | "INT4 quantization is shared between edge and cloud -- edge trades accuracy for latency, cloud trades accuracy for memory" |
| [Recommendation Engine](../3.12-recommendation-engine/00-index.md) | Multi-model serving, traffic splitting | "Recommendation systems often serve multiple models per request (retrieval + ranking), requiring model-level resource isolation" |

---

## 2025-2026 Talking Points

| Topic | Key Point | How to Bring Up |
|-------|-----------|-----------------|
| **Speculative Decoding** | Draft model proposes, target verifies in parallel -- 2-3x latency reduction | "For latency-sensitive LLM serving, speculative decoding converts sequential decode into parallel verification" |
| **Disaggregated Serving** | Separate prefill (compute-bound) from decode (memory-bound) onto different GPUs | "The Splitwise pattern right-sizes hardware per phase -- 30-50% cost reduction for large model serving" |
| **MoE Serving** | Only 2 of 8 experts active per token -- memory vs compute scheduling | "Mixture-of-Experts models need expert-parallel scheduling since load imbalance can leave 75% of experts idle" |
| **FP8 Quantization** | Standard in TensorRT-LLM v0.9+ -- 2x throughput with minimal accuracy loss | "FP8 has become the production default for inference -- unlike INT8 it preserves gradient-friendly dynamic range" |
| **Inference-Time Compute** | Variable compute budgets per request based on difficulty | "Not all tokens are equally hard -- adaptive compute allocates more FLOPs to complex reasoning steps" |
| **Prefix Caching** | RadixAttention reuses KV cache across requests sharing prompt prefixes | "For chat applications where system prompts are shared, prefix caching avoids redundant prefill computation" |
