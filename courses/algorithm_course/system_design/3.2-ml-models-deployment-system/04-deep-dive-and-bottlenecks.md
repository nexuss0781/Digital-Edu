...
### Slowest part of the process 1: GPU Memory

```mermaid
flowchart TB
    subgraph Problem["Memory Slowest part of the process"]
...
### Slowest part of the process 3: Network Bandwidth
...
### Slowest part of the process 4: Batch Formation Latency
...
## Deep Dive 4: 2025-2026 Inference Optimizations

### Speculative Decoding

Speculative decoding uses a smaller "draft" model to propose multiple tokens, verified by the target model in a single forward pass:

```
ALGORITHM SpeculativeDecoding

CONSTANTS:
    SPECULATION_LENGTH = 5  // Tokens to speculate ahead

STATE:
    draft_model: SmallModel    // ~1B parameters, fast inference
    target_model: LargeModel   // Full model, slow per-token

FUNCTION speculative_decode(prompt: Tokens, max_tokens: int) -> Tokens:
    output = []
    context = prompt

    WHILE len(output) < max_tokens:
        // Phase 1: Draft model generates candidates quickly
        draft_tokens = []
        draft_probs = []
        FOR i IN range(SPECULATION_LENGTH):
            prob = draft_model.forward(context + draft_tokens)
            token = sample(prob)
            draft_tokens.append(token)
            draft_probs.append(prob[token])

        // Phase 2: Target model verifies ALL candidates in one forward pass
        // This is a single forward pass over SPECULATION_LENGTH tokens (compute-bound, fast)
        target_probs = target_model.forward_batch(context, draft_tokens)

        // Phase 3: Accept/reject using modified rejection sampling
        accepted = 0
        FOR i IN range(SPECULATION_LENGTH):
            acceptance_ratio = target_probs[i][draft_tokens[i]] / draft_probs[i]

            IF random() < min(1, acceptance_ratio):
                output.append(draft_tokens[i])
                accepted += 1
            ELSE:
                // Resample from adjusted distribution
                adjusted = normalize(max(0, target_probs[i] - draft_probs))
                output.append(sample(adjusted))
        ...
        IF accepted == SPECULATION_LENGTH:
            bonus = sample(target_probs[SPECULATION_LENGTH])
            output.append(bonus)
...
| Draft Model | Target Model | Acceptance Rate | Latency Reduction | Throughput Impact |
|-------------|-------------|-----------------|-------------------|-------------------|
| 1B | 7B | 75-85% | 2.0-2.5x | -15% (draft overhead) |
| 1B | 70B | 60-70% | 1.5-2.0x | -5% (draft negligible) |
| 7B | 70B | 80-90% | 2.5-3.0x | -25% (draft significant) |

### Disaggregated Serving (Prefill-Decode Separation)

Separating prefill and decode onto different hardware pools enables right-sizing:

```
ALGORITHM DisaggregatedServing

COMPONENTS:
    prefill_pool: List<ComputeGPU>   // High FLOPS (e.g., H100 SXM)
    decode_pool: List<MemoryGPU>     // High HBM bandwidth (e.g., H100 NVL)
    kv_transfer: HighBandwidthBus    // NVLink or InfiniBand

FUNCTION serve_request(request: InferenceRequest) -> Response:
    // Phase 1: Route prefill to compute-optimized hardware
    prefill_worker = prefill_pool.get_available()
    kv_cache = prefill_worker.run_prefill(request.prompt)
    first_token = kv_cache.first_output_token

    // Phase 2: Transfer KV cache to decode worker
    decode_worker = decode_pool.get_available()
    transfer_start = now()
    decode_worker.receive_kv_cache(kv_cache)
    transfer_latency = now() - transfer_start
    // Typical: 5-15ms for 70B model, 4096 context

    // Phase 3: Decode on memory-optimized hardware
    stream = decode_worker.run_decode(kv_cache, request.max_tokens)

    RETURN Response(first_token, stream, transfer_latency)

FUNCTION optimize_pool_ratio(workload: WorkloadProfile):
    """Dynamically adjust prefill:decode GPU ratio"""
    avg_prompt_length = workload.avg_input_tokens
    avg_generation_length = workload.avg_output_tokens

    // Prefill is O(prompt_length^2), decode is O(generation_length)
    prefill_compute = avg_prompt_length^2 * PREFILL_FLOPS_PER_TOKEN
    decode_compute = avg_generation_length * DECODE_BANDWIDTH_PER_TOKEN

    optimal_ratio = prefill_compute / (prefill_compute + decode_compute)

    RETURN adjust_pool_sizes(optimal_ratio)
```

**Cost Comparison:**

| Configuration | Hardware | Monthly Cost | Throughput | Cost per 1M Tokens |
|---------------|----------|-------------|------------|---------------------|
| Co-located (baseline) | 8× H100 SXM | ~$200K | 1,000 req/s | $1.00 |
| Disaggregated | 4× H100 SXM + 6× L40S | ~$140K | 1,100 req/s | $0.63 |
| Disaggregated + Spec | 4× H100 SXM + 6× L40S | ~$140K | 900 req/s | $0.55 (lower latency) |

### MoE-Optimized Serving (2025)

Mixture-of-Experts models activate only a subset of parameters per token, requiring specialized scheduling:

| Challenge | Impact | Mitigation |
|-----------|--------|------------|
| Expert load imbalance | Some experts receive 10x more tokens | Expert-parallel scheduling with token redistribution |
| Memory for all experts | 8× expert weights in memory, only 2 active | Expert offloading to CPU with prefetching |
| Routing overhead | Expert selection adds 1-2ms per token | Batch routing decisions, cache routing tables |
| Communication volume | All-to-all for expert parallelism | Hierarchical routing: intra-node first, inter-node second |
