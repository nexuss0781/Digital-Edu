...
| Component | Vertical Limit | Horizontal Limit | Slowest part of the process |
|-----------|----------------|------------------|------------|
...
Every AI-driven operational capability in the ML deployment system (auto-scaling, dynamic batching, canary progression, drift-triggered retraining) must progress through all six stages before reaching autonomous production.

| Stage | Gate Criteria | Rollback Trigger |
|-------|--------------|------------------|
| **1. Offline Evaluation** | Scaling policy simulation on 90 days of historical traffic achieves <5% over-provisioning and 0 SLO violations; canary logic tested on recorded deployments with known-bad models correctly caught | Simulation shows SLO violations >1% of intervals; canary logic misses known-bad model in replay |
| **2. Shadow Mode** | Autonomous decisions logged but not executed for 14 days; human operator makes actual decisions; shadow vs actual agreement >90% | Agreement <85%; shadow decisions would have caused SLO violation or cost overrun |
| **3. Canary (5% traffic)** | Applied to single model endpoint; latency SLO maintained; GPU cost within 10% of manual baseline; no failed deployments | Latency SLO breach; cost exceeds 115% of baseline; any deployment failure attributed to autonomous decision |
| **4. Human-Reviewed Production** | Autonomous decisions execute but require human confirmation within 5-minute window (except emergency scaling); 30-day audit shows 0 incidents | Any incident caused by autonomous decision; human override rate >20% |
| **5. Autonomous Production** | Full autonomous operation for infrastructure decisions (scaling, batching, routing, rollback); model promotion remains human-gated; 90-day incident-free track record | Any production incident; cost anomaly >20%; latency SLO violation |
| **6. Instant Rollback** | Feature flag per autonomous capability; <30s rollback to manual/rule-based operation; automatic disable on consecutive failures; post-mortem within 4h | N/A — this is the rollback mechanism itself |
