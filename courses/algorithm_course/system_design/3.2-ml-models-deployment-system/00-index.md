...
> **Tier D — AI-Autonomous:** The ML deployment system operates autonomously for infrastructure-level decisions — auto-scaling GPU pools, dynamic batching, request routing, canary progression, and rollback. These are machine-speed operational decisions where human latency would degrade system performance. Human oversight is retained for model promotion policies, A/B test conclusions, and experiment governance.

| Decision | AI Role | Human Role | Override | Escalation |
|----------|---------|------------|----------|------------|
| Auto-scaling GPU pools | Autonomously scales based on queue depth, GPU utilization, and latency signals | Sets scaling policies, cost budgets, and maximum instance limits | Can manually set fixed replica count; override persists until cleared | Auto-escalate if scaling fails to meet latency SLO for >5 min |
| Dynamic batching configuration | Adjusts batch size and wait time in real-time based on throughput/latency trade-off | Sets latency SLO targets and acceptable throughput ranges | Can fix batch parameters to static values | Escalate if batching efficiency drops below 60% |
| Canary rollout progression | Automatically advances canary traffic (1% → 5% → 25% → 100%) based on error rate and latency gates | Defines progression gates, rollback thresholds, and bake time | Can pause, halt, or manually advance canary at any stage | Auto-rollback if error rate >baseline + 0.5% at any stage |
| A/B test traffic splitting | Routes traffic per experiment configuration with consistent hashing | Designs experiments, sets sample size requirements, interprets results | Can halt experiment and force 100% to control at any time | Escalate to ML lead if experiment runs >2x planned duration |
| Model version promotion | Recommends promotion based on A/B test statistical significance | Reviews results and approves promotion from staging to production | Human approval required for production promotion | Escalate if no decision within SLA; block auto-promotion |
| Request routing & load balancing | Autonomously routes to optimal model server based on health, latency, and capacity | Sets routing policies and priority rules | Can pin traffic to specific server pool | Escalate if no healthy backends available |
| Drift detection & retraining trigger | Monitors feature and prediction drift; triggers retraining pipeline when thresholds exceeded | Sets drift thresholds (PSI, KS) and reviews retraining results | Can disable auto-retraining; manual trigger always available | Escalate if drift detected but retraining pipeline fails |
...
| Design | Relevance |
|--------|-----------|
| [MLOps Platform](../3.4-mlops-platform/00-index.md) | End-to-end ML lifecycle; this system focuses on the serving/deployment slice |
| [Netflix Runway Model Lifecycle](../3.7-netflix-runway-model-lifecycle/00-index.md) | Model lifecycle management patterns; drift detection and retraining triggers |
| [Uber Michelangelo](../3.5-uber-michelangelo-ml-platform/00-index.md) | Feature store integration and unified prediction service patterns |
| [LLM Gateway / Prompt Management](../3.21-llm-gateway-prompt-management/00-index.md) | Token-level rate limiting and routing patterns for LLM inference |
| [Edge AI/ML Inference](../2.13-edge-ai-ml-inference/00-index.md) | On-device inference optimization; model quantization and distillation shared patterns |
| [Feature Store](../3.16-feature-store/00-index.md) | Online feature serving that feeds real-time inference pipelines |
| [Recommendation Engine](../3.12-recommendation-engine/00-index.md) | A/B testing infrastructure for model variants in production |
| [Distributed Rate Limiter](../1.1-distributed-rate-limiter/00-index.md) | Token bucket patterns applied to GPU-bound request admission |
...
|--------|-------------|---------------------|
| **2023** | vLLM + PagedAttention | Transformed LLM serving economics through virtual memory-style KV cache |
| **Early 2024** | Speculative decoding mainstream | Reduced decode latency 2-3x for auto-regressive models |
| **Mid-2024** | SGLang, Sarathi-Serve | RadixAttention for prefix caching; chunked prefill for latency isolation |
| **Late 2024** | Disaggregated serving (Splitwise) | Separated prefill and decode onto different hardware for cost optimization |
| **Q1 2025** | TensorRT-LLM v0.9+ | FP8 quantization + in-flight batching as production standard |
| **Mid-2025** | MoE-optimized serving | Expert-parallel scheduling for Mixture-of-Experts models (Mixtral, Grok) |
| **Late 2025** | Inference-time compute scaling | Variable compute budgets per request based on difficulty estimation |
| **Ongoing** | Multi-modal serving | Unified serving for text+image+audio models with heterogeneous compute |

---

> **Vendor Freshness Notice (2026-03-21):** This document references specific vendor products and ML frameworks (vLLM, TensorRT-LLM, SGLang, Triton, TorchServe, TFServing, MLflow, Ray Serve, PagedAttention). Verify version compatibility and feature availability against current vendor documentation before adopting in production architectures.
