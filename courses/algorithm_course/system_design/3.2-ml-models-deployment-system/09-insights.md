...
## Insight 11: Speculative Decoding Trades Parallel Compute for Sequential Latency
**Category:** Performance
**One-liner:** A smaller draft model proposes N tokens that the target model verifies in a single forward pass, converting N sequential decode steps into one parallel verification -- reducing wall-clock latency by 2-3x without changing output quality.
**Why it matters:** Auto-regressive LLM decoding is inherently sequential: each token depends on the previous one, making single-request latency irreducible by batching alone. Speculative decoding breaks this constraint by having a ~1B draft model generate 4-8 candidate tokens speculatively, then the full model verifies all candidates in one forward pass (which is compute-bound like prefill, not memory-bound like decode). Rejected tokens trigger resampling from the target distribution, mathematically guaranteeing identical output quality. The acceptance rate (typically 60-85% depending on draft-target alignment) determines the effective speedup. The key architectural insight is that this introduces a new scheduling dimension: the draft model consumes GPU cycles that could serve other requests, creating a per-request latency vs system throughput trade-off that the scheduler must balance dynamically based on current load.

---

## Insight 12: Disaggregated Serving Separates Prefill and Decode Onto Different Hardware Classes
**Category:** Cost Optimization
**One-liner:** Running prefill (compute-bound, benefits from FLOPS) on separate hardware from decode (memory-bandwidth-bound, benefits from HBM capacity) enables right-sizing GPU types per phase, reducing serving cost by 30-50% for large models.
**Why it matters:** When prefill and decode share the same GPU pool, the hardware must satisfy both compute and memory bandwidth requirements simultaneously -- resulting in expensive GPUs that are underutilized in at least one dimension. Disaggregated architectures (pioneered by Splitwise and DistServe) route prefill to compute-optimized accelerators and decode to memory-optimized instances. The KV cache is transferred between phases over high-bandwidth interconnect. This separation also eliminates the interference problem where a long prefill starves ongoing decode sequences of GPU cycles, improving tail latency for in-flight requests. The architectural cost is a new inter-phase communication path that adds ~5-15ms of KV cache transfer overhead per request, which is acceptable for models where the total generation time is 500ms+. For latency-sensitive short-generation workloads, co-located serving remains preferable -- making this a workload-dependent optimization rather than a universal improvement.
