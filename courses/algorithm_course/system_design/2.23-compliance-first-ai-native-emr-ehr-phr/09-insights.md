...
**Category:** Contention
**One-liner:** Consent verification sits on the critical path of every single API request, making it a latency tax that must be cached aggressively or it becomes the system's dominant Slowest part of the process.
...
**Why it matters:** A CDS system that tries to run all checks synchronously will either miss latency targets or compromise on the depth of its analysis. The architecture separates critical DDI checks (synchronous, blocking, 100ms budget) from guideline RAG queries (async, non-blocking, 500ms+ budget). During high system load (>80%), only moderate-or-higher severity alerts are processed, shedding low-priority checks. This tiered approach prevents the CDS from becoming a workflow Slowest part of the process while ensuring patient-safety-critical checks always complete in time.
...
## Insight 14: HIPAA Security Rule 2025 Eliminates the Addressable Escape Hatch That Enabled Weak Security

**Category:** Compliance Architecture
**One-liner:** The January 2025 NPRM converts all "addressable" HIPAA Security Rule specifications to "required," removing the documented-justification loophole that allowed covered entities to skip MFA, encryption, and network segmentation.

**Why it matters:** Since 2003, the HIPAA Security Rule allowed covered entities to classify certain controls as "addressable" -- meaning they could skip the control if they documented why it was not reasonable. In practice, this created a two-tier security landscape where well-funded health systems implemented MFA and encryption while under-resourced organizations documented exemptions. The Change Healthcare ransomware attack (February 2024, ~100M individuals affected) catalyzed the 2025 NPRM, which mandates MFA for all ePHI access, AES-256 encryption at rest and in transit, network segmentation with documented maps, 72-hour system restoration capability, 15-day critical patching timelines, and annual penetration testing. For system architects, this means the security controls that were previously "nice to have" are now compliance requirements with enforcement penalties -- the system must be designed to meet all of these from day one, not retrofitted later.

---

## Insight 15: Ambient Clinical Intelligence Creates a Documentation Accuracy vs. Liability Trade-off That Architecture Must Resolve

**Category:** System Modeling
**One-liner:** AI-generated clinical notes (Microsoft DAX Copilot processing 1.5M+ notes monthly, 50-70% documentation time reduction) must be architecturally treated as draft documents requiring explicit physician attestation before becoming part of the legal medical record.

**Why it matters:** Ambient clinical intelligence captures physician-patient conversations and generates structured clinical notes automatically. The efficiency gains are transformative: 15-20% of US physicians now use ambient AI, with early adopters reporting 50-70% documentation time reduction and significant burnout mitigation. However, an AI-generated note that enters the medical record without physician review creates legal and clinical liability: hallucinated medications, misattributed symptoms, or omitted critical findings become part of the record of care. The architecture must enforce a mandatory attestation step where the physician reviews, edits, and explicitly signs the AI-generated note before it transitions from draft to final status. The ONC HTI-1 DSI transparency requirements (effective January 2026) add another layer: the system must disclose that AI was involved in generating the documentation, including source attributes about the model's training data and performance characteristics. This creates a clear audit trail showing which parts of the record were AI-generated, physician-reviewed, and physician-attested.

---

## Insight 16: TEFCA Transforms Health Information Exchange from Point-to-Point Integration to Network-Based Discovery

**Category:** Architecture Evolution
**One-liner:** TEFCA's QHIN (Qualified Health Information Network) architecture enables a "query anywhere" model where a participating organization can discover and retrieve patient records from any other TEFCA participant without pre-existing bilateral agreements.

**Why it matters:** Before TEFCA, health information exchange required either bilateral data-sharing agreements (which do not scale -- N organizations need N*(N-1)/2 agreements) or membership in specific regional HIEs (creating coverage gaps). TEFCA's Common Agreement creates a single legal and technical framework where 6+ QHINs (including eHealth Exchange, CommonWell, Epic/Carequality, Health Gorilla) interconnect, enabling any participating organization to query any other. The architectural impact is profound: the system no longer needs to maintain individual integration endpoints for each exchange partner. Instead, it connects to one QHIN and gains access to the entire network -- covering 70%+ of US hospitals indirectly. The technical pattern is similar to BGP in networking: each QHIN handles routing, discovery, and trust within its network, while the QHIN-to-QHIN protocol handles cross-network queries. For the EHR system, this means implementing FHIR-based patient discovery and document retrieval against a single QHIN interface rather than dozens of individual HIE endpoints.
