...
## Search-Recommendation Convergence (2025-2026)

### Unified Discovery Architecture

TikTok's search and recommendation systems have converged: search queries now feed into the recommendation model as explicit interest signals, and search results are ranked using the same DLRM infrastructure as FYP.

```
ALGORITHM: SearchAwareRecommendation

INPUT:
    user_id: string
    search_query: string (optional, empty for FYP)
    search_history: list of recent queries (last 7 days)

PROCEDURE:
    // 1. If active search, blend search relevance with recommendation
    IF search_query IS NOT EMPTY:
        // Text embedding of query
        query_embedding = text_encoder.encode(search_query)

        // Retrieve candidates matching query
        search_candidates = search_index.query(
            query_embedding,
            filters = {status: "active", region: user.region},
            top_k = 2000
        )

        // Also retrieve FYP candidates (for blending)
        fyp_candidates = standard_retrieval(user_id, pool_size=3000)

        // Blend: 70% search relevance, 30% personalized
        candidates = weighted_merge(
            search_candidates, weight=0.7,
            fyp_candidates, weight=0.3
        )

    // 2. Use search history as implicit interest signal
    ELSE:
        // Encode recent search queries as interest signals
        search_interest = aggregate_embeddings(
            [text_encoder.encode(q) for q in search_history],
            decay_factor = 0.8  // Recent queries weighted higher
        )

        // Add to user embedding for retrieval
        augmented_user_embedding = concatenate(
            user_embedding,
            search_interest * search_weight
        )

        candidates = standard_retrieval_with_embedding(
            augmented_user_embedding,
            pool_size = 5000
        )

    // 3. Rank using DLRM (same model, search features added)
    ranked = dlrm_ranking(candidates, extra_features={
        "has_active_search": search_query IS NOT EMPTY,
        "search_query_embedding": query_embedding,
        "search_history_embedding": search_interest
    })
...
- context: fyp|explore|shop (where the search was initiated)

Response (200 OK):
{
    "suggestions": [
        {"query": "cooking recipes", "type": "trending", "search_volume": "high"},
        {"query": "cooking hacks kitchen", "type": "personalized"},
        {"query": "cooking with chef_name", "type": "creator"}
    ]
}
```
