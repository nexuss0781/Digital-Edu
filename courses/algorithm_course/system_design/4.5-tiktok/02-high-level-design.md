...
## TikTok Shop Integration Architecture

```mermaid
flowchart TB
    subgraph FeedLayer["Feed Layer"]
        FYP["For You Page"]
        ShopTab["Shop Tab"]
    ...
    subgraph CommerceServices["Commerce Services"]
        ProductCatalog["Product Catalog<br/>(Search + Browse)"]
    ...
    subgraph CommerceML["Commerce ML"]
        ProductRanking["Product Ranking<br/>(In-feed)"]
        PriceOptimizer["Price Optimizer"]
    ...
    subgraph CommerceStorage["Commerce Storage"]
        ProductDB[("Product DB<br/>(PostgreSQL)")]
        OrderDB[("Order DB<br/>(PostgreSQL)")]
        InventoryCache[("Inventory Cache<br/>(Redis)")]
    end

    FYP --> ProductRanking
    ShopTab --> ProductCatalog
    LiveShop --> CartService

    ProductRanking --> ProductCatalog
    ...
    OrderService --> SellerService

    SellerService --> ShippingService
    OrderService --> ReturnService

    AffiliateService --> ProductCatalog
    FraudDetection --> OrderService

    classDef feed fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef commerce fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef ml fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    classDef fulfill fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef storage fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class FYP,ShopTab,LiveShop feed
    class ProductCatalog,CartService,OrderService,InventoryService,PaymentService,AffiliateService commerce
    class ProductRanking,PriceOptimizer,FraudDetection ml
    class SellerService,ShippingService,ReturnService fulfill
    class ProductDB,OrderDB,InventoryCache storage
```

**Commerce Integration Points:**
- **FYP Feed:** Product videos ranked alongside entertainment using blended engagement + conversion objective
- **Live Shopping:** Real-time inventory management with optimistic locking during stream purchases
- **Affiliate System:** Creator-driven product promotion with attribution tracking
- **Payment Processing:** Multi-currency support, regional payment methods, escrow for seller protection
