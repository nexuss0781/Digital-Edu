...
## Case Study: Handling Super Bowl-Scale Traffic Spikes

### Scenario

During major live events (sporting events, cultural moments, trending challenges), TikTok experiences 3-5x traffic spikes within minutes. Content creation surges as millions simultaneously record and upload reactions, while viewership spikes as users search for event-related content.

### Traffic Profile

| Metric | Normal | Event Spike | Multiplier |
|--------|--------|-------------|------------|
| FYP Requests/sec | 60K | 200K+ | 3.3x |
| Video Uploads/sec | 400 | 1,500+ | 3.8x |
| Live Streams Started | 50K | 200K+ | 4x |
| Search Queries/sec | 20K | 100K+ | 5x |

### Response Strategy

```
PROCEDURE handle_traffic_spike(event_type):
    // 1. Pre-event scaling (if predictable like Super Bowl)
    IF event.predictable:
        T-2h: Scale API servers to 2x
        T-1h: Scale ML inference to 1.5x
        T-30m: Pre-warm CDN with anticipated content categories
        T-0: Monitor and auto-scale as needed

    // 2. Reactive scaling (unpredictable viral moments)
    IF spike_detected(threshold=1.5x_normal):
        IMMEDIATE:
            - HPA triggers pod scaling (30s to ready)
            - Reduce FYP candidate pool (5K → 2K) to halve inference cost
            - Enable aggressive caching (TTL 60s → 120s)
            - Pre-compute trending feed as fallback

        IF spike > 2x_normal:
            - Activate secondary CDN capacity
            - Reduce prefetch depth (5 → 3 videos)
            - Queue non-critical writes (view logs, analytics)

        IF spike > 3x_normal:
            - Serve cached/trending FYP for new sessions
            - Throttle uploads (queue with estimated wait time)
            - Disable non-essential features (commerce, search suggestions)

    // 3. Post-event scaling
    AFTER spike_subsides(30m_window):
        - Gradual scale-down (10% reduction per 5 minutes)
        - Process queued writes
        - Reconcile counters from event buffer
```
...
|--------|---------------------|
| Pre-warming CDN is cheaper than reactive cache fills | Trending Detector feeds pre-warming pipeline |
| ML inference is the first Slowest part of the process during spikes | Keep lightweight fallback models warm at all times |
| Upload spikes lag view spikes by 2-5 minutes | Use this lag to pre-scale transcoding |
| Live stream concurrency is hardest to scale | Pre-allocate transcoder pools for predictable events |
