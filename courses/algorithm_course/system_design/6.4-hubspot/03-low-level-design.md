...
// Time Complexity (Speed of the algorithm): O(N) where N = number of nodes in the execution path
// Space Complexity (Memory usage of the algorithm): O(1) per enrollment — state persisted to database
```
...
// Time Complexity (Speed of the algorithm): O(R × E) where R = rules, E = events per contact
...
## GDPR Cascade Delete Algorithm

When a GDPR deletion request is received for a contact, all related data must be purged across all subsystems within the compliance window.

```
FUNCTION gdpr_delete_contact(account_id: long, contact_id: long, request_id: string):
    // Phase 1: Mark contact as deleted (soft delete — prevents new writes)
    mark_deleted(account_id, contact_id)
    emit_event("GDPR_DELETE_INITIATED", {account_id, contact_id, request_id})

    // Phase 2: Cascade delete across all subsystems
    delete_from_hbase(account_id, contact_id)          // CRM object data
    delete_associations(account_id, contact_id)         // All bidirectional associations
    delete_activity_events(account_id, contact_id)      // Timeline events
    delete_from_search_index(account_id, contact_id)    // Search index entries
    delete_email_analytics(account_id, contact_id)      // Email sends, opens, clicks
    delete_workflow_history(account_id, contact_id)      // Workflow enrollment history
    delete_form_submissions(account_id, contact_id)     // Form submission data
    delete_from_lead_scoring(account_id, contact_id)    // Score history

    // Phase 3: Purge from Kafka (topic compaction won't retain deleted key)
    publish_tombstone(account_id, contact_id)           // Kafka tombstone message

    // Phase 4: Purge from backup/archive tiers
    schedule_backup_purge(account_id, contact_id, deadline=30_days)

    // Phase 5: Audit trail (records THAT deletion happened, not WHAT was deleted)
    create_audit_entry({
        action: "GDPR_CONTACT_DELETE",
        request_id: request_id,
        account_id: account_id,
        contact_id: contact_id,  // Only the ID, not the contact data
        subsystems_purged: ["hbase", "search", "analytics", "workflows", "forms"],
        completed_at: now()
    })

    RETURN DeletionCertificate(request_id, status="completed")
```
...
|-------------|-----------|-------------|---------------|
| **400** | `INVALID_PROPERTY_VALUE` | Property value doesn't match expected type | Check property definition for expected type |
| **400** | `INVALID_ASSOCIATION` | Association type not valid between these object types | Verify association definition |
| **401** | `EXPIRED_TOKEN` | OAuth access token has expired | Refresh using refresh_token grant |
| **403** | `SCOPE_INSUFFICIENT` | OAuth scope doesn't include required permission | Request additional scopes during OAuth flow |
| **404** | `OBJECT_NOT_FOUND` | CRM object does not exist or is deleted | Verify object ID; check if GDPR-deleted |
| **409** | `PROPERTY_CONFLICT` | Concurrent modification detected (version mismatch) | Re-read current state and retry with updated version |
| **429** | `RATE_LIMIT_EXCEEDED` | Per-account or per-app rate limit exceeded | Respect Retry-After header; implement exponential backoff |
| **502** | `UPSTREAM_ERROR` | Internal service dependency failed | Retry with exponential backoff; check status page |
...
```
FUNCTION deliver_webhook(subscription: WebhookSubscription, event: CRMEvent):
    payload = build_payload(subscription, event)
    signature = hmac_sha256(subscription.secret, payload)

    FOR attempt IN range(1, MAX_RETRIES + 1):  // MAX_RETRIES = 5
        response = http_post(
            url = subscription.target_url,
            headers = {
                "X-HubSpot-Signature-v3": signature,
                "X-HubSpot-Request-Timestamp": now(),
        ...
        IF response.status IN [200, 201, 202, 204]:
            record_delivery_success(subscription.id, event.id)
            RETURN SUCCESS

        IF response.status IN [410]:  // Gone — endpoint deactivated
    ...
    record_delivery_failure(subscription.id, event.id, response.status)
    IF consecutive_failures(subscription.id) > 100:
        pause_subscription(subscription.id)
        notify_app_developer("Webhook delivery consistently failing")
    RETURN FAILED
```
