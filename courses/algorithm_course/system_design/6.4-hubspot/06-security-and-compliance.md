...
### Hublet-Level Isolation

```
Isolation Boundaries:
  Network Level:
    - Each Hublet has its own AWS account and VPC
    - No cross-Hublet network routes exist (enforced by AWS account boundary)
    - Even HubSpot engineers cannot accidentally query cross-Hublet data

  Encryption Level:
    - Each Hublet has independent KMS encryption keys
    - Key rotation on independent schedules
    - Customer-managed keys (BYOK) for Enterprise tier

  Data Level:
    - HBase: separate clusters per Hublet
    - MySQL: separate Vitess clusters per Hublet
    - Kafka: separate clusters per Hublet
    - No shared databases, even for system metadata

  Access Level:
    - Separate IAM roles per Hublet
    - Break-glass access requires Hublet-specific approval
    - All cross-Hublet data access logged and alerted
```

### Tenant Isolation Within a Hublet

```
FUNCTION enforce_tenant_isolation(request: APIRequest):
    // Every database query is scoped by account_id
    account_id = extract_from_auth_token(request.token)

    // Account ID is injected into every query — not passed from client
    // This prevents parameter tampering
    query.add_filter("account_id", "=", account_id)

    // HBase row key starts with account_id — tenant data is co-located
    // but queries never cross account boundaries
    row_key_prefix = encode_account_id(account_id)
    scan.set_start_row(row_key_prefix)
    scan.set_stop_row(next_prefix(row_key_prefix))

    // Search index: tenant_id filter applied at query parse time
    search_query.add_must_filter("tenant_id", account_id)
```
...
| Scope | Access Level | Example Use Case |
|-------|-------------|-----------------|
| `crm.objects.contacts.read` | Read contact records | Dashboard widget displaying contact info |
| `crm.objects.contacts.write` | Create/update contacts | Lead capture form integration |
| `automation.workflows.read` | Read workflow definitions | Reporting on active workflows |
| `automation.workflows.write` | Create/modify workflows | Programmatic workflow management |
| `sales-email-read` | Read email engagement data | Analytics integration |
| `content` | Read/write CMS content | Website management tool |

### API Key Security

```
API Key Format:
  pat-{hublet}-{key_id}-{secret}

  pat:       Prefix identifier (Private App Token)
  hublet:    Routing identifier (na1, eu1, na2)
  key_id:    Unique identifier for key lookup
  secret:    Random 32-byte hex string

Security Properties:
  - Hublet embedded in key → routing without database lookup
  - Key can be revoked independently (key_id → revocation check)
  - Scopes defined at key creation time, immutable
  - Rate limit tier associated with key_id
  - Key rotation: create new key → update integrations → revoke old key
```

---

## Threat Model: Marketing Automation Specific

| Attack Vector | Risk | Mitigation |
|--------------|------|-----------|
| **Workflow abuse (infinite loops)** | Customer creates workflow that triggers another workflow in a loop, consuming resources | Loop detection: max 10 workflow-to-workflow chains; circuit breaker on enrollment rate per account |
| **Email spoofing via HubSpot** | Attacker uses HubSpot to send phishing emails from a legitimate-looking domain | DKIM/SPF enforcement; domain verification required before sending; abuse detection ML model |
| **API key leakage** | API key committed to public GitHub repo | GitHub secret scanning partnership; key rotation capability; scope limitation; rate limiting |
| **Contact data scraping via API** | Malicious app with `contacts.read` scope exports entire database | Bulk export rate limiting; anomaly detection on read patterns; audit logging |
| **GDPR right-to-erasure weaponization** | Competitor submits fraudulent deletion requests | Identity verification for GDPR requests; deletion request audit trail; 30-day soft-delete period |
...
|-------|--------|---------------|
| **In transit** | TLS 1.3 (all internal and external communication) | Automatic certificate rotation via internal CA |
| **At rest (HBase)** | AES-256 volume encryption | Per-Hublet KMS keys; 90-day rotation |
| **At rest (MySQL)** | AES-256 tablespace encryption | Per-Hublet KMS keys; transparent to application |
| **At rest (Kafka)** | AES-256 volume encryption | Per-cluster keys; rotation on broker restart |
| **At rest (Object storage)** | Server-side encryption (SSE-KMS) | Customer-managed keys (BYOK) for Enterprise |
| **Email in transit** | TLS (opportunistic) + DKIM signing | Per-customer DKIM keys; 2048-bit RSA |
| **API keys** | Hashed with HMAC-SHA256 before storage | Key material never stored in plaintext |

### Session Security

| Control | Implementation |
|---------|---------------|
| Session token | JWT with 15-minute expiry; HttpOnly, Secure, SameSite=Strict |
| Refresh token | 30-day TTL; rotated on each use; stored encrypted in database |
| Concurrent sessions | Maximum 10 per user; oldest terminated on new login |
| IP binding | Optional: session bound to IP range (Enterprise feature) |
| Idle timeout | 30 minutes (configurable per account) |
| Forced logout | Admin can terminate all sessions for any user |
