...
### Slowest part of the process 1: HBase CrmObjects Table as Single Point of Contention

**Why it's a Slowest part of the process**: Every CRM read/write from 100+ microservices hits this table. At 25M+ peak req/sec, even small inefficiencies compound.
...
### Slowest part of the process 2: Workflow Timer Service Database Polling

**Why it's a Slowest part of the process**: Millions of active workflow enrollments with delayed actions. Polling the `next_action_at` column requires scanning a potentially large index.
...
### Slowest part of the process 3: Email Template Rendering at Peak

**Why it's a Slowest part of the process**: A large campaign sending 1M emails requires rendering 1M personalized templates. At peak, multiple campaigns compete for rendering capacity.
...
## Critical Component 4: Lead Scoring Engine

### Why This Is Critical

Lead scoring determines which contacts are most likely to convert, directly driving sales team efficiency and workflow automation triggers. A poorly calibrated scoring system either floods sales with unqualified leads or misses high-intent prospects.

### How It Works Internally

```
FUNCTION calculate_lead_score(contact: Contact, model: ScoringModel) -> Score:
    score = 0

    // Dimension 1: Behavioral signals (real-time from Kafka events)
    FOR behavior IN contact.recent_behaviors:
        weight = model.behavior_weights[behavior.type]
        recency_factor = exponential_decay(behavior.timestamp, half_life=7_days)
        score += weight * recency_factor

    // Behavioral weights (configurable per customer):
    //   page_view(pricing):     +20
    //   page_view(docs):        +5
    //   email_open:             +2
    //   email_click:            +5
    //   form_submit(demo):      +50
    //   form_submit(newsletter): +10

    // Dimension 2: Demographic/firmographic signals (from CRM properties)
    FOR property IN model.demographic_properties:
        value = contact.properties[property.name]
        IF value IN property.value_weights:
            score += property.value_weights[value]

    // Demographic weights:
    //   job_title contains "VP|Director|C-level":  +30
    //   company_size > 500:                         +20
    //   industry IN target_industries:              +15

    // Dimension 3: Negative signals (decay)
    days_since_last_engagement = days_between(contact.last_activity, now())
    IF days_since_last_engagement > 30:
        score *= 0.5  // 50% decay for inactive contacts
    IF contact.properties["unsubscribed"]:
        score = 0  // Hard zero for unsubscribed

    RETURN Score(value=clamp(score, 0, 100), breakdown=score_breakdown)
```

### Scoring Update Strategy

| Trigger | Update Type | Latency |
|---------|-----------|---------|
| High-signal event (form submit, pricing page) | Real-time recalculation | < 5 seconds |
| Email engagement (open, click) | Real-time increment | < 10 seconds |
| Property change (job title, company size) | Real-time recalculation | < 10 seconds |
| Time-based decay | Batch recalculation (daily at 3 AM) | Up to 24 hours |
| Model change (weights updated) | Batch recalculation (all contacts) | Hours |

---

## Performance Tuning Strategies

### HBase Optimization Parameters

| Parameter | Default | When to Adjust | Trade-off |
|-----------|---------|---------------|-----------|
| `hbase.regionserver.global.memstore.size` | 0.4 | Increase for write-heavy workloads | More memory for writes vs. block cache for reads |
| `hbase.hregion.memstore.flush.size` | 128 MB | Increase for higher throughput | Larger flushes, less frequent compaction |
| `hbase.hregion.max.filesize` | 10 GB | Controls region split threshold | Larger regions = fewer splits, more data per server |
| `hbase.client.retries.number` | 10 | Reduce for faster failure detection | Faster timeouts vs. retry resilience |
| Dedup window | 100 ms | Increase for bursty workloads | Better dedup vs. slightly stale reads |

### Kafka Swimlane Tuning

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| Partitions per swimlane | 32-128 | Match to consumer count for parallelism |
| Consumer batch size | 500 messages | Balance throughput vs. action latency |
| Max poll interval | 5 minutes | Allow slow custom code actions to complete |
| Session timeout | 30 seconds | Detect consumer failures quickly |
| Rebalance cooldown | 60 seconds | Prevent rebalance storms during deploys |

### Email Delivery Optimization

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| SMTP connection pool per ISP | 10-50 connections | Match ISP's concurrent connection tolerance |
| Message batch size per connection | 50 | Minimize TCP handshake overhead |
| Retry backoff (421 response) | Exponential: 30s → 60s → 120s → 300s | Match ISP throttling recovery patterns |
| IP warm-up ramp | 100 → 500 → 2K → 10K → 50K/day over 4 weeks | Gradual reputation building per IP |
| Bounce threshold for IP rotation | > 5% hard bounce rate | Protect sender reputation before ISP penalizes |
| Feedback loop processing delay | < 5 minutes | Fast suppression of complaints to avoid repeated sends |

### Compression & Encoding Strategy

| Data Type | Encoding | Compression | Ratio | Trade-off |
|-----------|----------|-------------|-------|-----------|
| CRM properties (HBase) | Protobuf | Snappy | 3-4× | Fast decompression for real-time reads |
| Email analytics events | Avro | Zstd | 8-10× | High ratio for append-only analytics |
| Workflow execution logs | JSON → columnar | LZ4 | 5-6× | Balance of speed and compression for recent logs |
| Search index segments | Custom binary | Snappy | 2-3× | Read speed prioritized over compression ratio |
| Blob storage (attachments) | Raw binary | None (already compressed) | 1× | Most attachments are PDF/JPEG — already compressed |
| Kafka messages | Protobuf | LZ4 (broker-level) | 4-5× | Batch compression at broker reduces network bandwidth |
