...
| Space Complexity (Memory usage of the algorithm) | O(N × V) | N nodes × V virtual nodes each |
...
| [03 - Low-Level Design](./03-low-level-design.md) | Hash functions, data structures, Step-by-step plan in plain English |
...
| System | Origin | How They Use Consistent Hashing |
|--------|--------|--------------------------------|
| **Dynamo** | Major e-commerce provider | Partitioning + preference lists for replication |
| **DynamoDB** | Managed NoSQL service | Virtual nodes, auto-scaling partitions |
| **Cassandra** | Originally from a social platform | 256 vnodes default, token ranges |
| **Riak** | Basho | Ring-based partitioning, CRDTs |
| **Memcached** | Open source | Ketama algorithm with client-side hashing |
| **Redis Cluster** | Open source | Hash slots (16384 fixed slots, variant approach) |
| **Akamai CDN** | Akamai | Original use case - web cache distribution |
| **Discord** | Discord | Message routing to correct shard |
| **HAProxy** | Open source | Load balancing with bounded loads |
| **Envoy** | CNCF | Ring hash and Maglev load balancer |
| **ScyllaDB** | Open source | Tablet-based consistent hashing (2024+), replaces vnodes |
...
|---------|------|------------|
| **Distributed Key-Value Store** | [1.3](../1.3-distributed-key-value-store/00-index.md) | Consistent hashing is the partitioning strategy for distributed KV stores |
| **Distributed LRU Cache** | [1.4](../1.4-distributed-lru-cache/00-index.md) | Cache cluster sharding via consistent hashing minimizes cold misses on scale-out |
| **Distributed Load Balancer** | [1.2](../1.2-distributed-load-balancer/00-index.md) | Maglev/ring hash enables connection-affinity load balancing |
| **Content Delivery Network** | [1.15](../1.15-content-delivery-network-cdn/00-index.md) | CDN origin selection uses consistent hashing (Akamai's original use case) |
| **Service Discovery** | [1.10](../1.10-service-discovery-system/00-index.md) | Service instance selection for connection-affine routing |
| **Container Orchestration** | [2.2](../2.2-container-orchestration-system/00-index.md) | Pod-to-shard routing in stateful services uses consistent hashing |
| **Distributed Lock Manager** | [1.8](../1.8-distributed-lock-manager/00-index.md) | Lock partition assignment via consistent hashing for fault-tolerant lock services |
| **Multi-Region Active-Active** | [2.9](../2.9-multi-region-active-active/00-index.md) | Cross-region partition ownership via consistent hashing rings |
...
|-------------|-------------|
| **Tablet-based hashing (ScyllaDB)** | Replaces static vnodes with dynamically-sized tablets that split/merge based on data size, eliminating vnode count tuning |
| **Anchor Hash** | Memory-efficient algorithm achieving O(1) lookup with O(n) memory and minimal disruption, better than ring hash for large clusters |
| **Weighted consistent hashing in service meshes** | Envoy and other proxies integrate weighted ring hash for heterogeneous backend capacity |
| **DRA-aware partitioning** | Emerging pattern in GPU-enabled distributed caches where consistent hashing accounts for hardware capabilities |
| **Multi-probe consistent hashing** | Approach using k probes to achieve near-perfect balance without virtual nodes, reducing memory to O(n) |
