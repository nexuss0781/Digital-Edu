...
COMPLEXITY:
  ...
  Sampling: O(1) per item (Beta sampling is Constant Time (Same time regardless of data size))
```
...
## Algorithm 6: Hybrid Retrieval with Reciprocal Rank Fusion

```
ALGORITHM: HybridRetrievalRRF
INPUT: user_embedding, preference_embedding, context, constraints
OUTPUT: merged_candidates

PROCEDURE HybridRetrieve(user_emb, pref_emb, context, constraints):
    // Source 1: Collaborative ANN (user behavior similarity)
    collab_results = VectorDB.search(
        collection="item_embeddings",
        vector=user_emb,
        top_k=500,
        filter=constraints.to_filter()
    )

    // Source 2: Content ANN (preference-based similarity)
    content_results = VectorDB.search(
        collection="item_embeddings",
        vector=pref_emb,
        top_k=300,
        filter=constraints.to_filter()
    ...
    // Source 3: Trending/Popular (segment-weighted)
    trending_results = TrendingIndex.get(
        segment=context.user_segment,
        geo=context.geo_bucket,
        limit=200
    )

    // Source 4: Sparse retrieval (metadata match for tail queries)
    IF context.search_query IS NOT NULL:
        sparse_results = MetadataIndex.search(
    ...
    all_sources = [collab_results, content_results, trending_results, sparse_results]
    ...
    FOR source_idx, source IN enumerate(all_sources):
        FOR rank, item IN enumerate(source):
            ...
            IF item.item_id NOT IN item_scores:
                item_scores[item.item_id] = 0
                item_sources[item.item_id] = set()
            item_scores[item.item_id] += rrf_score
            item_sources[item.item_id].add(source_idx)

    // Sort by fused score
    merged = sort(item_scores.items(), key=score, descending=True)

    // Take top 500 candidates
    candidates = []
    FOR item_id, score IN merged[:500]:
        candidates.append({
            "item_id": item_id,
            "rrf_score": score,
            "sources": item_sources[item_id],
            "source_count": len(item_sources[item_id])
        })

    RETURN candidates

COMPLEXITY:
  Time: O(N log N) where N = total candidates across sources
  Space: O(N) for score aggregation
  Typical: ~1200 input candidates → 500 output, <15ms
```
