...
|-------|-----------|----------|
| Brute Force (Checking every single possibility) (API key) | 100 failures/min | Block IP, alert |
...
```
Threat: Attacker attempts to reconstruct user behavior from
leaked or scraped embedding vectors.

Risk Assessment:
  - User embeddings (512-dim) encode behavioral patterns
  - Research shows partial reconstruction is possible with
    access to the encoder model and embedding vectors
  - Higher risk when embeddings are exposed via API responses

Mitigation Strategy:
  1. Never expose raw embedding vectors in API responses
  2. Add calibrated noise to embeddings before storage:
     noisy_emb = emb + N(0, σ²) where σ chosen via DP analysis
  3. Use dimensionality reduction for external-facing embeddings
  4. Rotate embedding spaces periodically (quarterly retraining)
  5. Monitor for systematic embedding extraction patterns

Privacy Budget:
  - Noise level: σ = 0.05 (minimal accuracy impact)
  - Privacy guarantee: (ε=2.0, δ=1e-6)-differential privacy
  - Accuracy impact: <2% reduction in retrieval recall@100
```

### Recommendation Manipulation (Shill Attacks)

```
Threat: Coordinated fake interactions to boost specific items
in personalization results.

Detection Signals:
  - Sudden CTR spike for specific items from new accounts
  - Interaction patterns matching bot fingerprints
  - Geographic clustering of boost activity
  - Abnormal session-to-conversion ratios

Mitigation:
  1. Weight interactions by account trust score
  2. Require minimum account age for embedding influence
  3. Rate-limit per-item positive signals from new accounts
  4. Anomaly detection on item-level bandit parameter changes
  5. Human review queue for items with suspicious signal patterns

Bandit Protection:
  - Cap per-session reward contribution to any single item
  - Decay weight for new accounts: trust_weight = min(1, account_age_days / 30)
  - Separate bandit parameters for trusted vs untrusted traffic
```

### Real-Time Data Unlearning

```
Requirement: GDPR "right to erasure" demands removal of user
influence from trained models, not just raw data deletion.

Implementation:
  1. Immediate Actions (< 1 hour):
     - Delete user embedding from Vector DB
     - Delete user context from Feature Store
     - Delete session data from Redis
     - Publish deletion event to all downstream systems

  2. Bandit Parameter Adjustment (< 24 hours):
     - For each item the user interacted with:
       α_adjusted = α - user_reward_contribution
       β_adjusted = β - user_failure_contribution
     - Requires interaction log retention during deletion window

  3. Model Retraining (next cycle):
     - Exclude user data from training dataset
     - Retrain affected embedding models
     - Verify via membership inference test

  4. Verification:
     - Automated scan confirms no user_id references remain
     - Audit log records deletion completeness
