...
### Runbook: Bandit Regret Spike

```
Symptoms:
  - Alert: HighBanditRegret (>15% for 1 hour)
  - Exploration rate anomaly (too high or too low)
  - User engagement metrics declining

Diagnosis:
  1. Check exploration rate distribution
     → bandit_exploration_rate by segment
  2. Check for cold item influx
     → bandit_cold_item_coverage_ratio
  3. Check posterior parameter distribution
     → bandit_posterior_uncertainty histogram
  4. Check for data quality issues
     → feedback pipeline lag, missing rewards

Resolution:
  If excessive exploration (>25%):
    → Check for mass prior reset (deployment error)
    → Verify posterior store connectivity
    → Temporarily reduce exploration weight

  If insufficient exploration (<3%):
    → Check for posterior overflow (very large α+β)
    → Apply posterior decay to all items
    → Increase exploration floor

  If data quality issue:
    → Fix feedback pipeline
    → Backfill missing reward signals
    → Re-initialize affected posteriors from backup

Escalation:
  → If regret >20% for 2 hours, page ML on-call
```
...
```
Concept: Instead of alerting on instantaneous SLO violations,
track error budget consumption rate across multiple time windows.
...
Burn Rate Alerts:
  ...
  |--------|-----------|-----------------|----------|
  | 5 min  | 14.4x     | 2% in 5 min     | Critical |
  | 30 min | 6x        | 5% in 30 min    | Critical |
  | 6 hr   | 1x        | 10% in 6 hours  | Warning  |
  | 3 day  | 1x        | 10% in 3 days   | Info     |

Prometheus Implementation:
  # Fast burn (critical - pages immediately)
  sum(rate(api_requests_total{status_code=~"5.."}[5m]))
  /
  sum(rate(api_requests_total[5m])) > 14.4 * 0.0001

  # Slow burn (warning - notify during business hours)
  sum(rate(api_requests_total{status_code=~"5.."}[6h]))
  /
  sum(rate(api_requests_total[6h])) > 1 * 0.0001
```

### Personalization Quality SLO

```
Beyond availability, track personalization effectiveness as an SLO:

SLO: Personalization lift > 10% (measured via A/B test)
  - Measurement: Rolling 24-hour comparison
  - Error budget: Lift can drop below 10% for at most 7 hours/month

Alerting:
  - Warning: Lift < 12% for 4 hours (approaching SLO)
  - Critical: Lift < 10% for 2 hours (burning error budget)
  - Auto-action: If lift < 5% for 1 hour, switch to cached results
    and page ML team (likely model degradation)
```
