...
| LLM Gateway ([3.21](../3.21-llm-gateway-prompt-management/00-index.md)) | REST/gRPC | Outbound | LLM inference with routing and caching |
...
| Feature Store ([3.16](../3.16-feature-store/00-index.md)) | gRPC/Redis | Bidirectional | Online feature serving for ranking and bandits |
| Vector Database ([3.14](../3.14-vector-database/00-index.md)) | gRPC | Bidirectional | Embedding storage and ANN retrieval |
| Event Sourcing ([1.18](../1.18-event-sourcing-system/00-index.md)) | Kafka | Inbound | Immutable event log for replay and audit |

---

## Decision 6: Hybrid Retrieval Strategy (Dense + Sparse)

| Criterion | Dense Only (Embeddings) | Sparse Only (Metadata) | Hybrid (Dense + Sparse) |
|-----------|------------------------|----------------------|------------------------|
| **Recall for Popular Items** | Good | Excellent | Excellent |
| **Recall for Tail Items** | Poor (no interactions) | Good (metadata match) | Good |
| **Semantic Understanding** | Excellent | None | Excellent |
| **Cold Start Items** | Poor | Good | Good |
| **Latency** | 10-15ms | 5ms | 15-20ms |

**Decision: Hybrid Retrieval with Score Fusion**

**Architecture:**
```
Hybrid Retrieval Pipeline:
  1. Dense path: ANN search on user embedding → 500 candidates
  2. Sparse path: Metadata filter (category, price, availability) → 300 candidates
  3. Trending path: Popularity-weighted retrieval → 200 candidates
  4. Merge: Reciprocal Rank Fusion (RRF) to combine candidate lists
  5. Deduplicate: Remove duplicates, keeping highest rank
  6. Filter: Apply business rules (availability, content policy)
  7. Output: 500 merged candidates → deep ranking stage
...
**Rationale:**
- Dense retrieval excels at semantic similarity but misses items with no embedding history
- Sparse retrieval captures exact-match intent (specific brands, categories) that embeddings blur
- Hybrid approach improves recall@100 by 12-18% over dense-only in production systems
- RRF is parameter-free and robust across different scoring distributions
