...
| Data Category | Collection Consent | Storage Duration | Processing Basis | Deletion SLA |
|---------------|-------------------|------------------|-----------------|--------------|
| **Explicit Preferences** | Opt-in | Until deletion request | Consent | <24 hours |
| **Behavioral Signals** | Implied (T&C) | 90 days | Legitimate interest | <24 hours |
| **Session Context** | Implied | Session + 24h | Legitimate interest | Automatic (TTL) |
| **Inferred Embeddings** | N/A (derived) | Until source deleted | Derived from consented data | <48 hours |
| **Aggregated Analytics** | N/A (anonymous) | 1 year | Anonymized | N/A |

### Real-Time Unlearning Requirements

```
When a user requests data deletion:

Tier 1 - Immediate (<1 hour):
  - Remove user embedding from Vector DB
  - Delete session and context from Feature Store
  - Remove from edge caches (all PoPs)
  - Cease personalization (serve popular/trending)

Tier 2 - Propagated (<24 hours):
  - Adjust bandit parameters to subtract user's contribution
  - Remove from training datasets
  - Purge from event logs

Tier 3 - Model Cleanup (next training cycle):
  - Retrain affected models without user's data
...
| Scenario | Duration | Revenue Impact | Mitigation Cost |
|----------|----------|----------------|-----------------|
| **Full personalization outage** | 1 hour | ~$50K (based on 15% lift loss) | Graceful degradation |
| **Embedding pipeline failure** | 4 hours | ~$20K (stale but functional) | Batch fallback |
| **LLM reasoning unavailable** | 24 hours | ~$5K (template explanations) | Template fallback |
| **Single region failure** | 15 min | ~$2K (failover time) | Multi-region active-active |
| **Edge PoP failure** | 5 min | <$500 (auto-reroute) | CDN redundancy |
