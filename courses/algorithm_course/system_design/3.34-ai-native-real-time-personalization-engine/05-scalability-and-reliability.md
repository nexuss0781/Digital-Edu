...
### Embedding Replication Across Regions

```
Challenge: User embeddings must be available in all regions for
low-latency personalization, but streaming updates originate
from the user's home region.

Replication Topology:
  Primary Region (user's home):
    - Receives all streaming updates
    - Authoritative embedding version
    - Writes to local Vector DB

  Secondary Regions:
    - Async replication via Kafka MirrorMaker
    - Lag target: <5 seconds
    - Used for reads during region failover
    - No write conflicts (single-writer per user)

Edge Sync:
  - Streaming layer publishes embedding diffs (not full vectors)
  - Edge KV stores apply diffs to local copies
  - Sync interval: 30 seconds
  - Diff format: {user_id, delta_vector, version}
  - Full sync: On cache miss or version gap >5
```

### Conflict Resolution for Multi-Region Writes

```
Scenario: User switches regions mid-session (e.g., VPN change).

Resolution Strategy:
  1. Detect region change via session metadata
  2. Redirect streaming events to new home region
  3. Transfer user state (last 50 interactions, current embedding)
  4. Old region stops processing events for this user
  5. New region picks up from transferred state

Transfer Protocol:
  - State size: ~5KB per user (embedding + context)
  - Transfer latency: <2 seconds
  - During transfer: Edge serves cached results
...
| Test | Frequency | Scope | Expected Outcome |
|------|-----------|-------|------------------|
| **Kill random edge PoP** | Weekly | Per-region | Traffic shifts to next-nearest PoP, <5s recovery |
| **Kafka broker failure** | Monthly | Per-cluster | Consumer rebalance, <30s lag spike then recovery |
| **Vector DB shard failure** | Monthly | Per-shard | Replica promotion, <30s failover |
| **LLM cluster unavailable** | Weekly | Global | Circuit breaker opens, template fallback in <3s |
| **Redis primary failure** | Monthly | Per-cluster | Sentinel failover, <10s recovery |
| **Full region failure** | Quarterly | Per-region | Traffic reroutes, <15 min full recovery |
...
Steady-State Load Test:
  - Duration: 4 hours
  - Target: 1.5x peak QPS (750K QPS)
  - Validate: Latency SLOs maintained, no error rate increase

Spike Test (Flash Sale Simulation):
  - Ramp: 500K → 5M QPS in 5 minutes
  - Duration: 2 hours at peak
  - Validate: Auto-scaling triggers, graceful degradation activates

Soak Test:
  - Duration: 72 hours
  - Target: Normal peak QPS
  - Validate: No memory leaks, no state corruption, GC stability

Embedding Pipeline Stress Test:
  - Duration: 8 hours
  - Target: 3x normal event rate (180K events/sec)
  - Validate: Consumer lag recovers within 10 minutes
```
