...
| [1.2 - Rate Limiter](../1.2-rate-limiter/) | Per-wallet signing rate limits, gas sponsorship budget enforcement |
| [8.1 - Payment Processing System](../8.1-payment-processing-system/) | Transaction lifecycle, idempotency patterns, reconciliation |
| [15.1 - Metrics & Monitoring System](../15.1-metrics-monitoring-system/) | Time-series metrics for signing latency, HSM utilization, chain-specific dashboards |
| [2.14 - Event-Driven Architecture](../2.14-event-driven-architecture/) | Event bus patterns for transaction confirmations, policy change propagation |
...
|-----|-------|---------------|-----------|
| **2013--2017** | Single-key wallets (Bitcoin Core, Electrum) | HD derivation (BIP-32/44) | Single point of loss; seed phrase burden |
| **2017--2020** | Multi-sig wallets (Gnosis Safe, BitGo) | On-chain multi-party approval | Chain-specific; gas overhead; public governance |
| **2020--2023** | MPC wallets (Fireblocks, ZenGo) | Threshold signing without on-chain footprint | Complex cryptography; interactive latency |
| **2023--2025** | Account Abstraction (ERC-4337) | Programmable accounts; gas sponsorship; passkeys | Smart-contract-chain only; bundler dependency |
| **2025--present** | Hybrid (EIP-7702 + MPC + AA) | EOA delegation + MPC signing + modular smart accounts | Complexity of supporting all models simultaneously |
...
| Term | Definition |
|------|-----------|
| **DKG (Distributed Key Generation)** | Protocol where multiple parties jointly generate a public/private key pair without any single party knowing the complete private key |
| **TSS (Threshold Signature Scheme)** | Signing protocol where t-of-n parties can produce a valid signature without reconstructing the private key |
| **MPC-CMP** | Canetti-Makriyannis-Peled protocol; state-of-the-art MPC signing with pre-signing optimization |
| **FROST** | Flexible Round-Optimized Schnorr Threshold Signatures; 2-round signing for Schnorr-compatible chains |
| **ERC-4337** | Account Abstraction standard; replaces EOAs with smart contract wallets supporting custom validation, gas sponsorship, and batched transactions |
| **EIP-7702** | Pectra upgrade (May 2025); allows EOAs to temporarily delegate to smart contract code per-transaction |
| **ERC-7579** | Modular smart account standard; defines pluggable validators, executors, hooks, and fallback modules |
| **Paymaster** | Smart contract that sponsors gas fees for UserOperations in the ERC-4337 framework |
| **Bundler** | Off-chain service that collects UserOperations and submits them to the EntryPoint contract on-chain |
| **EntryPoint** | Singleton on-chain contract that processes bundled UserOperations in ERC-4337 |
| **Key Refresh** | Proactive secret sharing protocol that rotates MPC key shares without changing the derived public key |
| **HD Wallet** | Hierarchical Deterministic wallet (BIP-32/44); derives unlimited addresses from a single master key |
| **Session Key** | Temporary signing key with scoped permissions (limited amount, duration, and contract interactions) |
| **TEE (Trusted Execution Environment)** | Hardware-isolated processing environment (SGX, TrustZone, SEV-SNP) where code and data are protected from the host OS |
