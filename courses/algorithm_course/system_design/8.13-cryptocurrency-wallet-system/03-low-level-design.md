...
```
POST /v1/wallets/{wallet_id}/session-keys
Authorization: Bearer {jwt_token}

Request:
{
  "permissions": {
    "allowed_contracts": ["0xDEXRouter...", "0xTokenContract..."],
    "allowed_functions": ["swap(address,uint256)", "approve(address,uint256)"],
    "max_value_per_tx": "1000000000000000000",
    "max_total_value": "10000000000000000000",
    "valid_from": "2026-03-21T09:00:00Z",
    "valid_until": "2026-03-21T17:00:00Z",
    "chain": "ethereum"
  }
}
...
```
POST /v1/wallets/{wallet_id}/key-refresh
Authorization: Bearer {jwt_token}  // Requires Admin role
...
{
  "refresh_id": "ref_xyz456",
  "wallet_id": "wlt_xyz789",
  "status": "in_progress",
  "old_version": 3,
  "new_version": 4,
  "participating_nodes": [1, 2, 3],
  "estimated_completion": "2026-03-21T15:35:00Z"
}
```
...
| `POST /session-keys` | 20/min per wallet | Session key creation is moderately expensive |
| `POST /key-refresh` | 1/hour per wallet | Key refresh is computationally expensive; prevent abuse |
...
// Time Complexity (Speed of the algorithm): O(n^2) communication rounds where n = total_parties
// Space Complexity (Memory usage of the algorithm): O(n * threshold) per party for polynomial coefficients
```
...
// Time Complexity (Speed of the algorithm): O(t^2) for t signing parties (dominated by MtA rounds)
// Space Complexity (Memory usage of the algorithm): O(t) per party
...
// Time Complexity (Speed of the algorithm): O(1) for nonce acquisition; O(n) for gap detection where n = gap size
// Space Complexity (Memory usage of the algorithm): O(p) where p = number of pending transactions per address
```
...
// Time Complexity (Speed of the algorithm): O(P * R) where P = policies, R = rules per policy
// Space Complexity (Memory usage of the algorithm): O(1) per evaluation (stateless)
```
...
### Algorithm 6: Pre-Signing Triple Generation and Management

```
FUNCTION generate_pre_signing_triples(wallet_id, count, signing_parties):
    triples = []

    FOR i in range(count):
        // Each party generates random nonce share and blinding factor
        FOR each party P_j in signing_parties:
            k_j = RANDOM_SCALAR()          // Nonce share
            gamma_j = RANDOM_SCALAR()      // Blinding factor
            K_j = k_j * G                  // Nonce commitment

        // Parties perform MtA (Multiplicative-to-Additive) conversion
        FOR each pair (P_i, P_j):
            (alpha_ij, beta_ij) = MTA_PROTOCOL(k_i, gamma_j)
            // Result: alpha_ij + beta_ij = k_i * gamma_j

        // Each party computes their delta and sigma shares
        FOR each party P_j:
            delta_j = k_j * gamma_j + SUM(alpha_jm + beta_mj for all m != j)
            sigma_j = k_j * x_j + SUM(mu_jm + nu_mj for all m != j)

        // Combine delta shares to get combined nonce inverse
        delta = SUM(delta_j for all j) MOD curve_order
        R = (1/delta) * SUM(gamma_j * G for all j)
        r = R.x_coordinate MOD curve_order
        ...
        triple = {
            ...
            wallet_id: wallet_id,
            r: r,
            sigma_shares: {j: sigma_j for each party j},
            created_at: NOW(),
            expires_at: NOW() + 30_DAYS
        }
        triples.APPEND(triple)

    // Store triples in enclave (encrypted at rest)
    ENCLAVE_STORE(wallet_id, triples)
    RETURN triples.count

FUNCTION consume_triple(wallet_id, message_hash, signing_parties):
    triple = ENCLAVE_DEQUEUE(wallet_id)  // FIFO (First-In-First-Out, like a line at a store) from triple pool
    IF triple IS NULL:
        RETURN FALLBACK_TO_FULL_CEREMONY(wallet_id, message_hash)

    // Online phase: single round using pre-computed triple
    FOR each party P_j in signing_parties:
        partial_sig_j = triple.sigma_shares[j] * message_hash
                       + triple.r * triple.sigma_shares[j]
        partial_sig_j = partial_sig_j MOD curve_order

    s = SUM(partial_sig_j for all j) MOD curve_order
    signature = (triple.r, s)

    // Verify before returning
    IF NOT VERIFY_ECDSA(public_key, message_hash, signature):
        ABORT("Pre-signed triple produced invalid signature")

    RETURN signature

// Time Complexity (Speed of the algorithm): O(t^2 * count) for generation; O(t) for consumption
// Space Complexity (Memory usage of the algorithm): O(count) triples stored per wallet
```

### Algorithm 7: EIP-7702 Transaction Construction

```
FUNCTION build_eip7702_transaction(wallet_id, delegation_contract, calls):
    address = GET_WALLET_ADDRESS(wallet_id, chain="ethereum")
    nonce = ACQUIRE_NONCE("ethereum", address)

    // Build the authorization tuple
    authorization = {
        chain_id: CHAIN_CONFIG.ethereum.chain_id,
        address: delegation_contract,   // Contract code to delegate to
        nonce: nonce
    }

    // Sign the authorization (this grants the EOA smart account powers)
    auth_hash = KECCAK256(ENCODE_AUTHORIZATION(authorization))
    auth_signature = MPC_SIGN(wallet_id, auth_hash)

    // Build batched calls via the delegation contract's multicall
    calldata = ENCODE_MULTICALL(calls)

    // Construct EIP-7702 type-4 transaction
    transaction = {
        type: 0x04,
        chain_id: CHAIN_CONFIG.ethereum.chain_id,
        nonce: nonce,
        to: address,             // Self-call: EOA executes via delegated code
        value: 0,
        data: calldata,
        max_fee_per_gas: ESTIMATE_GAS_PRICE("ethereum"),
        max_priority_fee_per_gas: ESTIMATE_PRIORITY_FEE("ethereum"),
        gas_limit: ESTIMATE_GAS(address, calldata) * 1.15,
        authorization_list: [(authorization, auth_signature)]
    }

    // Sign the outer transaction envelope
    tx_hash = KECCAK256(ENCODE_TRANSACTION(transaction))
    tx_signature = MPC_SIGN(wallet_id, tx_hash)

    RETURN SERIALIZE_SIGNED_TX(transaction, tx_signature)

// Note: EIP-7702 requires TWO signatures per transaction:
// 1. Authorization signature (grants delegation)
// 2. Transaction signature (authorizes execution)
// Both are signed by the same key but over different payloads
```
