...
## Data Flow: Key Refresh (Proactive Secret Sharing)

```mermaid
sequenceDiagram
    participant Sched as Refresh Scheduler
    participant Orch as Signing Orchestrator
    participant TSS1 as Signer Node 1
    participant TSS2 as Signer Node 2
    participant TSS3 as Signer Node 3
    participant KS as Key Share Store
    participant Audit as Audit Log

    Sched->>Orch: Initiate key refresh (wallet_id, batch)
    Orch->>Orch: Acquire write lock on key share version
    Orch->>Orch: Wait for active signing sessions to drain

    par Generate Refresh Polynomials
        Orch->>TSS1: StartRefresh(wallet_id, version=2)
        Orch->>TSS2: StartRefresh(wallet_id, version=2)
        Orch->>TSS3: StartRefresh(wallet_id, version=2)
    end

    Note over TSS1,TSS3: Each node generates random polynomial and distributes evaluation points

    TSS1-->>TSS2: refresh_share_12 (encrypted)
    TSS1-->>TSS3: refresh_share_13 (encrypted)
    TSS2-->>TSS1: refresh_share_21 (encrypted)
    TSS2-->>TSS3: refresh_share_23 (encrypted)
    TSS3-->>TSS1: refresh_share_31 (encrypted)
    TSS3-->>TSS2: refresh_share_32 (encrypted)

    Note over TSS1,TSS3: Each node adds received refresh shares to existing share

    TSS1-->>Orch: RefreshComplete(share_1_v2, verification_proof)
    TSS2-->>Orch: RefreshComplete(share_2_v2, verification_proof)
    TSS3-->>Orch: RefreshComplete(share_3_v2, verification_proof)

    Orch->>Orch: Verify all proofs against unchanged public key
    Orch->>KS: Store new shares (version=2, status=PENDING)
    Orch->>KS: Activate version=2, deactivate version=1
    Orch->>Orch: Release write lock
    Orch->>Audit: Log key refresh event (wallet_id, v1→v2)
```

---

## Data Flow: Social Recovery via Guardians

```mermaid
sequenceDiagram
    participant LostUser as User (lost device)
    participant App as Recovery Portal
    ...
    participant Account as Smart Account (on-chain)

    LostUser->>App: Initiate recovery (wallet_id)
    App->>Recovery: Create recovery request
    Recovery->>Recovery: Verify user identity (KYC re-verification)

    par Notify Guardians
        Recovery->>G1: Recovery notification + approval link
        Recovery->>G2: Recovery notification + approval link
        Recovery->>G3: Recovery notification + approval link
    end

    G1->>Recovery: Approve (sign recovery_hash)
    G2->>Recovery: Approve (sign recovery_hash)

    Note over Recovery: 2-of-3 guardian threshold met

    Recovery->>Recovery: Start 48-hour time lock
    Recovery->>LostUser: Recovery approved; executable after delay

    Note over Recovery: 48 hours elapse (original owner can cancel)

    Recovery->>DKG: Generate new key shares (new device + platform + backup)
    DKG-->>Recovery: New shares created (same public key derivation)

    alt Smart Account (ERC-4337)
        Recovery->>Account: rotateOwner(new_public_key, guardian_signatures)
        Account->>Account: Verify guardian signatures on-chain
        Account-->>Recovery: Owner rotated
    end

    Recovery-->>App: Recovery complete; new wallet active
    App-->>LostUser: Wallet restored with same addresses
```
...
| **Key Refresh Service** | Scheduled and on-demand key share rotation | Batch-oriented; rate-limited by DKG ceremony cost |
| **Recovery Service** | Guardian management, recovery request lifecycle, time-lock enforcement | Horizontal; stateless between requests |
...
The wallet platform serves multiple classes of tenants with different isolation requirements:

| Tenant Type | Isolation Level | Key Share Separation | Policy Engine | Rate Limits |
|------------|----------------|---------------------|---------------|-------------|
| **Enterprise (institutional)** | Dedicated signer node pool | Dedicated HSM partition per org | Custom policy rules; self-managed | Custom per-contract SLA |
| **dApp (WaaS)** | Shared infrastructure, logical isolation | Shared HSM partition; unique encryption key per dApp | Platform-default policies; dApp-configurable limits | Per-dApp quotas |
| **Retail (direct)** | Shared infrastructure | Shared HSM partition; per-user encryption key | Platform-default policies | Standard tier-based limits |

### Event-Driven Architecture

Key system events are published to an event bus for downstream consumers:

| Event | Publisher | Consumers | Purpose |
|-------|----------|-----------|---------|
| `wallet.created` | Wallet Service | Billing, Analytics, Compliance | Trigger KYC check; update quota tracking |
| `transaction.signed` | Signing Orchestrator | Audit Logger, Analytics, Notification Service | Immutable audit record; user notification |
| `transaction.confirmed` | Chain Indexer | Balance Aggregator, Notification Service | Update cached balance; notify user |
| `policy.updated` | Policy Engine | All Signing Orchestrator instances | Flush policy cache; ensure new policy takes effect |
| `key.refreshed` | Key Refresh Service | Audit Logger, Compliance Service | Compliance record; verify refresh completion |
| `recovery.initiated` | Recovery Service | Notification Service, Security Dashboard | Alert guardians; track on security dashboard |
| `gas.price_spike` | Gas Oracle | AA Service, Notification Service | Adjust sponsorship limits; warn users of high fees |

---

## TEE/HSM Layering Architecture

The wallet system uses a layered security architecture where TEE and HSM serve complementary roles:

```mermaid
flowchart TB
    subgraph Application["Application Layer"]
        SIGN_ORCH["Signing Orchestrator"]
        POLICY["Policy Engine"]
    end

    subgraph TEE_Layer["TEE Layer (Computation)"]
        direction TB
        MPC_COMPUTE["MPC Computation<br/>(pre-signing, partial sig generation)"]
        KEY_CACHE["Key Share Cache<br/>(in-enclave, TTL 5 min)"]
        ATTEST["Remote Attestation<br/>(verified before each session)"]
    end

    subgraph HSM_Layer["HSM Layer (Key Storage)"]
        direction TB
        ROOT_KEY["Root Master Key<br/>(never leaves HSM)"]
        KEK["Key Encryption Keys<br/>(per-region)"]
        SHARE_ENC["Share Encryption/Decryption<br/>(FIPS 140-2 Level 3)"]
    end

    subgraph Cold["Cold Storage Layer"]
        AIR_GAP["Air-Gapped HSM<br/>(disaster recovery)"]
        BACKUP["Encrypted Backup Shares<br/>(3 geographic locations)"]
    end

    SIGN_ORCH --> MPC_COMPUTE
    POLICY --> MPC_COMPUTE
    MPC_COMPUTE --> KEY_CACHE
    KEY_CACHE -->|Cache miss: decrypt share| SHARE_ENC
    SHARE_ENC --> KEK --> ROOT_KEY

    HSM_Layer -.->|Daily encrypted snapshot| Cold

    classDef app fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef tee fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef hsm fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef cold fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class SIGN_ORCH,POLICY app
    class MPC_COMPUTE,KEY_CACHE,ATTEST tee
    class ROOT_KEY,KEK,SHARE_ENC hsm
    class AIR_GAP,BACKUP cold
```

**Why this layering?**
- **HSM** provides regulatory compliance (FIPS 140-2 certification), hardware tamper resistance, and root key protection---but is slow (~2,000 ops/sec) and expensive ($50K+/module)
- **TEE** provides fast computation (~50,000 ops/sec), programmable logic, and memory isolation---but has weaker tamper resistance and side-channel risks
- **Combined**: HSM protects the root encryption keys and performs rare but critical operations (key share encryption/decryption). TEE handles high-volume MPC computation with key shares cached in enclave memory. This achieves HSM-grade key protection with TEE-grade performance.

### Post-SGX TEE Migration Path

With Intel SGX deprecated (2024), the system must plan migration to alternative TEE technologies:

| TEE Technology | Status | Strengths | Limitations | Migration Priority |
|---------------|--------|-----------|-------------|-------------------|
| **Intel SGX** | Deprecated (current) | Widely deployed; well-understood security model | No new hardware; known side-channel attacks | Migrate by 2026 |
| **AMD SEV-SNP** | Production-ready | Full VM encryption; stronger memory isolation; no side-channel history | AMD-only; coarser granularity (VM-level, not app-level) | Primary target |
| **ARM CCA** | Emerging (2025--2026) | Mobile-native (for user-side key shares); realm isolation | Limited server deployment; new ecosystem | Secondary for mobile |
| **AWS Nitro Enclaves** | Production-ready | Cloud-native; easy deployment; attestation integration | Cloud-provider-specific; no bare-metal option | Interim solution |
| **RISC-V Keystone** | Research/early-stage | Open-source; no vendor lock-in; configurable isolation | Immature ecosystem; limited hardware availability | Long-term research |
