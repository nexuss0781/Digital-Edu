...
Understanding where the content moderation system fits in the broader platform architecture:

| External System | Dependency Type | Data Exchanged | Failure Impact |
|---|---|---|---|
| User service | Inbound | Account trust scores, user metadata | Lose trust-based threshold adjustments; fall back to default thresholds |
| Content delivery network | Outbound | Enforcement actions (remove/restrict) | Content visible despite removal decision; critical SLA risk |
| Notification service | Outbound | User notifications (violation notice, appeal updates) | Users unaware of enforcement; appeal deadlines start without notice |
| Legal/compliance system | Bidirectional | NCMEC reports, DSA submissions, forensic holds | Regulatory non-compliance; legal exposure |
| Analytics platform | Outbound | Aggregated moderation metrics | No impact on safety; impacts reporting and planning |
| Identity service | Inbound | Authentication, session management for reviewers | Reviewers locked out; review queue stalls |
| Cross-platform coalitions | Bidirectional | Hash updates, adversarial signals (GIFCT, Technology Coalition) | Delayed detection of known-bad content; miss new evasion techniques |

### Failure Domain Mapping

The system's failure domains are designed so that a failure in one domain does not cascade to safety-critical functions:

| Failure Domain | Components | Blast Radius | Isolation Mechanism |
|---|---|---|---|
| ML inference | GPU fleet, model serving | Classification unavailable; hash matching unaffected | Circuit breaker; fallback to human review |
| Human review platform | Workstation backend, assignment service | Reviews stall; ML auto-decisions continue | Independent from ML pipeline; SLA extension |
| Hash matching | Hash DB, matching service | Known-bad content detection degraded | Independent fleet; CPU-based (no GPU dependency) |
| Audit logging | Log cluster, chain verification | Enforcement paused (fail-safe) | Must-succeed dependency; system halts rather than lose audit trail |
| Policy engine | Rule store, evaluation service | Stale rules applied | Last-known-good rules cached in-process |
| Compliance reporting | DSA/NCMEC submission services | Reports delayed | Async; queued for retry; does not block enforcement |
...
|---|---|---|
| **V1: Keyword + Hash** | Basic content filtering | Keyword blocklists; exact hash matching; manual review queue |
| **V2: ML Classification** | Automated decision support | Specialized ML models per category; confidence-gated routing; basic human review queue |
| **V3: Policy Engine** | Configurable enforcement | Hot-reloadable policy rules; geo-specific variants; account trust scoring; SLA management |
| **V4: Human-First** | Reviewer-centric design | Wellness constraints in assignment; calibration injection; inter-rater reliability; multi-tier appeals |
| **V5: Regulatory Compliance** | DSA/NetzDG-native | Transparency database integration; automated NCMEC reporting; machine-readable reports; forensic holds |
| **V6: Adversarial Resilience** | Active defense | LLM-based contextual classification; cross-platform signal sharing; CIB detection; deepfake detection |
| **V7: AI-Native Moderation** | End-to-end AI | Multimodal foundation models for classification; automated policy generation from regulatory text; real-time adversarial adaptation |

> **Current industry state (2025-2026):** Most large platforms operate at V4-V5 maturity. V6 capabilities are emerging at the largest platforms. V7 remains aspirational.
