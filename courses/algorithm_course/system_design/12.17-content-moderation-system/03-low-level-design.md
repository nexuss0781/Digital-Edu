...
## Algorithm: Adversarial Normalization Pipeline
...
FUNCTION normalizeText(input: String) -> NormalizedText:
  ...
  // Map visually similar characters to canonical Latin form
  normalized = unicode_nfkc_normalize(input)
  IF normalized != input:
    transformations.append("unicode_nfkc")
  input = normalized

  // Stage 2: Homoglyph mapping
  // Cyrillic 'а' (U+0430) → Latin 'a' (U+0061), etc.
  homoglyph_map = loadHomoglyphDatabase()  // ~2,000 mappings
  FOR EACH char, position IN input:
    IF char IN homoglyph_map:
      input = input.replace(position, homoglyph_map[char])
      transformations.append("homoglyph:" + char)

  // Stage 3: Zero-width character removal
  // Remove zero-width spaces, joiners, directional markers
  zwc_pattern = [U+200B, U+200C, U+200D, U+FEFF, U+200E, U+200F, U+2060, U+2061]
  FOR EACH char IN zwc_pattern:
    IF char IN input:
      input = input.remove_all(char)
      transformations.append("zwc_removed:" + hex(char))

  // Stage 4: Leetspeak expansion
  leet_map = {'@': 'a', '3': 'e', '1': 'i', '0': 'o', '$': 's', '!': 'i', '7': 't', '4': 'a'}
  FOR EACH char, position IN input:
    IF char IN leet_map:
      input = input.replace(position, leet_map[char])
      transformations.append("leet:" + char)

  // Stage 5: Repeated character collapse
  // "haaaaaate" → "hate" (keep at most 2 consecutive identical chars)
  collapsed = collapse_repeats(input, max_consecutive = 2)
  IF collapsed != input:
    transformations.append("repeat_collapse")
  input = collapsed

  // Stage 6: Whitespace normalization
  // Collapse multiple spaces, tabs, newlines into single space
  input = collapse_whitespace(input)
...
**Note:** The `transformation_count` is itself a signal — content with many normalization transformations applied is more likely to be adversarially obfuscated and may warrant increased scrutiny.

---

## Algorithm: Reviewer Assignment with Wellness Constraints

```
FUNCTION assignNextItem(reviewer: Reviewer) -> ReviewTask?:

  // Step 1: Check wellness constraints
  IF reviewer.shift_elapsed > MAX_SHIFT_HOURS × 3600:
    RETURN null  // Shift ended; reviewer should log off

  IF reviewer.consecutive_harmful_count >= MAX_CONSECUTIVE_HARMFUL:
    triggerMandatoryBreak(reviewer)
    RETURN null  // Mandatory break required

  // Step 2: Determine eligible categories
  eligible_categories = ALL_CATEGORIES.copy()

  FOR EACH category IN HIGH_HARM_CATEGORIES:
    daily_count = reviewer.daily_exposure[category]
    IF daily_count >= reviewer.exposure_caps[category]:
      eligible_categories.remove(category)
      logWellnessEvent(reviewer.id, "cap_reached", category)

  IF eligible_categories IS EMPTY:
    // All categories capped; reviewer should take admin tasks
    RETURN null

  // Step 3: Check skill qualifications
  qualified_categories = eligible_categories.intersect(reviewer.certifications)
  qualified_languages = reviewer.language_certifications

  // Step 4: Find highest-priority eligible item
  candidate = reviewQueue.peekHighestPriority(
    categories = qualified_categories,
    languages = qualified_languages,
    geo_shard = reviewer.geo_shard
  )

  IF candidate IS NULL:
    // No items match reviewer's qualifications; try cross-shard
    candidate = reviewQueue.peekHighestPriority(
      categories = qualified_categories,
      languages = qualified_languages,
      geo_shard = ANY
  ...
  IF candidate IS NULL:
    RETURN null  // Queue empty for this reviewer's qualifications

  // Step 5: Assign
  candidate.status = "ASSIGNED"
  candidate.assigned_to = reviewer.id
  candidate.assigned_at = now()
  candidate.sla_remaining = candidate.sla_deadline - now()

  // Step 6: Update wellness counters
  IF candidate.content_category IN HIGH_HARM_CATEGORIES:
    reviewer.daily_exposure[candidate.content_category] += 1
    reviewer.consecutive_harmful_count += 1
  ELSE:
    reviewer.consecutive_harmful_count = 0  // Reset on non-harmful item
...
## Algorithm: Policy Rule Compilation to Decision Tree

```
FUNCTION compilePolicyRules(rules: List<PolicyRule>) -> DecisionTree:
  // Sort by precedence (emergency > geo > content_type > trust > global)
  sorted_rules = rules.sortBy(r => r.precedence_tier, ascending)

  // Build a decision tree indexed by content_type → category → geo_scope
  tree = new DecisionTree()

  FOR EACH rule IN sorted_rules:
    // Create path in tree: content_type → category → geo_scope
    node = tree.getOrCreate(
      content_type = rule.content_type_filter ?: "*",
      category = rule.category_filter ?: "*",
      geo_scope = rule.geo_scope ?: "global"
    )
    node.addRule(rule)

  // At each leaf, rules are ordered by precedence
  // First matching rule wins (unless rule.additive = true)

  RETURN tree


FUNCTION evaluatePolicy(tree: DecisionTree, content: ContentItem,
                         scores: ModelScores) -> PolicyDecision:

  // Walk tree to find applicable rules
  applicable_rules = tree.lookup(
    content_type = content.content_type,
    category = scores.primary_category,
    geo_scope = content.geo_context
  )

  FOR EACH rule IN applicable_rules:  // Already precedence-ordered
    IF rule.matches(content, scores):
      action = rule.action  // ALLOW | SOFT_LABEL | RESTRICT | REMOVE | ESCALATE
      IF rule.additive:
        // Additive rules accumulate; most severe wins
        accumulated_action = max_severity(accumulated_action, action)
        CONTINUE
      ELSE:
        RETURN PolicyDecision(action = action, rule_id = rule.id, rule_version = rule.version)

  // If no rule matched, apply default (most conservative)
  RETURN PolicyDecision(action = "ESCALATE", rule_id = "default", rule_version = 0)
```

---

## Algorithm: Viral Content Early Detection

```
FUNCTION monitorContentVelocity(content_stream: Stream<ViewEvent>):
  // Maintain a sliding window counter per content_id
  velocity_counters = new SlidingWindowCounter(window = 5_minutes)

  FOR EACH event IN content_stream:
    velocity_counters.increment(event.content_id)

    current_velocity = velocity_counters.getCount(event.content_id)

    IF current_velocity > VIRAL_THRESHOLD (10,000 views/5min):
      // Check if this content has been fully moderated
      moderation_state = getContentModerationState(event.content_id)

      IF moderation_state == "PENDING" OR moderation_state == "IN_REVIEW":
        // Viral content not yet fully moderated — high risk
        applyTemporaryShadowRestriction(event.content_id)
        boostPriority(event.content_id, priority_multiplier = 10)
        alertTrustAndSafety("viral_unmoderated", event.content_id, current_velocity)

      ELSE IF moderation_state == "ALLOWED" AND current_velocity > SUPER_VIRAL (100,000 views/5min):
        // Previously allowed content going super-viral — re-scan
        enqueueFreshScan(event.content_id, reason = "viral_velocity")
```

---

## Data Flow: NCMEC Reporting Pipeline

```
1. Hash match confirms CSAM:
   - pHash/PDQ/PhotoDNA distance < threshold against NCMEC hash DB

2. Immediate content removal:
   - content_item.enforcement_state = "REMOVED"
   - Content becomes invisible to all users within 500ms

3. Forensic preservation:
   - Content item and all metadata copied to forensic archive
   - Forensic hold applied (prevents all deletion, including GDPR erasure)

4. NCMEC report assembly:
   - Report payload constructed:
     - Platform content ID
     - User account identifier (for law enforcement referral)
     - Detection method (hash match category)
     - Matched hash identifier
     - IP address and device metadata (where legally permissible)
     - Timestamp of upload and detection
     - Uploaded file type and size

5. CyberTipline submission:
   - HTTPS POST to NCMEC CyberTipline API
   - Response: CyberTipline report number
   - Report number stored in moderation_decision record

6. Post-submission:
   - Account flagged for investigation
   - All other content from this account prioritized for review
   - Account trust score set to zero
   - If account has > 1 CSAM detection: immediate account suspension

7. Audit trail:
   - Complete chain recorded: detection → removal → NCMEC submission → account action
   - All timestamps, all decision makers (automated vs. human), all hashes
```
