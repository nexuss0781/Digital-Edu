...
### Topic 1: Multi-Stage Model Cascade vs. Single-Model Architecture

How would you compare a cascade of specialized models (each optimized for one content category) against a single large multimodal model that handles all categories? Consider latency, cost, recall, and the ability to update individual categories without affecting others. The cascade approach allows independent retraining per category (hate speech model can be updated without touching CSAM detection), while the single model simplifies deployment but creates a monolithic training/deployment dependency.

### Topic 2: Hash Inversion Attacks and Hash Database Security

Discuss the implications of research showing that perceptual hashes (PhotoDNA, PDQ, NeuralHash) are vulnerable to inversion attacks that can reconstruct approximate original images. How does this change the security model for hash databases containing CSAM hashes? What are the architectural implications for hash storage, access control, and cross-platform sharing?

### Topic 3: LLM-in-the-Loop for Policy-as-Prompt Evaluation

When should the system invoke an LLM for contextual classification? What are the latency, cost, and reliability implications? How do you prevent the LLM from being jailbroken by adversarial content that it's being asked to classify? What happens when the LLM produces hallucinated classifications?

### Topic 4: Reviewer Inter-Rater Reliability at Scale

How do you maintain consistent review quality across 30,000 reviewers in 50 countries? Discuss calibration injection, Cohen's kappa monitoring, cultural bias detection, and the feedback loop between expert panel decisions and reviewer training.

### Topic 5: Real-Time vs. Post-Publication Moderation Trade-offs

Pre-publication screening (blocking content before it goes live) provides maximum safety but adds latency and may over-moderate. Post-publication proactive scanning allows content to go live immediately but creates a window of exposure for harmful content. How do you decide which content types get pre-publication screening and which get post-publication scanning?

### Topic 6: Deepfake Detection as an Emerging Moderation Category

AI-generated deepfake content creates new challenges: the content may not match any known hash (it's synthetically generated), traditional classifiers trained on real images may not detect synthetic artifacts, and the adversarial arms race is accelerating. How would you architect a deepfake detection pipeline and integrate it with the existing moderation system?

---

## Comparison with Related Systems

| Dimension | Content Moderation | Spam Filter | Ad Safety | Content Recommendation |
|---|---|---|---|---|
| **Decision speed** | Sub-second for pre-pub; hours for human review | Milliseconds (inline with email delivery) | Sub-second (ad serving latency) | Sub-second (feed ranking) |
| **False positive cost** | Censorship; user trust erosion; regulatory risk | Lost legitimate email | Ad revenue loss | Reduced engagement |
| **False negative cost** | User harm; legal liability; regulatory fines | Spam in inbox | Brand safety incident | Irrelevant content |
| **Human-in-the-loop** | Essential (regulatory requirement) | Rare (user reports only) | Rare (brand appeals) | None (fully automated) |
| **Regulatory surface** | DSA, NetzDG, NCMEC, FOSTA-SESTA | CAN-SPAM (minimal) | Ad standards bodies | None |
| **Adversarial intensity** | Extremely high (active evasion) | High (spammers adapt) | Moderate (ad fraud) | Low (organic content) |
| **Model diversity** | Ensemble per category | 2-3 models | 2-3 models | Single ranking model |
| **Audit requirements** | Immutable, tamper-evident, legally discoverable | Minimal | Moderate | None |
...
|---|---|---|
| Pre-publication screening p99 target | 500ms | User-visible upload latency constraint |
| DSA illegal content removal SLA | 24 hours | Legal obligation; drives queue prioritization |
| NetzDG clearly illegal content SLA | 24 hours | German law; separate from DSA |
| NCMEC reporting SLA | 24 hours | Legal obligation; zero tolerance |
| CSAM reviewer daily exposure cap | 20 items | Wellness hard constraint; shapes capacity planning |
| Video frame reduction via keyframes | ~65% | Dominant GPU cost optimization |
| LLM Edge Case (Unusual or extreme situation) invocation rate | ~10% of Zone B | Cost constraint; only for uncertainty band ± 0.1 |
| Calibration injection rate | 5-10% of reviewer items | Quality measurement overhead |
| Cohen's kappa target | > 0.80 | Inter-rater reliability threshold |
| Appeal automated overturn rate | ~30% | Drives review volume projections |
| DSA appeal out-of-court overturn rate | ~52% | External benchmark for system accuracy |
...
|---|---|---|
| **Classification** | "Use ML models per content type" | "Ensemble with calibrated confidence, confidence-zone routing, LLM fallback for edge cases, adversarial normalization before inference" |
| **Human review** | "Queue uncertain items for review" | "Priority-weighted queue with SLA timers, skill-based assignment, wellness caps enforced at system level, inter-rater reliability via calibration injection" |
| **Compliance** | "Log moderation decisions" | "Immutable audit log with cryptographic chaining, DSA transparency database integration, NCMEC automated reporting pipeline, forensic hold mechanism for evidence preservation" |
| **Adversarial** | "Block bad keywords" | "Pre-classifier normalization pipeline for obfuscation, ensemble diversity against adversarial examples, cross-platform signal sharing, report bombing defense via reporter credibility scoring" |
| **Scale** | "Add more GPUs" | "Predictive GPU scaling with warm pool, keyframe optimization for video, batch aggregation with latency ceilings, graceful degradation hierarchy (Levels 0-4)" |

---

## System Design Trade-off Matrix

Understanding the trade-offs in content moderation is what separates good answers from great ones. Every design decision involves tension between competing concerns:

| Trade-off | Option A | Option B | How to Decide |
|---|---|---|---|
| **Pre-pub latency vs. safety** | Block upload until classification completes | Allow upload, scan async | Category-dependent: CSAM requires pre-pub; spam can be async |
| **False positives vs. false negatives** | Aggressive thresholds (high recall) | Conservative thresholds (high precision) | Varies by category: CSAM = maximize recall; NSFW = balance precision |
| **GPU cost vs. classification depth** | Full ensemble on every item | Tiered routing (lightweight → full) | Tier by content complexity; full ensemble only for high-risk |
| **Reviewer throughput vs. quality** | Minimize review time per item | Maximize context per review | Provide pre-rendered context to reduce research time without rushing |
| **Automation rate vs. trust** | Maximize auto-decisions | Route more to human review | Higher automation for clear cases; human review for borderline |
| **Data sovereignty vs. review SLA** | Keep all data in-region | Allow cross-region for safety-critical | Safety-critical items (CSAM) override data residency constraints |
| **Model freshness vs. stability** | Frequent retraining | Stable long-lived models | Category-dependent: adversarial categories need weekly updates |
| **Transparency vs. adversarial advantage** | Publish all decision logic | Keep thresholds secret | Publish outcomes (DSA requires it); keep thresholds and model details internal |

### Follow-Up Deep Dive Questions

These questions test whether the candidate can think beyond the initial design into operational depth:

**On ML pipeline:**
- "Your hate speech model shows 95% precision on English text but 72% on Hindi. How do you address this without separate models for every language?"
- "An adversary discovers that adding an invisible Unicode character to every word bypasses your classifier. How fast can you patch this, and what's the blast radius?"

**On human review:**
- "Your reviewer pool in the EU region has a 40% turnover rate annually. How does the system account for quality degradation during onboarding periods?"
- "Two reviewers disagree on the same item — one says violation, one says allowed. How does the system resolve this at scale without creating a Slowest part of the process?"

**On compliance:**
- "A German regulator audits your NetzDG compliance and finds that 3% of clearly illegal content took > 26 hours to remove. How does the system help you diagnose whether this was a staffing, routing, or classification problem?"
- "The DSA requires you to report the automation rate in your transparency report. But your system uses ML scores to prioritize human review. Is a human-reviewed-but-ML-triaged decision 'automated' or 'human'?"

**On adversarial resilience:**
- "A coordinated group creates 10,000 accounts and uses each one once to post CSAM. By the time your account trust system scores them, the content has already been posted. How do you handle this?"
- "Your cross-platform hash sharing partner has a data breach and their hash database is leaked. What are the security implications for your system?"

---

## Architecture Whiteboard Checkpoints

During a 45-minute interview, the candidate should hit these milestones to demonstrate comprehensive understanding:

| Checkpoint | By Minute | What Should Be on the Whiteboard |
|---|---|---|
| Content types and modalities | 7 | Text, image, video, audio with different pipeline requirements identified |
| Ingest → classify → act flow | 12 | Event-driven pipeline with hash matching and ML classification in parallel |
| Confidence zones | 15 | Zone A/B/C routing explained with different handling per zone |
| Policy engine separation | 18 | Policy rules decoupled from ML models; hot-reloadable |
| Human review queue | 22 | Priority-weighted queue with SLA timers and skill-based assignment |
| Reviewer wellness | 25 | Exposure caps and mandatory breaks mentioned as system constraints |
| Appeals workflow | 28 | Multi-tier appeals with automated re-review first |
| Regulatory compliance | 32 | DSA transparency, NCMEC reporting, NetzDG SLAs |
| Adversarial handling | 36 | Normalization pipeline, ensemble diversity, CIB detection |
| Scaling and degradation | 40 | GPU scaling strategy, graceful degradation levels, surge handling |
| Observability | 43 | Model drift detection, queue health monitoring, compliance dashboards |
