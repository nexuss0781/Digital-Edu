...
|---|---|
| **Sigstore GA and npm provenance rollout** | Keyless signing becomes default for CI/CD publishes; provenance attestation shifts from opt-in to expected |
| **xz-utils backdoor (2024)** | Highlighted maintainer trust as attack surface; drives multi-maintainer approval for critical packages and behavioral analysis beyond static scanning |
| **PyPI Trusted Publishers** | OIDC-based publishing eliminates long-lived API tokens for CI/CD; establishes pattern other ecosystems follow |
| **SBOM mandates (US EO 14028, EU CRA)** | Registry-generated SBOMs become a compliance requirement, not a feature; drives automated SBOM-on-publish |
| **AI-assisted malware detection** | ML models supplement signature-based scanning; behavioral anomaly detection catches zero-day malicious packages that evade YARA rules |
| **Cargo sparse index protocol** | HTTP-based sparse index replaces git-clone for registry metadata; reduces fresh-install resolution time by 50%+ |
...
Unlike most systems where data is routinely updated, a package registry's core Rule that never changes is that published artifact bytes are permanently immutable. This isn't just a policy—it's an architectural commitment that enables content-addressable storage, CDN caching without invalidation complexity, lockfile reproducibility, and supply chain integrity verification. The entire trust model of downstream software depends on the guarantee that `package@1.2.3` today contains exactly the same bytes as `package@1.2.3` yesterday. Violating this Rule that never changes doesn't just cause bugs—it undermines the security of every application that depends on the package.

### 2. Dependency Resolution Is NP-Complete

Package dependency resolution—finding a compatible set of versions satisfying all constraints across a transitive dependency graph—is provably NP-complete (reducible to 3-SAT). Real-world dependency graphs with millions of packages and complex version constraints (ranges, exclusions, optional dependencies, peer dependencies) cannot be solved by simple greedy algorithms. Production resolvers use techniques from SAT solver research: conflict-driven clause learning (CDCL), unit propagation, backjumping, and Practical rule of thumb variable ordering. The PubGrub algorithm (used by Dart and Rust) provides human-readable error messages when resolution fails—a crucial UX consideration when version conflicts arise.
...
| **Greedy vs Complete Resolution** | Greedy resolution (fast, may miss valid solutions) | SAT-based complete resolution (correct, slower) | SAT-based with Practical rule of thumb ordering for common cases; timeout with greedy fallback for pathological graphs |
...
| **AI Malware Classifier** | ML models for behavioral anomaly detection beyond signature matching | Accuracy on zero-day threats, training data freshness, inference latency |

---

## Related Patterns

| System | Relationship | Shared Pattern |
|---|---|---|
| [Content Delivery Network](../2.3-cdn/) | CDN-first serving architecture | CDN-as-primary-infrastructure, edge caching, origin shield, stale-while-revalidate; both systems treat CDN as the primary serving tier with origin as fallback |
| [Distributed Object Storage](../2.5-distributed-object-storage/) | Content-addressed blob storage backend | Immutable blob storage, content-addressable keys, multi-region replication, garbage collection of orphaned objects |
| [Container Registry](../12.4-container-orchestration-platform/) | Sibling artifact registry pattern | Image manifests ↔ package metadata, image layers ↔ artifact tarballs, content-addressed storage, vulnerability scanning pipeline |
| [CI/CD Pipeline](../9.9-cicd-platform/) | Publish automation and provenance | OIDC keyless signing for CI/CD, build provenance attestation (SLSA), automated publish triggers, trusted publisher verification |
| [Web Crawler / Search Engine](../16.4-text-search-engine/) | Full-text search with relevance ranking | Inverted index for package search, BM25 ranking, popularity-weighted scoring, near-real-time index updates on publish |
| [Digital Certificate Authority](../12.12-certificate-transparency/) | Transparency log infrastructure | Merkle tree append-only log, certificate/package signing, third-party monitoring, tamper-evidence via cryptographic proofs |
| [API Gateway](../2.2-api-gateway/) | Rate limiting and authentication at the edge | Token-scoped authentication, per-client rate limiting, request routing by path (publish vs download vs search) |
| [Vulnerability Management Platform](../15.5-vulnerability-scanner/) | Security scanning pipeline | Async scan queue, verdict aggregation, CVE integration, vulnerability propagation through dependency graphs |
