...
| **Authentication events** | 1 year | ~100K/day | Account security, Brute Force (Checking every single possibility) detection |
...
### 7.1 Supply Chain Health Index

A composite metric measuring the overall health of the package ecosystem:

| Component Metric | Weight | Measurement | Healthy Threshold |
|---|---|---|---|
| **Provenance coverage** | 25% | % of weekly publishes with valid SLSA provenance | > 40% |
| **2FA adoption (critical packages)** | 20% | % of top-10K package maintainers with 2FA enabled | > 90% |
| **Scan coverage** | 15% | % of versions scanned within SLO (10 min) | > 99% |
| **Advisory response time** | 15% | Median time from CVE disclosure to registry advisory | < 24 hours |
| **Stale dependency ratio** | 10% | % of packages with outdated direct dependencies (>2 major versions behind) | < 30% |
| **Malware detection rate** | 10% | Known malware caught by automated scanning vs community reports | > 95% |
| **Token hygiene** | 5% | % of active tokens that are scoped and time-limited | > 60% |

### 7.2 Package Quality Signals

Surfaced on package detail pages and used in search ranking:

| Signal | How Computed | Update Frequency |
|---|---|---|
| **Maintenance score** | Recent publish activity, issue responsiveness, dependency freshness | Weekly |
| **Security score** | 2FA enabled, provenance present, no known vulnerabilities, no install scripts | On publish + daily advisory scan |
| **Popularity score** | Weekly downloads (log-scaled), dependent count, star/favorite count | Daily |
| **Documentation score** | README present and non-trivial, API docs, examples, changelog | On publish |
| **Type coverage** | TypeScript types available (bundled or DefinitelyTyped) | On publish |
| **License clarity** | SPDX-valid license declared, no license conflicts in dependencies | On publish |

---

## 8. Operational Runbooks

### 8.1 Runbook: Mass Malware Detection

**Trigger:** `> 10 packages quarantined in 1 hour` alert fires.

```
1. TRIAGE (first 5 minutes)
   - Check quarantine reasons: are they from same publisher? Same scanner? Same pattern?
   - If single publisher: likely compromised account → escalate to account security
   - If diverse publishers, same pattern: likely new malware variant → verify scanner accuracy

2. ASSESS FALSE POSITIVE RISK
   - Review quarantine details for 3-5 representative packages
   - Check if any quarantined packages are in top-10K by downloads
   - If high false positive suspicion: pause scanner, investigate rule that triggered

3. CONTAIN
   - If confirmed malware: ensure all affected versions are quarantined
   - Notify downstream dependents via advisory system
   - If compromised account: force credential reset, revoke all tokens
   - If coordinated attack: engage security incident response team

4. COMMUNICATE
   - Update status page if >50 packages affected
   - Publish ecosystem-wide advisory if attack pattern is novel
```

### 8.2 Runbook: CDN Origin Failover

**Trigger:** `Origin error rate > 50% for 2+ minutes` alert fires.

```
1. VERIFY
   - Confirm origin is actually down (not just monitoring false positive)
   - Check CDN is serving stale content (verify X-Stale-Response headers)
   - Check publish path: is it also affected?

2. ASSESS IMPACT
   - CDN hit rate: if >98%, impact is limited to cache misses only
   - Check: are newly published packages accessible? (likely not)
   - Check: are long-tail packages accessible? (depends on cache state)

3. MITIGATE
   - If primary origin: promote secondary origin to active
   - If database issue: verify replica promotion
   - Extend CDN stale-while-revalidate TTL to reduce origin pressure during recovery

4. RECOVER
   - Drain publish queue after origin recovery
   - Verify metadata consistency between primary and replica
   - Run integrity check on any packages published during failover window
```

### 8.3 Runbook: Scan Queue Backlog

**Trigger:** `Scan queue depth > 5,000 for 15+ minutes` alert fires.

```
1. DIAGNOSE
   - Check if specific scanner type is slow (malware? behavioral? SBOM?)
   - Check for scanner pod crashes or resource exhaustion
   - Check if publish rate is abnormally high (automated spam?)

2. MITIGATE
   - Scale scanner fleet (auto-scaling should handle, check if limits are hit)
   - If single scanner type: temporarily bypass and mark as "scan_pending"
   - Prioritize: ensure top-10K packages are scanned first (priority queue)

3. PREVENT
   - Review scanner performance metrics for degradation trends
   - Increase auto-scaling limits if growth has outpaced capacity
   - Consider incremental scanning for patch releases (scan diff only)
```

---

## 9. SLO Burn Rate Monitoring

| SLO | Error Budget (30 days) | Burn Rate Alert (1h window) | Burn Rate Alert (6h window) |
|---|---|---|---|
| **Download success 99.99%** | 2,000,000 failures/month | > 14.4× (page immediately) | > 6× (page) |
| **Publish success 99.9%** | 1,500 failures/month | > 14.4× (page) | > 6× (page) |
| **Metadata freshness 99.9%** | 1,500 stale events/month | > 6× (warn) | > 3× (warn) |
| **Scan completion 99%** | 15,000 delayed scans/month | > 6× (warn) | > 3× (warn) |
| **Search availability 99.5%** | 21,600 minutes downtime equivalent | > 6× (warn) | > 3× (warn) |

**Error budget policy:** When 50% of monthly error budget is consumed in the first week, freeze non-critical deployments and prioritize reliability work. When 80% is consumed, halt all feature deploys until budget recovers.
