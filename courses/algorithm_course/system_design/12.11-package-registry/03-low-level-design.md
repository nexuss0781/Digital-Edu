...
**Publish Pipeline (Step-by-step plan in plain English):**
...
**Download flow (Step-by-step plan in plain English):**
...
**Search ranking (Step-by-step plan in plain English):**
...
FUNCTION choose_next_package(solution, incompatibilities):
    // Practical rule of thumb: choose package that appears in most incompatibilities
    // (VSIDS-like Practical rule of thumb from SAT solving)
...
## 7. Unpublish and Yank Logic

The registry supports two levels of version removal with distinct semantics:

```
FUNCTION handle_unpublish_request(user, package_name, version_string):
    package = lookup_package(package_name)
    version = lookup_version(package.id, version_string)

    // Authorization check
    IF user NOT IN package.maintainers WITH role "owner":
        RETURN 403 "Only owners can unpublish"

    // Time window check (≤72 hours since publish)
    hours_since_publish = hours_between(version.published_at, now())
    IF hours_since_publish > 72:
        RETURN 403 "Cannot unpublish after 72 hours. Use deprecation instead."

    // Dependent check — block if any public package depends on this version
    dependents = get_direct_dependents(package_name, version_string)
    IF dependents.count > 0:
        RETURN 409 "Cannot unpublish: " + dependents.count +
                   " packages depend on this version. Use yank instead."

    // Execute unpublish
    BEGIN TRANSACTION
        mark_version_unpublished(version.id)
        remove_from_resolution_candidates(version.id)
        insert_audit_event(user.id, package.id, version.id, "unpublish")
    COMMIT

    // Async cleanup
    async invalidate_cdn_cache(package_name)
    async rebuild_materialized_metadata(package.id)
    // Note: artifact blob retained for 30 days (forensics + potential restore)

    RETURN 200 { "unpublished": package_name + "@" + version_string }

FUNCTION handle_yank_request(user, package_name, version_string, reason):
    // Yank = soft removal: version still resolvable if already in lockfile,
    // but excluded from new resolution queries
    package = lookup_package(package_name)
    version = lookup_version(package.id, version_string)
    ...
    BEGIN TRANSACTION
        UPDATE version SET is_yanked = TRUE, yank_reason = reason
        insert_audit_event(user.id, package.id, version.id, "yank",
            { reason: reason })
    COMMIT

    async invalidate_cdn_cache(package_name)
    async rebuild_materialized_metadata(package.id)
    async notify_dependent_maintainers(package_name, version_string, reason)

    RETURN 200 { "yanked": package_name + "@" + version_string }
```

### Yank vs Unpublish vs Deprecate Semantics

| Action | Artifact Available? | In Resolution? | In Lockfile? | Reversible? |
|---|---|---|---|---|
| **Published** | Yes | Yes | Yes | N/A |
| **Deprecated** | Yes | Yes (with warning) | Yes | Yes (undeprecate) |
| **Yanked** | Yes (direct URL) | No (excluded from new resolution) | Yes (existing lockfiles honored) | Yes (unyank) |
| **Unpublished** | No (after grace period) | No | Broken (lockfile integrity fails) | No |

---

## 8. Batch Metadata API

For dependency resolution, the client needs metadata for many packages. A batch API reduces round trips:

```
POST /registry/-/metadata/batch
Content-Type: application/json

{
    "packages": [
        { "name": "@scope/pkg-a", "constraint": "^2.0.0" },
        { "name": "pkg-b", "constraint": ">=1.5.0 <3.0.0" },
        { "name": "pkg-c" }
    ],
    "format": "abbreviated"
}
...
**Response:** Array of abbreviated metadata objects, one per requested package.

```
FUNCTION handle_batch_metadata(request):
    results = {}
    // Parallelize lookups with bounded concurrency
    parallel_pool = new BoundedPool(max_concurrency=50)

    FOR EACH pkg_request IN request.packages:
        parallel_pool.submit(FUNCTION():
            metadata = get_metadata(pkg_request.name, request.format)
            IF pkg_request.constraint:
                // Filter to only matching versions (saves bandwidth)
                metadata.versions = filter_versions(metadata.versions,
                    pkg_request.constraint)
            results[pkg_request.name] = metadata
        )

    parallel_pool.await_all()
...
This reduces metadata fetch from O(n) HTTP requests to O(1) for the common case where all needed packages are known upfront (e.g., from a partially resolved dependency tree).
