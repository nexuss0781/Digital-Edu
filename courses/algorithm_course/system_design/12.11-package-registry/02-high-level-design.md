...
## 6. Mirroring and Private Registry Architecture

Production registries must support two additional read paths: read-through mirrors (for latency/availability) and private registries (for organizational packages).

```mermaid
flowchart TB
    subgraph Public["Public Registry (Authoritative)"]
        PUB_API["Public API"]
        PUB_CDN["Public CDN"]
        PUB_BLOB[("Blob Storage")]
        PUB_META[("Metadata DB")]
    end

    subgraph Mirror["Registry Mirror"]
        M_PROXY["Mirror Proxy"]
        M_CACHE[("Local Cache")]
        M_SYNC["Sync Worker"]
    end

    subgraph Private["Enterprise Private Registry"]
        P_API["Private API"]
        P_BLOB[("Private Blob Store")]
        P_META[("Private Metadata")]
        P_PROXY["Upstream Proxy"]
    end

    subgraph Dev["Developer / CI"]
        CLI["Package Manager CLI"]
    end

    CLI -->|"install @org/private-pkg"| P_API
    CLI -->|"install public-pkg"| P_PROXY
    P_PROXY -->|cache miss| PUB_CDN
    P_API --> P_META
    P_API --> P_BLOB

    CLI -->|"install from mirror"| M_PROXY
    M_PROXY -->|cache miss| PUB_CDN
    M_SYNC -->|"replicate changes"| PUB_API

    PUB_API --> PUB_META
    PUB_API --> PUB_BLOB

    classDef public fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef mirror fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef private fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef dev fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class PUB_API,PUB_CDN,PUB_BLOB,PUB_META public
    class M_PROXY,M_CACHE,M_SYNC mirror
    class P_API,P_BLOB,P_META,P_PROXY private
    class CLI dev
```

### Mirror Synchronization Protocol

Mirrors replicate the public registry for offline access, network isolation, or geographic latency reduction:

| Sync Mode | Description | Latency | Storage Cost |
|---|---|---|---|
| **Full replication** | Copy all packages and versions | Minutes (initial sync: days) | Full registry size (~8 TB) |
| **On-demand pull-through** | Cache packages on first request, serve cached on subsequent | First request: origin latency; subsequent: local | Proportional to usage |
| **Selective sync** | Replicate only packages matching configured scopes/patterns | Configurable | Subset of registry |
| **Change stream** | Subscribe to publish events, replicate incrementally | Near-real-time (seconds) | Full registry, kept current |

The change stream approach uses a **replication log** — an ordered sequence of publish events that mirrors consume:

```
FUNCTION mirror_sync_loop(mirror, last_sequence):
    WHILE TRUE:
        // Fetch changes since last processed sequence number
        changes = public_registry.get_changes(since=last_sequence, limit=1000)

        FOR EACH change IN changes:
            IF change.type == "version_published":
                // Download and cache the new version metadata + artifact
                metadata = fetch_metadata(change.package_name)
                artifact = fetch_artifact(change.artifact_hash)
                mirror.store(metadata, artifact)

            ELSE IF change.type == "version_yanked":
                mirror.mark_yanked(change.package_name, change.version)

            ELSE IF change.type == "advisory_published":
                mirror.update_advisory(change.advisory)

            last_sequence = change.sequence
            mirror.save_checkpoint(last_sequence)

        IF changes IS EMPTY:
            sleep(POLL_INTERVAL)  // 30 seconds
```

### Private Registry — Scope-Based Routing

Enterprise environments use private registries for internal packages alongside the public registry. The package manager routes requests based on scope:

```
FUNCTION resolve_registry(package_name, config):
    // Scoped package → check scope-to-registry mapping
    IF is_scoped(package_name):
        scope = extract_scope(package_name)
        IF scope IN config.scope_registries:
            RETURN config.scope_registries[scope]

    // Fallback chain: private proxy → public registry
    RETURN config.default_registry  // e.g., private proxy that falls through to public
```

This scope-based routing is the **primary defense against dependency confusion**: private packages use organizational scopes (`@myorg/utils`), ensuring they can never collide with public package names.
...
|---|---|---|
| **Immutability by default** | Published artifact bytes never change; mutable metadata (deprecation, advisories) is separate | Enables content-addressing, CDN infinite TTL, lockfile reproducibility |
| **Separation of concerns (read/write)** | Write path is CP (strong consistency); read path is AP (CDN-served, eventually consistent) | Independent scaling and optimization for each path |
| **Defense in depth** | Multiple independent security layers: auth → validation → scanning → provenance → transparency log | No single bypass compromises the entire supply chain |
| **Fail-open for reads, fail-closed for writes** | CDN serves stale content during outages; publish rejects on any uncertainty | Maximizes read availability while protecting write integrity |
| **Content-addressed everything** | Artifacts keyed by hash, integrity verified end-to-end | Deduplication, tamper detection, and cacheability from one primitive |
| **Async by default for non-critical paths** | Security scanning, download counting, search indexing, webhook delivery | Keeps publish latency low; bounded eventual consistency for secondary concerns |
