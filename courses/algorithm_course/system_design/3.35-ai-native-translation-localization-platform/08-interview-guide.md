...
### Advanced Topic 5: Hybrid NMT + LLM Cascade Architecture
- When to use NMT draft → LLM refinement vs direct LLM
- Cost-quality trade-off analysis (cascade saves 30-50% vs direct LLM)
- Self-hosted translation LLMs (Tower-13B) vs cloud APIs
- LoRA adapter hot-swapping for domain-specific translation

### Advanced Topic 6: LLM Translation Hallucination Detection
- Multi-signal detection: QE, alignment, entailment, entity preservation
- Why hallucination detection is mandatory for high-stakes content
- False positive rates and the cost of over-flagging
- Hallucination rate differences across language pairs (low-resource = higher risk)

---

## Case Study: From Single-Engine to Multi-Engine Orchestration

**Scenario:** A global software company translates 30M words/month across 45 language pairs using a single NMT engine. Human edit rates average 55% and COMET scores average 0.76.

**Challenge:** Translation costs are $0.04/word blended (NMT + heavy human post-editing), totaling $1.2M/month. Quality is inconsistent — marketing content is poor, but technical docs are acceptable.

**Solution architecture:**
1. **Content classification** categorizes each segment (technical, marketing, legal, UI string)
2. **Engine routing** sends technical content to NMT (strong baseline), marketing to LLM, UI strings to NMT
3. **QE-based routing** auto-approves NMT segments scoring >0.85 COMET, routes rest to light MTPE
4. **TM leverage** — historical translations reduce new translation volume by 42%
5. **Adaptive learning** — human corrections feed back to improve NMT within 24 hours

**Results:**

| Metric | Before | After | Change |
|--------|--------|-------|--------|
| Human edit rate | 55% | 28% | -49% |
| COMET score average | 0.76 | 0.84 | +10.5% |
| Cost per word | $0.04 | $0.012 | -70% |
| Monthly translation cost | $1.2M | $360K | -70% |
| Turnaround time p95 | 72 hours | 18 hours | -75% |

**Key lesson:** The biggest cost savings came not from better MT engines but from intelligent routing — sending each content type to the right engine and using QE to minimize unnecessary human review.

---

## Case Study: Handling LLM Hallucinations in Medical Translation

**Scenario:** A pharmaceutical company uses LLM translation for patient information leaflets across 28 languages. During quality audits, reviewers discover that 3.2% of LLM-translated segments contain hallucinated content — including altered dosage instructions and fabricated side effects.

**Root cause analysis:**
- LLM hallucination rate was highest for low-resource languages (Thai, Vietnamese, Arabic) at 5-8%
- Numeric values (dosages, frequencies) were altered in 1.5% of segments
- The QE model alone missed 40% of hallucinations because the translations were fluent (high COMET scores)

**Solution:**
1. **Multi-signal hallucination detector** combining QE, semantic entailment, entity preservation, and length ratio
2. **Mandatory human review** for all medical content regardless of QE score
3. **Entity preservation gate** — segments where numbers/units differ from source are always flagged
4. **NMT-only mode** for critical safety content (dosage tables, contraindications)
5. **Self-hosted Tower-13B** for non-critical sections, eliminating third-party data exposure

**Results:** Hallucination rate dropped from 3.2% to 0.1% (detected and corrected before delivery). Zero patient safety incidents post-implementation.
