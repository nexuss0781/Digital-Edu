...
## Insight 5: The NMT-Draft-Then-LLM-Refine Cascade Achieves Near-LLM Quality at Near-NMT Cost

**Category:** Cost Optimization
**One-liner:** Instead of choosing between cheap NMT and expensive LLM, the cascaded pipeline uses NMT for a fast draft then LLM to refine style and fluency — achieving COMET 0.86-0.89 at 30-50% lower cost than direct LLM translation.

**Why it matters:** Direct LLM translation is the highest-quality option but costs $30-80 per million words. NMT is 10x cheaper but produces lower fluency for creative content. The cascade pipeline asks the LLM to refine an NMT draft rather than translate from scratch, reducing token usage by 40-60% (edit instructions are shorter than full translations). The NMT draft also serves as an immediate fallback if the LLM refinement step fails or times out. This pattern mirrors the evolution at companies like Unbabel, where the Tower model family (7B-13B parameters fine-tuned on translation) demonstrated that smaller specialized LLMs can match general-purpose LLMs while being self-hostable at $5-15 per million words. The cascade also naturally reduces hallucination risk — the LLM edits an existing translation rather than generating one from scratch, constraining it closer to the source meaning.
...
## Insight 14: Multi-Signal Hallucination Detection Is Mandatory for LLM Translation in High-Stakes Domains

**Category:** System Modeling
**One-liner:** QE scores alone miss 40% of LLM hallucinations because fluent-but-wrong translations score high on COMET -- detecting hallucinations requires ensembling QE with source-target alignment, semantic entailment, and entity preservation signals.

**Why it matters:** LLM-based translation introduces a failure mode absent from NMT: the model can generate fluent, grammatically perfect translations that alter the source meaning or fabricate content entirely. This is especially dangerous for medical (altered dosage instructions), legal (changed contractual terms), and financial content. COMET-based QE scores fluency and adequacy together, so a hallucinated translation that reads well scores high despite being wrong. Multi-signal detection combines QE with cross-attention alignment analysis (detecting dropped or added content), natural language inference (verifying the translation entails the source meaning), entity preservation checking (numbers, dates, names must match), and length ratio anomaly detection. In production, hallucination rates for LLM translation range from 0.1% (high-resource European pairs) to 5-8% (low-resource languages like Thai or Vietnamese), making detection gates essential before auto-approval of any LLM-translated content.

---

## Insight 15: RAG-Augmented Translation Turns Few-Shot Context into a 0.08-0.15 COMET Improvement Without Fine-Tuning

**Category:** Caching
**One-liner:** Retrieving TM fuzzy matches, glossary terms, style guide rules, and domain corpus examples as LLM prompt context improves COMET scores by 0.08-0.15 -- matching fine-tuned model quality without any training.

**Why it matters:** Fine-tuning LLMs per customer domain is expensive, slow, and requires careful data curation. RAG-augmented translation achieves similar quality improvements by dynamically retrieving relevant context at inference time. TM fuzzy matches serve as few-shot translation examples (showing the LLM how similar sentences were previously translated), glossary terms enforce terminology consistency, and style guide excerpts maintain brand voice. The cost is 40-55% more tokens per request (context injection), but this is far cheaper than maintaining fine-tuned models per customer. The approach is also immediately adaptive -- when TM is updated with new approved translations, the next similar segment automatically benefits without any model retraining. The key architectural requirement is a fast retrieval pipeline: TM lookup, glossary matching, and style guide retrieval must all complete within 100ms to avoid adding noticeable latency to the translation pipeline.
