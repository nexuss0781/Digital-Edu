...
|-------|---------|---------|----------|
| **translation-jobs** | Async job processing | Work queue | Per-project FIFO (First-In-First-Out, like a line at a store) |
...
## Hybrid NMT + LLM Refinement Pipeline

The cascaded pipeline represents the 2025-era evolution from "NMT OR LLM" to "NMT THEN LLM":

```mermaid
flowchart LR
    subgraph Stage1["Stage 1: Draft"]
        Source["Source<br/>Segment"]
        NMT["NMT Engine<br/>(fast draft)"]
    end

    subgraph Stage2["Stage 2: Refine"]
        LLMRefine["LLM Refinement<br/>(style + fluency)"]
        RAGContext["RAG Context<br/>(TM + glossary +<br/>style guide)"]
    end
    ...
    subgraph Stage4["Stage 4: Route"]
        AutoApprove["Auto-<br/>Approve"]
        MTPE["Human<br/>MTPE"]
    end

    Source --> NMT
    NMT --> LLMRefine
    RAGContext --> LLMRefine
    LLMRefine --> QE
    LLMRefine --> Hallucination
    QE --> AutoApprove
    QE --> MTPE
    Hallucination -->|"flagged"| MTPE

    classDef draft fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef refine fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef validate fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef route fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class Source,NMT draft
    class LLMRefine,RAGContext refine
    class QE,Hallucination validate
    class AutoApprove,MTPE route
```

**Why cascade rather than direct LLM translation?**
- NMT draft is 10x cheaper and provides a strong baseline
- LLM refinement processes shorter edit instructions (not full translation), reducing token usage by 40-60%
- Combined COMET scores (0.86-0.89) match direct LLM translation at 30-50% lower cost
- NMT draft provides a fallback if LLM refinement fails or times out

**When to skip the cascade:**
- Simple UI strings where NMT alone scores >0.90 COMET — skip LLM refinement entirely
- Creative/marketing content where NMT draft quality is too low — use direct LLM translation
- Real-time translation where cascade latency is unacceptable — NMT only with speculative LLM
...
- [x] **Cascade vs Direct:** NMT+LLM cascade for quality-cost optimization, direct for creative content
