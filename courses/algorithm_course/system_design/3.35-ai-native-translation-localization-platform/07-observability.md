...
## Translation Quality Observability

### Hallucination Rate Tracking

| Metric | Description | Alert Condition |
|--------|-------------|-----------------|
| **Hallucination Rate** | % segments flagged by hallucination detector | >5% of LLM translations |
| **Hallucination by Language** | Per-language-pair hallucination rates | >2x average for any pair |
| **Entity Drop Rate** | Numbers/names missing in translation | >1% of segments |
| **Length Ratio Anomalies** | Translation length significantly different from expected | >3 standard deviations |
| **Semantic Entailment Score** | Source-translation entailment probability | Mean <0.85 |
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  COMET Score by Engine           Cost Efficiency                    │
│  ┌──────────────────────┐       ┌──────────────────────┐           │
│  │ NMT (DeepL):  0.85   │       │ NMT: $0.005/word     │           │
│  │ NMT+LLM:     0.88   │       │ NMT+LLM: $0.012/word │           │
│  │ Direct LLM:  0.89   │       │ Direct: $0.030/word   │           │
│  │ Tower-13B:   0.86   │       │ Tower: $0.008/word    │           │
│  └──────────────────────┘       └──────────────────────┘           │
│                                                                      │
│  Quality per Dollar              Hallucination Rate                 │
│  ┌──────────────────────┐       ┌──────────────────────┐           │
│  │ NMT+LLM:     73.3   │       │ NMT: 0.1% (near zero)│           │
│  │ Tower-13B:   107.5   │       │ NMT+LLM: 0.8%        │           │
│  │ NMT only:   170.0   │       │ Direct LLM: 2.1%      │           │
│  │ Direct LLM:  29.7   │       │ Tower-13B: 1.5%        │           │
│  └──────────────────────┘       └──────────────────────┘           │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
|--------|-------------|--------|
| **Learning Loop Latency** | Time from correction to model update | <24 hours |
| **Repeat Error Rate** | Same error recurring after correction | <5% |
| **QE Score Improvement** | COMET improvement over 30-day window | +0.01/month |
| **TM Hit Rate Trend** | TM coverage increasing as TM grows | +1%/month |
| **Human Edit Distance Trend** | Average edit distance decreasing over time | -2%/month |
