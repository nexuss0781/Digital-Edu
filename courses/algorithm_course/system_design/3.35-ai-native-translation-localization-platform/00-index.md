...
| **Human Override** | Professional translators review flagged translations; quality scores gate automated publishing | Domain expert |
...
- Human translators are a Slowest part of the process—need intelligent routing
...
- [3.15 RAG System](../3.15-rag-system/00-index.md) - RAG-augmented translation retrieves domain corpora, style guides, and approved translations as few-shot examples for LLM prompt injection
- [3.21 LLM Gateway](../3.21-llm-gateway-prompt-management/00-index.md) - LLM routing, cost optimization, and prompt versioning for translation prompt templates
- [3.23 LLM Inference Engine](../3.23-llm-inference-engine/00-index.md) - Efficient LLM serving with batching, PagedAttention, and speculative decoding for translation workloads
- [3.25 AI Observability & LLMOps](../3.25-ai-observability-llmops-platform/00-index.md) - Translation quality drift detection, QE model monitoring, COMET score tracking
- [3.31 AI-Native Document Processing](../3.31-ai-native-document-processing-platform/00-index.md) - Content extraction from DOCX, PDF, and image-based documents before translation
- [3.22 AI Guardrails & Safety](../3.22-ai-guardrails-safety-system/00-index.md) - Hallucination detection for LLM translations, prompt injection defense, output validation
- [3.14 Vector Database](../3.14-vector-database/00-index.md) - HNSW/IVF indexes powering TM fuzzy matching at 500M+ segments with sub-50ms latency
- [2.6 Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) - Translation job orchestration, batch processing, and deadline-aware scheduling

---

## Recent Industry Evolution (2025-2026)

### Hybrid NMT + LLM Refinement Pipelines

The dominant architectural pattern has shifted from "NMT OR LLM" to a cascaded pipeline: NMT produces a fast draft, then an LLM post-edits for fluency, style, and terminology adherence. This approach achieves near-LLM quality at near-NMT cost, since the LLM processes shorter edit instructions rather than full translations. Tower-family models (7B-13B parameters, fine-tuned on translation tasks) demonstrate that smaller specialized LLMs can match general-purpose LLMs at a fraction of inference cost.

### xCOMET and Word-Level Quality Estimation

Quality estimation has advanced beyond sentence-level scoring. xCOMET provides word-level and sentence-level error annotations, identifying specific error types (accuracy, fluency, terminology, style). GEMBA-MQM uses LLMs themselves as quality estimators, prompting them to identify MQM error categories with surprisingly high correlation to human judgments. Production systems now ensemble multiple QE signals (COMET, LLM-based evaluation, TM match confidence) for more robust quality prediction.

### RAG-Augmented Translation

Rather than relying solely on prompt engineering, leading platforms now use retrieval-augmented generation for translation: retrieving relevant TM segments, domain glossaries, previously approved translations, and style guide examples to inject as context into LLM translation prompts. This dramatically improves terminology consistency and domain adaptation without fine-tuning.

### LLM Translation Hallucination

A critical safety concern: LLM-based translation can produce fluent but factually incorrect translations ("hallucinations"), especially for low-resource languages and specialized domains. Detection systems combine QE scoring with source-target alignment verification and semantic entailment checking. For high-stakes content (medical, legal, financial), hallucination detection gates are mandatory before auto-approval.

### Self-Hosted Translation LLMs

The economics of LLM-based translation shift dramatically with self-hosted models. Tower-13B achieves COMET scores of 0.84-0.87 (competitive with GPT-4's 0.86-0.89) at $5-15 per million words versus $30-80 for cloud LLM APIs. Organizations translating 50M+ words/month increasingly deploy on-premises or VPC-hosted translation LLMs for both cost and data sovereignty reasons.
