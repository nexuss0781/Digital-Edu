...
## Self-Hosted Translation LLM Scaling

### Why Self-Host Translation LLMs

| Factor | Cloud LLM API | Self-Hosted (Tower-13B) |
|--------|---------------|------------------------|
| **Cost per 1M words** | $30-80 | $5-15 |
| **Latency p95** | 500ms-3s (network + inference) | 200ms-1s (inference only) |
| **Data sovereignty** | Content sent to third party | All data stays in VPC |
| **Throughput control** | Rate limited by provider | Limited only by GPU capacity |
| **Model customization** | Prompt engineering only | LoRA fine-tuning per domain |
| **Availability** | Dependent on provider | Fully controlled |

### Self-Hosted LLM Deployment Architecture

```mermaid
flowchart TB
    subgraph Inference["Self-Hosted LLM Cluster"]
        LB["Load<br/>Balancer"]
        GPU1["GPU Node 1<br/>Tower-13B"]
        GPU2["GPU Node 2<br/>Tower-13B"]
        GPU3["GPU Node 3<br/>Tower-13B"]
    end

    subgraph Optimization["Serving Optimizations"]
        Batch["Continuous<br/>Batching"]
        KVCache["KV Cache<br/>Management"]
        Quant["INT8/INT4<br/>Quantization"]
    end

    subgraph Adapters["Domain Adapters"]
        LoRA1["Legal<br/>LoRA"]
        LoRA2["Medical<br/>LoRA"]
        LoRA3["Marketing<br/>LoRA"]
        Base["Base<br/>Weights"]
    end

    subgraph Fallback["Fallback"]
        CloudLLM["Cloud LLM<br/>API"]
    end
    ...
    classDef fallback fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class LB,GPU1,GPU2,GPU3 inference
    class Batch,KVCache,Quant optimization
    class LoRA1,LoRA2,LoRA3,Base adapter
    class CloudLLM fallback
```

### GPU Capacity Planning for Self-Hosted LLMs

| Model | GPU Memory | Throughput (segments/sec) | Cost/Month (3 GPUs) |
|-------|-----------|--------------------------|---------------------|
| Tower-7B (INT8) | 8GB per GPU | 15-25 | $4,500 |
| Tower-13B (INT8) | 16GB per GPU | 8-15 | $9,000 |
| Tower-13B (FP16) | 28GB per GPU | 5-10 | $15,000 |
| Fine-tuned Llama-70B | 80GB per GPU (multi-GPU) | 2-5 | $45,000 |

### LoRA Adapter Hot-Swapping

```
ALGORITHM ServeWithLoRA(segment, domain, base_model)
  -- Select appropriate LoRA adapter based on content domain
  adapter = LoRARegistry.get(domain)

  IF adapter IS NOT NULL THEN
    -- Merge LoRA weights with base model (cached in GPU memory)
    model = MergeLoRA(base_model, adapter)
  ELSE
    model = base_model
  END IF

  -- Multiple LoRA adapters can share the same base model
  -- Only adapter weights need to be swapped (typically 1-5% of base parameters)
  -- Hot-swap latency: <10ms (adapter weights are small)

  RETURN model.generate(segment)
END ALGORITHM
```
