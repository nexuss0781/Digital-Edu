...
## Critical Component #4: LLM Translation Hallucination Detection

### Why This is Critical

LLM-based translation can produce fluent but factually incorrect output — "hallucinations" where the translation sounds natural but doesn't convey the source meaning. This is especially dangerous for:
1. **Medical content** — wrong drug dosage instructions
2. **Legal content** — altered contractual obligations
3. **Financial content** — incorrect figures or terms
4. **Low-resource languages** — where LLM training data is sparse
...
flowchart TB
    subgraph Input["Input"]
        Source["Source Text"]
        Translation["LLM<br/>Translation"]
    end

    subgraph Detection["Multi-Signal Detection"]
        QECheck["QE Score<br/>Check"]
        AlignCheck["Source-Target<br/>Alignment"]
        EntailCheck["Semantic<br/>Entailment"]
        LengthCheck["Length Ratio<br/>Anomaly"]
        NumCheck["Number/Entity<br/>Preservation"]
    end

    subgraph Scoring["Hallucination Score"]
        Ensemble["Weighted<br/>Ensemble"]
        Threshold{{"Score ><br/>Threshold?"}}
    end

    subgraph Action["Action"]
        Pass["Pass to<br/>QE Pipeline"]
        Flag["Flag for<br/>Human Review"]
        Reject["Re-translate<br/>with NMT"]
    end

    Source --> QECheck
    Translation --> QECheck
    Source --> AlignCheck
    Translation --> AlignCheck
    Source --> EntailCheck
    Translation --> EntailCheck
    Source --> LengthCheck
    Translation --> LengthCheck
    Source --> NumCheck
    Translation --> NumCheck

    QECheck --> Ensemble
    AlignCheck --> Ensemble
    EntailCheck --> Ensemble
    LengthCheck --> Ensemble
    NumCheck --> Ensemble

    Ensemble --> Threshold
    Threshold -->|"safe"| Pass
    Threshold -->|"suspicious"| Flag
    Threshold -->|"hallucinated"| Reject

    classDef input fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef detection fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef scoring fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef action fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class Source,Translation input
    class QECheck,AlignCheck,EntailCheck,LengthCheck,NumCheck detection
    class Ensemble,Threshold scoring
    class Pass,Flag,Reject action
```
...
|--------|--------|--------|---------|
| **QE Score** | COMET/xCOMET | 0.25 | General quality issues |
| **Source-Target Alignment** | Cross-attention scores from encoder | 0.25 | Dropped or added content |
| **Semantic Entailment** | NLI model (source entails translation?) | 0.20 | Meaning-altering hallucinations |
| **Length Ratio** | Character/word count ratio vs expected | 0.10 | Omissions or additions |
| **Entity Preservation** | Numbers, dates, names match | 0.20 | Critical data corruption |

### Hallucination Algorithm

```
ALGORITHM DetectHallucination(source, translation, language_pair)
  INPUT:
    source: source text
    translation: LLM translation output
    language_pair: source-target language pair
  OUTPUT:
    hallucination_score: [0, 1] (0 = safe, 1 = hallucinated)
    flagged_spans: list of suspicious translation spans

  -- Signal 1: QE-based detection
  qe_score = COMETScore(source, translation)
  qe_signal = 1.0 - qe_score  -- Low QE = high hallucination risk

  -- Signal 2: Alignment coverage
  alignment = CrossAttentionAlignment(source, translation)
  uncovered_source = FindUnalignedTokens(alignment, side="source")
  uncovered_target = FindUnalignedTokens(alignment, side="target")
  alignment_signal = (LEN(uncovered_source) + LEN(uncovered_target)) /
                     (LEN(source_tokens) + LEN(translation_tokens))

  -- Signal 3: Semantic entailment
  entailment_score = NLIModel.predict(source, translation)
  entailment_signal = 1.0 - entailment_score.entailment_prob

  -- Signal 4: Length ratio anomaly
  expected_ratio = GetExpectedLengthRatio(language_pair)
  actual_ratio = LEN(translation) / LEN(source)
  length_signal = ABS(actual_ratio - expected_ratio) / expected_ratio

  -- Signal 5: Entity preservation
  source_entities = ExtractEntities(source)  -- numbers, dates, names
  target_entities = ExtractEntities(translation)
  entity_signal = 1.0 - (INTERSECTION(source_entities, target_entities) /
                          MAX(LEN(source_entities), 1))

  -- Weighted ensemble
  hallucination_score = (0.25 * qe_signal +
                         0.25 * alignment_signal +
                         0.20 * entailment_signal +
                         0.10 * CLAMP(length_signal, 0, 1) +
                         0.20 * entity_signal)

  -- Identify flagged spans
  flagged_spans = uncovered_target + MissingEntities(source_entities, target_entities)

  RETURN {hallucination_score, flagged_spans}
END ALGORITHM
```

---

## Critical Component #5: RAG-Augmented Translation Context

### Why This is Critical

LLM translation quality depends heavily on context. Without domain-specific context, the LLM translates "bank" as financial institution when the document is about river geography. RAG-augmented translation retrieves relevant TM segments, glossary terms, style guide excerpts, and previously approved translations to inject as few-shot examples in the LLM prompt.

### RAG Context Assembly

```
ALGORITHM AssembleTranslationContext(segment, project)
  INPUT:
    segment: source text to translate
    project: project with TM, glossary, style guide
  OUTPUT:
    context: assembled context for LLM prompt

  context = {}

  -- Step 1: Retrieve TM fuzzy matches as translation examples
  tm_matches = TMService.query(segment.text, project.tm_store_id, min_score=0.60)
  context.examples = tm_matches[0:3]  -- Top 3 fuzzy matches as few-shot

  -- Step 2: Retrieve glossary terms present in source
  terms = GlossaryService.findTerms(segment.text, project.glossary_id)
  context.glossary = terms

  -- Step 3: Retrieve style guide rules for content type
  style_rules = StyleGuideService.getRules(project.style_guide_id, segment.content_type)
  context.style = style_rules

  -- Step 4: Retrieve surrounding sentences for document context
  context.preceding = GetPrecedingSegments(segment, count=2)
  context.following = GetFollowingSegments(segment, count=1)

  -- Step 5: Retrieve domain-specific corpus examples (if available)
  IF project.domain_corpus IS NOT NULL THEN
    domain_examples = VectorSearch(segment.embedding, project.domain_corpus, top_k=2)
    context.domain = domain_examples
  END IF

  -- Step 6: Assemble prompt with all context
  prompt = BuildTranslationPrompt(segment, context)
  -- Includes: system instructions, glossary terms, TM examples,
  -- style rules, document context, and the segment to translate
...
| Context Type | COMET Improvement | Cost Impact | When to Use |
|--------------|-------------------|-------------|-------------|
| TM few-shot examples | +0.03-0.05 | +15% tokens | Always (if TM available) |
| Glossary injection | +0.02-0.04 | +5% tokens | When terms detected |
| Document context | +0.04-0.08 | +20% tokens | Creative/legal content |
| Domain corpus | +0.02-0.03 | +10% tokens | Specialized domains |
| Style guide | +0.01-0.02 | +5% tokens | Brand-critical content |
| **Combined** | **+0.08-0.15** | **+40-55% tokens** | **Premium translation tier** |

---

## Performance Optimization Summary

| Component | Slowest part of the process | Optimization | Expected Improvement |
|-----------|------------|--------------|---------------------|
...
| Hallucination Detection | Multi-signal latency | Parallel signal computation | <200ms added latency |
| RAG Context Assembly | Retrieval latency | Pre-computed context cache | <100ms assembly time |
