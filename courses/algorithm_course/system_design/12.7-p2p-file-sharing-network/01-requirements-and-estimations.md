...
| **Distributed Storage** | P2P file sharing is transfer-focused; persistent distributed storage (like IPFS) has different requirements |
| **Real-time Communication** | P2P file sharing is not designed for real-time audio/video; latency tolerance is high |
...
P2P file sharing exhibits a unique relationship with CAP theorem because it has no central data store:

### DHT (Discovery Layer)

**AP — Availability over strict Consistency**

The DHT prioritizes availability: every node can process queries regardless of partitions. Under network partition, nodes in each partition continue answering queries with their local routing tables. This means peer lists may be incomplete (stale or partial) but are never unavailable. When partitions heal, routing tables self-repair through normal lookup traffic. The announce TTL (30 minutes) ensures stale peer entries naturally expire.

### Swarm (Data Transfer Layer)

**No central state to be consistent about.** Each peer maintains its own bitfield (local state), and piece data is immutable (content-addressed). There is no "write" operation that needs consensus — publishing a new piece is idempotent (the hash determines correctness). Two peers never disagree about the content of a piece — if the hash matches, the data is correct.
...
|---|---|
| **Asymmetric bandwidth** | Most peers have 3-10× more download than upload bandwidth; upload is the scarce resource that determines swarm health |
| **NAT prevalence** | 60-80% of peers are behind NAT; direct connectivity requires traversal techniques |
| **Peer churn is constant** | Peers join and leave continuously; system must be resilient by design, not by exception handling |
| **No persistent identity** | Peer IDs change between sessions; long-term reputation is impractical without centralized identity |
| **Content is immutable** | Once published, torrent content never changes; new content = new info-hash |
| **Piece hashing is the sole trust anchor** | All data integrity derives from cryptographic hash verification; no trusted third party |
| **Global internet heterogeneity** | Peers span diverse network conditions: 1 Mbps mobile to 10 Gbps datacenter; protocol must adapt |
| **Upload is the scarce resource** | Home ISPs typically offer 5-20× less upload than download bandwidth; upload allocation is the core optimization problem |
| **Protocol must be backward compatible** | v2 enhancements must interoperate with v1 clients through hybrid torrents; breaking backward compatibility fragments the network |

---

## Scenario Analysis: Content Size Impact

| File Size | Pieces (2 MiB) | Metadata Size | Time to First Piece | Typical Download Time (healthy swarm) |
|---|---|---|---|---|
| 10 MB | 5 | ~200 bytes | < 1 second | < 10 seconds |
| 100 MB | 50 | ~1.6 KB | < 2 seconds | 1-5 minutes |
| 1 GB | 512 | ~16 KB | < 5 seconds | 10-30 minutes |
| 4 GB | 2,048 | ~64 KB | < 10 seconds | 30-120 minutes |
| 50 GB | 25,600 | ~800 KB | < 15 seconds | 4-12 hours |
| 100 GB | 51,200 | ~1.6 MB | < 20 seconds | 8-24 hours |

---

## Failure Mode Impact Analysis

| Failure | Detection Time | User Impact | Automatic Recovery? |
|---|---|---|---|
| **Single peer disconnects** | Immediate (TCP/uTP timeout) | Negligible — other peers continue serving | Yes — PEX + DHT replace peer within seconds |
| **All trackers down** | 1-2 seconds (connection timeout) | Slight delay in initial peer discovery | Yes — DHT + PEX provide full fallback |
| **DHT network partition** | 10-30 seconds (lookup fails) | Slower peer discovery in affected partition | Yes — heals automatically when partition resolves |
| **Only seeder leaves** | Detected when piece unavailable | Download stalls for missing pieces | Partial — resumes if leechers collectively have all pieces |
| **ISP blocks P2P traffic** | Minutes (connection failures) | Cannot connect to most peers | Partial — MSE encryption may bypass; VPN required otherwise |
| **Content poisoning (bad data)** | Immediate (hash verification) | Wasted bandwidth for corrupt pieces | Yes — peer banned, piece re-requested from different peer |
| **Disk full on client** | Immediate (write failure) | Download pauses | No — requires user action to free space |
| **NAT mapping expires** | 30-120 seconds (incoming connections fail) | Reduced peer connectivity | Yes — UPnP refresh or new outbound connections |
| **Piece extinction (no peer has it)** | Gradual (piece availability drops to 0) | Download permanently stalled for that piece | No — requires new seeder with the complete file |
| **Client crash mid-download** | Immediate | Partial download preserved on disk | Yes — resume from last verified piece on restart |
