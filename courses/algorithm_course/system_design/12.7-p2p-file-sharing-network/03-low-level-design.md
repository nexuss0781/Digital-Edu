...
| Endgame Duplicate Cancellation | O(remaining × peers) | O(remaining) | Typically < 5 pieces × 50 peers |
| PEX Message Construction | O(added + dropped) | O(50 × 6) | Capped at 50 added/dropped per message |
| Announce Token Verification | O(1) | O(tokens) | HMAC comparison; token store bounded by TTL |
| Magnet Metadata Assembly | O(metadata_size / 16KiB) | O(metadata_size) | Fetched in 16 KiB pieces via ut_metadata extension |

---

## Magnet Link Resolution Protocol

Magnet links contain only the info-hash, requiring metadata to be fetched from the swarm before downloading can begin:

```
FUNCTION resolve_magnet_link(info_hash):
    // Step 1: Discover peers via DHT using info-hash
    peers = dht_get_peers(info_hash)

    // Step 2: Connect to peers and request metadata via ut_metadata extension
    FOR EACH peer IN peers:
        connection = handshake(peer, info_hash)
        IF connection.supports_extension("ut_metadata"):
            // Metadata is split into 16 KiB pieces
            metadata_size = connection.get_metadata_size()
            num_metadata_pieces = ceil(metadata_size / 16384)

            metadata_buffer = new byte[metadata_size]
            FOR piece_index IN range(num_metadata_pieces):
                // Request each metadata piece
                connection.send_metadata_request(piece_index)
                metadata_piece = connection.receive_metadata_piece(piece_index)
                metadata_buffer[piece_index * 16384 : ] = metadata_piece

            // Step 3: Verify metadata integrity
            computed_hash = SHA256(bencode(parse_info_dict(metadata_buffer)))
            IF computed_hash == info_hash:
                RETURN parse_torrent_metadata(metadata_buffer)
            ELSE:
                // Metadata from this peer is corrupted or fake
                disconnect(peer)
                CONTINUE  // Try next peer

    RETURN ERROR("Failed to resolve magnet metadata from any peer")
```

**Key property**: The info-hash in the magnet link serves as both a lookup key (DHT query target) and a verification hash (validates the metadata received from untrusted peers). A malicious peer cannot return fake metadata — any modification changes the hash.

---

## Selective File Download in Multi-File Torrents

```
FUNCTION configure_selective_download(torrent_metadata, selected_files):
    // Map selected files to their piece ranges
    needed_pieces = set()

    FOR EACH file IN selected_files:
        // Files may span piece boundaries
        first_piece = file.offset / torrent_metadata.piece_length
        last_piece = (file.offset + file.length - 1) / torrent_metadata.piece_length

        FOR piece_index IN range(first_piece, last_piece + 1):
            needed_pieces.add(piece_index)

    // Mark non-needed pieces as "don't download"
    FOR piece_index IN range(torrent_metadata.total_pieces):
        IF piece_index NOT IN needed_pieces:
            mark_piece_priority(piece_index, PRIORITY_SKIP)

    // Adjust bitfield: only express interest in needed pieces
    // Still upload any pieces we have (even from non-selected files) to maintain reciprocity
