...
| **3. Core Algorithm Deep Dive** | 12 min | Kademlia DHT lookup, rarest-first, tit-for-tat | Step-by-step plan in plain English or step-by-step walkthrough |
...
**What they're testing**: Do you understand that P2P has no SLA and performance depends on swarm health?

**Common mistake**: Promising specific download speeds or trying to add QoS guarantees.

**Strong answer**: You can't guarantee download speed in P2P — it depends on how many seeders exist, their upload bandwidth, NAT traversal success, and network conditions. What you CAN do: optimize piece selection (rarest-first maximizes piece diversity, which maximizes available sources), use web seeds as a performance floor (guaranteed minimum speed from HTTP server), prefer high-bandwidth peers (tit-for-tat naturally selects fast uploaders), and use parallel connections (aggregate bandwidth from 20-50+ peers). The fundamental trade-off: P2P offers no SLA but costs nothing to scale, while CDN offers SLAs but costs linearly with traffic.

---

## Scoring Rubric

| Dimension | Junior (1-3 years) | Mid-Level (3-7 years) | Senior/Staff (7+ years) |
|---|---|---|---|
| **Architecture** | "Peers download from each other" | Draws overlay network with tracker, DHT, swarm layers | Explains multi-layered discovery, overlay topology, and why DHT O(log n) works |
| **Algorithms** | Vaguely mentions "downloading pieces" | Explains rarest-first and basic tit-for-tat | Derives XOR routing, conflict between rarest-first and cold-start, endgame mode transitions |
| **Security** | "We hash the files" | Discusses piece verification and Sybil attacks | Full threat model: eclipse, poisoning, BEP 42, traffic analysis; explains MSE is obfuscation not security |
| **Scalability** | "More peers = more bandwidth" | Quantifies the self-scaling property with numbers | Models propagation curves, identifies seeder bandwidth as Slowest part of the process, discusses NAT traversal success rates |
| **Trade-offs** | 0-1 trade-offs | 2-3 trade-offs discussed | Tracker vs DHT, piece size, reciprocity strictness, CDN/P2P hybrid, privacy vs performance |
| **Game Theory** | Not mentioned | "Tit-for-tat is fair" | Explains Nash equilibrium, why optimistic unchoke prevents deadlock, free-rider economics |
...
|---|---|---|
| DHT nodes (mainline) | 15-25M concurrent | One of the largest deployed distributed systems |
| DHT lookup hops | ~20-25 for 10M nodes | O(log n), parallel α=3 |
| k-bucket size | 8 nodes | Balance between redundancy and maintenance cost |
| Piece size (typical) | 2 MiB | v2 provides 16 KiB block-level Merkle verification within pieces |
| Choking round interval | 10 seconds | Regular reciprocity evaluation |
| Optimistic unchoke interval | 30 seconds | Bootstrap/exploration window |
| NAT traversal success (UDP) | 82-95% | Fails mainly with symmetric NAT |
| Protocol overhead | ~0.2% | Wire protocol headers vs payload data |
| Announce interval | 30 minutes | Standard tracker re-announce period |
