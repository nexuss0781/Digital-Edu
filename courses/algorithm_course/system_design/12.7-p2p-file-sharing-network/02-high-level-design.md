...
| Phase | Latency | Slowest part of the process |
|---|---|---|
...
| **Consistent Hashing** | Kademlia's XOR distance metric partitions the ID space similarly to consistent hashing rings |
| **Anti-Entropy** | Periodic bucket refresh and announce re-propagation ensure routing table and peer store convergence |
| **Backpressure** | Choking mechanism applies backpressure to slow uploaders, preventing resource exhaustion |
...
### Discovery Priority and Fallback Logic

```
FUNCTION discover_peers(info_hash, target_count=80):
    discovered_peers = {}
    active_connections = 0

    // Start all discovery mechanisms in parallel
    parallel:
        // Layer 1: Tracker (fastest, most reliable when available)
        tracker_peers = announce_to_tracker(info_hash)
        merge_peers(discovered_peers, tracker_peers)

        // Layer 2: DHT (slowest initial, most resilient)
        dht_peers = dht_get_peers(info_hash)
        merge_peers(discovered_peers, dht_peers)

        // Layer 4: LSD (instant if LAN peers exist)
        lan_peers = lsd_discover(info_hash)
        merge_peers(discovered_peers, lan_peers, priority=HIGH)

    // Connect to discovered peers, preferring diverse IP ranges
    FOR EACH peer IN sort_by_diversity(discovered_peers):
        IF active_connections >= target_count:
            BREAK
        connection = attempt_connect(peer)
        IF connection.success:
            active_connections += 1
            // Layer 3: PEX kicks in once connected
            enable_pex(connection)

    // Background: continuously refine peer set
    WHILE downloading:
        new_peers = collect_pex_updates()
        replace_slow_peers_with_faster_candidates(new_peers)
```
