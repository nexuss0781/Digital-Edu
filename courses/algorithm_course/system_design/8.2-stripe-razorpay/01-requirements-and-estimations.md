...
North America:              35% (~35M txn/day)
  Peak hours: 11am-9pm EST (10 hours of heavy load)
  Primary payment methods: card (80%), ACH (15%), wallet (5%)

Europe:                     25% (~25M txn/day)
  Peak hours: 10am-8pm CET
  Primary payment methods: card (60%), SEPA (20%), iDEAL/Bancontact (15%), wallet (5%)
  Regulatory: PSD2/SCA mandatory for EEA-issued cards

Asia-Pacific:               30% (~30M txn/day)
  India: 12M txn/day (UPI 65%, card 25%, wallet 10%)
  Southeast Asia: 8M txn/day
  Japan/Korea: 5M txn/day
  ANZ: 5M txn/day
  Peak hours: follow-the-sun across time zones

Rest of World:              10% (~10M txn/day)
  Latin America (Pix, Boleto): 6M txn/day
  Middle East/Africa: 4M txn/day
```

---

## Failure Budget Calculations

```
--- Payment Path (99.999% target) ---
Annual allowed downtime:     5 minutes 15 seconds
Monthly allowed downtime:    26.3 seconds
Weekly allowed downtime:     6.05 seconds

At 1,157 avg TPS:
  Monthly lost transactions (budget): ~30,456 transactions
  Monthly lost revenue (at $50 avg):  ~$1.52M in GMV
  → Every second of downtime = ~$57,850 in unrealized GMV

--- Webhook Delivery (99.95% within 1 hour) ---
Daily events:               500,000,000
Allowed missed (1h window):  250,000 events/day
At $50 avg transaction:     ~$12.5M in delayed merchant notifications/day

--- Dashboard (99.9% target) ---
Annual allowed downtime:     8 hours 45 minutes
Monthly allowed downtime:    43.8 minutes
→ Acceptable for non-revenue-critical path
```
...
Compute:
  Payment path (dedicated):       $150,000  (high-availability, multi-AZ)
  Webhook workers:                $80,000   (burst-capable pool)
  Dashboard/API services:         $40,000
  Risk engine (GPU-backed):       $60,000
  Batch processing (payouts/recon): $20,000

Storage:
  Payment database (SSD):         $120,000  (200 GB/day × 30 days active)
  Ledger database (SSD):          $120,000  (200 GB/day × 30 days active)
  Redis cluster (idempotency):    $45,000   (200 GB in-memory)
  Tokenization vault:             $30,000   (250 GB encrypted)
  Object storage (archive):       $15,000   (tiered warm/cold)
  Webhook event log:              $25,000   (15 TB active window)

Networking:
  Bandwidth (650 MB/s peak):      $50,000
  Load balancers (dedicated):     $15,000
  CDN (SDK delivery):             $10,000

External:
  HSM (hardware security modules): $25,000  (per-partition leasing)
  Card network connectivity:      $10,000   (dedicated lines)
  3DS directory service:          $5,000

Compliance:
  PCI-DSS QSA audit (amortized):  $20,000   ($240K annual / 12)
  Penetration testing:            $10,000   (quarterly)
  SOC 2 audit (amortized):       $8,000

────────────────────────────────────────────
Total estimated monthly infra:    ~$858,000
Cost per transaction:             ~$0.00029 (at 100M txn/day)
Revenue per transaction:          ~$0.30 + 2.9% (platform fee)
Gross margin:                     >99% on infrastructure
```
...
6. **Geographic traffic distribution drives multi-region strategy**: With 30% of traffic from Asia-Pacific (predominantly UPI/local rails) and 25% from Europe (PSD2/SCA mandatory), the gateway must support region-specific payment methods and regulatory compliance while maintaining a unified ledger and reconciliation system.

7. **Infrastructure cost is negligible relative to revenue**: At ~$0.00029 per transaction in infrastructure cost versus ~$0.30+ in revenue per transaction, the gross margin exceeds 99%. This means the engineering priority is maximizing authorization rates and uptime (revenue) over minimizing infrastructure cost.
