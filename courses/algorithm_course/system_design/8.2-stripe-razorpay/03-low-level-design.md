...
-- CRITICAL Rule that never changes: For every journal_id, SUM(debits) = SUM(credits)
...
FUNCTION record_payment_in_ledger(payment_intent):
    ...
    BEGIN TRANSACTION:
        ...
        -- Rule that never changes CHECK: sum of debits = sum of credits for this journal
...
### Algorithm 5: Payout Calculation and Batching

```
FUNCTION compute_daily_payouts():
    -- Run daily at 00:00 UTC
    merchants = GET_MERCHANTS_WITH_PAYOUT_SCHEDULE("daily")

    FOR EACH merchant IN merchants:
        -- Determine settlement window
        settlement_delay = merchant.payout_delay_days  -- typically 2
        cutoff_date = TODAY() - settlement_delay business days

        -- Aggregate captured payments not yet paid out
        gross_captured = SUM(
            SELECT amount_received FROM PaymentIntent
            WHERE merchant_id = merchant.id
            AND status IN ("succeeded", "partially_refunded")
            AND captured_at <= cutoff_date
            AND payout_id IS NULL
        )

        -- Deductions
        platform_fees = SUM(fees for included transactions)
        refunds = SUM(refunds processed since last payout)
        dispute_holds = SUM(active dispute amounts)
        reserve_hold = FLOOR(gross_captured * merchant.reserve_percent / 100)

        net_payout = gross_captured - platform_fees - refunds - dispute_holds - reserve_hold

        IF net_payout <= 0:
            SKIP  -- negative balance carried forward
        ...
        payout = INSERT Payout(
            ...
            status: "pending",
            arrival_date: NEXT_BUSINESS_DAY(),
            transaction_count: count of included transactions
        )

        -- Mark included transactions
        UPDATE PaymentIntent SET payout_id = payout.id
        WHERE ... (all included transactions)

        -- Record in ledger
        record_payout_in_ledger(payout)

        -- Submit to banking rails
        submit_to_bank(payout, merchant.bank_account)
```

### Algorithm 6: Network Token Lifecycle Management

```
FUNCTION manage_network_token(payment_method):
    -- Network tokens (Visa Token Service, Mastercard MDES) replace raw PANs
    -- with network-issued tokens that have higher authorization rates

    IF payment_method.network_token IS NULL:
        -- Request network token from card network
        request = {
            pan: decrypt_from_vault(payment_method.card_token),
            merchant_id: payment_method.merchant_id,
            requestor_id: gateway_token_requestor_id
        }

        response = CALL card_network_token_service(request)

        IF response.status == "active":
            payment_method.network_token = response.network_token
            payment_method.network_token_cryptogram = response.cryptogram
            SAVE payment_method

    -- On card update notification from network (e.g., card reissued):
    FUNCTION handle_token_update_notification(notification):
        -- Card networks push updates when card is renewed/reissued
        payment_method = FIND_BY_NETWORK_TOKEN(notification.old_token)

        IF notification.type == "card_updated":
            -- Update stored token with new expiry, new cryptogram
            payment_method.card_exp_month = notification.new_exp_month
            payment_method.card_exp_year = notification.new_exp_year
            payment_method.network_token = notification.new_token
            SAVE payment_method
            -- Merchant's card-on-file continues working without customer action

        ELSE IF notification.type == "card_deleted":
            -- Card closed or reported stolen
            payment_method.status = "expired"
            SAVE payment_method
            NOTIFY merchant via webhook("payment_method.detached")
```
...
```
Payout {
    id:                  string       -- "po_001" (globally unique)
    merchant_id:         string       -- FK to Merchant
    amount:              integer      -- net payout amount in smallest currency unit
    currency:            string       -- ISO 4217
    status:              enum         -- pending | in_transit | paid | failed | canceled
    arrival_date:        date         -- expected arrival at merchant bank
    bank_account_id:     string       -- FK to merchant's bank account
    description:         string       -- "Payout for Jan 15, 2025"

    -- Breakdown
    gross_amount:        integer      -- total captured amount
    fee_amount:          integer      -- total platform fees deducted
    refund_amount:       integer      -- total refunds deducted
    dispute_amount:      integer      -- dispute holds deducted
    reserve_amount:      integer      -- rolling reserve held
    transaction_count:   integer      -- number of transactions included

    -- Banking
    bank_reference:      string       -- ACH trace / SEPA reference
    failure_code:        string       -- bank rejection reason (if failed)
...
Index: (merchant_id, status)                   -- merchant payout dashboard
Index: (status, arrival_date)                  -- batch processing
Index: (created_at)                            -- reconciliation
```
...
```
IdempotencyKey {
    key_hash:            string       -- SHA-256(merchant_id + ":" + idempotency_key)
    merchant_id:         string       -- scoping to merchant prevents cross-merchant collision
    request_fingerprint: string       -- SHA-256 of normalized request body
    status:              enum         -- in_progress | complete
    request_method:      string       -- "POST"
    request_path:        string       -- "/v1/payment_intents"
    response_code:       integer      -- HTTP status code of original response
    response_body_hash:  string       -- SHA-256 of response body (for verification)
    response_body:       bytes        -- compressed response body (for replay)
    created_at:          timestamp
    expires_at:          timestamp    -- created_at + 24 hours
}

-- Stored in Redis with TTL = 86400 seconds (24 hours)
-- Key format: idem:{key_hash}
-- Automatic expiry via Redis TTL (no cleanup job needed)

-- Database fallback table (for when Redis is unavailable):
IdempotencyKeyFallback {
