...
## Network Tokenization Security Model

Network tokenization (Visa Token Service, Mastercard MDES) adds an additional security layer beyond gateway tokenization:

```
Gateway Tokenization (existing):
  Raw PAN → Gateway Vault → Gateway Token (tok_xxx)
  Scope: within the gateway's system only
  Benefit: merchant never touches PAN

Network Tokenization (enhanced):
  Raw PAN → Card Network Token Service → Network Token (DPAN)
  Scope: recognized across the entire card network ecosystem
  Benefits:
  ├── Higher authorization rates (+2-5%) — issuer trusts network tokens more
  ├── Automatic card updates — token updated when card reissued
  ├── Domain-restricted — token only works for registered merchant
  ├── Reduced fraud liability — network guarantees token integrity
  └── Transaction-specific cryptogram — unique per authorization

Security Architecture:
  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
  │ Gateway Vault │ ──→ │  Network TSP  │ ──→ │ Issuing Bank │
  │ (PAN → Token) │     │ (Token + DPAN │     │ (Validates   │
  │              │     │  + Cryptogram)│     │  cryptogram) │
  └──────────────┘     └──────────────┘     └──────────────┘

  Authorization with network token:
  1. Gateway resolves gateway token → retrieves PAN from vault
  2. Gateway requests network token + dynamic cryptogram from TSP
  3. Authorization sent with network token (DPAN) + cryptogram
  4. Issuing bank validates cryptogram (proves token is legitimate)
  5. Higher approval rate because issuer has higher confidence
```
...
|---------|---------------|-----------|
| **Transaction monitoring** | Real-time pattern detection on payment flows | Unusual volume/velocity per merchant |
| **Structuring detection** | Identify split transactions designed to avoid reporting thresholds | Multiple txns summing to >$10,000 within 24h |
| **Suspicious Activity Reports (SAR)** | Automated filing with FinCEN (US) / FIU (other jurisdictions) | Triggered by risk rules + manual review |
| **Sanctions screening** | Screen merchant onboarding against OFAC, EU, UN sanctions lists | Every merchant, every beneficial owner |
| **Enhanced Due Diligence (EDD)** | Additional verification for high-risk merchant categories | Gambling, crypto, adult content, pharmaceuticals |
| **Cross-border monitoring** | Flag unusual cross-border payment patterns | Country risk scoring; embargo compliance |
...
├── Payment transactions:        7 years (tax + audit)
├── Ledger entries:              7 years (append-only, immutable)
├── Dispute records:             7 years (network rules)
├── Payout records:              7 years (tax reporting)
└── KYC/KYB documents:          5 years after relationship ends

Operational data (business requirement):
├── Webhook delivery attempts:  90 days active, 1 year archive
├── API request logs:           90 days (debugging, audit)
├── Idempotency keys:           24 hours (Redis TTL)
└── Session/auth tokens:        24 hours (active), 30 days (audit log)

Customer PII (privacy regulation):
├── Card PANs:                  Until customer deletes payment method
│                               or merchant requests deletion
├── Cardholder name:            Tied to payment method lifecycle
├── Billing address:            Tied to payment method lifecycle
└── Right to deletion:          GDPR Art. 17 — delete PII on request
                                EXCEPT where retention is legally required
                                (financial records exempt from deletion)

Deletion process:
├── Customer requests deletion via merchant
├── Merchant calls DELETE /v1/customers/{id}
├── Gateway: soft-delete customer record
├── Tokenization vault: delete encrypted PAN
├── Retain: anonymized transaction records (legally required)
├── Retain: ledger entries (reference_id remains, PII removed)
└── Confirmation: deletion certificate issued
```
