...
| Metric | Formula | Granularity | Business Use |
|--------|---------|-------------|--------------|
| **Gross Payment Volume (GPV)** | SUM(amount_received) for succeeded payments | Hourly/daily/monthly | Revenue tracking, investor reporting |
| **Take Rate** | platform_fees / GPV | Monthly | Pricing effectiveness |
| **Net Revenue** | platform_fees - network_fees - refund_fee_reversals | Monthly | Profitability |
| **Merchant Activation Rate** | merchants_with_first_payment / merchants_onboarded | Weekly | Onboarding funnel health |
| **Merchant Churn Rate** | merchants_with_zero_volume_30d / total_active_merchants | Monthly | Retention |
| **Authorization Lift** | (auth_rate_with_optimization - baseline) / baseline | Weekly | Smart routing effectiveness |
| **Dispute Rate** | disputes / total_transactions | Monthly per merchant | Risk monitoring, merchant health |

### Operational Health Scorecard

```
System Health Scorecard (updated every 5 minutes):

PAYMENT PATH:
  Authorization rate:      89.2%   ✅ (target: >85%)
  Gateway error rate:      0.003%  ✅ (target: <0.01%)
  Latency p99 (no 3DS):   2.4s    ✅ (target: <3s)
  Idempotent replays:      2.1%    ✅ (target: <5%)

FINANCIAL INTEGRITY:
  Ledger balance:          ✅ BALANCED
  Reconciliation status:   ✅ T+1 complete, T+2 in progress
  Payout on-time rate:     99.95%  ✅ (target: >99.9%)

WEBHOOK DELIVERY:
  First-attempt success:   96.2%   ✅ (target: >95%)
  1-hour delivery rate:    99.97%  ✅ (target: >99.95%)
  DLQ size:                87      ✅ (target: <1000)
  Disabled endpoints:      12      ✅ (target: <50/day)

SECURITY:
  Fraud block rate:        1.2%    ✅ (normal range: 0.5-2%)
  3DS challenge rate:      8.5%    ✅ (target: <15%)
  Vault access anomalies:  0       ✅ (target: 0)
  Failed auth attempts:    124     ✅ (normal range)
```
...
### Payment Incident Classification
...
Incident Severity Matrix:
                    ...
                    ┌────────────────┬────────────────┬────────────────┐
                    │  Single        │  Multiple      │  All           │
                    │  Merchant      │  Merchants     │  Merchants     │
┌───────────────────┼────────────────┼────────────────┼────────────────┤
│ Payment path      │  P2            │  P1            │  P0            │
│ degraded          │  (1h response) │  (15m response)│  (5m response) │
├───────────────────┼────────────────┼────────────────┼────────────────┤
│ Financial         │  P1            │  P0            │  P0            │
│ integrity issue   │  (15m)         │  (5m)          │  (immediate)   │
├───────────────────┼────────────────┼────────────────┼────────────────┤
│ Webhook delivery  │  P3            │  P2            │  P1            │
│ delayed           │  (next day)    │  (1h)          │  (15m)         │
├───────────────────┼────────────────┼────────────────┼────────────────┤
│ Dashboard         │  P3            │  P3            │  P2            │
│ unavailable       │  (next day)    │  (next day)    │  (1h)          │
└───────────────────┴────────────────┴────────────────┴────────────────┘
```
...
|--------|---------|--------|
| Time to detect (TTD) | How quickly alerting identified the issue | < 2 minutes for P0 |
| Time to mitigate (TTM) | How quickly customer impact was reduced | < 15 minutes for P0 |
| Time to resolve (TTR) | How quickly root cause was fully fixed | < 4 hours for P0 |
| Payments affected | Count and dollar value of impacted transactions | Minimize |
| False positive rate | Alert noise ratio | < 5% of alerts should be false |
| Customer communications sent | Status page updates + merchant notifications | Within 10 min of P0 |
