...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, API design with idempotency, algorithms (Step-by-step plan in plain English) |
...
| [8.5 - Fraud Detection System](../8.5-fraud-detection-system/) | Real-time ML scoring, velocity checks, rules engine for payment risk |
| [8.6 - Distributed Ledger Core Banking](../8.6-distributed-ledger-core-banking/) | Double-entry ledger at scale, settlement, interbank transfers |
| [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Installment payment orchestration, merchant integration, underwriting |
...
|---------|---------------------------|
| **State Machine** | PaymentIntent lifecycle with validated transitions and persisted audit trail |
| **Idempotency Key** | Client-provided keys with server-side deduplication for exactly-once semantics |
| **Double-Entry Bookkeeping** | Immutable append-only ledger with algebraic balance invariants |
| **Saga / Orchestration** | Multi-party authorization across gateway, acquirer, card network, issuing bank |
| **Circuit Breaker** | Per-acquirer and per-card-network fault isolation with fallback routing |
| **Event Sourcing** | Payment lifecycle events published to event bus for webhooks, analytics, ledger |
| **Tokenization Vault** | PCI-scoped isolated service converting PANs to opaque tokens |
| **Fan-Out Queue** | Per-endpoint webhook delivery with independent retry and backpressure |

---

## Evolution and Industry Trends (2024-2026)

| Trend | Impact on Payment Gateway Architecture |
|-------|---------------------------------------|
| **Real-Time Payment Rails** | FedNow (US), UPI (India 15B+ txn/month), Pix (Brazil) offer instant settlement alternative to T+2 card settlement; gateway must support multi-rail orchestration |
| **Network Tokenization Adoption** | Visa/Mastercard push network tokens as default; gateway must integrate Token Service Providers (TSPs) and handle lifecycle callbacks |
| **AI-Powered Fraud Detection** | Transformer-based models replace traditional gradient boosting; real-time feature computation at sub-50ms latency for behavioral signals |
| **Embedded Finance / BaaS** | Gateways evolve into financial platforms: treasury accounts, card issuing, capital lending---all through same API |
| **Account-to-Account (A2A) Payments** | Open banking payment initiation bypasses card networks entirely; lower fees, instant settlement, no chargebacks |
| **Stablecoin Settlement** | USDC/USDT as settlement currency between gateway and merchant; reduces FX cost on cross-border transactions |
| **Automated Dispute Resolution** | Visa CE 3.0 and Mastercard Collaboration enable automated representment based on pre-collected device/IP evidence |
| **Tap-to-Pay / SoftPOS** | Phone-as-terminal acceptance (NFC) eliminates hardware POS; gateway extends to in-person payments via merchant mobile app |
...
- Visa Token Service --- Network Tokenization Technical Guide (2025)
- Adyen Engineering --- Multi-Acquirer Payment Routing at Scale
- Federal Reserve --- FedNow Service Operating Procedures (2025)
- PCI Security Standards Council --- PCI-DSS v4.0.1 Summary of Changes (2025)
- Stripe Engineering Blog --- Scaling Infrastructure to Handle Billions of Requests
