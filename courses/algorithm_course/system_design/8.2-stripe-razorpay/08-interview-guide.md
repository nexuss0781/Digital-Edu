...
### If Interview Runs Short (Extra Topics to Volunteer)

| Topic | Time Needed | Signal It Shows |
|-------|-------------|-----------------|
| **Multi-acquirer smart routing** | 3-5 min | Operational maturity; revenue optimization |
| **Network tokenization vs. gateway tokenization** | 2-3 min | Deep payment domain knowledge |
| **Interchange optimization (Level 2/3 data)** | 2-3 min | Cost optimization; merchant value-add |
| **PSD2/SCA exemption engine** | 3-5 min | Regulatory depth; conversion optimization |
| **Multi-party payments (platform model)** | 5-7 min | Marketplace architecture; fund flow design |

### Architecture Whiteboard Checklist

When drawing the architecture, ensure these components appear:

```
Must-have (core):
☑ Client SDK (iframe tokenization)
☑ API Gateway with idempotency layer
☑ Payment Orchestrator (state machine)
☑ Tokenization Vault (CDE isolation)
☑ External: Acquirer → Card Network → Issuing Bank
☑ Ledger Service (double-entry)
☑ Event Bus → Webhook Delivery
☑ Redis (idempotency keys)
☑ PostgreSQL (payments, merchants)
...
### "How would you handle a marketplace model (like Shopify Payments)?"

**Key points:**
- Platform collects payment from customer, splits funds to multiple sellers
- Two models: **destination charges** (platform creates charge, specifies destination merchant) vs. **direct charges** (merchant charges directly, platform takes fee)
- Fund flow: Customer → Platform account → Split to seller accounts → Payout to each seller's bank
- Ledger complexity: must track platform's portion, each seller's portion, and application fees separately
- Regulatory: platform may need money transmitter license depending on fund flow

### "How would you support instant payouts?"

**Key points:**
- Standard T+2 provides fraud review window; instant payout removes that safety net
- Implementation: push-to-card (Visa Direct, Mastercard Send) or real-time payment rails (FedNow, UPI, Faster Payments)
- Risk mitigation: charge higher fee (1-1.5%), require merchant track record, hold reserve for new merchants
- Ledger: same double-entry, but settlement entries recorded immediately
- Availability: 24/7 including weekends (unlike ACH which follows banking hours)

### "What if a card network goes down entirely?"

**Key points:**
- Circuit breaker opens for that network; all Visa (or MC) transactions fail fast
- Merchant SDK can surface alternative payment methods: "Visa unavailable, try Mastercard or bank transfer"
- Queue-and-retry for non-interactive flows (subscriptions): hold and retry when network recovers
- Communication: status page update, proactive merchant notification, webhook with network_outage event
- Post-incident: reconciliation sweep to catch any transactions in ambiguous state during outage

---

## Evaluation Rubric: What Interviewers Look For

| Signal | Junior (Meets Bar) | Senior (Strong Hire) | Staff+ (Exceptional) |
|--------|-------------------|---------------------|---------------------|
| **Idempotency** | Mentions idempotency keys | Multi-layer defense (Redis + DB + network dedup) | Discusses key TTL trade-offs, in-progress race conditions, fingerprint mismatches |
| **State machine** | Identifies payment status flow | Draws full state diagram with error states | Discusses recovery semantics, reconciliation sweeper, settlement file reconciliation |
| **Ledger** | Mentions recording transactions | Double-entry with balance assertion | Discusses multi-tier reconciliation, anomaly detection, append-only Rule that never changes |
| **Webhooks** | Mentions event delivery | At-least-once with retry and signing | Per-endpoint isolation, backpressure, DLQ design, consumer idempotency burden |
| **Security** | Mentions encryption | Tokenization vault, CDE isolation | HSM key hierarchy, network tokenization, M-of-N custodian model |
| **Scale** | Mentions horizontal scaling | Multi-region with single-primary for writes | Smart routing, load shedding levels, chaos engineering approach |
