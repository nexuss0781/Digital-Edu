...
## Data Flow 4: Refund Processing

```
Merchant requests full refund for $99.00 payment (pi_xyz789)

1. Merchant Backend → API Gateway: POST /v1/refunds
   - Headers: Idempotency-Key: "refund_pi_xyz789_full"
   - Body: { payment_intent: "pi_xyz789", amount: 9900 }

2. API Gateway → Idempotency Layer:
   - Check idempotency key → MISS (new request)
   - Create record: { key, status: "in_progress" }

3. Idempotency Layer → Refund Service:
   - Validate: original payment exists and status == "succeeded"
   - Validate: refund amount <= amount_received - already_refunded
   - Create Refund: { id: "re_001", status: "pending" }

4. Refund Service → Acquiring Bank:
   - Submit reversal/credit request referencing original auth_code
   - For card payments: credit posted to cardholder within 5-10 business days
   - For UPI/bank transfer: reversal initiated on same rails

5. Refund Service → Ledger Service:
   - Reverse journal entries proportionally:
     DEBIT  merchant_receivable       $99.00
     CREDIT processor_settlement      $99.00
   - Reverse platform fee proportionally:
     DEBIT  platform_revenue          $2.87
     CREDIT merchant_receivable       $2.87
   - Net merchant impact: -$96.13 (original $99 - $2.87 fee refunded)

6. Refund Service → Event Bus: publish "charge.refunded"
   - Webhook delivered to merchant with refund details

7. Payment Orchestrator: transition PaymentIntent
   - Full refund:  "succeeded" → "refunded"
   - Partial refund: "succeeded" → "partially_refunded"
```

---

## Data Flow 5: Payout / Settlement Cycle

```
Daily payout processing for Merchant "merch_001" (T+2 schedule)

1. Payout Scheduler (runs at 00:00 UTC daily):
   - Query: all PaymentIntents captured 2+ business days ago
     AND not yet included in a payout
   - For merch_001, T-2 captured transactions:
     Gross captured:           $50,000.00
     Platform fees:            -$1,600.00  (2.9% + $0.30 × 412 txns)
     Refunds processed:          -$450.00
     Dispute holds:              -$200.00
     Reserve hold (10%):       -$4,775.00  (new merchant risk reserve)
     ─────────────────────────────────────
     Net payout amount:        $42,975.00

2. Payout Service → Ledger Service:
   - DEBIT  merchant_receivable    $42,975.00
   - CREDIT merchant_payout        $42,975.00

3. Payout Service → Banking Rails:
   - Submit ACH credit (US) / SEPA credit transfer (EU) / IMPS (India)
   - Include: merchant bank account details, amount, reference
   - ACH: settles next business day
   - SEPA Instant: settles within seconds
   - IMPS: settles within seconds

4. Confirmation:
   - Banking rail confirms funds sent
   - Update payout status: "paid"
   - Publish "payout.paid" event → webhook to merchant

5. Reserve release (for new merchants):
   - After 90-day probation with <1% dispute rate
   - Reserve percentage reduced: 10% → 5% → 0%
   - Released reserves included in next payout
```

---

## Data Flow 6: Multi-Currency Payment

```mermaid
sequenceDiagram
    actor Customer as Customer (EUR)
    participant GW as API Gateway
    participant PMT as Payment Orchestrator
    ...
    Customer->>GW: Pay €85.00 to USD merchant
    GW->>PMT: Create PaymentIntent (EUR)

    PMT->>FX: Get EUR→USD rate
    FX-->>PMT: Rate: 1.0824 (locked)

    Note over PMT: Presentment: €85.00<br/>Settlement: $91.00 USD<br/>FX spread: 1.5% ($1.37)

    PMT->>ACQ: Authorize €85.00 on EUR card
    ACQ->>NET: Route to issuing bank
    NET-->>ACQ: APPROVED
    ACQ-->>PMT: APPROVED

    PMT->>LEDGER: Record in both currencies
    Note over LEDGER: Journal 1 (EUR):<br/>DEBIT customer_payable €85.00<br/>CREDIT merchant_receivable_eur €85.00<br/><br/>Journal 2 (USD conversion):<br/>DEBIT merchant_receivable_eur €85.00<br/>CREDIT merchant_receivable_usd $91.00<br/><br/>Journal 3 (FX spread):<br/>DEBIT merchant_receivable_usd $1.37<br/>CREDIT fx_revenue $1.37

    PMT-->>Customer: Payment confirmed (€85.00)
```
...
│                                                       │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │
│  │  Plan Store   │  │  Billing     │  │  Dunning   │ │
│  │              │  │  Scheduler   │  │  Manager   │ │
│  │  Plans:       │  │              │  │            │ │
│  │  - Monthly    │  │  Cron: check │  │  On failure:│ │
│  │  - Annual     │  │  all active  │  │  Retry at:  │ │
│  │  - Usage-based│  │  subs daily  │  │  +1d, +3d,  │ │
│  │  - Tiered     │  │  at renewal  │  │  +5d, +7d   │ │
│  │              │  │  date        │  │            │ │
│  └──────────────┘  └──────┬───────┘  └─────┬──────┘ │
│                           │                 │        │
│                    ┌──────┴─────────────────┴──┐     │
...
│                    │    (reuses core payment    │     │
│                    │     path for each charge)  │     │
│                    └───────────────────────────┘     │
└─────────────────────────────────────────────────────┘

Dunning Strategy (smart retry):
├── Attempt 1: renewal date, 10:00 AM merchant TZ
├── Attempt 2: +1 day, different time (2:00 PM)
│   Rationale: avoid same-time failures (bank maintenance windows)
├── Attempt 3: +3 days, morning (9:00 AM)
│   Rationale: wait for paycheck deposit cycle
├── Attempt 4: +5 days, try alternate payment method if available
├── Attempt 5: +7 days, final attempt
│   On failure: cancel subscription, notify merchant via webhook
│
│ ML-optimized retry: analyze historical success patterns per
│ issuing bank × day-of-week × time-of-day to select optimal retry window
```
