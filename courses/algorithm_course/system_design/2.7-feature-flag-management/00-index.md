...
| **Cloud Provider Managed** | Major cloud providers | Cloud-scale | Managed service with native monitoring integration |
...
|---------|------|------------|
| **Configuration Management** | [1.11](../1.11-configuration-management-system/00-index.md) | Foundation for flag storage; feature flags are config with targeting and rollout semantics |
| **Content Delivery Network** | [1.15](../1.15-content-delivery-network-cdn/00-index.md) | Edge delivery architecture for global flag evaluation at CDN PoPs |
| **API Gateway** | [1.14](../1.14-api-gateway-design/00-index.md) | Request-level flag evaluation in gateway middleware; header injection |
| **Distributed Cache** | [1.4](../1.4-distributed-lru-cache/00-index.md) | Flag ruleset caching strategies; copy-on-write cache updates |
| **Consistent Hashing Ring** | [1.9](../1.9-consistent-hashing-ring/00-index.md) | Bucketing algorithm foundation for deterministic user assignment |
| **Container Orchestration** | [2.2](../2.2-container-orchestration-system/00-index.md) | Feature flags for canary deployments; integration with rollout strategies |
| **CI/CD Pipeline** | [2.4](../2.4-cicd-pipeline-build-system/00-index.md) | Deployment-triggered flag management; flag-as-code workflows |
| **Edge Computing Platform** | [2.8](../2.8-edge-computing-platform/00-index.md) | Edge-side evaluation for IoT and latency-critical applications |
...
|-------------|-------------|
| **OpenFeature SDK Specification** | Vendor-neutral feature flag SDK standard (CNCF project, v1.0 GA); enables provider-agnostic flag evaluation |
| **Flag-as-Code / GitOps** | Declaring flag configurations in git repositories; CI/CD-triggered flag changes with review workflows |
| **AI/ML Model Feature Flags** | Using feature flags for ML model version rollouts, A/B testing model performance, and multi-armed bandit optimization |
| **Server-Sent Events v2 patterns** | Improved SSE with binary encoding, compression, and multiplexed channels for lower bandwidth |
| **Warehouse-native experimentation** | Running experiment analysis directly in the data warehouse (dbt + experiment metrics) rather than in-platform |
| **Feature flag governance** | Automated stale flag detection, mandatory flag owners, lifecycle enforcement, and compliance audit trails |
