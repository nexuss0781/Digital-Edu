...
- Offline capability
- No database Slowest part of the process
...
| **Centralized evaluation** | Latency, availability Slowest part of the process | Local SDK evaluation |
...
|-------|-----------|---------------|
| **OpenFeature** | CNCF standard for vendor-neutral SDKs; provider interface pattern | When discussing SDK architecture |
| **Flag-as-Code** | Git-managed flag configs with CI/CD review; GitOps for flags | When discussing governance or change management |
| **Multi-armed bandits** | Auto-optimizing traffic allocation; Thompson Sampling | When discussing experimentation beyond A/B |
| **Warehouse-native analytics** | Run experiment analysis in data warehouse, not in-platform | When discussing experimentation architecture |
| **SRM detection** | Chi-squared test for sample ratio mismatch before any results | When discussing experiment validity |
| **Flag lifecycle governance** | Automated stale detection, mandatory owners, expiration dates | When discussing long-term maintenance |
...
| "What about feature flags for ML models?" | A/B testing model versions, gradual rollout, multi-armed bandits |
| "How do you prevent flag sprawl?" | Lifecycle management, archival policies, automated stale detection |
| "What if targeting rules become too complex?" | Segments, rule compilation, performance alerts |
| "How would you make this vendor-neutral?" | OpenFeature provider interface; standardized evaluation API |
