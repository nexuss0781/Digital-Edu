...
As LLMs evolve from single-turn chatbots to autonomous agents with tool access, guardrail requirements expand significantly:

| Requirement | Description | Priority |
|-------------|-------------|----------|
| **Tool Call Chain Validation** | Verify that multi-step tool call sequences don't combine safe individual actions into unsafe composite actions | P0 |
| **Indirect Prompt Injection Defense** | Detect malicious instructions embedded in tool outputs, web pages, or retrieved documents that attempt to hijack the agent | P0 |
| **Action Scope Enforcement** | Enforce maximum blast radius per agent action (e.g., file writes limited to sandbox directory) | P0 |
| **Autonomous Loop Detection** | Detect and terminate agents stuck in unsafe recursive loops or escalating behavior patterns | P1 |
| **Cross-Agent Contagion Prevention** | Prevent one compromised agent from injecting instructions into other agents in a multi-agent system | P1 |

### Multimodal Safety

| Requirement | Description | Priority |
|-------------|-------------|----------|
| **Image-in-Prompt Injection** | Detect malicious text rendered as images within multimodal prompts | P0 |
| **Audio Command Injection** | Detect hidden voice commands in audio inputs | P1 |
| **Cross-Modal Consistency** | Verify that text claims match image/audio content | P1 |

### Regulatory Compliance (EU AI Act)

| Requirement | Description | Deadline |
|-------------|-------------|----------|
| **Risk Classification** | Classify AI system risk level and apply corresponding guardrails | August 2025 |
| **Transparency Obligations** | Disclose AI-generated content to users | February 2025 |
| **Bias Detection** | Continuous monitoring for discriminatory outputs | August 2025 |
| **Human Oversight Hooks** | Enable human-in-the-loop for high-risk decisions | August 2025 |

### Benchmark Targets (2025)

| Benchmark | Target | Current State-of-Art |
|-----------|--------|---------------------|
| **Prompt Injection (BIPIA)** | >95% detection | ~92% (multi-stage cascade) |
| **Jailbreak (JailbreakBench)** | >90% detection | ~88% (classifier + LLM judge) |
| **PII Detection (Presidio benchmark)** | >94% precision, >89% recall | Presidio baseline |
| **Toxicity (ToxiGen)** | >96% detection | ~95% (fine-tuned classifier) |
| **Indirect Injection (InjecAgent)** | >85% detection | ~75% (emerging benchmark) |
| **Latency overhead (p99)** | <100ms | Varies by pipeline depth |
