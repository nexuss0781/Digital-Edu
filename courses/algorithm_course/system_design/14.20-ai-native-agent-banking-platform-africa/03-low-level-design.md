...
RebalanceRequest {
    rebalance_id:       UUID
    agent_id:           UUID
    request_type:       ENUM [E_FLOAT_TOP_UP, CASH_DEPOSIT, CASH_PICKUP, SWAP]
    status:             ENUM [PREDICTED, RECOMMENDED, ACCEPTED, IN_PROGRESS,
    ...
    // Source
    source_type:        ENUM [SUPER_AGENT, BANK_BRANCH, BANK_TRANSFER, CIT_VAN]
    source_id:          UUID (nullable)         // Super-agent or branch ID
    source_name:        String
    source_distance_km: Decimal(5,2)
    estimated_travel_minutes: Integer

    // Prediction context
    trigger:            ENUM [AI_PREDICTION, THRESHOLD_BREACH, MANUAL_REQUEST]
    urgency:            ENUM [LOW, MEDIUM, HIGH, CRITICAL]
    predicted_depletion_time: Timestamp (nullable)
    confidence_score:   Decimal(5,2)            // AI prediction confidence

    // Timing
    recommended_by:     Timestamp               // Complete rebalance before this time
    accepted_at:        Timestamp (nullable)
    completed_at:       Timestamp (nullable)
    expired_at:         Timestamp (nullable)

    // Outcome
    actual_amount:      Decimal(18,2) (nullable)// May differ from requested
    agent_e_float_after: Decimal(18,2) (nullable)
    agent_cash_after:   Decimal(18,2) (nullable)
...
CrossBorderTransaction {
    corridor_txn_id:    UUID
    domestic_txn_id:    UUID                    // Linked domestic transaction
    corridor_id:        String                  // e.g., "NG-GH" for Nigeria → Ghana

    // Sending side
    sending_country:    String                  // ISO 3166-1 alpha-2
    sending_agent_id:   UUID
    sending_amount:     Decimal(18,2)
    sending_currency:   String                  // ISO 4217

    // Receiving side
    receiving_country:  String
    receiving_agent_id: UUID (nullable)         // Assigned when recipient collects
    receiving_amount:   Decimal(18,2)
    receiving_currency: String

    // FX
    exchange_rate:      Decimal(10,6)           // Locked at transaction time
    rate_source:        String                  // Rate provider
    rate_locked_at:     Timestamp
    fx_spread:          Decimal(5,4)            // Platform spread over mid-market

    // Settlement
    settlement_batch_id: UUID (nullable)        // Assigned during netting
    settlement_status:  ENUM [PENDING, NETTED, SETTLED, DISPUTED]
    bilateral_position_impact: Decimal(18,2)    // Impact on corridor net position

    // Compliance
    sending_compliance: ENUM [CLEARED, HELD, FLAGGED]
    receiving_compliance: ENUM [CLEARED, HELD, FLAGGED]
    sanctions_screened: Boolean

    // Status
    status:             ENUM [INITIATED, FUNDS_HELD, AVAILABLE_FOR_PICKUP,
                              COLLECTED, REVERSED, EXPIRED]
    expires_at:         Timestamp               // Uncollected funds returned after 30 days
...
### Agent Retention Score (Exclusivity Context)
...
AgentRetentionScore {
    ...
    // Revenue metrics
    monthly_commission_avg: Decimal(18,2)       // 6-month rolling average
    revenue_trend:      ENUM [GROWING, STABLE, DECLINING]
    revenue_rank_pct:   Decimal(5,2)            // Percentile within region

    // Strategic value
    coverage_criticality: ENUM [LOW, MEDIUM, HIGH, IRREPLACEABLE]
    nearest_alternative_km: Decimal(5,2)        // Distance to next agent
    unique_customer_count: Integer              // Customers served only by this agent
    service_diversity:  Integer                 // Number of product types offered

    // Risk indicators
    churn_probability:  Decimal(5,4)            // ML-predicted 0-1
    competitor_exposure: ENUM [NONE, LOW, MEDIUM, HIGH]
    satisfaction_score: Decimal(5,2)            // From survey + behavioral signals
    support_ticket_trend: ENUM [DECREASING, STABLE, INCREASING]

    // Retention offer
    recommended_offer:  String                  // e.g., "Commission boost 15% for 6 months"
    offer_budget:       Decimal(18,2)           // Maximum retention spend for this agent
    offer_status:       ENUM [NOT_OFFERED, OFFERED, ACCEPTED, DECLINED]
