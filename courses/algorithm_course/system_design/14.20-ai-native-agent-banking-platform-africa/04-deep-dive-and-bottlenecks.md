...
**The 1.5x rule as a starting Practical rule of thumb.** New agents without historical data start with the 1.5x rule: maintain cash and e-float at 1.5x the previous day's transaction volume. As data accumulates (2+ weeks), the AI model gradually takes over, replacing the Practical rule of thumb with learned predictions.
...
### Slowest part of the process: Salary-Day Correlated Demand
...
### Slowest part of the process: 1:N Deduplication at Scale
...
### Slowest part of the process: Oversaturation in Urban Areas
...
## Deep Dive 6: Cross-Border Corridor Settlement

### The Problem

Agent banking platforms operating across multiple African markets (Nigeria-Ghana, Kenya-Tanzania, Ethiopia-Kenya) must support cross-border transactions where a customer deposits cash at an agent in one country and a recipient withdraws cash from an agent in another country. This creates a settlement problem that spans currencies, regulatory jurisdictions, and separate platform instances.

### Why This Is Hard

**1. Bilateral position management.** Every cross-border transaction creates a bilateral claim: the sending-country platform owes the receiving-country platform for the cash disbursed by the receiving agent. These claims must be tracked, netted, and settled periodically. If the settlement frequency is too low (daily), the bilateral position grows large, creating counterparty risk. If too high (per-transaction), settlement costs become prohibitive.

**2. FX rate risk.** A customer sends ₦100,000 from Nigeria at an NGN/GHS rate of 1:0.008 (GHS 800). If the rate moves to 1:0.0075 by settlement time, the receiving platform receives less NGN equivalent than it disbursed in GHS. The question of who bears this FX risk (sending platform, receiving platform, customer, or agent) is a business-critical design decision.

**3. Asymmetric compliance.** A transaction flagged by Nigerian AML rules but cleared by Ghanaian rules creates an asymmetric compliance state. The sending side holds the funds, but the receiving agent has already disbursed cash. The corridor settlement protocol must handle this gracefully.

### Corridor Settlement Architecture
...
---
flowchart LR
    subgraph Nigeria["Nigeria Platform"]
        NS[Sending Agent]
        NL[Nigeria Ledger]
        NC[Corridor Position<br/>Nigeria → Ghana]
    end

    subgraph Settlement["Settlement Layer"]
        NET[Bilateral Netting<br/>Every 4 hours]
        FX[FX Rate Lock<br/>At transaction time]
        BANK[International Bank<br/>Settlement Account]
    end

    subgraph Ghana["Ghana Platform"]
        GL[Ghana Ledger]
        GC[Corridor Position<br/>Ghana → Nigeria]
        GS[Receiving Agent]
    end
    ...
    classDef ng fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef settle fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef gh fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class NS,NL,NC ng
    class NET,FX,BANK settle
    class GL,GC,GS gh
```

### Settlement Protocol

1. **Transaction-time FX lock**: At the moment the customer initiates the transfer, the system locks the exchange rate for that transaction. The customer sees the exact amount the recipient will receive. This eliminates FX risk for the customer.
2. **Bilateral netting**: Every 4 hours (configurable per corridor), the system calculates the net position between the two platforms. If Nigeria sent GHS 50M worth of transactions to Ghana and Ghana sent NGN 30M worth to Nigeria, the net settlement is the difference.
3. **Position limits**: Each corridor has a maximum outstanding bilateral position (e.g., $500K). When the position approaches the limit, the system either (a) triggers an early settlement, or (b) temporarily suspends the corridor until settlement completes.
4. **Partial reversal handling**: If a sending-side transaction is reversed after the receiving side has disbursed, the corridor creates a "claim" entry that is netted in the next settlement cycle.

### Slowest part of the process: Regulatory Hold Asymmetry

When Nigerian compliance flags a transaction after Ghanaian agents have already disbursed cash, the platforms face a conflict: Nigeria holds funds, Ghana has already paid out. Resolution requires a bilateral dispute protocol where the receiving platform can claim against the sending platform's settlement reserve—a financial buffer maintained specifically for this scenario (typically 2% of daily corridor volume).

---

## Deep Dive 7: Agent Retention Under Exclusivity

### The Problem

Nigeria's CBN exclusivity mandate (April 2026) forces every agent to choose a single financial institution platform. Agents currently operating multi-platform POS terminals must pick one. This is a one-time event with permanent consequences: the platform that loses agents loses their transaction volume, customer base, and geographic coverage permanently.

### Retention Prediction Model

The system must identify at-risk agents before they switch. Key features:

| Feature | Signal Strength | Why |
|---|---|---|
| **Transaction volume trend** | High | Declining volume suggests the agent is shifting transactions to a competitor |
| **Commission complaints** | High | Agents who contact support about commission rates are price-sensitive and likely to switch for better terms |
| **Competitor terminal presence** | Very High | Agents already operating a competitor's POS terminal have established an alternative; switching cost is low |
| **Geographic exclusivity pressure** | Medium | Agents in areas where a competitor has stronger brand presence face customer pressure to switch |
| **Float satisfaction score** | Medium | Agents frustrated by slow rebalancing or insufficient float limits may attribute service issues to the platform |
| **Device age and quality** | Medium | Agents with old, malfunctioning devices perceive lower platform investment in their success |

### Retention Offer Optimization

Not all agents warrant the same retention investment. The system calculates a **retention value score** per agent:

```
Retention Value = (Monthly Commission Revenue × 12)
...
- **Strategic Location Premium**: Agents in underserved areas with no nearby alternatives receive 3-5x weight because losing them creates a coverage gap that cannot be filled quickly
- **Customer Base Value**: Agents with high customer loyalty (repeat biometric verifications from many unique customers) are worth more because their customers may follow them to the competitor platform

### Slowest part of the process: Information Asymmetry

The platform knows its own agents' behavior but has limited visibility into competitor offerings. Agents may receive competitor retention offers that the platform cannot see or match in real-time. The mitigation is proactive: make retention offers before the agent receives competitor offers, using the prediction model to identify at-risk agents 60-90 days before the exclusivity deadline.
