...
- Asks about the offline percentage before designing the architecture (shows they know it's a first-class concern, not an Edge Case (Unusual or extreme situation))
...
- The 1.5x rule as a Practical rule of thumb for new agents without history
...
|---|---|
| "Walk me through what happens when a customer deposits cash at an agent location." | Should describe: customer → agent → biometric verification → debit agent e-float → credit customer account → receipt. Bonus: mentions that agent accumulates physical cash while losing e-float. |
| "What happens if the network goes down during a transaction?" | Should mention offline capability and local processing. Bonus: describes store-and-forward with cryptographic signing and later reconciliation. |
| "Why can't we just use a cloud database for all transactions?" | Should identify latency and connectivity challenges. Bonus: mentions that 30-40% of agents have intermittent connectivity and financial services cannot depend on network availability. |

### Mid-Level Engineer (3–7 years)

| Question | Expected Depth |
|---|---|
| "Two agents process offline withdrawals for the same customer, exceeding the customer's balance. How do you resolve this?" | Should describe conflict detection during sync and the fact that physical cash already changed hands. Bonus: explains CRDT-style reconciliation where both transactions are accepted and compensating entries restore accounting integrity. |
| "Your float prediction model predicts an agent needs ₦200K of e-float tomorrow, but the nearest super-agent has only ₦80K. What does the system do?" | Should describe hierarchical fallback: check other super-agents, route to bank branch, adjust agent's operating limits. Bonus: considers travel-time optimization and cost of different rebalancing options. |
| "How do you handle biometric verification when a farmer's fingerprints are too worn to match?" | Should describe quality-adaptive thresholds and fallback chains. Bonus: explains multi-modal fusion (degraded fingerprint + facial together achieve acceptable composite accuracy) and compensating controls (PIN as supplementary factor). |

### Senior Engineer / Architect (7+ years)

| Question | Expected Depth |
|---|---|
| "The CBN mandates geo-fencing, but 25% of your transactions are offline. How do you architect compliance?" | Should describe the fundamental conflict between geo-fence validation and offline-first architecture. Bonus: proposes layered approach—device-side GPS capture for all transactions, server-side post-hoc validation during sync, cell-tower triangulation as secondary signal, and pattern analysis for detecting intentional circumvention vs. GPS drift. |
| "Design the agent retention system for the CBN exclusivity mandate. You have 6 months." | Should describe churn prediction, revenue-weighted agent scoring, competitive intelligence, and retention offer optimization. Bonus: recognizes this is a game-theoretic competition and discusses the strategic value of retaining agents in underserved areas vs. high-volume urban agents. |
| "How would you integrate a CBDC (eNaira) as a new transaction rail alongside cash and mobile money?" | Should describe adding a new wallet type, new settlement rail, and regulatory reporting path. Bonus: discusses that CBDC changes the float model (agents hold CBDC as a third liquidity type alongside cash and e-float), requires integration with the central bank's CBDC infrastructure, and creates opportunities (instant settlement) and risks (central bank visibility into all transactions). |
...
Essential components (must-have):
  □ Multi-channel edge layer: POS terminals, mobile apps, USSD, SMS
  □ Offline transaction processing with local ledger
  □ Double-entry ledger (separate from core banking)
  □ Float management service with cash and e-float tracking
  □ Biometric engine with on-device quality check + server dedup
  □ Fraud detection on every transaction
  □ Sync service with conflict resolution
  □ Regulatory compliance engine (configurable per jurisdiction)
  □ Core banking integration (async settlement)
  □ Agent management (onboarding, tiers, performance)

Differentiating components (above-bar):
  □ AI float prediction with super-agent routing optimization
  □ Store-and-forward with cryptographic signing
  □ Quality-adaptive biometric thresholds
  □ Graduated fraud response (surveillance → limits → investigation → suspension)
  □ Regional processing nodes for latency and resilience
  □ Agent placement optimizer using population density and economic indicators

Data flows to articulate:
  □ Cash-In: Customer cash → Agent e-float debit → Customer account credit
  □ Cash-Out: Customer account debit → Agent e-float credit → Agent gives cash
  □ Offline: Transaction → Local ledger → Cryptographic sign → Queue → Sync → Reconcile
  □ Float rebalancing: Prediction → Alert → Route to super-agent/branch → Execute
```
...
1. **Physical-digital awareness:** Understands that agent banking is fundamentally a physical logistics system (cash movement) that software must optimize, not replace. Designs for real-world constraints like travel distances, road conditions, and bank branch hours.

2. **Inclusion-centered design:** Recognizes that the primary users (agents and customers) have low digital literacy, worn fingerprints, basic devices, and intermittent connectivity. Designs for the hardest case, not the average case.

3. **Offline-first thinking:** Treats offline as the normal state, not an Edge Case (Unusual or extreme situation). Designs the transaction flow for offline first, then adds online optimizations.

4. **Fraud-empathy balance:** Understands the asymmetric cost of false positives (destroying an agent's livelihood) and calibrates fraud detection accordingly. Proposes graduated responses rather than binary suspend/allow.

5. **Regulatory architecture:** Treats multi-jurisdiction compliance as a first-class architectural concern, not an afterthought. Designs a programmable policy engine rather than hardcoded rules.

---

## Cross-Border Corridor Design Question

### Opening Prompt (for senior candidates)

> "A customer in Lagos deposits ₦100,000 and wants it available for cash withdrawal by a recipient in Accra, Ghana within 30 minutes. Walk me through the end-to-end flow."

### What a Strong Answer Covers

1. **FX rate locking**: Rate must be locked at transaction initiation time, not settlement time—customer needs to know the exact GHS amount the recipient will receive
2. **Dual-jurisdiction compliance**: Nigerian AML screening on the sending side, Ghanaian AML on the receiving side; transaction must clear both before funds are available
3. **Bilateral position tracking**: The transaction creates a claim from the Ghana platform against the Nigeria platform; this is tracked in a corridor ledger and settled periodically
4. **Agent float impact**: The receiving agent in Accra must have sufficient cash in GHS to disburse—the corridor system must check receiving-side float before confirming the transaction
5. **Failure modes**: What happens if the recipient never collects? (funds returned after 30-day expiry). What if Ghanaian compliance flags the transaction after Nigerian side has confirmed? (corridor dispute protocol with settlement reserve)

### Red Flag Answers

- Treats cross-border as a simple currency conversion ("just multiply by the exchange rate")
- Ignores the bilateral settlement problem ("the Ghana platform will just debit the Nigeria platform")
- Doesn't consider that the receiving agent needs physical cash in GHS
- Doesn't address the compliance asymmetry (one side flags, other side cleared)

---

## CBDC Integration Architecture Question

### Prompt (for architect-level candidates)

> "Nigeria's eNaira is gaining adoption. How would you integrate CBDC as a new transaction rail alongside cash and mobile money in your agent banking platform?"

### Key Challenges to Identify

1. **Three-sided float**: Agents now hold cash, e-float, AND eNaira tokens—the two-sided float conservation model breaks
2. **Settlement speed difference**: eNaira settles instantly (central bank ledger) while traditional e-float settles via batch clearing
3. **Central bank visibility**: All eNaira transactions are visible to the central bank in real-time—this changes the compliance model
4. **Conversion paths**: Cash → eNaira requires a two-step path (cash → e-float → eNaira) unless the platform creates a direct conversion mechanism
5. **Agent incentive**: Agents need a commission structure for eNaira transactions that makes it worthwhile to hold three liquidity types instead of two

### Exceptional Answer Adds

- The central bank has real-time visibility into all CBDC transactions—this fundamentally changes the compliance model (no need for platform-side STR filing for eNaira transactions, but new privacy concerns for customers)
- eNaira enables programmable money (conditional transfers, expiring vouchers)—agents become distribution points for government social programs
- The three-sided float model requires a new class of rebalancing: "triangle swaps" where cash, e-float, and eNaira are converted in a three-way exchange
