...
## Insight 11: On-Device LLMs Invert the Slowest part of the process from Compute to Memory Bandwidth

**Category:** Scaling
**One-liner:** For transformer-based models on edge devices, the inference Slowest part of the process is memory bandwidth (moving weights from DRAM to compute units) rather than arithmetic throughput, making quantization primarily a bandwidth optimization, not a compute optimization.
**Why it matters:** Edge NPUs and GPUs have sufficient TOPS (Tera Operations Per Second) to run 3B-parameter models in real time, but DRAM bandwidth is limited to 30-70 GB/s on mobile SoCs. A 3B-parameter FP16 model requires reading ~6 GB of weights per forward pass; at 50 GB/s bandwidth, the minimum latency floor is 120ms per token -- unacceptable for interactive use. INT4 quantization reduces the model to ~1.5 GB, bringing the bandwidth-limited latency to 30ms per token. This means quantization's primary value at the edge is reducing memory traffic, not reducing compute. Techniques like speculative decoding (using a small draft model to predict multiple tokens, then verifying with the large model) further amortize the bandwidth cost by processing multiple tokens per weight read.

---

## Insight 12: The Model Update Blast Radius Must Be Proportional to Validation Confidence

**Category:** Resilience
**One-liner:** Edge model updates should follow a staged rollout where the percentage of devices receiving the update is a function of cumulative validation metrics, not a fixed schedule.
**Why it matters:** Unlike server-side model deployments where rollback is instant, edge model updates are irreversible in practice -- you cannot force a device to downgrade if it is offline. A bad model update pushed to 100% of a billion-device fleet creates an incident that takes weeks to recover from (waiting for devices to come online and pull the fixed version). The staged rollout must be metric-driven: start with 0.1% of fleet (internal dogfood), expand to 1% if accuracy metrics hold, then 10%, 50%, 100%. At each stage, the system compares inference quality metrics (accuracy, latency, crash rate) between the new model cohort and the holdback. The key insight is that the expansion gates must be automatic -- human-in-the-loop approval does not scale across hundreds of model variants targeting different device tiers. This is the edge equivalent of the progressive config rollout pattern, but with a much higher blast radius because rollback requires physical device connectivity.
