...
## On-Device LLM Architecture (2025-2026)

### Small Language Model Serving on Edge
...
│  │  • Input context: 2K-8K tokens on device                     │   │
│  └──────────────────────────┬──────────────────────────────────┘   │
│                              │                                       │
│       ▼                                                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  KV-Cache Manager                                              │   │
│  │  • Pre-allocate KV cache for max context length               │   │
│  │  • Page attention for memory efficiency                       │   │
│  │  • Evict oldest entries under memory pressure                 │   │
│  └──────────────────────────┬──────────────────────────────────┘   │
│                              │                                       │
│       ▼                                                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  INT4/INT8 Transformer Decoder                                 │   │
│  │  • 1-7B parameters, INT4 quantized                           │   │
│  │  • Grouped-query attention (GQA) for memory efficiency       │   │
│  │  • NPU delegates: matrix multiply + softmax                  │   │
│  │  • CPU fallback: layer norm, residual connections            │   │
│  └──────────────────────────┬──────────────────────────────────┘   │
│                              │                                       │
│       ▼                                                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Speculative Decoding (optional, for latency)                  │   │
│  │  • Draft model (100M params) proposes K tokens                │   │
│  │  • Main model verifies in single forward pass                 │   │
│  │  • Achieves 2-3x tokens/sec improvement                      │   │
│  └──────────────────────────┬──────────────────────────────────┘   │
│                              │                                       │
│       ▼                                                              │
│  Streaming Token Output ("Here is the summary of...")               │
│                                                                      │
│  Memory Budget (flagship phone, 2025):                              │
│    Model weights (INT4, 3B params): ~1.5 GB                        │
│    KV cache (4K context):           ~200 MB                         │
│    Runtime overhead:                ~300 MB                          │
│    Total:                           ~2 GB (of 16 GB available)      │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Hybrid Cloud-Edge Inference Routing
...
flowchart TB
    ...
    Q --> Classify
    Classify -->|"Simple query<br/>(< 3B sufficient)"| SLM
    Classify -->|"Complex query<br/>(needs reasoning)"| LLM
    SLM -->|"< 30ms/token"| Q
    LLM -->|"~200ms TTFT"| Q
