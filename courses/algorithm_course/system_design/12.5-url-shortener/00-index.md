...
| Pattern | How It Applies to URL Shortening |
|---------|----------------------------------|
| **Cache-Aside / Write-Through** | The three-tier cache hierarchy (L1 in-process, L2 distributed, L3 database) uses cache-aside for reads (check cache first, backfill on miss) and write-through for creates (populate cache immediately on URL creation). |
| **CQRS** | Read path (redirect service) and write path (creation service) are separate services with independent scaling — the 100:1 ratio demands 100x more read capacity. |
| **Event Sourcing** | Click events are an append-only event log consumed by the analytics pipeline. Each redirect emits an immutable event; materialized views (hourly/daily rollups) are projections of this log. |
| **Circuit Breaker** | Protects the redirect hot path from database failures. When the URL store is unresponsive, the circuit opens and redirects are served from cache-only mode with degraded coverage. |
| **Consistent Hashing** | URL store sharding uses consistent hashing on the short code prefix, enabling horizontal scaling and minimizing data movement when shards are added or removed. |
| **Bulkhead** | Separate thread pools for redirect, creation, and analytics prevent a slow analytics pipeline from consuming resources needed by the latency-critical redirect path. |
