...
```
Per 1 billion redirects/month:

Compute (redirect service):
  ~40 instances × $150/month  = $6,000/month
  (115K QPS peak, ~2,900 QPS per instance)

Cache (distributed):
  Working set: 500M URLs × 250 bytes = 125 GB
  ~8 cache nodes × $500/month = $4,000/month

Storage (URL store):
  50B URLs × 300 bytes avg    = 15 TB
  Sharded across 128 nodes    ≈ $3,000/month (managed DB)

Analytics pipeline:
  Stream processing:           $2,000/month
  Columnar analytics store:    $1,500/month (90-day retention)

CDN / edge caching:
  10B edge requests/month      ≈ $5,000/month

Total: ~$21,500/month for 1B redirects → ~$0.0000215 per redirect
```
...
|--------|------------|-------------|
| Cache hit rate drop from 95% to 85% | Database load increases 3x → need more DB shards | Invest in larger cache cluster (cheaper than DB scaling) |
| 301 vs 302 redirect mix | 301 cached at CDN → 80% fewer origin requests | Encourage 301 for static links to reduce compute costs |
| Analytics retention | Raw events at 90 days: 10 TB; at 365 days: 40 TB | Aggressive rollup; keep only aggregated data beyond 90 days |
| Bot traffic percentage | 15-30% of redirects are bots; wasted compute | Bot detection in redirect path; skip analytics emission for known bots |
