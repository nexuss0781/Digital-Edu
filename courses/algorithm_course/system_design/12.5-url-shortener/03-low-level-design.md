...
## 6. Analytics Pipeline Data Model

### 6.1 Click Event Schema (Columnar Store)

```
TABLE click_events (columnar, partitioned by clicked_date)
  event_id        BIGINT        PRIMARY KEY    -- Snowflake ID
  short_code      CHAR(7)       NOT NULL       -- FK to URL
  clicked_at      TIMESTAMP     NOT NULL       -- Millisecond precision UTC
  clicked_date    DATE          NOT NULL       -- Partition key (derived from clicked_at)
  ip_hash         CHAR(64)      NOT NULL       -- SHA-256 of IP for privacy
  country_code    CHAR(2)       NULL           -- ISO 3166-1 alpha-2
  city            VARCHAR(128)  NULL           -- Geo-derived
  latitude        DECIMAL(8,5)  NULL           -- Approximate geo-coordinate
  longitude       DECIMAL(8,5)  NULL           -- Approximate geo-coordinate
  referrer_domain VARCHAR(256)  NULL           -- Truncated to domain only
  user_agent_raw  VARCHAR(512)  NULL           -- Original UA string
  device_type     VARCHAR(10)   NOT NULL       -- mobile | desktop | tablet | bot
  browser         VARCHAR(64)   NULL           -- Parsed browser name + version
  os              VARCHAR(64)   NULL           -- Parsed OS name + version
  is_unique       BOOLEAN       NOT NULL       -- First click from this ip_hash for this short_code in 24h
  is_bot          BOOLEAN       DEFAULT FALSE  -- Bot detection flag

PARTITIONING:
  BY RANGE (clicked_date) -- One partition per day
  RETENTION: 90 days raw, 2 years aggregated

INDEXES:
  PRIMARY KEY (event_id)
  INDEX idx_code_date (short_code, clicked_date)  -- Per-URL analytics queries
  INDEX idx_date_country (clicked_date, country_code) -- Geographic breakdown
```

### 6.2 Pre-Aggregated Rollup Tables

```
TABLE hourly_stats
  short_code      CHAR(7)       NOT NULL
  stat_hour       TIMESTAMP     NOT NULL       -- Truncated to hour
  total_clicks    INT           NOT NULL
  unique_clicks   INT           NOT NULL
  country_map     JSON          NOT NULL       -- {"US": 420, "GB": 180, ...}
  device_map      JSON          NOT NULL       -- {"mobile": 350, "desktop": 200, ...}
  referrer_map    JSON          NOT NULL       -- {"twitter.com": 150, "direct": 120, ...}
  bot_clicks      INT           DEFAULT 0

  PRIMARY KEY (short_code, stat_hour)

TABLE daily_stats
  -- Same structure as hourly_stats but aggregated to day
  -- Materialized from hourly_stats via scheduled rollup job

TABLE link_lifetime_stats
  short_code      CHAR(7)       PRIMARY KEY
  total_clicks    BIGINT        NOT NULL
  unique_clicks   BIGINT        NOT NULL
  peak_qps        INT           NOT NULL       -- Highest QPS ever recorded
  peak_hour       TIMESTAMP     NULL           -- When peak occurred
  first_click_at  TIMESTAMP     NULL
  last_click_at   TIMESTAMP     NULL
  top_country     CHAR(2)       NULL           -- Most frequent country
```

### 6.3 Analytics Aggregation Algorithm

```
ALGORITHM AggregateClickEvents(event_batch)
  INPUT: Batch of enriched click events from stream processor
  OUTPUT: Updated hourly rollup records

  // Group events by (short_code, truncated_hour)
  groups ← GROUP_BY(event_batch, e => (e.short_code, TRUNC_HOUR(e.clicked_at)))

  FOR EACH (key, events) IN groups:
    short_code, stat_hour ← key

    // Compute aggregates
    total ← COUNT(events)
    unique ← COUNT(DISTINCT events.ip_hash)
    countries ← COUNT_BY(events, e => e.country_code)
    devices ← COUNT_BY(events, e => e.device_type)
    referrers ← COUNT_BY(events, e => e.referrer_domain)
    bots ← COUNT(events WHERE is_bot = TRUE)

    // Upsert into hourly_stats (idempotent via deduplication)
    DB.UPSERT(
      table = hourly_stats,
      key = (short_code, stat_hour),
      increments = {
        total_clicks += total,
      ...
      merges = {
        country_map = MERGE_MAPS(existing.country_map, countries),
        device_map = MERGE_MAPS(existing.device_map, devices),
        referrer_map = MERGE_MAPS(existing.referrer_map, referrers)
      }
    )
    ...
    DB.UPSERT(
      table = link_lifetime_stats,
      key = short_code,
      increments = { total_clicks += total, unique_clicks += unique },
      updates = { last_click_at = MAX(events.clicked_at) }
    )
```

---

## 7. URL Reputation and Abuse Detection

### 7.1 Data Model

```
TABLE url_reputation
  long_url_hash   CHAR(64)      PRIMARY KEY    -- SHA-256 of long URL
  reputation_score DECIMAL(3,2) NOT NULL       -- 0.00 (malicious) to 1.00 (clean)
  last_checked_at TIMESTAMP     NOT NULL
  sources_flagged JSON          NULL           -- {"phishing_db": true, "malware_list": false}
  is_blocked      BOOLEAN       DEFAULT FALSE
  block_reason    VARCHAR(256)  NULL

TABLE abuse_reports
  report_id       UUID          PRIMARY KEY
  short_code      CHAR(7)       NOT NULL
  reporter_ip     CHAR(64)      NOT NULL       -- Hashed IP
  reason          VARCHAR(50)   NOT NULL       -- phishing | malware | spam | copyright
  reported_at     TIMESTAMP     NOT NULL
  status          VARCHAR(20)   DEFAULT 'pending' -- pending | reviewed | blocked | dismissed
```

### 7.2 URL Safety Check Algorithm

```
ALGORITHM CheckURLSafety(long_url)
  INPUT: URL to validate before shortening
  OUTPUT: SAFE | SUSPICIOUS | BLOCKED

  url_hash ← SHA256(long_url)

  // Check local cache first
  cached ← REPUTATION_CACHE.GET(url_hash)
  IF cached != NULL AND (NOW() - cached.last_checked_at) < 1 HOUR:
    IF cached.reputation_score < 0.3:
      RETURN BLOCKED
    IF cached.reputation_score < 0.7:
      RETURN SUSPICIOUS
    RETURN SAFE

  // Check against external reputation services (parallel)
  results ← PARALLEL_CHECK([
    safe_browsing_api.check(long_url),
    phishing_database.lookup(EXTRACT_DOMAIN(long_url)),
    malware_list.check(long_url),
    domain_age_check(EXTRACT_DOMAIN(long_url))
  ])

  // Compute composite score
  score ← weighted_average(results, weights = [0.4, 0.3, 0.2, 0.1])

  // Cache result
  REPUTATION_CACHE.SET(url_hash, {
    reputation_score: score,
    last_checked_at: NOW(),
    sources_flagged: results.flags
  }, TTL = 1 HOUR)

  IF score < 0.3:
    DB.INSERT(url_reputation, { url_hash, score, is_blocked = TRUE })
    RETURN BLOCKED
  IF score < 0.7:
    RETURN SUSPICIOUS  // Allow creation but flag for review
  RETURN SAFE
```

---

## 8. Vanity Domain Configuration

### 8.1 Data Model

```
TABLE vanity_domains
  domain          VARCHAR(256)  PRIMARY KEY    -- e.g., "brand.link"
  user_id         UUID          NOT NULL FK    -- Owner
  verified        BOOLEAN       DEFAULT FALSE  -- DNS verification status
  verification_token VARCHAR(64) NOT NULL      -- TXT record value for DNS verification
  ssl_cert_status VARCHAR(20)   DEFAULT 'pending' -- pending | provisioning | active | failed
  created_at      TIMESTAMP     NOT NULL

TABLE domain_dns_records
  domain          VARCHAR(256)  NOT NULL FK
  record_type     VARCHAR(10)   NOT NULL       -- CNAME | A | TXT
  record_value    VARCHAR(512)  NOT NULL
  verified_at     TIMESTAMP     NULL
```

### 8.2 Domain Verification Flow

```
ALGORITHM VerifyVanityDomain(domain, user_id)
  INPUT: Custom domain and owner
  OUTPUT: Verification status

  // Step 1: Generate verification token
  token ← RANDOM_STRING(64)
  DB.INSERT(vanity_domains, { domain, user_id, verification_token: token })

  // Step 2: Instruct user to add DNS records
  RETURN {
    instructions: [
      { type: "CNAME", host: domain, value: "redirect.platform.com" },
      { type: "TXT",   host: "_verify." + domain, value: token }
    ],
    status: "pending_verification"
  }

  // Step 3: Background verification (polled every 60 seconds)
  // Checks if DNS records are correctly configured
  FUNCTION BackgroundVerify(domain):
    txt_records ← DNS_LOOKUP(TXT, "_verify." + domain)
    cname_record ← DNS_LOOKUP(CNAME, domain)

    IF token IN txt_records AND cname_record == "redirect.platform.com":
      DB.UPDATE(vanity_domains, domain, { verified: TRUE })
      TRIGGER ssl_cert_provisioning(domain)  // Auto-provision TLS certificate
      RETURN VERIFIED
    RETURN PENDING
```
...
{
  "urls": [
    { "long_url": "https://example.com/page-1", "custom_alias": "p1" },
    { "long_url": "https://example.com/page-2" },
    { "long_url": "https://example.com/page-3", "expires_in_seconds": 86400 }
  ],
  "defaults": {
    "redirect_type": 302
  }
...
{
  "results": [
    { "index": 0, "status": 201, "short_code": "p1", "short_url": "https://short.ly/p1" },
    { "index": 1, "status": 201, "short_code": "b3X9kQ2", "short_url": "https://short.ly/b3X9kQ2" },
    { "index": 2, "status": 422, "error": "URL flagged as suspicious" }
  ],
  "summary": { "created": 2, "failed": 1 }
}

Rate Limits:
  - Free tier:   50 URLs per bulk request, 10 bulk requests/hour
  - Pro tier:    500 URLs per bulk request, 100 bulk requests/hour
  - Enterprise:  5,000 URLs per bulk request, unlimited
```

### 9.2 Bulk Analytics Export
...
Request Body:
{
  "short_codes": ["a1B2c3D", "x7Y8z9A", "p1"],
  "date_range": { "start": "2026-03-01", "end": "2026-03-15" },
  "format": "csv",
  "include_fields": ["clicked_at", "country_code", "device_type", "referrer_domain"]
}
...
Response (202 Accepted):
{
  "export_id": "exp-123",
  "status": "processing",
  "estimated_rows": 450000,
  "download_url": null  // Populated when ready
}

// Poll for completion
GET /api/v1/analytics/export/exp-123
Response (200 OK):
{
  "export_id": "exp-123",
  "status": "complete",
  "rows": 423891,
  "download_url": "https://exports.short.ly/exp-123.csv",
