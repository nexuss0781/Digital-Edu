...
| **Distributed KV** | In-memory cluster, managed NoSQL | High read throughput |
...
## Event Archival and Tiered Storage

Long-running event-sourced systems accumulate enormous event volumes. A tiered storage strategy keeps costs manageable while preserving the ability to replay.
...
│                                                                     │
│ Hot Tier (0-90 days):                                              │
│   Storage: NVMe SSD on event store cluster                        │
│   Access: All reads and subscriptions                              │
│   Latency: < 5ms                                                  │
│   Cost: $$$ per GB                                                │
│                                                                     │
│ Warm Tier (90 days - 1 year):                                     │
│   Storage: Standard SSD or tiered block storage                   │
│   Access: On-demand replay, rare queries                          │
│   Latency: < 50ms                                                 │
│   Cost: $$ per GB                                                 │
│                                                                     │
│ Cold Tier (1-7+ years):                                           │
│   Storage: Object storage (compressed, immutable)                 │
│   Access: Regulatory compliance, full rebuilds                    │
│   Latency: 100ms-1s (retrieval delay)                            │
│   Cost: $ per GB                                                  │
│                                                                     │
│ Migration Strategy:                                                │
│ • Events move from hot → warm → cold on schedule                 │
│ • Snapshots remain in hot tier (latest per stream)               │
│ • Projections use hot + warm tiers only                           │
│ • Full replay from cold tier: batch job, hours to complete        │
│                                                                     │
│ Cost Savings Example (2 TB/year event growth):                    │
│   All-hot 7 years: 14 TB × $0.25/GB/mo = $3,500/mo              │
│   Tiered: 2 TB hot + 2 TB warm + 10 TB cold                     │
│   = $500 + $200 + $50 = $750/mo (78% savings)                   │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
## Capacity Planning for Event Growth

```
┌────────────────────────────────────────────────────────────────────┐
│ EVENT STORE CAPACITY PLANNING                                      │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Growth Projection Formula:                                         │
│   Year_N_events = Year_1_events × growth_factor^(N-1)             │
│                                                                     │
│ Example (20% annual growth):                                       │
│   Year 1: 9B events (25M/day × 365)                              │
│   Year 2: 10.8B events                                            │
│   Year 3: 13B events                                              │
│   Year 5: 18.7B events                                            │
│   Cumulative after 5 years: ~60B events                           │
│                                                                     │
│ Storage at 700 bytes/event:                                        │
│   Cumulative 5-year raw: 42 TB                                   │
│   With indexes (1.4x): 59 TB                                     │
│   With compression (0.2x on cold): ~20 TB effective               │
│                                                                     │
│ Projection Rebuild Impact:                                         │
│   At 50K events/sec: 60B events = 13.9 days                      │
│   At 500K events/sec (parallel): 1.4 days                        │
│   Implication: full rebuilds become impractical beyond ~100B      │
│   events; incremental rebuilds or partial replay required         │
│                                                                     │
│ Scaling Triggers:                                                  │
│ • Add partition when write latency p99 > 20ms                    │
│ • Add projection worker when lag p99 > SLO threshold              │
│ • Archive to cold tier when hot storage > 80% capacity           │
│ • Split streams when average stream length > 50K events          │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
| **Fault tolerance** | Retry + dead letter + quarantine | Complexity in handling |
| **Event archival** | Tiered hot/warm/cold storage | Replay latency for old events |
| **Capacity planning** | Growth projections + scaling triggers | Rebuild time grows with data |
