...
## Key Algorithms (Step-by-step plan in plain English)

### 1. Geohash-Based Nearest Driver Query

```
Step-by-step plan in plain English: Find Nearest Available Drivers
...
```
Step-by-step plan in plain English: Compute Surge Multiplier for a Zone
...
```
Step-by-step plan in plain English: Dispatch Trip to Best Available Driver
...
```
Step-by-step plan in plain English: Fare Calculation
...
```
SCHEDULED_RIDE {
    uuid id PK                    "Scheduled ride identifier"
    uuid rider_id FK              "Requesting rider"
    float pickup_lat              "Pickup latitude"
    float pickup_lng              "Pickup longitude"
    string pickup_address         "Human-readable pickup"
    ...
    string dropoff_address        "Human-readable dropoff"
    string vehicle_type           "economy, premium, xl"
    timestamp scheduled_pickup_at "Requested pickup time"
    uuid pre_matched_driver_id FK "Driver assigned during pre-match window"
    string status                 "scheduled, pre_matched, assigned, converted_to_trip, cancelled, expired"
    uuid trip_id FK               "Linked trip after conversion"
    string city_id                "City for shard routing"
    timestamp created_at
    timestamp updated_at
}
```
...
```
RIDE_POOL {
    uuid id PK                    "Pool identifier"
    uuid driver_id FK             "Assigned driver"
    uuid vehicle_id FK            "Vehicle used"
    string status                 "open, full, in_progress, completed"
    int rider_count               "Current riders in pool (max 3)"
    int max_capacity              "Maximum riders allowed"
    jsonb waypoints               "Ordered list of pickup/dropoff stops"
    float total_route_distance_km "Current optimized route distance"
    int total_route_duration_min  "Current optimized route duration"
    string city_id                "City for shard routing"
    timestamp created_at
    timestamp updated_at
}
...
POOL_RIDER {
    uuid id PK                    "Pool-rider link identifier"
    uuid pool_id FK               "Associated pool"
    uuid rider_id FK              "Rider in this pool"
    uuid trip_id FK               "Individual trip record"
    float pickup_lat              "This rider's pickup"
    float pickup_lng
    float dropoff_lat             "This rider's dropoff"
    float dropoff_lng
    int pickup_sequence           "Order of pickup in waypoint list"
    int dropoff_sequence          "Order of dropoff in waypoint list"
    string status                 "waiting_pickup, in_vehicle, dropped_off, cancelled"
    float individual_fare         "This rider's allocated fare"
    float discount_percentage     "Pool discount applied (30-50%)"
...
```
SAFETY_ALERT {
    uuid id PK                    "Alert identifier"
    uuid trip_id FK               "Associated trip"
    uuid reporter_id              "User who triggered or system"
    string reporter_type          "rider, driver, system"
    string alert_type             "crash_detected, route_deviation, speed_violation,
                                   unexpected_stop, emergency_button, sos"
    string severity               "low, medium, high, critical"
    float alert_lat               "Location at alert time"
    float alert_lng
    jsonb detection_data          "Accelerometer data, speed, expected vs actual route"
    string status                 "detected, acknowledged, responding, resolved, false_alarm"
    uuid assigned_agent_id        "Safety team member handling"
    timestamp detected_at
    timestamp acknowledged_at
    timestamp resolved_at
    string resolution_notes       "How the alert was resolved"
}
```
...
```
DRIVER_EARNINGS {
    uuid id PK                    "Earnings record identifier"
    uuid driver_id FK             "Driver"
    string period_type            "daily, weekly"
    date period_start             "Period start date"
    date period_end               "Period end date"
    int trip_count                "Number of completed trips"
    float gross_fares             "Total fares before commission"
    float platform_commission     "Commission deducted"
    float surge_earnings          "Earnings from surge multiplier"
    float tips                    "Rider tips"
    float bonuses                 "Platform bonuses (quest, streak)"
    float tolls_reimbursed        "Toll reimbursements"
    float net_payout              "Final payout amount"
    string payout_status          "calculated, pending, paid, failed"
    string payout_method          "bank_transfer, instant_pay"
    float online_hours            "Total hours online"
    float active_hours            "Hours with active trips"
    float utilization_rate        "active_hours / online_hours"
    string currency               "ISO currency code"
    timestamp calculated_at
    timestamp paid_at
}
...
```
POST   /api/v1/trips/schedule
       Body: { pickup: {lat, lng, address}, dropoff: {lat, lng, address},
               vehicle_type: "economy", scheduled_pickup_at: "2026-03-22T08:00:00Z" }
       Response: { schedule_id: "sched_xxx", status: "scheduled", scheduled_pickup_at: "..." }

GET    /api/v1/trips/scheduled
       Response: { scheduled_rides: [{id, pickup, dropoff, scheduled_pickup_at, status}, ...] }

DELETE /api/v1/trips/schedule/{schedule_id}
...
```
POST   /api/v1/trips/{trip_id}/emergency
       Body: { alert_type: "sos", message: "Help needed" }
       Response: { alert_id: "alert_xxx", status: "detected", emergency_services_notified: true }

POST   /api/v1/trips/{trip_id}/share
       Body: { contacts: ["phone_1", "phone_2"], share_type: "real_time_link" }
       Response: { share_link: "https://...", expires_at: "..." }
```

### Pool Ride APIs

```
POST   /api/v1/trips/pool/request
       Body: { pickup: {lat, lng}, dropoff: {lat, lng}, max_detour_minutes: 10 }
       Response: { pool_id: "pool_xxx", matched: true, estimated_discount: "35%",
                   estimated_detour_minutes: 7, co_riders: 1 }

GET    /api/v1/trips/pool/{pool_id}/status
       Response: { pool_id, waypoints: [{type: "pickup", rider: "you", eta: 3},
                   {type: "pickup", rider: "co-rider", eta: 7}, ...],
                   current_leg: 2, total_legs: 4 }
```
...
### 5. Ride Pool Matching

```
Step-by-step plan in plain English: Pool Match Optimizer

FUNCTION optimize_pool_waypoints(pool, new_rider):
    // All current waypoints + new rider's pickup and dropoff
    all_points = pool.existing_waypoints + [new_rider.pickup, new_rider.dropoff]

    // Constraints
    constraints = {
        // Each rider must be picked up before dropped off
        pickup_before_dropoff: FOR rider IN all_riders: rider.pickup_index < rider.dropoff_index,
        // No rider experiences more than MAX_DETOUR additional time
        max_detour: FOR rider IN existing_riders: new_route_time[rider] - original_time[rider] <= MAX_DETOUR,
        // Driver route is reasonably efficient
        total_distance: new_total_distance <= original_distance * 1.5
    }

    // Try all valid orderings (small N, so Brute Force (Checking every single possibility) is feasible)
    // With 3 riders: 6 stops, ~720 permutations minus constraint violations
    best_ordering = null
    best_total_time = MAX_INT

    FOR ordering IN valid_permutations(all_points, constraints):
        total_time = compute_route_time(ordering)
        IF total_time < best_total_time:
            best_ordering = ordering
            best_total_time = total_time

    RETURN best_ordering, best_total_time


FUNCTION compute_pool_fare(rider, pool):
    // Each rider pays proportional to their segment of the route
    rider_direct_distance = routing_distance(rider.pickup, rider.dropoff)
    total_pool_distance = pool.total_route_distance
    rider_share = rider_direct_distance / total_pool_distance

    // Base fare (what they would pay for a solo ride)
    solo_fare = calculate_fare(rider_direct_distance, rider_estimated_duration)

    // Pool discount: 30-50% depending on number of co-riders and detour
    discount = BASE_POOL_DISCOUNT + (pool.rider_count - 1) * PER_RIDER_BONUS_DISCOUNT
    discount = min(discount, MAX_POOL_DISCOUNT)

    pool_fare = solo_fare * (1.0 - discount)
    RETURN pool_fare
```

### 6. Supply Repositioning Recommendation

```
Step-by-step plan in plain English: Driver Repositioning Suggestion

FUNCTION suggest_repositioning(driver_id, driver_location, city_id):
    // Get demand forecast for next 30 minutes
    demand_forecast = get_demand_forecast(city_id, time_window=30_minutes)
    current_supply = get_supply_by_zone(city_id)

    // Find zones with predicted undersupply
    undersupplied_zones = []
    FOR zone IN demand_forecast:
        predicted_demand = zone.forecast_demand
        current_drivers = current_supply[zone.h3_index]
        gap = predicted_demand - current_drivers

        IF gap > UNDERSUPPLY_THRESHOLD:
            distance = haversine(driver_location, zone.center)
            IF distance <= MAX_REPOSITION_DISTANCE_KM:
                undersupplied_zones.append({
                    zone: zone.h3_index,
                    gap: gap,
                    distance: distance,
                    predicted_surge: zone.predicted_surge,
                    estimated_wait_time: zone.predicted_wait_time
                })

    // Rank by expected earnings (predicted surge * predicted demand)
    undersupplied_zones.sort(by=predicted_surge * gap / distance, descending=true)

    IF undersupplied_zones:
        best = undersupplied_zones[0]
        RETURN {
            suggestion: "Drive to " + best.zone.name,
            distance: best.distance,
            predicted_wait: best.estimated_wait_time,
            predicted_surge: best.predicted_surge,
            incentive: compute_repositioning_incentive(best)
        }
    ELSE:
        RETURN {suggestion: null}  // Stay put; no undersupply nearby
```
