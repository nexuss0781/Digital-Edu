...
```
Step-by-step plan in plain English: Permission Model
...
```
Step-by-step plan in plain English: Location Fuzzing for Map Display
...
```
Step-by-step plan in plain English: GPS Spoofing Detection
...
|---|--------|-----------|--------|-------------------|
| T1 | GPS spoofing for surge/distance fraud | High | Medium | Multi-signal validation (speed, cell tower, sensor cross-check) |
| T2 | Account takeover (rider or driver) | Medium | High | Device fingerprinting + behavioral biometrics + step-up auth |
| T3 | Rider-driver collusion for fake trips | Medium | Medium | Pattern detection (same pair, circular trips, surge exploitation) |
| T4 | Data breach (location history, PII) | Low | Very High | Field-level encryption, access logging, per-document keys |
| T5 | Insider access to rider location data | Low | Very High | Audit logging on all location queries, role-based access, break-glass procedures |
| T6 | Payment fraud (stolen cards) | Medium | Medium | Tokenization, 3DS for high-risk, velocity checks |
| T7 | Physical safety incident during trip | Low | Critical | Crash detection, route deviation monitoring, emergency button, trip sharing |
| T8 | Supply manipulation (drivers gaming surge) | Medium | Low | Offline/online cycling detection, zone-entry timing analysis |
