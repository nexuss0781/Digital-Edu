...
| "Why not run everything at the nearest PoP?" | Understanding smart placement / data gravity | "Not all workloads benefit from user-nearest execution. A function making 5 origin DB calls at 80ms each adds 400ms of network overhead. Running near the origin eliminates those round-trips (local calls <1ms), adding only ~40ms of user-to-compute hop. Smart placement analyzes origin-call ratio and auto-routes data-intensive functions to PoPs near their databases." |
| "Why use WASM when V8 isolates already work?" | Understanding polyglot edge evolution | "V8 locks you into JavaScript/TypeScript. WASM with the Component Model enables compiled languages (Rust, Go, C++) with ~35μs cold starts, typed interfaces between components, and capability-based security. A Rust handler can call a Go validator and C++ ML module in the same sandbox. The trade-off is ecosystem maturity — debugging tools and library support lag behind JS." |
| "How would you add AI inference at the edge?" | Understanding tiered compute strategies | "Not all queries deserve GPU time. A three-tier strategy: (1) rule-based at edge for 60% of requests (regex, classification), (2) small quantized models (1-3B, INT4/INT8) for 30% at 50-100ms, (3) cloud GPU fallback for the 10% that need large models. A lightweight router model (<100M params) classifies which tier handles each request. This reduces cost 5-8x vs running everything on GPU." |
