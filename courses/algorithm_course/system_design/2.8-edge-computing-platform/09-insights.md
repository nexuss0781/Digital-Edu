...
**Why it matters:** Durable Objects guarantee single-instance consistency, which means migration must maintain this Rule that never changes across the transfer. The protocol works in three phases: (1) the old region stops accepting new requests and drains its queue, (2) state is transferred to the new region and verified, (3) the routing table is updated and the old region responds with 307 redirects to any requests that arrive during the transition. Idempotency keys make client retries safe. The critical design choice is to perform migrations during low-traffic periods and to make the transfer atomic -- there is never a moment when both regions believe they own the object. This is essentially a distributed lock transfer problem, and getting it wrong means either split-brain (two instances accepting writes) or unavailability (zero instances accepting writes).

---

## Insight 9: Smart Placement Inverts the Edge Paradigm -- Move Compute to Data, Not Data to Users

**Category:** Traffic Shaping
**One-liner:** Not all edge workloads benefit from running at the user-nearest PoP; workloads that make 5+ origin database calls per request are faster when placed near the database, because eliminating 5 round-trips to the origin saves more latency than the single extra hop to a non-nearest PoP.

**Why it matters:** The default edge computing assumption -- "run code closest to the user" -- optimizes for the wrong metric when the workload is data-intensive. Consider a function that authenticates a user (1 DB call), fetches their preferences (1 call), queries their dashboard data (3 calls), and assembles the response. Running at the user-nearest PoP means 5 origin round-trips at 80ms each = 400ms of network overhead. Running near the origin eliminates all 5 round-trips (local calls at <1ms), adding only one hop of user-to-compute latency (~40ms). Smart placement analyzes the ratio of origin calls to user-facing latency and automatically routes the function to a PoP near its data sources when that ratio exceeds a threshold. This "data gravity" principle extends to edge databases: instead of replicating the entire database to all PoPs, write operations route to the primary region while reads serve from local replicas, accepting the read-your-writes latency for global distribution.

---

## Insight 10: WebAssembly Component Model Enables Polyglot Edge Composition Without Language Lock-In

**Category:** Architecture
**One-liner:** The Wasm Component Model defines typed interfaces between compiled modules from different languages, enabling an edge function written in Rust to call a library compiled from Go, with the runtime enforcing interface contracts and capability-based security at composition time.

**Why it matters:** V8 isolates lock you into JavaScript/TypeScript. Containers support any language but have 100x worse cold starts. WebAssembly's Component Model provides the missing middle: compile any language to a Wasm component with a typed interface (using the WIT -- Wasm Interface Type -- specification), then compose components together regardless of source language. A request handler written in Rust can call a JSON schema validator compiled from Go and an ML inference module compiled from C++, all running in the same sandbox with sub-millisecond overhead. The capability-based security model means each component only accesses the specific host APIs it declares (HTTP, KV, clock), preventing a compromised library from accessing the network if it only declared filesystem access. WASI Preview 2 standardizes these host capabilities (wasi-http, wasi-keyvalue, wasi-messaging), making edge functions portable across platforms (Fastly Compute, Fermyon Spin, wasmCloud) without code changes. The trade-off is ecosystem maturity: debugging tools, source maps, and library availability for Wasm lag behind JavaScript and container ecosystems.

---

## Insight 11: Edge AI Inference Requires a Hybrid Tiered Strategy -- Not All Queries Deserve GPU Time

**Category:** Cost Optimization
**One-liner:** Running all AI inference at the edge wastes GPU resources on simple queries; a three-tier strategy (rule-based at edge for 60% of requests, small model at edge for 30%, cloud GPU fallback for 10%) reduces cost by 5-8x while maintaining p95 latency targets.

**Why it matters:** Edge AI inference faces a resource constraint that cloud inference does not: GPUs at edge PoPs are scarce and shared across thousands of tenants. Running a 7B parameter model for every request is neither feasible nor necessary. The three-tier approach classifies requests by complexity: simple pattern matching (regex, rule-based) handles 60% of requests (spam detection, content classification) with zero model cost. Small quantized models (1-3B parameters, INT4/INT8) handle 30% at the edge with acceptable latency (50-100ms). Complex queries (10%) fall back to cloud GPU inference where larger models are available. The classification itself runs at the edge using a lightweight router model (<100M parameters) that predicts which tier will produce an acceptable result. This mirrors the two-stage retrieval pattern in search (fast first stage, precise second stage) applied to inference: handle the easy cases cheaply and locally, escalate only when necessary.

---

## Insight 12: Edge Database Replication Creates a Write Funnel That Inverts Traditional Read/Write Scaling

**Category:** Replication
**One-liner:** Edge databases (distributed SQLite, edge KV) replicate reads to all PoPs but funnel writes to a single primary region, creating a topology where read capacity scales linearly with PoP count but write capacity is fixed -- the inverse of traditional sharded databases.

**Why it matters:** Traditional database scaling adds shards to increase write throughput, with reads distributed across replicas. Edge databases invert this: every PoP has a read replica (read capacity = N PoPs × per-PoP throughput), but all writes route to a single primary (write capacity = one region's throughput). This is perfectly suited to read-heavy workloads (90%+ reads) like content delivery, user preferences, and configuration. But write-heavy workloads (collaborative editing, real-time counters) cannot scale writes beyond the primary's capacity. The architectural implication is that write-heavy edge state must use a different primitive: Durable Objects (single-writer with strong consistency), CRDTs (multi-writer with automatic conflict resolution), or write-local-read-global patterns where each PoP writes to local state and merges asynchronously. The choice between these patterns depends on whether conflicts are acceptable (CRDTs), whether writes can be serialized (Durable Objects), or whether some write loss is tolerable (last-write-wins).
