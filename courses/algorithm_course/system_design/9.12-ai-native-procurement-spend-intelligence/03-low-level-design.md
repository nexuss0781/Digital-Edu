...
| `PREDICTION_LOG` | `model_id` + range on `predicted_at` | Hash (model) + Range (time) | Model-specific analysis with time-based Cutting off unnecessary steps |
...
// Time Complexity (Speed of the algorithm): O(d) where d = embedding dimension (dominated by model inference)
// Space Complexity (Memory usage of the algorithm): O(d + h) where h = vendor history size
```
...
// Time Complexity (Speed of the algorithm): O(n_features + n_news + G) where G = graph traversal cost
// Space Complexity (Memory usage of the algorithm): O(n_features + G_nodes)
```
...
// Time Complexity (Speed of the algorithm): O(H * F) where H = historical data points, F = features
// Space Complexity (Memory usage of the algorithm): O(H + I) where I = number of market indices
```
...
// Time Complexity (Speed of the algorithm): O(R + F) where R = number of rules, F = feature extraction
// Space Complexity (Memory usage of the algorithm): O(F)
```
...
| Algorithm | Time Complexity (Speed of the algorithm) | Space Complexity (Memory usage of the algorithm) | Dominant Factor |
|-----------|----------------|-----------------|-----------------|
...
## 7. Algorithm 5: Split Purchase Detection

Split purchasing occurs when requesters divide a large purchase into smaller POs to stay below the autonomous approval threshold. This undermines governance controls and must be detected proactively.

### Detection Pipeline

```
function detectSplitPurchases(newPO, recentPOs, config):
    // Rule-based detection (fast, deterministic)
    sameSenderSameSupplier = recentPOs.filter(
        po -> po.requester_id == newPO.requester_id
          AND po.supplier_id == newPO.supplier_id
          AND po.created_at > now() - config.window_hours
    )

    IF sameSenderSameSupplier.length > 0:
        combinedAmount = newPO.amount + SUM(sameSenderSameSupplier.amount)
        IF combinedAmount > config.autonomous_threshold
           AND newPO.amount < config.autonomous_threshold:
            RETURN SplitPurchaseAlert(
                type: "COMBINED_EXCEEDS_THRESHOLD",
                combined_amount: combinedAmount,
                related_pos: sameSenderSameSupplier,
                confidence: HIGH
            )

    // Statistical detection (threshold clustering)
    thresholdProximity = (config.autonomous_threshold - newPO.amount)
                         / config.autonomous_threshold
    IF thresholdProximity < 0.05:  // within 5% below threshold
        recentNearThreshold = recentPOs.filter(
            po -> po.requester_id == newPO.requester_id
              AND (config.autonomous_threshold - po.amount)
                  / config.autonomous_threshold < 0.10
              AND po.created_at > now() - 30 days
        )
        IF recentNearThreshold.length >= config.pattern_threshold:
            RETURN SplitPurchaseAlert(
                type: "CLUSTERING_NEAR_THRESHOLD",
                pattern_count: recentNearThreshold.length,
                confidence: MEDIUM
            )

    // ML-based detection (catches sophisticated circumvention)
    features = extractSplitFeatures(newPO, recentPOs)
    mlScore = splitDetectionModel.predict(features)
    IF mlScore > config.ml_threshold:
        RETURN SplitPurchaseAlert(
            type: "ML_PATTERN_DETECTED",
            score: mlScore,
            confidence: mlScore > 0.9 ? HIGH : MEDIUM
        )

    RETURN null  // no split purchase detected
```

### Feature Engineering for Split Detection

```
Split purchase features:
  - requester_po_count_7d: Number of POs by same requester in 7 days
  - requester_supplier_po_count_7d: POs to same supplier by same requester
  - amount_to_threshold_ratio: PO amount / autonomous threshold
  - combined_amount_to_threshold_ratio: Sum with related POs / threshold
  - requester_avg_po_amount_30d: Historical average PO amount for requester
  - amount_deviation: |PO amount - requester avg| / requester stddev
  - category_match_count: POs in same category by requester in window
  - timing_regularity: Std dev of time gaps between related POs (regular = suspicious)
```
