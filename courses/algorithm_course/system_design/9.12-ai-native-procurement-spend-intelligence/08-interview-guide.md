...
## 6. Extension Deep Probes

### Probe A: Agentic AI Orchestration for Complex Sourcing

**Scenario**: "A department head submits a free-text request: 'We need to replace our current logistics provider in Southeast Asia by Q3. Budget is $2M annually. Must have cold chain capability and ISO 14001 certification.'"

**What the interviewer explores**:
- How does the intake agent parse unstructured requirements into structured constraints?
- How does the sourcing agent search for suppliers matching multiple criteria (geography, capability, certification, budget)?
- How do agents coordinate: intake → budget validation → supplier matching → RFx generation → PO creation?
- How are agent boundaries defined? What happens when an agent is uncertain?
- How do you prevent agent hallucination in procurement decisions (factual grounding)?

**Strong answer elements**:
- RAG architecture: agents ground responses in contract database, supplier profiles, and spend history
- Structured output validation: agent outputs are validated against schemas before execution
- Confidence thresholds: agents escalate to humans when confidence drops below category-specific threshold
- Audit trail: every agent action logged with reasoning chain for SOX compliance

### Probe B: Cross-Tenant Data Strategy for Global Intelligence

**Scenario**: "Your platform serves 3,000 enterprise customers. How do you leverage aggregate spend data to improve intelligence while maintaining strict tenant isolation?"

**What the interviewer explores**:
- How do you train a global model without exposing individual tenant data?
- What if a tenant opts out of data sharing? How does architecture support this?
- How do you measure the value of aggregate data vs. tenant-specific data?
- How do you prevent the global model from leaking competitive intelligence?

**Strong answer elements**:
- Differential privacy (ε-DP) for global model training with formal guarantees
- Federated learning alternative: model updates aggregated without centralizing raw data
- Shapley value attribution to quantify each tenant's contribution to model accuracy
- Opt-out architecture: tenant data excluded from global training pipeline without degrading their local model

---

## 7. Key Technical Concepts Quick Reference

| Concept | Definition | Where It Applies |
|---------|------------|-----------------|
| **EWMA Temporal Smoothing** | Exponentially weighted moving average that dampens noise in time-series signals | Supplier risk score stability |
| **Hierarchical Classification** | Independent ML models at each taxonomy level (L1→L4), preventing error propagation | Spend classification engine |
| **Feature Store (Online/Offline)** | Dual-layer system: offline for training (point-in-time correct), online for inference (sub-ms latency) | All ML model features |
| **Training-Serving Skew** | Divergence between features computed during training vs. inference, causing silent accuracy degradation | Feature store prevents this |
| **Bi-Temporal Data Model** | Two time dimensions (transaction time + valid time) enabling point-in-time queries | Spend cube for SOX audit |
| **Corporate Hierarchy Graph** | Parent → subsidiary → division graph enabling spend aggregation at any organizational level | Vendor resolution pipeline |
| **Reservation Pattern** | Reserve budget → execute operation → confirm/release; prevents TOCTOU race conditions | Budget double-spend prevention |
| **Active Learning** | Strategically selecting the most informative samples for human labeling to maximize model improvement per annotation | Cold-start spend classification |
| **Differential Privacy** | Mathematical guarantee that individual records cannot be identified from model outputs (parameterized by ε) | Multi-tenant global model training |
| **Canary Deployment** | Route small percentage of traffic to new model version; monitor accuracy before full rollout | ML model version switching |

---

## 8. Behavioral Assessment Dimensions

| Dimension | What Interviewers Evaluate | Positive Signals |
|-----------|---------------------------|-----------------|
| **Financial Systems Thinking** | Does the candidate treat budget as a distributed transaction problem? | Mentions pessimistic locking, reservation pattern, SOX audit implications |
| **ML System Design Maturity** | Does the candidate go beyond "use ML" to address training-serving skew, cold-start, feedback loops? | Discusses feature store, point-in-time correctness, anti-oscillation, active learning |
| **Domain Awareness** | Does the candidate understand procurement-specific challenges? | Mentions vendor name resolution, split purchase detection, three-way matching |
| **Compliance Intuition** | Does the candidate proactively raise audit trail, explainability, and governance? | Designs for SOX from the start; mentions AI explainability for autonomous decisions |
| **Scale Calibration** | Does the candidate provide realistic numbers and identify bottlenecks? | Correctly sizes GPU requirements, identifies document processing as the Slowest part of the process, discusses tenant isolation |
