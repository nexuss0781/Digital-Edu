...
| **Centralized ML Platform** ✅ | Shared feature store; consistent model lifecycle; efficient GPU utilization; unified monitoring | Single team Slowest part of the process; more complex deployment; cross-team coordination |
...
flowchart LR
    ...
    subgraph FeatureEng["Feature Engineering"]
        FP["Feature Pipeline<br/>(Batch + Streaming)"]
        FS["Feature Store<br/>(Online + Offline)"]
    end

    subgraph Training["Model Training"]
        GT["Global Model<br/>Training"]
        TT["Tenant Fine-<br/>Tuning"]
        MR["Model Registry<br/>(Versioned Artifacts)"]
    end

    subgraph Serving["Model Serving"]
        SC["Spend<br/>Classification"]
        RS["Risk<br/>Scoring"]
        PO_D["PO Decision<br/>Engine"]
        PR["Price<br/>Optimization"]
    end

    subgraph Monitoring["ML Monitoring"]
        DR["Drift<br/>Detection"]
        AC["Accuracy<br/>Tracking"]
        FB["Feedback<br/>Loop"]
    end
    ...
    class ERP,INV,SIG,MKT ingestion
    class FP,FS feature
    class GT,TT,MR train
    class SC,RS,PO_D,PR serve
    class DR,AC,FB monitor
```
...
|----------|--------|-----------|
| **Global vs. tenant models** | Hybrid: global baseline + tenant fine-tuning | Global provides cold-start coverage; fine-tuning captures organization-specific patterns |
| **Training frequency** | Weekly global retraining; daily incremental fine-tuning | Balance freshness with compute cost; critical updates trigger ad-hoc retraining |
| **Feature computation** | Unified pipeline for online + offline | Eliminates training-serving skew; same code produces features for both training and inference |
| **Model deployment** | Blue-green with canary rollout | Shadow mode validates new models before traffic exposure; automatic rollback on accuracy degradation |
| **Privacy for global training** | Differential privacy (ε = 1.0) | Formal guarantee that individual tenant data cannot be reverse-engineered from global model weights |

---

## 8. Decision Deep Dive: Vendor Resolution Strategy

### Problem

The same supplier appears under dozens of name variations across data sources. Without resolution, spend analytics fragments and supplier risk scores are inconsistent.

### Options Evaluated

| Option | Pros | Cons |
|--------|------|------|
| **Rule-based normalization only** | Simple; fast; deterministic | Misses complex variations; high maintenance burden for rules |
| **ML-only matching** | Handles novel variations; learns from corrections | Requires training data; black-box decisions hard to audit; expensive at scale |
| **Multi-stage pipeline** ✅ | Each stage handles different difficulty levels; deterministic stages are auditable; ML handles long tail | More complex to build and operate; requires multiple matching engines |

### Decision

Multi-stage pipeline: exact hash → normalized name match → phonetic/edit distance → ML-based contextual match → periodic clustering for new duplicates. This processes 95% of matches deterministically (auditable, fast) and reserves ML for the remaining 5% of ambiguous cases.

The pipeline maintains a corporate hierarchy graph linking parents, subsidiaries, and divisions. Spend can be aggregated at any level dynamically. Human-confirmed matches feed back into the ML matcher as training examples.
