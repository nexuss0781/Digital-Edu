...
| Signal | Metric | Scale-Up Threshold | Scale-Down Threshold | Action |
|--------|--------|-------------------|---------------------|--------|
| **PO Creation Rate** | Pending PO queue depth | > 500 for 2 min | < 50 for 10 min | Scale PO Engine instances ±1 |
| **Classification Queue** | Unclassified transactions | > 5,000 for 5 min | < 500 for 15 min | Scale GPU classification workers ±2 |
| **Document Processing** | Document queue depth | > 100 for 5 min | < 20 for 15 min | Scale GPU OCR/NLP workers ±1 |
| **Risk Signal Ingestion** | Consumer lag | > 10 min lag for 5 min | < 1 min lag for 15 min | Scale signal consumers ±2 |
| **Dashboard Queries** | Active analytical queries | > 500 concurrent | < 100 concurrent | Scale read replicas ±1 |
| **Feature Store Latency** | Feature serving p99 | > 20ms for 3 min | < 5ms for 15 min | Scale online store shards |

### Pre-Scaling Events

```
Known demand spikes that trigger pre-emptive scaling:
  - Month-end close: Invoice volume surges 3-5x (schedule GPU scale-up 2 days before)
  - Quarter-end: Dashboard usage peaks during QBR preparation (add read replicas)
  - Annual contract renewal season: Contract processing volume peaks (scale NLP workers)
  - Fiscal year planning: Spend analytics queries surge (pre-warm materialized views)
  - Geopolitical events: Major events trigger risk signal bursts (auto-scale consumers)
```

---

## 6. Tenant Onboarding Scaling

### Data Migration Pipeline

```
New tenant onboarding involves migrating historical data:
1. ERP data extraction: Pull 3-5 years of PO, invoice, and supplier data
2. Document ingestion: Process historical contracts through NLP pipeline
3. Vendor resolution: Resolve and deduplicate supplier names
4. Spend classification: Classify historical transactions (backfill)
5. Feature store backfill: Compute historical features for model training
6. Tenant model fine-tuning: Train tenant-specific models on classified data

Scaling considerations:
  - Onboarding a large enterprise ($5B+ spend) generates 10M+ historical records
  - Backfill classification runs at 5,000 txn/sec → ~33 minutes for 10M records
  - Document processing: 100K contracts × 120 sec avg = ~3,300 GPU-hours
  - Total onboarding time: 2-4 weeks (dominated by document processing)
  - Onboarding workloads isolated from production via separate compute pools
```

### Multi-Tenant Resource Isolation

```
Resource isolation ensures noisy-neighbor prevention:
  - GPU inference: Tenant-fair scheduling with per-tenant request quotas
  - Database: Row-level isolation with mandatory tenant_id filter
  - Event Bus: Per-tenant consumer groups for independent processing
  - Cache: Tenant-scoped cache partitions (prevents one tenant's cache from evicting another's)
  - Rate Limiting: Per-tenant API rate limits (configurable per tier)

  Monitoring: per-tenant resource consumption tracked for capacity planning and billing
```
...
|----------|---------|---------------|
| **Two-Speed Document Processing** | 60% GPU reduction | Route structured electronic documents to CPU-based parsers; reserve GPUs for scanned/image documents |
| **Spot/Preemptible GPU for Training** | 70% training cost | Model training uses spot instances with 10-minute checkpointing; production inference uses on-demand |
| **Tiered Storage for Spend Cube** | 50% storage cost | Hot tier (90 days) on fast columnar storage; warm tier (1 year) on compressed storage; cold tier (7 years) on archive storage |
| **Materialized View Caching** | 40% query cost | Pre-compute common dashboard aggregations; serve from cache instead of scanning raw data |
| **Batch Inference Scheduling** | 30% inference cost | Non-urgent classification (historical backfill) runs during off-peak hours on cheaper compute |
| **Feature Store TTL Tuning** | 25% cache cost | Stable features (industry, geography) cached with long TTL; volatile features (risk signals) cached with short TTL |
...
```
Idempotency strategy for PO creation:
  - Client generates a unique idempotency_key per request (UUID v4)
  - PO Engine checks idempotency store before processing:
    IF idempotency_key exists in store:
      RETURN cached response (no re-processing)
    ELSE:
      Process PO creation
      Store result with idempotency_key (TTL: 24 hours)
      RETURN new response
  ...
  This prevents:
  - Client retries from creating duplicate POs
  - Network timeout retries from double-charging budgets
  - API Gateway retry policies from duplicating financial transactions
```

### Event Processing Idempotency

```
Event consumers use event_id for deduplication:
  - Each event carries a globally unique event_id
  - Consumer maintains a processed event log (bloom filter + DB check)
  - Before processing: check if event_id already processed
  - After processing: record event_id in processed log
  - Bloom filter provides fast negative check (99.9% accuracy)
  - DB check confirms for bloom filter positives (exact)
```
