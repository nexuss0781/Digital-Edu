...
### Slowest part of the process 1: Document Intelligence Pipeline (OCR + NLP) Throughput
...
### Slowest part of the process 2: Spend Cube Query Latency Under High Concurrency
...
### Slowest part of the process 3: Supplier Risk Signal Ingestion at Scale
...
## 6. Deep Dive: Three-Way Matching Engine

### Architecture Overview

The Three-Way Matching Engine automates the reconciliation of Purchase Orders, Goods Receipts, and Invoices---a historically manual process that consumes significant accounts payable resources. The engine must handle tolerance-based matching (allowing configurable variance for quantities and prices), partial deliveries (goods receipt covers only part of the PO), and multi-line matching (invoice line items map to different PO lines).

### Matching Pipeline

```
Invoice Received → Document Intelligence (OCR/parse) →
Line Item Extraction → PO Matching → Goods Receipt Matching →
Tolerance Check → Auto-Approve or Exception Queue
```
...
For each invoice line item:
  1. Match to PO: invoice_supplier_id = po_supplier_id
     AND invoice_item matches po_line_item (fuzzy matching for descriptions)
     AND invoice_unit_price within ±X% of po_unit_price (configurable, typically 2-5%)

  2. Match to Goods Receipt: po_id matches gr_po_id
     AND received_qty >= invoiced_qty (within tolerance)
     AND receipt_date <= invoice_date (goods received before invoiced)

  3. Tolerance evaluation:
     Price variance = |invoice_price - po_price| / po_price
     Quantity variance = |invoice_qty - received_qty| / po_qty

     IF price_variance < price_tolerance AND qty_variance < qty_tolerance:
       AUTO_APPROVE
     ELIF price_variance < 2x tolerance OR qty_variance < 2x tolerance:
       ROUTE to buyer for review (gray zone)
     ELSE:
       ROUTE to exception queue (significant deviation)
```
...
| Metric | Value |
|--------|-------|
| Auto-match rate (target) | > 85% of invoice lines |
| Matching latency (p95) | < 5 seconds per invoice |
| Exception resolution time | < 48 hours (SLO) |
| False positive rate (incorrect auto-match) | < 0.5% |

### Handling Partial Deliveries

When goods are received in multiple shipments against a single PO line, the engine maintains a running tally:

```
PO Line: 1000 units of Widget-A at $10/unit
  GR-1: 400 units received (2026-01-15)
  GR-2: 350 units received (2026-02-01)
  GR-3: 250 units received (2026-02-15) ← completes PO line

Invoice-1 (for GR-1 + GR-2): 750 units × $10 = $7,500
  Match: 750 units invoiced vs. 750 units received (GR-1 + GR-2) ✓
  Open GR balance: 250 units (GR-3)

Invoice-2 (for GR-3): 250 units × $10.15 = $2,537.50
  Match: 250 units invoiced vs. 250 units received (GR-3) ✓
  Price variance: 1.5% → within tolerance (2%) → AUTO_APPROVE
```
