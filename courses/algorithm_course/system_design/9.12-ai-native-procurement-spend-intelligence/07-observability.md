...
## 0. Observability Architecture

```mermaid
flowchart TB
    subgraph Sources["Telemetry Sources"]
        SVC["Core Services<br/>(PO, Approval, Sourcing)"]
        ML["ML Pipeline<br/>(Classification, Risk, Pricing)"]
        DOC["Document Intelligence<br/>(OCR, NLP)"]
        INT["External Integrations<br/>(ERP, Supplier Feeds)"]
    end

    subgraph Ingest["Collection & Ingestion"]
        OC["Telemetry Collector<br/>(Metrics, Logs, Traces)"]
        EP["Event Processor<br/>(Business Events)"]
    end

    subgraph Store["Storage & Analysis"]
        TS["Time-Series DB<br/>(Metrics)"]
        LS["Log Aggregator<br/>(Structured Logs)"]
        TR["Trace Store<br/>(Distributed Traces)"]
        AD["Audit Database<br/>(Immutable, 10-Year)"]
    end

    subgraph Analyze["Intelligence & Action"]
        AN["Anomaly Detection<br/>(Statistical + ML)"]
        AL["Alert Manager<br/>(Tiered Routing)"]
        DB["Dashboard Engine<br/>(Real-Time + Historical)"]
        RB["Runbook Automation<br/>(Incident Response)"]
    end
    ...
    class SVC,ML,DOC,INT source
    class OC,EP ingest
    class TS,LS,TR,AD store
    class AN,AL,DB,RB analyze
```
...
## 6. Procurement Health SLIs

| SLI | Description | Target | Measurement |
|-----|-------------|--------|-------------|
| **PO Completion Rate** | Percentage of requisitions that result in a delivered PO | > 95% | Requisitions reaching PO_DELIVERED state within 30 days |
| **Autonomous Accuracy** | Percentage of autonomous POs that would have been approved by a human | > 98% | Post-hoc sampling: 5% of autonomous POs reviewed by procurement team |
| **Classification Correctness** | Percentage of auto-classified transactions confirmed correct | > 95% (L2) | Monthly validation against human-labeled sample set |
| **Risk Score Predictive Value** | Percentage of high-risk alerts that precede actual supplier incidents | > 40% | Track alerts against subsequent supplier events within 90 days |
| **Budget Integrity** | Percentage of cost centers where committed spend ≤ budget | 100% | Continuous reconciliation; any violation is P1 incident |
| **Three-Way Match Rate** | Percentage of invoices auto-matched without exception | > 85% | Auto-matched / total invoices processed |
| **Spend Visibility** | Percentage of total spend classified at L2 or deeper | > 95% | Classified spend / total spend (by value) |
| **Supplier Coverage** | Percentage of spend associated with scored suppliers | > 90% | Spend with risk-scored suppliers / total spend |

---

## 7. Compliance and Audit Observability

### SOX Control Monitoring

```
Control Point                     Metric                           Alert
─────────────────────────────────────────────────────────────────────────
Separation of duties              Same user creates and approves   P1: immediate block
                                  same PO
Autonomous PO governance          Autonomous PO created above      P1: suspend autonomy for category
                                  configured threshold
Budget authorization              PO created without budget check  P1: PO flagged for review
Approval chain integrity          Approver not in authorized       P1: PO blocked
                                  chain for category/amount
Audit log completeness            Any PO state transition          P2: gap detected in audit trail
                                  without audit record
Model version tracking            Autonomous decision without      P2: decision not auditable
                                  logged model version + features
```
...
|--------|-------------|-----------------|
| `ai.bias.accuracy_by_supplier_size` | Classification accuracy segmented by supplier revenue quartile | > 5% variance between quartiles |
| `ai.explainability.coverage` | Percentage of autonomous decisions with complete explanation records | < 100% |
| `ai.human_override.rate` | Rate at which humans override AI recommendations | > 30% (indicates trust issue or accuracy problem) |
| `ai.model.version_in_production` | Current model versions serving traffic | Version age > 30 days (indicates stale model) |
| `ai.fairness.supplier_recommendation_diversity` | Entropy of supplier recommendations per category | Low entropy → over-concentration on few suppliers |
