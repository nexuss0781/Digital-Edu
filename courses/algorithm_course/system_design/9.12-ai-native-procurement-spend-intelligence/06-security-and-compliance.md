...
## 8. Incident Response for Procurement Security Events

### Security Incident Classification

| Type | Example | Severity | Response |
|------|---------|----------|----------|
| **Data Breach** | Supplier PII exfiltrated | P1 | Isolate affected systems; notify affected suppliers within 72h; engage forensics |
| **Unauthorized PO** | PO created bypassing approval chain | P1 | Freeze PO; audit approval workflow; investigate bypass mechanism |
| **Cross-Tenant Data Access** | Query returned data from wrong tenant | P1 | Block query path; audit all recent cross-tenant queries; investigate data access layer |
| **Sanctions Violation** | PO issued to sanctioned entity | P1 | Cancel PO; file regulatory report; investigate screening gap |
| **AI Manipulation** | Adversarial input to manipulate classification or risk score | P2 | Quarantine affected transactions; review model inputs; add input validation |
| **Credential Compromise** | API key or admin credentials exposed | P2 | Rotate credentials; audit access logs; review blast radius |

### Post-Incident Actions

```
After any P1 procurement security incident:
  1. Preserve evidence (audit logs, system snapshots, affected records)
  2. Determine blast radius (which tenants, suppliers, POs affected)
  3. Notify affected parties per regulatory requirements
  4. Root cause analysis within 48 hours
  5. Implement remediation controls
  6. Update threat model and security controls
  7. Conduct tabletop exercise for similar scenarios within 30 days
  8. Report to board/audit committee if financially material
```
