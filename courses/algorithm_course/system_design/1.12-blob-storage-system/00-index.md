...
| Aspect | Amazon S3 | Google Cloud Storage | Facebook Haystack | SeaweedFS |
|--------|-----------|---------------------|-------------------|-----------|
| **Scale** | 400T+ objects, 100s EB | Not disclosed | 260B photos, 20 PB | Billions of files |
| **Data Durability** | 11 nines | 11 nines | 3x replication | Erasure coding + replication |
| **Consistency** | Strong (since 2020) | Strong | Eventual | Strong (Raft consensus) |
| **Key Innovation** | ShardStore (Rust LSM), 5-of-9 EC | Colossus backend | 1 disk op/read | 24-byte/file memory index |
| **Storage Backend** | ShardStore with separated K/V | Colossus (GFS successor) | Log-structured volumes | Master + volume server |
| **Erasure Coding** | 5-of-9 (1.8x overhead) | Not disclosed | None (3x replication) | Configurable RS schemes |
| **Primary Use Case** | General cloud storage | GCP ecosystem | Photo serving | Self-hosted S3-compatible |
...
| **FastCDC** | Gear fingerprinting with sub-minimum skip and normalized distribution | High-throughput dedup | ~10x faster than Rabin-based CDC, near-identical dedup ratio |
...
|--------|---------------|-------------|----------------|------------------|
| **Amazon S3** | 400+ trillion | Hundreds of exabytes | 150M+ RPS, 1+ PB/s | ShardStore (Rust LSM), 5-of-9 erasure coding, CRDT consistency |
...
| **SeaweedFS** | Billions | Petabyte-scale | Enterprise | Haystack-inspired, O(1) seeks, 24 bytes/file index |
...
| **Chunking** | Fixed-length | Content-defined (CDC/FastCDC) | Fixed for simplicity; FastCDC when deduplication matters |
| **Metadata Storage** | Co-located | Separated service | Separated for scale (independent scaling of metadata vs data) |
| **Storage Tiering** | Single tier | Multi-tier lifecycle (5 tiers) | Multi-tier for cost optimization (67%+ savings); single for simplicity |
| **Storage Engine** | B-tree | LSM with separated K/V | LSM for write-heavy (ShardStore pattern); B-tree for read-heavy metadata |
| **Object Size Limit** | 5 TB | 50 TB (2025+) | 50 TB with multipart upload for modern workloads |
...
| Metadata Service | Separation from data, scalability | Treating metadata as trivial; it's often the Slowest part of the process |
...
|---------|-------------|------|
| **Content Delivery Network** | CDN uses blob storage as origin for edge caching | [1.15 CDN](../1.15-content-delivery-network-cdn/00-index.md) |
| **YouTube** | Video platform backed by blob storage for media segments | [5.1 YouTube](../5.1-youtube/00-index.md) |
| **Disney+ Hotstar** | Live + VOD streaming with blob-based media pipeline | [5.5 Disney+ Hotstar](../5.5-disney-hotstar/00-index.md) |
| **Snapchat** | Ephemeral media storage with aggressive lifecycle tiering | [4.8 Snapchat](../4.8-snapchat/00-index.md) |
| **Feature Store** | ML feature datasets stored as Parquet objects in blob storage | [3.16 Feature Store](../3.16-feature-store/00-index.md) |
| **RAG System** | Document corpus and embedding vectors stored in blob storage | [3.15 RAG System](../3.15-rag-system/00-index.md) |
| **Event Sourcing** | Event log snapshots and compacted segments persisted to blob storage | [1.18 Event Sourcing](../1.18-event-sourcing-system/00-index.md) |
| **Cloud Provider Architecture** | Blob storage as foundational primitive in cloud infrastructure | [2.1 Cloud Provider Architecture](../2.1-cloud-provider-architecture/00-index.md) |
...
- [ShardStore: S3's Storage Engine (SOSP 2021)](https://jamesbornholt.com/papers/shardstore-sosp21.pdf)
...
- [FastCDC: A Fast and Efficient CDC Approach](https://csyhua.github.io/csyhua/hua-tpds2020-dedup.pdf)
- [ACM Survey on Erasure Coding for Storage Systems (2025)](https://dl.acm.org/doi/full/10.1145/3708994)
