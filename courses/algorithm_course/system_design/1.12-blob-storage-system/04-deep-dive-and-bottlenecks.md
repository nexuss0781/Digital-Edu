...
### 1. Metadata Service Slowest part of the process

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: METADATA SERVICE                                       │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Symptoms:                                                          │
...
### 2. Network Bandwidth Slowest part of the process

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: NETWORK BANDWIDTH                                      │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Symptoms:                                                          │
...
### 3. Disk I/O Slowest part of the process

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: DISK I/O                                               │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Symptoms:                                                          │
...
### ShardStore: Production Storage Engine Architecture

```
┌────────────────────────────────────────────────────────────────────┐
│ DEEP DIVE: ShardStore (S3's Storage Engine)                        │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Overview:                                                          │
│   S3's per-node storage engine, written in ~40,000 lines of Rust  │
│   for type safety and performance.                                 │
│                                                                     │
│ Architecture: LSM Tree with Separated Key/Value                   │
│ ┌─────────────────────────────────────────────────────────────┐   │
│ │                                                              │   │
│ │   Write Path:                                                │   │
│ │   1. Shard data → append to data log on disk                │   │
│ │   2. Key (shard ID) + pointer → insert into LSM tree        │   │
│ │   3. LSM tree only stores keys + disk offsets (compact)     │   │
│ │                                                              │   │
│ │   Read Path:                                                 │   │
│ │   1. Look up key in LSM tree → get disk offset              │   │
│ │   2. Seek to offset in data log → read shard data           │   │
│ │   3. Verify checksum chain of custody                        │   │
│ │                                                              │   │
│ │   Key Innovation: Separating shard data from the LSM tree   │   │
│ │   reduces write amplification dramatically. Compaction only  │   │
│ │   rewrites small key/offset pairs, NOT the large shard data.│   │
│ │                                                              │   │
│ └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│ Crash Consistency:                                                 │
│   - Soft-updates protocol (no WAL required)                       │
│   - Ordering constraints on disk writes ensure consistency        │
│   - Lightweight formal verification in CI/CD pipeline             │
│                                                                     │
│ HDD Optimization:                                                  │
│   - Operation scheduling to minimize seek distance                │
│   - Write coalescing to maximize sequential throughput             │
│   - Background compaction rate-limited to avoid I/O contention   │
│                                                                     │
│ Checksum Chain of Custody:                                         │
│   ┌─────────────────────────────────────────────────────────────┐│
│   │ SDK computes checksum → transmitted via HTTP Trailers →    ││
│   │ API gateway verifies → storage node verifies →             ││
│   │ on-disk checksum stored alongside shard data →             ││
│   │ read-time verification before returning to client          ││
│   │                                                              ││
│   │ Any break in the chain = corruption detected before         ││
│   │ data reaches the client (prevents silent corruption)        ││
│   └─────────────────────────────────────────────────────────────┘│
│                                                                     │
│ Erasure Coding Configuration (5-of-9):                             │
│   - 5 data shards + 4 parity shards per object                   │
│   - Storage overhead: 1.8x (vs 3x for replication)               │
│   - Tolerates 4 simultaneous shard losses                         │
│   - 4 straggler nodes tolerated (vs 2 with 3x replication)      │
│                                                                     │
│ Workload Decorrelation:                                            │
│   - "Power of Two Random Choices" for shard placement             │
│   - Place on least-loaded of two randomly selected nodes          │
│   - Individual workloads bursty, but aggregate is smooth          │
│   - Millions of tenants → predictable throughput distribution    │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
## Deep Dive 4: FastCDC and Content-Defined Chunking
...
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Problem with Rabin-based CDC:                                      │
│   - Rabin fingerprint computation is CPU-intensive                │
│   - ~50 MB/s throughput per core (too slow for high-speed I/O)   │
│                                                                     │
│ FastCDC Solution (3 combined techniques):                         │
│                                                                     │
│ 1. GEAR FINGERPRINTING (replace Rabin hash)                       │
│    ┌─────────────────────────────────────────────────────────────┐│
│    │ // Precompute random table G[256]                           ││
│    │ fingerprint = 0                                             ││
│    │ for each byte b in stream:                                  ││
│    │     fingerprint = (fingerprint << 1) + G[b]                ││
│    │     if fingerprint & MASK == 0:                             ││
│    │         emit_chunk_boundary()                               ││
│    │                                                              ││
│    │ Key: Only XOR and shift — no modular arithmetic            ││
│    │ Result: ~10x faster than Rabin fingerprinting              ││
│    └─────────────────────────────────────────────────────────────┘│
│                                                                     │
│ 2. SUB-MINIMUM CHUNK CUT-POINT SKIPPING                          │
│    - Skip first min_size bytes (no boundary check needed)        │
│    - Reduces unnecessary hash comparisons by 30-50%              │
│                                                                     │
│ 3. NORMALIZED CHUNK-SIZE DISTRIBUTION                             │
│    ┌─────────────────────────────────────────────────────────────┐│
│    │ Use two masks:                                              ││
│    │   MASK_LARGE (fewer 1-bits) for [min_size, avg_size)       ││
│    │   MASK_SMALL (more 1-bits) for [avg_size, max_size)        ││
│    │                                                              ││
│    │ Effect: Chunks cluster around target average size           ││
│    │ Result: Tighter size distribution → better dedup           ││
│    └─────────────────────────────────────────────────────────────┘│
│                                                                     │
│ Performance Comparison:                                            │
│ ┌───────────────────────────────────────────────────────────────┐ │
│ │ Algorithm     │ Throughput/core │ Dedup Ratio │ Distribution  │ │
│ │───────────────┼─────────────────┼─────────────┼───────────────│ │
│ │ Rabin-CDC     │ ~50 MB/s       │ Baseline    │ Wide variance │ │
│ │ Gear-CDC      │ ~200 MB/s      │ -1%         │ Wide variance │ │
│ │ FastCDC       │ ~500 MB/s      │ -0.5%       │ Normalized    │ │
│ │ SIMD-FastCDC  │ ~1.2 GB/s      │ -0.5%       │ Normalized    │ │
│ └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
│ Use Cases:                                                         │
│   - Backup systems with incremental deduplication                 │
│   - Sync engines (detect changed regions efficiently)             │
