...
|---------|--------------|-----------------|
| Ignoring metadata | Metadata is often the Slowest part of the process | Design metadata service explicitly |
...
│                                                                     │
│ Edge Case (Unusual or extreme situation): If client abandons, lifecycle policy auto-aborts       │
...
|---------|--------|------------------|
| Treating metadata as trivial | Metadata often Slowest part of the process at scale | Design explicit metadata service with sharding strategy |
...
6. **"How does the storage engine handle write amplification on HDDs?"**
   - Separate shard data from index (ShardStore pattern)
   - LSM tree stores only keys + disk offsets (small)
   - Shard data appended to data log (sequential writes)
   - Compaction rewrites only small key/offset pairs, NOT data
   - Soft-updates crash consistency avoids WAL overhead

7. **"How would you design intelligent storage tiering?"**
   - 5-tier automated model: Frequent → Infrequent → Archive Instant → Archive → Deep Archive
   - Monitor per-object access patterns (last access timestamp)
   - Auto-demote after idle thresholds (30d, 90d, 180d)
   - Auto-promote on any access back to Frequent tier
   - Exempt objects < 128 KB (monitoring cost exceeds savings)
   - Real-world savings: ~67% on mixed workloads

8. **"What is the difference between 5-of-9 and 10-of-14 erasure coding?"**
   - 5-of-9: 1.8x overhead, tolerates 4 failures, used in production S3
   - 10-of-14: 1.4x overhead, tolerates 4 failures, higher throughput for large objects
   - Trade-off: wider stripe (more nodes) = lower overhead but higher repair network cost
   - 5-of-9 chosen for balance of overhead, repair speed, and straggler tolerance
