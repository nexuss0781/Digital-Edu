...
---
flowchart TB
    subgraph Edge["Edge Layer (Per Station)"]
        INS["Inspection\nMetrics"]
        CAM["Camera\nHealth"]
        DEV["Device\nHealth"]
        SYNC["Sync\nStatus"]
    ...
    subgraph Gateway["Factory Gateway"]
        AGG["Metric\nAggregator"]
        LBUF["Log\nForwarder"]
    end

    subgraph Cloud["Cloud Observability Stack"]
        ...
        subgraph Present["Presentation"]
            FACTORY_DASH["Factory Floor\nDisplay"]
            QM_DASH["Quality Manager\nDashboard"]
            FLEET_DASH["Fleet Overview\nDashboard"]
            INCIDENT["Incident\nManager"]
        end
    end

    INS & CAM & DEV & SYNC --> AGG
    ELOG --> LBUF
    AGG --> METRIC_IN
    LBUF --> LOG_IN
    AGG --> TRACE_IN
    ...
    TSDB --> ALERT & ANOMALY & SLA
    LOGDB --> ALERT
    ALERT --> INCIDENT
    SLA --> QM_DASH & FLEET_DASH
    ANOMALY --> ALERT
    TSDB --> FACTORY_DASH & QM_DASH & FLEET_DASH
    ...
    classDef present fill:#fffde7,stroke:#f57f17,stroke-width:2px

    class INS,CAM,DEV,SYNC,ELOG edge
    class AGG,LBUF gateway
    class METRIC_IN,LOG_IN,TRACE_IN ingest
    class TSDB,LOGDB,TRACEDB store
    class ALERT,ANOMALY,SLA process
    class FACTORY_DASH,QM_DASH,FLEET_DASH,INCIDENT present
```
