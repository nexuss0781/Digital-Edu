...
| FR-13 | **Diffusion-based synthetic defect generation** — When operators provide fewer than 30 defect examples per class, automatically generate photorealistic synthetic defect images using conditioned diffusion models; synthetic defects should vary in size, severity, position, and orientation while maintaining physical plausibility for the material type | Generate 10-50x synthetic expansion from seed defect examples; synthetic image quality validated by FID score against real defects; operator review of synthetic samples before inclusion in training |
| FR-14 | **Production schedule integration** — Accept production schedule data from factory MES/ERP systems to automatically switch inspection models during product changeovers, correlate defect rates with production batches, and adjust inspection parameters based on product type | Support OPC-UA, REST API, and CSV schedule import; model switching latency ≤ 3 seconds (preloaded); automatic batch-ID tagging of all inspection events |
| FR-15 | **Cross-factory model transfer** — Enable tenants with multiple factories to share trained models across locations, adapting a model trained at Factory A to Factory B with minimal additional training data from Factory B | Cross-factory model transfer with ≤ 50% of the training data normally required; domain adaptation layer handles factory-specific lighting and camera differences |
...
| Metric | Target |
|---|---|
| Tenant data isolation | 100% — zero cross-tenant data leakage; enforced at storage and query layer |
| Training data provenance | Every image traceable to upload source, timestamp, and labeler identity |
| Model lineage tracking | Every deployed model traceable to training dataset, hyperparameters, and validation metrics |
| Audit log integrity | Append-only with cryptographic hash chain; tamper-evident |
| Data residency compliance | Tenant-configurable data region; no cross-region transfer without explicit consent |

### Edge Hardware Compatibility

| Metric | Target |
|---|---|
| Supported edge device types | 10+ (from $35 single-board computers to $500 industrial edge servers) |
| Supported camera interfaces | USB3 Vision, GigE Vision, MIPI CSI-2 |
| Supported industrial protocols | Modbus TCP/RTU, OPC-UA, GPIO, EtherNet/IP |
| Model compilation targets | 5+ NPU/accelerator architectures (cross-compilation from single training pipeline) |
| Edge OS support | Minimal Linux (Yocto/Buildroot custom), standard Linux (Ubuntu/Debian ARM) |

### Operator Experience

| Metric | Target |
|---|---|
| Time to first trained model (new tenant) | ≤ 1 day from account setup to deployed model on first station |
| Training UI learning curve | Factory quality manager with no ML background can train first model independently after 30-minute guided tutorial |
| Validation report comprehensibility | 100% of metrics presented in operator language (catch rate, false reject rate, example images); zero ML jargon (no precision, recall, F1, AUC) |
| Active learning review session | ≤ 20 minutes per shift to review 20-50 flagged images |
| Model retraining trigger | Operator can initiate retraining with one click; system auto-suggests when performance degrades |

### Latency Budget Breakdown

```
Total budget: 150 ms from hardware trigger to GPIO actuation

Allocation:
  ┌────────────────────────────┬───────────┬────────────────────────────┐
  │ Phase                      │ Budget    │ Notes                      │
  ├────────────────────────────┼───────────┼────────────────────────────┤
  │ Trigger → capture start    │ 1-2 ms    │ Hardware interrupt latency │
  │ Camera exposure            │ 1-5 ms    │ Depends on lighting        │
  │ Frame transfer to device   │ 5-10 ms   │ USB3/GigE/MIPI interface   │
  │ Preprocessing              │ 5-10 ms   │ WB, ROI, resize, normalize │
  │ Model inference (INT8)     │ 30-80 ms  │ Core NPU compute           │
  │ Postprocessing + decision  │ 2-5 ms    │ NMS, threshold, severity   │
  │ GPIO actuation signal      │ 1-5 ms    │ Digital output assertion   │
  │ Margin                     │ 33-105 ms │ Safety buffer              │
  └────────────────────────────┴───────────┴────────────────────────────┘
```
