...
### Slowest part of the process: Thermal Throttling Under Sustained Load
...
### Slowest part of the process: The Cold-Start Problem for New Defect Types
...
4. **Positional variation**: If the part is not in exactly the same position and orientation for every capture, the model must learn to be position-Rule that never changes—wasting model capacity on a problem that should be solved mechanically.
...
### Slowest part of the process: The Camera-Model Resolution Mismatch
...
### Slowest part of the process: Model Drift from Tooling Wear
...
### Slowest part of the process: Multi-Product Lines and Model Switching
...
### Slowest part of the process: Vision Foundation Model Distillation for Edge

As vision foundation models (DINOv2, SAM, Florence) mature, there is growing interest in using their powerful feature representations for industrial defect detection. However, these models are orders of magnitude too large for edge inference:

```
Foundation model vs. edge-viable model comparison:

| Model             | Parameters | Size (FP32) | Inference Time  | Edge Viable? |
|-------------------|------------|-------------|-----------------|--------------|
| SAM (ViT-H)       | 636M       | 2.5 GB      | ~500 ms (GPU)   | No           |
| DINOv2 (ViT-B)    | 86M        | 344 MB      | ~100 ms (GPU)   | No           |
| DINOv2 (ViT-S)    | 22M        | 88 MB       | ~40 ms (GPU)    | Marginal     |
| EfficientNet-Lite0 | 4.7M      | 19 MB       | ~30 ms (NPU)    | Yes          |
| MobileNetV3-Small  | 2.5M      | 10 MB       | ~15 ms (NPU)    | Yes          |

Distillation approach:
  1. Use foundation model as teacher to label a large unlabeled
     industrial image dataset (millions of images from the platform)
  2. Train a lightweight student model (MobileNetV3) to reproduce
     the teacher's feature representations
  3. Student learns rich feature representations without needing
     labeled data — the teacher's features serve as supervision

Status (2025-2026):
  - Distilled MobileNetV3 achieves ~85% of DINOv2's feature quality
    at 1/30th the model size and 10x faster inference
  - Useful as an improved pre-trained backbone for the platform
  - Full foundation model inference on edge remains impractical
    until edge NPUs reach 20-50 TOPS at current price points
    (projected 2027-2028)
```

### Slowest part of the process: Defect Class Imbalance
...
## Deep Dive 4: Diffusion-Based Synthetic Defect Generation

### The Evolution Beyond Traditional Augmentation

Traditional data augmentation (rotation, flip, color jitter) creates variants of existing images but cannot generate genuinely new defect appearances. A factory with 30 scratch examples and traditional augmentation produces 300 variants of those 30 scratches — the model memorizes the specific scratches rather than learning the general pattern "what scratches look like."

Conditioned diffusion models change this fundamentally:

```
FUNCTION DiffusionSyntheticGeneration(defect_examples, good_images, defect_type, count):
    // Phase 1: Learn the defect distribution
    // Fine-tune a pre-trained diffusion model on the defect examples
    // Conditioned on: defect_type, severity_level, approximate_size
    defect_generator = FineTuneDiffusion(
        base_model = platform.defect_diffusion_backbone,
        training_images = defect_examples,
        conditioning = {
            "defect_type": defect_type,
            "material_class": InferMaterialClass(good_images)
        },
        fine_tune_steps = 500  // Few-step fine-tuning, not full training
    )

    // Phase 2: Generate diverse synthetic defects
    synthetic_set = []
    FOR i IN RANGE(count):
        // Sample random conditioning parameters
        severity = RandomUniform(0.3, 1.0)   // Low to high severity
        size = RandomUniform(0.01, 0.15)      // 1-15% of image area
        position = RandomPosition()            // Anywhere on part surface

        // Select random good-part background
        background = RandomChoice(good_images)

        // Generate defect with controllable parameters
        synthetic_defect = defect_generator.Generate(
        ...
        quality_score = AssessPhysicalPlausibility(synthetic_defect, defect_type)
        IF quality_score > 0.7:
            synthetic_set.APPEND(synthetic_defect)

    RETURN synthetic_set
```

### Key Advantages Over Traditional Synthesis

```
Comparison: Traditional vs. Diffusion-Based Synthesis

| Dimension            | Traditional (Cut-Paste)    | Diffusion-Based           |
|----------------------|----------------------------|---------------------------|
| Defect diversity     | Limited to existing shapes | Novel, physically plausible|
| Background blending  | Visible artifacts at edges | Seamless integration      |
| Controllability      | Size + position only       | Size, severity, texture,  |
|                      |                            | orientation, depth        |
| Material awareness   | None (defect looks same    | Material-conditioned      |
|                      | on any background)         | (scratch on metal vs.     |
|                      |                            | plastic looks different)  |
| Min examples needed  | 30-50 per defect class     | 10-20 per defect class    |
| Compute cost         | CPU, < 1 sec per image     | GPU, 2-5 sec per image    |
| Platform requirement | None                       | Pre-trained diffusion     |
|                      |                            | backbone (~500MB)         |
```

### Slowest part of the process: Synthetic Defect Validation

The biggest risk with synthetic data is generating images that look realistic to humans but confuse the training pipeline — creating defects that are out of distribution for the actual manufacturing process. For example, generating a "crack" on glass that follows a perfectly straight line when real glass cracks are always branching/irregular.

```
Validation pipeline for synthetic defects:
  1. Automated physical plausibility check:
     - Defect features (orientation, curvature, edge sharpness) compared
       to statistical distributions from real defect examples
     - Flag synthetic defects that fall outside 2σ of real defect distributions

  2. Feature-space diversity check:
     - Synthetic defects mapped to feature space using the domain backbone
     - Verify that synthetic set covers the feature-space region occupied
       by real defects (not clustered in a narrow sub-region)
     - Verify that synthetic set doesn't extend far beyond the real defect
       region (would be out-of-distribution and potentially harmful)

  3. Operator spot-check:
     - Present 20 synthetic images mixed with 10 real images
     - Operator marks each as "realistic" or "unrealistic"
     - If > 10% of synthetics marked unrealistic: adjust generation parameters

  4. Training impact validation:
     - Compare model trained with vs. without synthetic data on real test set
     - Synthetic data should improve test-set accuracy by > 2%
     - If accuracy degrades: synthetic distribution is misleading the model
```
