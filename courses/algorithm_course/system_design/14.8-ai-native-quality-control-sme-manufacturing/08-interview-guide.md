...
**If video inspection is chosen**:
- "How does continuous web inspection differ from discrete part inspection?"
- Expected: Line-scan cameras capture one row at a time; image is a continuous stream, not discrete frames; defect detection becomes a streaming inference problem; defects can span multiple scan lines (e.g., a tear in fabric); need temporal defect tracking (same defect seen across frames is ONE defect, not N defects); throughput measured in meters/minute, not parts/minute.

**If predictive quality is chosen**:
- "How would you correlate defect patterns with upstream process parameters?"
- Expected: Collect process data (temperature, pressure, speed, tool age) from PLC/SCADA alongside inspection results; time-align process events with inspection events accounting for production pipeline delay; use correlation analysis and ML regression to identify leading indicators; example — "when mold temperature exceeds 205C, scratch defect rate increases 3x within 100 parts"; present as actionable alerts: "Mold temperature rising — defect rate likely to increase within 15 minutes."
...
| Concept | What to Know | Common Mistake |
|---|---|---|
| **INT8 quantization** | Reduces model weights/activations to 8-bit integers; 2-4x speedup, 4x memory reduction; 0.5-1.5% accuracy loss typical; low-contrast defects lose disproportionate accuracy | Assuming accuracy loss is uniform across classes |
| **Transfer learning** | Initialize from pre-trained backbone, fine-tune on task-specific data; domain-specific backbones (industrial surfaces) are 5-10x more data-efficient than ImageNet | Assuming ImageNet is the best starting point for all domains |
| **Hardware trigger** | Photoelectric sensor/encoder fires hardware interrupt; < 1ms jitter; ensures every part captured at exact same position | Using software polling (timer-based) which introduces non-deterministic timing |
| **Shadow mode** | New model runs in parallel logging predictions without actuating; compared against production model on identical inputs | Deploying directly to production ("ship and monitor") |
| **Active learning** | Selectively request labels on uncertain examples; 10-50x more label-efficient than random sampling | Random sampling for retraining data |
| **Tiled inference** | Break high-res image into overlapping crops; process each crop at native model resolution; merge via NMS | Resizing full image to model input, losing small-defect detail |
| **Focal loss** | Down-weights easy examples, up-weights hard examples; handles extreme class imbalance (98% good, 2% defective) | Using standard cross-entropy with imbalanced data |
| **Two-stage cascade** | Stage 1: fast anomaly detector (5-10ms) filters obviously good parts; Stage 2: full classifier (50-80ms) runs only on flagged parts | Running full classifier on every part, wasting 95%+ of compute |
...
|---|---|
| **Physical-digital integration thinking** | Does the candidate naturally think about the physical manufacturing environment (lighting, vibration, temperature) and how it constrains the digital system? |
| **Cost consciousness** | Does the candidate track the cost implications of design choices? A $100 edge device vs. $150 edge device matters when multiplied by 10,000 stations |
| **Failure mode reasoning** | Does the candidate reason about what happens when components fail? Edge crash, camera failure, network outage, model regression? |
| **Operator empathy** | Does the candidate design for the factory quality manager (no ML expertise) rather than for an ML engineer? |
| **Latency budget awareness** | Does the candidate maintain a running latency budget, tracking how each pipeline stage consumes the 150ms total budget? |
| **Asymmetric risk awareness** | Does the candidate recognize that false negatives (missed defects) are catastrophically more expensive than false positives (rejected good parts)? |
...
```
Line speed:             10-120 parts/min
Inference budget:       50-100 ms
Total trigger-to-act:   ≤ 150 ms
Edge device cost:       $100-$150 (compute module)
Full station cost:      $500-$3,000
Traditional MV cost:    $20,000-$90,000
Min training images:    50 good + 20 defect per class
Model size (INT8):      5-30 MB
Edge NPU:               2-8 TOPS INT8
Defect rate:            0.1-5% of production
False positive target:  ≤ 3%
Recall target:          ≥ 95%
Offline buffer:         48 hours
Shadow mode:            500+ inspections before promotion
```

### Architecture Sketch Components

```
Edge side (per station):
  Trigger → Camera → Edge AI → GPIO → Reject Mechanism
                ↕ (async)
  Local SQLite + Image Buffer + Cloud Sync Agent

Cloud side (shared):
  API Gateway → [Training Pipeline | Fleet Management | Analytics]
  ↕
  [Image Store | Metadata DB | Model Registry | Time-Series DB]

Key link: Factory Gateway (aggregation proxy between edge fleet and cloud)
```
