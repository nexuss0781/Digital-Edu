...
Model version lifecycle:
  ...
  DRAFT → TRAINING → VALIDATING → READY → DEPLOYING → SHADOW → PRODUCTION → DEPRECATED

  Key invariants:
  1. Every deployed model has a complete lineage:
     training_job → dataset_snapshot → augmentation_config →
     architecture → training_metrics → quantization_config →
     compilation_target → validation_report → deployment_record

  2. Two production model versions may coexist across stations
     (rolling deployment), but never on the same station

  3. Every station has exactly one PRODUCTION model and at most
     one SHADOW model and one ROLLBACK model in memory

  4. Model artifacts are content-addressed (SHA-256 hash of weights):
     identical models are stored once even if trained by different tenants
     from the same domain template

  5. Model deletion cascades:
     delete model → orphan check (any station still running this version?)
     → if no: mark for cleanup → purge from object storage after 30 days
     → if yes: block deletion, notify operator
```

### Edge Device Memory Layout

```
Memory map for a typical 4GB edge device running inspection:

  Address Range       Usage                         Size
  ─────────────────────────────────────────────────────────
  0x0000_0000         Linux kernel + OS services    400 MB
  0x1900_0000         Edge runtime + libraries      200 MB
  0x2580_0000         NPU driver + inference engine 100 MB
  0x2F00_0000         Production model weights       30 MB
  0x30E0_0000         Shadow model weights (opt)     30 MB
  0x32C0_0000         Rollback model weights          30 MB
  0x34A0_0000         Camera frame buffer A           10 MB
  0x3540_0000         Camera frame buffer B           10 MB
  0x35E0_0000         Preprocessing workspace         30 MB
  0x37C0_0000         NPU activation memory          100 MB
  0x3DE0_0000         SQLite DB + WAL                 50 MB
  0x3FF0_0000         Cloud sync buffers              30 MB
  ─────────────────────────────────────────────────────────
  Total allocated:                                  1,020 MB
  Available headroom:                               3,076 MB
  Reserved for peak activations + overflow:         2,000 MB
  True free memory:                                 1,076 MB

  Design constraint: All memory allocated at startup. Zero dynamic
  allocation during inference path. This eliminates:
    - Garbage collection pauses (no managed runtime)
    - Memory fragmentation (fixed-size pools)
    - Out-of-memory crashes during sustained operation
