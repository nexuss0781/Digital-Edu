...
### Slowest part of the process #1: Event Bus Throughput

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: Message Broker Throughput                               │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Symptoms:                                                          │
...
### Slowest part of the process #2: Projection Processing Speed

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: Slow Projection Updates                                 │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Symptoms:                                                          │
│  • High projection lag                                             │
...
### Slowest part of the process #3: Projection Rebuild Time

```
┌────────────────────────────────────────────────────────────────────┐
│ Slowest part of the process: Slow Projection Rebuilds                                │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
...
## Case Study: E-Commerce Platform Migration to CQRS

### Context

A mid-sized e-commerce platform handling 50,000 orders/day migrated from a monolithic CRUD architecture to CQRS to address three growing pains: product listing pages taking 2-3 seconds to load (complex joins across 12 tables), order writes blocking reads during peak hours, and an inability to add new query patterns (recommendation feeds, seller analytics) without impacting write performance.

### Migration Strategy

```
┌────────────────────────────────────────────────────────────────────┐
│ CQRS Migration: Strangler Pattern Over 6 Months                    │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Phase 1 (Month 1-2): Dual-Write                                   │
│  ┌───────────────────────────────────────────────────────────┐     │
│  │ Write to existing DB (source of truth)                     │     │
│  │ Outbox pattern publishes events                            │     │
│  │ Shadow projections build read models in parallel           │     │
│  │ All reads still go to old DB                               │     │
│  │ Comparison: log discrepancies between old and new reads    │     │
│  └───────────────────────────────────────────────────────────┘     │
│                                                                     │
│  Phase 2 (Month 3-4): Gradual Read Migration                       │
│  ┌───────────────────────────────────────────────────────────┐     │
│  │ Product listing: switch to read model (verified accurate)  │     │
│  │ Order history: switch to read model                        │     │
│  │ Search: switch to dedicated search projection              │     │
│  │ Seller dashboard: still reads from old DB (complex)        │     │
│  └───────────────────────────────────────────────────────────┘     │
│                                                                     │
│  Phase 3 (Month 5-6): Full CQRS                                    │
│  ┌───────────────────────────────────────────────────────────┐     │
│  │ All reads from projections                                 │     │
│  │ Write model optimized (indexes removed, normalized)        │     │
│  │ Old read paths removed                                     │     │
│  └───────────────────────────────────────────────────────────┘     │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
|--------|-------------|------------|-------------|
| Product listing P99 | 2,800ms | 45ms | 62x faster |
| Order write P99 | 180ms | 85ms | 2x faster (fewer indexes) |
| Search response | 1,200ms | 120ms | 10x faster |
| Read/write scaling | Coupled | Independent | Scale reads 10x without write impact |
| New query pattern lead time | 2-3 weeks (schema changes) | 2-3 days (new projection) | 7x faster feature development |

### Key Lessons

1. **Shadow mode is essential**: Running projections in shadow for 4 weeks caught 23 discrepancies before any traffic switched
2. **Projection lag surprised the team**: The 200ms average lag was acceptable, but P99 spikes during peak writes (up to 5 seconds) caused visible inconsistencies that required client-side optimistic updates
3. **Operational complexity increased**: Monitoring went from 1 database to 5 data stores (write DB, list view, search index, analytics warehouse, cache), requiring dedicated observability investment
...
GraphQL is a natural fit for the CQRS query side, where each resolver can target a different optimized read model:
...
│  GraphQL Query:                                                     │
│  {                                                                  │
│    order(id: "123") {        → Read Model 1 (Detail View)          │
│      items { ... }                                                  │
│      customer {              → Read Model 2 (Customer Profile)     │
│        name                                                         │
│        orderHistory { ... }  → Read Model 3 (History)              │
│      }                                                              │
│    }                                                                │
│  }                                                                  │
│                                                                     │
│  Each resolver independently queries its optimized read model.     │
│  GraphQL's type system naturally maps to projection schemas.       │
│                                                                     │
│  Mutations map directly to commands:                               │
│  mutation { createOrder(input: {...}) { id } }                     │
│  → Command: CreateOrderCommand                                     │
│  → Return: command ID (not full entity)                            │
│                                                                     │
│  Subscriptions map to projection change streams:                   │
│  subscription { orderStatusChanged(orderId: "123") { status } }   │
│  → Projection emits change events via WebSocket                    │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
Event-driven serverless architectures align well with CQRS patterns, where projection workers are triggered by event arrival rather than running continuously:

| Aspect | Traditional CQRS | Serverless CQRS |
|--------|------------------|-----------------|
| Projection workers | Long-running processes | Function invocations per event batch |
| Scaling | Manual/auto-scaling | Automatic per-event |
| Cost model | Always-on infrastructure | Pay-per-event processing |
| Cold start | No | 100-500ms (affects projection lag) |
| Throughput | Predictable | Variable (concurrency limits) |
| Best for | High-volume, steady workloads | Bursty, cost-sensitive workloads |

**Trade-off:** Serverless projection workers eliminate idle infrastructure cost but introduce cold-start latency that directly impacts projection lag. For most systems, the cost savings (often 60-80% for bursty workloads) outweigh the additional 100-500ms of projection lag that only affects the first event in a burst.
