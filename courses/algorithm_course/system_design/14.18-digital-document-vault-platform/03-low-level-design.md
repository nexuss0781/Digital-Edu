...
```
Shard Key: subscriber_id (hash-partitioned, 256 shards)

Shard Assignment:
    shard_number = hash(subscriber_id) % 256

Co-located Tables per Shard:
    - subscribers          (partitioned by subscriber_id)
    - document_references  (partitioned by subscriber_id)
    - consent_records      (partitioned by subscriber_id — grantor side)
    - audit_events         (partitioned by subscriber_id + time-bucketed)

Global Tables (replicated to all shards):
    - issuers              (read-heavy, rarely updated, ~2,000 rows)
    - requesters           (read-heavy, rarely updated, ~2,500 rows)
    - document_type_configs (reference data)
    - certificate_registry  (PKI certificates for all issuers)
```
...
```
Primary Indexes:
    subscribers:          (subscriber_id) — UUID primary key
    document_references:  (document_id) — UUID primary key
    consent_records:      (consent_id) — UUID primary key
    audit_events:         (event_id) — UUID primary key

Secondary Indexes:
    document_references:  (subscriber_id, status) — vault listing query
    document_references:  (document_uri) — URI resolution lookup
    document_references:  (issuer_id, document_type, created_at) — issuer analytics
    consent_records:      (subscriber_id, status) — active consent listing
    consent_records:      (requester_id, status) — requester consent tracking
    consent_records:      (access_token_hash) — token validation (unique)
    audit_events:         (subscriber_id, timestamp) — activity log (range scan)
    audit_events:         (requester_id, timestamp) — requester audit (range scan)

Composite Indexes for Common Queries:
    consent_records:      (subscriber_id, requester_id, status, expires_at)
                          — "active consents from requester X for subscriber Y"
    document_references:  (subscriber_id, document_type, status)
                          — "all active driving licenses for subscriber Y"
```
...
L1 (In-Memory, per-node):
    doc:{document_id}:content     → Document content bytes (TTL: 5 min)
    doc:{document_id}:meta        → Document metadata JSON (TTL: 5 min)
    verify:{document_id}          → Verification result (TTL: 6 hours, tied to CRL frequency)
    session:{session_token_hash}  → Session data (TTL: 30 min web, 24 hr mobile)

L2 (Distributed Cache):
    doc:{document_uri}:content    → URI-keyed document content (TTL: issuer-configured, 1-24 hr)
    issuer:{issuer_id}:health     → Issuer health status (TTL: 30 sec)
    issuer:{issuer_id}:cert       → Issuer certificate chain (TTL: 24 hr)
    crl:{ca_id}:serial_set        → CRL serial numbers as sorted set (TTL: tied to CRL update)
    consent:{consent_id}:state    → Materialized consent state (TTL: until expiry)

L3 (Persistent Cache — Object Storage):
    docs/{issuer_id}/{doc_type}/{doc_id}/content   → Full document (TTL: 7 days)
    docs/{issuer_id}/{doc_type}/{doc_id}/verify     → Verification metadata
```

---

## Algorithm 4: Offline Verification Bundle Generation

```
FUNCTION generate_offline_bundle(subscriber_id, document_id):
    // Retrieve document and its current verification state
    doc_ref = metadata_db.get(document_id)
    doc_content = resolve_document(doc_ref, subscriber_id)
    verification = verify_document_signature(doc_content, doc_ref.issuer_id)

    IF verification.overall_status != VERIFIED:
        RETURN Error("Cannot generate offline bundle for unverified document")

    // Build the offline verification bundle
    bundle = {
        document_content: doc_content.bytes,
        document_uri: doc_ref.document_uri,
        issuer_certificate: issuer_registry.get_cert(doc_ref.issuer_id),
        issuer_public_key: extract_public_key(issuer_certificate),
        certificate_chain: issuer_registry.get_chain(doc_ref.issuer_id),
        verification_result: verification,
        verified_at: now(),
        bundle_expiry: now() + 72_HOURS,
        crl_snapshot_at: crl_cache.last_updated(doc_ref.issuer_id),
    }

    // Platform signs the bundle to attest it was verified at this time
    bundle.platform_attestation = platform_hsm.sign(
        hash(bundle.document_uri + bundle.verified_at + bundle.verification_result)
    )

    // Generate QR code payload for offline presentation
    bundle.qr_payload = encode_qr({
        uri: doc_ref.document_uri,
        verified_at: bundle.verified_at,
        platform_signature: bundle.platform_attestation,
        issuer_cert_thumbprint: thumbprint(bundle.issuer_certificate),
    })

    RETURN OfflineBundle(bundle)
```

## Algorithm 5: Issuer-Aware Request Coalescing

```
FUNCTION coalesced_resolve(document_ref, subscriber_id):
    issuer_id = document_ref.issuer_id
    doc_key = document_ref.document_uri

    // Check if another thread is already fetching this exact document
    pending = coalescing_map.get(doc_key)
    IF pending:
        // Wait for the in-flight request instead of making a duplicate
        RETURN pending.await(timeout=4000ms)

    // Check if we should coalesce at the issuer level (thundering herd)
    issuer_queue = issuer_coalescing.get(issuer_id)
    IF issuer_queue AND issuer_queue.pending_count > COALESCE_THRESHOLD:
        // Too many concurrent requests to this issuer; queue and batch
        future = issuer_queue.enqueue(document_ref)
        RETURN future.await(timeout=6000ms)

    // No coalescing needed; proceed with normal resolution
    promise = new Promise()
    coalescing_map.put(doc_key, promise, ttl=5000ms)

    TRY:
        result = resolve_from_issuer(document_ref)
        promise.complete(result)
        coalescing_map.remove(doc_key)
        RETURN result
    CATCH Exception as e:
        promise.fail(e)
        coalescing_map.remove(doc_key)
...
DocumentSchemaVersion {
    ...
    schema_version      : String        // Semantic version: "2.1.0"
    field_definitions   : List<FieldDef> // Expected fields in this version
    migration_rules     : Map<String, MigrationRule>  // v1→v2 field mappings
    effective_from      : Timestamp
    deprecated_at       : Timestamp      // Null if current
}

MigrationRule {
    source_field        : String        // Field name in old version
    target_field        : String        // Field name in new version
    transform           : Enum          // RENAME | SPLIT | MERGE | COMPUTE | DROP
    transform_logic     : String        // Step-by-step plan in plain English for transformation
}
```

When a requester fetches a document using a v2 consent token but the cached document is in v1 schema, the Vault Service applies migration rules at query time to present the document in the expected format. This avoids requiring issuers to re-push all historical documents when they update their schema.
