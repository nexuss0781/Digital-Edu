...
## 8. GDPR Data Subject Rights in Multi-Tenant ERP

ERP systems store extensive personal data across modules (HR salary data, CRM contact details, vendor representative information). GDPR compliance requires structured support for data subject rights while preserving financial record integrity.

### Right to Erasure vs. Accounting Retention

```Step-by-step plan in plain English
FUNCTION process_erasure_request(data_subject_id, tenant_id):
    // Identify all records containing the data subject's PII
    records = discover_personal_data(data_subject_id, tenant_id)

    FOR EACH record IN records:
        IF record.has_legal_retention_obligation:
            // Cannot delete — pseudonymize instead
            record.personal_fields = pseudonymize(record.personal_fields)
            record.erasure_status = "pseudonymized"
            record.retention_reason = record.legal_basis  // "SOX_7_year", "tax_record"
        ELSE:
            delete_record(record)
            record.erasure_status = "deleted"

    // Audit the erasure itself (without storing the erased PII)
    emit_audit_event("data_subject_erasure", {
        subject_ref: hash(data_subject_id),  // hashed reference, not PII
        records_deleted: count_deleted,
        records_pseudonymized: count_pseudonymized,
        tenant_id: tenant_id
    })

    RETURN { deleted: count_deleted, pseudonymized: count_pseudonymized,
             retained_reasons: collect_retention_reasons(records) }
```

| Module | Personal Data | Erasable? | Retention Override |
|--------|-------------|-----------|-------------------|
| HR | Name, address, salary, bank details | Pseudonymize after employment ends | Tax/labor law: 7-10 years |
| CRM | Contact name, email, phone, interaction history | Delete if no active contracts | Contract retention: duration + 6 years |
| Finance | Vendor representative name on invoices | Pseudonymize | SOX/GAAP: 7 years |
| Audit Trail | Actor name in audit records | Replace with pseudonym | WORM: 7-15 years |

### Data Portability Export

```Step-by-step plan in plain English
FUNCTION export_personal_data(data_subject_id, tenant_id):
    records = discover_personal_data(data_subject_id, tenant_id)
    export = { subject_id: data_subject_id, exported_at: now(), modules: {} }

    FOR EACH module IN group_by_module(records):
        export.modules[module.name] = {
            records: sanitize_for_export(module.records),  // remove internal IDs
            schema: get_export_schema(module.name),
            format: "JSON"
        }

    encrypted_package = encrypt_export(export, data_subject_public_key)
    RETURN { download_url: store_temporary(encrypted_package, ttl=72_HOURS) }
```
