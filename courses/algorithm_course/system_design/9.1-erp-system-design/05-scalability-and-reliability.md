...
Within each shard: **journal entries** partitioned by `accounting_period`, **audit logs** by `created_date` (weekly), **EAV values** by `entity_type`. Partition Cutting off unnecessary steps eliminates 90%+ of data scanning.
...
## 7. Zero-Downtime Platform Upgrades

ERP platform upgrades must preserve all tenant customizations while applying schema changes, code updates, and configuration migrations across 10,000 tenants without coordinated downtime.

### Rolling Upgrade Strategy

```Step-by-step plan in plain English
FUNCTION execute_platform_upgrade(release):
    // Phase 1: Schema compatibility validation
    FOR EACH tenant IN get_all_tenants():
        compat = validate_schema_compatibility(tenant, release)
        IF compat.breaking_changes IS NOT EMPTY:
            quarantine_tenant(tenant.id, compat.breaking_changes)
            notify_tenant_admin(tenant.id, "Upgrade requires manual review")

    // Phase 2: Backward-compatible schema migration (additive only)
    apply_additive_migrations(release.migrations)
    // New columns with defaults, new tables, new indexes (concurrent)

    // Phase 3: Rolling application deployment
    FOR EACH instance_group IN partition_by_region(app_instances):
        drain_connections(instance_group, grace_period=30_SECONDS)
        deploy_new_version(instance_group, release.version)
        health_check(instance_group, timeout=60_SECONDS)
        IF NOT healthy:
            rollback_instance_group(instance_group)
            ABORT_UPGRADE(reason="Health check failed")
        restore_connections(instance_group)

    // Phase 4: Cleanup migrations (drop deprecated columns after all instances updated)
    schedule_cleanup_migration(release.cleanup_migrations, delay=48_HOURS)

    // Phase 5: Extension compatibility verification
    FOR EACH tenant IN get_tenants_with_extensions():
        run_extension_compat_tests(tenant.id, release.version)
```

### Schema Versioning Rules

| Change Type | Strategy | Downtime Required |
|------------|---------|-------------------|
| Add column (nullable or with default) | Online DDL | None |
| Add new table | Online DDL | None |
| Add index | Concurrent index creation | None |
| Rename column | Add new → dual-write → migrate reads → drop old | None (3-phase) |
| Change column type | Shadow column → backfill → swap → drop | None (4-phase) |
| Drop column | Stop writes → verify unused → drop | None (deferred) |
| Drop table | Verify no references → archive → drop | None (deferred) |

---

## 8. Tenant Migration: Shared to Dedicated

As tenants grow from SMB to enterprise tier, they may need migration from shared infrastructure to dedicated resources without business disruption.

```Step-by-step plan in plain English
FUNCTION migrate_tenant_to_dedicated(tenant_id):
    tenant = load_tenant(tenant_id)
    ASSERT tenant.db_strategy == "shared_schema"

    // Step 1: Provision dedicated infrastructure
    new_db = provision_dedicated_database(tenant_id, tenant.region)
    apply_base_schema(new_db)

    // Step 2: Initial data copy (while tenant continues on shared DB)
    snapshot_lsn = capture_logical_replication_slot(tenant_id)
    copy_tenant_data(source=shared_db, target=new_db, tenant_id=tenant_id)

    // Step 3: Catch-up replication (apply changes since snapshot)
    apply_change_stream(snapshot_lsn, current_lsn, tenant_id, target=new_db)

    // Step 4: Cut-over (brief pause)
    set_tenant_mode(tenant_id, "read_only")  // ~5 seconds of read-only
    apply_final_changes(tenant_id, target=new_db)

    // Step 5: Switch routing
    tenant.db_strategy = "dedicated"
    tenant.db_connection_ref = new_db.host
    update_tenant_routing(tenant_id)
    invalidate_connection_pools(tenant_id)

    // Step 6: Verify and restore
    verify_data_integrity(tenant_id, source=shared_db, target=new_db)
    set_tenant_mode(tenant_id, "active")

    // Step 7: Cleanup (deferred)
    schedule_cleanup(shared_db, tenant_id, delay=7_DAYS)

    emit_event("TenantMigrated", { tenant_id, from: "shared", to: "dedicated" })
```
...
| Risk | Mitigation |
|------|-----------|
| Data loss during cut-over | Change stream replays all writes; final verification before activation |
| Extended read-only window | Pre-copy 99%+ of data; cut-over only syncs delta (< 5 seconds) |
| Routing inconsistency | Atomic routing table update with cache invalidation across all instances |
| Rollback needed | Shared DB data preserved for 7 days; reverse migration possible |
| Custom field schema mismatch | Schema validation before migration; EAV data copied with metadata |
| Active batch jobs | Drain batch queue for tenant before cut-over; resume on new infrastructure |
