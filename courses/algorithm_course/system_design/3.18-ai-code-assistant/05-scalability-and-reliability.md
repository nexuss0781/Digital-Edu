...
## Scaling for Agent Mode (2025-2026)

### Agent vs. Completion Scaling Comparison

| Dimension | Inline Completions | Agentic Tasks |
|-----------|-------------------|---------------|
| Request duration | 100-300ms | 30s-5min |
| Token consumption | ~1K per request | ~50K per task |
| Concurrency model | Short-lived, stateless | Long-lived, stateful |
| Resource pattern | GPU burst (inference) | GPU sustained + CPU (validation) |
| Failure handling | Retry or skip | Checkpoint and resume |
| Scaling trigger | Request rate (QPS) | Active task count |
...
Session State (stored in Redis):
  ...
  - step_index: current step in plan
  - context_snapshot: files read so far
  - edits_pending: generated but unapplied changes
  - retry_count: per step error correction attempts
  - token_budget_remaining: cost guardrail

Scaling Strategy:
  - Sessions are pinned to worker nodes (sticky routing)
  - Worker failure → session state recovered from Redis
  - Max 1000 concurrent agent sessions per GPU node
  - Backpressure: queue agent tasks when capacity exceeded
  - Priority: inline completions always preempt agent tasks
```
...
|----------|-----------|--------|
| **Request priority** | Inline completions get GPU priority over agent tasks | Protects core UX |
| **Token budgets** | Per-task and per-org token limits | Prevents runaway cost |
| **Model tiering** | Use smaller models for planning, larger for code gen | 40% cost reduction |
| **Incremental context** | Only re-send changed context between agent steps | 60% token savings |
| **Batch validation** | Group test executions across agent tasks | Reduces compute cost |
