...
| Trend | Key Point |
|-------|-----------|
| **Agentic workflows** | Multi-step autonomous tasks (read → plan → code → test → fix) |
| **Long context models** | 128K+ token windows reduce need for RAG retrieval |
| **Multi-file editing** | Atomic diff sets across multiple files |
| **Model routing** | Small model for simple completions, large model for complex edits |
| **On-device inference** | Privacy-preserving local code completion with quantized models |
| **Background indexing** | Continuous repository-wide embedding updates |
