...
flowchart TB
    subgraph Primary["Primary Region — Mumbai"]
        PA1[Protocol Adapter<br/>Fleet]
        DB1[(Order DB<br/>Primary)]
        IDX1[(Search Index<br/>Primary)]
        EVT1[Event Stream<br/>Primary]
    end

    subgraph DR["DR Region — Hyderabad"]
        PA2[Protocol Adapter<br/>Standby]
        DB2[(Order DB<br/>Replica)]
        IDX2[(Search Index<br/>Replica)]
        EVT2[Event Stream<br/>Mirror]
    end

    subgraph Edge["Edge — Delhi / Bangalore / Chennai"]
        CDN1[CDN PoP<br/>Catalog Images]
        CACHE1[Registry Cache<br/>NP Keys]
    end

    PA1 -->|sync replication| DB1
    DB1 -->|async replication<br/>RPO < 5s| DB2
    EVT1 -->|async mirror| EVT2
    IDX1 -->|rebuild from<br/>catalog store| IDX2
    PA1 --> CDN1
    PA1 --> CACHE1
    ...
    class PA1,DB1,IDX1,EVT1 primary
    class PA2,DB2,IDX2,EVT2 dr
    class CDN1,CACHE1 edge
```

### Data Sovereignty Constraints

| Data Type | Storage Location | Cross-Region Replication |
|---|---|---|
| **Payment data** | India only (RBI data localization mandate) | Within India (Mumbai ↔ Hyderabad) only |
| **Aadhaar/KYC data** | India only; encrypted at rest | No replication (tokenized hash stored; full data purged) |
| **Order/transaction data** | India only | Async replica for DR |
| **Catalog data** | India only (seller data is India-origin) | Full replica in DR region |
| **Analytics/aggregates** | India; anonymized exports permissible | Anonymized aggregates may be processed in any region |

### Active-Passive Failover Strategy

ONDC operates entirely within India, so multi-region is for disaster recovery, not latency optimization.

```
FailoverProcedure:

  Trigger: Primary region unavailable for > 2 minutes
    (confirmed by external health check from 3 independent probes)

  Step 1: DNS failover (30 seconds)
    - Update subscriber_url in ONDC registry to point to DR region
    - All NPs will discover new endpoint on next registry refresh (TTL: 1 hour)
    - Immediate effect for NPs that check registry on each request

  Step 2: Promote DR databases (2-3 minutes)
    - Order DB: promote async replica to primary (RPO < 5 seconds)
    - Settlement ledger: promote replica (RPO = 0, sync within India)
    - Event stream: switch consumers to mirror cluster

  Step 3: Rebuild search index (45 minutes)
    - Search operates in degraded mode (stale index) during rebuild
    - Live protocol fan-out used for search during rebuild period
    - Full index rebuild from catalog store in DR region

  Recovery time objective (RTO): < 5 minutes for core order processing
  Recovery point objective (RPO): < 5 seconds for order data; 0 for settlement
```
...
| Priority | Capability | Degraded Behavior | Trigger |
|---|---|---|---|
| **P0** | Protocol message processing | Never degraded; all resources prioritized | N/A |
| **P1** | Order confirmation and payment | Skip non-critical validation; async fraud check post-confirm | System load > 80% |
| **P2** | Search with ranking | Serve from stale index; skip AI ranking, use rule-based sorting | AI inference overloaded or search latency > 3s |
| **P3** | Catalog enrichment | Queue for batch processing; accept seller's raw catalog as-is | GPU cluster overloaded |
| **P4** | Trust score updates | Serve cached scores (up to 24 hours stale) | Trust computation backlog > 2 hours |
| **P5** | WhatsApp commerce | Redirect to web buyer app; send "temporarily unavailable" message | WhatsApp bot error rate > 10% |
| **P6** | Analytics and reporting | Stop real-time analytics; batch process after recovery | Event stream consumer lag > 1 hour |
| **P7** | Seller dashboard | Read-only mode; cached data; no real-time order updates | Dashboard backend overloaded |

---

## Capacity Planning for Seasonal Events

| Event | Expected Volume | Pre-Scaling Actions | Timing |
|---|---|---|---|
| **Diwali Sale** | 5-10× daily average | Double compute fleet, expand event stream to 128 partitions, pre-warm search index replicas | 48 hours before |
| **Republic Day Sale** | 3-5× daily average | 50% additional compute, expand critical consumer groups | 24 hours before |
| **Regional festivals** (Pongal, Onam, etc.) | 2-3× in specific regions | Scale region-specific NP connections, add search replicas for affected domain partitions | 24 hours before |
| **Monthly grocery restock** (1st/15th) | 1.5-2× for grocery domain | Scale grocery search partition replicas, pre-warm delivery slot availability | 12 hours before |
| **New buyer app launch** | Unpredictable burst | Rate limit new NPs initially (100 req/sec); increase after 24-hour observation | On announcement |
| **ONDC protocol upgrade** | No volume spike but increased error rate | Deploy multi-version adapter; expand monitoring; on-call staffing | 1 week before migration window |
...
|---|---|---|
| **Peak daily volume** | 5M orders/day sustained for 4 hours | p95 latency < 2s for search, < 4s for order confirmation |
| **Search fan-out stress** | 2000 QPS search with 50-NP fan-out | < 5% timeout rate; progressive rendering within 800ms |
| **Settlement batch** | 5M orders settlement in single batch run | Complete within 2 hours; zero discrepancy for > 99.9% |
| **NP failure cascade** | Simulate 10 NPs failing simultaneously | No impact on orders not involving failed NPs; graceful timeout for affected orders |
| **Protocol version mixed** | 50% NPs on v1.2.5, 50% on v1.3.0 | Zero translation errors; no schema validation failures at protocol boundary |

### Chaos Engineering Experiments

| Experiment | Injection | Expected Behavior |
|---|---|---|
| **Kill protocol adapter node** | Terminate 1 of 20 nodes | Load balancer removes within 10s; no failed messages |
| **Slow NP response** | Add 3s latency to 5 NPs | Timeout triggers; results from remaining NPs displayed; trust scores adjusted |
| **Event stream partition leader loss** | Kill partition leader | Follower promoted within seconds; consumer group rebalances; order state lag < 30s |
| **ONDC registry unreachable** | Block registry endpoint for 30 min | Serve from cached keys; new NP key rotations queued; alert fires within 2 min |
| **Payment gateway timeout** | Add 30s delay to payment API | Circuit breaker opens; traffic shifted to secondary gateway within 60s |
| **Search index corruption** | Corrupt 1 domain partition | Replica takes over reads; rebuild initiated; alert within 5 min |

---

## WhatsApp Commerce Scaling

### Challenge

WhatsApp conversations are stateful (session data per buyer) and long-lived (a buyer may take 10 minutes to complete an order via chat). This creates a different scaling pattern from stateless API calls:

- Each active conversation holds ~5 KB of session state
- Peak concurrent sessions: ~10,000 (based on 35 msg/sec with 5-minute avg session)
- Session state must be accessible with < 5ms latency for conversational UX
...
WhatsAppScaling:
  ...
  Session Store:
    ...
    - Sharded by buyer_phone_hash (consistent hashing)
    - 3 replicas per shard for fault tolerance
    - TTL: 30 minutes inactivity → session archived to persistent store
    - Total active memory: 10,000 sessions × 5 KB = ~50 MB (trivial)

  Bot Processing Fleet:
    - 15 stateless bot worker nodes (baseline)
    - Auto-scale to 45 nodes based on message queue depth
    - Each worker handles:
      1. Receive message from WhatsApp webhook
      2. Load session state from store (< 5ms)
      3. NLU inference (language detection + intent classification): 50-100ms
      4. Beckn protocol call if needed: 200-2000ms (depends on action)
      5. Format response for WhatsApp (carousel, text, payment link)
      6. Update session state in store
      7. Send response via WhatsApp Business API

  WhatsApp API Rate Limits:
    - Per WABA number: 1,000 messages/second
    - Per phone number (24-hour window): 250 business-initiated messages
    - Mitigation: Multiple WABA numbers with load balancing
    - Session window: 24 hours from last buyer message
      (after 24 hours, only template messages allowed — use for re-engagement)

  Degraded Mode:
    - If NLU inference overloaded: fall back to keyword matching (less accurate but faster)
    - If Beckn protocol calls timing out: show cached results with "prices may vary" disclaimer
    - If WhatsApp API rate limited: queue messages with 1-second delay; inform buyer of wait
```

---

## Network Growth: NP Onboarding Scaling

As ONDC grows from 175 NPs to 500+ NPs, the system faces O(N²) scaling challenges:

| Dimension | At 175 NPs | At 500 NPs | Scaling Impact |
|---|---|---|---|
| **Registry key cache size** | 175 entries × 2 KB | 500 entries × 2 KB | Trivial memory; refresh traffic 3× |
| **Search fan-out** | Max 50 NPs per query | Max 100+ NPs per query | Intelligent NP selection becomes critical; timeout budget tighter |
| **Trust score matrix** | 175 NP scores | 500 NP scores × per-seller scores | Computation time scales linearly; storage scales with sellers |
| **Protocol version matrix** | 175 × 3 versions | 500 × 4 versions | Version translation middleware gets more complex |
| **Circuit breaker state** | 175 per-NP breakers | 500 per-NP breakers | Memory and monitoring overhead increases linearly |

### Mitigation for N² Growth

1. **NP tiering** — Classify NPs into Tier 1 (top 20 by volume, always included in fan-out), Tier 2 (mid 50, included based on category match), Tier 3 (remaining, included only for long-tail queries). This bounds effective fan-out to ~30 NPs regardless of total NP count.

2. **NP clustering** — Group NPs by domain and geography. A grocery search in Bangalore only fans out to grocery NPs with Bangalore sellers (typically 5-10, not all 500).

3. **Hierarchical trust computation** — Compute NP-level trust first (coarse), then per-seller trust for NPs above a threshold (fine). This avoids computing trust for sellers on NPs that are already flagged as low-trust.

---

## Cost Attribution per Transaction

| Cost Category | Per-Order Cost (at 50M orders/month) | % of Total |
|---|---|---|
| **Protocol processing** (signature verification, schema validation, routing) | ₹0.20 | 13% |
| **Search** (fan-out, aggregation, AI ranking) | ₹0.30 | 20% |
| **AI inference** (catalog enrichment amortized, fraud detection per-order) | ₹0.25 | 17% |
| **Order state management** (database writes, event stream) | ₹0.15 | 10% |
| **Settlement computation** (ledger entries, reconciliation) | ₹0.10 | 7% |
| **Storage** (protocol logs, order records, catalog data) | ₹0.20 | 13% |
| **WhatsApp/notifications** (API calls, message delivery) | ₹0.10 | 7% |
| **Infrastructure overhead** (monitoring, DR, management) | ₹0.20 | 13% |
| **Total estimated** | **₹1.50** | **100%** |

At a seller NP commission of 3-8% on a ₹500 average order (₹15-40 revenue per order), the infrastructure cost of ₹1.50/order provides healthy unit economics at scale. The primary scaling lever is amortizing GPU inference costs across more orders.
