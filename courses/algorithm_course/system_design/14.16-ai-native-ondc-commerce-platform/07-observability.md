...
|---|---|---|---|
| **Federated search p95 latency** | ≤ 2 seconds | 21.6 minutes of violations/day | > 50% consumed → freeze non-critical search experiments |
| **Protocol message delivery** | 99.99% success | ~4,320 failed messages/month (at 1.7M/day) | > 75% consumed → halt protocol adapter deployments |
| **Order confirmation end-to-end** | ≤ 4 seconds (p95) | 72 minutes of violations/day | > 50% consumed → activate fast-path mode (skip async enrichment) |
| **Payment processing success** | ≥ 96% | 4% failure budget (~68K failed/month at 1.7M/day) | > 80% consumed → page on-call; investigate payment gateway health |
| **Settlement reconciliation accuracy** | ≥ 99.9% | ~50K discrepant orders/year | > 50% consumed → halt settlement algorithm changes; audit pipeline |
| **WhatsApp intent accuracy** | ≥ 80% | 20% misclassification budget | > 90% consumed → increase human-handoff rate; retrain NLU model |
...
ErrorBudgetAlerts:
  ...
  fast_burn:    5% of monthly budget consumed in 1 hour → P1 alert
  medium_burn:  10% of monthly budget consumed in 6 hours → P2 alert
  slow_burn:    50% of monthly budget consumed in 15 days → P3 alert (trend warning)
```

---

## Cross-Network Observability: What You Can't See

A critical limitation of observability in federated systems is the "black box" problem. The following table maps what the platform can and cannot observe:

| Dimension | Observable (Within Platform) | Inferred (From Protocol Signals) | Invisible (External NP Internal) |
|---|---|---|---|
| **Search processing** | Our ranking latency, index query time | External NP response latency (from timestamp deltas) | Why an NP was slow (their database issues, etc.) |
| **Order fulfillment** | Our order state transitions | Seller's packing speed (from on_status timestamps) | Seller's warehouse operations |
| **Logistics** | Tracking event timestamps from protocol messages | Delivery speed, route efficiency | Driver behavior, vehicle conditions |
| **Payment** | UPI transaction initiation and callback | Payment NP processing time (from callback delay) | Payment NP's internal fraud checks |
| **Trust** | Computed scores based on protocol data | NP behavior trends over time | Whether an NP is deliberately gaming metrics |

### Compensating Strategies for Blind Spots

1. **Protocol message timing analysis** — Even without seeing inside another NP, timestamp deltas between request and callback reveal processing performance. A sudden 3× increase in a seller NP's callback latency indicates internal issues.
2. **Cross-NP correlation** — If logistics NP reports "delivered" but buyer complaints spike for that NP pair, the discrepancy reveals problems invisible from either side alone.
3. **Network-wide anomaly baselines** — Compare each NP against the network average. An NP whose metrics deviate significantly from peers is likely experiencing issues or gaming, even if individual signals look normal in isolation.

---

## Synthetic Monitoring

| Synthetic Test | Frequency | What It Validates |
|---|---|---|
| **End-to-end order flow** | Every 5 minutes | search → on_search → select → on_select → init → on_init → confirm → on_confirm (sandbox mode, no real payment) |
| **Search result quality** | Every 15 minutes | Known product query returns expected seller in top 5 results |
| **Cross-lingual search** | Every 30 minutes | Same product searched in Hindi, Tamil, English returns comparable result sets |
| **Signature verification roundtrip** | Every 5 minutes | Sign a test message, verify with own public key from registry lookup |
| **Settlement computation** | Every hour | Run settlement computation for a synthetic order and verify amounts match expected splits |
| **WhatsApp bot conversation** | Every 30 minutes | Complete 5-turn conversation (greet → search → select → ask about delivery → order) in sandbox |
| **India Stack health** | Every 10 minutes | e-KYC sandbox OTP request; DigiLocker sandbox document pull; UPI sandbox payment intent |

---

## Runbook Templates

### Runbook 1: Federated Search Latency Degradation

```
Trigger: search.latency.p95 > 3 seconds for > 5 minutes

Step 1: Identify slow NPs
  Query: SELECT np_id, p95_latency FROM np_response_metrics WHERE p95_latency > 2000ms
  If specific NPs are slow → proceed to Step 2a
  If all NPs are slow → proceed to Step 2b

Step 2a: NP-specific slowness
  Action: Increase timeout tolerance for slow NPs to 3s
  Action: Reduce fan-out to slow NPs (fallback to pre-indexed results)
  Action: Notify NP operations team via ONDC support channel
  Monitor: search.latency.p95 should improve within 2 minutes

Step 2b: Platform-wide slowness
  Check: Protocol adapter CPU utilization (> 80% → scale up)
  Check: Search index query latency (> 500ms → check replica health)
  Check: AI ranking inference latency (> 200ms → check GPU cluster)
  Action: If compute-bound → trigger auto-scaling immediately (don't wait for normal threshold)
  Action: If AI-bound → activate rule-based ranking fallback (skip AI ranking)
```

### Runbook 2: Settlement Reconciliation Discrepancy Spike

```
Trigger: settlement.discrepancy_rate > 0.5% in daily batch

Step 1: Categorize discrepancies
  Query: GROUP BY discrepancy_type (rounding, timing, amount_mismatch, missing_entry)
  If mostly rounding → likely benign (common after protocol version changes)
  If amount_mismatches → escalate immediately (potential fraud or computation bug)

Step 2: For amount mismatches
  Action: Freeze affected settlements (do NOT disburse until investigated)
  Action: Pull signed protocol messages for affected orders
  Action: Recompute settlement amounts from protocol messages (source of truth)
  Action: Compare recomputed amounts with NP-reported amounts
  If platform computation matches protocol messages → NP reporting error → notify NP
  If platform computation differs from protocol messages → platform bug → escalate to engineering

Step 3: Resolution
  Action: Correct settlement amounts and release frozen settlements
  Action: Generate discrepancy report for compliance team
  Action: Update reconciliation rules if new Edge Case (Unusual or extreme situation) discovered
```

### Runbook 3: ONDC Gateway Connectivity Loss

```
Trigger: Gateway health check fails for 3 consecutive checks (60-second intervals)

Step 1: Confirm outage is not platform-side
  Action: Check outbound network connectivity to other external services
  Action: Check ONDC status page / community channel for known issues
  If platform network issue → escalate to infrastructure team

Step 2: Activate resilience measures
  Action: Queue all outbound protocol messages (local buffer, max 30 minutes)
  Action: Continue processing existing in-flight orders (state is local)
  Action: Switch search to pure pre-indexed mode (no live NP fan-out)
  Action: Post merchant status update: "New orders temporarily paused due to network issue"

Step 3: Recovery
  Action: When gateway reconnects, drain queued messages in order (oldest first)
  Action: Monitor for message ordering issues (confirm before on_status, etc.)
  Action: Resume normal operations; update merchant status page
  Action: Post-incident review: validate no orders were lost or duplicated
```
...
| Observability Component | Monthly Cost Estimate | % of Infrastructure |
|---|---|---|
| **Log ingestion and storage** | High (24 TB/year protocol logs + 10 TB/year app logs) | ~6% |
| **Distributed tracing** | Medium (trace every Beckn message, sample internal spans) | ~3% |
| **Metrics collection** | Low (aggregated metrics, low storage per metric) | ~1% |
| **Synthetic monitoring** | Low (7 synthetic tests at various intervals) | ~0.5% |
| **Alerting infrastructure** | Low (rule evaluation on aggregated metrics) | ~0.5% |
| **Dashboards and visualization** | Low | ~0.5% |
| **Total** | | **~11.5% of infrastructure** |

The 7-year regulatory retention requirement for protocol message logs is the dominant observability cost. Tiered storage (hot → warm → cold) reduces this by ~60% compared to keeping all logs in hot storage.
