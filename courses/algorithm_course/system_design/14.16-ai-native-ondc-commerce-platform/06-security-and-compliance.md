...
ThreatModel:
  ...
  Threat 7: Collusion Between NPs
    Attack: Buyer NP and seller NP collude to generate fake orders, inflate GMV, and extract
            COD advance payments without actual commerce
    Defense: Cross-NP order pattern analysis (detect statistically improbable buyer NP ↔ seller NP
             pairs); logistics-verified delivery confirmation required for COD advance; anomaly
             detection on order-to-delivery-confirmation ratios per NP pair

  Threat 8: AI Model Poisoning via Catalog Manipulation
    Attack: Adversarial NP floods catalogs with subtly manipulated entries to skew search ranking
            models (e.g., competitors injecting misleading attributes to degrade rival rankings)
    Defense: Catalog quality scoring isolates low-quality entries before they influence training data;
             model training uses verified-purchase feedback signals (harder to manipulate than raw
             catalog data); periodic model audits compare ranking outputs against human-curated baselines
```
...
|---|---|---|---|
| **Signature forgery** | Submit messages with invalid signatures using known and unknown keys | Weekly automated | 100% rejection rate; zero false accepts |
| **Replay attack** | Replay captured valid messages with original message_id | Weekly automated | 100% rejection within dedup window |
| **Schema injection** | Submit messages with XSS/SQL payloads in catalog description fields | Weekly automated | All payloads sanitized; no execution |
| **Privilege escalation** | Buyer NP attempts to send seller-only actions (on_search, on_confirm) | Monthly automated | All unauthorized actions rejected with appropriate NACK |
| **Rate limit bypass** | Attempt to exceed rate limits via distributed requests from multiple IPs | Monthly automated | Rate limits enforced per subscriber_id, not per IP |
| **Key rotation race condition** | Rotate NP signing key and immediately send messages with new key | Quarterly manual | Verify-refresh-retry pattern catches rotation within 1 failed attempt |
| **Cross-tenant data access** | Attempt to read seller A's catalog data via seller B's API credentials | Monthly automated | Zero cross-tenant data leakage |

---

## Compliance Audit Preparedness

| Audit Type | Triggering Authority | Data Required | Retrieval SLA |
|---|---|---|---|
| **GST audit** | GSTN / Tax authorities | Invoice records, TCS deductions, settlement amounts per seller | 24 hours for last 2 years |
| **DPDP Act review** | Data Protection Board of India | PII processing records, consent logs, deletion logs, breach history | 48 hours |
| **ONDC compliance audit** | ONDC secretariat | Protocol compliance scores, message logs, settlement accuracy, NP interaction records | 4 hours (real-time dashboard access) |
| **RBI payment data audit** | Reserve Bank of India | UPI transaction records, payment data storage location proof, settlement reconciliation | 24 hours |
| **Consumer court dispute** | District/State consumer forum | Complete order evidence chain — all signed protocol messages for the disputed order | 4 hours (automated evidence chain generation) |

---

## Zero Trust Architecture for Federated Commerce

In a federated network, the default trust posture is "trust nothing, verify everything." This manifests at every layer:

| Layer | Zero Trust Principle | Implementation |
|---|---|---|
| **Network** | Every NP-to-NP message is potentially malicious | TLS 1.3 for all communication; certificate pinning for registry; no plaintext fallback |
| **Identity** | Every NP identity claim must be cryptographically verified | Ed25519 signature on every message; registry-based key lookup; no implicit trust from IP address or domain |
| **Data** | Every data field from an external NP may be incorrect or adversarial | Schema validation rejects malformed data; content sanitization prevents injection; trust scoring quantifies reliability over time |
| **Authorization** | Every action must be validated against the NP's registered role and domain | Role-based action enforcement (buyer NPs can only send buyer actions); domain-scoping prevents cross-domain messages |
| **Financial** | Every settlement claim must be anchored to signed protocol evidence | No settlement without a corresponding signed on_confirm and delivery proof; double-entry ledger cross-verification |
| **Temporal** | Every message has a bounded validity window | TTL enforcement on all protocol messages; replay detection via message_id deduplication; timestamp skew tolerance of ±30 seconds |
