...
**Trade-off:** The adapter is a potential Slowest part of the process and single point of failure. Mitigated by deploying it as a stateless, horizontally scalable service behind a load balancer with health-check-based routing.
...
sequenceDiagram
    ...
    participant BNP as Buyer NP
    participant PG as Payment NP
    participant ONDC as ONDC Gateway
    participant SNP as Seller NP<br/>(AI Platform)
    participant LSP as Logistics NP
    participant S as Seller

    Note over B,S: Phase 1 — Payment Collection
    B->>BNP: Pay via UPI
    BNP->>PG: Process payment
    PG-->>BNP: Payment confirmed (UTR)
    BNP->>ONDC: confirm (with payment proof)
    ONDC->>SNP: confirm

    Note over B,S: Phase 2 — Fulfillment & Settlement Trigger
    SNP->>ONDC: on_status (DELIVERED)
    ONDC-->>BNP: on_status (DELIVERED)
    Note over SNP: Return window starts (7 days)

    Note over B,S: Phase 3 — Multi-Party Settlement (T+1)
    PG->>PG: Compute settlement splits
    PG->>BNP: Buyer NP commission (2-5%)
    PG->>ONDC: ONDC network fee (0.1%)
    PG->>LSP: Logistics charges
    PG->>SNP: Seller NP commission (3-8%)
    SNP->>S: Seller payout (after TCS deduction)

    Note over B,S: Phase 4 — Reconciliation
    SNP->>SNP: Verify: sum(payouts) = order_amount
    SNP->>SNP: Cross-check TCS against GSTN report
```
...
%%{init: {'theme': 'neutral'}}%%
sequenceDiagram
    participant U as Buyer<br/>(WhatsApp)
    participant Bot as WhatsApp Bot<br/>Engine
    participant NLU as NLU + Session<br/>Manager
    participant BPA as Beckn Protocol<br/>Adapter
    participant GW as ONDC Gateway
    participant SNP as Seller NPs

    U->>Bot: "मुझे 5kg आटा चाहिए"<br/>(I need 5kg atta)
    Bot->>NLU: Language detect + Intent classify
    NLU-->>Bot: {lang: "hi", intent: PRODUCT_SEARCH,<br/>entities: {product: "atta", qty: "5kg"}}
    Bot->>BPA: search (category: wheat_flour, qty: 5kg)
    BPA->>GW: Beckn search (signed)
    GW->>SNP: broadcast to qualifying NPs
    SNP-->>GW: on_search (catalog results)
    GW-->>BPA: aggregated on_search
    BPA-->>Bot: ranked results (top 5)
    Bot->>U: Carousel: [Aashirvaad 5kg ₹289,<br/>Pilsbury 5kg ₹265, Shaktibhog 5kg ₹245]

    U->>Bot: "सबसे सस्ता वाला"<br/>(The cheapest one)
    Bot->>NLU: Resolve "cheapest" against result_set
    NLU-->>Bot: {intent: SELECT, item: Shaktibhog}
    Bot->>BPA: select (Shaktibhog 5kg)
    BPA->>GW: Beckn select
    GW-->>BPA: on_select (quote: ₹245 + ₹30 delivery)
    Bot->>U: "Shaktibhog 5kg आटा ₹275 (₹245 + ₹30 डिलीवरी).<br/>ऑर्डर करें?"

    U->>Bot: "हाँ"
    Bot->>NLU: Resolve address from history
    Bot->>BPA: init + confirm flow
    BPA->>GW: Beckn init → confirm
    Bot->>U: "💳 UPI से पेमेंट करें: [Pay ₹275]"
    U->>U: Taps UPI link → Pays
    BPA-->>Bot: on_confirm (order confirmed)
    Bot->>U: "✅ ऑर्डर #4521 कन्फर्म!<br/>डिलीवरी: आज शाम 6 बजे तक"
```
...
### POS and Inventory Integration Pipeline

```
InventoryIntegrationPipeline:

  Pull-based sync (for sellers with existing POS/ERP):
    1. Platform provides API connector for popular Indian POS systems
       (Petpooja, Posist for restaurants; Gofrugal, Marg for retail)
    2. Connector polls seller's POS every 5 minutes for inventory changes
    3. Delta sync: only changed items pushed to catalog store
    4. Price/availability changes trigger real-time index update

  Push-based sync (for sellers with WhatsApp-only workflow):
    1. Seller sends WhatsApp message: "Rice 10kg - out of stock"
    2. NLU parses: {action: STOCK_UPDATE, item: "Rice 10kg", available: false}
    3. Catalog store updated; search index reflects within 30 seconds
    4. Active orders for this item enter HOLD state pending seller confirmation

  Image-based sync (for zero-tech sellers):
    1. Seller takes photo of current shelf/stock
    2. AI compares with last known inventory photo
    3. Detects: new items, missing items (out of stock), price label changes
    4. Generates delta catalog update with confidence scores
    5. Seller confirms/rejects changes via simple yes/no UI
```
...
NotificationPipeline:
  ...
  Channel priority (per event type):
    ORDER_CONFIRMED:    1. WhatsApp  2. SMS  3. Push notification  4. Email
    ORDER_STATUS:       1. WhatsApp  2. Push notification
    PAYMENT_RECEIVED:   1. SMS  2. WhatsApp  3. Push notification
    SETTLEMENT_COMPLETE: 1. Email  2. Dashboard notification
    CATALOG_ISSUE:      1. Dashboard  2. Email

  Delivery guarantees:
    Critical events (order, payment):  At-least-once delivery; retry 3× with backoff
    Informational events (analytics):  Best-effort delivery; no retry
    Compliance events (tax notices):   Exactly-once; confirmed read receipt required

  WhatsApp template management:
    - Pre-approved templates for each event type (24-hour window constraint)
    - Dynamic data injection: order_id, amount, delivery_time, item_names
    - Language-specific templates (Hindi, English, Tamil, Telugu, etc.)
    - Fallback: If WhatsApp delivery fails 2×, switch to SMS
```
...
|---|---|---|
| **Protocol adapter** | Stateless microservice (event-driven) | Must handle 1,160 msg/sec with zero shared state; horizontal scaling without coordination |
| **Digital signatures** | Hardware Security Module (HSM) for private key operations | Ed25519 signing requires private key access; HSM provides FIPS 140-2 Level 3 protection |
| **Event streaming** | Distributed log (partitioned by transaction_id) | Protocol messages are naturally events; partition by transaction_id ensures order-level locality |
| **Order state store** | Distributed SQL with strong consistency | Order state transitions require ACID semantics; settlement amounts demand exact consistency |
| **Catalog store** | Document database with flexible schema | Seller catalogs vary by domain (grocery schema ≠ fashion schema); document model handles heterogeneity |
| **Search index** | Hybrid inverted + vector index | Keyword search for exact queries; vector search for semantic/cross-lingual matching; reciprocal rank fusion to merge |
| **Trust score store** | In-memory key-value store with sorted sets | Trust scores read on every search request (870 QPS peak); sub-millisecond reads required |
| **Settlement ledger** | Separate distributed SQL cluster | Financial data isolated from operational workload; strict consistency; independent scaling |
| **WhatsApp session store** | In-memory store with persistence and TTL | Sessions are ephemeral (30-min inactivity TTL); speed critical for conversational UX |
| **AI inference** | GPU cluster for catalog enrichment; CPU cluster for search ranking | Catalog enrichment uses vision + language models (GPU); search ranking uses lightweight models (CPU-efficient) |
| **CDN** | Edge caching for catalog images and static assets | 130 MB/s peak image egress; CDN reduces origin load by ~70% |
