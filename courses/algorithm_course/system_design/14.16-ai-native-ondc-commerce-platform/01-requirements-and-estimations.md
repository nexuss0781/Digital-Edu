...
| Flow | Calculation | Peak Bandwidth |
|---|---|---|
| **Inbound protocol messages** | 1,160 msg/sec × 2 KB avg × peak factor | ~7 MB/s ingress |
| **Search fan-out (outbound)** | 870 QPS × 20 NPs avg × 1 KB | ~17 MB/s egress |
| **Search response aggregation** | 870 QPS × 20 NPs × 5 KB avg response | ~87 MB/s ingress |
| **Catalog sync (bulk)** | 500K events/day × 3 KB / 86,400 | ~17 KB/s sustained |
| **Catalog images (CDN)** | 870 QPS × 3 images/result × 50 KB | ~130 MB/s CDN egress |
| **Settlement data exchange** | 1.7M orders/day × 2 KB / 86,400 | ~40 KB/s sustained |
| **WhatsApp messages** | 35 msg/sec × 5 KB (with media refs) | ~175 KB/s |

### Cost Estimation Model

| Cost Component | Unit | Estimated Cost (monthly) |
|---|---|---|
| **Compute (protocol + services)** | 80 nodes avg utilization | Infrastructure unit cost × 80 |
| **GPU inference (catalog + search)** | 14 GPU nodes | Dominant cost driver; ~40% of total |
| **Storage (hot + warm + cold)** | ~5 TB hot, ~20 TB warm, ~100 TB cold | Tiered pricing; cold = ~5% of hot cost per GB |
| **Event streaming cluster** | 10 nodes, 3× replication | ~8% of total infrastructure |
| **Network egress (CDN + NP traffic)** | ~130 MB/s peak sustained | Significant at scale; CDN caching reduces origin egress by ~70% |
| **India Stack APIs** | e-KYC ₹5/verification, DigiLocker ₹2/pull | Variable; scales with new seller onboarding volume |
| **ONDC network fees** | 0.1% of GMV | At 50M orders × ₹500 avg = ₹25B GMV → ₹25M/month |

**Unit economics per order:**

| Scale | Infrastructure Cost/Order | ONDC Fee/Order | Target Revenue/Order |
|---|---|---|---|
| 50M orders/month | ₹1.50–2.00 | ₹0.50 | ₹15–40 (seller NP commission 3-8%) |
| 200M orders/month | ₹0.80–1.20 | ₹0.50 | ₹15–40 |
| 500M orders/month | ₹0.50–0.80 | ₹0.50 | ₹15–40 |
...
|---|---|---|
| **Beckn protocol async model** | Every interaction is request → ACK → callback (never synchronous request-response) | All UX flows must handle the async gap; timeout management is critical |
| **No centralized database** | Catalog, order, and user data distributed across independent NPs | Cannot do simple JOIN queries across buyer/seller data; search requires federated aggregation |
| **Digital signature overhead** | Every message requires Ed25519 signing (outbound) and verification (inbound) | ~2-5ms per message for HSM-based signing; caching reduces verification overhead |
| **Protocol version heterogeneity** | NPs may run different Beckn versions simultaneously (2-3 month migration windows) | Protocol adapter must handle multi-version translation; N×M compatibility matrix |
| **India Stack rate limits** | Aadhaar e-KYC: max 10 req/sec; DigiLocker: max 5 req/sec per registered app | Seller onboarding throughput capped by India Stack API limits; queue and batch required |
| **COD dominance** | ~60% of ONDC orders are Cash-on-Delivery | Settlement lag of 3-5 days for majority of orders; cash flow challenge for sellers |
| **22 language diversity** | Buyer queries and seller catalogs in any of 22 constitutionally recognized Indian languages | Cross-lingual search is not optional; multilingual embeddings are a hard requirement |
| **WhatsApp API constraints** | 24-hour session window; template-only messages outside session; rate limits per WABA number | Conversational commerce flows must complete within 24 hours or use template-based re-engagement |

---

## Competitive Landscape Context

| Platform | Model | Strengths | ONDC Differentiator |
|---|---|---|---|
| **Amazon India** | Centralized marketplace | Unified search, Prime logistics, 500M+ products | Seller locked to one platform; high commission (15-40%) |
| **Flipkart** | Centralized marketplace | Deep India penetration; Ekart logistics | Same lock-in; sellers can't reach buyers on other apps |
| **Meesho** | Social commerce marketplace | Low-commission reseller model | Still centralized; resellers locked to Meesho app |
| **JioMart** | O2O (online-to-offline) | Reliance Retail integration; kirana network | Proprietary network; not interoperable |
| **ONDC NPs** | Federated protocol network | No lock-in; any buyer app ↔ any seller app; government-backed protocol | Decentralization tax on UX; AI must close the quality gap vs. centralized platforms |

The competitive moat for an ONDC NP is not the platform itself (the protocol is open) but the **AI intelligence layer**: catalog normalization quality, search relevance, trust scoring accuracy, and WhatsApp commerce UX. These are the dimensions where NPs compete while the protocol ensures interoperability.
...
|---|---|---|
| ONDC monthly order growth ≥ 15% MoM through 2026 | Over-provisioned infrastructure; higher unit costs | Elastic scaling; reserved + spot instance mix |
| COD share decreases from 60% to 40% by 2027 | Settlement lag continues dominating cash flow | COD advance product; aggressive UPI incentives |
| Beckn protocol stabilizes at v1.3.x (no breaking v2.0 before 2027) | Major protocol adapter rewrite required | Abstract protocol handling behind adapter interface; budget 2 months for v2.0 migration |
| India Stack API availability ≥ 99.5% | Seller onboarding blocked during outages | Graceful degradation: manual document upload fallback; async verification queue |
| WhatsApp Business API pricing remains viable for commerce | WhatsApp commerce channel becomes cost-prohibitive | Multi-channel strategy: Telegram, RCS as backup conversational channels |
| ONDC gateway capacity scales with network growth | Gateway becomes Slowest part of the process; fan-out search degrades | Direct NP-to-NP communication for high-volume NP pairs; lobby ONDC for capacity investment |
...
## Domain-Specific Scale Variations

ONDC supports multiple commerce domains, each with distinct scale and latency characteristics:

| Domain | Code | Order Mix | Avg Order Value | Latency Tolerance | Special Requirements |
|---|---|---|---|---|---|
| **Grocery** | ONDC:RET10 | ~35% | ₹400-600 | 2s search | Weight-based pricing; FSSAI compliance; shelf life tracking |
| **Food & Beverage** | ONDC:RET11 | ~30% | ₹250-400 | 1s search (hyperlocal UX) | Real-time menu availability; 30-min delivery expectation; restaurant hours |
| **Fashion** | ONDC:RET12 | ~15% | ₹500-1500 | 3s search | Size/color variants; high return rate (20-30%); image-heavy catalog |
| **Electronics** | ONDC:RET14 | ~10% | ₹2000-10000 | 3s search | Warranty tracking; serial number registration; EMI support |
| **Health & Pharma** | ONDC:RET16 | ~5% | ₹300-800 | 2s search | Drug license validation; prescription upload; cold chain logistics |
| **Other** | Various | ~5% | Varies | 3s search | Home decor, sports, books, etc. |

### Implication for Capacity Planning

Grocery and food combined represent ~65% of order volume but have the tightest latency requirements. Infrastructure sizing must be optimized for these high-volume, low-latency domains, with other domains sharing a general-purpose pool.

---

## Case Study: Scale Implications of Festival Sales

During Diwali 2025, the ONDC network experienced:
- **5× peak order volume** (80M orders in the 5-day festival window vs. 16M in a normal month)
- **Search QPS spike to 4,000+** (vs. normal 870 peak)
- **Catalog update storm**: 1M+ sellers updating prices and inventory simultaneously for sale events
- **Payment gateway saturation**: UPI transaction volume strained payment NP capacity

The key lesson: festival scaling is not uniform across domains. Fashion and electronics spike 10× during Diwali sales, while grocery spikes only 1.5× (people eat the same amount during festivals). The scaling strategy must be domain-aware, not blanket.
