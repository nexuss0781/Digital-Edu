...
**Slowest part of the process:** During a major outage, a cascade of 50,000 alerts arrives in 5 minutes. The deduplication engine must process 170 alerts/second while maintaining consistent fingerprint state.
...
**Slowest part of the process:** Telephony providers typically limit concurrent outbound calls to 50-200 per account. During a P1 incident affecting multiple teams, the system may need to make 100+ calls within minutes.
...
**Slowest part of the process:** If 1,000 incidents are created in the same minute (during a storm), their escalation timers will all fire in the same minute (e.g., 5 minutes later). This creates a "thundering herd" of 1,000 simultaneous escalation events.
...
**Slowest part of the process:** Every incident requires resolving who is currently on-call, which involves evaluating schedule layers, overrides, and timezone conversions. During storms, this computation is repeated thousands of times per minute.

**Impact:** Schedule resolution becomes a CPU Slowest part of the process if computed from scratch on every request.
...
**Slowest part of the process:** In active-active multi-region, the same incident may be updated concurrently in both regions (e.g., engineer in Region A acknowledges while the escalation timer fires in Region B).
...
## 5. Deep Dive: On-Call Fairness and Burden Distribution

### 5.1 The Problem

On-call burden is not evenly distributed. Engineers at the beginning of a rotation week that includes a major incident receive disproportionate pages. Services with higher alert volumes page their on-call engineers more frequently. Without active management, this leads to burnout and attrition.

### 5.2 Burden Metrics

```
FUNCTION ComputeOnCallBurden(user_id, period):
    notifications = NotificationStore.GetForUser(user_id, period)

    RETURN {
        total_pages: COUNT(notifications),
        after_hours_pages: COUNT(notifications WHERE sent_at NOT IN business_hours),
        sleep_interruptions: COUNT(notifications WHERE sent_at BETWEEN 22:00 AND 06:00 local_time),
        p1_incidents: COUNT(DISTINCT incident_id WHERE severity = P1),
        avg_time_to_ack: AVG(acknowledged_at - sent_at),
        total_on_call_hours: SUM(on_call_shift_duration FOR shifts IN period),
        pages_per_on_call_hour: total_pages / total_on_call_hours
    }
```

### 5.3 Fairness Dashboard Alert

When burden exceeds thresholds, the system alerts the team manager:

| Metric | Yellow Threshold | Red Threshold | Action |
|---|---|---|---|
| Sleep interruptions/week | > 2 | > 4 | Suggest rotation swap; alert manager |
| After-hours pages/week | > 5 | > 10 | Review alert thresholds for assigned services |
| Total pages/week | > 15 | > 30 | Investigate noisy services; consider rotation size increase |
| Pages per on-call hour | > 0.5 | > 1.0 | Service is too noisy; needs dedup tuning or threshold adjustment |

---

## 6. Deep Dive: Runbook Safety and Blast Radius Control

### 6.1 Runbook Risk Classification

| Risk Level | Examples | Safeguards |
|---|---|---|
| **Read-only** | Collect logs, check health, query metrics | No approval needed; auto-execute |
| **Low-risk write** | Restart a single pod, clear cache | Responder can execute; logged |
| **Medium-risk write** | Scale service up/down, toggle feature flag | Team Admin approval required |
| **High-risk write** | Database failover, DNS change, rollback deploy | Two-person approval + audit trail |
| **Destructive** | Delete data, drop tables, decommission service | Disabled in automation; manual only |

### 6.2 Blast Radius Limiter

```
FUNCTION ExecuteRunbookStep(step, incident, context):
    // Pre-execution safety check
    IF step.blast_radius > incident.affected_service_count:
        REJECT("Step affects more services than incident scope")

    IF step.risk_level >= MEDIUM AND NOT HasApproval(step, incident):
        PAUSE("Awaiting approval from {required_approver}")
        NotifyApprover(step, incident)
        RETURN AWAITING_APPROVAL

    // Execute with timeout and rollback
    result = ExecuteWithTimeout(step, timeout=step.timeout_seconds)

    IF result.status == TIMEOUT:
        RollbackStep(step, context)
        RecordFailure(step, "Timeout after {step.timeout_seconds}s")
    ELSE IF result.status == ERROR:
        RollbackStep(step, context)
        RecordFailure(step, result.error)
    ELSE:
        RecordSuccess(step, result.output)
