...
### 3.6 Alert Storm Detection and Adaptive Response

```
FUNCTION DetectAndRespondToStorm():
    // Sample alert rate over sliding 60-second window
    currentRate = AlertRateCounter.GetRate(window=60s)
    baselineRate = AlertRateBaseline.Get(dayOfWeek=TODAY, hour=CURRENT_HOUR)

    stormMultiplier = currentRate / baselineRate

    IF stormMultiplier > 10:
        // Major storm — activate all storm mitigations
        ExtendDedupWindow(multiplier=2)  // 24h → 48h
        EnableBatchEscalation()  // Group escalations by team
        EnableNotificationPriority()  // P1 first, P3/P4 deferred
        ScaleComponent("dedup_engine", targetPods=20)
        ScaleComponent("notification_dispatcher", targetPods=30)
        PublishStormAlert("Major alert storm detected: {stormMultiplier}x normal")

    ELSE IF stormMultiplier > 5:
        // Moderate storm — partial mitigations
        ExtendDedupWindow(multiplier=1.5)  // 24h → 36h
        ScaleComponent("dedup_engine", targetPods=15)
        PublishStormAlert("Elevated alert rate: {stormMultiplier}x normal")

    ELSE IF stormMultiplier < 2 AND stormMode == ACTIVE:
        // Storm subsiding — revert mitigations gradually
        RevertDedupWindow()
        DisableBatchEscalation()
        DisableNotificationPriority()
        // Let auto-scaler handle pod count reduction
        PublishStormAlert("Alert storm subsiding; reverting mitigations")
```

### 3.7 Flapping Alert Suppression

```
FUNCTION HandleFlappingAlert(fingerprint):
    // Track state transitions for this fingerprint
    transitions = TransitionHistory.Get(fingerprint, window=1h)

    // Flapping detection: >4 trigger→resolve cycles in 1 hour
    triggerResolveCount = CountCycles(transitions, "triggered", "resolved")

    IF triggerResolveCount > 4:
        // Mark as flapping — suppress notifications but keep logging
        FingerprintStore.SetFlag(fingerprint, "flapping", TRUE)
        incident = GetActiveIncident(fingerprint)
        incident.addNote("Alert flagged as flapping ({triggerResolveCount} cycles in 1h). Notifications suppressed.")

        // Notify service owner once about the flapping pattern
        IF NOT AlreadyNotifiedFlapping(fingerprint, window=24h):
            NotifyServiceOwner(incident.service_id,
                "Flapping alert detected: {incident.title}. Consider adjusting thresholds.")
...
| Data Store | Shard Key | Shard Count | Rationale |
|---|---|---|---|
| **Incident DB** | `incident_id` hash | 16 shards | Even distribution; no hotspots |
| **Alert DB** | `fingerprint` hash | 32 shards | Same fingerprint → same shard for dedup locality |
| **Notification records** | `user_id` hash | 8 shards | Per-user notification audit efficient |
| **Timer Store** | `fire_at` time bucket | 4 partitions (15-min buckets) | Efficient "fire all timers in this second" |
| **Audit Log** | `timestamp` range | Monthly partitions | Range queries for compliance; old partitions archivable |
...
Incident Database Indexes:
  - (service_id, status, created_at) — active incidents per service
  - (assigned_to, status) — my open incidents dashboard
  - (severity, status, created_at) — P1/P2 active incidents list
  - (fingerprint, created_at) — dedup fingerprint lookups
  - GIN index on metadata JSON — custom field queries

Notification Database Indexes:
  - (incident_id, user_id) — notification history per incident
  - (user_id, sent_at) — per-user audit trail
  - (channel, status, sent_at) — channel health monitoring

Schedule Cache Key Design:
  - schedule:{schedule_id}:current → current on-call user (TTL: until next rotation boundary)
  - schedule:{schedule_id}:overrides → active overrides sorted set (TTL: until last override ends)
  - oncall:{team_id}:now → resolved on-call for team (TTL: 5 min, event-invalidated)
```

### 4.3 Postmortem Schema
...
Postmortem:
  ...
  timeline        : TimelineEntry[] (auto-generated from incident events)
  action_items    : ActionItem[]
  created_at      : TIMESTAMP
  published_at    : TIMESTAMP (nullable)
  author_id       : UUID (FK → User)
  reviewers       : UUID[] (FK → User)

ActionItem:
  id              : UUID
  postmortem_id   : UUID (FK → Postmortem)
  description     : TEXT
  ...
  completed_at    : TIMESTAMP (nullable)
  jira_ticket_id  : STRING (nullable, linked external ticket)
```

### 4.4 Notification Delivery Record Schema
...
NotificationDelivery:
  ...
  user_id         : UUID (FK → User)
  channel         : ENUM [phone, sms, push, email, slack, teams]
  provider        : STRING (e.g., "telephony_provider_a", "push_service")
  status          : ENUM [queued, sending, delivered, failed, no_answer, voicemail]
  attempt_number  : INT (1-based; increments on retry)
  failover_from   : UUID (nullable; FK → NotificationDelivery if this is a failover)
  dispatched_at   : TIMESTAMP
  delivered_at    : TIMESTAMP (nullable)
  acknowledged_at : TIMESTAMP (nullable; set when user ACKs via this channel)
  failure_reason  : STRING (nullable; e.g., "provider_timeout", "token_expired")
  latency_ms      : INT (delivered_at - dispatched_at)
  provider_ref    : STRING (external reference ID from provider)
```

Indexes:
- `(incident_id, user_id)` — find all notification attempts for a user on an incident
- `(channel, status, dispatched_at)` — channel health monitoring (failure rates by time)
- `(user_id, dispatched_at DESC)` — user's notification history for audit
