...
|----------|-------------|
| **What AI decides alone** | Real-time predictions within deployed model boundaries (ETA, pricing, fraud scores), feature computation (Palette online/offline), dynamic batching and serving optimization |
| **What AI recommends** | Model retraining candidates based on drift detection, feature importance rankings, A/B test winner suggestions, capacity scaling recommendations |
| **What requires human approval** | Model deployment to production (Gallery promotion), A/B experiment launch and conclusion, feature store schema changes, GenAI Gateway provider routing policy updates, Tier 1 model rollback decisions |
| **Deterministic source of truth** | Gallery model registry (versioned artifacts, metadata, lineage) and Palette feature store (canonical feature definitions) — inference is autonomous; model lifecycle governance requires human sign-off |
| **Rollback path** | Gallery version rollback with virtual model sharding re-routing; Palette feature version revert; GenAI Gateway provider failover; < 5 min rollback for Tier 1 models via automated canary reversal |
...
|------------|---------|--------|
| **Vector Store Platform** | Billion-scale embedding management with HNSW; 1.5B+ items, P99 120ms at 2K QPS | Powers RAG, semantic search, personalized recommendations |
| **Model Excellence Scores (MES)** | SRE-inspired quality framework: data quality, drift, accuracy, interpretability | 60% improvement in overall prediction performance |
| **Deployment Safety** | Shadow testing on 75%+ critical models; automatic rollback on threshold breaches | Target 100% shadow coverage by H2 2025 |
| **Palette Meta Store Re-architecture** | Server-side schema validation, incremental delta computation | Metadata updates from hours to minutes; 95% onboarding time reduction |
| **LLM Training Optimization** | DeepSpeed ZeRO-3 + Flash Attention on A100/H100; 2-3x throughput | Full-parameter, LoRA, QLoRA fine-tuning support |
| **GenAI Gateway Scale** | 16M queries/month, 30+ teams, PII redaction, cost guardrails | 20M chat summaries/week, agents respond 6s faster |
...
- [3.6 Netflix Metaflow](../3.6-netflix-metaflow-ml-workflow-platform/00-index.md) - Alternative ML workflow approach
- [3.9 Airbnb BigHead](../3.9-airbnb-bighead-ml-platform/00-index.md) - Comparable ML platform at peer company
- [1.3 Distributed Key-Value Store](../1.3-distributed-key-value-store/00-index.md) - Cassandra patterns used in online feature store

---

## Related Patterns

| Pattern | Relevance | Link |
|---------|-----------|------|
| **AI-Native Hybrid Search Engine** | Vector Store platform powers embedding-based retrieval; shared HNSW indexing patterns | [3.29 Hybrid Search](../3.29-ai-native-hybrid-search-engine/00-index.md) |
| **Multi-Agent Orchestration** | GenAI Gateway evolving toward multi-agent patterns for complex ML workflows | [3.24 Multi-Agent](../3.24-multi-agent-orchestration-platform/00-index.md) |
| **Distributed Job Scheduler** | Job Controller federates training across K8s clusters; shared scheduling patterns | [2.6 Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) |
| **Container Orchestration** | Kubernetes operators as control plane; GPU scheduling, pod lifecycle | [2.2 Container Orchestration](../2.2-container-orchestration-system/00-index.md) |
| **Feature Flag Management** | Model deployment uses canary and shadow modes analogous to feature flags | [2.7 Feature Flags](../2.7-feature-flag-management/00-index.md) |
| **Service Mesh Design** | mTLS between Michelangelo services; sidecar patterns for model serving | [2.11 Service Mesh](../2.11-service-mesh-design/00-index.md) |
| **Airbnb BigHead ML Platform** | Comparable architecture solving same problems; different trade-offs on feature store and serving | [3.9 Airbnb BigHead](../3.9-airbnb-bighead-ml-platform/00-index.md) |
| **Netflix Metaflow** | Alternative workflow-centric ML platform; different philosophy on model-as-code | [3.6 Netflix Metaflow](../3.6-netflix-metaflow-ml-workflow-platform/00-index.md) |
