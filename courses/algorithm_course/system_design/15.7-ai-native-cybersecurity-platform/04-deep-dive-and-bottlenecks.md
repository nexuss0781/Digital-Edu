...
### Slowest part of the process 1: Telemetry Ingestion Throughput
...
### Slowest part of the process 2: ML Inference Latency at Scale
...
### Slowest part of the process 3: Threat Hunting Query Performance
...
### Slowest part of the process 4: Graph Traversal for Alert Correlation
...
- Asynchronous graph enrichment: initial correlation uses a lightweight Practical rule of thumb; full graph analysis runs asynchronously and may merge incidents post-hoc

---

## Deep Dive 4: GenAI Security Copilot Pipeline

### Natural-Language to Structured Query Translation

The most impactful copilot capability is translating analyst intent ("show me all PowerShell executions from finance users in the last 7 days that connected to external IPs") into structured queries.

```
FUNCTION translate_nl_to_query(natural_language_input, analyst_context):
    // Step 1: Classify intent
    intent = classify_intent(natural_language_input)
    // intent ∈ {threat_hunt, incident_summary, explain_alert, generate_playbook}

    IF intent == "threat_hunt":
        // Step 2: Extract entities and constraints
        entities = extract_entities(natural_language_input)
        // entities: { process: "PowerShell", department: "finance",
        //             time_range: "7 days", network: "external" }

        // Step 3: Map to schema fields
        schema_mapping = map_to_event_schema(entities)
        // { process_name: "powershell.exe",
        //   user.department: "finance",
        //   time_range: [now - 7d, now],
        //   network.dst_is_internal: false }

        // Step 4: Generate structured query
        query = generate_kql(schema_mapping)
        // "events | where process_name =~ 'powershell.exe'
        //   and user_department == 'finance'
        //   and timestamp > ago(7d)
        //   and dst_ip NOT IN internal_ranges"

        // Step 5: Validate query safety
        validation = validate_query(query, analyst_context)
        IF validation.has_time_bound == false:
            query = add_default_time_bound(query, max_days = 30)
        IF validation.estimated_scan_size > MAX_SCAN_THRESHOLD:
            RETURN suggest_refinements(query, validation)

        // Step 6: Scope to analyst permissions
        query = apply_rbac_filter(query, analyst_context.role, analyst_context.tenant_id)

        RETURN QueryResult(
            generated_query = query,
            confidence = schema_mapping.confidence,
            explanation = "This query searches for PowerShell executions by finance " +
                         "department users that made connections to external IPs",
            requires_confirmation = true  // Never auto-execute
        )
```

### Incident Narrative Generation

For resolved incidents, the copilot generates human-readable attack narratives.

```
FUNCTION generate_incident_narrative(incident):
    // Assemble context window
    context = {
        alerts: incident.alerts.sorted_by(timestamp),
        affected_assets: incident.affected_assets.with_details(),
        affected_users: incident.affected_users.with_details(),
        kill_chain: incident.kill_chain_stages,
        mitre_mapping: incident.mitre_mapping,
        response_actions: incident.playbook_runs.with_results(),
        timeline: incident.timeline
    }

    // Structure the narrative
    narrative_sections = [
        generate_executive_summary(context),     // 2-3 sentences
        generate_attack_timeline(context),        // Chronological sequence
        generate_impact_assessment(context),      // What was compromised
        generate_response_summary(context),       // What was done
        generate_recommendations(context),        // What to do next
        generate_ioc_list(context)                // Extracted indicators
    ]

    // Safety check: ensure no PII leakage in narrative
    narrative = sanitize_for_sharing(join(narrative_sections))
...
| Hallucination Type | Detection Method | Prevention |
|-------------------|------------------|------------|
| **Fabricated IOCs** | Validate all IOCs mentioned in response against threat intel database | RAG retrieval ensures IOCs come from indexed data, not model generation |
| **Non-existent entities** | Cross-reference all entity names (users, assets, IPs) against asset inventory | Context window contains only real entity data; instruct model to reference, not invent |
| **Incorrect MITRE mapping** | Validate technique IDs against MITRE ATT&CK knowledge base | RAG with MITRE KB; output validation layer checks technique ID format and existence |
| **Wrong timestamps/sequences** | Compare narrative timeline against actual event timestamps | Timeline is generated from structured data, not model reasoning |
| **Overstated confidence** | Compare model's stated confidence against actual detection scores | Instruct model to quote exact confidence scores from alert data |

---

## Slowest part of the process 5: OCSF Schema Normalization at Ingestion Speed

**Problem:** Normalizing heterogeneous telemetry from dozens of source types into the OCSF common schema adds latency to the ingestion pipeline. With 2.2M events/sec, even 5ms per event requires 11,000 CPU-seconds per second of normalization capacity.

**Mitigation:**
- **Pre-compiled normalizers:** Each source type has a pre-compiled, optimized normalizer (not interpreted at runtime). Normalizer compilation happens at source registration time.
- **Agent-side pre-normalization:** Endpoint agents normalize their own telemetry to OCSF before forwarding, distributing normalization compute to the edge and reducing central pipeline load by ~60%.
- **Schema caching:** Field mappings and entity resolution caches (user_id → department, asset_id → criticality) are cached in-memory with TTL-based refresh.
- **Deferred enrichment:** Non-critical enrichments (vulnerability correlation, asset risk scoring) are applied asynchronously after the event enters the detection pipeline, not blocking the hot path.

### Normalization Performance Budget

| Stage | Latency Budget | Implementation |
|-------|---------------|----------------|
| Schema mapping (field rename + type conversion) | <0.5ms | Pre-compiled mapping function per source type |
| Entity resolution (user_id → canonical form) | <1ms | In-memory hash lookup with 99.9% cache hit rate |
| MITRE technique mapping | <0.5ms | Decision-tree classifier, not regex |
| Threat intel IOC matching | <2ms | Bloom filter pre-check + hash lookup for positives |
| Deterministic event ID generation | <0.1ms | Hash of (tenant_id, source, timestamp, content_hash) |
| **Total normalization** | **<4ms** | Below 5ms budget with margin |
