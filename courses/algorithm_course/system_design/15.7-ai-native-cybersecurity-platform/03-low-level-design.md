...
FUNCTION map_to_mitre(event):
    ...
    ELSE IF event.type == "auth_attempt":
        // T1110: Brute Force (Checking every single possibility)
...
POST /api/v1/copilot/hunt
  ...
  Body:
    ...
    context:                   // Optional investigation context
      ...
      time_range: {start, end}
    options:
      auto_execute: boolean    // false = preview query; true = execute
      max_results: int         // default: 100
  Response: 200 OK
    generated_query: string    // Structured query (KQL-like)
    confidence: float          // Query translation confidence
    explanation: string        // Human-readable explanation of what the query does
    results: list<Event>       // Only if auto_execute = true
    suggestions: list<string>  // Alternative query refinements

// Generate incident narrative
POST /api/v1/copilot/summarize
  Body:
    incident_id: UUID
    audience: enum (executive, technical, compliance)
    include_iocs: boolean
    include_recommendations: boolean
  Response: 200 OK
    narrative: string          // Markdown-formatted incident narrative
    confidence: float
    referenced_alerts: list<UUID>
...
POST /api/v1/copilot/explain
  Body:
    alert_id: UUID
    detail_level: enum (brief, detailed, expert)
  Response: 200 OK
    explanation: string
    mitre_context: string      // MITRE ATT&CK technique explanation
    similar_incidents: list<IncidentRef>
    recommended_actions: list<string>

// Generate playbook from description
POST /api/v1/copilot/generate-playbook
  Body:
    description: string        // Natural language playbook description
    trigger_conditions: string // When should this playbook run?
    response_actions: string   // What should it do?
  Response: 200 OK
    playbook_yaml: string      // Generated playbook in YAML
    validation_result: ValidationResult
    warnings: list<string>     // Safety concerns
    requires_review: boolean   // Always true for generated playbooks
```

---

## ITDR (Identity) Data Model Extension
...
│ tenant_id        : UUID                                 │
│ timestamp        : int64 (epoch_ms)                     │
│ identity_provider: enum (active_directory, entra_id,    │
│                          okta, ping, custom)             │
│ event_type       : enum (auth_success, auth_failure,    │
│                          mfa_challenge, mfa_success,     │
│                          mfa_failure, token_grant,       │
│                          token_refresh, token_revoke,    │
│                          consent_grant, role_change,     │
│                          group_change, password_change,  │
│                          service_principal_auth,          │
│                          privilege_escalation)            │
│ user_id          : string                               │
│ user_type        : enum (human, service_account,        │
│                          managed_identity, guest)        │
│ auth_method      : enum (password, mfa_push, mfa_totp,  │
│                          certificate, token, kerberos,   │
│                          saml, oauth)                    │
│ source_ip        : string                               │
...
│ target_resource  : string (nullable)                    │
│ risk_signals     : map<string, variant>                 │
│   # impossible_travel_flag, mfa_fatigue_count,          │
│   # new_device_flag, unusual_time_flag,                 │
│   # token_replay_detected, consent_to_unverified_app    │
└─────────────────────────────────────────────────────────┘
...
│ user_id          : string                               │
│ tenant_id        : UUID                                 │
│ auth_hour_histogram : list<float> (24 bins)             │
│ auth_geo_set     : set<GeoLocation>                     │
│ device_set       : set<string>                          │
│ application_set  : set<string>                          │
│ mfa_method_preference : map<string, float>              │
│ avg_sessions_per_day : float                            │
│ typical_session_duration_min : BaselineStats             │
│ role_set         : set<string>                          │
│ group_set        : set<string>                          │
│ service_principal_access_patterns : map<string, pattern>│
│ last_updated     : int64 (epoch_ms)                     │
│ risk_score       : float (0.0 - 100.0)                  │
└─────────────────────────────────────────────────────────┘
```
