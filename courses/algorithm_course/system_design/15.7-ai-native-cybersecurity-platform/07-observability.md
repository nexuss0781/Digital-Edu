...
### GenAI Copilot Dashboard

- **Usage:** Queries per analyst per day, query intent distribution (hunt / summarize / explain / generate), response latency
- **Quality:** Analyst satisfaction ratings on copilot responses, query accuracy (did generated KQL return relevant results?), hallucination rate (responses referencing non-existent entities)
- **Safety:** Prompt injection attempt detection rate, cross-tenant data leakage incidents (should be zero), blocked queries (RBAC enforcement)

---

## SOAR Execution Observability

Automated response actions are the highest-stakes operations the platform performs. Observability of the SOAR engine must be especially rigorous.

### Playbook Execution Metrics

| Metric | Description | Alert Threshold |
|--------|-------------|-----------------|
| `soar.playbook_trigger_latency_ms` | Time from alert to playbook start | p99 > 60s |
| `soar.step_execution_success_rate` | Percentage of steps completing successfully | <90% per playbook |
| `soar.approval_gate_timeout_rate` | Percentage of approval gates that timeout | >20% (indicates insufficient analyst coverage) |
| `soar.action_rollback_rate` | Percentage of response actions rolled back | >5% (indicates high false positive triggering) |
| `soar.integration_circuit_breaker_trips` | Count of circuit breaker activations per external system | >2/hour per integration |
| `soar.blast_radius_escalation_rate` | Percentage of playbooks hitting blast-radius caps | >10% (playbooks may need tuning) |
| `soar.mean_containment_time_sec` | Time from incident creation to first containment action | >300s for critical incidents |

### Response Action Audit Trail

Every automated response action generates an immutable audit record:

```
ResponseActionAuditEntry:
    action_id        : UUID
    timestamp        : epoch_ms
    incident_id      : UUID
    playbook_id      : UUID
    ...
    step_id          : string
    action_type      : enum (isolate, block_ip, disable_user, quarantine_file, ...)
    target           : string (endpoint_id, IP, user_id, file_hash)
    trigger          : enum (automated, analyst_approved, manual)
    confidence       : float (detection confidence at trigger time)
    result           : enum (success, failure, timeout, rolled_back)
    rollback_action  : ActionRef (nullable)
    executed_by      : string (system or analyst_id)
    blast_radius     : int (count of affected entities)
    duration_ms      : int
```

---

## Detection Coverage Observability

### MITRE ATT&CK Coverage Heatmap

The platform maintains a real-time coverage map showing which MITRE ATT&CK techniques have active detections and which have gaps.

| Coverage Level | Definition | Target |
|---------------|-----------|--------|
| **Level 3: Active + Tested** | Detection exists AND validated by red team in last 90 days | >50% of techniques |
| **Level 2: Active** | Detection rule or ML model covers the technique | >70% of techniques |
| **Level 1: Logged** | Relevant telemetry is collected but no detection exists | >85% of techniques |
| **Level 0: No coverage** | Neither telemetry nor detection for this technique | <15% of techniques |

### Detection Health Anomaly Detection

```
FUNCTION monitor_detection_health():
    // Detect "silent failures" — detection engines running but not producing alerts
    FOR EACH detection_rule IN active_rules:
        expected_trigger_rate = detection_rule.historical_trigger_rate_7d
        actual_trigger_rate = count_alerts(detection_rule.id, last_24h) / 24

        IF expected_trigger_rate > 0 AND actual_trigger_rate == 0:
            // Rule hasn't fired in 24h when it historically fires daily
            create_ops_alert(
                type = "detection_silent_failure",
                severity = "medium",
                detail = "Rule '{detection_rule.name}' has not triggered in 24h; " +
                         "7-day avg: {expected_trigger_rate}/hour",
                possible_causes = [
                    "Telemetry pipeline issue (events not reaching rule engine)",
                    "Rule regression after schema change",
                    "Legitimate: attack pattern no longer present"
                ]
            )

    // Detect abnormal alert volume (too many or too few)
    FOR EACH detection_engine IN [rule_engine, ml_engine, ueba_engine]:
        alert_rate = count_alerts(engine = detection_engine, last_1h)
        baseline = detection_engine.alert_rate_baseline_7d

        IF alert_rate < baseline * 0.2:
            create_ops_alert(type = "detection_underproduction", severity = "high")
        ELSE IF alert_rate > baseline * 5.0:
            create_ops_alert(type = "detection_overproduction", severity = "medium")
```
