...
| "Just use an LLM to detect all threats" | LLMs are not real-time inference engines; inference latency (100ms+) is too slow for per-event processing at 2.2M events/sec; hallucination is unacceptable for detection |
| "Put everything in a graph database" | Graph DBs are excellent for entity relationships but terrible for time-series telemetry queries; need a hybrid approach |

---

## Trade-Off Deep Dives

### Trade-Off 1: Detection Latency vs. Detection Quality

| Approach | Latency | Detection Quality | When to Use |
|----------|---------|-------------------|-------------|
| Agent-local rules only | <10ms | Low (known threats only) | Always — first line of defense |
| Streaming ML (fast classifier) | <100ms | Medium (trained patterns) | For events that pass bloom filter |
| Streaming ML (deep model) | <1s | High (novel patterns) | For events flagged by fast classifier |
| Batch behavioral (UEBA) | <15 min | Very high for insider threats | Continuous background analysis |
| Threat hunting (analyst) | Hours-days | Highest (human reasoning) | When automated detection misses |

**Expected discussion:** Candidates should articulate that these tiers are complementary, not competing. The right answer is "all of the above" with clear reasoning about which detection method covers which threat category.

### Trade-Off 2: Data Retention vs. Cost vs. Compliance

| Retention Strategy | Queryable? | Cost/TB/month | Compliance Value |
|-------------------|-----------|---------------|------------------|
| Full-fidelity hot store (24h) | Sub-second | $500 | Real-time detection only |
| Full-fidelity warm (30d) | Seconds | $50 | Threat hunting, investigation |
| Full-fidelity cold (1yr) | Minutes | $10 | Forensics, incident response |
| Sampled archive (7yr) | Hours | $4 | Compliance audits |
| Alert + context only (7yr) | Seconds | $2 | Post-incident legal review |

**Key insight:** The optimal strategy is *heterogeneous retention* — keep everything for 30 days (hunting), alert-associated events for 1 year (forensics), and sampled + alert-only for 7 years (compliance). This reduces cost by 100x compared to keeping everything at full fidelity for 7 years.

### Trade-Off 3: Multi-Tenant Shared vs. Dedicated Infrastructure

| Model | Pros | Cons | Best For |
|-------|------|------|----------|
| **Fully shared** (logical isolation) | Lowest cost; fastest onboarding | Noisy neighbor risk; shared failure domain; limited compliance | SMB tenants |
| **Shared compute, isolated storage** | Good cost/isolation balance; per-tenant encryption | Detection engine shared; burst capacity contention | Mid-market |
| **Dedicated per-tenant** | Full isolation; per-tenant SLAs; compliance-friendly | Highest cost; operational overhead at scale | Enterprise, regulated industries |

---

## Whiteboard Walkthrough

### Recommended Drawing Sequence

1. **Start with the detection pipeline** (left to right): `Agent → Bus → Normalize → Detect → Correlate → Respond`
2. **Add the detection engine detail** (box-in-box): Rules | ML | UEBA feeding into Correlation
3. **Add storage** below the pipeline: Hot → Warm → Cold (with arrows showing flow)
4. **Add the edge-cloud split**: Show agent with local detection vs. cloud detection with a clear boundary line
5. **Add external integrations**: Threat intel feeds in, SOAR actions out to firewalls/identity providers
6. **Add the feedback loop**: Analyst resolutions → Model retraining (close the loop)

### Key Diagrams to Draw

| Diagram | Purpose | Time Budget |
|---------|---------|-------------|
| Detection pipeline (end-to-end) | Shows data flow and architectural understanding | 3-4 min |
| ML model cascade | Shows scaling awareness and compute optimization | 2-3 min |
| Edge-cloud detection tiers | Shows resilience thinking | 2 min |
| SOAR playbook with approval gate | Shows operational maturity | 2 min |

---

## Scenario-Based Questions

### Scenario 1: "SolarWinds-Style Supply Chain Attack"

**Question:** "How would your platform detect a supply chain attack where a trusted software update installs a backdoor?"

**Expected answer:** "Supply chain attacks are among the hardest to detect because the malicious code arrives through a trusted channel. Detection relies on: (1) **Behavioral deviation** — the backdoor eventually does something the legitimate software never did (unexpected network connections, unusual child processes, registry modifications). UEBA detects this as anomalous for the entity. (2) **Network anomaly** — backdoor C2 traffic to a new domain or unusual beaconing pattern. (3) **Post-compromise activity** — lateral movement, credential dumping, or data staging that follows the initial backdoor activation. The key architectural insight is that the *initial installation* is nearly undetectable (it looks like a legitimate update), but the *post-exploitation behavior* is detectable if the platform has comprehensive behavioral baselines."

### Scenario 2: "Zero-Day Exploit in Production"

**Question:** "A zero-day exploit is being actively used against your organization. Your signature database has no match. How does the platform detect it?"

**Expected answer:** "By definition, a zero-day has no signature. Detection relies on: (1) **ML behavioral classification** — the exploit's payload execution pattern (process injection, privilege escalation, unusual memory access) triggers the ML classifier even without a signature. (2) **Process tree anomaly** — the exploited application spawns unexpected child processes (e.g., a PDF reader spawning cmd.exe). (3) **Network indicators** — the exploit downloads additional tools, contacts C2, or exfiltrates data. (4) **Threat intel correlation** — even without a specific signature, the C2 infrastructure may be linked to known threat actors. (5) **Vulnerability context** — the ASM/vulnerability management integration flags the exploited software as high-risk due to its internet exposure and known vulnerability class."

### Scenario 3: "Alert Storm During Ransomware Attack"

**Question:** "A ransomware attack hits 500 endpoints simultaneously. Your platform generates 50,000 alerts in 5 minutes. How does it handle this?"

**Expected answer:** "This is the alert correlation engine's most critical test: (1) **Deduplication** — 500 endpoints × 100 alerts each = 50,000 alerts, but they share the same ransomware IOCs, process patterns, and kill chain stage. The correlation engine should collapse these into 1-5 incidents (e.g., one per ransomware variant or per initial access vector). (2) **Automated triage** — SOAR immediately isolates endpoints showing encryption behavior (high-confidence, well-understood pattern). (3) **Blast-radius management** — the playbook has an escalation gate for >10 endpoints; SOC manager is immediately notified. (4) **Back-pressure** — the ingestion pipeline handles the 10x burst (agents increase verbosity during active incident); load shedding kicks in for non-essential enrichments. (5) **Prioritization** — identify the patient zero (initial access point) and the lateral movement path rather than treating all 500 endpoints equally."
...
6. **GenAI copilot safety:** How do you prevent prompt injection attacks through the security copilot, where adversarial content in telemetry manipulates the LLM?
7. **Identity threat detection:** How do you detect token theft and session hijacking that bypasses MFA?
8. **OCSF adoption:** How would you migrate from a proprietary event schema to the Open Cybersecurity Schema Framework without disrupting detection rules?

---

## Common Mistakes in System Design Interviews

### Mistake 1: Ignoring the Base Rate Problem

Candidates who design a single ML model and claim "99% accuracy" have not done the math. At 190B events/day, 1% false positive = 1.9B false alerts. The correct response is multi-signal corroboration + aggressive deduplication + adaptive thresholds.

### Mistake 2: Designing Detection Without Response

Detection without response is a notification system, not a security platform. Strong candidates design the full loop: detect → correlate → respond → feedback. They address blast radius, approval gates, and rollback.

### Mistake 3: Treating All Events Equally

Not all events deserve the same processing depth. The ML cascade pattern (bloom → fast → deep) is the correct answer. Candidates who propose running every event through a deep model have not considered the compute economics.

### Mistake 4: Cloud-Only Architecture

Candidates who design everything in the cloud forget that the endpoint needs to protect itself during cloud outages — which is exactly when an attacker may have disrupted cloud connectivity. The edge-cloud split is essential.

### Mistake 5: Ignoring the Human Element

Alert fatigue, analyst capacity limits, and approval gates are not "nice to haves" — they are core architectural constraints. The system exists to help humans make better decisions faster, not to replace them entirely.

---

## Architecture Decision Record (ADR) Template

Strong candidates document their decisions clearly. Here's the format:

```
ADR-001: Use streaming for primary detection pipeline

Status: Accepted
Context: Threat detection requires sub-second latency for critical threats.
         Batch processing would introduce minutes of delay.
Decision: Use stream processing for all Tier 1 (cloud real-time) detection.
          Use batch processing only for Tier 2 (behavioral baselines) and Tier 3 (threat hunting).
Consequences:
  - Stream state management adds complexity (checkpointing, exactly-once)
  - Must handle out-of-order events (configurable reorder window)
  - Lower latency enables automated response within seconds
  - Higher operational cost than batch-only approach
```
...
|--------|-------|---------------|
| Events/sec (large enterprise) | ~2.2M | Drives streaming architecture decision |
| Events/day | ~190B | Makes false positive math tangible |
| Benign-to-malicious ratio | ~50B:1 | Explains why 99.99% precision is insufficient |
| ML cascade filter ratio | ~95% at fast stage | Explains 20x GPU cost reduction |
| Hot store (24h) | ~15 TB | Sizes the streaming platform |
| Warm store (30d) | ~450 TB | Sizes the search cluster |
| MTTD target (critical) | <1 min | Drives real-time pipeline latency budget |
| MTTR target (automated) | <5 min | Drives SOAR execution latency |
| Analyst capacity | 50-100 alerts/day | Explains the 200x gap and correlation requirement |
| Agent local buffer | 48h | Determines offline protection window |
| False positive cost | ~$30/investigation | Makes alert fatigue economic argument concrete |
| Average breach cost | ~$4.5M | Makes false negative cost concrete |
