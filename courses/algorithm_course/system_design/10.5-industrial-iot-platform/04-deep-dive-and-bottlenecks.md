...
### 1.3 Ingestion Slowest part of the process Analysis

| Slowest part of the process | Symptoms | Mitigation |
|---|---|---|
...
### 2.3 Edge Slowest part of the process Analysis
...
### 3.4 Alert Engine Slowest part of the process Analysis
...
### 4.2 Gateway Slowest part of the process Analysis
...
1. Time-range Cutting off unnecessary steps:
...
### 5.3 TSDB Slowest part of the process Analysis
...
## 7. Deep Dive: Unified Namespace (UNS) Architecture

### 7.1 UNS as the Integration Backbone

The Unified Namespace replaces the traditional ISA-95 pyramid's point-to-point integrations with a single MQTT broker cluster serving as the central event bus:

```
Traditional ISA-95 Pyramid:        Unified Namespace:

   ┌─────┐                        ┌──────────────────────┐
   │ ERP │                        │   MQTT Broker Cluster  │
   └──┬──┘                        │   (Unified Namespace)  │
      │                           └──────────┬───────────┘
   ┌──▼──┐                                   │
   │ MES │                    ┌───┬───┬───┬───┼───┬───┬───┐
   └──┬──┘                    │   │   │   │   │   │   │   │
      │                      PLC SCADA MES ERP Twin ML  HMI ...
   ┌──▼──┐
   │SCADA│                    Every system publishes and
   └──┬──┘                    subscribes — no hierarchy,
...
### 7.2 UNS Topic Structure

```
Topic Hierarchy (ISA-95 aligned):

  {enterprise}/{site}/{area}/{line}/{cell}/{device}/{tag}

Examples:
  acme/houston/refinery-a/crude-unit/heater-h101/TT-4201/PV
  acme/houston/refinery-a/crude-unit/heater-h101/TT-4201/SP
  acme/houston/refinery-a/crude-unit/heater-h101/TT-4201/ALARM
  acme/houston/refinery-a/crude-unit/_metrics/OEE
  acme/houston/_site-metrics/energy/total_kwh

Benefits:
  - Any consumer subscribes to any level of granularity:
    acme/houston/+/+/+/+/ALARM    → all alarms at Houston
    acme/+/+/crude-unit/#          → all crude unit data globally
  - Adding a new analytics system: subscribe and go (zero producer changes)
  - M systems × N data sources = M+N connections (vs. M×N point-to-point)
```
