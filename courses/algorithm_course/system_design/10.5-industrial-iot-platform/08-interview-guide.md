...
### 5.5 Unified Namespace (UNS) Architecture
- Single MQTT broker cluster as "single source of truth" for all operational data
- Replaces ISA-95 pyramid and point-to-point integrations with hub-and-spoke event-driven architecture
- Topic hierarchy follows ISA-95 levels: `enterprise/site/area/line/cell/device/tag`
- Every system (PLC, SCADA, MES, ERP, historian, analytics) both publishes to and subscribes from the namespace
- Eliminates data silos and enables real-time event-driven integration across OT and IT

### 5.6 Industry 5.0 and Sustainability Features
- Energy monitoring at machine level with per-product-unit carbon footprint tracking
- Digital product passports for EU regulatory compliance (Ecodesign Regulation)
- Sustainability KPI dashboards alongside traditional OEE metrics
- Cobot safety zones with real-time workspace monitoring using LiDAR/vision
- Reconfigurable manufacturing: modular production lines with plug-and-produce capabilities

---

## 6. Advanced Interview Topics (2025-2026)

### 6.1 TSN (Time-Sensitive Networking) Integration

**Question:** "How does TSN change the industrial network architecture?"

**Strong answer:** "TSN (IEEE 802.1) enables converged networks where real-time control traffic and best-effort IT traffic share the same physical Ethernet infrastructure. TSN switches use time-aware scheduling (802.1Qbv) to guarantee bounded latency for control streams (sub-1ms cycle times) while allowing analytics traffic in remaining bandwidth. A Central Network Controller (CNC per 802.1Qcc) manages stream reservations. This replaces dedicated fieldbus networks with standard Ethernet, reducing cabling and switch costs. OPC UA over TSN and CC-Link IE TSN are the leading application profiles. The IIoT platform benefits because TSN-aware gateways can guarantee delivery timing for both control data and telemetry ingestion."

### 6.2 Private 5G for Manufacturing

**Question:** "When would you use 5G instead of Wi-Fi for industrial IoT?"

**Strong answer:** "Private 5G wins over Wi-Fi in three scenarios: (1) deterministic latency — URLLC provides sub-1ms guaranteed latency for AGV/AMR fleet coordination and real-time quality inspection, (2) harsh RF environments — metal structures, electromagnetic interference, and large factory floors where Wi-Fi coverage is unreliable, (3) massive sensor density — mMTC supports thousands of sensors per cell. The architecture uses an on-premises 5G core with local breakout so data never leaves the facility. Wi-Fi 6E wins on cost and IT team familiarity. Many plants deploy both: 5G for mission-critical mobility and Wi-Fi for office/monitoring."

### 6.3 Unified Namespace (UNS) Pattern

**Question:** "How would you integrate PLC, SCADA, MES, ERP, and analytics systems?"

**Strong answer:** "The Unified Namespace pattern: a centralized MQTT broker cluster as the single source of truth for all operational data. Topic hierarchy follows ISA-95: `enterprise/site/area/line/cell/device/tag`. Every system both publishes to and subscribes from this namespace. Adding a new analytics system requires zero changes to existing systems — just subscribe to relevant topics. This replaces the N×(N-1)/2 point-to-point integration problem with N pub/sub connections. The MQTT broker becomes the most critical infrastructure — must exceed 99.99% availability with clustered deployment and automatic failover."

### 6.4 Foundation Models for Industrial Time Series

**Question:** "How does generative AI apply to industrial IoT?"

**Strong answer:** "Three applications: (1) Foundation models for time series — pretrained on large industrial sensor datasets, then fine-tuned for specific equipment. This reduces the labeled data needed for predictive maintenance from months to days. (2) LLM-powered operator interfaces — natural language queries against time-series data ('show me the vibration trend for Pump P-4201 last Tuesday during the pressure spike'). (3) Automated root cause analysis — LLMs correlating alarm sequences, process data, and maintenance history to suggest probable root causes. The key challenge is explainability: maintenance engineers need to understand why the model flagged an alert, using SHAP values and attention maps, not just a confidence score."
