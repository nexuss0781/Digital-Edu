...
## 8. Industry 5.0 Sustainability Compliance

### 8.1 Emerging Regulatory Requirements

| Regulation | Jurisdiction | Key Requirements | Platform Impact |
|---|---|---|---|
| **EU Ecodesign Regulation** | EU | Digital product passports with lifecycle environmental data | Track energy consumption per product unit through production process |
| **EU Corporate Sustainability Reporting** | EU | Mandatory ESG reporting including Scope 1-3 emissions | Machine-level energy monitoring, carbon intensity calculation per product |
| **EU Battery Regulation** | EU | Battery passport with manufacturing, composition, and recycling data | Supply chain data integration, production traceability |
| **Carbon Border Adjustment Mechanism** | EU | Carbon tariff on imports based on embedded emissions | Per-product carbon footprint calculation from manufacturing data |

### 8.2 Sustainability Data Architecture

```
Sustainability Metrics Pipeline:

  Machine-level energy meters → MQTT (real-time kWh per machine)
  Production counters → MQTT (units produced per machine)
  Derived: kWh per unit = energy / production_count
  Grid carbon intensity → external API (gCO2/kWh)
  Derived: gCO2 per unit = kWh_per_unit × grid_carbon_intensity
  ...
  Digital Product Passport:
    ...
    raw_materials: [{material, source, quantity}]
    quality_certifications: [{cert_type, cert_id}]
