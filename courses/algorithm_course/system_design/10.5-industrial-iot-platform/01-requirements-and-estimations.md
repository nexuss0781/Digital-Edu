...
| **OPC UA over MQTT** | Support OPC UA Pub/Sub (Part 14) over MQTT for structured industrial data with semantic context over lightweight transport |
| **TSN Integration** | Ingest telemetry from Time-Sensitive Networking streams with deterministic timing guarantees for control-quality data |
| **Unified Namespace** | Support UNS topic hierarchy (`enterprise/site/area/line/cell/device/tag`) as the centralized event bus architecture |
