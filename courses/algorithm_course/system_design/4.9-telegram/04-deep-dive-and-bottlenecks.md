...
### Slowest part of the process #1: Channel Subscriber Database
...
### Slowest part of the process #2: Connection Server Memory
...
### Slowest part of the process #3: Search Index Update Lag
...
| Slowest part of the process | Impact | Mitigation | Result |
|------------|--------|------------|--------|
...
### 3. Topic-Based Threading in Large Supergroups

```
┌─────────────────────────────────────────────────────────────────┐
│            TOPIC THREADING ARCHITECTURE                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Supergroup: "Crypto Community" (180K members)                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                                                           │  │
│  │  Topic: "General"          │ PTS: 45892 │ Messages: 2.1M │  │
│  │  Topic: "Trading"          │ PTS: 23451 │ Messages: 890K │  │
│  │  Topic: "Technical"        │ PTS: 12340 │ Messages: 410K │  │
│  │  Topic: "Announcements"    │ PTS: 1230  │ Messages: 45K  │  │
│  │  Topic: "Off-Topic"        │ PTS: 8901  │ Messages: 320K │  │
│  │                                                           │  │
│  │  Each topic has:                                          │  │
│  │  • Independent PTS counter                                │  │
│  │  • Own notification settings per member                   │  │
│  │  • Separate fanout queue                                  │  │
│  │  • Independent pinned messages                            │  │
│  │  • Per-topic read state tracking                          │  │
│  │                                                           │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
│  Storage: Messages keyed by (group_id, topic_id, message_id)   │
│  Fanout: Each topic fans out independently to subscribers      │
│  Sync: Client tracks PTS per topic, not per group              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
|----------|--------|-----------|
| PTS per topic vs per group | Per topic | Cheaper sync but needs summary endpoint |
| Topic creation | Admin-only | Prevents spam topics |
| Default notification | Per-topic mute | Reduces noise in large groups |
| Message ordering | Within topic only | Cross-topic ordering not guaranteed |
