...
## Topic Threading Metrics

| Metric | Type | Description | Labels |
|--------|------|-------------|--------|
| `topic_messages_total` | Counter | Messages per topic | group_id, topic_id |
| `topic_active_users` | Gauge | Users active in topic (5-min window) | group_id, topic_id |
| `topic_fanout_duration_seconds` | Histogram | Per-topic fanout latency | group_id, topic_id |
| `topic_sync_requests_total` | Counter | getDifference calls per topic | group_id, topic_id |
| `topic_cross_topic_summary_latency` | Histogram | Summary endpoint latency | group_id |

## Mini App Observability

| Metric | Type | Description | Labels |
|--------|------|-------------|--------|
| `miniapp_sessions_active` | Gauge | Active Mini App sessions | bot_id |
| `miniapp_auth_validation_total` | Counter | initData validations | bot_id, status |
| `miniapp_auth_replay_rejected` | Counter | Replay attacks detected (stale auth_date) | bot_id |
| `miniapp_webview_load_seconds` | Histogram | Time to first interactive | bot_id |

## Stories Lifecycle Metrics

| Metric | Type | Description | Labels |
|--------|------|-------------|--------|
| `stories_created_total` | Counter | Stories published | - |
| `stories_expired_total` | Counter | Stories auto-deleted (24h TTL) | - |
| `stories_bucket_size_bytes` | Gauge | Size of each hourly story bucket | bucket_hour |
| `stories_bucket_drop_duration` | Histogram | Time to drop expired bucket | - |
| `stories_views_total` | Counter | Story views | story_id |
