...
## Insight 9: Topic-Based Threading Partitions a 200K-Member Supergroup Into Independent Message Streams

**Category:** Partitioning
**One-liner:** Instead of forcing 200K members into a single chronological feed, topic-based threads create independent message streams within the same group -- each with its own PTS counter, notifications, and read state.

**Why it matters:** A 200K-member supergroup with a single message stream is unusable: messages from thousands of concurrent conversations interleave, making it impossible to follow any thread. Topic-based threading solves this by partitioning the group into independent topics (e.g., "General," "Support," "Announcements"), each behaving like a separate chat with its own monotonically increasing PTS counter. Members can subscribe to specific topics, mute others, and search within a single topic. Architecturally, this transforms a single hot-partition (one chat_id receiving all messages) into N parallel streams that can be processed, stored, and fanned out independently. The fanout engine treats each topic as a separate channel with the same subscriber base but independent delivery queues. The key design decision is whether topic PTS is independent or derived from the group's global PTS -- independent PTS enables cheaper per-topic sync but requires a global "summary" endpoint for cross-topic unread counts.

---

## Insight 10: Mini App Session Bridging Extends MTProto Auth to WebView Without Token Exposure

**Category:** Security
**One-liner:** Telegram Mini Apps receive a signed `initData` payload containing the user's identity, verified by HMAC-SHA256 with the bot token, allowing the web app to authenticate without ever seeing the user's MTProto session key.

**Why it matters:** Mini Apps run in a WebView -- a fundamentally less trusted execution environment than the native Telegram client. Exposing the user's auth_key or session tokens to JavaScript would allow any Mini App to impersonate the user on Telegram. The session bridging pattern creates a one-way authentication channel: the Telegram client generates an `initData` string containing user_id, chat_id, auth_date, and a hash computed as HMAC-SHA256(data, HMAC-SHA256("WebAppData", bot_token)). The Mini App's backend verifies this hash against its known bot token, confirming the user's identity without receiving any Telegram credentials. The auth_date field prevents replay attacks (reject initData older than 5 minutes). This pattern -- deriving a verifiable identity token from a primary authentication session without exposing the session itself -- is the same principle behind OAuth access tokens and JWT delegation, but implemented with minimal overhead for a constrained mobile environment.

---

## Insight 11: Append-Only Message Log with Tombstone Deletion Enables Global "Delete for Everyone"

**Category:** Consistency
**One-liner:** When a user deletes a message "for everyone," the system appends a tombstone record rather than physically removing the message, ensuring that all devices and multi-DC replicas converge to the same state through their normal sync mechanism.

**Why it matters:** Physical deletion in a distributed, multi-device system is a coordination nightmare. The message exists on the sender's 3 devices, the recipient's 2 devices, the server's primary and 4 replica DCs, and possibly in the search index. Attempting synchronous deletion across all locations would fail for any offline device, creating permanent inconsistency. The tombstone approach treats deletion as just another state change: append a "message_id X is deleted" event to the PTS-tracked event log. Every device, upon syncing, sees the tombstone and removes the message locally. Replicas apply the tombstone through their normal async replication. Search indexes process tombstones in their batch pipeline. The tombstone is retained for a configurable period (typically 30 days) to ensure even long-offline devices eventually see it. This pattern is identical to how Cassandra handles deletes -- a tombstone is a record that says "this key no longer exists" -- and is the only correct approach for deletion in eventually consistent, multi-replica systems.

---

## Insight 12: Stories 24h TTL Storage Uses Ring Buffer Semantics to Avoid Garbage Collection

**Category:** Data Structures
**One-liner:** Stories are stored in time-bucketed partitions (one per hour) that are dropped as entire units after 24 hours, avoiding the cost of per-record garbage collection across billions of entries.

**Why it matters:** With 500M DAU potentially posting stories, the system writes millions of story records per minute -- all of which must be deleted exactly 24 hours later. Per-record TTL deletion (as used by Cassandra tombstones) creates massive garbage collection overhead: each expired record generates a tombstone, tombstones accumulate until compaction, and compaction consumes CPU and I/O. The ring buffer approach partitions stories into hourly buckets (24 buckets in a ring). Writing a story appends to the current hour's bucket. Deletion is simply dropping the entire bucket that has aged past 24 hours -- a single metadata operation regardless of how many stories were in that bucket. Views and engagement metrics are stored separately (they persist beyond 24 hours for analytics). This time-bucketed drop pattern is widely used: time-series databases drop old partitions, log systems rotate and delete log files, and CDN caches use time-based eviction tiers.
