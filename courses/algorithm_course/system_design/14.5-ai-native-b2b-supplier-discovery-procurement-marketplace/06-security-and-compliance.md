...
- **Zero-engagement Cutting off unnecessary steps:** Products that receive zero buyer interactions (no clicks, no inquiries) for 90 days are automatically demoted in search ranking and eventually deactivated with a notice to the supplier.
...
### Protecting the Matching and Trust Models

```
AI-specific threat vectors:

  Embedding space poisoning:
    Attack: Suppliers craft product descriptions specifically to maximize embedding
    similarity with popular buyer queries, regardless of actual product relevance
    (adversarial text optimization against the embedding model).
    Detection: Monitor the distribution of embedding similarities per supplier.
    Suppliers whose listings consistently achieve top-5% similarity scores
    across unrelated categories are flagged for adversarial text review.
    Prevention: Embedding models retrained periodically with adversarial examples;
    listing descriptions canonicalized through attribute extraction pipeline before
    embedding generation (reducing attacker control over embedding input).

  Trust score gradient exploitation:
    Attack: Suppliers reverse-engineer trust scoring weights by observing score
    changes after specific actions, then optimize for score maximization rather
    than genuine service quality (Goodhart's Law applied to trust scoring).
    Detection: Compare trust score with independent quality signals (buyer reorder
    rate, dispute rate) to detect divergence between trust score and actual quality.
    Prevention: Periodically rotate relative weights of trust signal categories
    (within ±10% band) to prevent precise optimization. Add noise injection (±2%
    random perturbation) to displayed scores to prevent exact gradient estimation.

  RFQ routing model gaming:
    Attack: Suppliers manipulate engagement prediction features (always responding
    instantly with placeholder bids, then revising) to increase selection probability.
    Detection: Track bid revision patterns — suppliers submitting initial bids within
    5 minutes then revising by >15% are flagged for response time metric gaming.
    Prevention: Engagement model uses quality-adjusted response — response counted
    only when bid is substantive (contains specific pricing, MOQ, and lead time).
```
...
Competitive intelligence protection:
  ...
  Aggregation attacks:
    Risk: Buyer uses price benchmarking API to reverse-engineer individual transaction
    prices through narrow queries (specific specification + small geography + recent window).
    Prevention: Minimum k-anonymity threshold of 10 transactions per benchmark query.
    Queries yielding <10 data points return parent category benchmark instead.

  Competitor reconnaissance:
    Risk: Suppliers create fake buyer accounts to search for competitors' pricing
    and catalog details, or identify which buyers source from competitors.
    Prevention: Buyer account verification (business GSTIN/PAN required for full access).
    Search analytics flag accounts with abnormal patterns (searching only competitor
    supplier names, never placing RFQs or orders).

  Data export controls:
    Risk: Bulk data extraction via API for competitive intelligence or resale.
    Prevention: Rate-limited export APIs with audit logging. Bulk catalog exports
    require explicit approval. API response payloads watermarked with requesting
    account ID to trace leaks.
```
