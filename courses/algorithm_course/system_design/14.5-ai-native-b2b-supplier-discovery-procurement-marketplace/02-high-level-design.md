...
**Implication:** The event-sourced design provides a complete audit trail for dispute resolution (the system can replay the exact sequence of events for any order), enables analytics on order lifecycle patterns (average time in each state, Slowest part of the process identification), and supports the escrow state machine which must be provably consistent (funds must never be both held and released). The escrow ledger is a separate double-entry bookkeeping system with its own consistency guarantees, connected to the order event stream via an event handler that validates every escrow state transition against the order state machine rules.

### Decision 5: Embedded Procurement Finance with Risk-Adjusted Terms

The platform integrates procurement financing directly into the order workflow, enabling buyers to defer payment while suppliers receive immediate settlement. Unlike consumer BNPL (which is unsecured, short-duration, and standardized), B2B procurement finance requires:

- **Risk-adjusted terms:** Credit terms (net-30, net-60, net-90) are dynamically priced based on buyer creditworthiness (derived from GST filing history, bank statement analysis, and platform transaction history), supplier reliability (trust score), and order characteristics (commodity vs. custom, domestic vs. cross-border).
- **Invoice factoring integration:** Suppliers can sell their platform-generated invoices to factoring partners at a discount for immediate liquidity. The platform's escrow guarantee and buyer credit assessment reduce the factoring discount rate (because default risk is lower on platform-verified transactions than on standalone invoices).
- **Dynamic discounting:** Suppliers offer graduated discounts for early payment: 2% discount for payment in 10 days, 1% for 20 days, net price for 30 days. The platform surfaces these offers to buyers and automates the early payment workflow when the buyer's treasury policy permits.

**Implication:** The finance layer must be architecturally separated from the marketplace layer (different regulatory requirements, different data retention, different audit needs). The finance service consumes signals from the marketplace (buyer transaction history, supplier trust scores, order details) but maintains its own credit decisioning engine, its own ledger, and its own compliance framework. Integration is via events: when an order is placed with "net-60" terms, the marketplace emits an `ORDER_FINANCE_REQUESTED` event that the finance service consumes, evaluates, and either approves (emitting `FINANCE_APPROVED` with terms) or rejects (falling back to escrow payment).

### Decision 6: Catalog Normalization Pipeline with Entity Resolution
...
**Implication:** Entity resolution is the most challenging step because false positives (merging distinct products) are far more costly than false negatives (failing to merge duplicates). A "2-inch stainless steel pipe" from Supplier A and a "DN50 SS pipe" from Supplier B may be the same product, but a "2-inch stainless steel pipe" and a "2-inch stainless steel tube" are different products (pipe vs. tube have different wall thickness standards). The entity resolution model uses a three-class classifier (duplicate / variant / distinct) with a conservative merge threshold (>0.95 for auto-merge) and maintains a "candidate duplicate" queue for manual review when similarity is 0.85-0.95. The variant class creates "alternative product" edges in the product graph without corrupting the catalog through false merges. The classifier uses an asymmetric cost function where false merges are penalized 50x more than missed duplicates.
...
| **Recommendation Engine** | Generate proactive supplier recommendations based on buyer procurement history, similar-buyer patterns (collaborative filtering), seasonal demand forecasts, and supply diversification needs; recommend alternative products when primary specifications unavailable | Receives buyer context from search/RFQ services; returns personalized supplier and product suggestions |
| **Conversational Procurement Assistant** | Natural language interface for procurement workflows: requirement articulation, supplier comparison, order tracking, dispute filing, and spend analytics queries across web chat, WhatsApp, and mobile; multi-turn context with intent classification and entity extraction | Receives natural language input from messaging channels; orchestrates underlying services via API calls |

---

## Data Flow: Catalog Normalization Pipeline

```mermaid
flowchart LR
    A[Supplier Uploads\nCatalog\nCSV/Excel/API] --> B[Format\nValidation &\nSchema Normalization]
    B --> C[Attribute\nExtraction\nNLP on Descriptions]
    C --> D[Unit\nNormalization\nImperial → Metric]
    D --> E[Category\nClassification\n5,000+ Leaf Nodes]
    E --> F[Entity\nResolution\nDuplicate Detection]
...
## Data Flow: Supplier Trust Score Lifecycle
...
flowchart TB
    ...
    subgraph Processing["Trust Score Computation"]
        ING[Signal\nIngestion\nKafka Partitioned\nby supplier_id]
        DEC[Exponential\nDecay\nApplication]
        AGG[Weighted\nAggregation\n5 Categories]
        MAN[Manipulation\nDetection\nAnomaly Check]
    ...
    subgraph Output["Trust Score Consumers"]
        SRK[Search\nRanking]
        RFR[RFQ\nRouting]
        SDH[Supplier\nDashboard]
        FRD[Fraud\nAlerts]
    ...
    class V,T,B,R,E signal
    class ING,DEC,AGG,MAN process
    class SRK,RFR,SDH,FRD output
```

---

## Decision 6: Agentic Procurement with Policy-Bounded Autonomy

The platform supports agentic procurement where AI agents autonomously execute routine procurement workflows on behalf of buyer organizations. Rather than treating every procurement action as requiring human approval, the system defines an autonomy boundary through a configurable policy engine:

**Autonomous actions (no human approval required):**
- Reorder standard consumables from pre-approved suppliers when inventory drops below reorder point
- Accept bids within ±5% of the historical average price for the same specification
- Place orders below a configurable value threshold (default: ₹2 lakhs) with suppliers above a trust score threshold (default: 0.7)

**Escalation triggers (human approval required):**
- New supplier qualification (never sourced before)
- Specifications outside historical norms (different material, dimension, or certification than previous orders)
- Order values above the autonomy threshold
- Cross-border transactions with compliance implications
- Price deviations >10% from historical average

**Implication:** The policy engine must evaluate each procurement action against the buyer organization's configured rules in <50ms (on the critical path of automated order placement). Policy rules are stored as a decision tree per buyer organization, supporting hierarchical approval chains (procurement officer → procurement head → CFO based on order value tiers). The agentic procurement path generates a complete audit trail showing which policy rule authorized each autonomous action, enabling post-hoc review of agent decisions.

---

## Decision 7: ESG and Sustainability Scoring in Supplier Selection

Buyers increasingly require suppliers to meet environmental, social, and governance (ESG) criteria. The platform integrates sustainability signals into the supplier profile and search ranking:

**ESG signal sources:**
- Self-declared certifications (ISO 14001, SA8000, FSC, organic, fair trade)
- Third-party ESG audit scores (integrated from ESG rating providers)
- Carbon footprint estimates per product category (derived from manufacturing process, material type, and transportation distance)
- Labor compliance indicators (no child labor certifications, living wage commitments)

**Search integration:** Buyers can filter search results by ESG criteria ("show only ISO 14001 certified suppliers") or weight ESG in ranking ("prioritize suppliers with lower carbon footprint"). The re-ranking model accepts ESG preference weights as buyer-configurable parameters.

**Implication:** ESG scoring introduces a new dimension to the trust index. Unlike transaction-based trust (which grows with order volume), ESG trust is certification-based and requires periodic reverification. The system maintains separate ESG score decay rates (annual reverification for certifications) distinct from transaction-based trust decay (90-day half-life). Scope 3 emissions tracking requires the platform to estimate upstream supply chain carbon footprint—a computationally intensive graph traversal across the buyer-supplier match graph that runs as a daily batch job.

---

## Cross-Service Event Flow

The platform uses an event-driven architecture where services communicate through a central event bus. Key event flows:

```
Catalog Update Flow:
  SUPPLIER_CATALOG_UPLOADED → Catalog Normalizer
    → LISTING_NORMALIZED → Taxonomy Classifier + Attribute Extractor
    → LISTING_CLASSIFIED → Embedding Generator
    → EMBEDDING_GENERATED → Search Index (keyword + vector insertion)
    → LISTING_SEARCHABLE → Supplier Dashboard (confirmation notification)

RFQ-to-Order Flow:
  RFQ_CREATED → Specification Parser
    → SPEC_PARSED → Supplier Matcher + Engagement Predictor
    → SUPPLIERS_MATCHED → RFQ Router (constrained optimization)
    → RFQ_DISTRIBUTED → Bid Collector (async, hours-long)
    → BIDS_COLLECTED → Bid Normalizer + TCO Calculator
    → BIDS_COMPARED → Award Recommender
    → AWARD_RECOMMENDED → Buyer Decision (async, human-in-loop)
    → AWARD_ACCEPTED → PO Generator + Escrow Creator
    → ORDER_CREATED → Trust Scorer (update supplier engagement signal)

Trust Signal Flow:
  ORDER_DELIVERED → Buyer Acceptance / Dispute
    → BUYER_ACCEPTED → Trust Scorer (positive transaction signal)
                     → Escrow Service (release funds)
                     → Price Intelligence (record transaction price)
    → BUYER_DISPUTED → Dispute Resolver (evidence collection)
                     → Escrow Service (hold funds)
                     → Trust Scorer (dispute signal, weighted by resolution)

Finance Flow:
  ORDER_CREATED + PAYMENT_TERMS_REQUESTED → Finance Engine
    → CREDIT_ASSESSED → Terms Approval / Rejection
    → FINANCE_APPROVED → Invoice Generated → Factoring Available
    → INVOICE_FACTORED → Supplier receives immediate payment
    → BUYER_PAYMENT_DUE → Collection at term maturity
```

Each event carries a correlation ID (the originating RFQ or order ID) for end-to-end tracing, and events are persisted to an append-only event store for audit and replay. Services consume events idempotently—processing the same event twice produces no side effects—ensuring consistency during failure recovery.
