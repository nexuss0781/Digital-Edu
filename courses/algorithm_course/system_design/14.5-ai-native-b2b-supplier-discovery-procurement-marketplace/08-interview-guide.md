...
| Criterion | Expectation |
|---|---|
| Field-aware embeddings | Explains why separate embeddings per attribute family improve B2B search precision; quantifies the impact vs. single-vector approaches |
| Exponential decay trust | Derives the need for time-weighted trust scoring; discusses half-life calibration and the seasonal decay problem |
| RFQ routing as optimization | Formulates RFQ distribution as a constrained optimization with engagement prediction; identifies it as a two-sided matching problem, not one-sided search |
| Escrow state machine | Designs the escrow lifecycle with dispute handling, milestone payments, timeout-based release, and forced arbitration for dispute deadlock |
| Cold start solution | Addresses the trust bootstrapping problem for new suppliers with verification-based credit, starter tier guarantees, and transaction velocity incentives |
| Cross-border compliance | Discusses sanctions screening architecture, cascading HS code classification, and trade documentation generation pipeline |
| Marketplace concentration | Identifies winner-take-all dynamics; proposes HHI monitoring, diversity constraints, exploration budgets, and category concentration alerts |
| Price benchmark methodology | Explains regime-switching models for commodity-linked products; discusses confidence scoring and sparse data category rollup |
| Agentic procurement | Discusses policy-bounded autonomy for AI agents, configurable autonomy boundaries, and agent decision audit trails |
| Supplier fatigue as tragedy of the commons | Recognizes that RFQ routing is a mechanism design problem, not just an optimization; discusses attention budgets and the commons analogy |
...
### Trade-off 5: Human-Controlled vs. Agentic Procurement

**Question:** "Should the platform support fully autonomous AI agents placing orders on behalf of buyers?"

**Analysis:**

| Approach | Pros | Cons |
|---|---|---|
| Human-only (AI recommends, human decides) | Maximum control; clear accountability; no risk of unwanted orders; simpler compliance | Slow for routine purchases; procurement team Slowest part of the process; cannot scale to high-frequency reordering |
| Fully autonomous agents | Maximum speed; eliminates procurement team Slowest part of the process for routine items; enables just-in-time reordering | Liability for incorrect orders; risk of agent exploitation; compliance challenges; buyer trust concerns |
| Policy-bounded autonomy (agent acts within configurable rules, escalates otherwise) | Balances speed and control; configurable per organization; clear audit trail of autonomous decisions | Complex policy engine; edge cases where agent boundary is ambiguous; requires organizational change management |

**Recommended approach:** Policy-bounded autonomy. Define autonomy boundaries per buyer organization: order value thresholds, approved supplier lists, specification similarity to historical orders, and price deviation limits. The agent operates autonomously within these boundaries and escalates to human decision-makers when any boundary is exceeded. Every autonomous action is logged with the policy rule that authorized it, enabling post-hoc audit.

---

## Extension Topics for Senior Candidates

### Extension 1: Predictive Procurement

If the candidate reaches the extension phase, explore how the platform could predict procurement needs before buyers articulate them:
- Inventory depletion forecasting from connected ERP/inventory systems
- Seasonal demand pattern detection from historical procurement cycles
- Supply disruption early warning (weather, geopolitical events, supplier financial stress) triggering proactive alternative supplier identification
- Commodity price movement prediction for optimal purchase timing

### Extension 2: Marketplace Network Effects and Competitive Moats

Explore the platform's defensibility:
- How does the buyer-supplier match graph create a data moat that competitors cannot replicate?
- What prevents supplier multi-homing (listing on multiple marketplaces)?
- How does transaction-verified pricing become more valuable than catalog-based pricing as volume grows?
- When does the marketplace reach liquidity escape velocity (enough suppliers and buyers in each category to sustain competitive pricing)?

### Extension 3: Multi-Tier Supply Chain Visibility

Explore extending the platform beyond Tier 1 suppliers:
- How would you model Tier 2/3 supplier relationships (your supplier's supplier)?
- What are the architectural implications of computing supply chain risk scores across multiple tiers?
- How does Scope 3 emissions tracking require traversing the supplier graph beyond direct procurement?
- What privacy boundaries exist when sharing sub-tier supplier information across competing buyers?

### Extension 4: Embedded Procurement Finance

Explore the integration of financing into the procurement workflow:
- How does the platform assess buyer creditworthiness using marketplace transaction data vs. traditional credit scoring?
- What are the architectural implications of maintaining a separate finance ledger with different consistency and regulatory requirements?
- How does dynamic discounting (early payment discounts) create a three-sided marketplace dynamic between buyers, suppliers, and finance providers?
- What fraud vectors does embedded finance introduce (e.g., fictitious invoices, credit stacking across platforms)?
...
|---|---|---|
| "How do you handle a supplier who lists 100,000 products but only actually stocks 5,000?" | Catalog quality vs. breadth trade-off | Listing quality scoring, zero-engagement Cutting off unnecessary steps, inventory verification sampling, listing-level vs. supplier-level penalties |
| "What happens when commodity prices crash 40% in a month?" | Price benchmark regime handling | Regime detection algorithm, pre-shift vs. post-shift benchmarks, buyer notification of market condition changes, benchmark confidence scoring |
| "How do you prevent a large enterprise buyer from using the platform just for price discovery and then procuring directly?" | Platform value proposition and stickiness | Escrow protection, procurement analytics, credit terms only available on-platform, match graph data that improves with usage, compliance documentation |
| "What if two buyers need the same supplier's limited capacity?" | Concurrent RFQ capacity contention | Capacity tracking, concurrent RFQ alerts, bid validity windows, first-committed-gets-priority with transparent capacity communication |
