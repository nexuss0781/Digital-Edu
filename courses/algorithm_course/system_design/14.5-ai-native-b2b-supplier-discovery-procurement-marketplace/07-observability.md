...
Search critical path:
  Slowest part of the process: Vector retrieval (Span 4) — 4 parallel ANN queries, latency = max(4 queries)
...
RFQ critical path:
  Slowest part of the process: Specification parsing (Span 1) — NLP extraction from uploaded documents
...
Order critical path:
  Slowest part of the process: Sanctions screening (Span 2) — cannot proceed without clearance
