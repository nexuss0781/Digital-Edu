...
- Discusses open banking integration for cashflow-based affordability
- Proposes pay-by-bank migration strategy for collection cost optimization
- Designs adaptive plan optimization as a per-transaction revenue maximization engine
- Considers BNPL-as-a-Service multi-tenant architecture with per-tenant model fine-tuning
- Discusses cell-based architecture for blast radius isolation across consumer cohorts
...
6. **Open banking integration**: How does access to real-time bank transaction data change the credit decision architecture? What happens when consent expires mid-plan?

7. **Pay-by-bank migration**: How do you transition consumers from card-based to bank-direct (A2A) payment collection to reduce processor fees without degrading the consumer experience?
...
|-------------|---------------|-----------------|
| **Treating BNPL as a payment system** | BNPL is fundamentally a lending system; ignoring credit risk assessment misses the core challenge | Lead with the credit decision pipeline as the architectural centerpiece |
| **Generic retry for all payment failures** | Retrying "insufficient funds" every 5 minutes wastes processor capacity and annoys consumers | Classify failure types; apply tailored retry strategies (paycheck-aligned for NSF, notification for expired cards) |
| **Synchronous audit logging** | Blocking the 2-second credit decision for audit writes adds unnecessary latency | Fire-and-forget to a durable queue; audit writes are eventually persistent |
| **Single global spending power lock** | Global mutex on consumer spending power creates contention at checkout | Time-bounded credit reservations with TTL (15 minutes) |
| **Storing raw credit bureau data** | Regulatory, storage, and privacy risk; PII over-retention | Store only derived risk scores and feature vectors; raw data purged after scoring |
| **Flat merchant discount rate** | Same rate across all verticals and risk profiles wastes margin optimization | Dynamic pricing by merchant category, volume tier, chargeback history, and competitive pressure |
| **Ignoring the collections lifecycle** | 5--15% of plans enter collections; treating this as Edge Case (Unusual or extreme situation) misses major revenue impact | Design collections as a first-class product flow with structured dunning, hardship programs, and charge-off workflows |
...
|--------|-------|---------------|
| Credit decision latency budget | 2 seconds | Checkout abandonment increases 7% per second |
| Bureau cache hit rate | ~70% | 30% of decisions pay the 300-500ms bureau API cost |
| Pay-in-4 share of plans | ~70% | Dominant product; simplest but lowest revenue per plan |
| Merchant discount rate range | 2-8% | Primary revenue source; varies 4x by vertical |
| First-attempt collection success | ~92% | Each 1% improvement = ~$45M annual recovery |
| Default rate (charge-off) | 2-4% | 1% swing = ~$70M annual P&L impact |
| Outstanding receivables | ~$4.5B | Total platform financial exposure |
| Active plans | 50M | Each a long-lived state machine |
| Late fee revenue share | ~15% | Significant but regulatory-constrained revenue |
| Peak-to-average ratio | 15x | Black Friday holiday spike |
