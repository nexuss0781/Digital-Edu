...
OpenBankingConsent {
    ...
    consumer_id         UUID        FK → Consumer
    provider            STRING      -- aggregation provider identifier
    accounts_scope      JSON        -- which accounts are accessible
...
### Consumer Risk Feature Vector

```
ConsumerRiskFeatures {
    consumer_id                 UUID        PRIMARY KEY
    computed_at                 TIMESTAMP
    -- Bureau-derived features
    bureau_score_range          STRING      -- e.g., "700-749"
    open_trade_lines            INT
    total_utilization_pct       DECIMAL
    derogatory_marks            INT
    credit_age_months           INT
    -- Platform-derived features
    plans_completed             INT
    plans_defaulted             INT
    on_time_payment_rate        DECIMAL     -- 0.0 to 1.0
    avg_days_late               DECIMAL
    total_lifetime_gmv          DECIMAL
    active_plan_count           INT
    months_since_first_plan     INT
    -- Velocity features
    plans_last_7_days           INT
    plans_last_30_days          INT
    merchants_last_30_days      INT
    checkout_attempts_today     INT
    -- Open banking features (nullable)
    monthly_income_detected     DECIMAL
    essential_expenses          DECIMAL
    disposable_income           DECIMAL
    overdraft_count_90d         INT
    balance_trend               ENUM        (rising, stable, declining)
...
INDEX: (computed_at)  -- staleness queries
```
...
DunningSequence {
    ...
    plan_id             UUID        FK → InstallmentPlan
    consumer_id         UUID        FK → Consumer
    stage               ENUM        (reminder_1, reminder_2, late_notice,
                                     hardship_offer, formal_notice,
                                     final_notice, charge_off_warning)
    channel             ENUM        (email, sms, push, in_app, phone)
    scheduled_at        TIMESTAMP
    sent_at             TIMESTAMP
    delivered_at        TIMESTAMP
    opened_at           TIMESTAMP   -- email open tracking
    action_taken        ENUM        (none, paid, updated_method, hardship_applied, ignored)
    template_version    STRING
    created_at          TIMESTAMP
}
INDEX: (plan_id, stage), (scheduled_at, sent_at)
```
...
JurisdictionConfig {
    ...
    state_code          STRING      -- ISO 3166-2 (null for country-level)
    max_apr             DECIMAL     -- null = no cap
    max_late_fee_fixed  DECIMAL     -- max flat late fee
    max_late_fee_pct    DECIMAL     -- max late fee as % of payment
    grace_period_days   INT
    cooling_off_days    INT         -- right of withdrawal (EU: 14 days)
    disclosure_template STRING      -- template identifier (TILA, SECCI, etc.)
    lending_license_id  STRING      -- platform's license in this jurisdiction
    bnpl_exempt_max_installments INT -- plans below this count may be exempt
    requires_creditworthiness_check BOOLEAN
    requires_affordability_check    BOOLEAN
    effective_from      DATE
    effective_to        DATE        -- null = currently active
    created_at          TIMESTAMP
}
INDEX: (country_code, state_code, effective_to)
```
...
```
GET /v1/consumer/plans
Purpose: List all consumer's installment plans
Headers: Authorization: Bearer <consumer_token>
Query params: status=active|completed|all, page, page_size
Response 200: {
    plans: [
        {
            plan_id: "plan_def456",
            merchant_name: "Electronics Co",
            total_amount: 600.00,
            remaining_balance: 300.00,
            plan_type: "pay_in_4",
            status: "active",
            next_payment: { amount: 150.00, due_date: "2026-04-06" },
            progress: { paid: 2, total: 4 }
        }
    ],
    spending_power: { available: 800.00, total: 1500.00 },
    on_time_rate: 0.97
}
```

```
GET /v1/consumer/plans/{plan_id}/schedule
Purpose: View full payment schedule for a plan
Headers: Authorization: Bearer <consumer_token>
Response 200: {
    plan_id: "plan_def456",
    installments: [
        { number: 1, amount: 150.00, status: "completed", paid_at: "2026-03-09" },
        { number: 2, amount: 150.00, status: "completed", paid_at: "2026-03-23" },
        { number: 3, amount: 150.00, status: "scheduled", due_date: "2026-04-06" },
        { number: 4, amount: 150.00, status: "scheduled", due_date: "2026-04-20" }
    ],
    total_paid: 300.00,
    remaining: 300.00,
    late_fees: 0.00
...
### Hardship Management API

```
POST /v1/plans/{plan_id}/hardship
Purpose: Request hardship accommodation
Headers: Authorization: Bearer <consumer_token>
Body: {
    reason: "job_loss",
    preferred_option: "extended_terms",
    supporting_info: "Lost employment on 2026-03-01"
}
Response 200: {
    hardship_id: "hrd_abc123",
    eligible_options: [
        {
            type: "extended_terms",
            new_installment_amount: 75.00,
            new_num_installments: 8,
            apr: 0.0,
            description: "Reduce payments by 50%, extend by 4 months"
        },
        {
            type: "reduced_payment",
            temporary_amount: 50.00,
            duration_months: 3,
            then_resume: 150.00,
            description: "Pay $50/month for 3 months, then resume $150"
        },
        {
            type: "waive_late_fees",
            waived_amount: 25.00,
            description: "Waive $25 in accrued late fees"
        }
    ],
...
```
FUNCTION compute_late_fee(payment, jurisdiction):
    config = get_jurisdiction_config(jurisdiction)

    -- Calculate fee based on jurisdiction rules
    fee_fixed = config.max_late_fee_fixed OR PLATFORM_DEFAULT_LATE_FEE
    fee_pct = payment.amount × (config.max_late_fee_pct OR 0.05)

    -- Apply the lesser of fixed or percentage (consumer-friendly)
    late_fee = MIN(fee_fixed, fee_pct)

    -- Additional caps
    late_fee = MIN(late_fee, payment.amount × 0.25)  -- never > 25% of payment
    late_fee = MIN(late_fee, PLATFORM_MAX_LATE_FEE)   -- absolute platform cap

    -- Check per-plan late fee accumulation cap
    plan = get_plan(payment.plan_id)
    IF plan.late_fees_accrued + late_fee > plan.total_amount × 0.10:
        -- Total late fees cannot exceed 10% of original plan amount
        late_fee = MAX(0, plan.total_amount × 0.10 - plan.late_fees_accrued)

    RETURN ROUND(late_fee, 2)
```

## Spending Power Recalculation Algorithm

```
FUNCTION recalculate_spending_power(consumer):
    -- Base spending power from credit tier
    base = TIER_SPENDING_POWER[consumer.credit_tier]

    -- Adjust based on repayment history
    IF consumer.on_time_rate >= 0.98 AND consumer.plans_completed >= 3:
        base = base × 1.3  -- 30% bonus for excellent history
    ELSE IF consumer.on_time_rate < 0.85:
        base = base × 0.5  -- 50% reduction for poor history

    -- Deduct outstanding commitments
    total_outstanding = get_total_outstanding(consumer.id)
    active_reservations = get_active_reservations(consumer.id)
    available = base - total_outstanding - SUM(active_reservations)

    -- Apply platform-wide cap
    available = MIN(available, PLATFORM_MAX_SPENDING_POWER)
    available = MAX(available, 0)  -- never negative

    -- Update consumer record
    consumer.spending_power = available
    RETURN available
```
