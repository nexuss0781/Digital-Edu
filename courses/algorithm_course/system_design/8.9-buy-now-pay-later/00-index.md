...
| [8.6 - Distributed Ledger/Core Banking](../8.6-distributed-ledger-core-banking/) | Double-entry accounting, reconciliation, regulatory audit infrastructure |
| [8.14 - Super App Payment Platform](../8.14-super-app-payment-platform/) | UPI/TPP integration, rewards engines, mini-app checkout embedding |
| [8.11 - UPI Real-Time Payment System](../8.11-upi-real-time-payment-system/) | Account-to-account payment rails, VPA resolution, A2A collection for installments |

---

## Related Architectural Patterns

| Pattern | How It Applies to BNPL |
|---------|----------------------|
| **Staged Pipeline with Parallel Fan-Out** | Credit decision orchestrates bureau pull, feature lookup, and device scoring in parallel within a 2-second budget |
| **Long-Lived State Machine** | Each installment plan is an independent state machine (active → overdue → delinquent → charge-off) surviving weeks/months of failures |
| **Time-Bounded Optimistic Reservation** | Credit reservations with TTL solve the concurrent-checkout double-spend problem without global locks |
| **Failure-Type-Specific Retry** | Payment collection classifies failures (insufficient funds, expired card, processor error) and applies distinct retry strategies per type |
| **Snapshot-Based Aggregation** | Settlement uses snapshot timestamps to prevent race conditions between late refunds and settlement cutoffs |
| **Per-Dependency Circuit Breaker** | Each external dependency (bureau, processor, card network) has its own circuit breaker with tailored fallback behavior |
| **Event-Sourced Audit Trail** | Every financial action produces an immutable, hash-chained audit event for 7-year regulatory retention |
| **Cell-Based Blast Radius Isolation** | Consumer cohorts are assigned to independent cells; a cell failure affects only ~10% of the consumer base |
...
|-------|----------------------|
| **CFPB treats BNPL as credit cards** (2024) | Dispute rights, periodic statements, and billing error procedures now mandatory; Dispute Resolution Service must implement Reg Z workflows |
| **EU Consumer Credit Directive** (2023/2225) | Creditworthiness assessment required even for interest-free plans; SECCI document generation added to checkout flow |
| **UK FCA regulation** (2025) | FCA authorization required; affordability assessment mandatory; Financial Ombudsman jurisdiction for complaints |
| **Open banking integration** | Real-time income verification and cashflow analysis enhance credit decisions; reduces reliance on credit bureau scores |
| **Pay-by-bank / A2A payments** | Account-to-account collection reduces processor fees from ~2.5% to ~0.5%; requires different retry architecture |
| **BNPL-as-a-Service** | White-label platform for banks/fintechs; multi-tenant architecture with per-tenant risk policies and branding |
| **Adaptive checkout** | Dynamic plan optimization per consumer-merchant-order tuple maximizes revenue per decision |
| **Alternative data scoring** | Rent payments, utility bills, gig income, and open banking data enable thin-file consumer approval |
...
- CFPB --- Interpretive Rule: BNPL Products as Credit Cards (2024)
- Affirm Engineering --- Real-Time Underwriting at Scale
- Affirm --- Adaptive Checkout: ML-Optimized Plan Selection
...
- UK FCA --- Buy Now Pay Later Regulatory Framework (2025)
- Marqeta --- Virtual Card Issuance for BNPL Platforms
- Steptoe --- BNPL Regulatory Compliance Under TILA
- Open Banking Europe --- Account Information and Payment Initiation for BNPL
- Plaid --- Real-Time Income Verification for Lending Decisioning
