...
## CFPB Interpretive Rule: BNPL as Credit Cards (2024+)

The US Consumer Financial Protection Bureau issued an interpretive rule treating BNPL products as credit cards under Regulation Z. This has significant architectural implications:

```
New Requirements Under CFPB Rule:

1. Dispute Rights (Reg Z §§ 1026.12-13):
   - Consumers can dispute charges with the BNPL provider (not just the merchant)
   - BNPL provider must investigate within 30 days
   - Consumer not liable during investigation (payments paused on disputed amount)
   - Billing error resolution procedures required

2. Refund Protections:
   - Merchants who accept returns must credit the BNPL provider
   - BNPL provider must credit the consumer's account within 7 business days
   - If merchant refuses return, consumer can dispute with BNPL provider

3. Periodic Statements:
   - Monthly statements required for active plans (previously optional for some BNPL)
   - Must include: balance, payment due date, minimum payment, late fee warning

4. Creditworthiness Assessment:
   - Must assess ability to repay (previously some Pay-in-4 products were exempt)
   - Documentation of assessment methodology required

Architectural Impact:
  - Dispute Resolution Service upgraded to handle Reg Z billing error procedures
  - Statement Generation Service added (monthly batch for all active plans)
  - Refund flow must enforce 7-business-day credit timeline
  - Credit decision audit must capture assessment methodology documentation
  - Consumer notification templates updated with Reg Z required disclosures
```
...
```
AML Framework for BNPL:

Transaction Monitoring:
  - Pattern detection: multiple plans for high-value goods (electronics, gift cards)
    shipped to different addresses → potential money laundering
  - Structuring detection: multiple plans just below reporting thresholds
  - Geographic risk scoring: transactions involving high-risk jurisdictions

Suspicious Activity Reporting (SAR):
  - Automated flagging when transaction patterns match known typologies
  - Manual review queue for borderline cases
  - SAR filing within 30 days of detection (BSA requirement)
  - SAR records retained for 5 years

Know Your Customer (KYC) Tiering:
  - Tier 1 (< $500 total outstanding): basic identity verification
  - Tier 2 ($500-$2,000): enhanced verification (address + identity document)
  - Tier 3 (> $2,000): full KYC (income verification + source of funds)

Sanctions Screening:
  - All consumers checked against OFAC SDN list at onboarding
  - Ongoing screening against updated sanctions lists (daily refresh)
  - Merchant screening for counter-terrorism financing
```
...
```
Data Breach Response (GDPR 72h / State Breach Laws):
...
Hour 1-4:  Assessment
  - Determine scope: which consumers/data affected
  - Classify severity: PII only vs. financial data vs. payment methods
  - Assess regulatory notification obligations

Hour 4-24: Notification preparation
  - Draft regulatory notification (DPA, state AG, CFPB as applicable)
  - Draft consumer notification
  - Brief executive team and legal counsel

Hour 24-72: Regulatory notification
  - Submit to relevant DPAs within 72h (GDPR)
  - Submit to state AGs per state breach notification laws
  - File with CFPB if consumer financial data involved

Day 3-30: Consumer notification and remediation
  - Notify affected consumers with required details
  - Offer credit monitoring if credit data exposed
