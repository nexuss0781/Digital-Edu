...
Time Complexity (Speed of the algorithm): O(n * m * log(m))
  where n = number of entities, m = number of feature records

Space Complexity (Memory usage of the algorithm): O(n * f)
...
Benefits:
- Date-based Cutting off unnecessary steps for training queries
