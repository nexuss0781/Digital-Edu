...
### Q: "How would you add LLM serving to this platform?"

**Good Answer:**
> "KServe already supports LLM serving via vLLM and TensorRT-LLM runtimes. The key difference from traditional ML serving is that LLMs need streaming responses (SSE), GPU memory management (PagedAttention), tensor parallelism across multiple GPUs, and much larger resource requirements. I'd use KServe's InferenceService with the vLLM runtime, configure tensor parallelism for models > 1 GPU, and set autoscaling based on GPU utilization rather than request concurrency. For cost, INT8/INT4 quantization reduces memory by 2-4x with minimal accuracy loss."

### Q: "How would you handle GenAI observability in this platform?"

**Good Answer:**
> "MLflow 3.x includes GenAI tracing and LLM-as-judge evaluation. For tracing, MLflow instruments LangChain/LlamaIndex chains to capture each step's input/output/latency. For evaluation, we can use LLM-as-judge metrics to assess output quality (relevance, faithfulness, toxicity) alongside traditional metrics. Drift detection with Evidently extends to embedding drift — monitoring whether input distributions shift in the embedding space. The platform would also integrate Langfuse for more detailed LLM observability if needed."

### Q: "When would you choose Flyte over Airflow for ML orchestration?"

**Good Answer:**
> "Airflow excels at general-purpose workflow orchestration and has the largest ecosystem. But Flyte is purpose-built for ML with strong typing, immutable task execution, automatic data lineage, and native Kubernetes integration. If the platform is exclusively for ML workloads and the team values type safety and reproducibility, Flyte is better. If the platform serves both data engineering and ML teams, or the team already knows Airflow, stick with Airflow. The 2025-2026 trend is that both have converged — Airflow 3.x added dataset-driven scheduling, and Flyte improved its plugin ecosystem."
...
| LLM Gateway & Prompt Management | MLflow Deployments Server patterns | 3.21 |
| AI Observability & LLMOps | GenAI monitoring with MLflow Tracing | 3.25 |
