...
## GenAI Extension Architecture

The OSS ML platform extends to support GenAI workloads through additional components integrated with the existing stack:

```mermaid
flowchart LR
    subgraph GenAI["GenAI Extensions"]
        direction TB
        Gateway["MLflow Deployments<br/>Server (AI Gateway)"]
        Tracing["MLflow Tracing<br/>(LLM Observability)"]
        Eval["MLflow Evaluate<br/>(LLM-as-Judge)"]
        VectorStore["Feast Vector<br/>Backend (RAG)"]
    end

    subgraph LLMServing["LLM Serving"]
        vLLM["vLLM Runtime"]
        KServeL["KServe<br/>(LLM InferenceService)"]
    end

    subgraph Existing["Existing Platform"]
        MLflow["MLflow Registry"]
        KServe2["KServe<br/>(Traditional ML)"]
        Feast["Feast Online Store"]
    end

    Gateway -->|Route| vLLM
    Gateway -->|Route| KServeL
    Tracing -->|Log| MLflow
    Eval -->|Metrics| MLflow
    VectorStore -->|Embeddings| Feast

    vLLM --> KServeL
    KServeL --> KServe2

    classDef genai fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef llm fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    classDef existing fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px

    class Gateway,Tracing,Eval,VectorStore genai
    class vLLM,KServeL llm
    class MLflow,KServe2,Feast existing
```

**Key integration points:**
- **MLflow Deployments Server** acts as a unified AI Gateway, routing requests to self-hosted (vLLM) or external LLM providers behind a single API
- **MLflow Tracing** instruments LangChain/LlamaIndex chains for full execution visibility
- **MLflow Evaluate** runs LLM-as-judge metrics (relevance, faithfulness, toxicity) as part of CI/CD
- **Feast Vector Backend** provides embedding retrieval for RAG, maintaining train-serve consistency
