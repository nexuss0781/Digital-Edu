...
```
Step-by-step plan in plain English: Circuit Breaker for Feature Lookup
...
Every AI-driven capability in the platform (auto-scaling policies, drift detectors, pipeline optimizers) must progress through these stages before reaching full autonomy.

| Stage | Gate Criteria | Rollback Trigger | Typical Duration |
|-------|--------------|------------------|------------------|
| **1. Offline Evaluation** | Accuracy on held-out dataset >= baseline; no regression on edge cases | N/A -- not serving traffic | 1-2 weeks |
| **2. Shadow Mode** | Shadow predictions match production decisions >= 95% agreement rate | Divergence > 10% from rule-based baseline for 1 hour | 2-4 weeks |
| **3. Canary (5% traffic)** | Error rate delta < 0.5% vs. control; latency p99 within SLO | Error rate delta > 1% or latency SLO breach for 15 min | 1-2 weeks |
| **4. Human-Reviewed Production** | ML recommendation + human approve/reject; approval rate > 90% | Rejection rate > 20% over rolling 24h window | 2-4 weeks |
| **5. Autonomous Production** | Full auto-decision with async audit; business metrics stable | Any SLO breach, model accuracy drop > 2%, or cost anomaly | Ongoing |
| **6. Instant Rollback** | One-click revert to previous stage; state preserved for forensics | Triggered manually or by automated SLO monitor | < 60 seconds |

> **ML Platform specifics:** Stage 2 (Shadow Mode) is especially critical for model promotion and auto-scaling decisions -- shadow the new policy alongside the existing one and compare outcomes before granting autonomy.
