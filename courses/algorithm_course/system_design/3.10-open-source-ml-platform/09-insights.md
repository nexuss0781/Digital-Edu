...
**Category:** Caching
**One-liner:** Replacing sequential individual Redis GET calls with pipelined MGET commands reduces feature fetch latency from N round trips to a single round trip, cutting the feature serving Slowest part of the process from the critical path.
...
## Insight 13: LLM Serving Requires Fundamentally Different Infrastructure Than Traditional ML Serving

**Category:** Scaling
**One-liner:** Traditional ML models serve in milliseconds on CPU with request-response APIs, while LLMs require multi-GPU inference with streaming responses, KV cache management, and autoscaling based on GPU memory utilization — necessitating purpose-built runtimes like vLLM within the existing KServe orchestration layer.

**Why it matters:** Adding LLM serving to an OSS ML platform is not simply deploying a larger model. LLMs require PagedAttention for memory-efficient KV cache management (2-4x throughput improvement), continuous batching to maximize GPU utilization across concurrent requests, tensor parallelism to distribute models across multiple GPUs, and streaming responses via Server-Sent Events. The KServe + vLLM integration solves these at the orchestration level — KServe handles scaling, canary, and traffic management while vLLM handles GPU-specific optimizations. Without this separation, teams end up building custom serving infrastructure that duplicates KServe's capabilities. The key architectural insight is that the serving orchestration layer (KServe) and the inference runtime (vLLM) should be independent, allowing runtime upgrades without changing deployment patterns.

---

## Insight 14: Feature Stores Are Evolving into AI Feature Platforms Supporting Vector Retrieval for RAG

**Category:** Data Structures
**One-liner:** Feast's addition of vector similarity search backends (Qdrant, Milvus, Elasticsearch) transforms the feature store from a tabular-data service into a unified AI feature platform that serves both traditional ML features and embedding-based retrieval for RAG applications.

**Why it matters:** Retrieval-Augmented Generation (RAG) applications need to retrieve relevant context (documents, user history, product descriptions) based on embedding similarity before generating responses. This retrieval step is functionally identical to feature serving — it takes an entity key (query embedding), looks up relevant data from an indexed store, and returns it at low latency. By adding vector backends to Feast, organizations can use a single feature platform for both traditional ML features (user demographics, transaction history) and RAG retrieval features (relevant documents, similar items). This unification prevents the proliferation of separate vector databases alongside feature stores, reduces operational complexity, and maintains the train-serve consistency guarantees that are the feature store's core value proposition.
