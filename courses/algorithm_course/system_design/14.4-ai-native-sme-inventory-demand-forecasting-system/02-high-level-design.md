...
The forecasting engine incorporates signals beyond historical sales data to improve accuracy during non-routine periods:

| Signal Category | Sources | Integration Method | Impact |
|---|---|---|---|
| **Calendar events** | Public holidays, religious festivals, school calendars per region | Pre-loaded calendar with demand multipliers learned from historical response | Reduces WAPE by 15-20% during holiday weeks; prevents under-ordering for festival demand |
| **Weather** | Temperature, precipitation, extreme weather forecasts per merchant location | Daily weather features appended to feature vector; learned correlations per category (e.g., ice cream sales vs. temperature) | Most impactful for food, beverages, outdoor products; 5-10% WAPE improvement for weather-sensitive categories |
| **Promotion calendar** | Merchant-registered promotions + detected channel promotions | Demand uplift multiplier applied to base forecast; cross-SKU cannibalization adjustments | Prevents stockouts during promotions; accounts for post-promotion demand dip |
| **Competitor signals** | Price changes on shared marketplace listings (where visible via API) | Competitive price differential as feature; learned elasticity per category | Limited availability (only for marketplace channels that expose competitor data); 3-5% accuracy improvement where available |
| **Economic indicators** | Regional CPI, consumer confidence index (monthly; public data) | Long-horizon trend adjustment; not useful for weekly forecasting | Marginal impact for SMEs; more useful for year-over-year growth modeling |

### Signal Quality and Fallback

```
ALGORITHM ExternalSignalIncorporation(sku_id, forecast_date):
    base_features ← get_sales_features(sku_id)
    enriched_features ← base_features.copy()

    // Each signal has a quality score and fallback
    FOR signal IN [calendar, weather, promotion, competitor]:
        data ← fetch_signal(signal, sku_id, forecast_date)
        IF data.available AND data.freshness < MAX_STALENESS[signal]:
            enriched_features.append(data.features)
        ELSE:
            // Fallback: use same-day-last-year or category average
            enriched_features.append(get_fallback(signal, sku_id, forecast_date))
            LOG WARN "Signal {signal} unavailable for {sku_id}; using fallback"
