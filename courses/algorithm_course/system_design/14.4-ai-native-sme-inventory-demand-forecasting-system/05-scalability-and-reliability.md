...
### Scaling Slowest part of the process Prediction

| Growth Milestone | Expected Slowest part of the process | Pre-emptive Action |
|---|---|---|
...
## Forecast Model Lifecycle Management

### Model Version Management

The forecasting engine maintains multiple model versions simultaneously to enable safe rollout and rollback:

```
ALGORITHM ModelVersionManagement(tenant_id, sku_id):
    // Each SKU has a "current" and "candidate" model assignment
    current_model ← get_assigned_model(tenant_id, sku_id, status=ACTIVE)
    candidate_model ← get_assigned_model(tenant_id, sku_id, status=CANDIDATE)

    IF candidate_model EXISTS:
        // Shadow evaluation: run candidate alongside current
        current_forecast ← run_model(current_model, features)
        candidate_forecast ← run_model(candidate_model, features)

        // Evaluate after 14-day observation window
        IF observation_days(candidate_model) >= 14:
            current_wape ← compute_wape(current_model, actuals, window=14)
            candidate_wape ← compute_wape(candidate_model, actuals, window=14)

            IF candidate_wape < current_wape * 0.95:  // 5% improvement threshold
                promote(candidate_model, status=ACTIVE)
                archive(current_model)
                LOG "Model promoted for {sku_id}: {candidate_model.type} (WAPE: {candidate_wape}) replaces {current_model.type} (WAPE: {current_wape})"
            ELSE:
                discard(candidate_model)
                LOG "Candidate rejected for {sku_id}: no improvement"

        RETURN current_forecast  // Always serve from current during evaluation

    RETURN run_model(current_model, features)
```

### Category-Level Model Retraining Pipeline

| Phase | Frequency | Scope | Compute Budget |
|---|---|---|---|
| **Feature store refresh** | Daily | All tenants — aggregate daily sales into feature vectors | 20 CPU-cores × 2 hours |
| **Category model retraining** | Weekly | Pooled anonymized data by product category (~500 categories) | 8 GPUs × 4 hours |
| **Per-SKU parameter update** | Daily | Individual SKU parameters updated with latest sales | 115 CPU-cores × 6 hours (within forecast batch) |
| **Model selection re-evaluation** | Monthly | All SKUs re-evaluated against full ensemble | 200 CPU-cores × 6 hours |
| **Cold-start analogous SKU refresh** | Weekly | Update similarity index for attribute-based transfer | 10 CPU-cores × 1 hour |

---

## Connection Pool and External API Management

### Channel API Rate Limit Budget Allocation

Each channel imposes rate limits that must be shared across all tenants syncing with that channel:

| Channel Type | Typical Rate Limit | Platform Allocation Strategy |
|---|---|---|
| **Major marketplace** | 5,000 calls/min (shared across all platform tenants) | Weighted allocation by tenant order volume; large tenants get proportionally more budget; micro tenants batched into fewer calls |
| **Online storefront API** | Per-tenant API key; 2-10 calls/sec | Direct allocation — each tenant uses their own rate limit |
| **Social commerce** | 100 calls/min per integration | Aggressive batching; update only changed SKUs; priority queue for low-stock items |
| **POS integration** | Local network; no external rate limit | No constraint — process immediately |

```
ALGORITHM ChannelRateBudgetAllocation(channel_type):
    total_budget ← get_channel_rate_limit(channel_type)
    tenants ← get_tenants_for_channel(channel_type)

    // Allocate proportional to recent sync volume
    total_sync_volume ← SUM(tenant.recent_sync_events for tenant IN tenants)

    FOR tenant IN tenants:
        tenant.budget ← MAX(
            total_budget * (tenant.recent_sync_events / total_sync_volume),
            MIN_BUDGET_PER_TENANT  // Minimum 1 call/sec even for micro tenants
        )

    // Reserve 20% of budget for burst handling (flash sales, reconciliation)
    burst_reserve ← total_budget * 0.20
    FOR tenant IN tenants:
        tenant.budget ← tenant.budget * 0.80

    RETURN tenant_budgets, burst_reserve
```

### Cache Warming Strategy

| Cache Layer | Warming Trigger | TTL | Invalidation |
|---|---|---|---|
| **Inventory position** | First API call per tenant per day; pre-warmed for top 100 tenants at market open | 30 seconds (short — must be near-real-time) | On every inventory mutation event |
| **Forecast results** | Forecast batch completion; pre-loaded into cache for all tenants | 24 hours | On forecast refresh or anomaly-triggered re-forecast |
| **SKU catalog** | Tenant login; channel adapter initialization | 1 hour | On catalog change event |
| **Channel connection state** | Service startup; post-failover | 5 minutes | On circuit breaker state change |
| **Reorder recommendations** | Forecast batch completion; merchant dashboard open | 24 hours | On forecast update or manual override |
| **Supplier lead time distributions** | Weekly recomputation; merchant PO workflow | 7 days | On new delivery observation |

---

## Tenant Migration and Shard Rebalancing

When a shard becomes hot (>50K tenants or >200K mutations/sec), tenants must be migrated:

```
ALGORITHM ShardRebalancing(hot_shard):
    // 1. Identify migration candidates (smallest tenants first — minimal impact)
    candidates ← hot_shard.tenants.sort_by(mutation_rate ASC)
    target_reduction ← hot_shard.mutation_rate - TARGET_MUTATIONS_PER_SHARD

    migration_set ← []
    cumulative_reduction ← 0
    FOR tenant IN candidates:
        migration_set.append(tenant)
        cumulative_reduction += tenant.mutation_rate
        IF cumulative_reduction >= target_reduction:
            BREAK

    // 2. Provision new shard or select underloaded shard
    target_shard ← find_underloaded_shard()
    IF target_shard IS NULL:
        target_shard ← provision_new_shard()

    // 3. Migrate tenants with dual-write (no downtime)
    FOR tenant IN migration_set:
        enable_dual_write(tenant, hot_shard, target_shard)
        copy_historical_data(tenant, hot_shard, target_shard)
        verify_consistency(tenant, hot_shard, target_shard)
        switch_reads(tenant, target_shard)
        disable_writes(tenant, hot_shard)
        cleanup(tenant, hot_shard)
        LOG "Migrated tenant {tenant.id} from shard {hot_shard.id} to {target_shard.id}"
```
