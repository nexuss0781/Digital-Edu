...
## Insight 9: Demand Censoring Makes Observed Sales a Biased Estimator of True Demand

**Category:** System Modeling

**One-liner:** When a SKU is out of stock for 3 of 7 days, the observed weekly sales underestimate true demand by the exact amount of demand that occurred during the stockout—and naive forecasting on this censored data creates a self-reinforcing cycle of under-ordering.

**Why it matters:** Classical forecasting assumes observed sales equal true demand. This assumption breaks whenever a stockout occurs—customers who would have purchased leave empty-handed (or buy from a competitor), and the demand that would have existed is never recorded. The observed data shows "zero sales on Tuesday" but the truth is "demand existed on Tuesday but couldn't be fulfilled." A forecast trained on this censored data predicts lower demand than reality, which leads to lower reorder quantities, which leads to more frequent stockouts, which leads to even more censored data, which leads to even lower forecasts. This is the demand censoring death spiral.

For SMEs, the problem is more severe because stockout rates are higher (5-15% of SKU-days vs. 1-2% for enterprise retail). A hero SKU that stocks out every Friday afternoon has its true Friday demand systematically underestimated. Over 3 months, the forecast may predict 20% less demand than actual, causing the system to carry 20% less safety stock, causing more stockouts, confirming the depressed forecast.

The production system addresses demand censoring through three mechanisms:

1. **Stockout detection and flagging**: Any sales period where the SKU's available-to-promise dropped to zero for more than 1 hour is flagged as "censored." These periods are excluded from the forecast's training data or, better, the unobserved demand is imputed.

2. **Demand imputation during stockouts**: For censored periods, the system estimates what demand would have been using the SKU's own demand pattern from non-stockout periods at the same day-of-week and time-of-day. If Tuesday's average non-stockout sales are 12 units but observed Tuesday sales (including stockout Tuesdays) are 8 units, the imputed demand is 12.

3. **Potential-adjusted forecasting**: The forecast model is trained on imputed demand, not observed sales. The reorder optimizer uses the imputed demand forecast, which naturally results in higher safety stock for frequently-stocked-out SKUs—breaking the death spiral.

---

## Insight 10: Supplier MOQ Creates a Quantization Effect That Distorts Optimal Order Quantities

**Category:** Cost Optimization

**One-liner:** When a supplier's minimum order quantity (MOQ) is 100 units and the optimal order quantity is 73 units, the system must round up to 100—adding 37% excess inventory that the newsvendor optimization never accounted for—and this quantization effect is most damaging for slow-moving SKUs where MOQ may exceed an entire month's demand.

**Why it matters:** The newsvendor-based (Q, R) optimization computes an "ideal" order quantity that balances holding costs, ordering costs, and stockout costs. But real suppliers impose minimum order quantities (MOQs), volume discount thresholds, and case-pack sizes that force the actual order to differ from the optimal quantity. For enterprise buyers with high-velocity SKUs, this rounding is trivial—a MOQ of 100 on a SKU selling 500/week adds only 20% excess at worst. For SME buyers with slow-moving SKUs, the effect can be catastrophic: a MOQ of 100 on a SKU selling 15/week means ordering 6.7 weeks of supply instead of the optimal 3 weeks, nearly doubling the holding cost and, for perishable items, creating significant waste risk.

The production system handles MOQ quantization as follows:

1. **Pre-optimization MOQ adjustment**: Before running the newsvendor optimizer, check if the optimal Q would fall below the MOQ. If so, re-run the optimization with Q constrained to ≥ MOQ, which changes the optimal reorder point R (since larger orders mean less frequent ordering, which changes the time between reviews).

2. **Multi-SKU MOQ consolidation**: When multiple SKUs from the same supplier each fall below MOQ individually, the system checks if consolidating them into a single PO meets the supplier's value-based MOQ (many suppliers have "minimum $500 order" rather than per-SKU MOQs). This frequently resolves MOQ violations without over-ordering any individual SKU.

3. **Shelf-life feasibility check**: For perishable items, if MOQ × unit_shelf_life < expected_sell_through_time, the system alerts the merchant that the MOQ is incompatible with the product's shelf life and suggests finding an alternative supplier with a lower MOQ or negotiating a smaller case size.

---

## Insight 11: Reconciliation Frequency Is Bounded by the Channel's Eventual Consistency Window—Not by Platform Choice

**Category:** Consistency

**One-liner:** Running reconciliation every 15 minutes against a channel whose internal consistency window is 60 seconds produces reliable drift detection, but running it against a channel that takes up to 5 minutes to propagate updates internally means 33% of reconciliation sweeps will detect "phantom drift" that resolves itself within minutes—generating false-positive correction events that actually introduce real drift.

**Why it matters:** The reconciliation engine's job is to detect when the channel's published quantity differs from the platform's expected quantity and issue corrections. But channels are not instantaneous: after the platform pushes quantity=50 to a channel, the channel's storefront may show 50 immediately or may take 30 seconds to 5 minutes to propagate the change through its own internal caches. If the reconciliation sweep reads the channel's quantity during this propagation window, it sees a stale value (e.g., 47 from before the update), concludes there's a drift of 3 units, and pushes a "correction" of 50 again. The channel receives this redundant push while still processing the first one, potentially causing a race condition that ends with the wrong quantity.

The production system solves this with a "read-wait-read" reconciliation protocol:

1. **Initial read**: Query the channel's current quantity for a SKU.
2. **Wait**: Pause for the channel's known consistency window (30 seconds for fast channels, 5 minutes for slow ones—configured per channel type based on empirical measurement).
3. **Confirmation read**: Query the channel again. If both reads agree, the value is stable and can be compared to the platform's expected quantity. If they differ, the channel is still propagating—skip this SKU in this sweep.
4. **Drift classification**: Only flag drift that persists across two consecutive reconciliation cycles (8 hours apart). Single-cycle drift is likely transient.

This approach reduces false-positive drift corrections by 85% compared to naive single-read reconciliation, at the cost of doubling the number of API calls per reconciliation sweep—a trade-off worth making when false corrections cause more harm than they prevent.

---

## Insight 12: The Merchant's Override Pattern Is a Feature Engineering Signal That the Model Is Missing

**Category:** Feedback Loop

**One-liner:** When a merchant consistently overrides the system's reorder recommendation for a specific SKU—ordering 150 units when the system recommends 100—the override is not noise but a signal that the merchant has demand knowledge (a large catering client, an upcoming local event, a seasonal pattern the system hasn't seen enough data to learn) that is absent from the model's feature set.

**Why it matters:** Most inventory platforms treat merchant overrides as "user preference" and discard the information after the PO is submitted. The override outcome (did the merchant's 150-unit order sell through, or did they end up with 50 units of excess?) is never fed back to the forecasting model. This wastes the most valuable signal in the system: a domain expert's real-time correction of the model's output.

The production system treats overrides as implicit feature engineering:

1. **Override logging with context**: Every override records the original recommendation, the merchant's chosen quantity, the reason (if provided via optional dropdown: "expected event," "large custom order," "gut feel," "supplier promotion"), and the eventual outcome (actual demand during the ordering horizon).

2. **Override accuracy scoring**: Track whether overrides were justified by comparing the merchant's quantity against actual demand. If overrides consistently beat the model (override-vs-actual WAPE < model-vs-actual WAPE), the merchant has information the model doesn't.

3. **Pattern extraction**: When a merchant consistently overrides upward for a specific SKU during specific calendar periods (e.g., every March for a seasonal product), the system detects this pattern and asks: "You've ordered more of this product in March for the past 2 years. Should we factor this seasonal boost into future forecasts?" If confirmed, the override pattern becomes a learned feature.

4. **Trust calibration**: The system maintains a per-merchant "trust score" based on historical override accuracy. Merchants with high trust scores see their overrides weighted into future forecasts automatically (as a Bayesian prior update). Merchants with low trust scores (overrides consistently worse than model) receive gentle feedback: "Last time you ordered 150, only 95 sold. The model's recommendation of 100 accounts for this pattern."

This feedback loop turns the human-in-the-loop from a liability (users overriding correct recommendations) into an asset (users providing domain knowledge that improves the model). Over 6 months, models for merchants who actively override show 8-12% WAPE improvement compared to merchants who passively accept all recommendations—because the overriding merchants' domain knowledge has been incorporated as learned features.
