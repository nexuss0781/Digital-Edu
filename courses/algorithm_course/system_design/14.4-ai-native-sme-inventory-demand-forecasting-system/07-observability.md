...
| SLO | Target | Fast Burn (5-min window) | Slow Burn (1-hour window) | Action |
|---|---|---|---|---|
| **Channel sync success rate** | 99.5% | Error rate > 5% (10x budget) | Error rate > 1% (2x budget) | Fast: page on-call + auto-scale sync workers. Slow: alert to investigate channel API degradation |
| **Inventory query API latency (p99)** | ≤ 200 ms | p99 > 1s (5x threshold) | p99 > 400 ms (2x threshold) | Fast: page on-call + shed non-critical queries. Slow: investigate database slow queries or hot tenant |
| **Webhook processing rate** | 99.9% processed | Drop rate > 1% | Drop rate > 0.2% | Fast: scale webhook receivers + investigate queue backup. Slow: check for schema changes or malformed payloads |
| **Forecast batch completion** | By 6 AM local | Projected completion > 7 AM at 50% done | Projected completion > 6:30 AM at 75% done | Fast: add compute capacity immediately. Slow: investigate slow tenants and consider preemption |

### Error Budget Tracking

```
ALGORITHM ErrorBudgetTracking(slo_name, period=30_days):
    target ← get_slo_target(slo_name)  // e.g., 99.5%
    total_events ← count_events(slo_name, period)
    failed_events ← count_failures(slo_name, period)

    error_budget_total ← total_events * (1 - target)  // Total allowed failures
    error_budget_consumed ← failed_events
    error_budget_remaining ← error_budget_total - error_budget_consumed
    burn_rate ← error_budget_consumed / (elapsed_days / period) / error_budget_total

    IF error_budget_remaining < 0:
        ALERT P1 "Error budget exhausted for {slo_name}. Freeze non-critical changes."
    ELSE IF burn_rate > 2.0:
        ALERT P2 "Error budget burning at {burn_rate}x rate for {slo_name}. Investigate."
    ELSE IF burn_rate > 1.5:
        ALERT P3 "Elevated error rate for {slo_name}. Monitor closely."

    RETURN ErrorBudgetReport(
        slo=slo_name,
        budget_total=error_budget_total,
        budget_consumed=error_budget_consumed,
        budget_remaining_pct=error_budget_remaining / error_budget_total * 100,
        burn_rate=burn_rate,
        projected_exhaustion_date=now + (error_budget_remaining / daily_burn_rate)
    )
```
...
### Forecast Compute Capacity Dashboard

| Metric | Visualization | Alert Threshold |
|---|---|---|
| **Batch window utilization** | Percentage of 6-hour window consumed by forecast run; trend over 30 days | > 80% window utilization (risk of missing deadline as tenants grow) |
| **Per-tenant compute time distribution** | Histogram of tenant forecast duration; top 10 slowest tenants highlighted | Any tenant consuming > 5% of total batch window |
| **Model evaluation throughput** | Model evaluations/sec; actual vs. required for on-time completion | Throughput < 90% of required rate at any point during batch |
| **SKU-location growth rate** | Monthly growth in total SKU-location combinations across all tenants | Projected to exceed compute capacity within 3 months |
| **Incremental forecast hit rate** | % of SKUs that skipped re-forecasting (used cached forecast) | Hit rate < 60% (too many SKUs requiring daily re-forecast; investigate data quality) |

### Channel API Health Tracking

| Metric | Per Channel | Platform-Wide |
|---|---|---|
| **API response time (p50/p95/p99)** | Trend per channel adapter; compare against channel's published SLA | Cross-channel comparison; identify degrading channels |
| **Rate limit consumption** | Current usage vs. allocated budget per channel | Total platform consumption vs. channel's global limit |
| **Error classification** | Breakdown by error type (rate limit, auth, server error, timeout) | Error type distribution shift detection |
| **Circuit breaker state history** | Timeline of CLOSED/HALF-OPEN/OPEN transitions per channel | Channels with frequent circuit breaker trips flagged for investigation |
