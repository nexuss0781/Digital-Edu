...
| "What percentage of products are perishable?" | Determines if FEFO and batch management is a first-class concern or Edge Case (Unusual or extreme situation) | Understanding that 35% perishable fundamentally changes the optimization objective |
...
### Supplier Dual-Sourcing Optimization

**Interviewer prompt**: "A merchant has two suppliers for the same SKU — one is cheap but unreliable (mean lead time 14 days, σ = 7 days), the other is expensive but reliable (mean lead time 7 days, σ = 1 day). How should the system recommend ordering?"

**Expected progression**:

| Level | Response |
|---|---|
| **Basic** | "Always use the cheapest supplier" — ignores the cost of lead time variance on safety stock |
| **Good** | "Calculate total cost including safety stock holding cost; the reliable supplier might be cheaper when you account for the extra safety stock the unreliable supplier requires" |
| **Strong** | "Split orders: base quantity from the cheap supplier (placed earlier to account for lead time), emergency replenishment from the reliable supplier when stock drops critically low. This hedges lead time risk while minimizing average cost." |
| **Exceptional** | "Model as a dual-sourcing stochastic optimization: order Q1 from cheap supplier at reorder point R1 (set high to account for variance), and order Q2 from expensive supplier at emergency reorder point R2 < R1. The optimal split depends on the lead time distributions, cost differential, and holding cost. The system should compute the expected total cost (ordering + holding + stockout) for various (Q1, R1, Q2, R2) combinations and recommend the minimum-cost policy." |

### Demand Cannibalization During Promotions

**Interviewer prompt**: "When a merchant runs 30% off on Product A, sales of substitute Product B drop 40%. How do you handle this in forecasting?"

| Level | Response |
|---|---|
| **Basic** | "Forecast each SKU independently" — misses cross-SKU effects entirely |
| **Good** | "Track which products are substitutes; when a promotion is registered for A, apply a dampening factor to B's forecast" |
| **Strong** | "Build a substitution matrix from historical promotion data: when A was promoted, measure the % decline in B, C, D. Apply these cross-elasticity coefficients to B's forecast during A's next promotion. Also model the post-promotion demand dip for A itself." |
| **Exceptional** | "The substitution matrix is asymmetric (promoting A hurts B differently than promoting B hurts A) and depends on discount depth. Build a promotion response model that takes (promoted_SKU, discount_depth, duration) as input and outputs demand multipliers for all related SKUs. Use this to adjust both reorder points and safety buffers across the substitute group during the promotion window." |

---

## Related Interview Questions

| # | Question | Key Concepts Tested | Difficulty |
|---|---|---|---|
| 1 | Design a warehouse management system with pick-pack-ship optimization | Spatial data structures; order batching; pick path optimization; wave planning | Hard |
| 2 | Design a real-time pricing engine for e-commerce | Price elasticity modeling; competitive pricing; A/B testing; margin optimization | Hard |
| 3 | Design a returns processing system for multi-channel retail | Reverse logistics; restocking workflows; refund state machines; quality inspection | Medium |
| 4 | Design a supply chain visibility platform | Track-and-trace; EDI integration; milestone events; exception detection; multi-party data sharing | Hard |
| 5 | Design a recipe-based manufacturing inventory system (BOM explosion) | Bill of materials; MRP logic; dependent demand; component availability | Medium |
| 6 | Design a loyalty program engine for retail | Points ledger; redemption flows; tiered benefits; cross-channel recognition; fraud prevention | Medium |
| 7 | Design a product catalog service for a marketplace | Category taxonomy; attribute management; variant modeling; search indexing; seller listings | Medium |
| 8 | Design a flash sale system that handles 100x traffic spikes | Queue-based fairness; inventory reservation; countdown timer consistency; overload protection | Hard |
| 9 | Design a demand forecasting system for a grocery delivery service | Cold chain constraints; substitution logic; intraday demand patterns; perishable waste optimization | Hard |
| 10 | Design a multi-warehouse order routing engine | Geographic proximity; inventory availability; shipping cost optimization; split shipment vs. single source trade-off | Medium |
