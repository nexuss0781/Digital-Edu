...
2. Query planning
3. Partition Cutting off unnecessary steps
4. Data scan
...
## Runbook: Feature Freshness SLA Breach

### Symptoms
- `feature_freshness_seconds` exceeds tier threshold (real-time: 60s, near-real-time: 900s, batch: 86400s)
- Consumer alerts: models receiving stale features

### Diagnosis Steps

```
1. Identify affected features:
   QUERY: features WHERE freshness_age > sla_threshold
   GROUP BY materialization_tier, source_pipeline

2. Check materialization pipeline status:
   IF batch tier:
     - Check batch job status (running/failed/stuck)
     - Check last successful completion time
...
3. Check source data:
   - Is source data arriving on time?
   - Has source schema changed?
   - Is source system healthy?

4. Check online store write path:
   - Is materialization completing but writes failing?
   - Is online store at capacity?
...
|------------|--------|---------------|
| Batch job failed | Restart from last checkpoint | 10-60 min |
| Streaming consumer lag | Scale up consumers, check for slow partitions | 5-15 min |
| Source data delayed | Notify source team, extend SLA temporarily | Depends on source |
| Online store write failures | Scale store, fix write path | 5-30 min |
| Schema change | Update transformation, rerun materialization | 30-120 min |
...
feature_store_slos:
  online_latency:
    target_p99: 10  # 10ms
    error_budget_monthly: 0.001  # 0.1% can exceed

    alerts:
      - name: "online_latency_fast_burn"
        windows: [1h, 5m]
        burn_rate: 14.4
        severity: "page"
        description: "Online serving latency spiking — check Redis cluster health"

      - name: "online_latency_slow_burn"
        windows: [6h, 30m]
        burn_rate: 6.0
        severity: "page"

  feature_freshness:
    target: 0.99  # 99% of features within SLA
    error_budget_monthly: 0.01

    alerts:
      - name: "freshness_fast_burn"
        windows: [1h, 5m]
        burn_rate: 14.4
        severity: "page"
        description: "Multiple features breaching freshness SLA"

      - name: "freshness_chronic"
        windows: [3d, 6h]
        burn_rate: 1.0
        severity: "ticket"
        description: "Slow freshness degradation — investigate pipeline health"

  data_quality:
    target: 0.999  # 99.9% of feature values pass validation
    error_budget_monthly: 0.001

    alerts:
      - name: "quality_fast_burn"
        windows: [1h, 5m]
        burn_rate: 14.4
        severity: "page"
        description: "Feature quality dropping — potential data corruption or source issue"
```
