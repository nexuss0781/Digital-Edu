...
**Threat:** An adversary manipulates input data to corrupt feature values, causing models to produce incorrect predictions systematically.

```
Attack vectors:
─────────────────────────────────────────────────────────

1. Source Data Manipulation:
   - Attacker compromises upstream data source
   - Corrupted values flow through materialization to online store
   - Detection: Statistical anomaly detection on feature distributions
   - Mitigation: Input validation, range checks, distribution monitoring

2. Materialization Pipeline Injection:
   - Attacker modifies transformation logic
   - Features computed incorrectly for all entities
   - Detection: Reconciliation between independently computed values
   - Mitigation: Code review, immutable pipeline definitions, signed artifacts

3. Online Store Direct Write:
   - Attacker bypasses pipeline, writes directly to online store
   - Single entity or small group affected
   - Detection: Write audit logs, unexpected write patterns
   - Mitigation: Write-only via materialization service (no direct access)

Defense in depth:
  Layer 1: Input validation (range, type, null checks)
  Layer 2: Distribution monitoring (detect drift from baseline)
  Layer 3: Cross-validation (compare batch and streaming values)
  Layer 4: Model output monitoring (detect prediction anomalies)
```

### Feature Privacy and Data Minimization
...
1. Feature Classification:
   - PUBLIC: non-sensitive (item_category, timestamp)
   - INTERNAL: business-sensitive (revenue, conversion_rate)
   - PII: personally identifiable (user_age, location)
   - RESTRICTED: highly sensitive (income, health_data)

2. Access Controls by Classification:
   - PUBLIC: any authenticated service
   - INTERNAL: team-level RBAC
   - PII: explicit approval + audit logging
   - RESTRICTED: privacy review + time-limited access

3. Data Minimization:
   - Feature registry tracks which models use which features
   - Unused features (no consumer for 90 days) → deprecation notice
   - PII features require justification renewal every 6 months
   - Aggregated features preferred over raw PII
     (age_bucket instead of birth_date,
