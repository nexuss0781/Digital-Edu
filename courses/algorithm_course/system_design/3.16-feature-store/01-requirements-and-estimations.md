...
| Scenario | Probability | Impact | Recovery Cost | Mitigation Investment |
|----------|-------------|--------|---------------|----------------------|
| Online store full outage (1h) | Low (1x/year) | All real-time models degrade to defaults | $50K lost revenue from degraded recs | $30K multi-region replication |
| PIT join bug (leaking future data) | Medium (2x/year) | Models trained on leaked data show inflated metrics, fail in prod | $200K wasted training + retraining all affected models | $20K PIT validation suite |
| Materialization pipeline stuck (4h) | Medium (3x/year) | Features stale by 4+ hours for streaming tier | $25K degraded model quality | $15K pipeline monitoring + auto-recovery |
| Online store corruption (bad write) | Low (0.5x/year) | Serving incorrect features → wrong predictions | $100K customer-facing errors | $10K write validation + rollback |
| Schema change breaks consumers | Medium (4x/year) | 1-3 models fail at serving time until hotfix | $75K per incident (SLA breach) | $5K schema compatibility checks |
