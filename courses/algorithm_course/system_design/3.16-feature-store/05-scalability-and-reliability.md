...
3. COLUMNAR OPTIMIZATION
   ...
   Benefits:
   - Column Cutting off unnecessary steps (read only needed features)
   - Predicate pushdown
...
| Test | Method | Expected Behavior | Recovery Target |
|------|--------|-------------------|-----------------|
| **Online store node failure** | Kill 1 Redis cluster node | Cluster rebalances, reads continue | < 30s |
| **Materialization pipeline failure** | Kill batch job mid-run | Checkpoint-based recovery, no duplicate writes | < 5 min |
| **Streaming pipeline lag spike** | Inject 10x traffic to Kafka | Autoscale consumers, bounded lag growth | < 10 min |
| **Offline store partition corruption** | Delete one date partition | Recompute from source data, serve from previous partition | < 2h |
| **Feature registry unavailable** | Block PostgreSQL access | Cached feature definitions serve, no new feature creation | Immediate |
| **Network partition (online ↔ offline)** | Block cross-service traffic | Online serves stale features, offline continues independently | Immediate |
...
Topology: Primary (Region A) → Replica (Region B) → Replica (Region C)

Write path:
  1. Materialization writes to Primary
  2. Async replication to Region B (< 100ms lag)
  3. Async replication to Region C (< 500ms lag)

Read path:
  - Each region reads from local replica (lowest latency)
  - If local replica stale > staleness_threshold:
      Fallback to primary region read (higher latency, fresh data)

Conflict handling:
  - Last-writer-wins (materialization is idempotent)
  - Version vector per entity key prevents lost updates
  - Full reconciliation during nightly materialization

Failure handling:
  - Region A down: Region B promotes to primary, Region C follows
  - Network partition: Each region serves independently (eventual consistency)
  - Split brain: Resolved on reconnection via materialization epoch comparison
```
