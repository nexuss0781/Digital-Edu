...
- Partition Cutting off unnecessary steps by time range
...
## Algorithm: On-Demand vs Materialized Feature Routing

**Purpose:** Automatically route feature retrieval requests to either the online store (pre-materialized) or the compute layer (on-demand) based on feature access patterns and SLA requirements.

```
FUNCTION get_features(entity_key, feature_list, latency_budget_ms):
    materialized_features = []
    on_demand_features = []
    results = {}

    // Step 1: Classify each feature by serving strategy
    FOR EACH feature IN feature_list:
        metadata = registry.get_metadata(feature)

        IF metadata.serving_strategy == "materialized":
            materialized_features.append(feature)
        ELSE IF metadata.serving_strategy == "on_demand":
            on_demand_features.append(feature)
        ELSE IF metadata.serving_strategy == "auto":
            // Auto-routing based on access pattern
            stats = access_tracker.get_stats(feature)
            IF stats.qps > QPS_THRESHOLD OR latency_budget_ms < 20:
                // High traffic or tight latency → must be materialized
                IF online_store.has(entity_key, feature):
                    materialized_features.append(feature)
                ELSE:
                    on_demand_features.append(feature)
            ELSE:
    ...
    // Step 2: Fetch materialized features (batch GET, fast path)
    IF materialized_features.length > 0:
        batch_result = online_store.batch_get(entity_key, materialized_features)
        results.merge(batch_result)

        // Track any missing materialized features for fallback
        missing = materialized_features.filter(f → f NOT IN batch_result)
        on_demand_features.extend(missing)  // Fallback to compute

    // Step 3: Compute on-demand features (parallel, slower path)
    IF on_demand_features.length > 0:
        remaining_budget = latency_budget_ms - elapsed_ms()

        // Compute in parallel with remaining latency budget
        futures = []
        FOR EACH feature IN on_demand_features:
            transform = registry.get_transform(feature)
            source_data = source_connector.fetch(transform.source, entity_key)
            futures.append(async compute(transform, source_data))

        // Wait with timeout
        computed = await_all(futures, timeout=remaining_budget)
        results.merge(computed)

        // Track access pattern for auto-promotion
        FOR EACH feature IN on_demand_features:
            access_tracker.record_access(feature)
            IF access_tracker.should_promote(feature):
                // Trigger async materialization for this feature
                materialization_queue.enqueue(feature, priority="high")
                EMIT metric("feature_promoted_to_materialized", feature)

    // Step 4: Fill defaults for any still-missing features
    FOR EACH feature IN feature_list:
        IF feature NOT IN results:
            results[feature] = registry.get_default(feature)
            EMIT metric("feature_default_used", feature)
...
**Promotion Criteria:**
```
FUNCTION should_promote(feature):
    stats = get_30_day_stats(feature)
    RETURN (
        stats.avg_qps > 10 AND             // Accessed frequently enough
        stats.p50_compute_latency > 50ms AND // Computation is expensive
        stats.unique_entities > 10000        // Broad access, not just a few entities
    )
```
