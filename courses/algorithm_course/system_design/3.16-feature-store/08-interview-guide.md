...
### Trap 5: "Why not just use a data warehouse for everything?"

**What they want:** Understanding of the fundamental access pattern mismatch.

**Best answer:**
"Data warehouses excel at analytical queries — scan millions of rows, aggregate,
join. But online feature serving needs the exact opposite: point lookups by entity
key returning 50-500 feature values in under 10ms. Running a SQL query per inference
request would add 200-500ms of latency, which is unacceptable for real-time ML.

The feature store dual-store pattern uses the warehouse (or data lake) as the offline
store for training data generation, and a key-value store as the online store for
serving. Materialization bridges the two, ensuring training and serving see the
same feature values."

### Trap 6: "How do you handle schema evolution?"

**What they want:** You understand the compatibility challenge with multiple consumers.

**Best answer:**
"Schema evolution is one of the hardest operational challenges because a single
feature might be consumed by 20+ models. The approach:
1. **Additive changes are safe**: Adding a new column to a feature group never breaks existing consumers
2. **Type widening is safe**: int32 → int64, float32 → float64 — consumers handle this
3. **Breaking changes require migration**: Renaming, removing, or narrowing types
4. **Versioned features**: Breaking changes create a new version (user_features_v2)
5. **Deprecation window**: Old version continues serving for 90 days
6. **Impact analysis**: Before any change, the lineage graph shows all affected models
7. **Compatibility registry**: Like a schema registry in event streaming

The key insight is that the feature store must enforce compatibility at write time,
not discover incompatibility at read time."
...
| # | Topic | Connection |
|---|-------|------------|
| 1 | [3.34 Personalization Engine](../3.34-ai-native-real-time-personalization-engine/00-index.md) | Feature store as backbone for real-time personalization signals |
| 2 | [3.7 Netflix Runway](../3.7-netflix-runway-model-lifecycle/00-index.md) | Feature consumption across model lifecycle stages |
| 3 | [3.5 Uber Michelangelo](../3.5-uber-michelangelo-ml-platform/00-index.md) | Palette — production feature store reference architecture |
| 4 | [3.4 MLOps Platform](../3.4-mlops-platform/00-index.md) | Feature store as core MLOps infrastructure |
| 5 | [3.33 AI Customer Service](../3.33-ai-native-customer-service-platform/00-index.md) | Customer features feeding intent routing |
| 6 | [3.14 Vector Database](../3.14-vector-database/00-index.md) | Embedding features as vectors |
| 7 | [1.18 Event Sourcing](../1.18-event-sourcing-system/00-index.md) | Event streams as feature data sources |
| 8 | [3.15 RAG System](../3.15-rag-system/00-index.md) | Features enriching retrieval context |
