...
## Insight 9: Late-Arriving Events Create a Retroactive Consistency Problem That Grows with Retention Window

**Category:** Consistency

**One-liner:** Every late-arriving event invalidates pre-computed aggregations across all materialized views that touch its time range — and longer retention windows mean more materialized state to reconcile.

**Why it matters:**
Mobile SDKs are the primary source of late-arriving events: a user goes underground on a subway, plays a game for 30 minutes, and re-emerges. The SDK flushes 30 minutes of events with timestamps 30 minutes in the past. For the analytics platform, these events arrive "on time" from the network perspective but are "late" from the event-time perspective — they belong to time windows that may have already been compacted into materialized views.

The challenge compounds with multiple layers of pre-aggregation. A late event at time T affects: the 1-minute rollup for T, the 1-hour rollup for T's hour, the daily rollup for T's date, any funnel that spans T's date range, any retention matrix whose cohort period includes T, and any cached query result whose date range includes T. Each of these materialized views must be either invalidated or incrementally updated.

The naive approach—invalidate all affected materialized views on every late event—creates an invalidation cascade that can overwhelm the system during periods of high late-event volume (e.g., a mobile app release to a region with poor connectivity). The production approach uses a staged invalidation model: hot rollups are updated immediately via idempotent merge; warm materialized views are marked "stale" and recomputed within 1 hour; cold aggregations are batch-recomputed nightly. Query results from stale materialized views carry a freshness flag rather than blocking the query. This makes late events a staleness problem rather than an availability problem.

---

## Insight 10: Event Taxonomy Governance Is a Social Engineering Problem Disguised as a Technical Problem

**Category:** System Modeling

**One-liner:** The technical infrastructure for event schema governance is trivial; the adoption challenge — convincing 500 product engineers to follow naming conventions — is the real Slowest part of the process.

**Why it matters:**
Every analytics platform that reaches 10,000+ event types encounters the same problem: a third of events have ambiguous names, property naming is inconsistent across teams, and self-serve analysis becomes impossible because analysts cannot find or trust the right events. The technical solution — a schema registry with validation rules and quality scores — is straightforward to build. What is not straightforward is getting adoption.

The insight is that governance must be designed as a positive-sum game, not a compliance burden. Teams that register event schemas should get benefits that non-registering teams don't: auto-generated documentation, priority support for event-related bugs, inclusion in cross-team analysis reports, and prominently displayed quality scores visible to leadership. Teams with poor governance scores should feel social pressure, not system enforcement — their events are still ingested, but their data quality dashboard shows a declining score that their VP can see.

The most effective governance mechanism observed in production is the "event owner" model: every event type has a designated owner (individual or team), and ownership is enforced socially, not technically. When an analyst has a question about an event's semantics, they contact the owner. When a schema violation appears, the owner is notified. When an event type's quality score drops, the owner's manager is CC'd. This social accountability layer is more effective than technical blocking because it aligns incentives: owners care about their events being useful because they're publicly responsible for them.

---

## Insight 11: The Query Router Is the Most Underappreciated Component — It Determines 80% of Query Latency

**Category:** Cost Optimization

**One-liner:** A query router that correctly selects the optimal execution path (cache → rollup → scan) reduces average query cost by 10× compared to always-scan.

**Why it matters:**
In a three-tier storage architecture with multiple pre-aggregation layers, the query router makes the single most impactful decision for query performance: which execution path should this query take? The options span five orders of magnitude in cost:
- L1 result cache hit: ~1ms, 0 compute
- Pre-computed rollup lookup: ~10ms, minimal compute
- Warm columnar scan with predicate pushdown: ~200ms, moderate compute
- Cold object storage scan: ~2000ms, high compute
- Cross-tier merge query: ~5000ms, highest compute

The naive approach treats the router as a simple cache-check-then-scan dispatcher. The production router performs query shape analysis: it parses the query to determine if its structure matches a materialized view (even if parameters differ), checks data freshness requirements against materialized view staleness, estimates scan cost based on partition statistics, and routes sub-queries to different tiers when the date range spans multiple storage tiers.

The router's decision quality directly determines the platform's cost structure. At 347 queries/second with a 30% cache hit rate, each 1% improvement in cache or rollup routing eliminates ~3.5 cold scans per second. Over a month, this translates to millions of avoided scans, each consuming 2 seconds of multi-core query executor time. The router is the leverage point where a small algorithmic improvement produces disproportionate cost and latency benefits.

---

## Insight 12: Multi-Tenant Query Isolation Cannot Be Achieved by Fair Scheduling Alone — It Requires Workload Classification

**Category:** Contention

**One-liner:** Fair-scheduling query resources across tenants fails because "fair" assumes all queries are equivalent, but a 30-second full-scan query from one tenant consumes 100× the resources of a 300ms cached query from another.

**Why it matters:**
The standard multi-tenant resource isolation approach is fair-scheduling: each tenant gets an equal share of query executor capacity. But analytics queries vary by four orders of magnitude in cost — a dashboard refresh that hits the result cache costs effectively nothing, while an ad hoc retention query over 12 months of data for a large project consumes a full query worker for 10+ seconds. Fair scheduling by tenant means a tenant running one expensive query appears to use the same quota as a tenant running 100 cheap cached queries, even though the compute consumption differs by 1000×.

The production approach classifies queries into cost tiers before execution and applies separate quotas per tier:
- **Tier 0 (free):** Cache hits and rollup lookups — no quota deduction
- **Tier 1 (cheap):** Hot store scans over < 1 day of data — 1 unit per query
- **Tier 2 (moderate):** Warm store scans over < 30 days — 5 units per query
- **Tier 3 (expensive):** Cold store scans over > 30 days — 25 units per query
- **Tier 4 (heavy):** Cross-tier merge queries with multiple breakdown dimensions — 50 units per query

Each tenant gets a quota budget per hour (e.g., 100 units for standard plan, 500 for enterprise). This means a standard-plan tenant can run 100 cheap queries or 2 expensive queries per hour — reflecting the actual resource consumption rather than the query count. The cost tier is estimated before execution using partition statistics and query shape analysis, avoiding the need to run the query first to determine its cost.
