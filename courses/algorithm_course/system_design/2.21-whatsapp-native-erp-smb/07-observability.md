...
## Conversation Economics Dashboard

A unique observability dimension for WhatsApp-native systems: tracking the cost efficiency of conversational interactions.
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  MESSAGE COST TODAY                  OPTIMIZATION METRICS        │
│  ──────────────────                  ───────────────────         │
│  Total Messages:     345,230         Consolidation Rate:  62%    │
│  Billable Messages:  138,092         Session Utilization: 78%    │
│  Cost: ₹4,14,276                     Flows vs Text:      45%    │
│  Cost/Tenant: ₹4.21                  Template Category:          │
│                                        Utility:    67%           │
│  COST TREND (7 days)                   Marketing:  22%           │
│  ──────────────────                    Auth:       11%           │
│  ₹ 4.50 ██████████                                              │
│  ₹ 4.21 ████████ ← Today                                       │
│  ₹ 3.80 ██████ (Target)            SAVINGS THIS MONTH           │
│                                     ──────────────────           │
│  PER-MODULE COST                    vs Unoptimized: ₹6.6 Cr     │
│  ──────────────                     Actual: ₹3.9 Cr             │
│  Orders:     42%                    Saved: ₹2.7 Cr (41%)        │
│  Invoicing:  28%                                                 │
│  Alerts:     15%                                                 │
│  Reports:     8%                                                 │
│  Other:       7%                                                 │
...
```yaml
  - name: CostPerTenantHigh
    severity: P3
    condition: >
      avg_over_time(message_cost_per_tenant[24h]) > 5.0
    for: 1d
    message: "Average cost per tenant exceeds ₹5/day"

  - name: ConsolidationRateLow
    severity: P2
    condition: >
      message_consolidation_rate < 0.5
    for: 6h
    message: "Message consolidation rate below 50% - wasting message budget"

  - name: SessionUtilizationLow
    severity: P3
    condition: >
      session_utilization_rate < 0.6
    for: 1d
    message: "WhatsApp session windows underutilized - schedule alerts during active sessions"
```
