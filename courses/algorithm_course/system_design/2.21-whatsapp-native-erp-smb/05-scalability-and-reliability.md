...
### Traffic Spike Handling (Step-by-step plan in plain English)
...
The system maintains multiple communication channels with automatic failover:

```
CHANNEL FAILOVER HIERARCHY
===========================

Primary:   WhatsApp Cloud API (via BSP)
           ↓ Circuit breaker trips (>5% failure rate)
Secondary: WhatsApp Cloud API (via backup BSP)
           ↓ Both BSPs unavailable
Tertiary:  SMS Gateway (P0/P1 messages only)
           ↓ SMS gateway failure
Quaternary: Companion App Push Notification
           ↓ App not installed / offline
Final:     Offline queue (sync on any channel recovery)

Each channel transition:
1. Logs the transition event for audit
2. Adjusts message format (SMS = text only, no PDFs)
3. Notifies the business about channel degradation
4. Automatically reverts when primary recovers
```

### ONDC Order Handling Under Load

```
ONDC ORDER SPIKE PATTERN
=========================

ONDC marketplace orders arrive independently of WhatsApp traffic.
During ONDC sales events, order volume can spike 20x.

Protection:
1. Dedicated ONDC webhook workers (separate from WhatsApp pool)
2. ONDC orders → internal queue → same order service
3. Rate limit ONDC acceptance based on fulfillment capacity
4. Auto-reject if inventory cannot fulfill within SLA

FUNCTION handle_ondc_order(ondc_request):
    # Check capacity before accepting
    capacity = get_fulfillment_capacity(ondc_request.seller_id)

    IF capacity.pending_orders > capacity.max_daily * 0.9:
        RETURN ondc_reject(reason="capacity_exceeded")

    IF NOT check_inventory(ondc_request.items):
        RETURN ondc_reject(reason="out_of_stock")

    # Accept and process
    order = create_order(source="ondc", payload=ondc_request)
    notify_business_via_whatsapp(order)

    RETURN ondc_accept(order_id=order.order_id)
```
...
Free Tier (80% of tenants):
─────────────────────────
- Shared webhook workers
- Standard priority queue
- 1000 messages/month included
- Basic NLU (template matching)
- No OCR

Basic Tier (15% of tenants):
───────────────────────────
- Shared webhook workers (higher priority)
- Priority queue: P0-P2 access
- 10,000 messages/month
- Edge NLU + basic CVM
- OCR: 100 receipts/month

Premium Tier (5% of tenants):
────────────────────────────
- Dedicated webhook workers (if volume justifies)
- Full priority queue access
- Unlimited messages
- Full AI stack (Edge + CVM)
- Unlimited OCR
- Dedicated support channel

Resource isolation:
- Premium tenants get dedicated Redis namespace
- Large premium tenants can be migrated to dedicated DB schema
- Rate limiting applies per-tier, not per-tenant
```
