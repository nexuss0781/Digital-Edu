...
| **WhatsApp Channels** | Broadcast API | One-to-many updates (late 2025) |
...
This system draws from and contributes to several architectural patterns seen across the StructWeave system design collection:

| Pattern | Source System | Application Here |
|---------|-------------|-----------------|
| **Priority Queue with Backpressure** | [Distributed Message Queue](../1.6-distributed-message-queue/00-index.md) | P0-P3 message prioritization under WhatsApp rate limits |
| **Token Bucket Rate Limiting** | [Distributed Rate Limiter](../1.1-distributed-rate-limiter/00-index.md) | Per-phone-number outbound message throttling |
| **Offline-First Sync with CRDTs** | [AI-Native Offline-First POS](../2.22-ai-native-offline-first-pos/00-index.md) | Companion app conflict resolution strategies |
| **Multi-Tenant RLS Isolation** | [AI Native Cloud ERP SaaS](../2.18-ai-native-cloud-erp-saas/00-index.md) | PostgreSQL RLS for 100K+ tenant isolation |
| **Circuit Breaker for External APIs** | [API Gateway Design](../1.14-api-gateway-design/00-index.md) | BSP failover and SMS fallback triggering |
| **Privacy-Preserving AI** | [LLM Gateway](../3.21-llm-gateway-prompt-management/00-index.md) | Meta CVM integration for ephemeral processing |
| **Event Sourcing for Audit** | [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) | Chain-hashed immutable audit logs for DPDP compliance |

---

## BSP Abstraction Layer

A critical architectural decision is abstracting BSP-specific APIs behind a unified gateway, enabling multi-BSP failover and cost optimization:

```
BSP ABSTRACTION ARCHITECTURE
=============================
...
│                                                       │
│  ┌─────────────┐   Canonical     ┌──────────────┐   │
│  │ Webhook      │   Message       │ Outbound     │   │
│  │ Normalizer   │   Format        │ Translator   │   │
│  │              │   ──────►       │              │   │
│  │ BSP-A format │                 │ BSP-A format │   │
│  │ BSP-B format │                 │ BSP-B format │   │
│  │ Direct Meta  │                 │ Direct Meta  │   │
│  └─────────────┘                  └──────────────┘   │
│                                                       │
│  ┌─────────────────────────────────────────────────┐ │
│  │ Router: Cost-optimize (P3 → cheapest BSP)        │ │
│  │         Reliability (P0 → most reliable BSP)     │ │
│  │         Failover (circuit breaker per BSP)       │ │
│  └─────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘
```

This abstraction enables:
- **Multi-BSP failover**: If BSP-A circuit breaker trips, route to BSP-B automatically
- **Cost optimization**: Route P3 marketing messages through the cheapest BSP
- **Migration path**: Add direct Meta API as another "BSP" behind the same interface
- **A/B testing**: Compare BSP delivery rates by splitting traffic
