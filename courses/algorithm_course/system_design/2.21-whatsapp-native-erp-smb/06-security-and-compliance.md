...
### DPDP Act Compliance Updates (2025-2026)

The Digital Personal Data Protection Act rules continue to be finalized. Key evolving requirements:

| Requirement | Status (2026) | Implementation |
|-------------|---------------|----------------|
| **Data Protection Board** | Operational | 72-hour breach notification pipeline |
| **Consent manager registration** | Mandatory | Platform registered as consent manager |
| **Significant data fiduciary** | Threshold TBD | Prepare for enhanced obligations if classified |
| **Cross-border transfer** | Restricted list | Block data transfer to non-approved geographies |
| **Children's data** | Strict prohibition | Age verification gate for business user onboarding |
| **Data Protection Impact Assessment** | Required for high-risk processing | DPIA for AI/NLU processing, OCR extraction |
...
Consent Lifecycle:
1. Collection: WhatsApp Flow with granular purpose selection
2. Storage: Versioned consent records (consent_v2, consent_v3)
3. Renewal: Re-consent when purposes change or annually
4. Withdrawal: "withdraw consent" command → 30-day wind-down
5. Audit: Complete consent history for regulatory inspection

Data Processing Register (DPDP Section 8):
- Maintain register of all processing activities
- Map data flows: collection → storage → processing → deletion
- Auto-generate register entries from audit log metadata
- Export-ready for Data Protection Board inspection
```
