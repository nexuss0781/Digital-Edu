...
| Module | Capability | Notes |
|--------|------------|-------|
| **WhatsApp Channels** | Broadcast product updates, offers | One-to-many without template approval (late 2025) |
| **AI Demand Forecasting** | Predict reorder timing from sales patterns | Meta CVM processes historical data ephemerally |
| **Multi-Business Dashboard** | Franchise/chain management view | Aggregated reporting across linked businesses |
| **ONDC Seller Rating** | Auto-collect and respond to buyer reviews | Reputation management via WhatsApp |
| **UPI Autopay** | Recurring payment collection | Subscription billing for B2B customers (UPI 3.0) |
...
- WhatsApp group-based operations (privacy and complexity concerns)
...
### WhatsApp Platform Constraints (2025-2026)

| Constraint | Value | Impact on Design |
|------------|-------|------------------|
| **Message rate limit** | 80 msg/sec/phone number | Requires priority queue + aggregation |
| **Conversation pricing** | Per-message model (July 2025) | Incentivizes batched/aggregated responses |
| **Template approval** | 24-72 hour review cycle | Pre-approve templates for all ERP operations |
| **Flows screen limit** | 10 screens per flow | Order wizard must fit within limit |
| **Media size limit** | 100 MB per message | Invoice PDFs, receipt images within budget |
| **Business-initiated window** | 24-hour session from last user message | Proactive alerts require template messages |
| **Catalog item limit** | 500 items per catalog | Pagination strategy for large inventories |
