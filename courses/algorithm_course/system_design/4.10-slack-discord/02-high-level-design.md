...
| **Cons** | Rebalancing on scale changes | Guild process can become Slowest part of the process |
...
### Decision 6: Presence Architecture

| Option | Pros | Cons |
|--------|------|------|
| **Dedicated presence servers (Slack)** | Isolated scaling, hash-based user affinity | Extra hop for status queries |
| **Co-located with session (Discord)** | Natural co-location, no extra service | Scales with connection tier |
| **External service (Redis pub/sub)** | Simple, off-the-shelf | Higher latency, no batching control |

**Trade-off**: Both platforms chose to co-locate presence logic near the connection layer — Slack hashes users to dedicated Presence Servers, Discord embeds status in the Session GenServer. The key insight is that presence data has unique characteristics: high write frequency, tolerance for staleness, and strong locality (most presence consumers share channels with the status-changing user). Externalizing to a generic pub/sub system loses the ability to batch, debounce, and selectively subscribe — all critical for handling the 9 AM login storm where millions of users transition to "online" within minutes.

---

### Architecture Decision Records

#### ADR-1: HTTP POST for Sends, WebSocket for Receives

**Context**: Should clients use the WebSocket for both sending and receiving messages, or use HTTP POST for sends?

**Decision**: Slack uses HTTP POST for message sends, WebSocket only for receiving events.

**Rationale**:
- HTTP POST provides clear request/response semantics — the client knows whether the send succeeded
- On mobile, HTTP works with background fetch APIs; WebSocket requires a persistent connection
- Crash safety: even if the WebSocket drops mid-send, the HTTP request is independently tracked
- Load balancing: HTTP can be round-robin across stateless webapp servers; WebSocket sends would be pinned to one gateway

**Consequences**: Slightly higher send latency (~50ms overhead) compared to WebSocket-only send, but significantly improved reliability and debuggability. Discord uses WebSocket for sends in some cases for lower latency, accepting the trade-off in failure semantics.
