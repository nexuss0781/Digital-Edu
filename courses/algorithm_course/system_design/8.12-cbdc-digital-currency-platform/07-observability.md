...
## Monetary Policy Observability

CBDC provides central banks with unprecedented real-time visibility into monetary dynamics---a capability that did not exist with physical cash.

### Real-Time Monetary Indicators

```
Velocity of Money (V = PQ/M):
  - Traditional: Estimated quarterly from GDP and M2 data (weeks delayed)
  - CBDC: Computed in real-time from transaction stream (seconds delayed)
  - Dashboard: Rolling 1h/24h/7d/30d velocity metrics per region

Geographic Flow Analysis:
  - Net CBDC inflow/outflow per state/province (hourly)
  - Cross-border corridor volume and direction (real-time)
  - Rural vs urban adoption and usage patterns

Programmable Token Effectiveness:
  - Stimulus utilization rate: % spent before expiry (target > 80%)
  - Subsidy leakage detection: spending outside intended categories
  - Geographic distribution of government disbursement spending
  - Time-to-first-spend after disbursement credit

Holding Pattern Analysis:
  - Distribution of CBDC balances by tier (Gini coefficient)
  - Waterfall mechanism activation frequency (early warning for bank stress)
  - Conversion rate: bank deposits → CBDC (trending up = potential instability)
  - Average holding duration before spend or redemption
```
...
|---------|-----------------|----------|
| Sudden velocity spike | Z-score > 3 on hourly velocity | Alert monetary policy team; potential speculation or panic |
| Regional drain | Net outflow > 20% of region's CBDC stock in 24h | Alert; possible regional crisis or natural disaster |
| Conversion acceleration | Deposit-to-CBDC rate > 5x baseline | Trigger stress-mode rate limits; alert financial stability team |
| Stimulus hoarding | < 30% utilization at 50% of expiry window | Policy review; consider extending or widening categories |
| Offline anomaly cluster | > 10 double-spend attempts from same geographic area | Investigate SE compromise or fraud ring; possible device cloning |
