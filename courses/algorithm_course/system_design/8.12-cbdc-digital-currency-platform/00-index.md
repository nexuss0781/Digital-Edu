...
| **Financial Integrity** | Absolute---total tokens in circulation must equal central bank's minted supply minus redeemed tokens at all times; supply Rule that never changes is verified every settlement cycle |
...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, API design, token lifecycle algorithms (Step-by-step plan in plain English) |
...
1. **Central Bank as Sole Issuer (Monetary Sovereignty)**: Unlike every other digital payment system where private entities create digital representations of money, a CBDC is the money itself---a direct liability of the central bank. The system must guarantee that the total supply of tokens exactly matches the central bank's issuance ledger at all times. This supply Rule that never changes is the most critical system property: any discrepancy (phantom tokens or missing tokens) constitutes a monetary policy failure. The system requires cryptographic supply verification at every settlement cycle, with the central bank's balance sheet serving as the ultimate source of truth.
...
| [8.5 - Fraud Detection System](../8.5-fraud-detection-system/) | AML pattern detection, risk scoring, SAR generation, real-time fraud prevention |
| [8.11 - UPI Real-Time Payments](../8.11-upi-real-time-payment-system/) | Real-time payment rail design, PSP integration, QR/intent flows---CBDC must interoperate with UPI |
| [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Credit overlay on payment rails, programmable repayment schedules---analogous to CBDC programmable conditions |

## Related Patterns

| Pattern | Application in CBDC |
|---------|-------------------|
| **Event Sourcing** | Core ledger records every token state transition as immutable events; enables full audit replay from mint to destruction |
| **CQRS** | Write path (transfers, minting) separated from read path (balance queries, analytics); intermediary read replicas absorb query load |
| **Saga Pattern** | Cross-border PvP settlement uses choreography-based sagas with compensating transactions for rollback |
| **Circuit Breaker** | Intermediary-to-central-bank calls use circuit breakers; prevents cascade failure during core ledger maintenance |
| **Two-Phase Commit** | Cross-shard transactions within the core ledger coordinate debit and credit atomically |
| **Bulkhead Isolation** | Each intermediary operates an independent sub-ledger; failure in one does not affect others |
| **Merkle Tree Verification** | Supply integrity verified via hierarchical hash proofs without exposing individual wallet data |
| **Hardware Root of Trust** | Secure Element-based offline payments anchor trust in tamper-resistant hardware, not protocol |
