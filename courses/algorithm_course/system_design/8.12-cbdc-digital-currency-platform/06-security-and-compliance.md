...
Quantum Computing Threat:
  Timeline: NIST estimates quantum computers capable of breaking
    ECDSA P-384 by 2035-2040; CBDC systems must plan migration now
  Mitigation:
    - Crypto-agility: all signature algorithms configurable without
      protocol-breaking changes
    - Hybrid signatures: dual-sign tokens with current ECDSA + post-quantum
      (CRYSTALS-Dilithium or SPHINCS+) during transition period
    - Key encapsulation: migrate HSM key exchange to ML-KEM (NIST PQC standard)
    - Timeline: begin testing post-quantum signatures by 2027; mandatory
      by 2030 for all new token batches
```

### Insider Threat Model

```
Threat: Rogue central bank employee attempts unauthorized minting
  Mitigation layers (defense in depth):
    1. M-of-N HSM ceremony (no single person can mint)
    2. Real-time supply reconciliation (mismatch detected in 15 min)
    3. External auditor holds one key shard (collusion requires external party)
    4. Immutable audit trail (HSM-signed, tamper-evident)
    5. Segregation of duties (minting team ≠ reconciliation team)
    6. Anomaly detection: mint operations outside scheduled windows
       trigger P0 alert to governor's office

Threat: Intermediary employee manipulates sub-ledger balances
  Mitigation layers:
    1. Merkle tree proofs (central bank detects tampering in 15 min)
    2. Hash-chain transaction logs (retroactive changes break chain)
    3. Multi-party authorization for manual balance adjustments
    4. Automated reconciliation flags any discrepancy > 0
```
