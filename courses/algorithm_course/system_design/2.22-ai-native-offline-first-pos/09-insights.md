...
## Insight 9: INT8 Quantization Is a First-Class Architectural Constraint, Not a Post-Hoc Optimization

**Category:** Performance
**One-liner:** POS terminal AI models must be designed for INT8 quantization from the start (quantization-aware training), achieving 4x memory reduction and 2-3x faster inference with only 1-2% accuracy loss.

**Why it matters:** POS terminals run on ARM-based SoCs with 2-4GB RAM shared between the OS, application, and AI models. A full FP32 MobileNetV3 for product recognition consumes ~14MB and runs inference in ~150ms on CPU. The INT8 quantized variant drops to ~3.5MB and runs in ~45ms, because INT8 arithmetic uses 4x less memory bandwidth and ARM NEON/Hexagon DSP units process INT8 operations 2-3x faster than FP32. The critical insight is that post-training quantization (PTQ) often causes 3-5% accuracy degradation on fine-grained product recognition tasks, while quantization-aware training (QAT) limits degradation to 1-2% by simulating quantization noise during training. This means the model architecture and training pipeline must be designed around the target precision from day one. MobileNetV3's inverted residual blocks and hard-swish activations were explicitly designed to be quantization-friendly -- choosing a model architecture that is not (e.g., models with dynamic ranges that vary widely across channels) would make edge deployment impractical regardless of how powerful the terminal hardware is.

---

## Insight 10: Store-and-Forward Payment Authorization Creates Bounded Financial Exposure Windows

**Category:** Security
**One-liner:** Offline payments accept transactions up to a configurable floor limit ($50-$200) using locally cached tokenized card data, creating a predictable maximum financial exposure per terminal per offline period.

**Why it matters:** The fundamental tension in offline POS payments is that authorization normally requires real-time communication with the payment processor, which is unavailable offline. The solution is store-and-forward: the terminal captures encrypted card data, performs local validation (Luhn check, expiry, BIN range), applies a floor limit, and queues the authorization for when connectivity returns. The bounded exposure calculation is straightforward: 5 terminals x $200 floor limit x 50 transactions per offline period = $50,000 maximum exposure per store. This is a business decision, not a technical one -- the store owner accepts this bounded risk because rejecting sales during an outage has a higher cost (lost revenue, customer frustration). The critical security requirement is that stored card data never touches disk unencrypted: the terminal uses a hardware security module (HSM) or secure enclave to encrypt the PAN immediately at capture, and only the tokenized reference is stored in the sync queue. This satisfies PCI-DSS requirement 3.4 (render PAN unreadable anywhere it is stored) even during extended offline periods.

---

## Insight 11: Network Partition Between Terminals Creates a Different Split-Brain Than Cloud Disconnection

**Category:** Partitioning
**One-liner:** A LAN partition splitting terminals into two groups triggers Raft's majority rule correctly -- the majority partition elects a leader and continues syncing, while the minority partition operates in degraded read-write mode with local-only persistence.

**Why it matters:** Most offline-first discussions focus on the "store vs. cloud" partition, but the more operationally dangerous partition is within the store itself -- for example, when a faulty switch splits 5 terminals into a group of 3 and a group of 2. Raft handles this correctly: only the majority partition (3 terminals) can elect a leader, so cloud sync and inter-terminal sync continue for those 3. The minority partition (2 terminals) cannot elect a leader, so they continue processing sales locally (offline-first guarantees this) but cannot sync with the other partition until the network heals. When the partition heals, the minority terminals discover the existing leader, sync their queued events, and the CRDTs merge correctly. The subtle danger is if the partition persists for hours: both groups may sell the same scarce inventory, and the post-merge oversell detection (Insight 4) must handle a potentially larger conflict window. Store operators should be alerted to LAN partitions because the business impact (potential oversells) scales with partition duration, unlike cloud disconnection where terminals still sync with each other.

---

## Insight 12: Differential Model Updates Amortize OTA Bandwidth by Transmitting Only Changed Layers

**Category:** Streaming
**One-liner:** Full AI model updates (20-100MB) are impractical over constrained store networks, so the system transmits only changed weight tensors (~2-5MB) verified by layer-wise checksums, mirroring the delta sync pattern used for data.

**Why it matters:** AI models on POS terminals need periodic updates -- new products added, recognition accuracy improvements, fraud pattern updates. A full MobileNetV3 model is ~14MB (INT8), but a fraud detection ensemble or multi-model pipeline can reach 50-100MB. Pushing full models to 100,000 stores with 5 terminals each means distributing 500,000 model copies. Differential updates solve this by treating model weights as a structured blob: each layer has a checksum, and the update server sends only layers whose checksums differ. In practice, fine-tuning a product recognition model for new SKUs changes only the final classification layers (~5-10% of total weights), reducing update size to 1-5MB. The update is applied atomically: the terminal downloads the delta, reconstructs the full model in a staging area, validates it against a test inference batch, and swaps it in. If validation fails, the terminal continues with the previous model. This pattern is directly analogous to the CRDT delta sync used for data -- the same principle of "send only what changed" applied to the AI layer of the system.
