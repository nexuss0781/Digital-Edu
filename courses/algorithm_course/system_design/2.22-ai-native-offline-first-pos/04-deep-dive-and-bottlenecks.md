...
### Slowest part of the process 1: Sync Bandwidth During Peak Hours
...
### Slowest part of the process 2: AI Model Inference Latency
...
### Slowest part of the process 3: Local Storage on Terminals
...
1. **Aggressive Cutting off unnecessary steps**: Delete synced events after 7 days
...
## Critical Component 4: Differential Model Update Pipeline

### Why This Is Critical

AI models on POS terminals need periodic updates: new products, improved accuracy, updated fraud patterns. But store networks are bandwidth-constrained (often shared with customer WiFi) and terminals cannot interrupt sales for updates.

### How It Works Internally

```
DIFFERENTIAL MODEL UPDATE PIPELINE:
═══════════════════════════════════════════════════════════════

Cloud Model Registry
  │
  ├── Full model: mobilenetv3_products_v47.tflite (14.2 MB)
  ├── Previous: mobilenetv3_products_v46.tflite (14.1 MB)
  └── Delta:    mobilenetv3_v46_to_v47.delta    (1.8 MB)
        │
        │ Layer-wise diff:
        │   conv1: unchanged (checksum match)
        │   conv2: unchanged
        │   ...
        │   fc_head: CHANGED (new product classes)
        │   softmax: CHANGED (output dimension change)
        │
        ▼
Leader Terminal (downloads delta first)
  │
  ├── 1. Download delta (1.8 MB vs 14.2 MB full)
  ├── 2. Apply delta to local v46 → reconstruct v47
  ├── 3. Validate: run test batch (50 known products)
  │      ├── Accuracy ≥ 95%? → ACCEPT
  │      └── Accuracy < 95%? → REJECT, alert, keep v46
  ├── 4. If accepted: distribute to followers via LAN
  └── 5. Atomic swap: symlink model_current → v47
        │
        ▼
Follower Terminals (receive from leader, not cloud)
  │
  ├── Download from leader (LAN speed: ~100 MB/s)
  ├── Local validation (same test batch)
...
| Parameter | Value | Rationale |
|-----------|-------|-----------|
| **Delta format** | Layer-wise binary diff | Only changed weights transmitted |
| **Typical delta size** | 1-5 MB (vs 14-100 MB full) | ~90% bandwidth reduction |
| **Validation batch** | 50 known products | Quick accuracy check before swap |
| **Accuracy threshold** | 95% on validation set | Rejects corrupted or regressed models |
| **Rollback window** | 24 hours | Previous model kept as fallback |
| **Update frequency** | Weekly (products), daily (fraud) | Balances freshness vs bandwidth |
| **Max staleness** | 30 days without update triggers alert | Ensures model currency |

### Quantization-Aware Deployment

```
MODEL PRECISION TRADE-OFFS (MobileNetV3 Product Recognition):
═══════════════════════════════════════════════════════════════

Precision    Size      Inference (CPU)    Inference (GPU)    Accuracy
─────────    ────      ──────────────     ──────────────     ────────
FP32         14.2 MB   ~150 ms            ~45 ms             98.2%
FP16         7.1 MB    ~95 ms             ~30 ms             98.1%
INT8 (PTQ)   3.6 MB    ~55 ms             ~20 ms             95.4%
INT8 (QAT)   3.5 MB    ~50 ms             ~18 ms             97.0%

PTQ = Post-Training Quantization (simple, lower accuracy)
QAT = Quantization-Aware Training (complex, preserves accuracy)

Recommendation: INT8 QAT for production deployment
  • 4x smaller than FP32 → fits in terminal memory budget
  • 3x faster inference → meets <100ms latency target
  • Only 1.2% accuracy drop → acceptable for product recognition
  • ARM NEON/Hexagon DSP optimized for INT8 operations
```
...
| Aspect | Square Offline | Toast Offline | This Design |
|--------|---------------|--------------|-------------|
| **Offline Duration** | 24 hours | Restaurant shift | 24+ hours (unlimited with local storage) |
| **Sync Model** | Queue-and-replay to cloud | Server-per-restaurant | Leader-aggregated delta sync |
| **Conflict Handling** | Last-write-wins | Restaurant server resolves | CRDTs + AI-powered resolution |
| **Terminal Discovery** | Cloud-configured | Pre-configured | mDNS zero-config |
| **AI Features** | Minimal | Kitchen display optimization | Full edge AI (recognition, fraud, voice) |
| **Leader Election** | None (cloud-dependent) | Fixed server | Raft consensus (any terminal) |
| **Payment Offline** | Store-and-forward | Store-and-forward | Store-and-forward + local token cache |
| **Model Updates** | Cloud push on connect | Cloud push | Differential delta via leader |
