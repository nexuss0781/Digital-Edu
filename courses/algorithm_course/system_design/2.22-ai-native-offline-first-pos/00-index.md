...
|----------|-------------|
| **What AI decides alone** | Product recognition from barcode/image scan, fraud risk scoring per transaction, demand prediction for reorder alerts |
| **What AI recommends** | Personalized product recommendations, markdown pricing suggestions, inventory reorder quantities |
| **What requires human approval** | Price overrides above discount threshold, void/refund authorization, offline payment limit increases, AI model updates pushed to terminals |
| **Deterministic source of truth** | Local SQLite database per terminal (CRDT-synchronized) for transactions; cloud Mother Server for aggregated truth — AI inference runs locally but pricing and inventory rules are deterministic |
| **Rollback path** | CRDT-based sync enables conflict-free merge on reconnect; transaction voiding with manager PIN; model rollback via atomic swap to previous version on terminal |
...
- [Distributed Lock Manager](../1.8-distributed-lock-manager/00-index.md) - Leader election patterns, Raft consensus
- [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) - Event-based sync, append-only logs
- [Edge Data Processing](../2.14-edge-data-processing/00-index.md) - Edge architecture, local-first compute
- [Edge AI/ML Inference](../2.13-edge-ai-ml-inference/00-index.md) - Local AI models, quantization
- [Multi-Region Active-Active](../2.9-multi-region-active-active/00-index.md) - CRDT patterns, conflict resolution
- [Distributed Key-Value Store](../1.3-distributed-key-value-store/00-index.md) - Consistent hashing, replication strategies
- [Service Discovery System](../1.10-service-discovery-system/00-index.md) - mDNS parallels, zero-config discovery
- [Secret Management System](../2.16-secret-management-system/00-index.md) - Offline credential caching, HSM integration
