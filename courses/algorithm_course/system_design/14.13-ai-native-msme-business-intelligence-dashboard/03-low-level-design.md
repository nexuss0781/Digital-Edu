...
```
Step-by-step plan in plain English: schema_mapping(entities, semantic_graph)
...
```
Step-by-step plan in plain English: validate_sql(sql, tenant_id, allowed_schema)
...
```
Step-by-step plan in plain English: select_visualization(intent, result_schema, row_count)
...
```
Step-by-step plan in plain English: detect_anomalies(tenant_id, kpi_name, new_value, timestamp)
...
The analytical warehouse uses a multi-level indexing strategy optimized for the two dominant query patterns: (1) tenant-scoped time-range aggregations and (2) dimensional drill-downs.

| Index Type | Columns | Purpose | Storage Overhead |
|---|---|---|---|
| **Partition index** | `tenant_id` (hash, 4096 buckets) | Eliminate 99.97% of data from scan path | Minimal (partition metadata only) |
| **Sort key** | `timestamp` within each partition | Range Cutting off unnecessary steps for time-based queries ("last month", "this week") | None (data physically sorted) |
| **Min/max index** | All numeric columns per columnar segment | Skip segments where the query predicate cannot match | ~0.1% of data size |
| **Bloom filter** | High-cardinality text columns (`product_name`, `customer_name`) | Fast membership testing for exact-match filters | ~1% of column size |
| **Dictionary encoding** | Low-cardinality columns (`category`, `channel`, `city`) | Compress repeated values; enable predicate pushdown on encoded values | Reduces column size by 5–10× |

### Semantic Graph Indexing

```
PRIMARY INDEX:   (tenant_id, node_type)     → fast lookup of all metrics/dimensions for a tenant
SECONDARY INDEX: (tenant_id, business_name) → fuzzy text search for NL entity resolution
SECONDARY INDEX: (tenant_id, physical_name) → reverse lookup from SQL column to business concept
FULL-TEXT INDEX: (business_aliases)          → search across all aliases for ambiguous entity resolution
```

### Query Log Indexing

```
PRIMARY INDEX:   (tenant_id, created_at)    → tenant-scoped query history
SECONDARY INDEX: (intent, created_at)       → cross-tenant query pattern analysis
SECONDARY INDEX: (result_hash)              → semantic cache deduplication
SECONDARY INDEX: (user_feedback, created_at) → accuracy tracking and model retraining dataset extraction
```

---

## Data Connector Interface Specification
...
INTERFACE ConnectorAdapter:
    // Discovery
    METHOD discover_schema() → SchemaDefinition[]
        // Returns list of tables with column names, types, sample values
        // Called during initial onboarding and schema drift detection

    // Full extraction
    METHOD extract_snapshot(table: String, batch_size: Int) → Iterator[RecordBatch]
        // Streams entire table in batches for initial data load
        // Must support resumption from a checkpoint (batch_id)

    // Incremental extraction
    METHOD extract_incremental(table: String, watermark: Timestamp) → Iterator[RecordBatch]
        // Returns records modified since the watermark
        // If CDC not available, falls back to timestamp-based extraction

    // Health check
    METHOD health_check() → HealthStatus
        // Verifies credentials are valid and source is reachable
        // Returns: HEALTHY, DEGRADED (slow), UNHEALTHY (unreachable), EXPIRED (credentials)

    // Schema comparison
    METHOD detect_drift(known_schema: SchemaDefinition) → DriftEvent[]
        // Compares current source schema against last-known schema
        // Returns: column_added, column_removed, column_renamed, type_changed

RECORD RecordBatch:
    batch_id     String          // For checkpoint-based resumption
    records      Map[]           // Array of column-name → value maps
    watermark    Timestamp       // Highest modification timestamp in this batch
    schema_hash  String          // Hash of the source schema at extraction time
```

### Supported Connector Types

| Source Type | Extraction Method | Auth Method | Typical Sync Latency | Schema Stability |
|---|---|---|---|---|
| Tally ERP | File export parsing (XML/JSON) | File system access / API key | 15–30 min (batch) | Low (custom fields common) |
| Zoho Books | REST API with pagination | OAuth 2.0 | 5–10 min (incremental) | Medium |
| POS systems | REST/SOAP API varies by vendor | API key | 10–15 min | Medium |
| E-commerce platforms | REST API (vendor-specific) | OAuth 2.0 | 5–10 min | High (API versioned) |
| Bank statements | File upload (PDF/CSV parsing) | N/A (manual upload) | On-demand | N/A (parsed at upload) |
| Spreadsheet upload | File upload (CSV/Excel parsing) | N/A | On-demand | N/A (schema per upload) |

---

## Benchmark Aggregation Pipeline

### Cohort Definition Algorithm

```
Step-by-step plan in plain English: define_cohorts(all_tenants)
    cohorts = []
    // Group by industry × geography × revenue band
    FOR EACH vertical IN unique(all_tenants.industry_vertical):
        FOR EACH geo IN unique(all_tenants.geography):
            FOR EACH band IN ["micro", "small", "medium"]:
                members = filter(all_tenants,
                    industry == vertical AND geography == geo AND revenue_band == band)
                IF len(members) >= 50:  // minimum cohort size
                    cohorts.append(Cohort(vertical, geo, band, members))
                ELSE:
                    // Try broader geography (state instead of city)
                    broader_geo = broaden_geography(geo)
                    members = filter(all_tenants,
                        industry == vertical AND geography == broader_geo AND revenue_band == band)
                    IF len(members) >= 50:
                        cohorts.append(Cohort(vertical, broader_geo, band, members))
                    // Else: no benchmark available for this combination
...
```
Step-by-step plan in plain English: rank_tenant_in_cohort(tenant, cohort, kpi)
    // Load all cohort members' KPI values
    values = load_kpi_values(cohort.member_ids, kpi)

    // Compute tenant's percentile rank
    tenant_value = load_kpi_value(tenant.id, kpi)
    rank = count(v for v in values if v < tenant_value) / len(values) * 100

    // Compute growth comparison
    tenant_growth = compute_growth(tenant.id, kpi, period=3_months)
    cohort_avg_growth = mean(compute_growth(m, kpi, 3_months) for m in cohort.member_ids)

    // Apply differential privacy noise to cohort statistics (not to tenant's own value)
    noisy_benchmarks = compute_private_benchmark(cohort, kpi, epsilon=1.0)

    RETURN {
        your_value: tenant_value,            // exact (not noisy — it's the tenant's own data)
        percentile: rank,                    // approximate (ordinal position, not revealing others' values)
        benchmarks: noisy_benchmarks,        // noisy (differentially private)
        your_growth: tenant_growth,          // exact
        cohort_avg_growth: cohort_avg_growth + gaussian_noise(0, noise_scale),  // noisy
        outperforming: tenant_growth > cohort_avg_growth
    }
```

---

## Multi-Language NL Processing

### Language Detection and Routing

```
Step-by-step plan in plain English: process_multilingual_query(raw_text, tenant)
    // Step 1: Detect language(s)
    detected = language_detector.detect(raw_text)
    // Handle code-mixed input (e.g., "last week ka revenue kya tha?")
    IF detected.is_code_mixed:
        primary_language = detected.dominant_language  // Hindi
        embedded_english = detected.english_segments   // ["revenue", "last week"]

    // Step 2: Translate non-English segments to English
    IF detected.primary != "en":
        translated = translate_to_english(raw_text, source=detected.primary)
        // Preserve business terms that are commonly used in English
        // (revenue, profit, sales, order — these are rarely translated by merchants)
        final_text = merge_translations(translated, embedded_english)
    ELSE:
        final_text = raw_text

    // Step 3: Route to NL pipeline with language metadata
    result = nl_to_sql_pipeline(final_text, tenant, source_language=detected.primary)

    // Step 4: Generate narrative in the merchant's preferred language
    narrative = generate_narrative(result, language=tenant.preferred_language)

    RETURN result, narrative
```

### Supported Languages and Accuracy Impact

| Language | NL-to-SQL Accuracy | Notes |
|---|---|---|
| English | 92% | Baseline; best-covered by training data |
| Hindi (Devanagari) | 88% | Strong coverage; most Indian MSME merchants |
| Hindi-English code-mixed | 85% | Common in practice; requires special tokenization |
| Tamil | 83% | Smaller training dataset; improving with feedback |
| Telugu | 82% | Similar to Tamil coverage |
| Marathi | 84% | Overlap with Hindi vocabulary helps |
| Bengali | 81% | Requires dedicated entity extraction tuning |
| Gujarati | 80% | Strong accounting vocabulary overlap (business community) |
