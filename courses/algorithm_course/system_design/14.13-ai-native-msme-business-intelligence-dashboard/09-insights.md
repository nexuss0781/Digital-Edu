...
> 12 architectural insights from building an AI-native BI platform for 2M+ MSME tenants — covering NL-to-SQL safety, semantic graph evolution, multi-tenant cost fairness, insight ranking, and the unique challenges of WhatsApp-first analytics delivery.
...
## Insight 10: Multi-Tenant Query Cost Fairness Requires a Token Economy, Not Just Rate Limiting

**Category:** Cost Optimization

**One-liner:** Traditional rate limiting (queries per minute) fails as a fairness mechanism because a simple "What is my revenue?" costs 100× less compute than "Correlate my sales with weather data across all product categories for the last 2 years" — fair resource allocation requires a query-cost token economy where each tenant has a compute budget measured in abstract cost units, not query counts.

**Why it matters:** At 2M tenants sharing a single analytical warehouse, resource fairness is critical. A free-tier tenant running one expensive query can consume more compute than a pro-tier tenant running 50 simple queries. Rate limiting by query count penalizes the pro-tier tenant unfairly. The production system assigns each query an estimated cost in "compute units" (CU) based on: estimated rows scanned × join complexity × aggregation cost. Each tenant tier has a daily CU budget (free: 100 CU, starter: 500 CU, growth: 2000 CU, pro: 10000 CU). A simple aggregation query costs 1-5 CU; a complex multi-table join with full-range scan costs 50-200 CU. Before execution, the query optimizer estimates the CU cost. If the query would exceed the remaining daily budget, the system suggests optimizations: "This query scans 2 years of data. Limiting to the last 6 months would reduce cost by 75% — shall I do that?" This approach aligns incentives: merchants are encouraged to ask focused questions (which the NL-to-SQL pipeline handles more accurately anyway), and the system can guarantee SLOs because no single tenant can exhaust shared compute resources. The CU budget also provides a natural monetization gradient — merchants who need heavy analytics upgrade their tier, funding the compute they consume.

---

## Insight 11: The Connector Health Topology Creates a Shared-Fate Group That Must Be Managed as a First-Class Entity

**Category:** External Dependencies

**One-liner:** When 200K tenants all connect to the same accounting software provider, the health of that single external API becomes a shared-fate dependency for 10% of the platform — connector health must be tracked not just per-connector and per-tenant, but per-source-system, with source-level circuit breakers and coordinated recovery.

**Why it matters:** The naive connector model treats each tenant's connector as independent: Tenant A's Tally connector and Tenant B's Tally connector are managed separately. But when Tally's API goes down, all 200K Tally connectors fail simultaneously. If each connector retries independently with exponential backoff, the platform generates 200K × 3 retries = 600K requests against a failing API within minutes — turning a source outage into a DDoS attack from the platform's own infrastructure. The production system introduces a "source health" abstraction: connector failure rates are aggregated per source type. When failure rate exceeds 30% for a source within a 5-minute window, the source is marked "degraded" and all connectors for that source enter coordinated backoff. A single health-check probe per source (not per connector) monitors recovery. When the probe succeeds, connectors resume in a staggered fashion (1% per minute) to avoid overwhelming the recovered source. This source-level health management reduces unnecessary API calls by 99.5% during outages and speeds recovery by 3× because the source isn't overwhelmed by a thundering herd on recovery. The source health model also enables proactive merchant communication: "Your Tally data may be delayed due to a Tally service issue. We're monitoring and will sync automatically when the service recovers."

---

## Insight 12: The Schema Mapping Confidence Score Is Both a Quality Signal and a Product Feature That Drives Engagement

**Category:** System Modeling

**One-liner:** The confidence score assigned to each semantic graph mapping (how certain the AI is that `vch_amt` means "transaction amount") is not just an internal quality metric — exposing it to the merchant as a "data understanding progress bar" transforms a cold-start liability into an engagement mechanism that drives the merchant to complete the onboarding loop.

**Why it matters:** Semantic graph cold-start is the system's weakest point: until the graph is accurate, NL queries produce unreliable results. The traditional approach hides mapping confidence from the merchant and only surfaces errors when a query fails. The production system inverts this by making confidence visible: a "Data Understanding" progress indicator on the dashboard shows "Your data is 73% mapped — help us understand 5 more columns to unlock full analytics." Each unmapped or low-confidence column is presented as a simple multiple-choice question in the merchant's language. Merchants who complete these questions (typically 10-15 questions, taking 3-5 minutes) achieve 95%+ graph accuracy and dramatically better query results. The engagement mechanics: (1) the progress bar creates completion motivation (the same psychology that drives profile completion on social platforms); (2) each answered question immediately expands the scope of answerable NL queries, providing instant gratification ("Now you can ask about supplier-wise breakdowns!"); (3) the questions are prioritized by query frequency — the system asks about columns that are most likely to be referenced in queries first, maximizing the ROI of each merchant interaction. Analytics show that merchants who complete the mapping questionnaire within the first session have 2.3× higher 30-day retention than those who skip it, making the confidence score both a technical quality metric and a critical product engagement lever.
