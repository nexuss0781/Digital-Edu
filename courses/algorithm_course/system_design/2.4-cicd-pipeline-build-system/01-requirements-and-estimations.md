...
7. **Merge Queue Management (2025)**
   - Serialized merge with speculative execution
   - Automatic rebase onto latest main before testing
   - Configurable concurrency and batch size
   - Priority merge for hotfixes

8. **Flaky Test Management**
   - Per-test pass/fail rate tracking across runs
   - Automatic quarantine when failure rate exceeds threshold
   - Auto-unquarantine when tests are fixed
   - Flaky test reports and owner notification
...
- Remote Build Execution protocol (Bazel RBE) — separate system with CI integration
- Container image building service (Kaniko, BuildKit) — embedded within job steps
