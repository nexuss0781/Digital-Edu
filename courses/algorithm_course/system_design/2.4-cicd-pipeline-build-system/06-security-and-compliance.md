...
```
Non-Hermetic Build Attack Surface:
┌──────────────────────────────────────────────────────────────┐
│  Source         Registry        CDN Mirror       Build Tools │
│  ┌─────┐       ┌─────┐        ┌─────┐          ┌─────┐     │
│  │ Git │──────>│ npm │──────>│ CDN │────────>│Build │     │
│  │     │       │ PyPI│       │     │         │     │     │
│  └─────┘       │ Maven│      └─────┘         └─────┘     │
...
│  Compromised   Dependency    CDN serves      Compiler     │
│  source repo   confusion     stale/bad       supply chain │
│                              version                       │
└──────────────────────────────────────────────────────────────┘

Hermetic Build Attack Surface:
┌──────────────────────────────────────────────────────────────┐
│  Source (only attack vector remaining)                        │
│  ┌─────┐                                                     │
│  │ Git │──────> [NETWORK SEVERED] ──────> Build              │
│  │     │        All deps vendored          (sandboxed)       │
│  └─────┘        Content-hash verified                        │
│    ↑                                                          │
│  Attack 1 (only): Compromised source repo                    │
│  Mitigated by: Code review, branch protection, signed commits│
└──────────────────────────────────────────────────────────────┘
```

### SLSA Level 3+ Attestation Implementation

```python
class SLSALevel3Attestation:
    """
    Generate SLSA Level 3 compliant build attestations.

    Level 3 requirements:
    - Hardened build platform (ephemeral, isolated)
    - Non-falsifiable provenance (platform-signed, not user-signed)
    - Source provenance (verified git commit, not arbitrary input)
    """

    def generate_l3_provenance(self, job, artifacts):
        """
        Platform-generated provenance that users cannot forge.

        Key difference from L1/L2: the provenance is signed by
        the build platform's identity, not the user's. Users
        cannot modify or replace the attestation.
        """
        provenance = {
            "_type": "https://in-toto.io/Statement/v1",
            "subject": [
                {
                    ...
                    "digest": {
                        ...
                        "sha512": artifact.content_hash_512
                    }
                }
                for artifact in artifacts
            ],
            ...
            "predicate": {
                "buildDefinition": {
                    "buildType": "https://cicd-platform.example/build/v1",
                    "externalParameters": {
                        "source": {
                            "uri": f"git+{job.repo_url}",
                            "digest": {"sha1": job.head_sha}
                        },
                        "workflow_ref": job.workflow_ref,
                        "inputs": job.sanitized_inputs
                    },
                    "internalParameters": {
                        "runner_id": job.runner_id,
                        "runner_image_digest": job.runner_image_hash,
                        "isolation_type": job.isolation_type,
                        "hermetic": job.hermetic_mode,
                        "network_disabled_during_build": job.network_blocked
                    },
                    "resolvedDependencies": self.resolve_deps(job)
                },
                "runDetails": {
                    "builder": {
                        "id": f"https://cicd-platform.example/builders/{job.builder_id}",
                        "builderDependencies": [
                            {
                                "uri": f"oci://{job.runner_image}",
                                "digest": {"sha256": job.runner_image_hash}
                            }
                        ],
                        "version": {
                            "platform": PLATFORM_VERSION,
                            "runner": job.runner_version
                        }
                    },
                    "metadata": {
                        "invocationId": job.run_url,
        ...
        # Platform signs with its own key (non-falsifiable)
        signed = self.platform_sign(provenance)

        # Submit to transparency log
        rekor_entry = self.submit_to_rekor(signed)

        return signed, rekor_entry

    def verify_before_deploy(self, artifact, policy):
        """
        Verify artifact meets SLSA policy before deployment.

        Called by deployment gates, admission controllers, etc.
        """
        attestation = self.fetch_attestation(artifact.digest)

        checks = [
            self.verify_platform_signature(attestation),
            self.verify_rekor_inclusion(attestation),
            self.verify_source_repo(attestation, policy.allowed_repos),
            self.verify_builder_id(attestation, policy.trusted_builders),
            self.verify_hermetic(attestation) if policy.require_hermetic else True,
            self.verify_branch(attestation, policy.allowed_branches),
            self.verify_review_count(attestation, policy.min_reviews),
        ]

        if not all(checks):
            raise PolicyViolation(
                f"Artifact {artifact.digest[:12]} failed SLSA verification"
            )
...
| Attack Vector | Non-Hermetic Risk | Hermetic Mitigation |
|--------------|-------------------|---------------------|
| Typosquatting (similar package name) | High — fetched at build time | Vendored deps verified by hash |
| Dependency confusion (internal vs public) | High — resolver may prefer public | All sources pinned in lockfile |
| Registry compromise | High — malicious update served | Content hash mismatch = build fails |
| CDN poisoning | Medium — stale/modified version | No CDN access during build |
| Abandoned package takeover | High — new maintainer pushes malware | Lockfile pins exact version + hash |
