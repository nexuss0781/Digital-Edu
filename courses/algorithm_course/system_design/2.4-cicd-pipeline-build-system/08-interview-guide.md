...
|--------|-------------|-------------|
| **Pros** | Global view, simple fair-share | No Slowest part of the process, region-local |
...
### "Why would you need a merge queue? Isn't CI enough?"

**Trap:** Not understanding the semantic conflict problem that optimistic CI can't catch.

**Good answer:**
"Standard CI tests each PR independently against the current main. But when PR A and PR B both pass CI and merge, their combined changes may break main — even without git merge conflicts. For example, PR A renames a function and PR B adds a call to the old name. Both pass CI independently, but combined they break.

A merge queue fixes this by serializing merges. Each PR is tested against main plus all preceding PRs already in the queue. PR B doesn't test against just main — it tests against main + PR A. This guarantees every commit on main has actually passed CI against the exact state it lands on.

For high-velocity repos, speculative execution tests multiple queue positions in parallel. If the queue is [PR#1, PR#2, PR#3], we speculatively test PR#3 against main+#1+#2 before #1 or #2 have merged. If #1 passes and merges, #2's speculative result is still valid. If #1 fails, we invalidate #2 and #3's results and re-test. With a lookahead of 3-5, this reduces queue wait time by 60-80% compared to serial testing."

### "What is a hermetic build and when would you enforce it?"

**Trap:** Treating hermetic builds as just 'no internet' without explaining the guarantees and trade-offs.

**Good answer:**
"A hermetic build has no network access during the compilation phase. All dependencies are pre-fetched and vendored in a resolution phase before the network is severed. This provides two guarantees:

First, reproducibility: the same source code always produces the same output, regardless of when or where it builds. A non-hermetic build can fail tomorrow because a transient dependency changed or a package registry went down.

Second, supply chain security: a compromised package registry cannot inject malicious code into a build that never contacts it during compilation. This is critical for SLSA Level 4 compliance.

The trade-off is setup complexity — all inputs must be explicitly declared. Tools like Bazel and Buck2 enforce this by sandboxing build actions and failing if undeclared inputs are accessed. For most organizations, hermetic builds are worth the setup cost for production release pipelines, while development builds can remain non-hermetic for faster iteration."

### "How do you handle flaky tests that undermine CI trust?"

**Trap:** Suggesting 'just rerun failed tests' without a systematic detection and quarantine strategy.

**Good answer:**
"Flaky tests are the single biggest threat to CI effectiveness. Even 1% per-test flakiness in a suite of 100 tests gives a 63% pipeline failure rate — mathematically, 1 minus 0.99 to the power of 100.

The solution is statistical detection with automatic quarantine. We track per-test pass/fail rates using a sliding window of 50-100 runs. When a test's failure rate exceeds a threshold — say 3% — it's automatically quarantined: it still runs but doesn't block the pipeline. The test owner gets notified.

Quarantined tests continue running to collect failure data. When the failure rate drops below 1% over 50+ runs — indicating the test was fixed — it's automatically un-quarantined and becomes blocking again.

We also analyze flakiness by environment — some tests only flake on specific runner labels or at specific times of day, which helps diagnose the root cause. This is essentially the circuit breaker pattern applied to test suites: isolate the failing component to prevent it from bringing down the system."
...
The real Slowest part of the process is usually runner capacity during bursts. We solve this with:
...
| No merge queue | Broken main at scale | Serialized merge with speculative execution |
| Manual flaky test mgmt | CI trust erosion | Statistical detection + auto-quarantine |
| Non-hermetic builds | Supply chain vulnerable | Vendor deps, sever network during build |
...
| Merge queue speedup (speculative) | 60-80% reduction | vs serial testing |
| 1% test flakiness × 100 tests | 63% pipeline failure | Quarantine essential |
| RBE cache hit rate (mature) | 90-99% | Monorepo incremental builds |
| Wasm runner startup | < 5ms | Lightweight tasks only |
| SLSA Level 3 requirement | Non-falsifiable provenance | Platform-signed attestation |
| Hermetic build characteristic | No network during compile | All deps pre-fetched |
...
- [ ] Merge queues: speculative execution at high velocity
- [ ] Hermetic builds: SLSA L3+, supply chain security
...
### Question 1: Merge Queue Design

"Your team merges 100+ PRs per day to a monorepo. Despite all PRs passing CI, main breaks 3-5 times per week. How do you solve this?"

**Key points to cover:**
- Explain why optimistic CI fails (semantic conflicts)
- Merge queue with speculative execution
- Path-based partitioning for monorepo (parallel queues per area)
- Hotfix priority lane bypassing the queue
- Trade-off: compute waste from invalidated speculative results vs queue throughput

### Question 2: Remote Build Execution

"A 10-million-line monorepo takes 2 hours to build on a single machine. How do you reduce this to minutes?"

**Key points to cover:**
- Distribute compilation at file granularity (individual actions)
- Content-addressable action cache (same inputs → skip)
- Cache hit rates of 90-99% for incremental changes
- Worker pool sizing: hundreds of remote workers
- Hermeticity requirement (all inputs declared for caching to work)

### Question 3: CI Pipeline Reliability at Scale

"Your platform runs 500M jobs/day. The success rate is 94% but users complain CI is 'always broken.' What's happening and how do you fix it?"

**Key points to cover:**
- Distinguish platform failures vs user errors vs flaky tests
- Flaky test math: even 1% flakiness × 100 tests = 63% pipeline failure
- Statistical detection with sliding window
- Auto-quarantine (runs but doesn't block)
- Quarantine dashboard and owner notifications
- Measurable impact: pipeline success rate with vs without quarantine
...
For the queue structure, I'd use Redis sorted sets where the score combines priority weight and timestamp. High-priority jobs (production deployments) get 3x weight, normal jobs (PR builds) get 1x, and scheduled jobs get 0.5x. Within the same priority, jobs are FIFO (First-In-First-Out, like a line at a store) by timestamp.
