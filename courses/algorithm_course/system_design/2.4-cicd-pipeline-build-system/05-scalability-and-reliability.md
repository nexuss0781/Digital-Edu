...
## Remote Build Execution (RBE) Scaling

### Distributed Compilation at File Granularity

Remote Build Execution distributes individual compilation actions across a worker pool, achieving near-linear speedup for large codebases.

```
Traditional CI:                    Remote Build Execution:

┌─────────────────────┐           ┌─────────────────────────────────┐
│ Single Runner       │           │ RBE Cluster                      │
│                     │           │                                   │
│ Compile file 1      │           │ ┌─────────┐ ┌─────────┐         │
│ Compile file 2      │           │ │Worker 1 │ │Worker 2 │         │
│ Compile file 3      │           │ │file 1   │ │file 2   │         │
│ ...                 │           │ │file 5   │ │file 6   │         │
│ Compile file 50000  │           │ └─────────┘ └─────────┘         │
│                     │           │ ┌─────────┐ ┌─────────┐         │
│ Time: 2 hours       │           │ │Worker 3 │ │Worker N │         │
└─────────────────────┘           │ │file 3   │ │file ... │         │
                                  │ │file 7   │ │file 50K │         │
                                  │ └─────────┘ └─────────┘         │
                                  │                                   │
...
class RemoteBuildExecutor:
    """
    Distribute compilation actions across remote workers.
    Content-addressable caching enables cross-build sharing.
    """

    def execute_action(self, action):
        """
        Execute a single build action (e.g., compile one file).

        Action = (command, input_files, environment, output_paths)
        """
        # 1. Compute action cache key
        action_key = self.compute_action_key(action)

        # 2. Check action cache (skip if identical action already ran)
        cached_result = self.action_cache.get(action_key)
        if cached_result:
            return cached_result  # Cache hit — 0 compute cost

        # 3. Upload input files to CAS (content-addressable storage)
        input_tree = self.upload_inputs(action.input_files)

        # 4. Submit to worker pool
        worker = self.scheduler.find_worker(action.platform)
        result = worker.execute(
            command=action.command,
            input_root=input_tree,
            environment=action.environment,
            timeout=action.timeout
        )

        # 5. Store result in action cache
        self.action_cache.put(action_key, result)

        return result

    def compute_action_key(self, action):
        """
        Deterministic key: hash of command + all input content hashes.
        Same inputs always produce same key → global deduplication.
        """
        hasher = hashlib.sha256()
        hasher.update(action.command.encode())
        hasher.update(action.environment_hash.encode())
        for input_file in sorted(action.input_files):
            hasher.update(input_file.content_hash.encode())
        return hasher.hexdigest()
```

### RBE Scaling Characteristics

| Metric | Value | Notes |
|--------|-------|-------|
| Cache hit rate (mature monorepo) | 90-99% | Most actions unchanged between commits |
| Speedup factor | 10-100x | Depends on parallelism and cache rate |
| Worker pool size | 100-1000 | Scales with concurrent builds |
| Action size (typical) | 1-10 MB inputs | Single file + headers |
| Action duration (typical) | 1-30 seconds | Single compilation unit |

---

## WebAssembly Runner Isolation (2025)

### Wasm Runners for Lightweight Tasks

WebAssembly-based runners provide near-instant startup for tasks that don't require full OS capabilities.

| Isolation Method | Startup Time | Memory Overhead | Security | Use Case |
|-----------------|--------------|-----------------|----------|----------|
| Full VM | 30-60s | 512 MB+ | Strongest | Legacy, compliance |
| MicroVM (Firecracker) | ~125ms | 128 MB+ | Strong | General-purpose public |
| Container | ~1s | 50 MB+ | Moderate | Private/trusted |
| WebAssembly | < 5ms | 10 MB+ | Strong (sandbox) | Linters, formatters, scripts |

### Wasm Runner Decision Framework

```
Job characteristics → Runner type:

  Needs OS-level access (docker, systemd)?
    → Yes: MicroVM or Container
    → No: Continue ↓

  Needs specific native binaries?
    → Yes: Container (pre-built image)
    → No: Continue ↓

  Duration < 30 seconds?
    → Yes: Wasm runner (< 5ms startup)
    → No: Continue ↓
