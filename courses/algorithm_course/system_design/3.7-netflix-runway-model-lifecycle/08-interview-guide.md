...
| Manual-only retraining | Doesn't scale, human Slowest part of the process | Policy-based automation with manual override |
...
As Netflix and other organizations shift toward foundation models and large language models, Runway's architecture extends to new challenges:

```
"Foundation models introduce lifecycle complexities beyond traditional ML:

1. EMBEDDING STABILITY: When a foundation model retrains, its embedding
   space may shift. All downstream models consuming those embeddings
   become stale -- not because they degraded, but because their input
   semantics changed. Runway tracks embedding signatures (cosine
   similarity between old and new embedding spaces) as a drift signal.

2. ADAPTER VERSIONING: Fine-tuned adapters (LoRA, QLoRA) create a
   tree of dependencies: base model → adapter → downstream consumer.
   The dependency graph must model this hierarchy, not just flat
   model-to-model edges.

3. EVALUATION WITHOUT GROUND TRUTH: For generative models, traditional
   ground truth (click/watch labels) is insufficient. Modern approaches
   use LLM-as-judge evaluation, where a separate model scores output
   quality. Runway's ground truth pipeline extends to support
   model-generated quality labels alongside human behavior signals.

4. COST-AWARE RETRAINING: Foundation model retraining costs orders
   of magnitude more than traditional ML models. Runway's policy
   engine must weigh staleness severity against retraining cost,
   potentially preferring adapter fine-tuning over full retraining."
```

### Regulatory Implications

```
"The EU AI Act (enforced 2025-2026) makes model lifecycle management
a compliance requirement, not just an engineering optimization:

- DOCUMENTATION: Every model needs a model card documenting training
  data, performance, known limitations, and monitoring procedures.
  Runway's registry and staleness history satisfy this requirement.

- POST-MARKET MONITORING: High-risk AI systems require continuous
  monitoring -- exactly what Runway's staleness detection provides.

- HUMAN OVERSIGHT: Tier 1 approval workflows align with the AI Act's
  requirement for human intervention in high-risk decisions.

This regulatory context elevates Runway from a 'nice-to-have'
platform tool to a 'must-have' compliance infrastructure."
```

### Continuous Evaluation Patterns

```
"Modern ML platforms are shifting from batch evaluation (check
staleness every N hours) to continuous evaluation:

- STREAMING DRIFT DETECTION: Compute drift metrics as a windowed
  stream aggregate rather than periodic batch jobs. This reduces
  detection latency from hours to minutes.

- SHADOW DEPLOYMENTS: Run retrained model in shadow mode alongside
  production, comparing outputs in real-time before promotion.
  Runway's canary deployment extends to shadow evaluation.

- AUTOMATED A/B TESTING: Instead of manual experiment setup,
  automatically create micro-experiments when a retrained model
  is ready, using statistical significance tests to decide
