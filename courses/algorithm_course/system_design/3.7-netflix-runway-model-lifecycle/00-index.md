...
|----------|-------------|
| **What AI decides alone** | Staleness score computation (PSI, KL divergence, performance decay), dependency graph construction, ground truth collection scheduling, drift metric aggregation |
| **What AI recommends** | Model retraining triggers based on staleness policies, cascade impact analysis when upstream models change, retraining priority ordering |
| **What requires human approval** | Retraining policy thresholds, model retirement decisions, dependency graph overrides, staleness policy configuration changes, production model replacement sign-off |
| **Deterministic source of truth** | Model Registry (versioned model catalog with dependency metadata) and event-sourced state store (full audit trail of model state transitions) — Runway detects and recommends; ML engineers and model owners authorize lifecycle actions |
| **Rollback path** | Model version rollback via registry; retraining cancellation through Maestro; staleness policy revert to previous configuration; event-sourced audit trail enables state reconstruction |
...
|--------|-------|---------|
| Netflix subscribers | 300M+ (as of 2025) | Global personalization at scale |
...
- [3.12 Recommendation Engine](../3.12-recommendation-engine/00-index.md) - Downstream consumer of Runway-managed models
- [3.16 Feature Store](../3.16-feature-store/00-index.md) - Axion integration for feature distribution tracking
- [3.25 AI Observability / LLMOps](../3.25-ai-observability-llmops-platform/00-index.md) - Model monitoring and drift detection parallels
