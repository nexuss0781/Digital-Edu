...
### Multi-Rail Traffic Distribution

At scale, the platform distributes traffic across multiple payment rails to optimize for cost, latency, and availability:

| Payment Rail | Peak TPS | Latency | Cost per Txn | Use Case |
|-------------|----------|---------|-------------|----------|
| UPI (via NPCI) | 50,000 | 800ms p50 | Zero (MDR-free) | Primary for all P2P and P2M |
| IMPS (bank-to-bank) | 5,000 | 1.5s p50 | 2-5 INR | UPI fallback; amounts > 1L |
| NEFT (batch) | N/A (batch) | 30-min batches | 2-5 INR | Bulk settlements; non-urgent transfers |
| Card Network (NFC) | 2,000 | 200ms p50 | 1-2% MDR | Contactless tap-to-pay |
| Internal Wallet | 10,000 | 50ms p50 | Zero | Small-value P2P between platform users |

**Routing Decision Logic**: For each transaction, the routing engine selects the optimal rail based on: amount (UPI limits at 1L/2L), latency requirement (user-facing vs. batch), cost (UPI preferred for zero-cost), and availability (if NPCI is degraded, route eligible transactions via IMPS). This multi-rail approach provides both cost optimization and availability resilience.

### Connection Pool Management

| External System | Pool Size | Connection TTL | Health Check | Draining |
|-----------------|-----------|---------------|--------------|----------|
| NPCI Switch | 500 persistent connections | 24 hours | Heartbeat every 30 seconds | Graceful drain on pod shutdown (5s) |
| Sponsor Banks (x3) | 200 per bank | 12 hours | Synthetic transaction every 60 seconds | Immediate failover to alternate bank |
| BBPS Central Unit | 100 connections | 24 hours | Health API every 30 seconds | Circuit breaker on failure |
| Card Networks | 150 per network | 24 hours | Network-provided health endpoint | Network-specific failover rules |
| Account Aggregator | 50 connections | 4 hours | Consent status check every 120 seconds | Queue requests during unavailability |
...
| Rewards Database | Write Slowest part of the process under campaign surge | Async reward crediting via durable queue; eventual consistency acceptable for rewards | < 30 seconds |
...
### Cross-Region Read Consistency

For latency-sensitive read operations served from the secondary region's replicas, the system tracks replication lag and adjusts behavior:

| Replication Lag | Read Behavior | User Impact |
|-----------------|---------------|-------------|
| < 1 second | Serve from nearest replica | No noticeable impact |
| 1-5 seconds | Serve from replica with staleness indicator | "Balance may take a moment to update" |
| 5-30 seconds | Route reads to primary region | Slightly higher latency |
| > 30 seconds | Trigger failover investigation; serve from primary | Elevated latency; ops alerted |

For critical reads (balance before payment, mandate execution check), the system always reads from the primary region to ensure strong consistency, regardless of replication lag.
...
| Encryption key backup | On every rotation | Indefinite (regulatory) | HSM-to-HSM replication with split-key custody |

### Backup Validation

Backups without validation are worthless. The platform runs automated backup verification:

| Validation | Frequency | Method |
|------------|-----------|--------|
| Snapshot restoration test | Weekly | Restore latest snapshot to isolated environment; run 100 synthetic transactions |
| WAL replay verification | Daily | Replay latest 1-hour WAL segment on restored snapshot; verify transaction count matches |
| Cross-region backup consistency | Monthly | Compare checksums between primary and secondary region backups |
| Recovery time measurement | Quarterly | Full DR drill measuring actual RTO/RPO against targets |
...
|----------|-----------------|-------------------|--------------|
| NPCI switch total outage | Block all NPCI endpoints | Circuit breaker opens; transactions queued; "pending" shown to user; pre-funded wallet P2P still works | UPI transactions only |
| Single bank API failure | Return 503 for one bank code | Bank-specific circuit breaker opens; other banks unaffected; affected users see "bank temporarily unavailable" | Users of that specific bank |
| Cache cluster partition | Network partition one cache shard | Requests fall through to database; latency increases but no errors; cache miss rate monitored | Slightly elevated latency |
| Fraud model timeout | Add 5-second delay to model inference | Rules-based fallback activates; transactions proceed with rule-based risk scoring | Risk scoring accuracy slightly reduced |
| Database primary failure | Kill primary database process | Standby promoted within 30 seconds; in-flight writes fail and retry; RPO=0 maintained via sync replication | Brief transaction failures during promotion |
| Reward budget counter corruption | Reset counter to zero | Budget check returns "exhausted"; no overspend occurs; reconciliation job detects and corrects | Campaign temporarily pauses rewards |

### Chaos Engineering Schedule

- **Weekly**: Automated single-service failure injection during off-peak (one service at a time, 5-minute duration)
- **Monthly**: Multi-service failure simulation (e.g., NPCI down + one bank down simultaneously)
- **Quarterly**: Full region failover drill with production-scale shadow traffic
- **Pre-festival**: Dedicated chaos session 2 weeks before Diwali/New Year to validate pre-scaled infrastructure
...
|----------|---------------|-----------|
| Tiered storage for transactions | 40% storage cost reduction | Queries on cold data (>90 days) take 2-5 seconds instead of milliseconds |
| Spot instances for batch workloads | 60% compute cost for settlement/reconciliation | Must handle interruptions; idempotent batch jobs resume from checkpoint |
| Reserved capacity for baseline load | 30% compute cost for always-on services | Requires accurate capacity forecasting; over-reservation wastes money |
| Cache hit rate optimization | 25% database query reduction | Stale reads for VPA resolution (bounded by bank validation) |
| Log sampling for non-critical paths | 50% log storage reduction | Reduced visibility for sampled paths; all errors and financial events retained at 100% |
| Compression for event store | 70% storage reduction for historical events | CPU overhead for decompression during replay; negligible for sequential reads |
| CDN for mini-app static assets | 80% origin traffic reduction | Cache invalidation latency for mini-app updates (mitigated by versioned URLs) |

---

## 6. Hot Partition Detection and Mitigation

| Scenario | Detection | Mitigation | Auto-Recovery |
|----------|-----------|------------|---------------|
| Celebrity VPA receiving millions of P2P payments | Inbound TPS per VPA exceeds 1,000 | Auto-migrate to dedicated shard; batch NPCI submissions; rate-limit senders | Revert when TPS drops below 100 for 30 minutes |
| Flash sale merchant spike | Merchant TPS exceeds 10x baseline | Pre-warm cache; allocate dedicated connection pool to acquiring bank | Release dedicated resources after sale window closes |
| Campaign budget counter hotspot | CAS failure rate exceeds 10% | Increase shard count (split hot shards); redistribute budget slices | Consolidate shards when contention drops below 1% |
| Month-end bill payment surge | Bill payment category TPS exceeds 3x | Scale bill payment pods; throttle non-critical API calls | Scale down after month-end window (typically 3 days) |

---

## 7. Data Migration and Version Strategy

### Rolling Upgrade Procedures

Payment systems cannot afford downtime for schema changes. All data migrations follow these principles:

1. **Backward-compatible schema changes only**: New columns are nullable or have defaults; no column removals in the same release
2. **Dual-write during migration**: New schema receives writes alongside old schema; reads from old schema until migration verified
3. **Shadow validation**: New schema outputs compared against old schema outputs for 24 hours before cutover
4. **Instant rollback**: Old schema retained for 7 days post-migration; feature flag controls which schema is active

### Schema Evolution Table

| Change Type | Approach | Downtime | Rollback |
|-------------|----------|----------|----------|
| Add nullable column | Online ALTER; no migration needed | Zero | Drop column |
| Add non-nullable column with default | Online ALTER; backfill asynchronously | Zero | Drop column |
| Rename column | Add new, dual-write, migrate reads, drop old | Zero | Revert reads to old column |
| Change column type | Add new column, dual-write with cast, migrate reads, drop old | Zero | Revert reads to old column |
| Add new table | Create table; no migration | Zero | Drop table |
| Split table | Create new tables, dual-write, migrate reads, stop writing to old | Zero | Revert reads to old table |
...
| Hot spot mitigation | Celebrity VPA detection, flash-sale pre-warming, festival pre-scaling, auto-recovery |
...
| Chaos engineering | Weekly single-service, monthly multi-service, quarterly region failover, pre-festival validation |
| Cost optimization | Tiered storage, spot instances for batch, cache hit rate optimization, log sampling |
| Schema evolution | Backward-compatible changes, dual-write migration, shadow validation, instant rollback |
