...
### Regulatory Data Retention Matrix

| Data Type | Minimum Retention | Storage State | Deletion Method |
|-----------|-------------------|---------------|-----------------|
| Financial transaction records | 10 years | Encrypted at rest; PII masked after 3 years | Pseudonymization after retention period |
| KYC documents (Aadhaar, PAN) | 5 years after account closure | HSM-encrypted; access-logged | Cryptographic erasure (delete encryption key) |
| Consent artifacts (AA) | Duration of consent + 2 years | Append-only store | Automated purge with audit trail |
| Access logs (admin/ops) | 5 years | Immutable, hash-chained | Natural expiry from tiered storage |
| User communication logs | 3 years | Encrypted | Batch deletion with compliance sign-off |
| Dispute and complaint records | 8 years | Indexed for retrieval | Pseudonymization |
| Merchant onboarding records | 5 years after termination | Encrypted | Cryptographic erasure |
...
### Fraud Detection ML Pipeline Security

The fraud detection pipeline itself is a high-value attack target. Adversaries who understand the model can craft transactions designed to evade detection.

| Threat | Attack Method | Defense |
|--------|--------------|---------|
| Model extraction | Probing with crafted transactions to map decision boundaries | Rate-limit API responses to only necessary fields; never expose raw risk scores to clients |
| Adversarial feature manipulation | Carefully controlling transaction attributes to stay below thresholds | Ensemble of independent models with different feature sets; anomaly detection on feature distributions |
| Training data poisoning | Flooding system with normal-looking fraudulent transactions to shift model baseline | Holdout validation set from verified clean transactions; drift detection on training data distributions |
| Model staleness exploitation | Evolving attack patterns after model deployment | Continuous retraining pipeline with 7-day sliding window; champion-challenger model deployment |

**Model Governance**:
- All model versions are signed and stored in an immutable registry
- Model deployment requires approval from both ML engineer and security reviewer
- Shadow mode validation runs for 48 hours before any model receives production traffic
- Rollback to previous model version is automated if false-positive rate exceeds 2x baseline
...
|-----------|-----------|-------|--------------|
| Automated SAST/DAST | Every CI build | All code changes | Automated pipeline |
| Penetration testing | Quarterly | Full platform including APIs, mobile app, mini-app sandbox | Third-party security firm |
| Red team exercise | Annually | End-to-end attack simulation including social engineering | External red team |
| PCI-DSS audit | Annually | Card data handling, NFC token management, encryption | Qualified Security Assessor (QSA) |
| Bug bounty | Continuous | All external-facing surfaces | Public bug bounty program |
| Dependency vulnerability scan | Daily | All third-party libraries and container images | Automated SCA tools |
...
| ID | Threat | Attack Surface | Likelihood | Impact | Mitigations |
|----|--------|---------------|------------|--------|-------------|
| T1 | SIM swap account takeover | Telecom provider social engineering | High | Critical | Device binding + hardware attestation; 24-hour cooling period; behavioral biometrics |
| T2 | QR code physical tampering | Merchant premises | Medium | High | Digital signature (HMAC) on QR data; prominent merchant name display; periodic physical audits |
| T3 | Cashback farming (fake accounts) | Registration flow | High | Medium | Device fingerprint dedup; graph-based fraud ring detection; 7-day ramp for new accounts |
| T4 | Mini-app data exfiltration | Third-party mini-app code | Medium | Critical | Sandboxed runtime; explicit permission model; no access to payment credentials; automated security scanning |
| T5 | Man-in-the-middle API interception | Network layer | Low | Critical | TLS 1.3 + certificate pinning; request signing with device key; payload encryption; nonce + timestamp replay protection |
| T6 | Insider threat (employee access) | Internal systems | Low | Critical | RBAC with quarterly access review; privileged action audit logging; break-glass access with multi-person approval |
| T7 | NFC relay attack | Wireless proximity | Low | High | Cryptogram includes terminal-unpredictable-number; distance bounding protocol; offline transaction limits |
| T8 | Mandate abuse (unauthorized recurring debit) | Mandate engine | Medium | High | Pre-debit notification 24 hours before; one-tap revocation; per-execution amount ceiling; bank-level MPIN for mandate creation |
| T9 | Settlement manipulation | Settlement batch process | Low | Critical | Snapshot isolation at cutoff; dual-approval for manual settlements; reconciliation mismatch alerts |
| T10 | Credential stuffing on merchant APIs | Merchant API endpoints | High | Medium | API key rotation every 90 days; IP whitelisting; webhook signature verification; rate limiting per merchant |

---

## 7. Zero-Trust Internal Network

### Principles

The platform operates on zero-trust principles where every service-to-service call is authenticated and authorized, regardless of network position.

| Layer | Mechanism | Enforcement |
|-------|-----------|-------------|
| Service identity | mTLS with short-lived certificates (72h validity, 48h rotation) | Service mesh sidecar intercepts all traffic |
| API authorization | JWT with service identity claims; each endpoint declares required claims | API gateway validates claims before routing |
| Secrets management | HSM-backed key management; no secrets in code, config files, or environment variables | Secrets injected at runtime via secrets manager |
| Network segmentation | Payment services in isolated network segment; no direct internet access | Firewall rules + service mesh policies |
| Database access | Per-service database credentials; read/write roles separated; no shared credentials | Credential rotation every 24 hours |
| Audit trail | Every internal API call logged with service identity, action, resource, and outcome | Tamper-proof append-only log with hash chaining |

### Blast Radius Containment

If a single service is compromised, the damage is contained:

- **Compromised mini-app runtime**: Cannot access payment databases, UPI credentials, or other mini-apps' data (sandbox isolation)
- **Compromised rewards engine**: Can only interact with rewards ledger; cannot initiate payments (no UPI/NFC permissions)
- **Compromised notification service**: Can send notifications but cannot read transaction data or modify records (write-only access)
- **Compromised settlement service**: Can view settlement data but requires dual-approval for any settlement modifications above threshold

### Key Rotation Schedule

| Key Type | Rotation Frequency | Method | Impact During Rotation |
|----------|-------------------|--------|----------------------|
| TDE database encryption keys | 90 days | Automatic via HSM; new key encrypts new data; old key decrypts existing data until background re-encryption completes | Zero downtime; transparent to application |
| mTLS service certificates | 48 hours (72h validity) | Automatic via certificate manager; new cert deployed to sidecar; graceful drain of connections using old cert | Zero downtime; brief dual-cert period |
| API keys (merchant) | 90 days | Warning at 60 days; hard revocation at 90 days; merchant must generate new key via portal | Merchant must update integration |
| JWT signing keys | 30 days | New key signs new tokens; old key validates existing tokens until expiry (15 min max) | Zero downtime; brief dual-key validation |
| NFC token keys | Managed by card network | Card network's token lifecycle management | No platform-side rotation needed |
| Encryption keys for PII fields | 180 days | Online re-encryption of encrypted columns during off-peak hours | Slightly elevated CPU during re-encryption |
