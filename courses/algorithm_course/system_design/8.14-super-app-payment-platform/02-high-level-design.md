...
| Monolith | Simpler deployment; lower latency for intra-process calls; easier debugging | Scaling Slowest part of the process when UPI peaks at 10x bill payment traffic; single deployment blocks all teams; any failure takes down entire platform |
...
| Option | Pros | Cons |
|--------|------|------|
| **Choreography-based saga** (chosen) | No central coordinator Slowest part of the process; each service reacts to events; natural fit for payment + reward + notification flow | Harder to trace full saga; requires compensating transactions for rollback |
| Orchestrator saga | Central visibility; easier to reason about flow | Orchestrator becomes Slowest part of the process and single point of failure at super-app scale |
...
## Data Flow: NFC Tap-to-Pay

```mermaid
%%{init: {'theme': 'neutral'}}%%
sequenceDiagram
    participant U as User Device
    participant HCE as HCE Module
    ...
    U->>HCE: User taps device on terminal
    HCE->>HCE: Select pre-computed cryptogram
    HCE->>T: APDU Response (tokenized card data + cryptogram)
    T->>CN: Authorization request (token + cryptogram + amount)
    CN->>TS: Validate token and cryptogram
    TS-->>CN: Token valid, map to real card
    CN->>CN: Authorize against issuing bank
    CN-->>T: Auth response (approved/declined + auth_code)
    T-->>U: Transaction result displayed

    par Async Reconciliation
        CN->>NFC: Settlement notification
        NFC->>FR: Post-transaction risk evaluation
        NFC->>NFC: Record transaction, update offline counter
        NFC->>U: Push notification with receipt
    end

    Note over HCE: If offline: consume pre-computed<br/>cryptogram, reconcile on next online session
```

**NFC Design Constraints**:

| Constraint | Value | Rationale |
|------------|-------|-----------|
| Max offline transactions | 5-10 per online session | Limits exposure from unvalidated offline taps |
| Max offline amount per tap | 5,000 INR | Regulatory cap for contactless without online auth |
| Cryptogram pre-computation batch | 10 cryptograms | Balance between availability and security exposure |
| Token refresh interval | 7 days before expiry | Ensures token is always valid for tap |
| Terminal response SLO | < 500ms | Payment terminal timeout standard |

---

## Data Flow: Account Aggregator Consent
...
sequenceDiagram
    ...
    participant FIU as FIU (Lender/Insurer)

    U->>SA: Request financial product (loan, insurance)
    SA->>SA: Check existing consents
    SA->>U: Show consent request (data categories, purpose, duration)
    U->>SA: Approve consent
    SA->>AA: Create consent artifact
    AA->>FIP: Forward consent for FIP approval
    FIP->>U: Redirect to bank auth (OTP/biometric)
    U->>FIP: Authenticate and approve
    FIP-->>AA: Consent approved
    AA-->>SA: Consent handle (ACTIVE)

    Note over SA,AA: Data fetch phase (separate from consent)

    SA->>AA: Request financial data (consent_handle, date_range)
    AA->>FIP: Fetch encrypted financial data
    FIP-->>AA: Encrypted data payload
    AA-->>SA: Encrypted data
    SA->>SA: Decrypt with session key
    SA->>FIU: Forward data for product evaluation
    FIU-->>SA: Eligible products with terms
    SA-->>U: Display eligible financial products
```

**Consent Lifecycle Rules**:
- Consent artifacts are immutable once created; changes require new consent
- User can revoke consent at any time; revocation propagates to AA and FIP within 60 seconds
- Data fetched under consent is encrypted end-to-end; AA acts as a blind relay
- Consent frequency determines how often data can be fetched (one-time, monthly, on-demand)
- Platform must delete fetched data when consent expires or is revoked

---

## Mini-App Lifecycle Architecture

```mermaid
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph Development["Developer Portal"]
        DEV[Developer] --> SUBMIT[Submit Mini-App Bundle]
        SUBMIT --> REVIEW[Automated Security Scan]
        REVIEW --> MANUAL[Manual Code Review]
        MANUAL --> APPROVE[Approval + Signing]
    end

    subgraph Runtime["User Device Runtime"]
        LAUNCH[User Taps Mini-App]
        LAUNCH --> FETCH[Fetch Bundle from CDN]
        FETCH --> SANDBOX[Create Sandboxed WebView]
        SANDBOX --> BRIDGE[JavaScript Bridge API]
        BRIDGE --> PAY_API[Payment API - Native Flow]
        BRIDGE --> PROFILE_API[Profile API - Consent Required]
        BRIDGE --> LOC_API[Location API - Permission Required]
    end

    subgraph Monitoring["Platform Monitoring"]
        WATCH[Resource Watchdog]
        WATCH --> CPU_Q[CPU Quota: 15% max]
        WATCH --> MEM_Q[Memory Quota: 150MB max]
        WATCH --> NET_Q[Network: Allowlisted domains only]
        WATCH --> KILL[Terminate if exceeded]
    end
    ...
    class DEV,SUBMIT,REVIEW,MANUAL,APPROVE dev
    class LAUNCH,FETCH,SANDBOX,BRIDGE,PAY_API,PROFILE_API,LOC_API runtime
    class WATCH,CPU_Q,MEM_Q,NET_Q,KILL monitor
```

**Mini-App Isolation Model**:

| Boundary | Mechanism | Failure Mode |
|----------|-----------|--------------|
| Process isolation | Separate WebView process per mini-app | Mini-app crash does not affect host app |
| Memory limit | 150MB hard cap with OOM killer | Exceeding limit terminates mini-app, shows error |
| CPU limit | 15% of device CPU, enforced via scheduler | Throttled; if sustained, terminated with warning |
| Network | Domain allowlist from manifest | Requests to non-allowed domains silently blocked |
| Storage | Isolated local storage, 10MB per mini-app | Over-limit writes rejected; cannot access host storage |
| Payment | Delegate to native payment flow | Mini-app never sees credentials or card data |
| Inter-app | No shared memory or messaging channels | Mini-apps are completely isolated from each other |
...
%%{init: {'theme': 'neutral'}}%%
flowchart LR
    subgraph Input["Transaction Aggregation"]
        TXN[(Transaction DB)] --> SNAP[Snapshot at Cutoff]
        SNAP --> AGG[Aggregate by Merchant]
    end

    subgraph Compute["Settlement Computation"]
        AGG --> FEE[Compute Platform Fees]
        FEE --> GST[Apply GST]
        GST --> HOLD[Compute Dispute Holds]
        HOLD --> NET[Net Settlement Amount]
    end

    subgraph Payout["Bank Transfer"]
        NET --> ROUTE{Amount > 2L?}
        ROUTE -->|Yes| RTGS[RTGS Transfer]
        ROUTE -->|No| NEFT[NEFT Transfer]
        RTGS --> CONFIRM[Bank Confirmation]
        NEFT --> CONFIRM
    end

    subgraph Reconciliation["Post-Settlement"]
        CONFIRM --> RECON[Reconciliation Check]
        RECON --> REPORT[Merchant Statement]
        RECON --> ALERT[Mismatch Alert]
    end

    classDef data fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef compute fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef payout fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef recon fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class TXN,SNAP,AGG data
    class FEE,GST,HOLD,NET compute
    class ROUTE,RTGS,NEFT,CONFIRM payout
    class RECON,REPORT,ALERT recon
```
