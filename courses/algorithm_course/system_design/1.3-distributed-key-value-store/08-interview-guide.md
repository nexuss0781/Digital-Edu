...
| "Redis is single-threaded - how would you improve throughput?" | Modern architecture awareness | "Single-threaded event loop wastes 31 of 32 cores. Thread-per-core design (DragonflyDB approach) partitions data across cores with no shared state, achieving 10-25x throughput. Cross-partition ops use lock-free queues. Trade-off: multi-key operations spanning cores need scatter-gather." |
| "How do you handle compaction latency spikes?" | Storage engine depth | "Compaction on the serving node competes for I/O with user requests. Two approaches: (1) Remote compaction -- offload merge work to separate compute nodes with shared storage, eliminating serving-path interference. (2) Rate-limited local compaction with I/O priority queues separating user reads from compaction I/O." |
| "Why would you separate keys from values in the LSM tree?" | Advanced storage design | "For large values (>1 KB), compaction rewrites the same value 10-30x across levels. Key-value separation stores values in a blob log and keys+pointers in the LSM. Compaction only rewrites tiny pointers, reducing write amplification from 30x to 3-5x. Trade-off: point reads need two I/Os and range scans lose spatial locality." |
...
| DragonflyDB throughput | 1M+ ops/sec | Thread-per-core on 32-core |
| KV-sep write amp | 3-5x | vs 10-30x standard LSM |
| Remote compaction p99 | Stable | No compaction-induced spikes |
