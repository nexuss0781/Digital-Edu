...
Dotted Version Vectors solution:
  ...
  Cutting off unnecessary steps rule: Can remove entries for nodes no longer in cluster
```
...
### Compaction Slowest part of the process: I/O Contention
...
### Slowest part of the process 1: Hot Partitions
...
### Slowest part of the process 2: Replication Lag
...
### Slowest part of the process 3: Tombstone Accumulation
...
### Slowest part of the process 4: Network Partitions
...
## Critical Component 5: Key-Value Separation (WiscKey Design)

### Why This Is Critical

Standard LSM compaction rewrites entire key-value pairs at each level. For workloads with large values (>1 KB), this creates excessive write amplification -- the same value is rewritten 10-30x across levels.

### How Key-Value Separation Works

```
Standard LSM Tree:
  MemTable → SSTable contains [key | value | metadata]
  Compaction rewrites full key-value pairs at each level
  Write amplification: 10-30x for 5 levels

Key-Value Separated (WiscKey/BlobDB):
  MemTable → SSTable contains [key | value_pointer | metadata]
  Values stored in separate blob log (vLog)
  Compaction rewrites only small key-pointer pairs
  Write amplification: 3-5x (10x reduction for 10KB+ values)

Value retrieval:
  1. Look up key in LSM tree → get value_pointer
  2. Read value from blob store at pointer offset
  3. Two I/O operations instead of one (trade-off)
```

### When to Use Key-Value Separation

| Value Size | Standard LSM | KV-Separated | Recommendation |
|-----------|-------------|-------------|----------------|
| < 256 bytes | 10-30x WA | Overhead > savings | Standard LSM |
| 256 bytes - 1 KB | 10-30x WA | ~5x WA | Break-even zone |
| 1 KB - 10 KB | 10-30x WA | 3-5x WA | **KV-separated** |
| > 10 KB | 10-30x WA | 2-3x WA | **KV-separated strongly preferred** |

### Blob Store Garbage Collection

```
Problem: Deleted/overwritten values leave dead entries in blob log

GC procedure:
  1. Scan blob log from tail
  2. For each entry, check if key still points to this blob offset
  3. If valid, copy to head of blob log
  4. If stale, skip (reclaim space)

Cost: Additional read I/O during GC
Mitigation: Run GC during low-traffic periods, limit GC bandwidth
```

---

## Critical Component 6: Remote Compaction

### Why This Is Critical

LSM compaction contends with user I/O on the same node, causing latency spikes. Remote compaction offloads this CPU- and I/O-intensive work to separate compute nodes.

```
Traditional (local) compaction:
  Serving node handles reads + writes + compaction
  Compaction spikes cause p99 latency to increase 5-10x
  Cannot isolate user I/O from compaction I/O

Remote compaction:
  Serving node: handles reads + writes only
  Compaction node: reads SSTables, merges, writes output
  SSTables stored on shared storage (accessible to both)
  Serving node picks up compacted files when ready

Requirements:
  - Shared/remote storage (object store, distributed filesystem)
  - Or: SSTables shipped to compaction node and back
  - Metadata service tracks SSTable versions
```

| Aspect | Local Compaction | Remote Compaction |
|--------|-----------------|-------------------|
| **p99 latency** | Spikes during compaction | Stable (no compaction I/O) |
| **Complexity** | Simple | Requires shared storage + coordination |
| **Cost** | Single node | Separate compute for compaction |
| **Recovery** | Compaction state is local | Compaction node failure is harmless (retry) |

---

## Summary: Critical Components and Bottlenecks

| Component | Critical Because | Main Slowest part of the process | Mitigation |
|-----------|-----------------|-----------------|------------|
| Consistent Hashing | Data distribution | Hot partitions | Virtual nodes, splitting |
| Vector Clocks | Conflict detection | Clock size growth | Cutting off unnecessary steps, dotted version vectors |
| LSM Compaction | Read performance | I/O contention | Rate limiting, scheduling, remote compaction |
| Replication | Durability | Lag | Quorum, read-your-writes |
| Tombstones | Correctness | Accumulation | TTL, compaction priority |
| Key-Value Separation | Write amplification | Blob GC overhead | GC scheduling, size threshold |
| Thread-per-core | Multi-core utilization | Cross-partition ops | Lock-free queues, partition-aware routing |
