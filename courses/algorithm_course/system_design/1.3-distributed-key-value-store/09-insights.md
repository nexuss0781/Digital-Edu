...
**Why it matters:** In a multi-master system, two clients can write to the same key simultaneously from different nodes. With last-write-wins (LWW), the higher timestamp wins and the other write is silently discarded -- if Client A adds item2 to a cart and Client B adds item3, one item vanishes. Vector clocks track per-node logical counters: {N1:2} vs {N2:1} are incomparable (neither dominates), proving the writes were concurrent. The system returns both versions as "siblings" and lets the application merge them (union: {item1, item2, item3}). The trade-off is complexity: vector clocks grow with the number of nodes that have written to a key, requiring Cutting off unnecessary steps strategies like dotted version vectors. CRDTs offer automatic merge for specific data types (sets, counters) but support a limited set of operations. The choice between LWW, vector clocks, and CRDTs depends on whether data loss is acceptable, tolerable with detection, or must be prevented by construction.
...
## Insight 10: Thread-Per-Core Architecture Replaces Single-Threaded Event Loops for Multi-Core Utilization

**Category:** Scaling
**One-liner:** The Redis-style single-threaded event loop wastes 95% of a 32-core server's capacity; thread-per-core shared-nothing designs (DragonflyDB, Seastar-based systems) achieve 10-25x throughput by eliminating cross-thread synchronization while partitioning data per core.

**Why it matters:** Redis's single-threaded model was a brilliant simplification in 2009: no locks, no race conditions, no complexity. But modern servers have 32-128 cores, and a single thread saturates one core at ~200K ops/sec while the other 31 sit idle. Vertical scaling means buying a faster clock, not more cores. Thread-per-core architecture assigns each CPU core its own data partition and event loop -- no shared state, no mutexes, no cache-line contention. Each core handles its own subset of keys independently. Cross-partition operations (like multi-key transactions) require inter-core messaging via lock-free queues. DragonflyDB demonstrates this approach, achieving over 1M ops/sec on commodity hardware. The trade-off is that operations spanning multiple partitions (MGET across 10 keys hashing to different cores) require scatter-gather coordination, adding latency compared to single-threaded execution where all keys are accessible instantly. For workloads dominated by single-key operations (which is most KV traffic), the throughput gain far outweighs the multi-key latency cost.

---

## Insight 11: Disaggregated Storage Separates Durability from Serving, Enabling Instant Failover

**Category:** Architecture
**One-liner:** Traditional KV stores couple serving and durability on the same node -- losing a node loses both capacity and data locality; disaggregated architectures write to a shared transaction log for durability and serve from in-memory state, enabling instant failover by attaching new compute to existing durable state.

**Why it matters:** In a traditional architecture, each node owns its data: the memtable, WAL, and SSTables live on local disk. When a node fails, the replacement must replay the WAL and rebuild in-memory state -- a process that can take minutes to hours depending on data volume. Disaggregated architecture separates these concerns: writes go to a distributed transaction log (a durable, replicated append-only log similar to a commit log service) for persistence, while serving nodes maintain in-memory state for reads. When a serving node fails, a new node attaches to the same log segment and rebuilds state from the transaction log -- or better, takes over from a warm standby that was already tailing the log. Recovery time drops from minutes to seconds. The storage layer (SSTable files) lives on shared storage (object store or distributed filesystem), accessible to any serving node. This enables independent scaling of compute (add serving nodes for throughput) and storage (add capacity without moving data), matching the elastic scaling model of cloud-native applications.

---

## Insight 12: Key-Value Separation Reduces Write Amplification from 30x to 3-5x for Large Values

**Category:** Data Structures
**One-liner:** Standard LSM compaction rewrites entire key-value pairs at each level; separating values into a log-structured blob store and keeping only keys+pointers in the LSM tree reduces write amplification proportionally to the value-to-key size ratio.

**Why it matters:** In a standard LSM tree, compaction at each level reads and rewrites all key-value pairs to produce sorted output. For a 1 KB value written to an LSM tree with 5 levels and a size ratio of 10, the same value is rewritten ~5 times during compaction -- 5 KB of I/O for 1 KB of user data (5x write amplification). With larger values (10 KB+), this amplification dominates disk bandwidth. Key-value separation (the WiscKey design, adopted by RocksDB's BlobDB and Badger) stores values in a separate log-structured blob store (vLog) while the LSM tree holds only keys and value pointers. Compaction now rewrites small key-pointer pairs (tens of bytes) instead of full values, reducing write amplification from 10-30x to 3-5x for workloads with values larger than 1 KB. The trade-off is read performance: point lookups now require two I/O operations (LSM for key → pointer, then blob store for value), and range scans lose spatial locality since values are scattered across the blob log. Garbage collection of the blob store adds operational complexity. The break-even point is roughly 1 KB values: below this, the overhead of indirection and GC exceeds the compaction savings.
