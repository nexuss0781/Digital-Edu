...
The deprecation of third-party cookies requires a dual-pipeline architecture that supports both identity-based and privacy-preserving bidding simultaneously.

### 8.1 Topics API Integration
...
flowchart LR
    ...
    subgraph DSP["DSP Bidder"]
        TC["Topic-Based\nCampaign Matcher"]
        CM["Contextual\nML Model"]
        BC["Bid\nCalculator"]
    end

    TP -->|"5 topics per epoch"| BA
    BA -->|"Topics in bid request\n(browsing_topics field)"| BR
    BR --> TC
    TC -->|"Matched campaigns\n(topic affinity scores)"| CM
    CM -->|"Cohort-level CTR\nprediction"| BC
    ...
    class TP,BA client
    class BR gateway
    class TC,CM,BC service
```

### 8.2 Dual Pipeline Architecture

The DSP operates two parallel evaluation pipelines, selected per bid request based on available signals:

| Signal Available | Pipeline | Feature Source | Model Type | Expected CPM Accuracy |
|---|---|---|---|---|
| Full user profile (cookie + consent) | Identity-based | Feature store user profile | User-level CTR/CVR model | Highest (baseline) |
| Topics API categories only | Cohort-based | Topic → campaign affinity scores | Cohort-level contextual model | ~70-80% of baseline |
| First-party publisher data only | Publisher-enriched | Publisher-provided segments | Publisher-segment model | ~60-75% of baseline |
| No user signals (full privacy) | Contextual-only | Page URL, content categories, device | Pure contextual model | ~40-60% of baseline |

### 8.3 Protected Audience API for Remarketing

```
Traditional remarketing flow (deprecated):
  User visits advertiser.com → DSP drops cookie → User visits publisher.com →
  DSP matches cookie → Bids high for known convertor

Privacy-preserving remarketing (Protected Audience):
  User visits advertiser.com → Browser stores interest group locally →
  User visits publisher.com → Browser runs on-device auction →
  Winning ad from interest group rendered → No user data leaves device

Key architectural implications:
  1. DSP submits bidding LOGIC to the browser, not bid RESPONSES
  2. Bidding function must be self-contained (no server calls during auction)
  3. Creative selection happens on-device with pre-loaded assets
  4. Reporting is aggregated (k-anonymity threshold) — no per-impression data
  5. DSP loses real-time control over individual bid decisions
```

---

## 9. CTV and Video Ad Delivery Extension

Connected TV (CTV) and video inventory have distinct architectural requirements compared to display:

| Dimension | Display (Banner) | Video (Pre-roll/Mid-roll) | CTV |
|---|---|---|---|
| **Bid request size** | ~2 KB | ~4 KB (VAST metadata) | ~5 KB (content metadata, device graph) |
| **Auction timeout** | 100ms | 200-500ms | 500ms-1s |
| **Creative delivery** | HTML/JS (client renders) | VAST XML → video player | VAST/SSAI (server-side ad insertion) |
| **Viewability** | Client-side SDK (IntersectionObserver) | Video player events (quartile tracking) | ACR (Automatic Content Recognition) |
| **Fraud profile** | Bot traffic, ad injection | Domain spoofing, forced-view bots | CTV-specific: app spoofing, SSAI manipulation |
| **CPM range** | $1-10 | $10-30 | $20-50+ |
| **User identity** | Cookie/Topics API | Cookie/Topics API | IP-based household graph, device ID |

### Server-Side Ad Insertion (SSAI) for CTV

```
Publisher content stream → SSAI stitching server → Unified stream to device
                                ↑
            Exchange auction (triggered by ad break marker in manifest)
                                ↑
            DSP bid with VAST creative URL

Key challenge: SSAI stitches the ad into the content stream server-side,
so the ad creative must be pre-transcoded to match the content stream's
bitrate ladder, codec, and resolution. This adds a creative preparation
step not present in display advertising.
```
