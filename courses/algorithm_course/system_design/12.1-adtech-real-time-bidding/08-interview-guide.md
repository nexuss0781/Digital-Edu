...
## 7. Mental Models for RTB System Design

### 7.1 The Latency Budget Decomposition

Every RTB design decision should be evaluated against the 100ms latency budget. Use this decomposition as a sanity check:

```
Total budget:                       100ms
  - Network RTT (exchange ↔ DSP):    20ms (physical limit)
  - Request parsing/validation:        2ms
  - Feature store lookup:             10ms (L1 hit) to 25ms (L3 fallback)
  - ML model inference:               15ms
  - Bid calculation + shading:         3ms
  - Response serialization:            2ms
  - Overhead/buffer:                   8ms

Key insight: every new feature or check added to the bid path
must "pay for itself" from this budget. A 5ms brand safety API call
means 5ms less for ML inference or feature lookup.
```

### 7.2 The Revenue Impact Calculator

When discussing trade-offs, quantify revenue impact to demonstrate business awareness:

```
Lost impressions/second = timeout_rate × QPS
Revenue loss/second = lost_impressions × win_rate × avg_CPM / 1000

Example: 1% timeout rate at 1M QPS with 20% win rate and $5 CPM:
  = 0.01 × 1,000,000 × 0.20 × $5.00 / 1000
  = $10/second = $864K/day

This makes latency optimization a direct revenue conversation,
not a vague "performance" discussion.
```

### 7.3 The Three-Tier Data Freshness Test

When deciding how to handle any data type in RTB, classify it:

| Tier | Staleness Tolerance | Example | Architecture |
|---|---|---|---|
| **Real-time** | <1 second | Budget remaining, bid price | In-process cache with sub-second refresh |
| **Near-real-time** | 5-60 seconds | Frequency cap counts, campaign status | Distributed cache with periodic sync |
| **Batch** | Minutes to hours | User profiles, ML models, fraud lists | Async pipeline with versioned deployment |

---

## 8. Red Flags in Candidate Answers

| Anti-Pattern | Why It's Wrong | What to Say Instead |
|---|---|---|
| "Use a distributed lock for budget deduction" | Adds 5-20ms per bid; serialization Slowest part of the process at 1M QPS | "Use budget leasing with local counters and periodic reconciliation" |
| "Store user profiles in a relational database" | Cannot serve 2B profiles at sub-10ms latency | "In-memory distributed KV store with tiered caching (L1/L2/L3)" |
| "Use strong consistency for frequency capping" | Cross-region consensus is physically impossible within 100ms budget | "Accept approximate counting with bounded over-delivery" |
| "Queue bid requests for processing" | Bid requests cannot be buffered; they expire immediately | "Process or drop immediately; no retry mechanism exists" |
| "Single centralized DSP for all regions" | 80ms cross-region RTT makes this impossible for EU/APAC inventory | "Edge-deployed bidder fleet with geo-local feature caches" |
| "Use second-price auctions" | Industry moved to first-price in 2019; ignoring this shows outdated knowledge | "First-price auction with bid shading to avoid overpayment" |
| "Run fraud detection asynchronously after bidding" | Bidding on fraudulent impressions wastes budget before detection | "Pre-bid IVT filtering within 5ms; post-bid forensics for SIVT" |

---

## 9. Key Metrics to Mention

Citing specific metrics demonstrates production awareness:

| Metric | Target | Why It Matters |
|---|---|---|
| **Bid rate** | 30-50% of received requests | Too low = wasting compute; too high = bidding on low-value inventory |
| **Win rate** | 15-25% of bids placed | Too low = underbidding; too high = likely overpaying |
| **Timeout rate** | <0.5% of bids | Every timeout = $0 revenue for that impression |
| **Budget pacing accuracy** | ±2% of daily target | Underspend = lost opportunity; overspend = advertiser trust damage |
| **Frequency cap accuracy** | ±1 impression | Over-delivery degrades UX; under-delivery wastes budget |
| **IVT detection rate** | >95% GIVT, >70% SIVT | Higher detection = less budget waste on fraud |
| **Impression discrepancy** | <5% SSP vs DSP count | >15% indicates tracking integration issue or fraud |
| **Model inference p99** | <15ms | Exceeding this compresses budget for other pipeline stages |
