...
## 6. Data Retention and Compliance Requirements

| Data Category | Retention Period | Regulatory Driver | Storage Tier |
|---|---|---|---|
| **Impression event logs** | 90 days raw; 2 years aggregated | Billing disputes, attribution windows, advertiser audit | Hot 7 days → warm 90 days → cold aggregated Parquet |
| **Bid-level logs** | 30 days (all bids); 90 days (wins only) | Bid shading model training, market analysis | Hot 7 days → warm 30 days → purge non-wins at 30 days |
| **User profiles / feature store** | TTL-based (30 days since last activity) | GDPR data minimization, storage cost | In-memory with SSD overflow; expire on inactivity |
| **Campaign metadata** | Duration of campaign + 7 years | Financial audit, tax compliance, advertiser contracts | Hot during flight → warm 1 year → cold archive |
| **Billing transactions** | 7 years | SOX compliance, financial audit requirements | Hot 90 days → warm 2 years → cold immutable archive |
| **Consent records** | Duration of consent + 5 years | GDPR Art. 7(1), CCPA, TCF audit trail | Encrypted at rest; append-only; never deleted |
| **Fraud detection logs** | 2 years | IVT audit, MRC accreditation, legal evidence | Warm 1 year → cold archive |
| **ads.txt / sellers.json snapshots** | 1 year (daily snapshots) | Supply chain verification audit trail | Warm 90 days → cold archive |
| **ML model artifacts** | 6 months (all versions); indefinite for production versions | Model reproducibility, bias audit, regulatory review | Object storage with versioning |
| **Creative assets and review decisions** | Duration of campaign + 2 years | Brand safety audit, legal disputes | Object storage; review metadata in relational DB |

### Privacy-Specific Retention Constraints

| Regulation | Constraint | System Impact |
|---|---|---|
| **GDPR (EU)** | Right to erasure; data minimization; purpose limitation | Crypto-shredding for user profiles; TTL-based expiry on feature store; consent check on every bid |
| **CCPA (California)** | Right to opt-out of sale; right to delete | Honor Global Privacy Control (GPC) signal in bid request; suppress user from feature store on opt-out |
| **COPPA (US)** | No behavioral targeting for children under 13 | Check `regs.coppa` flag; bid contextually only; no frequency capping or user-level features |
| **TCF 2.2 (Europe)** | Vendor-specific consent required per purpose | Parse `user.consent` TC string; verify DSP has consent for Purpose 1 (storage) and Purpose 4 (ad selection) before using user data |
| **Digital Services Act (EU)** | Transparency in ad targeting; ad repository | Log targeting criteria per impression; expose via public API for regulatory review |
