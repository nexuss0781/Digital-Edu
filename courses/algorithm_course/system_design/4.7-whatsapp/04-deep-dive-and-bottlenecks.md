...
## Critical Component 5: Multi-Device Architecture (2021+)

### Why This Is Critical

The original WhatsApp required the phone to proxy all messages to companion devices. If the phone went offline, web and desktop clients lost connectivity.

### Architecture Evolution

```
ORIGINAL MODEL (Phone-as-Proxy):
┌──────────┐      ┌──────────┐      ┌──────────┐
│  Contact │ ──── │  Phone   │ ──── │   Web    │
│          │      │ (Primary)│      │  Client  │
└──────────┘      └──────────┘      └──────────┘
Problem: Web offline when phone offline

CURRENT MODEL (Independent Devices):
┌──────────┐      ┌──────────┐
│  Contact │ ──── │  Phone   │  (Independent session)
│          │      └──────────┘
│          │      ┌──────────┐
│          │ ──── │   Web    │  (Independent session)
│          │      └──────────┘
│          │      ┌──────────┐
│          │ ──── │ Desktop  │  (Independent session)
└──────────┘      └──────────┘
Each device has own identity key + Signal sessions
```

### Device Linking Protocol

```
PROCESS: Link New Companion Device

1. INITIATION
   - New device generates its own identity key pair
...
2. PHONE AUTHENTICATION
   - Phone scans QR code
   - Phone signs companion's public key with its own identity key
   - Creates a chain of trust (phone vouches for companion)

3. KEY DISTRIBUTION
   - Phone sends signed companion key to server
   - Server adds companion to user's device list
   - Contacts are notified to create new sessions with companion

4. HISTORY TRANSFER (Optional)
   - Phone selectively transfers chat history
   - Transfer is end-to-end encrypted between phone and companion
   - Server never sees historical message content

CONSTRAINTS:
  - Maximum 4 companion devices + 1 phone
  - Each companion has independent encryption sessions
  - Senders encrypt once per recipient device (O(D) overhead)
```

### Channels Architecture (2023+)

WhatsApp Channels use a fundamentally different architecture from groups:

```
CHANNELS vs GROUPS:

| Aspect          | Groups (E2EE)        | Channels (No E2EE)     |
|─────────────────|──────────────────────|────────────────────────|
| Encryption      | Sender keys (E2EE)   | Server-encrypted       |
| Max size        | 1,024 members        | Unlimited followers    |
| Storage         | Store-and-forward    | Server-side persistent |
| Moderation      | Client-side only     | Server-side possible   |
| Fan-out         | Per-member encrypt   | Single broadcast       |
| Identity        | Phone-based          | Admin name (anonymous) |

DELIVERY ARCHITECTURE:
  - Admin posts update → server stores plaintext
  - Server pushes to followers (fan-out)
  - No sender-key distribution overhead
  - Server can enforce content policies
  - Updates auto-expire (default 30 days)
```
...
### Slowest part of the process 1: Connection Server Memory
...
### Slowest part of the process 2: Offline Queue Growth
...
### Slowest part of the process 3: Prekey Exhaustion
...
### Slowest part of the process 4: Group Message Fan-out
