...
### Trap Question: "How does multi-device work with E2EE if the server can't store messages?"

**Bad Answer:** "The phone relays messages to other devices"

**Good Answer:**
```
"The original design used phone-as-proxy, but the current
multi-device architecture (2021+) gives each companion device
its own identity key pair and independent Signal Protocol sessions.

Key mechanism:
1. DEVICE LINKING: Phone signs companion device's public key,
   creating a chain of trust (no central authority needed)

2. INDEPENDENT SESSIONS: Alice's phone and Alice's web have
   SEPARATE encryption sessions with each contact. No shared
   ratchet state -- no sync problems.

3. SENDER OVERHEAD: Messages must be encrypted once per
   recipient device (O(D)), but D is small (2-4 devices)

4. HISTORY TRANSFER: Since server has no history, the phone
   transfers past messages via E2E encrypted bulk transfer
   during linking. Server never sees historical content.

This means the web client works even when the phone is offline,
solving the biggest UX complaint of the original architecture."
```

### Trap Question: "WhatsApp Channels can have millions of followers -- how does that work with E2EE?"

**Bad Answer:** "They use sender keys like groups"

**Good Answer:**
```
"Channels deliberately DO NOT use E2EE. This is a conscious
architectural choice, not a limitation.

Why no E2EE:
- Channels are PUBLIC broadcasts, not private conversations
- Sender keys would require O(N) key distribution for millions
- Server needs to store and serve content (not store-and-forward)
- Content moderation requires server visibility

Architecture differences:
- Groups: E2EE with sender keys, 1024 max, store-and-forward
- Channels: Server-encrypted, unlimited followers, persistent storage

This shows that not every feature in a messaging app needs the
same security model. Matching security semantics to communication
semantics avoids imposing unnecessary overhead."
```
