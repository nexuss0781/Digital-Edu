...
flowchart TB
    subgraph Context["Main Context (LLM Window)"]
        ...
        FIFO (First-In-First-Out, like a line at a store)["Message Queue<br/>(FIFO (First-In-First-Out, like a line at a store), variable)"]
    end
    ...
    LLM -->|"Sends messages"| FIFO (First-In-First-Out, like a line at a store)
    ...
    class SYSTEM,PERSONA,HUMAN,FIFO (First-In-First-Out, like a line at a store) context
...
## Background Memory Processing Architecture

The background memory pattern (pioneered by LangMem and adopted across platforms) separates the write path from the conversation flow entirely:

```mermaid
flowchart TB
    subgraph Foreground["Foreground (User-Facing)"]
        direction TB
        CONV["Conversation Turn"]
        AGENT["Agent Loop"]
        FAST["Fast-Path Cache<br/>(Raw turn, immediate search)"]
        READ["Memory Reader<br/>(Retrieval)"]
    end

    subgraph Background["Background (Async)"]
        direction TB
        BGMGR["Background Memory Manager"]
        EXTRACT_BG["Extraction Pipeline"]
        DEDUP["Dedup & Reconciliation"]
        CONSOL_BG["Continuous Consolidation"]
    end
    ...
    classDef fg fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef bg fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef store fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class CONV,AGENT,FAST,READ fg
    class BGMGR,EXTRACT_BG,DEDUP,CONSOL_BG bg
    class VDB_S,GDB_S store
```

### Foreground vs Background Responsibilities

| Responsibility | Foreground (Sync) | Background (Async) |
|----------------|-------------------|---------------------|
| **Memory retrieval** | Query, rank, inject | - |
| **Fast-path cache** | Store raw turns for instant recall | - |
| **Entity extraction** | - | LLM-based extraction |
| **Embedding generation** | - | Batch embedding |
| **Deduplication** | - | Similarity-based dedup |
| **Graph updates** | - | Entity-relationship creation |
| **Consolidation** | - | Cluster and summarize |
| **Forgetting** | - | Decay and delete |

**Key insight:** The fast-path cache ensures that if a user says "my name is Alex" on turn N, the agent can recall it on turn N+1 even before formal extraction completes. This is implemented as a simple key-value store keyed by (user_id, session_id) containing the last K raw conversation turns, searchable by substring match.

---

## Letta V1 Stateful Agent Architecture

The Letta V1 architecture (2025-2026) evolves MemGPT's virtual context into a stateful agent runtime optimized for latest reasoning models:

```mermaid
flowchart TB
    subgraph Runtime["Stateful Agent Runtime"]
        direction TB
        LOOP["Agent Loop<br/>(ReAct-style)"]
        BLOCKS["Memory Blocks<br/>(Editable, typed)"]
        TOOLS_V1["Memory Tools<br/>(Read/Write/Search)"]
        CONV_API["Conversations API<br/>(Shared state across sessions)"]
    end

    subgraph Persistence["Persistent State"]
        CORE_MEM["Core Memory<br/>(Always in context)"]
        RECALL_MEM["Recall Memory<br/>(Conversation history)"]
        ARCHIVAL_MEM["Archival Memory<br/>(Vector DB)"]
    end
    ...
    class LOOP,BLOCKS,TOOLS_V1,CONV_API runtime
    class CORE_MEM,RECALL_MEM,ARCHIVAL_MEM persist
    class MCP_INT,CUSTOM ext
```
...
|---------|-------------|--------|
| **Editable Memory Blocks** | Named, typed blocks visible to and modifiable by the agent | Transparent memory management instead of implicit |
| **Conversations API** | Shared memory state across parallel user sessions | Multi-session coherence without manual sync |
| **Stateful Runtime** | Agent persists identity, behaviors, and learned preferences across deployments | Agents that genuinely learn and self-improve over time |
| **MCP Integration** | Standard protocol for memory tool access | Interoperability with any MCP-compatible framework |
