...
**Attack:** Adversary with access to stored embeddings attempts to reconstruct original text content.

```
Attack vector: Access embedding vectors → train inversion model →
reconstruct approximate original text from embeddings
```
...
| Factor | Assessment |
|--------|------------|
| **Feasibility** | Medium -- research demonstrates partial reconstruction possible |
| **Information recovered** | Topic, sentiment, key entities (not exact wording) |
| **Mitigation difficulty** | Medium -- requires architectural changes |

**Mitigations:**

1. **Embedding encryption at rest:** AES-256-GCM encryption of vector storage
2. **Dimensionality reduction:** Store PQ-compressed vectors (128 bytes) instead of full 1536-dim, reducing reconstruction fidelity
3. **Differential privacy noise:** Add calibrated noise to embeddings before storage (trade-off: ~3% recall degradation)
4. **Access controls on vector indices:** Separate read permissions for vector operations vs. raw vector access
5. **Embedding rotation:** Periodically re-embed with different models, invalidating trained inversion models

### Multi-Agent Memory Leakage

**Attack:** Agent in one workflow reads memories from a different workflow through shared scope misconfguration.

**Mitigations:**

1. **Scope enforcement at storage level:** Vector DB namespace isolation (not just application-level filtering)
2. **Memory scope audit:** Weekly automated scan for memories accessible outside their intended scope
3. **Read-only organizational memory:** Agents cannot write to shared organizational memory, only administrators can
4. **Handoff context validation:** Checksum verification on memory context passed between agents to detect tampering
