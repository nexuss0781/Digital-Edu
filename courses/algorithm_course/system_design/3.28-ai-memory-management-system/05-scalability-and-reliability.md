...
- Easy partition Cutting off unnecessary steps
...
## Case Study: Enterprise Customer Support Memory

### Scenario

A customer support platform with 2M agents serving 50M end users deploys AI memory to provide personalized, context-aware support across sessions.

### Scale Profile

| Metric | Value |
|--------|-------|
| **Active agents** | 2M support agents |
| **End users** | 50M with support histories |
| **Memories per user** | 500 average (support interactions) |
| **Daily memory writes** | 25M (new support interactions) |
| **Daily memory reads** | 200M (context retrieval per response) |
| **Peak QPS** | 8,000 reads, 1,500 writes |

### Architecture Decisions

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Sharding** | By customer_id (end user) | All support history for one customer on single shard |
| **Memory types** | Episodic (past tickets) + Semantic (preferences, account info) | Different retention and retrieval needs |
| **Extraction model** | Fine-tuned domain model | Higher accuracy for support-specific entities (product names, error codes) |
| **Consolidation** | Aggressive (7-day threshold) | Support context depreciates faster than personal assistant context |
| **Multi-agent sharing** | Shared scope per customer | Any agent serving a customer accesses the same memory |

### Results

| Metric | Before Memory | After Memory | Improvement |
|--------|---------------|--------------|-------------|
| **First-response resolution** | 35% | 52% | +49% |
| **Average handle time** | 12 min | 8 min | -33% |
| **Customer repeat explanations** | 3.2 per case | 0.8 per case | -75% |
| **Agent context load time** | Manual (45s) | Automatic (80ms) | -99.8% |

### Lessons Learned

1. **Domain-specific extraction beats general-purpose:** Fine-tuned extraction model for support entities (product SKUs, error codes, account types) achieved 93% accuracy vs 78% for generic GPT-4o-mini
2. **Aggressive consolidation is safe for support:** Support memories older than 7 days are rarely relevant; aggressive consolidation saved 60% storage with minimal impact
3. **Shared memory scope is essential:** Customers expect any agent to know their history -- private memory scopes would force re-explanation on agent handoff
