...
A composite metric tracking overall memory system effectiveness:
...
memory_health_score:
  components:
    extraction_quality:
      weight: 0.25
      metrics: [extraction_precision, extraction_recall, entity_accuracy]
      target: "> 85%"

    retrieval_quality:
      weight: 0.30
      metrics: [retrieval_relevance_at_5, retrieval_relevance_at_10, cache_hit_rate]
      target: "> 90% relevance"

    consolidation_quality:
      weight: 0.20
      metrics: [semantic_preservation, entity_coverage, fact_preservation]
      target: "> 85% preservation"

    system_performance:
      weight: 0.25
      metrics: [retrieval_latency_p95, write_latency_p95, error_rate]
      target: "latency < SLO, errors < 0.1%"
```

### Memory Drift Detection

Detects when memory quality degrades over time:

```yaml
drift_alerts:
  extraction_drift:
    metric: "weekly_extraction_precision"
    condition: "decrease > 5% over 2 weeks"
    action: "Review extraction model, check for data distribution shift"

  retrieval_drift:
    metric: "weekly_retrieval_relevance"
    condition: "decrease > 10% over 4 weeks"
    action: "Re-evaluate embedding model, check index health"

  consolidation_drift:
    metric: "consolidation_information_loss"
    condition: "increase > 15% over 2 weeks"
    action: "Review consolidation prompts, increase quality thresholds"

  memory_bloat:
    metric: "avg_memories_per_user"
    condition: "growth > 20% month-over-month with no proportional user growth"
    action: "Review extraction filters, tighten confidence thresholds"
```
...
graph_health:
  entity_coverage:
    description: "Percentage of extracted entities successfully stored in graph"
    target: "> 95%"
    alert: "< 90%"

  relationship_accuracy:
    description: "Accuracy of extracted relationships (sampled human review)"
    target: "> 80%"
    alert: "< 70%"

  graph_density:
    description: "Average edges per entity node"
    target: "3-10 edges"
    alert: "> 20 (traversal explosion risk) or < 1 (underutilized)"

  orphan_entities:
    description: "Entities with zero relationships"
    target: "< 5% of total entities"
    alert: "> 15%"
```
