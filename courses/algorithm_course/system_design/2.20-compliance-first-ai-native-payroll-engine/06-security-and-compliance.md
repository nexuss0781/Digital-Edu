...
## 9. AI Model Governance & EU AI Act (2025 Enforcement)

### 9.1 AI System Registration

As of 2025, the EU AI Act classifies payroll-affecting AI systems as **high-risk** under Annex III, Category 4 (employment, workers management, access to self-employment). Required documentation:

| Requirement | Implementation | Evidence |
|-------------|----------------|----------|
| **Technical documentation** | Model cards for each AI component (NER, LLM extraction, confidence scorer) | Versioned model documentation in compliance repository |
| **Quality management system** | Documented development, testing, and monitoring procedures | CI/CD pipeline with quality gates, test coverage reports |
| **Risk management** | Identified risks, mitigations, residual risk acceptance | Risk register with quarterly reviews, mitigation evidence |
| **Data governance** | Training data documentation, bias assessment | Data provenance records, demographic impact analysis |
| **Logging and traceability** | Automatic logging of all AI decisions | Immutable audit log with model version, inputs, outputs, confidence |
| **Human oversight measures** | Meaningful human control over AI outputs | Approval workflow analytics, override rate tracking |
| **Accuracy and robustness** | Performance benchmarks, adversarial testing | Quarterly extraction accuracy reports, red team results |

### 9.2 Bias Detection for Payroll AI

```
FAIRNESS MONITORING:

APPLICABLE WHEN AI INFLUENCES:
  - Overtime classification (exempt vs non-exempt recommendation)
  - Benefits eligibility determination
  - Garnishment priority interpretation
  - Compensation benchmarking suggestions

MONITORING METRICS:
  - Demographic parity: P(favorable_outcome | group_A) ≈ P(favorable_outcome | group_B)
  - Equalized odds: Equal true positive/false positive rates across groups
  - Protected categories: gender, race, age, disability, national origin

IMPLEMENTATION:
  1. Aggregate AI recommendation outcomes by demographic group (quarterly)
  2. Compute disparate impact ratio: min(group_rate) / max(group_rate)
  3. If ratio < 0.8 (four-fifths rule): flag for investigation
  4. Generate fairness report for compliance team
  5. If confirmed bias: retrain model, adjust prompts, or restrict AI scope

NOTE: Payroll CALCULATION is deterministic and rule-based, so bias applies only
to AI-recommended classifications and interpretations, not to the math itself.
```

### 9.3 Cross-Border Data Transfer Controls

| Transfer Scenario | Legal Basis | Technical Control |
|-------------------|-------------|-------------------|
| EU employee data processed in EU | GDPR Art. 6 (contract) | Data residency enforcement, region-locked processing |
| EU employee data accessed by US admin | Standard Contractual Clauses (SCCs) | Just-in-time access with audit, data minimization |
| AI model trained on multi-region data | Legitimate interest + DPIA | Federated learning or anonymized aggregate training |
| Audit data shared with regulators | Legal obligation | Encrypted transfer, access-controlled portals |
