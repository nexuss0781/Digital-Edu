...
3. Get pay run calculations
   ...
   - Strategy: Partition Cutting off unnecessary steps
...
## 7. Continuous Payroll Architecture

### 7.1 Event-Driven Payroll Processing

Traditional batch payroll calculates once per pay period. Continuous payroll recalculates on every relevant event (time approval, rate change, election update), maintaining a running net pay estimate.

```mermaid
flowchart LR
    subgraph Events["Triggering Events"]
        TimeApproval[Time Approved]
        RateChange[Rate Change]
        ElectionUpdate[Election Update]
        RuleActivation[Rule Activated]
        LocationChange[Location Change]
    end

    subgraph StreamProcessor["Stream Processing"]
        EventRouter[Event Router]
        DeltaCalc[Delta Calculator<br/>Incremental]
        RunningTotal[Running Total<br/>Accumulator]
        AnomalyCheck[Anomaly Checker]
    end

    subgraph State["State Store"]
        EmployeeState[(Current Period<br/>Running Totals)]
        RuleCache[(Active Rules<br/>Cached)]
    end

    subgraph Output["Outputs"]
        EWAEngine[EWA Availability<br/>Real-time]
        PreviewAPI[Pay Preview<br/>API]
        BatchSync[Batch Reconciliation<br/>At Period End]
    end

    Events --> EventRouter
    EventRouter --> DeltaCalc
    DeltaCalc --> RunningTotal
    RunningTotal --> AnomalyCheck
    DeltaCalc --> State
    AnomalyCheck --> Output
    ...
    class TimeApproval,RateChange,ElectionUpdate,RuleActivation,LocationChange event
    class EventRouter,DeltaCalc,RunningTotal,AnomalyCheck process
    class EmployeeState,RuleCache state
```

### 7.2 Continuous vs Batch Reconciliation

```
CONTINUOUS PAYROLL GUARANTEE:

Rule that never changes:
  running_total == batch_recalculation (within rounding tolerance)

Reconciliation Protocol:
  1. Continuous stream maintains running gross, taxes, deductions per employee
  2. At period close, batch engine recalculates from scratch using immutable snapshot
  3. Compare: continuous_net vs batch_net
  4. If |delta| <= $0.01: Accept batch result (authoritative)
  5. If |delta| > $0.01: Flag for investigation, use batch result
  6. Log reconciliation result for audit trail

Benefits:
  - Real-time EWA availability without batch dependency
  - Instant pay preview for employees and admins
  - Early detection of configuration errors (catch issues mid-period)
  - Batch remains authoritative for compliance (deterministic, auditable)
```

### 7.3 Global Payroll Consolidation

| Challenge | Solution | Architecture Pattern |
|-----------|----------|---------------------|
| **Multi-currency** | Calculate in local currency, consolidate in reporting currency | Dual-amount storage (local + reporting), rate-at-calculation |
| **Legal entity isolation** | Separate payroll runs per entity, consolidated reporting | Entity-partitioned databases with cross-entity aggregation views |
| **Cross-border transfers** | Employee moves between countries mid-year | Split-period calculation, dual-jurisdiction tax, treaty application |
| **Statutory reporting per country** | Country-specific report formats and deadlines | Pluggable report generators, country-specific compliance modules |
| **Intercompany cost allocation** | Shared employees across entities | Time allocation engine, transfer pricing rules, automatic journal entries |

## AI Release Ladder

| Stage | Description | Gate Criteria |
|-------|-------------|---------------|
| 1. Offline Evaluation | Benchmark against ground truth dataset | Meets baseline metrics |
| 2. Shadow Mode | Run parallel to production, compare outputs | No regression on key metrics |
| 3. Canary (1-5%) | Limited traffic, human review sampling | Error rate < threshold |
| 4. Human-Reviewed Production | Full traffic, human spot-checks | Acceptance rate > target |
| 5. Autonomous Production | AI acts within approved boundaries | Continuous monitoring + drift detection |
| 6. Instant Rollback | One-click revert to previous version | < 5 min rollback time |
