...
## 8. AI Model Observability (EU AI Act Compliance)

### 8.1 Model Performance Tracking

```
# Model Registry Metrics
payroll_ai_model_version{model_name, version}
payroll_ai_model_deployment_timestamp{model_name}
payroll_ai_model_training_data_size{model_name}

# Extraction Quality (Rolling 30-Day Window)
payroll_extraction_precision_30d{jurisdiction_type}
payroll_extraction_recall_30d{jurisdiction_type}
payroll_extraction_f1_score_30d{jurisdiction_type}

# Drift Detection
payroll_ai_confidence_distribution_shift{model_name}
payroll_ai_input_feature_drift{feature_name}
payroll_ai_output_distribution_change{rule_type}

# Fairness Monitoring (Quarterly)
payroll_ai_disparate_impact_ratio{protected_category, outcome}
payroll_ai_demographic_parity_gap{protected_category}
```

### 8.2 Compliance Observability Dashboard
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  MODEL HEALTH                        COMPLIANCE STATUS          │
│  ├── NER Model v3.2: ✓ Healthy      ├── EU AI Act: ✓ Compliant │
│  ├── LLM Extractor v2.1: ✓ Healthy  ├── NYC LL144: ✓ Audited  │
│  ├── Confidence Scorer v1.8: ✓      ├── EEOC: ✓ No Findings   │
│  └── Drift Score: 0.02 (< 0.1)     └── SOX AI: ✓ Documented  │
│                                                                 │
│  HUMAN OVERSIGHT METRICS             AI DECISION LOG            │
│  ├── Rules auto-approved: 0%        ├── Entries today: 1,245   │
│  ├── Human approval rate: 92%       ├── Storage used: 45 GB    │
│  ├── Override rate: 3%              ├── Retention: 10 years    │
│  └── Avg review time: 3.2 hours    └── Integrity: ✓ Verified  │
│                                                                 │
│  BIAS MONITORING (Q1 2026)                                      │
│  ├── Disparate Impact (gender): 0.94 (> 0.8 ✓)                │
│  ├── Disparate Impact (age): 0.91 (> 0.8 ✓)                   │
│  └── Next audit: 2026-04-01                                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 8.3 EWA-Specific Monitoring

```
# Earned Wage Access Metrics
payroll_ewa_requests_total{tenant, status}
payroll_ewa_amount_requested_dollars{tenant, bucket}
payroll_ewa_estimation_accuracy{variance_bucket}
payroll_ewa_negative_net_pay_prevented_total{tenant}
payroll_ewa_state_compliance_check_total{state, result}
```
