...
### Question 7: "How would you handle Earned Wage Access (on-demand pay)?"

**Trap**: Treating it as a simple "pay early" feature

**Good Answer**:
> "EWA fundamentally changes the calculation model from end-of-period to mid-period estimation:
> 1. **Real-time net pay estimation**: Must conservatively estimate taxes, deductions, and garnishments for the partial period
> 2. **Safety margin**: Apply 10-15% buffer because mid-period estimates are inherently uncertain
> 3. **Garnishment priority**: EWA cannot advance money that would otherwise go to garnishments — legal liability
> 4. **State compliance**: Multiple states now regulate EWA (CA, NV, CT, MO) with registration, fee disclosure, and repayment rules
> 5. **Reconciliation**: At period end, actual net pay minus EWA advances = final deposit. Track carefully to avoid negative net pay.
> 6. **Architecture**: Continuous payroll stream maintains running totals; EWA engine queries running state rather than recalculating from scratch."

### Question 8: "The EU AI Act just took effect. What changes to your architecture?"

**Trap**: Saying "we already have human review so we're compliant"

**Good Answer**:
> "The EU AI Act classifies employment-affecting AI as high-risk. Beyond human review, we need:
> 1. **AI system registration**: Each AI component (NER, LLM extraction, confidence scorer) needs a model card with documented performance
> 2. **Technical documentation**: Training data provenance, bias assessments, adversarial test results
> 3. **Conformity assessment**: Third-party audit of our AI system before deployment
> 4. **Incident reporting**: Serious incidents (wrong rule affecting many employees) must be reported to authorities
> 5. **Logging retention**: AI decision logs must be kept for the system's lifetime or 10 years
> 6. **Bias monitoring**: Quarterly fairness analysis for any AI that influences compensation outcomes
> 7. **Right to explanation**: Employees can demand a human-understandable explanation of how AI affected their pay
>
> Key insight: Human-in-the-loop is necessary but not sufficient. The Act requires end-to-end governance."

---

## System Comparison Matrix

| Dimension | AI-Native Payroll | Traditional SaaS Payroll | Bureau Service (ADP Full-Service) |
|-----------|-------------------|--------------------------|-----------------------------------|
| **Rule updates** | AI extracts + human approves (hours) | Vendor pushes updates (weeks) | Bureau handles everything |
| **Jurisdiction coverage** | Dynamic, AI-monitored | Static, vendor-maintained | Comprehensive but opaque |
| **Customization** | High (tenant-specific rules) | Low-Medium (configuration) | Very Low (standardized) |
| **Explainability** | Line-by-line with citations | Basic pay stub | Minimal |
| **Infrastructure cost** | High (GPU clusters, self-hosted AI) | Low (SaaS subscription) | None (outsourced) |
| **Compliance ownership** | Shared (AI + human) | Mostly vendor | Bureau assumes liability |
| **Time to market** | Slow (build AI infra) | Fast (subscribe) | Immediate |
| **Best for** | Multi-jurisdiction, compliance-heavy | Standard payroll needs | Companies wanting zero payroll ops |
...
| EU AI Act | Article 6, Annex III (High-risk AI systems) — enforcement began 2025 |
| Payroll Compliance | DOL FLSA Fact Sheets |
| Tax Engine APIs | Symmetry Tax Engine Documentation |
| Earned Wage Access | CFPB Advisory Opinion 2020-1, state-specific EWA statutes |
| Global Payroll | OECD Pillar Two model rules for global minimum tax |
| AI Governance | NIST AI Risk Management Framework (AI RMF 1.0) |
