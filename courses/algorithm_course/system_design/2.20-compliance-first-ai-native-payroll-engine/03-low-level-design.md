...
Time Complexity (Speed of the algorithm): O(n × m) where n = sections, m = avg extraction time per section
Space Complexity (Memory usage of the algorithm): O(n) for storing sections and extracted rules
```
...
Time Complexity (Speed of the algorithm): O(j × r) where j = jurisdictions, r = rules per jurisdiction
Space Complexity (Memory usage of the algorithm): O(l) where l = number of calculation lines
```
...
Time Complexity (Speed of the algorithm): O(j) where j = number of jurisdictions
Space Complexity (Memory usage of the algorithm): O(j) for jurisdiction data
```
