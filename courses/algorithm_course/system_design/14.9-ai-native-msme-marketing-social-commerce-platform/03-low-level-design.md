...
erDiagram
    ...
    WHATSAPP_BROADCAST {
        ...
        uuid msme_id FK
        string template_id
        string[] recipient_segments
        int total_recipients
        int delivered
        int read
        int replied
        int blocked
        float quality_impact_score
        timestamp sent_at
    }
    ...
    CATALOG_PRODUCT {
        ...
        string[] image_urls
        string availability
        jsonb platform_sync_status
        timestamp last_synced_at
    }

    MSME ||--o{ WHATSAPP_CONVERSATION : receives
    MSME ||--o{ WHATSAPP_BROADCAST : sends
    MSME ||--o{ CATALOG_PRODUCT : sells
    CATALOG_PRODUCT ||--o{ CREATIVE : featured_in
```
...
```
VideoConfig {
    duration_seconds: int,          // target: 8-15s for Reels/Shorts
    motion_template: string,        // "zoom_in", "pan_left", "parallax", "rotate_360"
    audio: {
        type: "trending" | "royalty_free" | "none",
        track_id: string,           // from audio library
        volume: float,              // 0.0 - 1.0
        beat_sync: boolean          // sync transitions to audio beats
    },
    text_animations: [
        {
            text: string,
            language: string,
            entrance: "fade_in" | "slide_up" | "typewriter" | "bounce",
            timing_seconds: float,   // when to appear relative to video start
            duration_seconds: float,
            position: "center" | "top_safe" | "bottom_safe"  // safe zones per platform
        }
    ],
    hook: {
        type: "product_reveal" | "price_reveal" | "before_after" | "countdown",
        duration_seconds: float     // first 2 seconds optimized for viewer retention
    },
    loop_optimization: {
        enabled: boolean,
        end_frame_matches_start: boolean,  // seamless loop
        avoid_natural_endings: boolean     // don't add fade-out or "The End" cues
    },
    platform_overrides: {
        instagram_reels: { trending_audio: true, caption_burn_in: true },
        youtube_shorts: { original_audio_preferred: true, title_seo: true },
        whatsapp_status: { max_duration_seconds: 30, no_audio: false }
    }
}
```

### WhatsApp Broadcast Template Schema

```
BroadcastTemplate {
    template_id: string,             // WhatsApp-assigned template ID
    name: string,                    // human-readable template name
    language: string,                // ISO 639-1 (e.g., "hi", "ta", "en")
    ...
    components: [
        {
            ...
            content: string | MediaSpec
        },
        {
            type: "BODY",
            text: string,            // with {{1}}, {{2}} variable placeholders
            variables: VariableSpec[]
        },
        {
            type: "FOOTER",
            text: string             // e.g., "Reply STOP to unsubscribe"
        },
        {
            type: "BUTTONS",
            buttons: [
                {
                    ...
                    text: string,
                    url_suffix: string    // for URL type; appended to base URL
                }
            ]
        }
    ],
