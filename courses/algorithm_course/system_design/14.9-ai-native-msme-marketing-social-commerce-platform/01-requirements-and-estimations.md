...
| FR-13 | **WhatsApp conversational commerce** — Integrate with WhatsApp Business API to manage product catalogs, automate order-taking via AI-powered chat, send broadcast campaigns to opted-in customers, and handle click-to-WhatsApp ad flows that convert social impressions into direct conversations | Catalog sync with ≤ 5 min latency; AI chat handles 80%+ of common queries (price, availability, delivery) without human intervention; broadcast quality rating maintained above "Medium" tier; conversation-to-order conversion tracking |
| FR-14 | **Short-form video commerce** — Generate platform-optimized vertical videos (Reels, Shorts, TikTok) from static product photos using AI motion graphics, trending audio integration, and shoppable product tags; support livestream shopping session scheduling and promotion | Video generation ≤ 90s; trending audio library refreshed daily; product tag click-through tracking; livestream scheduling with automated pre-event social promotion across connected platforms |
| FR-15 | **Hyperlocal marketing** — Target customers within configurable geographic radius (1–25 km) of MSME's physical location using platform geo-targeting APIs and IP-based audience estimation; support local inventory ads, neighborhood-level demand prediction, and event-triggered promotions (rain → umbrella ads, lunch hour → food delivery) | Geo-fence creation with map-based UI; local inventory sync from POS/catalog; weather and event trigger integration; walk-in attribution estimation via QR codes |
| FR-16 | **Voice commerce integration** — Accept voice-based marketing briefs in regional languages for MSMEs who prefer speaking over typing; generate audio content for WhatsApp voice broadcasts; support voice-based approval workflow | Speech-to-text in 12+ Indian languages; voice brief to content generation pipeline; audio clip generation for WhatsApp voice notes; voice-activated approval ("approved" / "change the color") |
...
| WhatsApp Business accounts managed | 200K (40% of active MSMEs) |
| WhatsApp conversations per day | 500K (customer-initiated + business-initiated) |
| Short-form videos generated per day | 225K (15% of content mix, growing to 45%) |
| Hyperlocal-enabled MSMEs | 100K (with geo-fence configurations) |
| Product catalog entries (across all MSMEs) | 10M (synced to WhatsApp + Instagram Shop + Facebook Shop) |
...
```
WhatsApp Business API tiers (by quality rating):
  Tier 1 (new account): 1,000 business-initiated messages/day
  Tier 2 (good quality): 10,000 messages/day
  Tier 3 (high quality): 100,000 messages/day
  Unlimited (excellent quality): no cap

Platform's aggregate WhatsApp messaging:
  Active MSMEs using WhatsApp: 200,000 (40% of 500K)
  Average broadcast messages per MSME per week: 3
  Daily broadcast volume: 200,000 × 3 / 7 ≈ 85,700 messages/day

  Per-MSME: 3 messages/week well within Tier 1 (1,000/day)
  Platform aggregate: 85,700/day across all MSME accounts
  Constraint: each message goes through the MSME's own account, so
  platform-level rate is sum of per-MSME rates (not a shared pool)

WhatsApp catalog sync:
  200,000 MSMEs × 50 products avg = 10M product entries
  Catalog update frequency: daily delta sync
  Daily catalog API calls: ~200,000 (one sync per MSME)
```

### Latency Budget Breakdown

```
Content Generation Critical Path:
  ┌─────────────────────────────────────────────────────────────┐
  │ Brief parsing + input validation              │   200 ms    │
  │ Product understanding (GPU)                   │  3,200 ms   │
  │ Layout generation (GPU)                       │  4,500 ms   │
  │ Visual rendering (GPU)                        │  8,000 ms   │
  │ Quality gate                                  │  1,500 ms   │
  │ Storage + notification                        │  1,800 ms   │
  │ ────────────────────────────────────────────── │ ────────── │
  │ Total (static image)                          │ ~19,200 ms  │
  │ SLO headroom (p95 ≤ 30s)                      │ 10,800 ms   │
  └─────────────────────────────────────────────────────────────┘

Text generation (2,800 ms) runs in parallel with layout + visual rendering.

Publishing Critical Path:
  ┌─────────────────────────────────────────────────────────────┐
  │ Schedule trigger + fetch                      │    50 ms    │
  │ Pre-publish validation (token, freshness)     │   200 ms    │
  │ Platform adaptation (format, caption)         │   500 ms    │
  │ Platform API call (upload + confirm)          │  6,000 ms   │
  │ Post-publish actions (tracking, notify)       │  1,500 ms   │
  │ ────────────────────────────────────────────── │ ────────── │
  │ Total per platform                            │ ~8,250 ms   │
  │ SLO headroom (p95 ≤ 15s)                      │  6,750 ms   │
  └─────────────────────────────────────────────────────────────┘
```
...
|---|---|
| No fully autonomous publishing (always require MSME approval) | Brand safety risk outweighs automation benefit; one bad AI post can damage MSME reputation permanently |
| No cross-MSME content sharing | Each MSME's content is unique to their brand; sharing templates or creatives between competitors would destroy trust |
| No real-time bidding integration | MSMEs' budgets are too small for RTB; platform-managed campaigns (where the social platform does RTB) are more cost-effective |
| No sentiment analysis of customer comments | Out of scope; this is a marketing creation platform, not a social listening or CRM tool |
| No credit/lending features | Financial services require separate licensing and compliance; ad spend facilitated through platform-native billing only |

### Failure Scenario Capacity

| Scenario | Additional Load | Buffer Required |
|---|---|---|
| Festival spike (Diwali, 3 days) | 5–10x content generation | 1,170 GPUs peak (reduced to 300 with pre-generation) |
| Platform API outage (1 platform, 2 hours) | Retry queue accumulation: 50K posts | 10 GB retry buffer; catch-up publishing at 2x rate |
| Partnership onboarding burst (10K MSMEs/day) | 10x onboarding pipeline; brand kit synthesis spike | Pre-provisioned onboarding pool; staggered setup over 48 hours |
| Model quality regression | 2–3x regeneration rate | Quality gate threshold triggers automatic rollback to previous model |
| WhatsApp quality rating cascade | Broadcast pause for 1,000+ MSMEs | Automated recovery protocol; alternative channels (Instagram DM, SMS) |
| Cross-platform algorithm change | 3x content regeneration requests | Queue regeneration as batch; prioritize scheduled content |
