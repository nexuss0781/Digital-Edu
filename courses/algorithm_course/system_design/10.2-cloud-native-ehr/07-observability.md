...
### 2.4 AI Documentation Metrics

```
Ambient AI Pipeline:
  ehr.ai.encounters_processed              {facility}               [counter]
  ehr.ai.note_generation_latency           {specialty}              [histogram]
  ehr.ai.hallucination_flags               {risk_level}             [counter]
  ehr.ai.attestation_time                  {specialty}              [histogram]
  ehr.ai.attestation_edit_rate             {specialty}              [gauge]
  ehr.ai.notes_rejected                    {facility, reason}       [counter]
  ehr.ai.asr_word_accuracy                 {language}               [gauge]
  ehr.ai.model_inference_latency           {model_version}          [histogram]

TEFCA Exchange:
  ehr.tefca.queries_sent                   {qhin, query_type}       [counter]
  ehr.tefca.queries_received               {qhin, query_type}       [counter]
  ehr.tefca.response_latency               {qhin}                   [histogram]
  ehr.tefca.patient_match_rate             {qhin}                   [gauge]
  ehr.tefca.consent_denials                {qhin, reason}           [counter]
  ehr.tefca.documents_exchanged            {direction, format}      [counter]
```
...
Alert: AI Hallucination Rate Spike
  condition: ai.hallucination_flags{risk_level="critical"} > 5x baseline for 30 min
  severity: P2
  action: Alert clinical informatics + consider pausing AI pipeline for review

Alert: TEFCA QHIN Connectivity Loss
  condition: tefca.response_latency{qhin=X} > 30s for 5 minutes OR error rate > 50%
  severity: P3
  action: Alert integration team; switch to cached external records
```
