...
## 6. Deep Dive: Ambient AI Documentation Pipeline

### 6.1 Pipeline Architecture

With ambient clinical documentation reaching 100% health system adoption, the AI documentation pipeline is a critical subsystem operating within strict clinical safety guardrails.

```
Processing Pipeline (per encounter):

Stage 1: Audio Capture (continuous during encounter)
  Input: Encrypted audio stream from exam room or mobile device
  Output: Time-stamped, speaker-diarized audio chunks
  Latency: Real-time streaming
  Privacy: Audio encrypted in transit (TLS 1.3), never persisted to disk

Stage 2: Medical ASR (near-real-time)
  Input: Audio chunks
  Output: Timestamped transcript with speaker labels
  Accuracy target: > 95% word accuracy for medical terminology
  Key challenge: Accented speech, overlapping speakers, ambient noise
  Terminology: Custom medical vocabulary (drug names, procedures, anatomy)

Stage 3: Clinical NLP Extraction (post-encounter)
  Input: Full encounter transcript
  Output: Structured clinical data elements:
    - Chief complaint and HPI
    - Review of systems (positive and negative findings)
    - Physical exam findings
    - Assessment (diagnoses with ICD-10/SNOMED codes)
    - Plan (orders, follow-up, patient education)
    - Medications discussed (new, continued, discontinued)
    - Allergies mentioned
  Model: Fine-tuned clinical LLM with FHIR-aware output formatting

Stage 4: Hallucination Detection (automated)
  Input: Extracted clinical data + patient's existing chart
  Checks:
    - Medication verification: Are extracted meds in RxNorm? Dose plausible?
    - Allergy consistency: Does extracted allergy match or contradict chart?
    - Diagnosis plausibility: Is assessment consistent with documented symptoms?
    - Temporal consistency: Do referenced dates/events match encounter history?
  Output: Confidence scores per extracted element; flagged inconsistencies

Stage 5: Note Generation (automated)
  Input: Extracted data + encounter template (specialty-specific)
  Output: FHIR-compliant DocumentReference with:
    - meta.tag: "ai-generated", model version, confidence score
    - relatesTo: linked to encounter
    - Inline annotations marking low-confidence or flagged content

Stage 6: Attestation (human-in-the-loop)
  Input: AI-generated note in provider's attestation queue
  Provider actions: Review, edit, accept, reject
  Output: Attested DocumentReference with provider digital signature
  SLA: Note available for attestation < 3 minutes post-encounter
  Critical: Unatested AI notes are NEVER committed to the CDR as final
```

### 6.2 Hallucination Risk Classification

```
Risk Categories:

Critical (hard block, cannot attest without resolution):
  - Medication not found in RxNorm or patient's known medications
  - Allergy that contradicts patient's documented allergy list
  - Diagnosis requiring high-risk intervention (surgery, chemotherapy)
    without supporting documented findings

High (soft block, highlighted with explanation):
  - Dosage outside therapeutic range for patient's age/weight
  - Symptom severity inconsistent with vital signs
  - Family history that doesn't match prior documentation

Medium (annotation, no block):
  - Social history details not previously documented
  - Review of systems findings with low confidence score
  - Plan items that may duplicate existing active orders

Low (informational):
  - Minor formatting or terminology preferences
  - Alternative code mappings available
```
...
| B9 | Ambient AI note generation latency | Post-encounter note must be in attestation queue < 3 min | **Medium** |
| B10 | TEFCA cross-QHIN query latency | Multi-hop QHIN queries for patient discovery + document retrieval | **High** |
...
**B9: Ambient AI Note Generation Latency**
- Pipeline begins processing during the encounter (streaming ASR), not after
- Clinical NLP extraction parallelized across note sections (HPI, ROS, Assessment, Plan)
- Pre-loaded patient context eliminates chart lookup during hallucination detection
- GPU-accelerated inference for clinical LLM; model quantization for latency optimization

**B10: TEFCA Cross-QHIN Query Latency**
- Cache recent cross-QHIN query results per patient (TTL 24h for treatment context)
- Pre-fetch external records for scheduled patients (batch query the night before)
- Parallel QHIN queries when patient has records at multiple organizations
- Progressive response: return local data immediately, append TEFCA results as they arrive
...
| Ambient AI pipeline failure | Note generation timeout | Clinician documents manually; encounter audio retained for retry | AI documentation unavailable |
| TEFCA QHIN connectivity loss | QHIN health check failure | Local records available; external records cached if previously retrieved | Cross-org data unavailable |

### 8.2 Clinical Safety During System Downtime
