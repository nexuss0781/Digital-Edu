...
### 4.3 AI Workload Scaling

```
Ambient AI Documentation Scaling:

Concurrent encounters with ambient AI: 200,000 at peak
Audio ingestion: 200,000 * 15 min avg encounter = 50,000 hours audio/day
ASR throughput: ~3,500 concurrent transcription streams
Clinical NLP inference: GPU-accelerated, batch up to 100 notes/second
Hallucination detection: CPU-bound chart lookups, ~500 checks/second

Scaling strategy:
  - ASR: horizontally scaled, stateless workers consuming audio stream queue
  - Clinical NLP: GPU pool with auto-scaling based on attestation queue depth
  - Hallucination detection: co-located with FHIR read replicas for chart access
  - SLO: note available in attestation queue < 3 min post-encounter (p95)
```
