...
| "Design an AI ambient scribe for clinical documentation" | Speech-to-text, clinical NLP, attestation workflow, hallucination detection, PHI handling |
| "Design a FHIR-based prior authorization system" | Da Vinci IG, coverage discovery, documentation templates, real-time payer decisions |
| "Design a public health surveillance system" | eCR, syndromic surveillance, FHIR Subscriptions, de-identification, reportable conditions |

---

## 7. Case Study Discussion Points

### 7.1 Case Study: TEFCA-Enabled Cross-System Patient Lookup

**Scenario:** A patient arrives at Hospital B's emergency department unconscious. Hospital B is not the patient's usual facility. The ED physician needs the patient's medication list, allergy history, and recent cardiac workup from Hospital A, which is connected via a different QHIN.

**Discussion points:**
- How does the MPI resolve patient identity across two organizations with different MRN schemes?
- What is the TEFCA query flow? (IHE XCPD patient discovery → IHE MHD/XCA document query → FHIR Bundle response)
- How does consent apply when the patient is unconscious? (Treatment purpose, break-the-glass if needed)
- What is the latency budget for cross-QHIN query? (Target: < 10 seconds for critical data)
- What data is returned — full chart or a curated summary? (C-CDA Continuity of Care Document)
- How does the receiving system validate the trustworthiness of returned data?

### 7.2 Case Study: Ambient AI Note Generates Clinically Implausible Content

**Scenario:** An ambient scribe generates a progress note that includes a medication dosage 10x above the therapeutic range. The note is in the attestation queue. The system must prevent this from being committed to the CDR without provider correction.

**Discussion points:**
- Where in the pipeline does hallucination detection occur? (Post-NLP, pre-attestation queue)
- What cross-references detect the implausible dosage? (RxNorm dose range lookup, patient weight, renal function)
- How does the UI surface the concern? (Inline highlight with explanation, not a blocking modal)
- What happens if the provider attests without correcting? (CDS fires dose-range alert at commit time)
- How is AI provenance tracked? (Model version, confidence score, source audio hash)
- What are the liability implications of AI-generated clinical content?

---

## 8. Emerging Architecture Patterns

### 8.1 FHIR-Native Lakehouse

The emerging pattern for clinical analytics replaces traditional ETL pipelines with a FHIR-native lakehouse:

| Layer | Traditional | FHIR-Native Lakehouse |
|---|---|---|
| **Operational** | Proprietary schema → ETL → warehouse | FHIR-native CDR (source of truth) |
| **Ingestion** | Batch ETL every 4-24 hours | CDC stream from FHIR event backbone |
| **Analytics** | Star schema in data warehouse | FHIR resources in lakehouse with typed columns |
| **AI/ML** | Feature engineering from denormalized tables | FHIR Bulk Export → ML training pipelines |
| **Latency** | Hours to days | Seconds to minutes |

**Key advantage:** The same FHIR resources used for clinical operations serve analytics and AI workloads without schema translation, reducing data quality drift between operational and analytical views.

### 8.2 USCDI Evolution and Architecture Impact

| USCDI Version | Year | Key Additions | Architecture Impact |
|---|---|---|---|
| v1 | 2020 | Core clinical data set | Baseline FHIR resource support |
| v3 | 2023 | Health insurance, SDOH, goals | New FHIR resource types, Coverage-based queries |
| v5 | 2025 | Disability status, sex parameters | Extension-heavy profiles, gender-inclusive data model |
| v7 (draft) | 2026 | Adverse events, nutrition, quality | 29 new data elements; expanded FHIR surface area |

Each USCDI version expands the mandatory data set that EHR systems must expose via FHIR APIs. The architecture must support additive profile binding without breaking existing integrations — a versioned FHIR conformance strategy.
