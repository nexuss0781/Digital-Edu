...
## 7. Ambient AI Documentation Integration

### 7.1 Ambient Documentation Data Flow
...
Patient-Provider Conversation
        ...
        ▼
┌──────────────────┐
│  Audio Capture    │  ← Encrypted streaming from exam room
│  Service          │     microphone or provider mobile device
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  Speech-to-Text   │  ← Medical-grade ASR with clinical vocabulary
│  Engine           │     Speaker diarization (provider vs. patient)
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  Clinical NLP     │  ← Extract: diagnoses, medications, allergies,
│  + LLM Pipeline   │     procedures, HPI, ROS, assessment/plan
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  Hallucination    │  ← Cross-reference extracted data against
│  Detection        │     patient's existing chart for consistency
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  Attestation      │  ← Provider reviews AI-generated note,
│  Queue            │     edits, and digitally signs
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  FHIR Commit      │  ← DocumentReference + Composition stored
│  (CDR)            │     with AI provenance metadata
└──────────────────┘
```

**Key architectural constraints:**
- Audio data is never persisted after transcription (PHI minimization)
- AI-generated content carries `meta.tag` indicating machine authorship until attested
- Provider attestation is cryptographically signed — the note is not "final" until signed
- All AI-generated content is traceable to a specific model version for audit and liability

### 7.2 Prior Authorization API Integration

The CMS Prior Authorization Rule (effective January 2026) mandates FHIR-based prior authorization for payers. The EHR integrates via the Da Vinci Prior Authorization Implementation Guide:

```
Provider Orders Service/Procedure
        │
        ▼
┌──────────────────┐
│  Coverage Check   │  ← Verify patient insurance via FHIR Coverage
└───────┬──────────┘
        │
        ▼
┌──────────────────┐
│  Prior Auth       │  ← Submit CRD Hook (coverage-requirements-discovery)
│  Decision Service │     Receive: auth required / not required / pend
└───────┬──────────┘
        │
   ┌────┴────────┐
   ▼             ▼
...
(proceed)      │
               ▼
        ┌──────────────────┐
        │  DTR Engine       │  ← Documentation Templates & Rules
        │  (auto-fill from  │     Pre-populate from patient chart
        │   patient chart)  │
        └───────┬──────────┘
                │
                ▼
        ┌──────────────────┐
        │  PAS Submit       │  ← Prior Authorization Support
        │  (FHIR Claim)    │     Real-time decision or pend
        └──────────────────┘
```
