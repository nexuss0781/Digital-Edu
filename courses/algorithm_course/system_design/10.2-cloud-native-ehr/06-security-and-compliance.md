...
| **CMS Prior Auth Rule** | Payer APIs | FHIR-based prior authorization (Jan 2026) | Da Vinci IG integration, real-time payer APIs |
| **TEFCA Common Agreement v2.1** | HIE participants | Technical and legal requirements for data exchange | QHIN connectivity, trust framework, mutual TLS |

---

## 8. AI-Specific Security Considerations

### 8.1 Ambient AI Documentation Security

```
PHI Handling in AI Pipeline:

Audio Data:
  - Encrypted in transit (TLS 1.3) from capture device to ASR service
  - Processed in-memory; NEVER written to persistent storage
  - No audio retention after transcription completes
  - Audio processing isolated in dedicated, PHI-cleared compute environment

Transcript Data:
  - Encrypted at rest during NLP processing
  - Retained only until attestation completes (max 72 hours)
  - Purged after attested note committed to CDR
  - Access restricted to ambient AI service account only

AI Model Security:
  - Models trained on de-identified clinical data only
  - No patient-specific data retained in model weights
  - Model versioning with rollback capability
  - Regular bias audits for demographic disparities in note quality

Provenance Tracking:
  - Every AI-generated element tagged with model version and confidence score
  - Attestation events cryptographically signed by provider
  - Complete lineage: audio → transcript → extraction → note → attestation
  - Audit trail records AI involvement for liability and quality review
```

### 8.2 TEFCA Trust Framework Security

```
TEFCA Security Requirements:

Certificate Management:
  - Mutual TLS (mTLS) between QHINs
  - PKI certificates issued by TEFCA-approved certificate authority
  - Certificate rotation every 90 days
  - Revocation checking via OCSP stapling

Query Authentication:
  - SAML assertions for organization-level authentication
  - Purpose-of-use header required on every query
  - Query source organization validated against TEFCA directory
  - Credential validation at both QHIN and participant levels

Data Minimization:
  - Outbound responses filtered by purpose-of-use
  - Treatment queries return clinical summary
  - Payment queries return encounter metadata and codes only
  - Sensitive categories (42 CFR Part 2) evaluated against patient consent

Logging:
  - Every TEFCA exchange logged with source QHIN, destination, patient, purpose
  - Retained for 7 years per HIPAA audit requirements
  - Cross-referenced with patient consent decisions
```
