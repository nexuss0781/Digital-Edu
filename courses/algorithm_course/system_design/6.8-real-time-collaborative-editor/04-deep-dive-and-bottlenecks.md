...
### Slowest part of the process 1: WebSocket Fan-Out for Popular Documents
...
### Slowest part of the process 2: CRDT Metadata Memory Overhead
...
### Slowest part of the process 3: Large Document Initial Load
...
## Case Study: Figma's Adoption of Eg-walker for Code Layers (2025)

### Problem

Figma's multiplayer system used property-level LWW CRDTs for design objects (frames, shapes, components). When Figma added "code layers"---structured text content attached to design components---they needed a text CRDT. Traditional Yjs/Automerge CRDTs would add 16-32 bytes of metadata per character, significantly increasing memory for text-heavy code layers.

### Solution: Eg-walker Hybrid

Figma adopted the eg-walker algorithm (Gentle & Kleppmann, EuroSys 2025), which achieves CRDT correctness without persistent per-character metadata:

```
How Eg-walker Works in Figma's Code Layers:

1. Storage: Operations are stored in a compact append-only log
   (just the edit itself: insert 'a' at position 5, delete position 7)

2. Common path (no concurrent edits):
   - Operations are applied directly to a flat text buffer
   - No CRDT metadata maintained
   - Memory usage: ~1 byte per character (essentially plain text)

3. Merge path (concurrent edits detected):
   - Build a temporary YATA CRDT from the operation log
   - Merge the concurrent branches using standard CRDT semantics
   - Extract the merged result as plain text
   - Discard the CRDT data structure
   - Store the merge operation in the log

4. Result:
   - Non-conflicting edits: O(1) memory overhead (no CRDT metadata)
   - Conflicting edits: Temporary spike to CRDT-level memory during merge, then released
   - For code layers where conflicts are rare, this is 10-100× more memory efficient
```

### Lesson

Eg-walker demonstrates that CRDT correctness and OT memory efficiency are not mutually exclusive. The key insight is that CRDT metadata is only needed during merge resolution, not during normal editing. For systems where concurrent conflicts are the exception rather than the rule (code editing, configuration files), eg-walker offers dramatically better memory characteristics.

**Applicability to block editors**: Block editors could use eg-walker for text within blocks (where conflicts are block-scoped and relatively rare) while retaining traditional CRDTs for tree structure (where moves and reparenting require persistent metadata for correct resolution).

---

## Case Study: AFFiNE's Open-Source CRDT-Native Block Editor (2024-2026)

### Context

AFFiNE is an open-source alternative to Notion built with CRDTs from the ground up. Unlike Notion (which retrofitted CRDTs onto an existing server-authoritative system), AFFiNE's BlockSuite framework was designed CRDT-native from day one.
...
1. CRDT Foundation:
   - Uses Yjs as the underlying CRDT library
   - Every block's content is a Y.XmlFragment (rich text CRDT)
   - Block tree is managed by Yjs Y.Map and Y.Array
   - Block properties use Y.Map

2. Block Flavours:
   - Each block type is a "flavour" (paragraph, heading, image, database, etc.)
   - Flavours are pluggable: third parties can define new block types
   - Each flavour defines its own schema, rendering, and CRDT integration

3. Sync Architecture:
   - Local-first: all edits go to local Yjs doc first
   - Sync via WebSocket to server using Yjs sync protocol
   - Server is optional: peer-to-peer sync via WebRTC is supported
   - Persistence: IndexedDB for client, PostgreSQL + blob store for server

4. Key Innovation: Document-Centric Model
   - Unlike Notion where the server manages the block tree,
     in AFFiNE the Yjs document IS the source of truth
   - The server stores serialized Yjs state and relays updates
   - The server never interprets or transforms the CRDT state
   - This means the server implementation is trivially simple
```

### Lesson

AFFiNE proves that a CRDT-native block editor is viable as a production architecture. The CRDT-native approach simplifies the server (it becomes a relay + persistence layer), enables peer-to-peer as a natural extension, and makes offline editing a freebie rather than a multi-year migration project. The trade-off is higher client-side complexity (the CRDT engine runs on the client) and larger client-side bundles (Yjs + WASM for performance).

---

## Slowest part of the process Summary

| # | Slowest part of the process | Severity | Root Cause | Primary Mitigation |
|---|-----------|----------|------------|-------------------|
| 1 | Broadcast fan-out | High | N-1 messages per edit per concurrent editor | Operation batching (50ms windows), viewport filtering |
| 2 | CRDT memory overhead | High | 4-32 bytes metadata per character | Block-level lazy loading, eg-walker for text |
| 3 | Large document load | Medium | Full operation log replay for state reconstruction | Periodic snapshots, progressive block loading |
| 4 | Tombstone accumulation | Medium | Deleted items persist as tombstones | GC with 30-day grace period |
| 5 | Cross-document synced blocks | Medium | Cross-document subscriptions create dependency graph | Reverse index of references, subscription management |
| 6 | Offline merge storms | Medium | Mass reconnection after outage | Queue-based processing, rate-limited reconnection |
| 7 | Block tree structural conflicts | Low (rare) | Concurrent moves create cycles or orphans | Cycle detection, orphan rescue, LWW for concurrent moves |
| 8 | Permission-CRDT tension | Low (rare) | CRDT always merges; permissions need rejection | Two-phase validation: sandbox merge → permission check → commit or rollback |
