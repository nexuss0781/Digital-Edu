...
### Operation Batching and Composition

```
ALGORITHM ComposeOperations(ops)
  // Compose multiple consecutive operations from same user into one
  // Called before sending to server (reduces network traffic)

  IF LENGTH(ops) <= 1:
    RETURN ops

  composed ← ops[0]
  FOR i FROM 1 TO LENGTH(ops) - 1:
    IF CAN_COMPOSE(composed, ops[i]):
      composed ← COMPOSE(composed, ops[i])
      // insert("H",5) + insert("i",6) → insert("Hi",5)
      // delete(5,1) + delete(5,1) → delete(5,2)
    ELSE:
      YIELD composed
      composed ← ops[i]
...
// Typical compression: 3-5 character inserts → 1 composed insert
// Reduces operations by 60-80% for typing bursts
```

### Document Partitioning for Very Large Documents

For documents exceeding 1 MB (e.g., book manuscripts, technical specifications):

```mermaid
flowchart TB
    subgraph Doc["Large Document"]
        direction TB
        S1["Section 1<br/>(own session)"]
        S2["Section 2<br/>(own session)"]
        S3["Section 3<br/>(own session)"]
    end

    subgraph Collab["Collaboration Instances"]
        C1["Instance A<br/>handles Section 1"]
        C2["Instance B<br/>handles Sections 2-3"]
    end
...
**Partitioning rules:**
- Each section is an independent OT session
- Cross-section references (links, table of contents) updated asynchronously
- Users in different sections never need operation transforms between them
- Section boundaries are at block level (headings, page breaks)

### WebSocket Connection Optimization

| Technique | Benefit |
|-----------|---------|
| **Connection multiplexing** | One WebSocket per client, multiplexed across documents (reduces connections by 3-5x) |
| **permessage-deflate** | WebSocket compression; 30-50% bandwidth reduction for operation messages |
| **Binary protocol** | Encode operations in binary (MessagePack/Protobuf) instead of JSON; 40-60% size reduction |
| **Selective presence** | Only send presence updates for visible viewport; reduces bandwidth for large documents |
| **Heartbeat optimization** | 30s heartbeat (not 5s); trust TCP keepalive for connection health |

---

## 5. Chaos Engineering for Document Collaboration

### Failure Injection Scenarios

| Scenario | What to Inject | Expected Behavior | Blast Radius |
|----------|---------------|-------------------|--------------|
| **Collaboration instance crash** | Kill instance hosting 40K sessions | Sessions rebuild from WAL + snapshot in <30s; clients auto-reconnect | All documents on that instance (brief interruption) |
| **Operation log write latency** | Inject 500ms latency on WAL writes | Operations buffer in memory; ACK latency increases; no data loss | All active editing sessions |
| **Network partition (client-server)** | Drop packets for 30s | Clients enter offline mode; buffer ops locally; reconcile on reconnect | Affected clients only |
| **WebSocket gateway failure** | Kill one gateway instance | Connections redistribute to healthy instances; clients reconnect in <5s | ~80K connections (fraction of total) |
| **Snapshot worker failure** | Kill all snapshot workers | Editing continues; document loads get slower as ops-since-snapshot grows | No immediate impact; degraded load times |

### Convergence Fuzz Testing

```
ALGORITHM ConvergenceFuzzTest(iterations)
  // Automated test: generate random operations, verify convergence

  FOR i IN 1..iterations:
    doc_state ← EMPTY_DOCUMENT()
    clients ← CREATE_SIMULATED_CLIENTS(count=RANDOM(2, 20))

    // Generate random concurrent operations
    FOR round IN 1..RANDOM(100, 10000):
      client ← RANDOM_CHOICE(clients)
      op ← GENERATE_RANDOM_OP(doc_state)  // insert, delete, format
      client.apply_local(op)
      server.process(op)

      // Randomly inject failures
      IF RANDOM() < 0.05:
        KILL_RANDOM_CLIENT(clients)
        RECONNECT_CLIENT(killed_client)
    ...
    FOR EACH client IN clients:
      ...
      ASSERT server_state == client_state,
        "DIVERGENCE at iteration {i}: server={server_state.hash}, client={client_state.hash}"

  REPORT(iterations, "All passed")
```
...
|----------|-----|-----|----------|
| **Single instance crash** | 0 (WAL is durable) | 10-30s | Snapshot + WAL replay on new instance; auto-discovery by clients |
| **Zone failure** | 0 (synchronous replication) | 1-5 min | Failover to replica zone; clients reconnect to healthy zone |
| **Region failure** | <1s (async cross-region replication) | 5-15 min | Promote secondary region; DNS failover; some ops may need reconciliation |
| **WAL corruption** | Last snapshot | 5-30 min | Restore from latest snapshot; operations since snapshot lost; alert affected users |
| **Full data loss** | Last backup | 1-4 hours | Restore snapshots + operation logs from backup; full document reconstruction |
...
stateDiagram-v2
    ...
    Active --> InstanceCrash: Collab instance dies
    Active --> NetworkPartition: Network split
    Active --> ClientDisconnect: Client loses connection

    InstanceCrash --> WALReplay: New instance loads snapshot + WAL
    WALReplay --> SessionRestored: All ops replayed
    SessionRestored --> ClientReconnect: Notify clients of new endpoint

    NetworkPartition --> ClientBuffering: Clients buffer ops locally
    ClientBuffering --> Reconciliation: Network restored
    Reconciliation --> Active: Buffered ops replayed

    ClientDisconnect --> ClientBuffering: Client enters offline mode
    ClientBuffering --> ClientReconnect: Connection restored
    ClientReconnect --> Active: Buffered ops sent + ACKed
```
