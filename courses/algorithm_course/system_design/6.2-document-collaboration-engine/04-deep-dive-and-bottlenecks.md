...
### Slowest part of the process #1: Hot Documents (Many Concurrent Editors)

**Problem**: A single document with 500+ concurrent editors creates a processing Slowest part of the process — every keystroke generates an operation that must be transformed and broadcast to 499 other users.
...
### Slowest part of the process #2: Operation Log Growth
...
### Slowest part of the process #3: WebSocket Connection Management
...
## 6. Case Study: Figma's Multiplayer Architecture

### Background

Figma uses a CRDT-based approach for its multiplayer design tool, diverging from the OT model used by Google Docs. Their engineering blog provides rare production insights.

### Key Technical Decisions

| Decision | Rationale |
|----------|-----------|
| **Last-Writer-Wins (LWW) CRDTs** | Design objects (shapes, text, layers) are independent; LWW per property avoids complex merge logic |
| **Server as single source of truth** | Despite using CRDTs, Figma maintains a central server for ordering; hybrid approach |
| **Operational compaction** | Group related property changes into single operations; reduce network traffic |
| **Reliability metric: 600ms** | Target: 95% of edits saved to server within 600ms of user action |

### Results and Lessons

- **Tombstone management**: Figma encountered documents with 10M+ CRDT tombstones; required periodic garbage collection
- **Memory pressure**: CRDT metadata overhead can be significant for large documents; Figma optimizes by Cutting off unnecessary steps history beyond a window
- **Convergence is not enough**: Even with guaranteed convergence, the result must match user intent; LWW can produce surprising results when properties conflict

---

## 7. CRDT Deep Dive: Yjs (YATA Algorithm)

### How YATA Maintains Character Order

The YATA (Yet Another Text Algorithm) CRDT assigns each character a unique ID consisting of `(client_id, clock)`. Characters form a linked list where each character knows its left and right neighbors:

```
ALGORITHM YATAInsert(character, position, doc)
  // Assign unique ID
  char.id ← (my_client_id, my_clock++)
  char.left ← doc.get_item_at(position - 1)
  char.right ← doc.get_item_at(position)
  char.origin_left ← char.left   // immutable reference
  char.origin_right ← char.right // immutable reference

  // Integration: find correct position among concurrent inserts
  // If multiple inserts at same position, order by client_id
  scan ← char.right
  WHILE scan IS NOT NULL AND scan.origin_left == char.origin_left:
    IF scan.id.client_id < char.id.client_id:
      // Other insert goes first; we go after it
      char.left ← scan
      scan ← scan.right
    ELSE:
      BREAK
  ...
  // Link into document
  char.right ← char.left.right
  char.left.right ← char
  IF char.right: char.right.left ← char

  RETURN char
```

**Key YATA properties:**
- No central server required for ordering
- Deterministic integration regardless of reception order
- Tombstone-based deletion (deleted characters marked but not removed)
- Memory overhead: ~16-32 bytes per character (ID, origin pointers, content)

### Tombstone Management

```
ALGORITHM GCTombstones(doc, threshold_ops)
  // Run periodically to reclaim memory from deleted characters

  tombstones ← COLLECT items WHERE item.deleted == TRUE
                AND item.delete_clock < (min_client_clock - threshold_ops)

  FOR EACH tombstone IN tombstones:
    // Only GC if all clients have seen the delete
    IF ALL_CLIENTS_PAST(tombstone.delete_clock):
      UNLINK(tombstone)
      FREE(tombstone)
      doc.gc_count ← doc.gc_count + 1

  // Figma lesson: without GC, documents can accumulate 10M+ tombstones
  // GC requires all clients to be past the delete point (safety check)
```
