...
### 4.3 Collaborative Undo Algorithm

```
ALGORITHM CollaborativeUndo(user_id, doc_session)
  // Find the last operation by this user
  last_op ← doc_session.undo_stacks[user_id].pop()
  IF last_op IS NULL:
    RETURN  // Nothing to undo

  // Compute inverse operation
  inverse_op ← ComputeInverse(last_op)
  //   insert(text, pos)    → delete(pos, len(text))
  //   delete(pos, len)     → insert(deleted_text, pos)
  //   format(pos, len, attr, val) → format(pos, len, attr, prev_val)

  // Transform the inverse against all operations that happened AFTER last_op
  current_version ← doc_session.version
  ops_after ← doc_session.log.range(last_op.version + 1, current_version)

  transformed_inverse ← inverse_op
  FOR EACH subsequent_op IN ops_after:
    transformed_inverse ← Transform(transformed_inverse, subsequent_op)

  // Apply as a normal forward operation (goes through full OT pipeline)
  transformed_inverse.metadata.is_undo ← TRUE
  transformed_inverse.metadata.undoes_version ← last_op.version
  ApplyOperation(doc_session, user_id, transformed_inverse)

  // Push to redo stack
  doc_session.redo_stacks[user_id].push(last_op)
```

### 4.4 Document Load Optimization

```
ALGORITHM OptimizedDocumentLoad(doc_id, user_id)
  // Step 1: Check session cache (hot documents already loaded)
  IF session_cache.has(doc_id):
    session ← session_cache.get(doc_id)
    RETURN session.current_state

  // Step 2: Find best snapshot
  snapshot ← snapshot_store.get_latest(doc_id)
  IF snapshot IS NULL:
    // New document or corrupted — reconstruct from full log
    snapshot ← EMPTY_DOCUMENT
    replay_from ← 0
  ELSE:
    replay_from ← snapshot.version + 1

  // Step 3: Replay operations since snapshot
  ops ← operation_log.range(doc_id, replay_from, LATEST)
  state ← snapshot.state
  FOR EACH op IN ops:
    state ← ApplyWithoutTransform(state, op)  // Already ordered; no transform needed

  // Step 4: Initialize in-memory session
  session ← NEW DocumentSession(
    doc_id: doc_id,
    state: state,
    version: snapshot.version + LEN(ops),
    connected_clients: [],
    undo_stacks: {},
    redo_stacks: {}
  )

  // Step 5: Trigger snapshot if too many ops were replayed
  IF LEN(ops) > SNAPSHOT_THRESHOLD:
    ENQUEUE_SNAPSHOT(doc_id, session.version, state)

  session_cache.put(doc_id, session)
  RETURN state
```
