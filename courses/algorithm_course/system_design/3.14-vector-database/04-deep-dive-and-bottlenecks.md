...
Very low selectivity (<0.1% match):
  → Brute Force (Checking every single possibility) on metadata-filtered set (small enough)
```
...
│  │                 │            │                         │ │
│  │ - Brute Force (Checking every single possibility)   │            │ - HNSW O(log n)         │ │
...
Performance:
  L0 Brute Force (Checking every single possibility) (10K vectors): ~1ms
...
### Slowest part of the process 1: Memory Pressure at Scale
...
### Slowest part of the process 2: Index Rebuild Latency
...
### Slowest part of the process 3: Filter Selectivity Impact
...
1. Hybrid execution planning
   ...
   - Choose strategy based on estimate:
     ...
     * Low selectivity: Metadata-first, Brute Force (Checking every single possibility) filtered set
...
## GPU-Accelerated Vector Search (2025-2026)

### NVIDIA cuVS and CAGRA

GPU-accelerated ANN search has matured significantly, with NVIDIA's cuVS library providing CAGRA (Cuda Approximate Graph-based Nearest Neighbors) as a production-ready alternative to CPU-based HNSW.

```
GPU vs CPU PERFORMANCE COMPARISON (100M vectors, 768 dims):
─────────────────────────────────────────────────────────
                        CPU (HNSW)      GPU (CAGRA)
Index Build Time        2-4 hours       15-30 minutes
Query Latency (p99)     8ms             1.5ms
Throughput (QPS)        10K             100K+
Memory                  RAM (500 GB)    VRAM (80 GB A100)
Cost (per 1M queries)   $0.15           $0.05
Recall@10               96%             96%
─────────────────────────────────────────────────────────

TRADE-OFFS:
- GPU: 5-10x faster search, 8x faster build, but limited by VRAM capacity
- CPU: Larger datasets per node, lower hardware cost, mature tooling
- Hybrid: GPU for hot data, CPU for warm/cold — emerging pattern in 2025
```

### Serverless Vector Database Architecture

```
SERVERLESS PATTERN (2025-2026):
─────────────────────────────────────────────────────────

COMPUTE-STORAGE SEPARATION:
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│ Query Tier  │     │ Index Tier  │     │ Storage Tier│
│ (Stateless) │ ──→ │ (Cached)    │ ──→ │ (Durable)   │
│ Scale to 0  │     │ Hot indexes │     │ Object store│
└─────────────┘     └─────────────┘     └─────────────┘

KEY BENEFITS:
- Pay-per-query pricing (no idle compute cost)
- Auto-scaling from 0 to 10K QPS in seconds
- Indexes loaded on-demand from object storage
- Cold collections cost only storage (pennies/GB/month)

CHALLENGES:
- Cold start latency: 500ms-2s for first query to dormant index
- Index loading: Streaming index from storage adds 100-500ms
- State management: Routing queries to nodes with warm indexes
```
