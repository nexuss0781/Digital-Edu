...
2. L0 buffer approach
   "Recent writes go to a small buffer that's searched
    via Brute Force (Checking every single possibility). Background process merges buffer
...
4. Query flow
   "Query searches both L0 buffer and HNSW index,
    ...
    Brute Force (Checking every single possibility) adds <1ms overhead."
```
...
### Trap 1: "Why not just use Brute Force (Checking every single possibility)?"

```
What they're testing: Do you understand scalability?

Bad answer: "Brute Force (Checking every single possibility) is fine for small datasets."

Good answer:
"Brute Force (Checking every single possibility) is O(n × d) per query. At 10M vectors with
...
Brute Force (Checking every single possibility) is fine up to ~10K vectors. Beyond that,
...
1. Use Brute Force (Checking every single possibility) (only viable <100K vectors)
...
│                                                              │
│  2025-2026 Trends:                                          │
│  • GPU acceleration (cuVS/CAGRA): 10x query throughput     │
│  • Serverless (compute-storage separation, scale-to-zero)  │
│  • Multi-tenancy (namespace isolation, noisy neighbor)     │
│  • Hybrid search (vector + BM25 + structured filters)      │
│  • Streaming updates (real-time RAG, sub-second freshness) │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
