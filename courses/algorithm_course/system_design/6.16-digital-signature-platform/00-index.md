...
| [8.4 - Digital Wallet](../8.4-digital-wallet/) | Cryptographic key management, HSM integration patterns |
| [2.8 - PKI & Certificate Management](../2.8-pki-certificate-management/) | X.509 certificate lifecycle, CA trust hierarchies |
| [9.12 - AI-Native Procurement Spend Intelligence](../9.12-ai-native-procurement-spend-intelligence/) | Contract workflow automation, compliance audit patterns |
...
|-------|----------------------|
| **Remote Online Notarization (RON)** | Video-based identity verification + notary witness sessions integrated into signing flows; requires real-time media infrastructure alongside document workflows |
| **AI-Powered Document Preparation** | ML models auto-detect signature, date, and initial fields from document text; reduces template creation time from minutes to seconds |
| **Decentralized Identity (DID/VC)** | Verifiable Credentials replacing centralized identity providers for signer authentication; enables self-sovereign signing without platform-managed certificates |
| **Post-Quantum Cryptography Migration** | NIST PQC standards (ML-DSA, SLH-DSA) beginning adoption for long-lived signatures that must remain valid for 10+ years; dual-signing (classical + PQC) during transition |
| **eIDAS 2.0 and EU Digital Identity Wallet** | European Digital Identity Wallet (EUDI) enables QES from mobile devices without QTSP integration; fundamentally changes the QES code path |
| **Embedded Signing Everywhere** | API-first signing embedded in CRM, HRIS, and ERP workflows via iFrames and SDKs; signing becomes a feature, not a destination |
| **Zero-Knowledge Proof Selective Disclosure** | Signers can prove attributes (e.g., "over 18," "authorized officer") without revealing full identity; regulatory compliance with minimal data collection |
| **Blockchain-Anchored Audit Trails** | Periodic hash anchoring to public blockchains as an alternative to RFC 3161 TSA; provides censorship-resistant temporal proof |
...
| Metric | Value |
|--------|-------|
| Daily envelopes (with bulk) | 5.7M |
| Peak signatures/second | 54 |
| Documents per envelope (avg) | 2.5 |
| Signers per envelope (avg) | 2.3 |
| Audit events per envelope | ~15 |
| HSM signing latency | 50--200ms |
| Document storage (Year 1) | ~10 PB |
| Email notifications/day | ~39M |
| Target availability | 99.99% |

---

## Key Trade-Offs in Digital Signature Platform Design

| Trade-Off | Option A | Option B | Tension |
|-----------|----------|----------|---------|
| **Per-envelope vs. global hash chain** | Per-envelope chains (parallel writes) | Global chain (single integrity proof) | Scalability vs. global tamper detection |
| **Server-side vs. client-side PDF rendering** | Server-rendered images (prevents PDF injection) | Client-rendered (lower latency, richer) | Security vs. user experience |
| **HSM for all signatures vs. SES-only software** | All through HSM (uniform security) | HSM only for AES/QES (SES via software) | Consistency vs. throughput/cost |
| **Synchronous vs. async sealing** | Seal immediately after last signature | Queue sealing for batch processing | User experience vs. resource efficiency |
| **Per-signer vs. per-org signing keys** | Per-signer keys (unique linkage per eIDAS) | Per-org keys (simpler management) | Compliance vs. operational simplicity |
| **Strict session IP binding** | Bind session to IP (prevents hijacking) | No IP binding (supports mobile roaming) | Security vs. mobile usability |
| **Embedded vs. database-stored signatures** | PKCS#7 embedded in PDF (self-verifying) | Signature in DB (platform-dependent) | Portability vs. implementation simplicity |
