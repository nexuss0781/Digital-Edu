...
> "A Zero Trust Security Architecture replaces traditional perimeter-based security with identity-centric, continuous verification. Here's how it works:
>
> **Core principle:** Never trust, always verify. Every request -- whether from inside or outside the network -- must be authenticated, authorized, and encrypted.
>
> **Architecture:** We separate control plane (IdP, PDP, CA, Device Trust) from data plane (PEP sidecars, access proxies). The control plane makes decisions, the data plane enforces them.
>
> **Identity:** Users authenticate via IdP with MFA. Services authenticate via SPIFFE workload identities with mTLS certificates. Devices prove integrity via TPM attestation.
>
> **Policy evaluation:** Every access request is evaluated by the PDP against ABAC policies considering subject identity, resource sensitivity, device posture, and contextual signals. Policies are compiled into optimized decision trees for sub-1ms evaluation.
>
> **mTLS everywhere:** Service-to-service communication uses mutual TLS with short-lived certificates (24h TTL) issued by SPIFFE/SPIRE. Jittered rotation prevents thundering herd on the CA.
>
> **Key challenges:**
> 1. PDP is the new single point of failure -- requires 99.999% availability
> 2. Policy evaluation on every request -- must add <10ms latency
> 3. Certificate lifecycle at scale -- thousands of workloads rotating simultaneously
>
> **Migration:** Deploy in permissive (log-only) mode first, analyze access patterns, then gradually enforce strict mode resource by resource."
...
|--------|-------|---------|
| **Policy evaluation latency** | < 10ms (p99) | On every request's critical path |
| **Certificate TTL** | 24 hours | Limits blast radius of key compromise |
| **Certificate rotation** | At 50% TTL (12h) | With deterministic jitter |
| **PDP availability** | 99.999% | Failure blocks all access |
| **Policy propagation** | < 30 seconds | Eventual consistency acceptable for most resources |
| **Decision cache hit rate** | > 90% | Reduces PDP load by 10x |
| **Policy compilation speedup** | 5-10x | Compiled trees vs interpreted JSON |
| **Enterprise adoption (2026)** | 60% of large enterprises | Up from <10% in 2023 |
| **Market size** | $31.6B (2025) | Growing 16.6% CAGR |
| **CISA ZTM pillars** | 5 pillars + 3 cross-cutting | Identity, Devices, Networks, Apps, Data |
