...
Detection Practical rule of thumb:
    skew = 0
...
### Slowest part of the process 1: Tail Sampler Memory Pressure
...
### Slowest part of the process 2: Hot Store Write Amplification
...
### Slowest part of the process 3: Tag-Based Search Over Large Time Ranges
...
### Edge Case (Unusual or extreme situation) 1: Trace Spanning Multiple Sampling Decisions

**Scenario**: A trace starts in Service A (head-sampled at 10%) and propagates to Service B (head-sampled at 1%). The trace is sampled-in by Service A's SDK, but Service B's SDK sees a 1% rate and might have dropped it.

**Resolution**: The W3C Trace Context spec addresses this: the `sampled` flag in the `traceparent` header is authoritative. Once a trace is marked as sampled by the root service, all downstream services must honor that decision regardless of their local sampling rate. The SDK must check the incoming trace flags before applying its own sampling decision.

### Edge Case (Unusual or extreme situation) 2: Fan-Out Explosion (10,000+ Spans per Trace)

**Scenario**: A batch processing service receives a single request that fans out to 10,000 downstream calls. The resulting trace has 10,000+ spans, overwhelming the tail sampler buffer and the trace visualization UI.

**Resolution**:
1. **At ingestion**: The collector detects traces exceeding a span count threshold (e.g., 1,000 spans) and applies sub-sampling within the trace—retaining the root span, all error spans, and a representative sample of the fan-out children
2. **At storage**: Store the full trace but mark it as "large trace" in metadata
3. **At query**: Progressive loading—return the top 3 levels immediately; paginate deeper levels on demand; collapse repeated spans into a summary ("500 calls to database-service, 3 errors")

### Edge Case (Unusual or extreme situation) 3: Trace Context Pollution via Baggage

**Scenario**: A developer adds large baggage items (e.g., a full JSON request body) to the trace context. Since baggage propagates to every downstream service, this multiplies bandwidth consumption by the trace fan-out factor.

**Resolution**: Enforce a maximum baggage size limit at the SDK level (e.g., 8 KB total). The collector validates baggage size and drops oversized baggage items with a warning metric. Document in SDK guidelines that baggage is for small metadata (tenant ID, feature flags), not payload data.

### Edge Case (Unusual or extreme situation) 4: Clock Drift Accumulation in Long Traces

**Scenario**: A trace passes through 20 services sequentially. Each hop introduces a small clock skew (±1ms). By the 20th service, accumulated skew can be 20ms, which is larger than some individual span durations.

**Resolution**: Apply clock skew correction relative to the immediate parent (not the root), so each correction is small. Use NTP sync quality metrics per host to weight the correction: hosts with poorer NTP sync get more aggressive correction. For traces with many hops, add a "cumulative_skew_adjustment" metadata field so engineers can judge whether timing data is reliable.

---

## Critical Component 4: Propagation Coverage Monitoring

### Why Is This Critical?

Context propagation is the tracing system's weakest link. A single service that fails to propagate the `traceparent` header breaks the trace into disconnected fragments, making the downstream portion invisible. At organizations with hundreds of services in multiple languages, achieving and maintaining 100% propagation coverage is an ongoing operational challenge.

### How It Works Internally

```
FUNCTION computePropagationCoverage():
    // For each service, compute the percentage of incoming traces
    // that have valid parent context vs. new root contexts

    FOR service IN allServices():
        totalIncomingTraces = countTraces(service, isRoot=FALSE)
        newRootTraces = countTraces(service, isRoot=TRUE, hasUpstreamCallers=TRUE)
        // newRootTraces where hasUpstreamCallers=TRUE indicates broken propagation
        // (the service started a new trace despite being called by another service)

        coverage = 1 - (newRootTraces / (totalIncomingTraces + newRootTraces))
        emit("propagation.coverage", service, coverage)

        IF coverage < 0.95:
            ALERT("Propagation gap detected",
                  service=service,
                  coverage=coverage,
                  impact="Approximately {newRootTraces} traces/min are fragmented")

// Detection Practical rule of thumb: A service that creates root spans (no parent) but is known
// to receive calls from other services (per the service map) likely has a
// propagation gap. Cross-referencing root span creation with the service
// dependency graph identifies instrumentation issues.
```
...
|---|---|---|---|
| **Custom HTTP client** | Service uses a hand-written HTTP client that doesn't inject trace headers | High root span rate for a service that has known callers | Instrument the custom client with OTel HTTP interceptor |
| **Thread context loss** | Async framework (e.g., async/await, goroutine) doesn't propagate thread-local context | Intermittent orphan spans; trace breaks at async boundaries | Use context-aware task scheduling; OTel provides language-specific context propagation utilities |
| **Message queue consumer** | Consumer reads messages but doesn't extract trace context from message headers | Traces end at the producer; consumer creates new unrelated traces | Instrument message consumer to extract and continue trace context |
| **gRPC interceptor missing** | gRPC service doesn't have OTel interceptor registered | All gRPC calls from this service create new traces | Add `otelgrpc.UnaryServerInterceptor()` to gRPC server setup |
| **Cross-language boundary** | Python service calls a Go service; trace context format differs between SDKs | Sporadic trace breaks between specific language pairs | Ensure both SDKs use W3C Trace Context (not B3); verify header name consistency |

---

## Critical Component 5: Cross-Tier Query Federation

### Why Is This Critical?

When a user queries for a trace by ID, the trace may span multiple storage tiers (hot, warm, cold) if it was recently compacted. The query service must transparently fetch from the correct tier without the user knowing which tier the data lives in. For search queries (by tag, service, time range), the query may need to scan indices across all tiers and merge results.

### How It Works Internally

```
FUNCTION queryTraceById(traceId):
    // Step 1: Check cache
    cachedTrace = queryCache.get(traceId)
    IF cachedTrace IS NOT NULL:
        RETURN cachedTrace

    // Step 2: Check hot store (fast path)
    hotResult = hotStore.getByTraceId(traceId)
    IF hotResult IS NOT NULL:
        assembledTrace = assembleTrace(hotResult.spans)
        queryCache.put(traceId, assembledTrace, ttl=5min)
        RETURN assembledTrace

    // Step 3: Check warm/cold stores via bloom filter cascade
    FOR block IN blockRegistry.getBlocksByTimeRange(last90days):
        // Load bloom filter from L3 cache (or fetch from object storage)
        bloomFilter = blockMetadataCache.getBloomFilter(block.id)

        IF bloomFilter.mightContain(traceId):
            // Bloom filter positive: read the Parquet block
            spans = objectStorage.readParquetBlock(block.path, filter="trace_id = {traceId}")
            IF spans IS NOT EMPTY:
                assembledTrace = assembleTrace(spans)
                queryCache.put(traceId, assembledTrace, ttl=10min)
                RETURN assembledTrace

    // Step 4: Trace not found in any tier
    RETURN NOT_FOUND

FUNCTION searchTraces(service, operation, tags, timeRange, limit):
    results = []

    // Step 1: Determine which tiers to search based on time range
    tiersToSearch = determineTiers(timeRange)
    // e.g., last 3 days → [HOT]; last 14 days → [HOT, WARM]; last 60 days → [HOT, WARM, COLD]

    // Step 2: Search each tier in parallel
    IF HOT IN tiersToSearch:
        hotResults = hotStore.searchByIndex(service, operation, tags, timeRange)
        results.addAll(hotResults)

    IF WARM IN tiersToSearch OR COLD IN tiersToSearch:
        // Use tag index and Parquet column statistics for Cutting off unnecessary steps
        candidateBlocks = tagIndex.search(service, operation, tags, timeRange)
        FOR block IN candidateBlocks (PARALLEL):
            blockResults = objectStorage.scanParquet(block.path,
                predicates=[service, operation, tags],
                timeRange=timeRange)
            results.addAll(blockResults)

    // Step 3: Deduplicate (trace may exist in both hot and warm during migration)
    results = deduplicateByTraceId(results)

    // Step 4: Sort by start time (descending) and limit
    results.sortByStartTime(DESC)
...
| Failure | Impact | Mitigation |
|---|---|---|
| **Bloom filter cache miss** | Each block requires a fetch from object storage to load the bloom filter, adding 50-100ms per block | Pre-warm bloom filter cache at query service startup; prioritize recent blocks |
| **Cross-tier deduplication miss** | Same trace appears twice in search results (once from hot, once from warm) | Deduplication by trace ID; brief overlap is acceptable |
| **Object storage timeout** | Warm/cold tier queries timeout, returning partial results | Return hot tier results immediately; warm/cold results as "loading"; fallback to "hot tier only" mode |
| **Block registry stale** | Query misses recently compacted blocks not yet registered | Block registry updated atomically after compaction; registration is the last step |
