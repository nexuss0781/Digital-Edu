...
## 4. Deep Dive: Engagement-Optimized vs Skill-Optimized Matching

### 4.1 The EOMM Paradigm

Engagement-Optimized Matchmaking (EOMM) represents a fundamental philosophical shift in matchmaking design. Traditional SBMM optimizes for a single objective: minimize skill variance between teams. EOMM optimizes for player engagement — maximizing the probability that players continue playing after a match.

**Why it matters architecturally:**

EOMM requires a fundamentally different data pipeline. Instead of just querying a player's current skill rating, the matchmaker must also query:
- **Churn prediction model:** Given this player's recent history (3 losses in a row, declining session lengths), what match outcome distribution maximizes their return probability?
- **Session state:** Is this the player's first match of the day (high engagement tolerance) or their tenth (fatigue-sensitive)?
- **Social context:** Is the player in a party (socially anchored, less likely to churn) or solo (churn-sensitive)?

```
EOMM Quality Function (extends standard quality function):

EngagementQuality = w₁ × SkillBalance + w₂ × LatencyFairness +
                    w₃ × PartySymmetry + w₄ × RoleBalance +
                    w₅ × RematchPenalty + w₆ × EngagementScore

Where:
  EngagementScore = Σ(ChurnRisk(player_i) × MatchSuitability(player_i, match))

  ChurnRisk: ML model output (0 = no churn risk, 1 = about to quit)
  MatchSuitability: How well this specific match serves the player's engagement needs
    - For high churn-risk players: prefer matches predicted to be close
    - For session-start players: standard skill matching
    - For winning-streak players: standard or slightly tougher matches

Default weight: w₆ = 0.05 (deliberately small — engagement is a tiebreaker, not a driver)
```

### 4.2 The Ethical Tension

The danger of EOMM is well-documented: taken to its extreme, the system might deliberately create losing matches to induce frustration that drives microtransaction purchases, or manufacture artificial win-streaks to create dopamine dependency. These outcomes are not hypothetical — research has demonstrated that unregulated engagement optimization can produce manipulative matching patterns.

**Architectural safeguards:**

| Safeguard | Implementation |
|---|---|
| **Engagement weight cap** | w₆ never exceeds 0.10; skill balance always dominates |
| **Streak protection** | System never deliberately creates losing streaks longer than 2; after 3 losses, engagement bias shifts toward balanced matches |
| **Audit trail** | Every match logs whether engagement factors influenced the pairing decision and by how much |
| **A/B testing gate** | All engagement model changes must pass a controlled experiment showing improved retention WITHOUT decreased match quality metrics |
| **Transparency report** | Quarterly publication of aggregate engagement factor statistics (percentage of matches influenced, average quality impact) |

### 4.3 Real-World Scale Reference (2025)

Major competitive titles in 2025 demonstrate the scale challenges that drive architectural decisions:

| Title Category | Concurrent Players (Peak) | Matches/Minute (Est.) | Matchmaking Approach |
|---|---|---|---|
| **Battle Royale (top tier)** | 30-48M across platforms | 300K+ (100-player lobbies) | Skill-bracket lobbies with bot fill; EOMM for casual modes |
| **Hero Shooter (top tier)** | 10-15M across platforms | 100-150K (5v5/6v6) | Strict SBMM for ranked, relaxed for casual, role queue |
| **Tactical Shooter** | 1-3M concurrent | 20-50K (5v5) | Strict SBMM, minimal EOMM, latency-priority for competitive |
| **Fighting Game (top tier)** | 100-500K concurrent | 50-100K (1v1) | Glicko-2, connection-quality-first matching |

These numbers validate the architectural targets: 10M concurrent with 100K+ matches/minute is the design point for a top-tier title. The system must handle 3-5x this during launch events and seasonal resets.

---

## 5. Slowest part of the process Analysis

### 5.1 Matching Computation at Scale
...
**Why it's a Slowest part of the process:** The matching algorithm must complete within the cycle window (1-2 seconds). If computation exceeds this, queue times increase and match quality degrades (stale pool snapshots).
...
### 5.2 Queue Starvation for Extreme Skill Outliers
...
**Why it's a Slowest part of the process:** Long queue times for top players are highly visible (streamers, pros, esports athletes). Negative perception disproportionately impacts brand.
...
**Why it's a Slowest part of the process:** The system must balance "provide a match eventually" against "don't put players at a latency disadvantage." Strict latency requirements shrink the pool; lenient requirements create unfair games.
...
### 5.4 Party Composition Imbalance
...
**Why it's a Slowest part of the process:** Party composition mismatches cause longer queue times for certain party sizes, even when total queue population is healthy.
...
### 5.5 Thundering Herd on Season Start
...
**Why it's a Slowest part of the process:** 3-5x normal queue rate in the first hour. Rating engine processing 3-5x the normal update volume. All players competing for placement matches creates artificial congestion.
...
### 5.6 Cross-Play Pool Fragmentation

**The problem:** Cross-play introduces 2-3x more pool partitions (input method × platform combinations). A pool that had 13K tickets now splits into 5K MnK + 4K controller + 4K cross-input, reducing matching efficiency.

**Why it's a Slowest part of the process:** Smaller pools increase queue times for all players and worsen the skill-outlier starvation problem. The top 0.1% in a controller-only pool is a fraction of the already-small overall top 0.1%.

**Solutions:**

| Strategy | Effect | Trade-off |
|---|---|---|
| **Soft input preference** | Input method is a quality factor (weight=0.15), not a hard partition | Mixed-input matches more frequent |
| **Timeout-based pool merge** | After 30s, input-restricted tickets become cross-input eligible | Longer waits before mixing |
| **Platform-aware skill adjustment** | Apply +/- 50 rating offset for known input-method advantages | Requires ongoing calibration per game |
| **Unified casual pools** | Casual modes default to cross-input matching | Only ranked maintains strict segmentation |
...
| **Engagement model stale** | Churn predictions based on outdated player behavior | Model prediction accuracy drops below baseline | Fall back to standard quality function (w₆ = 0), retrain model |
| **Trust factor database unavailable** | Cannot retrieve trust scores for matching | Trust service health check fails | Match without trust factor weighting; queue trust score updates |
| **Cross-play platform service outage** | Cannot determine player's input method | Platform detection timeout > 500ms | Default to cross-input pool; accept slightly suboptimal input matching |
