...
### 7.1 Input-Segmented Matching Pools

Cross-platform matchmaking introduces an additional dimension to pool partitioning: input method. The fundamental challenge is that mouse/keyboard players have a measurable accuracy advantage over controller players in aim-dependent games, making raw skill ratings non-comparable across input methods.

```
Pool hierarchy with cross-play:

Region × Game Mode × Rank Tier × Input Method

Example pools for NA-East Ranked Gold:
- "NA-East:Ranked:Gold:MnK"        (mouse/keyboard players)
- "NA-East:Ranked:Gold:Controller"  (controller players)
- "NA-East:Ranked:Gold:CrossInput"  (opt-in mixed input pool)

Overflow rules:
- MnK pool → CrossInput pool after 45s
- Controller pool → CrossInput pool after 45s
- CrossInput pool is always available but never forced
- Console-only queue restricts to controller players on console platforms
```

### 7.2 Platform Rating Calibration

Players who switch platforms (e.g., console to PC) have skill that transfers partially. The cross-platform adjustment factor handles this:

```
CrossPlatformSkill = SourcePlatformMu × TRANSFER_FACTOR + (1 - TRANSFER_FACTOR) × DEFAULT_MU

Where:
- TRANSFER_FACTOR varies by game genre (0.7 for strategy, 0.5 for shooters)
- New platform sigma is elevated (200 vs normal 350 for brand new)
- After 10 matches on new platform, sigma converges normally
```

### 7.3 Trust Factor Integration

The trust factor system creates soft segmentation where players with similar trust scores are preferentially matched together, without creating hard boundaries:

| Trust Tier | Score Range | Matching Behavior |
|---|---|---|
| **High Trust** | 80-100 | Matched with other high-trust players; fastest queue times |
| **Normal Trust** | 50-79 | Standard matching pool; majority of players |
| **Low Trust** | 20-49 | Preferentially matched with other low-trust players; slight queue time penalty |
| **Restricted** | 0-19 | Separate pool with longer queues; rehabilitative — score recovers with good behavior |

Trust factor is a soft preference in the quality function (weight = 0.05-0.10), not a hard constraint. It never prevents a match from forming — it only influences which match is preferred when multiple options exist.

---

## 8. ML-Assisted Matchmaking (2025+ Evolution)

Modern matchmaking systems increasingly incorporate machine learning beyond skill rating:

| ML Component | Input Features | Output | Impact |
|---|---|---|---|
| **Churn Prediction** | Loss streak length, session duration trend, queue abandonment rate | Per-player churn risk score (0-1) | Slight quality bias for at-risk players (avoid lopsided matches) |
| **Engagement Forecasting** | Historical session patterns, match outcomes, time-of-day | Predicted session continuation probability | Inform queue time vs quality trade-off per player |
| **Playstyle Clustering** | Heatmaps, ability usage patterns, aggression metrics | Playstyle vector (aggressive, defensive, objective-focused) | Optional playstyle diversity scoring in quality function |
| **Toxicity Prediction** | Chat logs, report history, ping behavior | Per-player toxicity risk | Feed into trust factor scoring |
| **Queue Time Estimation** | Pool state, time-of-day, seasonal patterns, event calendar | Accurate per-ticket wait estimate | Better player expectations, reduced queue abandonment |

**Critical constraint:** ML models influence matchmaking parameters but never override the core skill-fairness guarantee. Engagement optimization must not sacrifice competitive integrity — the system may slightly prefer a match that keeps an at-risk player engaged, but only when multiple equally-skilled options exist.

---

## 9. Technology-Agnostic Infrastructure Mapping
