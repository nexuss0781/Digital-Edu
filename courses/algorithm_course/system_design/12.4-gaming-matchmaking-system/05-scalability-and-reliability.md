...
The key Rule that never changes: **pool subdivision must never prevent matching**. Adjacent pools share an overlap zone where tickets are visible to workers in both pools. Tickets in the overlap zone have slightly higher conflict rates but ensure no artificial matching barriers.
...
## 6. Container-Orchestrated Game Server Management

### 6.1 Modern Fleet Architecture (2025+ Pattern)

Modern matchmaking systems leverage container orchestration platforms to manage game server fleets declaratively. The key architectural pattern uses a dedicated game server management layer that integrates with the container orchestrator's API to allocate, scale, and health-check game server pods.

```mermaid
flowchart TB
    subgraph Matchmaking["Matchmaking Layer"]
        MW[Match Worker]
        SA[Server Allocator]
    end

    subgraph Orchestrator["Container Orchestrator"]
        GS_API[Game Server<br/>Controller API]
        ALLOC[Fleet Allocator]
    end

    subgraph Fleet["Game Server Fleet"]
        subgraph Ready["Ready Pool"]
            GS1[Server Pod<br/>Ready]
            GS2[Server Pod<br/>Ready]
            GS3[Server Pod<br/>Ready]
        end
        subgraph Active["Allocated"]
            GS4[Server Pod<br/>In-Game]
            GS5[Server Pod<br/>In-Game]
        end
    end

    subgraph Scaling["Auto-Scaling"]
        HPA[Fleet Autoscaler]
        BUF[Buffer Policy:<br/>min 30% ready]
    end
    ...
    classDef scale fill:#fffde7,stroke:#f57f17,stroke-width:2px

    class MW,SA service
    class GS_API,ALLOC orch
    class GS1,GS2,GS3,GS4,GS5 fleet
    class HPA,BUF scale
```

### 6.2 Server Lifecycle States

```
Game Server Pod States:

  Creating → Starting → Ready → Allocated → In-Game → Shutdown → Recycled
              │                    │            │
              └── Health Failed ──→│            └── Crash → Emergency Re-queue
                                   │
                                   └── Timeout (no players connect in 30s) → Ready

Fleet Autoscaler Policy:
  - Buffer size: 30% of fleet capacity must be in Ready state
  - Scale-up: When Ready < 25%, add 20% more pods
  - Scale-down: When Ready > 50% for >15 min, remove 10% of Ready pods
  - Min replicas: region-specific floor (never below 50 per region)
  - Max replicas: budget cap per region per hour
```

### 6.3 Multi-Cluster Regional Deployment

Each region operates an independent orchestration cluster with its own fleet autoscaler, providing fault isolation. Cross-region server allocation requires the matchmaker to query multiple cluster APIs with a latency budget.

| Region | Cluster Capacity | Typical Ready Buffer | Peak Scaling |
|---|---|---|---|
| NA-East | 5,000 server pods | 1,500 ready | Up to 12,000 during events |
| EU-West | 6,000 server pods | 1,800 ready | Up to 15,000 during events |
| APAC-East | 4,000 server pods | 1,200 ready | Up to 10,000 during events |
| SA | 1,500 server pods | 500 ready | Up to 3,000 during events |
