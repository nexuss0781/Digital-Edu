...
## 8. Engagement & Trust Factor Observability

### 8.1 Engagement Model Metrics

```
Engagement Metrics:

// Model performance
engagement.churn_prediction_accuracy    // Rolling AUC for churn model
engagement.influence_rate               // % of matches where engagement factor changed the selection
engagement.avg_quality_delta            // Average quality score change due to engagement factor
engagement.streak_protection_triggers   // Times streak protection overrode standard matching

// Player outcome tracking
engagement.return_rate_by_churn_risk    // Segmented: did high-risk players return?
engagement.session_length_trend         // 7-day moving average per region
engagement.rage_quit_rate               // Matches ended by player disconnect within first 3 min
```

### 8.2 Trust Factor Dashboard

```
Trust Factor Metrics:

// Distribution
trust.tier_distribution                 // % of players in each trust tier
trust.score_migration_rate              // Players moving between tiers per day
trust.rehabilitation_rate               // Low-trust players recovering to normal per week

// Impact on matching
trust.same_tier_match_rate              // % of matches where all players share a trust tier
trust.cross_tier_match_rate             // % of matches mixing trust tiers
trust.low_trust_queue_premium           // Additional queue time for low-trust players

// Quality correlation
trust.report_rate_by_tier              // Reports per match, segmented by trust tier
trust.abandon_rate_by_tier             // Abandonment rate by trust tier
trust.satisfaction_by_tier             // Post-match survey scores by trust tier
```

### 8.3 Cross-Play Observability

```
Cross-Play Metrics:

// Pool utilization
crossplay.input_pool_sizes              // MnK vs Controller vs CrossInput per region
crossplay.cross_input_opt_in_rate       // % of players opting into mixed input
crossplay.input_overflow_rate           // Tickets that timed out of input-restricted pool

// Fairness validation
crossplay.win_rate_by_input_method      // Should be ~50% if calibration is correct
crossplay.platform_rating_offset_accuracy // Is the cross-platform adjustment factor working?
crossplay.input_method_quality_variance // Skill variance in cross-input matches vs same-input
```
