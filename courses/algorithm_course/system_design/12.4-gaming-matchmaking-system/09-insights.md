...
## Insight 16: Engagement Optimization Is the Most Dangerous Feature to Get Right

**Category:** System Modeling

**One-liner:** Engagement-Optimized Matchmaking (EOMM) represents a philosophical shift from "find the fairest match" to "find the match that keeps players playing"—and the architecture must include hard constraints preventing this optimization from degenerating into player manipulation.

**Why it matters:**

Traditional SBMM has a simple, defensible objective: minimize skill variance between teams. The system either achieves fair matches or it doesn't, and the metrics are transparent. EOMM adds a second, potentially conflicting objective: maximize the probability that each player continues playing after the match. Research has demonstrated that these objectives can diverge—the most engaging match is not always the fairest match, and vice versa.

The danger is not theoretical. Unconstrained engagement optimization can learn that alternating wins and losses (manufactured streaks) maximizes session length, that placing at-risk players into easy matches prevents churn but degrades the experience for their opponents, or that frustration-driven engagement (just barely losing) drives microtransaction purchases. These are not edge cases—they are natural optima for an ML model optimizing engagement without fairness constraints.

The architectural response must be structural, not policy-based. The engagement weight in the quality function is capped (never exceeding 10% of the total score). Streak protection is a hard constraint (the system never creates more than 2 consecutive predicted-lopsided matches for any player). Every match where the engagement factor changed the pairing decision is logged with an audit trail. Model updates must pass an A/B test gate showing improved retention without decreased match quality or fairness metrics.

The fundamental insight is that engagement optimization requires more engineering effort to constrain safely than to implement. The model itself is straightforward—predict churn probability, prefer matches that reduce it. The safeguards, auditing, transparency reporting, and ethical boundaries are the actual architectural challenge.

---

## Insight 17: Cross-Play Pool Fragmentation Is Population Starvation in Disguise

**Category:** Scaling

**One-liner:** Cross-platform matchmaking does not simply "add more players to the pool"—it fragments the pool by input method, creating 2-3x more partitions that can individually starve, and the only architectural solution is treating input method as a soft preference rather than a hard constraint.

**Why it matters:**

The naive expectation when enabling cross-play is that merging PC, console, and mobile populations creates a larger, healthier pool. In practice, fairness requirements demand input-method segmentation: mouse/keyboard players have a measurable accuracy advantage in aim-dependent games. This means the "merged" pool actually becomes three pools (MnK, controller, cross-input opt-in), each smaller than the original platform-specific pool in some regions.

The top 0.1% problem is dramatically worsened. If a regional ranked pool had 15 top-0.1% players, splitting by input method might leave 8 in MnK and 7 in controller—neither enough for a 5v5 match. The cross-input opt-in pool may have 2-3 players of that tier. Hard input partitioning makes high-rank matching nearly impossible in smaller regions.

The architectural solution is to treat input method as a soft preference (a factor in the quality function with weight 0.10-0.15) rather than a hard pool boundary. Players default to same-input matching, but after a configurable timeout (30-45 seconds), input restriction relaxes. At extreme skill tiers (top 1%), input restriction relaxes immediately because the population constraint dominates the input-fairness constraint. This is the same expanding-window philosophy applied to a new dimension—start restrictive, relax over time—and it reuses the exact same per-ticket temporal window infrastructure that handles skill and latency expansion.
