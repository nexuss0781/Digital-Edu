...
### 1.3 Cross-Play & Input Fairness

| ID | Requirement | Description |
|---|---|---|
| FR-14 | **Cross-Platform Matching** | Support opt-in cross-platform matchmaking (PC, console, mobile) with platform-aware skill calibration |
| FR-15 | **Input Method Segmentation** | Separate matching pools by input method (mouse/keyboard vs controller) by default, with cross-input opt-in |
| FR-16 | **Platform-Specific Ratings** | Maintain separate skill ratings per platform with a cross-platform adjustment factor when players switch input devices |

### 1.4 Fairness & Integrity

| ID | Requirement | Description |
|---|---|---|
| FR-17 | **Smurf Detection** | Identify new accounts performing significantly above their rating tier and accelerate their MMR convergence |
| FR-18 | **Boosting Prevention** | Detect and flag accounts with suspicious rank trajectories, IP/hardware changes, or party patterns |
| FR-19 | **Rematch Avoidance** | Prevent the same players from being matched repeatedly within a configurable time window |
| FR-20 | **Report Integration** | Player reports for match quality issues feed into matchmaking adjustments |
| FR-21 | **Engagement-Aware Matching** | Incorporate behavioral signals (session length, return-after-loss rate) to prevent churn-inducing match sequences without compromising competitive integrity |
| FR-22 | **Trust Factor Scoring** | Maintain a per-player trust score combining behavior reports, abandonment history, and communication toxicity to create trust-segmented matching pools |
...
| Cross-play opt-in rate is ~60% | If higher, input-segmented pools thin out; if lower, cross-play infrastructure underutilized |
| Engagement model accuracy > 70% AUC | If lower, engagement factor adds noise rather than signal to matching decisions |
...
| Cross-play requires input-fair matchmaking | Platform/input pools fragment the population, increasing queue times |
| Trust factor must not create indefinite "shadow queues" | Low-trust players must still find matches within 2x normal queue time |
