...
### 3.5 Trap: "Optimize matchmaking for engagement, not skill"

**The trap:** Candidate proposes engagement-optimized matchmaking (EOMM) as the primary objective, not recognizing the ethical and competitive integrity risks.

**Strong answer:** "Engagement optimization is valuable but dangerous as a primary objective. Research has shown that unregulated EOMM can create manipulative matching — deliberately creating losing streaks to drive microtransactions, or manufacturing win streaks to create dopamine dependency. The correct architecture uses engagement as a secondary signal (weight 0.05-0.10 in the quality function) that acts as a tiebreaker when multiple skill-equivalent matches exist. Critical safeguards include: streak protection (never create >2 consecutive lopsided matches for the same player), audit trails logging engagement influence, and A/B testing gates requiring retention improvement WITHOUT decreased match quality. Skill fairness must remain the dominant objective."

### 3.6 Trap: "Party skill = average of members"
...
- **NP-hardness of team formation:** With constraints (parties, roles), optimal team assignment is a variant of balanced partition—show you know greedy Practical rule of thumb + local search is the practical approach
...
- Input method fairness (controller vs mouse/keyboard) — the primary segmentation axis, not platform
- Platform-specific pools by default, cross-platform opt-in with input-method awareness
- Skill calibration differs by platform (same player may be Gold on console, Platinum on PC)
- Separate ratings per platform with cross-platform adjustment factor (transfer factor varies by genre: 0.7 for strategy, 0.5 for aim-dependent shooters)
- Pool fragmentation risk: 3 input pools × 12 regions × 5 modes × N rank tiers — must use soft preferences, not hard partitions, to avoid starvation
- Aim-assist calibration: controller players with aim-assist in cross-input lobbies require separate skill offset to maintain fairness

### 6.4 Engagement-Optimized Matchmaking (EOMM)

The 2025+ evolution of matchmaking adds engagement as a secondary optimization axis:
- Churn prediction models identify at-risk players (long loss streaks, declining session lengths)
- Engagement score is a weighted factor (0.05-0.10) in the quality function — tiebreaker, not driver
- **Critical guardrail:** engagement optimization must never override skill fairness; audit trails log every engagement-influenced decision
- Ethical tension: unregulated EOMM can create manipulative patterns (manufactured win/loss streaks); architectural safeguards include streak protection caps and quarterly transparency reports
- Trust factor system: behavioral scoring feeds into soft pool segmentation, matching well-behaved players together without creating hard "shadow queues"
