...
| **Database (unique index)** | Durable; zero data loss risk | 5-10ms latency; Slowest part of the process at high write rates | Safety net only; primary dedup must be in-memory |
...
| Variant | What Changes | Key Challenge |
|---|---|---|
| **Election-grade voting** | End-to-end verifiability (E2E-V); homomorphic tallying; independent audit | Balancing vote secrecy with verifiability; performance of cryptographic operations at scale |
| **Prediction market polls** | Votes backed by virtual/real currency; dynamic odds calculation | Market maker algorithm; arbitrage prevention; financial integrity |
| **IoT sensor voting** | Millions of device "votes" (readings) per second; no human interaction | Device authentication; anomalous reading detection; edge aggregation |
| **Corporate board voting** | Weighted votes by share ownership; proxy delegation; quorum tracking | Delegation cycles; weighted shard counters; legal compliance |
| **Live TV audience poll** | Extreme burst (30M votes in 60 seconds); SMS + app channels | Multi-channel ingestion; SMS dedup by phone number; result display within seconds |
| **Social media story poll** | Ephemeral (24-hour TTL); embedded in content feed; anonymous by default | Efficient ephemeral storage; inline rendering; privacy-preserving analytics |
| **Multi-question survey** | Sequential questions with branching logic | State machine per respondent; partial submission; question ordering effects |
| **Consensus poll** | Voting continues until supermajority threshold reached | Dynamic close condition; convergence monitoring; strategic voting defense |

### Connecting to Other Systems

When discussing the polling system, demonstrate breadth by connecting to related system design problems:

| Connection Point | Related System | What to Say |
|---|---|---|
| "This sharded counter pattern is the same as..." | Like/View Counters | "...but with the critical addition of deduplication, which like counters don't need" |
| "The Bloom filter pattern appears in..." | Web Crawlers, Fraud Detection | "...any system where uniqueness must be checked at extreme throughput" |
| "The CQRS pattern here is similar to..." | Stock Trading (order book vs portfolio view) | "...but simpler because we don't have the temporal complexity of T+1 settlement" |
| "The hot poll isolation is like..." | Hot partition handling in databases | "...bulkhead pattern applied at the application level, not the storage level" |
| "The closing reconciliation resembles..." | End-of-day batch in banking | "...a constraint satisfaction problem: all sources must agree before results are final" |
...
|---|---|---|---|
| **Problem identification** | Treats as standard CRUD | Identifies write contention as central challenge | Quantifies exact contention ratio (25K writes/sec vs 1K row capacity) |
| **Data model** | Single counter per option | Sharded counters with random distribution | Adaptive sharding with velocity-based scaling |
| **Deduplication** | DB check before each vote | Distributed set with atomic SADD | Layered: Bloom L1 + distributed set L2 + DB L3 |
| **Consistency model** | All strong or all eventual | Mentions eventual consistency for results | Split model: strong for dedup, eventual for results, exact at close |
| **Hot poll handling** | "Just scale up" | Dedicated resources for hot polls | Proactive detection, pre-provisioning, adaptive shard scaling |
| **Fault tolerance** | "Use replicas" | Discusses specific failure modes | Audit log as source of truth; counter rebuild procedure; reconciliation |
| **Real-time results** | "Poll the database" | WebSocket with periodic push | Hierarchical fan-out, adaptive frequency, delta encoding |

---

## 10. System Comparison

| Dimension | Polling/Voting | Like/Reaction Counter | View Counter | Rate Limiter |
|---|---|---|---|---|
| **Write concentration** | Extreme (100K/sec on 1 entity) | High (1K/sec per post) | High (10K/sec per video) | Very High (millions of checks) |
| **Deduplication** | Critical (one-vote-per-user) | Important (one-like-per-user) | Not required (views can repeat) | N/A |
| **Accuracy requirement** | Exact (at poll close) | Approximate acceptable | Approximate acceptable | Must not over-count |
| **Time boundary** | Yes (polls open/close) | No (permanent) | No (permanent) | Yes (sliding window) |
| **Result visibility** | Real-time with freshness SLA | Delayed acceptable | Heavily delayed acceptable | Not user-visible |
| **Consistency model** | Split (strong dedup + eventual results) | Eventual everywhere | Eventual everywhere | Strong (must enforce limits) |
| **Reconciliation** | Required at close | Not applicable | Not applicable | Not applicable |
...
|---|---|---|
| Single row INCR capacity | ~1,000/sec | This is why sharded counters exist |
| Shards needed at 100K v/s, 4 options | 50-100 per option | 25K/option ÷ 1K/shard = 25 minimum |
| Bloom filter memory (10M voters) | ~12 MB | Fits on every ingestion node |
| Dedup set memory (10M voters) | ~160 MB | Single cache node handles one viral poll |
| MGET latency for 500 shards | ~2-6ms | Aggregation fits within 100ms budget |
| WebSocket fan-out at 1M subscribers | ~976 MB/s | Why hierarchical fan-out is required |
| Vote acceptance latency target | < 50ms P99 | Budget: dedup (~5ms) + queue publish (~10ms) + overhead |
| Aggregation interval range | 100ms–5s | Adaptive: hot vs cold polls |
| Poll close drain timeout | 30 seconds | Captures all in-flight votes |
| Cross-region dedup sync window | < 500ms | Theoretical duplicate window; L3 DB catches any |

---

## 12. Architecture Sketch Guide

For a whiteboard session, draw in this order:

1. **Start with the hot counter problem**: Draw a single database row receiving 100K writes/sec. Show why it fails.
2. **Introduce sharded counters**: Fan the single row into N shards. Show writes distributed randomly.
3. **Add CQRS split**: Draw the write path (shards) separate from the read path (materialized cache).
4. **Add aggregation bridge**: Show the background worker connecting shards → cache.
5. **Add dedup on the write path**: Insert Bloom filter → distributed set before the queue.
6. **Add WebSocket fan-out**: Show aggregation worker pushing to WebSocket gateway → clients.
7. **Add poll lifecycle**: Show the state machine with the critical `Closing` reconciliation state.

**Key arrows to emphasize:**
- The vote queue between dedup and counter service (async boundary)
- The aggregation worker between shards and cache (CQRS bridge)
- The dedup store check on the write path (correctness gate)

---

## 13. Follow-Up Deep Dives

| Topic | Likely Follow-Up | What They're Testing |
|---|---|---|
| **Sharded counters** | "What if a shard node fails?" | Partial failure handling; stale reads; rebuild from audit log |
| **Deduplication** | "What about the race condition between SISMEMBER and SADD?" | Knowledge of SADD as atomic CAS; understanding of concurrent programming |
| **Aggregation** | "How do you keep results fresh at 100K votes/sec?" | Adaptive frequency; incremental vs full aggregation trade-off |
| **Poll closing** | "What if a vote arrives during the close?" | Closing state; queue drain; reconciliation procedure |
| **Multi-region** | "How do you prevent cross-region duplicate votes?" | Per-region dedup + async sync; L3 DB safety net; geographic affinity |
| **Anonymous voting** | "How do you enforce one-vote-per-person without accounts?" | Composite fingerprint; honest about limitations; recommend auth for integrity |
| **Ranked-choice** | "How would you implement instant-runoff at scale?" | Understanding that ranked-choice requires storing preferences, not just counts |
| **Bot detection** | "How do you tell humans from bots?" | Multi-signal scoring; behavioral analysis; CAPTCHA as fallback |
...
|---|---|---|
| No mention of write contention | Missed the central challenge | "The defining problem is 100K writes/sec on a single entity" |
| "Use blockchain for integrity" | Adds latency, throughput limits, operational cost | "Hash-chained audit log provides tamper evidence without consensus overhead" |
| "Strong consistency everywhere" | Kills write throughput | "Strong for dedup, eventual for results—each has different correctness needs" |
| "Auto-scale will handle spikes" | Auto-scale is too slow for 30-second viral ramps | "Pre-provision for predictable events; auto-scale for sustained load only" |
| No mention of dedup | Allows double voting | "Dedup is a hard constraint—every vote must be checked for uniqueness" |
| "Just use a message queue count" | Queue count ≠ unique vote count | "At-least-once delivery means duplicate messages; explicit dedup is required" |
| Single counter per option | Fails at > 1K writes/sec | "Must shard counters across N keys for parallel writes" |
| No poll closure handling | Loses in-flight votes | "Need explicit Closing state with queue drain and reconciliation" |

---

---

## 15. 60-Second Elevator Pitch

If the interviewer says "give me the 60-second version":

> "The core problem is **write concentration**: millions of votes on a single entity. Use **sharded counters** for writes, **materialized cache** for reads, connected by async aggregation. **Dedup** via Bloom filter + distributed set + DB constraint. Split consistency: strong for dedup, eventual for results. Hot poll isolation with adaptive sharding. Closing state with queue drain and reconciliation for exact final results."
