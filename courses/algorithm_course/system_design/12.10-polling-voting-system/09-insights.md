...
|---|-------|----------|-------------|
| 1 | Sharded Counters Transform Write Contention | Contention | Distribute writes across N independent keys |
| 2 | CQRS Is Architecturally Necessary | System Modeling | Irreconcilable write/read models demand separation |
| 3 | Bloom Filters as Zero-Network-Cost Dedup | Data Structures | Probabilistic fast-path rejection backed by authoritative slow-path |
| 4 | Closing State as Reconciliation Phase | Consistency | Transitional state drains in-flight votes before marking results authoritative |
| 5 | Unidirectional Shard Scaling | Scaling | Safe to add shards; unsafe to remove during active poll |
| 6 | SADD as Lock-Free Compare-and-Swap | Atomicity | Atomic set-add return value solves concurrent dedup without locks |
| 7 | Hierarchical Fan-Out for WebSocket Scale | Streaming | Edge servers reduce Slowest part of the process from O(subscribers) to O(edge nodes) |
| 8 | Split Consistency Is Principled Design | Consistency | Strong for dedup, eventual for results—optimal allocation |
| 9 | Proactive Hot Poll Detection | Traffic Shaping | Detect and isolate before saturation, not after |
| 10 | Audit Log as Ultimate Source of Truth | Resilience | Counters are materialized views; audit log can rebuild everything |
| 11 | Anonymous Dedup Is Best-Effort | Security | Without trusted identity, dedup relies on bypassable heuristics |
| 12 | Adaptive Aggregation Frequency | Cost Optimization | Match computation to rate of change, not uniform interval |
| 13 | Cross-Region Dedup Accepts Small Window | Consistency | Per-region dedup + async sync; L3 DB catches theoretical duplicates |
| 14 | Idempotency Keys Transform Retries | Atomicity | Client-generated UUIDs make unreliable networks a non-issue |
...
|-------|----------|-------------|
| **Write contention as the central challenge** | #1, #5, #6 | Everything flows from the hot counter problem: sharded counters, adaptive scaling, and lock-free dedup are all responses to extreme write concentration on a single logical entity |
| **Layered correctness guarantees** | #3, #6, #8, #13 | Each correctness concern is addressed at multiple layers: Bloom filter → distributed set → DB constraint for dedup; per-region → cross-region → reconciliation for multi-region consistency |
| **Eventual consistency where acceptable** | #2, #4, #8, #12 | The system earns performance by being honest about where staleness is tolerable (result tallies) and where it's not (dedup), rather than choosing one consistency model for everything |
| **Audit log as the immutable backbone** | #10, #14 | Sharded counters, caches, and dedup sets are all reconstructible from the append-only audit log; this separation of facts (events) from derived state (counters) enables recovery from any corruption |
| **Proactive over reactive scaling** | #5, #9, #12 | By the time infrastructure is saturated, votes are already lost; detection, isolation, and adaptive frequency must trigger well before saturation thresholds |
| **Security at scale is probabilistic** | #3, #11 | Both Bloom filters (1% false positive rate) and anonymous dedup (fingerprint-based) are probabilistic; the system's integrity depends on layering imperfect defenses into a robust overall guarantee |

| **Security at scale is probabilistic** | #3, #11 | Both Bloom filters (1% false positive rate) and anonymous dedup (fingerprint-based) are probabilistic; the system's integrity depends on layering imperfect defenses into a robust overall guarantee |
| **Reconciliation as the final arbiter** | #4, #10, #13 | Despite all the eventual consistency and probabilistic layers during active voting, the close-time reconciliation and audit log guarantee that final results are exactly correct |
