...
### Algorithm 7: Weighted Vote Increment

For polls with weighted voting (e.g., verified accounts count 2×, premium accounts count 3×), the counter increment must apply the voter's weight.

```
FUNCTION increment_weighted_vote(poll_id, option_id, user, shard_count):
    // Determine vote weight based on voter tier
    weight = get_vote_weight(user, poll_id)
    ...
    KV_STORE.INCRBY(key, weight)

    // Also track raw vote count (unweighted) for transparency
    raw_key = FORMAT("raw_shard:%s:%s:%d", poll_id, option_id, shard_index)
    KV_STORE.INCR(raw_key)

    RETURN { weighted_increment: weight, shard: shard_index }

FUNCTION get_vote_weight(user, poll_id):
    settings = GET poll_settings WHERE poll_id = poll_id
    IF NOT settings.weighted_voting_enabled:
        RETURN 1

    weight_rules = settings.weight_rules  // e.g., [{tier: "verified", weight: 2}, ...]
    FOR EACH rule IN weight_rules:
...
### Algorithm 8: Vote Delegation (Liquid Democracy)

For polls supporting delegation, a voter can delegate their vote to another user, who may further delegate, creating a delegation chain.

```
FUNCTION resolve_delegation(user_id, poll_id):
    visited = SET()
    current = user_id
    chain = [user_id]

    WHILE TRUE:
        delegation = GET delegation WHERE
            delegator_id = current AND poll_id = poll_id

        IF delegation IS NULL:
            // No further delegation; current user casts the vote
            RETURN { voter: current, chain: chain, weight: LENGTH(chain) }

        IF delegation.delegate_id IN visited:
            // Cycle detected! Break the cycle; original user votes directly
            LOG_WARN("Delegation cycle detected: %s", chain)
            RETURN { voter: user_id, chain: [user_id], weight: 1, cycle_broken: TRUE }

        visited.ADD(current)
        current = delegation.delegate_id
        chain.APPEND(current)

        IF LENGTH(chain) > MAX_DELEGATION_DEPTH:  // e.g., 10
            RETURN { voter: current, chain: chain, weight: LENGTH(chain) }

    // The resolved voter casts with accumulated weight
```

### Algorithm 9: Bloom Filter Sizing and Rotation

```
FUNCTION create_bloom_filter(poll_id, expected_voters):
    // Size the filter based on expected voter count
    // Using 10 bits per element for 1% false positive rate
    bits_needed = expected_voters * 10
    hash_functions = 7  // Optimal for 10 bits/element

    // If expected_voters is unknown, estimate from creator's follower count
    IF expected_voters IS NULL:
        creator = GET creator FROM polls WHERE poll_id = poll_id
        expected_voters = creator.follower_count * 0.05  // 5% engagement rate
        bits_needed = MAX(expected_voters * 10, 100_000)  // minimum 100K bits

    filter = BLOOM_FILTER(size=bits_needed, hash_count=hash_functions)

    RETURN filter

FUNCTION rotate_bloom_filter(poll_id, old_filter):
    // When false positive rate exceeds threshold, create larger filter
    current_fp_rate = old_filter.estimate_false_positive_rate()

    IF current_fp_rate > 0.05:  // 5% threshold for rotation
        new_size = old_filter.size * 2
        new_filter = BLOOM_FILTER(size=new_size, hash_count=7)

        // Rebuild from dedup set (authoritative source)
        members = DEDUP_STORE.SMEMBERS(FORMAT("voted:%s", poll_id))
        FOR EACH member IN members:
            new_filter.ADD(member)

        LOG("Rotated Bloom filter for poll %s: %d → %d bits (FP rate: %.2f%%)",
            poll_id, old_filter.size, new_size, current_fp_rate * 100)

        RETURN new_filter

    RETURN old_filter  // No rotation needed
```
...
| Data | Active Phase | Warm Phase | Cold Phase | Deletion |
|---|---|---|---|---|
| **Sharded counters** | During active poll (KV store) | 7 days after close (KV store) | N/A (deleted after warm) | 7 days post-close |
| **Dedup set** | During active poll (in-memory) | 24 hours after close | N/A | 24 hours post-close |
| **Bloom filter** | During active poll (in-process) | N/A | N/A | Immediate on close |
| **Result cache** | During active poll (100ms–5s TTL) | 7 days after close (long TTL) | Rebuild from DB on miss | N/A (cache eviction) |
| **Vote audit log** | 3 months (hot, indexed DB) | 9 months (warm, compressed) | 2+ years (cold, object storage) | Per retention policy |
| **Poll metadata** | Indefinite (relational DB) | N/A | N/A | On user deletion request |
| **Analytics data** | 30 days (time-series DB) | 11 months (downsampled) | 2+ years (aggregated only) | Per retention policy |

### Archival Pipeline

```
FUNCTION archive_closed_poll(poll_id, closed_at):
    // Phase 1: Immediate (T+0)
    // Results already written to DB during close procedure
    // Sharded counters retained for 7 days (verification window)

    // Phase 2: 7 days post-close
    SCHEDULE_AT(closed_at + 7_DAYS):
        cleanup_sharded_counters(poll_id)
        cleanup_dedup_set(poll_id)

    // Phase 3: 90 days post-close
    SCHEDULE_AT(closed_at + 90_DAYS):
        archive_vote_records_to_cold_storage(poll_id)
        drop_hot_indexes_for_poll(poll_id)

    // Phase 4: On GDPR deletion request
    // Anonymize voter identities; preserve aggregate counts
    // Hash user references with non-reversible salt
```
