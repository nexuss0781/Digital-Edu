...
| Persona | Use Case | Scale | Key Requirements |
|---|---|---|---|
| **Casual Creator** | Personal opinion polls ("Where should we eat?") | 10–100 votes | Simple creation UX; share via link; no bot concern |
| **Content Creator** | Audience engagement polls on social media | 10K–1M votes | Embeddable widgets; real-time result animation; fraud resistance |
| **Media Outlet** | Breaking news opinion polls during live broadcasts | 100K–5M votes | Sub-second result freshness; verifiable integrity; brand-safe embedding |
| **Enterprise** | Employee surveys, product feedback, board voting | 1K–50K votes | SSO/SAML auth; organizational access control; data residency |
| **Event Organizer** | Live audience polls during conferences, concerts | 5K–100K votes in minutes | Time-bounded; QR-code access; anonymous voting; instant results |
| **Political Researcher** | Public opinion tracking, referendum-style polls | 1M+ votes | Ranked-choice support; demographic analytics; anti-manipulation |
...
|---|---|---|---|
| Vote: per user per poll | 1 | Poll lifetime | Core one-vote-per-user Rule that never changes |
| Vote: per user global | 60 | 1 minute | Prevent automated voting across polls |
| Vote: per IP per poll | 10 | 1 minute | Limit shared-network ballot stuffing |
| Vote: per IP global | 200 | 1 minute | Prevent bot farms |
| Poll creation: per user | 10 | 1 day | Prevent poll spam (Basic tier) |
| Poll creation: per user (Premium) | 100 | 1 day | Higher limit for paying customers |
| Result API: per client | 120 | 1 minute | Prevent result scraping |
| WebSocket subscriptions: per connection | 20 | Concurrent | Limit per-connection fan-out cost |

---

## 7. Latency Budget Breakdown (Vote Cast Path)

| Phase | Budget | Operations |
|---|---|---|
| **Network + TLS** | 5ms | DNS, TCP handshake, TLS negotiation (amortized) |
| **Parse & Validate** | 1ms | JSON deserialization, field validation, poll status check |
| **Rate Limit** | 2ms | Token bucket check against user+IP counters |
| **L1 Bloom Filter** | 0.01ms | In-memory probabilistic duplicate check |
| **L2 Dedup Store** | 5ms | Network round-trip to distributed cache (SISMEMBER + SADD) |
| **Queue Publish** | 10ms | Persistent write to vote queue with durability guarantee |
| **Response** | 1ms | Serialize response with result snapshot from cache |
| **Total Budget** | **< 50ms P99** | Conservative sum: ~24ms typical, headroom for tail latency |
...
|---|---|---|---|---|
| Registered users | 25M | 100M | 250M | 500M |
| DAU | 5M | 20M | 50M | 100M |
| Daily votes | 15M | 60M | 150M | 300M |
| Peak v/s (platform) | 520 | 2,083 | 5,208 | 10,416 |
| Peak v/s (single poll) | 25K | 100K | 250K | 500K |
| Active polls (concurrent) | 10K | 50K | 100K | 200K |
| Storage (annual, votes) | 350 GB | 1.3 TB | 3.3 TB | 6.5 TB |
| Dedup memory (peak) | 12 GB | 50 GB | 125 GB | 250 GB |

---

## 9. Failure Budget Allocation

| SLO | Monthly Budget | Allocation Strategy |
|---|---|---|
| 99.99% vote acceptance | 4.3 min/month downtime | Reserve 50% for planned maintenance; 30% for incidents; 20% for unknown |
| 100% vote durability | 0 lost votes | No budget—any vote loss triggers post-mortem |
| 100% dedup accuracy | 0 double-votes | No budget—any double-vote triggers immediate investigation |
| < 500ms result freshness | 1.5 hr/day staleness allowed | Allocate to aggregation pipeline restarts; cache rebuilds |
| 99.9% result serving | 43 min/month downtime | Reserve 70% for infrastructure changes; 30% for cache failures |

---

## 10. Platform Integration Points

| Integration | Direction | Protocol | Latency Budget | Purpose |
|---|---|---|---|---|
| **Social media embed** | Inbound | JavaScript widget + REST API | < 200ms page load | Polls embedded in social feeds |
| **Chat platform bot** | Bidirectional | Platform-specific bot API | < 1s response | Vote via chat commands |
| **Live streaming overlay** | Outbound | WebSocket push | < 500ms result update | Real-time poll results on stream |
| **Analytics export** | Outbound | Batch CSV/webhook | Near real-time to daily | Vote data for external BI tools |
| **Auth provider** | Inbound | OIDC/SAML | < 500ms auth flow | SSO for enterprise polls |
| **CAPTCHA service** | Outbound | REST API | < 2s challenge flow | Bot verification on suspicious votes |

---

## 11. Compatibility Requirements

| Environment | Requirement | Rationale |
|---|---|---|
| **Mobile web** | Vote in < 3 taps from poll view | Most voters arrive via mobile social links |
| **Low bandwidth** | Vote payload < 300 bytes; result payload < 600 bytes | Functional on 2G/3G networks |
| **Offline tolerance** | Queue vote locally; submit when online | Mobile users with intermittent connectivity |
| **Embedded iframes** | CORS-safe; CSP-compatible; no third-party cookies required | Polls embedded on third-party sites |
| **Screen readers** | ARIA labels on all interactive elements | Accessibility compliance |
| **API versioning** | v1/v2 concurrent support; 12-month deprecation window | Breaking changes without breaking integrations |
