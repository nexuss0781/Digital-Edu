...
### Runbook: Poll Close Failure

```
TRIGGER: Poll close procedure fails or times out

STEP 1: Identify failure point
    - Check if status transition to 'closing' succeeded
    - Check if queue drain timed out (30s limit)
    - Check if aggregation worker responded to final aggregation request

STEP 2: Manual intervention
    IF queue drain timed out:
        - Force-drain remaining messages (increase timeout to 120s)
        - If messages are stuck: identify poison message and skip/DLQ it
    IF aggregation failed:
        - Run manual aggregation: read all shards, sum, compare with dedup
    IF status transition failed:
        - Force status to 'closing' via admin API; retry close procedure

STEP 3: Verify integrity
    - Run verify_vote_integrity(poll_id)
    - Compare dedup count, shard total, and audit log count
    - If all match: proceed to mark as 'closed'
    - If mismatch: trigger manual audit before closing

STEP 4: Finalize
    - Write authoritative results to cache and database
    - Broadcast poll_closed event to WebSocket subscribers
    - Log close completion with reconciliation report
```
...
|---|---|---|
| **Emission** | Application SDKs in each service | Emit counters, histograms, gauges in standard format |
| **Collection** | Sidecar collectors on each node | Aggregate per-node metrics; batch-ship every 10s |
| **Aggregation** | Central metrics aggregation service | Pre-aggregate by service, region, poll_id |
| **Storage** | Time-series database (14-day hot, 90-day warm) | Store at 10s resolution (hot) and 1m resolution (warm) |
| **Query** | Dashboard and alerting layer | Sub-second query for real-time dashboards |

### Trace-to-Metric Correlation

| Trace Event | Derived Metric | Alert Threshold |
|---|---|---|
| `dedup_service.l2_check` span duration | `dedup_latency_p99` | > 10ms triggers investigation |
| `vote_queue.enqueue` span failure | `queue_publish_error_rate` | > 0.1% triggers page |
| `counter_service.increment` span duration | `counter_increment_latency_p99` | > 5ms triggers investigation |
| `aggregation_worker.aggregate` span duration | `aggregation_cycle_time_p99` | > 50ms triggers worker scaling |
| `websocket_gateway.push` message count | `ws_push_rate` | > 80% node capacity triggers scaling |
...
| SLA | Leading Indicator | Detection Window | Preemptive Action |
|---|---|---|---|
| 99.99% vote acceptance | Error rate trending upward over 15 minutes | 30 min before breach | Scale ingestion tier; activate backup dedup path |
| < 50ms vote latency (P99) | P90 latency > 35ms (approaching P99 budget) | 15 min before breach | Investigate dedup store latency; scale cache nodes |
| < 500ms result freshness | Aggregation cycle skips detected | 5 min before breach | Assign dedicated aggregation worker; reduce frequency only if CPU-bound |
| 100% vote durability | Queue consumer lag growing without processing | Immediate | Page engineering; check for poison messages; activate fallback consumers |

### Error Budget Burn Rate Dashboard

| Budget | 1-Hour Burn Rate | Action |
|---|---|---|
| **Healthy** | < 1× (consuming budget at expected rate) | No action |
| **Warning** | 1–3× (consuming budget faster than expected) | Ticket: investigate root cause |
| **Elevated** | 3–6× (on track to breach within days) | Page: proactive scaling or fix needed |
| **Critical** | > 6× (will breach within hours) | Page immediately: incident response |

---

## 10. War Room Triggers

| Trigger | Criteria | War Room Actions |
|---|---|---|
| **Vote integrity alert** | Dedup-shard mismatch > 0 on any poll | Assemble: SRE + backend + data eng; pause affected poll; investigate |
| **Platform-wide latency spike** | P99 vote latency > 200ms across all regions | Assemble: SRE + platform eng; activate load shedding; scale all tiers |
| **Mass bot attack** | > 30% bot detection rate platform-wide | Assemble: SRE + security + product; enable aggressive CAPTCHA; block IP ranges |
| **Multi-region outage** | > 1 region unhealthy simultaneously | Assemble: SRE + infrastructure; verify DR failover; communicate via status page |
| **High-profile poll failure** | Vote loss or result error on poll with > 1M votes | Assemble: SRE + backend + leadership; manual audit; public communication plan |

---

## 11. Key Health Indicators (KHIs)

These are the 5 metrics that, if healthy, indicate the entire system is operating correctly:

| KHI | Target | Degraded | Critical |
|---|---|---|---|
| **Vote acceptance P99 latency** | < 50ms | 50–200ms | > 200ms |
| **Dedup-shard count delta** | 0 (for closed polls) | 1–10 (for active polls) | > 10 (integrity concern) |
| **Aggregation freshness** | < 500ms | 500ms–5s | > 5s |
| **Queue consumer lag** | < 1s | 1–10s | > 30s |
| **Vote durability** | 100% | N/A | Any loss = P1 incident |

### Dashboard Hierarchy

| Level | Audience | Refresh Rate | Content |
|---|---|---|---|
| **Executive** | Leadership | 5 min | Total votes/day, active polls, platform health status (green/yellow/red) |
| **Operations** | SRE team | 10s | All 4 dashboards detailed above; KHIs; error budget burn rate |
| **Investigation** | Engineers | 1s | Per-poll deep dive; trace search; shard-level metrics |
| **Audit** | Compliance | On-demand | Per-poll integrity report; reconciliation status; admin action log |
