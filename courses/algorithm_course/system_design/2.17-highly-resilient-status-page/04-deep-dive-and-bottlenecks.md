...
| Slowest part of the process | Cause | Impact | Severity |
|------------|-------|--------|----------|
...
## Case Study: CDN Provider Outage During Major Incident

### Scenario

A large SaaS platform experiences a database failure during peak hours. The status page must communicate this to 500K subscribers. Simultaneously, the primary CDN provider experiences a regional PoP failure affecting 30% of traffic in Europe.

### Timeline: Without Resilient Architecture

| Time | Event | Impact |
|------|-------|--------|
| T+0s | Database failure detected | Application errors spike |
| T+30s | Status page receives automated alert | Incident created via API |
| T+45s | Users flood to status page | Traffic spikes 50x |
| T+60s | CDN PoP failure begins | European users cannot load status page |
| T+120s | CDN cache expires globally | All users see errors or timeouts |
| T+300s | Manual intervention begins | Status page fully down for 3+ minutes |

**Outcome**: Status page unavailable for ~5 minutes during the most critical period. Users resort to social media and direct support, overwhelming those channels.

### Timeline: With Resilient Architecture

| Time | Event | System Response |
|------|-------|----------------|
| T+0s | Database failure detected | Automated monitoring creates incident |
| T+5s | Incident written to CRDT database | Replicated to all 3 regions in <500ms |
| T+6s | Event store triggers Pub/Sub | Edge KV updated, CDN cache purged, SSE broadcast |
| T+10s | Notification orchestrator begins fanout | 500K subscribers queued across 100 shards |
| T+30s | Users begin checking status page | CDN serves from edge cache (Tier 1) |
| T+45s | Traffic spikes 50x | Request coalescing: 50K simultaneous requests = 1 origin fetch |
| T+60s | CDN PoP failure begins in Europe | Health check interval starts (10s) |
| T+80s | Second failed health check | DNS weight set to 0 for affected CDN |
| T+85s | DNS propagation complete | European traffic reroutes to backup CDN |
| T+85-145s | DNS TTL expiration window | Some EU users see stale-while-revalidate content (Tier 3) |
| T+145s | All traffic on healthy CDN providers | Full service restored, EU users see fresh data |
| T+180s | Email notifications 80% delivered | SMS 95% delivered (priority lane) |
| T+300s | All notification channels complete | 99.5% delivery rate across all channels |

### Key Design Decisions Validated

| Decision | How It Helped |
|----------|---------------|
| **Multi-CDN with DNS failover** | EU traffic rerouted within 85 seconds, not minutes |
| **Stale-while-revalidate** | Users saw last-known-good status during CDN switchover |
| **Request coalescing** | 50x traffic spike produced exactly 1 origin request per cache interval |
| **Pre-sharded notification queues** | 500K subscriber fanout completed within SLA despite traffic spike |
| **CRDT multi-region sync** | Incident visible in all regions within 500ms, no coordination Slowest part of the process |
| **Edge KV + Tier 1 rendering** | 95% of reads served from edge in <50ms despite origin stress |

### Metrics From the Incident

```
Status Page Availability During Incident: 99.97%
  - Tier 1 (Edge KV): 94.8% of requests
  - Tier 2 (Origin): 3.1% of requests
  - Tier 3 (Stale cache): 2.0% of requests
  - Tier 4 (Static fallback): 0.1% of requests

CDN Failover Duration: 85 seconds (EU region)
Notification Delivery (5 min): 99.5%
  - Email: 99.2% (rate-limited to 1000/sec)
  - SMS: 99.8% (priority lane)
  - Webhook: 98.9% (some endpoints slow)
  - Slack: 99.6%

Peak Traffic: 52x normal (absorbed by CDN)
Origin Load Increase: 1.3x (request coalescing)
```
