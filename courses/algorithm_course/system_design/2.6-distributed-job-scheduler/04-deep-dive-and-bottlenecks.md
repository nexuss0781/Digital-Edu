...
### Dependency Resolution (Step-by-step plan in plain English)
...
### Slowest part of the process 1: Database Polling Hotspot
...
### Slowest part of the process 2: Queue Contention
...
### Slowest part of the process 3: Worker Starvation/Overload
...
### Slowest part of the process 4: Execution History Growth
...
## Deep Dive 4: Timezone and DST Handling

Timezone handling is one of the most subtle correctness challenges in a distributed scheduler. Failures here are silent -- the job runs at the wrong time or is skipped entirely, with no error logged.

### The DST Problem

```
Spring Forward (US/Eastern, second Sunday in March):
  01:59:59 EST → 03:00:00 EDT
  2:30 AM does not exist

  Job scheduled: "30 2 * * *" in US/Eastern
  Expected: Run at 2:30 AM
  Reality: 2:30 AM is invalid

  Options:
  ├── SKIP: Do not run this occurrence
  ├── NEXT_VALID: Run at 3:00 AM (next valid time)
  └── RUN_AFTER_GAP: Run immediately when gap ends


Fall Back (US/Eastern, first Sunday in November):
  01:59:59 EDT → 01:00:00 EST
  1:30 AM happens twice

  Job scheduled: "30 1 * * *" in US/Eastern
  Expected: Run at 1:30 AM
  Reality: Which 1:30 AM?

  Options:
  ├── FIRST: Run at 1:30 AM EDT (first occurrence)
  ├── SECOND: Run at 1:30 AM EST (second occurrence)
  └── BOTH: Run at both occurrences
```

### Implementation Strategy

```
FUNCTION calculate_next_run(cron_expr, timezone, current_time, dst_policy):
    // Convert current time to user's timezone
    local_time = convert_to_timezone(current_time, timezone)

    // Find next cron match in local timezone
    next_local = cron_expr.next_match(local_time)

    // Validate the local time exists (DST gap check)
    IF NOT timezone.is_valid_time(next_local):
        SWITCH dst_policy.gap_behavior:
            CASE SKIP:
                // Find next valid cron match after gap
                next_local = cron_expr.next_match(gap_end_time)
            CASE NEXT_VALID:
                next_local = timezone.next_valid_time(next_local)
            CASE RUN_AFTER_GAP:
                next_local = gap_end_time

    // Check for DST overlap (ambiguous time)
    IF timezone.is_ambiguous_time(next_local):
        SWITCH dst_policy.overlap_behavior:
            CASE FIRST:
                utc_time = timezone.to_utc(next_local, prefer_early=TRUE)
            CASE SECOND:
                utc_time = timezone.to_utc(next_local, prefer_late=TRUE)
            CASE BOTH:
                // Schedule two executions
                schedule_both_occurrences(next_local, timezone)
                RETURN

    // Store as UTC for scheduling
    RETURN timezone.to_utc(next_local)
```
...
```
Problem:
  Scheduler A clock: 10:00:00.000
  Scheduler B clock: 10:00:00.350
  Database clock:    10:00:00.150

  Job due at 10:00:00
  Scheduler A sees it as due, dispatches
  Scheduler B doesn't see it as due yet (0.35s behind)

Mitigations:
1. NTP synchronization (< 10ms skew target)
2. Use database server time (NOW()) not application time
3. Safety buffer accounts for residual skew
4. Monotonic clocks for duration measurements
```

---

## Deep Dive 5: Multi-Tenant Fair-Share Scheduling

In multi-tenant environments, fair-share scheduling prevents resource monopolization while maximizing overall throughput.
...
{
    "tenant_a": { "weight": 40, "guaranteed_workers": 20, "max_workers": 100 },
    "tenant_b": { "weight": 30, "guaranteed_workers": 15, "max_workers": 80 },
    "tenant_c": { "weight": 20, "guaranteed_workers": 10, "max_workers": 50 },
    "tenant_d": { "weight": 10, "guaranteed_workers": 5,  "max_workers": 30 }
}

Allocation Algorithm (Work-Conserving):
1. Assign guaranteed workers to each tenant
2. Calculate surplus = total_workers - sum(guaranteed)
3. Distribute surplus proportional to weight
4. If tenant not using allocation, redistribute to others
5. Re-evaluate every 10 seconds
```

### Admission Control

```
FUNCTION admit_job(tenant_id, job):
    tenant = get_tenant_quota(tenant_id)
    current_usage = get_current_usage(tenant_id)

    // Check hard limits
    IF current_usage.concurrent_jobs >= tenant.max_concurrent:
        RETURN REJECTED("Concurrent job limit exceeded")

    IF current_usage.daily_executions >= tenant.max_daily:
        RETURN REJECTED("Daily execution limit exceeded")

    // Check fair-share allocation
    IF current_usage.active_workers >= tenant.current_allocation:
        // Over quota -- queue with lower priority
        enqueue_with_priority(job, priority=OVER_QUOTA)
    ELSE:
        // Within quota -- queue with normal priority
        enqueue_with_priority(job, priority=job.priority)
...
| Slowest part of the process | Primary Solution | Secondary Solution |
|------------|------------------|-------------------|
...
| DST correctness | UTC storage + local resolution | Configurable DST policy |
| Tenant starvation | Fair-share allocation | Admission control |
