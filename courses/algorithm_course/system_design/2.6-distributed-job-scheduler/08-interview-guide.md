...
Slowest part of the process, I'd shard by tenant ID—each tenant's jobs live on one shard.
...
| "Why not use durable execution (Temporal)?" | Different trade-off: DAG is simpler for ETL, durable execution is better for complex workflows with fine-grained recovery |
| "How do you handle event-driven triggers?" | Event ingestion layer with deduplication, mapped to registered triggers, complements cron polling |

---

## Case Study: E-Commerce Batch Processing Platform

### Scenario

An e-commerce platform needs to schedule 500K+ jobs daily including: inventory sync (every 15 min per seller), price recalculation (hourly), order fulfillment batches (every 5 min), daily financial reconciliation, and weekly analytics rollups.

### Architecture Choices

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Scheduling** | Partitioned active-active (3 schedulers) | 500K jobs exceeds single-leader capacity |
| **Partitioning** | By seller_id hash | Natural tenant boundary; seller jobs stay together |
| **Queue topology** | 3 priority tiers (critical/standard/batch) | Fulfillment is critical; analytics is batch |
| **Worker pools** | 4 pools (fulfillment, sync, compute, batch) | Different resource profiles per workload |
| **Execution guarantee** | At-least-once with idempotency | Financial reconciliation uses deduplication keys |

### Critical Design Decisions

```
Priority Classification:
  P0 (Critical):    Order fulfillment, payment processing
  P1 (Standard):    Inventory sync, price updates
  P2 (Batch):       Analytics, reports, ML training

Worker Pool Sizing:
  fulfillment-pool:  50 workers, never auto-scaled below 30
  sync-pool:         100 workers, auto-scaled 50-200
  compute-pool:      30 workers, high-CPU instances
  batch-pool:        variable, spot instances, 20-500

Peak Handling:
  Flash sales trigger 10x inventory sync volume
  Pre-scale sync-pool to 200 workers 30 min before sale
  Pause P2 batch jobs during sale window
  Resume batch backlog overnight
```

### Lessons Learned

1. **Seller-based partitioning creates hot partitions**: Top 1% of sellers generate 30% of jobs. Solution: add sub-partitioning within seller hash buckets based on job volume
2. **Financial reconciliation requires exactly-once semantics**: Solved with idempotency keys derived from (date, seller_id, reconciliation_type), not with distributed transactions
3. **Checkpoint granularity matters for large batch jobs**: Daily analytics over 100M orders needed checkpointing every 100K records, not every 30 seconds, because the per-checkpoint overhead dominated small batches
...
- "What's the expected number of scheduled jobs and executions per day?"
- "Are workflows primarily cron-based, event-driven, or DAG-based?"
- "Is this multi-tenant? How isolated do tenants need to be?"
- "What's the longest-running job we need to support?"

### To Show Depth

- "Do we need to support event-driven triggers alongside cron?"
- "What's the timezone distribution of users? Is DST handling critical?"
- "Is there a cost ceiling per execution or per tenant?"
- "How important is the order of execution within a DAG -- strict or best-effort?"

### To Show Production Thinking

- "How do we handle partial DAG failures -- does the whole workflow fail?"
- "What's the deployment strategy for scheduler upgrades without missed jobs?"
- "Are there compliance requirements that affect execution logging?"
- "What's the failure budget -- can we occasionally run a job twice, or is that unacceptable?"
