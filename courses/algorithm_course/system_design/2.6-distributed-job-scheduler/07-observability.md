...
```
Monthly Error Budget (99.99% availability):
  Total minutes: 43,200 (30 days)
  Allowed downtime: 4.32 minutes

Budget Tracking:
  Remaining budget: 2.1 minutes (51% consumed)
  Burn rate: 1.7x (above 1.0 = consuming faster than expected)

  Alert thresholds:
  ├── Burn rate > 2.0 for 5 min:  WARNING  (fast burn)
  ├── Burn rate > 10.0 for 1 min: CRITICAL (very fast burn)
  └── Budget < 1 minute remaining: CRITICAL (budget exhausted)
```

---

## Execution Lifecycle Tracing

A complete execution trace spans multiple services and can take minutes to hours. Correlating all events for a single execution requires a consistent identifier propagated across boundaries.

### Correlation Strategy

```
Identifiers propagated through execution lifecycle:

  trace_id:       End-to-end trace (API request to completion)
  execution_id:   Unique per execution attempt
  job_id:         Stable across all executions of a job
  dag_run_id:     Groups all tasks in a DAG execution
  tenant_id:      Enables per-tenant log filtering

Header propagation:
  API → Scheduler:   traceparent, X-Execution-ID
  Scheduler → Queue: trace_id embedded in message metadata
  Queue → Worker:    trace_id extracted from message metadata
  Worker → External: traceparent (W3C standard)
```

### Debugging Workflow

```
Scenario: User reports "my 9 AM job didn't run"

Investigation Steps:
1. Search by job_id + expected schedule time
   → Find: Was the job queried by the scheduler?

2. If queried: Check execution record
   → Find: Was an execution created? What status?

3. If execution created: Check queue
   → Find: Was the message enqueued? Consumer lag?

4. If enqueued: Check worker
   → Find: Did a worker pick it up? Heartbeats?

5. If picked up: Check execution logs
   → Find: Did it complete? Error? Timeout?

Common root causes:
├── Job was paused (status != ACTIVE)
├── DST gap skipped the execution
├── Scheduler failover during dispatch window
├── Worker crashed without checkpoint
