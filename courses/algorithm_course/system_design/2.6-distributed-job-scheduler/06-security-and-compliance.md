...
## Job Code Supply Chain Security

Workers execute job payloads that may include user-provided code or references to external dependencies. Securing this supply chain is critical.

### Threat: Malicious Job Payload

```
Attack Vectors:
1. Code injection via job parameters
   → Payload: {"command": "generate_report; rm -rf /"}
   → Mitigation: Never construct shell commands from parameters

2. Dependency confusion
   → Job references internal package name that attacker publishes publicly
   → Mitigation: Pin dependencies, use private registries with priority

3. Container image tampering
   → Job specifies compromised container image
   → Mitigation: Image signing (cosign/Notary), allow-listed registries only

4. Exfiltration via job output
   → Job reads secrets and writes them to execution output
   → Mitigation: Output scanning, network egress filtering
```

### Job Sandboxing Levels

| Level | Mechanism | Overhead | Use Case |
|-------|-----------|----------|----------|
| **L0** | No sandbox (trusted internal) | None | Internal ops jobs only |
| **L1** | Process isolation (namespaces) | Low | Standard tenant jobs |
| **L2** | Container with seccomp + AppArmor | Medium | Untrusted payloads |
| **L3** | MicroVM (Firecracker-style) | High | Code execution, multi-tenant SaaS |
