...
| Execution latency P99 | > 30 seconds | Investigate Slowest part of the process |
...
## Deployment Strategies for Schedulers

Upgrading a distributed scheduler without missing jobs or creating duplicate executions requires careful coordination.

### Rolling Deployment with Drain

```
Deployment sequence for 3-scheduler partition cluster:

1. Mark Scheduler-3 as draining
   └── Stops accepting new partition assignments
   └── Completes in-flight dispatches (max 60s)

2. Reassign Scheduler-3's partitions to Scheduler-1 and Scheduler-2
   └── Partition 6 → Scheduler-1
   └── Partition 7 → Scheduler-2
   └── Partition 8 → Scheduler-1

3. Deploy new version to Scheduler-3
   └── Health check passes
   └── Registers with coordination service

4. Rebalance partitions across all 3 schedulers
   └── Each scheduler gets 3 partitions again

5. Repeat for Scheduler-1 and Scheduler-2

Total deployment time: ~5 minutes
Jobs missed: Zero (partitions always assigned)
Duplicate risk: Low (drain ensures clean handoff)
```

### Blue-Green for Major Version Upgrades

```
When schema changes or protocol changes prevent rolling:

1. Deploy green cluster alongside blue (separate partition namespace)
2. Migrate tenants one at a time from blue to green
3. For each tenant:
   a. Pause job dispatch for tenant (< 1s gap)
   b. Reassign tenant's partitions to green cluster
   c. Green cluster resumes dispatch
4. After all tenants migrated, decommission blue cluster

Risk: Requires double infrastructure during migration
Benefit: Zero-downtime even for breaking changes
```
...
|----------|-------------------|------------|
| Kill scheduler leader | Standby takes over < 10s, no jobs missed | Count jobs dispatched vs expected |
| Kill 50% of workers | Queue depth increases, auto-scaling triggers | Jobs eventually complete, no permanent loss |
| Inject 5s network latency to DB | Scheduler polling slows, circuit breaker may trip | Jobs delayed but not lost |
| Partition coordination service | Split-brain prevention activates, one side fences | No duplicate dispatches |
| Fill database disk | Write failures trigger degraded mode | Existing queued jobs continue executing |
| Corrupt checkpoint data | Worker detects invalid checkpoint, restarts job from beginning | Job completes (with longer duration) |
