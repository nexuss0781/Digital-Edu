...
|-----------|---------------|---------------|
| **Cardinality explosion** | Series count grows combinatorially with labels; unbounded labels (user_id) can create billions of series overnight | Flexibility (any label) vs. safety (enforced caps) |
| **Gorilla compression assumptions** | 12x compression depends on regular timestamps and stable values; degrades silently for irregular data | Compression efficiency vs. data model generality |
| **Head block memory pressure** | Each active series consumes ~320 bytes of memory for index + chunk; 100M series = 32 GB | Memory (more series per node) vs. cost (more nodes) |
| **Compaction contention** | Background compaction competes with ingestion and queries for disk I/O and CPU | Write throughput vs. query performance vs. compaction speed |
| **Out-of-order ingestion** | Push-based agents introduce clock skew and network delays; rejecting late data loses information | Memory (OOO buffer) vs. data loss (reject late samples) |
| **Multi-resolution stitching** | Queries spanning raw + downsampled tiers must produce correct aggregations | Aggregation correctness (sum/count tuple) vs. simplicity (single resolution) |
| **Meta-monitoring paradox** | TSDB cannot monitor itself without circular dependency | Operational simplicity vs. independent meta-monitor infrastructure |

## Emerging Trends (2025-2026)

| Trend | Description | Impact |
|-------|-------------|--------|
| **Columnar TSDB engines** | InfluxDB 3.0 with Apache Arrow + Parquet; QuestDB with columnar storage | Decouples write format from read format; enables data lake interop |
| **Native histograms** | Single-series distribution storage (22x cardinality reduction) | Makes histogram monitoring practical at scale |
| **AI-driven anomaly detection** | ML models trained on metric streams for automated root cause analysis | Extends TSDB from passive storage to active intelligence |
| **OpenTelemetry convergence** | Unified metrics, logs, traces in common format; OTLP as standard protocol | Simplifies ingestion; enables cross-signal correlation |
| **Serverless TSDB** | Pay-per-query pricing with auto-scaling; no infrastructure management | Reduces ops burden; shifts cost model |
| **Edge TSDB** | Lightweight TSDB running at the edge, syncing to cloud | Enables offline-capable IoT monitoring with eventual consistency |

## System Characteristics Summary

| Characteristic | Value |
|----------------|-------|
| Primary data flow | Append-only, time-ordered writes → range-based aggregation reads |
| Consistency model | Eventual (within seconds); AP choice |
| Write:Read ratio | 100:1 to 1000:1 at ingestion layer |
| Key scaling constraint | Cardinality (series count), not volume (bytes) |
| Compression ratio | 12x typical (Gorilla); degrades to 2-3x for irregular data |
| Primary Slowest part of the process | Index memory (proportional to active series count) |
| Partitioning axis | Time (immutable blocks per time range) |
| Query engine model | Inverted index → posting list intersection → chunk scan → aggregation |
| Storage tiers | Hot (head block) → Warm (local blocks) → Cold (object storage) |
| Retention strategy | Tiered: full resolution → 5-min rollup → 1-hr rollup |

## When to Choose This Architecture

| Scenario | Choose TSDB | Choose Alternative |
|----------|-----------|-------------------|
| Dense, regular metric streams | TSDB (excellent compression, purpose-built) | General DB wastes storage at this scale |
| Need sub-second metric freshness | TSDB (head block → immediate queryability) | Batch ETL if minutes acceptable |
| High-cardinality label queries | TSDB inverted index (posting list intersection) | Search engine if full-text needed |
| Sparse event data (< 100 events/min) | Event store or log system | TSDB overhead not justified |
| Need complex joins across entities | Relational or analytical DB | TSDBs have no join capability |
| String/text values | Log aggregation | Gorilla only works on numeric float64 |
| Random-access point lookups | Key-value store | TSDBs optimized for range scans, not point reads |
| Need ACID transactions | Relational DB | TSDBs prioritize throughput over transaction guarantees |
