...
- No single instance is a Slowest part of the process for a popular bank — the ring assigns multiple instances per bank
...
### DR Scenario: Data Center Loss During Peak Hours

```
Timeline for full DC-A failure during Diwali peak (32K TPS):

T+0s:     DC-A goes offline. Health probes from GLB fail.
T+5s:     GLB marks DC-A unhealthy after 3 consecutive probe failures.
T+6s:     All traffic redirected to DC-B. DC-B absorbs 100% load.
T+6-30s:  In-flight transactions at DC-A timeout. PSPs retry with same RRN.
          DC-B dedup store returns cached results for already-completed txns.
          New attempts processed normally at DC-B.
T+30s:    Full traffic convergence. DC-B processing at 32K TPS (within 50K capacity).
T+2min:   Settlement engine standby at DC-B promoted to active.
T+5min:   Operational team confirms DC-B stable. Alert ops to investigate DC-A.
T+30min:  DC-A root cause identified. Begin recovery if hardware intact.

User impact: 5-30s of elevated failure rates for in-flight transactions at DC-A.
             No committed transaction data lost (synchronous replication).
             New transactions succeed within 6 seconds of failure detection.
```
...
|-----------|--------|-------------------|-----------|
| Kill random switch instances | Switch cluster | Load balancer redirects; no user-facing impact | Weekly |
| Simulate bank CBS timeout (top 5 banks) | Per-bank circuit breaker | Circuit opens within 30s; transactions fail gracefully | Monthly |
| Inject 50ms latency to transaction store writes | Transaction state store | Latency SLO degrades but within 30s hard timeout | Monthly |
| Partition between data centers | DC-to-DC link | Surviving DC absorbs 100% traffic within 30s | Quarterly |
| Corrupt VPA cache entries | VPA resolution | Cache miss detected; synchronous fallback to PSP lookup | Monthly |
| Simulate settlement file corruption | Settlement engine | Hash chain validation catches it; regenerate from transaction log | Quarterly |
| Trigger simultaneous mandate execution spike | Mandate processing pool | Mandate pool scales independently; P2P/P2M unaffected | Monthly |
| Simulate deduplication store partition failure | Dedup store | Persistent tier serves as fallback; latency increases to ~10ms | Quarterly |

---

## Capacity Planning Summary

| Component | Current Capacity | Peak Utilization | Headroom | Scale Trigger |
|-----------|-----------------|-----------------|----------|---------------|
| Switch instances | 50K TPS | 32K TPS (festival) | 36% | TPS > 70% sustained 60s |
| Transaction state store | 100K writes/sec | 96K writes/sec (peak) | 4% (tight) | p99 write latency > 100ms |
| VPA cache cluster | 1B entries | 400M entries | 60% | Hit rate < 95% |
| Deduplication store | 200K ops/sec | 64K ops/sec | 68% | Lookup p99 > 10ms |
| Settlement workers | 8 parallel windows | 6 active windows | 25% | Queue depth > 10M |
| Message queue | 300K msgs/sec | 256K msgs/sec | 15% | Consumer lag > 5s |
| Audit log store | 1B writes/day | 700M writes/day | 30% | Storage utilization > 80% |
| PKI signature operations | 250K ops/sec | 192K ops/sec | 23% | Signature latency > 5ms |
