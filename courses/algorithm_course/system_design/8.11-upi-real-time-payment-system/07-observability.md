...
| `psp_queue_depth_high` | Any PSP queue depth > 10,000 for 5 minutes | Notify PSP; check for downstream Slowest part of the process |
...
## 8. UPI Lite Observability

UPI Lite introduces unique observability challenges because transactions occur on-device without real-time server visibility.

| Metric | Source | Description | Alert Threshold |
|--------|--------|-------------|-----------------|
| `upi_lite.sync_backlog` | Server | Pending unsynced Lite transactions | > 100K across all wallets |
| `upi_lite.avg_offline_duration` | Sync event | Average time between syncs | p95 > 24 hours |
| `upi_lite.reconciliation_delta` | Sync engine | Balance mismatch post-sync | Any non-zero delta > ₹1 |
| `upi_lite.active_wallets` | Server | Wallets with balance > 0 | N/A (tracking metric) |
| `upi_lite.topup_volume` | Transaction store | Lite top-up value per hour | Spike > 5x baseline |
| `upi_lite.sync_failure_rate` | Sync engine | Failed sync attempts | > 2% of sync events |
...
- Explain how UPI Lite transactions are observed when they happen offline and only become visible at sync time
