...
|-----------|---------------|-------------------|
| **Central switch as throughput Slowest part of the process** | Every transaction traverses NPCI; no bilateral shortcuts | Horizontal partitioning of switch by bank-pair or VPA-hash; active-active multi-datacenter deployment |
...
| [8.12 - CBDC Digital Currency Platform](../8.12-cbdc-digital-currency-platform/) | Offline NFC payments, programmable money, central bank settlement |
...
|---------|-----------------|
| **Hub-and-Spoke** | NPCI central switch routes all inter-participant messages; eliminates N² bilateral integrations |
| **Saga / Compensating Transactions** | Auto-reversal protocol undoes partial failures (debit succeeded, credit failed) via compensating debit reversal |
| **Idempotent Operations** | RRN + UPI Request ID guarantee exactly-once processing across network retries and switch failovers |
| **Circuit Breaker** | Per-bank circuit breakers isolate slow or failing CBS systems from affecting the overall payment network |
| **Stateless Processing** | Switch instances hold no in-process state; all transaction state externalized to distributed store |
| **Event Sourcing** | Every message leg logged as immutable event for audit trail, dispute resolution, and settlement reconciliation |
| **Bulkhead Isolation** | Separate processing pools per transaction type (P2P, P2M, Mandate, Lite) prevent cross-contamination |
| **Two-Phase Commit (soft)** | Debit-then-credit with auto-reversal acts as a soft 2PC with guaranteed eventual consistency |
| **Protocol Extension** | Credit Line on UPI extends existing message format with lending fields without changing routing infrastructure |
| **Hardware Root of Trust** | Device-bound cryptographic keys and SIM binding create possession-factor authentication without dedicated hardware |
