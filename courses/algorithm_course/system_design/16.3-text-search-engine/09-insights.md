...
## Insight 11: Disaggregated Storage Transforms Search Cluster Economics by Decoupling the Compute-Storage Scaling Axis

**Category:** Cost Optimization

**One-liner:** Separating segment storage (on object storage) from query execution (on stateless compute nodes) enables independent scaling of each dimension, reducing total cost of ownership by 40-60% for time-series and log-based search workloads.

**Why it matters:** Traditional search clusters tightly couple compute and storage on every data node: adding storage requires adding CPU and memory, and adding query capacity requires provisioning disk that may go unused. This coupling creates a "provision for the larger dimension" problem---a write-heavy workload needs many data nodes for disk throughput even if CPU is underutilized, while a query-heavy workload needs nodes for CPU/memory even if disk is mostly empty. Disaggregated storage (pioneered by searchable snapshots and serverless search offerings) moves segment data to object storage, where durability is provided by the storage layer (11 nines) rather than by replicas. Compute nodes become stateless with a local SSD cache for hot segments. This has three transformative effects: (1) cold/warm-tier data costs $0.01-0.02/GB/month on object storage vs. $0.10-0.25/GB/month on local NVMe, a 5-25x reduction; (2) compute nodes can be scaled to zero during off-peak hours (serverless model); and (3) recovery from node failure takes seconds (reassign shard to new compute node, cache warms from object storage) instead of minutes (copy segment files from replica). The trade-off is higher query latency for cache-miss segments (50-200ms for initial fetch from object storage), making this approach ideal for cold-tier data where occasional latency is acceptable, but not yet suitable for hot-tier search where sub-20ms p50 is required.

---

## Insight 12: The Analysis Chain Is the Most Underappreciated Architectural Decision---It Determines Both Recall Quality and Index Size, and Cannot Be Changed Without Full Reindex

**Category:** Search

**One-liner:** The choice of tokenizer, stemmer, synonym expansion, and stop-word filtering at index time permanently shapes what queries can match which documents, and changing the analysis chain requires reindexing the entire corpus.

**Why it matters:** When a document is indexed, the analysis chain transforms raw text into a sequence of tokens that become entries in the inverted index. This transformation is permanent---the original text is stored in `_source`, but the inverted index only contains the analyzed tokens. If "running shoes" is stemmed to "run shoe", then a query for "running" will find it (because the query is also stemmed to "run"), but a query for the exact phrase "running shoes" will match on "run shoe" (which works for phrase queries because positions are preserved, but with caveats for multi-word synonyms). The implications cascade: choosing an aggressive stemmer increases recall (more matches) but decreases precision (irrelevant matches); adding synonyms at index time expands the index size by 20-50% but improves recall; removing stop words reduces index size by 10-20% but breaks phrase queries containing stop words ("to be or not to be" becomes meaningless). These decisions interact: stemming + synonym expansion + stop-word removal creates a combinatorial space of possible token sequences that must be mentally modeled to predict query behavior. And the critical constraint: the analysis chain is baked into the inverted index. To change it (e.g., add a new language stemmer, change the synonym list, switch from standard to custom tokenizer), you must reindex every document---an operation that takes hours to days for billion-document indexes. This makes the analysis chain the highest-consequence, lowest-frequency decision in search system design, and the one most likely to be made hastily during initial setup and regretted later.
