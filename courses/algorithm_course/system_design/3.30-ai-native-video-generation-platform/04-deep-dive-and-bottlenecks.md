...
### Slowest part of the process 1: VRAM for Long Videos
...
### Slowest part of the process 2: Temporal Consistency Degradation
...
### Slowest part of the process 3: 3D VAE Decoding Latency

**Problem:** VAE decode is non-parallelizable Slowest part of the process
...
### Slowest part of the process 4: Multi-GPU Synchronization Overhead
...
### Slowest part of the process 5: Model Cold Start and Cache Thrashing

**Problem:** Loading a 10B parameter DiT model from storage to GPU takes 15-30s, creating unacceptable latency when a worker needs a different model than what's cached.

```
Model Loading Times:

Model Size    NVMe → GPU    S3 → GPU     Impact
5B (10 GB)    5s             30s          Acceptable
10B (20 GB)   12s            60s          Significant
14B (28 GB)   18s            90s          Critical
30B+ (60 GB)  30s            180s         Unacceptable
```

**Mitigations:**

| Strategy | Loading Savings | Trade-off | Implementation |
|----------|----------------|-----------|----------------|
| Model pinning per pod | Eliminates loads | Less flexibility | Dedicate pods per model |
| NVMe pre-staging | 80% vs S3 | NVMe capacity | Cache popular models locally |
| Speculative loading | Hide latency | VRAM overhead | Load next model during generation |
| Model sharding | Load partial | Complexity | Only load active expert shards (MoE) |
| Lazy quantization | 50% transfer time | Marginal quality | Transfer INT8, dequantize on GPU |

---

## Deep Dive 7: Mixture-of-Experts (MoE) for Video DiT

### Why MoE for Video Generation

Dense DiT models face a scaling wall: quality improves with parameters, but VRAM and compute scale linearly. MoE breaks this constraint by activating only a fraction of parameters per token:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    MoE Video DiT Architecture (Race-DiT)                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Input: Video Patch Token [D=4096]                                          │
│                    │                                                        │
│                    ▼                                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │                                                                      │   │
│  │  Route to 2-of-8 experts based on token content                     │   │
│  │  Token type determines routing:                                     │   │
│  │    • Static background → Experts 0, 3 (spatial focus)               │   │
│  │    • Fast motion → Experts 1, 5 (temporal focus)                    │   │
│  │    • Fine detail → Experts 2, 6 (high-frequency)                    │   │
│  │    • Transitions → Experts 4, 7 (scene change)                      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                    │                                                        │
│           ┌───────┴───────┐                                                │
│           ▼               ▼                                                │
│  ┌─────────────┐  ┌─────────────┐                                         │
│  │  Expert 1    │  │  Expert 5    │  (selected by router)                  │
│  │  FFN 4096→   │  │  FFN 4096→   │                                        │
...
│         ▼                ▼                                                 │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Weighted Sum: output = g1 × expert1(x) + g5 × expert5(x)          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  Parameters: 8 experts × 4096×16384×2 = 1B per MoE layer                  │
│  Active per token: 2 experts = 250M params                                │
│  Effective capacity: 4× dense equivalent                                   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### MoE Load Balancing for Video

```
LOAD_BALANCING_CHALLENGE:

  Problem: Router learns to send most tokens to a few "popular" experts,
  leaving others underutilized. This is worse for video than text because
  video has spatial/temporal locality -- nearby patches are similar.
  ...
  Solution: Auxiliary load-balancing loss
    ...
    Where:
      ...
      α = 0.01 (balance weight)
      N = number of experts

  Video-Specific Enhancement:
    - Temporal diversity: ensure each temporal chunk uses diverse experts
    - Spatial diversity: ensure experts see patches from all spatial regions
    - Prevents "expert collapse" where temporal experts ignore spatial info

  Expert Utilization Target: Each expert handles 10-15% of tokens
  (ideal: 12.5% for 8 experts, k=2)
```

### MoE vs Dense Comparison

| Metric | Dense 10B | MoE 30B (2-of-8) | Improvement |
|--------|-----------|-------------------|-------------|
| **Effective Parameters** | 10B | 30B | 3× capacity |
| **Active Parameters** | 10B | ~8B | 20% less compute |
| **VRAM (weights)** | 20 GB | 60 GB | 3× more memory |
| **VRAM (active)** | 20 GB | 16 GB | 20% less active |
| **Quality (VBench)** | 0.91 | 0.94 | +3% |
| **Inference Time** | 45s | 38s | 15% faster |

---

### Slowest part of the process Summary

| Slowest part of the process | Severity | Primary Mitigation | Cost |
|------------|----------|-------------------|------|
...
| Model cold starts | Medium | NVMe pre-staging + model pinning | Flexibility trade-off |
