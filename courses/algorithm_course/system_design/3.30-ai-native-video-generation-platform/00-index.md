...
This is an **AI-autonomous system** operating within a sandboxed environment. AI drives core decision-making and execution autonomously, with hard boundaries preventing cross-system effects. Human supervisors monitor via dashboards and can intervene through circuit-breaker controls. The platform autonomously generates, edits, and composes video content within its rendering sandbox — outputs require editorial review before publication.
...
| **Human Override** | Creative directors review generated content; safety filters auto-block policy-violating outputs | Domain expert |
...
| Primary Slowest part of the process | VRAM for 3D latents, temporal attention O(T^2) | GPU compute during denoising |
...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, APIs, core algorithms (DiT, TSAM, 3D VAE Step-by-step plan in plain English) |
...
### Generation Performance by Model (2025-2026 Landscape)
...
| **Wan 2.1** | DiT (14B) | 1080p | 5-20s | None | ~40s | 48 GB |
| **Kling 2.0** | DiT + 3D Reconstruction | 1080p | 10s | Post-sync | ~25s | 40 GB |
| **CogVideoX-5B** | DiT (5B) | 720p | 6s | None | ~15s | 24 GB |
...
| **Pika Labs 2.0** | Optimized for creative workflows, scene-aware editing | Consumer + API |
| **Wan 2.1 (Alibaba)** | 14B DiT, open-source, strong motion coherence | Open-source |
| **Kling 2.0 (Kuaishou)** | 3D reconstruction-aware generation, camera control | Consumer + API |
| **Minimax Hailuo** | Long-form generation, character consistency | Consumer |
...
## Case Study: Open-Source Video Generation at Scale (2025-2026)

**Challenge:** Open-source video generation models (Mochi 1, Open-Sora 2.0, CogVideoX, Wan 2.1) democratized access to 10B+ parameter video DiTs, but serving them at consumer scale required solving GPU orchestration, checkpoint recovery, and temporal consistency without proprietary infrastructure.

**Architecture Decisions:**
- **Model serving:** Adapted vLLM's paged attention for 3D temporal attention, reducing VRAM fragmentation by 40% for variable-duration requests
- **Progressive generation:** Implemented hierarchical generation (keyframes first at full quality, then interpolation at reduced steps) to serve 30s videos within 45GB VRAM
- **Fleet heterogeneity:** Mixed H100 (cinema tier), A100 (standard), and consumer GPUs with NVFP8 quantization for preview tier
- **Safety pipeline:** Combined open-source ConceptGuard with C2PA reference implementation for regulatory compliance

**Results:** Achieved 100K videos/day on a 200-GPU mixed fleet (vs. 2000+ GPUs for equivalent proprietary quality), with 99.2% checkpoint recovery success rate and <120ms audio sync using post-processing alignment.

**Key Lesson:** Open-source video generation at scale is viable but requires fundamentally different orchestration than image generation -- the 100x longer job durations (minutes vs seconds) make checkpoint-based recovery and fair queuing non-negotiable, while tiered hardware routing is essential for cost sustainability.

---

## Related Systems

- [3.20 AI Image Generation Platform](../3.20-ai-image-generation-platform/00-index.md) - Image generation (compare 2D vs 3D latent, shared DiT architecture)
- [3.23 LLM Inference Engine](../3.23-llm-inference-engine/00-index.md) - Inference optimization patterns (KV cache vs temporal attention cache)
- [3.22 AI Guardrails & Safety System](../3.22-ai-guardrails-safety-system/00-index.md) - Content safety architecture (video-specific deepfake detection)
- [3.19 AI Voice Assistant](../3.19-ai-voice-assistant/00-index.md) - Audio generation patterns (shared vocoder architecture)
- [1.15 Content Delivery Network (CDN)](../1.15-content-delivery-network-cdn/00-index.md) - Video delivery infrastructure (HLS/DASH streaming)
- [1.12 Blob Storage System](../1.12-blob-storage-system/00-index.md) - Large video file storage (multi-tier lifecycle)
- [2.4 CI/CD Pipeline & Build System](../2.4-cicd-pipeline-build-system/00-index.md) - GPU cluster orchestration patterns (model deployment pipelines)
- [1.5 Distributed Log-Based Broker](../1.5-distributed-log-based-broker/00-index.md) - Queue patterns for long-running job management
