...
|--------|-------------|-----------|--------|
| **Subject Consistency** | Face/object identity across frames | <0.85 | Flag for model investigation |
| **Motion Smoothness** | Optical flow variance between frames | >2.0 std dev | Check temporal attention |
| **Temporal Flickering** | Pixel intensity variance over time | >0.15 | Check VAE decoder |
| **Aesthetic Score** | CLIP aesthetic predictor | <5.0 | Check guidance scale |
| **Text Alignment** | CLIP similarity (prompt vs frames) | <0.25 | Check text encoder |
| **Audio-Visual Sync** | Cross-correlation peak offset | >120ms | Check joint attention |
...
QUALITY_MONITORING_PIPELINE:
  ...
  Sample Rate: 1% of completed generations (randomly sampled)

  Per-Sample Analysis:
    1. Extract 10 evenly-spaced frames
    2. Compute VBench metrics subset:
       - Subject consistency (face/object identity)
       - Motion smoothness (optical flow)
       - Temporal flickering (pixel variance)
    3. Compare against model-specific baselines
    4. Flag if any metric drops >10% below baseline

  Aggregate Analysis (hourly):
    - Mean quality scores per model
    - Quality vs duration correlation
    - Quality vs GPU utilization (detect thermal throttling)
    - Quality drift detection (sudden drops = model issue)

  Alert:
    IF mean_quality < baseline - 2*std_dev for 1 hour:
        ALERT: Quality degradation detected
        ACTION: Check model weights, VAE health, GPU temperatures
```
