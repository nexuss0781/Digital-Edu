...
|--------|-------------|------------|
| **Video-to-video style transfer attacks** | Users upload real videos of people and request "style transfer" to bypass face detection | Detect real faces in V2V inputs, require consent for identifiable individuals |
| **Audio deepfake coupling** | Generate convincing video + audio of public figures making statements | Cross-reference generated speech content against known figures; flag political content |
| **Multi-generation evasion** | Generate slightly different versions iteratively until safety detectors miss | Rate-limit per-user, track prompt similarity, flag rapid regeneration patterns |
| **Watermark removal services** | Third-party tools strip C2PA manifests and invisible watermarks | Multi-layer watermarking (frequency + temporal), forensic fingerprinting in latent space |
| **Synthetic training data poisoning** | Generated videos used to train models that inherit biases | Track generation provenance, restrict bulk API access patterns |
