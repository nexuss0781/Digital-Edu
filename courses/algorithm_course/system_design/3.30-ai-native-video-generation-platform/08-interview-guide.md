...
### Trap 6: "How would you support 2-minute videos without running out of GPU memory?"

**What Interviewer Wants:** Understanding of long-form generation strategies and memory management

**Best Answer:**
> "Three complementary strategies:
>
> 1. **Hierarchical generation:** Generate sparse keyframes first (every 2-4 seconds) at full quality using full temporal attention. Then interpolate intermediate frames between keyframes using reduced-step diffusion, where each interpolation chunk only needs to attend to its bounding keyframes.
>
> 2. **Sliding window + anchor frames:** Replace full temporal attention (O(T^2), impossible at 2880 frames) with a sliding window of 16 recent frames plus anchor frames every 32 frames. This gives O(T×W) complexity where W=16, making memory bounded regardless of duration.
>
> 3. **Chunked VAE decoding:** Decode the video in temporal chunks of 1+T/4 frames, caching intermediate features for cross-chunk consistency. Each chunk fits in a fixed VRAM budget."

---

### Trap 7: "Why not use Mixture-of-Experts to reduce inference cost?"

**What Interviewer Wants:** Understanding of sparse architectures and their trade-offs for video

**Best Answer:**
> "MoE is actually very promising for video DiT -- Race-DiT and Veo 3 likely use it. With 8 experts and top-2 routing, you get 4× the effective parameters while activating only 25% per token. But video has unique MoE challenges:
>
> 1. **Spatial/temporal locality:** Nearby video patches are similar, so routers tend to send whole regions to the same expert, causing load imbalance. You need auxiliary losses to enforce diversity.
>
> 2. **VRAM trade-off:** Even though you activate fewer parameters, all expert weights must be in memory. A 30B MoE needs 60 GB for weights versus 20 GB for a 10B dense model. The compute savings come at a memory cost.
>
> 3. **Expert specialization:** The benefit is that experts naturally specialize -- some handle static backgrounds, others handle motion, others handle fine texture. This is more useful for video than text because the input diversity is higher."
