...
### Slowest part of the process 1: Feed Crawler Throughput
...
### Slowest part of the process 2: DAI Stitching Latency
...
### Slowest part of the process 3: Search Index Update Latency
...
### Slowest part of the process 4: CDN Cache Fragmentation from DAI

**Problem:** SSAI produces a unique audio stream per listener (different ads inserted). This means each stitched response is uncacheable at the CDN — defeating the purpose of edge caching for ad-supported content.

**Mitigation:**
- **Segment-based caching** — Cache content segments and ad segments separately at CDN; stitch at edge
- **Common ad manifests** — For popular episodes, pre-compute top 10 ad combinations; cache each variant
- **Stitching at edge PoPs** — Deploy lightweight DAI stitchers at CDN edge to keep stitched audio close to listeners
- **Premium bypass** — Premium (ad-free) users get direct CDN-cached audio (no fragmentation)
- **Manifest caching** — Cache the stitching manifest (ad placement decisions) separately from audio; reuse for similar listener cohorts

### Slowest part of the process 5: Transcription GPU Contention

**Problem:** AI transcription (Whisper-based) requires GPU compute. At 50K new episodes/day, a backlog can build during publishing spikes (e.g., "New Music Friday" for music podcasts, or news events triggering mass publishing).

**Mitigation:**
- **Priority queuing** — Top-subscribed shows get GPU priority; long-tail episodes transcribed in batch
- **Spot/preemptible GPU instances** — Use spot pricing for non-urgent transcription (60-70% cost savings)
- **Multi-model fallback** — Use faster (lower-accuracy) model during spikes; re-transcribe with high-quality model during off-peak
- **External transcript ingestion** — If feed includes `<podcast:transcript>` tag (Podcast 2.0), use creator-provided transcript; skip GPU entirely
- **Batch scheduling** — Accumulate episodes and transcribe in optimized batches (GPU parallelism is more efficient than one-at-a-time)

### Deep Dive 4: AI-Generated Podcast Pipeline

The emergence of text-to-podcast tools (NotebookLM-style) creates a new content creation path that differs fundamentally from traditional RSS ingestion:

```
AI Podcast Generation Pipeline:
├── INPUT PROCESSING
│   ├── Accept: article URL, PDF, raw text, or topic prompt
│   ├── Content extraction: readability parsing, table extraction, image OCR
│   ├── Token limit: max 50K tokens (roughly 100 pages)
│   └── Safety check: reject prohibited topics before expensive generation
│
├── SCRIPT GENERATION (LLM)
│   ├── Analyze source: identify key themes, arguments, data points
│   ├── Structure: introduction, main segments, conclusion
│   ├── Dialogue format: Host asks questions, Guest explains
│   │   ├── Natural turn-taking (3-8 sentences per turn)
│   │   ├── Clarification exchanges ("Wait, so you're saying...")
│   │   └── Summary/transition phrases between topics
│   ├── Target duration control: adjust depth per segment
│   └── Fact-check pass: verify claims against source material
│
├── SPEECH SYNTHESIS (Neural TTS)
│   ├── Voice assignment: distinct voice per speaker
│   ├── Prosody injection: emphasis markers from script
│   ├── Filler insertion: "um", "you know" (0.5-2% of words)
│   ├── Pacing variation: faster for excitement, slower for emphasis
│   └── Cross-talk: slight overlaps at turn boundaries (50-100ms)
│
├── AUDIO POST-PROCESSING
│   ├── Loudness normalization: -16 LUFS (EBU R128)
│   ├── Noise floor: add subtle room tone for naturalness
│   ├── Compression: gentle dynamic range compression
│   └── Mastering: final loudness and format encoding
│
└── PUBLICATION
    ├── Metadata: AI-generated flag mandatory in RSS
    ├── Source attribution: link to original content
    ├── Moderation: queue for review if deepfake score > 0.5
    └── Standard pipeline: transcode, CDN, search index

Latency budget:
├── Script generation: 30-60s (LLM inference)
├── Speech synthesis: 2-5 min for 15-min episode (GPU TTS)
├── Post-processing: 30s
└── Total: ~3-6 min for a 15-min episode
```

### Slowest part of the process 6: Video Podcast Storage and Delivery

**Problem:** Video podcasts are 10-50× larger than audio (500MB-2GB per episode). Adding video support to even 10% of the catalog would add 20+ PB of storage and significantly increase CDN egress costs.

**Mitigation:**
- **Lazy video transcoding** — Only transcode video variants when the episode receives its first video playback request (not on ingestion)
- **Adaptive resolution** — Fewer video quality variants for long-tail (480p only); full ladder (1080/720/480) for top shows
- **Audio-first CDN** — Video served from separate CDN tier with different cost optimization (lower availability tolerance for video vs audio)
- **Video retention policy** — Purge video variants after 6 months of no video playback; keep audio indefinitely
- **Client-side bandwidth detection** — Aggressively push audio-only for mobile data connections
