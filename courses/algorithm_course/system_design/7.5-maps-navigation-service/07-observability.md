...
| Pipeline | Input Signal | Detection Method | Alert Action |
|---|---|---|---|
| **CDN miss storm** | CDN cache hit rate drops below 95% | Sudden decrease detector (z-score > 3) | Page infra on-call; activate stale-serve mode |
| **Traffic data poisoning** | Speed observations per segment | Outlier detection (median absolute deviation) | Quarantine suspicious probes; roll back segment speeds |
| **Route quality regression** | Route distance ratio (actual vs expected) | Drift detector over 15-min window | Alert routing team; compare canary vs baseline |
| **Geocoding accuracy drop** | Sampled result confidence scores | Moving average drops below threshold | Alert data team; check for index corruption |
| **Tile generation regression** | Tiles-per-hour and generation latency | Baseline comparison with seasonal adjustment | Alert pipeline team; check data source integrity |
| **ETA accuracy degradation** | Post-trip ETA vs actual duration ratio | Running accuracy below 80% over 1-hour window | Alert traffic team; check real-time vs historical blend weights |

---

## Subscriber-Facing Observability

### API Health Dashboard (Exposed to Enterprise Clients)

```
┌─────────────────────────────────────────────────────────┐
│  Maps Platform Status — Last 24 Hours                   │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Tile API      ████████████████████████████ 99.99% ✓    │
│  Route API     ███████████████████████████░ 99.96% ✓    │
...
│                                                         │
│  Incidents:                                             │
│  [RESOLVED] 03:12-03:18 UTC — Route API elevated        │
│  latency (p99 > 3s) due to graph redeploy. Impact: 0.04%│
│  of routing requests experienced degraded performance.  │
│                                                         │
│  Upcoming Maintenance:                                  │
│  2026-03-22 02:00 UTC — CH graph rebuild (no user impact│
│  expected; canary deployment with automatic rollback)    │
└─────────────────────────────────────────────────────────┘
```
...
|---|---|---|---|---|
| **SEV-1** | Complete service outage for a major subsystem | CDN total failure; all routing down; graph corruption affecting > 50% queries | < 5 min page | Status page update within 10 min; exec notification |
| **SEV-2** | Significant degradation to a major subsystem | CDN hit rate < 90%; routing latency > 5s p99; traffic freshness > 30 min | < 15 min page | Status page update within 30 min |
| **SEV-3** | Minor degradation, limited user impact | ETA accuracy < 80%; geocoding accuracy < 90%; single region routing slow | < 30 min warn | Internal notification; no public status update |
| **SEV-4** | Potential issue, no current user impact | CH rebuild taking > 6 hours; Redis shard memory > 90%; Kafka lag > 50K | Next business day | Ticket created; prioritized in sprint |
...
Impact: [% users affected] [which subsystem] [geographic scope]

Timeline:
  [HH:MM] — Detection: [how detected — alert, customer report, manual]
  [HH:MM] — Response: [first responder actions]
...
Lessons:
  - What went well: [detection speed, failover, communication]
  - What could improve: [monitoring gaps, runbook gaps, automation]
```
...
Real-time visualization of request distribution across the globe, critical for capacity planning:

```
┌─────────────────────────────────────────────────────────┐
│  Global Request Heatmap — Live                          │
│                                                         │
│  Tile req/sec by region:                                │
│  ■ NA: 12.1M   ■ EU: 9.8M   ■ APAC: 10.5M            │
│  ■ LATAM: 1.8M ■ MEA: 0.8M                            │
│                                                         │
│  Route req/sec by region:                               │
│  ■ NA: 18K     ■ EU: 15K    ■ APAC: 20K               │
│  ■ LATAM: 3K   ■ MEA: 2K                              │
│                                                         │
│  Hotspots (current):                                    │
│  🔴 New York: 2.1× normal (rush hour)                   │
│  🔴 Tokyo: 1.8× normal (rush hour)                      │
│  🟡 London: 1.3× normal (moderate traffic)              │
│  🟢 Sydney: 0.7× normal (off-peak)                      │
└─────────────────────────────────────────────────────────┘
```
...
| Metric | Trending Window | Action Trigger |
|---|---|---|
| CDN egress growth rate | 30-day rolling | > 15% month-over-month → plan CDN contract expansion |
| Route Service memory utilization | 7-day rolling | > 80% across fleet → plan graph partitioning or machine upgrade |
| Kafka partition throughput | 14-day rolling | > 70% of per-partition limit → plan partition expansion |
| Redis shard memory | 7-day rolling | > 75% → plan shard split or TTL reduction |
| Tile origin compute | 30-day rolling | > 60% sustained → add origin capacity |
| Object storage growth | 30-day rolling | Track for cost forecasting (tiles are additive) |

These capacity forecasting signals feed into quarterly infrastructure planning reviews, ensuring the platform scales ahead of demand rather than reactively.
