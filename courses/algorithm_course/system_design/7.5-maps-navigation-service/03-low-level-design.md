...
### 8. EV-Constrained Route Planning

```
FUNCTION planEVRoute(origin, destination, vehicle, initialSoC):
    // Phase 1: Compute unconstrained shortest path
    basePath = queryRoute(origin, destination, hierarchicalGraph)

    // Phase 2: Simulate energy consumption along path
    remainingSoC = initialSoC
    chargingStops = []

    FOR EACH edge IN basePath.edges:
        // Energy model: f(distance, elevation_delta, speed, temperature, payload)
        energyUsed = computeEnergyConsumption(edge, vehicle)
        remainingSoC -= energyUsed

        IF remainingSoC < vehicle.minSafeBuffer:  // typically 10-15%
            // Find nearest compatible charging station
            station = findNearestCharger(
                edge.midpoint,
                vehicle.connectorType,
                maxDetour = 10km
            )

            // Compute optimal charging amount
            // Charge just enough to reach next station or destination
            targetSoC = computeOptimalChargeTarget(
                station, destination, vehicle, remainingPath
            )
            chargingDuration = station.chargeCurve(remainingSoC, targetSoC)

            chargingStops.append({
                station: station,
                arrivalSoC: remainingSoC,
                departureSoC: targetSoC,
                chargingTime: chargingDuration
            })
            remainingSoC = targetSoC

    // Phase 3: Re-route through charging stops
    finalRoute = buildRouteWithStops(origin, chargingStops, destination)

    RETURN {
        route: finalRoute,
        chargingStops: chargingStops,
        totalDrivingTime: finalRoute.drivingTime,
        totalChargingTime: SUM(stop.chargingTime FOR stop IN chargingStops),
        arrivalSoC: remainingSoC
    }
```

### 9. Tile Delta Invalidation

```
FUNCTION invalidateTilesForChange(changeGeometry):
    // Compute bounding box of the geographic change
    bbox = computeBoundingBox(changeGeometry)

    affectedTiles = []

    // For each zoom level, find affected tiles
    FOR zoom FROM 0 TO MAX_ZOOM:
        topLeft = latLngToTile(bbox.north, bbox.west, zoom)
        bottomRight = latLngToTile(bbox.south, bbox.east, zoom)

        FOR x FROM topLeft.x TO bottomRight.x:
            FOR y FROM topLeft.y TO bottomRight.y:
                affectedTiles.append((zoom, x, y))

    // Batch delete from object storage
    objectStorage.batchDelete(
        [formatTileKey(z, x, y) FOR (z, x, y) IN affectedTiles]
    )

    // Send CDN cache purge (batch API for efficiency)
    cdnPurgeUrls = [
        formatTileURL(z, x, y) FOR (z, x, y) IN affectedTiles
    ]
    cdn.batchPurge(cdnPurgeUrls)

    // Proactively regenerate high-priority tiles (popular areas)
    FOR EACH (z, x, y) IN affectedTiles:
        IF z <= 12 OR isPopularTile(z, x, y):
            tileQueue.enqueue((z, x, y), priority=HIGH)

    RETURN {
        tilesInvalidated: LENGTH(affectedTiles),
        proactiveRegenQueued: countHighPriority
    }
```
...
|---|---|---|---|
| **Object Storage (tiles)** | `{zoom}/{x}/{y}` path prefix | N/A (inherently distributed) | Tile address IS the storage key |
| **Redis (traffic)** | `edge_id` hash | 64 shards | Even distribution; co-locates temporal data per edge |
| **Spatial DB (geocoding)** | Country → Region | 12 regional clusters | Locality of geocoding queries; users search their region |
| **Session Store** | `session_id` hash | 32 shards | Even distribution; each session accessed by one user |
| **Time-Series DB** | `edge_id` + time range | 128 shards | Range queries per edge efficient; time-based retention easy |

### Index Design

```
Spatial DB Indexes:
  - B-Tree on Address(country, city, street) — structured lookup
  - GiST spatial index on Address(lat, lng) — reverse geocoding
  - GIN full-text index on Address(display_name, alt_names) — fuzzy forward geocoding
  - GIN full-text + GiST compound on POI(name, lat, lng) — location-biased search

Redis Key Patterns:
  - traffic:{edge_id}:{bucket}          → current speed data (TTL: 2h)
  - traffic:{edge_id}:incidents         → active incidents (TTL: 24h)
  - route_cache:{hash(origin,dest,mode)} → cached route (TTL: 5min)
  - tile_coalesce:{zoom}:{x}:{y}       → request coalescing lock (TTL: 30s)
```
...
|---|---|---|---|
| **L1: Client** | `tile-{z}-{x}-{y}-{etag}` | 12 hours | Device-local tile cache |
| **L2: CDN Edge** | `/tiles/{z}/{x}/{y}.mvt` | 12 hours (invalidation-aware) | Global edge cache |
| **L3: Origin Cache** | `tile:{z}:{x}:{y}` | Indefinite (invalidated on change) | Avoid regeneration |
| **Route Cache** | `route:{hash}` | 5 min | Short-lived; traffic changes invalidate quickly |
| **Geocode Cache** | `geocode:{normalized_query}:{region}` | 24 hours | Address data rarely changes |
| **Autocomplete** | `ac:{prefix}:{region}` | 1 hour | Popular prefix results |
