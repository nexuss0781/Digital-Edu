...
```
Step-by-step plan in plain English: Credential Management
...
```
Step-by-step plan in plain English: SSRF Prevention in Connector Proxy
...
| **DORA (EU)** | Financial services | Operational resilience testing, ICT risk management, incident reporting |
| **FedRAMP** | US Government customers | Standardized security assessment, continuous monitoring |

### DORA Compliance Considerations (EU Financial Sector)

For European financial services customers, the Digital Operational Resilience Act (DORA, effective January 2025) imposes requirements on ICT service providers:

| DORA Requirement | Platform Implementation |
|-----------------|------------------------|
| **ICT risk management** | Threat model documentation, annual penetration testing, vulnerability scanning |
| **Incident reporting** | Major incident notification within 4 hours; root cause analysis within 1 month |
| **Operational resilience testing** | Annual DR drills, chaos engineering exercises, third-party audit |
| **Third-party risk management** | Connector proxy security assessment, LLM provider risk evaluation |
| **Information sharing** | Participation in sector-specific threat intelligence sharing |
...
The AI Copilot introduces new attack surfaces that require specific controls:

| Threat | Mitigation |
|--------|------------|
| **Prompt injection via schema** | Sanitize table/column names before including in LLM context; reject names containing control characters or prompt-like patterns |
| **Generated SQL injection** | All AI-generated queries pass through the same parameterization pipeline as human-written queries; no special path |
| **Data leakage to LLM** | Only schema metadata (table names, column names, types) sent to LLM; never actual data rows, credentials, or query results |
| **Jailbreak via user prompt** | System prompt instructs LLM to generate only valid app definition JSON; output validated against schema before use |
| **LLM credential exposure** | LLM API keys stored in the platform's credential store (same envelope encryption as connector credentials); never sent to client |

---

## Expression Injection Prevention

Binding expressions (`{{...}}`) are a common attack vector in no-code platforms. The expression evaluator must prevent injection attacks where user-controlled data is interpreted as code.

```
Step-by-step plan in plain English: Safe Expression Evaluation

FUNCTION evaluate_expression_safely(expression, context):
    // Step 1: Parse expression into AST (not string eval)
    ast = parse_to_ast(expression)

    // Step 2: Validate AST nodes against allowlist
    FOR node IN ast.walk():
        IF node.type == "CallExpression":
            IF node.callee NOT IN ALLOWED_FUNCTIONS:
                RAISE ExpressionError("Function not in allowlist")
        IF node.type == "MemberExpression":
            IF node.property == "__proto__" OR node.property == "constructor":
                RAISE ExpressionError("Prototype access not allowed")

    // Step 3: Evaluate against frozen context
    result = ast.evaluate(deep_freeze(context))
    RETURN result

// ALLOWED_FUNCTIONS: filter, map, reduce, find, includes,
//   slice, indexOf, toString, toFixed, toUpperCase, toLowerCase,
//   trim, split, join, startsWith, endsWith, JSON.parse, JSON.stringify
```

**Why AST-based evaluation**: String-based evaluation (e.g., passing strings to eval-like constructs) allows attackers to break out of the expression context. AST-based parsing ensures that only valid expressions (member access, function calls, arithmetic) are evaluated, and the allowed function set is explicitly controlled. This is the lesson from CVE-2025-68613: n8n used string-based expression evaluation, which allowed prototype chain traversal to reach system-level command execution.

---

## Supply Chain Security for Custom Components

Custom components (iframe-embedded or SDK-built) introduce third-party code into the platform:

| Control | Implementation |
|---------|---------------|
| **Iframe sandboxing** | `sandbox="allow-scripts"` attribute; no `allow-same-origin` (prevents parent DOM access) |
| **CSP headers** | Strict Content-Security-Policy on custom component iframes; `connect-src 'self'` only |
| **Communication channel** | `postMessage` API with origin validation; messages schema-validated before processing |
| **Component review** | Org-level marketplace with admin approval for new custom components |
| **Resource limits** | Custom component iframes have CPU/memory limits enforced by the browser |
| **Subresource integrity** | SDK-based components loaded with SRI hashes; tampering detected |
...
| 10 | Sandbox code generation primitives (eval-like constructs) removed | Required |
| 11 | Expression evaluator uses AST-based parsing, not string evaluation | Required |
| 12 | AI-generated queries pass through same parameterization pipeline as human queries | Required |
| 13 | Custom component iframes sandboxed without `allow-same-origin` | Required |
| 14 | Schema metadata sent to LLM; never actual data rows or credentials | Required |
| 15 | Workflow step results do not contain credentials or tokens in error messages | Required |

---

## Incident Response for No-Code Platforms

| Incident Type | Detection | Immediate Action | Post-Incident |
|--------------|-----------|-----------------|---------------|
| **Sandbox escape** | Crash rate spike, unexpected outbound connections | Disable transforms for affected app; isolate org | V8 patch, expression audit, root cause analysis |
| **SSRF exploitation** | Blocked URL alerts, unusual connector destinations | Block connector; review all connectors for affected org | Tighten URL validation rules, update blocklist |
| **Cross-tenant leakage** | Org isolation validation failure | Halt affected queries; engage incident response | Full audit of data access paths; customer notification |
| **Credential exposure** | Audit log anomaly, HSM access spike | Rotate all affected credentials; revoke sessions | Key rotation, access review, customer notification |
| **Mass query injection** | Parameterization failure alerts | Disable affected connector type; review all queries | Patch parameterization logic, regression tests |
