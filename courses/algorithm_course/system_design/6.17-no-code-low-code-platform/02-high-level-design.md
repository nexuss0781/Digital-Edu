...
### Flow 4: AI-Assisted App Generation
...
sequenceDiagram
    ...
    B->>AI: "Build an order management dashboard<br/>connected to our production DB"
    AI->>DCS: Introspect schema for connected DB
    DCS-->>AI: Tables: orders, customers, products<br/>Columns: id, status, total, customer_id...

    AI->>LLM: Generate app definition<br/>Context: schema, component library,<br/>binding syntax, user prompt
    LLM-->>AI: Proposed app definition JSON<br/>(pages, components, queries, bindings)

    AI->>AI: Validate generated definition<br/>(type-check bindings, verify table/column refs)
    AI-->>B: Preview of generated app

    B->>B: Review, adjust components in builder
    B->>ADS: PUT /apps/{id}/definition (save adjusted version)
    ADS->>MDS: Store definition
    ADS-->>B: Saved
```

### Flow 5: Workflow Execution (Server-Side Automation)

```mermaid
%%{init: {'theme': 'neutral'}}%%
sequenceDiagram
    participant T as Trigger (Webhook/Schedule/Event)
    participant WE as Workflow Engine
    participant QEE as Query Execution Engine
    participant DCS as Data Connector Service
    participant MQ as Message Queue
    participant ALS as Audit Log Store

    T->>WE: Trigger workflow "onNewOrder"
    WE->>WE: Load workflow definition<br/>(steps, conditions, error handlers)

    WE->>QEE: Step 1: Run query "getOrderDetails"
    QEE->>DCS: Execute against connector
    DCS-->>QEE: Order data
    QEE-->>WE: Step 1 result

    WE->>WE: Step 2: Evaluate condition<br/>(order.total > 10000?)

    alt High-value order
        WE->>QEE: Step 3a: Run "notifyManager" (REST API call)
        QEE-->>WE: Notification sent
    else Standard order
        WE->>QEE: Step 3b: Run "autoApprove" (SQL UPDATE)
        QEE-->>WE: Order approved
    end

    WE->>MQ: Audit: workflow.completed
    MQ->>ALS: Persist audit event
    WE-->>T: Workflow completed (3 steps, 450ms)
```
...
flowchart TB
    ...
    subgraph Region1["Primary Region"]
        subgraph Compute1["Compute"]
            AG1[API Gateway]
            QEE1[Query Execution<br/>Engine x10]
            V8P1[V8 Isolate<br/>Pool]
            WE1[Workflow<br/>Engine]
            AIC1[AI Copilot<br/>Service]
        end
        subgraph Data1["Data"]
            MDS1[(Metadata Store<br/>Primary)]
            MDS1R[(Read Replicas<br/>x4)]
            CS1[(Credential<br/>Store)]
            CACHE1[(Distributed<br/>Cache)]
            ALS1[(Audit Log<br/>Store)]
        end
        subgraph Proxy1["Connector Proxy"]
            DCS1[Data Connector<br/>Service x8]
            CP1[Connection<br/>Pools]
        end
    end

    subgraph Region2["Secondary Region (Read-Heavy)"]
        AG2[API Gateway]
        QEE2[Query Execution<br/>Engine x4]
        MDS2R[(Read Replicas<br/>x2)]
        CACHE2[(Distributed<br/>Cache)]
        DCS2[Data Connector<br/>Service x4]
    end
    ...
    classDef data fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef cache fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef proxy fill:#e0f7fa,stroke:#00695c,stroke-width:2px

    class CDN,GLB edge
    class AG1,QEE1,V8P1,WE1,AIC1,AG2,QEE2 compute
    class MDS1,MDS1R,CS1,ALS1,MDS2R data
    class CACHE1,CACHE2 cache
    class DCS1,CP1,DCS2 proxy
```
...
| Approach | Latency | Context Quality | Cost | Offline Support |
|----------|---------|----------------|------|-----------------|
| **Server-side LLM proxy (Chosen)** | 1-5s generation | Full schema + component library context | Platform-managed | No |
| **Client-side LLM API calls** | 1-5s | Limited by browser context | User pays directly | No |
| **Fine-tuned model per platform** | 0.5-2s | Platform-native understanding | High training cost | Possible with small models |

**Decision**: Server-side LLM proxy. The AI Copilot Service sends the builder's natural language prompt along with enriched context (connected database schema, component library catalog, binding syntax reference, existing app structure) to an LLM. The response is validated against the app definition schema before presenting to the builder. This keeps API keys server-side, enables context enrichment that would be impossible client-side, and allows the platform to swap LLM providers without client changes.

### Decision 7: Environment & Branching Model

| Approach | Complexity | Safety | Developer Experience |
|----------|-----------|--------|---------------------|
| **Copy-on-write branching (Chosen)** | Medium | High (isolated changes) | Good (familiar git-like mental model) |
| **In-place editing with undo** | Low | Low (no isolation) | Simple but risky |
| **Full git-backed storage** | High | Very high | Complex for non-developers |

**Decision**: Copy-on-write branching. When a builder starts editing a published app, the platform creates a development branch (a copy of the current published definition). Changes are made to the branch. Publishing merges the branch to the published slot. This prevents accidental production breakage during editing and supports multiple builders working on different features of the same app. Merge conflicts are resolved at the component level (last-write-wins for same component, auto-merge for different components).
...
| **Single-tenant vs. Multi-tenant** | Multi-tenant with org-level isolation; dedicated infra for enterprise tier | Cost efficiency for smaller orgs; isolation guarantees for large ones |
| **Monolith vs. Microservices** | Service-oriented: Query Engine, Connector Proxy, Permission Engine, AI Copilot as separate services | Independent scaling; fault isolation between planes |
| **Cache-aside vs. Write-through** | Cache-aside for app definitions (read-heavy); write-through for permissions (consistency-critical) | Different consistency requirements per data type |

---

## Component Interaction Matrix

| Component | Depends On | Depended By | Failure Impact |
|-----------|-----------|-------------|----------------|
| **API Gateway** | — | All clients | Total outage for affected region |
| **App Runtime Service** | Cache, Metadata Store | End-users | Deployed apps unavailable |
| **Query Execution Engine** | Connector Proxy, Sandbox, Permissions | App Runtime, Workflow Engine | Queries fail; UI shows stale/empty data |
| **Permission Engine** | Cache, Metadata Store | Query Engine, App Runtime | All permission checks fail; fail-closed |
| **Data Connector Service** | Credential Store, Customer DBs | Query Engine | Queries to affected connector fail |
| **V8 Isolate Pool** | — | Query Engine | Transforms unavailable; raw data fallback |
| **AI Copilot Service** | LLM Provider, Connector Service | Builder App | AI features degrade; manual building unaffected |
| **Workflow Engine** | Query Engine | External triggers | Automations stop; manual execution still works |
| **Collaboration Service** | WebSocket infra | Builder App | Presence indicators disappear; editing continues |
