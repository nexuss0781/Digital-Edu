...
### Disaster Recovery Tiers by Component

| Component | DR Tier | Strategy | Justification |
|-----------|---------|----------|---------------|
| **Metadata Store** | Tier 1 (Critical) | Multi-AZ sync replication + cross-region async | App definitions are the platform's core value |
| **Credential Store** | Tier 1 (Critical) | Multi-AZ + HSM replication | Without credentials, no queries can execute |
| **Query Execution Engine** | Tier 2 (Important) | Stateless; restart in new region | No state to recover; needs credential store and cache |
| **Distributed Cache** | Tier 3 (Replaceable) | Cold start from metadata store | Cache can be rebuilt; brief latency spike acceptable |
| **Audit Log Store** | Tier 2 (Important) | Cross-region streaming replication | Compliance requirement; up to 1 min data loss acceptable |
| **AI Copilot Service** | Tier 3 (Replaceable) | Failover to alternative LLM provider | AI features are non-critical; manual building continues |
...
When a popular app definition's cache entry expires or is invalidated, hundreds of concurrent requests may simultaneously miss the cache and hit the database.

```
Step-by-step plan in plain English: Singleflight Cache Fetch

STRUCTURE InflightRequest:
    key: string
    waiters: list[channel]
    result: any
    error: any

inflight_map = ConcurrentHashMap()

FUNCTION get_with_singleflight(cache, key, fetch_func):
    // Fast path: cache hit
    ...
    existing = inflight_map.put_if_absent(key, new InflightRequest(key))
    IF existing IS NOT NULL:
        // Another goroutine/thread is already fetching this key
        RETURN existing.wait()  // Block until leader completes

    // This thread is the leader: fetch from source
    TRY:
        result = fetch_func(key)
        cache.set(key, result, ttl=300)
        inflight_map[key].complete(result)
        RETURN result
    CATCH error:
        inflight_map[key].complete_with_error(error)
        RAISE error
    FINALLY:
...
Workflows (server-side automations) have different scaling characteristics from interactive queries:

| Dimension | Interactive Queries | Workflows |
|-----------|-------------------|-----------|
| **Latency requirement** | <1s p99 | <30s per step (minutes total) |
| **Execution model** | Stateless, one-shot | Stateful, multi-step |
| **Failure handling** | Return error to client | Retry with backoff, error handlers |
| **Concurrency** | 2,300 QPS peak | 100 concurrent workflows |
| **Resource consumption** | CPU burst (sandbox) | I/O wait (connector calls) |

**Scaling strategy**:
- Separate workflow execution pool from query execution pool (no resource contention)
- Persistent step state: each step's result is persisted before the next step starts (crash recovery)
- Distributed task queue: workflow steps are enqueued and executed by any worker (horizontal scaling)
- Workflow timeout: max 5 minutes per workflow, max 100 steps (prevent runaway automations)
- Dead letter queue: workflows that fail all retries are sent to a DLQ for manual inspection

---

## AI Copilot Service Scaling

The AI Copilot Service has unique scaling challenges due to LLM inference latency:

| Concern | Strategy |
|---------|----------|
| **LLM latency (1-10s)** | Async generation with streaming response; builder sees progressive output |
| **Context window limits** | Schema summarization: send table names + key columns, not full DDL |
| **Rate limiting** | 20 requests/min per builder; queue excess requests |
| **Cost management** | Cache identical prompts (same schema + similar natural language); use smaller models for autocomplete, larger for full app generation |
| **Fallback** | If LLM provider is unavailable, AI features degrade gracefully; manual building unaffected |
...
| **Multi-region failover** | Simulate primary region outage | Verify secondary promotes, <5 min recovery |
| **AI Copilot degradation** | Block LLM provider endpoint | Verify graceful fallback, manual building unaffected |
| **Workflow storm** | 500 webhooks trigger simultaneous workflows | Verify queue backpressure, no query engine impact |

---

## Version Management at Scale

App version history grows linearly and becomes a storage concern at scale:

| Strategy | Details |
|----------|---------|
| **Version compaction** | After 90 days, retain only published versions + daily snapshots (discard auto-save checkpoints) |
| **Definition deduplication** | Store component tree diffs between versions rather than full copies (reduces storage by 60-80%) |
| **Lazy version loading** | Version list API returns metadata only (label, author, timestamp); full definition loaded on demand |
| **Archive policy** | Versions older than 1 year moved to cold storage; accessible but with higher latency (2-5s load) |
| **Hard limit** | Max 10,000 versions per app; oldest auto-save versions purged when limit reached |

---

## Connection Pool Lifecycle Management

Connection pools to customer databases must be managed carefully to balance resource usage against connection establishment cost:

```
Step-by-step plan in plain English: Connection Pool Lifecycle

FUNCTION manage_pool_lifecycle():
    // Runs every 60 seconds
    FOR pool IN pool_registry.all():
        // Evict idle pools (no queries in 5 minutes)
        IF pool.last_used_at < now() - 300s:
            pool.drain()        // Close all connections gracefully
            pool_registry.remove(pool.connector_id)
            metrics.decrement("connector.pool.active_pools")
            CONTINUE

        // Health-check validation: test idle connections
        FOR conn IN pool.idle_connections():
            IF NOT conn.is_valid():    // SELECT 1 or ping
                pool.invalidate(conn)  // Remove bad connection
                metrics.increment("connector.pool.invalid_connections")

        // Scale pool based on recent demand
        recent_qps = metrics.get("connector.query.qps", pool.connector_id, window=60s)
        IF recent_qps > pool.max_connections * 0.8:
            // Pool is near capacity; alert but do not auto-expand
            // (expanding beyond max could overwhelm customer DB)
            alert("connector_pool_near_capacity", {
                connector_id: pool.connector_id,
                utilization: recent_qps / pool.max_connections
            })
```

### Connection Pool Sizing Guidelines

| Customer Database Size | Pool Min | Pool Max | Idle Timeout | Rationale |
|----------------------|----------|----------|-------------|-----------|
| **Small** (< 50 max connections) | 1 | 5 | 60s | Avoid exhausting customer's connection limit |
| **Medium** (50-200 max connections) | 2 | 20 | 300s | Standard configuration |
| **Large** (200+ max connections) | 5 | 50 | 300s | Enterprise tier, dedicated connector |
| **Shared** (multi-app connector) | 2 | 20 | 300s | Pool shared; per-app rate limits apply |

---

## Scaling Decision Matrix

| Scenario | Symptom | Scaling Action | Recovery Time |
|----------|---------|---------------|---------------|
| **Query QPS spike** | CPU > 60% on query engine | Auto-scale query engine instances | 30s (warm pool) |
| **Large app load surge** | Cache hit rate < 80% | Scale cache cluster + increase TTL temporarily | 5 min |
| **Builder save contention** | p99 save latency > 2s | Add metadata store read replicas; check write conflicts | 10 min |
| **Connector overload** | Pool utilization > 80% | Add connector proxy instances; alert customer | 2 min |
| **Workflow backlog** | DLQ depth > 100 | Scale workflow workers; investigate failing steps | 5 min |
| **Regional latency** | End-user p99 > 500ms from a region | Deploy connector proxy in that region | Hours (infrastructure) |
