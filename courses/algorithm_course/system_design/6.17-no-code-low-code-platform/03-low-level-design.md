...
### Core Execution (Step-by-step plan in plain English)

```
Step-by-step plan in plain English: Query Execution Pipeline
...
```
Step-by-step plan in plain English: Dependency Graph for Reactive Bindings
...
Server-side workflows extend the platform beyond interactive apps into automation. Workflow definitions follow the same JSON metadata pattern as app definitions.

```
WORKFLOW DEFINITION SCHEMA (stored in APP_VERSION.definition for app_type="workflow"):

{
  "schema_version": 1,
  "trigger": {
    "type": "webhook" | "schedule" | "app_event" | "manual",
    "config": {
      "schedule": "0 9 * * MON-FRI",     // cron (for schedule type)
      "webhook_path": "/hooks/on-order",   // path (for webhook type)
      "app_id": "app-789",                 // source app (for app_event type)
      "event": "query.approveOrder.success" // event name
    }
  },
  "steps": [
    {
      ...
      "config": {
        ...
        "query_type": "sql",
        "sql": "SELECT * FROM orders WHERE id = {{trigger.data.order_id}}"
      },
      "timeout_ms": 10000,
      "on_error": "abort" | "continue" | "goto:step-error-handler"
    },
    {
      "id": "step-2",
      ...
      "type": "condition",
      "config": {
        "expression": "{{steps.step-1.data[0].total > 10000}}",
        "true_branch": "step-3a",
        "false_branch": "step-3b"
      }
    },
    {
      "id": "step-3a",
      ...
      "type": "query",
      "config": {
        "connector_id": "conn-slack-api",
        "query_type": "rest",
        "method": "POST",
        "url": "/api/chat.postMessage",
        "body": {
          "channel": "#high-value-orders",
          "text": "Order {{steps.step-1.data[0].id}} requires approval (${{steps.step-1.data[0].total}})"
        }
      },
  ...
  "error_handler": {
    "step_id": "step-error-handler",
    "type": "query",
    "config": {
      "connector_id": "conn-slack-api",
      "query_type": "rest",
      "method": "POST",
      "body": { "channel": "#alerts", "text": "Workflow failed: {{error.message}}" }
    }
  },
...
The platform introspects connected database schemas to power autocomplete in the SQL editor and AI-assisted query generation.

```
Step-by-step plan in plain English: Schema Introspection Pipeline

FUNCTION introspect_schema(connector_id):
    config = get_decrypted_config(connector_id)

    SWITCH config.type:
        CASE "postgresql":
            schema = query_pg_catalog(config)
        CASE "mysql":
            schema = query_information_schema(config)
        CASE "mongodb":
            schema = sample_collection_schemas(config)
        CASE "rest_api":
            schema = parse_openapi_spec(config) OR NULL

    // Cache schema for autocomplete (refresh on demand)
    cache.set(f"schema:{connector_id}", schema, ttl=3600)
    RETURN schema


FUNCTION query_pg_catalog(config):
    connection = acquire_connection(config)
    ...
    // Build structured schema map
    schema = {}
    FOR row IN tables:
        IF row.table_name NOT IN schema:
            schema[row.table_name] = { columns: [], primary_keys: [], foreign_keys: [] }
        schema[row.table_name].columns.append({
    ...
    pk_data = connection.execute("SELECT ... FROM pg_constraint WHERE contype = 'p'")
    fk_data = connection.execute("SELECT ... FROM pg_constraint WHERE contype = 'f'")

    RETURN schema


FUNCTION sample_collection_schemas(config):
    // For schemaless databases, sample documents to infer schema
    connection = acquire_connection(config)
    collections = connection.list_collections()

    schema = {}
    FOR coll IN collections:
        sample = connection.find(coll, limit=100)
        inferred = infer_schema_from_documents(sample)
        schema[coll] = inferred
