...
| Metric | Type | Labels | Alert Threshold | Purpose |
|--------|------|--------|----------------|---------|
| `ai.generation.duration` | Histogram | `generation_type` (scaffold, query, binding) | p99 > 15s | LLM response time |
| `ai.generation.success_rate` | Gauge | `generation_type` | < 90% | Generation quality |
| `ai.generation.validation_failures` | Counter | `failure_reason` | Rate > 20% | Generated output quality |
| `ai.token.usage` | Counter | `model`, `direction` (input/output) | Budget threshold | Cost tracking |
| `ai.provider.errors` | Counter | `provider`, `error_type` | Any 5xx | LLM provider health |

### Workflow Metrics

| Metric | Type | Labels | Alert Threshold | Purpose |
|--------|------|--------|----------------|---------|
| `workflow.execution.duration` | Histogram | `workflow_id`, `trigger_type` | p99 > 60s | End-to-end workflow time |
| `workflow.step.duration` | Histogram | `workflow_id`, `step_id` | p99 > 10s | Per-step latency |
| `workflow.failure.count` | Counter | `workflow_id`, `failure_type` | Rate > 5% | Workflow reliability |
| `workflow.dlq.depth` | Gauge | `org_id` | > 50 | Dead letter queue buildup |
| `workflow.concurrent` | Gauge | `org_id` | > 80% of limit | Workflow concurrency pressure |
...
Each data connector requires proactive health monitoring beyond reactive circuit breakers:

```
Step-by-step plan in plain English: Connector Health Check Pipeline

FUNCTION run_connector_health_checks():
    // Runs every 60 seconds for all active connectors
    FOR connector IN get_active_connectors():
        TRY:
            start = now()
            SWITCH connector.type:
                CASE "postgresql", "mysql":
                    result = execute_health_query(connector, "SELECT 1")
                CASE "mongodb":
                    result = execute_health_query(connector, "db.runCommand({ping:1})")
                CASE "rest_api":
                    result = execute_health_request(connector, "HEAD", connector.health_url)

            latency = now() - start
            update_health_status(connector.id, {
                status: "healthy",
                latency_ms: latency,
                checked_at: now()
            })

            metrics.histogram("connector.health.latency", latency, {
        ...
        CATCH error:
            update_health_status(connector.id, {
                status: "degraded" IF was_recently_healthy ELSE "down",
                error: error.message,
                checked_at: now()
            })

            IF consecutive_failures(connector.id) > 3:
                alert("connector_health_degraded", {
                    connector_id: connector.id,
                    org_id: connector.org_id,
                    failure_count: consecutive_failures(connector.id)
                })
```

---

## Observability for Multi-Tenant Debugging

A unique challenge in no-code platform observability: when an end-user reports "the app is slow," the on-call engineer must trace through multiple tenant boundaries to identify the root cause.

**Diagnostic flow**:

1. **Start with app_id**: Identify the affected app from the user's report
2. **Check app-level metrics**: `query.execution.duration{app_id="X"}` — is latency elevated?
3. **Identify slow queries**: `connector.query.duration{app_id="X"}` — which connector is slow?
4. **Check connector health**: `connector.health.status{connector_id="Y"}` — is the connector healthy?
5. **Trace a sample request**: Follow trace_id through API Gateway → Permission Engine → Query Engine → Connector Proxy → Customer DB
6. **Check tenant isolation**: Verify no cross-tenant impact; confirm the issue is isolated to one org's connector

**Key principle**: Every metric, log line, and trace span includes `org_id`, `app_id`, and `user_id`. This enables precise filtering without exposing one org's data to another org's admins. Platform operators see all orgs; org admins see only their own.
