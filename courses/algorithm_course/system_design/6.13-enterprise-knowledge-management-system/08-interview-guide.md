...
|-----------|----------------|
| "Design a KMS with AI-powered search that answers questions instead of returning page links" | Introduce RAG pipeline: chunking strategy (block-aligned), embedding generation (ingestion pipeline), vector store alongside inverted index, hybrid retrieval (BM25 + dense vector with reciprocal rank fusion), cross-encoder reranking, permission-filtered context window, LLM generation with citation grounding. Key trade-off: latency (2-5s for generated answers vs 200ms for keyword results) |
| "Design for a heavily regulated industry where nothing can ever be deleted" | Append-only version storage, immutable audit log on write-once storage, legal hold as a first-class entity (prevents modification/deletion of held pages and all versions), soft-delete only (status flag, never hard delete), tamper-evident export with hash chains for legal proceedings. Compliance officers need a separate UI to search retained content invisible to regular users |
| "Design a KMS that federates across multiple acquired companies with different identity systems" | Federated identity mapping (SCIM sync across IdPs), cross-workspace search with permission translation, content migration vs federation trade-off (migrate for unified permissions, federate for autonomy), unified user profile with multiple identity bindings, cross-workspace link resolution |
| "Design a KMS optimized for developer documentation with code examples" | Syntax-highlighted code blocks as first-class block types, executable code snippets (sandboxed execution), API documentation auto-generated from OpenAPI specs, code search with trigram indexing (not just full-text), version-aware documentation linked to code repository branches |

---

## Case Study Discussion Points

### Case Study 1: AI-Powered Knowledge Discovery at Scale

**Scenario**: An enterprise with 5M+ pages discovers that employees spend an average of 25 minutes per day searching for information, with only 40% of searches resulting in a satisfactory answer.

**Discussion points**:
- Why does keyword search fail? Query-document vocabulary mismatch: users search "how to set up staging" but the page is titled "Pre-Production Environment Provisioning Guide"
- RAG architecture: chunk pages at block boundaries, generate embeddings, store in vector index alongside inverted index. Hybrid retrieval combines keyword precision with semantic recall
- Permission challenge in RAG: the generated answer must never leak information from pages the user cannot access. Post-retrieval permission filtering removes inaccessible chunks before LLM context assembly
- Freshness: when a page is edited, its chunks must be re-embedded within 30 seconds (same SLO as keyword search index). Stale embeddings cause the RAG system to cite outdated procedures
- Trust: every generated answer includes clickable citations to source blocks. Users can verify claims and report inaccuracies, creating a feedback loop for retrieval quality improvement

### Case Study 2: Permission Inheritance Cascade During Organizational Restructuring

**Scenario**: A company reorganizes from 5 divisions to 3, merging two engineering teams. 200K pages across 500 spaces need permission updates. Half the spaces have page-level overrides at various depths.

**Discussion points**:
- Bulk permission change cannot be synchronous—it would lock the permission engine for minutes. Design as an async job with progress tracking
- Cache invalidation strategy: invalidating all permission caches at once causes a thundering herd of cold cache misses. Staged invalidation with priority queuing (invalidate high-traffic pages first, background-fill the rest)
- Audit trail: every permission change must be logged with before/after state for compliance. The bulk operation generates 200K+ audit events—batch them into the audit log store
- Rollback: if the reorganization is reversed, can you undo the permission changes? With immutable audit log, you can replay the inverse operations, but page-level overrides that were manually adjusted during the transition period create merge conflicts
...
|---------|-------------|--------|
| **RAG-Powered Enterprise Search** | Retrieval-augmented generation replaces "10 blue links" with synthesized answers grounded in organizational knowledge | Requires embedding pipeline, vector store, LLM inference as core infrastructure; changes search from a read-only index query to a compute-intensive generation task |
| **Knowledge Graph Extraction** | AI automatically identifies relationships between pages, people, projects, and concepts from link structure, @mentions, and content analysis | Enables "organizational intelligence": recommended experts, knowledge gap detection, impact analysis for page changes |
| **AI Content Governance** | Automated detection of stale content (pages not updated in 12+ months that are still frequently accessed), contradictory information across pages, and compliance violations | Shifts governance from manual review to continuous automated scanning with human-in-the-loop resolution |
| **Conversational Knowledge Interface** | Chat-based interface where users ask questions and receive answers synthesized from multiple pages, with follow-up conversation context | Requires session-aware retrieval (incorporate prior questions), streaming generation, and permission-aware conversation history |
...
|-----------|----------|----------|----------------|
| **Embedding freshness** | Re-embed on every page edit (consistent but expensive) | Batch re-embed hourly (cheaper but stale) | Re-embed on edit via async pipeline (same pattern as search indexing). Stale embeddings cause RAG to cite outdated content—freshness SLO must match keyword index |
| **RAG answer quality vs latency** | Full pipeline: retrieve → rerank → generate (2-5s, high quality) | Cached answers for popular queries (100ms, potentially stale) | Full pipeline for unique queries; cache generated answers for repeated questions with cache invalidation when source pages change |
| **AI summary trust model** | Show AI summaries prominently (faster knowledge discovery) | Require click-through to source (slower but verifiable) | Default to prominent display with clear AI provenance badge; regulated industries can configure mandatory source verification |
| **Vector index sharding** | Shard by space (natural isolation, simpler permissions) | Global index with permission post-filter (better cross-space recall) | Global index with post-filter—mirrors keyword search architecture. Space-sharded vectors miss cross-space relationships |

---

## "What Would You Do Differently with AI?"

| Without AI | With AI (2025+) | Architectural Change |
|------------|-----------------|---------------------|
| Keyword search with BM25 ranking | Hybrid BM25 + vector search with RAG-generated answers | Add embedding pipeline, vector store, LLM inference service; search latency budget increases from 200ms to 2-5s for generated answers |
| Manual page labeling and categorization | AI auto-categorization with confidence scores | Add classification pipeline to page save event processing; provenance metadata for AI-generated labels |
| Browse page tree to find information | Conversational interface with follow-up questions | Add session-aware retrieval, streaming generation, conversation history store |
| Manual content governance (admin reviews stale pages) | Automated staleness detection and contradiction scanning | Add content quality pipeline: page freshness scoring, cross-page consistency checks, alert generation |
| Static page templates | AI-generated template suggestions based on content type | Template recommendation engine using embedding similarity to existing high-quality pages |
| Author writes all content | AI drafts initial content, author reviews and refines | AI writing assistant with provenance tagging (`ai-assisted`); attestation workflow before publish |

---

*Next: [09 - Insights ->](./09-insights.md)*
