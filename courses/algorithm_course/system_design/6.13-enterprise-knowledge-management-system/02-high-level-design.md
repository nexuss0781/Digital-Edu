...
## RAG-Powered Search Architecture

### Why RAG Changes the Architecture

Traditional KMS search returns a ranked list of pages. RAG-powered search returns a synthesized answer with citations to specific pages and blocks. This is not a search UI change---it is a fundamental architectural addition that introduces a retrieval-generation pipeline alongside the existing inverted index search.
...
flowchart LR
    subgraph Ingestion["Content Ingestion Pipeline"]
        direction TB
        PE[Page Event<br/>Create/Edit]
        CH[Block Chunker<br/>Semantic Boundaries]
        EM[Embedding Model<br/>768-dim Vectors]
    ...
    subgraph Query["RAG Query Pipeline"]
        direction TB
        UQ[User Query]
        QE[Query Encoder<br/>Same Embedding Model]
        RR[Retriever<br/>Top-K Chunks]
        RK[Reranker<br/>Cross-Encoder]
        PF[Permission Filter<br/>Batch Check]
        GEN[Generator<br/>LLM with Context]
    ...
    class PE,CH,EM,VS ingest
    class UQ,QE,RR,RK,PF,GEN,ANS query
```
...
|----------|--------|-----------|
| **Chunking strategy** | Block-aligned (one chunk per content block) | Block-based content model provides natural semantic boundaries; avoids splitting mid-sentence or mid-table |
| **Embedding model** | Sentence-transformer class, 768 dimensions | Balances retrieval quality against index size; domain-fine-tuned on enterprise documentation corpus |
| **Retrieval** | Hybrid: BM25 keyword + dense vector, fused via reciprocal rank fusion | Keyword search handles exact terms (error codes, product names); vector search handles semantic queries ("how do we handle outages") |
| **Reranking** | Cross-encoder reranker on top-20 candidates | 20-35% accuracy improvement over first-stage retrieval; acceptable latency (<200ms) on small candidate set |
| **Permission enforcement** | Post-retrieval filter before generation | Same model as keyword search; ensures generated answers never cite pages the user cannot access |
| **Citation grounding** | Every generated sentence must cite a source chunk | Prevents hallucination; enables users to verify answers by clicking through to source pages |
