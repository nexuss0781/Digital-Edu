...
### AI/RAG Pipeline Throughput

```
Embedding generation (page create/edit):
  Pages requiring embedding per day: 2.5M (500K new + 2M edits)
  Average chunks per page: 8 (one per block, merged small blocks)
  Total chunks: 2.5M x 8 = 20M chunks/day
  Embedding latency per chunk: ~10ms (batch inference)
  Embedding dimension: 768 (sentence-transformer class)
  Embedding storage: 500M pages x 8 chunks x 768 dims x 4 bytes = ~12 TB

AI summarization requests per day:
  Auto-summary on create/edit: 500K/day (subset of high-traffic pages)
  On-demand summary requests: 2M/day (10M DAU x 20% request summaries)
  Average summary generation time: 2-5 seconds (LLM inference)
  Peak concurrent inference requests: ~500

RAG query throughput:
  AI-assisted search queries: 5M/day (subset of 30M total searches)
  Average retrieval: 10 chunks retrieved, 5 reranked, top 3 used for generation
  RAG response latency target: p50 < 2s, p99 < 5s
  Concurrent RAG queries at peak: ~200

Vector store size:
  Total chunks: 500M pages x 8 chunks = 4B chunks
  Embedding storage: 4B x 768 dims x 4 bytes = ~12 TB
  HNSW index overhead: ~1.5x = ~18 TB total vector index
```
