...
### Serialization Verification Requirements

| Milestone | Date | Requirement |
|-----------|------|-------------|
| **Large Dispenser** | Nov 27, 2025 | Pharmacies with 26+ staff must verify serialization |
| **Small Dispenser** | Nov 27, 2026 | All remaining pharmacies |
| **Full Interoperability** | 2027+ | End-to-end transaction data exchange |

### Verification Workflow

```
DSCSA PACKAGE VERIFICATION (During Receiving):

1. Scan GS1 DataMatrix barcode on package
   - Extract: NDC, Serial Number, Lot, Expiry

2. Submit verification request to NABP Pulse
   - Pulse routes to manufacturer's verification service
   - Response: VERIFIED, NOT_FOUND, or SUSPECT

3. Based on response:
   - VERIFIED: Accept into dispensable inventory
   - NOT_FOUND: Quarantine, investigate with supplier
   - SUSPECT: Quarantine, notify FDA within 24 hours

4. Store EPCIS event (GS1 standard):
   - Event type: ObjectEvent (observation)
   - Business step: receiving
   - Disposition: active (or in_quarantine)
   - Retention: 6 years minimum

5. Link serial number to inventory batch record
   - Enables full traceability from manufacturer to patient
```
...
|-------|---------|---------|
| **Event Time** | 2025-08-15T10:30:00Z | When verification occurred |
| **EPC List** | urn:epc:id:sgtin:... | Serialized product identifier |
| **Business Step** | urn:epcglobal:cbv:bizstep:receiving | Receiving event |
| **Disposition** | urn:epcglobal:cbv:disp:active | Package status |
| **Read Point** | Pharmacy location GLN | Where verification happened |
| **Verification Status** | VERIFIED | NABP Pulse response |
