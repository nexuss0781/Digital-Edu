...
### Trap 6: "How does the system handle DSCSA serialization verification?"

**What interviewer wants**: Test awareness of current compliance deadlines and supply chain traceability

**Best answer**:
> "DSCSA requires package-level verification of drug serialization—large dispensers by November 2025, all pharmacies by November 2026, with penalties up to $500K per violation. During receiving, each package's GS1 DataMatrix barcode is scanned and verified against the manufacturer's serialization data via NABP Pulse (the verification router). Verification events are stored as GS1 EPCIS (Electronic Product Code Information Services) transactions with 6-year retention. The key design choice is to batch-verify during receiving rather than at dispensing time—this keeps the dispensing workflow latency unchanged while ensuring every package is verified before reaching the shelf. Suspect and illegitimate products trigger quarantine workflows that prevent them from entering dispensable inventory."

### Trap 7: "How would you handle the NCPDP D.0 to F6 standard transition?"

**What interviewer wants**: Understanding of protocol migration in regulated environments

**Best answer**:
> "NCPDP F6 is required by January 2028 and introduces new transaction types for electronic prior authorization. I'd implement a protocol adapter layer that abstracts the claim submission format from the business logic. During the transition period, the adapter routes to D.0 or F6 based on the payer's supported version—many large payers will adopt F6 early, while smaller ones will lag. The adapter also handles response translation so the upstream Claims Service sees a unified interface regardless of protocol version. This is the same transitional shim pattern used in any protocol migration—you avoid a big-bang cutover by supporting both versions simultaneously."
...
| **Ignoring DSCSA** | Major compliance gap for 2025-2026 | Discuss serialization verification at receiving |
| **No protocol migration plan** | NCPDP F6 deadline 2028 | Mention adapter layer for D.0 → F6 transition |
...
- **DSCSA**: Drug Supply Chain Security Act (serialization, Nov 2025-2026 enforcement)
- **NCPDP F6**: Next-generation telecommunication standard (replaces D.0, deadline Jan 2028)
- **GS1 EPCIS**: Electronic Product Code Information Services (DSCSA interoperability)
- **NABP Pulse**: Verification router for drug serialization
...
### Variation 5: Hub-and-Spoke Dispensing

**Additional considerations**:
- Centralized robotic hub fills 60-70% of routine prescriptions
- Spoke pharmacies handle walk-ins, controlled substances, counseling
- Real-time inventory visibility across hub and all connected spokes
- Transportation compliance (temperature monitoring, chain of custody)
- DSCSA serialization verification at both hub and spoke receiving
- Different state regulations on hub-and-spoke models (not all states permit)
...
> "A Pharmacy Operating System requires **compliance-first architecture** because DEA and HIPAA violations can shut down operations. The core design uses **OPA for policy enforcement**, ensuring every transaction is validated against regulations. For inventory, I'd use **CRDTs** to enable conflict-free multi-terminal sync with offline capability. Drug data lives in a **graph database** for fast DDI and substitution queries. **AI features** include demand forecasting, substitution ranking, and prior authorization automation—self-hosted for HIPAA compliance. **DSCSA track-and-trace** serialization verification is enforced at receiving with 6-year EPCIS retention. The system needs **24-hour offline capability** with limits on controlled substances, **real-time PMP integration** for patient safety, and an **NCPDP protocol adapter** for the D.0→F6 transition by 2028."
