...
### Flow 4: Human Agent Handoff with Context Transfer
...
sequenceDiagram
    ...
    participant DM as Dialog Manager
    participant CTX as Context Store
    participant HO as Handoff Orchestrator
    participant Q as Agent Queue
    participant Agent as Human Agent
    participant ASR as ASR Engine (Live)

    DM ->> DM: Detect escalation trigger<br/>(3 failed clarifications OR<br/>frustration detected)
    DM ->> CTX: Fetch full session context
    CTX ->> DM: Cart state, transcript, intent history
    DM ->> HO: Request handoff with context
    HO ->> HO: Generate AI summary<br/>"User wants to return item from order #4521,<br/>wrong item delivered, frustrated"
    HO ->> Q: Queue with priority + language

    DM ->> User: "Main aapko hamari team se<br/>connect kar raha hoon..."

    Q ->> Agent: Assign call + context package
    Note over Agent: Agent sees: full transcript,<br/>cart state, AI summary,<br/>user language, sentiment score

    HO ->> Agent: Transfer call audio
    Agent ->> User: "Namaste, main Priya bol rahi hoon.<br/>Aapka order #4521 mein<br/>galat item aaya, sahi?"

    Note over ASR,Agent: Live ASR feeds real-time<br/>transcription to agent screen
    ASR -->> Agent: Real-time transcript of user speech

    User ->> Agent: Explains issue in detail
    Agent ->> User: Resolves complaint
```
...
| **Session state management** | Redis cluster with synchronous replication for active sessions; async persistence to durable store on session end | Voice sessions are stateful and long-lived (minutes, not milliseconds); losing state mid-call forces restart. Redis provides sub-millisecond reads for dialog state, cart contents, and language context | Redis memory cost for 25K concurrent sessions (~50 GB); sync replication adds 1–2 ms write latency; must handle Redis failover without dropping active calls |
| **Synonym dictionary architecture** | Weighted knowledge graph rather than flat lookup table; edges encode phonetic similarity, regional prevalence, temporal relevance, and user history | Flat synonym tables fail for context-dependent mappings ("atta" = wheat flour in Hindi, any flour in Maharashtra, buckwheat during Navratri). Graph traversal enables multi-hop resolution through phonetic + semantic + contextual signals | Higher query latency than flat lookup (10 ms vs. 2 ms); graph maintenance complexity; requires continuous enrichment from production voice interactions |
| **Consent management** | Per-channel, per-purpose consent model with verbal consent capture verified by ASR | TRAI regulations, privacy law, and WhatsApp policies each impose different consent requirements; a unified consent state machine tracks transactional, promotional, and recording consent independently | Verbal consent verification adds 5–10 seconds to call start; consent state must be queryable in real-time before every outbound call; multi-channel consent synchronization is complex |

---

## Architecture Decision Records

### ADR-1: Why Streaming ASR Over Batch Processing for Phone Calls

**Context:** Phone calls produce continuous audio. Two processing approaches exist: (1) streaming — process audio in 200 ms chunks as they arrive, emitting partial hypotheses, or (2) endpoint-then-batch — wait for the user to finish speaking (VAD endpoint), then process the complete utterance in one batch inference.

**Decision:** Streaming ASR for all phone call audio; batch only for WhatsApp voice notes.

**Rationale:** Streaming enables three critical optimizations impossible with batch processing:
1. **Speculative NLU execution** — begin NLU processing on partial transcripts; if the intent is clear early ("add to cart..."), start preparing the response before the user finishes speaking, saving 200–500 ms.
2. **Early intent detection** — for simple confirmations ("haan", "nahi"), the system can begin responding after the first 500 ms of audio rather than waiting for the full 700 ms endpointing silence.
3. **Progressive confidence monitoring** — per-chunk confidence scores enable mid-utterance model switching: if a Hindi model's confidence drops suddenly (user switched to Tamil), the system can route to the multilingual model before the utterance completes.

**Trade-off:** Streaming requires causal (left-to-right) ASR models that cannot use future audio context, resulting in 5–8% higher WER than bidirectional models used in batch mode. This is acceptable for real-time calls but WhatsApp voice notes use the higher-accuracy batch path.

### ADR-2: Why Hybrid Dialog Management Over Pure LLM

**Context:** LLMs can generate natural conversational responses and track dialog context. Using an LLM as the primary dialog manager would simplify the architecture to: ASR → LLM → TTS.

**Decision:** Rule-based state machine for transactional flows, LLM-assisted for open-ended interactions and response generation.

**Rationale:** Commerce transactions require deterministic behavior:
- An LLM might "confirm" an order without actually calling the order service
- LLM inference adds 500–2000 ms per turn — exceeding the 1.2s latency budget
- LLM cost per turn (₹0.05–0.50) at 15M daily NLU calls is prohibitive compared to rule-based (₹0.001)
- LLMs cannot reliably maintain structured slot state (product=X, qty=Y) across 20+ turns
- Hallucinated prices or discounts create financial liability

LLMs are used for: (1) generating natural language responses from structured dialog actions, (2) handling open-ended queries ("what rice is good for biryani?"), and (3) complaint resolution where empathetic, flexible responses matter more than transactional precision.

### ADR-3: Why Per-Language ASR Models Over Universal Multilingual

**Context:** A single multilingual model (e.g., Whisper large-v3) supports 99 languages and simplifies deployment to a single model variant.

**Decision:** Three-tier architecture — language-specific models for top 8, fine-tuned multilingual for mid-tier, base multilingual with adapters for low-resource — with multilingual as universal fallback.

**Rationale:** WER comparison on commerce-domain audio:

| Approach | Hindi WER | Tamil WER | Santali WER | Code-Mixed WER |
|---|---|---|---|---|
| Universal Whisper large | 15% | 22% | 38% | 28% |
| Language-specific fine-tuned | 10% | 12% | — | — |
| With domain adaptation | 8% | 11% | — | 20% |
| Multilingual + adapter | — | — | 25% | — |

The 5–7% WER gap for high-traffic languages translates directly to wrong product matches and wrong orders. At 3M daily sessions, a 5% WER improvement prevents ~150K ASR errors per day. The operational cost of managing 30+ model variants is justified by the accuracy gain on the languages that handle 90% of traffic.
