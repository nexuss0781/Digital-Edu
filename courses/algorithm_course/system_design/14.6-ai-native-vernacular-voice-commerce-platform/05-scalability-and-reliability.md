...
## Session Migration and Drain

### Graceful Server Drain

Unlike stateless web services where a server can be drained in seconds, voice servers host active calls that last minutes. Draining a media server or GPU node requires careful coordination:

| Phase | Duration | Action |
|---|---|---|
| **Announce drain** | T+0 | Mark server as "draining" in load balancer; no new calls routed |
| **Active call completion** | T+0 to T+10 min | Active calls continue on draining server; most calls complete within 5 minutes |
| **Long-call warning** | T+5 min | For calls exceeding 5 minutes on draining server: flag for priority completion |
| **Forced migration** | T+10 min | Remaining calls (< 1% typically) migrated to new server with session state transfer; user hears brief "ek second" pause |
| **Server shutdown** | T+12 min | Server drained; safe to terminate for maintenance or scaling |
...
| Scale | GPU Count | Monthly GPU Cost | Per-Session Cost (GPU only) | Optimization Applied |
|---|---|---|---|---|
| 500K DAU (Year 1) | 30 A100s | $54,000 | ₹0.90 | Base models, no distillation |
| 2M DAU (Year 2) | 120 A100s | $216,000 | ₹0.72 | INT8 quantization + micro-batching |
| 5M DAU (Year 3) | 250 A100s | $375,000 | ₹0.60 | Distilled models + on-device ASR for 20% of traffic |
| 10M DAU (Year 4+) | 400 A100s | $500,000 | ₹0.42 | Mature caching + on-device + model efficiency gains |
...
Voice sessions span minutes, not milliseconds. During a 5-minute call, the dialog state is read and written 10–20 times (once per conversational turn). Consistency requirements differ by data type:

| Data Type | Consistency Model | Rationale |
|---|---|---|
| **Active dialog state** | Strong consistency (synchronous replication) | Mid-call state loss forces restart; user must never hear a repeated confirmation or lost cart item |
| **Cart contents** | Strong consistency within session; eventually consistent cross-session | Active session cart must be atomic; if user resumes on WhatsApp 2 hours later, 5-second staleness is acceptable |
| **User profile** | Eventually consistent (30-second lag) | Language preferences and purchase history update slowly; no harm from brief staleness |
| **Product catalog** | Eventually consistent with TTL-based cache invalidation (15 minutes) | Price changes propagate through cache; voice sessions are short enough that 15-minute staleness rarely matters |
| **Synonym dictionary** | Eventually consistent (daily refresh to edge caches) | New synonyms are mined weekly; real-time consistency unnecessary |
| **Audio recordings** | Write-once, read-never in real-time path | Audio flows to object storage asynchronously; not read during the session that generated it |

### Cross-Region Data Synchronization

| Data | Sync Strategy | Conflict Resolution |
|---|---|---|
| **Session state** | Not replicated cross-region; session is pinned to region of call origination | If region fails, session is lost; user redials and starts new session in failover region |
| **User profile** | Async replication with last-writer-wins | Rare conflict: user unlikely to call from two regions simultaneously |
| **Product catalog** | Replicated from central catalog service to regional read replicas every 15 minutes | Central is authoritative; regional caches are read-only |
| **Synonym dictionary** | Full replication to all regions daily; delta sync every 6 hours | Centrally curated; no regional writes |
| **Order state** | Central order service with region-local caching of recent orders for status queries | Write to central; read from cache with fallback to central |

---

## Graceful Degradation Ladder

When the system is under extreme load or partial failure, it degrades through a sequence of steps that preserve the highest-value functionality while shedding lower-priority work:

| Level | Trigger | Action | What Still Works | What Degrades |
|---|---|---|---|---|
| **L0: Normal** | All metrics green | Full service | Everything | Nothing |
| **L1: Pressure** | GPU utilization > 80% for 10 min | Throttle batch ASR (WhatsApp); reduce TTS quality to smaller models | All real-time calls; WhatsApp with slight delay | WhatsApp latency increases to 8–10 s; TTS sounds slightly less natural |
| **L2: Stressed** | GPU utilization > 90%; concurrent calls > 20K | Pause all outbound campaigns; switch low-traffic languages to multilingual model | All inbound calls in all languages | Outbound campaigns paused; WER increases 5–10% for < 10% of languages |
| **L3: Critical** | Concurrent calls > 24K; GPU fallback to CPU | Activate CPU inference for overflow; degrade ASR to smaller models for all languages | All calls connected; ASR functional but slower | Latency increases to 1.5–2 s; WER increases 5% across board |
| **L4: Emergency** | Regional failure or GPU pool total failure | Route to backup region; DTMF-only IVR mode for affected calls; schedule callbacks | IVR basic ordering with keypad; callbacks scheduled | Full voice AI unavailable for affected region; users get basic IVR |
...
|---|---|---|---|
| **Single GPU node failure** | 0 (no data loss) | < 1 second | Kubernetes reschedules; inflight requests retry on next audio chunk |
| **Redis shard failure** | 0 (synchronous replica) | < 5 seconds | Redis Sentinel promotes replica; active sessions continue on new primary |
| **Regional data center partial** | 0 for sessions in unaffected nodes | 2–5 minutes | Auto-scale remaining capacity; overflow to nearest region |
| **Regional data center total** | Session state lost for active calls | 5–15 minutes | DNS failover; all new calls route to backup; active calls dropped with callback scheduling |
| **SIP carrier outage** | 0 (no data involved) | < 30 seconds | Automatic failover to backup carrier; reroute via alternate trunk |
| **Object storage failure** | Audio segments in flight lost | 1–4 hours | Cross-region replication restores from replica; recent segments may have gaps |
| **Central order service failure** | Orders queued locally | 15–30 minutes | Orders accepted to local queue; confirmed to user; processed when central recovers |
