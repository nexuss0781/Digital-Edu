...
- **Voice biometric authentication** — No voiceprint-based identity verification; authentication uses phone number, OTP, and order knowledge. Voiceprints create biometric data liability without sufficient accuracy for commerce-grade authentication in noisy environments
- **Autonomous price negotiation** — No automated bargaining or dynamic pricing via voice; prices are fixed from the catalog. Negotiation introduces liability (system agreeing to unsustainable prices) and trust issues (different users getting different prices)
- **Multi-party conference commerce** — No support for multiple speakers placing a joint order on the same call. Speaker diarization in noisy environments is unreliable, and conflicting instructions from multiple speakers create cart management chaos
...
The 1.2-second end-to-end latency target must be allocated across pipeline stages, with each stage having a strict budget:

| Stage | Budget (ms) | Notes |
|---|---|---|
| Audio capture + encoding | 50 | Client-side; includes codec encoding |
| Network transit (inbound) | 50–150 | Varies by network quality; 50 ms urban, 150 ms rural |
| VAD endpointing | 350–500 | Context-adaptive; largest variable component |
| ASR inference | 100–200 | Per-chunk streaming; INT8 quantized models |
| NLU processing | 50–100 | Intent + entity extraction; lightweight transformer |
| Commerce backend | 50–150 | Product lookup, cart update, inventory check |
| Response generation | 30–80 | Template filling or LLM generation |
| TTS synthesis | 100–200 | Streaming; first-byte latency, not full synthesis |
| Network transit (outbound) | 50–150 | Return audio to caller |
| **Total budget** | **830–1,580** | Must stay under 1,200 ms at p95 |

The budget is met through speculative execution (NLU starts on partial ASR), streaming TTS (first audio byte before full response), and response caching (common phrases pre-synthesized). Without these optimizations, the sequential pipeline would take 1,500–2,100 ms.
...
| Requirement | Specification |
|---|---|
| **Multi-tenancy** | Support multiple commerce backends (different MSME sellers) on shared voice infrastructure; per-tenant product catalogs, branding (TTS voice, greeting), and billing |
| **Model lifecycle management** | Canary deployment of ASR/TTS models per language; instant rollback within 60 seconds; A/B testing with traffic splitting; shadow mode for pre-production evaluation |
| **Disaster recovery** | RPO 0 for session state (synchronous replication); RTO < 15 minutes for regional failover; automated callback scheduling for dropped calls during failures |
| **Capacity planning** | Auto-scaling GPU pools based on real-time concurrent call count; pre-provisioning for known spikes (festivals, promotions) with 2–24 hour advance notice |
| **Cost monitoring** | Per-session cost tracking (GPU, telephony, storage); per-language cost allocation; automated alerts when per-transaction cost exceeds ₹2 threshold |
| **Human annotation pipeline** | Support for 10+ concurrent annotators per language; annotation quality measurement (inter-annotator agreement > 85%); weekly annotation targets for each language |

---

## Benchmark Comparison

| Dimension | Traditional IVR Commerce | Voice Assistant Commerce (Alexa/Google) | AI-Native Vernacular Voice Commerce (This System) |
|---|---|---|---|
| **Language support** | 1–3 languages with rigid menu trees | 10–20 languages, general-purpose ASR | 22+ Indian languages with dialect awareness, commerce-domain adaptation, and code-mixing support |
| **Interaction model** | DTMF keypad navigation; "Press 1 for groceries" | Wake word → command → response; single-turn or limited multi-turn | Free-form conversational dialog; multi-turn negotiation; proactive suggestions; context across sessions |
| **Product resolution** | Fixed catalog codes; user must know product IDs | Text search on ASR transcript; works for popular brand names | Multi-stage vernacular name resolution with phonetic matching, regional synonym dictionary, and purchase history |
| **Target user** | Any phone user willing to navigate menu trees | Smartphone owner with stable internet and smart speaker | Non-literate and semi-literate users on basic phones; 300M+ Indians without alternative digital commerce access |
| **Audio quality handling** | Narrowband only; no noise handling | Wideband; near-field microphone; quiet environments | Narrowband PSTN + wideband smartphone; outdoor market noise; channel-specific ASR models |
| **Payment** | DTMF-based card entry or agent transfer | Linked payment on device; voice confirmation | UPI collect via separate device; DTMF OTP; COD fallback; no card numbers via voice |
| **Latency architecture** | Pre-recorded prompts; minimal processing delay | Cloud ASR with 500–1500 ms round-trip | Streaming pipeline with speculative NLU, adaptive endpointing, and TTS caching; sub-1.2 s target |
| **Commerce conversion** | 5–8% (menu fatigue, abandonment) | 10–15% (limited product browsing capability) | 20–30% (conversational, personalized, repeat-customer optimization) |
...
|---|---|
| **Sub-₹2 voice processing cost per transaction** | Requires model distillation, INT8 quantization, audio-aware batching, and aggressive TTS caching to fit GPU inference costs within MSME unit economics (average order ₹200–500) |
| **22+ languages with 1000x data disparity** | Cannot use uniform ASR architecture; requires three-tier model strategy (language-specific, fine-tuned multilingual, adapter-based) with cross-lingual transfer for low-resource languages |
| **Code-mixing as default speech pattern** | Monolingual ASR models fail; requires merged-vocabulary multilingual acoustic models and code-mix-aware language models trained on limited annotated corpora |
| **8 kHz narrowband PSTN as primary channel** | High-frequency acoustic features destroyed; requires narrowband-specific ASR training, confusable-pair disambiguation, and commerce domain LM to compensate |
| **Zero visual fallback for non-literate users** | Every interaction must be audio-complete; chunked confirmations, progressive disclosure, working memory limits (5–7 items) shape dialog design |
| **TRAI telephony regulations** | Outbound calls restricted to 9 AM–9 PM; DND registry compliance; caller ID display; consent management per call type |
| **Real-time conversation constraint** | Voice-in to voice-out under 1.2 seconds; no queuing for phone calls; GPU pools are capacity-reserved, not throughput-optimized |
...
| Cost Category | Monthly Cost | Derivation |
|---|---|---|
| GPU infrastructure (120 A100s) | $216,000 | 120 GPUs × $1.50/hr on-demand (blended spot + reserved) × 720 hrs/month |
| SIP trunk and telephony | $45,000 | 60,000 channel capacity × usage-based per-minute billing × 4.2M call minutes/day |
| Object storage (audio) | $8,250 | 165 TB × $0.05/GB tiered storage |
| Compute (non-GPU microservices) | $18,000 | Dialog manager, NLU, API gateways, media servers across 4 regions |
| Database and cache | $12,000 | Redis cluster (session state), PostgreSQL (profiles, transcripts), synonym search index |
| Bandwidth | $15,000 | ~13.3 Gbps peak; data transfer across regions and to carriers |
| WhatsApp Business API | $9,000 | Per-conversation pricing × 1.8M daily WhatsApp sessions |
| Human annotation (ASR QA) | $6,000 | 10 annotators × quality review sampling for WER measurement |
| **Total monthly** | **~$329,250** | |
| **Per-transaction cost** | **~₹1.83** | $329,250 / (3M sessions/day × 30 days); well within ₹2 target |

### Unit Economics Breakdown per Voice Session

| Component | Cost per Session | % of Total | Optimization Lever |
|---|---|---|---|
| ASR inference (real-time) | ₹0.72 | 39% | INT8 quantization, VAD-gated silence skipping, micro-batching |
| TTS synthesis | ₹0.24 | 13% | Response audio caching (40%+ hit rate), model distillation |
| NLU inference | ₹0.09 | 5% | Lightweight transformer with language-specific heads |
| Telephony (SIP + carrier) | ₹0.45 | 25% | Carrier rate negotiation, trunk utilization optimization |
| Storage (audio + transcripts) | ₹0.06 | 3% | Opus compression (8:1), aggressive TTL enforcement |
| Compute (microservices) | ₹0.12 | 7% | Horizontal scaling with right-sizing per region |
| WhatsApp API (if applicable) | ₹0.08 | 4% | Per-conversation pricing tier optimization |
| Overhead (monitoring, infra) | ₹0.07 | 4% | Shared infrastructure costs amortized |
| **Total** | **₹1.83** | **100%** | Target: keep below ₹2.00 at all times |

### SLO Summary

| SLO | Target | Measurement |
|---|---|---|
| End-to-end voice latency (p95) | ≤ 1.2 s | Voice-in to first audio byte out |
| ASR WER (top 8 languages) | ≤ 15% | Human-reviewed sample, per language |
| Product resolution rate | ≥ 88% | Spoken reference → catalog match |
| Order conversion (inbound) | ≥ 25% | Commerce sessions → orders |
| Platform availability | 99.95% | Excluding scheduled maintenance |
| Call drop rate | ≤ 1% | Technical disconnects, not user hangups |
| Human escalation rate | ≤ 8% | Sessions transferred to agents |
