...
| **Code-mixing handling** | Identifies code-mixing as the default pattern (not Edge Case (Unusual or extreme situation)); proposes multilingual acoustic model and cross-lingual NLU | 5 |
...
## Follow-Up Probes for Strong Candidates

### Probe 1: "How would you handle a regional festival spike where a language that normally gets 2% of traffic suddenly gets 30%?"

**What to listen for:**
- Model loading strategy: are Pongal/Onam language models pre-loaded before the festival, or loaded on demand?
- GPU pool rebalancing: can GPUs be dynamically reassigned from low-traffic language pools to the spiking language?
- Catalog readiness: does the product catalog have festival-specific items (pongal rice, onam sadhya ingredients) with vernacular synonyms already loaded?
- Pre-synthesized TTS: common festival greetings and product descriptions pre-cached in TTS audio for the surge language

### Probe 2: "What happens when a user's phone cuts out mid-order and they call back 5 minutes later?"

**What to listen for:**
- Session resumption: is the cart state persisted? Can the system recognize the caller and restore context?
- Partial order handling: if the user had confirmed some items but not placed the order, what state is restored?
- Audio gap: the system has no audio for the 5-minute gap; does it prompt "Lagta hai call cut gaya tha. Aapke cart mein 3 items the, ₹248. Wahi se continue karein?" or force restart?
- Cross-channel resumption: if the user calls from a different number or switches to WhatsApp, can the session still be restored?

### Probe 3: "How do you measure whether your voice commerce system is actually better than a human agent?"

**What to listen for:**
- Multi-dimensional comparison: not just cost per interaction but also conversion rate, order accuracy, customer satisfaction, availability (24/7 vs. business hours), and language coverage
- A/B testing framework: randomly route some calls to human agents and compare outcomes
- Hybrid baseline: measure the AI+human-handoff system against pure-human and pure-AI to find the optimal automation boundary
- Recognition that voice AI excels at high-volume repetitive orders (weekly groceries) but human agents win on complex complaints, emotional situations, and first-time user onboarding

### Probe 4: "How would you expand this system from India to Sub-Saharan Africa?"

**What to listen for:**
- Language challenge shift: African languages have even less ASR training data than Indian low-resource languages; cross-lingual transfer from related Bantu languages, tone-language ASR challenges
- Infrastructure differences: higher reliance on USSD and feature phones; 2G prevalent; data costs prohibitively expensive for voice notes
- Payment integration: M-Pesa and mobile money instead of UPI; agent-based cash-in/cash-out
- Regulatory differences: different DND regimes, data localization requirements, and consent frameworks per country
- Commerce pattern differences: different product catalogs, different quantity expressions, different negotiation norms
...
|---|---|
| **"Just use Whisper/GPT-4o for everything"** | Ignores latency constraints, GPU costs at scale, narrowband audio challenges, and commerce domain specificity |
| **"Store all audio for model training"** | Privacy liability; ignores consent, biometric data classification, and storage costs |
| **"Language detection is a solved problem"** | Code-mixing makes it fundamentally harder; language can change mid-sentence; detection must be continuous, not one-shot |
| **"Treat ASR output like typed text"** | Ignores confidence scores, n-best alternatives, disfluency handling, and phonetic error patterns that are fundamentally different from typos |
| **"Add a visual fallback for edge cases"** | The primary user base is non-literate; visual fallback is not available; this is the core design constraint, not an Edge Case (Unusual or extreme situation) |
| **"Scale by adding more servers"** | Voice sessions are stateful and GPU-bound; cannot horizontally scale like stateless web requests; language-aware routing and model warm-up add complexity |
