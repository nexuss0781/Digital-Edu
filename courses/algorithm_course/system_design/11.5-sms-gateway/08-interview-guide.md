...
- Identifying that the carrier SMSC is the Slowest part of the process you cannot control
...
**Minimum viable whiteboard diagram (draw in < 3 minutes):**

```
Customer → [REST API] → [Compliance] → [Routing Engine] → [Carrier Queue]
                                                              ↓
             [Webhook] ← [DLR Processor] ← [SMPP Pool] ← [Queue Consumer]
                                               ↓
                                          [Carrier SMSC]
                                               ↓
                                          [Recipient Phone]
...
- Mark the API → Queue boundary as "202 accepted here — async from this point"
- Label the queue as "carrier-partitioned with priority lanes"
- Circle the Carrier SMSC and write "external, TPS-limited — you cannot scale this"
- Draw a feedback arrow from DLR Processor → Routing Engine labeled "carrier health scores"
...
| Dimension | Weight | What Demonstrates Excellence |
|---|---|---|
| **Architecture** | 25% | Async pipeline with carrier-partitioned queues; priority lanes for OTP vs marketing; clear separation of API/routing/SMPP/DLR tiers |
| **Carrier Integration** | 20% | SMPP protocol awareness; TPS as fundamental constraint; window-based backpressure; connection pooling with per-carrier topology |
| **Reliability** | 20% | Carrier failover algorithm; DLR state machine with timeout handling; graceful degradation modes; in-flight message recovery |
| **Compliance** | 15% | TCPA/10DLC on critical path; per-message evaluation; opt-out enforcement; $1,500/violation risk awareness |
| **Trade-offs** | 10% | Cost vs delivery rate; sync vs async; carrier-partitioned vs destination-partitioned queues |
| **Production Awareness** | 10% | GSM-7/UCS-2 encoding; concatenation segment billing; traffic pumping detection; carrier trust scoring |

### Score Levels

| Level | Score | Characteristics |
|---|---|---|
| **Expert** | 9-10 | Explains SMPP window backpressure, quantifies carrier TPS constraints, discusses UDH concatenation reference collisions, identifies traffic pumping as economic attack vector |
| **Strong** | 7-8 | Async pipeline with priority queues, multi-carrier routing with failover, DLR correlation challenge, TCPA compliance on critical path |
| **Competent** | 5-6 | API → queue → carrier flow, basic health checks, mentions encoding differences, awareness of opt-out requirements |
| **Developing** | 3-4 | Synchronous carrier submission, single queue, no compliance discussion, treats SMS as "just send text" |
| **Insufficient** | 1-2 | No protocol awareness, no queue architecture, ignores carrier constraints, no compliance |

### Red Flags (Immediate Score Reduction)

| Red Flag | Why It Matters |
|---|---|
| Synchronous API-to-carrier design | Couples API availability to carrier latency — will fail in production |
| No mention of carrier TPS limits | Missing the fundamental constraint of the entire system |
| "Just retry failed messages" without validity period | OTP codes expire; infinite retry wastes money and annoys recipients |
| Single message queue for all carriers | Cannot enforce per-carrier rate limits — will get throttled/blocked |
| Compliance as an afterthought | TCPA violations are $500-$1,500 per message — must be on critical path |

### Follow-Up Depth Probes

| Probe Question | What It Tests |
|---|---|
| "A carrier starts returning ESME_RTHROTTLED on 30% of submissions — what happens?" | Window backpressure understanding, carrier failover, queue depth management |
| "A customer sends a marketing campaign with emojis to 5M recipients — walk through the system" | UCS-2 encoding → 3x segments → 15M carrier submissions; cost multiplication awareness |
| "How do you detect if a new customer is doing traffic pumping?" | Fraud detection: velocity checks, destination risk scoring, progressive trust model |
| "Two carriers both claim to serve UK mobiles — how do you choose?" | Multi-factor routing: cost, delivery rate, latency, current utilization, carrier health score |
