...
## SMPP Connection Pool Scaling

### Connection-to-Carrier Topology

Unlike stateless HTTP services, SMPP connections are persistent TCP sessions with carrier-specific state (sequence numbers, window tracking, bind credentials). Scaling the SMPP tier requires careful connection management.

```
SMPP Connection Pool Architecture:

Carrier A (100 TPS limit):
├── Instance 1: 3 connections (window=20 each → 60 PDU capacity)
├── Instance 2: 3 connections (window=20 each → 60 PDU capacity)
└── Total: 6 connections, 120 PDU effective capacity → matches 100 TPS with headroom

Carrier B (5000 TPS limit):
├── Instance 1: 10 connections (window=50 each → 500 PDU capacity)
├── Instance 2: 10 connections (window=50 each → 500 PDU capacity)
├── Instance 3: 10 connections (window=50 each → 500 PDU capacity)
├── ...
└── Total: 50+ connections across 5+ instances → matches 5000 TPS

Carrier C (20 TPS limit):
├── Instance 1: 1 connection (window=20 → 20 PDU capacity)
└── Total: 1 connection → sufficient for 20 TPS
```

### Connection Draining During Scaling

When scaling down SMPP connector instances or during rolling deploys, in-flight PDUs must complete before connections close:

```
FUNCTION drain_smpp_instance(instance_id):
    // 1. Stop accepting new messages from queue
    pause_queue_consumer(instance_id)

    // 2. Wait for in-flight SMPP window to drain
    FOR EACH connection IN instance.connections:
        WHILE connection.in_flight_count > 0:
            WAIT(100ms)
            IF elapsed > 30s:
                LOG_WARNING("Force-closing with {} in-flight PDUs", connection.in_flight_count)
                BREAK

    // 3. Send unbind to carrier
    FOR EACH connection IN instance.connections:
        send_unbind(connection)
        WAIT_FOR(unbind_resp, timeout=5s)

    // 4. Close TCP connections
    close_all_connections(instance)

    // 5. Reassign carrier pool to remaining instances
    redistribute_carrier_pool(instance.carrier_assignments)
```

### Carrier-Aware Auto-Scaling

| Signal | Scale-Out Trigger | Scale-In Trigger | Action |
|---|---|---|---|
| Window utilization | > 80% across all connections to a carrier | < 20% for 15+ min | Add/remove connections (within carrier TPS limit) |
| Queue depth per carrier | > 5,000 messages for > 2 min | Consistently empty | Add connector instance for that carrier |
| DLR processing lag | > 60s behind | < 5s consistently | Scale DLR processing consumers |
| SMPP response latency | p99 > 2s from specific carrier | p99 < 200ms for 30+ min | Alert only (carrier-side issue — cannot add capacity) |
...
### Load Testing for SMS Gateways

```
SMS-Specific Load Test Scenarios:

1. Sustained throughput: 35K msg/sec for 4 hours
   - Validate queue depth stays bounded
   - Verify per-carrier TPS enforcement holds
   - Monitor memory growth (correlation cache must not leak)

2. Flash sale simulation: Ramp from 5K to 50K msg/sec in 60 seconds
   - Auto-scaling response time for API and routing tiers
   - Queue absorption during scale-out lag
   - Priority preservation (OTP messages unaffected by marketing burst)

3. Carrier failure during load: Kill primary carrier at peak
   - Failover routing activates within 60 seconds
   - In-flight messages on failed carrier are re-routed
   - Queue depth redistribution across alternate carriers

4. DLR flood: Simulate 200K DLRs/sec (4x normal)
   - DLR processing tier scales independently
   - Correlation cache hit rate remains > 95%
   - Webhook dispatch keeps pace (no growing backlog)

5. Slow customer webhook: Simulate 500ms response from top-10 webhooks
   - Per-endpoint circuit breaker triggers at threshold
   - Other customers' webhooks unaffected (bulkhead isolation)
   - Webhook retry queue grows gracefully without memory pressure
```
