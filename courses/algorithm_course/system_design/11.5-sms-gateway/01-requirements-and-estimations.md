...
| Feature | Depends On | Conflicts With | Notes |
|---|---|---|---|
| FR-3 Delivery Tracking | FR-2 Carrier Routing (carrier message_id for DLR correlation) | — | DLR accuracy depends on carrier fidelity |
| FR-6 Concatenation | FR-7 Encoding (segment size depends on GSM-7 vs UCS-2) | — | Concatenation reference ID has 8-bit collision risk at high throughput |
| FR-8 Opt-Out | FR-4 Inbound (MO keyword detection triggers opt-out) | FR-12 Campaigns (opt-out must override campaign targeting) | Opt-out cache must be on critical path before routing |
| FR-10 Two-Way | FR-4 Inbound + FR-13 Number Pooling (sticky sender for session continuity) | — | Session tracking requires number-to-conversation mapping |
| FR-12 Campaigns | FR-13 Number Pooling + FR-8 Opt-Out | — | Campaign throttling must respect per-carrier TPS, not just aggregate rate |
| FR-15 RCS Fallback | FR-2 Routing (routing engine decides RCS vs SMS) | FR-6 Concatenation (RCS has no segment limits) | RCS capability check adds latency to routing decision |
| FR-17 MMS | FR-2 Routing (not all carriers support MMS via same connection) | — | MMS uses MM7/SOAP or HTTP API, not SMPP — separate connection tier |
...
- Real-time bidding for SMS routes (fixed carrier agreements, not auction-based)
- Physical SIM-based messaging (grey routes via SIM farms)
...
| **Routing decision time** | < 5ms | Must not become throughput Slowest part of the process |
...
| Requirement | Target | Justification |
|---|---|---|
| **Peak message throughput** | 50,000+ msg/sec | Must handle promotional spikes (3x average) without message loss |
| **Carrier connections** | 3,000+ concurrent SMPP sessions | Cover 400+ carriers with redundant connection pools |
| **Number inventory** | 50M+ numbers under management | Support global customer base across all number types |
| **Horizontal scaling** | Stateless tiers scale independently | API, routing, compliance, webhook tiers must auto-scale without coordination |
