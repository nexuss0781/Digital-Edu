...
### Topic: Leaderboard as a Platform Service

```
When designing a leaderboard system that serves MULTIPLE games (platform approach),
the architecture shifts from single-tenant optimization to multi-tenant isolation:

Key Questions:
  1. "How do you prevent one game's traffic spike from affecting another game's SLOs?"
     → Noisy neighbor isolation: per-game resource quotas, separate sorted set
        instances for high-traffic games, shared instances for small games

  2. "How do you handle different scoring models across games?"
     → Leaderboard configuration as code: scoring_strategy, sort_order,
        tiebreak_policy, reset_schedule are all per-leaderboard configs.
        The ranking engine is generic; game-specific logic lives in config.

  3. "How do you charge game developers for leaderboard usage?"
     → Metering dimensions: score updates/sec, rank queries/sec, entries stored,
        WebSocket connections, snapshot storage. Usage-based pricing aligns
        incentives — games pay for what they use.

  4. "How do you manage schema evolution when you support 1000+ game-specific
     metadata fields across different games?"
     → Metadata stored as schemaless MAP<STRING,STRING> in score events.
        Game-specific indexing handled at the analytics layer, not ranking engine.
        Ranking engine only cares about player_id and score — everything else
        is pass-through.
```

### Topic: Leaderboard Integrity During Live Esports

```
During a $1M prize tournament broadcasted to 10M viewers:

  Constraints unique to live esports:
    - Zero tolerance for rank errors (prize money at stake)
    - Sub-second score propagation (broadcast delay is the Slowest part of the process)
    - Auditability: every rank change must be traceable to a score event
    - Freeze capability: operator must be able to freeze rankings instantly
    - Rollback capability: if a score is invalidated mid-tournament,
      all subsequent ranks must be retroactively recalculated

  Architectural response:
    - Dedicated single-instance sorted set (no sharding, exact ranks always)
    - Synchronous replication (strong consistency over availability)
    - Write-ahead log with deterministic replay for audit
    - Admin "freeze" API that atomically stops accepting new scores
    - "Rollback to event X" API that replays events, skipping the invalidated one
```
