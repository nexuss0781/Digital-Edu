...
## Real-Time Notification Fan-Out Architecture

The notification subsystem must push rank changes to millions of connected clients without overwhelming the ranking engine or notification infrastructure.
...
flowchart TB
    ...
    subgraph ChangeDetect["Change Detection"]
        CD[Rank Change<br/>Detector]
        CF[Change Filter<br/>Threshold-based]
    end

    subgraph FanOut["Fan-Out Layer"]
        FO1[Fan-Out Worker 1<br/>Subscriptions A-M]
        FO2[Fan-Out Worker 2<br/>Subscriptions N-Z]
        FO3[Fan-Out Worker N]
    end

    subgraph Delivery["Delivery Layer"]
        WS1[WebSocket Server 1<br/>500K connections]
        WS2[WebSocket Server 2<br/>500K connections]
        WS3[WebSocket Server N<br/>500K connections]
    end

    RE -->|rank change stream| CD
    CD --> CF
    CF -->|filtered changes| FO1
    CF -->|filtered changes| FO2
    CF -->|filtered changes| FO3
    ...
    classDef delivery fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class RE engine
    class CD,CF detect
    class FO1,FO2,FO3 fanout
    class WS1,WS2,WS3 delivery
```

### Notification Filtering Strategy

Not every score update warrants a push notification. The change filter applies threshold-based rules to reduce fan-out volume:

```
Filtering Rules:
  Rule 1: Top-100 changes → always notify all subscribers of that leaderboard
  Rule 2: Rank change > 10 positions → notify the affected player only
  Rule 3: Rank change 1-10 positions → batch and notify every 5 seconds
  Rule 4: Friend's rank change → notify if friend is in top-50 of friend board
  Rule 5: No rank change (score update didn't affect ordering) → no notification

Result: 100K score updates/sec → ~15K notification events/sec (85% filtered)
```
