...
Sharding Decision Matrix:
  ...
  Trigger 4: Hot leaderboard Slowest part of the process → Read + write on same board
...
### Edge-Optimized Read Path

For global games requiring sub-50ms rank queries worldwide, deploy lightweight read-only leaderboard caches at edge PoPs:

```
Edge Node Architecture:
  Each edge PoP maintains:
    - Top-1000 cache per active leaderboard (push-updated every 2-5 seconds)
    - Around-me query capability for locally-cached entries
    - Query routing logic to forward cache misses to origin

  Cache Update Flow:
    1. Origin ranking engine emits change events to invalidation channel
    2. Change events fan out to all edge PoPs (pub/sub over persistent connections)
    3. Edge node applies incremental updates to local sorted set replica
    4. Cache miss for deep ranks → async fetch from origin, cache locally (60s TTL)

  Edge Capacity:
    - Top-1000 per leaderboard × 100 active leaderboards = 100K entries
    - Memory per edge node: 100K × 120 bytes = ~12 MB (trivial)
    - Edge node handles: ~50K queries/sec per PoP
    - 30 edge PoPs × 50K = 1.5M global queries/sec at edge

  Latency Profile:
    - Edge cache hit (top-1000): < 5ms
    - Edge cache miss (origin fetch): 50-200ms (depends on geographic distance)
    - P99 weighted average: ~15ms (85% edge hit rate)
```

### Graceful Degradation Under Partition

```
Edge Partition Scenarios and Responses:

  Scenario 1: Edge PoP loses connectivity to origin
    Response: Serve stale cached data with "last_updated" timestamp in response
    Client UX: Display "Updated X seconds ago" badge
    Max stale duration: 5 minutes before edge returns errors

  Scenario 2: Origin region down, edge PoPs operational
    Response: All queries served from edge cache (read-only mode)
    Score submissions queued at edge with bounded buffer (10K events)
    Auto-drain when origin recovers

  Scenario 3: Multiple origin regions down (catastrophic)
    Response: Promote warm standby region to primary
    Edge PoPs reconnect to new origin within 30 seconds
    Score replay from event log backfills ranking gaps
```
