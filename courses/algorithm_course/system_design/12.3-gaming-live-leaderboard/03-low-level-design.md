...
Time-weighted scoring ensures leaderboards reflect recent activity rather than all-time accumulation. Players who stop playing gradually lose rank, keeping leaderboards fresh and engagement-driven.

```
FUNCTION compute_decay_score(player_id, leaderboard_id, config):
    raw_score = ZSCORE("lb:" + leaderboard_id, player_id)
    IF raw_score == NULL:
        RETURN NULL

    last_active = GET_LAST_ACTIVE(player_id, leaderboard_id)
    elapsed_hours = (NOW() - last_active) / 3600

    // Exponential decay: score halves every decay_half_life hours
    decay_factor = 0.5 ^ (elapsed_hours / config.decay_half_life_hours)
    ...
    RETURN {
        ...
        hours_inactive: elapsed_hours
    }

FUNCTION apply_decay_batch(leaderboard_id, config):
    // Periodic job (every 15-60 minutes) updates decayed scores
    // Only update entries where decay has changed score by > 1 point
    entries = ZRANGE("lb:" + leaderboard_id, 0, -1, WITHSCORES)

    pipeline = BEGIN_PIPELINE()
    updated_count = 0

    FOR entry IN entries:
        decayed = compute_decay_score(entry.member, leaderboard_id, config)
        IF ABS(decayed.decayed_score - entry.score) > 1.0:
            pipeline.ZADD("lb:" + leaderboard_id, decayed.decayed_score, entry.member)
            updated_count += 1

    pipeline.EXECUTE()
    RETURN {updated: updated_count, total: len(entries)}
```

> **Trade-off**: Batch decay updates create brief periods where displayed scores are stale by the decay amount accumulated since the last batch. A 15-minute batch interval with a 24-hour half-life means maximum staleness of ~0.7% of score—imperceptible to players but keeping rankings dynamic.

---

## Composite Leaderboard Scoring

For games where ranking depends on multiple metrics (kills, assists, objectives, win rate), composite scores combine dimensions into a single sortable value.

```
FUNCTION compute_composite_score(metrics, weights, normalization):
    // metrics: {kills: 45, assists: 12, objectives: 8, win_rate: 0.65}
    // weights: {kills: 0.3, assists: 0.15, objectives: 0.25, win_rate: 0.3}
    // normalization: {kills: {min: 0, max: 100}, assists: {min: 0, max: 50}, ...}

    composite = 0.0

    FOR metric_name, value IN metrics:
        // Normalize to [0, 1] range
        norm = normalization[metric_name]
        normalized = (value - norm.min) / (norm.max - norm.min)
        normalized = CLAMP(normalized, 0.0, 1.0)

        // Apply weight
        composite += normalized * weights[metric_name]

    // Scale to integer range for sorted set precision
    // Use 6 digits for composite, leaving 7 for tiebreak
    final_score = ROUND(composite * 1000000)
...
EXAMPLE:
    ...
    normalization = {
        ...
        objectives: {min: 0, max: 20},
        win_rate: {min: 0, max: 1.0}
    }

    composite = (45/100 × 0.3) + (12/50 × 0.15) + (8/20 × 0.25) + (0.65 × 0.3)
              = 0.135 + 0.036 + 0.100 + 0.195
              = 0.466

    final_score = 466000  (stored in sorted set)
```
