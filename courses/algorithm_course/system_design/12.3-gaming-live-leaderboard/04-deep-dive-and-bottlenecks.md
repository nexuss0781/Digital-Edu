...
### Slowest part of the process 1: Single Sorted Set Memory Limits
...
### Slowest part of the process 2: Cross-Shard Rank Computation
...
### Slowest part of the process 3: Reset Storms
...
### Slowest part of the process 4: Friend Leaderboard at Scale
...
### Slowest part of the process 5: Hot Leaderboard Problem
...
## Deep Dive 4: Real-Time Rank Streaming with Delta Compression

### The Fan-Out Problem

When a score update changes the top-100, every connected spectator must be notified. During a tournament with 5M connected viewers, a single rank change triggers 5M WebSocket messages — a fan-out ratio of 1:5,000,000.

```
FUNCTION compute_rank_delta(leaderboard_id, affected_entries):
    // Instead of sending the full top-100, compute the minimal diff
    previous = GET_CACHED_STATE(leaderboard_id, "top_100")
    current = ZREVRANGE("lb:" + leaderboard_id, 0, 99, WITHSCORES)
    ...
    FOR i IN [0..99]:
        IF current[i] != previous[i]:
            delta.changes.append({
                rank: i + 1,
                player_id: current[i].member,
                score: current[i].score,
                previous_rank: find_in_previous(previous, current[i].member)
            })

    SET_CACHED_STATE(leaderboard_id, "top_100", current, delta.version)

    RETURN delta

EXAMPLE:
    Full top-100 payload: 100 × 48 bytes = 4.8 KB per client
    Delta (avg 2-3 rank changes per update): 3 × 48 bytes = 144 bytes per client
    Bandwidth saving per 5M clients: (4.8 KB - 144 B) × 5M = 23.3 TB → 720 GB
    Reduction: 97% bandwidth savings via delta compression
```

### Client-Side Delta Application

```
FUNCTION apply_delta_client_side(local_leaderboard, delta):
    IF delta.version != local_leaderboard.version + 1:
        // Version gap detected — request full snapshot
        REQUEST_FULL_LEADERBOARD()
        RETURN

    FOR change IN delta.changes:
        // Remove player from old position if they moved
        IF change.previous_rank != NULL:
            local_leaderboard.remove_at(change.previous_rank)

        // Insert at new position
        local_leaderboard.insert_at(change.rank, {
            player_id: change.player_id,
            score: change.score
        })

    local_leaderboard.version = delta.version
```

---

## Industry-Scale Reference Points (2025–2026)

The following production patterns illustrate how large-scale gaming platforms solve leaderboard challenges at the frontier of scale:

```
Pattern: Tiered In-Memory Architecture
  Context: Games with 100M+ registered players and 10M+ DAU
  Approach: Three-tier memory architecture:
    - L1: Hot leaderboards (top 50 by query volume) on dedicated
      high-memory instances with 5+ read replicas each
    - L2: Active leaderboards on shared instances with 2-3 replicas
    - L3: Dormant leaderboards serialized to object storage,
      loaded on-demand with 500ms load time
  Result: 90% cost reduction vs. keeping all leaderboards in memory,
          with < 5ms latency for 99.7% of queries

Pattern: Edge-Accelerated Rank Queries
  Context: Global esports with sub-50ms latency requirements worldwide
  Approach: Read replicas at 30+ edge PoPs, top-1000 pushed via
    invalidation channel every 2 seconds, deeper ranks served on-demand
  Result: P99 query latency reduced from 180ms (centralized) to 22ms (edge),
          with 85% CDN cache hit rate during tournaments

Pattern: ML-Powered Anti-Cheat Pipeline
  Context: Competitive games where manual review cannot scale
  Approach: Real-time inference pipeline evaluates each score submission:
    - Feature extraction: 12 behavioral features per submission
    - Ensemble model: gradient-boosted trees + isolation forest
    - Inference latency budget: < 15ms (within 50ms sync validation budget)
    - False positive rate target: < 0.01% (1 in 10,000 legitimate scores flagged)
  Result: 94% detection rate for score manipulation with 0.005% false positive rate,
          replacing a team of 8 manual reviewers

Pattern: Serverless Leaderboard for Indie/Mobile Games
  Context: Games with unpredictable traffic patterns (0 to 100K players in hours)
  Approach: Serverless functions + managed sorted set service
    - Auto-scaling from 0 to peak with no pre-provisioning
    - Pay-per-query pricing model
    - Built-in seasonal reset via managed TTL keys
  Result: $0.02/1000 queries at low scale vs. $500+/month for dedicated instances,
          with acceptable 20-50ms latency
```
