...
## Insight 10: RAG Preserves Privilege Boundaries Where Fine-Tuning Dissolves Them
**Category:** Security
**One-liner:** Using retrieval-augmented generation with per-tenant document stores instead of fine-tuning on client data maintains attorney-client privilege by keeping each tenant's knowledge isolated at inference time rather than baked into shared model weights.
**Why it matters:** Fine-tuning a shared model on Firm A's contracts creates a risk that the model's learned patterns leak into results for Firm B, breaching attorney-client privilege in a way that may waive privilege for all of Firm A's documents. RAG avoids this by keeping client knowledge in isolated retrieval stores, never touching model weights. Each tenant's playbooks, precedents, and contract templates are retrieved at inference time and injected into the prompt context, providing personalization without cross-contamination. For firm-specific optimization, per-tenant LoRA adapters trained in isolated compute provide fine-tuning benefits while maintaining privilege boundaries — the adapter weights stay within the tenant's data boundary and are never shared.

---

## Insight 11: Jurisdiction-Aware Analysis Requires Source-Language Processing, Not Translation
**Category:** System Modeling
**One-liner:** Analyzing legal documents in their source language and translating only the final summary preserves legal nuance that is lost when translating first and analyzing second.
**Why it matters:** Legal terminology is jurisdiction-specific, not just language-specific. "Force majeure" in French civil law has a narrower scope than in English common law; "Gewährleistung" (warranty) in German law includes statutory protections that have no English equivalent. Running clause extraction and risk assessment in the source language, using jurisdiction-specific clause taxonomies from the knowledge graph, preserves these distinctions. Translation-first approaches produce English text that strips away the jurisdiction-specific legal meaning, leading to false negatives on risk detection (the system fails to flag clauses that are risky under the source jurisdiction's law). The architecture routes documents through language-specific NLP pipelines with jurisdiction-aware models, producing analysis results in a language-neutral structured format that can be rendered in any language.

---

## Insight 12: Tiered Confidence Routing Preserves Senior Attorney Time for the 10% That Matters
**Category:** Traffic Shaping
**One-liner:** Routing clauses to auto-accept (>95% confidence), junior review (80-95%), or senior partner review (<80% or material risk) ensures that expensive senior attorney time is spent only on the 10% of clauses that contain 85% of deal-critical issues.
**Why it matters:** In a 12,000-document due diligence review, roughly 60% of clauses are boilerplate that match known patterns with high confidence, 30% are minor variations that a junior associate can validate quickly, and only 10% involve genuinely novel language, cross-jurisdiction conflicts, or material risk that requires senior judgment. Without confidence-based routing, senior attorneys spend most of their time confirming obvious results. The tiered system acts as a force multiplier — in practice, it converts an 8-week, 40-associate project into a 3.5-week effort by ensuring that every tier of legal expertise is applied to the appropriate difficulty level, mirroring how triage works in emergency medicine.
