...
- "One Slowest part of the process I anticipate is LLM latency for explanations. I'd mitigate with..."
...
#### Trap 6: "Why not just train on your clients' contracts to improve accuracy?"

**What They're Testing**: Understanding of attorney-client privilege and data isolation.

**Good Answer**:
> "Training on client data is the single most dangerous architectural decision in legal AI. Client contracts are protected by attorney-client privilege — using Firm A's contracts to improve results for Firm B breaches privilege, potentially waives it for all of Firm A's documents, and exposes the platform to disbarment proceedings and malpractice claims. Instead, the architecture uses:
> 1. **Synthetic training data**: Generated from public filings, template libraries, and law review articles
> 2. **Federated fine-tuning**: LoRA adapters trained per-tenant that never leave the tenant's boundary
> 3. **Retrieval augmentation**: Each tenant's playbooks and precedents used at inference time via RAG, not baked into model weights
>
> The key insight is that RAG preserves privilege boundaries while fine-tuning dissolves them."

#### Trap 7: "How do you handle documents in 15 different languages across a cross-border deal?"

**What They're Testing**: Practical multi-language architecture, not just saying 'we support i18n.'

**Good Answer**:
> "Cross-border due diligence is the hardest document processing challenge because legal terminology doesn't translate literally. My approach:
> 1. **Language detection at ingestion**: Route to language-specific OCR and NLP pipelines (not a single multilingual model, which underperforms on legal terminology)
> 2. **Jurisdiction-aware clause taxonomy**: The same concept (e.g., 'force majeure') has different legal meanings and scope in French civil law vs. English common law — the knowledge graph models these as separate jurisdiction-specific interpretations
> 3. **Analyze in source language, summarize in target**: Running clause extraction in the original language preserves legal nuance; only the final summary and risk assessment are translated
> 4. **Language-specific confidence thresholds**: Lower confidence triggers earlier attorney escalation for languages with less training data
>
> The architecture insight is that translation is the wrong abstraction — you need jurisdiction-aware legal analysis, not word-for-word translation."
