...
### 7.1 Why Event Sourcing Is Non-Negotiable for Banking

| Alternative | Problem in Banking Context |
|------------|---------------------------|
| Mutable balance updates | Lost audit trail; reconciliation gaps; cannot prove how a balance was reached |
| Database triggers for audit | Triggers can be disabled; audit trail is a secondary artifact, not the source of truth |
| Change Data Capture (CDC) | Captures what changed, not why; business intent is lost in CDC events |
| **Event sourcing** | **Events ARE the source of truth; audit trail is inherent; intent is captured** |

### 7.2 Transaction Ordering Guarantees

```
Within a single account: strictly ordered (serial processing per account)
Across accounts: causally ordered (transfer debit happens-before credit)
Across entities: eventual consistency (inter-entity settlement via clearing)

Implementation:
  - Account-level: partition events by account_id → single writer per account
  - Causal ordering: distributed transaction coordinator ensures debit commits
    before credit is attempted
  - Cross-entity: settlement events processed by clearing engine with idempotency
```
