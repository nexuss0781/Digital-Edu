...
| Mistake | Why It's Wrong | Correct Approach |
|---|---|---|
| Single database for all data | Slowest part of the process, no read/write optimization | CQRS with event store + read projections |
...
### 7.1 Why CQRS for Banking

| Approach | Write Path | Read Path | Verdict |
|----------|-----------|-----------|---------|
| Single model (CRUD) | Simple UPDATE balance | Same table, same indexes | Fails — audit trail lost, write-read contention at 50K TPS |
| Separate read replicas | Primary handles writes | Replicas handle reads | Better — but replica lag means stale balances |
| **CQRS + Event Sourcing** | **Append event to immutable log** | **Materialized projections** | **Chosen — decouples write throughput from read patterns; full audit by design** |

### 7.2 Why Double-Entry Is Non-Negotiable

```
Single-entry ledger:
  Account A: balance = $10,000
  → How did it get there? Unknown. Balance was UPDATE'd in place.
  → Where did $500 come from yesterday? Requires separate audit log.
  → Does the bank's total balance sheet balance? Manual reconciliation.

Double-entry ledger:
  Every transaction creates balanced journal entries:
    DEBIT:  A's cash account     +$500
    CREDIT: B's cash account     -$500
  → How did A get $10,000? Sum all debit entries. Provable.
  → Total debits = total credits? Always. Mathematical guarantee.
  → Reconciliation? Unnecessary — the ledger IS reconciled by construction.
```

### 7.3 Whiteboard Sketch Guide
...
Essential diagram for a 45-minute interview:
  ...
  └──────────┘          └────┬─────┘
       ...
       ↓                     ↓                     ↓
  ┌──────────┐        ┌───────────┐        ┌───────────┐
  │ Account  │        │Transaction│        │ Compliance │
  ...
  Label: "ACID writes" on event store path
  Label: "event sourced, immutable" on event store
  Label: "real-time ML scoring" on fraud path
  Label: "ISO 20022" on external interfaces
```
...
|----------|----------------|
| Using mutable balance fields without event sourcing | Doesn't understand banking audit requirements — regulators require full provenance |
| Single-entry bookkeeping | Misses the fundamental constraint — double-entry is mandatory for financial integrity |
| No mention of ACID transactions | Critical for financial systems — eventual consistency is unacceptable for balance mutations |
| Proposing a single relational database for everything | Doesn't understand CQRS need — write-optimized event store and read-optimized projections have different requirements |
| Ignoring regulatory compliance | Banking is one of the most regulated industries — compliance is a first-class architectural concern |
| No disaster recovery discussion | Banks require RPO=0 and RTO < 60s — this is a defining constraint, not an afterthought |
| Treating multi-currency as a display concern | Multi-currency affects the ledger model, FX position management, and regulatory reporting |
