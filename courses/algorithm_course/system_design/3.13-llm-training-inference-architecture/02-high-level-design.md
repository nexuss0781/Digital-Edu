...
### Decision 3: Disaggregated vs Colocated Serving (2025+)

| Factor | Colocated (Traditional) | Disaggregated (Prefill-Decode Split) |
|--------|------------------------|--------------------------------------|
| **Architecture** | Single pool handles both phases | Separate prefill and decode GPU pools |
| **Scaling** | Coupled - must scale both together | Independent - scale prefill and decode separately |
| **GPU Selection** | Same GPU type for both | Compute-optimized for prefill, memory-optimized for decode |
| **KV Transfer** | Local (zero-copy) | Network transfer between pools (requires fast interconnect) |
| **Complexity** | Simple deployment | Complex KV cache transfer, routing |
| **Utilization** | 40-60% (phases fight for resources) | 70-85% (each pool optimized for its workload) |
| **Best For** | Small deployments, low latency | High-throughput, cost-sensitive production |
...
│                                                               │
│  ┌──────────┐    ┌────────────┐    ┌──────────────────────┐  │
│  │ Request  │───▶│  Scheduler │───▶│  Prefill Pool         │  │
│  │ Gateway  │    │  & Router  │    │  (Compute-optimized)  │  │
│  └──────────┘    └─────┬──────┘    │  B200 × N instances   │  │
│                        │           └─────────┬────────────┘  │
│                        │                     │ KV cache       │
│                        │                     │ transfer       │
...
│                                                               │
│  Benefits:                                                    │
│  - Prefill: high compute → use FP8, pack many prompts        │
│  - Decode:  high memory BW → use large batches, quantize     │
│  - Scale each pool independently based on traffic shape      │
└──────────────────────────────────────────────────────────────┘
```

**Recommendation:** Disaggregated serving for production deployments >50 instances; colocated for simpler/smaller deployments.

### Decision 4: Inference Batching Strategy
...
### Decision 5: Quantization Strategy
...
| **FP8 (E4M3)** | 8 | 2x | <0.5% | 4x vs FP32 | Training/Inference |
| **INT8 (W8A8)** | 8 | 2x | <1% | 2x vs FP16 | Inference |
| **MXFP4** | 4 | 3-4x | <1% | 4x vs FP16 | Inference (B200+) |
| **INT4 (W4A16)** | 4 | 3-4x | 1-3% | 4x vs FP16 | Inference |
| **AWQ** | 4 | 3-4x | <1% | 4x vs FP16 | Inference (best open) |
| **GPTQ** | 4 | 3-4x | 1-2% | 4x vs FP16 | Inference |
| **2-bit (QuIP#)** | 2 | 5-8x | 3-8% | 8x vs FP16 | Edge/memory-extreme |

**Recommendation:** AWQ for production inference on current hardware, MXFP4 on next-gen (B200+), FP8 for latency-sensitive applications.

### Decision 6: KV Cache Strategy
...
| **Prefix Caching (Hash)** | Excellent + reuse | Moderate | Repeated prompts |
| **Radix Attention (Trie)** | Best reuse (3-5x vs hash) | Complex | Multi-turn chat, branching |
| **Sliding Window** | Fixed memory | Simple | Very long contexts |
| **Quantized KV (FP8/INT4)** | 2-4x reduction | Moderate | Memory-constrained |
| **KV Cache Compression** | 4-8x reduction | Complex | Ultra-long context (128K+) |

**Recommendation:** PagedAttention with radix-tree prefix caching for production. Add KV quantization for long-context workloads.
...
| Layer | Options | Recommendation |
|-------|---------|----------------|
| **Framework** | vLLM, TensorRT-LLM, SGLang, LMDeploy | vLLM (general), SGLang (advanced routing) |
| **Attention** | Flash Attention 3, FlashInfer | Flash Attention 3 (Hopper+) |
| **Quantization** | GPTQ, AWQ, MXFP4, bitsandbytes | AWQ (current), MXFP4 (B200+) |
| **Load Balancing** | nginx, Envoy | Envoy (gRPC + KV-cache-aware routing) |
| **Orchestration** | Kubernetes, Ray, SkyPilot | Ray Serve / Kubernetes |
| **Monitoring** | Prometheus + Grafana | Prometheus + Grafana |
| **Disaggregated Transfer** | RDMA, UCX, NVLink C2C | RDMA for inter-node KV transfer |
