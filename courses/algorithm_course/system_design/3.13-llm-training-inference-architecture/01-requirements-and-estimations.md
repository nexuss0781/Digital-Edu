...
6. **Mixture-of-Experts Training**
   - Expert parallelism with load-balanced routing
   - Auxiliary loss for expert utilization balancing
   - Shared expert + routed expert architectures
   - Dynamic top-k routing with capacity factors

7. **Long-Context Training**
   - Context parallelism for sequences >128K tokens
   - Ring attention across GPU groups
   - Progressive context length extension
...
6. **Disaggregated Serving**
   - Separate prefill and decode into independent GPU pools
   - Transfer KV cache between prefill and decode instances
   - Independent auto-scaling per phase
   - Prefill-optimized and decode-optimized GPU configurations

7. **Long-Context Inference**
   - Support for 128K-1M+ token contexts
...
| Feature | Reason |
|---------|--------|
| Fine-tuning APIs (LoRA, QLoRA) | Covered in MLOps Platform design |
| RAG integration | Covered in RAG System design |
| Agent orchestration | Covered in Multi-Agent Orchestration design |
| Custom kernel development (Triton, CUDA) | Implementation detail |
| Hardware procurement and cluster networking | Infrastructure concern |
| Embedding model serving | Simpler serving pattern, covered in Vector Database |
...
| **Max Context** | 128K-1M tokens | Long-context support |
| **Disaggregated Transfer** | <50ms | KV cache transfer between prefill/decode |
...
| Year | GPU Generation | Memory | FP8 TFLOPS | FP4 TFLOPS | Cost/hr |
|------|----------------|--------|------------|------------|---------|
| 2024 | H100 | 80 GB | 1,979 | - | $3.00 |
| 2025 | H200/B200 | 141-192 GB | 4,500 | 9,000 | $4.00 |
| 2025 | GB200 NVL72 | 13.8 TB (rack) | 324,000 (rack) | 648,000 (rack) | $50/rack-hr |
| 2026 | Rubin/R100 | 256+ GB | 12,000+ | 24,000+ | $6.00 |

### Quantization Impact on Inference Economics

| Precision | Memory vs FP16 | Throughput Gain | Quality Impact | Hardware Req |
|-----------|---------------|-----------------|----------------|-------------|
| FP16 | 1x (baseline) | 1x | None | Any GPU |
| FP8 (W8A8) | 0.5x | 2x | <0.5% | H100+ |
| INT8 (W8A8) | 0.5x | 2x | <1% | Any GPU |
| INT4 (W4A16) | 0.25x | 2-3x | 1-3% | Any GPU |
| FP4 (MXFP4) | 0.25x | 3-4x | <1% | B200+ |
| INT4 (AWQ) | 0.25x | 3-4x | <1% | Any GPU |
| 2-bit (GPTQ) | 0.125x | 4-6x | 3-8% | Any GPU |
