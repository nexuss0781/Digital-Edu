...
**Key Differentiator:** Modern LLM infrastructure combines memory-efficient distributed training (ZeRO/FSDP sharding across thousands of GPUs) with inference optimizations (KV cache paging, speculative decoding, quantization) to enable trillion-parameter models to be trained in weeks and served at millions of requests per day with <200ms time-to-first-token. The 2025-era shift toward **disaggregated prefill-decode architectures**, **FP4/microscaling quantization**, and **Mixture-of-Experts routing at 600B+ active parameters** represents the next inflection point in both training efficiency and inference cost reduction.
...
| **What AI decides autonomously** | Continuous batching scheduler decisions, KV cache block allocation and eviction, speculative decoding draft-verify cycles, auto-scaling inference replicas based on queue depth, request routing across model instances, prefill-decode pool rebalancing |
| **What AI recommends (human approves)** | Quantization strategy selection (INT8/FP4/FP8) based on quality benchmarks, parallelism configuration for new model deployments, checkpoint frequency tuning, training hyperparameter adjustments, spot instance preemption strategies |
| **What requires human judgment** | Training run initiation and budget allocation, model architecture changes, production model weight updates, safety alignment modifications, GPU cluster capacity procurement |
| **Escalation trigger** | MFU drops below 40%, training loss diverges or plateaus, inference TTFT p99 exceeds SLO, GPU node failure rate > 1% per hour, KV cache OOM across cluster |

> **Rationale:** LLM infrastructure operates at a scale where real-time decisions (batching, cache management, routing) must be fully autonomous -- human latency would degrade throughput by orders of magnitude. However, training initiation and model weight changes carry significant cost and safety implications requiring human oversight.
...
| Primary Slowest part of the process | Communication (AllReduce) | Memory bandwidth (KV cache) |
...
| Emerging Pattern | FP8/FP4 mixed precision, context parallelism | Disaggregated prefill-decode, radix attention |
...
| **Disaggregated Scheduler** | Separate prefill/decode pools | Independent scaling of phases |
...
| GPU | Memory | FP16 TFLOPS | FP8 TFLOPS | FP4 TFLOPS | NVLink BW | Best For |
|-----|--------|-------------|------------|------------|-----------|----------|
| H100 SXM | 80 GB | 989 | 1,979 | - | 900 GB/s | Training |
| H200 SXM | 141 GB | 989 | 1,979 | - | 900 GB/s | Inference |
| B200 | 192 GB | 2,250 | 4,500 | 9,000 | 1,800 GB/s | Both |
| GB200 NVL72 | 192 GB/GPU (13.8 TB rack) | 2,250 | 4,500 | 9,000 | 1,800 GB/s | Mega-scale |
| TPU v6e (Trillium) | 32 GB | - | - | - | 4,800 GB/s (ICI) | Training |
...
| Model | Parameters | Architecture | FP16 Memory | INT8 Memory | INT4 Memory |
|-------|------------|-------------|-------------|-------------|-------------|
| LLaMA-3-8B | 8B | Dense | 16 GB | 8 GB | 4 GB |
| LLaMA-3-70B | 70B | Dense | 140 GB | 70 GB | 35 GB |
| LLaMA-3-405B | 405B | Dense | 810 GB | 405 GB | 203 GB |
| DeepSeek-V3 | 671B (37B active) | MoE (256 experts) | 1.3 TB | 671 GB | 336 GB |
| Mixtral 8x22B | 141B (39B active) | MoE (8 experts) | 282 GB | 141 GB | 71 GB |
| Qwen-2.5-72B | 72B | Dense + GQA | 144 GB | 72 GB | 36 GB |
...
| >200B MoE | 4D + EP (TP+PP+DP+EP) | 4096+ GPUs, TP=8, PP=8, DP=16, EP=16 |
...
|---------|-------------|------------|
| **Training** | Megatron-LM, DeepSpeed, torchtitan, Nanotron | NVIDIA NeMo, MosaicML |
| **Serving** | vLLM, TensorRT-LLM, SGLang, LMDeploy | Anyscale, Together, Fireworks |
| **Orchestration** | Ray, Kubernetes, SkyPilot | SageMaker, Vertex AI |
| **Fine-Tuning** | Axolotl, LLaMA-Factory, Unsloth | Scale AI, Anyscale |
| **Monitoring** | Prometheus + Grafana | Datadog, Weights & Biases |

### 2025-2026 Architecture Trends

| Trend | Description | Impact |
|-------|-------------|--------|
| **Disaggregated Serving** | Separate prefill and decode into independent pools | Independent scaling, better GPU utilization |
| **FP4/MXFP4 Quantization** | Sub-4-bit weight quantization with microscaling | 2x memory reduction vs INT8 with minimal quality loss |
| **Radix Attention** | Trie-based prefix sharing across requests | 3-5x cache hit rate improvement over hash-based prefix caching |
| **Context Parallelism** | Split long sequences across GPUs during training | Enable 1M+ context training without memory explosion |
| **Multi-Token Prediction** | Predict multiple future tokens in one forward pass | 2-3x training data efficiency, native speculative decode |
| **Expert Offloading** | Load MoE experts on-demand from CPU/NVMe | Serve 600B+ MoE models on fewer GPUs |
| **Distillation-at-Scale** | Train small models from large model outputs | 10-50x inference cost reduction |
...
|  disaggregated serving, radix attention, context parallelism, FP4,    |
|  multi-token prediction, EAGLE, Medusa, chunked prefill, GQA/MQA     |
|                                                                       |
+-----------------------------------------------------------------------+
```
...
|--------|-------------|----------------|
| [3.2 ML Models Deployment](../3.2-ml-models-deployment-system/00-index.md) | Model serving infrastructure | Shared inference optimization patterns (batching, quantization) |
| [3.4 MLOps Platform](../3.4-mlops-platform/00-index.md) | Training pipeline management | Experiment tracking, hyperparameter tuning orchestration |
| [3.23 LLM Inference Engine](../3.23-llm-inference-engine/00-index.md) | Deep dive on serving frameworks | vLLM/TensorRT-LLM internals, PagedAttention implementation |
| [3.24 Multi-Agent Orchestration](../3.24-multi-agent-orchestration-platform/00-index.md) | Multi-model coordination | Agent-based routing, multi-model inference pipelines |
| [3.28 AI Memory Management](../3.28-ai-memory-management-system/00-index.md) | KV cache and context management | Long-context memory, retrieval-augmented context |
| [3.14 Vector Database](../3.14-vector-database/00-index.md) | Embedding storage for RAG | Training data retrieval, inference-time augmentation |
| [2.6 Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) | Training job orchestration | GPU cluster scheduling, priority-based resource allocation |
| [2.13 Edge AI/ML Inference](../2.13-edge-ai-ml-inference/00-index.md) | On-device inference | Quantized model deployment, edge-cloud hybrid serving |

---

> **Vendor Freshness Notice:** This document references specific hardware, models, and frameworks (H100, B200, LLaMA, DeepSeek, Mixtral, Qwen, vLLM, TensorRT-LLM, Megatron-LM, DeepSpeed, torchtitan, Nanotron, NVIDIA NeMo). The LLM infrastructure landscape evolves extremely rapidly -- new GPU architectures, model families, and serving frameworks emerge quarterly. Verify hardware specifications, framework features, and benchmark numbers against official documentation before making infrastructure decisions. Last reviewed: 2026-03.
