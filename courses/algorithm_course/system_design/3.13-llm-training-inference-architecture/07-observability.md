...
| Metric | Description | Target | Alert Threshold |
|--------|-------------|--------|-----------------|
| **Expert Utilization** | Tokens per expert (balance) | Uniform | >2x mean deviation |
| **Routing Entropy** | Distribution uniformity | High | <90% of max entropy |
| **All-to-All Time** | Expert communication | <10% of step | >20% of step |
| **Dropped Tokens** | Tokens exceeding expert capacity | <1% | >5% |
| **Auxiliary Loss** | Load balancing loss value | Low, stable | Spike >3x |
...
| **Prefix Cache Hit Rate** | Radix tree cache hits | >40% | <20% |

#### Disaggregated Serving Metrics

| Metric | Description | Target | Alert Threshold |
|--------|-------------|--------|-----------------|
| **KV Transfer Latency** | Prefill→Decode KV transfer | <50ms p99 | >100ms |
| **KV Transfer Throughput** | GB/s of KV cache transferred | >50 GB/s | <20 GB/s |
| **Prefill Pool Utilization** | Compute utilization | 80-90% | <50% or >95% |
| **Decode Pool Utilization** | Memory bandwidth utilization | 70-80% | <40% or >90% |
| **Phase Queue Imbalance** | Prefill vs decode queue ratio | ~1:1 | >3:1 |
...
| **Low MFU** | MFU < 40% for 1 hour | Warning | Investigate Slowest part of the process |
...
inference_prefix_cache_hit_rate{model="claude-70b"} 0.65
inference_speculative_accept_rate{model="claude-70b"} 0.85

# MoE Metrics
training_expert_utilization{job_id="job-123", expert="0"} 0.42
training_expert_utilization{job_id="job-123", expert="1"} 0.38
training_all_to_all_time_seconds{job_id="job-123"} 0.15
training_dropped_tokens_percent{job_id="job-123"} 0.003

# Disaggregated Serving Metrics
disagg_kv_transfer_latency_seconds_bucket{le="0.05"} 9500
disagg_kv_transfer_latency_seconds_bucket{le="0.1"} 9900
disagg_prefill_pool_utilization{pool="prefill-1"} 0.85
disagg_decode_pool_utilization{pool="decode-1"} 0.72
```
...
This system's observability practices align with the broader AI observability and governance standards defined at the platform level. For comprehensive guidance on AI-specific metrics, drift detection frameworks, and governance dashboards, see:

| Reference | Relevance to This System |
|-----------|--------------------------|
| [3.25 AI Observability Platform](../3.25-ai-observability-platform/00-index.md) | Centralized model quality monitoring for training loss curves and inference output quality, MFU tracking across GPU clusters, and serving latency dashboards |
| [3.26 AI Governance & Compliance](../3.26-ai-governance-compliance/00-index.md) | Safety evaluation audit trails for model weight releases, training data provenance tracking, and compliance reporting for model deployment decisions |

**Key integration points:**
- Training loss, MFU, and checkpoint metrics feed into the centralized AI Observability dashboards
- Inference TTFT/TPS and quality evaluations are exported as standardized serving health indicators
- Model weight release decisions and safety evaluation results are tracked in the governance audit trail
