...
## Deep Dive 4: Disaggregated Prefill-Decode Architecture

### Why This Is Critical

In traditional colocated serving, prefill (compute-bound) and decode (memory-bandwidth-bound) compete for the same GPU resources. A long prefill for one request delays token generation for all concurrent decode requests, causing TTFT spikes. Disaggregated serving separates these phases into independent GPU pools.
...
flowchart TB
    subgraph Router["Intelligent Router"]
        direction LR
        ReqRouter["Request<br/>Router"]
        KVTracker["KV Cache<br/>Tracker"]
    end

    subgraph PrefillPool["Prefill Pool (Compute-Optimized)"]
        direction LR
        P1["Prefill GPU 1<br/>B200 FP8"]
        P2["Prefill GPU 2<br/>B200 FP8"]
        PN["Prefill GPU N<br/>B200 FP8"]
    end

    subgraph Transfer["KV Transfer Layer"]
        RDMA["RDMA / NVLink C2C"]
    end

    subgraph DecodePool["Decode Pool (Memory-Optimized)"]
        direction LR
        D1["Decode GPU 1<br/>H200 INT8"]
        D2["Decode GPU 2<br/>H200 INT8"]
        DM["Decode GPU M<br/>H200 INT8"]
    end

    Router --> PrefillPool
    PrefillPool -->|"KV cache"| Transfer
    Transfer -->|"KV cache"| DecodePool
    DecodePool -->|"Tokens"| Router

    classDef router fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef prefill fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef transfer fill:#e0f7fa,stroke:#00695c,stroke-width:2px
    classDef decode fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class ReqRouter,KVTracker router
    class P1,P2,PN prefill
    class RDMA transfer
    class D1,D2,DM decode
```

### KV Cache Transfer Protocol

```
PROTOCOL: Disaggregated KV Transfer

STEPS:
1. Prefill instance processes prompt, generates KV cache
2. KV cache is serialized and sent to decode instance via RDMA
3. Decode instance reconstructs block table from received KV data
4. Token generation begins immediately

TRANSFER_SIZE:
    70B model, 1K prompt:
    KV per token = 5.2 MB × 1000 tokens = 5.2 GB
    RDMA transfer time = 5.2 GB / 100 GB/s = 52 ms

    Acceptable if decode pool is already loaded and warm.
    Optimization: pipeline KV transfer with prefill computation.

OPTIMIZATION: Partial Transfer
    - Transfer only last few layers initially
    - Decode starts generating with partial KV
    - Remaining layers transfer in background
    - Reduces visible transfer latency to <20ms
```

### Benefits and Trade-offs

| Aspect | Colocated | Disaggregated |
|--------|-----------|---------------|
| GPU utilization | 40-60% | 70-85% |
| Prefill-decode interference | High | None |
| TTFT variance | High (decode delays prefill) | Low (independent pools) |
| Scaling flexibility | Coupled | Independent per phase |
| KV transfer overhead | Zero | 20-50ms per request |
| Operational complexity | Low | High |
| Cost at scale (>100 instances) | Higher | 20-40% lower |

---

## Deep Dive 5: MoE Training and Expert Load Balancing

### Why This Is Critical

Mixture-of-Experts (MoE) models like DeepSeek-V3 (671B total, 37B active) achieve superior quality per FLOP but introduce unique challenges: expert routing imbalance, All-to-All communication overhead, and expert parallelism coordination.

### Expert Routing Imbalance

```
PROBLEM: Without load balancing, popular experts receive disproportionate traffic.

Imbalanced routing (no auxiliary loss):
    Expert 0: 15% of tokens  (overloaded)
    Expert 1: 12% of tokens
    Expert 2: 0.5% of tokens (underutilized)
    ...
    Expert 255: 0.1% of tokens (wasted capacity)

Balanced routing (with auxiliary loss):
    Each expert: ~0.4% of tokens (256 experts, near-uniform)
    Variance: <0.1%
...
AUXILIARY_LOSS:
    ...
    This loss penalizes any correlation between routing
    frequency and routing probability, encouraging uniformity.
```

### DeepSeek-V3 Innovations (Case Study)

| Innovation | Description | Impact |
|-----------|-------------|--------|
| **Shared + Routed** | 2 shared experts always active + 6 routed | Stable baseline + specialized routing |
| **FP8 Training** | Train in FP8 from start (no FP16 warmup) | 2x training throughput |
| **Multi-Token Prediction** | Predict next 2 tokens simultaneously | Better training signal, native speculative decode |
| **Load Balancing without Aux Loss** | Dynamic bias terms replace auxiliary loss | No quality degradation from balancing |

---

## Slowest part of the process Analysis

### Slowest part of the process 1: Communication Overhead in Distributed Training
...
### Slowest part of the process 2: Memory Bandwidth in Inference Decoding
...
### Slowest part of the process 3: Pipeline Bubbles
...
### Slowest part of the process 4: KV Cache Memory Exhaustion
...
### Slowest part of the process 5: MoE All-to-All Communication

**Problem:** MoE models require All-to-All communication at every expert layer to route tokens to their assigned experts across GPUs.

**Symptoms:**
- MoE training slower than expected for active parameter count
- Network saturation during expert layers
- Expert utilization imbalance despite auxiliary loss

**Root Cause Analysis:**

```
All-to-All for MoE routing:
Each GPU sends/receives tokens to/from all other GPUs

For DeepSeek-V3 (256 experts, EP=16):
- Tokens per batch: 8192
- Active experts per token: 8
- Data per All-to-All = 8192 × 8 × hidden_size × 2 bytes
                      = 8192 × 8 × 7168 × 2
                      = ~900 MB per MoE layer

With 60 MoE layers per step:
- Total All-to-All = 54 GB per step
- At 400 Gb/s InfiniBand: 1.08 seconds of pure transfer
```
...
| Strategy | Description | Improvement |
|----------|-------------|-------------|
| **Expert parallelism** | Distribute experts across fewer groups | Reduce All-to-All scope |
| **Token dropping** | Drop tokens exceeding expert capacity | Reduce traffic |
| **Expert offloading** | Load experts on-demand from CPU | Reduce GPU memory, increase latency |
| **Hierarchical routing** | Route within node first, then across | Better locality |
| **Communication overlap** | Overlap All-to-All with shared expert computation | Hide latency |
