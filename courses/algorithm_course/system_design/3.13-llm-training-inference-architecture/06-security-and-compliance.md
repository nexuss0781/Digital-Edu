...
1. Output Watermarking (Inference-Time)
   - Embed statistical signal in token sampling
   - Green/red list partitioning per position
   - Detectable with ~25 tokens, invisible to users
   - Trade-off: slight quality degradation with aggressive watermarking

ALGORITHM:
    FUNCTION watermarked_sample(logits, position, secret_key):
        # Hash position + key to partition vocabulary
        hash_val = hash(position, secret_key, prev_token)
        green_tokens = vocab where hash(token, hash_val) > 0.5
        red_tokens = vocab - green_tokens

        # Boost green token probabilities
        logits[green_tokens] += delta  # delta ≈ 2.0
        return sample(softmax(logits))

    FUNCTION detect_watermark(text, secret_key):
        green_count = count tokens in green list
        z_score = (green_count - n/2) / sqrt(n/4)
        return z_score > threshold  # threshold ≈ 4.0

2. Weight Watermarking (Training-Time)
   - Embed identifier in model weights
   - Survives fine-tuning and quantization
   - Used to detect unauthorized model distribution

3. Membership Inference Defense
   - Prevent detection of whether specific data was in training set
   - Differential privacy noise during training
   - Output perturbation at inference time
```
...
| **EO 14110 (US)** | Foundation models | Safety testing, red-teaming, reporting | US deployment |
| **NIST AI RMF** | AI systems | Risk management, trustworthiness characteristics | Global best practice |
...
|---------|---------|-----------------|
| Failed logins | Brute Force (Checking every single possibility) detection | >10 in 5 min |
