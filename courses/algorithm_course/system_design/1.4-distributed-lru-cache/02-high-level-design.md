...
### Decision 4: Eviction Policy Selection

| Option | Hit Rate | Memory Overhead | Scan Resistance | Complexity |
|--------|----------|-----------------|-----------------|------------|
| **LRU** | Good | Lowest (pointers only) | Poor | Lowest |
| **LFU** | Better for skewed | Higher (counters) | Good | Medium |
| **W-TinyLFU** | Best overall | Low (4-bit CMS) | Excellent | High |
| **S3-FIFO (First-In-First-Out, like a line at a store)** | Near-optimal | O(1) per entry | Excellent | Medium |
| **ARC** | Excellent adaptive | 2x metadata | Good | High |

**Recommendation:** LRU for simple deployments. W-TinyLFU for production systems requiring best hit rate (used by Caffeine/JVM caches). S3-FIFO (First-In-First-Out, like a line at a store) for flash-based tiers or when implementation simplicity matters.

**Rationale:**
- LRU admits every item on first access — a single database scan can evict the entire hot working set
- W-TinyLFU uses a Bloom filter admission gate: new items enter a 1% window, and must demonstrate frequency before promotion to the 99% main cache
- S3-FIFO (First-In-First-Out, like a line at a store) achieves near-optimal hit rates with three FIFO (First-In-First-Out, like a line at a store) queues (small, main, ghost) — no linked-list pointer manipulation, making it flash-friendly
- At Netflix scale (14 PB cached), a 5% hit rate improvement from W-TinyLFU over LRU translates to hundreds of thousands of avoided database queries per second

### Decision 5: Client Connection Model
...
| ✅ Single-threaded vs Multi-threaded | Multi-threaded (2025+) | Shared-nothing cores for vertical scaling |
...
### Option 3: Vertical Scaling with Multi-Threaded Cache (2025+)
...
flowchart TB
    subgraph Region["Single Region"]
        subgraph AZ1["AZ-1"]
            App1[App Servers]
            DFly1["DragonflyDB<br/>64 cores, 512 GB<br/>2M+ QPS"]
        end
        subgraph AZ2["AZ-2"]
            App2[App Servers]
            DFly2["DragonflyDB<br/>64 cores, 512 GB<br/>(Replica)"]
        end
    end

    App1 --> DFly1
    App2 --> DFly1
    DFly1 -->|Replication| DFly2
```

**Pros:** Eliminates cluster coordination, simpler operations, higher per-node throughput
**Cons:** Larger blast radius per node failure, vertical scaling limits

### Deployment Comparison

| Aspect | Single Region Multi-AZ | Multi-Region | Vertical (Multi-threaded) |
|--------|----------------------|--------------|---------------------------|
| Latency | 1-2ms same region | 1-2ms local, 50-200ms cross-region | < 1ms (no inter-node hops) |
| Availability | 99.99% | 99.999% | 99.99% (with replica) |
| Consistency | Strong within region | Eventual across regions | Strong within instance |
| Cost | $$ | $$$$ | $ (fewer nodes) |
| Complexity | Medium | High | Low |
| Use case | Most applications | Global, zero-downtime required | High-throughput single-region |

### Architecture Decision: Clustered vs Vertical (2025)

The 2025 landscape introduces a meaningful choice between traditional clustered caching and vertical scaling with multi-threaded engines:

| Factor | Clustered (Valkey/Memcached) | Vertical (DragonflyDB/Garnet) |
|--------|------------------------------|-------------------------------|
| Max throughput | Unlimited (horizontal) | 4M+ QPS per instance |
| Working set | Unlimited (sharded) | Limited by single node memory |
| Failure blast radius | 1/N of keyspace | 100% of keyspace |
| Operational complexity | High (cluster management) | Low (single process) |
| Data locality | Cross-node for multi-key ops | All data local |
| Best fit | > 1 TB working set, multi-region | < 1 TB, single-region, high QPS |

**Recommendation:** Start with a vertical multi-threaded engine for working sets under 500 GB. Migrate to clustered architecture when working set exceeds single-node capacity or multi-region is required.
