...
## Access Control Lists (ACLs) — 2025 Capabilities

Modern cache engines (Valkey 8, Redis 7+) support fine-grained ACLs that go beyond simple password authentication:

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    ACL CONFIGURATION (Valkey/Redis 7+)                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Per-User ACL Rules:                                                     │
│                                                                          │
│  user product_service on >secure_password_hash                           │
│    ~product:*            # Can access keys matching product:*            │
│    +get +mget +set       # Allowed commands: read and write              │
│    -del -flushdb         # Denied: delete operations                     │
│                                                                          │
│  user analytics_reader on >another_password_hash                         │
│    ~product:*            # Read-only access to product keys              │
│    ~metrics:*            # And metrics keys                              │
│    +get +mget +scan      # Read-only commands                            │
│    -set -del -flushdb    # No write access                               │
│                                                                          │
│  user admin on >admin_password_hash                                      │
│    ~*                    # Access all keys                               │
│    +@all                 # All commands                                  │
│    -@dangerous           # Except dangerous commands (FLUSHALL, DEBUG)   │
│                                                                          │
│  Benefits:                                                               │
│  ├── Principle of least privilege per service                            │
│  ├── Key pattern restrictions prevent cross-tenant access                │
│  ├── Command restrictions prevent accidental destructive operations      │
│  └── Audit trail per authenticated user                                 │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**Threat:** An attacker measures cache response time differences to infer whether specific keys exist, revealing information about other users' data.

**Attack Pattern:**
- GET for `user:12345:premium_status` returns in 0.1ms (cache hit) → user exists and has been queried recently
- GET for `user:99999:premium_status` returns in 0.5ms (cache miss + DB query) → user may not exist

**Mitigations:**

| Mitigation | Implementation | Effectiveness |
|------------|----------------|---------------|
| Constant-time responses | Add artificial delay to equalize hit/miss timing | High (but adds latency) |
| Key hashing | Store under `HMAC(key, secret)` so attacker cannot guess keys | High |
| Namespace isolation | Separate cache instances per tenant | Highest (strongest isolation) |
| Noise injection | Add random jitter to response times | Medium |
