...
### Multi-Threaded Engine Metrics (2025)

Modern multi-threaded cache engines expose additional metrics critical for capacity planning:

```yaml
# DragonflyDB / Garnet specific metrics

# HELP cache_per_core_qps Requests per second per CPU core
# TYPE cache_per_core_qps gauge
cache_per_core_qps{core="0"} 62500
cache_per_core_qps{core="1"} 61800
cache_per_core_qps{core="63"} 63200

# HELP cache_tiered_storage_bytes Bytes stored per storage tier
# TYPE cache_tiered_storage_bytes gauge
cache_tiered_storage_bytes{tier="dram"} 68719476736
cache_tiered_storage_bytes{tier="nvme"} 4398046511104

# HELP cache_tier_promotion_total Items promoted from cold to hot tier
# TYPE cache_tier_promotion_total counter
cache_tier_promotion_total 1234567

# HELP cache_tier_eviction_total Items evicted from hot tier to cold
# TYPE cache_tier_eviction_total counter
cache_tier_eviction_total 987654

# HELP cache_cross_shard_operations Multi-key operations crossing shard boundaries
# TYPE cache_cross_shard_operations counter
cache_cross_shard_operations{operation="mget"} 45678

# HELP cache_client_tracking_keys Keys being tracked for client-side caching
# TYPE cache_client_tracking_keys gauge
cache_client_tracking_keys 2345678

# HELP cache_invalidation_messages_total Server-pushed invalidations sent
# TYPE cache_invalidation_messages_total counter
cache_invalidation_messages_total 9876543
```

| Metric | Alert Threshold | Why It Matters |
|--------|-----------------|----------------|
| Per-core QPS imbalance | > 30% variance | Indicates key skew in shared-nothing architecture |
| Tiered storage promotion rate | > 10K/sec sustained | Hot data is larger than DRAM tier — scale up DRAM |
| Client tracking keys | > 80% of server memory | RESP3 tracking metadata can consume significant memory |
| Cross-shard operation rate | > 20% of total ops | Multi-key commands are slower in shared-nothing engines |

### Eviction Policy Observability

```yaml
# W-TinyLFU specific metrics
# HELP cache_admission_filter_rejects Items rejected by admission filter
# TYPE cache_admission_filter_rejects counter
cache_admission_filter_rejects 567890

# HELP cache_window_evictions Items evicted from W-TinyLFU window space
# TYPE cache_window_evictions counter
cache_window_evictions 345678

# HELP cache_main_evictions Items evicted from W-TinyLFU main space
# TYPE cache_main_evictions counter
cache_main_evictions 12345

# HELP cache_sketch_reset_total Count-Min Sketch counter resets
# TYPE cache_sketch_reset_total counter
cache_sketch_reset_total 42
```

**Insight:** If `admission_filter_rejects` is high relative to total misses, W-TinyLFU is effectively protecting the cache from scan pollution. If it is near zero, the workload has high reuse and a simpler LRU would suffice.
