...
| Server-pushed invalidation | RESP3 client tracking handles this at protocol level |
| Tiered storage management | Handled transparently by modern cache engines |
...
## 2025 Hardware and Software Evolution

| Generation | Throughput/Node | Memory/Node | Key Advance |
|------------|----------------|-------------|-------------|
| Memcached (classic) | 50-100K QPS | 64 GB | Single-threaded, slab allocator |
| Redis 7 (2023) | 100-200K QPS | 64 GB | Multi-threaded I/O, RESP3 client tracking |
| Valkey 8 (2025) | 200-400K QPS | 128 GB | Community fork, multi-threaded commands, dual-channel replication |
| DragonflyDB (2025) | 1-4M QPS | 1 TB | Shared-nothing multi-threaded, single instance replaces cluster |
| Garnet (2025) | 500K-1M QPS | 512 GB | .NET thread-per-core, tiered storage (DRAM + NVMe) |

**Tiered Storage Economics (2025):**

| Tier | Capacity | Latency | Cost/GB/month | Use Case |
|------|----------|---------|---------------|----------|
| DRAM | 64-512 GB | < 0.1ms | $6-8 | Hot working set |
| CXL-attached memory | 1-4 TB | 0.3-0.5ms | $1.5-3 | Warm data, extended capacity |
| NVMe SSD (Garnet-style) | 4-16 TB | 0.5-2ms | $0.10-0.30 | Cold but cacheable data |

**Impact on Capacity Planning:** A single DragonflyDB instance with 512 GB DRAM can replace a 10-node Redis cluster, reducing operational complexity and inter-node network traffic. Tiered storage (Garnet) enables caching 10-50x more data at 2-5x latency, shifting the cost curve for large working sets.
