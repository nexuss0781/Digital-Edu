...
The LRU cache uses two complementary data structures to achieve O(1) Time Complexity (Speed of the algorithm) for all operations:
...
### LRU Cache Operations (Step-by-step plan in plain English)
...
W-TinyLFU splits the cache into a small admission window (1%) and a main space (99%), using a frequency sketch as an admission gate:

```
CLASS WTinyLFUCache:
    window: LRUCache(capacity * 0.01)         // 1% — admits all new items
    main_protected: LRUCache(capacity * 0.79)  // 79% — frequently accessed
    main_probation: LRUCache(capacity * 0.20)  // 20% — admitted but not yet proven
    sketch: CountMinSketch(width=1024, depth=4) // Frequency estimator
    sample_count: int = 0
    RESET_THRESHOLD: int = 10 * capacity        // Halve counters periodically

    FUNCTION get(key):
        sketch.increment(key)
        sample_count += 1
        IF sample_count >= RESET_THRESHOLD:
            sketch.halve_all_counters()
            sample_count = 0

        IF key IN main_protected:
            RETURN main_protected.get(key)   // Move to MRU position
        ELSE IF key IN main_probation:
            // Promote to protected on second access
            value = main_probation.remove(key)
            main_protected.put(key, value)
            RETURN value
    ...
    FUNCTION put(key, value):
        ...
        IF key IN main_protected OR key IN main_probation:
            // Update in place
            update_existing(key, value)
            RETURN

        // New item enters window
        evicted_from_window = window.put(key, value)

        IF evicted_from_window IS NOT null:
            // Admission decision: compare frequencies
            victim = main_probation.peek_lru()
            IF victim IS null OR sketch.estimate(evicted_from_window.key) > sketch.estimate(victim.key):
                // Admit window evictee to main probation
                main_probation.put(evicted_from_window.key, evicted_from_window.value)
            // Else: window evictee is discarded (not admitted)
```

**Why W-TinyLFU matters:**
- A database backup scan touching 1M keys only pollutes the 1% window, not the 99% main cache
- The Count-Min Sketch uses ~4 bytes per counter × 1024 counters × 4 hash functions = 16 KB total
- Periodic counter halving handles frequency drift (items that cool down)
- Hit rate improvement: 5-15% over pure LRU across web, database, and analytics workloads
- Used by Caffeine (JVM), the highest-performance in-process cache library

---

### S3-FIFO (First-In-First-Out, like a line at a store) Eviction Algorithm (2025)

S3-FIFO (First-In-First-Out, like a line at a store) uses three FIFO (First-In-First-Out, like a line at a store) queues with a frequency bit — no linked-list pointer manipulation, making it cache-line friendly and flash-storage optimal:

```
CLASS S3FIFOCache:
    small: FIFOQueue(capacity * 0.10)   // 10% — new items
    main: FIFOQueue(capacity * 0.90)    // 90% — promoted items
    ghost: GhostQueue(capacity * 0.90)  // Metadata only (evicted keys)

    FUNCTION get(key):
        IF key IN small:
            small.mark_accessed(key)  // Set frequency bit
            RETURN small.get(key)
        IF key IN main:
            main.mark_accessed(key)   // Set frequency bit
            RETURN main.get(key)
        RETURN NOT_FOUND

    FUNCTION put(key, value):
        IF key IN small OR key IN main:
            update_existing(key, value)
            RETURN

        // Evict from small queue if full
        WHILE small.is_full():
            evicted = small.dequeue()
            IF evicted.accessed:
                // Promoted: was accessed at least once in small
                evicted.accessed = false
                main.enqueue(evicted)
                evict_from_main_if_needed()
            ELSE:
                // Not accessed — add to ghost and discard
                ghost.add(evicted.key)

        // Insert into small (or main if ghost hit)
        IF key IN ghost:
            ghost.remove(key)
            main.enqueue(key, value)     // Ghost hit → direct to main
            evict_from_main_if_needed()
        ELSE:
            small.enqueue(key, value)

    FUNCTION evict_from_main_if_needed():
        WHILE main.is_full():
            evicted = main.dequeue()
            IF evicted.accessed:
                evicted.accessed = false
                main.enqueue(evicted)     // Re-insert (second chance)
            ELSE:
                DISCARD evicted           // True eviction
```

**S3-FIFO (First-In-First-Out, like a line at a store) advantages over LRU:**
- No per-access pointer manipulation — FIFO (First-In-First-Out, like a line at a store) queues are sequential, cache-line friendly
- Near-optimal hit rates (within 1-2% of Belady's optimal) across diverse workloads
- Flash-storage friendly: sequential writes, no random metadata updates
- Lower CPU overhead per operation than LRU or W-TinyLFU
...
### RESP3 Client-Side Caching Protocol

The RESP3 protocol (2025) introduces server-pushed invalidation, enabling client-side L1 caches that stay consistent without polling:

```
CLIENT-SIDE CACHING PROTOCOL (RESP3):

1. Client subscribes to tracking:
   CLIENT TRACKING ON REDIRECT <client-id> BCAST PREFIX product:

2. Client reads and caches locally:
   GET product:123 → "Widget, $29.99"
   [Client stores in local L1 cache]

3. Another client modifies the key:
   SET product:123 "Widget, $34.99"

4. Server pushes invalidation to all tracking clients:
   > INVALIDATE product:123

5. Client evicts from L1, next read goes to server:
   [L1 miss] → GET product:123 → "Widget, $34.99"

BENEFITS:
├── Zero polling overhead (server pushes invalidation)
├── Bounded staleness (invalidation within replication lag)
├── Automatic L1/L2 consistency
└── Reduced server load by 50-90% for read-heavy keys
```
