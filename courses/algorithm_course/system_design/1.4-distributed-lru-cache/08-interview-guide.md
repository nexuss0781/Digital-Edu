...
4. **Step-by-step plan in plain English to share:**
   ```
   FUNCTION get(key):
...
|--------|------|------|----------|
| LRU | Simple, effective | Not frequency-aware, scan-vulnerable | General purpose |
| LFU | Keeps frequently used | Slow to adapt | Stable patterns |
| W-TinyLFU | Best hit rate, scan-resistant | More complex | Production systems (used by Caffeine) |
| S3-FIFO (First-In-First-Out, like a line at a store) | Near-optimal, flash-friendly | Newer, less battle-tested | Tiered storage, sequential I/O |
| ARC | Adapts to patterns | Complex, patented | Variable workloads |

```
"I'd start with LRU for simplicity. For production at scale, W-TinyLFU
gives 5-15% better hit rates by using a frequency sketch as an admission
gate — it prevents scan operations from polluting the cache. If we're
using tiered storage with flash, S3-FIFO (First-In-First-Out, like a line at a store) is optimal because it uses
sequential FIFO (First-In-First-Out, like a line at a store) queues instead of random pointer updates."
```
...
### Trap 6: "DragonflyDB claims 25x Redis throughput. Why not just use that?"

**What they're testing:** Can you evaluate new technology critically?

**Strong Response:**
```
"DragonflyDB achieves 25x throughput by using a shared-nothing
multi-threaded architecture — each core has its own hash table and
event loop, eliminating the single-threaded Slowest part of the process.

But there are trade-offs to consider:

First, blast radius. A single DragonflyDB instance failure loses 100%
of cached data, versus 1/N for a clustered approach.

Second, multi-key operations like MGET that span multiple internal
shards require cross-shard coordination, which can be slower than
in a single-threaded engine.

Third, the ecosystem is younger — Valkey (the Redis community fork)
offers multi-threaded I/O with full Redis compatibility and a mature
ecosystem.

I'd use DragonflyDB for single-region deployments under 500 GB where
throughput per node matters. For multi-region or large working sets,
clustered Valkey with proven replication is safer."
```

### Trap 7: "How would you handle the Redis license change?"

**What they're testing:** Awareness of industry landscape

**Strong Response:**
```
"Redis switched to a dual license (RSALv2 + SSPLv1) in 2024, which
restricts managed service providers. The community responded with
Valkey under the Linux Foundation, which is a drop-in Redis fork
under the permissive BSD license.

For new deployments, I'd evaluate:
1. Valkey: Drop-in Redis replacement, active community, multi-threaded
   improvements, no licensing risk
2. DragonflyDB: If we need extreme single-node throughput
3. Garnet: If we need tiered storage (DRAM + NVMe)

The key point is that the RESP protocol is standardized, so the
client libraries work across all three — migration between engines
is a server-side operation, not an application change."
```
...
| "Use Redis for everything" | Not system design thinking | Discuss trade-offs, mention Valkey/DragonflyDB/Garnet |
...
| Treating cache as a database | Cache is ephemeral | "Data can always be reconstructed from source" |
| Ignoring the delete-set race | Subtle consistency bug | Mention versioned entries or ADD (set-if-not-exists) |
| Using LRU without discussing alternatives | Misses scan-resistance problem | Mention W-TinyLFU for production workloads |
...
| W-TinyLFU hit rate improvement | 5-15% over LRU | Scan-resistant admission policy |
| DragonflyDB single-node throughput | 1-4M QPS | 25x vs single-threaded Redis |
| Netflix EVCache scale | 22K servers, 14 PB | 400M ops/sec, Ketama hashing |
| Consistent hashing rebalancing | K/(N+1) keys move | ~3-10% on node add |
| XFetch stampede reduction | 1000 DB queries → 1 | Probabilistic early refresh |
| Tiered storage cost ratio | DRAM:NVMe = 60:1 | $6/GB vs $0.10/GB per month |
...
6. "Compare clustered vs single-node multi-threaded cache for a 500K QPS workload"
7. "Design a tiered caching system with DRAM and NVMe for a 5 TB working set"
8. "How would you migrate from Redis to Valkey with zero downtime?"
