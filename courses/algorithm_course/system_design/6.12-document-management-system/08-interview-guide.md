...
### Trap: "Why not just use a cloud file storage service with a database on top?"

**What they're testing**: Whether you understand the governance layer complexity.

**Good answer**:
> "Cloud file storage solves sync and sharing. A DMS solves governance. The check-in/check-out lock requires a distributed lock service with fencing tokens — that alone is a non-trivial distributed systems problem. Legal hold requires making documents immutable at the storage level, which means the deletion path must check hold status at every layer. Retention policies must override normal deletion but be overridden by legal hold. ACL inheritance across deep folder hierarchies needs a pre-computed permission cache. Full-text search across binary formats needs a content extraction pipeline. None of these can be 'bolted on' to a file storage service — they shape the fundamental architecture."

### Trap: "What happens when a user goes on vacation with a document checked out?"

**What they're testing**: Edge Case (Unusual or extreme situation) handling in distributed locking.

**Good answer**:
> "Three mechanisms handle this: (1) TTL-based expiry — locks default to 8 hours but are configurable per library. When the TTL expires, the lock is automatically released. (2) Admin break-lock — an admin can force-release any lock, which generates a notification to the original holder and an audit log entry. (3) Delegation — before going on leave, a user can delegate their locks to a colleague, which transfers the fencing token and preserves the edit chain. The fencing token is critical here: if user A's lock expires, user B acquires the lock and gets fencing token 43, then user A comes back and tries to check in with fencing token 42 — the system rejects the stale write."

### Trap: "How do you handle a legal hold that conflicts with GDPR right-to-erasure?"

**What they're testing**: Compliance nuance.

**Good answer**:
> "This is a real tension in enterprise DMS. Legal hold takes precedence — GDPR Article 17(3)(e) explicitly allows retention for legal claims defense. The system marks the document as 'held, deletion requested' and logs the GDPR request. When the legal hold is lifted, the system automatically re-evaluates the pending deletion request. The key is that we never silently ignore a GDPR request — we acknowledge it, document the legal basis for retention, and track it for eventual fulfillment."
...
|--------|-------|---------|
| **50ms** | Permission evaluation p99 | With pre-computed permission cache |
| **500ms** | Search query p99 | Full-text across 1B+ documents |
| **500ms** | Lock acquisition p99 | Distributed lock with fencing token |
| **5 min** | Search index freshness | NRT indexing from message queue |
| **10s** | Thumbnail generation target | Async via worker queue |
| **94%** | Delta compression savings | 50 versions of 5MB Office doc |
| **80/20** | Read/write ratio | Read-heavy workload pattern |
| **11 nines** | Document durability target | 99.999999999% — no content loss |
