...
```
Step-by-step plan in plain English: Office Format Delta Optimization
...
```
Step-by-step plan in plain English: Delta Chain Optimization
...
```
Step-by-step plan in plain English: Content-Defined Chunking for Deduplication
...
```
Step-by-step plan in plain English: Optimistic Mode Conflict Detection
...
```
Step-by-step plan in plain English: Chunked Upload for Large Documents
...
```
Step-by-step plan in plain English: Lazy Page Rendering for Large PDFs
...
```
Step-by-step plan in plain English: Multi-Layer Search Cache
...
```
Step-by-step plan in plain English: OCR Processing Pipeline
...
```
Step-by-step plan in plain English: Secure External Sharing
...
```
Step-by-step plan in plain English: Retention Sweeper
...
```
Step-by-step plan in plain English: Permission Evaluation Cache
...
```
Step-by-step plan in plain English: Batch Permission Filtering
...
### Case Study 1: SharePoint's Multi-Tenant Content Database Architecture

**Context**: SharePoint serves 200M+ monthly active users across 400K+ organizations. Each organization (tenant) has wildly different content volumes — from hundreds of documents to tens of millions.

**Architecture Choices:**
- Content databases are the unit of tenancy and scaling. Each content database can hold 1-many site collections (tenants)
- Large tenants (>100GB) get dedicated content databases
- Small tenants are packed into shared content databases (up to 300GB each)
- Metadata and content are stored in the same database (SQL Server) — a deliberate choice for transactional consistency at the cost of storage efficiency
- Search uses a federated index architecture: each content database has its own crawl, and the query broker merges results across indexes

**Key Lessons:**
- **Same-database content+metadata** simplifies transactions (version create + metadata update + lock release are atomic) but creates scaling pressure that requires careful database sizing
- **Federated search** allows tenants to be moved between databases without global re-indexing — the search federation layer adapts to the new database location
- **Site collection as isolation boundary**: permissions, storage quotas, and retention policies are scoped to site collections, providing per-tenant control within shared infrastructure

### Case Study 2: Box's Legal Hold and eDiscovery Architecture

**Context**: Box manages 150B+ content items and serves enterprise customers with heavy compliance requirements — financial institutions, healthcare providers, and government agencies.

**Architecture Choices:**
- Legal hold is implemented as a metadata flag on the content object that overrides all lifecycle operations (retention, deletion, versioning cleanup)
- The deletion path has a mandatory hold-check gate: before any destructive operation, the system queries the hold index. This is a synchronous check, not eventual
- eDiscovery uses a separate search index that includes held content regardless of access restrictions. Access to eDiscovery results requires a special privilege (Legal Hold Admin) that is separate from content access permissions
- Hold status is propagated across versions: placing a hold on a document holds all versions, not just the current one

**Key Lessons:**
- **Hold-check must be synchronous**: an eventually consistent hold check could allow deletion of a document between the hold being placed and the check propagating. This is legally indefensible
- **eDiscovery requires a separate permission model**: legal teams need to search content they would normally be unable to access. The eDiscovery permission grant is temporary, audited, and revocable
- **Version-level holds vs document-level holds**: document-level is simpler but holds unnecessary versions. Some implementations allow version-range holds (hold versions 3-7 only) for more targeted preservation
