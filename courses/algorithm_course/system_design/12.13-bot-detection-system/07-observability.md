...
|-----|-----------|-------------|
| **Availability** | Successful edge evaluations / total requests (excluding 4xx) | Edge proxy logs, aggregated per minute |
| **Edge scoring latency** | p99 time from request receipt to scoring decision | Server-side histogram at edge PoPs |
| **False positive rate** | Confirmed false positives / total human sessions | Appeal pipeline + session recovery analysis |
| **Bot detection rate** | Detected bots / total bot sessions (estimated via honeypots) | Honeypot telemetry + downstream fraud correlation |
| **Model freshness** | Hours since last successful model deployment to all edge nodes | Model registry timestamp vs current time |
| **Challenge accessibility** | CAPTCHA solve rate for accessibility-mode sessions | Challenge outcome logs filtered by accessibility flag |

### SLO Definitions

| SLO | Target | Error Budget (30-day) | Burn Rate Alert |
|-----|--------|-----------------------|-----------------|
| Availability | 99.99% | 4.32 min | >2x burn → page; >5x burn → incident |
| Edge scoring p99 | < 5ms | 0.01% of requests >5ms | >14.4x burn (1h window) |
| False positive rate | < 0.1% of human traffic | 0.1% threshold | >3x burn (6h window) |
| Bot detection rate | > 95% of known bots | 5% miss tolerance | >5x burn (12h window) |
| Model freshness | < 12 hours | 0 violations | Any model >12h → P2 |
| Challenge accessibility | > 99% human solve rate | 1% fail tolerance | Any endpoint <95% → P1 |

### Error Budget Policy

```
When error budget is exhausted (0% remaining):
  1. Freeze model deployments (use current known-good model)
  2. Widen challenge thresholds (prefer false negatives over false positives)
  3. Redirect engineering to reliability: FP root cause analysis, model calibration
  4. Resume normal velocity when budget is >25% remaining

When error budget is healthy (>50% remaining):
  1. Normal model deployment cadence (daily)
  2. Permitted to experiment with aggressive thresholds
  3. Can run red team exercises against production traffic
```

---

## Dashboard 4: Attack Campaign Tracker
...
│ ATK-2026-042 │ Credential   │ Active · 45K req/min · 92%    │
│              │ Stuffing     │ blocked                        │
│ ATK-2026-041 │ Price        │ Active · 12K req/min · 88%    │
│              │ Scraping     │ blocked                        │
│ ATK-2026-040 │ Inventory    │ Mitigated · 0 req/min          │
│              │ Hoarding     │ (campaign ended 4h ago)         │
├──────────────┴──────────────┴───────────────────────────────┤
│ Campaign Detail: ATK-2026-042                                │
│ ├─ Source: 2,400 residential IPs across 18 ASNs              │
│ ├─ Fingerprints: 850 unique (anti-detect browser cluster)    │
│ ├─ Behavioral signature: low mouse variance (σ=0.03)         │
│ ├─ TLS fingerprint: JA4 mismatch rate 78%                    │
│ ├─ Detection signals: behavioral clustering + TLS anomaly    │
│ └─ Action: PoW difficulty raised to 22 bits for /login       │
├─────────────────────────────────────────────────────────────┤
│ Historical Campaigns (last 30 days)                          │
│ Total: 23 campaigns detected                                 │
│ Avg detection time: 4.2 minutes                              │
│ Avg mitigation time: 12.8 minutes                            │
│ Worst miss rate: 18% (ATK-2026-035, residential proxy farm)  │
└─────────────────────────────────────────────────────────────┘
```
...
| Trace Type | Base Rate | Elevated Rate Trigger | Max Rate |
|-----------|----------|----------------------|----------|
| Blocked requests | 100% | Always captured | 100% |
| Challenged requests | 100% | Always captured | 100% |
| False positive appeals | 100% | Always captured | 100% |
| Allowed (high confidence, score <0.2) | 0.1% | Latency >2x p99 | 10% |
| Allowed (borderline, score 0.2-0.5) | 1% | Score distribution shift | 20% |
| Cloud deep model escalations | 10% | GPU queue depth >10K | 50% |
| Beacon processing | 0.01% | Feature worker lag >10s | 1% |
| Model deployment events | 100% | Always captured | 100% |
