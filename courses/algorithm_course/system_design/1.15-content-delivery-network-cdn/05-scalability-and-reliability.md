...
## Edge Compute Scaling (2025-2026)

### WebAssembly Workload Distribution

As CDNs evolve into distributed compute platforms, scaling considerations expand beyond bandwidth and cache capacity to include compute resource management at edge PoPs:

```
Edge Compute Resource Model:
┌─────────────────────────────────────────────────────────────┐
│ Per-PoP Compute Budget                                      │
├─────────────────────────────────────────────────────────────┤
│ CPU allocation:    70% cache serving, 30% edge compute      │
│ Memory allocation: 80% cache store, 20% Wasm modules        │
│ Isolation:         Per-tenant Wasm sandboxes (memory-safe)   │
│ Cold start:        < 0.5ms per Wasm module instantiation     │
│ Concurrency:       10,000+ concurrent Wasm instances/server  │
└─────────────────────────────────────────────────────────────┘

Scaling Triggers:
  Cache-only PoP     → compute-enabled PoP:    Edge function demand > threshold
  Compute capacity   → horizontal scale:       Wasm CPU utilization > 70%
  AI inference load  → dedicated inference PoP: ML model requests > threshold
```
...
|--------|------|------|----------------|
| Global PoPs (major providers) | ~900 | 1,200+ | 1,500+ |
| Edge function adoption | 25% | 37% | 50%+ |
| AI API traffic at edge | Minimal | Emerging | 15-20% of API traffic |
| Encrypted traffic | 90% | 95% | 97%+ |
| Avg. PoP bandwidth capacity | 10 Tbps | 15 Tbps | 20+ Tbps |
...
| **Edge Compute** | Wasm module replication across PoPs | Seconds (stateless) |
