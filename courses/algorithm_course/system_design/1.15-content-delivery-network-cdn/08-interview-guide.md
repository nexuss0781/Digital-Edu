...
### Q: "How are CDNs evolving beyond caching?"

**Strong Answer:**

CDNs are becoming distributed compute platforms. Three key evolutions:

1. **Edge Compute via WebAssembly**: PoPs now run application logic (auth validation, A/B testing, personalization) with sub-millisecond cold starts. The CDN's cache proximity becomes compute proximity -- every PoP is a potential application server.

2. **Semantic Caching for AI APIs**: Instead of exact URL matching, edge nodes use vector embeddings to find semantically similar cached responses. This achieves 40-70% hit rates on AI API traffic (vs 10-15% exact match), reducing inference costs by up to 73%.

3. **HTTP/3/QUIC Adoption**: Independent stream multiplexing eliminates head-of-line blocking. 0-RTT resumption removes a full round-trip on reconnection. The challenge for anycast CDNs is QUIC connection migration when BGP reroutes packets to a different PoP.

**What interviewers look for:** Understanding that CDNs are evolving from "cache + serve" to "cache + compute + serve" -- and the architectural trade-offs this creates (state management at edge, compute resource allocation per PoP, semantic vs exact cache matching).

### Q: "How do modern CDNs handle AI-era DDoS attacks?"

**Strong Answer:**

Traditional rule-based DDoS mitigation (rate limits, geo-blocks) fails against sophisticated application-layer attacks that mimic legitimate traffic. Modern CDNs deploy lightweight ML models at every PoP that classify traffic based on behavioral fingerprints -- request timing patterns, TLS fingerprints (JA3/JA4), session behavior sequences, and geographic distribution anomalies. The model distinguishes "flash crowd from viral content" from "botnet attack" by analyzing behavioral signatures rather than just volume. Key constraint: inference must complete in sub-millisecond timeframes within the request path, requiring compact models (gradient-boosted trees with < 100 features) rather than deep neural networks.
