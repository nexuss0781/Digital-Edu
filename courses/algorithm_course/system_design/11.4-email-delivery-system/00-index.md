...
|---|---|
| **Multi-Stage Queue** | Priority → domain → connection pipeline prevents head-of-line blocking across ISPs |
| **Circuit Breaker** | Per-ISP circuit breakers prevent cascading failures when an ISP starts rejecting |
| **Bulkhead Isolation** | Transactional and marketing paths have dedicated queues, MTA pools, and IP allocations |
| **Bloom Filter + Cache + Store** | Three-layer suppression lookup: probabilistic filter → cache → persistent store |
| **PID Controller** | Adaptive ISP throttling infers acceptable rates from SMTP response signal feedback |
| **Connection Pooling** | Per-destination-domain SMTP connection pools amortize handshake overhead |
| **Event Sourcing** | Message lifecycle (accepted → queued → delivered → opened) as append-only event log |
| **Exponential Backoff with Jitter** | SMTP retry, webhook delivery, and IP warming all use jittered exponential backoff |
| **Tiered Storage** | Hot → warm → cold → archive with automated lifecycle enforcement |
| **Noisy Neighbor Isolation** | Tiered IP pools with quality gates prevent bad senders from degrading shared reputation |
...
| System | Relationship |
|---|---|
| [1.1 Distributed Cache](../1.1-distributed-cache/00-index.md) | Suppression lookup uses distributed cache as middle tier between bloom filter and persistent store |
| [1.5 Distributed Search Engine](../1.5-distributed-search-engine/00-index.md) | Analytics query engine for engagement metrics and deliverability dashboards |
| [1.7 Rate Limiter](../1.7-rate-limiter/00-index.md) | Per-customer API rate limiting and per-ISP sending rate control |
| [4.3 Notification System](../4.3-notification-system/00-index.md) | Email as a notification channel; shared delivery pipeline patterns |
| [8.1 Payment Processing System](../8.1-payment-processing-system/00-index.md) | Similar dual-mode processing: real-time transactions + batch settlements |
| [11.1 WebSocket-Based Chat System](../11.1-websocket-based-chat-system/00-index.md) | Connection pooling and per-destination routing patterns |
| [15.3 Distributed Tracing System](../15.3-distributed-tracing-system/00-index.md) | End-to-end message tracing through the multi-stage delivery pipeline |
...
| Source | Focus |
|---|---|
| RFC 5321 (SMTP) | Core email transfer protocol specification |
| RFC 6376 (DKIM) | DomainKeys Identified Mail cryptographic signing standard |
| RFC 7489 (DMARC) | Domain-based Message Authentication, Reporting, and Conformance |
| RFC 8617 (ARC) | Authenticated Received Chain for forwarding authentication preservation |
| RFC 8058 | One-Click Unsubscribe for marketing emails |
| M³AAWG Best Practices | Messaging, Malware, and Mobile Anti-Abuse Working Group operational guidelines |
| Google Postmaster Tools docs | Gmail-specific deliverability signals and sender guidelines |
