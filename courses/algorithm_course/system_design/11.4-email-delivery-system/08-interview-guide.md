...
| | Cons: 5ms+ per lookup × 115K/sec = Slowest part of the process | Cons: Bloom filter has false positives (0.1%); requires multi-layer approach |
...
### 8.1 V1 → V2 → V3 Progression

| Aspect | V1 (MVP) | V2 (Growth) | V3 (World-Class) |
|---|---|---|---|
| **Ingestion** | REST API only, sync validation | REST API + SMTP relay, async validation | REST + SMTP + SDK + webhook-triggered, idempotent with dedup |
| **Queue** | Single priority queue | Priority + domain-partitioned queues | Three-stage: priority → domain → connection with per-ISP back-pressure |
| **MTA** | Single MTA process, 5 IPs | MTA fleet with connection pooling, 50 IPs | Distributed MTA fleet, 2000+ IPs, adaptive throttling, IP warming automation |
| **Authentication** | SPF + DKIM (RSA-2048) | + DMARC alignment validation | + ARC sealing + BIMI + MTA-STS + Ed25519 dual-signing |
| **Bounce handling** | Log SMTP response codes | Classify hard/soft, basic suppression | ML-enhanced classification, sub-categories, automated reputation scoring |
| **Tracking** | Open pixel + click redirect | + Unsubscribe tracking | + Bot detection (Apple MPP, scanners), human vs. machine classification |
| **IP management** | Shared pool, manual warming | Tiered pools (shared/dedicated) | Automated warming, health scoring, dynamic pool reassignment, quarantine |
| **Analytics** | Batch daily reports | Real-time dashboard with ISP breakdown | Predictive deliverability, engagement cohorts, send-time optimization |
| **Compliance** | CAN-SPAM basics | + GDPR right to erasure | + CCPA + CASL + one-click unsub (RFC 8058) + ISP mandate compliance |
| **Scale** | 1M emails/day | 100M emails/day | 3B+ emails/day with sub-second transactional delivery |

### 8.2 Architecture Skeleton (Whiteboard Code)

```
// Core pipeline — draw this first in any interview

FUNCTION email_pipeline(message):
    // Stage 1: Ingest + Validate
    validate_api_key(message.sender)
    check_rate_limit(message.sender)
    dedup_check(message.idempotency_key)

    // Stage 2: Suppress + Authenticate
    IF is_suppressed(message.recipient):       // Bloom filter → cache → DB
        RETURN SUPPRESSED
    rendered = render_template(message)         // MJML → HTML
    signed = dkim_sign(rendered, message.domain) // RSA-2048 + Ed25519

    // Stage 3: Queue + Route
    priority = classify_priority(message)       // transactional vs. marketing
    domain = extract_domain(message.recipient)
    enqueue(priority_queue, domain_queue, signed)

    // Stage 4: Deliver
    ip = select_sending_ip(domain, priority)    // reputation-aware pool selection
    throttle = get_isp_throttle(domain)         // adaptive rate control
    result = smtp_deliver(signed, ip, throttle)

    // Stage 5: Handle result
    SWITCH result:
        CASE DELIVERED:  emit_event("delivered"); update_ip_score(ip, +1)
        CASE DEFERRED:   requeue_with_backoff(message); update_throttle(domain, -1)
        CASE BOUNCED:    classify_bounce(result); update_suppression()
        CASE BLOCKED:    open_circuit_breaker(domain); alert_ops()
...
|---|---|---|---|---|
| **Fail mode for suppression** | Fail-open (send anyway) | Fail-closed (block sends) | **B** | Legal liability > temporary unavailability |
| **DKIM algorithm** | RSA-2048 only | Dual-sign RSA + Ed25519 | **B** | Ed25519 faster (15x), RSA for compatibility; dual-sign covers both |
| **IP strategy** | All shared | Tiered (shared/premium/dedicated) | **B** | Reputation isolation; bad actors contained; premium pricing enabled |
| **Queue architecture** | Single queue + priority | Three-stage (priority → domain → connection) | **B** | Per-ISP drain rates; no head-of-line blocking; independent throttling |
| **Open tracking** | Count all pixel loads | Bot-filtered "human opens" | **B** | Since 2025, raw opens are 30-50% bots; decisions on bad data = bad outcomes |
| **Bounce retry** | Fixed schedule (every 30m) | Adaptive per-ISP exponential backoff | **B** | ISPs monitor retry patterns; uniform retry triggers blocks |
| **Template rendering** | Client-side only | Server-side MJML + client passthrough | **Both** | Flexibility: MJML for design consistency; passthrough for power users |
| **Webhook batching** | One event per POST | Batched (up to 100/POST) | **B** | 100x fewer HTTP connections; customer endpoints handle batches better |
| **Connection pooling** | New connection per message | Persistent pools per destination domain | **B** | Saves 30-50ms per message in handshake overhead; essential at scale |
| **Suppression storage** | Relational DB | Bloom filter + cache + KV store | **B** | Sub-microsecond for 99.9% of lookups at 115K/sec; DB cannot compete |
...
|---|---|---|---|
| **Deliverability** | 90% inbox placement | 95% inbox with ISP monitoring | 98%+ with predictive placement, automated ISP-specific optimization |
| **Speed** | < 30s transactional | < 5s transactional P95 | < 1s transactional P50, sub-second for critical paths |
| **Scale** | 10M/day | 1B/day | 3B+/day with linear cost scaling |
| **Reputation** | Manual IP management | Tiered pools with quality gates | Automated warming, health scoring, dynamic reassignment, ML-based risk prediction |
| **Analytics** | Batch daily reports | Real-time dashboard | Predictive analytics, send-time optimization, engagement-based segmentation |
| **Compliance** | Basic CAN-SPAM | GDPR + CAN-SPAM + one-click unsub | Full regulatory coverage + proactive ISP mandate compliance + audit trail |
| **Bot detection** | None (raw pixel counts) | IP-based filtering | Multi-signal classification: IP + UA + timing + datacenter detection + Apple MPP handling |
| **Failure handling** | Basic retry | Circuit breakers + exponential backoff | Adaptive per-ISP throttling, predictive backoff, reputation-aware routing around degraded paths |

---

## 11. Whiteboard Walkthrough Script (45-Minute Pacing)

| Minute | Say | Draw |
|---|---|---|
| 0–2 | "Let me clarify scope: transactional + marketing? What scale?" | Nothing yet |
| 2–5 | "The core abstraction is message lifecycle: ingest → authenticate → queue → deliver → track" | Linear pipeline diagram |
| 5–8 | "The key insight: this isn't an SMTP server problem, it's a reputation management problem" | Add ISP feedback loop to diagram |
| 8–12 | "Queue architecture is the heart: three stages — priority, domain, connection" | Multi-stage queue diagram |
| 12–16 | "ISP throttling makes this unique — per-domain adaptive rate control" | Add throttle controller + circuit breaker |
| 16–20 | "Suppression is compliance-critical: bloom filter → cache → KV store, fail-closed" | Suppression check inline in pipeline |
| 20–25 | "IP reputation management: tiered pools, automated warming, quality gates" | IP pool diagram with health scores |
| 25–30 | "Tracking infrastructure: open pixel + click proxy + bot detection" | Add tracking servers + event pipeline |
| 30–35 | "Deep dive: adaptive ISP throttling — PID-like controller from SMTP signals" | Throttle algorithm Step-by-step plan in plain English |
| 35–40 | "Failure modes: circuit breakers per ISP, bulkhead for transactional vs. marketing" | Failure cascade diagram |
| 40–45 | "Scale: 3B emails/day, 115K/sec peak, 2000 IPs, 24 PB storage" | Annotate numbers on diagram |
