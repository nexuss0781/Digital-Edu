...
## Case Study: Embedding Version Mismatch During Rolling Update

### Scenario

During a model update, the embedding pipeline updated user embeddings to model v2 while item embeddings were still indexed at v1. For 23 minutes, recommendation quality degraded across 40% of traffic.
...
T0: Model v2 training completes, embedding pipeline triggers
T0-T10m: User embedding pipeline updates 500M user vectors to v2
T10m: User embedding store now serves v2 embeddings
T10m-T30m: Item embedding pipeline still processing 100M items (slower, larger corpus)

T10m-T33m (23-minute window):
  - Retrieval queries use v2 user embeddings against v1 item index
  - Dot product similarity scores are meaningless (different vector spaces)
  - Top candidates are essentially random subset, not most relevant
  - Ranking model partially compensates but receives low-quality candidates

T33m: Item embedding pipeline completes, ANN index rebuilt with v2
T33m+: Normal operation restored
```

### Root Cause

The embedding pipeline treated user and item embedding updates as independent jobs rather than an atomic operation. No version consistency check existed at serving time.

### Fix Applied

```
VERSIONED EMBEDDING PROTOCOL:

1. Both user and item embedding pipelines write to version-namespaced keys:
   user_emb:v2:{user_id} and item_emb:v2:{item_id}

2. ANN index built from item_emb:v2:* (new version namespace)

3. Active version pointer remains at v1 during update

4. Atomic version switch only after BOTH conditions met:
   a. Item ANN index built from v2 embeddings
   b. User embedding coverage at v2 >= 99.5%

5. Serving always reads user_emb:{active_version}:{user_id}
   and queries ANN index built from item_emb:{active_version}:*

6. Rollback: flip active version pointer back to v1 (instant)
```
...
|--------|-----------|-----------|
| Version mismatch window | 23 minutes | 0 (atomic switch) |
| Recommendation quality during update | -35% CTR | No degradation |
| Rollback time | 30+ minutes (full re-index) | < 1 second (pointer flip) |
| Storage cost | 1x | 2x during transition (two versions) |

---

## AI Release Ladder

Every AI-driven capability in the recommendation engine (ranking models, embedding models, exploration policies, diversity re-rankers) must progress through these stages before reaching full autonomy.

| Stage | Gate Criteria | Rollback Trigger | Typical Duration |
|-------|--------------|------------------|------------------|
| **1. Offline Evaluation** | AUC/NDCG on held-out dataset >= baseline; no regression on cold-start cohort | N/A -- not serving traffic | 1-2 weeks |
| **2. Shadow Mode** | Shadow ranking agrees with production ranking >= 85% on top-10 overlap; latency within budget | Shadow CTR prediction divergence > 20% from production for 4 hours | 2-4 weeks |
| **3. Canary (5% traffic)** | CTR delta < 1% vs. control; latency p99 within SLO; diversity metrics stable | CTR drop > 2% or latency SLO breach for 15 min | 1-2 weeks |
| **4. Human-Reviewed Production** | A/B test with statistical significance; ML team reviews quality metrics daily | Engagement metric regression > 3% with p-value < 0.05 | 1-4 weeks |
| **5. Autonomous Production** | Full model serving with async quality monitoring; business metrics stable | Any SLO breach, CTR drop > 5%, diversity collapse, or user complaint spike | Ongoing |
| **6. Instant Rollback** | Model version pointer flip to previous version; ANN index rollback via blue-green swap | Triggered manually or by automated quality monitor | < 1 second (pointer flip) |

> **Recommendation Engine specifics:** Stage 3 (Canary) requires careful attention to novelty effects -- new models often show initial CTR boosts that regress. Run canary for at least 7 days to account for day-of-week patterns.
