...
WHY NOT Brute Force (Checking every single possibility)?
...
- Recall: ~95% at 10x faster than Brute Force (Checking every single possibility)
...
1. FEATURE FETCH LATENCY
   ...
   - Feature importance Cutting off unnecessary steps (top 100 features only)
...
> "The biggest shift in 2025-2026 is using LLMs as an augmentation layer rather than a replacement. Key patterns:
>
> **1. Foundation Model Embeddings for Cold Start:** Using pre-trained language models to generate item embeddings from descriptions. This reduces cold start windows from hours to seconds — new items are immediately retrievable via content similarity without waiting for interaction data.
>
> **2. LLM as Contextual Feature Generator:** Instead of manual feature engineering, LLMs generate rich semantic tags from item content in batch. These become features for the ranking model.
>
> **3. Conversational Recommendation:** Users can refine recommendations through natural language ('show me something like X but more Y'). The LLM translates this into embedding space modifications.
>
> **Critical constraint:** LLMs are never on the critical serving path. At 1M QPS with 100ms budget, LLM inference is pre-computed and stored as features."

### Graph Neural Networks at Scale

> "GNN-based recommendations (PinSage-style) have matured significantly. The key innovation is mini-batch graph sampling — rather than processing the full user-item graph, sample local neighborhoods for each target node. This enables GNN training and inference on graphs with billions of edges. The resulting embeddings capture higher-order relationships (user→item→item→user paths) that two-tower models miss."
...
| **Spotify** | Discover Weekly | "How Discover Weekly Works" | Two-tower, audio features, LLM explanations |
...
| **LinkedIn** | Economic Graph | "Feed Ranking with Multi-Objective Optimization" | Multi-stakeholder optimization (member, creator, platform) |
