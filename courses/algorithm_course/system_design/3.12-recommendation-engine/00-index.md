...
|-----------|---------|
| **Tier** | **B -- AI-Augmented** |
| **What AI decides autonomously** | Candidate retrieval and ranking scores, real-time feature aggregation, embedding similarity computation, exploration/exploitation balance within configured bounds, diversity re-ranking, ANN index refresh scheduling |
| **What AI recommends (human approves)** | Model retraining schedule based on drift signals, A/B test traffic allocation changes, new feature candidates from automated discovery, cold start strategy adjustments, ranking objective weight tuning |
| **What requires human judgment** | Model promotion to production serving, ranking objective weight changes (engagement vs. diversity vs. freshness), major embedding model swaps, privacy-impacting feature additions, A/B test conclusions |
| **Escalation trigger** | CTR drop > 5% over 1 hour, embedding version mismatch detected, feature freshness SLA breach, model drift score exceeds threshold, diversity metric falls below configured floor |

> **Rationale:** Recommendation engines make millions of real-time serving decisions autonomously (ranking, retrieval), but changes to the models and objectives that drive those decisions carry business risk and require human approval gates.
...
- [3.2 ML Models Deployment System](../3.2-ml-models-deployment-system/00-index.md) - Model serving infrastructure, GPU fleet management
- [3.4 MLOps Platform](../3.4-mlops-platform/00-index.md) - Training pipeline, experiment tracking, model registry
- [3.14 Vector Database](../3.14-vector-database/00-index.md) - ANN search deep dive, HNSW indexing, sharded vector retrieval
- [3.16 Feature Store](../3.16-feature-store/00-index.md) - Online/offline feature serving, point-in-time correctness
- [1.4 Distributed LRU Cache](../1.4-distributed-lru-cache/00-index.md) - Feature and embedding caching patterns
- [2.7 Feature Flag Management](../2.7-feature-flag-management/00-index.md) - A/B testing infrastructure, experiment allocation
- [3.34 AI-Native Real-Time Personalization Engine](../3.34-ai-native-real-time-personalization-engine/00-index.md) - Real-time user context, session-aware personalization
- [3.15 RAG System](../3.15-rag-system/00-index.md) - Retrieval-augmented generation, vector similarity patterns

---

> **Vendor Freshness Notice:** This document references specific vendor products and technologies (Pinecone, Milvus, Weaviate, FAISS, Qdrant, Triton, TorchServe, TFServing, PyTorch, TensorFlow, JAX, Redis, DynamoDB, ScyllaDB). The recommendation systems tooling landscape evolves rapidly. Verify vendor capabilities, pricing, and API compatibility against official documentation before making architectural decisions. Last reviewed: 2026-03.
