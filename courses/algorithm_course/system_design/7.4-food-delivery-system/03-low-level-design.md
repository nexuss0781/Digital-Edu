...
The critical gap between "order persisted in PostgreSQL" and "event published to Kafka" requires the transactional outbox pattern:

```
FUNCTION createOrderWithOutbox(request):
    BEGIN TRANSACTION:
        // Step 1: Insert order record
        order = INSERT INTO orders (id, customer_id, ..., status)
                VALUES (gen_uuid(), request.customer_id, ..., 'PLACED')

        // Step 2: Insert event into outbox table (same transaction)
        INSERT INTO outbox (id, aggregate_type, aggregate_id, event_type, payload, created_at)
        VALUES (gen_uuid(), 'order', order.id, 'OrderPlaced', serialize(order), now())

    COMMIT TRANSACTION

    // Step 3: Background publisher polls outbox and publishes to Kafka
    // If Kafka publish succeeds → mark outbox row as published
    // If Kafka publish fails → retry on next poll cycle
    // Guaranteed: no event is lost, no event is published without the DB write
...
OUTBOX {
    uuid id PK
    string aggregate_type        -- "order", "driver", "payment"
    uuid aggregate_id            -- order_id, driver_id, etc.
    string event_type            -- "OrderPlaced", "DriverAssigned", etc.
    jsonb payload                -- serialized event data
    boolean published            -- false initially
    timestamp created_at
    timestamp published_at       -- set when Kafka publish confirmed
}
Index: (published, created_at) WHERE published = false  -- for polling
```

**Publisher loop (runs as a separate process):**

```
EVERY 100ms:
    events = SQL("SELECT * FROM outbox WHERE published = false ORDER BY created_at LIMIT 100")
    FOR EACH event IN events:
        success = kafka.publish(event.event_type, event.payload)
        IF success:
            SQL("UPDATE outbox SET published = true, published_at = now() WHERE id = :event.id")
    // Published events cleaned up by background job after 7 days
```

---

## 7. Restaurant Kitchen Load Modeling

Accurate dispatch timing depends on knowing the restaurant's current kitchen capacity:

```
FUNCTION estimateKitchenLoad(restaurant_id):
    // Count active orders at this restaurant
    active_orders = SQL("""
    ...
    // Get restaurant's throughput capacity (orders/hour)
    capacity = getRestaurantCapacity(restaurant_id)

    // Load factor: 0.0 = idle, 1.0 = at capacity, >1.0 = overloaded
    load_factor = active_orders / (capacity / 4)  // 15-min window

    // If overloaded, signal to throttle incoming orders
    IF load_factor > 1.2:
        triggerOrderThrottle(restaurant_id)

    RETURN {
        active_orders: active_orders,
        load_factor: load_factor,
        estimated_queue_delay_mins: max(0, (load_factor - 1.0) * 15),
        should_throttle: load_factor > 1.2
    }
```

This kitchen load estimate feeds into both the ETA model (higher load → longer prep time) and the dispatch timing decision (higher load → dispatch driver later).
