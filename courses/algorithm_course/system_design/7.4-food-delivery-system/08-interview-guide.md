...
| **15-28 min** | Deep Dive: Dispatch + Driver Matching | Geo-indexing, scoring algorithm, batching, acceptance flow, ETA coordination | Dispatch flow diagram, scoring Step-by-step plan in plain English, race condition handling |
...
| **Autonomous delivery integration** | Mixed-fleet dispatch: human drivers + robots + drones; constraint layer for weather, weight, regulations |
| **Ghost kitchen platform** | Virtual brand management, multi-brand kitchen routing, brand-to-kitchen mapping |
| **Sustainability optimization** | Carbon-aware routing, EV/bike courier incentives, packaging waste tracking |

---

## 7. Scaling Discussion Framework

When the interviewer asks "How does this scale?", use this structured framework:

### 7.1 Scale Axes

| Axis | Current | 10× | Approach |
|------|---------|------|----------|
| **Orders/day** | 5M | 50M | Add geo-shards; more Kafka partitions; PostgreSQL read replicas |
| **Cities** | 30+ | 300+ | Each city is an independent shard; geo-router handles city resolution; new cities are config-only deploys |
| **Concurrent drivers** | 500K | 5M | Shard Redis Geo by sub-zone within cities; partition location-updates topic by sub-zone |
| **Menu catalog** | 100M items | 1B items | Elasticsearch cluster scaling; tiered caching (hot restaurants in Redis, warm in ES, cold in PostgreSQL) |
| **Concurrent tracking** | 200K WebSocket | 2M WebSocket | Add WebSocket gateway instances; each handles 50K; Redis Pub/Sub scales with subscriber count |

### 7.2 Scaling Slowest part of the process Sequence

As the system scales 10×, the first Slowest part of the process hit (in order):

1. **Dispatch contention** (hit at 2-3×): More orders competing for same drivers → sub-zone partitioning within cities
2. **Redis Geo GEORADIUS CPU** (hit at 3-5×): Large sorted sets slow GEORADIUS → shard by sub-zone, not just city
3. **Kafka consumer lag** (hit at 5×): Location consumers fall behind → add partitions + consumers per topic
4. **PostgreSQL write throughput** (hit at 5-8×): Order writes saturate single shard → re-shard with more partitions
5. **WebSocket gateway count** (hit at 8-10×): Connection count exceeds fleet capacity → auto-scale based on connection count

---

## 8. Whiteboard Walkthrough: What to Draw

### 8.1 Architecture Diagram (5-8 minutes)

Draw these components in this order:

```
1. Three client types (customer, restaurant tablet, driver) at top
2. API Gateway + WebSocket Gateway in the middle
3. Core services: Order, Menu, Dispatch, Location, ETA, Payment, Notification
4. Data stores: PostgreSQL, Redis (emphasize Geo), Kafka, Elasticsearch
5. Arrows showing:
   - Order flow: Customer → Gateway → Order → Payment → Kafka → Dispatch
   - Location flow: Driver → WebSocket → Kafka → Location → Redis Geo
   - Tracking flow: Redis Geo → Pub/Sub → WebSocket → Customer
```

### 8.2 Deep Dive Diagram (8-10 minutes)

For the dispatch deep dive, draw:

```
1. Order event arrives from Kafka
2. GEORADIUS query to Redis → candidate drivers
3. Scoring function (distance, ETA, acceptance rate, rating)
4. Atomic assignment (Lua script: GET-CHECK-SET)
5. Offer to driver via push notification
6. Accept/decline flow with fallback to next candidate
7. Batch evaluation branch (can this be stacked?)
```

### 8.3 Data Model Sketch (3-5 minutes)

Only draw the core entities, not all fields:

```
Customer --(places)--> Order --(from)--> Restaurant
Order --(delivered by)--> Driver
Order --(paid via)--> Payment
Driver --(has many)--> LocationHistory
...
| Treating all restaurants equally in dispatch | Some restaurants prep in 8 min, others in 40 min; same dispatch timing wastes drivers or delays food | Per-restaurant dispatch timing calibrated on historical prep time distributions |
| No outbox pattern for events | Order persisted in DB but Kafka publish fails → lost events, stale downstream state | Transactional outbox: write event in same DB transaction as order; background publisher polls outbox |
| Polling-only for tracking | 200K customers polling every 10s = 20K req/sec wasted bandwidth when driver hasn't moved | WebSocket primary with HTTP polling fallback; push-based saves 80% of tracking bandwidth |

---

## 10. Interviewer Signals and Adaptation

| Interviewer Says | What They Want | How to Adapt |
|-----------------|----------------|-------------|
| "Tell me more about the dispatch" | They want algorithmic depth | Dive into scoring function, GEORADIUS, Lua-script atomicity, batching VRP |
| "How do you handle failures?" | They want resilience patterns | Discuss circuit breakers, saga pattern, graceful degradation tiers, chaos engineering |
| "What about scale?" | They want numbers and sharding | Quote peak QPS, explain geo-sharding, describe auto-scaling triggers |
| "Let's simplify—assume a single city" | They want to focus on core design, not distribution | Drop multi-region, geo-sharding; focus on dispatch, ETA, state machine |
| "What would you do differently if starting today?" | They want opinionated insights | Discuss RL-based dispatch over scoring, gRPC over REST for internal comms, event sourcing for orders |

---

## 11. Quick Reference: Key Numbers to Memorize

| Number | Context |
|--------|---------|
| **5M orders/day** | Total platform daily volume |
| **580 orders/sec** | Peak burst rate |
| **100K writes/sec** | Driver location update rate at peak |
| **500K concurrent** | Online drivers at peak |
| **700K WebSocket** | Total concurrent connections (drivers + tracking customers) |
| **<30s** | Driver assignment SLA |
| **<500ms** | Order placement API latency target |
| **±5 min** | ETA accuracy target (80% of orders) |
| **35 min** | Average order-to-delivery time |
| **14,000:1** | Menu read:write ratio → aggressive caching |
| **3-8 km** | Typical dispatch radius |
| **$0.04** | Infrastructure cost per order |

---

## 12. Comparison Cheat Sheet: Food Delivery vs. Similar Systems

Use this when the interviewer asks "How is this different from X?":

| Dimension | Food Delivery | Ride-Hailing | E-Commerce | Hotel Booking |
|-----------|--------------|-------------|-----------|--------------|
| **Marketplace sides** | 3 (customer + restaurant + driver) | 2 (rider + driver) | 2 (buyer + seller) | 2 (guest + hotel) |
| **Real-time matching** | Yes (driver to order, timed to prep) | Yes (driver to rider, immediate) | No (inventory check) | No (availability check) |
| **Physical degradation** | Yes (food quality) | No | No | No |
| **ETA complexity** | 3-stage (prep + pickup travel + delivery) | 2-stage (pickup + trip) | Shipping estimate (hours/days) | N/A |
| **Location tracking** | During delivery only | Entire trip | Package tracking (sparse) | N/A |
| **Batching** | Yes (stacked orders) | Yes (shared rides) | Yes (order consolidation) | N/A |
| **Surge pricing** | Per zone, smoothed | Per cell, real-time | Rare (seasonal sales) | Revenue management |
| **Primary geo index** | Redis Geo (GEORADIUS) | H3 hexagonal grid | None (warehouse-based) | None |

---

## 13. Senior-Level Discussion Points

These points differentiate a senior candidate:

1. **Why not microservices for everything?** The dispatch engine benefits from being a single, in-memory optimizer that can consider all pending orders simultaneously. Breaking it into smaller services would require cross-service coordination that adds latency to a time-critical path.

2. **When to break the geo-sharding model**: If a customer near a city boundary orders from a restaurant in the adjacent city, the order must route to the restaurant's shard. The geo-router uses restaurant location (not customer location) as the shard key.

3. **The ETA-trust paradox**: Showing a pessimistic ETA (40 min) that arrives in 30 min creates delight. Showing an optimistic ETA (25 min) that arrives in 30 min creates frustration. The ETA model should be calibrated slightly pessimistic (5-10% padding) to maximize customer satisfaction, even at the cost of lower initial conversion.

4. **Why idempotency matters more in food delivery than most systems**: Every state transition can be retried (network issues, app crashes). A non-idempotent "mark as delivered" could trigger double payment capture. Every API endpoint must handle duplicate calls gracefully.

5. **Cost-per-order as an architectural constraint**: At $0.04 infra cost per order and ~$2-3 revenue per order, infrastructure is ~1.5% of revenue. This means expensive ML models (ETA, fraud) are justified only if they improve conversion or reduce fraud by measurable amounts.
