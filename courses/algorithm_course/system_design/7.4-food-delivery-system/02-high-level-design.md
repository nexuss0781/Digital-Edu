...
## 6. Mixed-Fleet Dispatch Architecture (2025+)

As platforms integrate autonomous delivery (robots, drones) alongside human drivers, the dispatch architecture must evolve:

```mermaid
%%{init: {'theme': 'neutral', 'look': 'neo'}}%%
flowchart TB
    subgraph Order["Order Intake"]
        OE["Order Event"]
        FE["Fleet Eligibility<br/>Classifier"]
    end

    subgraph FleetDispatch["Multi-Modal Dispatch Engine"]
        HR["Human Driver<br/>Dispatch"]
        RD["Robot Dispatch<br/>(sidewalk bots)"]
        DD["Drone Dispatch<br/>(aerial)"]
        FM["Fleet Mode<br/>Selector"]
    end

    subgraph Constraints["Constraint Layer"]
        WX["Weather<br/>Service"]
        GF["Geofence &<br/>Regulation DB"]
        WL["Weight/Size<br/>Limits"]
    end

    subgraph Execution["Execution"]
        DA["Driver App"]
        RC["Robot Control<br/>System"]
        DC["Drone Flight<br/>Controller"]
    end
    ...
    classDef constraint fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef exec fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class OE,FE order
    class HR,RD,DD,FM dispatch
    class WX,GF,WL constraint
    class DA,RC,DC exec
```

### 6.1 Fleet Mode Selection Logic

```
FUNCTION selectFleetMode(order):
    // Check basic eligibility
    IF order.weight_kg > ROBOT_WEIGHT_LIMIT AND order.weight_kg > DRONE_WEIGHT_LIMIT:
        RETURN "human_only"

    eligible_modes = ["human"]

    // Robot eligibility
    distance = haversine(order.restaurant, order.delivery_address)
    IF distance <= ROBOT_MAX_RANGE_KM
       AND order.weight_kg <= ROBOT_WEIGHT_LIMIT
       AND isRobotZone(order.delivery_address)
       AND weather.precipitation < ROBOT_RAIN_THRESHOLD:
        eligible_modes.append("robot")

    // Drone eligibility
    IF distance <= DRONE_MAX_RANGE_KM
       AND order.weight_kg <= DRONE_WEIGHT_LIMIT
       AND isDroneZone(order.delivery_address)
       AND weather.wind_speed < DRONE_WIND_LIMIT
       AND hasLandingZone(order.delivery_address):
        eligible_modes.append("drone")

    // Score each mode by cost and ETA
    best_mode = argmin(eligible_modes, mode =>
        WEIGHT_COST * estimateCost(mode, order) +
        WEIGHT_ETA * estimateETA(mode, order)
    )

    RETURN best_mode
```

### 6.2 Ghost Kitchen / Virtual Brand Architecture

Ghost kitchens introduce a many-to-one mapping between restaurant brands and physical kitchens:

| Concept | Traditional Model | Ghost Kitchen Model |
|---------|------------------|---------------------|
| **Brand → Kitchen** | 1:1 (one brand, one location) | N:1 (5-10 virtual brands share one kitchen) |
| **Menu Ownership** | Restaurant manages directly | Platform or brand operator manages; kitchen just prepares |
| **Prep Time Model** | Per-restaurant historical data | Per-kitchen × per-brand (same kitchen, different prep times per cuisine) |
| **Dispatch Pickup** | Restaurant address = brand address | Kitchen address ≠ brand display address; driver navigates to kitchen |
| **Order Routing** | Direct to restaurant tablet | Aggregated order screen showing all brands; kitchen prioritizes across brands |

**Data model impact**: The Menu Service must support `virtual_brand_id → physical_kitchen_id` mapping. The ETA Service must model prep time per kitchen-brand pair, not per brand alone. The dispatch engine uses the physical kitchen location for driver routing, not the virtual brand's display address.

---

## 7. Technology Comparison: Key Architectural Choices

| Component | DoorDash Approach | Uber Eats Approach | Trade-off |
|-----------|-------------------|-------------------|-----------|
| **Dispatch** | DeepRed (mixed-integer optimization) | Market-clearing auction-based matching | Optimization is more globally optimal but computationally heavier; auction is faster but locally greedy |
| **ETA** | Multi-model pipeline (GBT for prep, transformer for travel) | DeepETA (single deep learning model, end-to-end) | Multi-model is more interpretable and allows independent tuning; single model may capture cross-stage correlations better |
| **Location Index** | Redis Geo (city-sharded) | H3 hexagonal grid + custom geo index | Redis Geo is simpler operationally; H3 provides uniform spatial granularity for analytics |
| **Event Backbone** | Apache Kafka | Custom event bus on top of gRPC | Kafka provides durable replay; custom bus has lower latency but less durability |
| **Search** | Elasticsearch with ML ranking | Custom search engine with transformer-based ranking | ES is faster to build; custom engine allows deeper ML integration |
| **Surge Pricing** | Zone-based with EWMA smoothing | H3-cell-level with ML demand forecasting | Zone-based is simpler; cell-level provides sub-neighborhood granularity |

---

## 8. Data Pipeline: Analytics & ML Training

Beyond the real-time operational flow, a parallel data pipeline feeds analytics and ML model training:

```
Real-time events (Kafka) → Stream processor → Aggregated metrics (1-min windows)
                         → Raw event archive (object storage, Parquet format)
                         → Feature store (Redis for online, columnar store for offline)

Daily batch jobs:
  1. Export order + delivery data → ML training dataset
  2. Compute per-restaurant prep time distributions → update baseline models
  3. Compute per-zone supply/demand patterns → update predictive auto-scaling
  4. Run ETA accuracy analysis → generate model retraining triggers
  5. Run financial reconciliation → flag discrepancies
```

| Pipeline | Input | Output | Frequency | Latency Requirement |
|----------|-------|--------|-----------|-------------------|
| **ETA Training** | Historical orders + deliveries + traffic | Updated ETA models (prep, travel, correction) | Daily | Batch (hours) |
| **Demand Forecast** | Historical orders + events + weather | Per-zone demand predictions (next 2h) | Every 15 min | Near-real-time |
| **Fraud Model** | Order features + outcomes | Updated fraud scoring model | Weekly | Batch |
| **Search Ranking** | Click-through, conversion, ratings | Restaurant ranking model | Daily | Batch |
| **Feature Store** | Real-time events | Online features for ML inference | Continuous | <100ms freshness |
