...
## Insight 16: Transactional Outbox for Reliable Event Publishing

**Category:** Resilience

**One-liner:** Writing order events to a database outbox table within the same transaction as the order itself, then publishing asynchronously from a background poller, eliminates the dual-write problem where an order is persisted but its Kafka event is lost.

**Why it matters:** The most dangerous data inconsistency in an event-driven food delivery system is a "silent order"---an order that exists in PostgreSQL but was never published to Kafka. Without the Kafka event, the dispatch service never learns about the order, the restaurant never receives a notification, and the customer sees "Order Confirmed" but nothing happens. Direct Kafka publishing after the DB commit introduces a window where the process can crash between the commit and the publish. The transactional outbox eliminates this window by making the event part of the same ACID transaction as the order. A background publisher polls the outbox table for unpublished events and publishes them to Kafka with at-least-once semantics. If the publisher crashes mid-publish, the next poll picks up the same event (it is still marked unpublished). If Kafka acknowledges and the publisher crashes before marking it published, the next poll re-publishes (downstream consumers must be idempotent). The key trade-off is increased latency: the event reaches Kafka 100-200ms after the DB commit (polling interval) rather than immediately. For food delivery, this latency is invisible---the restaurant notification arriving 200ms later has zero impact. An alternative approach is Change Data Capture (CDC), where a tool like Debezium tails the PostgreSQL WAL and publishes changes to Kafka. CDC avoids the polling overhead but adds operational complexity (WAL configuration, CDC connector maintenance). The outbox pattern is simpler and sufficient for food delivery's event throughput (~600 events/sec at peak).

---

## Insight 17: Mixed-Fleet Dispatch as a Constraint Satisfaction Problem

**Category:** Emerging

**One-liner:** Integrating autonomous delivery vehicles (robots, drones) alongside human drivers transforms dispatch from a single-mode matching problem into a constraint satisfaction problem where each mode has different speed, range, weight, weather, and regulatory constraints that the optimizer must evaluate per-order.

**Why it matters:** A human driver can carry a 20 kg grocery order in rain across a 15 km distance. A sidewalk robot cannot: it moves at 6 kph, carries up to 10 kg, and must avoid operation during heavy rain (sensor degradation). A drone can carry 2 kg over 10 km in 10 minutes but cannot operate in wind over 40 kph or in restricted airspace. When the dispatch engine has access to all three modes, it must solve a richer optimization: for each order, determine which modes are eligible (constraint filtering), estimate cost and ETA for each eligible mode, and assign the globally optimal mode considering the fleet's current utilization. This is more complex than single-mode dispatch because the modes are not interchangeable---rejecting a robot assignment does not free up a human driver, and vice versa. The production solution is a two-phase approach: first, a lightweight constraint classifier determines eligible modes (sub-millisecond, rule-based); second, the existing dispatch optimizer runs independently per mode, and the fleet mode selector picks the best result. This additive architecture means introducing a new delivery mode (e.g., cargo bikes) requires only a new constraint ruleset and optimizer instance, with zero changes to existing dispatch logic.
