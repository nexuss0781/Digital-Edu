...
| F2 | **Ledger Posting** | Record every financial event as immutable double-entry ledger entries (debit + credit); enforce the Rule that never changes that total debits equal total credits; support multi-leg journal entries for complex transactions |
...
1. **Ledger writes are the dominant Slowest part of the process**: At 139,000 peak writes/sec across all ledger entries, the ledger database cluster is the most write-intensive component. Sharding by account ID with append-only entries (never update, only insert) is essential. Each shard should handle < 20,000 writes/sec to maintain sub-100ms latency.
...
End-to-end latency for the critical posting path:

| Step | Budget | Component | Notes |
|------|--------|-----------|-------|
| **Client → API Gateway** | 10ms | Network (TLS termination) | Internal network; external adds 30-50ms |
| **Authentication + authorization** | 5ms | Auth service | JWT validation + RBAC check |
| **Rate limiting** | 2ms | Gateway | Token bucket check |
| **Idempotency check** | 3ms | Distributed cache | Cache hit for duplicate detection |
| **Journal validation** | 5ms | Posting service | Double-entry Rule that never changes check; account status |
| **Balance check + lock** | 30ms | Primary shard DB | SELECT FOR UPDATE on account row |
| **Ledger entry insert** | 20ms | Primary shard DB | Insert debit + credit entries |
| **Balance update** | 10ms | Primary shard DB | Update materialized balance (same transaction) |
| **Transaction commit** | 40ms | Primary shard DB | Synchronous commit with DR replication |
| **Cache invalidation** | 5ms | Distributed cache | Async but within response path |
| **Event emission** | 5ms | Event store | Async; fire-and-forget from client perspective |
| **Response serialization** | 3ms | Posting service | Posting reference + balance_after |
| **Gateway → Client** | 10ms | Network | Return path |
| **Total budget** | **~148ms** | | Well within 500ms SLO; 350ms headroom |

The dominant latency contributor is the database transaction (balance check + insert + commit with sync replication = ~100ms). For cross-shard sagas, add ~100-200ms for the second shard's debit/credit cycle.
...
|----------------|---------------|-----------------|-----------|
| **Accounts +50%** | Total accounts > 150M | Add shards (16 → 24); rebalance account distribution | 2-4 weeks |
| **Peak TPS doubles** | Sustained peak > 80K TPS | Double posting service instances; add shard replicas | 1-2 days (auto-scale handles most) |
| **New banking entity** | Business decision | Provision entity schema, encryption keys, CoA, regulatory config | 4-6 weeks |
| **New currency** | Business decision + regulatory approval | Add currency to FX engine; create nostro accounts; configure rate feeds | 2-3 weeks |
| **Regulatory change** | Regulation effective date | Update reporting logic, data retention, API changes | 3-6 months |
| **10× growth** | Total transactions > 5B/day | Sub-city sharding; dedicated posting clusters per entity; streaming interest accrual | 6-12 months |
| **Month-end spike** | Historical peak patterns | Pre-scale 2× compute 24h before month-end; extend batch window by 1h | Automated |

---

## Cost Estimation Model

| Cost Component | Formula | Monthly Estimate (at 500M txn/day) |
|---------------|---------|-------------------------------------|
| **Compute (posting + services)** | ~150 pods × $0.05/pod-hour × 730h | ~$548K |
| **Primary DB cluster (16 shards)** | 16 × 2 replicas × $0.20/hour × 730h | ~$467K |
| **DR DB cluster** | 16 shards × $0.15/hour × 730h | ~$175K |
| **Distributed cache** | 30 nodes × $0.08/node-hour × 730h | ~$175K |
| **Event store** | 20 brokers × $0.10/broker-hour × 730h | ~$146K |
| **Object storage (archival)** | ~50 TB/month × $0.023/GB | ~$1.2K |
| **Batch compute (EOD)** | 50 workers × 6h/day × 30 × $0.10/hour | ~$9K |
| **HSM (key management)** | Dedicated HSM cluster | ~$50K |
| **Regulatory reporting infra** | Analytical store + compute | ~$100K |
| **Total estimated infrastructure** | | **~$1.67M/month** |

*Note: Financial infrastructure costs are dominated by redundancy requirements (sync replication, HSM, DR). A system without RPO=0 guarantees would cost roughly 40% less.*

---

## Geographic Distribution of Load

Transaction volume follows business hours and payment cycles:

| Time Window | % of Daily Transactions | Peak Driver | Notes |
|-------------|------------------------|-------------|-------|
| **08:00-10:00** | ~20% | Business opening: standing orders, direct debits trigger | Highest batch volume |
| **10:00-14:00** | ~30% | Retail payments, card transactions, lunch-hour mobile payments | Sustained high TPS |
| **14:00-17:00** | ~20% | Afternoon business activity, interbank settlements | Settlement deadlines |
| **17:00-00:00** | ~15% | Evening retail, international payments from other time zones | Declining volume |
| **00:00-06:00** | ~10% | EOD batch processing (interest, fees, reconciliation) + international | Batch competes with real-time |
| **Month-end (last 3 days)** | +50% spike | Salary runs, rent payments, subscription renewals, fee postings | Pre-scale required |
| **Quarter-end** | +80% spike | All of month-end + regulatory reporting, provisioning, capital calcs | Extended batch window |
