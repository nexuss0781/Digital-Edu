...
| **Consistency Model** | Strong consistency for ledger posting (double-entry Rule that never changes must hold at all times); eventual consistency for reporting views and analytics |
...
|---------|---------------------------|-----------------|
| **CQRS** | Separate posting (write) from balance queries (read) with different consistency guarantees | 6.8 Real-Time Editor, 8.3 Trading Platform |
| **Event Sourcing** | Immutable ledger entries as events; materialized balances as projections | 1.5 Distributed Log Broker, 8.4 Digital Wallet |
| **Saga Pattern** | Cross-shard transfers with compensating transactions | 7.4 Food Delivery, 7.6 Flight Booking |
| **Sub-Account Sharding** | Split hot accounts into N independently-lockable sub-accounts | 8.4 Digital Wallet (hot wallets) |
| **Strangler Fig** | Incremental migration from legacy mainframe to new core | 6.3 Multi-Tenant SaaS (legacy migration) |
| **Crypto-Shredding** | Per-customer encryption keys destroyed for GDPR erasure | 10.1 Healthcare (patient data erasure) |

## ADR Summary

| # | Decision | Choice | Rationale |
|---|----------|--------|-----------|
| ADR-1 | Ledger model | Immutable append-only | Financial records must never be modified; corrections via reversing entries |
| ADR-2 | Cross-shard coordination | Saga over 2PC | 2PC holds distributed locks; saga provides recovery without blocking |
| ADR-3 | Balance storage | Materialized (write-through cache) | Summing ledger entries per query is O(n); materialized balance is O(1) |
| ADR-4 | Interest accrual | Batch processing | Real-time per-transaction accrual creates excessive entries; batch is industry standard |
| ADR-5 | Multi-tenant isolation | Shared infra, separate keys + schema | Cost-efficient while maintaining regulatory data segregation |
| ADR-6 | DR replication | Synchronous for ledger | RPO=0 is non-negotiable for financial records |
| ADR-7 | Product configuration | Declarative catalog (DSL) | Enables product launches without code deployment |
| ADR-8 | Audit integrity | Cryptographic hash chain | Tamper-evident without blockchain overhead |
...
| [1.5 - Distributed Log-Based Broker](../1.5-distributed-log-based-broker/) | Event streaming for ledger events, audit trail |
| [8.5 - Fraud Detection](../8.5-fraud-detection/) | AML monitoring, transaction scoring |
| [7.6 - Flight Booking](../7.6-flight-booking-system/) | Saga pattern for multi-step booking, compensating transactions |
| [6.3 - Multi-Tenant SaaS](../6.3-multi-tenant-saas-platform-architecture/) | Tenant isolation, data segregation, shared infrastructure |
...
- 10x Banking --- Cloud-Native Core Banking Architecture Whitepaper
- Mambu --- Composable Banking: SaaS Core Banking Design Principles
- Bank of England --- The Future of Finance: Core Banking Infrastructure
