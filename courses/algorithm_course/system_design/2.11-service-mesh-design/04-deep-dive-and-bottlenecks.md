...
### Slowest part of the process #1: Sidecar Resource Overhead
...
### Slowest part of the process #2: Control Plane Scaling
...
### Slowest part of the process #3: mTLS Handshake Overhead
...
## Critical Component #4: eBPF Data Plane

### Why Critical?

eBPF-based service meshes bypass userspace entirely for L3/L4 processing, achieving near-zero overhead but with different trade-offs than sidecar models.
...
│  │  │              eBPF PROGRAMS                          ││ │
│  │  │  ┌───────────┐  ┌────────────┐  ┌──────────────┐  ││ │
│  │  │  │ Socket    │  │  Traffic   │  │   Identity    │  ││ │
│  │  │  │ Redirect  │  │  Encrypt   │  │   Injection   │  ││ │
│  │  │  │ (O(1)     │  │  (WireGuard│  │   (SPIFFE)    │  ││ │
│  │  │  │  routing) │  │   or mTLS) │  │               │  ││ │
│  │  │  └───────────┘  └────────────┘  └──────────────┘  ││ │
│  │  └─────────────────────────────────────────────────────┘│ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                              │
│  Key Advantage: iptables uses O(n) rule chains              │
│                 eBPF maps use O(1) hash lookups              │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### eBPF vs Sidecar Trade-offs

| Aspect | eBPF (Cilium) | Sidecar (Envoy) | Ambient (Ztunnel) |
|--------|---------------|-----------------|-------------------|
| **L3/L4 Processing** | Kernel-level, O(1) | Userspace, iptables | Userspace (Rust), tproxy |
| **L7 Processing** | Envoy (optional) | Full Envoy | Waypoint (optional) |
| **Connection Overhead** | None (socket redirect) | iptables + proxy | tproxy + ztunnel |
| **mTLS Implementation** | WireGuard or IPsec | In-proxy TLS | In-ztunnel TLS |
| **Observability** | Hubble (eBPF-based) | Envoy metrics/traces | Ztunnel + waypoint |
| **Kernel Requirement** | 5.4+ (5.10+ recommended) | Any | Any |

---

## Critical Component #5: WebAssembly (Wasm) Extensions

### Why Critical?

Wasm enables safe, sandboxed custom logic at the proxy layer without rebuilding the proxy binary. This is the primary extensibility mechanism for Envoy-based meshes.

```
WASM EXTENSION LIFECYCLE:
═══════════════════════════════════════════════════════════════

Compile ──► Distribute ──► Load ──► Execute ──► Monitor

  Rust/Go/    OCI Registry   Envoy     Sandboxed    Metrics
  C/C++/      or ConfigMap   hot       per-request  and
  AssemblyScript              reload    execution    logging

Filter Chain Integration:
─────────────────────────────────────────────────────────────
  Envoy HTTP Filters:
  [...] → JWT Authn → RBAC → [WASM Custom] → Router → [...]
                              ▲
                              │
                        Custom logic:
                        • Header manipulation
...
Performance:
─────────────────────────────────────────────────────────────
• ~10-20% slower than native C++ filters
• Memory-isolated (cannot crash the proxy)
• Hot-reloadable without proxy restart
• Multiple Wasm VMs per proxy (one per filter)
```
...
| **Wasm filter count** | 0 | Minimize | Avoid unless needed |
| **eBPF socket redirect** | Off (sidecar) | Enable (Cilium) | Enable (Cilium) |

### Benchmark Reference (2025 Data)

```
COMPARATIVE LATENCY (p99 at 2000 RPS):
═══════════════════════════════════════════════════════════════

Implementation         p99 Latency      Memory/Pod    Notes
─────────────────────────────────────────────────────────────
No mesh (baseline)     ~15ms            0 MB          Reference
Linkerd                ~28ms            10 MB         Lowest overhead
Istio Ambient          ~39ms            5 MB/node     Shared ztunnel
Istio Sidecar          ~178ms           60 MB         Full L7 features
Cilium (eBPF)          ~20ms            1 MB/node     Kernel-level

Note: Numbers vary significantly by workload, cluster size,
and configuration. Always benchmark in your own environment.
```
