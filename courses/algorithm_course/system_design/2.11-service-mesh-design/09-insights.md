...
## Insight 11: Ambient Mode's Two-Tier Architecture Separates L4 Concerns from L7 On-Demand
**Category:** Architecture
**One-liner:** Istio ambient mode splits the data plane into an always-on per-node L4 proxy (ztunnel) and optional per-service L7 waypoint proxies, making L7 processing a pay-for-what-you-use cost.
**Why it matters:** The sidecar model forces every pod to carry the full weight of L7 proxy processing even when only L4 features (mTLS, basic routing) are needed. Ambient mode recognizes that the majority of mesh traffic needs only identity and encryption (L4), while header-based routing, retries, and authorization policies (L7) apply to a subset of services. By deploying ztunnel as a DaemonSet for universal L4 and adding waypoint proxies only where L7 is required, the architecture achieves ~90% memory reduction for large deployments. This two-tier pattern — always-on baseline with on-demand enrichment — is broadly applicable to any infrastructure system where the common case is simpler than the worst case.

---

## Insight 12: Gateway API as the Convergence Point for North-South and East-West Traffic Configuration
**Category:** Extensibility
**One-liner:** Gateway API unifies ingress (north-south) and mesh (east-west) traffic configuration under a single role-oriented standard, replacing both Ingress resources and mesh-specific CRDs.
**Why it matters:** Historically, configuring north-south traffic (Ingress, LoadBalancer) and east-west traffic (VirtualService, DestinationRule) required completely separate APIs, often from different vendors. Gateway API introduces a role-oriented model where infrastructure admins manage GatewayClass and Gateway resources while application developers manage HTTPRoute and GRPCRoute. The same HTTPRoute can configure both an ingress gateway and a mesh waypoint proxy. This eliminates the cognitive overhead of learning two configuration systems and enables portable traffic policies across mesh implementations. Istio ambient mode and Cilium both use Gateway API natively, signaling that mesh-specific CRDs are being phased out in favor of this standard.
