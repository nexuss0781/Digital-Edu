...
### Trap 5: "Why not just use eBPF instead of a service mesh?"

```
What They're Testing: Understanding of L4 vs L7 trade-offs

Poor Answer: "eBPF is better because it's faster."

Better Answer:
"eBPF and traditional service mesh address different layers:

eBPF excels at:
• L3/L4 networking (O(1) hash-based routing vs iptables O(n))
• Network policy enforcement in kernel
• Connection-level visibility without proxies
• Identity-based encryption (WireGuard)

But eBPF alone cannot:
• Do header-based routing (HTTP headers require L7 parsing)
• Implement retries with semantic awareness
• Apply per-request authorization policies
• Generate HTTP-level metrics and traces

This is why Cilium still uses Envoy for L7 features and
why Istio ambient mode separates L4 (ztunnel) from L7
(waypoint). The real answer is layered: eBPF for L3/L4,
userspace proxy for L7 — but only where L7 is needed.

For a workload that only needs mTLS and network policy,
eBPF alone may be sufficient. For HTTP routing, retries,
and authorization, you need a proxy layer."
```

### Trap 6: "What's the difference between Gateway API and Istio CRDs?"

```
What They're Testing: Understanding of the configuration landscape

Better Answer:
"Gateway API is the Kubernetes-standard replacement for both
Ingress resources and mesh-specific CRDs. Key differences:

Gateway API:
• Role-oriented: infra admin → GatewayClass/Gateway, app dev → HTTPRoute
• Portable across implementations (Istio, Cilium, Linkerd)
• Handles both north-south AND east-west traffic
• Istio ambient mode uses Gateway API natively for waypoint configuration

Istio CRDs (VirtualService, DestinationRule):
• Istio-specific, not portable
• More mature feature coverage
• Still needed for some advanced features (EnvoyFilter)
• Gradually being superseded by Gateway API equivalents

The industry direction is clear: Gateway API is the convergence
point. New deployments should prefer Gateway API where feature
coverage allows."
```
...
- [API Gateway](../1.14-api-gateway-design/00-index.md) - North-south traffic; Gateway API convergence
- [Zero Trust](../2.10-zero-trust-security-architecture/00-index.md) - Security model implemented by mesh
- [Load Balancer](../1.2-distributed-load-balancer/00-index.md) - LB algorithms (P2C, consistent hashing)
- [Service Discovery](../1.10-service-discovery-system/00-index.md) - How services find each other
- [Container Orchestration](../2.2-container-orchestration-system/00-index.md) - Mesh runs atop K8s; CNI, sidecar injection
- [Reverse Proxy](../1.13-high-performance-reverse-proxy/00-index.md) - Envoy proxy internals
