...
❌ Disabling sidecar for "performance"
   Why: Creates security gap (use ambient mode instead)
...
❌ Skipping mesh for "internal-only" services
   Why: Lateral movement after breach; zero-trust means internal ≠ trusted

❌ Using EnvoyFilter CRDs for security logic
   Why: Bypasses standard policy evaluation; use AuthorizationPolicy instead
```

---

## Supply Chain Security for Mesh Components

```
MESH COMPONENT SUPPLY CHAIN:
═══════════════════════════════════════════════════════════════

Verify mesh components themselves are not compromised:
─────────────────────────────────────────────────────────────

1. Image Signing
   • Verify Istio/Envoy images via cosign signatures
   • Use admission controllers to reject unsigned images
   • Pin to specific digests, not floating tags

2. SBOM (Software Bill of Materials)
   • Istio publishes SBOMs for each release
   • Scan for known CVEs before deployment
   • Monitor for Envoy proxy vulnerabilities

3. Wasm Extension Security
   • Wasm filters run sandboxed (memory-isolated)
   • Verify Wasm module signatures before loading
   • Audit resource limits (memory, CPU cycles)
   • Use OCI registries with signature verification

4. Control Plane Hardening
   • Run Istiod with minimal RBAC permissions
   • Network policies restrict control plane access
   • Webhook certificates rotated automatically
   • Admission webhooks use failClosed policy
```
