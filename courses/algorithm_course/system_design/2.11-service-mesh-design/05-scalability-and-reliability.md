...
• No central Slowest part of the process for data path
...
## Ambient Mode Scaling Advantages

```
AMBIENT VS SIDECAR SCALING:
═══════════════════════════════════════════════════════════════

Resource Comparison at 1,000 Pods on 50 Nodes:
─────────────────────────────────────────────────────────────

Sidecar Model:
  Proxies:           1,000 (one per pod)
  Total Memory:      60 GB (60 MB × 1,000)
  xDS Connections:   1,000 (one per proxy)
  Config Generation: Per-proxy (O(services × proxies))

Ambient Model:
  Ztunnels:          50 (one per node)
  Waypoints:         10-20 (per namespace/service, on-demand)
  Total Memory:      ~1 GB (5 MB × 50 + waypoints)
  xDS Connections:   50-70 (ztunnels + waypoints)
  Config Generation: Per-node + per-waypoint

Improvement:
  Memory:            ~98% reduction
  xDS Connections:   ~95% reduction
  Control Plane CPU: ~90% reduction

Key Insight: Ambient mode scales with NODE count, not POD count.
Adding pods to existing nodes has zero mesh overhead increase.
```
...
| **2000 pods** | Memory pressure | Sidecar resource scoping or ambient migration |
| **5000 pods** | Control plane CPU > 80% | Multi-cluster or sidecar-less (ambient/eBPF) |
| **10000+ pods** | Architectural limit | Multi-cluster mandatory |
| **Multi-region** | Cross-region latency | Regional meshes with federation |
| **AI workloads** | GPU pod churn, bursty scale | Ambient mode (avoids sidecar injection delay) |
