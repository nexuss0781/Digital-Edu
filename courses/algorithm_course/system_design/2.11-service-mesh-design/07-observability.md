...
```
OPENTELEMETRY IN SERVICE MESH:
═══════════════════════════════════════════════════════════════

Evolution: Mesh-native observability → OpenTelemetry convergence
─────────────────────────────────────────────────────────────
...
Before (Proprietary):
  ...
  Envoy → Jaeger (traces)
  Envoy → Fluentd (logs)
  Three separate pipelines, three protocols

After (OpenTelemetry):
  Envoy → OpenTelemetry Collector → Any Backend
  Single pipeline, OTLP protocol, unified correlation
...
Benefits:
  • Correlated metrics, traces, and logs via trace_id
  • Tail-based sampling in collector (capture errors)
  • Vendor-agnostic: switch backends without mesh changes
  • Resource attributes link to mesh identity (SPIFFE ID)
```

### Hubble Observability (eBPF-Based)

```
HUBBLE (CILIUM OBSERVABILITY):
═══════════════════════════════════════════════════════════════

  Unlike Envoy metrics (userspace), Hubble captures flows
  directly from eBPF programs in the kernel:

  Capabilities:
  • L3/L4 network flow visibility
  • L7 protocol visibility (HTTP, gRPC, Kafka, DNS)
  • Network policy verdict logging
  • Service dependency mapping
  • Latency distribution per service pair

  Key Difference from Envoy:
  • Zero application overhead (kernel-level capture)
  • Sees ALL traffic, not just mesh-enrolled pods
  • Includes DNS resolution visibility
  • Network policy drop reasons
```
...
| **OTel** | Use OpenTelemetry Collector as pipeline | Vendor-agnostic, tail sampling |
| **Correlation** | Link trace_id across metrics, traces, logs | Faster root cause analysis |
