...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Kubernetes CRDs (Istio)        Gateway API (Standard)          │
│  ─────────────────────          ─────────────────────           │
│  • VirtualService               • HTTPRoute                     │
│  • DestinationRule              • GRPCRoute                     │
│  • Gateway (Istio)              • Gateway (K8s standard)        │
│  • AuthorizationPolicy          • ReferenceGrant                │
│  • PeerAuthentication           • GatewayClass                  │
│                                                                  │
│  Pros: Fine-grained control     Pros: Portable, role-oriented   │
│  Cons: Istio-specific           Cons: Still maturing for mesh   │
│                                                                  │
│  Note: Gateway API is the convergence point — unifying          │
│  north-south (Ingress) and east-west (mesh) configuration       │
│  under a single standard. Istio ambient mode uses Gateway       │
│  API natively for waypoint proxy configuration.                 │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Decision 5: HBONE Tunneling Protocol

```
┌─────────────────────────────────────────────────────────────────┐
│                    HBONE vs Direct mTLS                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Direct mTLS (Sidecar)              HBONE (Ambient)             │
│  ─────────────────────              ───────────────              │
│  • Direct TCP connection            • HTTP CONNECT tunnel        │
│  • Requires iptables intercept      • Works through firewalls    │
│  • Metadata in TLS extension        • Per-request metadata       │
│  • Per-connection identity          • Multiplexed streams        │
│                                                                  │
│  HBONE = HTTP-Based Overlay Network Encapsulation               │
│  • Uses HTTP/2 CONNECT for tunneling                            │
│  • Carries mTLS within the tunnel                               │
│  • Enables per-request routing at L4 tunnel endpoints           │
│  • Foundation for ambient mesh inter-node communication         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
|-------|-----------|-------------------|
| **Control Plane** | Config Management | Kubernetes CRDs, Gateway API, etcd |
| **Control Plane** | Service Discovery | Kubernetes, Consul |
| **Control Plane** | Certificate Authority | Built-in CA, cert-manager, Vault, SPIRE |
| **Data Plane** | Sidecar Proxy | Envoy (C++), linkerd2-proxy (Rust) |
| **Data Plane** | Node Proxy | Ztunnel (Rust), Cilium Agent (eBPF) |
| **Data Plane** | L7 Proxy (Ambient) | Waypoint proxy (Envoy-based) |
| **Data Plane** | Sidecar Injection | Mutating webhook, CNI plugin, init container |
| **Data Plane** | Extensibility | WebAssembly (Wasm), Lua, ext-proc, eBPF programs |
| **Networking** | Tunneling | HBONE (HTTP CONNECT), WireGuard (Cilium) |
| **Observability** | Metrics | Prometheus, OpenTelemetry Collector |
| **Observability** | Tracing | Jaeger, Zipkin, Tempo, OpenTelemetry |
| **Observability** | Visualization | Kiali, Grafana, Hubble (Cilium) |
