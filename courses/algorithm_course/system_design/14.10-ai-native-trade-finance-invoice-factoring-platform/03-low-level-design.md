...
|---|---|---|---|
| `invoice` | `uploaded_at` (monthly) | Range partitioning | Time-based queries dominate; old invoices are rarely accessed; enables efficient partition Cutting off unnecessary steps and archival |
...
## Algorithm 5: Invoice Fingerprint for Cross-Platform Deduplication

```
FUNCTION GenerateInvoiceFingerprint(invoice):
    // Layer 1: Deterministic fingerprint from structured fields
    // This fingerprint can be shared with industry dedup registries
    // without revealing commercial details
    canonical_data = CONCATENATE(
        NormalizeGSTIN(invoice.seller_gstin),
        NormalizeGSTIN(invoice.buyer_gstin),
        NormalizeInvoiceNumber(invoice.invoice_number),
        FormatDate(invoice.invoice_date, "YYYY-MM-DD"),
        FormatAmount(invoice.net_amount, precision=2)
    )
    exact_fingerprint = SHA-256(canonical_data)

    // Layer 2: Fuzzy fingerprint for near-duplicate detection
    // Tolerant of minor field variations (e.g., amount rounded differently)
    fuzzy_data = CONCATENATE(
        NormalizeGSTIN(invoice.seller_gstin),
        NormalizeGSTIN(invoice.buyer_gstin),
        FormatDate(invoice.invoice_date, "YYYY-MM"),  // month granularity
        ROUND(invoice.net_amount, -3)  // round to nearest thousand
    )
    fuzzy_fingerprint = SHA-256(fuzzy_data)

    // Layer 3: Document fingerprint (for same-document detection)
    IF invoice.document_type == "PDF":
        doc_fingerprint = SHA-256(invoice.document_bytes)
    ELIF invoice.document_type == "IMAGE":
        // Perceptual hash for images (tolerant of scan quality differences)
        doc_fingerprint = PerceptualHash(invoice.document_bytes)
    ELSE:
        doc_fingerprint = NULL

    RETURN InvoiceFingerprint(
        exact = exact_fingerprint,
        fuzzy = fuzzy_fingerprint,
        document = doc_fingerprint,
        created_at = NOW()
    )

FUNCTION CheckDuplication(fingerprint):
    // Check 1: Exact match (same invoice fields)
    IF ExactFingerprintExists(fingerprint.exact):
        existing = GetExistingByFingerprint(fingerprint.exact)
    ...
    // Check 2: Fuzzy match (similar invoice, possible amount/date variation)
    fuzzy_matches = GetFuzzyMatches(fingerprint.fuzzy)
    IF LEN(fuzzy_matches) > 0:
        FOR EACH match IN fuzzy_matches:
            similarity = ComputeFieldSimilarity(fingerprint, match)
            IF similarity > 0.85:
                RETURN DuplicateResult(
                    is_duplicate = TRUE,
                    confidence = similarity,
                    match_type = "FUZZY_FIELD_MATCH",
                    existing_invoice = match
                )

    // Check 3: Cross-platform registry check
    IF IndustryRegistry.IsAvailable():
        registry_result = IndustryRegistry.CheckFingerprint(fingerprint.exact)
        IF registry_result.found:
            RETURN DuplicateResult(
                is_duplicate = TRUE,
                confidence = 0.95,
                match_type = "CROSS_PLATFORM",
                platform = registry_result.platform_id
            )

    // Check 4: IRN uniqueness (for e-invoices)
    IF fingerprint.irn IS NOT NULL:
        IF IRNExists(fingerprint.irn):
            RETURN DuplicateResult(is_duplicate = TRUE, confidence = 1.0, match_type = "IRN_DUPLICATE")
...
## Algorithm 6: Financier Portfolio Optimization

```
FUNCTION OptimizeFinancierMatching(invoice, eligible_financiers):
    // Multi-objective matching that balances MSME rate,
    // financier portfolio health, and platform economics

    scored_financiers = []

    FOR EACH financier IN eligible_financiers:
        portfolio = GetPortfolioState(financier.id)

        // Objective 1: Rate competitiveness (40% weight)
        // Lower rate = better for MSME = higher score
        rate_score = 1.0 - (financier.offered_rate_bps - min_rate_bps) /
                           (max_rate_bps - min_rate_bps)

        // Objective 2: Portfolio diversification (25% weight)
        // Penalize if this deal increases concentration
        buyer_exposure = portfolio.GetBuyerExposure(invoice.buyer_id)
        industry_exposure = portfolio.GetIndustryExposure(invoice.industry)
        concentration_penalty = MAX(
            (buyer_exposure + invoice.amount) / portfolio.total_limit - 0.10,  // 10% buyer limit
            (industry_exposure + invoice.amount) / portfolio.total_limit - 0.25,  // 25% industry limit
            0
        ) * 5  // Amplify penalty
        diversification_score = MAX(0, 1.0 - concentration_penalty)

        // Objective 3: Financier health (20% weight)
        // Prefer financially stable financiers over rate-aggressive weak ones
        health_score = 0.0
        health_score += 0.4 * (portfolio.crar - 0.15) / 0.10  // CRAR headroom
        health_score += 0.3 * (1.0 - portfolio.npa_rate / 0.05)  // NPA headroom
        health_score += 0.3 * portfolio.utilization_score  // Not over-extended
        health_score = CLAMP(health_score, 0, 1)

        // Objective 4: Platform value (15% weight)
        // Consider long-term relationship and platform fee
        platform_fee = ComputePlatformFee(invoice, financier.offered_rate_bps)
        relationship_score = financier.activity_score  // Active financiers preferred
        value_score = 0.6 * (platform_fee / max_platform_fee) + 0.4 * relationship_score

        // Weighted composite score
        composite = rate_score * 0.40 +
                    diversification_score * 0.25 +
                    health_score * 0.20 +
                    value_score * 0.15

        scored_financiers.APPEND({
            financier = financier,
            composite_score = composite,
            breakdown = { rate_score, diversification_score, health_score, value_score }
        })

    // Sort by composite score, return top matches
    scored_financiers.SORT(BY composite_score, DESCENDING)
    RETURN scored_financiers[:10]  // Top 10 matches
```
...
| Data Type | Raw Size/Day | Encoding Strategy | Compressed Size/Day | Ratio |
|---|---|---|---|---|
| Invoice documents (PDF) | 2.5 TB | Store as-is (PDFs are already compressed); extract and index text separately | 2.5 TB (no further compression) | 1:1 |
| Extracted invoice fields (JSON) | 50 GB | Column-oriented storage with dictionary encoding for enum fields (status, currency, industry) | 8 GB | 6:1 |
| Ledger entries | 5 GB | Fixed-width encoding for amounts (8-byte decimal); dictionary encoding for account codes; run-length encoding for repeated deal_ids in batch entries | 1.2 GB | 4:1 |
| Audit event log | 10 GB | JSON with dictionary encoding for repeated fields (actor_role, event_type, entity_type); delta encoding for timestamps | 2.5 GB | 4:1 |
| Credit features | 2 GB | Column-oriented with sparse encoding (many features are NULL for most buyers); quantization for float features | 400 MB | 5:1 |
| GST cache | 1.5 GB | JSON with dictionary encoding for HSN codes and GSTIN prefixes (first 2 digits = state code, highly repetitive) | 300 MB | 5:1 |
