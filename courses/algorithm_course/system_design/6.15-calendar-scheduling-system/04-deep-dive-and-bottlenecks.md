...
// Edge Case (Unusual or extreme situation): last day of every month
...
```
Step-by-step plan in plain English: Materialization Window Strategy
...
```
Step-by-step plan in plain English: Free-Busy Cache Layer
...
### Slowest part of the process: Multi-User Intersection
...
```
Step-by-step plan in plain English: Timezone Database Update Handling
...
```
Step-by-step plan in plain English: Reminder Timer Queue
...
```
Step-by-step plan in plain English: Availability Rule Evaluation
...
```
Step-by-step plan in plain English: Idempotent Booking
...
| Rank | Slowest part of the process | Impact | Mitigation |
|------|-----------|--------|------------|
...
## Deep Dive 6: CalDAV Sync at Scale — The Delta Sync Problem

### The Problem

CalDAV clients (Apple Calendar, Thunderbird, DAVx5) synchronize calendars by polling the server. Without an efficient delta mechanism, each sync requires downloading the entire calendar—potentially thousands of events.

### Sync Token Architecture

```
Step-by-step plan in plain English: Efficient CalDAV Delta Sync

STRUCTURE ChangeLog:
    sequence: bigint              // Auto-incrementing per calendar
    calendar_id: UUID
    event_uid: string             // iCalendar UID
    change_type: "created" | "updated" | "deleted"
    etag: string                  // Content hash of the event
    timestamp: timestamp

// On each event mutation, append to change log:
FUNCTION on_event_change(calendar_id, event_uid, change_type, event_data):
    etag = sha256(serialize_ical(event_data))
    INSERT INTO change_log (calendar_id, event_uid, change_type, etag)
    // sequence auto-assigned by database

// CalDAV sync-report handler:
FUNCTION handle_sync_report(calendar_id, client_sync_token):
    // Client sends their last-known sync token (maps to sequence number)
    last_seq = decode_sync_token(client_sync_token)

    // Return all changes since that sequence
    changes = SELECT * FROM change_log
              WHERE calendar_id = calendar_id
                AND sequence > last_seq
              ORDER BY sequence ASC

    // Compact: if same event changed multiple times, keep only latest
    compacted = compact_changes(changes)

    // Convert to CalDAV multistatus response
    response = MultiStatusResponse()
    FOR change IN compacted:
        IF change.change_type == "deleted":
            response.add_removal(change.event_uid)
        ELSE:
            event = get_event_by_uid(change.event_uid)
            ical = convert_to_icalendar(event)
            response.add_update(change.event_uid, ical, change.etag)

    new_token = encode_sync_token(changes.max_sequence())
    response.set_sync_token(new_token)
    RETURN response

// Initial sync (no sync token): send all events
FUNCTION handle_initial_sync(calendar_id):
    events = get_all_events(calendar_id)
    FOR event IN events:
        ical = convert_to_icalendar(event)
        response.add_update(event.ical_uid, ical, compute_etag(event))
    response.set_sync_token(encode_sync_token(current_max_sequence()))
    RETURN response
```

### CalDAV vs. JMAP Comparison

| Aspect | CalDAV (RFC 4791) | JMAP Calendar (RFC 8984) |
|--------|-------------------|--------------------------|
| **Transport** | WebDAV over HTTP (XML) | JSON over HTTP |
| **Sync model** | Client polls with sync-token | Push via EventSource + queryChanges |
| **Batch operations** | One request per operation | Multiple operations in single request |
| **Data format** | iCalendar text (RFC 5545) | JSCalendar JSON (RFC 8984) |
| **Recurrence** | RRULE in text | Structured JSON recurrence rules |
| **Adoption (2025)** | Apple, Thunderbird, Nextcloud, Fastmail | Fastmail, Cyrus — limited |
| **Interop maturity** | 20+ years, well-understood edge cases | New, fewer implementations |

### Sync Conflict Resolution

| Conflict Type | Detection | Resolution |
|---------------|-----------|------------|
| Both sides changed different fields | Compare per-field ETags | Merge: accept both changes |
| Both sides changed same field | Field-level conflict | Last-writer-wins (server timestamp) |
| Client deleted, server updated | Missing UID vs. updated ETag | Ask user: keep or delete? |
| Server deleted, client updated | Client sends PUT for deleted UID | Recreate event (client wins) |
| Concurrent recurring instance modifications | Same instance_id, different overrides | Merge non-conflicting overrides; last-writer-wins for conflicts |

---

## Deep Dive 7: Team Booking and Round-Robin Distribution

### The Problem

A sales team of 20 members shares a single booking page. When a prospect books a demo, the system must assign them to a team member based on configurable distribution rules while preventing any individual team member from being double-booked.

### Distribution Algorithms

| Algorithm | How It Works | Best For |
|-----------|-------------|----------|
| **Equal distribution** | Assign to member with fewest bookings in the current period | Ensuring fairness across team |
| **Availability-first** | Assign to member with most remaining availability | Maximizing booking conversion |
| **Weighted round-robin** | Each member has a weight (e.g., senior=3, junior=1); distribution proportional to weight | Mixed experience levels |
| **Fixed rotation** | Strict sequential assignment (Alice → Bob → Carol → ...) | Simple, predictable |
| **Routing form** | Booker answers intake questions; answers route to specific member or sub-team | Specialized expertise matching |

### Architectural Challenges

```
Challenge: Concurrent bookings to round-robin page

Timeline:
  T=0: Guest A books 10:00 AM → assigned to Alice (fewest bookings)
  T=0: Guest B books 10:30 AM → also assigned to Alice (fewest bookings)
  T=1: Both reservations try to lock Alice's calendar...

Problem: The distribution function ran twice with stale booking counts.

Solution: Serialize distribution decisions per team:
  1. Booking requests enter a per-team queue
  2. A single worker processes the queue sequentially
  3. Each request: select member → lock slot → create event → update counts
  4. Next request sees updated counts

Trade-off: Adds 100-500ms latency per booking, but prevents
assignment conflicts. Acceptable for booking flows where the
guest already expects a 2-3 second confirmation wait.
```

### Multi-Timezone Team Availability

For a globally distributed team, the booking page must show slots where at least one team member is available:

```
Team: Alice (US-East), Bob (CET), Carol (IST)
Duration: 30 minutes

Slot computation:
  1. Load working hours per member, convert to UTC
  2. Load free-busy bitmap per member
  3. For each 15-min slot in the booking window:
     - Check if ANY member is both within working hours AND free
     - If yes, slot is bookable (mark which members are eligible)
  4. When guest selects a slot:
     - Run distribution algorithm only on eligible members for that slot
     - Lock selected member's calendar
     - Create event in member's and guest's timezone
```
