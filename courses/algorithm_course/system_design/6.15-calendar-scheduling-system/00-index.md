...
|-----------|-------------------|-------------------|
| **Recurrence** | A rule generates infinite instances; modifications create exception trees; series splits create identity chains | Data model, expansion algorithm, series modification API |
| **Timezone** | Wall-clock intent must survive DST transitions; IANA database changes require re-expansion; cross-timezone attendees see different local times | Storage format, expansion engine, reminder scheduling, IANA update pipeline |
| **Concurrency** | Booking double-booking prevention; concurrent RSVP updates; cross-shard invitation consistency | Distributed locking, saga pattern, eventual consistency |
| **Scale** | 2B free-busy queries/day; 1.5B reminders/day; Monday-morning traffic spikes | Bitmap caching, timer partitioning, pre-computation, auto-scaling |
| **Interop** | CalDAV/iCal/Exchange produce different representations of the same events; recurrence exceptions vary by implementation | Sync service translation layer, conflict resolution |
...
| Pattern | Relevance to Calendar & Scheduling |
|---------|-------------------------------------|
| **CQRS (Command Query Responsibility Segregation)** | Separate write model (event mutations with strong consistency) from read model (calendar views, free-busy queries with eventual consistency). Event writes go to primary DB; reads served from materialized views and caches. |
| **Event Sourcing** | Every calendar change (event created, RSVP updated, instance modified) stored as an immutable domain event. Current calendar state derived by replaying events. Enables audit trails, conflict resolution, and undo. |
| **Materialized View** | Pre-computed recurring event instances and free-busy bitmaps are materialized views refreshed on event changes. Trades write amplification for sub-10ms read latency. |
| **Distributed Timer / Scheduler** | Reminder delivery requires precise wall-clock-time firing across billions of entries. Timer queue partitioned by fire-time with worker-based claiming. |
| **Optimistic vs. Pessimistic Locking** | Booking slot reservation uses pessimistic distributed locks to prevent double-booking. Calendar view rendering uses optimistic concurrency with ETags. |
| **Saga Pattern** | Cross-service booking flow (lock slot → verify availability → create event → notify attendees) orchestrated as a saga with compensating actions on failure. |
| **Bitmap Index** | Free-busy availability represented as bit arrays (1 bit per 15-min slot). Enables sub-millisecond intersection queries via bitwise OR across multiple users. |
| **Fan-Out on Write vs. Fan-Out on Read** | Small meetings: fan-out on write (pre-create per-attendee reminders). Large meetings (>1K attendees): fan-out on read (single group reminder, expand at fire time). |
...
| [1.4 - Distributed Cache](../1.4-distributed-cache/) | Free-busy bitmap caching, calendar view caching at scale |
| [6.8 - Real-Time Collaborative Editor](../6.8-real-time-collaborative-editor/) | Conflict resolution for concurrent calendar edits, offline sync |
| [2.2 - Distributed Locking](../2.2-distributed-locking/) | Booking slot lock acquisition, resource reservation atomicity |
...
|------------|----------------------|
| **AI scheduling assistants** (Reclaim.ai acquired by Calendly 2024, Clockwise by Atlassian) | Constraint satisfaction layer consuming free-busy bitmaps; team-level calendar optimization |
| **JMAP Calendar (RFC 8984)** | Modern JSON-over-HTTP calendar protocol with batched requests, push notifications, and efficient delta sync — potential CalDAV successor |
| **JSCalendar (RFC 8984 data format)** | Structured JSON event representation (JSEvent, JSTask) with richer semantics than iCalendar text format |
| **Meeting cost analytics** | Read-only analytics service computing meeting cost from attendee count × duration × hourly rate; drives policies like meeting-free days |
| **Privacy-preserving availability** | Cross-organization scheduling using minimal-disclosure availability sharing with slot quantization and jitter |
| **Hybrid work calendar integration** | Working-location per day, in-office/remote status, office day coordination; desk/room booking integration |
| **Round-robin and team booking** | Distribution algorithms (equal, availability-weighted, priority-weighted) for routing bookings across team members |
| **Calendar protocol interop challenges** | Cross-system recurrence exception handling remains a major pain point (Google ↔ Microsoft ↔ Apple) |
...
- RFC 8984 (JMAP Calendar) --- Modern Calendar API Protocol
- RFC 9073 --- New iCalendar Properties (STYLED-DESCRIPTION, PARTICIPANT)
- CalDAV (RFC 4791) --- Calendar Synchronization Protocol
- Calendly Engineering --- Availability Engine and Booking Architecture
- Cal.com Architecture --- Open-Source Scheduling Infrastructure
