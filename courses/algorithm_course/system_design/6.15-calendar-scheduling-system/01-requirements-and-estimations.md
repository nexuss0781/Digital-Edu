...
### Timezone Complexity Budget

```
IANA timezone database: 594 timezone identifiers (2025)
DST rule changes per year: 5-10 updates to tzdata
Affected recurring events per timezone change: up to 10M events
Re-expansion time budget: must complete within 4 hours of tzdata release
  → 10M events / 4 hours = ~694 events/second expansion rate
  → With 50 workers: ~14 events/second per worker (feasible)
```

### Recurring Event Materialization Budget

```
New recurring events per day: 200M * 40% recurring = 80M
Average instances per series (6-month window):
  - Daily: ~180 instances
  - Weekly: ~26 instances
  - Monthly: ~6 instances
  - Weighted average: ~40 instances per series

Materialization writes per day: 80M * 40 instances = 3.2B instance writes
  → Peak: 3.2B / 86,400 * 5x = ~185K writes/second
  → Requires batch insert optimization (1,000 instances per batch)
```

### Free-Busy Cache Invalidation Budget

```
Events that change free-busy state per day:
  - New events: 200M
  - Updated events: 100M (estimated 50% update rate)
  - Cancelled events: 20M
  Total: 320M events/day

Average attendees per event: 5
Cache invalidations per day: 320M * 5 = 1.6B
  → Peak: 1.6B / 86,400 * 5x = ~92K invalidations/second
  → With 5-second debounce/coalescing: ~18K actual re-computations/second
```

### Cross-Timezone Meeting Scheduling Budget

```
Distributed team meetings (spanning 3+ timezones): ~15% of all meetings
  → 200M * 0.15 = 30M cross-timezone meetings/day
Working-hours overlap window calculation:
  - US-Pacific to India Standard Time: 7:30 PM PST = 9:00 AM IST (2.5 hours overlap)
  - US-Eastern to CET: 9:00 AM EST = 3:00 PM CET (4 hours overlap)
  - 3-way US+EU+APAC overlap: ~0-2 hours depending on locations
Scheduling assistant queries for cross-timezone: ~5x more expensive than same-timezone
  → Must compute availability across working-hours rules per participant
```

---

## AI-Assisted Scheduling Requirements (2025-2026)

### Intelligent Time Blocking

| Feature | Requirement | Metric |
|---------|-------------|--------|
| **Focus time protection** | Auto-block uninterrupted work periods when calendar has fragmented gaps | Increase contiguous 2+ hour blocks by 30% |
| **Meeting clustering** | Group meetings together to minimize context switches | Reduce daily context switches by 25% |
| **Smart duration suggestions** | Analyze historical meeting data to suggest optimal duration | Default to 25/50 min (not 30/60) when historical data shows early endings |
| **Energy-aware scheduling** | Respect user's high/low energy period preferences | Schedule demanding meetings in declared high-energy windows |
| **Recurring meeting audit** | Flag recurring meetings with consistent low attendance or frequent cancellations | Alert organizer when <50% attendance for 3+ consecutive occurrences |
| **Buffer insertion** | Auto-add buffer time between back-to-back meetings | Configurable 5-15 min buffers based on meeting type |
...
Constraint satisfaction problem:
  Inputs:
    - N participants with availability bitmaps
    - Per-participant working hours and timezone
    - Meeting duration requirement
    - Priority weights (organizer preference, participant seniority)
    - Buffer constraints (min gap from previous/next meeting)
    - Room/resource availability (optional)

  Objective: Minimize max inconvenience across all participants
    where inconvenience = distance from preferred working hours

  Solver: CP-SAT (constraint programming) or greedy with priority queue
  Latency: <2s for N≤20, <10s for N≤100

  Output: Ranked list of 3-5 suggested slots with inconvenience scores
```
