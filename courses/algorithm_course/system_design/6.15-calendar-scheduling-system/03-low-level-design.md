...
```
Step-by-step plan in plain English: RRULE Expansion Engine
...
```
Step-by-step plan in plain English: Free-Busy Bitmap Computation
...
```
Step-by-step plan in plain English: Booking Service
...
## Algorithm 4: Timezone-Aware Recurrence Expansion

```
Step-by-step plan in plain English: DST-Correct Recurrence Expansion

FUNCTION expand_with_dst_handling(master_event, window_start, window_end):
    tz = load_timezone(master_event.start_timezone)
    local_start = to_local(master_event.start_time, tz)
    wall_clock_time = local_start.time()  // e.g., "09:00"
    duration = master_event.end_time - master_event.start_time
    instances = []
    ...
    WHILE true:
        // Generate next local date per recurrence rule
        current_local = next_local_occurrence(current_local, master_event.rrule)

        IF current_local IS NULL:
            BREAK

        // Combine wall-clock time with local date
        candidate = current_local.date() + wall_clock_time

        // Handle DST gap: "spring forward" makes some wall-clock times invalid
        IF NOT is_valid_local_time(candidate, tz):
            // 2:30 AM doesn't exist during spring-forward
            candidate = next_valid_time(candidate, tz)
            // Moves to 3:00 AM (or equivalent)

        // Handle DST overlap: "fall back" makes some wall-clock times ambiguous
        IF is_ambiguous_local_time(candidate, tz):
            // 1:30 AM occurs twice during fall-back
            candidate = resolve_ambiguity(candidate, tz, prefer="first")
            // Use first occurrence (standard time)

        // Convert to UTC using the correct offset for THIS date
        utc_start = to_utc(candidate, tz)
        utc_end = utc_start + duration

        IF utc_start > window_end:
            BREAK
        IF utc_start >= window_start:
            instances.append(EventInstance(
                original_start = utc_start,
                start_time = utc_start,
                end_time = utc_end,
                utc_offset = get_utc_offset(candidate, tz)
            ))
...
## Algorithm 5: Round-Robin Booking Distribution

```
Step-by-step plan in plain English: Team Booking with Round-Robin Assignment

STRUCTURE TeamBookingLink:
    team_id: UUID
    members: list[TeamMember]
    distribution_mode: "equal" | "availability" | "weighted"
    event_type_id: UUID

FUNCTION select_team_member(booking_link, slot_start, slot_end):
    members = booking_link.members

    SWITCH booking_link.distribution_mode:
        CASE "equal":
            // Assign to member with fewest bookings this period
            counts = {}
            FOR member IN members:
                counts[member.id] = count_bookings(member.id, current_period())
            eligible = [m FOR m IN members IF is_available(m, slot_start, slot_end)]
            RETURN min_by(eligible, m -> counts[m.id])

        CASE "availability":
            // Assign to member with most remaining availability
            eligible = []
            FOR member IN members:
                IF is_available(member, slot_start, slot_end):
                    remaining = count_free_slots(member.id, today())
                    eligible.append((member, remaining))
            RETURN max_by(eligible, (m, r) -> r).member

        CASE "weighted":
            // Assign based on configured weights, with availability filter
            eligible = [m FOR m IN members IF is_available(m, slot_start, slot_end)]
            total_weight = sum(m.weight FOR m IN eligible)
            adjusted_weights = {}
            FOR member IN eligible:
                // Reduce weight proportionally to bookings already received
                bookings = count_bookings(member.id, current_period())
                expected = member.weight / total_weight * total_bookings()
                adjusted_weights[member.id] = member.weight * max(0.1, 1 - bookings / max(expected, 1))
            RETURN weighted_random_select(eligible, adjusted_weights)


FUNCTION compute_team_slots(booking_link, date_range, viewer_timezone):
    // A slot is available if ANY team member is available
    all_slots = {}

    FOR member IN booking_link.members:
        member_slots = compute_available_slots(
            member.booking_config, date_range, viewer_timezone
        )
        FOR slot IN member_slots:
            IF slot.key NOT IN all_slots:
                all_slots[slot.key] = slot
                all_slots[slot.key].available_members = []
            all_slots[slot.key].available_members.append(member.id)

    RETURN sort_by(all_slots.values(), "start")
```

---

## Sync Protocol: Change Log Architecture

```
STRUCTURE ChangeLogEntry:
    sequence: bigint          // Monotonically increasing per calendar
    calendar_id: UUID
...
// CalDAV sync-token maps to the sequence number
// Client sends: "Give me all changes since sequence 4521"
// Server returns: entries 4522, 4523, 4524 + new sync-token "4524"

FUNCTION process_sync_request(calendar_id, client_sync_token):
    last_seq = parse_sequence(client_sync_token)

    changes = SELECT * FROM change_log
              WHERE calendar_id = calendar_id
                AND sequence > last_seq
              ORDER BY sequence ASC
              LIMIT 500  // Pagination for large change sets

    results = []
    FOR change IN changes:
        IF change.change_type == "deleted":
            results.append(SyncDeletion(event_uid = change.event_uid))
        ELSE:
            event = get_event_by_uid(change.event_uid)
            ical = convert_to_icalendar(event)
            results.append(SyncUpdate(
                event_uid = change.event_uid,
                etag = change.etag,
                ical_data = ical
            ))

    new_token = encode_sequence(changes.last().sequence IF changes ELSE last_seq)
    RETURN SyncResponse(changes = results, sync_token = new_token)
