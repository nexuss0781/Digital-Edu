...
- Faster provisioning (direct cloud API, bypasses ASG)
- Better bin-packing (right-sizes per workload)
- Supports spot/preemptible instances natively
- Consolidation: automatically migrates pods to fewer, cheaper nodes
- Disruption budgets: respects PDBs during consolidation
...
1. Managed Control Plane Approach:
   - Distributed database-backed control plane storage (replacing etcd)
   - Custom scheduler with better parallelism
   - Aggressive watch caching
   - Example: Major cloud providers support 15,000-65,000 nodes

2. Cell Architecture (Borg-inspired):
   - Multiple independent clusters (cells)
   - Global scheduling layer above cells (Karmada, Admiralty)
   - Each cell: 1,000-5,000 nodes
   - Cross-cell service discovery via multi-cluster mesh

3. Scheduler Optimization:
   - Multi-scheduler with workload-class affinity
   - Gang scheduling for batch/ML workloads
   - Bin-packing algorithms with NUMA-aware placement
   - DRA for structured hardware claims (GPUs, FPGAs)

4. etcd Alternatives:
   - K3s with SQLite/PostgreSQL via Kine adapter
   - Custom storage backends with watch translation layer
   - Distributed SQL databases for global-scale control planes

5. Multi-Cluster Federation (2025+):
   - Karmada: Propagate resources across member clusters
   - Submariner: Cross-cluster networking and service discovery
   - Liqo: Virtual node abstraction for seamless multi-cluster
```

### WebAssembly (Wasm) Workloads

```
Wasm as complement to containers (2025+):

Traditional Container:
- Cold start: 1-5 seconds
- Image size: 100MB-1GB
- Isolation: Linux namespaces + cgroups
- Density: 30-100 pods/node

Wasm Workload:
- Cold start: < 1ms
- Module size: 1-10MB
- Isolation: Wasm sandbox (no kernel dependency)
- Density: 1,000+ instances/node

Integration approaches:
1. SpinKube: Wasm runtime as containerd shim
2. wasmCloud: Wasm-native orchestration platform
3. RuntimeClass: Route Wasm pods to Wasm-capable nodes

Use cases: Edge functions, serverless handlers, plugin systems
Not suitable for: Stateful services, GPU workloads, raw I/O
```
