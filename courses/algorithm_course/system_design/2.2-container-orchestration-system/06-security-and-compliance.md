...
| **OIDC Tokens** | Human users via IdP | IdP-managed | Integrates with enterprise identity providers |
...
- kms: External KMS (cloud provider KMS services)
```
...
- Supports Vault, cloud provider secret managers, and other backends
...
```
Supply chain security stack (2025+):
...
2. SBOM (Software Bill of Materials):
   - Attach SBOM to container images via OCI artifacts
   - Scan SBOMs for known vulnerabilities
   - Admission policies requiring SBOM presence
...
3. SLSA Framework (Supply-chain Levels for Software Artifacts):
   ...
   - Level 4: Full dependency verification

4. Admission Enforcement:
   - Kyverno/OPA policies requiring signed images
   - ValidatingAdmissionPolicy with image verification
   - Registry allowlisting via ImagePolicyWebhook
```

### Runtime Security

| Layer | Tool Category | Protection |
|-------|---------------|------------|
| **Build-time** | Image scanning, SBOM generation | Known CVEs, license compliance |
| **Admission-time** | Signature verification, policy engines | Unsigned/untrusted images blocked |
| **Runtime** | eBPF-based monitoring, Falco | Anomalous syscalls, file access, network connections |
| **Response** | Automated pod termination, quarantine | Contain compromised workloads |

**Cross-reference:** Supply chain security integrates with [CI/CD Pipeline](../2.4-cicd-pipeline-build-system/00-index.md) for build-time signing and [Secret Management](../2.16-secret-management-system/00-index.md) for signing key lifecycle.
