...
| **Node sampling** | Only consider subset of nodes (configurable `percentageOfNodesToScore`) | Reduces filter time for large clusters |
...
| **Scheduling queue hints** | Event-driven requeueing from unschedulable queue (v1.30+) | Reduces unnecessary scheduling retries |

#### DRA Integration with Scheduler

```
DRA scheduling flow (v1.31+ beta):

1. Pod has resourceClaims in spec
2. Pre-Filter: DRA plugin checks ResourceClaim exists
3. Filter: DRA plugin evaluates structured parameters
   - Parse CEL expressions from ResourceClaimParameters
   - Match against node's ResourceSlice inventory
   - Ensure device availability on candidate node
4. Reserve: DRA plugin allocates device on selected node
5. Post-Bind: DRA driver configures device (CDI injection)

Key insight: DRA moves device matching into the scheduler's
filter/score pipeline, enabling co-scheduling of GPU pods
with NUMA-aware topology constraints.
```

**Cross-reference:** The constraint satisfaction approach used in DRA scheduling parallels the resource matching in [Distributed Job Schedulers](../2.6-distributed-job-scheduler/00-index.md), where jobs specify resource requirements beyond simple CPU/memory.
...
### Slowest part of the process 1: etcd Write Throughput
...
flowchart LR
    ...
    Slowest part of the process[Single leader<br/>handles all writes]
```
...
### Slowest part of the process 2: API Server Request Handling

**Problem:** API server becomes Slowest part of the process under high QPS or many watches.
...
### Slowest part of the process 3: Scheduler Throughput
...
| Component | Related System | Connection |
|-----------|---------------|------------|
| etcd consensus | [Distributed KV Store](../1.3-distributed-key-value-store/00-index.md) | etcd is a specialized Raft-based KV store; same consistency-availability trade-offs apply |
| Service endpoints | [Service Discovery](../1.10-service-discovery-system/00-index.md) | EndpointSlices implement a watch-based service registry pattern |
| kube-proxy / Cilium | [eBPF Observability](../15.4-ebpf-observability-platform/00-index.md) | eBPF replaces iptables for O(1) service routing and deep network observability |
| Network policies | [Zero Trust Architecture](../2.10-zero-trust-security-architecture/00-index.md) | Network policies implement microsegmentation; Cilium adds identity-based enforcement |
| Scheduler preemption | [Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) | Priority-based preemption parallels batch job scheduling with SLA tiers |
