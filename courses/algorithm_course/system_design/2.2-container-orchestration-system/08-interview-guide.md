...
### Q: "Why is the scheduler centralized? Isn't that a Slowest part of the process?"

**Good answer:**
- Centralized provides global view for optimal placement
- Sufficient for tested scale (~5,000 nodes, 100+ pods/sec)
- Slowest part of the process mitigations:
...
### Q: "Why would you replace kube-proxy with eBPF?"

**Good answer:**
- kube-proxy creates iptables rules per endpoint -- O(n) packet processing
- In large clusters (10K+ services), iptables updates take minutes
- eBPF (Cilium) uses kernel hash maps for O(1) service routing
- Benefits: 40%+ latency reduction, identity-based network policies, deep observability
- Trade-off: Requires kernel 5.10+ and introduces eBPF complexity
- Not just networking -- also enables runtime security monitoring without agents
...
## Modern Topics to Demonstrate Depth (2025+)

| Topic | Key Point | When to Raise |
|-------|-----------|---------------|
| **eBPF networking** | Replaces iptables with O(1) kernel hash maps; Cilium replaces kube-proxy | When discussing service routing or network policies |
| **Gateway API** | Role-oriented ingress; GatewayClass → Gateway → HTTPRoute separation | When discussing external traffic routing |
| **DRA for GPUs** | Structured parameters replace integer-only device plugins | When discussing ML/GPU workload scheduling |
| **Sidecar containers** | Native lifecycle ordering (KEP-753); sidecars start before main containers | When discussing service mesh or multi-container patterns |
| **ValidatingAdmissionPolicy** | In-process CEL-based validation; 10x faster than webhooks | When discussing admission control or policy |
| **Wasm runtimes** | Sub-millisecond cold starts; 10x density; complementary to containers | When discussing edge or serverless workloads |
| **Karpenter** | Pod-granular node provisioning; consolidation; replaces cluster autoscaler | When discussing autoscaling or cost optimization |
...
6. "Would you like me to discuss GPU/accelerator scheduling with Dynamic Resource Allocation?"
...
**You:** "The scheduler uses a two-phase algorithm. First, filtering eliminates nodes that can't run the pod - checking resources, taints, affinity. Then, scoring ranks the remaining nodes based on factors like balanced utilization and image locality. Let me walk through the Step-by-step plan in plain English..."
