...
| **Cloud-native (VPC CNI)** | Pod IPs from VPC | Native performance, no overlay | Cloud-specific, IP exhaustion risk |
| **eBPF (Cilium)** | Kernel-level | High performance, advanced observability, replaces kube-proxy | Kernel version requirements (5.10+) |

**Recommendation:** Match CNI to infrastructure. Cloud-native for managed clusters, Cilium for advanced use cases. eBPF-based CNIs increasingly becoming default in 2025+ due to performance and observability benefits.

### 6. Gateway API vs Ingress

| Approach | Pros | Cons | Verdict |
|----------|------|------|---------|
| **Ingress** | Simple, well-known, widely supported | Limited expressiveness, single resource for all concerns | Legacy |
| **Gateway API** | Role-oriented (infra vs app), expressive routing, multi-protocol | Newer, requires Gateway controller | **Preferred (GA since v1.1)** |

**Rationale:** Gateway API separates infrastructure concerns (GatewayClass, Gateway) from application routing (HTTPRoute, GRPCRoute), enabling platform teams and app teams to work independently. Supports traffic splitting, header-based routing, and cross-namespace references natively.

### 7. Device Plugin vs Dynamic Resource Allocation (DRA)

| Approach | Pros | Cons | Verdict |
|----------|------|------|---------|
| **Device Plugins** | Simple, mature | Integer-only resources, no structured claims | Legacy for GPUs |
| **DRA (Structured Parameters)** | Rich claim semantics, vendor-neutral, scheduler-integrated | Newer (beta v1.31+) | **Emerging standard** |

**Rationale:** DRA allows workloads to request specific GPU models, VRAM sizes, or accelerator features through structured parameters. The scheduler evaluates DRA claims during filtering, preventing scheduling to nodes that cannot satisfy hardware requirements.
...
PVC creation → Controller → CSI Driver → Storage Backend
                    ...
                    ┌───────────┴───────────┐
                    ↓                       ↓
            Block CSI Driver         Distributed FS Driver
                    ↓                       ↓
          Cloud Block Storage API    Ceph RBD/CephFS
```

### Gateway API Architecture

```
External Traffic → GatewayClass → Gateway → HTTPRoute → Service → Pods
                      ↓              ↓           ↓
                  Controller     Listener    Routing Rules
                  (vendor)     (ports,TLS)  (path, header, weight)
```
