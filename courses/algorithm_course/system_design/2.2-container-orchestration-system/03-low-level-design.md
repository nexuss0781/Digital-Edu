...
erDiagram
    GATEWAYCLASS {
        ...
        string controllerName "vendor-specific controller"
        object parametersRef "optional vendor config"
    }
    ...
    GATEWAY {
        ...
        string kind "Gateway"
        object metadata "name, namespace"
        string gatewayClassName "reference to GatewayClass"
        array listeners "port, protocol, hostname, TLS config"
        object status "addresses, conditions"
    }

    HTTPROUTE {
        string apiVersion "gateway.networking.k8s.io/v1"
        string kind "HTTPRoute"
        array parentRefs "Gateway references"
        array hostnames "matching hostnames"
        array rules "matches, filters, backendRefs"
    }

    GATEWAYCLASS ||--o{ GATEWAY : "defines"
    GATEWAY ||--o{ HTTPROUTE : "routes"
    HTTPROUTE ||--o{ SERVICE : "targets"
```

### Dynamic Resource Allocation (DRA) Model

```
DRA Resource Hierarchy:

ResourceClass
├── driverName: "gpu.vendor.com"
├── parametersRef → ConfigMap (structured parameters)
└── suitableNodes: nodeSelector

ResourceClaim
├── resourceClassName: "gpu-class"
├── allocationMode: WaitForFirstConsumer | Immediate
└── parametersRef → ResourceClaimParameters
    ├── requests:
    │   ├── deviceName: "gpu"
    │   ├── count: 2
    │   └── selectors:
    │       ├── cel: "device.attributes['memory'] >= '16Gi'"
    │       └── cel: "device.attributes['model'] == 'A100'"
    └── constraints:
        └── requests: ["gpu"]
            matchAttribute: "device.attributes['numa-node']"
...
| ResourceClaims | Until pod deleted | Owner reference cascade |
...
**ValidatingAdmissionPolicy (CEL-based, GA in v1.30):**
```
In-process validation without webhook overhead:
...
- No network round-trip latency (in-process CEL evaluation)
- No webhook availability dependency
- Type-safe expressions with IDE support
- ~10x faster than webhook-based validation
```
...
**Step-by-step plan in plain English (Deployment Controller):**
