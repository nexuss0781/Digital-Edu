...
- [x] Analyzes serialization Slowest part of the process and parallelization strategy
...
| "How would you handle voice chat integration?" | Proximity-based voice routed through edge relays; separate unreliable/unordered channel; spatial attenuation on client |
| "How would you support tournament spectating at massive scale?" | Spectator server re-broadcasts delayed stream; CDN fan-out for 100K+ viewers; separate tick rate for spectator stream |

---

## 8. Architecture Decision Records

### 8.1 Why Single-Threaded Tick Loop

| Considered | Rationale for Rejection |
|------------|------------------------|
| **Multi-threaded simulation** | Non-deterministic ordering of entity updates causes desyncs; lock contention on shared state exceeds tick budgets |
| **Actor-per-entity model** | Message passing overhead between thousands of actors adds latency; spatial queries require global state |
| **Coroutine-based** | Complex control flow; harder to profile and debug tick overruns |

**Chosen**: Single-threaded fixed-timestep loop with parallelized serialization phase only (read-only after simulation completes).

### 8.2 Why Ring Buffer for Tick History (Not Append Log)

| Approach | Memory | Access Pattern | Why Ring Buffer Wins |
|----------|--------|---------------|---------------------|
| Append log | Unbounded | Sequential replay | Requires GC / truncation management |
| Ring buffer (256 slots) | Fixed ~7 MB | Random access by tick | O(1) lookup, zero allocation after init, bounded memory |
| Copy-on-write snapshots | Variable | Random access | Higher overhead; useful only for deep history |

**Chosen**: Fixed-size ring buffer — zero-allocation steady state, deterministic memory footprint, perfect for the 200 ms rewind window.

### 8.3 Whiteboard Sketch Guide

```
Essential diagram for a 45-minute interview:

  ┌──────────┐   UDP    ┌──────────┐        ┌──────────────────────┐
  │ Client 1 │ ───────→ │  Edge    │ ─────→ │  Game Server         │
  │ (predict)│ ←─────── │  Relay   │ ←───── │  ┌─ Input Receiver   │
  └──────────┘          └──────────┘        │  ├─ Tick Simulator   │
  ┌──────────┐                              │  ├─ Physics Engine   │
  │ Client N │ ───→ (same pattern) ───→     │  ├─ Combat Resolver  │
  └──────────┘                              │  ├─ Interest Manager │
                                            │  └─ Replication Eng  │
       ┌──────────┐   ┌───────────┐        └──────────────────────┘
       │Matchmaker│──→│Fleet Mgr  │──→ allocates server instances
       └──────────┘   └───────────┘

  Show: inputs flow left-to-right, snapshots right-to-left
  Label: "UDP, unreliable sequenced" on game traffic
  Label: "delta compressed, interest filtered" on snapshots
```

### 8.4 Red Flags to Avoid

| Red Flag | Signal It Sends |
|----------|----------------|
| Proposing microservices for the game server | Misunderstanding real-time constraints — inter-service RPC adds latency per tick |
| Suggesting a message queue between simulation phases | Queues add unpredictable latency; tick loop must be synchronous |
| Storing game state in a database during match | Disk I/O in a 16 ms tick budget is impossible; all state must be in-memory |
| Ignoring bandwidth calculation | Shows lack of systems thinking — bandwidth is the binding constraint |
| Proposing "just add more servers" for one match | One match = one simulation = one server; horizontal scaling is across matches, not within |
