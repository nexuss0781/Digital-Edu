...
|--------|-------------|----------------|
| [12.8 — WebRTC Infrastructure](../12.8-webrtc-infrastructure/00-index.md) | **Shared constraint** — real-time UDP streaming with jitter and loss | Edge relay topology, adaptive bitrate under bandwidth pressure |
| [1.2 — Distributed Load Balancer](../1.2-distributed-load-balancer/00-index.md) | **Fleet allocation** — distributing match sessions across servers | Health-based routing, weighted regional allocation, graceful draining |
| [5.6 — Google Photos](../5.6-google-photos/00-index.md) | **Delta sync** — differential state transfer to heterogeneous clients | Per-client baseline tracking, progressive data delivery |
| [12.1 — AdTech Real-Time Bidding](../12.1-adtech-real-time-bidding/00-index.md) | **Sub-10 ms budget** — fixed-deadline processing pipelines | Tick budget decomposition, load shedding under time pressure |
| [9.4 — Inventory Management](../9.4-inventory-management-system/00-index.md) | **Conflict resolution** — simultaneous mutations on shared state | Deterministic ordering, last-writer-wins vs first-writer-wins policies |
| [11.4 — Email Delivery System](../11.4-email-delivery-system/00-index.md) | **Fan-out replication** — one-to-many delivery with per-recipient customization | Edge fan-out, priority-based sending under throughput limits |
