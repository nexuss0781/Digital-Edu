...
```
STRUCTURE: AdaptiveJitterBuffer

PURPOSE: Absorb network timing variance so snapshot playback
         remains smooth even when packets arrive unevenly.

PARAMETERS:
  min_buffer_ticks: 2          // Minimum cushion
  max_buffer_ticks: 8          // Maximum before increasing latency excessively
  adaptation_rate: 0.05        // How quickly buffer adjusts

STATE:
  target_buffer_ticks: float = 3.0
  arrival_jitter_window: RingBuffer<float>(size=60)

ON_SNAPSHOT_RECEIVED(snapshot, arrival_time):
  expected_arrival = last_arrival + tick_interval
  jitter = abs(arrival_time - expected_arrival)
  arrival_jitter_window.push(jitter)

  // Adaptive sizing: buffer must cover P95 jitter
  jitter_p95 = percentile(arrival_jitter_window, 0.95)
  desired_ticks = ceil(jitter_p95 / tick_interval) + 1

  target_buffer_ticks += (desired_ticks - target_buffer_ticks) * adaptation_rate
  target_buffer_ticks = clamp(target_buffer_ticks, min_buffer_ticks, max_buffer_ticks)

  // Impact: higher buffer = smoother playback but increased interpolation delay
  interpolation_delay = target_buffer_ticks * tick_interval
```
