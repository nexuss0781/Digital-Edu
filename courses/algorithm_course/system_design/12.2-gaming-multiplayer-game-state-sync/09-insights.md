...
**Why it matters**: Without edge relays, a 100-player match requires 100 individual UDP sends per tick, each requiring a kernel syscall. Edge relays transform this into ~3 regional sends. The relay duplicates and distributes to local players, which is cheap (stateless packet rewriting). This is functionally identical to a CDN's edge cache pattern, but for real-time UDP streams rather than HTTP responses. The server's networking Slowest part of the process drops by ~97%, and the bonus is DDoS protection — game server IPs are never exposed to clients.
...
**Why it matters**: Individual sendto() syscalls incur ~2-5 µs of kernel context-switch overhead each. At 100 packets per tick at 60 Hz, that's 6,000 syscalls/second consuming up to 30ms/second of pure overhead. The Linux sendmmsg() syscall batches all packets into one context switch. This is a specific instance of a general systems principle: amortize fixed overhead across batch operations. The same principle drives database write batching, network packet coalescing, and GPU draw call batching. In latency-critical systems, the syscall boundary is often the hidden Slowest part of the process.
