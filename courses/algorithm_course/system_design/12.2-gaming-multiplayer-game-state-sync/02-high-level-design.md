...
| **Ordering** | Application manages ordering per-channel | Strict FIFO (First-In-First-Out, like a line at a store) ordering — unnecessary for state updates |
...
| **Telemetry** | Unreliable | Unordered | Client performance metrics, desync reports — sampled and non-blocking |

### Congestion Control Strategy

```
Unlike TCP's general-purpose congestion control, game protocols require
application-aware flow management:

  Approach: Fixed-budget sending (no additive increase / multiplicative decrease)
    - Server allocates a per-client bandwidth budget (e.g., 60 Kbps)
    - Budget is spent each tick by the priority accumulator
    - If budget is exceeded: shed lowest-priority entities, never throttle tick rate

  Packet loss adaptation:
    - If ack gap indicates > 3% loss: reduce entity count per packet
    - If loss > 10%: reduce snapshot send rate (60 → 30 Hz)
    - If loss > 20%: notify client of degraded connection quality

  Key difference from TCP: the server never pauses sending.
  Silence is worse than reduced data — the client needs *something*
  every tick to maintain interpolation.
```
