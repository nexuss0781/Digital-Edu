...
|----------|-------------|
| **What AI decides alone** | DAG parallelization via futures-based compilation, operator resource allocation, dynamic batch sizing, auto-scaling of FBLearner Predictor serving instances |
| **What AI recommends** | Operator resource estimates, workflow optimization suggestions, model performance comparisons across experiments, auto-generated UI configurations |
| **What requires human approval** | Workflow deployment to production, model promotion to Predictor serving, experiment design and launch, resource quota changes across teams, MWFS orchestration policy updates |
| **Deterministic source of truth** | Workflow metadata database (operator state, DAG structure, execution logs) and model artifact store — the platform automates execution; engineers define workflows and control production deployments |
| **Rollback path** | Workflow version rollback via metadata store; Predictor model rollback to previous container version; operator-level retry and replay from checkpoints; MWFS event replay for state reconstruction |
...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, APIs, algorithms (Step-by-step plan in plain English) |
...
- **Database Slowest part of the process**: ~1.7TB monolithic database
...
| System | Relationship |
|--------|-------------|
| [3.4 MLOps Platform](../3.4-mlops-platform/00-index.md) | Generic MLOps concepts that FBLearner pioneered |
| [3.5 Uber Michelangelo](../3.5-uber-michelangelo-ml-platform/00-index.md) | Feature store-centric ML platform; compare Palette vs FBLearner Feature Store |
| [3.6 Netflix Metaflow](../3.6-netflix-metaflow-ml-workflow-platform/00-index.md) | Python-native workflow orchestration; compare decorators vs futures |
| [3.7 Netflix Runway](../3.7-netflix-runway-model-lifecycle/00-index.md) | Model lifecycle management; compare deployment patterns |
| [2.6 Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) | General scheduling and DAG execution patterns underlying FBLearner |
| [2.2 Container Orchestration System](../2.2-container-orchestration-system/00-index.md) | Worker scheduling, GPU resource management, Tupperware parallels |
| [3.9 Airbnb Bighead ML Platform](../3.9-airbnb-bighead-ml-platform/00-index.md) | Alternative ML platform architecture; compare unified vs modular approach |
| [3.24 Multi-Agent Orchestration Platform](../3.24-multi-agent-orchestration-platform/00-index.md) | DAG-based workflow parallels; agent task scheduling patterns |
