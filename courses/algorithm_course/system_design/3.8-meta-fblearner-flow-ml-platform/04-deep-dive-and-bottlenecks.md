...
### Slowest part of the process 1: Monolithic Database (Pre-MWFS)
...
flowchart LR
    ...
    subgraph Slowest part of the process["Single Database"]
...
### Slowest part of the process 2: GPU Queue Starvation
...
### Slowest part of the process 3: Large DAG Compilation
...
### Slowest part of the process 4: Gang Scheduling for Large Model Training (2024-2025)

**Problem:** LLM fine-tuning jobs require 64-1,024+ GPUs allocated simultaneously. Incremental allocation causes deadlocks.

**Impact:**
- Jobs holding partial GPU allocations block each other
- GPU fragmentation wastes capacity across the cluster
- Large training runs may wait days in queue

**Mitigation:**

```
ALGORITHM GangScheduler(job, gpu_pool)
    required_gpus = job.resource_requirements.gpus

    // All-or-nothing check
    available = find_contiguous_gpus(gpu_pool, required_gpus)

    IF available.count >= required_gpus:
        // Allocate all at once
        allocate_all(available[:required_gpus], job)
        RETURN SCHEDULED
    ELSE:
        // Cannot partially allocate -- queue the job
        // Meanwhile, backfill smaller jobs into remaining capacity
        backfill_candidates = queue.jobs_fitting(gpu_pool.free_capacity)
        FOR candidate IN backfill_candidates:
            schedule_backfill(candidate, gpu_pool)

        RETURN QUEUED(estimated_wait=estimate_drain_time(gpu_pool, required_gpus))


    // Failure handling mid-training
    ON gpu_failure(failed_gpu, job):
        IF job.supports_elastic_training:
            replace_gpu(failed_gpu, job)
        ELSE:
            // Must restart from checkpoint with full allocation
            release_all_gpus(job)
            save_checkpoint(job)
            re_queue_with_priority_boost(job)
```

### Slowest part of the process 5: Checkpoint Storage for Large Models

**Problem:** Large model checkpoints (70B parameters = ~140GB per checkpoint) dominate storage costs during long training runs.

**Impact:**
- 10,000-step training with 500-step intervals = 20 checkpoints = 2.8TB per run
- Thousands of concurrent fine-tuning runs = PB-scale checkpoint storage
- Checkpoint writes compete with training I/O bandwidth
...
1. Tiered storage
   ├── Hot (< 48h):   High-performance distributed storage
   ├── Warm (2-7d):   Standard object storage
   └── Cold (> 7d):   Archive storage (80% cost reduction)

2. Incremental checkpoints
   ├── Full checkpoint every 5,000 steps
   └── Delta checkpoints every 500 steps (only changed parameters)
   └── Reduces storage by ~60% for fine-tuning (few params change)

3. Async checkpoint writes
   ├── Overlap checkpoint I/O with next training step
   └── Prevents GPU idle time during checkpoint save
   └── Requires double memory buffer (trade memory for throughput)
```
...
| Slowest part of the process | Severity | Root Cause | Solution | Status |
|------------|----------|------------|----------|--------|
...
| Gang scheduling deadlocks | High | Partial GPU allocation | All-or-nothing + backfill | Active (2024-2025) |
| Checkpoint storage | High | Large model checkpoint volume | Tiered storage, incremental checkpoints | Active (2024-2025) |
