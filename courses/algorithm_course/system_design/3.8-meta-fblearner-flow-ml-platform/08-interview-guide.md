...
| **Scaling** | Limited (1.7TB DB Slowest part of the process) | Horizontal (sharded) |
...
## Case Study: LLM Fine-Tuning at Scale (2024-2025)

### Scenario

Meta needs to fine-tune LLaMA models across 350K+ H100 GPUs for product teams building customized models for Ads ranking, content recommendation, and safety classification.

### Architecture Decisions

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Workflow pattern** | Futures-based DAG with MWFS orchestration | Fine-tuning has natural stages (data prep → training → eval → deploy) |
| **Resource allocation** | Priority-based with preemption | LLaMA fine-tuning can consume 1,000+ GPUs per run |
| **Checkpointing** | Every 500 steps (configurable by model size) | H100 failures are frequent at 350K scale; checkpoint cost < restart cost |
| **Model registry** | Versioned with lineage tracking | Must trace which base model, dataset version, and hyperparameters produced each fine-tuned model |
| **Serving** | FBLearner Predictor with A/B testing | Fine-tuned models need canary deployment before full rollout |

### Key Challenges

```
Challenge 1: GPU Cluster Fragmentation
  Problem: 1,024-GPU training runs need contiguous allocation
  Solution: Gang scheduling -- all-or-nothing allocation with backfill for small jobs

Challenge 2: Training Data Versioning
  Problem: Fine-tuning results depend on exact dataset version
  Solution: Immutable dataset snapshots with content-addressed storage

Challenge 3: Cost Attribution
  Problem: A single fine-tuning run can cost $100K+ in compute
  Solution: Per-team budgets with real-time cost tracking in FBLearner UI

Challenge 4: Reproducibility
  Problem: Fine-tuning results must be exactly reproducible for compliance
  Solution: Sealed workflow definitions capturing code, data, config, and random seeds
```
...
| Metric | Target | Why |
|--------|--------|-----|
| GPU utilization | > 85% | At 350K GPUs, 1% waste = $10M+ annually |
| Checkpoint success rate | > 99.9% | Failed checkpoints mean wasted training |
| Training-to-serving gap | < 2 hours | Time from trained model to production serving |
| Experiment turnaround | < 24 hours | Iteration speed for product teams |

### Lessons Learned

1. **Gang scheduling is essential for large model training**: Allocating GPUs incrementally leads to jobs holding partial allocations indefinitely. All-or-nothing with a timeout prevents deadlocks.
2. **Checkpoint storage dominates costs for large models**: A 70B parameter model checkpoint is ~140GB. At 500-step intervals over a 10,000-step run, that's 2.8TB of checkpoints per run. Tiered storage (hot → cold after 48h) reduces costs 70%.
3. **Auto UI generation extends naturally to experiment tracking**: The same type system that generates launch forms also generates comparison dashboards for hyperparameter sweeps.
