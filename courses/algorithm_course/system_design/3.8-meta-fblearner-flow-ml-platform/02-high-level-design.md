...
| **Horizontally Scalable** | SQL + distributed queues | No single point of Slowest part of the process |
...
## LLM-Era Platform Adaptations (2024-2026)

The emergence of large language models has driven significant adaptations to FBLearner's architecture:

### Infrastructure Scale Shift

| Era | Primary Workload | Typical GPU Need | Training Duration | Key Challenge |
|-----|------------------|------------------|-------------------|---------------|
| **2016-2022** | Ranking/Recommendation | 1-8 GPUs | Minutes-hours | Feature engineering |
| **2023-2024** | LLM pre-training | 1,000-16,000 GPUs | Weeks-months | GPU cluster coordination |
| **2024-2025** | LLM fine-tuning | 8-1,024 GPUs | Hours-days | Resource allocation fairness |
| **2025-2026** | Inference optimization | Variable | N/A | Quantization, distillation |

### New Workflow Patterns

```
Traditional ML Workflow (2016-2022):
  Data Prep → Feature Engineering → Train → Evaluate → Deploy
  Duration: Minutes to hours
  Resources: 1-8 GPUs

LLM Fine-Tuning Workflow (2024-2026):
  Base Model Selection → Data Curation → LoRA/QLoRA Adapter Training
  → Merged Model Evaluation → Safety Evaluation → Canary Deployment
  Duration: Hours to days
  Resources: 8-1,024 GPUs
  New constraints: Gang scheduling, checkpoint management, cost tracking

LLM Inference Optimization Workflow (2025-2026):
  Trained Model → Quantization (INT8/INT4) → Distillation
  → Speculative Decoding Setup → Latency Benchmarking → Deploy
  Duration: Hours
  Resources: Variable (CPU for quantization, GPU for benchmarking)
```

### Platform Comparison (Updated for LLM Era)

| Capability | FBLearner Flow | Metaflow | Kubeflow | Ray/Anyscale |
|------------|----------------|----------|----------|--------------|
| **Traditional ML** | Excellent | Excellent | Good | Good |
| **LLM Fine-tuning** | Excellent (internal) | Good (w/ Ray) | Limited | Excellent |
| **Multi-GPU Training** | Native (350K GPUs) | Via plugins | Via operators | Native |
| **Gang Scheduling** | Yes (MWFS) | No | Limited | Yes |
| **Cost Tracking** | Per-team budgets | Basic | Basic | Per-job |
| **Auto UI** | From type system | Cards (manual) | Pipeline UI | Dashboard |
