...
The defining Rule that never changes of a digital wallet system is: **the ledger must always balance**. Every financial operation is recorded as a journal entry containing one or more debit entries and one or more credit entries, where the total debits exactly equal the total credits. This is not merely a best practice---it is the fundamental mechanism that prevents money from being created or destroyed within the system.

```
Rule that never changes: For every journal entry J:
  SUM(debit_entries in J) = SUM(credit_entries in J)

Global Rule that never changes: Across the entire ledger:
  SUM(all debits) = SUM(all credits)

If either Rule that never changes is violated → the system has a bug that may be creating or losing money.
```
...
A "hot wallet" is a wallet that receives an extreme number of concurrent transactions, creating a database row-level lock Slowest part of the process. In a digital wallet system, this primarily affects:
...
### The Contention Slowest part of the process
...
## Deep Dive 6: Promotional Balance and Cashback Lifecycle

### Promotional Balance Architecture

```
A digital wallet typically manages three balance types per currency:

Available Balance:   Withdrawable, transferable, spendable (real money)
Held Balance:        Reserved for pending transactions (not spendable)
Promotional Balance: Non-withdrawable cashback (spendable at merchants only)

Spending Priority (when user pays $30):
  1. Use promotional balance first: min(promotional, amount) → $5 promo used
  2. Remainder from available balance: $30 - $5 = $25 from available

This order maximizes promotional spend (platform wants promo used)
and preserves real balance (user prefers real money kept longer).
```

### Cashback Crediting

```
Journal Entry for Cashback Award ($5 cashback on $50 merchant payment):

Journal J-CB-001: Cashback Credit
┌─────────────────────────────┬──────────┬──────────┐
│ Account                     │ Debit    │ Credit   │
├─────────────────────────────┼──────────┼──────────┤
│ Platform Promotional Expense│ $5.00    │          │
│ User Promotional Wallet     │          │ $5.00    │
├─────────────────────────────┼──────────┼──────────┤
│ Total                       │ $5.00    │ $5.00    │
└─────────────────────────────┴──────────┴──────────┘

Cashback rules:
  - Credited to promotional balance (cannot withdraw to bank)
  - Expires after 90 days (expired promo reduces liability)
  - Cannot be transferred P2P (prevents gaming)
  - Usable only for merchant payments
  - Budget cap per campaign (stop crediting when campaign budget exhausted)
```

### Promotional Balance Expiry

```
FUNCTION expirePromotionalBalances():
    // Runs daily at midnight
    expiredEntries = db.query(
        "SELECT wallet_id, SUM(amount_cents) as expired_total
    ...
    FOR EACH entry IN expiredEntries:
        BEGIN TRANSACTION
            db.execute(
                "UPDATE wallet_balances SET promotional_cents = promotional_cents - ?
                 WHERE wallet_id = ?",
                entry.expired_total, entry.wallet_id
            )
            db.execute(
                "INSERT INTO ledger_entries (...) VALUES (...)",
                journalId, entry.wallet_id, "DEBIT", entry.expired_total
            )
            db.execute(
                "INSERT INTO ledger_entries (...) VALUES (...)",
                journalId, "PLATFORM_PROMO_EXPIRY_RECOVERY", "CREDIT", entry.expired_total
            )
            db.execute(
                "UPDATE promotional_credits SET status = 'EXPIRED'
                 WHERE wallet_id = ? AND status = 'ACTIVE' AND expires_at < NOW()",
                entry.wallet_id
            )
        COMMIT

    // Platform recovers expired promotional liability → reduces expense
```
...
### Slowest part of the process 1: Ledger Write Throughput
...
### Slowest part of the process 2: Per-Wallet Lock Contention
...
### Slowest part of the process 3: Cross-Shard P2P Transfers
...
### Slowest part of the process 4: Fraud Detection Latency
...
### Slowest part of the process 5: Reconciliation at Scale
