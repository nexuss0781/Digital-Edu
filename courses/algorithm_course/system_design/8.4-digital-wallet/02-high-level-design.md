...
```mermaid
flowchart TB
    subgraph Intraday["Intraday: Individual Payments"]
        PAY[Customer Payment<br/>Debit User · Credit Merchant]
        FEE[Fee Extraction<br/>Credit Platform Fee Account]
        HOLD[Optional Hold<br/>Dispute Window: 24-72h]
    end

    subgraph EOD["End of Day: Settlement Calculation"]
        AGG[Aggregate Net Settlement<br/>SUM credits - SUM debits - holds]
        RECON[Reconciliation Check<br/>Verify against transaction log]
        APPROVE[Settlement Approval<br/>Auto or manual based on amount]
    end

    subgraph SETTLE["T+1: Bank Settlement"]
        INIT[Initiate Bank Transfer<br/>Debit Escrow Account]
        BANK[Bank Processing<br/>ACH / Wire / RTGS]
        CONFIRM[Settlement Confirmed<br/>Mark settled in ledger]
    end

    PAY --> FEE --> HOLD
    HOLD --> AGG --> RECON --> APPROVE
    APPROVE --> INIT --> BANK --> CONFIRM

    classDef intraday fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef eod fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef settle fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class PAY,FEE,HOLD intraday
    class AGG,RECON,APPROVE eod
    class INIT,BANK,CONFIRM settle
```

### Settlement Ledger Entries

```
End-of-Day Settlement for Merchant M (net $2,450 after fees):

Journal J-SETTLE-001: Merchant Daily Settlement
┌─────────────────────────┬──────────┬──────────┐
│ Account                 │ Debit    │ Credit   │
├─────────────────────────┼──────────┼──────────┤
│ Merchant M Wallet       │ $2,450   │          │
│ Escrow Bank Account     │          │ $2,450   │
├─────────────────────────┼──────────┼──────────┤
│ Total                   │ $2,450   │ $2,450   │ ← Balanced
└─────────────────────────┴──────────┴──────────┘

The merchant wallet balance decreases, and the escrow bank initiates
an actual bank transfer to the merchant's linked bank account.
After bank confirmation, escrow balance reconciles automatically.
```
...
### Decision: Pessimistic vs. Optimistic Locking for Balance Operations

```
                Pessimistic (FOR UPDATE)      Optimistic (Version CAS)
────────────────────────────────────────────────────────────────────────
Lock behavior   Blocks concurrent readers     No blocking; fails on stale
Retry needed    No (waits for lock)           Yes (re-read + retry on CAS fail)
Throughput      Limited by lock hold time     Higher under low contention
Contention      Handles high contention       Degrades with retries under load
Complexity      Simpler (DB manages waits)    App-level retry logic needed
Deadlock risk   Yes (mitigated by lock order) No deadlocks possible
Best for        Hot wallets, guarantees       Cold wallets with rare conflicts

Recommendation: Pessimistic for the default path.
  - Lock duration is 3-5ms (very short)
  - Most wallets see < 5 TPS (no meaningful contention)
  - Simpler code, no retry logic, deterministic behavior
  - For hot merchant wallets, sub-wallet sharding avoids the
    contention problem entirely rather than switching strategies
```

### Decision: Single Escrow Bank vs. Multi-Bank Float

```
Single Escrow Bank:
  + Simpler reconciliation (one bank statement to match)
  + Single contractual relationship
  - Single point of failure (bank outage = no top-ups or withdrawals)
  - Regulatory risk (bank license revocation)

Multi-Bank Float:
  + Redundancy (if Bank A is down, route to Bank B)
  + Better negotiation leverage on fees
  + Regulatory diversification
  - Complex reconciliation (multiple bank statements)
  - Float management across banks (rebalancing)
  - Each bank needs separate escrow accounting

Recommendation: Start with single escrow, add second bank once
volume exceeds $1B monthly or regulatory requirements demand it.
Multi-bank adds operational complexity that is justified only at scale.
```
