...
| **Hot Wallet Pattern** | Popular merchants and system accounts require sub-wallet sharding to distribute write contention across multiple rows |
| **Settlement Lifecycle** | Intraday credits accumulate → end-of-day net calculation → T+1 bank settlement → reconciliation → holdback for disputes |
...
|-----------|--------|--------|
| **Programmable Money (CBDC)** | Central Bank Digital Currencies (digital yuan, digital euro) integrate directly with wallet ledgers; new settlement rail alongside bank transfers | Pilot → Production |
| **Account Abstraction (ERC-4337)** | Gasless wallet UX for crypto-native wallets; social recovery replaces seed phrases; session keys for streaming payments | Production |
| **Real-Time Gross Settlement (RTGS) APIs** | Instant settlement replaces T+1 batch; reduces escrow float requirements; enables instant withdrawal | Expanding |
| **Embedded Finance APIs** | Wallet-as-a-Service (BaaS) enables any app to embed wallet functionality; shifts from standalone wallet apps to platform-embedded wallets | Production |
| **AI-Driven Fraud Prevention** | Graph neural networks model transaction networks; transformer-based anomaly detection; real-time synthetic identity detection | Production |
| **Open Banking / PSD3** | Standardized APIs for bank account access; consent-driven data sharing; enables seamless top-up without card networks | EU: Production; Global: Expanding |
| **Passkey Authentication** | FIDO2/WebAuthn replaces OTP and PIN; phishing-resistant; device-bound credentials; cross-device sync | Production |
| **Offline Payments** | NFC-based offline payment protocols (e.g., CBDC offline mode); store-and-forward transactions for low-connectivity regions | Pilot |
...
|---------|--------|-----------|
| **Double-Entry Ledger Consistency** | [8.6 - Distributed Ledger Core Banking](../8.6-distributed-ledger-core-banking/) | Same ledger Rule that never changes (debits = credits) but at core banking scale with general ledger / sub-ledger hierarchy |
| **Saga-Based Distributed Transactions** | [8.2 - Stripe/Razorpay](../8.2-stripe-razorpay/) | Payment orchestration uses saga pattern for multi-party settlement; same compensation mechanics as cross-shard P2P |
| **Hot Partition / Write Contention** | [8.7 - Cryptocurrency Exchange](../8.7-cryptocurrency-exchange/) | Order book hot symbols share the same row-contention problem as hot merchant wallets; solved with sharded counters |
| **Tiered KYC / Progressive Verification** | [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Same progressive identity verification pattern; credit assessment adds affordability check on top of KYC |
| **Real-Time Fraud Scoring** | [8.5 - Fraud Detection System](../8.5-fraud-detection-system/) | Shared feature engineering (velocity, device, behavioral); digital wallet embeds fraud scoring inline rather than as a sidecar |
| **Idempotent Financial Operations** | [8.3 - Zerodha](../8.3-zerodha/) | Stock trading uses same idempotency patterns for order placement; both require exactly-once semantics for financial operations |
| **Event-Driven Audit Trail** | [1.5 - Distributed Log-Based Broker](../1.5-distributed-log-based-broker/) | Kafka-backed immutable event log underpins both transaction audit trails and CDC for analytics pipelines |
| **Multi-Channel Notification** | [4.1 - Notification System](../4.1-notification-system/) | Transaction notifications across push, SMS, email with delivery guarantees and user preference routing |
...
| [8.5 - Fraud Detection System](../8.5-fraud-detection-system/) | Real-time fraud scoring pipeline, feature engineering, ML inference |
| [8.6 - Distributed Ledger Core Banking](../8.6-distributed-ledger-core-banking/) | Core banking ledger patterns, GL/SL hierarchy, regulatory reporting |
| [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Credit-linked wallet patterns, tiered KYC, affordability assessment |
...
- Bank for International Settlements --- CBDC Design Considerations for Retail Payments
- FIDO Alliance --- Passkey Authentication for Financial Services
- Federal Reserve --- FedNow Instant Payment Service Design Principles
- European Central Bank --- Digital Euro Design Principles and Architecture
- Financial Stability Board --- Regulation of Crypto-Asset Activities and Stablecoins
- World Bank --- Global Findex Database: Financial Inclusion Insights
