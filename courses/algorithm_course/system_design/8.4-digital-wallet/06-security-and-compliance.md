...
Tamper evidence:
  - Each audit record includes: SHA-256(previous_record_hash + current_record_data)
  - Periodic Merkle root publication to an immutable store
  - Any gap or modification in the chain breaks the hash sequence → detectable
  - Audit records are write-only: no service account has DELETE or UPDATE permission
