...
**Quick touch on:**
- Security: Brute Force (Checking every single possibility) protection, credential stuffing detection
...
### "How do you prevent a Brute Force (Checking every single possibility) attack?"
...
| Ignoring security | Incomplete design | Mention Brute Force (Checking every single possibility), token security, audit |
...
**Wrap-up:**
- [ ] Security: Brute Force (Checking every single possibility), credential stuffing
- [ ] Compliance: audit logging, data residency
...
## Emerging Interview Questions (2025-2026)

### Q1: "How would you design a passwordless authentication system using passkeys?"

**What they're testing:** Understanding of WebAuthn/FIDO2, platform authenticators vs. synced passkeys, and the security-UX trade-offs.

**Strong answer framework:**

1. **Registration ceremony:** User creates a credential — the authenticator generates a public/private key pair, the private key stays on the device (or syncs via cloud keychain), and the public key is registered with the server. No shared secret ever leaves the device.

2. **Authentication ceremony:** Server sends a challenge, the authenticator signs it with the private key bound to the site's origin, and the server verifies against the stored public key. Phishing-resistant because the origin is embedded in the signature — a fake site gets a different origin.

3. **Synced passkeys vs. device-bound:** Synced passkeys (iCloud Keychain, Google Password Manager) solve the multi-device problem but shift trust to cloud provider. Device-bound keys (hardware security keys) provide stronger guarantees but no recovery if lost.

4. **Migration strategy:** Offer passkeys alongside passwords, prompt users to create passkeys at login, track adoption rate. Don't remove password fallback until >90% adoption in the tenant.

### Q2: "How does OAuth 2.1 differ from OAuth 2.0, and why do these changes matter?"

**What they're testing:** Awareness of modern security standards and ability to reason about protocol evolution.

**Key points:**
- PKCE mandatory for all clients (not just public clients) — prevents authorization code interception even for confidential clients
- Implicit and ROPC grants removed — these were the source of most OAuth security incidents
- DPoP tokens bind access tokens to a client's key pair — stolen tokens are useless without the private key
- Refresh token rotation required — every refresh produces a new token, enabling reuse detection
- Exact redirect URI matching required — prevents open redirect attacks

### Q3: "A customer reports that users are being locked out despite using correct passwords. How do you investigate?"

**What they're testing:** ITDR thinking, understanding of identity attack patterns, and graduated response design.

**Investigation framework:**
1. Check for MFA fatigue attacks (attacker has password, bombarding user with push notifications)
2. Check for credential stuffing triggering rate limiting on the customer's IP range (shared corporate egress IP)
3. Check for token theft causing session anomaly detection to trigger step-up auth
4. Check for bot detection false positives on legitimate traffic patterns
5. Review whether a security stamp invalidation cascaded (admin changed a shared policy)

**Key insight:** Most "lockout" incidents are either (a) a security feature correctly responding to an actual attack, or (b) a false positive from overly aggressive risk scoring. The investigation must determine which before changing thresholds.

### Q4: "How would you implement fine-grained authorization for a multi-tenant document sharing platform?"

**What they're testing:** ReBAC vs. RBAC trade-offs, Zanzibar-style relationship tuples, and graph traversal optimization.

**Strong answer:** Start with the relationship model — `user:alice` is `editor` of `document:doc1`, `document:doc1` is `in` `folder:engineering`, `folder:engineering` is `owned_by` `org:acme`. Permission checks traverse this graph: can Alice read doc1? Direct check → then check folder membership → then check org-level defaults. Cache computed permissions aggressively (decision cache), bound traversal depth (5 hops max), and use materialized views for hot paths (e.g., "all documents Alice can access" computed at login time and updated incrementally via change events).
...
| Topic | Connection |
|-------|------------|
| [1.14 API Gateway Design](../1.14-api-gateway-design/00-index.md) | Token validation at the gateway level, rate limiting per identity |
| [2.10 Zero Trust Security](../2.10-zero-trust-security-architecture/00-index.md) | IAM as the foundation — every request authenticated and authorized |
| [2.11 Service Mesh](../2.11-service-mesh-design/00-index.md) | mTLS and service-to-service authorization |
| [2.16 Secret Management](../2.16-secret-management-system/00-index.md) | HSM-backed signing keys, dynamic secrets |
| [15.7 AI-Native Cybersecurity](../15.7-ai-native-cybersecurity-platform/00-index.md) | ITDR integration for identity threat detection |
