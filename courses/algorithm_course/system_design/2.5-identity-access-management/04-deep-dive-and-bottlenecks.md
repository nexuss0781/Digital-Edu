...
### Slowest part of the process Analysis: Policy Evaluation

| Slowest part of the process | Symptom | Cause | Solution |
|------------|---------|-------|----------|
...
### Slowest part of the process Analysis: Session Management
...
### Slowest part of the process Analysis: Token Management
...
| **Token introspection** | Network Slowest part of the process | Every request calls introspection | Cache introspection results, use JWTs |
...
### Slowest part of the process Analysis: MFA
...
## Deep Dive 5: OAuth 2.1 and Modern Token Security

### OAuth 2.1 Consolidation

OAuth 2.1 (draft finalized 2025) consolidates OAuth 2.0 security best practices into a single specification, removing insecure patterns:

| Change | OAuth 2.0 | OAuth 2.1 | Why |
|--------|-----------|-----------|-----|
| **PKCE** | Optional (recommended) | Mandatory for all clients | Prevents authorization code interception |
| **Implicit Grant** | Allowed | Removed | Tokens exposed in URL fragments, no refresh tokens |
| **ROPC Grant** | Allowed | Removed | Exposes credentials to client, incompatible with MFA |
| **Refresh Token Rotation** | Optional | Required | Detects token theft via reuse detection |
| **Bearer Token in URI** | Allowed | Prohibited | Tokens leaked in server logs, referrer headers |
| **Exact Redirect URI** | Recommended | Required | Prevents open redirect attacks |

### DPoP (Demonstration of Proof-of-Possession) Tokens

```
FUNCTION create_dpop_proof(method: String, uri: String, private_key: Key) -> String:
    """
    Create a DPoP proof JWT that binds an access token to the client's key pair.
    Unlike bearer tokens, DPoP tokens are useless if stolen -- the attacker
    cannot produce valid proofs without the private key.
    """
    header = {
        typ: "dpop+jwt",
        alg: "ES256",
        jwk: private_key.public_key()    // Include public key for verification
    }
    ...
    payload = {
        ...
        htu: uri,                          // Target URI
        iat: now(),                        // Issued at
        nonce: server_provided_nonce       // Server-managed replay prevention
    }

    RETURN sign_jwt(header, payload, private_key)

FUNCTION validate_dpop_bound_token(access_token: JWT, dpop_proof: JWT) -> Boolean:
    """
    Validate that the access token is bound to the DPoP proof's key.
    """
    // 1. Verify DPoP proof signature using embedded public key
    IF NOT verify_jwt_signature(dpop_proof, dpop_proof.header.jwk):
        RETURN FALSE

    // 2. Check proof is recent (prevent replay)
    IF abs(now() - dpop_proof.payload.iat) > 60_seconds:
        RETURN FALSE

    // 3. Verify method and URI match the current request
    IF dpop_proof.payload.htm != request.method:
        RETURN FALSE
    IF dpop_proof.payload.htu != request.uri:
        RETURN FALSE

    // 4. Verify token binding -- access token's cnf.jkt must match proof's key
    token_thumbprint = access_token.cnf.jkt
    proof_thumbprint = compute_jwk_thumbprint(dpop_proof.header.jwk)

    RETURN token_thumbprint == proof_thumbprint
```

### Slowest part of the process Analysis: Token Security

| Slowest part of the process | Symptom | Cause | Solution |
|------------|---------|-------|----------|
| **DPoP overhead** | Increased latency per request | Asymmetric key operations for every API call | Cache proof validation, use efficient curves (Ed25519) |
| **PKCE code_verifier storage** | Memory pressure on auth server | Storing verifiers for pending auth codes | Short TTL (10 min), clean up on code exchange |
| **Refresh rotation race** | False-positive family revocation | Network retry sends old refresh token | Grace period (30s) for duplicate refresh requests |

---

## Deep Dive 6: Identity Threat Detection and Response (ITDR)

### ITDR Architecture

Traditional SIEM systems detect infrastructure-level threats. ITDR adds a purpose-built identity layer that detects credential-based attacks, session hijacking, and privilege escalation patterns that generic security tools miss.

```
FUNCTION evaluate_identity_threat(event: AuthEvent) -> ThreatAssessment:
    """
    Multi-signal identity threat detection combining
    real-time heuristics with ML-based anomaly detection.
    """
    signals = []

    // Signal 1: Credential stuffing detection
    // Look for distributed attacks across many accounts from few IPs
    ip_failure_rate = get_failure_rate_by_ip(event.ip, window=5_minutes)
    IF ip_failure_rate > threshold.credential_stuffing:
        signals.append(Signal("credential_stuffing", severity=HIGH,
            evidence=f"IP {event.ip} failed {ip_failure_rate} logins in 5min"))

    // Signal 2: Password spray detection
    // Many accounts, few passwords per account, distributed IPs
    account_attempt_pattern = analyze_attempt_distribution(
        window=15_minutes, min_accounts=50)
    IF account_attempt_pattern.is_spray:
        signals.append(Signal("password_spray", severity=HIGH,
            evidence=f"Spray pattern: {account_attempt_pattern.summary}"))

    // Signal 3: Token abuse detection
    // Same token used from multiple distinct networks simultaneously
    IF event.type == "token_validation":
        concurrent_locations = get_token_usage_locations(
            event.token_id, window=1_minute)
        IF len(concurrent_locations) > 1:
            IF any_impossible_travel(concurrent_locations):
                signals.append(Signal("token_theft", severity=CRITICAL,
                    evidence=f"Token used from {concurrent_locations}"))

    // Signal 4: Privilege escalation patterns
    // User gains permissions inconsistent with normal behavior
    IF event.type == "permission_change":
        baseline = get_user_permission_baseline(event.user_id)
        IF event.new_permissions.exceeds(baseline, threshold=3_standard_deviations):
            signals.append(Signal("privilege_escalation", severity=HIGH,
                evidence=f"Anomalous permission grant: {event.new_permissions}"))

    // Signal 5: MFA fatigue (push bombing)
    // Attacker repeatedly triggers MFA prompts hoping user approves
    IF event.type == "mfa_prompt":
        recent_prompts = count_mfa_prompts(event.user_id, window=10_minutes)
        IF recent_prompts > 5:
            signals.append(Signal("mfa_fatigue", severity=HIGH,
                evidence=f"{recent_prompts} MFA prompts in 10min"))

    // Aggregate and respond
    threat = aggregate_signals(signals)

    IF threat.severity >= CRITICAL:
        auto_respond(event.user_id, action="lock_and_revoke")
    ELIF threat.severity >= HIGH:
        auto_respond(event.user_id, action="step_up_auth")

    RETURN threat
```

### Case Study: Financial Services Platform Identity Attack

**Context:** A mid-sized fintech platform (2M users, 500K daily active) experienced a coordinated credential stuffing attack during peak trading hours.

**Attack Timeline and Response:**

| Time | Event | ITDR Response |
|------|-------|---------------|
| T+0 | Failure rate spikes: 15K failed logins/min from 200 IPs | Credential stuffing signal fires (severity: HIGH) |
| T+30s | Pattern analysis confirms: 3 passwords tried per account, rotating across accounts | Password spray signal upgrades severity to CRITICAL |
| T+45s | Automated response: rate limiting tightened to 2 attempts/min per account, CAPTCHA enforced | 95% of attack traffic blocked |
| T+2min | 12 accounts compromised before detection (credentials reused from other breaches) | Session anomaly detection catches 10 of 12 via new-device signal |
| T+3min | Compromised accounts locked, sessions revoked, forced password reset | Mean time to containment: 3 minutes |
| T+10min | Threat intel feeds updated with attacker IP ranges | Future attacks from same infrastructure pre-blocked |

**Key Metrics:**
- **Without ITDR:** Estimated 500+ account compromises over 30+ minutes before manual detection
- **With ITDR:** 12 compromises in 45 seconds, 10 auto-contained, 2 caught on next login
- **False positive rate:** 0.3% (legitimate users affected by rate limiting during attack window)

### Slowest part of the process Analysis: ITDR

| Slowest part of the process | Symptom | Cause | Solution |
|------------|---------|-------|----------|
| **Signal correlation latency** | Delayed threat detection | Cross-service event aggregation | Stream processing (event bus), not batch |
| **False positives** | Legitimate users locked out | Aggressive thresholds | Per-user behavioral baselines, graduated responses |
| **MFA fatigue detection** | Delayed recognition | Low prompt count threshold | Combine with location/device signals, not just count |
| **Credential breach databases** | Stale breach data | Infrequent updates | Real-time breach notification integration (Have I Been Pwned API) |
