...
**Category:** Caching
**One-liner:** A three-tier policy cache (L1 in-process at <1ms, L2 distributed Redis at <5ms, L3 database at <50ms) ensures that the vast majority of authorization decisions are made without any network call, turning a potential Slowest part of the process into an O(1) memory lookup.
...
## Insight 14: Passkeys Eliminate Phishing by Binding Authentication to Origin, Not User Memory

**Category:** Security
**One-liner:** WebAuthn/passkeys bind the cryptographic credential to the relying party's origin (domain), making it physically impossible for a phishing site to obtain a valid authentication response -- the authenticator simply refuses to sign challenges for the wrong domain.

**Why it matters:** Passwords are phishable because the security relies on the user recognizing the correct site. Passkeys shift this trust from the user to the protocol: during registration, the authenticator creates a key pair bound to the exact origin (e.g., `login.example.com`). During authentication, the browser includes the origin in the data signed by the authenticator. A phishing site at `login-example.com` receives an authentication response bound to the phishing domain, which the legitimate server rejects. The user does not need to notice the URL difference -- the cryptography handles it. Synced passkeys (stored in iCloud Keychain, Google Password Manager) solve the multi-device problem that previously limited WebAuthn adoption, but introduce a dependency on cloud provider security. The platform design trade-off is between synced passkeys (better UX, dependent on cloud provider) and device-bound credentials (stronger security, single-device limitation). Most production systems support both and let the tenant's security policy decide.

---

## Insight 15: Identity Threat Detection and Response (ITDR) Catches Attacks That Perimeter Security Misses

**Category:** Security
**One-liner:** ITDR systems detect identity-layer attacks (credential stuffing, MFA fatigue, token theft, privilege escalation) by correlating authentication events across time and users -- catching patterns invisible to per-request inspection or traditional SIEM.

**Why it matters:** Over 80% of breaches involve compromised credentials, yet traditional security infrastructure (firewalls, WAFs, SIEM) focuses on network and application layers. A credential stuffing attack -- distributing login attempts across thousands of IPs, each making only 2-3 attempts -- stays below per-IP rate limits and looks like normal failed logins. ITDR detects this by aggregating failure patterns across accounts: 50 accounts each getting 3 attempts with the same 3 passwords in 15 minutes is a spray attack, regardless of IP distribution. Similarly, MFA fatigue (sending repeated push notifications hoping the user approves one) is invisible without cross-event correlation: 8 MFA prompts for the same user in 10 minutes, none from the user's usual device, is an attack pattern. The architectural insight is that identity security requires a dedicated correlation engine operating on authentication event streams, not just log analysis after the fact.
