...
> **Tier B — AI-Augmented:** AI capabilities generate suggestions (schema designs, formulas, workflows, queries) that administrators review and approve before activation. No AI output modifies production metadata, permissions, or tenant configurations without explicit human confirmation.

| Decision | AI Role | Human Role | Override | Escalation |
|----------|---------|------------|----------|------------|
| Schema design from natural language | Generates object/field/relationship definitions from NL description | Admin reviews schema, modifies as needed, approves deployment | Admin can reject entirely or edit any generated element | Escalate to platform architect if schema conflicts with existing objects |
| Formula generation | Translates NL business rules to formula expressions with AST validation | Admin reviews generated formula, tests in sandbox, activates | Admin can manually write formula; AI suggestion is optional | Escalate if formula references cross-object fields requiring security review |
| Workflow automation proposals | Suggests triggers, conditions, and actions from NL process description | Admin reviews workflow logic, tests execution path, activates | Admin designs workflow manually; AI proposal is a starting point | Escalate to tenant admin if workflow modifies record ownership or sharing |
| Permission configuration | Recommends OWD, sharing rules, and FLS based on described access patterns | Security admin reviews, validates against compliance requirements, applies | Security admin has sole authority over permission changes | Escalate to compliance officer if recommended permissions widen access |
| Natural language query (SOQL generation) | Translates NL to SOQL-equivalent query, executes against data | User reviews query results; sensitive queries require admin approval | User can refine query; admin can restrict NL query access per profile | Escalate if query scans >100K records or accesses restricted objects |
| Document processing & field extraction | Extracts field values from uploaded documents via OCR + LLM | User reviews extracted values before record creation/update | User corrects any extraction error; no auto-commit to records | Escalate if extraction confidence <70% on any field |
...
- [Identity & Access Management (IAM)](../2.5-identity-access-management/00-index.md) - AuthN/AuthZ patterns, multi-layer permission models
- [Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) - Async workflow execution, batch processing
- [Feature Flag Management](../2.7-feature-flag-management/00-index.md) - Declarative configuration patterns, tenant-scoped toggles
- [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) - Audit trails, history tracking, metadata changelog
- [CQRS Implementation](../1.19-cqrs-implementation/00-index.md) - Read/write separation for metadata vs data planes
- [AI Native Cloud ERP SaaS](../2.18-ai-native-cloud-erp-saas/00-index.md) - AI-native enterprise patterns, domain-specific metadata
- [LLM Gateway & Prompt Management](../3.21-llm-gateway-prompt-management/00-index.md) - Prompt registry, model routing for AI formula generation
- [AI Guardrails & Safety System](../3.22-ai-guardrails-safety-system/00-index.md) - Output validation pipelines for AI-generated metadata
