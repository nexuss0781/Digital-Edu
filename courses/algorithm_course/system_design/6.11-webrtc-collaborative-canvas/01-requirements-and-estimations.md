...
### CRDT tombstone growth for active boards

```
Given:
- Active board: 5,000 live objects
- Average object lifetime: 2 weeks (created, moved, eventually deleted)
- Board active for 6 months: ~60K create + ~55K delete = ~55K tombstones
- Tombstone size: ~100 bytes (ID + timestamp + vector clock)
- Tombstone overhead: 55K x 100 = 5.5 MB
- After GC (coordinated every 7 days): reduce to ~8K tombstones = 800 KB
- Without GC: tombstones grow unbounded, degrading merge performance
```
...
|---|---|---|
| **Browser memory limits** | Chrome tabs limited to ~4 GB; large boards with 50K+ objects exhaust memory | Progressive loading by spatial chunks; evict off-screen object details |
| **IndexedDB storage limits** | Varies by browser: 50–80% of available disk for offline storage | Compress CRDT state; evict stale offline boards after 30 days |
| **WebRTC data channel MTU** | Max ~64 KB per message; large operations require fragmentation | Fragment and reassemble large CRDT messages; batch small operations |
| **Mobile device GPU** | Older mobile GPUs struggle with > 2,000 canvas draw calls per frame | WebGL batched rendering; aggressive level-of-detail on mobile |
| **Network variability** | Users range from fiber (1ms RTT) to cellular (200ms+ RTT) | Adaptive sync frequency; operation batching tuned to RTT |
