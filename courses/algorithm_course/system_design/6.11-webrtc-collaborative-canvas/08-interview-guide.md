...
| | **Cons**: No offline support; must define transform functions for every op pair; central server Slowest part of the process | **Cons**: Tombstone accumulation; LWW "jumps" for concurrent moves; GC complexity |
...
| **Recommendation** | LWW with smooth animation. The jump is brief and rare (requires two users dragging the same shape simultaneously). Adding complexity for a rare Edge Case (Unusual or extreme situation) is not worth it. |
...
## Advanced Discussion Points (2025–2026 Context)

### Emerging Topics Interviewers May Raise

| Topic | Key Points to Demonstrate |
|---|---|
| **Loro/Automerge 2.0 vs Y.js** | Rust-based CRDTs compiled to WASM offer 10–100x faster merge for large documents; discuss when the performance difference matters (boards with 50K+ objects) |
| **WebGPU for canvas rendering** | GPU compute shaders enable spatial queries and rendering for 100K+ object boards; discuss progressive enhancement from Canvas 2D → WebGL → WebGPU |
| **WebTransport replacing WebSocket** | QUIC-based transport with multiplexed streams eliminates head-of-line blocking; connection migration survives network switches without reconnect |
| **AI-generated diagram operations** | LLM output must enter the CRDT pipeline as standard operations; discuss how AI and human edits compose and can be independently undone |

### Distinguishing "Great" Answers

| Question Area | Good Answer | Great Answer |
|---|---|---|
| **CRDT choice** | "Use CRDTs for conflict-free merging" | "Compose LWW-Map for properties, OR-Set for object presence, fractional indexing for z-order; explain why text CRDTs (sequence) don't work for 2D spatial data" |
| **Transport** | "Use WebSocket for real-time sync" | "WebSocket as primary with optional WebRTC P2P enhancement; separate unreliable channel for ephemeral cursor state from reliable channel for durable CRDT ops" |
| **Performance** | "Render visible objects only" | "R-tree spatial index for O(log n + k) viewport queries; level-of-detail rendering with zoom-dependent simplification; OffscreenCanvas on Web Worker to avoid main thread blocking" |
| **Offline** | "Cache board data locally" | "CRDT state in IndexedDB/OPFS; on reconnect, exchange state vectors to sync only missing ops; tombstone GC requires coordination to prevent resurrection" |
| **Scaling** | "Shard boards across servers" | "Board-affinity routing to stateful CRDT nodes; hot board migration with operation draining; cursor relay is stateless and independently scalable" |

### Common Trap Questions and How to Navigate Them

| Trap Question | Why It's a Trap | Strong Response Strategy |
|---|---|---|
| "Why not use OT instead of CRDTs?" | Interviewers expect you to know both, not blindly advocate CRDTs | OT requires a central server for transformation; CRDTs allow true offline editing and peer-to-peer merge. For a canvas with offline support and multi-device sync, CRDTs eliminate the single-point-of-failure transformation server. OT is viable when a central server is always available (e.g., Google Docs). |
| "Why not render everything with SVG/DOM?" | Tests understanding of rendering performance at scale | SVG/DOM works for <1,000 elements but creates a DOM node per shape. At 10K+ objects, DOM diffing and layout recalculation destroy frame rates. Canvas 2D or WebGL renders arbitrary object counts in a single draw pass. Trade-off: Canvas loses native hit-testing and accessibility, requiring manual implementation. |
| "Can't you just use a database for collaboration?" | Tests understanding of real-time sync vs request-response | Database polling adds 100–500ms latency per update, making cursor tracking and live drawing impossible. Real-time collaboration requires push-based transport (WebSocket/WebRTC) with sub-50ms delivery. The database stores durable state; the sync layer handles real-time distribution. |
| "How do you handle a board with 1 million objects?" | Tests whether you understand practical limits vs theoretical design | No user views 1M objects simultaneously. Spatial chunking loads only visible regions; background chunks are evicted from memory. The CRDT state for 1M objects (~1.5 GB) cannot fit in browser memory, so the client maintains a partial replica with on-demand chunk loading from the server. |
| "What happens when two users undo at the same time?" | Tests deep understanding of undo in collaborative systems | Per-user undo stacks (not global) ensure each user undoes only their own operations. Concurrent undos are independent CRDT operations that merge without conflict. The challenge is undo of operations that have been causally depended upon by other users' edits. |
| "Why not just broadcast diffs over a message queue?" | Tests understanding of ordering guarantees and merge semantics | Message queues provide at-least-once delivery but not causal ordering. Two shape moves arriving out of order produce wrong final state without CRDT merge logic. A raw diff broadcast also lacks conflict resolution—concurrent property edits on the same shape need LWW or similar semantics. |
| "How do you test a collaborative canvas?" | Tests whether candidate thinks beyond unit tests | Deterministic CRDT property tests (commutativity, idempotency) validate merge correctness. Simulated network partitions with random operation injection test convergence. Visual regression tests compare rendered canvas output across browsers. Chaos testing with random disconnects during active drawing validates reconnect and state recovery. |

### Architecture Decision Record Template

When discussing design choices in an interview, structure responses using this pattern:

| Element | Purpose |
|---|---|
| **Context** | What constraints or requirements drive this decision (e.g., "boards with 50K objects, 300 concurrent users") |
| **Decision** | The specific architectural choice made (e.g., "CRDT with LWW-Map over OT") |
| **Consequences** | Both positive trade-offs and accepted downsides (e.g., "gains offline merge, accepts tombstone GC complexity") |
| **Alternatives considered** | What was rejected and why (e.g., "OT rejected because it requires central transformation server") |
