...
## Board Lifecycle Architecture

### Board State Transitions

```mermaid
stateDiagram-v2
    [*] --> Created : User creates board
    Created --> Active : First editor connects
    Active --> Active : Operations + cursor sync
    Active --> Idle : All editors disconnect (5 min timeout)
    Idle --> Active : Editor reconnects
    Idle --> Cold : No access for 24 hours
    Cold --> Active : Board opened (load from snapshot)
    Active --> Exporting : Export requested
    Exporting --> Active : Export complete
    Active --> Snapshotting : Snapshot threshold reached
    Snapshotting --> Active : Snapshot persisted
    Cold --> Archived : No access for 90 days
    Archived --> Cold : Board accessed (restore from archive)
```

**State-dependent resource allocation:**

| State | CRDT State | WebSocket | Cursor Relay | Snapshot |
|---|---|---|---|---|
| **Active** | In-memory on CRDT sync node | Persistent connection per editor | Active broadcast channel | Every 200 ops or 10 min |
| **Idle** | In-memory with 5-min eviction timer | Closed; reconnect triggers reload | Stopped | Final snapshot on transition |
| **Cold** | On disk (latest snapshot in object storage) | None | None | Already persisted |
| **Archived** | Compressed in cold storage tier | None | None | Compressed with op log |

### Late Joiner Board Load Sequence

```mermaid
sequenceDiagram
    participant C as New Client
    participant API as API Server
    participant CRDT as CRDT Sync Engine
    participant SNAP as Snapshot Store
    participant RELAY as Cursor Relay

    C->>API: GET /boards/{id}
    API->>API: Auth + permission check
    API->>SNAP: Fetch latest snapshot
    SNAP-->>API: Snapshot (50 KB avg)
    API->>CRDT: Get ops since snapshot
    CRDT-->>API: Recent ops (0-200 ops)
    API-->>C: Board state bundle
    C->>C: Apply snapshot + replay ops
    C->>CRDT: Open CRDT sync channel
    CRDT-->>C: Subscribe to live ops
    C->>RELAY: Join cursor broadcast
    RELAY-->>C: Current cursor positions of all editors
    Note over C: Board is interactive
```

---

## AI-Assisted Canvas Features (2025–2026)

Modern collaborative canvas platforms are integrating AI capabilities that create new architectural requirements:

| AI Feature | Architecture Impact | Data Flow |
|---|---|---|
| **AI diagramming** | User describes diagram in natural language; LLM generates CRDT operations to create shapes + connectors | Text prompt → LLM service → batch CRDT ops → canvas render |
| **Sticky note clustering** | AI groups semantically similar sticky notes by embedding similarity | Board objects → embedding service → k-means clustering → layout ops |
| **Auto-layout** | AI repositions objects to optimize visual clarity (force-directed, hierarchical) | Object graph → layout engine → batched move operations via CRDT |
| **Content summarization** | AI summarizes text content across all sticky notes on a frame | Text extraction → LLM → summary text object creation |
| **Smart connectors** | AI suggests connections between related shapes based on content | Object embeddings → similarity search → connector creation suggestions |

**Key architectural constraint:** AI-generated operations must be applied through the same CRDT pipeline as user operations—they are not "server-side mutations" but operations that enter the CRDT log and merge with concurrent user edits. This ensures AI and human edits compose cleanly and can be undone independently.

---

## Emerging Technology Integration (2025–2026)

| Technology | Impact on Canvas Architecture |
|---|---|
| **Loro CRDT framework** | Rust-based CRDT with 10–100x faster merge than Y.js for large documents; enables wasm-compiled client-side CRDT engine |
| **OffscreenCanvas + Worker rendering** | Move canvas rendering to Web Worker; prevents main thread blocking during complex board rendering |
| **WebGPU adoption** | GPU-accelerated canvas rendering for 100K+ object boards; compute shaders for spatial queries |
| **WebTransport (HTTP/3)** | Replaces WebSocket with QUIC-based transport: lower latency, multiplexed streams, connection migration on network switch |
| **Origin Private File System (OPFS)** | Faster offline storage than IndexedDB; enables large CRDT state caching with file-system-like performance |
