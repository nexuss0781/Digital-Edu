...
## Insight 11: MCP Servers Turn ERP Data into Discoverable AI Tools

**Category:** Extensibility
**One-liner:** Model Context Protocol (MCP) servers expose each ERP module's data and actions as typed, discoverable tools that any AI agent can invoke without hard-coded integrations, transforming the ERP from a monolithic data silo into a composable AI-accessible service mesh.

**Why it matters:** Traditional ERP AI integrations require bespoke code for every data access pattern: querying vendor spend requires one custom function, creating a journal entry requires another, and checking inventory levels requires a third. MCP standardizes this through a tool discovery protocol where each ERP module registers its capabilities (tools, resources, prompts) with the MCP server. An agent can call `GET /mcp/tools` to discover that `query_vendor_spend`, `create_journal_entry`, and `check_inventory` exist, along with their parameter schemas. This is architecturally transformative because it decouples the AI platform from the ERP modules. New modules automatically become AI-accessible by registering MCP tools, without modifying the agent orchestrator. The protocol also enforces tenant context propagation -- every MCP tool invocation carries the tenant_id, ensuring data isolation is maintained through the AI layer. In 2025-2026, MCP has become the de facto standard for AI-to-application integration, with major ERP vendors adopting it for their agentic platforms.

---

## Insight 12: Compliance-as-Code Maps Regulatory Controls to Executable Policies

**Category:** Architecture Evolution
**One-liner:** Express SOC 2, GDPR, and HIPAA controls as machine-executable policy rules (OPA/Rego) that are continuously evaluated against live system state, replacing annual audit snapshots with real-time compliance attestation.

**Why it matters:** Traditional compliance operates on a snapshot model: auditors visit annually, review evidence, and issue a report. Between audits, drift is invisible. Compliance-as-code inverts this by encoding each control as an executable policy. SOC 2 CC6.1 (logical access controls) becomes a Rego policy that verifies every role assignment goes through the approval workflow and no user has both "create" and "approve" permissions on the same resource type. GDPR Article 17 (right to erasure) becomes a policy that verifies deletion requests complete within 30 days and cryptographic erasure certificates exist. These policies run continuously against the live system: every deployment, configuration change, and access pattern is evaluated in real-time. Violations trigger alerts, not just audit findings months later. For an AI-native ERP, this is especially critical because AI agents create new compliance surface area -- every autonomous agent action must be evaluated against the same control framework that governs human actions. The shift from periodic to continuous compliance reduces audit preparation from weeks to hours and catches violations in minutes rather than months.
