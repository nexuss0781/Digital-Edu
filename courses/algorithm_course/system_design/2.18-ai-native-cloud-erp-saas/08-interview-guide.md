...
The architecture pattern stays the same; we're scaling each component independently based on its Slowest part of the process."
...
### Question 6: "How does MCP differ from just building REST APIs for your AI?"

**Bad Answer**: "MCP is just a wrapper around REST."

**Good Answer**: "MCP provides three capabilities that REST APIs alone cannot:

1. **Dynamic Tool Discovery**: Agents call `GET /mcp/tools` to discover what operations exist at runtime. When we add a new ERP module (say, Project Accounting), its MCP tools are immediately discoverable by all agents without code changes. With pure REST, you'd need to update every agent's hardcoded API client.

2. **Structured Resource Access**: MCP defines a resource protocol where agents can navigate ERP data hierarchically (tenant → company → vendor → invoices). This gives the LLM structured context for answering questions, rather than requiring us to pre-build every possible query combination.

3. **Tenant Context Propagation**: MCP carries tenant context through the entire tool invocation chain. When the Finance Agent calls `query_vendor_spend` via MCP, the tenant isolation is enforced at the protocol level, not just the application level.

The real value is composability. MCP turns the ERP from a monolithic data silo into a toolbox that any AI agent can use. Third-party agents can integrate with our ERP by connecting to the MCP server without custom API clients."

---

### Question 7: "How do you handle the period-end close with AI agents?"

**Bad Answer**: "The AI runs the close automatically."

**Good Answer**: "Period-end close is the highest-stakes ERP operation - it locks fiscal periods, generates financial statements, and triggers regulatory reporting. We use a hybrid approach:

1. **AI-Assisted Reconciliation**: Before close, the Finance Agent runs automated reconciliation - matching bank statements, intercompany transactions, and accruals. It flags exceptions but does not auto-resolve them during close.

2. **Close Checklist Orchestration**: A saga-based workflow engine manages the close sequence: verify all sub-ledgers balanced → post adjusting entries → run trial balance → generate statements. Each step has human approval gates.

3. **Anomaly Detection During Close**: The AI monitors for unusual patterns - large last-minute journal entries, round-number adjustments, entries by users who don't normally post - and flags them for review before the period locks.

4. **Post-Close AI Analysis**: After the period locks, the Analytics Agent generates variance analysis, comparing actuals to budget and prior periods, with natural language explanations for significant variances.

The key principle: AI assists and accelerates the close process but never autonomously locks a period or posts material adjusting entries. The CFO's approval is the final gate."
