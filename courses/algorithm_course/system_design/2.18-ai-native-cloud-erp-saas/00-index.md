...
|----------|-------------|
| **What AI decides alone** | Document field extraction, anomaly flagging on transactions, RAG-based query responses, routine data entry suggestions |
| **What AI recommends** | Invoice approval routing, budget variance explanations, vendor payment prioritization, month-end close action items |
| **What requires human approval** | Financial postings to general ledger, payment execution above threshold, agent-initiated procurement actions, policy or workflow changes |
| **Deterministic source of truth** | ERP Database (double-entry ledger, immutable event store) — AI agents propose actions but financial transactions require human confirmation at governance gates |
| **Rollback path** | Event sourcing enables replaying to any point-in-time; reversing journal entries via compensating transactions; agent action audit trail with one-click undo |
...
- [Identity & Access Management (IAM)](../2.5-identity-access-management/00-index.md) - AuthN/AuthZ patterns, RBAC+ABAC policy engine
- [Secret Management System](../2.16-secret-management-system/00-index.md) - HSM key hierarchy, DEK/KEK rotation
- [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) - Immutable audit trails, event replay
- [CQRS Implementation](../1.19-cqrs-implementation/00-index.md) - Read/write separation for financial reporting
- [Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) - Period-end close, batch payroll processing
- [AI Agent Orchestration Platform](../3.17-ai-agent-orchestration-platform/00-index.md) - Multi-agent patterns, tool use, planning
- [LLM Gateway / Prompt Management](../3.21-llm-gateway-prompt-management/00-index.md) - Model routing, prompt versioning, guardrails
- [RAG System](../3.15-rag-system/00-index.md) - Retrieval-augmented generation, chunking strategies
...
|-------------|--------|--------------------------|
| **Agentic ERP workflows** | Agents handle end-to-end processes (procure-to-pay, hire-to-retire) autonomously | Multi-step planning with tool use, human-in-loop gates at financial boundaries |
| **MCP (Model Context Protocol) adoption** | Standardized AI-to-data bridge replacing custom integrations | MCP servers expose ERP data as structured tools; agents discover capabilities dynamically |
| **Reasoning models in finance** | Chain-of-thought for complex reconciliation, variance analysis | Higher latency budget for financial reasoning tasks; separate inference queues |
| **Confidential computing** | GPU TEEs (Trusted Execution Environments) for inference | Tenant data encrypted even during GPU processing; eliminates trust-the-operator assumption |
| **Small language models (SLMs)** | 3B-8B parameter models match 70B on domain-specific ERP tasks | Lower GPU requirements; enables edge deployment for offline ERP access |
| **Compliance-as-code maturity** | OPA/Rego policies for SOC2/GDPR control mapping | Automated continuous compliance attestation; shift-left compliance testing |

---

## References

- Oracle NetSuite SuiteCloud AI Platform - Multi-agent architecture, MCP integration, SuiteAgent autonomous workflows
- SAP Joule Architecture - BTP integration, Work Zone deployment, process mining with AI co-pilot
- Workday Illuminate - 800B-parameter proprietary LLM, AI Gateway for responsible AI
- Microsoft Dynamics 365 Copilot - Agent-driven ERP operations, Dataverse grounding, autonomous agents (2025)
- Anthropic Model Context Protocol (MCP) - AI-data integration standard, tool discovery, resource access
- vLLM Project - PagedAttention, continuous batching, high-throughput LLM serving
- NVIDIA Run:ai / DGX Cloud - GPU cluster management, fractional GPU scheduling for AI workloads
- Zoho Zia Agent Studio - No-code agent builder, private LLM infrastructure on H100 clusters
