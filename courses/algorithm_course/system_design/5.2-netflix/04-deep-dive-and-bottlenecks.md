...
### Foundation Model Evolution (2025+)

Netflix's personalization architecture is evolving from many specialized models toward a central foundation model:

```
EVOLUTION: Specialized Models → Foundation Model

BEFORE (Traditional Hydra):
  ┌──────────────────────────────────────────────────────────┐
  │  Homepage Model  │  Search Model  │  Notification Model  │
  │  (own features)  │ (own features) │  (own features)      │
  │  (own training)  │ (own training) │  (own training)      │
  └──────────────────────────────────────────────────────────┘
  Problem: N models × M pipelines × P deployment cycles
  Problem: Search behavior doesn't inform homepage ranking

AFTER (Foundation Model + Task Heads):
  ┌──────────────────────────────────────────────────────────┐
  │              Central Foundation Model                     │
  │  Ingests: views, searches, ratings, browse, messaging    │
  │  Learns: unified user + item embeddings                  │
  ├──────────────────────────────────────────────────────────┤
  │  Homepage  │  Search  │  Notifications │  Email │ Push   │
  │  Head      │  Head    │  Head          │  Head  │ Head   │
  └──────────────────────────────────────────────────────────┘
  Benefit: Shared representations, reduced model count
  Benefit: Cross-signal learning across all surfaces
  Benefit: Better cold start (new content benefits immediately)
```

**Why This Matters:**

| Aspect | Specialized Models | Foundation Model |
|--------|-------------------|------------------|
| Training Pipelines | N per surface | 1 shared + N lightweight |
| Feature Engineering | Per-surface | Unified feature store |
| Cold Start | Per-surface signal wait | Immediate from shared knowledge |
| Cross-Surface Learning | None | All signals inform all surfaces |
| Operational Overhead | High (N × M) | Lower (1 + N heads) |
...
| Rank | Slowest part of the process | Impact | Cause | Mitigation |
|------|------------|--------|-------|------------|
...
### Slowest part of the process 1: ISP Network Congestion
...
### Slowest part of the process 2: Feature Store Latency
...
### Slowest part of the process 3: Live Event Scaling

```
PROBLEM:
  - Live events have unprecedented concurrency (65M+)
  - CDN not designed for live (no pre-caching)
  - Manifest refresh storms (clients polling every 2s)
  - Recommendation systems can't run per-user inference at live scale

MITIGATION (Learned from Tyson vs Paul 2024, WWE Raw 2025):

1. SEPARATE ARCHITECTURE
   - Live Origin Service (not VOD origin)
   - EVCache for live manifests (time-critical updates)
   - Different CDN URLs for live content
   - Automated orchestration minimizes human error across
     ingest, encoding, packaging, and origin setup

2. PRE-WARMING
   - Deploy additional capacity before event
   - Pre-warm ALL Open Connect appliances with live URLs
   - Coordinate with ISPs on expected traffic
   - Redundant regional ingest pipelines for resilience

3. PHASED RECOMMENDATION APPROACH
   - Prefetching phase: Pre-compute and cache personalized
     "what to watch next" recommendations for all active profiles
     hours before event start
   - Broadcasting phase: During event, push low-cardinality
     cache-invalidation signals (e.g., "event ended") rather
     than triggering full inference pipelines
   - Converts O(n) per-user computation to O(1) signal propagation
...
5. GRACEFUL DEGRADATION
   - Accept higher latency (10-30s vs real-time)
   - ABR aggressively biases toward lower quality
   - Prioritize stream continuity over max quality
   - Audio continuity prioritized over video quality

CASE STUDY: WWE Raw (January 2025)
  - Netflix began weekly live sports streaming
  - Required sustained live infrastructure (not one-off events)
  - Weekly cadence validated reliability of live pipeline
  - Demonstrated infrastructure readiness for regular live programming
```
