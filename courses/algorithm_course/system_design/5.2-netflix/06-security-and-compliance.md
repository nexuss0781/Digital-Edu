...
### Household Verification (Account Sharing Enforcement)

Netflix's crackdown on password sharing (2023+) introduced technical mechanisms to detect and enforce single-household usage:

```
HOUSEHOLD VERIFICATION ARCHITECTURE:

SIGNALS:
  - IP address analysis (stable home IP vs varied IPs)
  - Device fingerprinting (registered devices vs new)
  - Geographic clustering (devices in same metro area)
  - Network fingerprint (same ISP, consistent latency)
  - Wi-Fi BSSID (when available)

VERIFICATION FLOW:
  1. Device connects from new IP
  2. System checks against household IP history
  3. If divergent: prompt verification
     - SMS/email code to account holder
     - Temporary access while verifying
  4. If confirmed out-of-household:
     - Prompt to add "extra member"
     - Or block device

DESIGN PRINCIPLES:
  - Never interrupt active playback for verification
  - Travel detection (short-term different IPs are OK)
  - Graduated enforcement (warn → prompt → block)
  - Revenue impact: converted ~100M freeloading households
```
