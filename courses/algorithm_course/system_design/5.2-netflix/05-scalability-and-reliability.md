...
### Live Event Scaling Case Study

Netflix's transition from one-off live events (Tyson vs Paul, Dec 2024) to regular live programming (WWE Raw, Jan 2025+) required evolving from pre-event surge engineering to sustained live infrastructure.

```
CASE STUDY: WWE Raw Weekly Live Streaming (2025)

CHALLENGE:
  - Weekly live sports broadcast (not one-off)
  - Cannot pre-warm infrastructure for each episode
  - Sustained operational overhead
  - Mix of live viewers and VOD replay

ARCHITECTURE ADAPTATIONS:
  1. PERSISTENT LIVE INFRASTRUCTURE
     - Dedicated Live Origin Service pool (always warm)
     - Reserved Open Connect capacity for live
     - Standing ISP coordination agreements

  2. AUTOMATED EVENT ORCHESTRATION
     - Pipeline automation: ingest → encode → package → origin
     - Minimizes human error across weekly operations
     - Redundant regional ingest pipelines

  3. DUAL-MODE CDN OPERATION
     - VOD: predictive placement, proactive caching
     - Live: push-based segment distribution
     - Same Open Connect appliances, different content paths

  4. POST-EVENT VOD CONVERSION
     - Live segments automatically assembled into VOD
     - Re-encoded with Context-Aware Encoding for optimal quality
     - Distributed via standard proactive caching

VALIDATION:
  - Demonstrated sustained weekly live capability
  - Infrastructure readiness for future live expansion
  - Proved CDN can serve both live and VOD simultaneously
```
