...
### Trap 6: "How do recommendations work during a live event with 65M viewers?"

**Bad Answer:** "Just run the recommendation model for each user"

**Good Answer:**
```
"You can't run per-user Hydra inference at live scale -- it would
overwhelm the feature store and model serving infrastructure.

Netflix uses a PHASED approach:

1. PREFETCHING PHASE (Hours Before Event)
   - Pre-compute and cache 'what to watch next' recommendations
     for all active profiles
   - Front-load the expensive inference computation
   - Store results in EVCache with event-specific TTL

2. BROADCASTING PHASE (During Event)
   - Push low-cardinality cache-invalidation signals
   - e.g., 'event ended' triggers cache refresh for post-event row
   - One signal type fans out to millions of cached entries
   - No per-user computation during peak concurrency

This converts an O(n) per-user computation problem into an
O(1) signal propagation problem. The key insight is that during
a live event, everyone is watching the same thing -- personalization
matters most for what comes AFTER the event, and that can be
pre-computed."
```

### Trap 7: "Why would Netflix move from many specialized models to a foundation model?"

**Bad Answer:** "Foundation models are the trend"

**Good Answer:**
```
"Netflix maintained separate models for homepage, search,
notifications, push, and email -- each with its own training
pipeline, feature set, and serving infrastructure.

Problems with specialized models:
  - Operational sprawl: N models × M pipelines × P deployments
  - Missed cross-signal learning: search behavior doesn't
    inform homepage ranking
  - Cold start per surface: new content waits for surface-specific
    signal accumulation

Foundation model approach:
  - Central model ingests ALL interaction signals across surfaces
  - Learns unified user + item embeddings
  - Lightweight task-specific heads fine-tune for each surface
  - Similar to how LLMs provide general capabilities adapted
    via fine-tuning

Benefits:
  - Cross-signal learning: search behavior DOES inform homepage
  - Better cold start: new content benefits immediately from
    shared understanding
  - Reduced compute: train shared representations once
  - Operational simplicity: 1 pipeline + N lightweight heads"
```
