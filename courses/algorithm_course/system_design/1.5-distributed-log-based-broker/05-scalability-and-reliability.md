...
|--------|---------|-----------------|
| Disk usage | Per broker | > 70% → add storage or enable tiered storage |
...
## Tiered Storage Scaling (Kafka 4.0+)

### Capacity Planning with Tiered Storage

Tiered storage fundamentally changes the capacity planning equation by decoupling retention-driven storage from broker sizing.

```
BEFORE TIERED STORAGE:
    Broker count = max(
        throughput_based,
        storage_based,       ← Often the Slowest part of the process
        partition_based,
        3
    )

    storage_based = (daily_data × retention_days × RF) / disk_per_broker
    Example: (100GB/day × 30 days × 3) / 10TB = 0.9 → 1 broker (storage)
    Example: (100GB/day × 90 days × 3) / 10TB = 2.7 → 3 brokers (storage)

AFTER TIERED STORAGE:
    Broker count = max(
        throughput_based,
        local_storage_based,  ← Only hot data (1-7 days)
        partition_based,
        3
    )

    local_storage = (daily_data × local_retention_days × RF) / disk_per_broker
    remote_storage = (daily_data × (total_retention - local_retention)) / object_storage
    Example: (100GB/day × 1 day × 3) / 10TB = 0.03 → 1 broker
             remote: 100GB/day × 89 days = 8.9TB on object storage

    Result: Storage is no longer a factor in broker sizing
```

### Scaling Strategies with Cloud-Native Alternatives

| Strategy | Scaling Speed | Data Movement | Best For |
|----------|--------------|---------------|----------|
| **Kafka (traditional)** | Minutes-hours | Full partition data replicated to new broker | On-premise, low-latency |
| **Kafka (tiered storage)** | Minutes | Only local tier replicated; remote stays | Hybrid cloud |
| **AutoMQ (WAL + S3)** | Seconds | Metadata-only reassignment (data stays in S3) | Cloud-native |
| **WarpStream** | Instant | No data movement (agents are stateless) | Cloud-native, cost-sensitive |

### Elastic Scaling for Cloud-Native Brokers
...
│                                                                  │
│  Traditional Kafka:                                              │
│  1. Add broker → 2. Reassign partitions → 3. Replicate data    │
│  Timeline: 10-60 minutes (data-dependent)                       │
│  Limitation: Cannot scale faster than data transfer speed       │
│                                                                  │
│  AutoMQ (S3Stream):                                              │
│  1. Add broker → 2. Reassign partitions (metadata only)        │
│  Timeline: Seconds                                               │
│  Reason: Data already in S3, broker just needs to know pointer  │
│                                                                  │
│  WarpStream:                                                     │
│  1. Add agent → Done (stateless)                                │
│  Timeline: Immediate                                             │
│  Reason: No data locality — any agent reads/writes any data     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
