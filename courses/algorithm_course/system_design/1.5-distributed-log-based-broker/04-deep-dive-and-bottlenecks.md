...
### KIP-848: Server-Side Consumer Protocol (GA in Kafka 4.0)
...
│                                                                  │
│  Status: GA in Kafka 4.0 (March 2025)                            │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### KIP-932: Share Groups (Early Access in Kafka 4.0)

Share Groups introduce **queue semantics** into the log-based broker, allowing multiple consumers to read from the same partition without exclusive assignment.
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Consumer Group (traditional):                                   │
│  Partition P0 ──→ Consumer A (exclusive)                        │
│  Partition P1 ──→ Consumer B (exclusive)                        │
│  Partition P2 ──→ Consumer C (exclusive)                        │
│                                                                  │
│  ✓ Per-partition ordering guaranteed                             │
│  ✗ Parallelism limited to partition count                       │
│  ✗ Rebalance on member changes                                  │
│                                                                  │
│  Share Group (KIP-932):                                          │
│  Partition P0 ──→ Consumer A, B, C (shared)                     │
│  Partition P1 ──→ Consumer A, B, C (shared)                     │
│  Partition P2 ──→ Consumer A, B, C (shared)                     │
│                                                                  │
│  ✗ Per-partition ordering NOT guaranteed                         │
│  ✓ Parallelism independent of partition count                   │
│  ✓ No rebalance needed (consumers just connect)                 │
│                                                                  │
│  Record Lifecycle in Share Groups:                               │
│  Available → Acquired → Acknowledged/Released                   │
│  • Records are "locked" when acquired by a consumer             │
│  • If not ack'd within timeout, released to another consumer    │
│  • Max delivery attempts before dead-lettering                  │
│                                                                  │
│  Use Cases:                                                      │
│  • Task distribution (where ordering is not required)           │
│  • Load balancing across consumers without partition coupling   │
│  • Workloads that previously needed a separate message queue     │
│                                                                  │
│  Status: Early Access in Kafka 4.0, GA target Kafka 4.2         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
### Slowest part of the process 1: Partition Hot Spots
...
### Slowest part of the process 2: Consumer Lag
...
CAUSES:
    ...
    4. External dependency Slowest part of the process (database, API)
...
SOLUTIONS:
    ...
    │ DB Slowest part of the process          │ Batch writes, connection pool│
    │ Network                │ Increase fetch.max.bytes     │
    └────────────────────────────────────────────────────────┘
```

### Slowest part of the process 3: Replication Lag
...
### Slowest part of the process 4: Controller Overload
...
## Deep Dive 4: Tiered Storage (KIP-405, GA in Kafka 4.0)

### Architecture

Tiered storage separates "hot" recent data on local broker disks from "cold" historical data on object storage, breaking the coupling between retention duration and broker disk capacity.
...
│  │ Local Tier (NVMe/SSD)                                    │    │
│  │ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐    │    │
│  │ │ Segment 5│ │ Segment 6│ │ Segment 7│ │ Active   │    │    │
│  │ │ (ready   │ │ (local)  │ │ (local)  │ │ Segment  │    │    │
│  │ │ to tier) │ │          │ │          │ │ (writing) │    │    │
│  │ └──────────┘ └──────────┘ └──────────┘ └──────────┘    │    │
│  │      │                                                   │    │
│  │      ▼ RemoteLogManager                                  │    │
...
│         ▼                                                        │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ Remote Tier (Object Storage)                             │    │
│  │ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐    │    │
│  │ │ Segment 0│ │ Segment 1│ │ Segment 2│ │ Segment 3│... │    │
│  │ │ (oldest) │ │          │ │          │ │          │    │    │
│  │ └──────────┘ └──────────┘ └──────────┘ └──────────┘    │    │
│  │                                                          │    │
│  │ Cost: ~10x cheaper per GB than local NVMe                │    │
│  │ Latency: 50-200ms (vs <5ms local)                        │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Segment Lifecycle with Tiered Storage

```
LIFECYCLE of_segment(segment):
    1. ACTIVE: Being written by leader
       → Eligible for consumer reads from local disk

    2. CLOSED: Rolled (reached segment.bytes or segment.ms)
       → Still on local disk
       → Replicated to followers normally

    3. TIERED: Copied to remote tier by RemoteLogManager
       → remote.log.copy.interval.ms controls frequency
       → RemoteStorageManager plugin handles object storage writes
       → Remote segment index created for offset → object storage mapping

    4. LOCAL_DELETE: local.retention.ms exceeded
       → Local copy deleted from broker disk
       → Remote copy still available

    5. REMOTE_DELETE: retention.ms (overall) exceeded
       → Remote copy deleted from object storage

CONFIGURATION:
    remote.storage.enable=true
    remote.log.storage.class=<RemoteStorageManager plugin>
    local.retention.ms=86400000         # Keep 1 day locally
    retention.ms=7776000000             # Keep 90 days total (remote)
    remote.log.copy.interval.ms=30000   # Copy every 30s
```

### Cost Impact

| Configuration | Broker Disk Needed | Object Storage | Total Monthly Cost (est.) |
|--------------|-------------------|----------------|--------------------------|
| **Before tiered storage** (7-day retention) | 100 TB | 0 | $$$$ (NVMe) |
| **Before tiered storage** (90-day retention) | 1.3 PB | 0 | $$$$$$$$$ (13× broker disk) |
| **With tiered storage** (90-day, 1-day local) | 100 TB | 1.2 PB | $$$$$ (3-9× cheaper) |

### Consumer Transparency

Consumers are unaware of the local/remote distinction. When a consumer requests an offset in the remote tier:

```
FUNCTION handle_fetch_with_tiered_storage(partition, fetch_offset):
    IF fetch_offset >= local_log_start_offset:
        // Data is local — fast path
        RETURN read_from_local_log(partition, fetch_offset)
    ELSE IF fetch_offset >= remote_log_start_offset:
        // Data is in remote tier — slow path
        remote_segment = remote_log_manager.find_segment(partition, fetch_offset)
        RETURN remote_storage_manager.fetch(remote_segment, fetch_offset)
        // Latency: 50-200ms (object storage round-trip)
    ELSE:
...
## Case Study 1: WarpStream — Zero-Disk Broker Architecture

WarpStream (acquired by Confluent, September 2024) represents the most radical rethinking of the log-based broker storage model: **stateless agents that write directly to object storage with no local disk**.

### Architecture Comparison

```
┌─────────────────────────────────────────────────────────────────┐
│           TRADITIONAL vs WARPSTREAM ARCHITECTURE                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Traditional (Kafka/Redpanda):                                  │
│  ┌──────────────┐  replication  ┌──────────────┐                │
│  │ Broker 1     │──────────────▶│ Broker 2     │                │
...
│  WarpStream:                                                     │
│  ┌──────────────┐  ┌──────────────┐                              │
│  │ Agent 1      │  │ Agent 2      │  (both stateless)           │
│  │ [no disk]    │  │ [no disk]    │                              │
│  └──────┬───────┘  └──────┬───────┘                              │
│         │                  │                                     │
│         ▼                  ▼                                     │
...
### What WarpStream Eliminates

| Traditional Concern | WarpStream Approach | Trade-off |
|--------------------|--------------------| ---------|
| Replication (RF=3) | Object storage provides 11-nines durability | No inter-broker data transfer |
| Partition reassignment | Metadata pointer update only | No data movement when scaling |
| Disk provisioning | Zero local disk | No capacity planning for broker storage |
| Broker state recovery | Nothing to recover (stateless) | Instant replacement |
| Network egress | BYOC model — data stays in customer's cloud | No cross-network data transfer |

### Latency Profile

| Operation | Traditional Kafka | WarpStream (S3 Standard) | WarpStream (S3 Express) |
|-----------|------------------|-------------------------|------------------------|
| Produce (p99) | 5-20ms | 400-600ms | ~33ms |
| Consume (recent) | 2-10ms | 50-200ms | 10-33ms |
| Consume (old) | 2-10ms | 50-200ms | 50-200ms |

### When WarpStream Fits

- Cloud-native workloads where latency > 100ms is acceptable
- High-retention, cost-sensitive pipelines (log aggregation, analytics)
- Environments where operational simplicity trumps raw latency
- Confluent Freight Clusters for high-throughput, cost-optimized workloads

### When WarpStream Does NOT Fit

- Low-latency event processing (<50ms end-to-end requirement)
- On-premise deployments (no object storage)
- Latency-critical financial or gaming workloads

---

## Case Study 2: The Cloud-Native Broker Spectrum (2025)

The log-based broker ecosystem has evolved from a single model (Kafka: tightly coupled compute + storage) into a spectrum of storage architectures:

```
┌─────────────────────────────────────────────────────────────────┐
│            CLOUD-NATIVE BROKER ARCHITECTURE SPECTRUM              │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Tight Coupling ◄─────────────────────────► Loose Coupling      │
│                                                                  │
│  Redpanda        Kafka           AutoMQ          WarpStream     │
│  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐    │
│  │ C++ core │   │ JVM      │   │ Kafka    │   │ Stateless │    │
│  │ Raft/part│   │ KRaft    │   │ fork     │   │ agents    │    │
│  │ Local    │   │ Local +  │   │ WAL tier │   │ Object    │    │
│  │ NVMe     │   │ Tiered   │   │ + S3     │   │ storage   │    │
│  │ only     │   │ Storage  │   │          │   │ only      │    │
│  └──────────┘   └──────────┘   └──────────┘   └──────────┘    │
│                                                                  │
│  Latency:       Latency:       Latency:       Latency:          │
│  <5ms           <10ms local    ~10ms (WAL)    400-600ms (S3)    │
│                 50-200ms cold  50-200ms (S3)  ~33ms (Express)   │
│                                                                  │
│  Scaling:       Scaling:       Scaling:       Scaling:           │
│  Data movement  Data movement  Seconds        Instant            │
│  (minutes)      (minutes)      (metadata)     (metadata)        │
│                                                                  │
│  Cost Model:    Cost Model:    Cost Model:    Cost Model:        │
│  NVMe × RF × N Local + Object WAL + Object   Object only       │
│  (most $/GB)    (moderate)     (~10× cheaper) (~80% savings)    │
│                                                                  │
│  Best For:      Best For:      Best For:      Best For:          │
│  Ultra-low      General        Cloud-native   High-retention     │
│  latency        purpose        cost-sensitive  cost-sensitive    │
│  On-premise     Hybrid         Cloud           Cloud only        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
The spectrum is driven by one fundamental question: **who provides durability?**

| Architecture | Durability Provider | Replication Overhead | Scaling Model |
|-------------|--------------------|--------------------|---------------|
| Traditional (Kafka, Redpanda) | Broker-managed replication (RF=3) | 3× write amplification, inter-broker network | Stateful (data movement required) |
| Hybrid (Kafka + Tiered Storage) | Local for recent + object storage for old | 3× for local, 1× for remote | Semi-stateful |
| WAL + Object (AutoMQ) | Block storage WAL + object storage | 1× WAL + 1× S3 | Stateless (seconds to reassign) |
| Pure Object (WarpStream) | Object storage exclusively | 1× only (S3 provides durability) | Stateless (instant) |

### The Confluent Tableflow Bridge

Confluent Tableflow (GA for Iceberg, 2025) auto-materializes Kafka topics as Iceberg or Delta Lake tables, enabling direct SQL queries over streaming data without a separate ETL pipeline. This bridges the gap between streaming and analytics:

```
BEFORE Tableflow:
  Producer → Kafka → Flink/Spark → Parquet → Data Warehouse → SQL

AFTER Tableflow:
  Producer → Kafka → (Tableflow auto-materializes) → Iceberg Table → SQL

Impact:
  • Eliminates the "connect, transform, load" pipeline
  • Iceberg tables stay in sync with Kafka topics (near real-time)
  • Analytics engines query Iceberg directly
  • Enables the Kappa Architecture at lower operational cost
```

This convergence of streaming and batch processing, driven by tiered storage and Tableflow, is reviving interest in the **Kappa Architecture** — a single streaming pipeline replacing the dual Lambda Architecture of separate batch and stream processing paths.
