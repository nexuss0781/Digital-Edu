...
STRUCT ConsumerGroup:
    ...
    leader_id: string            # Member ID of group leader (classic protocol)
    members: Member[]
    # KIP-848 additions (Kafka 4.0+):
    target_assignment_epoch: int32  # Server-computed assignment version
    assignor: string                # Server-side assignor class
...
### Share Group Data Model (KIP-932, Early Access in Kafka 4.0)

Share Groups introduce queue-style semantics where multiple consumers can read from the same partition without exclusive assignment.

```
STRUCT ShareGroup:
    group_id: string
    state: enum { EMPTY, STABLE, DEAD }
    members: ShareGroupMember[]
    # No generation_id — no rebalance protocol needed
    # No leader_id — server manages assignment

STRUCT ShareGroupMember:
    member_id: string
    client_id: string
    # No partition assignment — members share all partitions

STRUCT SharePartitionState:
    topic: string
    partition: int32
    start_offset: int64              # Earliest unacknowledged offset
    state_batches: AcquiredBatch[]   # Per-record delivery tracking

STRUCT AcquiredBatch:
    first_offset: int64
    last_offset: int64
    delivery_count: int32            # Number of delivery attempts
    member_id: string                # Consumer that acquired this batch
    state: enum { AVAILABLE, ACQUIRED, ACKNOWLEDGED, ARCHIVED }
    acquire_timestamp: int64         # For timeout-based release

RECORD LIFECYCLE:
    AVAILABLE → ACQUIRED (consumer fetches)
    ACQUIRED → ACKNOWLEDGED (consumer acks)
    ACQUIRED → AVAILABLE (timeout or explicit release)
    ACQUIRED → ARCHIVED (max delivery attempts exceeded → dead letter)

CONFIGURATION:
    group.share.record.lock.duration.ms=30000  # Acquire timeout
    group.share.max.delivery.count=5           # Max attempts before DLQ
```
...
| `remote.storage.enable` | `false` | Enable tiered storage (Kafka 4.0+) |
| `local.retention.ms` | `-2` (use retention.ms) | Local tier retention before tiering |
| `remote.log.copy.interval.ms` | `30000` | Frequency of copying to remote tier |
