...
**Why it matters:** Each partition is assigned to exactly one consumer within a consumer group. A topic with 6 partitions can have at most 6 parallel consumers; a 7th consumer sits idle. Under-partitioning limits throughput scaling; over-partitioning increases metadata overhead, leader election time, and end-to-end latency (more partitions = more replication coordination). Kafka allows adding partitions but not removing them (removing would invalidate key-based routing guarantees). The recommended Practical rule of thumb: partition count >= (target throughput / throughput per partition), with headroom for future growth. LinkedIn uses 8-32 partitions for most topics. The critical constraint is that changing partition count breaks key ordering for existing keys (hash(key) % old_count != hash(key) % new_count), making partition expansion disruptive for any consumer relying on key-based co-location.
...
## Insight 11: Tiered Storage Decouples Retention Cost from Broker Capacity

**Category:** Cost Optimization
**One-liner:** By transparently offloading older log segments to object storage while keeping recent data on local disks, tiered storage (KIP-405, GA in Kafka 4.0) breaks the coupling between retention duration and broker disk capacity -- enabling 90-day retention with the broker count that previously supported only 7 days.

**Why it matters:** Before tiered storage, extending retention from 7 to 30 days required 4.3× more disk per broker, which in practice meant either massive disk provisioning or adding brokers purely for storage (not throughput). Tiered storage introduces two tiers: a "local tier" on broker SSDs that serves recent data at <5ms latency, and a "remote tier" on object storage that holds older segments at ~10× lower cost per GB. The RemoteLogManager handles the lifecycle: when a local segment exceeds the local.retention.ms threshold, it is copied to object storage and eventually deleted from local disk. Consumers are unaware of the distinction -- fetching an offset in the remote tier triggers a transparent read from object storage via the RemoteStorageManager plugin. The critical trade-off is latency: fetching cold data incurs 50-200ms (object storage round-trip) versus <5ms for local data, creating a two-tier latency profile that consumers must tolerate. The architectural lesson is that storage capacity planning and compute capacity planning are finally independent: a 5-broker cluster can now retain the data that previously required 20 brokers, achieving 3-9× storage cost reduction. This is the same disaggregated-storage principle that cloud-native databases adopted years ago, now arriving in streaming infrastructure.

---

## Insight 12: Diskless Brokers Invert the Storage-Compute Coupling That Defined Log-Based Systems

**Category:** Scaling
**One-liner:** WarpStream's zero-disk architecture (acquired by Confluent, September 2024) and AutoMQ's S3Stream demonstrate that the foundational assumption of log-based brokers -- that brokers must manage local disks and replicate data themselves -- can be inverted, with stateless agents writing directly to object storage.

**Why it matters:** Traditional log-based brokers (Kafka, Redpanda) tightly couple compute and storage: each broker manages local disk segments, replicates data to followers, and requires partition reassignment (data movement) when scaling. WarpStream eliminates all three: stateless agents write directly to object storage, object storage provides durability (no inter-broker replication), and partition reassignment requires zero data movement (agents just update metadata pointers). The trade-off is write latency: 400-600ms p99 on standard object storage tiers versus 5-10ms on local disk, though express object storage tiers reduce this to ~33ms. AutoMQ takes a middle path: a WAL (Write-Ahead Log) tier on cloud block storage provides low-latency writes (~10ms), with async flush to object storage for durable long-term retention. This creates a spectrum of storage architectures -- tight coupling (Kafka on local NVMe) for lowest latency, moderate coupling (AutoMQ WAL + S3) for balanced cost/latency, and no coupling (WarpStream on pure object storage) for maximum elasticity and cost efficiency. The key insight is that the "right" architecture depends on the deployment environment: on-premise with NVMe favors tight coupling; cloud-native favors loose coupling. Confluent's Freight Clusters (powered by WarpStream) demonstrate this at scale, claiming ~90% throughput cost savings for workloads that can tolerate the higher latency. This is the streaming equivalent of the monolith-to-microservices transition in application architecture -- not universally better, but transformative for the right workloads.
