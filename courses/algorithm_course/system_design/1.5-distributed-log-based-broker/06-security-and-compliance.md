...
### Data Sovereignty with Tiered Storage (Kafka 4.0+)

Tiered storage introduces new compliance considerations because data flows to object storage, which may span regions or jurisdictions.

```
┌─────────────────────────────────────────────────────────────────┐
│                    TIERED STORAGE COMPLIANCE                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Key Considerations:                                             │
│                                                                  │
│  1. Data Residency                                               │
│     • Object storage bucket must be in the same region           │
│       as the broker if data residency laws apply                │
│     • Verify object storage provider's replication policy       │
│       (cross-region replication may violate GDPR)               │
│                                                                  │
│  2. Encryption at Rest                                           │
│     • Object storage should use server-side encryption          │
│     • Consider customer-managed keys (CMK) for sensitive data   │
│     • Key rotation must cover both local and remote tiers       │
│                                                                  │
│  3. Access Control                                               │
│     • Object storage IAM policies must restrict access          │
│     • Only the RemoteStorageManager should have write access    │
│     • Audit logging should cover remote tier access             │
│                                                                  │
│  4. GDPR Right to Erasure                                       │
│     • Tombstone-based deletion works the same for tiered data  │
│     • BUT: compaction on remote tier depends on                 │
│       RemoteLogManager scheduling                              │
│     • Verify that remote segments are actually deleted          │
│       after retention expires                                   │
│                                                                  │
│  5. Cloud-Native Brokers (WarpStream, AutoMQ)                   │
│     • BYOC (Bring Your Own Cloud) model: data stays in         │
│       customer's account — simplifies compliance                │
│     • Object storage permissions managed by customer            │
│     • No data leaves customer's network boundary               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
