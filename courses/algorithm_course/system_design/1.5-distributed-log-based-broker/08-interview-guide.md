...
4. **Step-by-step plan in plain English:**
   ```
   while (running):
...
### Trap 5: "Why would you need a separate message queue if you already have Kafka?"

**What they're testing:** Understanding the log-vs-queue semantic difference and awareness of Share Groups.

**Strong Response:**
```
"Historically, there were good reasons to run both:

1. Log-based (Kafka): Event streaming, replay, per-partition ordering
2. Queue-based (RabbitMQ/SQS): Task distribution, per-message ack, priorities

The key difference is consumption model:
- Kafka: Each partition assigned to exactly one consumer. Scaling requires
  adding partitions. Ordering guaranteed.
- Queue: Any consumer can pick up any message. Natural load distribution.
  No ordering guarantee.

However, KIP-932 Share Groups (Kafka 4.0) blur this boundary. Share Groups
allow multiple consumers to read from the same partition without exclusive
assignment — essentially queue semantics within Kafka.

So my answer is evolving: for NEW systems on Kafka 4.0+, evaluate whether
Share Groups can handle your queue workloads before adding a separate
message queue. But for systems needing message priorities, complex routing
(topic exchanges), or per-message TTLs, a dedicated queue still makes sense."
```

---

### Trap 6: "Why partition by key instead of round-robin?"
...
### Kafka 4.0 and the ZooKeeper Removal

```
"A key 2025 development is Kafka 4.0, which completely removes ZooKeeper.
KRaft is now the only metadata management option. This simplifies operations
significantly — one system to deploy, monitor, and operate instead of two.

The deeper impact is on scaling: KRaft handles metadata more efficiently than
ZooKeeper, supporting larger partition counts per cluster and faster controller
failover (milliseconds vs seconds)."
```

### Tiered Storage Changes the Capacity Planning Equation

```
"Tiered storage (KIP-405, GA in Kafka 4.0) is architecturally significant
because it decouples storage capacity from compute capacity.

Before: Increasing retention from 7 to 30 days meant 4.3x more broker disk.
After: Old segments transparently offload to object storage at ~10x lower cost.

The trade-off is a two-tier latency profile: <5ms for recent data (local),
50-200ms for historical data (object storage). Consumers must tolerate this
or applications need to distinguish hot vs cold access patterns."
```

### Share Groups Blur the Queue-vs-Log Boundary

```
"KIP-932 Share Groups (Early Access in Kafka 4.0) introduce queue semantics
into the log-based broker. Multiple consumers can read from the same partition
without exclusive assignment — no rebalance needed.

This is significant because it eliminates the main reason teams chose a
separate message queue (RabbitMQ/SQS) alongside Kafka: task distribution
workloads that don't need per-partition ordering.

The trade-off: Share Groups provide at-least-once delivery with per-record
acknowledgment, not the per-partition ordering guarantee of consumer groups.
Use consumer groups when ordering matters; Share Groups when it doesn't."
```

### Cloud-Native Alternatives (WarpStream, AutoMQ, Redpanda)

```
"The broker landscape has fragmented into a spectrum based on storage coupling:

- Redpanda: C++ thread-per-core, Raft per partition, lowest latency
- Kafka: JVM, KRaft, tiered storage for cost optimization
- AutoMQ: Kafka fork, WAL on block storage + S3, stateless brokers
- WarpStream: Zero disk, stateless agents, object storage only

The right choice depends on the deployment: on-premise with NVMe → Redpanda
or Kafka for lowest latency. Cloud-native, cost-sensitive → AutoMQ or
WarpStream. WarpStream trades 400ms latency for ~80% cost savings and
zero operational complexity (no replication, no partition reassignment)."
```
...
| "ZooKeeper for coordination" | Outdated (removed in 4.0) | KRaft is now the only option |
| Ignoring consumer groups | Core concept | Explain partition assignment |
| "Just use WarpStream for everything" | Wrong cost/latency trade-off | Discuss the storage coupling spectrum |
...
| Consumer group rebalance (cooperative) | 1-10 seconds | KIP-848 further reduces |
| Consumer group rebalance (KIP-848) | Sub-second | Server-side, heartbeat-based |
| ISR lag threshold | 10,000 messages or 30 seconds | replica.lag.time.max.ms |
| Retention (local, tiered storage) | 1-7 days | Hot data on local disk |
| Retention (remote, tiered storage) | 30-90+ days | Cold data on object storage |
| Replication factor | 3 | Standard for production |
| Tiered storage cost savings | 3-9× | vs. all-local retention |
| WarpStream p99 latency | 400-600ms (S3) / 33ms (Express) | Object storage bound |
| KRaft controller failover | 1-5 seconds | vs 30+ seconds with ZooKeeper |
...
5. Storage coupling: Tight (NVMe, low latency) vs loose (object storage, low cost)
6. Consumer Groups vs Share Groups: Ordering vs parallelism flexibility
```
...
In 2025+: KRaft-only (ZooKeeper removed), tiered storage for cost-effective
long retention, KIP-848 for faster rebalancing, and Share Groups emerging
as queue semantics within the log-based model."
```
