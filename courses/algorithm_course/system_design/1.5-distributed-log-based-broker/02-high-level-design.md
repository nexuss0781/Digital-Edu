...
|--------|-------------|------|------|
| ZooKeeper | External coordination service | Mature, battle-tested | **Removed in Kafka 4.0** |
| **KRaft (Required)** | Internal Raft-based quorum | Single system, faster failover, simpler ops | Only option since Kafka 4.0 (March 2025) |

**Status:** ZooKeeper was completely removed in Kafka 4.0 (March 2025). KRaft is the only metadata management option.

**Rationale:**
- Single system to operate (no separate ZK cluster)
- Faster controller failover (milliseconds vs seconds)
- Better scalability (millions of partitions)
- No ZooKeeper znodes limits
- Simpler deployment, monitoring, and operations
...
**Recommendation:** Time-based for events (7 days), compaction for state topics. With tiered storage (Kafka 4.0+), use 1-7 day local retention + 30-90 day remote retention for cost-effective long retention.

### Decision 6: Storage Architecture (2025+)

| Option | Description | Latency | Cost | Scaling |
|--------|-------------|---------|------|---------|
| **Local only** | All data on broker NVMe/SSD | <5ms | High (NVMe × RF) | Stateful (data movement) |
| **Tiered (Recommended)** | Hot on local, cold on object storage | <5ms hot, 50-200ms cold | Moderate | Semi-stateful |
| WAL + Object | Write to block storage WAL, flush to object storage | ~10ms | Low | Stateless (seconds) |
| Object only | Direct write to object storage | 400-600ms | Lowest | Stateless (instant) |

**Recommendation:** Tiered storage for general-purpose Kafka. Evaluate WAL+Object (AutoMQ) or Object-only (WarpStream) for cloud-native, cost-sensitive, or latency-tolerant workloads.

**Rationale:**
- Tiered storage is the lowest-risk upgrade — transparent to consumers, 3-9× storage cost reduction
- Cloud-native alternatives trade latency for operational simplicity and cost
- On-premise deployments should use local-only or tiered
