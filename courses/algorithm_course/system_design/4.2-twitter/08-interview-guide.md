...
### "How would you implement Community Notes?"

**Trap**: Treating it as a simple voting system

**Good Answer**:
> "Community Notes is not majority-vote. It uses matrix factorization to find bridging consensus:
> 1. Each rater has an intercept (helpfulness tendency) and a factor (latent opinion position)
> 2. A note scores high only if users from different factor positions rate it helpful
> 3. This means partisan pile-ons are mathematically ineffective
> 4. Trade-off: convergence takes 30 min to 4 hours, so notes may appear delayed
> 5. Architecture: separate scoring pipeline from timeline, batch model refit, confidence thresholds"

### "How would you integrate Grok AI into the timeline?"

**Trap**: Adding AI inline and blowing the latency budget

**Good Answer**:
> "The timeline already uses 220 CPU-seconds in 1.5s. Adding LLM inference synchronously would break the budget. I'd use a dual-inference architecture:
> 1. **Async enrichment**: AI summaries and translations render progressively after initial timeline load
> 2. **Parallel pipeline**: AI-generated reply suggestions run independently with their own latency budget
> 3. **Resource isolation**: Strict GPU allocation — ranking ML and generative AI cannot starve each other
> 4. **Cache AI outputs**: Many users see the same trending content; cache AI summaries per tweet, not per user"

### "How do you reduce infrastructure costs by 60%?"

**Trap**: Just saying "use fewer servers"

**Good Answer**:
> "Cost reduction at this scale requires surgical trade-offs:
> 1. **Cache tier audit**: Remove caches with <20% hit rate — they cost more than the DB load they save
> 2. **Service consolidation**: Merge low-traffic microservices into shared processes (lose isolation, save overhead)
> 3. **Redundancy calibration**: Reduce replication factor for non-critical data (timelines: 2 replicas, tweets: keep 3)
> 4. **GPU multiplexing**: Share GPU pools across inference workloads with priority scheduling
> 5. **Storage tiering**: Move tweets >30 days to cold storage with on-demand hydration
> 6. **Critical rule**: Never reduce write-path replication or auth redundancy — the blast radius is catastrophic"
