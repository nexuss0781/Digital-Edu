...
5. [Slowest part of the process Analysis](#Slowest part of the process-analysis)
...
### Slowest part of the process 1: Timeline Assembly Latency
...
ROOT CAUSES:
  ...
  2. ML model inference Slowest part of the process (GPU saturation)
...
### Slowest part of the process 2: Celebrity Fan-out Write Amplification
...
### Slowest part of the process 3: Search Indexing Latency
...
### Slowest part of the process 4: Trends False Positives
...
## Deep Dive 5: Community Notes Consensus Algorithm

### Overview

Community Notes (formerly Birdwatch) uses a **bridging-based consensus algorithm** that surfaces notes agreed upon by users who typically disagree. This is architecturally unique -- it is not a simple voting system.

### Bridging Algorithm Architecture

```mermaid
flowchart TB
    subgraph Input["Input Layer"]
        Notes[Community Notes<br/>Written by Contributors]
        Ratings[User Ratings<br/>Helpful / Not Helpful]
    end

    subgraph Model["Matrix Factorization Model"]
        RatingMatrix[Rating Matrix<br/>Users × Notes]
        Factorization[Factor Decomposition<br/>User Intercept + Factor]
        NoteScoring[Note Scoring<br/>Bridging Requirement]
    end

    subgraph Consensus["Consensus Decision"]
        ThresholdCheck{Score > 0.40<br/>AND Raters ≥ 5?}
        Display[Display Note<br/>on Tweet]
        Suppress[Suppress Note]
    end

    Notes --> RatingMatrix
    Ratings --> RatingMatrix
    RatingMatrix --> Factorization
    Factorization --> NoteScoring
    NoteScoring --> ThresholdCheck
    ThresholdCheck -->|Yes| Display
    ThresholdCheck -->|No| Suppress

    classDef input fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef model fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef decision fill:#fff3e0,stroke:#e65100,stroke-width:2px
    class Notes,Ratings input
    class RatingMatrix,Factorization,NoteScoring model
    class ThresholdCheck,Display,Suppress decision
```

### Scoring Algorithm

```
ALGORITHM: Community Notes Bridging Score

The key insight: decompose each user into an intercept (general helpfulness tendency)
and a factor (position on a latent opinion spectrum, roughly correlating with political leaning).

MATRIX FACTORIZATION:
  rating(user_u, note_n) ≈ μ + intercept_u + intercept_n + factor_u × factor_n

WHERE:
  μ = global mean rating
  intercept_u = user's tendency to rate helpful/not helpful
  intercept_n = note's overall helpfulness (THIS IS THE SCORE)
  factor_u = user's position on latent spectrum
  factor_n = note's appeal along that spectrum

BRIDGING REQUIREMENT:
  A note with high intercept_n means it is rated helpful REGARDLESS of the rater's factor_u.
  If a note only appeals to one side (high factor_n), its intercept_n will be low
  even if it has many "helpful" ratings -- because the factor term explains those ratings.

PRACTICAL IMPLICATIONS:
  - Partisan notes get low scores (factor explains the ratings)
  - Bridging notes get high scores (intercept explains the ratings)
  - The model must re-fit periodically as new ratings arrive
  - Convergence time: 30 minutes to 4 hours depending on rating velocity
  - Notes only display after confidence interval narrows sufficiently
```
...
|--------------|-------------|--------|------------|
| **Slow convergence** | Not enough diverse raters for bridging signal | Note displayed too late to be useful | Pre-seed with engaged contributors, lower initial threshold |
| **Coordinated rating** | Groups organize to artificially rate notes | False consensus | Factor decomposition naturally detects coordination (same factor_u) |
| **Model staleness** | Ratings arrive faster than model can refit | Outdated scores displayed | Incremental model updates, batch refit every 30 minutes |
| **Edge cases** | Objective facts rated unhelpful by one faction | True notes suppressed | Intercept weighting accounts for this, but extreme cases may need manual review |

---

## Deep Dive 6: Infrastructure Cost Optimization (Post-Acquisition)

### Cost Reduction Architecture Patterns

The post-acquisition infrastructure changes provide valuable lessons in cost optimization:

| Optimization | Technique | Trade-off |
|--------------|-----------|-----------|
| **Service consolidation** | Merge microservices with low traffic into shared processes | Reduced isolation, harder debugging |
| **Cache tier reduction** | Remove intermediate cache layers where hit rates were <20% | Higher DB load, some latency increase |
| **Redundancy reduction** | Reduce replication factor from 3 to 2 for non-critical data | Lower durability, acceptable for cache-like data |
| **GPU sharing** | Multiplex ML inference workloads on shared GPU pools | Noisy neighbor effects, requires careful scheduling |
| **Region consolidation** | Reduce from 3 active regions to 2 + 1 warm standby | Higher latency for some users, reduced blast radius protection |
| **Storage tiering** | Move cold tweets (>30 days) to cheaper storage with on-demand hydration | First-load latency for old content, acceptable for most users |

### Lessons Learned

```
COST vs RELIABILITY TRADE-OFF FRAMEWORK:

For each reduction, evaluate:
  1. Blast radius: How many users affected if this fails?
  2. Recovery time: How fast can we restore from failure?
  3. User impact: Degraded experience vs hard failure?
  4. Revenue impact: Does this affect ads/subscriptions?

SAFE TO REDUCE:
  - Cache redundancy (graceful degradation to DB)
  - ML model replicas (fallback to simpler model)
  - Logging verbosity (reduce storage, keep critical events)

DANGEROUS TO REDUCE:
  - Write path replication (data loss risk)
  - Auth service redundancy (complete lockout)
  - Payment processing reliability (revenue loss)
```
