...
### 2.3 Graceful Degradation Dependency Map
...
---
flowchart TB
    subgraph Critical["Critical Path (cannot degrade)"]
        Q["Message Queues"]
        PA["Provider Adapters"]
    end

    subgraph Degradable["Degradable (serve from cache/fallback)"]
        DR["Device Registry<br/>(stale cache on failure)"]
        UP["User Preferences<br/>(default-allow on failure)"]
        TS["Template Service<br/>(raw text fallback)"]
    end

    subgraph NonBlocking["Non-Blocking (skip on failure)"]
    ...
    classDef degradable fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef nonblocking fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px

    class Q,PA critical
    class DR,UP,TS degradable
    class AN,SEG,SCHED nonblocking
```
...
flowchart LR
    subgraph Hot["Hot Tier (0-7 days)"]
        H1["Notification Log<br/>Fast SSD storage"]
        H2["Delivery Events<br/>In-memory + SSD"]
        H3["Device Registry<br/>Primary + Cache"]
    end

    subgraph Warm["Warm Tier (7-30 days)"]
        W1["Notification Log<br/>Standard storage"]
        W2["Delivery Events<br/>Columnar format"]
    end

    subgraph Cold["Cold Tier (30+ days)"]
        CL1["Notification Archive<br/>Compressed columnar"]
        CL2["Analytics Aggregates<br/>Pre-computed rollups"]
    end

    subgraph Purge["Purge"]
        P1["Auto-delete<br/>per retention policy"]
    end

    Hot -->|"7-day rotation"| Warm
    Warm -->|"30-day rotation"| Cold
    ...
    classDef purge fill:#f5f5f5,stroke:#616161,stroke-width:2px
...
| Data Type | Hot | Warm | Cold | Total Retention |
|---|---|---|---|---|
| **Device tokens** | Active (indefinite) | N/A | Deactivated tokens: 90 days | Active tokens live until deactivated; deactivated purged after 90 days |
| **Notification log** | 7 days | 30 days | 1 year (aggregates only) | Individual records: 30 days; aggregates: 1 year |
| **Delivery events** | 7 days | 30 days | N/A | 30 days total; aggregated into analytics store |
| **User preferences** | Active (indefinite) | N/A | Deleted user prefs: 30 days (for undo) | Matches user account lifecycle |
| **Campaign metadata** | 90 days | 1 year | 2 years | Full campaign audit trail for compliance |
| **Consent records** | Active | N/A | 5 years (GDPR requirement) | Must demonstrate consent was obtained |
| **Analytics aggregates** | Real-time (streaming) | 1 year (hourly) | 3 years (daily rollups) | Supports trend analysis and year-over-year comparison |

### 4.3 Capacity Planning Formula

```
FUNCTION project_capacity(current_devices, growth_rate, months_ahead):
    // Device registry growth
    projected_devices = current_devices × (1 + growth_rate)^months_ahead
    registry_storage = projected_devices × 600 bytes

    // Daily notification volume (assuming 25 notifications per active device)
    active_rate = 0.20  // 20% DAU
    daily_notifications = projected_devices × active_rate × 25

    // Storage per 30-day window
    notification_log = daily_notifications × 30 × 500 bytes
    delivery_events = daily_notifications × 30 × 3 × 200 bytes  // 3 events per notification

    // Peak throughput requirement
    peak_qps = (daily_notifications / 86400) × 17  // 17x peak-to-average ratio

    // Provider connection requirements
    apns_connections = (peak_qps × 0.30) / 500      // 30% iOS, 500 streams/conn
    fcm_connections = (peak_qps × 0.60) / 500       // 60% Android, 500 tokens/batch

    RETURN {
        registry: registry_storage,
        log_30d: notification_log,
        events_30d: delivery_events,
        peak_throughput: peak_qps,
        apns_conns: apns_connections,
        fcm_conns: fcm_connections
    }
```
...
|---|---|---|
| **Device token registration** | Strong (synchronous replication) | Lost token = lost ability to reach device; difficult to recover |
| **User preference changes** | Strong (write-through to cache) | Sending to opted-out user is a compliance violation; must be consistent within 24 hours |
| **Notification deduplication** | Strong (within dedup window) | Duplicate OTP or payment notification damages user trust |
| **Delivery status tracking** | Eventual (< 5 seconds) | Dashboard freshness is not critical; users don't poll individual notification status |
| **Analytics aggregation** | Eventual (< 5 minutes) | Campaign metrics are trend indicators, not real-time decision inputs |
| **Rate limit counters** | Best-effort (local + periodic sync) | Slight over/under-count is acceptable; strict enforcement not required at per-user level |
| **Segment membership** | Eventual (15-minute refresh) | Pre-computed segments can tolerate stale membership for scheduled campaigns |
