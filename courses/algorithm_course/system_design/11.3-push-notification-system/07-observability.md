...
| **Queue depth growing** | Any queue depth increasing for 15+ consecutive minutes | P2 | Check consumer health; investigate Slowest part of the process; consider scaling consumers |
...
| **Delivery latency degradation** | Transactional P95 latency > 1 second for 5 minutes | P2 | Check each pipeline stage for Slowest part of the process; profile slow component |
...
| **Notification delivery delayed** | 1. Check queue depths at each pipeline stage<br/>2. Identify which stage has growing backlog<br/>3. Check consumer health for that stage | Scale consumers for Slowest part of the process stage. If provider throttling: enable aggressive pacing. If registry slow: check cache and replicas. |
...
|---|---|---|---|
| **Token invalidation spike** | Invalid token rate > 3× rolling 7-day average for 15 min | P2 | Pause token cleanup to investigate; alert team; may indicate mass app update or provider certificate rotation |
| **Delivery rate cliff** | Per-provider delivery success rate drops > 20% within 5 min | P1 | Open circuit breaker for affected provider; queue sends; page on-call |
| **Engagement anomaly** | Campaign open rate < 0.5% (vs category average of 5-8%) | P3 | Flag campaign for review; may indicate template rendering bug or deep link failure |
| **Fan-out stall** | Active campaign with > 100K queued but 0 sends for 3+ min | P1 | Check fan-out worker health; verify queue consumer assignment; auto-restart stuck partitions |
| **Credential rotation failure** | Provider auth errors (401/403) after credential refresh | P1 | Roll back to previous credential version; alert security team; manual investigation |
| **Tenant traffic spike** | Single tenant's volume exceeds 10× their 7-day average | P2 | Apply tenant-level throttling; notify tenant admin; may indicate compromised API key |

### 6.2 Provider Health Scoring

```
FUNCTION calculate_provider_health(provider, window = 5_MINUTES):
    metrics = get_provider_metrics(provider, window)

    // Component scores (0-100)
    success_score = (metrics.success_rate / target_success_rate) × 100
    latency_score = MAX(0, 100 - (metrics.p95_latency_ms / target_p95_ms) × 100)
    throttle_score = MAX(0, 100 - (metrics.throttle_rate × 500))
    connection_score = (metrics.healthy_connections / metrics.total_connections) × 100
    ...
    IF health < 30:
        ...
        alert_p1("Provider " + provider + " health critical: " + health)
    ELSE IF health < 60:
        enable_pacing(provider)
        alert_p2("Provider " + provider + " health degraded: " + health)
...
## 7. Notification Debugging Workflow

When investigating why a specific notification was not delivered:

```
FUNCTION debug_notification(notification_id):
    // Step 1: Check notification log
    record = notification_log.get(notification_id)
    IF NOT record:
        RETURN "Notification not found — may have been rejected at ingestion"

    // Step 2: Check current status
    SWITCH record.status:
        CASE "REJECTED":
            RETURN "Rejected at validation: " + record.rejection_reason
        CASE "SUPPRESSED":
            RETURN "Suppressed: " + record.suppression_reason
            // Common: opted_out, frequency_cap, quiet_hours, deduplicated
        CASE "QUEUED":
            RETURN "Still in queue — check queue depth and consumer health"
        CASE "SENDING":
            RETURN "In-flight to provider — may be retrying"
        CASE "SENT":
            // Provider accepted but not yet confirmed delivered
            RETURN "Sent to " + record.provider + " at " + record.sent_at +
                   " — awaiting delivery confirmation (provider-dependent)"
        CASE "FAILED":
            RETURN "Failed after " + record.retry_count + " retries: " +
                   record.error_code + " (" + record.provider + ")"
        CASE "DELIVERED":
            RETURN "Delivered at " + record.delivered_at +
                   " — if user didn't see it, check device notification settings"

    // Step 3: Check device status
    device = device_registry.get(record.device_id)
    IF device.status == "INVALID":
        RETURN "Device token was invalidated at " + device.invalidated_at
    IF device.last_active > 60_DAYS_AGO:
        RETURN "Device appears dormant (last active: " + device.last_active + ")"

    RETURN "No obvious issue found — escalate with trace_id: " + record.trace_id
```
...
### 7.1 Runbook: Provider Credential Rotation

```
TRIGGER: Scheduled (quarterly) or emergency (compromise suspected)

STEPS:
1. Generate new credential in provider console (APNs key, FCM service account, etc.)
2. Upload new credential to secret vault as NEW version (do NOT replace active)
3. Deploy credential to staging adapter pool
4. Send 100 test notifications through staging → verify delivery
5. If staging succeeds:
   a. Update production adapter pool to use NEW credential version
   b. Monitor error rates for 15 minutes
   c. If error rate < 0.1%: mark new credential as ACTIVE
   d. If error rate spikes: roll back to previous credential version
6. After 7 days with no issues: deactivate previous credential version

ROLLBACK:
- Secret vault retains previous 5 credential versions
- Rollback is a configuration change (no deployment needed)
- Rollback time: < 2 minutes
```

### 7.2 Runbook: Mass Token Invalidation Event

```
TRIGGER: Token invalidation rate > 5× baseline for 30+ minutes

DIAGNOSTIC STEPS:
1. Check which provider(s) are returning invalid-token errors
   → If APNs only: check if APNs certificate was rotated or revoked
   → If FCM only: check if FCM server key was regenerated
   → If all providers: check if app was removed from store or had breaking update

2. Check invalidation pattern:
   → Uniform across all token ages: provider-side change (cert rotation)
   → Only old tokens (> 90 days): normal cleanup, may have been deferred
   → Only recent tokens: app bug in token registration (new app version)

3. Check app store reviews for mass uninstall signals

RESPONSE:
- If provider cert rotation: expected behavior; monitor until invalidation
  rate returns to baseline. Old tokens tied to old cert are permanently invalid.
- If app bug: escalate to app team; pause notification sends to affected
  app version; coordinate fix.
- If mass uninstall: no system action needed; token cleanup will purge
  naturally. Alert product team for investigation.
```

### 7.3 Runbook: Campaign Fan-Out Stuck

```
TRIGGER: Active campaign with 0 sends for > 5 minutes despite queued messages

DIAGNOSTIC STEPS:
1. Check fan-out worker health: are workers running and consuming?
2. Check queue consumer group: are partitions assigned?
3. Check device registry: is registry responding to batch lookups?
4. Check provider adapters: are provider queues accepting messages?

COMMON CAUSES:
- Consumer group rebalance (workers restarting): wait 2 min for rebalance
- Device registry overloaded: scale cache cluster; enable stale-serve
- Queue partition leader election: wait for ISR to elect new leader (< 30s)
- Provider circuit breaker open: check provider health; wait for half-open test

ESCALATION:
- If not resolved in 10 minutes: page on-call engineering
- If campaign is time-sensitive (breaking news): manually reassign partitions
  and restart fan-out workers
```
