...
### 4.2 Token Lifecycle Overview

Device tokens flow through a lifecycle managed by the device registry:
...
---
stateDiagram-v2
    [*] --> REGISTERED: SDK registers token
    REGISTERED --> ACTIVE: First successful send
    ACTIVE --> ACTIVE: Successful sends
    ACTIVE --> REFRESHED: Provider issues new token (FCM canonical ID)
    REFRESHED --> ACTIVE: New token validated
    ACTIVE --> STALE: No send success for 60 days
    STALE --> ACTIVE: Successful send (user returned)
    STALE --> INVALID: Provider returns 410/UNREGISTERED
    ACTIVE --> INVALID: Provider returns 410/UNREGISTERED
    STALE --> PURGED: Validation sweep confirms stale
    INVALID --> PURGED: Cleanup job removes
    PURGED --> [*]
```

### 4.3 Provider Connection Management
...
Every notification transitions through a well-defined state machine from creation to terminal state:
...
---
stateDiagram-v2
    [*] --> ACCEPTED: API receives request
    ACCEPTED --> VALIDATING: Schema + auth check
    VALIDATING --> REJECTED: Invalid payload/auth
    VALIDATING --> RESOLVING: Valid request
    RESOLVING --> RESOLVED: Devices found
    RESOLVING --> SUPPRESSED: User opted out / frequency cap
    RESOLVED --> RENDERING: Template lookup
    RENDERING --> SCHEDULED: Deferred delivery
    RENDERING --> QUEUED: Immediate send
    SCHEDULED --> QUEUED: Timer fires
    QUEUED --> SENDING: Worker picks up
    SENDING --> SENT: Provider accepted (200)
    SENDING --> RETRYING: Provider error (5xx/429)
    RETRYING --> SENDING: Retry attempt
    RETRYING --> FAILED: Max retries exceeded
    SENT --> DELIVERED: Provider confirms delivery
    SENT --> EXPIRED: TTL exceeded before delivery
    DELIVERED --> OPENED: User tapped notification
    DELIVERED --> DISMISSED: User swiped away
    OPENED --> CLICKED: User tapped action button
    REJECTED --> [*]
    SUPPRESSED --> [*]
    FAILED --> [*]
    EXPIRED --> [*]
...
**Terminal states:** REJECTED, SUPPRESSED, FAILED, EXPIRED, CLICKED, DISMISSED. Each terminal state is recorded in the notification log with the reason and timestamp for compliance auditing.

---

## 7. Feedback Processing Pipeline

Provider feedback closes the control loop that keeps the notification system healthy:
...
---
flowchart TB
    subgraph Providers["Provider Feedback Channels"]
        AF["APNs HTTP/2 Response<br/>(inline: 200, 400, 410, 429)"]
        FF["FCM Response<br/>(per-token errors in multicast)"]
        HF["HMS Response<br/>(batch error codes)"]
        WF["Web Push Response<br/>(201, 404, 410, 429)"]
    end

    subgraph FeedbackPipeline["Feedback Processing"]
        FP["Feedback<br/>Parser"]
        TC["Token<br/>Classifier"]
        RA["Rate Limit<br/>Analyzer"]
        DT["Delivery<br/>Tracker"]
    end

    subgraph Actions["System Actions"]
        DR["Device Registry<br/>(deactivate tokens)"]
        RL["Rate Limiter<br/>(adjust pacing)"]
        AN["Analytics<br/>(delivery status)"]
        AL["Alert Manager<br/>(anomaly detection)"]
        FB["Fallback Trigger<br/>(escalate to email/SMS)"]
    end
    ...
    TC -->|"410/UNREGISTERED"| DR
    TC -->|"invalid token spike"| AL
    RA -->|"429/QUOTA_EXCEEDED"| RL
    RA -->|"sustained throttle"| AL
    DT -->|"delivered/opened"| AN
    DT -->|"failed after retries"| FB

    classDef provider fill:#fce4ec,stroke:#b71c1c,stroke-width:2px
    classDef pipeline fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef action fill:#e1f5fe,stroke:#01579b,stroke-width:2px

    class AF,FF,HF,WF provider
    class FP,TC,RA,DT pipeline
    class DR,RL,AN,AL,FB action
```

### 7.1 Feedback Classification

| Feedback Signal | Source | Action | Urgency |
|---|---|---|---|
| **Invalid token (410/UNREGISTERED)** | APNs, FCM, HMS | Immediately deactivate token in registry; decrement user device count | Real-time (< 30s) |
| **Token refresh** | FCM (canonical registration ID) | Update token in registry; deactivate old token | Real-time |
| **Rate limit (429)** | All providers | Back off send rate; activate campaign pacing; alert if sustained | Real-time |
| **Server error (5xx)** | All providers | Retry with exponential backoff; open circuit breaker if sustained | Real-time |
| **Delivery confirmation** | APNs (via APNs feedback service), FCM (delivery receipts) | Update notification status to DELIVERED; record latency | Eventual (< 5 min) |
| **Subscription expired (404)** | Web Push endpoints | Remove subscription; trigger re-subscription prompt on next app open | Real-time |
...
- API layer: tenant_id extracted from auth token
- Queue layer: tenant_id in message metadata for routing
- Registry: tenant_id is partition key prefix
- Analytics: row-level filtering by tenant_id
- Credentials: per-tenant encryption keys in vault
```

**Noisy neighbor prevention:** Per-tenant rate limiting at the API gateway ensures one tenant's campaign burst cannot consume shared provider quota. Token buckets are sized per tenant tier (Free: 10K/day, Growth: 1M/day, Enterprise: custom).
