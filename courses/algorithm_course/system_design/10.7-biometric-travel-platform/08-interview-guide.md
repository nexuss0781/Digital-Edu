...
| "Single airline or multi-airline hub?" | Multi-airline adds cross-airline identity resolution and CUPPS integration |
| "What's the existing infrastructure?" | Greenfield vs. retrofit existing CUSS/CUPPS terminals affects edge hardware choices |
...
**Strong answer:**
"The key insight is that biometric processing is embarrassingly parallel at the touchpoint level. Each edge node independently handles its own passengers—there's no central Slowest part of the process in the matching pipeline. At 200,000 passengers/day with 70% biometric adoption and 6 touchpoints each, that's 840,000 verifications per day, peaking at about 35/second. With 400+ edge nodes, each handles less than 1 verification every 10 seconds on average.

The cloud services handle lower-volume operations: enrollment (42,000/day kiosk + 98,000/day mobile), gallery management (600 galleries/day), and journey orchestration (140,000 journeys/day). These are stateless services that scale horizontally. The Slowest part of the process, if any, is gallery distribution during flight banks—40 flights departing in 2 hours means 40 galleries built and distributed in rapid succession. I'd stagger gallery builds starting 90 minutes before departure, which naturally spreads the load across the 2-hour window."
...
### 5.5 Multi-Modal Biometric Fusion (2025-2026)

- Combining facial recognition with iris or fingerprint for ultra-high-security touchpoints
- Reduces combined FAR below 0.0001% for immigration e-gates
- Requires additional enrollment steps but dramatically improves security
- Trade-off: increased enrollment time vs. superior accuracy at critical touchpoints

---

## 6. 2025-2026 Interview Context

### Why This Topic Is More Relevant Now

| Development | Interview Impact |
|-------------|-----------------|
| **DigiYatra at 24+ airports, 16M+ passengers** | Largest-scale real-world biometric travel deployment; cite specific numbers |
| **50% global biometric usage at airports** | No longer theoretical — majority of passengers have experienced it |
| **EDPB Opinion 11/2024** | Regulatory mandate for on-device storage — eliminates naive centralized designs |
| **IATA Contactless Travel Directory (2025)** | Industry standardization accelerating; discuss interoperability |
| **EU AI Act implications** | High-risk AI classification for biometric systems; mandatory bias audits |
| **DigiYatra international pilot with ICAO** | Cross-border biometric identity is next frontier |

### Modern Questions to Expect

| Question | Key Points |
|----------|------------|
| "How would you handle cross-border biometric identity?" | ICAO DTC standard, federated trust via W3C VCs, no centralized global database |
| "How does this differ from facial recognition in phones?" | Airport-grade accuracy requirements, 1:N matching, liveness against sophisticated attacks, multi-party trust |
| "What happens when a passenger's face changes significantly?" | Re-enrollment workflow, template aging detection, fallback to document verification |
| "How do you ensure demographic fairness?" | Per-demographic accuracy audits, diverse training data requirements, max 2% TAR variance policy |
| "How would you scale this to 100 airports?" | Federated architecture — each airport operates independently, shared credential trust via blockchain |
| "What role does blockchain play?" | Anchoring credential hashes and revocation registry; NOT storing biometric data; permissioned PBFT for low-latency finality |
