...
Strategy 1: Brute-Force Batch Comparison (Baseline)
  ...
  Verdict: Brute Force (Checking every single possibility) is sufficient for N < 10,000
...
Resolving a Decentralized Identifier (DID) requires fetching the DID Document, which contains the public keys needed for signature verification. This can be a latency Slowest part of the process.
...
Slowest part of the process: Gallery distribution
...
## Case Study: DigiYatra — India's National Biometric Travel Platform (2022-2026)

### Context

DigiYatra is India's national biometric boarding system, launched in 2022 at 7 airports. By February 2026, it had expanded to 24+ airports and processed over 16 million passengers. It represents the world's largest opt-in facial recognition deployment for air travel.

### Architecture Decisions

| Decision | DigiYatra Choice | Rationale |
|----------|-----------------|-----------|
| **Identity anchor** | Aadhaar e-KYC (domestic), e-Passport NFC (international pilot) | Leverage existing national identity infrastructure |
| **Template storage** | DigiLocker-based with 24-hour auto-deletion | Privacy compliance with India's DPDP Act |
| **Edge processing** | Dedicated e-gates at terminal entry, security, and boarding | Sub-second verification at each touchpoint |
| **Gallery management** | Per-flight galleries pre-staged to boarding gates | Enable 1:N walk-through boarding |
| **International expansion** | ICAO pilot trial agreement (2025) | Testing DTC-based cross-border identity |

### Results

| Metric | Achievement |
|--------|-------------|
| Total passengers processed | 16M+ (as of Feb 2026) |
| Average dwell time reduction | 25% compared to manual processing |
| Airport coverage | 24+ airports (targeting all Tier-1 by mid-2027) |
| Throughput per biometric lane | 20-25 passengers/minute |
| Template deletion compliance | 100% within 24-hour window |

### Architectural Insight

DigiYatra validates that a privacy-preserving, opt-in biometric system can achieve mainstream adoption (16M+ passengers) when backed by an existing national identity infrastructure (Aadhaar). The key lesson: the enrollment friction problem is solved by anchoring to a pre-existing identity credential rather than requiring standalone biometric enrollment. This reduces enrollment time from 90 seconds (standalone) to under 30 seconds (Aadhaar-linked).
