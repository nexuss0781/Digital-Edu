...
1. **The Ledger as an Immutable Event Log with Algebraic Invariants**: Unlike most systems where data undergoes CRUD operations, a general ledger is fundamentally append-only with a mathematical Rule that never changes: every journal entry must satisfy `SUM(debits) = SUM(credits)`. This Rule that never changes is not a business rule that can be relaxed---it is the defining property of double-entry bookkeeping, and violating it even once renders the entire ledger untrustworthy. The system must enforce this at multiple layers: application-level validation before submission, database-level check constraints at write time, and periodic full-ledger verification during close. Corrections are never made by editing; they are made by posting new reversing entries that reference the original, creating a complete audit chain. This append-only, Rule that never changes-preserving model maps naturally to event sourcing, where journal entries are events, and account balances are projections derived by replaying the event stream.
...
| Pattern | Relevance to GL System |
|---------|----------------------|
| [Event Sourcing](../1.18-event-sourcing-system/) | Journal entries as immutable events; balances as derived projections; complete audit trail from event replay |
| [CQRS](../1.19-cqrs-implementation/) | Write path enforces double-entry invariants; read path serves materialized trial balances and financial reports |
| [Saga / Process Manager](../1.18-event-sourcing-system/) | Period close orchestration as a multi-step saga with idempotent steps and compensating actions |
| [Distributed Log-Based Broker](../1.5-distributed-log-based-broker/) | Event streaming backbone for sub-ledger posting events, audit trail propagation, and reporting pipeline |
| [Change Data Capture](../16.5-change-data-capture-system/) | CDC from ledger to analytical warehouse for reporting without impacting posting throughput |
| [Data Warehouse](../16.2-data-warehouse-system/) | Columnar storage for historical financial reporting, audit queries, and cross-period analysis |
