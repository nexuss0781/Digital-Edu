...
sequenceDiagram
    ...
    JES->>JES: Enforce double-entry Rule that never changes (sum debits = sum credits)
...
- **Each phase emits progress events** consumed by a close dashboard for real-time visibility. Slowest part of the process detection highlights tasks blocking the close.
...
**Matching algorithm (Step-by-step plan in plain English):**
...
| **Optimistic locking for concurrent postings** | Version-based CAS on account balance records | Pessimistic locks on hot accounts (cash, revenue) Slowest part of the process during month-end close and payroll | Retry overhead under contention; requires idempotent posting logic |
| **Partitioning by fiscal period** | Ledger data partitioned by period; closed partitions independently archived | Financial queries are period-scoped; partition Cutting off unnecessary steps eliminates irrelevant data; cold storage for closed periods | Cross-period queries (YTD) fan out; partition boundary management |
| **Double-entry validation at service boundary** | Balanced-entry Rule that never changes enforced before persistence | API-boundary validation prevents corruption and gives immediate feedback; DB constraints as safety net | Write-path latency; multi-currency needs real-time exchange rate lookup |
...
|-----------|-----------------|------------------|
| **Journal Entry Service** | Accept, validate, and persist journal entries; enforce double-entry Rule that never changes; support manual, recurring, and system-generated entries; manage entry lifecycle (DRAFT > PENDING_APPROVAL > APPROVED > POSTED > REVERSED) | COA Service, Period Mgmt, GL Posting Engine, Event Store |
...
| **Inventory Ledger Service** | Maintain inventory valuation (FIFO (First-In-First-Out, like a line at a store), LIFO (Last-In-First-Out, like a stack of plates), weighted average); generate COGS entries on movements; support revaluation and write-downs | Primary DB, Journal Entry Service |
...
| **Partitioning (Time-Based)** | Yes | Ledger data partitioned by fiscal period. Partition Cutting off unnecessary steps for period-scoped queries; independent archival of closed periods. |
...
## 7. Multi-Currency Data Flow

Multi-currency transactions traverse three conversion layers before reaching the general ledger. Each layer preserves the original amounts for audit while computing derived amounts for reporting.

```mermaid
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph Source["Transaction Layer"]
        TXN["Source Transaction\n(Transaction Currency)"]
    end

    subgraph Conversion["Conversion Layer"]
        RATE["Rate Lookup Service"]
        FC["Functional Currency\nConversion"]
        RC["Reporting Currency\nConversion"]
    end

    subgraph Storage["Storage Layer"]
        JL["Journal Line\n(3 currency amounts stored)"]
    end

    subgraph Periodic["Period-End Processing"]
        REVAL["Revaluation Engine"]
        TRANS["Translation Engine\n(for consolidation)"]
        CTA["CTA Entry\n(Translation Adjustment)"]
    end

    TXN -->|"amount in source currency"| FC
    RATE --> FC & RC
    FC -->|"functional amount = source × spot rate"| JL
    RC -->|"reporting amount = functional × closing rate"| JL
    TXN -->|"original amount preserved"| JL

    JL --> REVAL
    REVAL -->|"unrealized gain/loss entries"| JL
    JL --> TRANS
    TRANS -->|"BS: closing rate, P&L: avg rate"| CTA
    ...
    class TXN client
    class RATE,FC,RC,REVAL,TRANS service
    class JL,CTA data
```

**Rate types and their usage:**

| Rate Type | When Used | Source |
|-----------|-----------|--------|
| **Spot rate** | At transaction time for functional currency conversion | Market data feed, cached with 15-minute staleness threshold |
| **Closing rate** | Period-end for balance sheet revaluation and consolidation translation | Central bank published rate at period close date |
| **Average rate** | Period average for income statement translation during consolidation | Computed as weighted average of daily closing rates |
| **Budget rate** | For budget-to-actual variance analysis at planned exchange rates | Manually set at budget approval time |
| **Historical rate** | For non-monetary item translation (equity, fixed assets) | Rate at original transaction date, immutable once recorded |

---

## 8. Consolidation Hierarchy and Flow

Multi-entity consolidation follows a bottom-up aggregation pattern where subsidiary trial balances are translated, intercompany transactions are eliminated, and minority interests are computed before the consolidated statements are produced.

```mermaid
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph Subsidiaries["Subsidiary Level"]
        S1["Entity A\n(USD functional)"]
        S2["Entity B\n(EUR functional)"]
        S3["Entity C\n(JPY functional)"]
        S4["Entity D\n(GBP functional)"]
    ...
    subgraph SubGroup["Sub-Group Consolidation"]
        SG1["Sub-Group 1\n(A + B)"]
        SG2["Sub-Group 2\n(C + D)"]
    end

    subgraph Translation["Currency Translation"]
        TR["Translate to\nReporting Currency (USD)"]
    end

    subgraph Elimination["Elimination Processing"]
        ICE["Intercompany\nElimination"]
        INV["Investment\nElimination"]
        MIN["Minority Interest\nComputation"]
    end

    subgraph Parent["Consolidated Output"]
        CTB["Consolidated\nTrial Balance"]
    ...
    S1 & S2 --> SG1
    S3 & S4 --> SG2
    SG1 & SG2 --> TR --> ICE --> INV --> MIN --> CTB --> CFS
    ...
    class SG1,SG2,TR service
    class ICE,INV,MIN service
    class CTB,CFS data
```

**Consolidation dependency rules:**
- A parent entity cannot consolidate until all direct subsidiaries have hard-closed their period
- Sub-group consolidation can proceed as soon as its member entities are closed, enabling parallel consolidation across independent sub-groups
- Intercompany elimination requires both sides of each transaction to be posted and reconciled
- Minority interest is computed after elimination, using ownership percentages from the investment register
