...
## 6. Sub-Ledger to GL Reconciliation Deep Dive

### The Distributed Consistency Problem

Sub-ledgers (AP, AR, Fixed Assets, Inventory) maintain detailed transactional records. The GL maintains summary control accounts. These must agree at all times---a discrepancy means either a posting was lost, double-posted, or incorrectly summarized. This is a distributed consistency problem where two independent data stores must agree on totals without shared transactions.
...
---
flowchart TB
    subgraph SubLedgers["Sub-Ledger Detail"]
    ...
    subgraph Posting["Summarization Pipeline"]
    ...
    subgraph Recon["Reconciliation Monitor"]
        CHK["Continuous Checker\n(every 60s)"]
        ALT["Discrepancy Alert"]
        REM["Auto-Remediation"]
    end

    AP & AR & FA & INV --> AGG --> VAL --> CTRL --> TB
    CTRL --> CHK
    AP & AR & FA & INV --> CHK
    CHK -->|Mismatch| ALT --> REM
    ...
    class AP,AR,FA,INV client
    class AGG,VAL service
    class CTRL,TB data
    class CHK,ALT,REM queue
```

### Reconciliation Algorithm

```
FUNCTION reconcileSubledgerToGL(subledgerType, entityId, periodId):
    -- Compute sub-ledger total from detail records
    subledgerTotal = SELECT SUM(amount)
        FROM subledger_transactions
        WHERE type = subledgerType AND entity_id = entityId
          AND period_id = periodId AND status = POSTED

    -- Fetch GL control account balance
    controlAccountId = getControlAccount(entityId, subledgerType)
    glBalance = getClosingBalance(controlAccountId, periodId)

    discrepancy = subledgerTotal - glBalance

    IF ABS(discrepancy) == 0:
        RETURN ReconciliationResult(status=BALANCED)

    IF ABS(discrepancy) <= MATERIALITY_THRESHOLD:
        logWarning("Sub-materiality discrepancy: " + discrepancy)
        RETURN ReconciliationResult(status=BALANCED_WITH_WARNING, amount=discrepancy)

    -- Investigate: find missing or duplicate postings
    slPostings = getSubledgerPostings(subledgerType, entityId, periodId)
    glEntries = getGLEntriesForControlAccount(controlAccountId, periodId)

    missing = slPostings.FILTER(p -> NOT EXISTS(glEntries, e -> e.reference == p.id))
    duplicates = glEntries.GROUP_BY(reference).FILTER(g -> g.count > 1)

    RETURN ReconciliationResult(status=DISCREPANCY, amount=discrepancy,
        missing_postings=missing, duplicate_entries=duplicates)
```

The reconciliation monitor runs continuously during close periods and on a 15-minute cadence during normal operations. Any discrepancy above materiality blocks the period close.

---

## 7. Continuous Close Architecture

### Moving Beyond Batch Close

Traditional month-end close batches all reconciliation, accruals, and adjustments into a 3--5 day window. Continuous close spreads these activities throughout the month:

| Activity | Traditional Close | Continuous Close |
|----------|------------------|-----------------|
| Accruals | Batch at month-end | Daily automated accruals with auto-reversal |
| Sub-ledger reconciliation | Monthly manual review | Real-time control account monitoring |
| Bank reconciliation | Monthly batch | Daily incremental matching |
| Intercompany reconciliation | Monthly with manual matching | Real-time reciprocal posting with instant matching |
| FX revaluation | Month-end batch | Weekly flash revaluation with month-end final |
| Trial balance verification | After all adjustments | Continuous balance monitoring |

**Impact on close duration**: Organizations implementing continuous close reduce the close window from 5--10 business days to 1--2 business days. The month-end process becomes a final verification pass rather than a massive batch operation.

```
FUNCTION continuousCloseScheduler(entityId):
    -- Daily tasks (run every business day)
    SCHEDULE daily AT 22:00 entity_timezone:
        runDailyAccruals(entityId)
        reconcileAllBankAccounts(entityId, date=TODAY)
        reconcileSubledgers(entityId, date=TODAY)
        verifyTrialBalanceIntegrity(entityId)

    -- Weekly tasks
    SCHEDULE weekly ON friday AT 18:00:
        runFlashFXRevaluation(entityId)
        generateFlashTrialBalance(entityId)
        runIntercompanyReconciliation(entityId)

    -- Month-end (reduced to verification + final adjustments)
    SCHEDULE on LAST_BUSINESS_DAY AT 09:00:
        runFinalFXRevaluation(entityId)
        verifyAllReconciliationsComplete(entityId)
        generateFinalTrialBalance(entityId)
        IF allChecksPass(): initiateSoftClose(entityId)
```

---

## 8. Slowest part of the process Analysis
...
| **Large reconciliation batch** | 500K+ bank transactions/month; pipeline takes 4--6 hours blocking close | Incremental daily reconciliation; parallel matching across bank accounts; subset-sum Cutting off unnecessary steps with max group size 20 |
...
| **Sub-ledger posting lag** | High-volume sub-ledgers (AP/AR) generate thousands of GL entries; batch posting delays cause reconciliation breaks | Event-driven micro-batch posting (every 60s); control account monitoring with automated gap detection |
| **Currency triangulation storms** | Exotic currency pairs without direct rates require multi-hop conversion; rate lookup latency spikes during revaluation | Pre-compute triangulated rates at import time; cache all cross-rates in memory; fallback rate hierarchy (direct > triangulated > prior-day) |
