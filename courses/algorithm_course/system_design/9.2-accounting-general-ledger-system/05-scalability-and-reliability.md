...
parallelizes across shards, partition Cutting off unnecessary steps eliminates irrelevant periods.
```
...
## 7. Event Store Compaction and Retention

### Event Store Growth Management

The event-sourced ledger accumulates events indefinitely. Without compaction, the event store for a large enterprise reaches 500 GB+ per year. The compaction strategy preserves audit compliance while managing storage costs.

**Snapshot-Based Compaction**: At each period close, the system captures a full balance snapshot for every account in the entity. Events older than the snapshot become archivable because balances can be reconstructed from the snapshot forward rather than from genesis.

```
Event Store Lifecycle:
  Active:    Current period events       → Primary database, fully indexed
  Archived:  Prior-period events         → Compressed columnar storage
  Snapshot:  Period-end balance captures  → Immutable snapshot store

Reconstruction: balance(account, period_N) =
    snapshot(period_N-1) + replay(events from period_N)
    -- NOT: replay(all events from genesis to period_N)
```

**Compaction Rules**:
- Events in the current + previous period: retain in hot storage with full indexes
- Events older than 2 periods but within retention window: compress and migrate to warm columnar storage; retain only primary key + hash for chain verification
- Events beyond retention window: verify hash chain integrity, archive Merkle root, purge raw events with auditor sign-off

### Connection Pool Management

Database connection pools are a critical Slowest part of the process during month-end when posting, reconciliation, and reporting all compete for connections.

| Pool | Min | Max | Timeout | Priority |
|------|-----|-----|---------|----------|
| Posting (OLTP) | 20 | 100 | 5s | Highest |
| Approval workflow | 5 | 20 | 10s | High |
| Reconciliation | 10 | 50 | 30s | Medium |
| Reporting (OLAP) | 5 | 30 | 60s | Low |
| Batch close | 0 | 40 | 120s | Configurable |

**Overflow strategy**: When the posting pool exhausts connections during month-end peaks, the system borrows from the reporting pool (degrading report latency) rather than rejecting postings. Reporting queries are redirected to read replicas when the primary pool is saturated.

---

## 8. Elastic Scaling Patterns

### Queue-Based Load Leveling for Posting

During month-end storms, the posting queue absorbs bursts that exceed engine capacity:
...
flowchart LR
    subgraph Sources["Posting Sources"]
        M[Manual Entries]
        SL[Sub-Ledger Batches]
        AUTO[Automated Accruals]
    end

    subgraph Queue["Priority Queue"]
        PQ{{"Priority Router"}}
        Q1["P0: Close-Critical\n(Revaluation, Elimination)"]
        Q2["P1: Reconciliation\nEntries"]
        Q3["P2: Standard\nPostings"]
    end

    subgraph Engine["Posting Engine Pool"]
        PE1["Engine 1"]
        PE2["Engine 2"]
        PE3["Engine N\n(auto-scaled)"]
    end
    ...
    class M,SL,AUTO client
    class PQ,Q1,Q2,Q3 queue
    class PE1,PE2,PE3 service
```

**Backpressure signals**: When queue depth exceeds 10,000 entries or p99 latency exceeds 2 seconds, the system:
1. Scales posting engine instances (cold start: 30s from container pool)
2. Enables batch aggregation mode (group entries by account for fewer lock acquisitions)
3. Defers non-critical reconciliation tasks to reduce database load
4. Notifies the close orchestrator of estimated delay

### Read Replica Topology

```
Primary (Posting)
  ├── Sync Replica (Failover, RPO=0)
  ├── Async Replica 1 (Trial Balance queries, lag < 3s)
  ├── Async Replica 2 (Financial Reporting, lag < 5s)
  ├── Async Replica 3 (Ad-hoc Analytics, lag < 10s)
  └── CDC Stream → Analytical Warehouse (lag < 30s)

Month-end addition:
  ├── Async Replica 4 (Close-period reporting, spun up D-3)
  └── Async Replica 5 (Auditor queries, spun up D-1)
```
...
| Event store management | Snapshot-based compaction; period-boundary snapshots; Merkle root archival |
| Connection pooling | Isolated pools per workload; overflow borrowing; replica redirection under pressure |
| Elastic scaling | Queue-based load leveling; priority routing; calendar-driven pre-provisioning |
