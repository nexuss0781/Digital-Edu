...
### 6.1 Why MQTT Over HTTP for Vehicle Telemetry

| Approach | Header Overhead | Connection Model | Offline Handling | Verdict |
|----------|----------------|-----------------|-----------------|---------|
| HTTP polling | ~500 bytes/request | Stateless (reconnect each time) | No built-in mechanism | Rejected — excessive overhead at 500K vehicles |
| HTTP long-poll | ~500 bytes initial | Stateful but fragile | No disconnect awareness | Rejected — poor fit for push telemetry |
| WebSocket | ~2 bytes/frame | Persistent, bidirectional | No built-in session resume | Viable for dashboards, not vehicles |
| **MQTT** | **2 bytes/msg** | **Persistent with session state** | **Last Will + clean session resume** | **Chosen — purpose-built for IoT telemetry** |

### 6.2 Why Polyglot Persistence

| Data Type | Access Pattern | Chosen Store | Why Not a Single DB |
|-----------|---------------|-------------|-------------------|
| Current vehicle positions | Spatial queries (nearest, within polygon) | In-memory geospatial index | Relational DB spatial queries too slow at 100K reads/sec |
| Historical telemetry | Time-range scans per vehicle | Time-series database | Relational DB write amplification at 100K writes/sec |
| Fleet/driver/vehicle entities | CRUD with joins and transactions | Relational database | Time-series DBs lack join capability |
| Compliance records | Append-only, immutable, auditable | Immutable event store | Time-series DBs allow deletion/modification |
| Route optimization cache | Key-value with TTL | Distributed cache | Persistent storage unnecessary for ephemeral solutions |

### 6.3 Whiteboard Sketch Guide

```
Essential diagram for a 45-minute interview:

  ┌──────────┐  MQTT   ┌──────────┐  Stream   ┌───────────────────────┐
  │ Vehicle  │ ──────→ │ MQTT     │ ───────→  │ Stream Processing     │
  │ Edge Unit│ ←────── │ Broker   │           │ (normalize, geofence, │
  │ GPS+OBD  │         │ Cluster  │           │  enrich, route check) │
  └──────────┘         └──────────┘           └───────────┬───────────┘
                                                          │
                       ┌──────────────────────────────────┼──────────┐
                       ↓                ↓                 ↓          ↓
                  ┌─────────┐    ┌───────────┐    ┌──────────┐ ┌─────────┐
                  │ Position│    │ Time-Series│    │ Geofence │ │ ELD/HOS │
                  │ Cache   │    │ DB         │    │ Engine   │ │ Service │
                  └─────────┘    └───────────┘    └──────────┘ └─────────┘
                       ↓
                  ┌──────────────┐     ┌──────────────┐
                  │ Route        │     │ Analytics /  │
                  │ Optimizer    │     │ ML Pipeline  │
                  └──────────────┘     └──────────────┘

  Show: telemetry flows left-to-right
  Label: "MQTT QoS 0/1" on telemetry, "QoS 2" on compliance events
  Label: "polyglot persistence" on the data layer
```
...
|----------|----------------|
| Proposing HTTP polling for vehicle telemetry | Doesn't understand IoT communication patterns or cellular bandwidth costs |
| Storing all data in a single relational database | Hasn't considered 100K writes/sec throughput or spatial query requirements |
| Using Dijkstra's algorithm for route optimization | Confusing shortest-path (P) with vehicle routing (NP-hard) |
| Assuming reliable vehicle connectivity | Never worked with field-deployed systems — tunnels, rural dead zones exist |
| No mention of edge computing or offline operation | Missing the core architectural differentiator for fleet/IoT systems |
| Treating all telemetry with equal priority | Should distinguish compliance-critical (never drop) from operational (best-effort) |
