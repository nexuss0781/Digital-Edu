...
### 6.3 Alert Suppression and Deduplication

```
Suppression rules to prevent alert fatigue:

  - Geofence alerts: suppress duplicate entry/exit for same vehicle + zone within 5 minutes
  - HOS warnings: suppress repeated "< 30 min remaining" if already acknowledged
  - Maintenance alerts: batch multiple DTC codes from same vehicle into single alert
  - Connectivity loss: suppress individual vehicle disconnect alerts; group by region if > 10 vehicles offline in same cell tower area

  De-escalation: auto-resolve alerts when condition clears
    (vehicle returns to authorized zone → auto-close unauthorized zone alert)
```

---

## 7. Edge Device Observability

### 7.1 Device Health Metrics

| Metric | Collection | Alert Condition |
|--------|-----------|-----------------|
| `edge.buffer_utilization_pct` | Reported on reconnect | > 80% (risk of data loss) |
| `edge.gps_fix_quality` | Every telemetry report | No fix for > 5 minutes while engine running |
| `edge.cellular_signal_dbm` | Every 60 seconds | < -110 dBm sustained (coverage gap) |
| `edge.firmware_version` | On connection | Behind current version by > 2 releases |
| `edge.reboot_count_24h` | Daily heartbeat | > 3 reboots (hardware instability) |
| `edge.clock_drift_ms` | Every sync | > 5 seconds drift from server time |
