...
## 6. Data Durability and Retention

### 6.1 Tiered Retention Policy

| Tier | Duration | Storage | Data Granularity | Access Pattern |
|------|----------|---------|-----------------|----------------|
| **Hot** | 0–90 days | SSD-backed time-series | Full resolution (raw) | Real-time queries, dashboards |
| **Warm** | 90 days–1 year | HDD-backed time-series | 1-minute averages | Trip replay, analytics |
| **Cold** | 1–3 years | Object storage | 5-minute averages | Compliance audits, dispute resolution |
| **Archive** | 3–7 years | Compressed object storage | 15-minute summaries | Regulatory retention, legal discovery |

### 6.2 Automated Downsampling

```
Downsampling Pipeline (continuous aggregation):

  Raw GPS (10-sec interval) → At 90 days:
    GROUP BY vehicle_id, floor(timestamp / 60s)
    → 1-min averages: position, speed, heading

  1-min averages → At 1 year:
    GROUP BY vehicle_id, floor(timestamp / 300s)
    → 5-min averages with min/max speed, distance traveled

  5-min averages → At 3 years:
    GROUP BY vehicle_id, floor(timestamp / 900s)
    → 15-min summaries with daily aggregates

  Storage savings:
    Raw: 146 TB/year
    After 90-day downsample: 24 TB for year 1 data
    After 1-year downsample: 5 TB for year 2-3 data
    Total 3-year cost: ~90% less than retaining raw
```

### 6.3 Compliance Data Durability

```
ELD/HOS records use a separate durability path:

  - Write to immutable event store (append-only, tamper-evident)
  - Synchronous replication to 3 availability zones
  - Cryptographic hash chain: record_hash = SHA-256(prev_hash || record_data)
  - Retention: minimum 6 months (FMCSA), configurable up to 7 years
  - No downsampling — compliance records are retained at full fidelity
  - Backup: daily snapshots to cross-region object storage
```
