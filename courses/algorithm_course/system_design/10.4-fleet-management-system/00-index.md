...
| **Processing Model** | Real-time stream processing for tracking; batch + Practical rule of thumb optimization for routing |
...
| Term | Definition |
|------|-----------|
| **Telematics** | Remote vehicle monitoring combining telecommunications and informatics — the hardware/software stack on a vehicle that collects and transmits data |
| **OBD-II** | On-Board Diagnostics port providing standardized vehicle engine data (RPM, temperature, fault codes) |
| **CAN Bus** | Controller Area Network — the internal vehicle bus connecting ECUs, sensors, and actuators |
| **ELD** | Electronic Logging Device — FMCSA-mandated device for automatically recording driving time |
| **HOS** | Hours of Service — federal regulations limiting driver on-duty and driving time to prevent fatigue |
| **VRPTW** | Vehicle Routing Problem with Time Windows — NP-hard optimization problem of assigning stops to vehicles respecting capacity, time, and driver constraints |
| **Geohash** | Hierarchical spatial encoding that maps latitude/longitude to short alphanumeric strings for efficient spatial indexing |
| **Dead Reckoning** | Estimating vehicle position using IMU/accelerometer when GPS signal is unavailable |
| **DVIR** | Driver Vehicle Inspection Report — pre-trip and post-trip safety inspection document |
| **IFTA** | International Fuel Tax Agreement — system for distributing fuel tax revenue among jurisdictions based on miles driven |
...
|-----------|--------|-------|
| **Domain Complexity** | ★★★★☆ | Multi-domain (logistics, compliance, geospatial, optimization) with regulatory mandates |
| **Scale Challenge** | ★★★★☆ | 500K+ connected vehicles streaming telemetry at 100K+ events/sec |
| **Latency Sensitivity** | ★★★☆☆ | Sub-3s for tracking, sub-100ms for geofences, 10–60s acceptable for route optimization |
| **Data Model Complexity** | ★★★★★ | Polyglot persistence: time-series, geospatial, relational, and edge-local stores |
| **Operational Complexity** | ★★★★☆ | Edge-cloud continuum, multi-region, 100+ device manufacturers |
| **Interview Frequency** | ★★★☆☆ | Common at logistics, IoT, and mobility companies; occasionally at general tech firms |
...
|--------|-------------|----------------|
| [9.8 — Supply Chain Management](../9.8-supply-chain-management/00-index.md) | **Shared domain** — end-to-end logistics visibility and tracking | Shipment lifecycle tracking, multi-party coordination, compliance audit trails |
| [12.8 — WebRTC Infrastructure](../12.8-webrtc-infrastructure/00-index.md) | **Edge relay** — geographically distributed relay infrastructure | Edge fan-out patterns, NAT traversal, adaptive quality under bandwidth constraints |
| [9.4 — Inventory Management](../9.4-inventory-management-system/00-index.md) | **Spatial optimization** — warehouse location and availability queries | Nearest-available entity queries, geo-partitioned indexes, real-time stock/vehicle status |
| [11.5 — SMS Gateway](../11.5-sms-gateway/00-index.md) | **Store-and-forward** — buffered delivery with guaranteed semantics | Offline buffering, exactly-once delivery, backpressure handling on reconnection |
| [9.1 — ERP System](../9.1-erp-system-design/00-index.md) | **Compliance engine** — regulatory recording and audit trails | Tamper-evident records, multi-jurisdiction compliance, immutable event logs |
| [12.1 — AdTech Real-Time Bidding](../12.1-adtech-real-time-bidding/00-index.md) | **High-throughput ingestion** — processing millions of events per second | Stream processing partitioning, time-series storage, real-time enrichment pipelines |
