...
## Insight 11: Hybrid Model Routing for Conversational Latency Classes

**Category:** Performance
**One-liner:** Route acknowledgments and transitions through a small (7B) model while reserving the large (70B+) model for complex question generation, cutting average inference latency by 40% without sacrificing question quality.

**Why it matters:** Not every interviewer utterance requires the same cognitive depth. "That's a great point, let's move on to discuss..." is a formulaic transition that a 7B model generates in 50ms, while "Design a rate limiter that handles bursty traffic across multiple regions" demands reasoning from a 70B+ model at 300-500ms. A hybrid router classifies the next utterance type (acknowledgment, transition, follow-up, technical probe, closing) and dispatches to the appropriate model tier. This saves 40% of GPU cost during an average 20-minute interview where ~60% of AI utterances are lightweight. The pattern generalizes to any conversational AI system: customer support (FAQ vs complex troubleshooting), tutoring (encouragement vs explanation), and therapy bots (reflections vs clinical assessments).

---

## Insight 12: Regulatory Fragmentation as an Architecture Forcing Function

**Category:** Architecture
**One-liner:** The collision of EEOC Title VII, EU AI Act, NYC AEDT, Illinois AIVPA, Colorado AI Act, and GDPR forces a jurisdiction-resolution layer that dynamically configures evaluation pipelines, consent flows, and data residency per candidate — making compliance a first-class architectural concern rather than a policy overlay.

**Why it matters:** A candidate in New York City triggers NYC Local Law 144 (annual bias audits, 10-day advance notice), EEOC Title VII (four-fifths rule), and potentially GDPR (if the employer is EU-based). A candidate in Munich triggers the EU AI Act (emotion recognition prohibited, high-risk documentation required) and GDPR. A candidate in Colorado triggers the state's AI Act (reasonable care to avoid discrimination). Without a jurisdiction-resolution layer that maps candidate location to applicable regulations and configures the pipeline accordingly, the system either applies the most restrictive ruleset globally (degrading the product where unnecessary) or risks non-compliance in specific jurisdictions. The resolver pattern — determine jurisdiction, fetch applicable rules, configure pipeline, log compliance evidence — is transferable to any AI system operating across regulatory boundaries: financial robo-advisors, autonomous vehicles, and healthcare triage systems.
