...
## Speculative LLM Generation Algorithm

### Why Speculative Generation Matters

In a standard pipeline, LLM generation begins only after the final ASR transcript arrives. With speculative generation, the LLM starts generating on partial transcripts, gaining a 100-200ms head start.

```
ALGORITHM SpeculativeGeneration(asr_stream, conductor_context):
    INPUT:
        asr_stream: Stream of partial transcripts from ASR
        conductor_context: Current conversation state
    OUTPUT:
        Response text (streamed to TTS)

    INITIALIZE:
        speculative_response = NULL
        confidence_threshold = 0.7  // Minimum ASR confidence for speculation
        divergence_threshold = 0.3  // Max edit distance ratio to keep speculation

    ON asr_partial(partial_text, confidence, is_final):
        IF is_final:
            // Final transcript available
            IF speculative_response IS NOT NULL:
                divergence = edit_distance(partial_text, speculative_input) / len(partial_text)

                IF divergence < divergence_threshold:
                    // Speculation was close enough - continue streaming
                    LOG speculation_hit(divergence)
                    CONTINUE streaming speculative_response

                ELSE:
                    // Speculation diverged - abandon and regenerate
                    LOG speculation_miss(divergence)
                    CANCEL speculative_response
                    GENERATE new_response(partial_text, conductor_context)
                    STREAM new_response TO tts

            ELSE:
                // No speculation was running
                GENERATE response(partial_text, conductor_context)
                STREAM response TO tts

        ELSE IF confidence >= confidence_threshold AND word_count(partial_text) >= 5:
            // High-confidence partial - start speculating
            IF speculative_response IS NULL:
                speculative_input = partial_text
                speculative_response = GENERATE_ASYNC(partial_text, conductor_context)
                LOG speculation_started(partial_text)

    METRICS:
        speculation_hit_rate: ~60-70% (depends on ASR stability)
        latency_savings: 100-200ms on hits
        wasted_compute: ~30-40% speculation misses (acceptable trade-off)

COMPLEXITY:
    Time: O(1) per event, O(t) for LLM generation where t = output tokens
    Space: O(context_size) for maintaining speculative state
```

### Jurisdiction Resolution Algorithm

```
ALGORITHM ResolveJurisdiction(candidate, employer, interview_config):
    INPUT:
        candidate: {location, ip_address, provided_address}
        employer: {hq_location, operating_jurisdictions}
        interview_config: {default_mode, overrides}
    OUTPUT:
        Resolved pipeline configuration

    // Step 1: Determine candidate jurisdiction
    candidate_jurisdiction = COALESCE(
        candidate.provided_address.jurisdiction,
        geolocate(candidate.ip_address).jurisdiction,
        employer.hq_location.jurisdiction  // Fallback
    )

    // Step 2: Collect applicable regulations
    applicable_regs = []
    IF candidate_jurisdiction IN eu_member_states:
        applicable_regs.append("EU_AI_ACT")
        applicable_regs.append("GDPR")
    IF candidate_jurisdiction == "US-NY-NYC":
        applicable_regs.append("NYC_AEDT_LL144")
    IF candidate_jurisdiction IN ["US-CA", "US-FL", "US-IL"]:
        applicable_regs.append("TWO_PARTY_CONSENT")
    IF candidate_jurisdiction == "US-CO":
        applicable_regs.append("COLORADO_AI_ACT")
    IF candidate_jurisdiction == "US-IL":
        applicable_regs.append("ILLINOIS_AIVPA")
    // EEOC applies to all US candidates
    IF candidate_jurisdiction.startswith("US"):
        applicable_regs.append("EEOC_TITLE_VII")

    // Step 3: Compute most restrictive configuration
    config = interview_config.default_mode

    FOR reg IN applicable_regs:
        restrictions = get_restrictions(reg)
        config = merge_restrictions(config, restrictions)
        // merge_restrictions takes the MORE restrictive option for each setting

    // Step 4: Configure pipeline
    pipeline_config = {
        emotion_analysis: NOT ("EU_AI_ACT" IN applicable_regs),
        facial_analysis: NOT ("EU_AI_ACT" IN applicable_regs),
        recording_consent: "TWO_PARTY_CONSENT" IN applicable_regs ? "explicit" : "implied",
        bias_audit_mode: "NYC_AEDT_LL144" IN applicable_regs ? "annual_third_party" : "internal",
        advance_notice_days: "NYC_AEDT_LL144" IN applicable_regs ? 10 : 0,
        impact_assessment: "COLORADO_AI_ACT" IN applicable_regs,
        data_residency: "GDPR" IN applicable_regs ? candidate_jurisdiction.region : NULL,
        applicable_regulations: applicable_regs
    }

    LOG jurisdiction_resolution(candidate_jurisdiction, applicable_regs, pipeline_config)
