...
| Threat Vector | Detection Method | Confidence Level |
|---------------|------------------|-----------------|
| Proxy/impersonation | Voice fingerprint consistency across session | High |
| Real-time LLM assistance | Response latency analysis (typing → speaking gap) | Medium |
| Reading from scripts | Vocabulary shift detection, prosody monotony | Medium |
| Prompt injection via speech | Transcript pattern matching for meta-commands | High |
| Pre-shared question databases | Dynamic question generation eliminates fixed sets | Built-in |

```
ALGORITHM FraudSignalAggregator(session_signals):
    INPUT:
        session_signals: Continuous stream of behavioral signals during interview
    OUTPUT:
        Fraud risk score and flagged anomalies

    signals = {
        voice_consistency: compare_voice_embeddings(session_start, current_segment),
        response_timing: analyze_response_delays(question_times, answer_times),
        vocabulary_shift: detect_register_changes(transcript_segments),
        gaze_pattern: analyze_attention_focus(video_frames),  // Optional
        prompt_injection: scan_for_meta_commands(transcript)
    }

    risk_score = weighted_average(signals, weights={
        voice_consistency: 0.3,
        response_timing: 0.25,
        vocabulary_shift: 0.2,
        gaze_pattern: 0.1,
    ...
    IF risk_score > 0.7:
        ...
        LOG fraud_alert(session_id, risk_score, signals)
    ELIF risk_score > 0.5:
        LOG fraud_warning(session_id, risk_score, signals)

    // Never auto-reject — always human review for fraud determinations
    RETURN {risk_score, flagged_signals: [s for s in signals if s.score > threshold]}
```
...
Every AI capability in the AI Interviewer System (question generation, evaluation scoring, speech processing, integrity detection, adverse impact monitoring) must progress through all six stages before reaching autonomous production.

| Stage | Gate Criteria | Rollback Trigger |
|-------|--------------|------------------|
| **1. Offline Evaluation** | Rubric alignment >0.85 with expert human interviewers on 500+ mock sessions; bias audit passes 4/5ths rule across all protected groups; latency budget validated on synthetic load | Fail alignment threshold; any protected group disparity >20%; mouth-to-ear latency >400ms on synthetic load |
| **2. Shadow Mode** | AI runs parallel to human interviewers for 60 days; score correlation >0.80; no adverse impact signals; transcript quality verified by linguistics team | Correlation <0.75; adverse impact signal in any 14-day window; transcript error rate >5% |
| **3. Canary (5% traffic)** | Live interviews with informed candidate consent; post-interview candidate experience survey >4.0/5; human reviewer validates every AI evaluation; no regulatory complaints | Candidate satisfaction <3.5/5; any regulatory complaint; human override rate >40% |
| **4. Human-Reviewed Production** | All AI evaluations require human reviewer sign-off before advancing candidate; 90-day audit shows score stability; EEOC/EU AI Act compliance review complete | Override rate increasing trend; compliance finding; candidate appeal upheld rate >10% |
| **5. Autonomous Production** | Limited to initial screening stage only; final-round interviews always human-involved; continuous adverse impact monitoring with 7-day lookback; 180-day track record | Any adverse impact signal; model drift detected; regulatory guidance change; candidate escalation pattern |
| **6. Instant Rollback** | Feature flag per AI component (ASR, question gen, scoring, integrity); <60s rollback to human-only interview mode; automatic rollback on bias alert; incident review within 12h | N/A — this is the rollback mechanism itself |
