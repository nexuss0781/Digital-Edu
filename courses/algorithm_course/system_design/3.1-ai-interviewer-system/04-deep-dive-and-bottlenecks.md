...
### Slowest part of the process 1: LLM Response Latency
...
| Context Cutting off unnecessary steps | -20% latency | Potential context loss |
...
### Slowest part of the process 2: Concurrent Session Scaling
...
### Slowest part of the process 3: Recording Storage
...
### Case Study 1: HireVue's Facial Analysis Pivot (2024)

**Context:** HireVue was the market leader in AI video interviewing, analyzing 25,000+ data points per interview including facial expressions, eye gaze, and vocal characteristics.

**What Happened:** Following sustained criticism from researchers who demonstrated gender and racial bias in facial analysis scoring, and anticipation of the EU AI Act's prohibition on emotion recognition in hiring, HireVue removed facial analysis from its assessment model entirely in 2024.

**Architecture Impact:**
- Evaluation pipeline was redesigned to rely solely on transcript content and audio clarity metrics
- Scoring models retrained without visual features, requiring new rubric calibration
- Concordance with human evaluators initially dropped from ~85% to ~78% before content-only models were fine-tuned
- After retraining, content-only models achieved 82-88% concordance, comparable to the previous multimodal approach

**Lesson:** Emotion and appearance analysis provide marginal scoring accuracy gains (~5%) but introduce substantial bias risk and regulatory exposure. Content-only evaluation is both safer and nearly as effective. Design evaluation engines as modular pipelines where any modality can be removed without architectural rework.

### Case Study 2: Mercor's Scale-First AI Interview Architecture

**Context:** Mercor, an AI-native hiring platform, needed to conduct 300,000+ technical interviews across diverse engineering domains while maintaining evaluation quality.

**Architecture Choices:**
- Domain-specific fine-tuned LLMs rather than general-purpose models for evaluation
- 20-minute structured interview format optimized for technical screening (not behavioral)
- Rubric templates per job family (backend, frontend, ML, data engineering) with weighted competency dimensions
- Async evaluation with multi-model consensus to handle burst interview volumes

**Results:**
- 70-80% reduction in time-to-screen vs human technical phone screens
- 85-92% concordance with subsequent human interviewer decisions on technical roles
- Per-interview cost under $2 at scale (vs $50-150 for recruiter-conducted phone screens)

**Lesson:** Constraining the interview format (20 minutes, structured, domain-specific) dramatically simplifies the LLM conductor's task and improves evaluation consistency. The trade-off is that open-ended behavioral assessment suffers — a dedicated system design for behavioral interviews would differ significantly.
