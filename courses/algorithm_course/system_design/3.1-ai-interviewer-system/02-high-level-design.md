...
### Decision 4: Native Speech-to-Speech vs Cascaded Pipeline (2025 Evolution)

The landscape shifted significantly in 2024-2025 with native multimodal models:

| Factor | Cascaded (ASR → LLM → TTS) | Native Multimodal (GPT-4o style) |
|--------|---------------------------|----------------------------------|
| **Transcript availability** | Full transcript at ASR stage | Requires separate transcription pass |
| **Per-component auditability** | Each stage independently logged | Single black-box inference |
| **Vendor diversity** | Mix best-of-breed (Deepgram ASR + Claude LLM + ElevenLabs TTS) | Single vendor lock-in |
| **Emotion/prosody access** | Audio features available at ASR stage | Model may use prosody internally (uncontrollable) |
| **EU AI Act compliance** | Can prove emotion features are not used | Cannot prove what internal features the model uses |
| **Latency floor** | ~300ms (optimized streaming) | ~300ms (native) |
| **Development velocity** | Higher — more integration work | Lower — single API call |

**2025 Recommendation:** Cascaded remains mandatory for enterprise hiring due to audit trail requirements. However, consider a **hybrid approach**: use native multimodal for conversational flow (question generation, acknowledgments) while running a parallel cascaded pipeline for compliance artifacts (transcript, evaluation evidence). This adds infrastructure cost but provides the best user experience with full compliance coverage.

### Decision 5: LLM Deployment
