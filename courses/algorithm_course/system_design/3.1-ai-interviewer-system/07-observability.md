...
## Warm-Pool and Cold-Start Monitoring

### GPU Warm-Pool Metrics

| Metric | Description | Target | Alert |
|--------|-------------|--------|-------|
| Warm instances available | Pre-warmed GPU instances ready for interviews | >20% of expected peak | <10% |
| Cold-start frequency | Interviews hitting unwarmed instances | <5% | >10% |
| TTS voice cache hit rate | Pre-loaded voice models in memory | >95% | <90% |
| Question bank cache freshness | Time since last refresh of cached questions | <60 min | >120 min |
| WebRTC pre-connection rate | Candidates with pre-established connections | >80% | <60% |

### Interview Quality Score (Composite)
...
COMPOSITE_QUALITY_SCORE:
  components:
    - name: latency_score
      weight: 0.30
      calculation: 1.0 - (p95_mouth_to_ear / 500ms)  // 0 at 500ms, 1.0 at 0ms

    - name: completion_score
      weight: 0.20
      calculation: completion_rate  // 0.98 = 0.98

    - name: asr_accuracy_score
      weight: 0.15
      calculation: 1.0 - (word_error_rate / 0.2)  // 0 at 20% WER, 1.0 at 0%

    - name: turn_taking_score
      weight: 0.15
      calculation: 1.0 - (overlap_events / total_turns)

    - name: evaluation_consensus_score
      weight: 0.10
      calculation: cohens_kappa  // 0.6-1.0 range

    - name: candidate_satisfaction_score
      weight: 0.10
      calculation: avg_survey_rating / 5.0

  overall: weighted_sum(components)
  target: >= 0.85
  alert: < 0.75
```
...
All AI components in this system (speech recognition, question generation, evaluation scoring, integrity detection, adverse impact monitoring) must integrate with the organization's AI observability and evaluation infrastructure.

| Capability | Reference Design | Integration Point |
|------------|-----------------|-------------------|
| **Drift Detection & Alerting** | [AI Observability & LLMOps Platform](../3.25-ai-observability-llmops-platform/00-index.md) | Score distribution drift, ASR accuracy degradation, question diversity monitoring |
| **Model Evaluation & Benchmarking** | [AI Model Evaluation & Benchmarking Platform](../3.26-ai-model-evaluation-benchmarking-platform/00-index.md) | Rubric alignment benchmarks, adverse impact audits, inter-rater reliability with human baselines |
| **Feedback Loop Tracking** | [AI Observability & LLMOps Platform](../3.25-ai-observability-llmops-platform/00-index.md) | Human override rates per scoring dimension, candidate experience correlation, downstream hiring outcome tracking |

> **Cross-Reference Note:** The observability patterns in this document cover system-level metrics (latency, throughput, errors). For AI-specific observability — including evaluation fairness dashboards, score distribution monitoring, and compliance audit pipelines — see the referenced designs above.
