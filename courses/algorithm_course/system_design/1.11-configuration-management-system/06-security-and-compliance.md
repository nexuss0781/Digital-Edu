...
## Configuration Supply Chain Security
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Problem: How to verify config changes came from authorized         │
│  sources, not injected by a compromised pipeline?                   │
│                                                                      │
│  Solution: Cryptographic signing of config changes                  │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                               │   │
│  │  Write Path with Signing:                                     │   │
│  │                                                               │   │
│  │  1. CI/CD pipeline generates config change                    │   │
│  │  2. Pipeline signs change with its private key:               │   │
│  │     signature = sign(key + value + timestamp, pipeline_key)  │   │
│  │  3. Writes to config store with signature metadata:          │   │
│  │     PUT /config/payment/rate-limit                            │   │
│  │     Value: {"limit": 2000}                                    │   │
│  │     Metadata: {"sig": "...", "signer": "ci-pipeline-1"}      │   │
│  │                                                               │   │
│  │  Read Path with Verification:                                 │   │
│  │                                                               │   │
│  │  1. Client reads config + signature                           │   │
│  │  2. Verifies signature against known public keys              │   │
│  │  3. Rejects unsigned or tampered config                       │   │
│  │                                                               │   │
│  │  Benefits:                                                    │   │
│  │  • Detects config injection from compromised servers         │   │
│  │  • Non-repudiation (know exactly who changed what)           │   │
│  │  • Works even if config store itself is compromised          │   │
│  │                                                               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  EMERGENCY CONFIG ACCESS                                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Normal Path:                                                        │
│    Git PR → Review → Merge → GitOps → Config Store                 │
│    (Takes 15-30 minutes minimum)                                    │
│                                                                      │
│  Emergency Path (break-glass):                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  1. Oncall authenticates with emergency credentials          │   │
│  │  2. Writes directly to config store with "emergency" flag    │   │
│  │  3. System immediately pages security team                   │   │
│  │  4. All emergency writes logged to immutable audit trail     │   │
│  │  5. Post-incident: emergency change must be back-ported to   │   │
│  │     git within 24 hours or auto-reverted                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Controls:                                                           │
│  • Emergency credentials stored in sealed vault, require 2-of-3   │
│  • Time-limited tokens (15 minutes max)                             │
│  • Automatic PR creation from emergency changes                     │
│  • Reconciliation check: git and config store must converge        │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
| **Validation** | Semantic validation at write path | P0 |
...
| **Signing** | Cryptographic signing of config changes | P1 |
| **Break-glass** | Emergency access with time-limited tokens | P2 |
