...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Goal: Correlate config changes with application behavior           │
│                                                                      │
│  Config Change Event:                                                │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  {                                                            │   │
...
│  │    "new_value": {"limit": 2000},                              │   │
│  │    "revision": 50001,                                         │   │
│  │    "source": "ci-pipeline-main-abc123",                       │   │
│  │    "git_commit": "abc123def456",                              │   │
│  │    "propagation_time_ms": 85,                                 │   │
│  │    "instances_notified": 500                                  │   │
│  │  }                                                            │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Correlation Pipeline:                                               │
│  1. Config change → annotate monitoring dashboards                  │
│  2. Overlay change events on error rate / latency graphs            │
│  3. Alert if error rate increases within 5 min of config change     │
│  4. Auto-correlate: "error spike started 2 min after config rev     │
│     50001 changed /config/payment/rate-limit"                       │
│                                                                      │
│  Propagation Monitoring:                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  config_propagation_duration_seconds                          │   │
│  │  • Time from config write to last client acknowledgement     │   │
│  │  • Label by key prefix, datacenter                           │   │
│  │  • Alert if: p99 > 5s (instances running stale config)       │   │
│  │                                                               │   │
│  │  config_instances_on_version                                  │   │
│  │  • Gauge of instances running each config revision           │   │
│  │  • Shows rollout progress in real time                       │   │
│  │  • Alert if: old_version > 0 after 5 min (stragglers)       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  CONFIG ACCESS IN DISTRIBUTED TRACES                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Include config access as spans in distributed traces:              │
│                                                                      │
│  Trace: POST /api/payment                                           │
│  ├── span: api-gateway (2ms)                                       │
│  ├── span: payment-service (50ms)                                  │
│  │   ├── span: config.get("/config/payment/rate-limit") (0.5ms)   │
│  │   │   └── tags: {source: "cache", revision: 50001}             │
│  │   ├── span: rate-limit-check (1ms)                              │
│  │   └── span: process-payment (48ms)                              │
│  └── span: response (1ms)                                          │
│                                                                      │
│  Benefits:                                                           │
│  • See which config values are used per request                    │
│  • Detect cache miss storms after config changes                   │
│  • Correlate slow requests with config lookup latency              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
