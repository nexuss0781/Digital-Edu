...
│                                                              │
│  Guarantees:                                                 │
│  • FIFO (First-In-First-Out, like a line at a store) per watch (events in revision order)               │
...
### Trap 4: "How do you handle the leader being a Slowest part of the process?"

**What they want:** Understanding of scaling limitations.

**Good answer:**
> "The leader Slowest part of the process is fundamental to consensus-based systems. Mitigations:
...
| Ignoring the leader Slowest part of the process | All writes go through one node | Mention batching, read scaling, sharding |
...
9. How do you handle the leader Slowest part of the process at scale?
...
### How Would You Integrate GitOps with a Config Management System?

**Good answer:**
> "Git becomes the source of truth for desired state, while the config store serves as the runtime distribution layer. A GitOps controller (like ArgoCD or Flux) reconciles the Git state into the config store. The critical design decision is the ownership model: which keys are git-managed (reconciled by the controller, direct writes reverted) versus runtime-managed (ephemeral locks, circuit breaker states, excluded from reconciliation). Without this separation, the reconciliation loop either reverts emergency fixes or ignores drift -- both are dangerous."

### How Would You Prevent a Global Outage from a Bad Config Change?

**Good answer:**
> "Defense in depth: First, write-time validation that checks not just syntax but semantics -- a rate limit of 0 is valid JSON but operationally catastrophic. Second, progressive rollout: canary namespace (1%) → regional (25%) → global, with automatic rollback if error rates exceed thresholds. Third, blast-radius limits: no single config change can affect more than X% of instances simultaneously. The 2025 CDN outage that took down a major cloud provider for 8 hours happened because the config passed syntactic validation but had no semantic checks, no progressive rollout, and no blast-radius limits."

### What Role Does AI/ML Play in Configuration Management?

**Good answer:**
> "Three emerging patterns: drift detection uses ML to identify config values that deviate from known-good baselines, catching the slow creep of misconfiguration. Anomaly correlation detects when config changes coincide with metric anomalies, enabling faster incident root-cause identification. Predictive validation uses historical data to predict the impact of a config change before it's applied -- if similar changes caused incidents before, the system flags the risk. These are not replacing human judgment but augmenting the safety pipeline."
...
| "Just push config changes globally" | No blast-radius control | Describe progressive rollout with canary stages |
| "Schema validation in CI is enough" | Runtime bypasses CI | Defense in depth: CI + proxy + write-path validation |
