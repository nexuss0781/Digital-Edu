...
**Why it matters:** Without this rule, a new leader could commit an entry from a previous term that was only on a minority of nodes, and a subsequent leader could overwrite it. This subtle Rule that never changes prevents a class of data loss bugs that only surface during leader transitions under partial replication. It is one of the most commonly misunderstood aspects of Raft and a frequent source of bugs in custom implementations.
...
## Insight 9: GitOps Reconciliation Creates a Two-Source-of-Truth Problem

**Category:** Operational Architecture
**One-liner:** When Git is the source of truth for desired config state and the config store is the source of truth for runtime state, reconciliation loops must handle conflicts without silent data loss.
**Why it matters:** Modern GitOps workflows (ArgoCD, Flux) treat Git repositories as the authoritative source for configuration, with controllers that reconcile desired state into the config store. But runtime systems also mutate config state -- auto-scaling thresholds, circuit breaker states, and emergency overrides happen directly in the config store. This creates a fundamental tension: the GitOps controller may revert a legitimate emergency change because it does not match the Git-committed state. The solution requires a clear ownership model: Git-managed keys are reconciled by the controller, while runtime-managed keys are excluded from reconciliation and separately audited. Without this separation, the reconciliation loop becomes either destructive (reverting emergency fixes) or permissive (ignoring drift), and neither outcome is acceptable for production infrastructure.

---

## Insight 10: Configuration Validation Must Happen at Write Time, Not Just Deploy Time

**Category:** Safety
**One-liner:** Schema validation and semantic checks at the config store's write path prevent the class of outages where syntactically valid but semantically invalid config propagates to all instances simultaneously.
**Why it matters:** The October 2025 front-door CDN outage that took a major cloud provider offline for 8 hours was caused by a configuration change that passed syntactic validation but had incorrect values that cascaded across all regions. Traditional config systems are schema-agnostic -- they store opaque bytes. Adding a validation layer at the write path (either as a server-side plugin or a proxy) that checks config against registered schemas, range constraints, and dependency rules creates a critical safety net. This validation must include semantic checks: a rate limit of 0 is syntactically valid JSON but operationally catastrophic. The pattern mirrors database constraints -- just as a database rejects rows violating foreign keys, a config store should reject values violating operational invariants.

---

## Insight 11: Compaction Timing Creates a Hidden Dependency Between Watch Consumers and Storage Policy

**Category:** Operational Coupling
**One-liner:** If the compaction policy removes historical revisions that a slow or reconnecting watch client needs, the client must reinitialize its entire state rather than catching up incrementally.
**Why it matters:** Compaction is necessary to bound storage growth -- without it, the revision history grows without limit. But the compacted revision becomes a hard boundary: any watch client that has not consumed events up to that revision loses the ability to catch up incrementally and receives a "compacted" error, forcing a full state re-read. This creates an invisible coupling between the storage administrator's compaction schedule and every watch client's consumption rate. If compaction is set too aggressively (retain only 1 hour of history), any client offline for more than 1 hour triggers a full re-sync that spikes load. If compaction is too conservative (retain 7 days), storage grows unboundedly. The optimal retention window must account for the maximum expected client downtime, which often crosses team boundaries and is not documented.

---

## Insight 12: Multi-Datacenter Config Federation Requires Choosing Between Latency, Consistency, and Operational Simplicity -- Pick Two

**Category:** Distributed Systems Trade-offs
**One-liner:** Stretched clusters, regional clusters with async replication, and WAN-federated clusters each sacrifice one of the three properties, and the choice must align with the organization's failure mode preference.
**Why it matters:** A stretched cluster (nodes across DCs participating in the same Raft group) provides strong consistency and operational simplicity but imposes WAN latency (50-100ms) on every write. Regional clusters with async replication provide low latency and operational simplicity but sacrifice cross-DC consistency, requiring conflict resolution for the ~0.1% of writes that overlap during failover. WAN federation (the Consul model) provides consistency and low-latency local reads but introduces operational complexity with forwarded writes and complex failure modes when the primary DC is unreachable. No architecture provides all three properties, and the failure mode of each is different: stretched clusters fail by becoming slow (all writes pay WAN cost), regional clusters fail by diverging (split-brain risk during failover), and federated clusters fail by partial unavailability (secondary DCs cannot write). The choice depends on whether the organization would rather be slow, inconsistent, or partially unavailable during a DC failure.
