...
## Advanced Scaling: Learner Nodes and Watch Proxies

### Learner (Non-Voting) Nodes

```
┌─────────────────────────────────────────────────────────────────────┐
│  LEARNER NODE ARCHITECTURE                                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Problem: Adding voting members increases quorum size, slowing      │
│  writes. But read demand keeps growing.                              │
│                                                                      │
│  Solution: Learner nodes replicate data but do NOT vote.            │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  Voting Members (3):                                          │   │
...
│  │  (US-West)     (EU-Central)  (AP-South)                      │   │
│  │                                                               │   │
│  │  Learner properties:                                          │   │
│  │  • Receive replicated log entries                             │   │
│  │  • Serve serializable reads (bounded staleness)              │   │
│  │  • Do NOT participate in elections                            │   │
│  │  • Do NOT count toward quorum                                │   │
│  │  • Can be promoted to voter if needed                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Use cases:                                                          │
│  • Geo-distributed read replicas                                    │
│  • High-read-volume endpoints                                       │
│  • Safe addition (doesn't risk quorum during catch-up)             │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Watch Proxy for Scale
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Problem: 100,000 watch connections overwhelm the config cluster.  │
│                                                                      │
│  Solution: Watch proxy layer fans out notifications.                │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                                                               │   │
│  │  Config Cluster (5 nodes)                                    │   │
│  │       │                                                       │   │
│  │       │ ← 10 watch connections (one per proxy)               │   │
│  │       ▼                                                       │   │
│  │  Watch Proxy Layer (10 stateless proxies)                    │   │
│  │       │                                                       │   │
│  │       │ ← 100,000 watch connections from clients             │   │
│  │       ▼                                                       │   │
│  │  100,000 client applications                                 │   │
│  │                                                               │   │
│  │  Proxy responsibilities:                                      │   │
│  │  • Single watch to cluster, multiplex to clients             │   │
│  │  • Event buffer for slow clients                             │   │
│  │  • Connection management and backpressure                    │   │
│  │  • Horizontally scalable (stateless)                         │   │
│  │                                                               │   │
│  │  Result: Cluster handles 10 watches instead of 100,000       │   │
│  │                                                               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
| Learner nodes | Geo-distributed reads | Non-voting replicas |
| Watch proxy | Scale watch connections | Fan-out intermediary layer |
| Progressive rollout | Limit config blast radius | Canary → regional → global |
