...
| **GitOps Controller** | Reconcile Git state to config store | ArgoCD/Flux sync loop |
| **Policy Engine** | Validate config before write | OPA/Rego admission control |
...
flowchart LR
    ...
    subgraph CI["CI Pipeline"]
        ...
        Policy["Policy<br/>Check"]
    end

    subgraph GitOps["GitOps Controller"]
        Reconciler["Reconciler<br/>(ArgoCD/Flux)"]
    end
    ...
    PR -->|"Review +<br/>Approve"| Main
    Main --> Validate --> Policy
    Policy -->|"Apply"| Reconciler
    Reconciler -->|"Write"| CM
    CM -->|"Watch"| S1 & S2
    ...
    class PR,Main git
    class Validate,Policy ci
    class Reconciler gitops
    class CM config
    class S1,S2 app
```
...
│  Key Ownership Model:                                                │
│                                                                      │
│  Git-Managed Keys (reconciled by GitOps controller):                │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  /config/payment/rate-limit        ← From git, reconciled   │   │
│  │  /config/payment/feature-flags     ← From git, reconciled   │   │
│  │  /config/order/timeout             ← From git, reconciled   │   │
│  │                                                               │   │
│  │  If someone edits directly → controller reverts to git state │   │
│  │  If git changes → controller writes new value                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Runtime-Managed Keys (excluded from reconciliation):               │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  /runtime/circuit-breaker/payment   ← Set by service mesh   │   │
│  │  /runtime/emergency/maintenance     ← Set by oncall          │   │
│  │  /locks/*                           ← Ephemeral, runtime     │   │
│  │  /election/*                        ← Ephemeral, runtime     │   │
│  │                                                               │   │
│  │  GitOps controller ignores these prefixes                    │   │
│  │  Separate audit trail for runtime changes                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Conflict Resolution:                                                │
│  • Emergency override: Operator writes directly + creates PR        │
│  • GitOps controller pauses on annotation "config/skip-reconcile"  │
│  • Reconciliation resumes after PR merges with override value       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
