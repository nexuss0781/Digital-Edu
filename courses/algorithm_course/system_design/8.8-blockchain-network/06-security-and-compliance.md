...
```
Bridge exploits are the #1 source of funds lost in crypto:
  - Ronin Bridge: $625M (compromised validator keys)
  - Wormhole: $320M (signature verification bypass)
  - Nomad: $190M (Merkle proof verification flaw)

Bridge architectures by trust model:

1. Multisig Bridge (Weakest):
   - M-of-N trusted signers validate cross-chain transfers
   - Risk: Key compromise → total bridge drain
   - Example: Ronin used 5-of-9 multisig; attacker got 5 keys

2. Optimistic Bridge (Medium):
   - Assume validity; 7-day challenge window for fraud proofs
   - Risk: If all watchers are offline, invalid transfer succeeds
   - Mitigation: Multiple independent watchers + incentivized monitoring

3. Light Client Bridge (Strongest):
   - On-chain light client verifies source chain consensus
   - Risk: Consensus bug in light client implementation
   - Cost: High gas for on-chain verification (~500K-1M gas)

4. ZK Bridge (Emerging):
   - ZK proof of source chain consensus verification
   - Risk: Prover bugs, trusted setup (if applicable)
   - Advantage: Cheapest on-chain verification (~300K gas)

Defense principles:
├── Never trust a single entity (distribute key management)
├── Enforce withdrawal delays (time for fraud detection)
├── Cap bridge TVL relative to security budget
├── Monitor bridge contracts continuously
└── Require multi-layer verification (consensus + state proof)
```
...
1. Time-Locked Operations:
   - Critical parameter changes require delay (24-72 hours)
   - Allows community to review and exit before changes take effect
   - Prevents flash governance attacks

2. Circuit Breakers:
   - Pause contract if anomalous conditions detected:
     ├── Price deviation > 10% in single block
     ├── Withdrawal volume > 20% of TVL in 1 hour
     ├── Gas consumption pattern indicates exploit
     └── Oracle staleness (no update for > 1 hour)

3. Rate Limiting:
   - Cap maximum withdrawal per address per period
   - Cap maximum single transaction size
   - Gradual unlock for large operations

4. Formal Verification Targets:
   - Accounting invariants (total supply = sum of balances)
   - Access control (only authorized callers for admin functions)
   - State machine transitions (valid lifecycle states)
   - Overflow/underflow safety (for all arithmetic)

5. Upgrade Safety:
   - Proxy patterns (UUPS, Transparent) with governance delay
   - Storage layout compatibility verification
   - Emergency pause capability with multi-sig control
   - Immutability commitment after maturity period
```
