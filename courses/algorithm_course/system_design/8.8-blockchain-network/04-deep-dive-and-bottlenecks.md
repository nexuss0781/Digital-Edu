...
```
1. State Cutting off unnecessary steps:
...
Slowest part of the process: Large blocks (high gas usage) take longer to propagate
...
| State growth | High | Increasing hardware requirements | State Cutting off unnecessary steps, expiry, statelessness |
...
| BLS signature aggregation | Medium | Computation scales with validator count | Committee-based aggregation; subnet pre-aggregation |
| Blob propagation overhead | Low-Medium | Additional bandwidth for EIP-4844 data | Separate gossip topics; KZG commitment validation |
| Archive node storage | High | 15+ TB and growing for full history | State Cutting off unnecessary steps; EIP-4444 historical data expiry; dedicated archive service |

---

## Deep Dive 7: Parallel EVM Execution

### The Sequential Slowest part of the process

Current EVM execution is strictly sequential: each transaction in a block runs after the previous one completes. This is because any transaction might read state modified by a prior transaction. At 30M gas per block with average 200K gas per complex transaction, this limits blocks to ~150 transactions and ~12.5 TPS effective.

### Speculative Parallelism Approach

```
FUNCTION parallelExecuteBlock(transactions, state):
    -- Assume no conflicts; execute all in parallel
    threads = []
    FOR tx IN transactions:
        thread = SPAWN executeTransaction(state.snapshot(), tx)
        threads.append(thread)

    -- Wait for all to complete
    results = AWAIT_ALL(threads)

    -- Detect conflicts: did any two transactions access the same state?
    readSets = [result.stateReads for result in results]
    writeSets = [result.stateWrites for result in results]

    conflicts = detectConflicts(readSets, writeSets)

    IF conflicts is EMPTY:
        -- No conflicts! Apply all results (5-10x faster)
        applyAll(results)
    ELSE:
        -- Re-execute conflicting transactions sequentially
        FOR (txA, txB) IN conflicts:
            reExecuteSequentially(txA, txB, state)

    -- Typical conflict rate: 10-30% of transactions
    -- Net speedup: 3-7x versus fully sequential

Trade-offs:
├── Requires speculative execution + rollback capability
├── Memory overhead for multiple state snapshots
├── Deterministic conflict resolution required
└── Access lists (EIP-2930) improve conflict prediction
```
