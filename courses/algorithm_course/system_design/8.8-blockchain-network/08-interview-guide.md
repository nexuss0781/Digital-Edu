...
| **Scaling & Security** | 8 min | L2 rollups, MEV, eclipse attacks, state Cutting off unnecessary steps |
...
### "How would you design a cross-chain bridge?"

**Key points to cover:**
- Three approaches: multisig bridge (trusted committee), light client bridge (verify source chain consensus on-chain), optimistic bridge (fraud-proof based)
- Light client bridge is most trust-minimized but expensive in gas (verify consensus on-chain)
- Bridge exploits are the #1 source of losses in crypto ($2B+ in 2022): Ronin ($625M), Wormhole ($320M), Nomad ($190M)
- Design principle: minimize trust assumptions; prefer validity proofs over economic assumptions
- Withdrawal delays (7 days for optimistic L2) are a security mechanism, not a bug

### "How would you implement account abstraction?"

**Key points to cover:**
- ERC-4337 creates a parallel mempool for UserOperations (separate from regular tx mempool)
- Bundlers aggregate UserOps into regular transactions
- EntryPoint contract validates custom signature schemes, executes UserOps
- Enables: social recovery wallets, gas sponsorship (Paymaster), batched transactions, session keys
- Trade-off: additional gas overhead (~30-40K gas for EntryPoint validation) vs. vastly better UX

### "What happens during a hard fork upgrade?"

**Key points to cover:**
- Hard fork = consensus rule change that old nodes reject
- Activation: scheduled at specific epoch/block number (announced months in advance)
- All node operators must upgrade before activation
- Non-upgraded nodes: split from the network, follow old rules
- Risk: contentious hard forks create permanent chain splits (e.g., Ethereum / Ethereum Classic)
- Mitigation: governance process, testnet validation, gradual client rollout, community consensus
...
☑ P2P gossip network (mesh topology)
☑ Consensus layer (fork choice + finality gadget)
☑ Execution layer (EVM + state management)
☑ State trie (Merkle Patricia → Verkle)
☑ Mempool with fee-based priority
☑ Block structure (header → body → state root)

Should-mention:
☑ Slot/epoch timing (12s slots, 32 slots/epoch)
☑ Committee-based attestation with BLS aggregation
☑ CL ↔ EL Engine API separation
☑ EIP-1559 fee market (base fee + priority tip)

Depth signals:
☐ PBS (Proposer-Builder Separation)
☐ Blob transactions (EIP-4844)
☐ Inactivity leak mechanism
☐ Verkle tree transition plan
