...
## MEV and Builder Metrics

| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `mev.builder.blocks_won` | Counter | Blocks won per builder per day | Single builder > 40% (centralization risk) |
| `mev.relay.block_value_eth` | Histogram | ETH value of builder bids | Sudden spike (possible exploit) |
| `mev.relay.bid_count` | Counter | Total bids received per slot | 0 bids (relay failure) |
| `mev.censorship.txns_delayed` | Gauge | Transactions pending > 10 blocks despite adequate fee | > 100 (possible censorship) |
| `mev.sandwich.detected` | Counter | Sandwich attack patterns detected in blocks | Informational (track trend) |
...
```
Ecosystem Health Dashboard (cross-node aggregation):

Decentralization Metrics:
├── Nakamoto coefficient (consensus): Min entities to control 33% stake
│   Healthy: > 10  |  Warning: 5-10  |  Critical: < 5
├── Client diversity (EL): No single client > 33%
│   Current risk: Geth at ~42% (above threshold)
├── Geographic distribution: Validators across 50+ countries
│   Risk: >50% in single jurisdiction enables regulatory shutdown
└── Builder diversity: No single builder > 30% of blocks

Chain Health Metrics:
├── Slot utilization: 99.5%+ (blocks produced in slots)
├── Finality streak: Consecutive finalized epochs
├── Reorg frequency: < 5 depth-1 reorgs per day
├── Attestation participation: > 95%
└── Inactivity leak active: NEVER (indicates finality failure)
```

---

## Consensus Health Automation

```
Automated response patterns for validator operators:

1. Missed Attestation Recovery:
   WHEN consensus.attestation.missed > 3 consecutive:
     CHECK p2p.peers.connected >= 10
     CHECK p2p.block.propagation_ms < 4000
     CHECK execution.block.processing_time_ms < 3000
     IF peers < 10: restart p2p layer, add manual peers
     IF propagation slow: check bandwidth, ISP issues
     IF execution slow: check disk I/O, memory pressure
     ELSE: check validator key availability

2. Finality Stall Detection:
   WHEN consensus.finality.delay_epochs > 2:
     ALERT severity=WARNING: "Finality delayed, monitor participation"
   WHEN consensus.finality.delay_epochs > 4:
     ALERT severity=CRITICAL: "Inactivity leak activated"
     LOG expected_recovery_time based on offline_validator_percentage
     MONITOR validator balance drain rate

3. Reorg Response:
   WHEN consensus.fork.reorg_depth == 1:
     LOG informational (normal behavior)
   WHEN consensus.fork.reorg_depth == 2:
     ALERT severity=WARNING: investigate proposer timing
   WHEN consensus.fork.reorg_depth >= 3:
     ALERT severity=CRITICAL: possible attack
     CHECK for coordinated builder behavior
     NOTIFY exchange partners to increase confirmation requirements

4. Client Update Monitoring:
   WHEN client_version < latest_release:
     IF release contains security fix: ALERT severity=HIGH
     IF release is routine: ALERT severity=LOW
     TRACK days_behind_latest for compliance reporting
```
...
```
Long-term observability queries for network research:

1. Fee Market Analysis:
   - Track base_fee_gwei over 30-day windows
   - Correlate with gas_used_ratio trends
   - Identify congestion patterns by day/hour
   - Compare pre/post-EIP-4844 rollup costs

2. Validator Performance Ranking:
   - Aggregate attestation inclusion rates per validator
   - Track proposal success rates over epochs
   - Identify validators consistently missing duties
   - Correlation: geographic region vs performance

3. MEV Trend Analysis:
   - Builder market share over time (weekly aggregation)
   - Average block value trend (MEV extraction efficiency)
   - Sandwich attack frequency and victim impact
   - Censorship metrics: OFAC-compliant vs non-compliant blocks

4. State Growth Projection:
   - Monthly state trie size delta
   - Contract creation rate vs destruction rate
   - Storage slot growth by contract category (DeFi, NFT, L2)
   - Projected storage requirements at current growth rate
```
