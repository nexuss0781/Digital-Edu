...
| State size growth | < 50 GB/year | With state Cutting off unnecessary steps and expiry |
...
## Hardware Requirements by Node Type

```
--- Full Node (Minimum) ---
CPU:             4+ cores (modern x86-64 or ARM64)
RAM:             16 GB (32 GB recommended for trie caching)
Storage:         2 TB NVMe SSD (random IOPS critical for trie access)
Bandwidth:       25 Mbps upload + download sustained
Disk I/O:        10,000+ random IOPS (HDD is NOT viable)
Cost estimate:   ~$100-200/month (cloud) or ~$1,500 upfront (home)

--- Archive Node ---
CPU:             8+ cores
RAM:             64 GB
Storage:         16+ TB NVMe SSD (growing ~2-3 TB/year)
Bandwidth:       50+ Mbps
Purpose:         Serve historical state queries (DApps, explorers)
Cost estimate:   ~$500-1,000/month (cloud)

--- Validator Node ---
CPU:             4+ cores (dedicated, not shared)
RAM:             32 GB
Storage:         2 TB NVMe SSD
Bandwidth:       25+ Mbps with high uptime (stable connection)
Staking requirement: 32 ETH (~$80K at $2,500/ETH)
Uptime requirement:  99.5%+ (penalties for missed attestations)
Cost estimate:   ~$150/month + 32 ETH stake

--- Light Client ---
CPU:             1 core (mobile-capable)
RAM:             256 MB
Storage:         < 1 GB (headers only)
Bandwidth:       1 Mbps intermittent
Purpose:         Verify specific state claims via Merkle/Verkle proofs
```
...
```
--- Validator Economics ---
Annual staking yield:         ~4-5% APR (varies with total staked)
Daily reward per validator:   ~0.003-0.004 ETH
Annual reward per validator:  ~1.1-1.5 ETH
Block proposal reward:        ~0.005-0.03 ETH (includes MEV tips)
Sync committee reward:        ~0.1-0.3 ETH per 256-slot period

Revenue sources:
├── Attestation rewards:      ~85% of total validator income
├── Block proposal rewards:   ~10% (including priority fees)
├── MEV tips (via PBS):       ~5% (highly variable)
└── Sync committee:           <1% (rare selection)

Penalty structure:
├── Missed attestation:       -0.000011 ETH per miss (~$0.03)
├── Missed proposal:          No direct penalty (missed opportunity)
├── Inactivity leak:          Quadratic increase after 4 epochs without finality
└── Slashing:                 Min 0.5 ETH + correlation penalty (up to full 32 ETH)

--- Fee Market Economics ---
Base fee (burned):            Adjusts +/-12.5% per block toward 50% gas usage
Priority fee (to proposer):  Market-driven; 1-5 gwei typical, 100+ gwei during congestion
Daily ETH burned:             ~1,000-3,000 ETH ($2.5M-$7.5M)
Annual ETH burned:            ~500K-1M ETH (potentially deflationary)
Blob fee:                     Separate market; ~0.01-1 gwei per blob gas unit
```

---

## Cost Estimation for Layer 2 Operations

```
--- Rollup Cost Per Transaction ---

Optimistic Rollup:
  L1 data cost:              ~16 gas/byte × ~100 bytes/tx = ~1,600 gas
  At 25 gwei base fee:       1,600 × 25 = 40,000 gwei = 0.00004 ETH (~$0.10)
  Compression ratio:          ~10x (100 bytes compressed from ~1 KB original)
  Batch amortization:         Fixed overhead spread across 1,000-10,000 txns

ZK Rollup:
  L1 data cost:              ~50 bytes/tx (state diffs, smaller than optimistic)
  Proof verification:        ~400K gas per batch (amortized across N txns)
  At 1,000 txns/batch:       400 gas/tx overhead for proof
  Total cost per tx:          ~$0.05-0.20

With EIP-4844 blobs:
  Blob cost:                  ~10-100x cheaper than calldata
  Optimistic rollup tx cost:  ~$0.001-0.01
  ZK rollup tx cost:          ~$0.005-0.05
  Impact:                     Rollup costs drop by 10-50x
```
...
```
--- Liveness Budget ---
Target: 99.5% of slots produce blocks (SLO)
Total slots/year: 2,628,000 (12s each)
Allowed missed slots: 13,140 per year (~36/day)
Budget per validator: ~0.5% missed attestations before penalties accelerate

--- Finality Budget ---
Target: 99.9% of epochs finalize
Total epochs/year: 82,125
Allowed missed finalizations: ~82 per year (~1.6/week)
If finality stalls > 4 epochs: inactivity leak activates
At 50% offline: finality recovers in ~18 days (leak drains offline stake)
At 34% offline: finality recovers in ~36 days

--- Data Durability ---
Replication factor: ~10,000 full nodes
Probability of data loss: effectively 0
  (would require >99.99% of nodes to fail simultaneously)
Blob data: available for ~18 days; permanent availability
  requires at least one archive/data availability node

--- Network Partition Tolerance ---
Partition affecting < 33% of stake: No impact on finality
Partition affecting 33-50% of stake: Finality stalls, chain continues
Partition affecting > 50% of stake: Chain may halt
Recovery: Automatic via inactivity leak (no manual intervention)
```
...
```
--- Global Propagation Model ---
Intra-region (same continent):       20-80ms RTT
Inter-region (cross-continental):    100-300ms RTT
Worst case (opposite hemispheres):   200-400ms RTT

--- Message Propagation Targets ---
Block (to 95% of network):         < 4,000ms
Transaction (to 95% of network):   < 3,000ms
Attestation (to aggregator):       < 2,000ms

--- Gossip Hop Budget ---
Average hops to reach all nodes:    ~6 (in mesh of 10,000)
Latency per hop:                    ~50ms (validate + forward)
Total propagation estimate:         ~300ms (parallel fan-out)
With validation overhead:           ~1,000-2,000ms realistic

--- Bandwidth Budget Per Node ---
Block relay:          ~100 KB/block × 5 blocks/min = ~8 KB/s
Transaction relay:    ~250 B/tx × 20 tx/s = ~5 KB/s
Attestation relay:    ~200 B × 500 attestations/slot = ~8 KB/s
Blob relay:           ~128 KB × 3 blobs/block × 5/min = ~32 KB/s
Total sustained:      ~53 KB/s (~0.4 Mbps) typical
Peak (congestion):    ~5-10 Mbps
```
...
8. Validator economics assume ~30M ETH total staked; yield decreases as total stake increases
9. Hardware requirements are designed so that hobbyist operators can run full nodes (decentralization preservation)
