...
| **Device Types** | Lights, switches, sensors (motion, temperature, humidity, door/window, soil moisture), locks, cameras, thermostats, speakers, appliances, blinds, garage doors, irrigation, solar inverters, battery storage, EV chargers, heat pumps, water heaters |
...
| Capability | Description |
|---|---|
| **Energy Monitoring** | Real-time energy consumption per device, room, and home via smart metering clusters |
| **Usage Analytics** | Historical trends, cost estimation, efficiency scoring, carbon footprint tracking |
| **Smart Scheduling** | Schedule high-energy devices during off-peak hours using real-time and forecasted tariff data |
| **Solar Integration** | Coordinate device usage with solar panel production, battery storage (BESS), and net metering |
| **EV Charging Management** | Smart EV charging with departure-time targets, rate adjustment, and state-of-charge reporting |
| **Bidirectional Energy (V2G)** | Vehicle-to-grid and vehicle-to-home discharge during peak pricing or grid emergencies |
| **Demand Response** | Participate in utility demand-response and virtual power plant programs via automated load shedding |
| **Tariff Optimization** | Ingest real-time and forecasted energy tariffs (time-of-use, dynamic pricing) to minimize cost |
| **Carbon-Aware Scheduling** | Shift flexible loads to time windows with lowest grid carbon intensity |

### 1.9 Camera and Media Management

| Capability | Description |
|---|---|
| **Live Streaming** | WebRTC-based live video/audio with two-way communication and PTZ controls |
| **Event Recording** | Motion-triggered clip recording with configurable retention and storage location (local/cloud) |
| **Edge AI Detection** | On-device person, pet, package, and vehicle detection without cloud dependency |
| **Privacy Zones** | User-defined masking zones excluded from recording and detection processing |
| **Multi-Stream** | Simultaneous high-res recording stream and low-res monitoring stream per camera |
| **Remote Access** | STUN/TURN-mediated access for viewing cameras from outside the home network |

### 1.8 Edge AI and Proactive Automation

| Capability | Description |
|---|---|
| **On-Device Inference** | Run ML models on hub NPU (6+ TOPS) for vision, audio, and behavior pattern recognition |
| **Activity Learning** | Learn occupant patterns (wake time, arrival, sleep) to suggest and auto-create automations |
| **Predictive Comfort** | Anticipate temperature/lighting needs based on time, weather forecast, and historical behavior |
| **Anomaly Detection** | Detect unusual device patterns (water leak, appliance malfunction, security anomaly) locally |
| **Proactive Suggestions** | LLM-powered natural language automation suggestions based on observed usage patterns |

### 1.10 OTA Firmware Updates
...
| **Matter commissioning** | Requires Thread border router or Wi-Fi for IP-based communication; NFC onboarding added in Matter 1.4.1 |
| **Matter camera bandwidth** | Camera support (Matter 1.5) uses WebRTC streams requiring full TCP transport — higher hub resource demand |
| **BLE range** | ~10m effective range; requires BLE mesh or bridge for whole-home coverage |
| **Wi-Fi band congestion** | Too many Wi-Fi devices degrade network performance; prefer low-power protocols for sensors |
| **Thread 1.4 credential sharing** | Thread 1.4 standardizes credential sharing across border routers, solving multi-mesh fragmentation |
| **Thread network size** | ~250 devices per Thread partition; supports multiple partitions via credential sharing |

### 5.2 Edge Hardware Constraints

| Constraint | Impact |
|---|---|
| **Hub CPU** | ARM-based SoC with integrated NPU (6+ TOPS for int4/int8 inference); enables local ML but constrains model size |
| **Hub RAM** | 2-8 GB; bounds the number of cached rules, shadow states, and concurrent ML model loads |
| **Hub storage** | 16-64 GB flash; limits local event history, firmware cache, and camera clip buffer |
| **Hub NPU** | Neural processing unit handles vision/audio models but limited to quantized models under ~500M parameters |
| **Power consumption** | Hub must operate within 5-15W for always-on operation; NPU inference adds 2-5W burst |
| **Radio concurrency** | Limited simultaneous protocol operations across shared radio hardware |
| **Display resources** | Hubs with touchscreens (7-inch typical) require UI rendering budget alongside automation processing |
