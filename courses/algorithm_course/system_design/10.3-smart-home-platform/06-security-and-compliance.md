...
## 6. Zero-Trust IoT Architecture

### 6.1 Zero-Trust Principles for Smart Homes

Traditional home network security assumes devices inside the network are trusted. Zero-trust eliminates this assumption — every device must continuously prove its identity and authorization:

```
Zero-Trust Implementation:

1. Device Identity Verification:
   - Every device interaction requires cryptographic attestation
   - Hardware-backed device identity (secure element or TPM)
   - Deterministic boot attestation before device enters any network segment
   - Continuous re-verification on each access (not just at pairing time)

2. Network Micro-Segmentation:
   Recommended Home Network Architecture:
     Primary VLAN:  Computers, phones (full internet access)
     IoT VLAN:      Smart home devices (hub-mediated internet only)
     Camera VLAN:   Cameras (isolated, hub-mediated access only)
     Guest VLAN:    Untrusted devices (internet only, no local access)

   Firewall rules:
     - IoT VLAN → Hub: ALLOW (device communication)
     - IoT VLAN → Internet: DENY (all cloud via hub proxy)
     - Camera VLAN → Hub: ALLOW (management + stream relay)
     - Camera VLAN → Internet: DENY
     - Cross-VLAN: DENY by default

3. Least-Privilege Access:
   - Devices can only communicate with the hub, not with each other directly
   - Hub enforces capability-level permissions (a motion sensor cannot issue lock commands)
   - Time-bounded session tokens with automatic expiry
   - API partners receive scoped tokens with explicit capability grants

4. Continuous Monitoring:
   - Device behavior profiling (expected traffic patterns per device type)
   - Anomaly alerts when device deviates from baseline
   - Automatic quarantine for devices exhibiting suspicious behavior
   - Hub-level intrusion detection monitoring all protocol traffic
```
...
### 7.1 Device Vulnerability Lifecycle
...
| **EU Cyber Resilience Act** | EU | Security-by-design, vulnerability handling, SBOM; effective 2025 | Secure development lifecycle, mandatory incident reporting within 24h |
| **EU Data Act** | EU | IoT data portability, fair access to device-generated data; effective Sept 2025 | Data export APIs, third-party access to non-personal device data |
| **US Cyber Trust Mark** | US | Voluntary IoT security certification program with consumer-facing label | Device attestation, OTA update capability, vulnerability disclosure |
| **US IoT Cybersecurity Act** | US | Strong auth, encryption, OTA updates for connected devices | Mandatory security baselines for government-procured IoT |
| **ETSI EN 303 645** | Global (IoT) | IoT security baseline requirements | 13 security provisions for consumer IoT |
| **Matter Certification** | Global | CSA certification requirements | Interoperability and security testing; Matter 1.4.2 security enhancements |

### 8.2 ETSI EN 303 645 Compliance
