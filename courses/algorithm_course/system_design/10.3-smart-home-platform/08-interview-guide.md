...
### 4.5 "How do you handle camera data differently from other devices?"

**Trap:** Candidate treats cameras as "just another device type" with the same data path.

**Correct:** "Cameras are architecturally distinct in two dimensions: data volume (1-4 Mbps per camera vs. bytes/second for sensors) and privacy sensitivity (video of home life vs. temperature readings). Video streams use peer-to-peer WebRTC connections or dedicated relay servers — never the MQTT telemetry pipeline. With Matter 1.5, cameras use WebRTC with STUN/TURN for both local and remote access. Only metadata (motion events, AI detection results, thumbnails) flows through the standard path. Edge AI on the hub runs person/pet/vehicle detection locally so raw video never leaves the home unless the user explicitly enables cloud recording — and even then, clips are encrypted with a per-home key the cloud never possesses (zero-knowledge model)."

### 4.6 "Why not just use HTTP for everything?"
...
## 6. Advanced Interview Topics (2025-2026)

### 6.1 Matter 1.5 and Protocol Convergence

**Question:** "How does Matter change the smart home architecture?"

**Strong answer:** "Matter adds an application-layer standard on top of Thread and Wi-Fi, but it doesn't replace Zigbee or Z-Wave for existing installations. The hub serves four Matter roles: commissioner, controller, Thread border router, and bridge for legacy devices. Matter 1.5 extends to cameras (WebRTC-based), closures, and energy management. The critical architectural impact is multi-admin — a single device can be controlled by up to 5 fabrics simultaneously (hub, voice assistant, phone app), each with independent encryption. Our device shadow pattern handles this by subscribing to state changes from all fabrics, but automation conflict resolution now must account for actions from controllers outside our platform."

### 6.2 Energy Management as a Platform Feature

**Question:** "How would you add energy management to the smart home platform?"

**Strong answer:** "Energy management transforms the platform from a device controller into a bidirectional energy participant. We ingest real-time tariffs, solar production, battery state-of-charge, and EV charging status. A reinforcement learning optimizer shifts flexible loads — EV charging, water heating, battery charge/discharge — to minimize cost while meeting comfort constraints. The platform can participate in virtual power plant programs, aggregating thousands of homes' batteries for grid-balancing. The key architectural addition is the tariff data pipeline: utility tariff structures (time-of-use, dynamic pricing, demand charges, carbon intensity) must be normalized across hundreds of utility formats into a standard model."

### 6.3 Edge AI and the Split-Compute Pattern

**Question:** "Where does ML fit in a smart home system?"

**Strong answer:** "ML runs at two layers with a split-compute pattern. The hub's NPU (6+ TOPS) runs quantized models for latency-sensitive, privacy-preserving tasks: person vs. pet detection on cameras (~50ms), anomaly detection on sensor patterns, and activity recognition. The cloud handles fleet-wide model training, complex multi-home reasoning, and LLM-based automation suggestions. The key insight is that hub inference preserves privacy — raw video never leaves the home — while achieving 50ms latency that cloud inference (300-800ms) cannot match. Model lifecycle management (versioning, A/B testing, NPU-specific quantization, rollback) adds OTA-like complexity to the edge software pipeline."

---

## 7. Case Study: Industry Architecture Patterns

### 7.1 Local-First with Cloud Intelligence Pattern

A major open-source smart home platform demonstrates the local-first model: 3,400+ integrations running entirely on local hardware, with sub-50ms sensor-to-actuator latency. All automations execute locally. Cloud is optional — used for remote access and voice processing only. This architecture attracts privacy-conscious users but limits advanced ML features that require cloud-scale compute. The lesson: local-first is viable and increasingly popular, but the split-compute pattern (local inference + cloud enrichment) offers the best balance of privacy, latency, and intelligence.

### 7.2 Multi-LLM Agentic Architecture Pattern

A major voice-first smart home platform replaced its legacy decision-tree voice processing with a multi-LLM agentic architecture. A central LLM orchestrator delegates to specialized "experts" (Smart Home Expert, Energy Expert, Entertainment Expert). The system maintains contextual memory persisting across days and devices — remembering that "the usual" means a specific combination of lighting and temperature settings. This pattern enables proactive suggestions ("I noticed you always dim the lights at 9 PM — would you like me to automate that?") that were impossible with rule-based NLU.
...
| "Design a home security monitoring system" | 24/7 monitoring center, emergency dispatch integration, false alarm reduction, cellular backup |
| "Design a smart irrigation system" | Environmental sensors (soil moisture, weather), water conservation optimization, zone scheduling |
...
|---|---|---|
| **Local command latency** | < 50ms | Hub to device via protocol bridge |
| **Cloud command latency** | < 300ms | App to device via cloud + hub |
| **MQTT connections per broker** | ~100K | Persistent TCP + TLS sessions |
| **Devices per home (avg)** | 15-25 | Growing 3-5 per year per home |
| **Edge AI inference** | ~50ms | Person detection on hub NPU |
| **Hub boot to operational** | < 30s | After power loss recovery |
| **Reconnect jitter window** | 0-120s | Prevents thundering herd |
