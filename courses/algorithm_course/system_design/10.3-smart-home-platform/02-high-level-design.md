...
flowchart TB
    ...
    subgraph Intelligence["Intelligence Layer"]
        ...
        ENERGY[Energy<br/>Optimizer]
        ANOMALY[Anomaly<br/>Detection]
        SUGGEST[LLM Automation<br/>Suggestions]
        EDGEAI[Edge AI<br/>Coordinator]
    end
    ...
    EVTBUS --> ENERGY & ANOMALY & SUGGEST & EDGEAI
    SHADOW --> EVTBUS
    CMD --> EVTBUS
    ...
    class NLU,ENERGY,ANOMALY,SUGGEST,EDGEAI intelligence
...
**Energy Optimizer:** Ingests real-time and forecasted utility tariffs, solar production data, battery state-of-charge, and EV charging status. Uses reinforcement learning models to dynamically shift household consumption — pre-cooling before peak pricing, charging EVs during off-peak windows, and coordinating battery discharge for virtual power plant participation. Processes tariff data structures that include time-varying rates, demand charges, and carbon intensity signals.

**Anomaly Detection:** Monitors device behavior patterns for anomalies (device offline longer than usual, unusual sensor readings, potential security breaches, water leak signatures). Uses edge-trained baseline models with cloud-side fleet-wide anomaly correlation. Triggers notifications and safety responses.

**LLM Automation Suggestions:** Analyzes user behavior patterns (same devices controlled at same times) and generates natural-language automation suggestions using large language models. The LLM orchestrator delegates to specialized sub-systems (energy expert, comfort expert, security expert) to propose context-aware routines. Uses collaborative filtering across opt-in homes for popular automation templates.

**Edge AI Coordinator:** Manages the lifecycle of ML models deployed to home hubs — including model selection based on hub hardware capability, quantization for NPU targets, versioned model distribution via OTA, and A/B testing of model variants across the fleet. Coordinates split-compute pipelines where hub NPUs produce keyframes and metadata while cloud models handle enrichment and complex reasoning.
...
│  │ │ 1.4 BR  │  │  ┌──────────────────────────┐   │
│  │ ├─────────┤  │  │ Edge AI Engine (NPU)       │   │
│  │ │ BLE 5.4 │  │  │ (person detection, anomaly │   │
│  │ │ Radio   │  │  │  detection, activity learn) │   │
│  │ └─────────┘  │  └──────────────────────────┘   │
│  └─────────────┘                                   │
│                                                    │
│  ┌──────────────────┐  ┌───────────────────────┐  │
│  │ MQTT Client       │  │ Device Manager        │  │
│  │ (cloud sync,      │  │ (pairing, firmware,   │  │
│  │  command relay)    │  │  health, Thread cred) │  │
│  └──────────────────┘  └───────────────────────┘  │
└──────────────────────────────────────────────────┘
```
...
**Edge AI Engine:** Runs quantized ML models on the hub's NPU (6+ TOPS) for local inference — person/pet/vehicle detection on camera feeds, activity pattern recognition, and anomaly detection on sensor streams. Produces metadata and event classifications without sending raw data to the cloud. Models are versioned and distributed via the Edge AI Coordinator service.
...
**Thread 1.4 Border Router:** The hub functions as a Thread border router, bridging Thread mesh devices to the IP network. Thread 1.4 standardizes credential sharing — when a second border router (e.g., a smart speaker) joins the network, it can join the existing Thread mesh rather than creating a separate partition, eliminating the fragmentation problem that plagued earlier Thread deployments.
...
### 4.5 Edge AI with Split-Compute Pipeline

| Decision | Run ML inference on hub NPU for latency-sensitive tasks; use cloud for enrichment |
|---|---|
| **Context** | Camera person detection, anomaly detection, and activity recognition need fast inference but models are large |
| **Decision** | Hub NPU runs quantized models (int4/int8) for real-time detection (~50ms). Cloud handles complex reasoning, fleet-wide learning, and model training. Hub produces keyframes and metadata; cloud enriches with context |
| **Rationale** | Local inference preserves privacy (raw video never leaves home), achieves 50ms latency (vs. 300-800ms cloud), and reduces cloud bandwidth by 75%+. NPUs rated at 6+ TOPS handle quantized vision models efficiently |
| **Trade-off** | Hub hardware cost increases ($50-100); model distribution adds OTA complexity; quantized models have lower accuracy than full-precision cloud models |
| **Mitigation** | Model A/B testing across fleet; graceful fallback to cloud when local confidence is low; model versioning with automatic rollback on accuracy regression |

### 4.6 Home-Level Data Partitioning
