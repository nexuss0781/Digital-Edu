...
### 2.6 Energy Management APIs

```
GET    /v1/homes/{home_id}/energy/dashboard              # Energy overview (production, consumption, cost)
GET    /v1/homes/{home_id}/energy/tariff                  # Current and forecasted tariff schedule
GET    /v1/homes/{home_id}/energy/devices                 # Energy-producing/consuming device summary
POST   /v1/homes/{home_id}/energy/schedule                # Set energy optimization preferences
GET    /v1/homes/{home_id}/energy/history                 # Historical energy data (daily/weekly/monthly)
POST   /v1/homes/{home_id}/energy/vpp/enroll              # Enroll in virtual power plant program
GET    /v1/homes/{home_id}/energy/vpp/status              # VPP participation status and credits
```

### 2.7 MQTT Topic Structure
...
Energy:
  home/{home_id}/energy/production                    # Solar/battery production data
  home/{home_id}/energy/consumption                   # Aggregate energy consumption
  home/{home_id}/energy/tariff                        # Tariff updates from utility
  home/{home_id}/energy/vpp/command                   # VPP dispatch commands
...
CAPABILITY_TO_MATTER_CLUSTER:
  ...
  "door_lock"         → 0x0101 (Door Lock)
  "thermostat"        → 0x0201 (Thermostat)
  "window_covering"   → 0x0102 (Window Covering)
  "energy_metering"   → 0x0091 (Electrical Energy Measurement)
  "ev_charger"        → 0x0099 (Energy EVSE)
  "battery_storage"   → 0x0096 (Device Energy Management)
  "camera"            → 0x0551 (Camera AV Stream Management)
```
...
ev_charger:
  commands: [start_charging, stop_charging, set_target_soc, set_departure_time]
  state: {
    is_charging: BOOLEAN,
    charge_rate_kw: FLOAT,
    vehicle_soc_pct: INT (0-100),
    target_soc_pct: INT,
    departure_time: TIMESTAMP,
    v2g_enabled: BOOLEAN,
    v2g_discharge_kw: FLOAT
  }
  events: [charging_started, charging_complete, vehicle_connected, vehicle_disconnected]

solar_inverter:
  commands: [set_export_limit]
  state: {
    current_production_kw: FLOAT,
    daily_production_kwh: FLOAT,
    panel_status: ENUM (producing, idle, fault),
    export_limit_kw: FLOAT
  }
  events: [production_started, production_stopped, fault_detected]

battery_storage:
  commands: [charge, discharge, set_reserve_pct, set_mode]
  state: {
    soc_pct: INT (0-100),
    charge_rate_kw: FLOAT,
    discharge_rate_kw: FLOAT,
    capacity_kwh: FLOAT,
    mode: ENUM (self_consumption, time_of_use, backup, vpp),
    reserve_pct: INT
  }
  events: [low_soc, full_charge, grid_discharge_requested]

energy_tariff:
  commands: []  // Read-only, ingested from utility
  state: {
    current_rate_kwh: FLOAT,
    current_period: STRING,
    next_rate_kwh: FLOAT,
    next_period_start: TIMESTAMP,
    carbon_intensity_gco2: FLOAT
  }
  events: [rate_changed, peak_period_approaching]
```
