...
Energy Management:
  smarthome.energy.production_kw           {source: solar|battery} [gauge]
  smarthome.energy.consumption_kw          {category}             [gauge]
  smarthome.energy.tariff_rate             {utility_id, period}   [gauge]
  smarthome.energy.optimization_savings    {region}               [counter: currency]
  smarthome.energy.vpp_dispatch_commands   {region}               [counter]
  smarthome.energy.vpp_contributed_kwh     {region}               [counter]
  smarthome.energy.carbon_avoided_gco2     {region}               [counter]

Edge AI:
  smarthome.edgeai.inference_latency_ms    {model, hub_generation} [histogram]
  smarthome.edgeai.model_version           {model, hub_generation} [gauge: distribution]
  smarthome.edgeai.detection_accuracy      {model, category}      [gauge]
  smarthome.edgeai.npu_utilization_pct     {hub_generation}       [histogram]
  smarthome.edgeai.fallback_to_cloud       {model, reason}        [counter]
  smarthome.edgeai.model_ota_success       {model}                [counter]
```
