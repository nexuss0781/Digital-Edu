...
| GPU Fleet | 1.3M | 2M+ | 4M+ | 8M+ |
| Data Center Capacity | 5 GW | 10+ GW | 20 GW | 40 GW |
...
Current State (2026):
  • Infrastructure CapEx: $115-135B annually
  • GPU fleet: 1.3M+ (NVIDIA Blackwell and Rubin generations)
  • Data center capacity: 10+ GW by late 2026
  • Active projects in 9+ countries
  • 30 data centers planned (26 in US)
  • Largest single site: 5 GW Hyperion (Louisiana)

Year 1 (+10% DAU):
  • Add 2 new regions (Africa, Middle East)
  • Increase cache cluster size by 30%
  • Deploy next-gen inference accelerators
  • Scale GPU fleet to 2M+ for ranking and AI moderation

Year 3 (+25% DAU):
  • Major shard rebalancing
  • Full LLM-based content moderation pipeline
  • Implement edge-native feed serving
  • Campus-level data center architecture (multi-GW sites)

Year 5 (+50% DAU):
  • Full edge compute for feed generation
  • Custom silicon for inference (beyond GPUs)
  • AI-driven auto-scaling and capacity planning
  • Projected $600B cumulative infrastructure investment
```
...
│  • Feed ranking inference: ~40% of GPU fleet                            │
│  • Content moderation (LLM-based): ~20%                                 │
│  • Recommendation (unconnected content): ~15%                           │
│  • Model training (offline): ~15%                                       │
│  • Other AI workloads (ads, search): ~10%                               │
│                                                                         │
│  SCALING APPROACH:                                                      │
│  • Multi-generational GPU strategy (Blackwell → Rubin)                  │
│  • Custom inference accelerators for high-volume models                  │
│  • External cloud commitments ($40B+ since Oct 2025)                    │
│  • Nebius partnership: $27B over 5 years for elastic GPU capacity       │
│                                                                         │
│  EFFICIENCY OPTIMIZATIONS:                                              │
│  • Model quantization: INT8/INT4 inference for ranking models           │
│  • Batched inference across concurrent feed requests                    │
│  • Model distillation: Smaller models for initial filtering stages      │
