...
Final_Score = w1 × Engagement + w2 × Value + w3 × Integrity + w4 × Satisfaction
...
Where:
  ...
  Satisfaction = UTIS_score (User True Interest Survey prediction)

Trade-off Example:
  • Clickbait: High engagement, low value, low satisfaction → Demoted
  • Thoughtful article: Medium engagement, high value, high satisfaction → Promoted
  • Misinformation: Any engagement score → Heavily demoted (integrity override)
  • Outrage content: High engagement, low satisfaction → Demoted via UTIS
...
Weight Tuning:
  ...
  • UTIS calibration: In-feed surveys ask "How well does this match your interests?"
```

### Unconnected Content Retrieval (2025-2026 Evolution)

```
UNCONNECTED CONTENT PIPELINE:
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                         │
│  UP TO 50% OF FEED NOW COMES FROM NON-FOLLOWED ACCOUNTS               │
│                                                                         │
│  Traditional Pipeline (Social Graph):                                   │
│  • Fan-out pushed content from friends                                  │
│  • Fan-out pulled content from celebrities                              │
│  • Page and Group content                                               │
│                                                                         │
│  New Pipeline (Interest Graph):                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ 1. User Interest Embedding                                      │   │
│  │    • Computed from interaction history, dwell time, saves        │   │
│  │    • Updated in near-real-time via streaming pipeline            │   │
│  │                                                                 │   │
│  │ 2. Content Embedding                                            │   │
│  │    • Generated at content creation time                          │   │
│  │    • Text, image, and video embeddings concatenated              │   │
│  │                                                                 │   │
│  │ 3. Approximate Nearest Neighbor Retrieval                       │   │
│  │    • Match user vector against content corpus                    │   │
│  │    • Return top-K candidates (typically 500-1000)                │   │
│  │                                                                 │   │
│  │ 4. Merge with Social Candidates                                 │   │
│  │    • Interleave social and interest candidates                   │   │
│  │    • Apply same ranking pipeline (engagement + value + integrity)│   │
│  │    • Balance ratio via diversity constraints                     │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  IMPLICATIONS:                                                          │
│  • Feed generation now requires ANN index lookup in addition to TAO     │
│  • Candidate pool size increased (social ~1500 + interest ~1000)        │
│  • Ranking models must learn cross-domain signals                       │
│  • Freshness boost applied heavily (50% more same-day content surfaced) │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## Deep Dive 4: AI Content Moderation Pipeline

### Architecture Overview

```
┌─────────────────────────────────────────────────────────────────────────┐
│                  AI CONTENT MODERATION PIPELINE (2025-2026)              │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  INLINE STAGE (Pre-Distribution):                                       │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ Every post/comment passes through BEFORE entering fan-out        │   │
│  │                                                                 │   │
│  │ Text Classifier (LLM-based)                                     │   │
│  │   • Hate speech, violence, harassment detection                  │   │
│  │   • Scam and fraud pattern recognition                           │   │
│  │   • Misinformation cross-referencing                             │   │
│  │   • LLMs now exceed human performance in select policy areas     │   │
│  │                                                                 │   │
│  │ Image/Video Classifier                                          │   │
│  │   • CSAM detection (hash-based + ML)                             │   │
│  │   • Violence and graphic content                                 │   │
│  │   • Nudity classification (artistic vs explicit)                 │   │
│  │                                                                 │   │
│  │ Hash Matching                                                   │   │
│  │   • Known bad content (terrorism, exploitation)                  │   │
│  │   • PhotoDNA and video fingerprinting                            │   │
│  │   • Near-duplicate detection for viral harmful content           │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  DECISION ROUTING:                                                      │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ Confidence > 95%  →  Auto-remove (no human review needed)        │   │
│  │ Confidence 70-95% →  Publish with reduced distribution + queue   │   │
│  │ Confidence < 70%  →  Publish normally                            │   │
│  │                                                                 │   │
│  │ Special cases:                                                  │   │
│  │ • Known hash match → Instant block (0 latency tolerance)         │   │
│  │ • Scam detection → 5,000+ attempts caught daily by AI alone      │   │
│  │ • Emerging crisis → Model hot-swap within hours                  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  POST-DISTRIBUTION MONITORING:                                          │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ • Continuous re-scoring as content goes viral                    │   │
│  │ • User reports feed back into classifier training                │   │
│  │ • Appeals process for overturned decisions                       │   │
│  │ • Up to 90% of risk assessments now automated                    │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  DEPLOYMENT INDEPENDENCE:                                               │
│  • Integrity models updated independently from ranking models           │
│  • Can deploy new policy classifiers within hours during crises          │
│  • Separate model serving infrastructure from feed ranking               │
│  • Human reviewers retained for high-risk and complex decisions          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## Deep Dive 5: Memcached at Scale
...
### Slowest part of the process 1: TAO Cache Invalidation Lag
...
### Slowest part of the process 2: Feed Generation Latency
...
### Slowest part of the process 3: Write Amplification
...
### Slowest part of the process 4: Hot Shards

```
PROBLEM:
  Celebrity's shard receives disproportionate traffic
  Single MySQL shard becomes Slowest part of the process
