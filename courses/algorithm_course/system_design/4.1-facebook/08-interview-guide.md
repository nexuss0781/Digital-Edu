...
## Modern Architecture Questions (2025-2026)

### "How has the shift to AI-recommended content changed the architecture?"

```
GOOD ANSWER:
"The biggest architectural shift is that up to 50% of feed content now comes
from accounts the user doesn't follow. This fundamentally changes the system:

1. CANDIDATE RETRIEVAL now has THREE sources instead of two:
   • Pushed content from friends (fan-out on write)
   • Pulled content from celebrities (fan-out on read)
   • Retrieved content from interest-based embeddings (ANN search)

2. The RANKING PIPELINE is heavier:
   • 100+ prediction models per feed request
   • UTIS (User True Interest Survey) adds satisfaction as a signal
   • Must balance social and algorithmic content

3. INFRASTRUCTURE scaled dramatically:
   • 1.3 million+ GPUs for inference
   • $115-135 billion annual infrastructure spend
   • Sequence learning architecture for longer behavior histories

The system has evolved from 'show me what my friends posted' to
'show me what I'll find valuable' -- while keeping social content as
a distinct signal rather than abandoning it entirely."
```

### "How does AI content moderation work at this scale?"

```
GOOD ANSWER:
"Content moderation is an INLINE pipeline stage, not a background process.
Every piece of content passes through multiple AI classifiers BEFORE
entering the fan-out path:

1. Text classifiers (LLM-based, exceed human performance in some areas)
2. Image/video classifiers (nudity, violence, CSAM)
3. Hash matching (known bad content, instant block)

The key insight is the CONFIDENCE-BASED ROUTING:
• High confidence (>95%): Auto-remove, no human needed
• Medium confidence (70-95%): Publish with reduced distribution, queue for review
• Low confidence (<70%): Publish normally

This catches 5,000+ scam attempts daily that humans couldn't detect.
The moderation models deploy independently from ranking models, so
during a crisis, integrity classifiers can be updated within hours
without touching the engagement models."
```
...
|-----------|----------------|
| **Instagram** | More visual focus, Explore tab, Stories, shared TAO infrastructure |
| **Twitter/X** | Public by default, shorter content, trends, asymmetric follow graph |
| **LinkedIn** | Professional graph, job recommendations, different engagement signals |
| **TikTok** | Pure interest graph (no social graph), short video, full recommendation-driven |
