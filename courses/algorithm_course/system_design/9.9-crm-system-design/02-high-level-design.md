...
```mermaid
flowchart TB
    subgraph AILayer["AI & Intelligence Services"]
    ...
    subgraph DataFeed["Data Feeds"]
        CRM_DATA[CRM Record Data]
        EMAIL_DATA[Email & Activity Streams]
        CALL_DATA[Call Transcripts]
        WEB_DATA[Web Engagement Signals]
    end

    subgraph Output["AI Outputs"]
        SCORE_OUT[Lead/Opportunity Scores]
        DRAFT_OUT[Email & Response Drafts]
        INSIGHT_OUT[Deal Insights & Alerts]
        FORECAST_OUT[Predictive Forecasts]
        ACTION_OUT[Recommended Actions]
    end
    ...
    PREDICT --> SCORE_OUT
    PREDICT --> FORECAST_OUT
    COPILOT --> DRAFT_OUT
    CONV --> INSIGHT_OUT
    NLP --> INSIGHT_OUT
    RECOMMEND --> ACTION_OUT

    classDef ai fill:#e8eaf6,stroke:#283593,stroke-width:2px
    classDef feed fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef output fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px

    class COPILOT,PREDICT,NLP,CONV,RECOMMEND ai
    class CRM_DATA,EMAIL_DATA,CALL_DATA,WEB_DATA feed
    class SCORE_OUT,DRAFT_OUT,INSIGHT_OUT,FORECAST_OUT,ACTION_OUT output
```

The AI layer operates as a set of inference services that consume CRM data and produce augmented outputs:

**AI Copilot Service**: Generates contextual drafts (follow-up emails, meeting summaries, proposal outlines) using the record context (Opportunity stage, Account history, Contact communication preferences). Operates within the user's security scope --- only accesses records the user can see --- to prevent information leakage through AI-generated content.

**Predictive Analytics Engine**: Trains per-tenant models for deal win probability, expected close date, and churn risk. Uses features extracted from opportunity stage velocity (time between stage transitions), email engagement patterns, and historical conversion data. Models are retrained weekly with automatic drift detection.

**Conversation Intelligence**: Processes call recordings and meeting transcripts via speech-to-text followed by NLP entity extraction. Identifies competitor mentions, pricing discussions, objection patterns, and action items. Auto-populates CRM fields (next steps, competitor, budget discussed) from unstructured conversation data.

**Next-Best-Action Engine**: Combines predictive scores, deal stage, activity recency, and engagement patterns to recommend the optimal next action for each opportunity (schedule follow-up call, send case study, involve technical resource, escalate to manager). Actions are surfaced as contextual suggestions in the record detail view.

---

## Event Mesh Architecture

Beyond point-to-point event routing, the CRM platform uses an event mesh pattern to support complex event flows across domain services, platform services, and external consumers:

```
Event Categories and Routing:

RECORD EVENTS (high volume, low latency)
├── Record Created / Updated / Deleted
├── Routed to: Workflow Engine, Search Indexer, Analytics Pipeline, CDC Subscribers
└── Retention: 72 hours in event bus; 30 days in CDC replay store

PLATFORM EVENTS (tenant-defined, medium volume)
├── Custom events published by tenant automation or API
├── Routed to: Streaming API subscribers, Flow triggers, External webhooks
└── Retention: 24 hours; configurable replay window

SYSTEM EVENTS (low volume, high importance)
├── Metadata changes, Security model changes, User provisioning
├── Routed to: Cache invalidation, Audit log, Admin notifications
└── Retention: 90 days in audit store

ENGAGEMENT EVENTS (high volume, async processing)
├── Email opens, Link clicks, Page visits, Form submissions
├── Routed to: Scoring Engine, Activity Logger, Marketing Analytics
└── Retention: 90 days hot; 2 years warm
```

Event ordering guarantees vary by category: record events for the same record are strictly ordered (same partition key), while engagement events are ordered per-lead but may arrive out of order across leads. The event mesh handles back-pressure by applying per-tenant quotas on event publishing rate, preventing any single tenant's automation from flooding the event bus and delaying event delivery for other tenants.
