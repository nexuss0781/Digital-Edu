...
Platform upgrades must deploy without affecting tenants. The deployment follows a canary rollout across pods:

```
Deployment Pipeline:

Phase 1 — Canary Pod (1 pod, ~5,000 tenants)
├── Deploy new version to a single pod
├── Monitor for 4 hours: error rate, latency, governor violations
├── Automated rollback if error rate > 0.1% or p99 latency > 2x baseline
└── Human approval gate before proceeding

Phase 2 — Early Adopter Pods (5 pods, ~25,000 tenants)
├── Deploy to 5 pods in parallel
├── Monitor for 12 hours
├── Include pods from different regions for geographic coverage
└── Automated rollback per-pod if metrics degrade

Phase 3 — General Availability (remaining pods)
├── Deploy to remaining pods in rolling batches of 10
├── Each batch monitored for 1 hour before proceeding
├── Total deployment window: 24-48 hours across all pods
└── Rollback to previous version available for 7 days per pod
```

**Database schema migrations** are handled with expand-and-contract: new columns are added (expand) without removing old columns, application code is deployed to use new columns, then old columns are removed (contract) in a subsequent release. This ensures backward compatibility during the rolling deployment window when different pods run different application versions.

---

## Noisy Neighbor Detection and Mitigation

Multi-tenant platforms must detect and contain tenants that consume disproportionate resources. The noisy neighbor detection system operates at two levels:

**Transaction-level** (immediate): Governor limits enforce per-transaction resource caps. A tenant's single API request cannot exceed 100 SOQL queries or 10 seconds of CPU time, regardless of the tenant's size or plan tier.

**Aggregate-level** (trending): The platform tracks per-tenant resource consumption over sliding windows (1 hour, 24 hours) and computes a "noisy neighbor score" --- the ratio of actual consumption to expected consumption based on the tenant's size and plan tier. When the score exceeds 3x for sustained periods:

1. **Alert**: Notify the platform team and the tenant's account manager
2. **Throttle**: Reduce the tenant's concurrent API connections by 50%
3. **Isolate**: If throttling is insufficient, migrate the tenant to a dedicated pod (for Enterprise tier) or enforce stricter rate limits (for lower tiers)
4. **Root cause**: Work with the tenant to identify and optimize the problematic automation or integration
