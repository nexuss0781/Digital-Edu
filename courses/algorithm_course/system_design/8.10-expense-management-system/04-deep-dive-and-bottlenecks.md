...
### Slowest part of the process 1: OCR Pipeline Throughput During Month-End Surge
...
### Slowest part of the process 2: Policy Engine for Complex Multi-Rule Organizations
...
### Slowest part of the process 3: Reimbursement Batch Processing and Bank Integration
...
## 7. Deep Dive: Card Reconciliation Timing Windows

### The Problem

Corporate card transactions traverse multiple stages between the card swipe and final settlement, each producing slightly different data. The matching engine must handle these temporal discrepancies without creating false matches or missing real ones.

### Transaction Timeline

```
Card Swipe → Authorization (immediate) → Clearing (1-3 days) → Settlement (2-5 days)
                    ↓                           ↓                        ↓
              Amount: $47.50              Amount: $52.30           Amount: $52.30
              (pre-tip)                   (with tip + tax)         (final)
              Merchant: "SQ *CAFE"        Merchant: "SQUARE CAFE"  Merchant: "SQUARE INC"
```

The authorization amount may differ from the settlement amount (tips, tax adjustments, partial captures). The merchant name may change at each stage. The date shifts from authorization to posting.

### Matching Strategy by Transaction Stage

| Stage | When Available | Amount Accuracy | Merchant Accuracy | Match Confidence |
|-------|---------------|-----------------|-------------------|-----------------|
| **Authorization** | Real-time (seconds) | Low (pre-tip, pre-tax) | Low (terminal ID) | Preview only — do not auto-match |
| **Clearing** | 1-3 business days | High (final amount) | Medium (acquirer name) | Suitable for auto-matching |
| **Settlement** | 2-5 business days | Exact | Variable (processor name) | Highest confidence; reconciliation source |

```
FUNCTION handle_card_event(event):
    SWITCH event.stage:
        "authorization":
            -- Create provisional record; do NOT attempt matching yet
            INSERT CardTransaction(status="provisional", amount=event.amount, ...)
            NOTIFY_EMPLOYEE("card_activity", event)

        "clearing":
            -- Update with final amount; attempt matching
            UPDATE CardTransaction SET amount = event.amount,
                merchant = event.merchant, status = "cleared"
                WHERE authorization_id = event.auth_id
            ENQUEUE("card_matching", { transaction_id: txn.id })

        "settlement":
            -- Reconciliation: verify matched amounts are correct
            UPDATE CardTransaction SET settled_amount = event.amount, status = "settled"
            IF txn.matched_expense_id IS NOT NULL:
                verify_match_amount(txn, event.amount)
```

### Reconciliation Window

Unmatched card transactions are tracked through escalating action windows:

| Window | Action | Target |
|--------|--------|--------|
| 0-7 days | Passive — wait for receipt submission | Employee |
| 7-14 days | Gentle reminder: "Submit receipt for $52.30 at Square Cafe" | Employee |
| 14-21 days | Escalation: notify manager of outstanding transactions | Manager |
| 21-30 days | Compliance alert: flag as unsubstantiated expense | Finance |
| 30+ days | Auto-categorize as "policy violation — missing receipt" | Audit trail |

---

## 8. Slowest part of the process Summary

| Slowest part of the process | Severity | Peak Impact | Primary Mitigation | Secondary Mitigation |
|-----------|----------|-------------|-------------------|---------------------|
| OCR pipeline throughput | **High** | Month-end surge: 10x normal volume | GPU auto-scaling on queue depth | Client-side preprocessing; batch inference |
| Policy engine under complex rules | **Medium** | Large enterprises: 500+ rules × 50 items | Rule indexing + short-circuit eval | Compiled rule cache; async budget checks |
| Reimbursement batch processing | **Medium** | ACH cutoff timing; bank API rate limits | Rolling batches throughout day | Multi-rail routing; pre-validated bank details |
| Card reconciliation timing | **Low-Medium** | Settlement delays create matching uncertainty | Stage-aware matching; authorization preview | Escalation timer windows |
| Audit log write throughput | **Low** | 7.5M events/day during peaks | Batched writes (100ms window) | Time-partitioned storage; async consumers |
| Budget enforcement contention | **Low** | Concurrent submissions to same cost center | SELECT FOR UPDATE with short lock window | Reservation pattern with async commit |

### Cross-Slowest part of the process Interactions

Month-end surges amplify all bottlenecks simultaneously. The OCR queue grows, backing up policy evaluations (which depend on OCR-extracted fields). Approval queues fill as managers are overwhelmed. Reimbursement batches grow larger, competing for bank API quotas. The mitigation strategy must address these as a system, not individually:

1. **T-48 hours before month-end**: Pre-scale OCR workers, add read replicas, expand connection pools
2. **T-24 hours**: Defer non-critical background jobs (analytics aggregation, model retraining, reconciliation reports)
3. **During surge**: Priority queues ensure user-facing operations (submit, approve) take precedence over background work (matching, GL posting)
4. **T+24 hours after month-end**: Drain deferred queues; run reconciliation checks; restore normal scaling
