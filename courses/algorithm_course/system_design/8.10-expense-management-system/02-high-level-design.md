...
| Monolith with modules | Simpler deployment; strong consistency | Scaling Slowest part of the process at OCR; single failure domain; slower team velocity |
...
## 7. Multi-Tenant Data Isolation Architecture

Multi-tenancy is a first-class concern in expense management because policy evaluation leaking across tenants could approve expenses that violate a company's compliance controls. Isolation operates at every layer:

```mermaid
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph Isolation["Multi-Tenant Isolation Layers"]
        direction TB
        L1["API Gateway\nTenant header injection from JWT"]
        L2["Service Layer\nTenant-scoped queries enforced\nby middleware"]
        L3["Database Layer\nRow-level security policies\nHash-partitioned by org_id"]
...
|----------------|-----------|-------------|
| **API Layer** | JWT contains `org_id`; gateway injects tenant header on every request | All downstream services receive tenant context without trusting client input |
| **Service Layer** | Middleware appends `WHERE org_id = ?` to every database query | Prevents accidental cross-tenant data access even with developer errors |
| **Database** | Row-level security policies enforce `org_id` filtering at the DB engine level | Defense-in-depth: even raw SQL queries cannot access other tenants |
| **Object Storage** | Receipts stored under `{org_id}/receipts/{year}/{receipt_id}` | Path-based isolation; signed URLs scoped to tenant prefix |
| **Cache** | All keys prefixed with `org:{org_id}:` | Cache poisoning across tenants is structurally impossible |
| **Event Bus** | Events include `org_id`; consumers filter by tenant | Analytics and audit processing is tenant-isolated |

**Enterprise tier override**: Organizations requiring regulatory isolation (financial institutions, government) receive dedicated database shards and isolated compute pools.

---

## 8. Email Receipt Forwarding Pipeline

Many expense platforms support forwarding receipt emails to a dedicated address (e.g., `receipts@expenses.company.com`). The ingestion pipeline must handle attachments, inline images, and email-body receipts.

```
FUNCTION process_receipt_email(email):
    -- Extract sender identity
    org_id, employee = resolve_sender(email.from_address)
    IF employee IS NULL:
        BOUNCE(email, "unrecognized_sender")
        RETURN

    -- Extract receipt images from attachments and inline content
    receipts = []
    FOR EACH attachment IN email.attachments:
        IF attachment.mime_type IN ["image/jpeg", "image/png", "application/pdf"]:
            receipts.APPEND(attachment.content)
    IF LEN(receipts) == 0:
        -- Check for inline images in HTML body
        inline_images = extract_inline_images(email.html_body)
        receipts.EXTEND(inline_images)
    IF LEN(receipts) == 0:
        BOUNCE(email, "no_receipt_found")
        RETURN

    -- Process each receipt
    FOR EACH receipt_image IN receipts:
        receipt_id = store_receipt(org_id, employee.id, receipt_image)
        ENQUEUE("ocr_processing", { receipt_id, org_id, employee_id: employee.id })

    NOTIFY(employee, "receipts_received", { count: LEN(receipts) })
```
...
Approved expenses must flow into the company's accounting system. Since ERPs vary widely (dozens of platforms with different APIs, schemas, and sync models), the integration layer uses an adapter pattern.

| Integration Pattern | Use Case | Sync Model |
|--------------------|----------|------------|
| **Webhook push** | Modern cloud ERPs with webhook support | Near-real-time; event-driven on approval |
| **Batch file export** | Legacy ERPs requiring file-based import | Scheduled; CSV/XML files generated on configurable cadence |
| **API polling** | ERPs with REST APIs but no webhook support | Periodic pull by ERP; expense service exposes paginated export endpoint |
| **Direct DB write** | On-premise ERPs with database-level integration | Connector writes journal entries directly to ERP database tables |

```
FUNCTION post_to_erp(report, org_id):
    erp_config = GET_ERP_CONFIG(org_id)
    adapter = get_erp_adapter(erp_config.type)  -- e.g., sap, netsuite, quickbooks

    -- Map expense data to ERP-specific journal entry format
    journal_entries = []
    FOR EACH item IN report.items:
        gl_account = map_to_gl_account(item.category, erp_config.chart_of_accounts)
        journal_entries.APPEND({
            debit_account: gl_account,
            credit_account: erp_config.ap_account,
    ...
    -- Submit with idempotency key to prevent double-posting
    result = adapter.post_journal_entries(
        journal_entries,
        idempotency_key = report.report_id + "_" + report.version
    )

    IF result.success:
        UPDATE ExpenseReport SET erp_status = "posted", erp_reference = result.reference_id
        audit_log("erp_posted", report.report_id, { reference: result.reference_id })
    ELSE:
        ENQUEUE_RETRY("erp_posting", { report_id: report.report_id, attempt: 1 })
        audit_log("erp_posting_failed", report.report_id, { error: result.error })
```
