...
**Good answer**: "Receipt images (1--5 MB each) bloat table sizes, degrade query performance, and make the database the Slowest part of the process for both reads and writes. Object storage scales horizontally, supports CDN integration, provides lifecycle policies for archival, and costs 10--50x less per GB. The database stores only metadata and a URI reference."
...
## Technology Awareness Signals

Mentioning these demonstrates awareness of the expense management domain beyond basic CRUD:

| Signal | Context | Impact |
|--------|---------|--------|
| **LayoutLMv3** | Multi-modal transformer for receipt field extraction | Shows awareness of current ML architecture for document understanding |
| **Perceptual hashing (pHash)** | Near-duplicate receipt detection across image variations | Demonstrates knowledge of fuzzy matching beyond exact-hash comparison |
| **Declarative policy DSL** | Non-engineer-configurable rules with compiled runtime evaluation | Shows understanding of business rule engines vs. hard-coded logic |
| **Hash-chained audit log** | Tamper-evident financial audit trail for SOX compliance | Demonstrates knowledge of cryptographic integrity guarantees |
| **Budget reservation pattern** | Pessimistic locking for concurrent budget enforcement | Shows understanding of financial consistency under concurrency |
| **Card network settlement stages** | Authorization → clearing → settlement timing windows | Demonstrates domain expertise in card reconciliation challenges |
| **Calendar-aware pre-scaling** | Proactive capacity planning for predictable month-end surges | Shows operational maturity beyond reactive auto-scaling |
| **CQRS for reporting** | Separate read model for analytics without impacting transactional writes | Demonstrates architecture pattern knowledge applied to a concrete problem |

---

## Estimation Practice

### Worked Example: Receipt Storage Cost

```
Given:
  - 2M receipts/day, average 1.5 MB each
  - 7-year retention requirement
  - Hot storage: $0.023/GB/month, Warm: $0.012/GB/month, Cold: $0.004/GB/month

Calculation:
  Daily raw:    2M × 1.5 MB = 3 TB/day
  After dedup:  ~2 TB/month (30% dedup rate on email-forwarded duplicates)
  7-year total: 2 TB × 12 × 7 = 168 TB

  Hot (0-90d):   6 TB × $0.023 = $138/month
  Warm (90d-2y): 42 TB × $0.012 = $504/month
  Cold (2-7y):   120 TB × $0.004 = $480/month
  Total storage: ~$1,122/month = ~$13.5K/year

  Key insight: Cold storage dominates volume (71%) but costs less than warm (42%)
               because the per-GB rate drops 3x. Lifecycle policies are essential.
```
