...
| Slowest part of the process Approvers | Table (approver, pending count, avg response time) | Workflow engine |
...
| `ReimbursementCycleTimeIncrease` | p50 cycle time > 7 days (rolling 7-day window) | Identify Slowest part of the process stage; check for approval backlog |
...
| `ApprovalSLABreach` | 1. Identify Slowest part of the process approvers (query workflow engine). 2. Check if approvers are OOO without delegates configured. 3. Trigger escalation notifications. 4. If systemic, notify org admins for bulk reassignment. | Product On-Call |
...
Financial accuracy is non-negotiable. These metrics track the integrity of the money flow from expense approval through reimbursement to GL posting.

| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `reimbursement.amount_mismatch_rate` | Gauge | % of reimbursements where paid amount differs from approved amount | > 0% (any mismatch is critical) |
| `gl_posting.reconciliation_gap` | Gauge | $ difference between approved totals and GL-posted totals per period | > $100 aggregate |
| `card_matching.orphan_transaction_age_p95` | Histogram | 95th percentile age of unmatched card transactions | > 14 days |
| `fx_rate.stale_rate_usage` | Counter | FX conversions using cached rates older than configured threshold | > 5% of conversions |
| `budget.overcommit_rate` | Gauge | % of cost centers where spent + reserved exceeds allocated | > 0% |
| `reimbursement.duplicate_prevention_catches` | Counter | Duplicate reimbursement attempts caught by idempotency | Any occurrence logged for investigation |

### Compliance Readiness Dashboard

```
┌──────────────────────────────────────────────────────────────────┐
│ Compliance Readiness                           2026-03-21 14:00  │
├──────────────────┬──────────────────┬────────────────────────────┤
│ Audit Trail      │ Data Retention   │ Policy Coverage            │
│ ──────────────   │ ──────────────   │ ──────────────             │
│ Chain Valid: 100% │ Hot: 28.4 TB    │ Orgs with active rules: 98%│
│ Completeness: 100%│ Warm: 24.1 TB  │ Avg rules per org: 142     │
│ Lag: 0.3s        │ Cold: 156.2 TB   │ Stale rules (>1yr): 3.2%  │
│ [SOX: PASS]      │ [Retention: OK]  │ [Coverage: OK]             │
├──────────────────┴──────────────────┴────────────────────────────┤
│ Receipt Integrity     │ FX Accuracy          │ Budget Controls    │
│ ──────────────        │ ──────────────       │ ──────────────     │
│ Hash verified: 100%   │ Rate staleness: 0.1% │ Over-budget: 0     │
│ Missing receipts: 0.4%│ Source diversity: 4/5 │ Near-limit: 12     │
│ 7yr available: 100%   │ Variance < 0.5%: 99% │ Untracked: 0       │
│ [IRS: PASS]           │ [Treasury: OK]       │ [Controls: OK]     │
└──────────────────────────────────────────────────────────────────┘
```

### Anomaly Detection Operational Metrics

| Metric | Description | Purpose |
|--------|-------------|---------|
| `anomaly.detection_rate` | % of expenses flagged as anomalous | Track model effectiveness; sudden changes indicate model drift |
| `anomaly.false_positive_rate` | % of flagged expenses confirmed as legitimate | Model tuning signal; > 30% indicates over-sensitive model |
| `anomaly.investigation_time_p50` | Median time to resolve flagged anomalies | Operational efficiency of compliance team |
| `anomaly.patterns_detected` | Top anomaly categories (weekend spend, round numbers, split-to-avoid) | Identifies emerging fraud patterns across the platform |
