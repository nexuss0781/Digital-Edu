...
Durable execution frameworks represent a paradigm shift from explicit state machine management to **transparent persistence of program execution**. Instead of saving saga state after each step, the framework journals every external interaction and replays the execution on failure.
...
sequenceDiagram
    participant C as Client
    participant DE as Durable Engine
    participant H as Execution History
    participant INV as Inventory
    participant PAY as Payment

    C->>DE: Start OrderWorkflow
    DE->>H: Record WorkflowStarted

    DE->>INV: Reserve Inventory
    DE->>H: Record ActivityScheduled
    INV-->>DE: Success
    DE->>H: Record ActivityCompleted

    DE->>PAY: Process Payment
    DE->>H: Record ActivityScheduled

    Note over DE: Engine crashes here

    Note over DE: Engine restarts
    DE->>H: Replay history
    Note over DE: Skip Reserve (already done)
    Note over DE: Resume at Payment

    PAY-->>DE: Success
    DE->>H: Record ActivityCompleted
    DE->>H: Record WorkflowCompleted
    DE-->>C: Order Complete
```

### How Durable Execution Differs from Saga Orchestration

```
┌────────────────────────────────────────────────────────────────────┐
│ DURABLE EXECUTION vs SAGA ORCHESTRATION                             │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Saga Orchestration:                                                │
│   Developer defines:                                               │
│   • State machine (RUNNING, COMPENSATING, COMPLETED, FAILED)      │
│   • Explicit step list with compensations                         │
│   • Idempotency key generation per step                           │
│   • State persistence after each transition                       │
│   • Recovery logic (read state, determine next step)              │
│                                                                     │
│ Durable Execution:                                                  │
│   Developer writes:                                                │
│   • Sequential code (function calls)                              │
│   • Compensation functions (same as saga)                         │
│   Framework handles:                                               │
│   • Automatic journaling of all side effects                      │
│   • Deterministic replay on failure                               │
│   • Idempotency via replay (no keys needed)                       │
│   • State recovery via history replay                             │
│                                                                     │
...
│                                                                     │
│ What DOES change:                                                   │
│   • No explicit state machine code                                │
│   • No manual idempotency implementation                          │
│   • Simpler debugging (replay exact execution)                    │
│   • Lower accidental complexity                                   │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```

### Design Decision: When to Use Durable Execution vs Traditional Saga

| Factor | Traditional Saga | Durable Execution |
|--------|-----------------|-------------------|
| **Team expertise** | Familiar with state machines | Prefer writing sequential code |
| **Existing infra** | Message queue + state store already deployed | Willing to adopt new runtime |
| **Latency sensitivity** | Direct service calls, minimal overhead | Journaling adds 1-5ms per step |
| **Debugging needs** | State queries sufficient | Need full execution replay |
| **Vendor independence** | Custom implementation, no lock-in | Framework dependency |
...
| **Durable Execution** | Sequential code, journaled | Execution history | Complex workflows with failure recovery |
