...
## Insight 11: Durable Execution Eliminates Accidental Complexity but Not Essential Complexity
**Category:** Architecture
**One-liner:** Workflow-as-code frameworks like Temporal remove the need for explicit state machines, idempotency stores, and recovery logic, but compensation design, isolation gaps, and CAP trade-offs remain the developer's responsibility.
**Why it matters:** The rise of durable execution frameworks (Temporal, Restate, DBOS) represents the most significant shift in distributed transaction tooling since the saga pattern was popularized. By journaling every external interaction and replaying deterministically on failure, these frameworks eliminate entire categories of bugs: forgotten state persistence, missing idempotency keys, incorrect recovery logic. A developer writes `result = await reserveInventory(orderId)` and the framework ensures this runs exactly once even across crashes. However, this creates a dangerous illusion of simplicity. The essential complexity remains untouched: you still need compensation functions for every reversible step, concurrent sagas still see intermediate states (no isolation), and network partitions still force consistency-availability trade-offs. The architectural lesson is that durable execution is a better *implementation substrate* for sagas, not a replacement for understanding distributed transaction theory.

---

## Insight 12: Semantic Locking Converts the Saga Isolation Problem from a Framework Concern to a Data Model Concern
**Category:** Contention
**One-liner:** Adding a "processing" status to business entities (inventory items, payment authorizations) gives sagas an application-level isolation mechanism that no coordinator framework can provide generically.
**Why it matters:** The lack of isolation in sagas is often cited as their biggest weakness compared to 2PC. Two concurrent order sagas can both read inventory as "available" and both proceed to reserve it, leading to overselling. The textbook answer is "accept eventual consistency," but many business domains cannot tolerate this. Semantic locking solves this at the data model level: an inventory item transitions from AVAILABLE → PROCESSING (locked to a specific saga) → RESERVED. Other sagas querying the same item see PROCESSING and can either wait or abort early. This approach works because the lock is embedded in the business entity's lifecycle, not in an external lock manager. The lock's TTL and cleanup are domain-specific (a "stale PROCESSING" reaper runs every 5 minutes). The insight is that saga isolation is not a framework problem to be solved generically -- it is a domain modeling problem where each resource type defines its own locking semantics.
