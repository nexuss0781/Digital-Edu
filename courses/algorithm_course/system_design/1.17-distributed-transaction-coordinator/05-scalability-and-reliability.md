...
## Saga Isolation Strategies

One of the most challenging production concerns is that sagas provide **no isolation** between concurrent transactions. Two sagas operating on the same resources can read intermediate, uncommitted state.
...
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ 1. Semantic Locking (Pessimistic)                                  │
│    ──────────────────────────────                                  │
│    Mark resources with a "processing" flag during saga execution   │
│    Other sagas check flag and either wait or abort                 │
│                                                                     │
│    Example: Inventory has status field                              │
│      status = 'AVAILABLE' → can be reserved                       │
│      status = 'PROCESSING_ORDER_123' → blocked for other orders   │
│      status = 'RESERVED' → committed, can be released             │
│                                                                     │
│    Risk: If saga fails without cleanup, resource stays locked     │
│    Mitigation: TTL on lock, background reaper process             │
│                                                                     │
│ 2. Commutative Updates (CRDTs)                                     │
│    ──────────────────────────────                                  │
│    Design operations that produce correct results regardless of   │
│    execution order                                                 │
│                                                                     │
│    Example: Instead of SET balance = 100                          │
│             Use INCREMENT balance BY -50                           │
│             Order of increments doesn't matter                    │
│                                                                     │
│ 3. Version-Based Conflict Detection (Optimistic)                   │
│    ──────────────────────────────────────────                     │
│    Read version at saga start, check at each write                │
│    If version changed, abort and retry the saga                    │
│                                                                     │
│ 4. Saga-Level Read Snapshots                                       │
│    ──────────────────────────────                                  │
│    Capture resource state at saga start                            │
│    Detect drift at commit time                                    │
│    Retry or compensate if drift exceeds threshold                 │
│                                                                     │
│ Production Recommendation:                                         │
│ • Use semantic locking for high-contention resources (inventory)  │
│ • Use commutative updates where business logic permits            │
│ • Use version checks as a safety net for remaining cases          │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
| **Saga Isolation** | Semantic locking + commutative updates | Application-level control |
