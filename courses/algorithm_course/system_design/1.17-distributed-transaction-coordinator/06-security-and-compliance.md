...
## Transaction Replay Security

Durable execution frameworks and saga orchestrators that support replay introduce a unique security concern: **execution history contains a full record of every external interaction**, including sensitive data passed to participants.
...
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Risk: Execution histories contain sensitive payloads               │
│   • Payment amounts, customer IDs, inventory quantities            │
│   • Potentially PII or card data passed as activity inputs         │
│   • Historical records enable full transaction reconstruction      │
│                                                                     │
│ Mitigations:                                                        │
│                                                                     │
│ 1. Payload Encryption in Execution History                         │
│    • Encrypt activity inputs/outputs before journaling             │
│    • Use per-workflow encryption keys                               │
│    • Key rotation: rotate without re-encrypting old histories     │
│                                                                     │
│ 2. Data Retention Policies for Execution Histories                 │
│    • Completed workflows: archive and purge after 90 days          │
│    • Running workflows: retain full history                        │
│    • Compliance: align retention with SOX/PCI-DSS requirements     │
│                                                                     │
│ 3. Access Control on Replay Operations                              │
│    • Replay should require admin privileges                        │
│    • Audit log every replay invocation                             │
│    • Mask sensitive fields in debugging UIs                        │
│                                                                     │
│ 4. Deterministic Replay Integrity                                   │
│    • Hash-chain execution events to detect tampering               │
│    • Replay divergence detection: alert if replay produces         │
│      different results than original execution                     │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
| **Replay Security** | Encrypted history, access controls | Data minimization |
