...
### Trap 6: "Why not just use Temporal/durable execution for everything?"

**Bad Answer**: "Yes, durable execution solves distributed transactions completely."

**Good Answer**:
- Durable execution dramatically reduces accidental complexity (no manual state machines, idempotency)
- But it does NOT eliminate fundamental trade-offs:
  - Still need compensation logic (business-level undo)
  - Still eventual consistency (no cross-service isolation)
  - Adds journaling latency (1-5ms per activity)
  - Introduces framework dependency (vendor risk)
- Best for: complex multi-step workflows where developer productivity matters
- Not ideal for: ultra-low-latency transactions, simple 2-3 step flows where overhead isn't justified

### Trap 7: "How do you test a saga with 10 steps and 5 possible failure points?"

**Bad Answer**: "Write integration tests for each failure scenario."

**Good Answer**:
- Integration tests cover known scenarios but miss rare interleavings
- Use **deterministic simulation testing**: control all non-determinism (network, clocks, disk) and explore millions of interleavings with a fixed seed
- Property-based invariants to check: "every saga ends in COMPLETED, COMPENSATED, or FAILED -- never stuck", "no double compensations", "idempotency holds under all retry patterns"
- Reproducibility: if a test fails, the seed recreates the exact same interleaving
...
| "Durable execution removes all complexity" | Compensation + isolation remain | Distinguish accidental vs essential complexity |
