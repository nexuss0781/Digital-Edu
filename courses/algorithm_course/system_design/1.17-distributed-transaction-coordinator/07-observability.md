...
## Saga-Aware Distributed Tracing

Standard distributed tracing (OpenTelemetry) captures individual request spans but struggles with saga transactions because a single saga spans multiple asynchronous invocations over minutes or hours.

```
┌────────────────────────────────────────────────────────────────────┐
│ SAGA-AWARE TRACING EXTENSIONS                                       │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Challenge: A 5-step saga creates 5+ separate trace trees            │
│ (one per step invocation). Standard tracing cannot link them.      │
│                                                                     │
│ Solution: Saga Context Propagation                                  │
│                                                                     │
│ 1. Saga-Level Trace Link                                           │
│    • Create a root "saga span" that is NOT a parent of step spans │
│    • Instead, use OpenTelemetry "links" to connect step traces     │
│    • Saga span carries: saga_id, saga_type, start_time            │
│    • Each step span links back to the saga span                    │
│                                                                     │
│ 2. Compensation Trace Correlation                                   │
│    • Compensation spans link to both:                               │
│      - The saga root span (which saga)                             │
│      - The original step span (what is being compensated)          │
│    • Enables "show me the original action and its compensation"    │
│                                                                     │
│ 3. Cross-Service Saga View                                          │
│    • Aggregate all spans linked to a saga_id                       │
│    • Reconstruct the full saga timeline:                           │
│      Step 1 (50ms) → Step 2 (200ms) → Step 3 FAILED               │
│      → Comp 2 (100ms) → Comp 1 (50ms)                             │
│    • Show parallel vs sequential execution                         │
│                                                                     │
│ Implementation:                                                     │
│ • Propagate saga_id as a baggage item in OpenTelemetry context    │
│ • Use span events to mark state transitions                        │
│ • Create a custom trace exporter that builds saga timelines        │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
For mission-critical systems, a background reconciliation process should verify that the saga's recorded state matches the actual state across all participants.

| Check | Method | Frequency |
|-------|--------|-----------|
| **Completed saga verification** | Query each participant for order state | Every 5 minutes |
| **Stuck saga detection** | Sagas in RUNNING > timeout threshold | Every 1 minute |
| **Compensation completeness** | Verify all compensated steps were actually reversed | Every 15 minutes |
| **Orphan detection** | Resources in PENDING/PROCESSING with no active saga | Every 30 minutes |
...
| **Tracing** | End-to-end trace, span breakdown, saga-aware linking | Distributed tracing |
...
| **Reconciliation** | Participant state vs saga state | Background jobs |
