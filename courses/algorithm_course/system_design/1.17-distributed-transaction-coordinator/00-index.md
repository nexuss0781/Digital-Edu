...
| **Scaling** | Orchestrator can Slowest part of the process | Natural distribution |
...
## The Durable Execution Paradigm (2024-2026 Shift)

The most significant evolution in distributed transaction coordination is the rise of **durable execution** frameworks. Rather than requiring developers to manually design state machines, compensation logic, and idempotency layers, durable execution engines persist function call stacks and replay them on failure -- making ordinary sequential code inherently fault-tolerant.

| Aspect | Traditional Saga | Durable Execution |
|--------|-----------------|-------------------|
| **Developer Model** | Explicit state machine + compensation | Write sequential code; framework handles failures |
| **State Persistence** | Manual: save after each step | Automatic: event-sourced execution history |
| **Failure Recovery** | Re-read state, resume from last step | Replay deterministic execution from history |
| **Compensation** | Developer writes reverse operations | Developer writes reverse operations (same) |
| **Idempotency** | Developer implements per-step | Built-in via deterministic replay |
| **Debugging** | Query saga state store | Replay exact execution, inspect each step |
| **Examples** | Custom orchestrators, Seata | Temporal, DBOS, Restate |

**Key insight**: Durable execution does not eliminate the fundamental distributed transaction trade-offs (consistency vs availability, compensation complexity). It dramatically reduces the **accidental complexity** of implementing them correctly.
...
| **Temporal.io** | Durable Execution | Replay-based recovery, multi-language SDKs | General workflow orchestration |
| **Restate** | Durable Execution | Lightweight, invocation-level journaling | Low-latency microservice transactions |
| **DBOS** | Durable Execution | Database-as-OS, transactional function execution | Database-centric workflows |
...
| [1.18 Event Sourcing System](../1.18-event-sourcing-system/00-index.md) | Event-sourced saga state, audit trails |
| [1.19 CQRS Implementation](../1.19-cqrs-implementation/00-index.md) | Read/write separation for transaction status queries |
| [1.9 Consensus Protocol System](../1.9-consensus-protocol-system/00-index.md) | Coordinator election, replicated state machine |
| [2.4 CI/CD Pipeline Build System](../2.4-cicd-pipeline-build-system/00-index.md) | DAG-based workflow execution parallels saga orchestration |
