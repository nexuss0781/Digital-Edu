...
Primary Slowest part of the process: LLM Generation (83% of latency)
```
...
High cost                   Large context              Aggressive Cutting off unnecessary steps
...
## Case Study: Enterprise Knowledge Base RAG at Scale

```
┌────────────────────────────────────────────────────────────────────┐
│ CASE STUDY: ENTERPRISE KNOWLEDGE BASE RAG                          │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Context:                                                            │
│ • Enterprise with 500K internal documents (policies, procedures)   │
│ • 50K daily queries from 10K employees                             │
│ • Documents span: HR policies, technical docs, legal contracts     │
│ • Multi-language: English (80%), 4 other languages (20%)           │
│ • Access control: 200 document groups, role-based filtering        │
│                                                                     │
│ Architecture Decisions:                                             │
│                                                                     │
│ 1. Hybrid chunking by document type                                │
│    • HR policies: section-aware (header boundaries), 500 tokens    │
│    • Technical docs: hierarchical parent-child, 300/1200 tokens    │
│    • Legal contracts: clause-level, 200 tokens, zero overlap       │
│    • Result: 15% accuracy improvement over uniform chunking        │
│                                                                     │
│ 2. Tiered retrieval                                                │
│    • Stage 1: Hybrid BM25 + dense, top-100 candidates (40ms)      │
│    • Stage 2: Access control filter (removes 30-60% of results)   │
│    • Stage 3: Cross-encoder rerank, top-10 (60ms)                 │
│    • Total retrieval: ~120ms                                       │
│                                                                     │
│ 3. Query complexity routing                                        │
│    • Simple (70%): Single-shot RAG, <1.5s, cheaper model          │
│    • Complex (25%): Multi-query retrieval, <3s, standard model    │
│    • Analytical (5%): Agentic RAG, <15s, premium model            │
│    • Result: 60% cost reduction vs. always using premium model    │
│                                                                     │
│ 4. Freshness strategy                                              │
│    • Real-time: CDC on policy database (5-minute index update)    │
│    • Batch: Nightly re-ingestion of 200-500 changed documents     │
│    • Model migration: Quarterly embedding model upgrades           │
│                                                                     │
│ Results:                                                            │
│ • Answer accuracy (RAGAS faithfulness): 0.91                      │
│ • Query p50 latency: 1.1s (streaming TTFT: 180ms)                │
│ • Cost per query: $0.008 (blended across tiers)                   │
│ • Documents correctly cited: 94%                                  │
│ • Hallucination rate (unsupported claims): 3.2%                   │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
Graph RAG augments vector retrieval with entity relationship traversal, enabling multi-hop reasoning over facts scattered across documents.
...
│                                                                     │
│ Retrieval (parallel):                                              │
│   Query → Entity recognition → Graph traversal → Entity facts     │
│   Query → Embedding → Vector search → Text chunks                 │
│                                                                     │
│ Context assembly:                                                   │
│   Merge entity facts + text chunks → Deduplicate → LLM context   │
│                                                                     │
│ Best for:                                                           │
│ • Organizational knowledge (people, teams, projects)              │
│ • Regulatory compliance (linked requirements)                      │
│ • Medical knowledge (drug interactions, conditions)               │
│                                                                     │
│ Cost: LLM extraction at ingestion adds $1-5 per 1000 documents   │
│ Benefit: 25-40% accuracy improvement on relationship queries      │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
Modern RAG systems handle not just text but images, tables, and diagrams embedded in documents.
...
│                                                                     │
│ Document Ingestion:                                                │
│   Text paragraphs → Text embedding model → Text index             │
│   Tables → Table-to-text conversion → Text embedding → Index     │
│   Images/diagrams → Vision model captioning → Text embedding      │
│   Code blocks → Code-specific embedding → Code index              │
│                                                                     │
│ Key decisions:                                                     │
│ • Tables: Convert to markdown for embedding (preserves structure) │
│ • Images: Generate descriptive captions via vision model           │
│ • Diagrams: OCR + layout analysis + captioning                    │
│ • Charts: Data extraction + statistical summary                   │
│                                                                     │
│ Retrieval:                                                         │
│ • Single embedding space (all modalities as text) is simplest     │
│ • Separate indices per modality with fusion is more accurate      │
│ • Multi-modal embeddings (CLIP-style) enable cross-modal search   │
│                                                                     │
│ Challenges:                                                         │
│ • Table structure lost in flat text conversion (30% accuracy drop)│
│ • Image captions miss spatial relationships                        │
│ • Code semantics not captured by text embeddings                  │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
| 1.1 | 2026-03 | Added enterprise case study, Graph RAG, multi-modal RAG patterns |
