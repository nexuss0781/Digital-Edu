...
|----------|-------------|
| **What AI decides alone** | Query embedding generation, vector similarity retrieval, chunk ranking and reranking, context window assembly, streaming token generation |
| **What AI recommends** | Chunking strategy changes based on retrieval quality metrics, reranker model upgrades, prompt template revisions, hybrid search weight adjustments |
| **What requires human approval** | Knowledge base source additions or removals, access control policy changes per document collection, LLM model swaps, prompt template changes affecting answer tone or safety, citation policy modifications |
| **Deterministic source of truth** | Document ingestion registry (source, version, ACL), prompt template version store, evaluation benchmark results (RAGAS scores) |
| **Rollback path** | Prompt template rollback via version pointer; embedding index rebuild from source documents; LLM model rollback via gateway routing configuration |
...
| [09 - Key Insights](./09-insights.md) | 12 non-obvious architectural insights |
...
| Design | Relationship |
|--------|--------------|
| [3.14 Vector Database](../3.14-vector-database/00-index.md) | Foundation for dense retrieval -- embedding storage, ANN search, and index management |
| [3.13 LLM Training & Inference](../3.13-llm-training-inference-architecture/00-index.md) | LLM serving component -- generation latency, batching, and KV-cache optimization |
| [3.18 AI Code Assistant](../3.18-ai-code-assistant/00-index.md) | RAG applied to code -- AST-aware chunking, repository-scale retrieval |
| [3.12 Recommendation Engine](../3.12-recommendation-engine/00-index.md) | Shares two-stage retrieval + reranking architecture; embedding-based candidate generation |
| [3.17 AI Agent Orchestration Platform](../3.17-ai-agent-orchestration-platform/00-index.md) | Agentic RAG patterns -- multi-step retrieval, tool use, planning-based query decomposition |
| [16.3 Text Search Engine](../16.3-text-search-engine/00-index.md) | BM25 and sparse retrieval foundations for hybrid search |
| [3.16 Feature Store](../3.16-feature-store/00-index.md) | Feature serving patterns applicable to embedding storage and freshness management |
| [1.4 Distributed LRU Cache](../1.4-distributed-lru-cache/00-index.md) | Caching strategies applicable to embedding cache and RAGCache KV-state management |
