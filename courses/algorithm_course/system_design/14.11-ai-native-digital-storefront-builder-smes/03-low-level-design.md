...
### Algorithm 5: Channel Taxonomy Mapping

Each channel has its own product category taxonomy that differs from the platform's canonical taxonomy. A "women's silk kurta" maps to different category paths across channels.

```
FUNCTION mapToChannelTaxonomy(canonicalCategory, channel):
    // Step 1: Check direct mapping cache
    directMapping = lookupMapping(canonicalCategory, channel)
    IF directMapping AND directMapping.confidence > 0.95:
        RETURN directMapping.channelCategory

    // Step 2: Hierarchical fallback
    // Canonical: "Clothing > Women > Ethnic Wear > Kurta > Silk Kurta"
    // Try each level from most specific to least specific
    categoryParts = canonicalCategory.split(" > ")
    FOR i FROM categoryParts.length DOWNTO 1:
        partialCategory = categoryParts[0:i].join(" > ")
        mapping = lookupMapping(partialCategory, channel)
        IF mapping AND mapping.confidence > 0.80:
            RETURN mapping.channelCategory

    // Step 3: Semantic similarity matching
    // Compare canonical category embedding against channel taxonomy embeddings
    channelTaxonomy = fetchChannelTaxonomy(channel)
    canonicalEmbedding = embedCategory(canonicalCategory)

    bestMatch = NULL
    bestSimilarity = 0
    FOR channelCat IN channelTaxonomy:
        similarity = cosineSimilarity(canonicalEmbedding, channelCat.embedding)
        IF similarity > bestSimilarity:
            bestSimilarity = similarity
            bestMatch = channelCat

    IF bestSimilarity > 0.75:
        // Cache the mapping for future use
        cacheMapping(canonicalCategory, channel, bestMatch, confidence: bestSimilarity)
        RETURN bestMatch.channelCategory
    ELSE:
        // No confident mapping; use channel's "Other" category and flag for manual review
        flagForReview(canonicalCategory, channel, bestMatch, bestSimilarity)
        RETURN channel.defaultCategory


FUNCTION buildMappingCache(channel):
    // Pre-compute mappings for all canonical categories during channel onboarding
    canonicalCategories = fetchAllCanonicalCategories()  // ~500 categories
    channelTaxonomy = fetchChannelTaxonomy(channel)

    FOR canonicalCat IN canonicalCategories:
        bestMatch = semanticMatch(canonicalCat, channelTaxonomy)
        IF bestMatch.confidence > 0.80:
            cacheMapping(canonicalCat, channel, bestMatch)
        ELSE:
            // Queue for manual mapping by operations team
            queueManualMapping(canonicalCat, channel, bestMatch)
```

### Algorithm 6: Storefront SEO Metadata Generation

```
FUNCTION generateSEOMetadata(store, products):
    // Homepage SEO
    homepageMeta = {
        title: truncate(store.businessName + " — " + store.categoryLabel + " | Shop Online", 60),
        description: generateMetaDescription(store, products.topByRevenue(5)),
        canonicalUrl: store.primaryUrl,
        jsonLd: generateOrganizationSchema(store),
        ogImage: store.logoUrl OR products[0].heroImage
    }

    // Product page SEO (per product)
    FOR product IN products:
        productMeta = {
            title: truncate(product.title + " — " + store.businessName, 60),
            description: truncate(product.descriptions[store.primaryLanguage].seoMetaDescription, 160),
            canonicalUrl: store.primaryUrl + "/products/" + product.slug,
            jsonLd: generateProductSchema(product, store),
            ogImage: product.images[0].url
        }
        product.seoMetadata = productMeta

    // Category page SEO
    FOR category IN store.categories:
        categoryMeta = {
            title: truncate(category.name + " — " + store.businessName, 60),
            description: generateCategoryDescription(category, category.productCount),
            canonicalUrl: store.primaryUrl + "/collections/" + category.slug,
            jsonLd: generateCollectionSchema(category, category.products)
        }
        category.seoMetadata = categoryMeta

    // Sitemap generation
    sitemap = generateSitemap(store, products, store.categories)
    submitToSearchEngines(sitemap)

    RETURN {homepageMeta, productMetas, categoryMetas, sitemap}
...
FUNCTION generateProductSchema(product, store):
    ...
    RETURN {
        ...
        "@type": "Product",
        "name": product.title,
        "description": product.descriptions[store.primaryLanguage].description,
...
### Algorithm 7: Merchant Onboarding Intelligence

```
FUNCTION assessMerchantReadiness(merchantInput):
    // Evaluate the quality and completeness of merchant's input
    // to determine the optimal store creation path
    ...
    FOR image IN merchantInput.images:
        analysis = analyzeImageQuality(image)
        // Scores: resolution (> 800px), lighting, background cleanliness, focus
        image.qualityTier = classifyTier(analysis)
        // Tier 1: professional (>90% confidence), Tier 2: acceptable (70-90%), Tier 3: poor (<70%)

    readinessScore.imageQuality = mean(merchantInput.images.map(i => i.qualityTier))

    // Catalog completeness
    productsWithPrices = merchantInput.products.filter(p => p.price > 0).length
    productsWithAttributes = merchantInput.products.filter(p => p.attributes.nonEmpty).length
    readinessScore.catalogCompleteness = (productsWithPrices + productsWithAttributes) / (2 * merchantInput.products.length)

    // Business information completeness
    hasBusinessName = merchantInput.businessName.length > 2
    hasCategory = merchantInput.category IS NOT NULL
    hasDescription = merchantInput.description.length > 20
    hasLanguage = merchantInput.language IS NOT NULL
    readinessScore.businessInformation = (hasBusinessName + hasCategory + hasDescription + hasLanguage) / 4

    readinessScore.overall = weightedAverage(
        imageQuality: 0.4,
        catalogCompleteness: 0.3,
        businessInformation: 0.3
    )

    // Determine store creation path
    IF readinessScore.overall > 0.8:
        RETURN {path: "FAST_TRACK", estimatedTime: "90 seconds", guidance: []}
    ELSE IF readinessScore.overall > 0.5:
        guidance = generateGuidance(readinessScore)
        // e.g., "Retake product photos with better lighting for best results"
        RETURN {path: "GUIDED", estimatedTime: "3 minutes", guidance: guidance}
    ELSE:
        guidance = generateDetailedGuidance(readinessScore)
        RETURN {path: "ASSISTED", estimatedTime: "5 minutes", guidance: guidance}
```

---

## State Machine: Store Lifecycle

```
CREATING → DRAFT → PUBLISHED → ACTIVE → SUSPENDED → DELETED
                                  ↑          ↓
                                  ←───── REACTIVATED

States:
  CREATING: Store generation pipeline in progress (AI analyzing, content generating)
  DRAFT: Store generated but not yet published by merchant (preview mode)
  PUBLISHED: Store live and accessible to customers; channel sync initiated
  ACTIVE: Store has received at least one order (merchant engagement confirmed)
  SUSPENDED: Store temporarily disabled (by merchant or platform due to policy violation)
  DELETED: Store permanently removed; data retention policies apply

Transitions:
  CREATING → DRAFT: All AI pipeline stages complete; storefront rendered
  DRAFT → PUBLISHED: Merchant clicks "Go Live"; DNS provisioned; channels connected
  PUBLISHED → ACTIVE: First order placed (automated transition)
  ACTIVE → SUSPENDED: Merchant request or policy violation detected
  SUSPENDED → PUBLISHED: Merchant reactivates or policy violation resolved
  ANY → DELETED: Merchant requests deletion (after order settlement)
  CREATING → DELETED: Merchant abandons store creation (cleanup after 24 hours)
```
...
| Cache Layer | Store | Content | TTL | Invalidation |
|---|---|---|---|---|
| **CDN edge** | Storefront HTML, images, assets | Rendered pages, optimized images | 5 min (pages), 1 year (assets with content hash) | Product-to-URL dependency purge |
| **Origin shield** | Pre-rendered page cache | Same as edge, higher capacity | 15 min | Same purge path as edge |
| **Application cache** | Distributed cache cluster | Product metadata, inventory, merchant config | 30s (inventory), 5 min (product), 1 hr (config) | Event-driven invalidation |
| **Database query cache** | Database server | Query results for common patterns | Connection-level; 10s | Automatic on write to affected table |

### Cache Warming Strategy

| Scenario | Warm Strategy |
|---|---|
| **New store published** | Pre-render all pages; push to CDN; warm application cache with product data |
| **Server restart** | Lazy warming — first request populates cache; stale-while-revalidate prevents user-visible latency |
| **Shard migration** | Pre-warm new shard's local cache from migrated data before cutting traffic over |
| **Festival pre-scaling** | Pre-warm CDN edge caches with top 10% of storefronts by traffic volume |
