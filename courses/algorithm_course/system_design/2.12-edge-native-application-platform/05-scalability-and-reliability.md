...
When a single primary becomes the Slowest part of the process:
...
### Fluid Compute Scaling Model

Traditional edge scaling replicates functions to all PoPs thinly. Fluid Compute takes a different approach: **dense global compute** that places workers near data sources, handling multiple requests concurrently per worker.

```
// Fluid Compute: Single worker handles multiple requests
// Like a traditional server, but with serverless elasticity

FUNCTION fluid_compute_scaling(function_id, region):
    // Each worker handles N concurrent requests
    concurrent_requests_per_worker = 50  // vs 1 in classic isolate model

    current_rps = get_regional_rps(function_id, region)
    avg_duration_ms = get_avg_duration(function_id, region)

    // Little's Law: concurrent = throughput × latency
    concurrent_needed = current_rps * (avg_duration_ms / 1000)
    workers_needed = ceil(concurrent_needed / concurrent_requests_per_worker)

    // Place workers in regions aligned with data, not at every PoP
    data_region = get_primary_db_region(function_id)
    IF region == data_region:
        // Dense: more workers near data
        RETURN workers_needed * 1.5  // 50% headroom
    ELSE:
        // Sparse: fewer workers in distant regions
        RETURN max(1, workers_needed * 0.5)

// Cost model: Active CPU pricing — never charged for idle time
// Result: 85-95% cost reduction vs traditional per-invocation pricing
```

| Metric | Classic Isolate | Fluid Compute | Improvement |
|--------|----------------|---------------|-------------|
| Requests per worker | 1 | 50+ | 50x density |
| Cold start impact | Per-request | Amortized | ~0% cold starts |
| Cost model | Per invocation | Active CPU time | 85-95% reduction |
| Data alignment | Random PoP | Near data source | Lower DB latency |
...
### Configuration Change as a Single Point of Risk

**Lesson from 2025 incidents**: The most dangerous failure mode for edge platforms is not hardware failure but **global configuration change**. A single bad configuration deployed instantly to 300+ PoPs can take down the entire platform in seconds.

**Aegis-Style Configuration Safety:**

```
FUNCTION deploy_configuration_change(change):
    // Step 1: Validate in staging
    staging_result = apply_to_staging(change)
    IF NOT staging_result.pass:
        RETURN reject("Staging validation failed")

    // Step 2: Canary to single PoP (1% traffic)
    canary_pop = select_canary_pop()
    apply_to_pop(canary_pop, change)
    monitor_canary(canary_pop, duration=15m)

    IF canary_metrics.error_rate > baseline * 1.5:
        rollback_pop(canary_pop, change)
        RETURN reject("Canary degradation detected")

    // Step 3: Progressive rollout
    FOR phase IN [10%, 25%, 50%, 100%]:
        pops = select_pops_for_phase(phase)
        apply_to_pops(pops, change)
        monitor_phase(pops, duration=30m)

        IF phase_metrics.degraded:
            rollback_all(change)
            RETURN reject("Rollout degradation at " + phase)

    // Step 4: Lock and audit
    mark_change_complete(change)
    audit_log(change, "deployed successfully")

// Critical systems: 7-day canary period enforced
// Detection target: 60-90 seconds
// Status page update target: within 5 minutes of detection
```
