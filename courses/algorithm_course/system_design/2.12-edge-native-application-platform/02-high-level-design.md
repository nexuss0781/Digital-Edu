...
### Decision 6: Edge Runtime Model (2025+)

| Option | Cold Start | State Model | Concurrency | Best For |
|--------|------------|-------------|-------------|----------|
| **V8 Isolate (Classic)** | 5-10ms | Stateless | One request per isolate | Simple functions, middleware |
| **Fluid Compute** | 5-10ms | Stateless (shared worker) | Multiple requests per worker | High-throughput APIs |
| **Container + Actor Sidecar** | 500ms-2s (sleepable) | Actor-managed persistent state | Full concurrency | Long-running, stateful, GPU |
| **WebAssembly (WASI)** | < 1ms | Component Model (WIT interfaces) | Depends on host | Polyglot, portable, security-critical |

**Recommendation**: V8 isolates with Fluid Compute for the majority of workloads. Containers with actor sidecars for stateful services or GPU inference. Wasm for polyglot components requiring maximum portability.

### Decision 7: Compute Placement Strategy

| Option | Description | Pros | Cons | Verdict |
|--------|-------------|------|------|---------|
| **Thin Everywhere** | Replicate compute to all 300+ PoPs | Lowest latency | High replication cost, cold starts | For static/cached |
| **Dense Global** | Concentrate compute near data in 10-20 regions | Data locality, warm pools | Slightly higher latency for distant users | **Default for dynamic** |
| **Hybrid** | Static at all PoPs, dynamic at regional clusters | Best of both | Routing complexity | For mixed workloads |

**Recommendation**: Fluid Compute / Dense Global — place compute near data sources rather than replicating thinly everywhere. This reduces cold starts, improves cache hit rates, and aligns compute with database primaries.
...
| Isolate vs Container | **Isolate** for fast, **Container** for stateful/GPU | Match runtime to workload |
| Vendor-specific vs Standard | **WinterTC** common API with platform extensions | Portability + capability |
