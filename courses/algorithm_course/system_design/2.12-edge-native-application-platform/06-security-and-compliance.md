...
Wasm provides a fundamentally different security model from V8 isolates:

| Property | V8 Isolate | WebAssembly (WASI) |
|----------|------------|---------------------|
| Memory model | Shared heap with bounds checking | Linear memory, no access outside bounds |
| System calls | Blocked by runtime policy | Capability-based (explicitly granted) |
| File system | No access | Only if host grants capability |
| Network | fetch() only | Only if host grants capability |
| Code injection | Prevented by CSP + sandbox | Impossible (no eval, no dynamic code loading) |
| Module size | JavaScript bundle | Compiled binary (smaller, faster) |

**WASI Capability Model:**

```
// Host grants specific capabilities to Wasm module
// Module CANNOT access anything not explicitly granted
...
capabilities = {
    ...
    "wasi:keyvalue/store": {
        namespace: "tenant_123",  // Scoped to tenant
        max_keys: 10000
    }
    // No filesystem, no raw sockets, no environment variables
}

module = instantiate_wasm("app.wasm", capabilities)
// Module can only use granted capabilities — deny by default
```

**Advantage for Multi-Tenant**: Wasm's capability-based security provides stronger tenant isolation than V8 isolates because access to every external resource must be explicitly granted by the host. A compromised module cannot access resources outside its capability set, even through language-level exploits.

---

## Supply Chain Security for Edge Deployments

Edge platforms face unique supply chain risks because code is deployed globally and instantly:

| Threat | Risk | Mitigation |
|--------|------|------------|
| **Compromised dependency** | Malicious code runs at every PoP | Lockfile pinning, SRI hashes, dependency audit |
| **Build pipeline compromise** | Injected code in build artifacts | Signed builds, reproducible builds, SBOM |
| **Credential leakage** | API keys in deployed bundles | Secret scanning, env-only secrets, runtime injection |
| **Typosquatting** | Wrong package installed | Allow-list registries, Wasm component verification |

```
// Pre-deployment security checks (in CI/CD pipeline)
FUNCTION security_gate(deployment):
    // 1. Dependency audit
    vulnerabilities = scan_dependencies(deployment.lockfile)
    IF vulnerabilities.critical.count > 0:
        RETURN block("Critical vulnerabilities found")

    // 2. Secret scanning
    secrets = scan_for_secrets(deployment.bundle)
    IF secrets.count > 0:
        RETURN block("Secrets detected in bundle")

    // 3. SBOM generation
    sbom = generate_sbom(deployment)
    store_sbom(deployment.id, sbom)

    // 4. Bundle size limits (prevents supply chain bloat)
    IF deployment.bundle.size > max_bundle_size:
        RETURN block("Bundle exceeds size limit")

    // 5. Sign deployment artifact
    signature = sign_artifact(deployment.bundle, platform_key)
    deployment.signature = signature

    RETURN allow(deployment)
```
