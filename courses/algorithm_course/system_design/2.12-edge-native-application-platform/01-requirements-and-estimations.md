...
| **Container Workloads** | Edge containers with actor sidecars for stateful services | P2 |
| **Edge AI Inference** | Sub-3B parameter SLM inference at edge for personalization | P2 |
| **Runtime Portability** | WinterTC-compliant APIs for multi-platform deployment | P1 |
...
| Long-running processes | Edge functions have timeout limits (50ms-300s); use containers for longer |
| Full RDBMS features | No stored procedures, triggers, or complex joins at edge |
| Large model inference | Models > 3B parameters exceed edge memory; use origin or GPU containers |
...
|----------|-------|-----------|
| CPU Time | 50ms (edge isolate), 300s (edge function), 30min (container) | Fair sharing by runtime tier |
| Memory | 128 MB (isolate), 2 GB (container) | Edge location constraints |
| Request Body | 1 MB (isolate), 100 MB (container) | Prevents abuse |
| Response Body | 10 MB | CDN caching limits |
| Subrequests | 50 (isolate), 1000 (container) | Prevents cascading failures |
| Wasm Module Size | 10 MB compiled | Cold start + memory budget |
