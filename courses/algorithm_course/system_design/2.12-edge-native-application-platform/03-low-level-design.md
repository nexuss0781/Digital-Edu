...
```
// Middleware function signature (Step-by-step plan in plain English)
...
### Algorithm 7: Actor-Sidecar Container Lifecycle Management

```
// The actor (Durable Object) manages the container lifecycle
// State lifecycle is decoupled from container lifecycle

CLASS ContainerSidecar:
    state = SLEEPING  // SLEEPING, STARTING, RUNNING, DRAINING
    container_id = null
    persistent_storage = DurableObjectStorage()
    last_activity = now()
    idle_timeout = 300s  // Sleep after 5 min idle

    FUNCTION handle_request(request):
        // Wake container if sleeping
        IF state == SLEEPING OR state == null:
            state = STARTING
            container_id = provision_container(self.config)
            wait_for_healthy(container_id, timeout=30s)
            state = RUNNING

        // Track activity for sleep management
        last_activity = now()

        // Proxy request to container
        response = forward_to_container(container_id, request)

        // Persist any state changes from container
        IF response.headers["X-State-Update"]:
            persistent_storage.put(response.state_key, response.state_value)

        RETURN response

    FUNCTION manage_lifecycle():
        // Called periodically by the runtime
        IF state == RUNNING AND (now() - last_activity) > idle_timeout:
            // Drain and sleep
            state = DRAINING
            drain_connections(container_id, timeout=30s)
            stop_container(container_id)
            state = SLEEPING
            container_id = null
            // State survives in persistent_storage

    FUNCTION get_state(key):
        // State is always available, even when container is sleeping
        RETURN persistent_storage.get(key)
```

### Algorithm 8: D1 Sessions API for Sequential Consistency

```
// The Sessions API ensures sequential consistency across read replicas
// Each session tracks a "bookmark" — the last-seen write position

CLASS D1Session:
    bookmark = null  // Opaque position token

    FUNCTION query(sql, params):
        // Send bookmark with query to ensure reads see prior writes
        response = d1.execute({
            sql: sql,
            params: params,
        ...
        // The replica will wait until it has replicated past this position
        IF response.bookmark:
            self.bookmark = response.bookmark
    ...
    FUNCTION write(sql, params):
        ...
        // Bookmark ensures subsequent reads see this write
        self.bookmark = response.bookmark
        RETURN response.results

// Usage pattern:
session = D1Session()
session.write("INSERT INTO posts (title) VALUES (?)", ["Hello"])
// Subsequent read is guaranteed to see the insert
posts = session.query("SELECT * FROM posts ORDER BY id DESC LIMIT 10")
```
