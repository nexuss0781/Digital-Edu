...
### Deep Dive 4: Actor Model for Stateful Edge Computing

#### Why This Is Critical

Edge computing has historically been stateless — isolates process a request and die. But real applications need coordination: rate limiters, collaborative sessions, WebSocket hubs, and container orchestration. The actor model provides strongly consistent, globally addressable state without a separate database.

#### How It Works Internally

Each actor (Durable Object) is:
- **Globally unique**: One instance per ID across the entire network
- **Single-threaded**: No concurrent access, no locks needed
- **Location-pinned**: Runs in a single PoP (typically near first accessor)
- **Persistent**: Built-in storage survives restarts and migrations
- **Addressable**: Any edge function worldwide can message it by ID

```mermaid
flowchart TB
    subgraph Edge["Edge Functions (Stateless)"]
        F1["Function @ US-West"]
        F2["Function @ EU-West"]
        F3["Function @ APAC"]
    end

    subgraph Actors["Durable Actors (Stateful)"]
        A1["Room:doc_123<br/>@ US-East<br/>(single-threaded)"]
        A2["Counter:global_rate<br/>@ EU-West<br/>(single-threaded)"]
        A3["Container:svc_456<br/>@ US-West<br/>(manages lifecycle)"]
    end

    F1 -->|"WebSocket relay"| A1
    F2 -->|"WebSocket relay"| A1
    F3 -->|"Increment counter"| A2
    F1 -->|"Proxy request"| A3

    A3 -->|"Forward"| Container["Container Instance"]
...
**Actor Addressing and Routing:**

```
FUNCTION route_to_actor(actor_type, actor_id):
    // Compute unique global identifier
    global_id = hash(actor_type + ":" + actor_id)

    // Determine which PoP hosts this actor
    // (consistent hashing across available PoPs)
    hosting_pop = actor_registry.get_location(global_id)

    IF hosting_pop == null:
        // First access: create actor in nearest PoP to caller
        hosting_pop = current_pop
        actor_registry.register(global_id, hosting_pop)

    IF hosting_pop == current_pop:
        RETURN local_actor_runtime.get_or_create(global_id)
    ELSE:
        // Forward request to hosting PoP
        RETURN remote_call(hosting_pop, global_id, request)
```

**Use Cases for Actor-Model at Edge:**

| Pattern | Actor Per | State | Example |
|---------|-----------|-------|---------|
| **Collaborative editing** | Document ID | CRDT state, cursors | Real-time doc editing |
| **Rate limiting** | API key | Request count, window | Global rate limiter |
| **WebSocket hub** | Room/channel | Connected clients | Chat, live notifications |
| **Container sidecar** | Service instance | Lifecycle state, config | Stateful edge services |
| **Leader election** | Resource group | Current leader, epoch | Coordination |

#### Failure Modes

| Failure | Impact | Mitigation |
|---------|--------|------------|
| Actor PoP goes down | Actor unavailable until migrated | Auto-migration to nearby PoP, persistent state survives |
| Actor overloaded | Requests queue (single-threaded) | Shard actors by sub-ID, load-aware routing |
| Network partition | Cross-PoP messages fail | Queue messages, replay on reconnect |
| Storage full | Actor can't persist | Alert, archive old state, capacity limits |

---

## Case Study: Edge Platform Incident Patterns (2025)

### Incident 1: Bot Management Feature Breaks Global Routing

**What happened**: A bug in the bot management feature's file generation logic caused a global outage affecting ~20% of web pages and one-third of the top 10K websites. Impact included major consumer platforms going offline.

**Root cause**: Configuration change to a bot management file was deployed globally without adequate canary testing. The generated file contained a logic error that caused the request routing layer to reject legitimate traffic.

**Lesson**: Edge platforms are uniquely dangerous for configuration bugs because deployment is global and instant. Unlike traditional deployments where a bad change affects one region, an edge configuration change can take down the entire internet-facing surface simultaneously.

**Design implication**: Mandatory canary deployments for all configuration changes. The platform now enforces 7-day canary periods for critical systems, automated testing coverage at 92%, and detection within 60-90 seconds.

### Incident 2: Control Plane Dependency Cascades to Edge

**What happened**: An upstream provider outage cascaded through a feature flag provider into the edge platform's control plane (dashboard, APIs, builds, logs). The control plane was unavailable for several hours.

**Key insight**: Production traffic serving was largely unaffected because it runs on the edge, independent of the control plane. This validated the architectural decision to separate data plane (edge runtime) from control plane (management).

**Design implication**: Edge platforms must maintain **static stability** — existing deployments continue working even when the control plane is entirely down. The edge runtime should have zero runtime dependencies on the control plane. New deployments can wait; serving existing traffic cannot.
...
### Slowest part of the process 1: Write Amplification in Global Replication
...
### Slowest part of the process 2: Cold Start for Edge Functions
...
### Slowest part of the process 3: Database Connection Overhead
...
### Slowest part of the process 4: Replication Lag Causing Primary Fallback
