...
## Configuration Change Observability (Aegis Pattern)

Given that global configuration changes are the highest-risk operation for edge platforms, dedicated observability for config changes is essential:

### Config Change Tracking Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `config_change_deployments_total` | Counter | change_type, author | Config changes deployed |
| `config_change_rollbacks_total` | Counter | change_type, reason | Config rollbacks triggered |
| `config_change_canary_duration_seconds` | Histogram | change_type | Time in canary before promotion |
| `config_change_blast_radius_pops` | Gauge | change_id | Number of PoPs affected |
| `config_change_detection_latency_seconds` | Histogram | severity | Time to detect config-related degradation |

### Config Change Correlation

```
// When anomaly detected, automatically check for recent config changes
FUNCTION correlate_anomaly_with_config_changes(anomaly):
    // Look for config changes in the 30 minutes before anomaly
    window_start = anomaly.detected_at - 30m
    recent_changes = config_audit_log.query(
        time_range=(window_start, anomaly.detected_at),
        status="deployed"
    )

    IF recent_changes.count > 0:
        // High correlation: anomaly started after a config change
        FOR change IN recent_changes:
            time_delta = anomaly.detected_at - change.deployed_at
            IF time_delta < 5m:
                // Strong correlation — flag as probable cause
                anomaly.probable_cause = change
                anomaly.recommended_action = "rollback_config:" + change.id

    RETURN anomaly

// Auto-rollback for critical metrics
FUNCTION auto_rollback_check():
    FOR change IN active_canary_changes:
        metrics = get_metrics_since(change.deployed_at)

        IF metrics.error_rate > change.baseline_error_rate * 2:
            trigger_rollback(change)
            alert("Auto-rollback triggered for config change " + change.id)
```

---

## Edge AI Inference Observability

For platforms supporting edge AI inference (SLM models at edge):

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `edge_ai_inference_total` | Counter | model_id, pop | Total inference requests |
| `edge_ai_inference_duration_seconds` | Histogram | model_id, pop | Inference latency |
| `edge_ai_tokens_per_second` | Gauge | model_id, pop | Throughput |
| `edge_ai_model_load_seconds` | Histogram | model_id | Model loading time |
| `edge_ai_memory_bytes` | Gauge | model_id, pop | Model memory footprint |
| `edge_ai_cache_hit_ratio` | Gauge | model_id, pop | KV cache / prefix cache hit rate |
