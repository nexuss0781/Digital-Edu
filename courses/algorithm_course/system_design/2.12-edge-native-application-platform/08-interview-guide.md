...
4. **Primary Slowest part of the process**: Single writer limits throughput
...
Modern interviews increasingly probe awareness of the latest edge computing developments:

### Container-at-Edge Pattern

**"When would you use containers instead of isolates at the edge?"**

```
Containers make sense when you need:
1. Long-running processes (WebSocket servers, queue consumers)
2. GPU access (ML inference, image processing)
3. Existing containerized services (migration path)
4. Large memory footprint (> 128MB)

Key innovation: Actor-sidecar pattern
- A Durable Object manages container lifecycle (start, stop, sleep)
- State persists in the actor even when the container is stopped
- Decouples state lifecycle from compute lifecycle
- Container sleeps after idle timeout, wakes on next request
```

### Fluid Compute vs Thin-Everywhere

**"Should you deploy functions to every edge PoP?"**

```
No — "thin everywhere" is often suboptimal:

Thin Everywhere (Classic):
- Function replicated to 300+ PoPs
- Many PoPs are cold (low traffic → cold starts)
- Far from data (cross-region DB calls)

Dense Global (Fluid Compute):
- Workers concentrated in 10-20 regions near data
- Each worker handles multiple concurrent requests
- Better cache locality, fewer cold starts
- 85-95% cost reduction via active CPU pricing

Recommendation: Edge middleware and static at all PoPs,
dynamic compute concentrated near data sources.
```

### WebAssembly at Edge

**"What role does WebAssembly play in edge computing?"**

```
Wasm is becoming the third edge runtime alongside isolates and containers:

Advantages:
- Sub-1ms cold starts (vs 5-10ms for V8 isolates)
- Language-agnostic (Rust, Go, Python, JS all compile to Wasm)
- Capability-based security (stronger isolation than V8)
- WinterTC + WASI: standardized interfaces for portability

Current status (2025-2026):
- WASI 0.3.0 adds concurrency and zero-copy streaming
- Wasm Component Model enables composable microservices
- Major CDN provider acquired leading Wasm framework (Fermyon)
- Production use for event-driven, security-critical workloads
```
...
| "Why not deploy containers everywhere instead of isolates?" | Runtime trade-off awareness | "Containers have 500ms+ cold starts vs 5ms for isolates. We use isolates for request-response workloads, containers with actor sidecars for stateful/long-running services." |
| "How do you avoid vendor lock-in on edge platforms?" | Standards awareness | "WinterTC (Ecma TC55) defines a minimum common API. We write to that standard and use platform-specific extensions sparingly." |
...
5. **Actor lifecycle**: Durable Object sidecar managing container sleep/wake
6. **Config canary**: Progressive rollout with auto-rollback on anomaly
...
- Containers at edge without understanding the actor-sidecar pattern
- No config change safety (canary, rollback) for global deployments
- Ignoring WinterTC / runtime portability
