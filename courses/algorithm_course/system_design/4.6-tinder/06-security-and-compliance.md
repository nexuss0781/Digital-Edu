...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                     FAKE PROFILE DETECTION (2025+)                       │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Prevention (Layered):                                                  │
│  • Face Check: Mandatory video selfie liveness verification (2025)     │
│    - Liveness detection (anti-spoofing, anti-deepfake)                 │
│    - Face-to-profile-photo matching (embedding distance)               │
│    - Cross-account duplicate detection (ban evasion)                   │
│    - Privacy: video deleted after processing, non-reversible vectors   │
│  • Phone number verification (SMS OTP)                                 │
│  • Social login linkage (Google, Apple)                                │
│  • Rate limit new accounts per device fingerprint                      │
│                                                                         │
│  Detection (Real-Time + Batch):                                        │
│  • ML model trained on known fake/scam profiles                        │
│  • Reverse image search for stolen photos                              │
│  • Behavioral analysis (swipe patterns, message patterns)              │
│  • NLP analysis of bio for copy-paste scam templates                   │
│  • User reports aggregation with severity scoring                      │
│  • "Pig butchering" scam detection (crypto/investment mentions)        │
│                                                                         │
│  Signals of fake profile:                                               │
│  • Too-perfect photos (stock images, AI-generated faces)               │
│  • Generic bio copied from web or generated by LLM                     │
│  • Unusual swipe patterns (all right swipes, no selectivity)           │
│  • Immediate ask to move to external platform (WhatsApp, Telegram)     │
│  • Multiple reports from different users                               │
│  • Account creation from VPN/proxy IP addresses                        │
│  • Face Check failure or refusal to complete verification              │
│                                                                         │
│  Response:                                                              │
│  • Shadow ban (user doesn't know they're limited)                      │
│  • Manual review queue with ML pre-screening for urgency               │
│  • Permanent ban with face vector retained to block re-registration    │
│                                                                         │
│  Results (2025):                                                        │
│  • 60%+ reduction in exposure to bad actors                            │
...
#### 4. Bot Detection & AI-Generated Content
...
• Face Check liveness verification (blocks automated accounts)

Response:
• Temporary throttling
• CAPTCHA challenges
• Shadow ban (reduce visibility)
• Account termination with face vector ban
```

#### 5. AI-Generated Content & Deepfake Detection (2025+)
...
│                                                                         │
│  Photo Analysis:                                                        │
│  • GAN/diffusion model artifact detection (frequency analysis)         │
│  • Inconsistent lighting/shadow detection across face regions          │
│  • Metadata analysis (AI tools leave fingerprints in EXIF)             │
│  • Cross-reference against known AI face databases                     │
│  • Face Check liveness as primary defense (AI photos fail liveness)   │
│                                                                         │
│  Bio/Text Analysis:                                                     │
│  • LLM-generated text detection (perplexity analysis, watermarks)     │
│  • Template matching against known scam bio patterns                   │
│  • Inconsistency detection (bio details vs photo context)              │
│                                                                         │
│  Deepfake Video Detection (Face Check specific):                        │
│  • Temporal consistency analysis across video frames                   │
│  • Micro-expression authenticity verification                          │
│  • Eye reflection analysis (deepfakes produce inconsistent reflections)│
│  • Audio-visual synchronization (if voice notes used)                  │
│                                                                         │
│  Message Content:                                                       │
│  • Romance scam pattern detection (escalation to financial requests)   │
│  • External link scanning for phishing                                 │
│  • "Pig butchering" keyword and behavioral pattern matching            │
│  • Automated response detection (identical messages across matches)    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
