...
| Slowest part of the process | Detection | Solution |
|------------|-----------|----------|
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                     MODERN TINDER INTERVIEW TOPICS                       │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  If the interviewer asks about AI/ML in dating:                        │
│  • Chemistry: multimodal AI fusing questionnaire + camera roll +       │
│    behavioral signals to predict meaningful interaction probability    │
│  • Key shift: from engagement optimization to conversation quality     │
│  • On-device inference for privacy (camera roll never uploaded)        │
│  • Cold-start: questionnaire provides immediate signal for new users  │
│                                                                         │
│  If the interviewer asks about trust/safety:                           │
│  • Face Check: mandatory video selfie liveness verification            │
│  • Three properties: liveness + photo consistency + uniqueness         │
│  • Privacy-preserving: video deleted, only face vectors retained      │
│  • 60% bad-actor reduction, network trust effects                     │
│  • AI-generated content detection (GAN/diffusion artifacts)           │
│                                                                         │
│  If the interviewer asks about API gateway at scale:                   │
│  • TAG: decentralized gateway (per-team instances, shared framework)  │
│  • Configuration-driven (YAML), not code-driven                       │
│  • Serves multiple dating brands across 190+ countries                │
│  • Boot-time config processing eliminates runtime overhead            │
│                                                                         │
│  If the interviewer asks about geospatial evolution:                   │
│  • Adaptive container-based geoshard balancing (not fixed levels)     │
│  • Dynamic load scoring: active_users × α + query_rate × β           │
│  • S2 vs H3 comparison: S2 for sharding, H3 for proximity search     │
│  • ~3 shards queried per recommendation (down from many more)        │
│                                                                         │
│  If the interviewer asks about competitive landscape:                  │
│  • Bumble: AI-first rewrite (Bee assistant, Q2 2026 tech stack)      │
│  • Hinge: deep learning mutual compatibility, Gale-Shapley algorithm │
│  • Industry trend: swipe fatigue → intent-aware matching              │
│  • Shift from digital-only to hybrid online/offline experiences       │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|-----------------|----------------|----------------------|
| Design WhatsApp | 1:1 chat focus | WebSocket, message delivery, E2E encryption |
| Design Facebook | Social graph, not geo | ML recommendations, feed ranking |
| Design Uber | Location tracking | Geospatial indexing (H3 vs S2), real-time dispatch |
| Design Netflix | Video streaming | Recommendation engine, two-tower models |
| Design Twitter | Public posts | Fan-out patterns, real-time notifications |
| Design Identity Verification | KYC/biometric focus | Face Check patterns, liveness detection, privacy-preserving biometrics |
