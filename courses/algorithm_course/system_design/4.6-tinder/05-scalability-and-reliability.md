...
### Container Orchestration (2025 Infrastructure)
...
│                                                                         │
│  Scale:                                                                 │
│  • 200+ services deployed as containers                                │
│  • 1,000 nodes across multiple regions                                 │
│  • 15,000 pods running at any time                                     │
│  • 48,000 running containers                                           │
│                                                                         │
│  Benefits vs Previous EC2 Deployment:                                   │
│  • Scheduling: Traffic serving in seconds (vs minutes on bare EC2)    │
│  • Density: Multiple services share nodes, improving utilization       │
│  • Self-healing: Automatic pod restart on failure                      │
│  • Rolling updates: Zero-downtime deployments per service              │
│  • Resource isolation: CPU/memory limits per service                   │
│                                                                         │
│  Service Distribution by Language:                                      │
│  • Node.js: API-facing services (async I/O for high concurrency)      │
│  • Java/Scala: Backend services (TAG gateway, data processing)        │
│  • Go: Performance-critical services (geoshard routing, match workers)│
│  • 30+ source code repositories                                       │
│                                                                         │
│  Deployment Strategy:                                                   │
│  • Canary deployments (5% → 25% → 100%)                               │
│  • Automatic rollback on error rate spike                              │
│  • Blue-green for critical services (match detection, swipe)          │
│  • Feature flags for gradual feature rollout                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### Adaptive Geoshard Container Strategy

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     ADAPTIVE GEOSHARD LOAD BALANCING                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Traditional (Static):                                                  │
│  • Fixed S2 level for all geoshards                                    │
│  • Manhattan and rural Montana get same-size cells                     │
│  • Result: 100x load imbalance between shards                         │
│                                                                         │
│  Adaptive (Container-Based):                                            │
│  • Each S2 cell gets a dynamic load score:                             │
│    load_score = (active_users × α) + (query_rate × β)                 │
│  • Adjacent cells grouped into "containers" until capacity threshold   │
│  • Dense areas: Level 8 cells (~22.5 miles) → many small containers   │
│  • Sparse areas: Level 7 cells (~45 miles) → few large containers     │
│  • Each container = one Elasticsearch geoshard                         │
│                                                                         │
│  Auto-rebalancing:                                                      │
│  • Hourly evaluation of container load scores                          │
│  • Split: container exceeds 2x target load → subdivide                │
│  • Merge: adjacent containers both below 0.3x target → combine        │
│  • Reindex users to new shards via background job                     │
│                                                                         │
│  Result:                                                                │
│  • ~3 shards queried on average per recommendation request            │
│  • 12-56% reduction in p99 search latency                             │
│  • $1M+ annual infrastructure cost savings                            │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| Event-driven spikes | Super Bowl, New Year's Eve | Pre-scale based on historical data, Kafka buffering |
