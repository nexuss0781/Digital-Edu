...
### AI & Trust Services (2025+)

| Service | Responsibility | Notes |
|---------|----------------|-------|
| **Chemistry Service** | Multimodal intent-aware matching | Fuses questionnaire, camera roll, behavioral signals |
| **Face Check Service** | Liveness verification, deepfake detection | Mandatory onboarding step, cross-account detection |
| **Content Moderation Service** | AI photo/text/message scanning | Hybrid ML + human review pipeline |
| **Scam Detection Service** | Romance scam and fraud pattern detection | NLP for "pig butchering", financial request patterns |
...
| Auth endpoints | 5/min | Per IP | Brute Force (Checking every single possibility) prevention |
