...
│                                                                         │
│  5. If Redis is Slowest part of the process:                                            │
...
| Chemistry | Read | Read | Read | Consume |
| Face Check | Write | - | - | Produce |

---

## ML Model & Trust Monitoring (2025+)

### Chemistry & Recommendation Model Metrics
...
│                                                                         │
│  Chemistry Model:                                                       │
│  • Prediction latency (p50/p99): target <50ms                         │
│  • Feature coverage: % users with all 3 signal towers populated       │
│  • Cold-start ratio: % users with questionnaire-only (no behavioral)  │
│  • A/B test lift: conversation rate (Chemistry vs TinVec-only)        │
│  • Model drift: weekly distribution shift in Chemistry scores         │
│                                                                         │
│  TinVec Embedding Pipeline:                                             │
│  • Embedding freshness: age of oldest active user embedding           │
│  • Embedding coverage: % active users with current-day embeddings     │
│  • ANN recall@100: quality of approximate nearest neighbor search     │
│  • Training pipeline health: daily job completion, loss convergence   │
│                                                                         │
│  Recommendation Quality:                                                │
│  • Swipe-through rate: swipes / profiles shown (engagement)           │
│  • Match-to-conversation rate: conversations started / matches        │
│  • Exploration effectiveness: match rate on exploration candidates     │
│  • Fairness metrics: visibility distribution across user segments     │
│  • Time-to-first-match for new users (cold-start quality)             │
│                                                                         │
│  Face Check Pipeline:                                                   │
│  • Verification completion rate: % users completing Face Check        │
│  • Liveness pass rate: should be >95% for legitimate users            │
│  • Deepfake detection rate: true positive rate on synthetic media     │
│  • Cross-account duplicate catch rate                                  │
...
| Alert | Condition | Severity | Response |
|-------|-----------|----------|----------|
| **Face Check pipeline down** | Verification success rate = 0 for 5 min | P1 | Block new registrations, page on-call |
| **Deepfake detection model degraded** | False negative rate > 5% | P2 | Increase manual review ratio |
| **Scam report spike** | Report rate > 3x normal for 1 hour | P2 | Activate enhanced NLP scanning |
| **Chemistry model latency** | p99 > 200ms for 10 min | P3 | Fallback to TinVec-only ranking |
| **Embedding pipeline stale** | Oldest embedding > 48 hours | P3 | Investigate training pipeline failure |
