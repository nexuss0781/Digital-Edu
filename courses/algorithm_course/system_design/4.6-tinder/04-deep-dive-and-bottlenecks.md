...
### Slowest part of the process 1: Hot Geoshards

**Problem:**
- Dense urban areas (NYC, LA, London) have 10-100x more users than rural areas
- Causes uneven load distribution
- Single geoshard becomes Slowest part of the process
...
### Slowest part of the process 2: Swipe Velocity Spikes
...
### Slowest part of the process 3: Match Notification Storms
...
### Slowest part of the process 4: Photo CDN Origin Load

**Problem:**
- 50 GB/s peak bandwidth for photo delivery
- CDN cache misses hit origin
- Origin can become Slowest part of the process for new/updated photos
...
## Critical Component 4: Chemistry -- Multimodal Intent-Aware Matching (2025+)

### Why This Is Critical

Traditional swipe-based recommendations optimize for engagement metrics (swipe-right rate, session length), but high match rates do not correlate with meaningful conversations or real-world dates. Chemistry shifts the optimization target to "probability of meaningful interaction," requiring a fundamentally different ML architecture.

### Architecture Deep Dive

```mermaid
flowchart TB
    subgraph SignalCollection["Signal Collection"]
        Questionnaire["Personality Questionnaire<br/>(Values, Humor, Goals)"]
        CameraRoll["On-Device Camera Analysis<br/>(Lifestyle, Activities, Social)"]
        Behavioral["Behavioral Signals<br/>(Dwell Time, Revisits, Response)"]
    end

    subgraph FeatureEngineering["Feature Engineering"]
        ExplicitTower["Explicit Signal Tower<br/>(Questionnaire Embeddings)"]
        ContextualTower["Contextual Signal Tower<br/>(Visual Lifestyle Vectors)"]
        ImplicitTower["Implicit Signal Tower<br/>(Behavioral Patterns)"]
    end

    subgraph FusionLayer["Multimodal Fusion"]
        Transformer["Cross-Attention Transformer<br/>(Signal Fusion)"]
        ChemistryScore["Chemistry Score<br/>(Interaction Probability)"]
    end

    subgraph Serving["Serving Layer"]
        TinVec["TinVec Score<br/>(Swipe Prediction)"]
        Combined["Combined Ranking<br/>(Chemistry × TinVec)"]
        SwipeQueue["Final Swipe Queue"]
    end

    Questionnaire --> ExplicitTower
    CameraRoll --> ContextualTower
    Behavioral --> ImplicitTower

    ExplicitTower --> Transformer
    ContextualTower --> Transformer
    ImplicitTower --> Transformer

    Transformer --> ChemistryScore
    ChemistryScore --> Combined
    TinVec --> Combined
    Combined --> SwipeQueue
    ...
    class Questionnaire,CameraRoll,Behavioral signal
    class ExplicitTower,ContextualTower,ImplicitTower feature
    class Transformer,ChemistryScore fusion
    class TinVec,Combined,SwipeQueue serve
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                     CHEMISTRY SIGNAL TAXONOMY                           │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  EXPLICIT SIGNALS (from questionnaire):                                 │
│  • Relationship goals (casual, serious, exploratory)                   │
│  • Communication style (text-heavy, phone calls, in-person)            │
│  • Values alignment (family, career, adventure, creativity)            │
│  • Humor style (dry wit, slapstick, wordplay)                          │
│  • Social energy (introvert, extrovert, ambivert)                      │
│                                                                         │
│  CONTEXTUAL SIGNALS (from on-device camera roll analysis):              │
│  • Travel frequency (airport/landmark detection)                       │
│  • Outdoor activity level (hiking, beach, skiing photos)               │
│  • Social group size (solo vs group photos)                            │
│  • Pet ownership (pet detection in photos)                              │
│  • Food/dining preferences (restaurant vs home cooking)                │
│  NOTE: Analysis runs on-device; only lifestyle vectors uploaded,       │
│  never raw photos. Requires explicit user consent.                     │
│                                                                         │
│  IMPLICIT SIGNALS (from in-app behavior):                               │
│  • Profile dwell time (seconds spent on each profile)                  │
│  • Profile revisit frequency (returning to view same profile)          │
│  • Messaging response time (hours between messages)                    │
│  • Conversation depth (message length, question ratio)                 │
│  • Emoji/GIF usage patterns (emotional expression style)               │
│  • Session timing (morning browser vs evening swiper)                  │
│                                                                         │
│  PRIVACY ARCHITECTURE:                                                  │
│  • Camera roll analysis: on-device only, Apple/Android ML frameworks   │
│  • Behavioral signals: aggregated, not individual interactions         │
│  • All signals are opt-in with granular consent toggles                │
│  • Users can view and delete their Chemistry profile anytime           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|---------|-----------|----------|
| On-device inference latency | Camera roll analysis on older phones | Batch processing during charging, progressive enrichment |
| Chemistry score freshness | User preferences evolve over time | Re-compute weekly, invalidate on major preference changes |
| Cold-start problem | New users have no behavioral signals | Questionnaire provides immediate signal; contextual signals from first photos |
| A/B testing complexity | Chemistry vs TinVec-only ranking | Shadow scoring: compute both, serve one, measure conversation quality difference |
| Model size for mobile | On-device model must be small | Distilled model (<50MB) with quantization, updated via OTA |

---

## Critical Component 5: Face Check Verification Pipeline (2025+)

### Why This Is Critical

Dating platforms have a unique trust requirement: users make real-world safety decisions based on digital profiles. Fake profiles, catfishing, and romance scams erode trust and drive user churn. Face Check provides mandatory identity verification with deepfake detection.
...
flowchart TB
    subgraph Capture["Video Capture"]
        Selfie["User Records<br/>Video Selfie (3-5 sec)"]
    end

    subgraph Pipeline["Verification Pipeline"]
        Liveness["Liveness Detection<br/>(Anti-Spoofing)"]
        FaceExtract["Face Embedding<br/>Extraction"]
        PhotoMatch["Profile Photo<br/>Matching"]
        DeepfakeDetect["Deepfake<br/>Detection"]
        CrossAccount["Cross-Account<br/>Duplicate Check"]
    end

    subgraph Storage["Privacy-Preserving Storage"]
        FaceVector["Non-Reversible<br/>Face Vector"]
        FaceMap["Encrypted<br/>Face Map"]
        DeleteVideo["Delete Video<br/>Selfie"]
    end

    subgraph Outcome["Verification Outcome"]
    ...
    Selfie --> Liveness
    Liveness -->|Pass| FaceExtract
    Liveness -->|Fail| Retry

    FaceExtract --> PhotoMatch
    FaceExtract --> DeepfakeDetect
    FaceExtract --> CrossAccount

    PhotoMatch -->|Match| FaceVector
    PhotoMatch -->|No Match| Retry
    DeepfakeDetect -->|Clean| FaceVector
    DeepfakeDetect -->|Suspicious| Flagged
    CrossAccount -->|Unique| FaceVector
    CrossAccount -->|Duplicate| Flagged

    FaceVector --> FaceMap
    FaceMap --> DeleteVideo
    DeleteVideo --> Verified
    ...
    class Selfie capture
    class Liveness,FaceExtract,PhotoMatch,DeepfakeDetect,CrossAccount pipeline
    class FaceVector,FaceMap,DeleteVideo storage
    class Verified,Retry,Flagged outcome
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                     FACE CHECK VERIFICATION PROPERTIES                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  PROPERTY 1: LIVENESS                                                   │
│  • Confirms person is physically present (not a photo/video replay)    │
│  • Analyzes micro-expressions, eye blinks, head movement               │
│  • Detects 2D photos held up to camera                                 │
│  • Detects screen replays (moire pattern detection)                    │
│  • 3D depth analysis where hardware supports it                        │
│                                                                         │
│  PROPERTY 2: PHOTO CONSISTENCY                                          │
│  • Extracts face embedding from video selfie                           │
│  • Compares against embeddings from all profile photos                 │
│  • Cosine similarity threshold: >0.85 for pass                        │
│  • Handles: lighting variation, aging (within reason), makeup          │
│  • Rejects: different person, heavy filters that alter face shape      │
│                                                                         │
│  PROPERTY 3: UNIQUENESS                                                 │
│  • Face vector compared against database of banned account vectors     │
│  • Detects same face used across multiple accounts (ban evasion)       │
│  • ANN search across millions of vectors in <100ms                     │
│  • False positive rate: <0.001% (aggressive threshold for safety)      │
│                                                                         │
│  PRIVACY GUARANTEES:                                                    │
│  • Video selfie deleted immediately after processing                   │
│  • Only non-reversible encrypted face map retained                     │
│  • Face vector: sufficient for matching, not reconstruction            │
│  • GDPR Article 9 compliant (biometric data with explicit consent)    │
│  • User can request face data deletion at any time                    │
│                                                                         │
│  RESULTS (2025 rollout):                                                │
│  • 60%+ reduction in bad-actor exposure                                │
│  • 40%+ decline in user reports of suspicious behavior                 │
│  • Verified users receive visibility boost in recommendations          │
│  • Network effect: verified users attract more verified users          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
