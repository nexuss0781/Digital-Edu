...
| LLM incident analysis | 3s | 8s | 12s | <15s |
| Incident query | 20ms | 100ms | 500ms | <1s |
| Runbook execution | 5s | 30s | 120s | <300s |

---

## Case Study: Alert Storm During Cascading Failure

### Scenario

A regional network partition caused cascading failures across 200+ microservices. The AIOps platform received 47,000 alerts in 3 minutes — a 500x spike over normal volume.
...
T0: Network partition isolates 3 availability zones
T0-T30s: Health checks fail across 200 services
T30s-T60s: Retry storms amplify failure, dependent services start failing
T60s-T180s: 47,000 alerts generated across all monitoring tools

AIOps Pipeline Response:

T0-T5s: Deduplication reduces 47,000 alerts to 12,000 unique alerts
T5s-T10s: Cooldown suppression removes repeat alerts (-3,000)
T10s-T20s: Dynamic-X-Y correlation clusters remaining 9,000 alerts into 8 incidents
  - Incident 1: Network connectivity (3 AZ partition) — 31,000 raw alerts
  - Incident 2: Database replication lag — 8,000 raw alerts
  - Incident 3: Queue consumer backlog — 4,000 raw alerts
  - Incidents 4-8: Individual service cascading failures — 4,000 raw alerts
T20s-T30s: RCA identifies network partition as root cause via:
  - Timing: network alerts preceded all others by 15 seconds
  - Topology: all affected services are downstream of partitioned network segment
  - Change correlation: no recent deployments or config changes (rules out change-caused)
T30s: 3 pages sent (one per primary incident) instead of 47,000 alerts

WITHOUT AIOps: On-call receives 47,000 alerts across 5 monitoring tools
```

### Key Metrics

| Metric | With AIOps | Without AIOps |
|--------|-----------|---------------|
| Alerts received by on-call | 3 incident pages | 47,000 raw alerts |
| Time to identify root cause | 30 seconds | 15-45 minutes (manual triage) |
| MTTR | 8 minutes (automated remediation) | 45 minutes (manual) |
| Alert suppression rate | 99.99% | 0% |
| False incident rate | 0/8 incidents | N/A |

---

## AI Release Ladder

Every AI-driven capability in AIOps (anomaly detectors, correlation models, RCA engines, auto-remediation) must progress through these stages before reaching full autonomy.

| Stage | Gate Criteria | Rollback Trigger | Typical Duration |
|-------|--------------|------------------|------------------|
| **1. Offline Evaluation** | Precision/recall on labeled incident dataset >= baseline; no RCA misattributions on known incidents | N/A -- not processing live alerts | 1-2 weeks |
| **2. Shadow Mode** | Shadow detections agree with production rule-based alerts >= 90%; no missed P0 incidents | Shadow divergence > 15% from rule-based baseline for 2 hours | 2-4 weeks |
| **3. Canary (5% traffic)** | False positive rate < 5% on canary stream; no missed critical alerts; MTTR improvement measurable | False positive rate > 10% or any missed P0 incident | 2-3 weeks |
| **4. Human-Reviewed Production** | AI generates incident + RCA + suggested runbook; SRE approves/rejects; approval rate > 85% | Rejection rate > 25% over rolling 24h window | 3-6 weeks |
| **5. Autonomous Production** | Full auto-correlation, auto-RCA, auto-remediation with async audit; MTTR meets SLO | Any SLO breach, false positive spike > 2x baseline, or auto-remediation causes incident | Ongoing |
| **6. Instant Rollback** | One-click revert to rule-based detection; all in-flight incidents preserved | Triggered manually or by meta-monitoring SLO breach | < 60 seconds |

> **AIOps specifics:** Stage 4 is critical and often extended -- SREs must build trust in RCA accuracy before auto-remediation is enabled. The shadow mode comparison against legacy rule-based alerts is the key safety gate.
