...
|-----------|---------|
| **Tier** | **B -- AI-Augmented** |
| **What AI decides autonomously** | Alert deduplication and noise suppression, anomaly detection threshold tuning, alert correlation and incident grouping, topology graph updates from telemetry, metric baseline recalculation |
| **What AI recommends (human approves)** | Root cause identification and ranking, runbook selection for remediation, incident severity classification, alert routing to on-call teams, model retraining triggers |
| **What requires human judgment** | Automated remediation execution on production systems, runbook creation and modification, escalation policy changes, cross-team incident coordination, suppression rule changes for critical alerts |
| **Escalation trigger** | False positive rate > 10% for 1 hour, RCA confidence score < 70%, auto-remediation failure, MTTR exceeding SLO, anomaly detection accuracy drop > 5% |

> **Rationale:** AIOps operates on critical production infrastructure. While noise suppression and correlation are safe to automate, remediation actions that modify production systems (restarts, rollbacks, scaling) require human approval until trust is established through the release ladder.
...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, API design, core algorithms (Step-by-step plan in plain English) |
...
- [3.39 AI-Native Proactive Observability Platform](../3.39-ai-native-proactive-observability-platform/00-index.md) - Proactive anomaly prediction, observability pipelines
- [3.2 ML Models Deployment System](../3.2-ml-models-deployment-system/00-index.md) - Model serving for anomaly detection, blue-green deployment
- [3.15 RAG System](../3.15-rag-system/00-index.md) - RAG for incident knowledge retrieval, runbook search
- [3.21 LLM Gateway & Prompt Management](../3.21-llm-gateway-prompt-management/00-index.md) - LLM routing for incident analysis, prompt versioning
- [1.18 Event Sourcing System](../1.18-event-sourcing-system/00-index.md) - Event-driven architecture, incident timeline reconstruction
- [2.6 Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) - Scheduled remediation, periodic model retraining
- [3.16 Feature Store](../3.16-feature-store/00-index.md) - Feature computation for anomaly detection models
- [1.15 Content Delivery Network](../1.15-content-delivery-network-cdn/00-index.md) - Edge-based metric collection, distributed telemetry
...
- IBM Watson AIOps - 90+ tool integration, NLP for log analysis, knowledge graphs
- Apache SkyWalking AIOps Engine - Open-source APM with pluggable AIOps
- Salesforce PyRCA - Open-source root cause analysis library with causal inference
- Dynamic Alert Suppression for AIOps (ACM ICSE) - Multi-dimensional alert correlation
- A Survey of AIOps in the Era of LLMs (ACM Computing Surveys) - Comprehensive 2025 survey

---

> **Vendor Freshness Notice:** This document references specific vendor products (Datadog Watchdog, IBM Watson AIOps, Splunk ITSI, New Relic Applied Intelligence, Apache SkyWalking, Salesforce PyRCA). The AIOps vendor landscape evolves rapidly. Verify vendor capabilities, pricing, and integration compatibility against official documentation before making architectural decisions. Last reviewed: 2026-03.
