...
## 5. Follow-Up Probes for Strong Candidates

| Probe | What It Tests | Strong Answer Includes |
|---|---|---|
| "A flu pandemic 10x your video traffic overnight. Walk through the first 30 minutes of your SFU scaling response." | Operational maturity under pressure | Predictive scaling from appointment forecast, reactive auto-scaling at 70% threshold, cascading SFU recruitment from cold-standby pool, graceful degradation to audio-only if SFU capacity exhausted, real-time MOS monitoring to verify quality during scale-out |
| "A provider's license gets revoked mid-consultation. How does your system respond?" | Real-time authorization and safety | Credential verification cache invalidation, in-session notification to provider with grace period, session transfer to backup provider rather than abrupt termination, consent and clinical note preservation, audit trail of the handoff |
| "Your consent engine has a bug that returned 'authorized' for 48 hours when it should have returned 'denied' for a subset of patients. Walk through your response." | Breach response and incident management | HIPAA breach risk assessment (4-factor test), audit log forensics to identify all affected accesses, patient notification if breach is substantiated, root cause analysis of consent cache invalidation bug, regulatory filing if > 500 patients affected |
| "Two patients in different countries need a consultation with the same specialist. How do you handle data residency?" | Multi-region compliance awareness | Per-jurisdiction PHI storage rules, session data stays in patient's region while specialist receives a view-only encrypted stream, cascade SFU routes media without storing in specialist's region, consent includes cross-border data sharing agreement |

---

## 6. Red Flags

| Candidate Behavior | Why It's Concerning |
|---|---|
| Treats HIPAA as "just add encryption" | Misses segmentation, audit trails, minimum necessary, consent — encryption is one of many controls |
| Designs scheduling without conflict prevention | Doesn't recognize double-booking as a patient safety issue, not just a UX bug |
| Uses MCU for video without discussing E2EE implications | Doesn't understand that MCU decodes media, breaking encryption and creating a PHI exposure surface |
| Stores all data in one database "for simplicity" | Ignores heterogeneous access patterns (relational, time-series, binary, append-only) and blast radius of a single breach |
| Ignores no-show problem entirely | Misses the largest source of provider capacity waste (15-25% baseline) and the prediction/overbooking opportunity |
| Suggests P2P WebRTC without SFU | Doesn't account for session recording, multi-party consults, or server-side quality management |

---

## 7. Cross-System Connections to Demonstrate

Mentioning these cross-connections during an interview signals principal-level breadth:

| Connection | How to Mention |
|---|---|
| "Our SFU architecture mirrors Discord's relay system — both face the same N-squared fanout problem at different scales" | Shows awareness of WebRTC patterns beyond healthcare |
| "The no-show prediction model is structurally similar to e-commerce cart abandonment prediction — same features (lead time, engagement, history), different domain consequence" | Connects healthcare-specific problem to general ML patterns |
| "Our PHI segmentation is analogous to PCI DSS cardholder data isolation — separate encryption scopes limiting breach blast radius" | Shows cross-domain security architecture knowledge |
| "The event-driven audit trail uses the same append-only, hash-chained architecture as financial ledgers and blockchain consensus logs" | Links compliance requirement to general distributed systems pattern |
