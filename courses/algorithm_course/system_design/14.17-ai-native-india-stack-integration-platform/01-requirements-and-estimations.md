...
| **API Gateway Latency** | < 50ms overhead (p99) for request routing, auth, rate limiting | Gateway must not become a Slowest part of the process for latency-sensitive DPI calls |
...
|------------|-----------|
| 70% of workflows are loan origination (most complex) | Digital lending is the primary use case driving India Stack adoption |
| Average user has accounts at 2-3 FIPs | Multi-FIP consent requires parallel fetches with per-FIP timeout management |
| 85% of eKYC succeed on first OTP attempt | UIDAI reports ~85% first-attempt success; balance require retry or offline fallback |
| AA data fetch succeeds 92% within 30 seconds | Weighted average across FIPs; large banks ~96%, small banks ~78% |
| Consent approval takes 30 seconds to 4 hours | Depends on user's AA app readiness; in-branch is fast, digital-first is async |
| 5% of workflows are flagged for fraud review | Based on industry patterns for digital lending in India |
| Each tenant processes 20K--50K workflows/month | Mid-market NBFC/fintech; large banks may process 500K+/month |
| Feature extraction extracts 200+ features in < 2 seconds | CPU-intensive; requires 16-core machines; scales linearly with transaction count |
| Fair Use templates constrain consent scope to purpose-specific parameters | Since June 2025; prevents over-broad consent requests |
