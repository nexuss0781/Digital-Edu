...
|-------------|------------|-----------|-----------|
| **Consent state** | Strong (serializable) | Database transaction with row-level lock on consent record | Consent transitions must be atomic; cannot be in ACTIVE and REVOKED simultaneously |
| **Identity graph updates** | Eventual (1-second convergence) | Async event from DPI adapter on new identifier discovery | Identity links are additive; eventual consistency acceptable |
| **Workflow state** | Strong (serializable) | Database transaction with optimistic lock on workflow instance | Workflow step transitions must be atomic; crash recovery depends on accurate checkpoint |
| **Audit log writes** | Strong (durable, append-only) | Synchronous write to replicated, append-only store | Regulatory requirement: every event must be durably recorded before proceeding |
| **Feature store writes** | Eventual (5-second convergence) | Async write after feature extraction completes | Feature store is read by scoring service; brief delay acceptable |
| **Credit score publication** | Eventual (webhook-based) | Async notification to tenant after scoring completes | Business client receives score via webhook; retry on failure |
| **DPI quota counters** | Eventual (shared counter with periodic sync) | Distributed counter with per-node local count; synced every 1 second | Slight over-counting acceptable (better to rate-limit slightly early than exceed DPI quota) |
...
flowchart LR
    ...
    WE -->|workflow.step_completed / failed| EB
    AA -->|aa.data_fetched / consent_callback| EB
    KYC -->|ekyc.verified / failed| EB
    CM -->|consent.approved / revoked / expired| EB
    CS -->|score.computed / fraud_flagged| EB
    ...
    class WE,AA,KYC,CM,CS service
    class EB queue
    class FD,FEX,NOTIFY,BILLING,AUDIT,ANALYTICS data
```
...
|-------|-----------|---------------|---------|
| `ekyc.verified` | eKYC Adapter | Identity Graph, Workflow Engine, Fraud Detection | Successful Aadhaar authentication |
| `consent.approved` | Consent Manager | Workflow Engine (resume), AA Adapter (initiate fetch) | User approves AA consent on AA app |
| `consent.revoked` | Consent Manager | Data Lifecycle Manager (delete raw data), Tenant Notification | User revokes consent or DataLife expires |
| `aa.data_fetched` | AA Adapter | Feature Extraction, Fraud Detection | FIP returns encrypted financial data |
| `features.extracted` | Feature Extraction | Credit Scorer, Feature Store | 200+ features computed from AA data |
| `score.computed` | Credit Scorer | Workflow Engine (proceed to next step), Tenant Webhook | Credit score and risk assessment completed |
| `fraud.flagged` | Fraud Detection | Workflow Engine (pause for review), Tenant Alert | Cross-DPI fraud signal detected |
| `workflow.completed` | Workflow Engine | Billing, Analytics, Tenant Webhook | End-to-end workflow finished successfully |
| `document.verified` | Document Verification | Workflow Engine, Identity Graph | DigiLocker document validated by AI |
...
|-------------|-----------|----------|---------|
| UIDAI (Aadhaar Auth/eKYC) | Outbound | XML over HTTPS; RSA-2048 encryption | Identity verification via OTP or biometric |
| Account Aggregators (3+) | Outbound | REST (ReBIT spec); curve25519 + AES-256 | Consent management and financial data fetch |
| FIPs (via AAs) | Indirect | Via AA protocol | Source of financial data (banks, MFs, insurance) |
| DigiLocker (NeGD) | Outbound | REST with OAuth 2.0 | Verified document fetch (PAN, GST, Udyam) |
| eSign Providers (CDAC, licensed ESPs) | Outbound | REST with PKI cert signing | Aadhaar-based electronic signatures |
| NPCI (UPI) | Outbound | UPI protocol via sponsoring bank | Real-time payment disbursement and collection |
| Tenant business systems | Inbound/Outbound | REST API + Webhooks | Workflow initiation, score delivery, event notifications |
| HSM / Key Management | Internal | PKCS#11 | Cryptographic key generation and management |
| SMS/Email gateway | Outbound | HTTPS | OTP delivery, consent notifications, workflow status |

---

## Sandbox and Testing Architecture

| Environment | Purpose | DPI Behavior |
|-------------|---------|--------------|
| **Sandbox** | Tenant integration testing | Mock DPI responses; configurable latency/failure injection; no real Aadhaar/AA calls |
| **UAT** | Pre-production validation | UIDAI sandbox environment; AA test AAs; DigiLocker test issuer; eSign test ESP |
| **Production** | Live traffic | Real DPI calls; all compliance controls active; audit logging enabled |

```
Sandbox Mock Behavior:
  - eKYC: Returns synthetic KYC data for test Aadhaar numbers (999999990019)
  - AA: Simulates consent approval callback after 3-second delay
  - AA Data: Returns synthetic bank statement data (configurable scenarios)
  - DigiLocker: Returns sample PDF documents with test digital signatures
  - eSign: Simulates signing flow without real OTP
  - UPI: Returns success/failure based on configurable test VPAs

Failure Injection:
  - Configurable per DPI: timeout, error code, partial response
  - Latency injection: add configurable delay to simulate slow FIPs
  - Rate limit simulation: trigger 429 responses after N requests
  - Consent rejection: simulate user rejecting consent at AA level
```
...
| Concern | Approach |
|---------|----------|
| **Data isolation** | Logical isolation via `tenant_id` on all tables; row-level security in consent store, feature store, and identity graph |
| **DPI quota allocation** | Per-tenant configurable share of platform's DPI API quota; prevents noisy-neighbor exhaustion of shared quotas |
| **Configuration isolation** | Per-tenant: workflow templates, consent scope templates, fraud thresholds, fallback policies, webhook endpoints |
| **Compute isolation** | Shared compute for API gateway and adapters; dedicated GPU slices for high-volume tenants (credit scoring) |
| **Billing** | Per-API-call metering: per-eKYC, per-AA-fetch, per-eSign, per-score; volume discounts configurable |

---

## Data Lifecycle Management

The platform manages data with different retention policies per regulatory body:
...
flowchart TB
    subgraph Ingestion["Data Ingestion"]
        EKYC_DATA[eKYC Response]
        AA_DATA[AA FIData]
        DOC_DATA[DigiLocker Documents]
    end
    ...
    subgraph Storage["Retention Tiers"]
        RAW["Raw DPI Data<br/>Retained: Consent DataLife<br/>Deletion: On revocation/expiry"]
        FEATURES["Derived Features<br/>Retained: DataLife parameter<br/>Deletion: At DataLife expiry"]
        SCORES["Credit Scores<br/>Retained: 7 years<br/>Status: Read-only after revocation"]
        CONSENT["Consent Artefacts<br/>Retained: 7 years (RBI)<br/>Never deleted during retention"]
        AUDIT["Audit Log<br/>Retained: 7 years<br/>Never deleted"]
    end
    ...
    class EKYC_DATA,AA_DATA,DOC_DATA ingest
    class FE,CS,FD process
    class RAW,FEATURES,SCORES,CONSENT,AUDIT storage
```

| Data Category | Retention Policy | Deletion Trigger | Regulatory Authority |
|---------------|-----------------|------------------|---------------------|
| **Raw Aadhaar data** | Delete immediately after eKYC purpose fulfilled | Session end | UIDAI (Aadhaar Act) |
| **Raw AA FIData** | Consent DataLife parameter (typically 3-12 months) | Consent revocation or DataLife expiry | RBI (AA Framework) |
| **DigiLocker documents** | Per consent; typically session-scoped | Workflow completion | MeitY |
| **Derived features** | Consent DataLife | DataLife expiry (even if consent still active) | RBI + DPDP Act |
| **Credit scores** | 7 years (lending regulation) | Marked read-only on consent revocation; deleted at 7-year mark | RBI |
| **Consent artefacts** | 7 years | Never during retention period | RBI |
| **Audit log entries** | 7 years | Never during retention period | All regulators |
| **Identity graph edges** | Until user requests deletion | DPDP Act deletion request | DPDP Act |
