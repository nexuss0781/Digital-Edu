...
| **Time-to-Decision** | End-to-end latency from workflow start to credit assessment result | < 60 seconds (p50) | > 120 seconds p50 (workflow Slowest part of the process) |
...
## DPI Weather Service Dashboard

### Predictive Health Monitoring

| Panel | Visualization | Data Source |
|-------|---------------|-------------|
| DPI health forecast (next 4 hours) | Color-coded timeline per DPI | Historical pattern analysis + current metrics |
| FIP latency heatmap (all FIPs × time of day) | Heatmap with p95 latency | Per-FIP latency histograms over 30-day window |
| Predicted degradation windows | Calendar view with highlighted risk periods | Monthly/seasonal pattern detection |
| Current vs. predicted success rate per DPI | Dual line chart (actual vs. forecast) | Real-time metrics + EWMA prediction |
| Workflow initiation advisory | Traffic light (green/yellow/red) per workflow type | Composite DPI health affecting each workflow type |
| AA routing recommendation | Table showing recommended AA per FIP with reasoning | Multi-AA performance comparison |

### Consent Lifecycle Monitoring

| Panel | Visualization | Data Source |
|-------|---------------|-------------|
| Active consents by state (PENDING/ACTIVE/PAUSED) | Stacked bar chart by tenant | Consent store aggregation |
| Consent approval rate (last 24 hours) | Gauge per tenant | Consent events |
| Consent-to-fetch conversion time | Histogram | Workflow step timestamps |
| Expired consents pending data cleanup | Counter with aging breakdown | Consent store + feature store TTL scan |
| Consent revocation cascade completion rate | Progress indicator | Data lifecycle manager events |
