...
| DPDP Requirement | Implementation | Data Scope |
|-----------------|----------------|------------|
| **Consent before processing** | Every DPI data access gated by explicit consent; consent artefact records purpose and scope | All personal data |
| **Purpose limitation** | Data used only for declared purpose in consent; Fair Use templates enforce scope | AA data, eKYC data |
| **Data minimization** | Request minimum scope per Fair Use template; extract features, delete raw data | AA FIData |
| **Storage limitation** | TTL-based deletion aligned with consent DataLife; automated purge on expiry | All DPI-sourced data |
| **Right to access** | API for data principals to view what data platform holds about them | Identity graph, consent records, features |
| **Right to correction** | Corrections flow through source DPI (platform reflects corrected data on next fetch) | Identity data |
| **Right to erasure** | Deletion cascade: raw data → features → mark scores read-only → retain audit trail | All except regulatory audit records |
| **Data breach notification** | Automated detection triggers 72-hour notification pipeline to Data Protection Board | All personal data |
| **Consent manager registration** | Platform registered as Consent Manager where applicable; compliance dashboard for Data Fiduciary obligations | Consent operations |
| **Cross-border transfer** | Data processed within India; no cross-border transfer without explicit consent and adequacy determination | All personal data |
