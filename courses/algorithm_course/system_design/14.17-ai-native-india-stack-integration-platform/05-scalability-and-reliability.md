...
## DPI-Specific Resilience Patterns

### Multi-AA Routing for FIP Coverage

Different AAs have partnerships with different FIPs. The platform maintains a routing table that maps each FIP to its best-performing AA:

```
AA Routing Strategy:

FIP Routing Table (per-FIP):
  - FIP: Bank X
    Primary AA: AA1 (avg latency: 3.2s, success rate: 96.1%)
    Secondary AA: AA2 (avg latency: 4.8s, success rate: 93.5%)
    Notes: AA1 has direct connectivity; AA2 routes through intermediary

  - FIP: Cooperative Bank Y
    Primary AA: AA3 (avg latency: 45s, success rate: 78.2%)
    Secondary AA: none (only AA3 has connectivity)
    Notes: Extend timeout to 90s; accept partial data

Routing Logic:
  1. For each FIP in consent, select primary AA from routing table
  2. If primary AA circuit breaker is OPEN, use secondary AA
  3. If no AA available for FIP, mark FIP as UNAVAILABLE in workflow
  4. If >50% of FIPs unavailable, notify user and suggest retry later
  5. Update routing table metrics continuously (EWMA of latency and success)
```

### Consent-Aware Rate Limiting

The platform must respect both its own API quotas with DPI providers and the per-consent frequency limits:

```
Rate Limiting Layers:

Layer 1: Platform-level DPI quota
  - UIDAI: 50,000 auth requests/hour (platform-wide allocation)
  - AA providers: per-AA negotiated quota (varies by AA)
  - DigiLocker: 10,000 document fetches/hour
  - Distributed counter with 1-second sync interval

Layer 2: Per-tenant allocation
  - Each tenant gets a configurable share of platform quota
  - Premium tenants: guaranteed 20% of platform quota
  - Standard tenants: shared remaining 80% with fair queuing
  - Burst allowance: 2x allocated rate for 5 minutes

Layer 3: Per-consent frequency limit
  - Consent artefact specifies Frequency: {unit: MONTH, value: 1}
  - Platform enforces: max 1 data fetch per month per consent
  - Tracked in consent store; rejected at consent validation layer

Layer 4: Per-user velocity limit (fraud prevention)
  - Max 3 eKYC attempts per Aadhaar per 24 hours
  - Max 5 AA consent requests per user per 24 hours
  - Prevents consent stuffing and identity verification abuse
```

### Idempotent DPI Interactions

Each DPI adapter implements idempotency to handle retries safely:

| DPI | Idempotency Mechanism | Key |
|-----|----------------------|-----|
| **eKYC** | Session-based; OTP valid for 10 minutes; same session retries return cached result | Session token |
| **AA Consent** | Consent artefact ID is generated by platform; duplicate creation returns existing consent | Consent artefact ID |
| **AA Data Fetch** | Fetch session ID per consent; duplicate fetch returns cached data within session | Session ID + consent ID |
| **DigiLocker** | Document URI + access token; duplicate fetch returns same document | Document URI |
| **eSign** | Document hash + signer ID; duplicate signing returns existing signature | Document hash |
| **UPI** | Transaction reference ID; NPCI guarantees idempotent processing | Transaction reference |
