...
| Edge Case (Unusual or extreme situation) | Challenge | Resolution |
|---|---|---|
...
## Cross-Cutting Slowest part of the process Analysis

### Slowest part of the process 1: Consent Approval Wait Time
...
### Slowest part of the process 2: Encryption Key Management at Scale
...
### Slowest part of the process 3: Multi-Tenant DPI Quota Contention
...
### Slowest part of the process 4: Audit Log Write Throughput
...
### Race Condition 1: Consent Revocation During Active Data Fetch

**Scenario**: User revokes AA consent from their AA app while the platform is mid-way through fetching FIData from 3 FIPs. FIP 1 has already returned data; FIP 2 and FIP 3 are still pending.

```
FUNCTION handle_concurrent_revocation(fetch_session, revocation_event):
    // Revocation callback arrives from AA

    // 1. Stop pending FIP fetches
    cancel_pending_fip_requests(fetch_session.session_id)

    // 2. Handle already-received data
    FOR fip_response IN fetch_session.completed_responses:
        IF fip_response.decrypted:
            // Data was already decrypted and processed
            // Mark for immediate deletion
            schedule_immediate_deletion(fip_response.data_id)

        IF fip_response.features_extracted:
            // Features were already computed
            // Check DataLife: if expired, delete; otherwise schedule
            IF consent.data_life_expired():
                delete_features(fip_response.feature_set_id)
            ELSE:
                schedule_deletion_at_data_life_expiry(fip_response.feature_set_id)

    // 3. If credit score was already computed, mark as source_revoked
    IF fetch_session.score_computed:
        mark_score_source_revoked(fetch_session.score_id)

    // 4. Update workflow state
    workflow.mark_step_cancelled("aa_data_fetch", reason: "consent_revoked")

    // 5. Notify business client
    webhook.send(tenant, "consent.revoked", { workflow_id, impact: "data_fetch_cancelled" })
```

### Race Condition 2: Duplicate Consent Callbacks

**Scenario**: AA sends consent approval callback twice (network retry). The platform processes both, potentially triggering two parallel data fetches for the same consent.

**Mitigation**: Idempotent callback processing using consent artefact ID as dedup key. First callback transitions consent to ACTIVE and initiates fetch. Second callback finds consent already ACTIVE and returns existing fetch session.

### Race Condition 3: Workflow Timeout and Late DPI Response

**Scenario**: Workflow step times out after 60 seconds waiting for FIP response. Platform marks step as FAILED. FIP response arrives 5 seconds later.

**Mitigation**: Late response handler checks workflow step state. If step is already FAILED but response is valid, store data with `late_response` flag. On workflow retry, use the cached late response instead of re-fetching (saving a DPI call and reducing user friction).

---

## Data Quality Challenges

### FIP Data Quality Variance

| Quality Issue | Frequency | Detection | Handling |
|--------------|-----------|-----------|----------|
| **Stale data** (balance from yesterday, not today) | 2-5% of FIP responses | Check response timestamp vs. current time; flag if > 24 hours stale | Accept for scoring with staleness penalty; flag in credit report |
| **Incomplete transaction history** | 3-8% (varies by FIP) | Check if transaction date range covers requested period | Adjust feature extraction to handle gaps; note coverage % in features |
| **Duplicate transactions** | 1-2% | Transaction ID deduplication during parsing | Deduplicate before feature extraction; log for FIP quality monitoring |
| **Inconsistent account naming** | 10-15% | Same account appears with different names across FIPs | Fuzzy matching on account number + branch code; ignore name for matching |
| **Missing FIP from consent** | 5-10% | FIP listed in consent but returns "account not found" | Score with available FIPs; flag as partial coverage; adjust confidence |
