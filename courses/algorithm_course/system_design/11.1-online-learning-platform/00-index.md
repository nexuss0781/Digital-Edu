...
|-----------|--------|-------|
| **Domain Complexity** | ★★★★☆ | Content graph dependencies, assessment integrity, DRM licensing, credential verification |
| **Scale Challenge** | ★★★★★ | 5M+ concurrent video streams at 15 Tbps; CDN is the primary scaling lever |
| **Latency Sensitivity** | ★★★☆☆ | Video TTFB < 2s; progress sync < 1s; assessments tolerate seconds |
| **Data Model Complexity** | ★★★★☆ | Event-sourced progress, hierarchical content DAG, polyglot persistence |
| **Operational Complexity** | ★★★★☆ | Multi-CDN orchestration, multi-DRM packaging, transcoding pipeline |
| **Interview Frequency** | ★★★★☆ | Very common — spans video streaming, content management, and e-learning |
...
| Term | Definition |
|------|-----------|
| **ABR** | Adaptive Bitrate — client-side algorithm that selects video quality based on network conditions and buffer level |
| **CMAF** | Common Media Application Format — single segment format serving both HLS (Apple) and DASH (Google/Microsoft), reducing storage by ~40% |
| **HLS / DASH** | HTTP Live Streaming / Dynamic Adaptive Streaming over HTTP — the two dominant adaptive streaming protocols |
| **DRM** | Digital Rights Management — encryption preventing unauthorized content redistribution (Widevine, FairPlay, PlayReady) |
| **IRT** | Item Response Theory — statistical framework for adaptive question selection based on estimated learner ability |
| **Open Badges** | W3C standard for portable, verifiable digital credentials with embedded metadata and cryptographic signatures |
...
|--------|-------------|----------------|
| [5.6 — Google Photos](../5.6-google-photos/00-index.md) | **Media pipeline** — upload, transcode, CDN-deliver at scale | Multi-resolution encoding, content-addressed storage, thumbnail generation |
| [12.8 — WebRTC Infrastructure](../12.8-webrtc-infrastructure/00-index.md) | **Live sessions** — real-time video for cohort-based learning | Signaling, media relay, adaptive quality under bandwidth pressure |
| [6.14 — Customer Support Platform](../6.14-customer-support-platform/00-index.md) | **Ticketing + assessment** — structured workflows with SLA tracking | Queue-based assignment, quality scoring, escalation paths |
| [9.9 — CRM System](../9.9-crm-system-design/00-index.md) | **Learner lifecycle** — enrollment, engagement, retention analytics | User lifecycle events, engagement scoring, churn prediction |
| [11.4 — Email Delivery System](../11.4-email-delivery-system/00-index.md) | **Notification pipeline** — deadline reminders, certificate delivery | Multi-channel delivery, template rendering, delivery tracking |
| [14.2 — AI Conversational Commerce](../14.2-ai-native-conversational-commerce-platform/00-index.md) | **AI-powered personalization** — recommendation and adaptive paths | LLM integration with guardrails, context-aware suggestions |
