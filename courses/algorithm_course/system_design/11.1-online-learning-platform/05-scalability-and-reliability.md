...
```
Degradation levels (applied progressively under load):

Level 0 — NOMINAL:
  Full functionality. All features operational at target SLOs.

Level 1 — ELEVATED (CDN hit ratio drops below 90%):
  - Disable video preview auto-play on browse pages
  - Reduce recommendation computation frequency (6h → 12h)
  - Defer non-critical analytics pipeline jobs

Level 2 — HIGH (progress event consumer lag > 30s):
  - Batch progress events at 15-second intervals instead of 5
  - Disable real-time engagement heatmaps
  - Defer certificate generation to off-peak queue

Level 3 — CRITICAL (video playback failures > 1%):
  - Serve lower-quality default bitrate (720p max)
  - Disable offline download initiation (existing downloads unaffected)
  - Shed non-essential API endpoints (recommendations, analytics)
  - Auto-redirect to secondary CDN for affected regions

Level 4 — EMERGENCY (core services degraded):
  - Read-only mode for catalog (no new enrollments)
  - Progress events buffered client-side for later sync
  - Display "service degraded" banner with estimated recovery
```
