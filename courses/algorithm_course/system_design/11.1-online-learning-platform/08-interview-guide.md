...
### 6.1 Why Event Sourcing for Progress (Not CRUD)

| Approach | Write Pattern | Recovery | Cross-Device Sync | Verdict |
|----------|-------------|---------|-------------------|---------|
| CRUD (UPDATE progress SET pct=65%) | Mutable; last writer wins | Lost if write fails mid-flight | Conflicts on concurrent updates | Rejected — data loss risk too high |
| **Event Sourcing** | **Append-only; immutable** | **Replay events to rebuild state** | **Merge events from all devices** | **Chosen — financial-grade durability** |

### 6.2 Why Multi-CDN (Not Single Provider)

| Factor | Single CDN | Multi-CDN |
|--------|-----------|-----------|
| Reliability | Single point of failure | Health-based failover in seconds |
| Cost | No pricing leverage | Providers compete; 15-25% savings |
| Performance | Best-effort global coverage | Route to best-performing CDN per region |
| Vendor lock-in | High | Low — can shift traffic dynamically |

### 6.3 Why CMAF for Video Packaging

| Without CMAF | With CMAF |
|-------------|-----------|
| Separate HLS segments + DASH segments | Single segment format serves both |
| 2x storage cost for video renditions | ~40% storage reduction |
| Separate DRM packaging per format | CENC-based common encryption |
| Two transcoding outputs per rendition | One output, two manifest formats |

### 6.4 Whiteboard Sketch Guide

```
Essential diagram for a 45-minute interview:

  ┌──────────┐          ┌─────────┐        ┌──────────────────────┐
  │ Learner  │ ──CDN──→ │  Multi- │ ←──── │ Transcoding Pipeline │
  │ (Web/App)│          │  CDN    │        │ (Upload → Encode →   │
  └────┬─────┘          │  Edge   │        │  DRM → Distribute)   │
       │                └─────────┘        └──────────────────────┘
       │ API                                        ↑
       ↓                                     ┌──────┴──────┐
  ...
  │    ↓              ↓           ↓         │
  │ ┌────────┐  ┌──────────┐  ┌─────────┐  │
  │ │Course  │  │Progress  │  │Assess-  │  │
  │ │Service │  │Service   │  │ment     │  │
  │ │        │  │(event    │  │Engine   │  │
  │ │        │  │ sourced) │  │         │  │
  │ └────────┘  └──────────┘  └─────────┘  │
  │ ┌────────┐  ┌──────────┐  ┌─────────┐  │
  │ │Recom-  │  │Credential│  │Search   │  │
  │ │mend.   │  │Service   │  │Engine   │  │
  │ └────────┘  └──────────┘  └─────────┘  │
  └────────────────────────────────────────┘

  Label: CDN handles 95%+ of bandwidth
  Label: Event-sourced progress (never lose learner state)
  Label: Server-side grading only (no answers to client)
```

### 6.5 Red Flags to Avoid

| Red Flag | Signal It Sends |
|----------|----------------|
| Sending quiz answers to the client for "faster grading" | Fundamental security misunderstanding — answers inspectable via dev tools |
| Using a single relational DB for progress events | Doesn't grasp write volume (500K events/sec) or access pattern mismatch |
| Designing video delivery without mentioning CDN | Missing the dominant architectural component (95%+ of bandwidth) |
| No mention of DRM for paid content | Ignoring instructor IP protection — destroys marketplace trust |
| Storing video in a database | Confusing blob storage with relational storage; no CDN integration |
| Proposing real-time recommendation computation | Doesn't understand cost of nearest-neighbor search at 100M user scale |
| Ignoring content hierarchy and prerequisites | Misses the DAG-based dependency problem unique to learning platforms |
