...
Rule that never changes: For any edge E,
...
### Slowest part of the process 1: Graph Traversal at Scale
...
### Slowest part of the process 2: Real-Time Job Matching
...
### Slowest part of the process 3: Feed Freshness
...
### Slowest part of the process 4: InMail Delivery Latency
...
## Deep Dive 5: AI-Assisted Hiring Pipeline

### Why Agentic Recruiting is Critical

The LinkedIn Hiring Assistant (2025) represents a shift from search-based recruiting to agentic AI that autonomously manages candidate pipelines. This creates new architectural challenges around LLM orchestration, recruiter trust, and real-time integration with existing systems.

### Architecture

```mermaid
flowchart TB
    subgraph Recruiter["Recruiter Interface"]
        Brief["Job Brief Input"]
        Review["Candidate Review"]
        Approve["Approve/Reject Outreach"]
    end

    subgraph Agent["Hiring Assistant Agent"]
        Planner["Task Planner<br/>(LLM Orchestrator)"]
    ...
    subgraph Backend["Backend Services"]
        Galene["Galene Search"]
        GLMix["GLMix Scorer"]
        Graph["LIquid Graph<br/>(Network Proximity)"]
        Profile["Profile Service"]
    end

    subgraph Safety["Safety & Quality"]
        Guard["AI Guardrails<br/>(Bias Detection)"]
        Human["Human-in-Loop Gate"]
        Audit["Audit Trail"]
    end
    ...
    classDef recruiter fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef agent fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef backend fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef safety fill:#fce4ec,stroke:#880e4f,stroke-width:2px

    class Brief,Review,Approve recruiter
    class Planner,Sourcer,Scorer,Drafter agent
    class Galene,GLMix,Graph,Profile backend
    class Guard,Human,Audit safety
```

### Agent Orchestration Pipeline

```
FUNCTION HiringAssistantPipeline(job_brief):
    // Step 1: Parse brief into structured requirements
    requirements = LLM.Extract(job_brief, schema=[
        "title", "skills_required", "skills_preferred",
        "experience_range", "location_flexibility",
        "company_size_pref", "industry_focus"
    ])

    // Step 2: Multi-source candidate generation
    candidates = []

    // Source A: Galene text search with semantic expansion
    search_results = Galene.Search(
        query = requirements.to_search_query(),
        filters = {experience: requirements.experience_range},
        limit = 500
    )
    candidates.extend(search_results)

    // Source B: LiGNN embedding similarity
    job_embedding = EmbeddingService.Encode(requirements)
    similar = EmbeddingService.NearestNeighbors(
        job_embedding, k=200, index="member_embeddings"
    )
    candidates.extend(similar)

    // Source C: Graph proximity to existing team
    team_members = JobService.GetTeam(job_brief.company_id)
    FOR each member in team_members:
        second_degree = LIquid.GetSecondDegree(member.id)
        graph_candidates = FilterBySkills(second_degree, requirements)
        candidates.extend(graph_candidates[:50])

    // Step 3: Deduplicate and score
    candidates = Deduplicate(candidates)
    FOR each candidate in candidates:
        candidate.fit_score = GLMix.Score(candidate, requirements)
        candidate.response_likelihood = ResponseModel.Predict(
            candidate, job_brief.company_id
        )
        candidate.bias_check = BiasDetector.Evaluate(candidate)

    // Step 4: Rank and diversify
    ranked = Sort(candidates, key=combined_score)
    diverse_shortlist = DiversityFilter(ranked, top_k=25)

    // Step 5: Draft personalized outreach
    FOR each candidate in diverse_shortlist:
        outreach = LLM.Draft(
            template = "recruiter_inmail",
            context = {
                candidate_profile: candidate.public_summary,
                job_highlights: requirements.key_selling_points,
                mutual_connections: LIquid.MutualConnections(
                    recruiter_id, candidate.id
                ),
                tone: "professional_personalized"
            }
        )
        candidate.draft_outreach = outreach

    // Step 6: Human-in-the-loop review
    RETURN PresentForReview(diverse_shortlist)
```
...
| Failure | Impact | Mitigation |
|---------|--------|------------|
| LLM generates biased outreach (gendered language) | Legal liability, brand damage | Bias detection guardrail with blocked-term dictionary + LLM review pass |
| Agent over-contacts passive candidates | Candidate experience degradation | Per-candidate contact frequency limits (max 1 outreach per 90 days per company) |
| GLMix score disagrees with LLM assessment | Conflicting signals confuse recruiter | Present both scores with explanation; recruiter makes final decision |
| Search returns insufficient diverse candidates | Pipeline bias, compliance risk | Mandatory diversity constraints in DiversityFilter; flag if constraints unmet |
| Agent halluccinates candidate qualifications | Recruiter trusts incorrect information | Ground all candidate details in verified profile data; never generate unverified claims |

---

## Deep Dive 6: Economic Graph as Cross-Product Data Fabric

### Why the Economic Graph Matters Architecturally

LinkedIn's Economic Graph is not a marketing concept -- it is a concrete data architecture that models the relationships between 1.2 billion members, 67 million companies, 15 million job listings, 41,000 skills, and 200,000 schools as a unified knowledge graph. Every product surface queries this graph for different projections.

### Cross-Product Signal Propagation

```
EVENT: Company X announces 500 layoffs

PROPAGATION VIA KAFKA:

1. JOB MARKETPLACE (latency: <5 min):
   - Mark affected members as "Open to Work" (with consent)
   - Boost job recommendations for affected skill profiles
   - Surface Company X competitors hiring similar roles

2. RECRUITER PLATFORM (latency: <10 min):
   - Alert recruiters with saved searches matching affected skills
   - Update company headcount in Sales Navigator
   - Increase InMail credit allocation for recruiters targeting affected pool

3. FEED RANKING (latency: <30 min):
   - Boost industry analysis content about Company X's sector
   - Surface "how to navigate layoffs" collaborative articles
   - Increase visibility for career transition coaches

4. LINKEDIN LEARNING (latency: <1 hour):
   - Recommend reskilling courses based on demand gap analysis
   - Surface certificates valued by companies hiring similar roles

5. ECONOMIC RESEARCH (latency: <24 hours):
   - Update Workforce Report with sector trend data
   - Adjust skill demand forecasts in Economic Graph Insights
```

### Skill Taxonomy Normalization

```
FUNCTION NormalizeSkill(raw_skill_text):
    // 41,000+ standardized skills in taxonomy
    // Members type free-text; system maps to canonical form

    // Step 1: Exact match
    IF raw_skill_text IN skill_taxonomy:
        RETURN skill_taxonomy[raw_skill_text]
    ...
    aliases = {
        ...
        "JS": "JavaScript",
        "React.js": "React",
        "AWS": "Cloud Computing",  // Vendor-neutral canonical
        "GCP": "Cloud Computing"
    }
    IF raw_skill_text IN aliases:
        RETURN skill_taxonomy[aliases[raw_skill_text]]

    // Step 3: Embedding similarity
    embedding = EmbeddingService.Encode(raw_skill_text)
    nearest = EmbeddingService.NearestNeighbors(
        embedding, k=5, index="skill_embeddings"
    )
    IF nearest[0].similarity > 0.85:
        RETURN nearest[0].canonical_skill

    // Step 4: LLM classification (batch, not real-time)
    ENQUEUE_FOR_BATCH_CLASSIFICATION(raw_skill_text)
    RETURN TemporarySkill(raw_skill_text)  // Use raw until classified
```
...
|----------|----------|----------|-------------------|
| **Skill normalization** | Free-text only (flexible) | Strict taxonomy (consistent) | **Hybrid** -- free-text mapped to taxonomy |
| **Cross-product latency** | Synchronous (consistent) | Async via Kafka (fast) | **Async** -- eventual consistency acceptable |
| **Economic Graph scope** | Members + companies only | Full professional economy | **Full scope** -- includes skills, schools, courses |
| **Signal propagation** | Per-product data silos | Unified event stream | **Unified** -- single Kafka topic, per-product consumers |
