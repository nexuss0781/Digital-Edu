...
## 4. Capacity Growth Management

### Storage Tier Migration Pipeline

```mermaid
flowchart LR
    subgraph Hot["Hot Tier (SSD)"]
        H["Blocks accessed<br/>within 7 days"]
    end

    subgraph Warm["Warm Tier (HDD)"]
        W["Blocks accessed<br/>7-30 days ago"]
    end

    subgraph Cold["Cold Tier (SMR/Archive)"]
        C["Blocks not accessed<br/>for 30+ days"]
    end

    H -->|"7 days no access"| W
    W -->|"30 days no access"| C
    C -->|"Accessed again"| H
    W -->|"Accessed again"| H
...
**Migration rates** (at Dropbox scale):
- Hot → Warm: ~5 PB/day (most blocks cool quickly after initial burst)
- Warm → Cold: ~2 PB/day (steady-state migration)
- Cold → Hot: ~200 TB/day (access-driven promotions, much lower volume)

### Hardware Generation Planning

| Generation | Capacity/Server | Drives | Power (W/TB) | Era |
|-----------|----------------|--------|--------------|-----|
| Gen 5 | 480 TB raw | 60x 8TB HDD | 0.65 | 2018 |
| Gen 6 | 1 PB raw | 60x 16TB SMR | 0.40 | 2020 |
| Gen 7 | 1.5 PB raw | 60x 24TB SMR | 0.30 | 2023 |
| Gen 8 | 2.4 PB raw | 60x 40TB HAMR | 0.20 (projected) | 2025-2026 |

**HAMR (Heat-Assisted Magnetic Recording)** drives represent the next generation of storage density, enabling 40+ TB per drive. At Dropbox's scale (~5,000 storage servers), each generation transition reduces the physical footprint by 30-40% while increasing capacity.

### Shard Health Scoring

```
ALGORITHM ShardHealthScore(shard_id)
  // Composite score from 0 (critical) to 100 (perfect)

  qps_score ← 100 - (current_qps / max_qps * 100)        // headroom
  latency_score ← 100 - (p99_latency / slo_latency * 100)
  repl_score ← IF replication_lag < 1s THEN 100 ELSE MAX(0, 100 - lag_seconds * 10)
  disk_score ← 100 - (disk_usage_pct)
  error_score ← 100 - (error_rate * 1000)                  // 0.1% → 0 score
  ...
  IF health < 30: ALERT("P1: Shard critical", shard_id)
  IF health < 60: ALERT("P3: Shard degraded", shard_id)
...
## 5. Chaos Engineering for File Storage

### Failure Injection Scenarios

| Scenario | What to Inject | Expected Behavior | Blast Radius |
|----------|---------------|-------------------|--------------|
| **Storage node failure** | Kill 1-3 storage nodes in a zone | Erasure coding serves reads from remaining fragments; auto-repair kicks in | Affected blocks only; no data loss |
| **Metadata DB primary failure** | Kill primary MySQL instance | Replica promoted within 30s; brief write unavailability | All users on affected shard |
| **Network partition** | Isolate one availability zone | Cross-zone traffic reroutes; in-zone operations continue | Zone-isolated users |
| **Cache invalidation storm** | Flush entire metadata cache | Thundering herd protection (singleflight) limits DB load | Global latency spike (recover in ~60s) |
| **Clock skew** | Advance/retard clocks by 30s | Server-assigned sequence numbers (not timestamps) maintain ordering | No impact (by design) |
| **Block storage slowdown** | Inject 500ms latency on disk I/O | Circuit breakers trigger; uploads queued; downloads served from cache/CDN | Upload latency degradation |
| **Message queue partition** | Isolate message broker | Sync still works (pull-based cursors); notifications delayed | Notification latency spike |

### Steady-State Hypothesis Testing

| Hypothesis | Metric | Acceptable Deviation |
|-----------|--------|---------------------|
| "Users can always list their files" | Metadata read success rate | Must stay >99.9% |
| "Uploaded files are never lost" | Block durability (fragment count) | Every block maintains ≥6 of 9 fragments |
| "Sync completes within 10 seconds" | p99 sync completion time | May degrade to 30s but not beyond |
| "Shared files are accessible" | Download success rate for shared content | Must stay >99.5% |

---

## 6. Data Integrity Verification

### End-to-End Checksum Chain

Every file has a verifiable integrity chain from client to storage and back:

```
Client upload:
  1. Client computes file_hash = SHA-256(entire file)
  2. Client chunks file → computes block_hash = SHA-256(each block)
  3. Client sends block_hashes to server for dedup check
  4. Client uploads new blocks; server re-computes SHA-256 and verifies match
  5. Client sends commit with {block_list, file_hash}
  6. Server verifies SHA-256(CONCAT(block_hashes)) == file_hash

Storage:
  7. Each block is erasure-coded (6+3); fragment checksums stored
  8. Background scrubbing re-hashes fragments every 14 days
  9. Any mismatch triggers fragment repair from healthy copies

Client download:
  10. Client downloads blocks, verifies each block_hash
  11. Client reassembles file, verifies file_hash matches metadata
  12. If mismatch: discard and re-download from different storage nodes
```

**Key principle**: Checksums are verified at every boundary crossing (client → server, server → storage, storage → client). Silent data corruption is detected at the earliest possible stage.

### Consistency Verification Between Metadata and Storage

```
ALGORITHM ConsistencyAudit()
  // Runs daily; samples 0.1% of files
  sample ← SELECT file_id, block_list, content_hash
    FROM file_versions
    ORDER BY RANDOM()
    LIMIT 100000
  ...
  FOR EACH file IN sample:
    FOR EACH block_hash IN file.block_list:
      IF NOT BLOCK_EXISTS(block_hash):
        ALERT("Orphaned reference", file.file_id, block_hash)
        inconsistencies ← inconsistencies + 1

    // Verify file hash matches block composition
    computed ← SHA256(CONCAT(file.block_list))
    IF computed != file.content_hash:
      ALERT("Metadata inconsistency", file.file_id)
      inconsistencies ← inconsistencies + 1

  RETURN {sampled: LENGTH(sample), inconsistencies: inconsistencies}
```
