...
### 4.3 Share Link Access State Machine
...
stateDiagram-v2
    ...
    Created --> Revoked: Creator revokes

    Active --> Active: Subsequent accesses
    Active --> Expired: TTL reached
    Active --> Revoked: Creator revokes
    Active --> MaxedOut: Download limit reached
    ...
    state Active {
        [*] --> PasswordCheck: Has password?
        PasswordCheck --> Authenticated: Correct password
        PasswordCheck --> Blocked: 5 failed attempts
...
### 5.1 Rename Detection via Content Hashing

When a file disappears from one path and appears at another, the sync engine must distinguish a rename/move from a delete+create pair:

```
ALGORITHM DetectRename(disappeared_files, appeared_files)
  INPUT: list of paths that disappeared, list of paths that appeared
  OUTPUT: list of (old_path, new_path) rename pairs

  // Build content hash → path mapping for disappeared files
  disappeared_map ← {}
  FOR EACH path IN disappeared_files:
    hash ← LOOKUP_CONTENT_HASH(path)
    disappeared_map[hash] ← path

  rename_pairs ← []
  remaining_appeared ← []

  // Match appeared files by content hash
  FOR EACH path IN appeared_files:
    hash ← LOOKUP_CONTENT_HASH(path)
    IF hash IN disappeared_map:
      old_path ← disappeared_map[hash]
      APPEND rename_pairs, (old_path, path)
      DELETE disappeared_map[hash]
    ELSE:
      APPEND remaining_appeared, path

  // Unmatched disappeared = true deletes
  // Unmatched appeared = true creates

  RETURN rename_pairs

// This prevents re-uploading all blocks when a file is simply moved
// Content hash makes rename detection O(1) per file
```

### 5.2 Namespace Journaling for Sync Cursors

The sync cursor system uses a **journal** (append-only log of changes per namespace) to enable efficient delta computation:

```
ALGORITHM GetChangesSinceCursor(namespace_id, cursor)
  INPUT: namespace identifier, opaque cursor (encoded sequence number)
  OUTPUT: list of changes, new cursor

  last_seq ← DECODE_CURSOR(cursor)

  changes ← SELECT node_id, change_type, name, path, version, content_hash
    FROM namespace_journal
    WHERE namespace_id = namespace_id
    AND sequence_number > last_seq
    ORDER BY sequence_number ASC
    LIMIT 1000

  // Compact: if same node_id appears multiple times, keep only latest
  compacted ← {}
  FOR EACH change IN changes:
    compacted[change.node_id] ← change

  new_cursor ← ENCODE_CURSOR(MAX(changes.sequence_number))

  RETURN {
    entries: VALUES(compacted),
    cursor: new_cursor,
    has_more: LENGTH(changes) == 1000
  }

// The journal is the source of truth for "what changed"
// Compaction prevents sending intermediate states (e.g., file edited 5x → only latest version)
// Cursor is opaque to clients; server can change encoding without breaking clients
```
