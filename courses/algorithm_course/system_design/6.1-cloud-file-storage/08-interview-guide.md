...
3. **Metadata is harder than blob storage**: At scale, the metadata layer (file trees, permissions, versions, sync cursors) is the Slowest part of the process --- not the blob storage. Dropbox's Edgestore/Panda stack is more complex than Magic Pocket.
...
| Dimension | Cloud File Storage | Object Storage | Collaborative Editor | CDN |
|-----------|-------------------|---------------|---------------------|-----|
| **Core challenge** | N-device bidirectional sync | Exabyte-scale durable storage | Real-time character-level sync | Global content distribution |
| **Consistency** | Eventual (content) + Strong (metadata) | Eventual | Strong (OT/CRDT) | Eventual |
| **Write pattern** | Write-heavy during sync bursts | Write-once-read-many | Continuous small writes | Write-once at origin |
| **Conflict model** | Conflicted copies | Last-write-wins | Auto-merge (OT/CRDT) | N/A (immutable) |
| **Latency target** | <5s sync propagation | <100ms first byte | <100ms keystroke propagation | <50ms edge delivery |
| **Key algorithm** | Three-tree merge + FastCDC | Erasure coding | Operational transformation | Consistent hashing |
...
|-----|----------|------------------------|-----------|
| ADR-001 | Content-defined chunking (FastCDC) | Fixed-size chunking | 50-70% better dedup ratio; handles insertions gracefully |
| ADR-002 | Conflicted copies for conflict resolution | Last-write-wins, CRDT | Files are opaque blobs; no semantic merge possible; preserves both versions |
| ADR-003 | Sharded SQL for metadata | NoSQL (Cassandra), NewSQL | Need strong consistency + complex queries (tree traversal, permissions) |
| ADR-004 | Erasure coding (6+3) for warm/cold blocks | Triple replication | 50% storage cost reduction at same or better durability |
| ADR-005 | Long-poll primary, WebSocket optional | WebSocket-only, SSE | Long-poll is more firewall-friendly; WebSocket as progressive enhancement |
| ADR-006 | Namespace-scoped dedup (not global) | Global cross-user dedup | Security risk of hash-based existence probing outweighs storage savings |
| ADR-007 | Node-ID-based operations (not path-based) | Path-based operations | Decouples rename from edit; eliminates move+edit race condition |
| ADR-008 | Rust for sync engine (Nucleus) | Go, Java, C++ | Ownership system prevents concurrency bugs at compile time |

---

## 12. Whiteboard Drawing Guide

### Step-by-Step Diagram Progression

**Phase 1: Basic upload/download (first 5 minutes)**
```
Client → API Gateway → Block Service → Block Storage
                     → Metadata Service → Metadata DB
```

**Phase 2: Add sync (next 5 minutes)**
```
Add: Sync Service between Gateway and Block/Metadata
Add: Notification Service → Other Clients (WebSocket/long-poll)
Add: Three-tree model annotation on Sync Service
```

**Phase 3: Add scale (next 5 minutes)**
```
Add: CDN in front of Block Service for downloads
Add: Cache layer between Metadata Service and DB
Add: Message Queue for async processing
Add: Sharding annotation on Metadata DB
```

**Phase 4: Add reliability (next 5 minutes)**
```
Add: Erasure coding annotation on Block Storage (6+3 across 3 zones)
Add: Primary-Replica annotation on Metadata DB
Add: Circuit breaker annotations on service-to-service calls
```
...
|--------|-------|------------|
| Google Drive MAU | 2B+ | Establishing scale context |
| Dropbox storage | 3+ EB | Justifying custom infrastructure |
| Dedup ratio | 3:1 (enterprise) | Storage savings calculation |
| FastCDC speed | 3-12x faster than Rabin | Chunking algorithm choice |
| Erasure coding overhead | 1.5x (vs 3x for triple replication) | Cost optimization discussion |
| Metadata QPS | Millions/sec (Edgestore) | Metadata scaling discussion |
| Sync latency target | <5 seconds | Notification architecture |
| Magic Pocket savings | $75M over 2 years | Build vs buy discussion |

---

## 13. Follow-Up Deep Dive Questions

Interviewers may probe specific areas after the initial design. Be prepared for these follow-up threads:

### Thread A: "Walk me through what happens when I edit a file offline and my colleague edits the same file"

**Key points to hit:**
1. Both devices queue changes locally (offline mode writes to local tree only)
2. When online: both devices upload blocks and attempt to commit
3. First commit succeeds (optimistic concurrency with version check)
4. Second commit gets 409 Conflict (base version stale)
5. Sync engine creates "conflicted copy" for second writer
6. Both versions preserved; user resolves manually
7. Mention: three-tree model detects this via divergent diffs from sync tree

### Thread B: "How would you handle a user with 5 million files in one folder?"

**Key points to hit:**
1. Directory listing uses pagination (cursor-based, not offset-based)
2. Sync uses cursors (never fetches full tree, only deltas since last cursor)
3. This namespace becomes a hot shard → shard splitting if needed
4. Aggressive caching (Chrono) absorbs read amplification
5. Client-side virtual scrolling for UI performance
6. Consider recommending the user restructure into subfolders (UX guidance)

### Thread C: "What would you change if we needed to support 100 GB files?"

**Key points to hit:**
1. Increase max chunk size (maybe 8-16 MB to reduce chunk count)
2. Parallel upload with more concurrent streams (32-64 instead of 8-16)
3. Resumable upload protocol (each block can be retried independently)
4. Streaming sync becomes critical (other devices start downloading during upload)
5. Consider server-side assembly for very large files (reduce client memory pressure)
6. Progress UI becomes essential (show block-level progress, not just file-level)
