...
## Real-Time Generation Architecture (2025-2026)

Consistency distillation and 1-4 step models enable a fundamentally different architecture path for interactive use cases:

```mermaid
flowchart LR
    subgraph Interactive["Interactive Canvas"]
        Canvas["Editor Canvas"]
        Brush["Brush / Mask"]
        Params["Real-Time<br/>Parameter Sliders"]
    end

    subgraph StreamPipeline["Streaming Diffusion Pipeline"]
        LatentStream["Latent<br/>Streaming"]
        ConsistModel["Consistency<br/>Model (1-step)"]
        IncrDecode["Incremental<br/>VAE Decode"]
    end

    subgraph Delivery["Progressive Delivery"]
        WSStream["WebSocket<br/>Frame Stream"]
        Progressive["Progressive<br/>JPEG/WebP"]
        FinalRender["Final Hi-Res<br/>Render (queued)"]
    end

    Canvas --> LatentStream
    Brush --> LatentStream
    Params --> LatentStream
    LatentStream --> ConsistModel
    ConsistModel --> IncrDecode
    IncrDecode --> WSStream
    WSStream --> Canvas
    IncrDecode --> FinalRender

    classDef interactive fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef pipeline fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef delivery fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class Canvas,Brush,Params interactive
    class LatentStream,ConsistModel,IncrDecode pipeline
    class WSStream,Progressive,FinalRender delivery
```

**Key Differences from Standard Pipeline:**

| Aspect | Standard (Queued) | Interactive (Real-Time) |
|--------|-------------------|------------------------|
| Latency target | 8-60s E2E | <200ms per update |
| Model type | Full diffusion (20-50 steps) | Consistency/LCM (1-4 steps) |
| Communication | HTTP polling / webhook | WebSocket streaming |
| GPU allocation | Shared queue | Dedicated session GPU |
| Use case | Final renders, batch | Canvas editing, previews |

**Architecture Implications:**
- **Session affinity**: Interactive sessions require a dedicated GPU with the model and LoRA pre-loaded for the duration (5-30 minutes)
- **Speculative generation**: On parameter change, start a new generation immediately, canceling the in-flight one if superseded
- **Quality tiering**: Preview at 512px with consistency model, then queue a full-quality render at user's target resolution

---

## Video Generation Extension

The same architecture extends to video generation with temporal modifications:

```
Video Generation Delta:
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│  SHARED WITH IMAGE GEN         NEW FOR VIDEO                       │
│  ─────────────────────         ──────────────                      │
│  API Gateway + Auth            Temporal Attention Layers            │
│  Prompt Processing (CLIP/T5)   Frame-to-Frame Consistency          │
│  Queue Management              10x VRAM Budget (40-80 GB)          │
│  Safety Pipeline               Temporal Safety (motion analysis)   │
│  CDN Delivery                  Chunked Video Encoding              │
│                                                                     │
│  GPU SCHEDULING IMPACT:                                            │
│  ──────────────────────                                            │
│  Image: ~10s per request, 30 GB VRAM                               │
│  Video: ~60-300s per request, 60-80 GB VRAM                        │
│  → Video requests need separate queue tier and dedicated A100/H100 │
│  → Cannot share GPU pool with image gen (preemption too costly)    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
