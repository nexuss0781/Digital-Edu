...
## Modern Design Questions (2025-2026)

### Q1: How would you add real-time interactive generation to this platform?

> **Strong Answer:** "Real-time generation fundamentally changes the serving model. Instead of queue-based async processing, we need session-based GPU allocation:
>
> 1. **Dedicated session GPU**: When a user enters canvas/interactive mode, allocate a GPU with the selected model + LoRA pre-loaded. The session lasts 5-30 minutes.
> 2. **Consistency/LCM models**: Use 1-4 step distilled models (~50-200ms) instead of full 30-step diffusion.
> 3. **WebSocket streaming**: Stream progressive results to the client as each denoising step completes.
> 4. **Speculative cancellation**: If the user changes a parameter while generation is in-flight, cancel and restart immediately.
>
> The cost model shifts from per-image to per-session. A 10-minute interactive session at $0.30/GPU-minute costs $3 vs $0.02 per queued image. This means interactive mode should be premium-tier only."

### Q2: How would you handle the transition from UNet to DiT model architectures?

> **Strong Answer:** "This is a fleet management problem, not just a model swap. DiT models (Flux, SD3) are 2-10x larger than SDXL's UNet, so VRAM per worker increases dramatically while throughput per GPU may actually decrease despite faster per-image generation. I'd run a hybrid fleet:
>
> - **Separate warm pools** for UNet and DiT models (different VRAM profiles)
> - **Traffic-based routing**: Route to DiT fleet for models that require it, keep UNet fleet for the SDXL ecosystem
> - **Phased migration**: Start with 10% DiT capacity, increase as user demand shifts
> - **LoRA compatibility layer**: Existing SDXL LoRAs don't work with DiT, so we need a parallel fine-tuning pipeline
>
> The key insight is that capacity planning can't just scale linearly — the VRAM-per-request vs requests-per-GPU trade-off changes fundamentally."

### Q3: How would you design the content provenance system for regulatory compliance?

> **Strong Answer:** "With the EU AI Act and similar regulations mandating AI content labeling, provenance must be built into the generation pipeline:
>
> 1. **C2PA metadata**: Embed generation metadata (model version, prompt hash, timestamp, safety scores) into image EXIF at the watermarking stage
> 2. **Invisible watermarking**: Use learned watermark embedding (Stable Signature) that survives JPEG compression, cropping, and screenshots
> 3. **Provenance ledger**: Append-only log of all generations with content hashes for after-the-fact verification
> 4. **Verification API**: Public endpoint that checks whether an image was generated by our platform
>
> The architectural impact is small (<10ms per image for watermark embedding) but the compliance surface is large — every output must be traceable."

### Updated Red Flags

| Red Flag | Why It's Wrong | What to Say Instead |
|----------|---------------|-------------------|
| "Just add more GPUs for faster generation" | Single image can't be parallelized across GPUs (except DistriFusion for high-res) | "More GPUs increase throughput; latency requires fewer steps or faster models" |
| "Same architecture as LLM serving" | Fixed VRAM, multi-step, model composition all differ | "Contrast: fixed VRAM budget, multi-model composition, binary output" |
| "Block everything at the prompt level" | Adversarial prompts bypass text classifiers | "Dual-layer: pre-gen saves GPU cost, post-gen catches visual issues" |
| "One queue is fine" | No tier differentiation = no monetization, no fairness | "Multi-tier with weighted scheduling and starvation prevention" |
| "DiT is just a bigger UNet" | Fundamentally different architecture and VRAM profile | "DiT uses flow matching, different sampling, and changes capacity planning" |
| "Safety models only make things safer" | New classifiers can increase false positives | "Treat safety updates like any model update: canary rollout + monitoring" |
