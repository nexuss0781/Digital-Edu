...
## Insight 12: Flow Matching Replaces Fixed Noise Schedules, Changing the Inference Performance Envelope

**Category:** Architecture
**One-liner:** DiT architectures with flow matching learn a direct velocity field between noise and data distributions, enabling fewer denoising steps for equivalent quality and fundamentally changing GPU scheduling assumptions.

**Why it matters:** Traditional UNet-based diffusion (SD 1.5, SDXL) uses a fixed noise schedule (e.g., linear or cosine) that dictates how noise is added and removed over a predetermined number of steps. Flow matching (used in SD3, Flux) instead learns a continuous velocity field that defines the shortest path from noise to data, meaning the model can traverse from noise to image in fewer, more efficient steps. Flux Schnell achieves strong results in 4 steps where SDXL needs 30 — a 7.5x compute reduction per image. For platform architecture, this shifts the warm pool calculus: DiT models are larger (2-8B parameters vs 860M for SDXL UNet) but generate faster per request, meaning each GPU serves more requests per hour despite higher VRAM consumption. The same DiT backbone also extends naturally to video generation (adding temporal attention layers), making architecture investments in DiT serving infrastructure directly transferable to video generation workloads. The generalized insight: when a new model architecture changes the compute-per-request vs model-size trade-off, the entire capacity planning model must be re-derived rather than scaled linearly from previous assumptions.
