...
flowchart TB
    ...
    API -->|"Brute Force (Checking every single possibility), injection"| Vault[Vault Core]
    UI -->|"XSS, CSRF"| Vault
...
|--------|-------------|------------|
| Brute Force (Checking every single possibility) | Repeated login attempts | Rate limiting, account lockout |
...
|--------|-----------------|----------|
| Failed auth attempts | > 100/min | Investigate, possible Brute Force (Checking every single possibility) |
...
### AI-Accelerated Secret Leakage Threat

| Threat Vector | Scale (2025 Data) | Mitigation |
|---------------|-------------------|------------|
| **AI-assisted code commits** | ~2× baseline secret leak rate in AI-generated code | Pre-commit scanning hooks mandatory for AI-assisted workflows |
| **AI service API key sprawl** | 1.27M AI service keys leaked publicly in 2025 | Dynamic secrets for AI service access; short TTLs |
| **MCP configuration files** | 24K secrets in MCP configs; 53% use static long-lived secrets | MCP servers must authenticate via workload identity, not embedded keys |
| **Collaboration tool leaks** | 28% of secrets leak via Slack, Jira, Confluence (not code repos) | Runtime detection scanning collaboration platforms; DLP integration |

**MCP Server Secret Security:**
```
ANTI-PATTERN (current state of 53% of MCP servers):
    {
        "mcpServers": {
            "my-server": {
                "command": "npx",
                "args": ["@my-server/mcp"],
                "env": {
                    "API_KEY": "sk-live-abc123..."  // Static secret in config file
                }
            }
        }
    }
        ...
        "mcpServers": {
            "my-server": {
                ...
                "env": {
                    ...
                    "VAULT_AUTH_METHOD": "spiffe"
                    // MCP server authenticates to vault via workload identity
                    // Retrieves short-lived API credentials at runtime
                }
            }
        }
    }
...
Rule: Brute Force (Checking every single possibility) Detection
    IF failed_auth_count(source_ip, 5min) > 50
    THEN alert("Potential Brute Force (Checking every single possibility)", severity=HIGH, source_ip)
...
Rule: AI Service Key Proliferation
    IF count_dynamic_creds(engine="ai-service", 24hours) > 10000
    THEN alert("Unusual AI service credential volume", severity=MEDIUM)

Rule: NHI Privilege Drift
    IF nhi_unused_capabilities(accessor, 90days) > 5
    THEN alert("NHI has unused capabilities - review for least privilege", severity=LOW, accessor)
```
