...
## SPIFFE Workload Identity Authentication
...
sequenceDiagram
    ...
    Note over Workload,SPIRE: Workload obtains SVID from SPIRE
    Workload->>SPIRE: Request SVID
    SPIRE-->>Workload: X.509 SVID (short-lived cert)

    Workload->>Vault: POST /v1/auth/spiffe/login (present SVID)
    Vault->>Vault: Validate SVID signature against trust bundle
    Vault->>Vault: Extract SPIFFE ID from SAN
    Vault->>Policy: Map SPIFFE ID to policies
    Policy-->>Vault: Attached policies
    Vault-->>Workload: Vault token + lease_duration

    Workload->>Vault: GET /v1/secret/data/myapp (token)
    Vault->>Engine: Read with policy check
    Engine-->>Vault: Secret data
    Vault-->>Workload: Response

    Note over Workload: No static secrets exchanged
    Note over Workload: Identity is cryptographic, not password-based
```

**SPIFFE Auth Method Characteristics:**

| Aspect | Details |
|--------|---------|
| **Identity Format** | `spiffe://trust-domain/workload/service-name` |
| **SVID Types** | X.509 certificates (mTLS) and JWT tokens |
| **Trust Validation** | SPIFFE trust bundle verified against configured trust domains |
| **Policy Mapping** | SPIFFE ID patterns mapped to Vault policy sets |
| **SVID Minting** | Vault can issue SPIFFE-compliant SVIDs to authenticated clients |
| **Rotation** | SVIDs are short-lived (minutes to hours); automatic re-authentication |

---

## Secrets Synchronization Architecture

```mermaid
flowchart LR
    subgraph Vault["Vault (Source of Truth)"]
        KV[KV Engine]
        Sync[Secrets Sync Engine]
    end

    subgraph Destinations["Sync Destinations"]
        AWS[Cloud Secret Manager A]
        GCP[Cloud Secret Manager B]
        K8S[Kubernetes Secrets]
        GH[CI/CD Variables]
    end

    subgraph Import["Import Sources"]
        ExtAWS[External Cloud Secrets]
        ExtGCP[External Cloud Secrets]
    end

    KV --> Sync
    Sync -->|"Real-time sync"| AWS
    Sync -->|"Real-time sync"| GCP
    Sync -->|"Real-time sync"| K8S
    Sync -->|"Real-time sync"| GH

    ExtAWS -->|"Bulk import"| KV
    ExtGCP -->|"Bulk import"| KV

    classDef vault fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef dest fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef import fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class KV,Sync vault
    class AWS,GCP,K8S,GH dest
    class ExtAWS,ExtGCP import
```
...
| Feature | Behavior |
|---------|----------|
| **Direction** | Vault → External (sync); External → Vault (import) |
| **Trigger** | Real-time on secret update; version-aware to prevent stale overwrites |
| **Deletion Safety** | Force-delete flag required; version fallback prevents accidental external deletion |
| **Multi-Destination** | Single secret synced to multiple cloud providers simultaneously |
| **Use Case** | Centralized management with secrets available where workloads actually run |

---

## Post-Quantum Cryptography in Transit Engine

| Algorithm | Standard | Purpose | Status |
|-----------|----------|---------|--------|
| **ML-DSA** | FIPS 204 | Lattice-based digital signatures | Experimental (sign/verify) |
| **SLH-DSA** | FIPS 205 | Hash-based digital signatures | Experimental (sign/verify) |
| **ML-KEM** | FIPS 203 | Lattice-based key encapsulation | Planned |
| **Hybrid Mode** | N/A | Classical + PQC combined | Recommended default |

**Cryptographic Agility Pattern:**
```
Current: AES-256-GCM + RSA/ECDSA → Transit encrypt/sign
Future:  AES-256-GCM + ML-KEM/ML-DSA → Transit encrypt/sign (PQC)
Bridge:  Hybrid mode → Both classical and PQC algorithms active simultaneously

Key rotation supports algorithm migration:
  vault:v1:classical-ciphertext  → vault:v2:hybrid-ciphertext → vault:v3:pqc-ciphertext
  Rewrap API migrates ciphertext between algorithm versions without exposing plaintext
```
...
| **KMS outage (auto-unseal)** | Unseal failure on restart | Fall back to recovery keys | Shamir recovery or KMS restoration |
| **SPIFFE trust bundle expiry** | SVID validation failure | Reject new auth; existing tokens valid | Refresh trust bundle from SPIRE server |
