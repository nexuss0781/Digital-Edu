...
### Trap 5: "How do you prepare for quantum computing threats?"

**What Interviewer Wants**: Test awareness of post-quantum cryptography and forward-thinking architecture.

**Best Answer**:
> "The primary threat is 'harvest now, decrypt later' -- adversaries capture encrypted data today to decrypt once quantum computers are viable. This matters for secrets with long retention periods like audit logs.
>
> **Preparation:**
> 1. **Cryptographic agility**: Design the system so algorithms can be swapped without re-architecting. The Transit engine's versioned key model already supports this -- ciphertext tagged with `vault:v1:` can be rewrapped to `vault:v2:` with a different algorithm without exposing plaintext.
> 2. **Hybrid mode**: Run classical + PQC algorithms in parallel (ECDSA + ML-DSA). Valid if either algorithm remains secure.
> 3. **NIST standards**: ML-DSA (FIPS 204) for signatures, ML-KEM (FIPS 203) for key exchange, SLH-DSA (FIPS 205) for hash-based signatures are all finalized.
> 4. **Timeline**: CNSA 2.0 requires pure PQC by 2035; hybrid deployments recommended starting now.
>
> The key insight is that cryptographic agility is more important than any specific algorithm choice."
...
### Emerging Topics (Bonus Credit)

- [ ] Post-quantum cryptography readiness (ML-DSA, SLH-DSA in Transit)
- [ ] SPIFFE/SPIRE workload identity replacing static credentials
- [ ] Non-human identity governance (144:1 NHI-to-human ratio)
- [ ] Secrets sync to external destinations
- [ ] AI-accelerated secret leakage risks (28.65M leaked secrets in 2025)
...
- [ ] No awareness of NHI scale problem
