...
## Deep Dive 4: Post-Quantum Cryptographic Migration

### The "Harvest Now, Decrypt Later" Threat

Long-lived secrets encrypted with classical algorithms face a deferred threat: adversaries capture encrypted data today and decrypt it once quantum computers become capable. For a secret management system storing credentials with multi-year retention (audit logs, archived secrets), this threat is immediate.

### PQC Algorithm Integration in Transit Engine

```
FUNCTION pqc_sign(key_name: string, data: bytes, algorithm: string) -> SignatureResponse:
    """
    Sign data using post-quantum algorithms via the Transit engine.
    Supports ML-DSA (lattice-based) and SLH-DSA (hash-based).
    """
    key = storage.read(f"transit/keys/{key_name}")

    IF algorithm == "ml-dsa-65":
        // FIPS 204: Module-Lattice-Based Digital Signature
        // Security level 3 (~128-bit classical equivalent)
        signature = ml_dsa_sign(key.private_key, data, parameter_set="ML-DSA-65")

    ELIF algorithm == "slh-dsa-shake-256f":
        // FIPS 205: Stateless Hash-Based Digital Signature
        // Conservative choice: security relies only on hash function security
        signature = slh_dsa_sign(key.private_key, data, parameter_set="SLH-DSA-SHAKE-256f")

    ELIF algorithm == "hybrid-ecdsa-ml-dsa":
        // Hybrid mode: both classical and PQC signatures
        // Valid if EITHER algorithm remains secure
        ecdsa_sig = ecdsa_sign(key.classical_private_key, data)
        ml_dsa_sig = ml_dsa_sign(key.pqc_private_key, data)
        signature = combine_hybrid_signature(ecdsa_sig, ml_dsa_sig)
...
FUNCTION pqc_migration_rewrap(key_name: string, ciphertext: string) -> RewrapResponse:
    """
    Rewrap ciphertext from classical to PQC algorithm without exposing plaintext.
    Enables gradual migration of encrypted data.
    """
    // 1. Decrypt with current (classical) key version
    plaintext = transit_decrypt(key_name, ciphertext)

    // 2. Re-encrypt with latest (PQC or hybrid) key version
    new_ciphertext = transit_encrypt(key_name, plaintext, key_version="latest")

    // 3. Secure-zero plaintext from memory
    secure_zero(plaintext)

    RETURN RewrapResponse(
        ciphertext=new_ciphertext,
        key_version=get_latest_key_version(key_name)
    )
```

### PQC Migration Path

| Phase | Timeline | Action | Algorithm |
|-------|----------|--------|-----------|
| **Assessment** | Now | Inventory all cryptographic usage; identify long-lived secrets | Current (AES-256, RSA, ECDSA) |
| **Hybrid** | 2025-2028 | Deploy hybrid classical+PQC for new signatures and key exchange | ECDSA + ML-DSA, RSA + ML-KEM |
| **Migration** | 2028-2032 | Rewrap existing ciphertext; rotate all signing keys | ML-KEM, ML-DSA, SLH-DSA |
| **Pure PQC** | 2032-2035 | Deprecate classical algorithms; CNSA 2.0 compliance | Pure PQC only |

---

## Deep Dive 5: Non-Human Identity Lifecycle

### The NHI Scale Problem

Non-human identities (service accounts, API keys, machine credentials, CI/CD tokens) outnumber human identities 144:1 in modern organizations. 97% have excessive privileges, and 71% are not rotated within recommended timeframes. The secret management system is the control plane for governing this population.

### NHI Governance Architecture

```mermaid
flowchart TB
    subgraph Discovery["NHI Discovery"]
        Scan[Identity Scanner]
        Audit[Audit Log Analysis]
        Cloud[Cloud IAM Inventory]
    end

    subgraph Governance["NHI Governance"]
        Catalog[Identity Catalog]
        Policy[Privilege Policy Engine]
        Lifecycle[Lifecycle Manager]
    end

    subgraph Remediation["Automated Remediation"]
        Rotate[Credential Rotation]
        Scope[Privilege Scoping]
        Expire[Forced Expiration]
    end
    ...
    Catalog --> Policy
    Policy -->|"Excessive privileges"| Scope
    Policy -->|"Stale credentials"| Rotate
    Policy -->|"Orphaned identities"| Expire

    Lifecycle --> Rotate
    Lifecycle --> Scope
    Lifecycle --> Expire

    classDef discover fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef govern fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef remediate fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class Scan,Audit,Cloud discover
    class Catalog,Policy,Lifecycle govern
    class Rotate,Scope,Expire remediate
```

### NHI Lifecycle Step-by-step plan in plain English

```
FUNCTION audit_nhi_population():
    """
    Periodic scan of all non-human identities managed by the vault.
    Flags stale, overprivileged, or orphaned identities.
    """
    all_tokens = list_all_active_tokens()
    all_leases = list_all_active_leases()

    FOR token IN all_tokens:
        age = now() - token.created_at
        last_used = now() - token.last_used_at

        // Flag stale tokens (not used in 30 days)
        IF last_used > 30 days AND NOT token.is_periodic:
            flag_for_review(token, reason="stale", last_used=token.last_used_at)

        // Flag overprivileged tokens (sudo on broad paths)
        IF "sudo" IN token.capabilities AND token.path_pattern.endswith("*"):
            flag_for_review(token, reason="overprivileged", paths=token.authorized_paths)

        // Flag orphaned tokens (parent service decommissioned)
        IF NOT service_exists(token.metadata.get("service_name")):
            flag_for_review(token, reason="orphaned", service=token.metadata.get("service_name"))

    RETURN NHIAuditReport(
        total_nhis=len(all_tokens),
        stale_count=count_flagged("stale"),
        overprivileged_count=count_flagged("overprivileged"),
        orphaned_count=count_flagged("orphaned"),
        nhi_to_human_ratio=len(all_tokens) / count_human_identities()
    )
```
...
### Slowest part of the process 1: Lease Explosion
...
### Slowest part of the process 2: Cold Start Latency
...
### Slowest part of the process 3: Policy Evaluation at Scale
...
### Slowest part of the process 4: Certificate Generation Throughput
...
| Slowest part of the process | Symptom | Trigger Threshold | Primary Mitigation |
|------------|---------|-------------------|-------------------|
