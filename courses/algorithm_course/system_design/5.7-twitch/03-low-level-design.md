...
// Time Complexity (Speed of the algorithm): O(N) where N = number of origin DCs (~10-20)
// Space Complexity (Memory usage of the algorithm): O(N) for candidate scoring
```
...
// Time Complexity (Speed of the algorithm): O(1) for publish; O(N/E) for delivery per Edge
//   where N = viewers, E = Edge nodes
// Space Complexity (Memory usage of the algorithm): O(1) per message at PubSub level
```
...
// Time Complexity (Speed of the algorithm): O(N log N) for sorting, O(N * F) for ML scoring
// Space Complexity (Memory usage of the algorithm): O(N) for candidate list
```
...
### 3.6 Clip Creation Algorithm

```
ALGORITHM CreateClip(channel_id, user_id, timestamp, duration)
  // Clips are 5-60 second excerpts from live or recent streams

  // Step 1: Rate limit (max 600 clips/minute per channel)
  IF rate_limiter.check(channel_id, "clip_create") > LIMIT:
    RETURN ERROR("Rate limit exceeded")

  // Step 2: Locate HLS segments covering the clip window
  stream ← active_streams.get(channel_id)
  IF stream IS NULL:
    // Check VOD for recent streams
    stream ← vod_store.get_recent(channel_id, within=24h)

  start_time ← timestamp - duration
  segments ← segment_index.range(stream.id, start_time, timestamp)

  // Step 3: Generate clip (server-side segment stitching)
  clip_id ← GENERATE_UUID()
  clip_video ← stitch_segments(segments, start_time, timestamp)

  // Step 4: Create thumbnail (extract frame at midpoint)
  thumbnail ← extract_frame(clip_video, offset=duration/2)
  ...
  clip_record ← {
    ...
    creator_id: user_id,
    stream_id: stream.id,
    title: "",  // User sets later
    duration: duration,
    view_count: 0,
    created_at: NOW(),
    video_url: upload_to_storage(clip_video),
    thumbnail_url: upload_to_storage(thumbnail)
  }
  database.insert("clips", clip_record)

  // Step 6: Dedup check (same channel + overlapping time window)
  similar_clips ← database.query(
    "SELECT * FROM clips WHERE channel_id = ? AND ABS(created_at - ?) < 5s",
    channel_id, timestamp
  )
  IF LEN(similar_clips) > 1:
    LOG_WARN("Potential duplicate clip", clip_id, similar_clips)

  RETURN clip_record
```

### 3.7 Subscription Lifecycle State Machine

```mermaid
stateDiagram-v2
    [*] --> Inactive: User visits channel

    Inactive --> PendingPayment: User clicks Subscribe
    PendingPayment --> Active: Payment confirmed
    PendingPayment --> Inactive: Payment failed

    Active --> GracePeriod: Renewal payment fails
    Active --> Cancelled: User cancels
    Active --> Gifted: Received gift sub

    GracePeriod --> Active: Retry payment succeeds
    GracePeriod --> Expired: Grace period (7 days) ends

    Cancelled --> Expired: Current period ends
    Cancelled --> Active: User re-subscribes

    Gifted --> Active: Gift period active
    Gifted --> Expired: Gift period ends

    Expired --> Inactive: Entitlements revoked
    Expired --> Active: User re-subscribes

    note right of Active: Entitlements:<br/>Ad-free, emotes,<br/>badge, sub-only chat
    note right of GracePeriod: Entitlements maintained<br/>during retry window
```
