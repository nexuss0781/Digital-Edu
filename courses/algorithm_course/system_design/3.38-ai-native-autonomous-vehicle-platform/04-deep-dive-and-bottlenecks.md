...
| Rank | Slowest part of the process | Current | Target | Impact | Solution |
|------|------------|---------|--------|--------|----------|
...
| 5 | **Memory Bandwidth** | 80% util | 60% util | Thermal, power | Feature compression, Cutting off unnecessary steps |

### Slowest part of the process 1: Camera CNN Inference
...
### Slowest part of the process 2: BEV Feature Generation
...
### Slowest part of the process 3: Trajectory Optimization
...
## Case Study: End-to-End Neural Architecture Migration (2024-2025)

### Context

A major autonomous vehicle platform transitioned from a modular perception-prediction-planning pipeline to an end-to-end neural architecture, where a single transformer model maps multi-camera inputs directly to trajectory outputs.
...
├────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Cameras → [BEV Encoder] → Detections → [Tracker] → Tracks         │
│                                                                      │
│  LiDAR → [PointPillars] → Detections ──┘                           │
│                                                                      │
│  Tracks → [Wayformer] → Predicted Trajectories                     │
│                                                                      │
│  Predictions + Map → [Lattice Planner] → Trajectory                │
│                                                                      │
│  Interface losses at every → arrow:                                  │
│  • BEV encoder discards depth uncertainty                           │
│  • Tracker loses appearance features                                 │
│  • Planner cannot reason about perception confidence                │
│                                                                      │
├────────────────────────────────────────────────────────────────────┤
│ END-TO-END (After)                                                   │
├────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  8 Cameras → [Multi-Camera Encoder → Temporal Fusion →              │
│               Scene Tokens → Planning Decoder] → Trajectory         │
│                                                                      │
│  Single loss function: imitation loss + safety constraints          │
│                                                                      │
│  Benefits:                                                           │
│  • No information Slowest part of the process between modules                        │
│  • Model learns which features matter for driving                   │
│  • Simpler deployment (one model vs. five)                          │
│                                                                      │
│  Challenges:                                                         │
│  • No intermediate representations for debugging                    │
│  • Harder to certify (ISO 26262 requires explainability)            │
│  • Requires 10-100x more training data                              │
│                                                                      │
└────────────────────────────────────────────────────────────────────┘
```
...
| Metric | Modular Pipeline | End-to-End Neural | Change |
|--------|------------------|-------------------|--------|
| Disengagement rate | 0.8 per 1000 mi | 0.3 per 1000 mi | 62% improvement |
| Narrow gap navigation | 45% success | 82% success | Shape features preserved |
| Unprotected left turns | 60% smooth | 78% smooth | Joint prediction-planning |
| Perception latency | 65ms | 45ms | Single forward pass |
| Safety envelope violations | 0.02% | 0.05% | More aggressive planning |
| Debugging time per incident | 2 hours | 8 hours | No intermediate representations |

### Key Lesson

The end-to-end model significantly improved driving performance but required a strengthened safety envelope (Insight 7) to compensate for the loss of modular interpretability. The safety monitor on a separate SoC (Insight 4) became even more critical as the primary planning system became a "black box" -- the independent safety check is the last line of auditable defense.

---

## Emerging Patterns (2025-2026)

### Vision-Language-Action Models (VLAs) for Driving

VLAs combine visual perception with natural language understanding and action generation, enabling vehicles to reason about complex driving scenarios:
...
│  Inputs:                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │ Camera Images │  │ Route Text   │  │ Traffic Rules│              │
│  │ (8 cameras)   │  │"Turn left at"│  │"Stop at red" │              │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘              │
│         │                  │                  │                      │
│         └──────────────────┼──────────────────┘                      │
│                            ▼                                         │
...
│         └────────┘ └────────┘ └────────┘                             │
│                                                                      │
│  "I'm slowing down because the pedestrian ahead is looking at       │
│   their phone and might step into the road"                          │
│                                                                      │
└────────────────────────────────────────────────────────────────────┘
```

**Impact:** VLAs provide natural language explanations of driving decisions, addressing the explainability gap in end-to-end models. This has direct implications for regulatory certification and post-incident analysis.

### World Models for Predictive Simulation

World models learn to predict future sensor observations from current actions, enabling the vehicle to "imagine" the consequences of its driving decisions:

| Aspect | Traditional Simulation | World Model Simulation |
|--------|----------------------|------------------------|
| Environment | Hand-crafted 3D assets | Learned from driving data |
| Realism | High but synthetic | Photorealistic, data-driven |
| Scenario diversity | Manual scenario design | Generative (infinite variations) |
| Compute | Hours per scenario | Milliseconds per rollout |
| On-vehicle use | Not feasible | Real-time "what-if" reasoning |

**Key innovation:** World models running on-vehicle enable real-time "what-if" planning: the planner imagines 10-50 future rollouts per planning cycle, evaluating candidate trajectories against predicted futures rather than simple kinematic models. This dramatically improves handling of complex interactive scenarios (merging, yielding, negotiating intersections) where other agents' responses to the ego vehicle's actions matter.
