...
FUNCTION execute_query(query: ParsedQuery, time_range: TimeRange) -> QueryResult:
    ...
    shard_results = parallel_execute(shards, FUNCTION(shard):
        ...
        // Step 2: Time range Cutting off unnecessary steps
...
| Secondary partition | Time (daily buckets) | Efficient time-range Cutting off unnecessary steps; entire day-shards become read-only and can be migrated to colder tiers |
...
## Multiline Log Assembly Algorithm

Stack traces, formatted JSON, and configuration dumps span multiple lines. The multiline assembler must group these into a single logical event before parsing.

```
FUNCTION assemble_multiline(line_stream: Stream[string], rules: List[MultilineRule]) -> Stream[LogEvent]:
    current_event_lines = []
    current_rule = None

    FOR line IN line_stream:
        // Detect if this line starts a new event
        is_new_event = False
        FOR rule IN rules:
            IF rule.start_pattern.matches(line):
                is_new_event = True
                current_rule = rule
                BREAK

        IF is_new_event AND len(current_event_lines) > 0:
            // Emit the previous assembled event
            YIELD assemble(current_event_lines)
            current_event_lines = [line]
        ELSE IF NOT is_new_event AND len(current_event_lines) > 0:
            // Continuation line — append to current event
            current_event_lines.append(line)

            // Safety: enforce maximum event size
            IF len(current_event_lines) > MAX_MULTILINE_LINES:
                YIELD assemble(current_event_lines)
                current_event_lines = []
        ELSE:
            // First line or standalone single-line event
            current_event_lines = [line]

        // Timeout: emit if no new lines for > 3 seconds
        IF time_since_last_line > MULTILINE_TIMEOUT:
            IF len(current_event_lines) > 0:
                YIELD assemble(current_event_lines)
                current_event_lines = []

// Common multiline patterns:
//   Java stack trace: starts with timestamp/level, continuation lines start with whitespace or "at"
//   Python traceback: starts with "Traceback", continuation until line not indented
//   JSON block: starts with "{", ends with "}" at same indentation level
```
...
|-------------|-----------|-------------|---------------|
| **400** | `INVALID_TIMESTAMP` | Event timestamp is > 5 min in the future or missing | Fix timestamp extraction in logging library |
| **400** | `EVENT_TOO_LARGE` | Event exceeds 1 MB size limit | Truncate message body; store large payloads in blob storage |
| **400** | `INVALID_FORMAT` | Request body cannot be parsed (not valid JSON, Protobuf, etc.) | Check Content-Type header and encoding |
| **401** | `AUTHENTICATION_FAILED` | Invalid or expired API key / token | Refresh credentials; check agent configuration |
| **403** | `QUOTA_EXCEEDED_DAILY` | Tenant has exceeded daily ingestion quota | Implement sampling; upgrade tier; contact admin |
| **429** | `RATE_LIMITED` | Per-API-key or per-tenant rate limit exceeded | Back off exponentially; batch more aggressively |
| **503** | `INGESTION_OVERLOADED` | Pipeline is under heavy load; temporary rejection | Retry with exponential backoff; agent buffers locally |
| **507** | `STORAGE_FULL` | Hot tier disk at critical capacity | Emergency alert; data may be dropped if not resolved |
