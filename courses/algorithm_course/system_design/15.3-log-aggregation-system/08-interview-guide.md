...
| 15-30 min | **Deep Dive** | Pick 1-2 critical components (indexing engine, storage tiering, or query execution) and go deep | Detailed data flow for chosen component; trade-off analysis; failure mode discussion; Step-by-step plan in plain English for core algorithm |
| 30-40 min | **Scale & Trade-offs** | Capacity math; Slowest part of the process identification; cost optimization; operational concerns | Capacity estimation table; top 3 bottlenecks with mitigations; cost analysis across indexing strategies |
...
| "Is there a chargeback model---do teams pay for their log usage?" | Cost awareness drives architecture decisions |
| "What percentage of logs are ever queried after ingestion?" | Justifies selective indexing; most logs are write-only |

---

## Evolution Path: What Changes at 10x and 100x Scale

| Dimension | Current (17 TB/day) | 10x (170 TB/day) | 100x (1.7 PB/day) |
|-----------|---------------------|-------------------|--------------------|
| **Indexing** | Hybrid (inverted hot, columnar cold) | Bloom-filter-primary; inverted only for ERROR streams | Full compute-storage separation; indexing is a batch job |
| **Queue** | Single cluster, 32 partitions | Federated clusters per data stream class | Regional queue clusters with cross-region mirroring |
| **Storage** | Tiered local + object | Object-storage-primary; local SSD as cache only | Multi-region object storage with intelligent caching |
| **Query** | Centralized query pool | Query-per-tier pools (hot, cold specialists) | Federated query across regional clusters |
| **Cost driver** | Storage (60% of TCO) | Storage (75%) + compute for bloom/scan | Network (cross-region) + storage + inference (NL query) |
...
### Deep Dive 1: The Ingestion-to-Searchable Pipeline (Latency Breakdown)

An interviewer may ask: "Walk me through exactly what happens between a log event being emitted and becoming searchable." This tests end-to-end system understanding.

```
Timeline of a single log event:

0 ms    : Application writes log to stdout
1-5 ms  : OTel Collector tails stdout, adds to in-memory batch
5-5000 ms: Collector batch fills (5MB or 5s timeout)
5001 ms : Batch sent to message queue (LZ4 compressed, gRPC)
5050 ms : Queue ACKs durable write (replicated across brokers)
5051 ms : Stream processor consumes batch
5060 ms : Parse format (JSON/logfmt detection)
5065 ms : Extract timestamp, normalize severity
5070 ms : Enrich with metadata (host, region, cluster)
5075 ms : PII redaction scan
5080 ms : Routing rule evaluation → select pipeline
5081 ms : Forward to index writer (assigned by shard hash)
5082 ms : Index writer appends to WAL
5100 ms : Event added to in-memory buffer
5100-10100 ms: Waiting for refresh cycle (1-5s)
10100 ms: Refresh: in-memory buffer → searchable segment
10100 ms: EVENT NOW SEARCHABLE (total: ~5-10 seconds)
```

### Deep Dive 2: Cost Optimization Strategies

If the interviewer asks "This system costs $2M/year---how would you cut it in half?", present this structured analysis:

| Optimization | Savings | Implementation Effort | Risk |
|-------------|---------|----------------------|------|
| Switch cold tier from inverted index to label-only | 30-40% of storage cost | Medium (reindex cold data) | Slower cold-tier full-text search |
| Implement log sampling (10% DEBUG, 50% INFO) | 40-60% of ingestion volume | Low (routing rule change) | Missing specific DEBUG entries during investigation |
| Drop health-check and heartbeat logs, convert to metrics | 10-20% of total volume | Low (collector-level filter) | None (metrics preserve signal) |
| Switch from SSD to HDD for warm tier | 50% of warm storage cost | Medium (node migration) | Slightly slower warm-tier queries |
| Increase compression level (LZ4 → ZSTD) for warm/cold | 20-30% of storage cost | Low (configuration change) | Slightly higher CPU for decompression |
| Implement per-tenant chargeback | Indirect (teams self-optimize) | Medium (cost attribution system) | Political (teams may resist) |

### Deep Dive 3: Multi-Region Log Aggregation

```
Scenario: Company has services in 3 regions (US-East, EU-West, AP-Southeast).
Question: How do you aggregate logs across regions?

Approach 1: Regional autonomy (recommended)
  - Each region has its own complete log pipeline
  - Local queries answered locally (low latency, data residency compliant)
  - Cross-region queries via federated query (fan out to all regions)
  - Pro: Data residency compliance; local query speed; region failure isolation
  - Con: Cross-region query latency; no single pane of glass (without federation)

Approach 2: Central aggregation
  - All logs shipped to a single central region
  - Pro: Simple; single cluster to manage; global search
  - Con: Cross-region bandwidth cost; data residency violations; single point of failure

Approach 3: Hybrid (federated with central analytics)
  - Real-time queries served by regional clusters
  - Sampled + aggregated logs replicated to central cluster for analytics
  - Pro: Best of both; data residency compliant; central analytics
  - Con: Complexity; different freshness for local vs. central data
```
...
| Dimension | Elasticsearch/ELK | Grafana Loki | CrowdStrike LogScale | ClickHouse | Splunk |
|-----------|-------------------|-------------|---------------------|------------|--------|
| **Indexing** | Full-text inverted | Label-only | Bloom filter + scan | Columnar + sparse | Proprietary (TSIDX) |
| **Storage backend** | Local disk (SSD/HDD) | Object storage | Local SSD + object | Local disk (MergeTree) | Local disk + SmartStore |
| **Compression** | 1.5-3x | 10-20x | 10-20x | 15-50x | 5-15x |
| **Search speed (full-text)** | Sub-second | Seconds-minutes | Sub-second | Moderate (1-5s) | Sub-second |
| **Aggregation speed** | Moderate | Slow | Moderate | Very fast | Fast |
| **Ingestion cost** | High (index overhead) | Very low | Low (bloom only) | Low (append-only) | High (proprietary) |
| **Operational complexity** | High (JVM tuning, merge storms) | Low (object storage) | Medium | Medium (MergeTree) | Medium (managed) |
| **Best for** | Interactive debugging | Cost-sensitive, label-heavy | Security analytics | Log analytics + BI | Enterprise SIEM |
| **TCO at 10 TB/day** | $$$$ | $ | $$ | $-$$ | $$$$$ |
...
|-------------|-------------|-----------------|
| **Index everything, retain everything** | Costs explode exponentially; most indexed data is never queried | Selective indexing by severity + access pattern; tiered retention |
| **Single indexing strategy for all tiers** | Full-text on cold tier wastes storage; label-only on hot tier frustrates engineers | Hybrid: inverted index for hot, columnar/bloom for cold |
| **Synchronous parsing in the agent** | CPU-intensive parsing on every application host; steals compute from the application | Push raw logs to queue; centralize parsing in dedicated processor pool |
| **No backpressure mechanism** | When indexers slow down, the entire pipeline backs up and agents drop logs | Message queue as buffer; explicit backpressure signals at each layer |
| **The log system logs to itself** | Positive feedback loop during failures; amplifies problems | Meta-monitoring stack: emit metrics (not logs) to independent monitoring |
| **Using the same refresh interval for all data** | 1-second refresh for DEBUG logs wastes I/O; 30-second refresh for ERROR logs delays incident response | Adaptive refresh by severity: fast for ERROR, slow for DEBUG |
| **Ignoring the incident-load correlation** | Provisioning for average load means the system fails when most needed | Pre-provision 30% burst headroom; priority-based degradation plan |
| **Skipping PII redaction "for now"** | Once PII is indexed, it cannot be un-indexed; compliance debt compounds | PII redaction in critical path from day 1; fail-closed gate |
...
|----------|--------------|-------------|
| **30 minutes** | Requirements → pipeline architecture → one indexing deep dive → capacity estimate | Multi-region, advanced degradation, cost optimization, PII pipeline |
| **45 minutes** | Requirements → full architecture → indexing trade-offs → capacity → reliability + degradation | Multi-region, advanced pattern clustering, detailed cost model |
| **60 minutes** | Everything above + multi-region strategy + cost optimization + PII/compliance + meta-observability | None—cover everything; explore edge cases in depth |

---

## Whiteboard Sketch Sequence

When drawing this system on a whiteboard, follow this sequence for maximum clarity:

1. **Start with the write path** (left to right): Sources → Collectors → Queue → Processors → Indexers → Storage
2. **Add the storage tiers** (vertical stack): Hot → Warm → Cold → Frozen with lifecycle arrows
3. **Add the read path** (right side): Query Frontend → Query Executors → Storage (fan-out arrows)
4. **Add the buffer detail**: Show the queue as the reliability backbone with retention and replay
5. **Add PII inline**: Mark the PII redaction between processor and indexer as a fail-closed gate
6. **Add the meta-monitoring**: Separate box below the main pipeline, receiving metrics (not logs)
7. **Add capacity numbers**: Write key numbers on the diagram (events/s at each stage, storage sizes per tier)
...
flowchart LR
    subgraph External["External Boundary"]
        Apps["5,000 Microservices"]
        Infra["Infrastructure"]
        Security["Security Systems"]
    end

    subgraph LogPlatform["Log Aggregation Platform"]
        direction TB
        Collect["Collection"]
        Buffer["Buffer (72h)"]
        Process["Process + PII"]
    ...
    subgraph Consumers["Consumers"]
        ...
        SIEM["SIEM (downstream)"]
    end

    Apps & Infra & Security --> Collect
    Collect --> Buffer --> Process --> Index
    Index --> Query
    Query --> Engineers & Dashboards & Alerts & SIEM
    ...
    class Apps,Infra,Security external
    class Collect,Buffer,Process,Index,Query platform
    class Engineers,Dashboards,Alerts,SIEM consumer
```
