...
Each layer of the log aggregation pipeline scales independently based on its specific Slowest part of the process:
...
```
FUNCTION verify_data_integrity(shard: Shard) -> IntegrityReport:
    // Periodic verification (daily for hot, weekly for warm, monthly for cold)

    // Step 1: Verify segment checksums
    FOR segment IN shard.segments:
        stored_checksum = segment.metadata.checksum
        computed_checksum = compute_checksum(segment.data)
        IF stored_checksum != computed_checksum:
            report.add_corruption(segment.id, "data checksum mismatch")

    // Step 2: Verify index consistency
    // Sample 1% of indexed terms; verify posting lists point to valid documents
    sample_terms = random_sample(segment.term_dictionary, rate=0.01)
    FOR term IN sample_terms:
        posting_list = get_posting_list(term)
        FOR doc_id IN posting_list:
            IF NOT document_exists(doc_id):
                report.add_corruption(segment.id, "dangling posting list entry")

    // Step 3: Verify event count consistency
    // Queue committed offset vs. indexed document count (should match within tolerance)
    expected_count = queue_committed_offset - previous_committed_offset
    actual_count = count_documents(shard, time_range)
    IF abs(expected_count - actual_count) / expected_count > 0.001:
        report.add_warning("document count drift: expected={expected}, actual={actual}")
...
```
FUNCTION calculate_cluster_capacity(requirements: Requirements) -> ClusterSpec:
    // Ingestion capacity
    peak_events_per_sec = requirements.avg_events_per_sec * requirements.peak_factor
    indexer_count = ceil(peak_events_per_sec / EVENTS_PER_INDEXER)  // ~100K events/s per indexer
    processor_count = ceil(peak_events_per_sec / EVENTS_PER_PROCESSOR)  // ~50K/s per processor
    broker_count = max(3, ceil(requirements.avg_bytes_per_sec / BYTES_PER_BROKER))  // ~200 MB/s per broker

    // Storage capacity
    daily_compressed_bytes = requirements.daily_raw_bytes / requirements.compression_ratio
    hot_tier_bytes = daily_compressed_bytes * requirements.hot_retention_days
    warm_tier_bytes = daily_compressed_bytes * requirements.warm_retention_days
    cold_tier_bytes = daily_compressed_bytes * requirements.cold_retention_days

    // Index overhead (varies by strategy)
    index_overhead_factor = get_index_overhead(requirements.indexing_strategy)
    // Full-text: 1.5-3x, Label-only: 0.1-0.15x, Bloom: 0.05-0.1x
    hot_storage_total = hot_tier_bytes * (1 + index_overhead_factor) * requirements.replication_factor

    hot_storage_nodes = ceil(hot_storage_total / BYTES_PER_STORAGE_NODE)
    query_executor_count = ceil(requirements.peak_concurrent_queries / QUERIES_PER_EXECUTOR)

    RETURN ClusterSpec(
        brokers = broker_count,
        processors = processor_count,
        indexers = indexer_count,
        hot_storage_nodes = hot_storage_nodes,
        query_executors = query_executor_count,
        warm_storage_bytes = warm_tier_bytes,
        cold_storage_bytes = cold_tier_bytes
    )
```

### Scaling Decision Matrix

| Metric | Threshold | Scaling Action | Lead Time |
|--------|-----------|---------------|-----------|
| Consumer lag > 5 min sustained | Warning | Auto-scale processors + indexers | 2-5 min (container startup) |
| Hot tier disk > 80% | Warning | Add hot storage nodes OR accelerate tier transitions | 10-30 min (node provisioning) |
| Query p99 > 5s sustained | Warning | Add query executors | 2-5 min (stateless scale) |
| Ingestion events/s > 80% capacity | Warning | Pre-scale indexers before capacity is reached | 5-10 min |
| Agent drops > 0 sustained | Critical | Emergency scale all layers; increase queue retention | Immediate (all hands) |

---

## Deployment Strategy: Rolling Blue-Green Per Layer

```
Deployment sequence (zero-downtime):

Phase 1: Deploy new query executors (stateless, instant)
  - Start new executor instances with updated code
  - Health check passes → add to load balancer pool
  - Drain old executors → remove from pool → terminate

Phase 2: Deploy new stream processors (stateless, instant)
  - Same as query executors; consumer group rebalances automatically

Phase 3: Deploy new indexers (stateful, careful)
  - For each indexer instance:
    1. Pause consumption on owned partitions
    2. Flush in-memory buffer to storage
    3. Sync WAL to disk
    4. Commit consumer offsets
    5. Restart with new code
    6. Replay WAL (catch up missed events)
    7. Resume consumption
  - Rolling: one indexer at a time; queue absorbs the pause

Phase 4: Queue broker rolling restart
  - For each broker:
    1. Migrate partition leaders to other brokers
    2. Restart broker
    3. Reassign partitions back
  - ISR ensures no data loss during broker restart

Total deployment time: ~30-60 minutes for full fleet
Queue buffer provides tolerance for indexer pauses.
```
...
stateDiagram-v2
    ...
    Ingested --> InMemory : Buffered in index writer
    InMemory --> HotTier : Refresh cycle (1-5s)
    HotTier --> WarmTier : Age > 7 days
    WarmTier --> ColdTier : Age > 30 days
    ColdTier --> FrozenTier : Age > 90 days
    FrozenTier --> Deleted : Retention expired
    Deleted --> [*]

    HotTier --> Deleted : Debug retention (7 days)
    WarmTier --> Deleted : Short retention (30 days)

    state HotTier {
        [*] --> ActiveSegment : New segment created
        ActiveSegment --> MergedSegment : Segment merge
        MergedSegment --> ReadOnly : Force-merge complete
    }

    state ColdTier {
        [*] --> SearchableSnapshot : Upload to object storage
        SearchableSnapshot --> CachedBlock : On query (block fetched)
    }

    state FrozenTier {
        [*] --> Archived : Maximum compression
        Archived --> Rehydrated : On-demand request
        Rehydrated --> Archived : TTL expired (7 days)
    }
```
