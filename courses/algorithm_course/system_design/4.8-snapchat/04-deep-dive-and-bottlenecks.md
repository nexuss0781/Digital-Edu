...
### Slowest part of the process 1: AR Inference on Low-End Devices
...
### Slowest part of the process 2: Deletion Backlog During Peak
...
### Slowest part of the process 3: Celebrity Story Fan-Out
...
### Slowest part of the process 4: Snap Map Location Updates
...
Key Slowest part of the process mitigations:
...
## Critical Component 5: My AI Safety Pipeline

### Why This Is Critical

Snapchat's My AI (LLM-powered chatbot) serves a user base where ~40% are under 25 and a significant portion are minors. Every AI response must pass through safety guardrails before reaching the user.

### Safety Pipeline Architecture

```
MY AI SAFETY PIPELINE
======================
...
User Message
    ...
    ▼
┌──────────────────────────┐
│ Input Classifier          │  Detect harmful intent, PII, crisis signals
│ (Pre-LLM filter)          │  Latency: ~20ms
└──────────┬───────────────┘
           │
           ▼
┌──────────────────────────┐
│ LLM Inference             │  Generate response
│ (GPT-powered)             │  Latency: ~300-500ms
└──────────┬───────────────┘
           │
           ▼
┌──────────────────────────┐
│ Output Safety Classifier  │  Block policy violations
│ (Post-LLM filter)         │  Latency: ~50ms
└──────────┬───────────────┘
           │
           ▼
┌──────────────────────────┐
│ Age-Appropriate Filter    │  Additional restrictions for <18 users
│ (Per-user policy)         │  Latency: ~10ms
└──────────┬───────────────┘
           │
           ▼
User Response
...
CRISIS DETECTION:
- Self-harm mentions → Immediate helpline resources (bypass LLM)
- Abuse reports → Flag for human review
- Predatory patterns → Account safety team notification

CONVERSATION MEMORY:
- Limited context window (last 20 messages)
- No persistent memory across sessions
- User can clear history anytime
- PII auto-detected and stripped from context
```
...
|--------|--------|-------------|
| **Policy violation rate** | <0.01% | Violations reaching user / total responses |
| **False positive rate** | <5% | Benign messages blocked / total |
| **Crisis detection recall** | >99% | Detected crises / total crisis signals |
| **Safety latency overhead** | <100ms | Pre + post filter time |
| **Age filter accuracy** | >99.5% | Correctly applied age restrictions |
