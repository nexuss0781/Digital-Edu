...
### Scenario 1: "Walk me through what happens when a deal closes in CRM"

```
1. Sales rep clicks "Won" on deal in CRM UI
2. CRM writes deal status = "won" to OLTP DB (optimistic lock check)
3. CRM publishes "deal.closed" event to event bus with deal data
4. Event bus assigns sequence number and routes to subscription registry
5. Registry returns subscribers: [Books, Projects, Cliq, Analytics]

6. Saga orchestrator sequences dependent steps:
   Step 1 (Books): Create invoice from deal data
     - Maps deal fields → invoice fields via integration mapping
     - Books validates amount, tax, customer exists
     - Returns invoice_id (INV-4521)
   Step 2 (Projects): Create project referencing invoice
     - Receives invoice_id from Step 1
     - Creates project with deal milestones
     - Returns project_id (P-8901)
   Parallel (Cliq): Send notification to sales team channel
   Parallel (Analytics): Record deal-won event for dashboard

7. If Step 1 fails: retry 3x with exponential backoff
   If permanently fails: saga compensates (revert deal status)
8. Audit log records full saga trace with timestamps
9. Search index updated within 5 seconds across all products
```

### Scenario 2: "How would you migrate a large tenant to a dedicated shard?"

```
Migration sequence (zero-downtime):

1. Assessment: Tenant identified as "large" (>500K records, >20% shard QPS)
2. Provision: New dedicated shard provisioned with same schema
3. Initial sync: Full data copy from shared shard → dedicated shard
   - Background job reads with snapshot isolation
   - No impact on live traffic (reads from shared shard)
4. Catch-up: Continuous CDC (change data capture) replicates delta changes
5. Switchover preparation:
   - Pause writes for this tenant (brief: <500ms)
   - Drain remaining CDC events
   - Verify record counts match between source and target
6. DNS/routing update: Region router updated to point org_id → new shard
7. Resume writes: New writes go to dedicated shard
8. Validation: Run integrity checks for 24 hours
9. Cleanup: Remove tenant data from shared shard after validation period

Duration: ~2-4 hours for 1M-record tenant
User impact: <1 second write pause during switchover
```

### Scenario 3: "Design the Zia agent skill matching system"

```
User says: "Send an email to all customers with overdue invoices"

1. Intent classification: Zia agent classifies intent as "bulk_action"
2. Skill matching against 700+ registered skills:
   - Match: "email.send_bulk" (Zoho Mail skill)
   - Match: "books.query_overdue" (Zoho Books skill)
   - Combined plan: query → filter → email

3. Permission check: Does user have:
   - Books: read access to invoices? ✓
   - Mail: send permission? ✓
   - Bulk action: within daily limit? ✓

4. Plan generation:
   Step 1: Query overdue invoices from Books
   Step 2: Extract customer emails from results
   Step 3: Generate email content (Zia LLM)
   Step 4: Send bulk email via Mail (HIGH RISK → requires confirmation)

5. User confirmation: "This will email 47 customers. Proceed?"
6. Execute with audit trail logging each step
7. Report: "47 emails sent successfully, 2 bounced"
```
...
| Related Question | How Zoho Suite Knowledge Helps |
|---|---|
| "Design Salesforce" | Very similar — multi-tenant CRM + platform (Force.com vs AppOS) |
| "Design a multi-tenant SaaS platform" | Direct overlap with tenant isolation, governor limits, sharding |
| "Design an ERP system" | Zoho Books + People covers core ERP (finance + HR) |
| "Design a workflow automation engine" | Zoho Flow + Deluge scripting directly applicable |
| "Design a low-code platform" | Zoho Creator architecture maps to this |
| "Design an AI assistant for enterprise" | Zia agent orchestration directly applicable |
| "Design a customer support platform" | Zoho Desk architecture is a component |
| "Design a unified search system" | SearchIQ cross-product search directly applicable |
