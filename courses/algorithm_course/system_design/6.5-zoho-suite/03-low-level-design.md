...
// Time Complexity (Speed of the algorithm): O(S) where S = number of subscriptions for this event
...
// Time Complexity (Speed of the algorithm): O(1) for shard resolution (consistent hashing)
...
// Time Complexity (Speed of the algorithm): O(N log N) for ranking, where N = total hits across all products
