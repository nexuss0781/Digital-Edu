...
```
SLO budget tracking (rolling 30-day window):

Search availability SLO: 99.9% (budget: 43.2 minutes downtime/month)
├── Spent this month:     18.5 minutes (42.8% of budget)
├── Burn rate (current):  0.6 minutes/day (healthy)
├── Projected month-end:  18.5 + (0.6 × 12 remaining days) = 25.7 minutes → SAFE
└── Alert: if projected > 38 minutes (88% budget) → warn; > 43 minutes → critical

Booking availability SLO: 99.99% (budget: 4.3 minutes downtime/month)
├── Spent this month:     1.2 minutes (27.9% of budget)
├── Burn rate:            0.07 minutes/day
└── Status: SAFE

Search latency SLO: p99 < 3s for 99% of 5-min windows
├── Compliant windows this month:  8,640 / 8,640 (100%)
├── Budget: 86 non-compliant windows allowed
├── Last violation:       3 days ago (GDS timeout cascade, 8 windows)
└── Status: SAFE
```
...
1. Freeze all non-critical deployments for 48 hours
2. All engineering effort redirected to reliability improvements
3. Post-mortem required for the SLO breach
4. Resume normal deployment velocity only after:
   ├── Root cause identified and remediated
   ├── Error budget replenished to > 20%
   └── Reliability improvement deployed (e.g., better circuit breaker tuning)
```
...
| Metric | Description | Granularity | Target |
|--------|-------------|-------------|--------|
| `revenue.gross.daily` | Total booking revenue | Daily, by region | Monitoring (trend) |
| `revenue.per_booking.avg` | Average booking value | Hourly | > $800 |
| `revenue.ancillary.attach_rate` | % bookings with ancillary purchases | Daily | > 35% |
| `revenue.ancillary.revenue_per_booking` | Average ancillary revenue per booking | Daily | > $45 |
| `conversion.search_to_book` | End-to-end conversion rate | Hourly | > 1% |
| `conversion.by_channel` | Conversion split: web / mobile / agent | Daily | Monitor parity |
| `abandonment.at_payment` | Users who start but don't complete payment | Hourly | < 25% |
| `abandonment.at_hold` | Users who hold but don't pay | Hourly | < 30% |
| `price_change.modal_shown_rate` | % bookings where price changed between search and hold | Hourly | < 8% |
| `price_change.acceptance_rate` | % users who accept the new price | Hourly | > 60% |

### Fare Prediction Model Metrics

| Metric | Description | Target | Alert |
|--------|-------------|--------|-------|
| `fare_prediction.accuracy` | % correct direction predictions (7-day window) | > 70% | < 60% |
| `fare_prediction.calibration` | Predicted probability vs. actual frequency | Within 5% | > 10% gap |
| `fare_prediction.user_engagement` | % searches where prediction is displayed | Monitoring | Sudden drop |
| `fare_prediction.influence_on_conversion` | Conversion rate lift when prediction shown | > 5% lift | Negative lift |
| `fare_prediction.regret_rate` | % users who bought on "buy now" and price dropped within 48h | < 15% | > 25% |

### NDC vs. GDS Distribution Dashboard

```
Channel distribution (real-time):

Search sources:
├── GDS (Amadeus):     35% of results
├── GDS (Sabre):       28% of results
├── GDS (Travelport):  22% of results
├── NDC (airline direct): 12% of results
└── LCC portals:        3% of results

Booking sources:
├── GDS total:         78% of bookings (declining ~2%/quarter)
├── NDC direct:        18% of bookings (growing ~2%/quarter)
└── LCC portals:        4% of bookings

NDC health:
├── BA NDC: ✅ healthy (p50: 450ms, success: 99.2%)
├── LH NDC: ✅ healthy (p50: 620ms, success: 98.8%)
├── AA NDC: ⚠️ degraded (p50: 1.8s, success: 94.1%) — investigating
├── AF NDC: ✅ healthy (p50: 550ms, success: 99.0%)
└── SQ NDC: ❌ down (circuit open since 14:22 UTC) — fallback to GDS

Cost per booking by channel:
├── GDS average:     $3.75 per booking (GDS fees + API costs)
├── NDC average:     $0.85 per booking (API costs only)
└── Monthly savings from NDC shift: $52K (growing as NDC volume increases)
```
...
### Scenario 1: GDS Provider Latency Spike

```
Detection:  gds.{provider}.latency.p99 > 3s for 5 minutes
Impact:     Search latency degradation, reduced results from affected provider

Steps:
1. Verify: check provider status page, confirm not a local network issue
2. Assess: is it one provider or multiple? (if multiple → possible network issue)
3. Mitigate:
   ├── If single provider: circuit breaker should auto-open; verify it did
   ├── Extend cache TTL from 3 min → 5 min (reduce load on remaining providers)
   └── If critical (Amadeus): consider raising timeout from 3s → 5s temporarily
4. Communicate: update internal status page
5. Resolve: monitor until provider latency normalizes, circuit closes
6. Post-incident: log duration, affected bookings, cost impact
```

### Scenario 2: Hold Expiry Backlog Growing

```
Detection:  inventory.hold_release.pending_count > 500
Impact:     Phantom "sold out" seats — inventory shows unavailable but holds have expired

Steps:
1. Verify: check hold release worker health — is it running?
2. Assess: count of expired holds, rate of growth
3. Mitigate:
   ├── If worker crashed: restart immediately
   ├── If GDS cancel calls failing: check GDS connectivity
   ├── If volume spike: scale hold release workers (increase batch size 100 → 500)
   └── Emergency: run manual batch release with FOR UPDATE SKIP LOCKED
4. Impact: users may see "sold out" on flights that actually have seats
5. Resolve: backlog cleared, verify inventory counts match GDS
```
