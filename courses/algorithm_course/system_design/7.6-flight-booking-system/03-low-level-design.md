...
FUNCTION predictFareTrend(origin, destination, departureDate, cabin):
    // ML-based fare direction prediction
    features = buildFeatures(origin, destination, departureDate, cabin)

    // Feature vector
    features.daysUntilDeparture = (departureDate - today()).toDays()
    features.dayOfWeek = departureDate.dayOfWeek()
    features.isHoliday = holidayCalendar.check(departureDate, origin, destination)
    features.currentLoadFactor = getAvgLoadFactor(origin, destination, departureDate)
    features.historicalPriceSlope = get7DayPriceSlope(origin, destination, departureDate)
    features.competitorDelta = getCurrentVsCompetitorPrice(origin, destination, departureDate)
    features.seasonalIndex = getSeasonalIndex(origin, destination, departureDate.month)
    features.routePopularity = getRouteSearchVolume(origin, destination, 7) // last 7 days

    // Predict direction (UP / DOWN / STABLE)
    prediction = farePredictionModel.predict(features)

    // Calibrate confidence
    confidence = plattScaling(prediction.rawScore)

    IF confidence < 0.60:
        RETURN {direction: "STABLE", confidence: confidence, recommendation: "NEUTRAL"}
    ELSE IF prediction.direction == "UP" AND confidence >= 0.75:
        RETURN {direction: "UP", confidence: confidence, recommendation: "BUY_NOW"}
    ELSE IF prediction.direction == "DOWN" AND confidence >= 0.75:
        RETURN {direction: "DOWN", confidence: confidence, recommendation: "WAIT"}
    ELSE:
        RETURN {direction: prediction.direction, confidence: confidence, recommendation: "NEUTRAL"}
```
