...
### Trade-Off 1: GDS Breadth vs. NDC Depth

```
Dimension:     Coverage (400+ airlines via GDS) vs. Content richness (NDC direct)

GDS-heavy (current):
├── Pros: single integration for all airlines, mature tooling, reliable
├── Cons: $0.50-2.00 per call, limited content (no images, basic bundles),
│         airlines pushing surcharges on GDS-distributed bookings
└── When: broad market coverage is priority, engineering capacity is limited

NDC-heavy (future):
├── Pros: 7.5× lower API cost, richer offers, dynamic bundles, airline-preferred
├── Cons: per-airline integration (70+ APIs), inconsistent quality, coverage gaps
└── When: major carrier coverage is sufficient, cost reduction is strategic priority

Recommended: hybrid with progressive NDC migration
├── Start: GDS for all, NDC for top 10 airlines (70% of volume)
├── Year 2: NDC for top 30 airlines, GDS fallback only
├── Year 3+: GDS as reliability backstop, NDC as primary channel
└── Never fully drop GDS: long-tail airlines, interline ticketing, complex itineraries
```

### Trade-Off 2: Search Freshness vs. GDS Cost

```
Dimension:     Accurate real-time pricing vs. API cost control

Fresh (1-min cache, 40% hit rate):
├── GDS calls: 60M × 5 × $0.75 = $225M/day
├── Price-change rate at booking: ~2%
└── User trust: very high (what they see is almost always what they pay)

Balanced (3-min cache, 80% hit rate):
├── GDS calls: 20M × 5 × $0.75 = $75M/day
├── Price-change rate at booking: ~8%
└── User trust: good (occasional "price changed" modal)

Aggressive (10-min cache, 92% hit rate):
├── GDS calls: 8M × 5 × $0.75 = $30M/day
├── Price-change rate at booking: ~20%
└── User trust: degraded (frequent "price changed" → user frustration → abandonment)

Sweet spot: 3-minute TTL
├── Best ratio of cost savings to user experience
├── Re-verification at hold time catches the 8% price changes
└── Most users browsing (not booking) get fast, cheap results
```

### Trade-Off 3: Seat Hold Duration vs. Inventory Utilization

```
Dimension:     Longer hold (better checkout UX) vs. shorter hold (less inventory lockup)

Short hold (5 min):
├── Pros: seats return quickly if user abandons, higher utilization
├── Cons: rushed checkout, higher abandonment, worse conversion
└── Risk: users feel pressured → negative brand impact

Standard hold (15 min):
├── Pros: industry standard, sufficient for payment form + 3DS
├── Cons: bots can lock inventory for 15 min per hold
└── Mitigation: progressive TTL for repeat holders

Extended hold (30-60 min):
├── Pros: relaxed checkout, supports corporate approval workflows
├── Cons: significant inventory lockup, denial-of-inventory risk
└── When: corporate/agent bookings only, with higher trust threshold

Recommendation: 15 min default, 10 min for repeat-no-pay users, 30 min for agents
```
...
When drawing on a whiteboard, hit these components in order:

```
1. Client → API Gateway → BFF (1 minute)
2. Search path: Search Service → [GDS fan-out] → Cache (3 minutes)
   ├── Draw the fan-out: parallel calls to 5 providers
   ├── Show cache layers: L1 in-process, L2 Redis
   └── Show circuit breaker per provider
3. Booking path: Orchestrator → [Hold → Pay → Ticket] saga (3 minutes)
   ├── Show two-phase: hold creates PNR, then payment + ticketing
   ├── Show compensating transactions on each step
   └── Show outbox pattern for payment-captured-but-not-ticketed
4. Data stores: PostgreSQL, Redis, Kafka, Object Storage (2 minutes)
5. External systems: GDS boxes, Payment Gateway (1 minute)
```
...
### Scenario 1: "A major airline just launched a 50% off flash sale. Traffic is 10× normal."

**Strong answer:** "This is a cache stampede plus capacity surge scenario. First, I'd rely on the autoscaler to add search instances (pre-warm pool helps). Second, the flash sale routes will all miss cache simultaneously — the lock-based stampede prevention kicks in, so only 1 request per route hits GDS. Third, I'd temporarily extend cache TTL from 3 to 5 minutes for the sale routes to reduce GDS load. Fourth, I'd prioritize booking GDS calls over search calls (search can serve from cache, bookings need real-time GDS). The risk is GDS rate limit throttling — if that happens, we serve cached results with a 'prices may vary' banner."

### Scenario 2: "A user reports they were charged but never received a ticket."

**Strong answer:** "This is the most critical failure mode — the payment-captured-but-not-ticketed state. I'd first check the saga state table: what step is the booking stuck at? If ISSUE_TICKET failed, the outbox pattern should have it queued for retry. I'd check the retry log — did GDS return an error? Common causes: GDS timeout during issuance, fare class closed between hold and ticket. If retries exhausted, the booking is in the manual operations queue. The ops team can issue via GDS terminal. The user should see 'booking confirmed, ticket processing' status. We never auto-refund in this state because the seat is still held. If the hold has expired AND manual ticketing failed, then we process a refund with an apology."

### Scenario 3: "We're seeing a 40% increase in chargeback rates this month."

**Strong answer:** "That's a serious signal. I'd investigate in layers: (1) Check if chargebacks are concentrated on specific routes — one-way international flights departing within 48 hours are classic fraud patterns. (2) Check if the ML fraud model's feature distributions have shifted — if attackers changed tactics, the model may not be catching new patterns. (3) Check if 3DS2 adoption changed — did a gateway update reduce 3DS challenges? (4) Look for card testing patterns — rapid small transactions followed by large bookings. Immediate mitigations: lower the fraud score threshold for manual review (from 66 to 50), require 3DS2 for all international one-way bookings, add velocity limits on new accounts. Long-term: retrain the fraud model on the new chargeback data."
...
| **AI fare prediction** | Gradient boosted models predicting price direction; features: load factor trajectory, seasonal signals, competitor delta; advisory only, not guaranteed |
| **ONE Order migration** | IATA's single order record replacing PNR + EMD + E-ticket; simplifies data model but requires industry-wide migration; OTAs must support both during transition |
| **Sustainability / carbon** | ICAO CO₂ calculator methodology; per-aircraft-type emission factors; EU SAF mandate (2025); carbon offset as ancillary product |
| **Conversational search** | LLM extracts intent → structured search API → AI re-ranks results; NL is input layer, not data source; no hallucinated flights |

---

## ADR Template: Key Architecture Decision Records

### ADR-001: GDS as Authoritative Inventory Source

**Status:** Accepted

**Context:** The platform needs to display seat availability and process bookings. We could either sync all airline inventory into our database (internal authority) or query the GDS in real-time (external authority).

**Decision:** GDS/CRS systems remain the single source of truth for seat availability. Our local database maintains a cached mirror for fast search, but all booking operations go through the GDS.

**Consequences:**
- Every booking operation depends on GDS availability (external dependency risk)
- Cache staleness is accepted for search (3-min TTL) but re-verified at booking
- We cannot guarantee seat availability shown in search results
- GDS per-call cost drives aggressive caching strategy
...
|---------|-------------|-----------------|
| **Building a real-time inventory sync engine** | 400+ airlines × 100K flights × 26 classes = impossible to sync | Cache-first with on-demand verification; GDS is authoritative |
| **Same cache TTL for all routes** | JFK→LHR changes every minute; BNA→TYS barely changes daily | Adaptive TTL based on route popularity and fare volatility |
| **Treating NDC as a GDS replacement** | NDC coverage is 70 airlines; GDS covers 400+ | Hybrid: NDC for depth (major carriers), GDS for breadth |
| **Single payment gateway** | Gateway outages block all bookings | Primary + failover gateway; automatic switch on 3 consecutive failures |
| **Flat rate limiting for all users** | Travel agents need 10× the rate of regular users | Tiered rate limiting by user role, with token bucket per API key |
...
|--------|-------|---------------|
| Search-to-book ratio | 100:1 | Optimize 99% for search performance |
| GDS API latency | 500ms-2s | Floor for search response time; drives caching |
| GDS API cost | $0.50-2.00/call | Caching saves hundreds of millions per day |
| Seat hold TTL | 15 minutes | Industry standard, GDS-enforced |
| Cache hit rate target | 80%+ | Economic break-even for GDS costs |
| Fare classes per flight | Up to 26 | Domain complexity driver |
| ATPCO rule categories | 31 | Fare rules engine complexity |
| PNR mandatory fields | 5 (PRINT) | Phone, Received-from, Itinerary, Name, Ticketing |
| GDS market share | ~97% | Amadeus + Sabre + Travelport combined |
| NDC airlines | 70+ | Growing, major carriers first |
