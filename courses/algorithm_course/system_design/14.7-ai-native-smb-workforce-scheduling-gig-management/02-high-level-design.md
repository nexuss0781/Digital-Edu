...
## Data Flow: Clock-In Verification
...
---
sequenceDiagram
    participant W as Employee App
    participant API as API Gateway
    participant ATT as Attendance Service
    participant GEO as Geofence Verifier
    participant BIO as Biometric Verifier
    participant SPOOF as Spoof Detector
    participant DB as Schedule Database
    participant TS as Timesheet Service
    participant N as Notification

    W->>API: Clock-in (GPS, device data, biometric token)
    API->>ATT: Verify clock-in

    par Synchronous checks
        ATT->>DB: Get shift details + location geofence
        DB-->>ATT: Shift + location data
        ATT->>GEO: Geofence check (lat, lon, radius)
        GEO-->>ATT: Pass/fail + distance
    end

    alt Geofence failed
        ATT-->>API: Rejected (outside geofence)
        API-->>W: Clock-in rejected
    else Geofence passed
        ATT-->>API: Accepted (GPS verified)
        API-->>W: Clock-in accepted ✓

        par Async enrichment
            ATT->>BIO: Verify biometric (if enabled)
            BIO-->>ATT: Match score
            ATT->>SPOOF: Multi-signal analysis
            SPOOF-->>ATT: Spoof probability
        end

        ATT->>TS: Create timesheet entry
        ATT->>DB: Store clock event

        alt Async checks flagged
            ATT->>N: Alert manager (suspicious clock-in)
        end
    end
```
...
sequenceDiagram
    ...
    participant API as API Gateway
    participant S as Schedule Service
    participant GIG as Gig Matching
    participant N as Notification
    participant GW as Gig Workers (Pool)
    participant C as Compliance Engine

    M->>API: Employee called off (shift_id)
    API->>S: Mark shift as open
    S->>S: Attempt internal rearrangement
    Note over S: Check if existing employees<br/>can cover without overtime

    alt Internal coverage found
        S->>C: Validate rearrangement
        C-->>S: Compliance check passed
        S->>N: Notify affected employees
    else No internal coverage
        S->>GIG: Broadcast open shift
        GIG->>GIG: Score & rank candidates

        GIG->>N: Tier 1 — notify top 5 workers
        N->>GW: "Shift available: Today 2-10 PM, $22/hr"

        Note over GIG: Wait 10 minutes

        alt Tier 1 fills shift
            GW->>API: Accept shift
            API->>GIG: Process acceptance
            GIG->>C: Validate gig assignment
            C-->>GIG: Compliance check passed
            GIG->>S: Update schedule with gig worker
            GIG->>N: Confirm to manager + gig worker
        else Tier 1 unfilled
            GIG->>N: Tier 2 — notify next 10 workers
            Note over GIG: Wider radius, same rate
        end
    end
```
...
The schedule lifecycle follows a strict state machine with compliance gates at key transitions:
...
---
stateDiagram-v2
    [*] --> Draft: Generate/Create
    Draft --> Validating: Submit for validation
    Validating --> Draft: Validation failed\n(violations found)
    Validating --> Approved: All checks passed
    Approved --> Published: Manager approves\n+ compliance sign-off
    Published --> Active: Week starts
    Active --> Active: Mid-week modifications\n(re-validates each change)
    Active --> Completed: Week ends
    Completed --> Archived: Retention period\n(7 years for compliance)
    Published --> Published: Shift swap executed\n(new version created)
    Active --> Active: Gig worker assigned\n(new version created)
    ...
    note right of Published
        Every modification after publication:
        - Creates new schedule version
        - Calculates premium pay impact
        - Immutable event in audit log
    end note
```
...
### POS Integration Pipeline

The demand forecasting engine depends on POS data quality. The integration layer handles diverse POS systems through an adapter pattern:

| Integration Layer | Responsibility |
|---|---|
| **Adapter per POS system** | Translates POS-specific webhook formats into a normalized transaction event schema; handles authentication, retry, and idempotency per POS API |
| **Event normalizer** | Maps POS transaction events to a common schema: `{location_id, timestamp, transaction_total, item_count, covers (for restaurants)}` |
| **Deduplication engine** | Detects and eliminates duplicate webhook deliveries using idempotency keys; handles POS systems that retry webhooks aggressively |
| **Gap detector** | Monitors for periods of no POS data (POS offline, integration broken); alerts the forecast pipeline to reduce confidence for affected periods |
| **Backfill service** | When a POS integration reconnects after a gap, pulls historical data to fill the gap; reconciles with any data received during the outage |

### Payroll Export Pipeline

Timesheet data flows from the attendance system to payroll providers:

```
Timesheet finalization (end of pay period)
    ↓
Manager review + exception resolution (approve/reject flagged entries)
    ↓
Compliance final check (overtime calculations, premium pay aggregation)
    ↓
Format conversion (map to payroll provider's schema)
    ↓
Export delivery (API push or file upload, per provider)
    ↓
Reconciliation (confirm payroll provider received and processed all entries)
    ↓
Audit record (what was exported, to whom, when, any discrepancies)
```

### Notification Delivery Architecture

The notification system must handle 500,000 notifications in 30 minutes during Sunday evening publishing peaks while maintaining delivery SLOs:

| Channel | Delivery Target | Fallback Strategy |
|---|---|---|
| **Push notification** | < 3 seconds | If device token invalid → fall back to SMS |
| **SMS** | < 10 seconds | If carrier rejects → retry with alternate originating number; if persistent failure → email fallback |
| **In-app message** | Immediate (WebSocket) | If user offline → queued until next app open |
| **Email** | < 60 seconds | Used for non-urgent notifications (weekly schedule summary, compliance reports) |

Priority ordering ensures compliance-critical notifications (schedule changes triggering premium pay, overtime warnings) are delivered before informational messages (new schedule available, shift reminder).
...
|---|---|---|
| **API framework** | Lightweight async framework (event-loop based) | High concurrency for 100K+ concurrent users; low memory per connection for WebSocket support |
| **Schedule database** | Relational database with JSONB support | Schedule data is relational (shifts reference employees reference locations); JSONB for flexible constraint metadata; row-level security for multi-tenancy |
| **Time-series store** | Columnar time-series database | Demand forecast data, POS events, and clock-in events are time-series-shaped; columnar compression reduces storage 5–10x; optimized for range queries by time |
| **Event log** | Append-only distributed log | Immutable audit trail for compliance; ordered event stream for event-sourced schedule state; replayable for debugging and analytics |
| **Cache** | Distributed in-memory store | Published schedules cached for sub-10ms reads; employee availability pre-cached for solver; tenant-prefixed key space for isolation |
| **Solver runtime** | Constraint-programming library with local-search extension | OR-Tools or equivalent constraint solver; supports both CP-SAT (for feasibility proving) and local search (for optimization); time-budgeted execution |
| **ML inference** | Gradient-boosted ensemble (demand forecasting); lightweight neural network (facial recognition) | GBM provides interpretable forecasts with feature importance; neural network for biometric matching runs on-device |
| **Message queue** | Durable message broker with topic-based routing | Notification fan-out; POS event ingestion; solver request queuing; dead-letter queues for retry |
| **Search** | Full-text search engine | Employee search by name/skill/certification; schedule search by date/location/role; audit log search for compliance queries |
