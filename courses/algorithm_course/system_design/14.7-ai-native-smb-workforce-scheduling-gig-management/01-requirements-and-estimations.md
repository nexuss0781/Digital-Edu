...
| Data Flow | Peak Bandwidth | Calculation |
|---|---|---|
| Clock-in events (ingress) | 10 MB/s | 50K events/min × 200 bytes/event × 1 min/60s |
| POS webhook ingestion | 5 MB/s | 10K events/s × 500 bytes/event |
| Schedule generation responses | 25 MB/s | 500 concurrent × 50 KB result |
| Push notification payloads | 3 MB/s | 100K notifications/min × 2 KB/notification |
| Demand forecast data (internal) | 2 MB/s | 100K businesses × 96 intervals × 50 bytes, refreshed 4x/day |
| WebSocket updates (real-time dashboard) | 8 MB/s | 50K concurrent connections × 160 bytes/update × 1 update/s |
| Facial recognition templates (pre-load) | 15 MB/s peak | 50K templates × 2 KB each, loaded in 15-min window before shift |
| **Total peak ingress** | **~30 MB/s** | |
| **Total peak egress** | **~50 MB/s** | |
...
| Cost Component | Monthly Cost (50-employee business) | Derivation |
|---|---|---|
| **Solver compute** | $2.50 | ~4 schedule generations/week × $0.15/solve (shared solver pool) |
| **Database storage** | $0.80 | ~50 KB/schedule × 4 versions/week × 4 weeks + timesheets |
| **Notification delivery** | $1.20 | ~50 employees × 8 notifications/week × $0.006/notification (push+SMS blend) |
| **API serving** | $1.50 | ~500 API calls/day × 30 days × shared infrastructure |
| **Demand forecasting** | $0.50 | ~0.05 GPU-hours/month for model inference |
| **Biometric verification** | $0.80 | ~100 verifications/day × 30 days (GPU compute for matching) |
| **POS integration** | $0.30 | Webhook processing + storage |
| **Infrastructure overhead** | $2.40 | Monitoring, logging, security, backups (25% of direct costs) |
| **Total COGS per business** | **$10.00** | |
| **Target monthly price** | **$30–80** | 3–8x gross margin (standard SaaS) |

### Scale-Dependent Cost Dynamics

| Scale | Businesses | COGS/business | Notes |
|---|---|---|---|
| Early (1K) | 1,000 | $25 | Dedicated infrastructure not yet amortized; solver pool over-provisioned for reliability |
| Growth (10K) | 10,000 | $12 | Solver pool well-utilized; database sharding amortized; notification batching effective |
| Scale (100K) | 100,000 | $8 | Full multi-tenant efficiency; spot instance pricing for solver; bulk notification rates |
| Mature (500K) | 500,000 | $5 | Demand forecast model training amortized across massive fleet; gig marketplace has liquidity |

---

## Case Studies: Industry-Specific Requirements

### Restaurant Scheduling (40% of target market)

| Dimension | Requirement |
|---|---|
| **Peak pattern** | Dual daily peaks (lunch 11:30–1:30, dinner 6–9 PM); Friday/Saturday 2x weekday volume |
| **Role complexity** | 5–8 roles (host, server, bartender, line cook, prep cook, dishwasher, expeditor, busser); certification requirements for alcohol service (ServSafe, TIPS) |
| **Compliance focus** | Tip credit calculations affect overtime rates; split shifts common (AM prep + PM service); minor work restrictions for under-18 bussers |
| **Gig usage** | High for front-of-house (servers, hosts); low for kitchen (skill-specific, food safety requirements) |
| **Integration** | POS integration critical for demand forecasting (covers, average check); kitchen display system for real-time capacity signals |

### Healthcare Clinic Scheduling (15% of target market)

| Dimension | Requirement |
|---|---|
| **Peak pattern** | Appointment-driven (more predictable than retail/restaurant); Monday and post-holiday surges |
| **Role complexity** | 10–15 roles with strict certification (RN, LPN, MA, NP, PA, front desk, billing, lab tech); some roles require specific patient-to-staff ratios mandated by state health boards |
| **Compliance focus** | State-mandated nurse-to-patient ratios; mandatory rest between shifts for clinical staff; on-call requirements; HIPAA extends to scheduling (schedule reveals who works where, which can reveal patient information in specialized clinics) |
| **Gig usage** | Moderate for administrative staff; high for per-diem nurses and traveling healthcare workers; licensing verification required per state |
| **Integration** | EHR/EMR system integration for appointment volume; insurance verification queue depth as demand signal |

### Retail Store Scheduling (30% of target market)

| Dimension | Requirement |
|---|---|
| **Peak pattern** | Weekend peaks; seasonal surges (holiday season 3–5x normal); event-driven spikes (sale days, product launches) |
| **Role complexity** | 3–5 roles (cashier, floor, stockroom, keyholder, visual merchandiser); key-holder coverage requirement (at least one per shift) |
| **Compliance focus** | Predictive scheduling laws most aggressively enforced in retail (San Francisco, NYC); on-call shift restrictions; clopening prohibitions |
| **Gig usage** | High for seasonal surges (holiday temp workers); low for regular operations (brand training requirements) |
| **Integration** | POS for transaction volume; inventory management system for stockroom staffing; foot traffic counters for floor coverage |
...
|---|---|
| **NP-hard scheduling problem** | Cannot find provably optimal solutions in reasonable time; must use Practical rule of thumb/metaheuristic solvers with anytime termination |
| **10-second interactive generation SLO** | Solver must balance solution quality against time budget; cannot use algorithms that need minutes to converge |
| **Multi-tenant shared infrastructure** | Per-tenant data isolation must be enforced at every layer; noisy-neighbor protection for solver pool |
| **100+ jurisdiction compliance rules** | Rules-as-data architecture required; imperative code cannot scale to the rule change velocity |
| **Mobile-first SMB users** | Sub-3-tap interactions for common operations; offline capability for schedule viewing; push notification reliability |
| **Biometric privacy regulations** | BIPA, GDPR Article 9 compliance; on-device processing; explicit consent workflows; separate data stores |
| **Blended workforce (W-2 + 1099)** | Behavioral firewall between employee and gig worker treatment; classification-aware audit trail |
| **Event-sourced schedule state** | Every modification is an immutable event; materialized views for reads; complexity of event replay and snapshotting |
...
|---|---|
| SMB managers will adopt AI-generated schedules | If managers always override AI output, the platform degrades to a calendar tool; monitoring override rates is critical for product-market fit |
| POS integration provides reliable demand signals | If POS data is delayed or unreliable, demand forecasting accuracy degrades; fallback to historical patterns is required |
| Gig workers are available in sufficient density per metro | If gig supply is thin, fill rates drop below 50% and the feature loses trust; geographic expansion must be supply-led |
| Labor law changes can be encoded within 72 hours | If rule encoding takes weeks, businesses operate under stale rules; buffer with conservative interpretations |
| Employees have smartphones with GPS capability | If significant portion of workforce uses feature phones, GPS verification degrades; SMS-based clock-in with manager confirmation as fallback |
| Biometric consent rates exceed 60% | If most employees decline biometric verification, the anti-spoofing capability is limited; GPS-only verification must remain robust independently |

---

## Competitive Landscape Context

| Platform | Strength | Gap This System Addresses |
|---|---|---|
| **Deputy** | Multi-country compliance; strong POS integrations; 100+ country support | Limited constraint solver (greedy assignment, not optimization); gig marketplace is external |
| **7shifts** | Deep restaurant-specific features; demand forecasting from POS data; 50K+ locations | Single-industry focus; no gig marketplace; limited cross-location coordination |
| **When I Work** | Simple UI; fast onboarding (< 5 minutes); strong mobile experience | No AI-powered optimization; compliance limited to basic overtime; no demand forecasting |
| **Homebase** | Free tier drives adoption; payroll integration; HR features (hiring, onboarding) | Basic scheduling (template-based, not optimized); limited compliance; no gig integration |
| **Quinyx** | Enterprise-grade AI scheduling; union rule support; Scandinavian compliance expertise | Enterprise-priced ($15+/user/month); over-featured for SMBs; complex setup |
| **Sling** | Free scheduling; communication features (group messaging, newsfeed); time tracking | No optimization engine; no compliance enforcement; limited analytics |

### Differentiation Strategy

The target system differentiates through the **combination** of capabilities that no single competitor delivers:

1. **AI-powered constraint optimization** (not just schedule templates) — at SMB price points
2. **Integrated gig marketplace** (not external staffing agencies) — for seamless gap-filling
3. **Compliance-first architecture** (not compliance-as-afterthought) — with jurisdiction-aware rules-as-data
4. **Cross-business intelligence** (not single-tenant analytics) — leveraging the network effect of 100K+ businesses
