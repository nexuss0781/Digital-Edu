...
|---|---|---|---|
| Product SDKs (web/mobile) | Bidirectional | Ruleset download; event uploads; assignment logging | No assignment = control fallback; no events = stale metrics |
| Feature flag service | Unified | Experiments use flags as delivery mechanism | Flag-experiment decoupling means no consistency gap |
| Analytics warehouse | Outbound | Assignment records; event exports; SQL templates | Warehouse-native analysis unavailable; platform-native analysis unaffected |
| Identity service | Inbound | User authentication; entity ID resolution | Server-side assignment requires identity; SDK cache unaffected |
| CDN | Outbound | Ruleset distribution to global edge locations | SDKs serve stale cache; 60-second refresh delay becomes unbounded |
| Notification service | Outbound | Alerts (SRM, guardrail, experiment completion) | Delayed alerting; experiments continue running but issues not surfaced |
| CI/CD pipeline | Integration | Feature flag toggles on deployment | Experiments can gate deployments; platform outage may block deploys |

### Failure Domain Mapping

| Failure Domain | Components | Blast Radius | Isolation Mechanism |
|---|---|---|---|
| Assignment path | SDK cache, ruleset manager, CDN | All experiment assignments | SDK local cache; control fallback; no central dependency |
| Event ingest | Gateway, queue, processor | Metric freshness; no assignment impact | SDK buffering; pipeline isolation from assignment |
| Streaming analysis | Aggregator, metric store | Real-time dashboard accuracy | Batch path provides authoritative backup |
| Batch analysis | Aggregator, CUPED engine | CUPED-adjusted results; statistical engine input | Streaming estimates available as fallback |
| Statistical engine | Job queue, workers | Result computation | Results Store retains last-known-good; stale but valid |
| Config store | Key-value cluster | Experiment management (CRUD) | Cached rulesets continue serving; new experiments cannot be created |
...
|---|---|---|
| **V1: Manual A/B** | Basic split testing | Random assignment; manual metric calculation; single experiment at a time |
| **V2: Hash-Based Assignment** | Deterministic stickiness | Hash-based buckets; concurrent experiments; basic p-value computation |
| **V3: Pipeline Infrastructure** | Event-driven metrics | Streaming + batch metric computation; automated dashboards; SRM detection |
| **V4: Statistical Rigor** | Sequential testing + CUPED | Always-valid p-values; variance reduction; pre-registration enforcement |
| **V5: Platform at Scale** | Layered isolation + guardrails | Mutual exclusion layers; automated kill-switches; 10,000+ concurrent experiments |
| **V6: Experimentation Culture** | Organizational transformation | Self-service UI; warehouse-native analysis; bandit support; experimentation as deployment safety net |

> **Current industry state (2025-2026):** Leading tech companies operate at V5-V6 maturity. Most mid-size companies are at V2-V3. The gap between V3 and V5 — adding statistical rigor, isolation, and automated safety — is where the largest ROI improvement occurs.
