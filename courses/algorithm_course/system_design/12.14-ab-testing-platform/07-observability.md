...
├─────────────────────────┼──────────┼──────────┼─────────────────┤
│ Assignment p99 latency  │ ≤ 5ms    │   12%    │ ██░░░░░░░ GREEN │
│ Assignment availability │ 99.99%   │   35%    │ █████░░░░ GREEN │
│ Event pipeline loss     │ 0.001%   │    0%    │ ░░░░░░░░░ GREEN │
│ Streaming freshness     │ ≤ 5 min  │   48%    │ ███████░░ GREEN │
│ Batch freshness         │ ≤ 90 min │   22%    │ ███░░░░░░ GREEN │
│ SRM detection time      │ ≤ 30 min │    0%    │ ░░░░░░░░░ GREEN │
│ Guardrail detection     │ ≤ 15 min │    5%    │ █░░░░░░░░ GREEN │
│ Dashboard query p95     │ ≤ 3s     │   60%    │ █████████ YELLOW│
└─────────────────────────┴──────────┴──────────┴─────────────────┘
```
...
```
FUNCTION evaluateExperimentBurnRate(slo, window_1h, window_6h):
  budget = slo.monthly_error_budget

  burn_1h = (errors_in(1h) / budget) × (30 × 24)
  burn_6h = (errors_in(6h) / budget) × (30 × 24 / 6)

  IF slo.name == "event_pipeline_loss":
    // Near-zero tolerance: any confirmed loss is critical
    IF errors_in(1h) > 100:
      TRIGGER P0_ALERT("Event pipeline data loss detected")
    RETURN
...
### Runbook: Batch Aggregator Overdue

**Trigger:** Batch computation has not completed within 120 minutes of scheduled start

```
Step 1: Diagnose
  - Check batch worker health: are workers running?
  - Check event log availability: is the data source accessible?
  - Check for partition rebalancing or schema changes that may block processing

Step 2: Assess impact
  - How many experiments are affected?
  - Are any experiments approaching decision points (scheduled stop date)?
  - Are streaming estimates available as fallback?

Step 3: Mitigate
  IF worker failure:
    - Restart failed workers; check for resource exhaustion
    - Scale up worker count temporarily
  IF data source unavailable:
    - Identify storage or network issue
    - Extend batch deadline; notify experiment owners
  IF schema change conflict:
    - Roll back schema change if safe
    - Update batch processor to handle new schema

Step 4: Post-incident
  - Verify batch results match streaming estimates (within tolerance)
  - Root cause: was this capacity, code, or infrastructure?
  - Adjust batch scheduling or worker provisioning to prevent recurrence
```

### Runbook: Assignment Cache Expiry Across Many SDKs

**Trigger:** > 1% of SDK instances reporting assignment fallback to control

```
Step 1: Diagnose
  - Check CDN health: is the ruleset endpoint responding?
  - Check ruleset compilation: is the latest ruleset valid?
  - Check SDK heartbeat metrics: which regions/versions are affected?

Step 2: Immediate action
  - If CDN issue: failover to backup CDN endpoint
  - If ruleset compilation failure: serve last-known-good ruleset
  - Alert experiment owners that assignment coverage is degraded

Step 3: Monitor
  - Track SDK fallback rate in real-time
  - When fallback rate returns to < 0.1%, confirm recovery
  - Check SRM across all running experiments post-recovery

Step 4: Post-incident
  - Calculate: how many assignments fell back to control?
  - Annotate affected experiments with "assignment degradation window"
  - Review SDK cache TTL settings (increase if too aggressive)
```
...
Every assignment request (for the server-side fallback path) carries a trace ID:

```
├── SDK cache miss detection               [0.1ms]
├── Assignment service request              [0.5ms]
│   ├── Authentication + rate limiting      [0.2ms]
│   ├── Ruleset cache lookup                [0.05ms]
│   ├── Targeting rule evaluation           [0.3ms]
│   ├── Hash computation (SHA-256)          [0.01ms]
│   ├── Bucket → variant mapping            [0.01ms]
│   └── Response serialization              [0.1ms]
├── Assignment log write (async)            [5ms]
└── SDK cache update                        [0.05ms]
```

### Event Pipeline Trace Structure

```
├── SDK event batch flush                   [1ms]
├── Event Gateway receipt                   [2ms]
│   ├── Schema validation                   [0.5ms]
│   ├── HMAC verification                   [0.3ms]
│   └── Queue write + ACK                   [8ms]
├── Event Processor                         [5ms]
│   ├── Dedup check (Bloom + hash map)      [0.5ms]
│   ├── Experiment context enrichment       [1ms]
│   └── Event log write                     [3ms]
├── Stream Aggregator                       [10ms]
│   ├── Window assignment                   [0.1ms]
│   ├── Partial aggregate update            [0.5ms]
│   └── Checkpoint write (periodic)         [5ms]
└── (Batch path, async)
    ├── Batch aggregator read               [varies]
    ├── CUPED computation                   [varies]
    └── Statistical engine job              [50-500ms]
```

Traces are sampled at 1% for normal events, 100% for events that trigger guardrail alerts or SRM warnings.
