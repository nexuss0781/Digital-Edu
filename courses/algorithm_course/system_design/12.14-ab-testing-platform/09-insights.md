...
**Why it matters:** The naive implementation of sticky assignment stores a row in a database for every (user, experiment) pair. At internet scale this creates a lookup Slowest part of the process — 500K assignments/second means 500K database reads per second, with the query latency sitting directly on the product request's critical path. Hash-based assignment eliminates this entirely: the same inputs always produce the same output, so stickiness is a consequence of the function's determinism, not a stored fact.
...
**Why it matters:** The naive approach to experiment isolation is a scheduler that prevents two experiments from targeting the same users simultaneously. This creates a global constraint that becomes a Slowest part of the process: every new experiment start must compare its targeting population against all running experiments. With 10,000 concurrent experiments, this comparison is O(N) per experiment start and requires a distributed lock or coordination service to maintain consistency. The complexity scales poorly.
...
## Insight 9: The Ruleset Is the Only Runtime Dependency — Everything Else Is Eventually Consistent

**Category:** Architecture

**One-liner:** By compiling all experiment configurations into a single compact ruleset that SDKs cache locally, the assignment path has zero runtime dependencies — no database, no network call, no service availability requirement.

**Why it matters:** Most distributed systems accept that some services must be available for the critical path to function. The A/B testing platform achieves a rare property: the hot path (assignment) has *no* runtime dependencies. The SDK downloads the ruleset once, caches it, and evaluates all assignments locally using a pure mathematical function. The assignment service, config store, CDN, and even the internet itself can be offline, and assignments continue correctly from the cached ruleset. This makes the platform's assignment availability effectively equal to the application's own availability — it cannot be a source of outages.

The design principle behind this is the separation of configuration distribution (eventually consistent, asynchronous, best-effort refresh) from configuration evaluation (synchronous, deterministic, zero-dependency). By accepting 60-second staleness in configuration (new experiments take up to 60 seconds to reach all SDKs), the platform eliminates the most dangerous failure mode: a synchronous dependency on a central service that blocks every product request when it goes down. This pattern is applicable far beyond experimentation — any system where configuration changes are infrequent relative to configuration reads can benefit from push-based eventual distribution with local deterministic evaluation.

---

## Insight 10: Pre-Registration Is Not Just Good Statistics — It Is a Platform Design Constraint That Prevents Organizational Dysfunction

**Category:** System Modeling

**One-liner:** Requiring analysts to declare their primary metric, minimum detectable effect, and run duration *before* the experiment starts prevents the post-hoc fishing that transforms experimentation from a truth-seeking mechanism into a confirmation-bias amplifier.

**Why it matters:** Without pre-registration, a motivated analyst can run an experiment, look at 20 metrics, find one that is significant at p < 0.05 (expected by chance alone with 20 metrics), and present it as the "primary finding." This is not scientific fraud — it is a natural consequence of human incentives when there are no structural constraints. Pre-registration enforces honesty: the primary metric is declared before data collection begins, and the platform applies different statistical treatment to the primary metric (uncorrected p-value, since it was pre-specified) versus secondary metrics (FDR-corrected, since they are exploratory).

The platform implementation is straightforward — the experiment creation API requires `primary_metric_id`, `min_detectable_effect`, and `target_power` fields, and the dashboard visually distinguishes primary from secondary metrics. But the organizational impact is profound: teams that adopt pre-registration make better product decisions because they are measuring what they intended to measure, not retroactively selecting the metric that tells the story they wanted to tell. This is a case where a simple technical constraint (a required field in an API) produces a disproportionate improvement in decision quality across the entire organization.

---

## Insight 11: The Event Log's 90-Day Raw Retention Is Not a Storage Cost — It Is Insurance Against Every Future Metric Definition Change

**Category:** Cost Optimization

**One-liner:** Retaining 115 TB of raw events for 90 days seems expensive until you realize it eliminates the need to re-run experiments when metric definitions change — each re-run costs weeks of time, millions of user exposures, and engineer attention.

**Why it matters:** In a pre-aggregation system, if an analyst discovers that the metric definition was wrong (counted duplicate purchases, excluded mobile events, used the wrong attribution window), the experiment must be re-run from scratch. Re-running a 14-day experiment costs 14 days of calendar time, re-exposes users to a potentially suboptimal treatment, and consumes engineering resources to configure and monitor the new run. At 500 experiments stopped per day, even a 2% re-run rate means 10 experiments per day being wasted.

The append-only event log eliminates this cost entirely. If the metric definition was wrong, simply update the definition and re-derive results from the existing raw data. No new experiment is needed. The re-computation cost is ~4 minutes of batch processing time per experiment, versus 14 days of wall-clock time for a re-run. At $0.01/GB for compressed storage, the 115 TB event log costs ~$1,150/day — approximately the cost of one engineer-hour. The value it provides (any metric computable retroactively on any completed experiment) is worth orders of magnitude more. This is a textbook example of choosing operational flexibility over storage cost minimization — the storage is bounded and predictable, but the value of retroactive analysis is unbounded.

---

## Insight 12: Experiment Velocity Is the Product — Statistical Rigor Is What Makes Velocity Sustainable

**Category:** Scaling

**One-liner:** The platform's value is not in any single experiment's result but in the cumulative pace at which the organization can learn — and that pace is only sustainable if every result is trustworthy.

**Why it matters:** Organizations that run 100+ experiments per month consistently outperform those that run a handful, measured by product improvement velocity. But this advantage only holds if the experiment results are trustworthy. A single false positive that ships a harmful change erodes trust in the platform, causing teams to second-guess experimental results, demand manual review, and slow down their experimentation cadence. A single undetected SRM that invalidates a month of experiments has the same effect. The platform's investment in statistical safeguards (sequential testing, SRM detection, CUPED, guardrails, pre-registration) is not overhead — it is the foundation that makes high-velocity experimentation sustainable.

The quantitative argument: a platform running 5,000 concurrent experiments with a 5% false positive rate produces 250 false positives per cycle. Even a few of these shipping to production creates visible user harm and organizational distrust. Reducing the false positive rate to 1% (via sequential testing, pre-registration, and metric correction) reduces false positives to 50 — still non-zero, but manageable with guardrail protection. The cost of this rigor (CUPED computation, sequential testing overhead, SRM checks) is a few percent of total platform compute — negligible compared to the cost of organizational distrust in experiment results.
