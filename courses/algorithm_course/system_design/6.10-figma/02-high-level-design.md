...
## Design Token & Variable System Architecture

Design tokens (variables in Figma) represent a cross-cutting architectural concern — they affect how properties are stored, how component overrides work, and how design-to-code handoff functions.

```mermaid
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph TokenDef["Token Definition Layer"]
        VC[Variable Collection<br/>e.g., "Brand Colors"]
        VM[Variable Modes<br/>e.g., Light / Dark]
        VV[Variable Values<br/>Per-mode resolution]
    end

    subgraph Binding["Token Binding Layer"]
        NB[Node Property Binding<br/>fill → var(primary)]
        CB[Component Binding<br/>Instance overrides reference vars]
        SB[Style Binding<br/>Shared styles reference vars]
    end

    subgraph Resolution["Runtime Resolution"]
        MR[Mode Resolver<br/>Which mode is active?]
        CR[Cascade Resolver<br/>Inherited mode from parent frame]
        FR[Final Value<br/>Concrete color/spacing/etc.]
    end

    subgraph Export["Export & Handoff"]
        CT[CSS Custom Properties]
        TK[Design Token JSON<br/>W3C Format]
        DM[Dev Mode Inspector<br/>Shows resolved + token name]
    end
    ...
    classDef token fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef binding fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef resolve fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef export fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class VC,VM,VV token
    class NB,CB,SB binding
    class MR,CR,FR resolve
    class CT,TK,DM export
```

### Variable Resolution Algorithm

```
FUNCTION resolve_property(node, property_name, active_modes):
    binding = node.property_bindings[property_name]

    IF binding == NULL:
        // No variable bound — use literal value
        RETURN node.properties[property_name]

    variable = GET_VARIABLE(binding.variable_id)
    collection = GET_COLLECTION(variable.collection_id)

    // Determine active mode for this collection
    // Modes cascade: child frames inherit parent's mode unless overridden
    mode = resolve_mode(node, collection, active_modes)

    // Look up value for this mode
    value = variable.values_by_mode[mode]

    // Variable may reference another variable (aliasing)
    IF value.type == ALIAS:
        RETURN resolve_property(node, property_name,
                               active_modes WITH alias resolution)
...
The AI service layer augments the design workflow without replacing the CRDT-based collaboration model.

```
AI Feature Categories and Integration Points:

  1. Layout Suggestions (non-destructive):
     - Input: Selected frame's scene graph subtree
     - Processing: Layout model infers auto-layout parameters
     - Output: Suggested property changes presented as a preview
     - User action: Accept / reject / modify suggestions
     - Integration: Accepted suggestions become normal CRDT operations

  2. Content Generation (additive):
     - Input: Frame dimensions + context (nearby text, page theme)
     - Processing: Image generation model creates fill content
     - Output: Generated image inserted as an asset
     - Integration: Image uploaded to asset store, node property updated via CRDT

  3. Design-to-Code (read-only):
     - Input: Selected frame's scene graph with resolved variables
     - Processing: Code generation model produces framework-specific code
     - Output: Code displayed in Dev Mode panel
     - Integration: No scene graph modification — output is a separate artifact

  4. Smart Rename (property modification):
     - Input: Node tree with auto-generated names ("Frame 47", "Rectangle 12")
     - Processing: Semantic analysis of content and hierarchy
     - Output: Meaningful names ("Hero Section", "CTA Button")
     - Integration: Name property updates via batch CRDT operations
```
