...
| | **Cons**: No offline support; need transform functions per property pair; central server Slowest part of the process | **Cons**: LWW can lose concurrent edits to same property; clock synchronization matters |
...
| **"Why not just use SVG?"** | Test rendering knowledge | "SVG elements are DOM nodes. At 500K objects, the DOM becomes the Slowest part of the process—layout recalculation, memory per DOM node, limited compositing. Figma renders everything via WebGL from C++ compiled to WASM, bypassing DOM entirely. This enables 60 FPS with complex effects, boolean operations, and custom text rendering." |
...
| **10x plugin usage** | Plugin bridge becomes Slowest part of the process; iframe overhead | Web Worker plugins (shared memory); plugin batching APIs |
...
## Advanced Discussion Topics (Staff+ Level)

### Topic: Design Token System as a Cross-Cutting Concern

```
Design tokens (variables) cut across multiple architectural layers:

  CRDT Layer: Variable bindings stored as property values on nodes —
    when a variable changes, all bound nodes must re-resolve

  Component Layer: Instances can override variable bindings independently
    from the main component — adds a third dimension to override tracking

  Rendering Layer: Mode resolution (light/dark) cascades through the
    scene graph tree — a parent frame's mode selection affects all
    children unless overridden

  Export Layer: Design-to-code handoff must emit both resolved values
    AND token references — developers need `var(--color-primary)`,
    not just `#4a90d9`

  Key insight: Variables transform the scene graph from a tree of
    concrete values to a tree of symbolic references that resolve
    based on context. This is analogous to how CSS custom properties
    work — and it introduces the same cascade resolution complexity.
```

### Topic: WebGPU Migration Path

```
WebGL 2.0 is Figma's current rendering backend. WebGPU offers:
  - Compute shaders (complex effects computed on GPU, not CPU)
  - Better memory management (explicit buffer lifecycle)
  - Multi-threaded command encoding (parallel draw call submission)
  - Reduced driver overhead (modern API design)

Migration strategy:
  1. Abstraction layer: Renderer targets an internal GPU API, not WebGL directly
  2. Dual backend: WebGPU on supported browsers, WebGL fallback on older ones
  3. Progressive enhancement: Compute-shader-accelerated blur, shadow, blend
     on WebGPU; fragment-shader fallback on WebGL
  4. Performance target: 2-3x improvement on complex scenes (100K+ nodes with effects)

Risk: WebGPU adoption varies by browser/OS. Safari was the last major browser
  to ship WebGPU support. Figma must maintain both backends indefinitely.
```
