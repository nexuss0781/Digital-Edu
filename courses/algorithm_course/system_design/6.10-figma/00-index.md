...
| [03 - Low-Level Design](./03-low-level-design.md) | Scene graph data model, API design, CRDT algorithms (Step-by-step plan in plain English) |
...
| Related Topic | Connection to Figma |
|---|---|
| [6.2 Google Docs](../6.2-google-docs/00-index.md) | Text-based OT/CRDT collaboration — Figma uses LWW property CRDTs instead of sequence CRDTs, but shares multiplayer relay architecture principles |
| [6.8 Real-Time Collaborative Editor](../6.8-real-time-collaborative-editor/00-index.md) | Block-based CRDT architecture — both systems solve collaborative convergence but Figma's 2D property model differs fundamentally from linear text |
| [6.11 Miro/Whiteboard](../6.11-miro/00-index.md) | Infinite canvas collaboration with spatial multiplayer — shares viewport-aware broadcasting and spatial indexing but Figma adds precision vector rendering |
| [1.19 CQRS Implementation](../1.19-cqrs-implementation/00-index.md) | Operation log (write path) separated from scene graph snapshots (read path) — the operation log + periodic snapshot pattern is a form of CQRS |
| [1.18 Event Sourcing System](../1.18-event-sourcing-system/00-index.md) | Every edit as an immutable operation in an append-only log enables version history, undo, and state reconstruction — pure event sourcing applied to visual documents |
| [12.2 Gaming: Multiplayer Game State Sync](../12.2-gaming-multiplayer-game-state-sync/00-index.md) | Real-time state synchronization across clients with latency compensation — Figma's multiplayer shares delta compression and client prediction patterns |
| [1.12 Blob Storage System](../1.12-blob-storage-system/00-index.md) | Scene graph binary blob storage, content-addressable image assets, and CDN distribution directly leverage blob storage patterns |
...
| Metric | Value |
|---|---|
| **Daily Active Users** | 4M+ (2025) |
| **Monthly Active Users** | 15M+ |
| **Total Files** | 50M+ |
| **Peak Concurrent Sessions** | 200,000+ |
| **Max Editors Per File** | 500 |
| **Scene Graph Nodes (max)** | 500,000 per file |
