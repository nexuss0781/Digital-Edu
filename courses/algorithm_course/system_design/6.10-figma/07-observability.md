...
```
Step-by-step plan in plain English: Trace Context in WebSocket Messages
...
```
Step-by-step plan in plain English: Client Error Report
...
| **Session state corruption** | Internal Rule that never changes violation in CRDT state | Force snapshot reload for all clients in session |
