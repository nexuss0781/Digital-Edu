...
Fractional indexing takes a different approach: each node has a `sort_order` property (a rational number). Reordering means setting `sort_order` to a value between the two adjacent nodes. This is a single LWW property write—no tombstones, no deletion, no insertion. Concurrent reorders of different nodes produce no conflict (they write to different nodes' sort_order). Concurrent reorders of the same node resolve via LWW (last move wins), which is the correct semantic—the layer ends up where the last user dragged it. The only Edge Case (Unusual or extreme situation) is precision exhaustion after many insertions between the same pair of nodes, which is solved by periodic rebalancing (re-assigning evenly spaced values to all siblings). This rebalancing is rare in practice because designers rarely insert dozens of layers between the same two siblings.
...
**Why it matters**: In an OT-based system (like the original Google Docs architecture), the server is the brains of the operation—it must transform every incoming operation against all concurrent operations to produce a correct merged result. This makes the server a complex, stateful Slowest part of the process that must be rigorously correct. If the transform function has a bug, documents diverge permanently.
...
## Insight 9: Design Tokens Transform the Scene Graph From Concrete Values to Symbolic References

**Category**: Data Modeling

**One-liner**: Variables (design tokens) convert each node property from a simple value to a context-dependent reference that resolves differently based on mode, scope, and aliasing—introducing CSS-like cascade resolution into the CRDT model.

**Why it matters**: Without variables, a node's fill color is a concrete value like `#4a90d9`. The CRDT model is straightforward: LWW on the value. With variables, that fill might be `var(color/primary)` which resolves to `#4a90d9` in light mode and `#a8c7e8` in dark mode. The scene graph now stores symbolic references, not concrete values, and the rendering pipeline must resolve them contextually.

This has cascading architectural implications. The CRDT layer must distinguish between "the user changed the resolved value" (which should create an override detaching from the variable) and "the user changed which variable is bound" (which should update the binding). Mode resolution cascades through the tree: a parent frame set to "dark mode" causes all children to resolve variables in dark mode unless a child explicitly overrides the mode. This cascading resolution is analogous to CSS custom property inheritance—and it introduces the same debugging complexity when a value "comes from somewhere" and the designer doesn't know where.

The deeper insight is that design tokens transform Figma from a "what you see" tool to a "what you mean" tool. The scene graph no longer stores appearance; it stores semantic intent. This is what makes design-to-code handoff meaningful—developers receive `var(--color-primary)` instead of a hex value, and theme switching works because the semantic binding is preserved. But it also means the rendering pipeline must perform a resolution pass before every frame, and the operation log must capture binding changes as first-class operations distinct from value changes.

---

## Insight 10: The Thick Client Architecture Inverts the Traditional Reliability Model

**Category**: Resilience

**One-liner**: By running the complete rendering engine and CRDT merge logic in the browser via WebAssembly, Figma makes the client self-sufficient—server outages degrade collaboration but never prevent individual work.

**Why it matters**: In most web applications, the server is the brain and the client is a thin display layer. If the server goes down, the application is unusable. Figma inverts this: the WASM engine in the browser contains the entire scene graph, the complete rendering pipeline, and the CRDT merge logic. The server provides three services: persistence (operation log), collaboration (relay), and authentication.

If the multiplayer server crashes, each client continues editing locally with full fidelity—60 FPS rendering, all tools working, undo/redo functional. Edits accumulate in a local queue (IndexedDB). When the server recovers, clients reconnect and the CRDT model guarantees that all queued operations merge correctly regardless of ordering. This means a 5-minute server outage affecting 200K concurrent sessions results in zero data loss and zero interruption of individual work—only collaboration pauses.

This architecture has a non-obvious cost: the client binary is 5+ MB of WebAssembly, and the client must maintain the full scene graph in browser memory (potentially 200+ MB for large files). These are acceptable trade-offs because the alternative—a thin client that can't function without the server—would make Figma unusable during any network hiccup, which is fatal for a tool that designers spend 8+ hours per day in. The thick client architecture is why Figma feels "native" despite running in a browser—it has native-level resilience because it has native-level self-sufficiency.

---

## Insight 11: Multiplayer Undo Requires Per-User Operation Stacks, Not Global Undo

**Category**: Consistency

**One-liner**: In a collaborative editor, pressing Ctrl+Z must undo only the current user's last action—not the last action globally—requiring per-user undo stacks that can selectively reverse operations in a shared CRDT state.

**Why it matters**: In a single-user application, undo is trivial: pop the last operation from the stack and apply its inverse. In a collaborative editor, "undo" becomes ambiguous. If User A moves a rectangle and User B changes its color, and User A presses Ctrl+Z, should the color change be undone (last global operation) or should the rectangle move be undone (last operation by User A)? Users universally expect the latter—undo should reverse their own actions, not someone else's.

Implementing per-user undo in a CRDT system requires each client to maintain its own operation stack and generate "inverse operations" that counteract specific past operations. For property changes, the inverse is straightforward: set the property back to its previous value. But the previous value might have been overwritten by another user in the meantime. If User A set x=100, User B set x=200, and User A undoes, the inverse should set x back to whatever it was before User A's change—which might be User B's value of 200, not the original value. This "selective undo" must inspect the CRDT history to find the correct "pre-A" state.

For structural operations (create/delete nodes, reparent), undo is even more complex. Undoing a "delete" must re-insert the node, but another user might have created a node at the same position in the layer order. The undo must re-insert at a valid position without conflicting with concurrent structural changes. This is why multiplayer undo is often limited in scope—Figma batches plugin operations into a single undo group, and complex structural undos may prompt a "cannot undo" message rather than risk corruption.

---

## Insight 12: The Operation Log Is Simultaneously a Collaboration Channel, Version History, and Audit Trail

**Category**: System Architecture

**One-liner**: The append-only operation log serves three distinct purposes—real-time sync, version history browsing, and compliance auditing—and the tension between their conflicting requirements shapes the entire storage architecture.

**Why it matters**: The operation log records every CRDT operation: "User A set node X's fill to #FF0000 at timestamp T." For real-time collaboration, the log must be low-latency and high-throughput—hundreds of operations per second per file, with sub-10ms append latency. For version history, the log must be browsable and replayable—a user should be able to scrub to "yesterday at 3 PM" and see the file's state at that moment. For compliance auditing, the log must be immutable and retained—enterprise customers need to know who changed what and when, for months or years.

These three purposes have conflicting requirements. Real-time sync wants the log in memory with fast append. Version history wants periodic snapshots so that replaying from the beginning isn't necessary. Auditing wants long-term retention with tamper-evidence. The solution is a tiered log architecture: hot log (in-memory, last 500 operations for real-time sync), warm log (on disk, last 30 days for version history), cold log (object storage, compressed, retained per compliance policy). Snapshots at the warm-tier boundary mean version history never needs to replay more than 30 days of operations.

The deeper architectural insight is that the operation log is the single source of truth for the file's history. The binary scene graph blob is a materialized view (reconstructable from the log). Version history is a read model over the log. Audit queries are analytics over the log. This event-sourced foundation gives Figma capabilities that would be impossible with mutable state: branch a file at any point in time, compute a visual diff between any two versions, answer "who changed this property and when?" without a separate audit system. The log isn't just a persistence mechanism—it's the foundational data model that makes the entire product possible.
