...
### Embedding Compression for Storage Cost

| Strategy | Implementation | Storage Savings | Quality Impact |
|----------|---------------|-----------------|----------------|
| **Matryoshka truncation** | Truncate 1024 → 256 dims at query time | 75% | -2% NDCG |
| **Binary quantization** | 1 bit per dimension | 97% | -3% NDCG |
| **Two-tier architecture** | Binary in-memory + float32 on SSD | 90%+ | Minimal (verify step) |
| **Product quantization** | Cluster-based compression | 85% | -5% recall |
| **QAMA training** | Quantization-aware Matryoshka training | 97%+ | -3% NDCG |

**Two-tier storage flow:**
```
Query → Binary-quantized retrieval (L1, 32 GB in-memory)
      → Top-100 candidates
      → Fetch full float32 embeddings (L2, SSD)
      → Re-score with full precision
      → Apply cross-encoder reranking
      → Return top-10 results
```

This pattern enables billion-scale search with commodity hardware: a single high-memory node (64 GB RAM) holds the binary-quantized index for 1B vectors, while full-precision embeddings reside on SSD for verification.
...
6. **Matryoshka + binary quantization** - 32-125x embedding storage reduction
7. **SPLATE for CPU search** - Eliminate GPU requirement for retrieval (GPU only for offline indexing)
