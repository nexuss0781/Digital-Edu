...
### Normalization Implementation Step-by-step plan in plain English
...
### DF-FLOPS Regularization for Production SPLADE

DF-FLOPS (Document Frequency-aware FLOPS) regularization addresses SPLADE's production latency problem. Standard SPLADE generates long posting lists for high-frequency terms, causing retrieval latency 10x worse than BM25. DF-FLOPS penalizes high-document-frequency terms during training, producing sparser vectors with shorter posting lists:

```
Standard SPLADE:
  - Avg posting list length: 50,000 entries
  - Retrieval latency: 100-200ms on 10M docs
  - Many high-DF expansion terms bloat indexes

DF-FLOPS SPLADE:
  - Avg posting list length: 5,000 entries (10x shorter)
  - Retrieval latency: 10-20ms on 10M docs
  - Expansion terms constrained to low-DF vocabulary

SPLADE-Doc variant:
  - Eliminates query encoder entirely
  - Uses literal binary query tokens (1 or 0)
  - Query latency competitive with BM25 (10-100ms)
  - Trade-off: slightly lower effectiveness than full SPLADE
```

**Impact:** DF-FLOPS makes learned sparse retrieval production-viable by closing the latency gap with BM25 while retaining the semantic expansion benefits that give SPLADE its quality advantage.
...
### ACORN: Filtered HNSW Without Post-Processing

Traditional filtered vector search applies metadata filters after HNSW retrieval (post-filtering) or restricts the search graph to matching nodes (pre-filtering). Both degrade: post-filtering returns fewer results than requested, pre-filtering fragments the graph and reduces recall. ACORN integrates filtering directly into HNSW graph traversal:

```
Traditional post-filter:
  1. HNSW search → top-1000 unfiltered
  2. Apply metadata filter → may return only 20 results
  Problem: insufficient results, wasted computation

Traditional pre-filter:
  1. Restrict graph to nodes matching filter
  2. HNSW search on restricted graph
  Problem: graph connectivity broken, low recall

ACORN approach:
  1. During HNSW traversal, follow ALL edges (maintain graph connectivity)
  2. Only add nodes matching filter to candidate set
  3. Continue traversal through non-matching nodes as "bridges"
  Result: full graph connectivity + strict filtering
  Performance: maintains >95% recall even when filter eliminates 99% of nodes
```

**Impact:** ACORN makes hybrid search with strict metadata constraints viable without the recall degradation that plagues pre/post-filtering approaches.

### Embedding Compression with Matryoshka + Binary Quantization

```
Compression tier comparison (1024-dim embeddings, 1B vectors):

Tier              | Bytes/vector | Total storage | Quality loss | Use case
─────────────────┼──────────────┼───────────────┼──────────────┼──────────────────
Full float32      | 4,096        | 4.0 TB        | 0%           | Ground truth
Matryoshka 256d   | 1,024        | 1.0 TB        | ~2%          | Reduced dims
Binary 1024d      | 128          | 128 GB        | ~3%          | Binary quantized
MRL 256 + binary  | 32           | 32 GB         | ~5%          | Maximum compression
INT8 quantized    | 1,024        | 1.0 TB        | ~1%          | Standard quantization

Production two-tier pattern:
  L1 (in-memory): Binary-quantized MRL embeddings (32 bytes/vector)
    → Fast candidate retrieval from full corpus
    → 32 GB for 1B vectors (fits single high-memory node)
  L2 (on-disk): Full float32 embeddings (4,096 bytes/vector)
    → Reranking verification for top-100 candidates
    → SSD-backed, fetched on demand
```
...
### Slowest part of the process 1: Embedding Generation at Index Time
...
### Slowest part of the process 2: Dense-Sparse Index Synchronization
...
### Slowest part of the process 3: Score Calibration Across Retrievers
...
| SPLADE latency | Long posting lists | DF-FLOPS regularization | 10x latency reduction |
| Filtered search | Pre/post-filter recall loss | ACORN graph traversal | >95% recall with strict filters |
| Embedding storage | 4 TB for 1B vectors | Matryoshka + binary quant | 32 GB (125x reduction) |
