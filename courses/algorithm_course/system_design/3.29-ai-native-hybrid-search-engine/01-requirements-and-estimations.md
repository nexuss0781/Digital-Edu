...
### Storage with Embedding Compression

| Compression Strategy | Bytes/Vector | 1B Vector Storage | Quality Loss | When to Use |
|---------------------|-------------|-------------------|-------------|-------------|
| **Float32 (baseline)** | 4,096 | 4.0 TB | 0% | Ground truth, reranking |
| **INT8 quantization** | 1,024 | 1.0 TB | ~1% | Standard optimization |
| **Matryoshka 256d** | 1,024 | 1.0 TB | ~2% | Dimension reduction |
| **Binary 1024d** | 128 | 128 GB | ~3% | In-memory candidate retrieval |
| **MRL 256d + binary** | 32 | 32 GB | ~5% | Maximum compression |

**Production two-tier pattern:**
- **L1 (in-memory):** Binary-quantized MRL embeddings → fast candidate retrieval → 32 GB for 1B vectors
- **L2 (on-disk):** Full float32 embeddings → reranking verification → SSD-backed, fetched on demand
- **Effective storage with two-tier:** 32 GB (hot) + 4 TB (cold) vs 12 TB (3x replicated float32)
