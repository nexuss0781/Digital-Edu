...
### Trap 7: "How do you handle embedding storage at billion scale?"

> **What they want:** Practical cost and infrastructure awareness.

> **Answer:** "1 billion documents at 1024 dimensions in float32 is 4 TB before replication -- 12+ TB with 3x replication. We use a two-tier approach: Matryoshka embeddings allow dimension truncation (1024 → 256 dims = 4x reduction), and binary quantization (1 bit per dimension) reduces each vector to 128 bytes. In production, binary-quantized embeddings live in-memory for fast candidate retrieval (32 GB for 1B vectors), while full float32 embeddings stay on SSD for reranking verification. This 'retrieve cheap, verify precise' pattern gives us 32-125x storage reduction with 3-5% quality loss."

### Trap 8: "Why not use a PostgreSQL extension instead of a dedicated vector database?"

> **What they want:** Platform selection nuance.

> **Answer:** "pgvectorscale with DiskANN indexing recently benchmarked at 471 QPS with 99% recall on 50M vectors -- 11.4x better than some purpose-built vector databases. For teams already on PostgreSQL with moderate scale (under 100M vectors), this eliminates operational complexity of a separate vector database. The trade-off is at extreme scale: purpose-built systems like Vespa offer native hybrid search (BM25 + vector in one engine), GPU acceleration, and billion-vector-optimized sharding. The decision boundary is roughly: <100M vectors with existing PostgreSQL → pgvectorscale; >100M or need native hybrid fusion → dedicated platform."

### Trap 9: "How does agentic RAG change hybrid search design?"

> **What they want:** Modern RAG architecture understanding.

> **Answer:** "Agentic RAG transforms search from single-shot request/response into an iterative reasoning loop. The LLM agent issues 3-5 queries per user question, reformulating based on intermediate results. This changes the search API contract: we need low-latency mode (skip reranking for agent queries), confidence signals (so the agent knows when results are sufficient), session-aware caching (sequential queries share context), and query suggestion APIs (so the agent can reformulate efficiently). The optimization target shifts from NDCG@10 for a single query to answer accuracy across the full retrieval chain."
...
| **Cross-encoder speed** | 10-100 docs/s | Reranking Slowest part of the process |
...
| **DF-FLOPS SPLADE latency** | 10-20ms | vs 100-200ms standard SPLADE |
| **Matryoshka + binary quant** | 32-125x | Storage reduction |
| **Agentic RAG accuracy** | 85-95% | vs 60-70% single-shot |
| **pgvectorscale** | 471 QPS @99% recall | 50M vectors, PostgreSQL |
