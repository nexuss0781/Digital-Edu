...
| **pgvectorscale** | Scale | DiskANN, binary quantization | Very High | PostgreSQL integration |

**Recommendation:** Vespa for full hybrid, Qdrant for vector-first with external BM25. pgvectorscale for PostgreSQL-native deployments at moderate scale (benchmarks show 11.4x better QPS than purpose-built vector databases at 50M vectors with 99% recall using statistical binary quantization and DiskANN indexing).
...
| **Matryoshka-enabled** | Variable | Very Good | 20-50ms | Dimension-truncatable |

**Recommendation:** E5-large-v2 or BGE-M3 for on-prem, text-embedding-3-small for API. Prefer Matryoshka-enabled models for flexibility -- truncate dimensions at query time to trade quality for latency/storage.
...
Modern hybrid search engines increasingly serve as tools within agentic RAG loops rather than standalone query endpoints. The architectural implications:

```mermaid
flowchart LR
    subgraph Agent["LLM Agent Loop"]
        direction TB
        THINK["Reason about<br/>query strategy"]
        QUERY["Formulate<br/>search query"]
        EVAL["Evaluate<br/>result quality"]
        REFINE["Refine query<br/>or strategy"]
    end

    subgraph Search["Hybrid Search Engine"]
        API["Search API"]
        HYBRID["Hybrid Pipeline"]
        META["Confidence<br/>Signals"]
    end
    ...
    HYBRID --> META
    META --> EVAL
    EVAL -->|"Insufficient"| REFINE
    REFINE --> QUERY
    EVAL -->|"Sufficient"| ANSWER["Generate<br/>Answer"]

    classDef agent fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef search fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef answer fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class THINK,QUERY,EVAL,REFINE agent
    class API,HYBRID,META search
    class ANSWER answer
```

### API Adaptations for Agentic Consumers

| Capability | Purpose | Implementation |
|------------|---------|----------------|
| **Confidence scores** | Agent decides whether to iterate | Return retrieval confidence alongside results |
| **Query suggestions** | Agent can reformulate automatically | "Did you mean" alternatives in response |
| **Session context** | Subsequent queries share context | Session-aware caching, context propagation |
| **Strategy hints** | Agent selects retrieval mode | `search_type: dense_heavy / sparse_heavy / balanced` |
| **Low-latency mode** | Support 3-5 queries per user question | Skip reranking, smaller candidate sets |
