...
## PCI DSS v4.0 Enhanced Requirements (Effective March 2025)

| New Requirement | Impact on Billing System | Implementation |
|-----------------|-------------------------|---------------|
| **Req 3.6.1.1**: Maintain inventory of cryptographic keys | Track all encryption keys used for payment data, their purpose, custodian, and expiration | Key inventory database with automated alerts for approaching expiration |
| **Req 6.4.2**: WAF protection for public-facing web apps | Billing API and customer portal must be behind a web application firewall | Deploy WAF with billing-specific rules (detect invoice manipulation attempts, parameter tampering) |
| **Req 8.3.6**: MFA for all access to CDE | Multi-factor authentication required for any human access to systems processing payment data | Enforce MFA on dashboard access; service-to-service uses mTLS (no human authentication) |
| **Req 11.6.1**: Change and tamper detection on payment pages | Customer portal payment form must detect unauthorized modifications | Subresource Integrity (SRI) for gateway JS SDK; Content Security Policy headers; automated page integrity monitoring |
| **Req 12.3.1**: Targeted risk analysis for flexible requirements | Document risk analysis for customized PCI controls | Annual risk assessment covering billing-specific payment flows, gateway integrations, and token storage |

### Network Token Security Model

```
Traditional Gateway Token Flow:
  Customer → Merchant → Gateway Token → Gateway stores PAN
  Risk: If gateway is compromised, all stored PANs are exposed
  Risk: Token is gateway-specific; switching gateways requires re-tokenization

Network Token Flow:
  Customer → Card Network Token (VTS/MDES) → Network manages PAN mapping
  Benefit: Token is domain-restricted (cannot be used by other merchants)
  Benefit: Automatic lifecycle management (card reissue updates token transparently)
  Benefit: Higher authorization rates (issuer trusts network-level tokens)
  Benefit: Reduced PCI scope (merchant never handles PAN, even transiently)

Billing System Integration:
  - Store network token alongside gateway token in payment_methods table
  - Prefer network token for recurring charges (higher auth rate)
  - Fall back to gateway token if network token unavailable
  - Monitor token health: network tokens that start declining may indicate
    card cancellation not yet reflected in token lifecycle updates
```
