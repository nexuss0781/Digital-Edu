...
| FR-18 | **E-Invoicing Compliance** | Submit invoices to government e-invoicing portals (IRP, ZATCA, SDI, Peppol) per jurisdiction mandates; store government-issued references (IRN, UUID, QR codes) |
| FR-19 | **Spend Alerts & Budget Controls** | Real-time spend estimation for usage-based billing; configurable budget thresholds with alerting; hard/soft spending caps |
| FR-20 | **Billing Schedule Flexibility** | Support custom billing anchors (anniversary-based, calendar-month-based), billing-date spreading, and timezone-aware scheduling |
...
| **E-Invoicing** | Government portal submission | Real-time submission with < 5 second latency; fallback queue for portal outages |
| **Auditability** | Financial operation traceability | Every state change to financial entities produces an immutable audit log entry with before/after state |
| **Webhook Reliability** | Event delivery to merchant integrations | At-least-once delivery; 99.9% first-attempt success within 30 seconds; exponential retry for failures |
| **Multi-Tenancy** | Tenant isolation and fair resource allocation | Logical data isolation via row-level security; per-tenant rate limiting; noisy-neighbor prevention |
...
| **Credit Wallet Accuracy** | Credit application matches wallet balance with zero discrepancy | Per invoice | Zero tolerance---credits are a financial liability on the balance sheet |
| **E-Invoice Submission Latency** | Government portal submission within 5 seconds of finalization | Per invoice (for mandated jurisdictions) | Regulatory deadline compliance; portal timeouts handled via retry queue |
| **Billing Accuracy Score** | > 99.95% of invoices require no corrections | 30-day rolling | Financial accuracy is a trust metric; corrections below 5 per 10,000 invoices |
...
| E-invoicing compliance required for India, EU, and Saudi Arabia | Government mandates require real-time invoice submission to regulatory portals |
| Prepaid credit wallet support for 20% of customers | Growing trend toward commit-spend models in cloud and API platforms |
| Network tokenization preferred over gateway tokenization | Improves authorization rates by 2--4% and reduces involuntary churn from card reissuance |
...
|----------|----------|-----------|
| **Billing day = 31, month = February** | Bill on February 28 (or 29 in leap years) | Subscriptions with billing_day > last day of month are shifted to month's last day |
| **Upgrade and downgrade in same billing period** | Track as separate obligation segments; calculate proration per segment | Prevents over/under-charging from compounding changes |
| **Zero-amount invoice** | Generate but do not charge; mark as paid immediately | May occur after credit application; needed for revenue recognition records |
| **Currency with zero decimal places (JPY, KRW)** | Store as integer with no subunit; rounding uses integer math | Prevents fractional-yen errors in calculations |
| **Invoice amount exceeds payment method limit** | Split into multiple payment attempts or request payment method with higher limit | Some payment methods have per-transaction limits |
| **Subscription cancellation during active dunning** | Stop dunning retries; settle final invoice balance; handle proportional credit | Prevents continued charging of a cancelled customer |
| **Clock skew between billing clock and subscription timezone** | Record absolute timestamps in UTC; apply timezone only for billing day interpretation | Prevents early/late billing across timezone boundaries |
| **Usage event with future timestamp** | Reject events with timestamp > current time + 5 minutes tolerance | Prevents billing for usage that has not occurred |
