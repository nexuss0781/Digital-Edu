...
|---------|-----------------|----------|
| **Invoice amount outlier** | Statistical model: flag invoices > 3 standard deviations from customer's historical average | Hold invoice for manual review before finalization; alert merchant finance team |
| **Sudden MRR drop** | Rolling comparison: current-hour MRR delta vs. 7-day and 30-day baseline | Alert if MRR drops > 2% within a single billing run; investigate for billing run failures or mass cancellations |
| **Credit note burst** | Rate monitoring: credit notes issued per support agent per hour | Threshold > 20 credit notes/hour/agent triggers review; may indicate systematic billing error or fraud |
| **Usage spike** | Per-customer usage deviation from 30-day moving average | Alert customer proactively if usage > 5x average (prevent bill shock); investigate if usage spike correlates with system error |
| **Payment method concentration risk** | Analyze payment method diversity per customer segment | Alert if > 90% of revenue for a segment routes through a single gateway or card network |
| **Tax calculation drift** | Compare tax percentages on today's invoices against yesterday's for same jurisdiction | Alert if effective tax rate changes > 0.5% (may indicate tax service misconfiguration or rate update) |

### Billing Accuracy Scoring

The system maintains a continuous billing accuracy score that tracks correctness over time:

```
Billing Accuracy Score = (
    invoices_without_corrections / total_invoices_finalized
) × 100

Target: > 99.95% (< 5 corrections per 10,000 invoices)

Decomposition:
  - Pricing accuracy:    correct base charge / total invoices
  - Proration accuracy:  correct proration / prorated invoices
  - Tax accuracy:        correct tax / taxable invoices
  - Usage accuracy:      correct usage charge / usage-based invoices
  - Discount accuracy:   correct discount application / discounted invoices
```
...
### Runbook 1: Billing Run Stalled

```
Symptom: No new invoices generated for > 30 minutes during active billing run

Diagnosis Steps:
  1. Check billing worker health: are workers running and consuming from partition queue?
  2. Check partition queue depth: are partitions assigned but not being processed?
  3. Check database connection pool: is the primary DB accepting connections?
  4. Check advisory lock table: are locks held by dead workers?
  5. Check external dependencies: is the tax service responding?

Resolution by Root Cause:
  - Dead workers: Scale up new workers; they will auto-assign to unprocessed partitions
  - Database saturation: Reduce batch size; add read replicas; check for long-running queries
  - Stale advisory locks: Release locks held by workers with no heartbeat for > 10 minutes
  - Tax service down: Enable tax-pending mode; generate invoices without tax; queue for recalculation
  - Partition skew: Split oversized partitions; reassign to additional workers
```

### Runbook 2: Payment Success Rate Drop

```
Symptom: Payment success rate drops below 85% for > 15 minutes

Diagnosis Steps:
  1. Check per-gateway success rates: is one gateway degraded or all?
  2. Check decline code distribution: is a specific decline code spiking?
  3. Check if new merchant onboarded with misconfigured gateway credentials
  4. Check card network status pages for known outages
  5. Verify no recent code deployment affected payment orchestration logic

Resolution by Root Cause:
  - Single gateway degraded: Open circuit breaker; route to fallback gateways
  - Specific decline code spike: If "do_not_honor" spike, check for card network advisory
  - Gateway credential issue: Roll back to previous credentials; notify merchant
  - Network-wide outage: Pause payment processing; queue charges; resume when network recovers
  - Code regression: Rollback deployment; reprocess queued payments
```

### Runbook 3: Usage Ingestion Lag

```
Symptom: Usage event processing lag exceeds 5 minutes

Diagnosis Steps:
  1. Check ingestion worker throughput: events/sec per worker
  2. Check deduplication layer: is bloom filter rebuild causing latency?
  3. Check partition consumer lag: is a specific partition behind?
  4. Check time-series store write latency: is storage saturated?
  5. Check for event replay storms from client SDKs

Resolution by Root Cause:
  - Throughput saturation: Scale ingestion workers horizontally
  - Bloom filter rebuild: Switch to standby bloom filter; rebuild in background
  - Partition hotspot: Rebalance customer assignments across partitions
  - Storage saturation: Increase write throughput (more shards); archive old data
  - Client replay storm: Identify client; apply per-client rate limiting; dedup handles correctness
```
...
### Cost-of-Billing Metrics

| Metric | Calculation | Target | Purpose |
|--------|-------------|--------|---------|
| **Cost per invoice** | Total billing infrastructure cost / invoices generated | < $0.02 | Track efficiency of billing run operations |
| **Cost per payment attempt** | Payment processing cost / payment attempts | < $0.05 (excluding gateway fees) | Optimize payment orchestration efficiency |
| **Cost per usage event** | Metering infrastructure cost / usage events processed | < $0.0001 | Ensure metering scales cost-effectively |
| **Gateway fee optimization** | (Lowest possible fee - actual fee) / total gateway fees | < 5% gap | Measure effectiveness of cost-based gateway routing |
| **Dunning ROI** | Revenue recovered / dunning infrastructure + processing cost | > 10:1 | Ensure dunning investment is net positive |
| **Revenue per compute dollar** | Total billed revenue / total infrastructure cost | > 1000:1 | Overall billing system cost efficiency |
