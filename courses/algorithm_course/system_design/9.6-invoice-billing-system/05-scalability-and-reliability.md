...
## Global Billing Architecture

### Multi-Region Active-Passive Billing

For globally distributed billing platforms, the system uses an active-passive model per region rather than active-active, because financial operations require strict serialization per subscription.

```
Region Strategy:
  - Each subscription is assigned to a home region based on merchant location
  - All billing operations for a subscription execute in its home region
  - Read replicas in other regions serve customer portal and dashboard queries
  - Cross-region failover promotes the standby to active with < 15-minute RTO

Why Not Active-Active:
  - Invoice generation must be exactly-once per subscription per period
  - Active-active would require distributed locking across regions (high latency)
  - Financial ledger writes require serializable isolation (cross-region transactions impractical)
  - The cost of occasional failover is far lower than the cost of distributed transaction overhead

Regional Data Residency:
  - EU customer data processed in EU region (GDPR compliance)
  - Indian customer data processed in India region (DPDP Act)
  - Usage events routed to the customer's home region for aggregation
  - Cross-region metering requires region-aware event routing at ingestion
```

### Billing Run Pre-Computation

To reduce the critical-path duration of billing runs, the system pre-computes predictable charges:

```
FUNCTION pre_compute_billing(billing_date):
    // Run 24 hours before billing date for subscriptions with no recent changes

    candidates = SELECT * FROM subscriptions
                 WHERE billing_day = billing_date.day + 1
                   AND status = 'active'
                   AND last_modified_at < NOW() - 24 hours
                   AND has_usage_component = false

    FOR sub IN candidates:
        pre_computed = PreComputedInvoice {
            subscription_id: sub.id,
            base_charges: calculate_base_charges(sub),
            discounts: calculate_discounts(sub),
            estimated_tax: estimate_tax(sub),
            computed_at: NOW(),
            valid_if_unchanged: true  // Invalidated if subscription changes before billing
        }
        CACHE.set("precompute:{sub.id}", pre_computed, TTL: 48 hours)

    // During billing run, check cache first:
    // If pre-computed AND subscription.last_modified_at < pre_computed.computed_at:
    //     Use pre-computed charges (skip recalculation)
    //     Only compute tax (may have rate changes) and finalize
    // Else:
