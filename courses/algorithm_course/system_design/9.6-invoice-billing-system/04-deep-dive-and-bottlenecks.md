...
| **Database write Slowest part of the process** | High write throughput during billing run saturates DB | Batch inserts: accumulate 100 invoices in memory, write in single batch transaction; use write-ahead pattern with async persistence |
...
### Slowest part of the process 1: Billing Run Concentration on Day 1
...
### Slowest part of the process 2: Usage Aggregation Convergence Delay
...
### Slowest part of the process 3: Payment Gateway Latency and Reliability
...
### Slowest part of the process 4: PDF Generation at Scale
...
## Critical Component 5: Prepaid Credit Wallet

### Why This Is Critical

Prepaid credit wallets are increasingly common in usage-based billing (cloud commit-spend models, promotional credits, service credits for SLA violations). The wallet creates a subsidiary ledger within the billing system that must handle credit issuance, prioritized application, expiration, partial consumption, and reversal---while maintaining consistency with the main invoice ledger.

### How It Works Internally

#### Credit Type Priority

```
PRIORITY_ORDER = [
    EXPIRING_CREDITS,     // Apply first: credits expiring soonest, to avoid waste
    PROMOTIONAL_CREDITS,  // Apply second: sign-up bonuses, marketing credits
    COMMIT_CREDITS,       // Apply third: contractual committed spend
    GOODWILL_CREDITS      // Apply last: most flexible; no expiration typically
]
```

#### Credit Application During Invoicing

```
FUNCTION apply_wallet_to_invoice(customer_id, invoice):
    BEGIN TRANSACTION
        wallet = SELECT * FROM credit_wallets WHERE customer_id = customer_id FOR UPDATE

        IF wallet.total_balance <= 0:
            RETURN CreditResult { applied: 0, remaining_due: invoice.total }

        credits = DB.get_credits(customer_id, status: ACTIVE, ORDER BY: priority, expiration)
        total_applied = 0
        ...
        FOR credit IN credits:
            ...
            // Skip expired credits (mark them expired)
            IF credit.expires_at IS NOT NULL AND credit.expires_at < NOW():
                credit.status = EXPIRED
                wallet.total_balance -= credit.remaining_balance
                DB.save(credit)
                CONTINUE

            consume = MIN(invoice.total - total_applied, credit.remaining_balance)
            credit.remaining_balance -= consume
            total_applied += consume

            IF credit.remaining_balance == 0:
                credit.status = FULLY_CONSUMED

            // Record consumption for rev-rec (commit credits trigger revenue recognition)
            INSERT INTO credit_ledger_entries (
                wallet_id, credit_id, invoice_id,
                amount: -consume, entry_type: CONSUMPTION
            )

        wallet.total_balance -= total_applied
        DB.save(wallet)

    COMMIT

    RETURN CreditResult { applied: total_applied, remaining_due: invoice.total - total_applied }
```
...
| Failure | Impact | Mitigation |
|---------|--------|------------|
| **Credit applied but payment for remainder fails** | Credits consumed but invoice not fully paid; reversal needed? | Policy decision: credits are NOT reversed on payment failure. Customer retains the partial payment. Dunning pursues only the remaining amount |
| **Credit expires between application and invoice finalization** | Credit was valid at calculation time but expired before finalization | Lock credits during invoice generation (advisory lock on wallet); finalization is atomic |
| **Concurrent credit application on two invoices** | Two invoices see the same credit balance and both try to consume it | Row-level lock on credit_wallet row; only one invoice can apply credits at a time |
| **Commit credit shortfall at contract end** | Customer committed to $100K/year but only consumed $80K | True-up invoice generated at contract end for the shortfall amount ($20K); tracked via commitment ledger |
