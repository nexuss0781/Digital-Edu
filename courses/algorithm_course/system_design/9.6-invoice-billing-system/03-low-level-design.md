...
## Algorithm 5: Prepaid Credit Wallet Application

The credit wallet applies credits to invoices before charging payment methods, following priority rules for credit type and expiration.

```
FUNCTION apply_credits_to_invoice(customer_id, invoice_amount):
    // Fetch available credits ordered by priority:
    // 1. Expiring soonest first (avoid credit waste)
    // 2. Promotional credits before commit credits
    // 3. Goodwill credits last (most flexible)
    credits = DB.get_available_credits(
        customer_id,
        status: ACTIVE,
        ORDER BY: expiration_date ASC, type_priority ASC
    )
    ...
    FOR credit IN credits:
        ...
        IF credit.expiration_date IS NOT NULL AND credit.expiration_date < NOW():
            credit.status = EXPIRED
            DB.save(credit)
            CONTINUE

        apply_amount = MIN(remaining_amount, credit.remaining_balance)

        credit.remaining_balance -= apply_amount
    ...
    // Persist all credit applications atomically
    BEGIN TRANSACTION
        FOR entry IN applied_credits:
            DB.save_credit_application(entry)
            DB.update_credit(entry.credit_id)
        INSERT INTO credit_wallet_ledger (
            customer_id, invoice_id, entries: applied_credits,
            total_applied: invoice_amount - remaining_amount
        )
    COMMIT

    RETURN CreditApplicationResult {
        total_applied: invoice_amount - remaining_amount,
        remaining_due: remaining_amount,
        entries: applied_credits
    }
```

---

## Algorithm 6: Sequential Invoice Number Generation

Invoice numbers must be sequential per tenant (legal requirement in many jurisdictions). The system uses a tenant-specific sequence with gap detection.

```
FUNCTION generate_sequential_number(tenant_id):
    // Use database sequence per tenant for atomicity
    // Format: {tenant_prefix}-{year}-{sequence}
    // Example: ACME-2025-000142

    BEGIN TRANSACTION
        seq = SELECT * FROM invoice_sequences
    ...
    prefix = seq.prefix  // e.g., "ACME" or "INV"
    year = CURRENT_YEAR()
    formatted = FORMAT("{prefix}-{year}-{ZERO_PAD(next_number, 6)}")

    RETURN formatted

// Gap detection runs daily to identify missing numbers
FUNCTION detect_invoice_number_gaps(tenant_id, date_range):
    numbers = DB.get_invoice_numbers(tenant_id, date_range)
    expected = SEQUENCE(MIN(numbers), MAX(numbers))
    gaps = expected - SET(numbers)

    IF LEN(gaps) > 0:
        ALERT("Invoice number gaps detected", {
            tenant_id: tenant_id,
            gaps: gaps,
            severity: "HIGH"  // Gaps may indicate voided or lost invoices
        })
...
## E-Invoicing Compliance Adapter

```
INTERFACE EInvoiceAdapter:
    FUNCTION submit(invoice) -> ComplianceResult
    FUNCTION check_status(reference_id) -> SubmissionStatus
    FUNCTION cancel(reference_id) -> CancellationResult

// Country-specific implementations
FUNCTION submit_india_irp(invoice):
    // Convert invoice to GST e-invoice JSON schema
    payload = transform_to_gst_schema(invoice)
    payload.sign(tenant.gstin_certificate)

    response = IRP_API.generate_irn(payload)

    IF response.success:
        invoice.irn = response.irn
        invoice.signed_qr = response.signed_qr_code
        invoice.ack_number = response.ack_number
        invoice.ack_date = response.ack_date
        RETURN ComplianceResult.SUCCESS

    IF response.error_code == "DUPLICATE":
        // Invoice already submitted; fetch existing IRN
        existing = IRP_API.get_irn_by_doc(invoice.invoice_number)
        RETURN ComplianceResult.ALREADY_SUBMITTED(existing)

    RETURN ComplianceResult.FAILURE(response.error)

FUNCTION submit_saudi_zatca(invoice):
    // Convert to ZATCA UBL 2.1 format
    ubl_doc = transform_to_zatca_ubl(invoice)
    ubl_doc.sign(tenant.zatca_certificate)

    // Phase 2 (integration): submit for clearance
    response = ZATCA_API.clearance(ubl_doc)

    IF response.clearance_status == "CLEARED":
        invoice.zatca_uuid = response.uuid
        invoice.cleared_at = response.clearance_date
        RETURN ComplianceResult.SUCCESS

    RETURN ComplianceResult.FAILURE(response.validation_errors)
```
