...
| E-invoicing government portals | Outbound | HTTPS / REST | Submit invoices to IRP (India), ZATCA (Saudi), SDI (Italy), Peppol (EU) |
| Network tokenization service | Outbound | HTTPS | Provision and manage network-level card tokens (VTS, MDES) |
| Customer notification service | Outbound | HTTPS / SMTP | Dunning emails, invoice delivery, payment confirmation, spend alerts |

---

## Cross-Service Consistency Model

| Data Domain | Consistency Requirement | Mechanism | Rationale |
|-------------|------------------------|-----------|-----------|
| **Invoice finalization** | Strong (serializable) | Database transaction with row-level lock on subscription | Invoice amounts must be exactly correct; no partial writes |
| **Payment recording** | Strong (serializable) | Database transaction with row-level lock on invoice | Payment must update invoice status atomically; prevents double-payment |
| **Usage event ingestion** | Eventual (5-minute convergence) | Async pipeline with dedup and aggregation | High throughput (100K+/sec) requires relaxed consistency; billing waits for convergence |
| **Revenue recognition** | Eventual (24-hour SLA) | Async event consumption from invoice finalization | Rev-rec complexity must not block billing run; guaranteed complete before period close |
| **Subscription state change** | Strong (serializable) | Database transaction with advisory lock | State transitions must be atomic; invalid transitions rejected |
| **Credit wallet balance** | Strong (serializable) | Database transaction with row-level lock on wallet | Prevents over-application of credits; exact balance required |
| **Webhook delivery** | At-least-once | Async with retry queue and merchant-side idempotency | Delivery guarantee; merchants handle dedup via event ID |
