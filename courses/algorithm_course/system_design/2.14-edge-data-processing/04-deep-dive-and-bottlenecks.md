...
### Slowest part of the process 1: CPU Saturation Under Load
...
### Slowest part of the process 2: Memory Pressure from Window State
...
### Slowest part of the process 3: Storage I/O During Outages
...
### Slowest part of the process 4: Network Sync Backlog
...
## Critical Component 4: WebAssembly Extension Points

### Why This Is Critical

Every edge deployment has unique requirements: a factory needs vibration FFT analysis, a retail store needs sales event enrichment, a wind farm needs power curve normalization. Hard-coding these into the core engine creates a maintenance nightmare across thousands of heterogeneous deployments.
...
Event Ingestion (native, hot path)
  │
  ├── Protocol Translation (native)
  ├── Schema Validation (native)
  │
  ▼
Extension Point: Pre-Processing Filter (Wasm sandbox)
  │
  ├── Memory limit: 16 MB per function
  ├── CPU budget: 5ms max per event
  ├── No filesystem/network access
  ├── Panic → sandbox killed, event passed through unmodified
  │
  ▼
Windowing Engine (native, hot path)
  │
  ▼
Extension Point: Post-Aggregation Enrichment (Wasm sandbox)
  │
  ├── Access to: read-only window state, event metadata
  ├── Output: enriched aggregate record
  │
  ▼
Sync Manager (native)
```
...
| Path | Native | With Wasm Extension | Overhead |
|------|--------|---------------------|----------|
| **Ingestion (no filter)** | 10K events/sec | 10K events/sec | 0% (bypassed) |
| **Ingestion (with filter)** | 10K events/sec | 7-8K events/sec | 20-30% |
| **Aggregation emit** | 1K windows/sec | 900 windows/sec | ~10% |
| **Memory per sandbox** | N/A | 16 MB max | Fixed ceiling |

The key design decision: Wasm extensions are **optional per-deployment configuration**, not a mandatory processing step. Deployments that don't need custom logic run at full native speed. Extensions are hot-loadable — the engine fetches new Wasm modules from the control plane and instantiates them without restart.

---

## Edge Orchestration: K3s for Fleet Management

### Why It Matters

Managing thousands of edge nodes as individual snowflakes with custom scripts does not scale. Lightweight Kubernetes distributions make container orchestration viable at the edge.

### Comparison

| Feature | Standard Kubernetes | K3s | KubeEdge |
|---------|-------------------|-----|----------|
| **Control plane RAM** | 2+ GB | 512 MB | Agent: 256 MB |
| **Binary size** | ~1 GB (multi-binary) | ~70 MB (single binary) | ~50 MB (edge agent) |
| **etcd replacement** | etcd (distributed) | SQLite (embedded) | Cloud-side etcd |
| **Startup time** | 60-120s | 15-30s | 10-20s (agent) |
| **Offline operation** | Limited | Continues running pods | Full offline autonomy |
| **Min hardware** | 2 CPU, 2GB RAM | 1 CPU, 512MB RAM | 1 CPU, 256MB RAM |

### Deployment Pattern for Edge Data Processing

```
EDGE FLEET MANAGEMENT:
═══════════════════════════════════════════════════════════════

Cloud Control Plane (standard K8s)
  │
  ├── Fleet Manager (manages edge registrations)
  ├── GitOps Controller (pushes desired state)
  ├── Image Registry (edge-optimized container images)
  │
  ▼ (over intermittent network)

Edge Node 1 (K3s)                Edge Node 2 (K3s)
┌─────────────────────┐          ┌─────────────────────┐
│ K3s Agent            │          │ K3s Agent            │
│ ┌─────────────────┐ │          │ ┌─────────────────┐ │
...
Key benefit: Same deployment manifests, same images,
same GitOps workflow as cloud — just on smaller hardware.
```
