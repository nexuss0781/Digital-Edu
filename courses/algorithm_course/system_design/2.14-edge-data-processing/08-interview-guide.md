...
### Trap 5: "How do you deploy and update processing logic across thousands of edge nodes?"

**What interviewer wants:** Understanding of edge fleet management

**Bad answer:** "We SSH into each node and update."

**Good answer:** "This is a fleet management problem. I'd use a GitOps approach with lightweight Kubernetes (K3s) on each edge node. Processing logic is packaged as container images. A cloud-side control plane pushes desired state; each edge node's agent reconciles locally. For custom per-deployment logic, WebAssembly modules can be hot-loaded without restarting the processing engine. Updates roll out in waves (canary → 10% → 50% → 100%) with automatic rollback if error rates spike. The key constraint is bandwidth — we use delta container image layers and schedule updates during off-peak hours."

### Trap 6: "Why not use WebAssembly for all edge processing instead of native code?"

**What interviewer wants:** Understanding of Wasm trade-offs at the edge

**Bad answer:** "Wasm is better because it's portable."

**Good answer:** "Wasm provides excellent sandboxing and portability, but has a 10-30% performance overhead compared to native code. For the hot path — raw event ingestion at 10K+ events/sec, buffer writes, and window state management — this overhead matters. For user-defined enrichment functions that run at 1-5ms per event, the overhead is negligible. The right architecture uses native code for the core engine (ingestion, windowing, sync) and Wasm sandboxes for extensibility points (custom filters, transformations, alerting rules). This is the same pattern CDN edge platforms use: the HTTP handling is native, but user logic runs in Wasm."
...
- KubeEdge and K3s for lightweight edge orchestration
- WasmEdge runtime for edge-native user-defined functions
- EMQX and HiveMQ for MQTT edge message brokers
