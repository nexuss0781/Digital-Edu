...
## Insight 11: WebAssembly Enables Safe, Portable User-Defined Processing at the Edge
**Category:** Extensibility
**One-liner:** Wasm sandboxes allow users to deploy custom transformation, filtering, and enrichment logic to edge nodes without restarting the core processing engine, with memory isolation preventing user code from crashing the system.
**Why it matters:** Edge data processing pipelines need customization per deployment: a factory floor needs vibration pattern detection, a retail store needs sales event enrichment, a wind farm needs power curve normalization. Hard-coding these into the edge engine means releasing new firmware for every customer change. WebAssembly solves this by providing a sandboxed execution environment where user-defined functions run with strict memory limits (e.g., 16MB per function), CPU time budgets (e.g., 5ms max per event), and no direct access to the host filesystem or network. The Wasm module is compiled once and runs identically on ARM, x86, and RISC-V edge hardware without recompilation. If a user function panics or exceeds its resource budget, the sandbox terminates it without affecting the host engine. This is the same isolation model used by CDN edge compute platforms, applied to IoT data processing. The trade-off is a 10-30% performance overhead compared to native code, which is acceptable because user-defined functions typically handle enrichment and filtering, not the hot path of raw ingestion.

---

## Insight 12: K3s Reduces the Kubernetes Footprint to Fit Resource-Constrained Edge Nodes
**Category:** Scaling
**One-liner:** K3s replaces etcd with SQLite, bundles all control plane components into a single ~70MB binary, and runs on 512MB RAM, making container orchestration viable for edge nodes that cannot run standard Kubernetes.
**Why it matters:** Standard Kubernetes requires 2GB+ RAM for the control plane alone (etcd, API server, scheduler, controller-manager as separate processes), which is prohibitive on edge hardware where total system RAM is often 2-4GB. K3s solves this by replacing etcd with an embedded SQLite database (or optionally an external database for HA), packaging all control plane components into a single process, and removing cloud-provider-specific code. The result is a fully conformant Kubernetes distribution that starts in under 30 seconds on a Raspberry Pi. For edge data processing, this means the same container images, Helm charts, and GitOps workflows used in the cloud can deploy edge processing pipelines without modification. The practical impact is massive: instead of managing N edge nodes as N snowflakes with custom deployment scripts, operators manage them as a fleet of Kubernetes nodes with declarative configuration. KubeEdge and OpenYurt extend this further by adding offline autonomy (the edge node continues running pods even when disconnected from the cloud control plane), which is essential for the local-first edge processing pattern.
