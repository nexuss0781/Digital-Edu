...
## Reference Deployment: Smart Retail Chain

**Assumptions:**
- 3,000 stores, 1 edge node per store
- 50 sensors per store (temperature, occupancy, POS events, cameras)
- 5 events/second per sensor average
- 150 bytes average event size (after compression)
- 24-hour local retention
...
| Metric | Calculation | Result |
|--------|-------------|--------|
| **Events per Second** | 50 sensors × 5 events/sec | 250 events/sec |
| **Events per Day** | 250 × 86,400 seconds | 21.6M events/day |
| **Raw Data Rate** | 250 events/sec × 150 bytes | 37.5 KB/sec |
| **Raw Data per Day** | 37.5 KB/sec × 86,400 sec | 3.2 GB/day |
| **Aggregated Data per Day** | 3.2 GB ÷ 10 | 320 MB/day |
| **Fleet Total (cloud)** | 3,000 stores × 320 MB aggregates | 960 GB/day |

### Hardware Recommendation

| Tier | Stores | Hardware | Rationale |
|------|--------|----------|-----------|
| **Minimal** | Small stores (< 20 sensors) | ARM SBC, 4GB RAM, 64GB eMMC | Low traffic, basic aggregation |
| **Standard** | Medium stores (20-100 sensors) | Industrial PC, 8GB RAM, 256GB SSD | Full windowing, local alerts |
| **High** | Flagship stores (100+ sensors, cameras) | Edge server, 16GB RAM, 512GB NVMe | ML inference, video analytics |

This contrast with the industrial deployment highlights why edge hardware must be tiered: the per-store event rate in retail (250/sec) is 20x lower than manufacturing (5,000/sec), allowing much cheaper hardware.
