...
| Network | More concurrent connections | Usually not Slowest part of the process |
...
#### Pattern 4: Learner (Non-Voting) Nodes for Read Scaling
...
│  └──────┼───────────────────────────────────────────────────┘   │
│         │                                                        │
│         │ Async replication (no quorum impact)                   │
│         │                                                        │
│    ┌────┴─────────────┬─────────────────┐                       │
│    ▼                  ▼                 ▼                        │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                      │
...
│  └──────────┘  └──────────┘  └──────────┘                      │
│                                                                  │
│  Key constraint: Learner reads are eventually consistent        │
│  (few ms behind leader). Acceptable for watches, NOT for        │
│  lock acquisition.                                               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

**When to use learner nodes:**

| Use Case | Learner OK? | Why |
|----------|------------|-----|
| Watch notifications | Yes | Events are inherently async |
| Lock status dashboard | Yes | Stale by seconds is acceptable |
| Lock acquisition | **No** | Must be linearizable |
| Lease renewal | **No** | Must go through leader |
| Contention monitoring | Yes | Metrics can tolerate lag |
