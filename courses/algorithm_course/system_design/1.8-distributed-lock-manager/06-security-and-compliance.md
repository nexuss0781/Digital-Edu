...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Per-client quotas prevent a single service from exhausting     │
│  lock resources and starving other services:                    │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Quota Type          │ Default  │ Purpose                   │  │
│  │─────────────────────│──────────│───────────────────────────│  │
│  │ Max concurrent locks│ 1,000    │ Prevent lock table bloat  │  │
│  │ Max leases          │ 500      │ Prevent lease heap growth │  │
│  │ Max lock key size   │ 1 KB     │ Prevent oversized keys    │  │
│  │ Max metadata size   │ 64 KB    │ Prevent data abuse        │  │
│  │ Max waiters per lock│ 1,000    │ Bound queue memory        │  │
│  │ Lock request rate   │ 100/sec  │ Prevent lock storm        │  │
│  │ Max watch streams   │ 500      │ Prevent connection exhaust│  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
│  Enforcement: Reject requests that would exceed quota with      │
│  RESOURCE_EXHAUSTED gRPC status code. Log the rejection for    │
│  audit and alert if a client consistently hits quota limits.   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Lock Storm Mitigation

A misconfigured client can generate thousands of lock requests per second, overwhelming the Raft leader. Protection strategies:

| Strategy | Description | When to Apply |
|----------|-------------|---------------|
| Per-client rate limiting | Token bucket per client identity | Always |
| Connection limits | Max gRPC connections per source IP | Always |
| Request queuing | Bounded queue with backpressure | High load |
| Priority-based admission | Critical locks (leader election) get priority over batch locks | Peak load |
| Circuit breaker | Reject all requests from a client after repeated quota violations | Abuse detection |
