...
| Do we need fairness (FIFO (First-In-First-Out, like a line at a store) ordering)? | Affects algorithm complexity |
...
### Scenario 3: "Design a lock for Kubernetes controller leader election"

**Key considerations:**
- 50+ controllers each need a single active leader
- Leaders must detect predecessor failure quickly (< 30s)
- Split-brain (two active leaders) causes duplicate reconciliation

**Approach:**
- etcd-based locks using Kubernetes Lease objects
- Each controller creates a Lease with 15s TTL, renewing every 5s (TTL/3)
- Fencing token = Lease's `resourceVersion` (etcd revision)
- Controller reconciliation loop checks fencing token before writing
- If lease renewal fails twice consecutively, controller stops reconciliation immediately

**Real-world validation:** This is exactly how Kubernetes built-in controllers (kube-controller-manager, kube-scheduler) implement leader election, processing ~10K lock operations/minute across 50 clusters.

### Scenario 4: "Design a lock for rate limit quota"
...
### Case Study 1: Disruptive Rejoin Causes 47-Second Lock Outage

**Context:** A 5-node etcd cluster (pre-3.5, no Pre-Vote) backing a distributed lock service for 200+ microservices. One node experienced a network partition lasting 3 minutes during a datacenter switch maintenance.

**What happened:**
1. Node C was partitioned for 180 seconds
2. Node C attempted 36 elections, incrementing its term from 42 to 78
3. When the partition healed, Node C sent an AppendEntries rejection with term=78 to the leader (Node A, term=42)
4. Node A stepped down immediately upon seeing the higher term
5. All 1,200 active leases were suspended during the ensuing election
6. A new leader was elected in 2.1 seconds, but lease renewal for all clients required an additional 45 seconds as every client had to rediscover the leader, reconnect, and renew

**Root cause:** Standard Raft allows partitioned nodes to increment their term indefinitely without any chance of winning an election.

**Fix:** Upgraded to etcd 3.5 with Pre-Vote enabled. Pre-Vote prevents the partitioned node from incrementing its term because no peer will grant a pre-vote to a node with a stale log. Subsequent partition events (occurred 4 more times over 6 months) caused zero leader disruptions.

**Key takeaway:** Pre-Vote should be considered mandatory for any production lock service, not an optional optimization.

### Case Study 2: Missing Fencing Tokens Cause Silent Data Corruption

**Context:** A financial services team used Redlock for coordinating database schema migrations across 12 service instances. They chose Redlock because they already had Redis deployed and wanted to avoid adding ZooKeeper or etcd.

**What happened:**
1. Service A acquired the Redlock and began a migration (adding a column)
2. Service A experienced a full GC pause lasting 35 seconds (JVM with 16GB heap)
3. Redlock TTL (30s) expired; Redis automatically released the lock
4. Service B acquired the same Redlock and began the same migration
5. Service B completed the migration (column added) and released the lock
6. Service A resumed from the GC pause, continued the migration believing it still held the lock
7. Service A attempted to add the same column (idempotent in this case) but also wrote a migration checkpoint record with stale metadata, overwriting Service B's checkpoint
8. Subsequent migration steps used the corrupted checkpoint, applying migrations out of order

**Root cause:** Redlock provides no fencing tokens. There was no mechanism for the database to reject Service A's stale writes.

**Fix:** Migrated to etcd-based locks. Each migration step now includes the etcd revision as a fencing token in the migration checkpoint write. The checkpoint table validates that the fencing token is monotonically increasing, rejecting any stale write. Additionally, migration hold times were reduced by running the migration outside the lock and only holding the lock for the final checkpoint write (lock coarsening → lock minimization).
