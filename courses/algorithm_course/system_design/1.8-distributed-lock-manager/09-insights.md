...
## Insight 5: Leader Slowest part of the process Is the Price of Linearizability
...
**Why it matters:** Adding followers to a Raft cluster improves read throughput and fault tolerance but does nothing for write throughput -- every write still funnels through the single leader for log serialization and replication. etcd typically achieves 10K-50K writes/sec; ZooKeeper 10K-100K. For systems requiring higher lock throughput, the solution is namespace sharding: partition the lock keyspace across multiple independent clusters (e.g., /locks/payment on cluster A, /locks/inventory on cluster B). Each cluster has its own leader and independent throughput ceiling. The trade-off is that cross-shard locks (acquiring locks on both payment and inventory atomically) become impossible without a higher-level coordination protocol. This single-leader Slowest part of the process is inherent to any linearizable system and is the fundamental reason why distributed locks should be used sparingly -- if you need 100K+ lock operations per second on a single key, redesign the system to use optimistic concurrency or lock-free algorithms instead.
...
## Insight 9: Pre-Vote Prevents a Partitioned Node from Disrupting a Stable Cluster

**Category:** Consensus
**One-liner:** Without Pre-Vote, a node isolated by a network partition repeatedly increments its Raft term, and when the partition heals, forces the healthy leader to step down -- disrupting all active locks for no good reason.

**Why it matters:** In standard Raft, a follower that stops receiving heartbeats transitions to candidate and increments its term. A node partitioned from the cluster for 5 minutes might increment its term 50+ times. When the partition heals, the node sends messages with its artificially high term, forcing the stable leader (and all its active leases) to step down. The Pre-Vote extension (implemented in etcd 3.5+, TiKV, and CockroachDB) adds a preliminary election phase where the candidate first asks peers "would you vote for me?" without incrementing its term. Since the partitioned node has a stale log, no peer will pre-vote for it, so its term never increases. This eliminates the "disruptive rejoin" problem entirely. For a lock service, this is critical: an unnecessary leader election means all lease renewals are temporarily suspended, all in-flight lock acquisitions fail, and clients must rediscover the new leader -- seconds of disruption caused by a network event that already healed.

---

## Insight 10: Deterministic Simulation Testing Finds Lock Bugs That Integration Tests Cannot

**Category:** Testing
**One-liner:** By controlling every source of nondeterminism (network delays, clock ticks, disk I/O, thread scheduling) through a single random seed, deterministic simulation can explore failure scenarios that are astronomically unlikely in real-time testing but catastrophic in production.

**Why it matters:** Traditional integration testing runs the lock service with real clocks, real networks, and real concurrency, then injects failures (kill a node, add latency) and checks invariants. This approach has two fatal limitations: it cannot exhaustively explore the failure space (there are billions of possible interleavings), and failures are not reproducible (running the same test twice produces different execution orders). Deterministic simulation replaces all real I/O with simulated equivalents controlled by a single PRNG seed. A test that fails at seed 847293 will fail at seed 847293 forever, making debugging tractable. Applied to distributed lock managers, simulation testing has uncovered edge cases at the intersection of lease expiry, leader election, and network partition healing -- scenarios where three events must occur within a specific 50ms window to trigger a double-grant bug. These bugs are invisible to Jepsen (which uses real time and cannot control interleaving granularity) but exploitable in production given enough time and scale.

---

## Insight 11: Learner Nodes Scale Lock Status Reads Without Increasing Write Quorum

**Category:** Scaling
**One-liner:** Raft learner (non-voting) nodes replicate the log but don't participate in elections or quorum, enabling read scaling for watches and lock status queries without adding write latency.

**Why it matters:** Adding a 6th voting node to a 5-node Raft cluster increases the quorum from 3 to 4, meaning every write now requires one additional acknowledgment -- directly increasing write latency. But many lock service workloads are read-heavy: thousands of clients watching lock state, querying lock status, or monitoring contention metrics. Learner nodes receive the replicated log (and thus have a consistent view of lock state, modulo replication lag) but don't vote, so they don't increase the write quorum. This enables cross-region read replicas: a learner in a remote region can serve local watch notifications and lock status queries, while all write operations (acquire, release, renew) still go through the primary cluster. The critical constraint is that learner reads are not linearizable -- they may be a few entries behind the leader. For lock acquisition this is unacceptable, but for watches (which are event-driven and tolerate minor delivery delay) and dashboards it is perfectly safe.

---

## Insight 12: Lock Coarsening Trades Granularity for Throughput When Contention Is High

**Category:** Contention
**One-liner:** When hundreds of fine-grained locks are acquired and released in a tight loop, replacing them with a single coarser lock that covers a broader scope can increase throughput 10x by reducing consensus round-trips.

**Why it matters:** Each lock acquisition requires a Raft consensus round-trip (propose → replicate → commit → apply), typically 5-20ms. If a batch operation needs 100 fine-grained locks (one per record), the sequential acquisition takes 500-2000ms. Lock coarsening replaces these 100 individual locks with a single lock on the containing scope (e.g., lock the entire table instead of 100 rows), reducing consensus overhead to a single round-trip. The trade-off is reduced concurrency: other clients that need any record in that table must wait. This mirrors the database world's lock escalation strategy (row locks → page locks → table locks) and is appropriate when: (a) the operation is short-lived (seconds, not minutes), (b) contention on individual records is already high, or (c) the batch must be atomic anyway. The lock manager can support this pattern by allowing hierarchical lock paths (`/database/users` vs `/database/users/row-123`) with intention locks that signal "I will lock children" to prevent deadlocks between coarse and fine-grained lockers.
