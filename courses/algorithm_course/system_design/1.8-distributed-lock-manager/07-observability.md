...
## Pre-Vote and Election Health Metrics

| Metric | Type | Description | Labels |
|--------|------|-------------|--------|
| `raft_prevote_requests_total` | Counter | Pre-vote requests sent | status |
| `raft_prevote_rejected_total` | Counter | Pre-votes rejected (stale log or active leader) | reason |
| `raft_disruptive_rejoin_prevented_total` | Counter | Times Pre-Vote prevented a disruptive rejoin | - |
| `raft_election_duration_seconds` | Histogram | Time from election start to leader confirmed | - |
| `raft_learner_replication_lag_entries` | Gauge | Entries a learner node is behind the leader | learner_id |

### Lock Coarsening and Contention Dashboard
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Locks by Hold Duration                Lock Escalation Events    │
│  ┌─────────────────────────────┐      ┌─────────────────────┐  │
│  │  < 100ms   ████████████ 78% │      │ Coarsen: 12/hour    │  │
│  │  100ms-1s  ████         15% │      │ Fine→Coarse: 8      │  │
│  │  1s-10s    █             5% │      │ Contention spikes: 3 │  │
│  │  > 10s     ▎             2% │      │                      │  │
│  └─────────────────────────────┘      └─────────────────────┘  │
│                                                                  │
│  Top Contended Lock Paths (last 1h)                              │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  Path                    │ Waiters │ Avg Wait │ Acq/min │   │
│  │  /db/migrations          │  12     │  4.2s    │    45   │   │
│  │  /cache/rebuild          │   8     │  1.8s    │   120   │   │
│  │  /election/scheduler     │   3     │  0.1s    │    20   │   │
│  │  /batch/export           │   2     │  0.5s    │    60   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
