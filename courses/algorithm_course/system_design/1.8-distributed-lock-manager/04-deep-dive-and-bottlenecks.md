...
### Slowest part of the process 1: Leader as Single Write Point
...
### Slowest part of the process 2: Cross-Region Latency
...
### Slowest part of the process 3: Watch Storm
...
### Slowest part of the process 4: Lock Contention Hot Spots
...
## Deep Dive 4: Pre-Vote Protocol and Disruptive Rejoin Prevention

### The Problem: Partitioned Nodes Destabilize the Cluster

```
┌─────────────────────────────────────────────────────────────────┐
│            DISRUPTIVE REJOIN (WITHOUT PRE-VOTE)                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Timeline:                                                       │
│  ─────────────────────────────────────────────────────────────► │
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Node C (partitioned):                                     │  │
│  │  term=5   ║ No heartbeat → election timeout               │  │
│  │           ║ term=6 → RequestVote → no responses           │  │
│  │           ║ term=7 → RequestVote → no responses           │  │
│  │           ║ term=8 → ...                                  │  │
│  │           ║ term=50 → partition heals                     │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Leader (Node A), term=5:                                  │  │
│  │  Stable operation, serving locks, renewing leases         │  │
│  │                          │                                │  │
│  │                          ▼ (partition heals)              │  │
│  │           Receives message from C with term=50            │  │
│  │           MUST step down (higher term!)                   │  │
│  │           All leases suspended during re-election!        │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
│  Result: A healthy cluster is disrupted by a stale node         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### The Solution: Pre-Vote Extension

```
ALGORITHM pre_vote_election():
    // Phase 1: Pre-Vote (does NOT increment term)
    pre_vote_term = current_term + 1  // hypothetical, not committed
    pre_vote_responses = 0

    FOR peer IN peers:
        response = peer.pre_vote(
            term = pre_vote_term,
            last_log_index = my_last_log_index,
            last_log_term = my_last_log_term
        )
        IF response.granted:
            pre_vote_responses += 1

    IF pre_vote_responses < majority:
        // No chance of winning → do NOT increment term
        // This prevents the disruptive rejoin problem
        RETURN  // Stay as follower

    // Phase 2: Real Vote (now safe to increment term)
    current_term += 1
    // ... proceed with normal Raft election


ALGORITHM handle_pre_vote_request(candidate_term, candidate_log):
    // Grant pre-vote only if candidate's log is at least as up-to-date
    IF candidate_term <= current_term:
        RETURN (granted = false)
    IF candidate_log < my_log:
        RETURN (granted = false)
    IF recently_heard_from_leader:  // Within election timeout
        RETURN (granted = false)  // Leader is alive, no need for election
    RETURN (granted = true)
```

**Key properties of Pre-Vote:**
- A partitioned node's term never increases (no pre-vote majority available)
- A node behind on log entries cannot start an election
- A node that recently heard from the leader won't grant pre-votes
- Adopted by etcd 3.5+, TiKV, CockroachDB, and most production Raft implementations

---

## Deep Dive 5: Deterministic Simulation Testing for Lock Correctness
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Integration Testing (Jepsen-style):                            │
│  + Real processes, real network, real failures                  │
│  + Finds real bugs in real deployments                          │
│  - Cannot control interleaving (nondeterministic)               │
│  - Cannot reproduce failures reliably                          │
│  - Explores tiny fraction of state space                       │
│  - Slow (minutes per test run)                                 │
│                                                                  │
│  Deterministic Simulation:                                       │
│  + Controls ALL nondeterminism via single PRNG seed             │
│  + 100% reproducible failures                                  │
│  + Explores vastly more state space                            │
│  + Fast (thousands of test runs per second)                    │
│  - Requires abstracting all I/O                                │
│  - Bugs in simulator can mask real bugs                        │
│                                                                  │
│  Best practice: Use BOTH. Simulation for development,          │
│  Jepsen for production validation.                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Simulation Architecture for Lock Manager Testing

```
STRUCTURE SimulatedLockCluster:
    rng: SeededRandom          // Single source of randomness
    clock: SimulatedClock      // Controllable time
    network: SimulatedNetwork  // Controllable delays, partitions
    disks: SimulatedDisk[]     // Controllable write latency, failures
    nodes: SimulatedNode[]     // Lock manager instances

FUNCTION run_simulation(seed, duration, invariants):
    cluster = SimulatedLockCluster(seed)
    cluster.bootstrap(node_count=5)

    WHILE cluster.clock.now() < duration:
        // Advance simulation by one event
        event = cluster.next_event()  // Deterministic from seed

        SWITCH event.type:
            CASE TIMER_FIRE:
                node = cluster.nodes[event.node_id]
                node.handle_timer(event.timer_id)
            CASE NETWORK_DELIVER:
                node = cluster.nodes[event.dest]
                node.handle_message(event.message)
            CASE CLIENT_REQUEST:
                cluster.submit_lock_request(event.request)
            CASE FAULT_INJECT:
                cluster.inject_fault(event.fault)  // Partition, crash, etc.

        // Check invariants after EVERY event
        FOR Rule that never changes IN invariants:
            IF NOT Rule that never changes.check(cluster):
                REPORT "Rule that never changes violated at seed={}, event={}"
                RETURN FAILURE

    RETURN SUCCESS

// Key invariants for lock managers:
INVARIANTS:
    - At most one holder per lock key at any point in time
    - Fencing tokens are strictly monotonically increasing
    - Lease expiry always releases associated locks
    - Watch notifications are delivered in revision order
    - No committed entries are lost during leader election
```
...
| Leader Slowest part of the process | Single write point, shard if needed |
