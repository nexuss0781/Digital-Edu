...
| Component | Scaling Type | Slowest part of the process | Strategy |
|-----------|--------------|------------|----------|
...
| Prompt caching | 20-40% input tokens | Cache invalidation complexity |
| Context distillation | 30-50% per turn | Distillation LLM cost |
...
```
Phase 1: Deploy new agent version to "green" pool
Phase 2: Route 5% of new tasks to green pool (canary)
Phase 3: Monitor for 30 minutes:
  - Success rate >= baseline
  - Latency P95 <= baseline + 20%
  - Cost per task <= baseline + 10%
Phase 4: If metrics pass, shift 100% to green
Phase 5: Keep blue pool warm for 1 hour (rollback window)
```

**Critical consideration:** Agent updates affect downstream agents through handoffs. A new version of Agent A may produce different handoff context structure, breaking Agent B. Version compatibility testing must cover the full pipeline, not just individual agents.

### Workflow Affinity and Locality

```
Affinity Rules:
1. All agents in a workflow prefer the same region
   (reduces cross-region handoff latency)
2. Agents sharing memory prefer the same zone
   (reduces memory access latency)
3. Supervisor and workers co-locate when possible
   (reduces coordination overhead)

Implementation:
- Workflow-level region tag assigned at creation
- Agent scheduler applies soft affinity (prefer same zone)
- Hard affinity only for latency-critical workflows
```

### Graceful Shutdown and Draining

```
On node shutdown signal:
1. Stop accepting new tasks (mark agents as "draining")
2. Wait for in-flight agent turns to complete (timeout: 60s)
3. For tasks mid-execution:
   a. Save checkpoint
   b. Handoff context to shared memory
   c. Re-queue task for pickup by another node
4. Deregister agents from registry
5. Shutdown
```

### Multi-LLM Provider Resilience

| Scenario | Primary | Fallback | Trigger |
|----------|---------|----------|---------|
| Provider outage | Provider A | Provider B | 3 consecutive failures |
| Rate limiting | Provider A | Provider B (overflow) | 429 responses |
| Cost spike | Best-value | Next-best | Cost threshold exceeded |
| Latency degradation | Fastest | Next-fastest | P95 > 2x baseline |

**Token budget recalculation:** Different providers have different pricing. When falling back, the cost-aware routing must recalculate token budgets to prevent overspend on a more expensive fallback provider.
