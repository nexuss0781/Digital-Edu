...
3. Find Slowest part of the process span:
...
Beyond linear traces, multi-agent workflows benefit from a graph-based visualization showing which agents communicated with which, how many messages were exchanged, and where bottlenecks occurred:

```
Visualization Types:

1. Timeline View (Gantt-style):
   ├── Agent 1: [===Task 1===]---[=Handoff=]
   ├── Agent 2:                    [====Task 2====]---[=Handoff=]
   ├── Agent 3:                                         [====Task 3====]
   └── Shows: parallel execution, sequential bottlenecks, idle time

2. Flow Graph View:
   Agent 1 ──(3 msgs)──▶ Agent 2 ──(1 msg)──▶ Agent 3
      │                      │
      └──(2 msgs)──▶ Agent 4 ◀──(1 msg)──┘
   Shows: communication patterns, fan-out/fan-in, unexpected flows

3. Flame Graph View:
   workflow.execution (15.3s)
   ├── agent_1.turn (3.2s)
   │   ├── llm.call (2.8s)
   │   └── tool.search (0.3s)
   ├── handoff.execute (0.5s)
   ├── agent_2.turn (8.1s)
   │   ├── llm.call (4.2s)
   │   ├── llm.call (3.5s)  ← retry
   │   └── tool.database (0.2s)
   └── agent_3.turn (3.5s)
   Shows: where time is spent, which LLM calls dominate
```
...
|---------|------------------|-----------|--------|
| Agent producing unusually long output | Token count z-score | > 3σ from mean | Alert, review prompt |
| Agent making excessive tool calls | Tool call count per turn | > 10 per turn | Circuit break, investigate |
| Agent hallucinating tool names | Tool call validation failure rate | > 5% | Stop agent, retrain |
| Agent stuck in retry loop | Same tool call repeated | > 3 identical calls | Break loop, escalate |
| Agent ignoring handoff context | Context utilization score | < 20% used | Review handoff schema |

### SLI/SLO Dashboard Mapping

| SLO | SLI (Metric) | Good Threshold | Warning | Critical |
|-----|--------------|----------------|---------|----------|
| Task completion | `workflow_completion_total{status="completed"}` / total | > 99.5% | < 98% | < 95% |
| Handoff reliability | `handoff_success_total{status="success"}` / total | > 99.9% | < 99% | < 97% |
| E2E latency | `workflow_duration_seconds` P95 | < 30s | > 45s | > 60s |
| Agent availability | Healthy agents / total agents | > 95% | < 90% | < 80% |
| Cost per task | `cost_per_task_dollars` P50 | < $0.10 | > $0.50 | > $1.00 |
