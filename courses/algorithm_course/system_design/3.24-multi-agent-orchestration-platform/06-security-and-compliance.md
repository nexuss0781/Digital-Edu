...
```
MCP Tool Access Flow:

1. Agent needs to call a remote MCP tool
2. Agent's OAuth client requests scoped access token:
   - scope: "tool:database:read" (minimal required scope)
   - audience: MCP server URL
3. Authorization server issues short-lived token (5 min TTL)
4. Agent calls MCP tool with Bearer token
5. MCP server validates:
   a. Token not expired
   b. Token scope covers requested operation
   c. Token audience matches this server
6. Tool executes within authorized scope
```
...
|------------|----------|---------|
| **Read-only** | Search, query, lookup | Allow with scope check |
| **Write** | File write, database insert | Require explicit grant |
| **Destructive** | Delete, drop, overwrite | Require confirmation + audit |
| **External** | HTTP requests, API calls | Domain allowlist + rate limit |

### A2A Security Model

**Agent Card Trust Chain:**

```
Trust Verification:

1. Agent B receives task request from Agent A
2. Fetch Agent A's card: GET {agent_a_url}/.well-known/agent.json
3. Verify card signature (issued by trusted registry)
4. Verify request signature (matches card's public key)
5. Check authorization:
   - Is Agent A in the allowed callers list?
   - Does Agent A's tenant match (for multi-tenant)?
   - Is the requested capability within Agent A's delegation scope?
6. Process if all checks pass

Trust Levels:
├── Same tenant, same team: Full trust (all capabilities)
├── Same tenant, different team: Scoped trust (declared interfaces only)
├── Cross-tenant: Explicit federation agreement required
└── External (unknown): Reject by default
```
...
| Threat | Mitigation |
|--------|------------|
| Malicious agent package | Agent definition signing, approved registry |
| Compromised tool server | MCP server allowlisting, integrity monitoring |
| Poisoned training data | Model provenance tracking, evaluation benchmarks |
| Dependency vulnerabilities | Container scanning, pinned versions |
| Agent prompt tampering | Immutable prompt storage, change audit trail |
