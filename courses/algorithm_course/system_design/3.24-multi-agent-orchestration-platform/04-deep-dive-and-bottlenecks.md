...
### Slowest part of the process 1: Context Window Explosion
...
### Slowest part of the process 2: Communication Latency
...
### Slowest part of the process 3: State Consistency
...
### Slowest part of the process 4: Orchestrator Scalability

**Problem:** Central orchestrator can become a Slowest part of the process.
...
### Optimization 5: Prompt Caching for Repeated Context

**Problem:** Agents in the same workflow share significant context overlap -- system prompts, shared memory fragments, and workflow definitions are repeated across turns.

**Solution:** Leverage prompt caching at the LLM gateway level.

```
Cache Strategy:
├── System prompt: Cached per agent definition (TTL: until version change)
├── Shared context: Cached per workflow step (TTL: until memory update)
├── Tool schemas: Cached per MCP server (TTL: until schema change)
└── Handoff context: Cached per handoff (TTL: single use)

Savings:
- System prompts (typically 500-2000 tokens): Cached for all turns
- Shared memory (typically 1000-5000 tokens): Cached within workflow
- Net reduction: 20-40% of input tokens across a 5-agent pipeline
```

---

## Slowest part of the process 5: Prompt Injection Propagation

**Problem:** Multi-agent systems amplify prompt injection risks because injected content propagates through handoffs.

```
Attack Chain:
User Input → Agent A (injection embedded in findings)
           → Handoff → Agent B (injection in "trusted" internal context)
           → Handoff → Agent C (injection triggers unauthorized action)
```

**Why Multi-Agent Makes This Worse:**
- Single-agent: Injection scope limited to one context window
- Multi-agent: Injection travels through handoff context, bypassing per-agent input guards
- Internal handoff data is often treated as trusted, creating a bypass path

**Mitigation Layers:**

| Layer | Technique | Trade-off |
|-------|-----------|-----------|
| **Input boundary** | Guardrails filter on all user-facing inputs | Latency (+50-100ms) |
| **Handoff boundary** | Re-scan handoff context with injection detectors | Latency (+30ms per handoff) |
| **Structured data** | Use typed JSON schemas, not free-text, for handoffs | Reduced flexibility |
| **Sandboxed tool calls** | Agents cannot execute destructive ops without confirmation | User friction |
| **Output boundary** | Final output filtered before returning to user | Latency (+50ms) |

**Key Design Decision:** Treat inter-agent communication as an untrusted boundary, applying the same sanitization rigor as user-facing input. This is counter-intuitive (internal traffic is usually trusted) but essential because any agent in the chain may have processed tainted data.

---

## Slowest part of the process 6: Cross-Framework Agent Interoperability

**Problem:** Enterprise deployments often use agents built with different frameworks (LangGraph for complex workflows, CrewAI for team tasks, custom agents for legacy systems). Without protocol-level interop, agents cannot collaborate across framework boundaries.

**Manifestations:**
- Agent A (LangGraph) cannot delegate to Agent B (CrewAI) without custom adapters
- Tool bindings are framework-specific, requiring duplicate tool implementations
- State serialization formats differ, causing context loss during cross-framework handoffs

**Solution: Protocol-Level Abstraction**

```
Before (framework-coupled):
  LangGraph Agent → LangGraph-specific API → LangGraph Agent only

After (protocol-decoupled):
  Any Agent → A2A Protocol → Any Agent (regardless of framework)
  Any Agent → MCP Protocol → Any Tool (regardless of implementation)
```

**Implementation:**

| Concern | Protocol | Benefit |
|---------|----------|---------|
| Agent discovery | A2A Agent Cards | Framework-agnostic capability advertisement |
| Task delegation | A2A task lifecycle | Standardized submit/status/result flow |
| Tool access | MCP servers | Single tool implementation, any agent can use |
| State transfer | JSON Schema + A2A context | Framework-neutral serialization |

**Trade-off:** Protocol overhead adds 10-30ms per inter-agent call compared to in-process communication, but enables architectural flexibility that is essential at enterprise scale.
