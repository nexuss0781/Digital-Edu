...
| Pro | Con |
|-----|-----|
| Clear accountability | Supervisor Slowest part of the process |
...
- **Agents**: Lightweight Python classes with instructions and tool bindings
- **Handoffs**: Transfer functions for agent-to-agent routing
- **Guardrails**: Input/output validators with tripwire pattern (halt on violation)
- **Tracing**: Built-in span instrumentation for observability
- **State**: Stateless by default; context passed through handoff parameters

### Google Agent Development Kit (ADK) Approach

- **Agents**: Hierarchical composition with `LlmAgent` and `SequentialAgent`
- **Sessions**: Session-based state management with context processors
- **Tools**: First-class MCP tool integration via `MCPToolset`
- **Communication**: Native A2A protocol support for cross-framework interop
- **Deployment**: Container-based with Agent Engine managed runtime
...
```mermaid
flowchart LR
    subgraph Framework1["Framework A (e.g., LangGraph)"]
        AG1[Agent 1]
        AG2[Agent 2]
    end

    subgraph Protocol["Protocol Layer"]
        A2AH[A2A Protocol Handler]
        MCPH[MCP Gateway]
    end

    subgraph Framework2["Framework B (e.g., CrewAI)"]
        AG3[Agent 3]
        AG4[Agent 4]
    end
    ...
    class AG1,AG2,AG3,AG4 framework
    class A2AH,MCPH protocol
    class T1,T2,T3 tools
```

**Key benefit:** Agents built in different frameworks can collaborate through A2A, while all agents share a common tool ecosystem through MCP -- enabling best-of-breed framework selection per agent role.

### Agent Card Discovery Flow

```
1. Agent publishes Agent Card at well-known endpoint:
   GET /.well-known/agent.json → AgentCard (capabilities, auth, endpoint)

2. Orchestrator discovers agents:
   - Static registry: Pre-configured agent endpoints
   - Dynamic discovery: Crawl well-known endpoints
   - DNS-based: SRV records for agent services

3. Capability matching:
   AgentCard.capabilities → matched against task requirements

4. Communication established:
   - Auth negotiated per Agent Card spec
   - Task submitted via A2A task/send endpoint
