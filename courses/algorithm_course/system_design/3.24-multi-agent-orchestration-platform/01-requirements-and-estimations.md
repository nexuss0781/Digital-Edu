...
| Context distillation | 30-50% token reduction per turn | LLM cost to compress |
| Prompt caching | 20-40% input token savings | Cache staleness risk |

---

## Protocol Compliance Requirements

### MCP (Model Context Protocol) Conformance

| Requirement | Description |
|-------------|-------------|
| **Tool discovery** | Agents dynamically discover available tools via MCP server manifests |
| **Schema validation** | All tool inputs/outputs validated against declared JSON schemas |
| **Transport support** | Support both stdio and HTTP+SSE transports |
| **Authentication** | OAuth 2.0 resource server pattern for remote MCP servers |
| **Versioning** | Protocol version negotiation on connection |

### A2A (Agent-to-Agent Protocol) Conformance

| Requirement | Description |
|-------------|-------------|
| **Agent Cards** | Publish JSON-LD agent cards at `/.well-known/agent.json` for discovery |
| **Task lifecycle** | Support full task lifecycle: submitted → working → input-required → completed/failed |
| **Streaming** | Server-Sent Events (SSE) for long-running task progress |
| **Push notifications** | Webhook-based notifications for asynchronous task updates |
| **Authentication** | Support multiple auth schemes (OAuth 2.0, API keys, agent card signatures) |

---

## Deployment Topology Requirements

| Topology | Use Case | Agent Count | Latency Budget |
|----------|----------|-------------|----------------|
| **Single-node** | Development, testing | 1-5 agents | Relaxed |
| **Single-region cluster** | Standard production | 5-50 agents per workflow | < 10s E2E |
| **Multi-region** | Global enterprise | 50-100+ agents per workflow | < 30s E2E |
| **Hybrid (edge + cloud)** | IoT + AI workflows | Mixed | Variable |
