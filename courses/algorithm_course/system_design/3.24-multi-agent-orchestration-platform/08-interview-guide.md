...
| **Slowest part of the process** | Supervisor can be overloaded | No single Slowest part of the process |
...
| **Single orchestrator** | Slowest part of the process at scale | Stateless orchestrators, partitioned |
...
- "Do agents need to interoperate across different frameworks?"
- "What's the LLM provider strategy -- single or multi-provider?"
...
An enterprise needs a multi-agent pipeline that processes incoming contracts: extract terms → validate compliance → flag risks → generate summary.

### Architecture Choice: Sequential Pipeline with Supervisor Oversight

```
User → Supervisor Agent
         ├── Extraction Agent (MCP: document parser tool)
         ├── Compliance Agent (MCP: regulation database tool)
         ├── Risk Agent (MCP: precedent search tool)
         └── Summary Agent
         → Final Report
```
...
| Decision | Choice | Why |
|----------|--------|-----|
| Pattern | Sequential with supervisor | Each step depends on previous; supervisor catches errors |
| Memory | Centralized shared | Small team, strong consistency needed for compliance |
| Handoff | Structured JSON with full context | Compliance requires complete audit trail |
| Model routing | Frontier model for risk/compliance, efficient model for extraction/summary | Balance cost and accuracy |
| Failure handling | Supervisor retry + human escalation | Compliance errors must not pass silently |

### Metrics That Matter

| Metric | Target | Why |
|--------|--------|-----|
| Extraction accuracy | > 98% | Bad extraction cascades to all downstream agents |
| End-to-end latency | < 60s per document | Batch processing window constraints |
| Cost per document | < $0.50 | 10,000 documents/day budget |
| Compliance flag recall | > 99.5% | Regulatory requirement |

### Lessons Learned

1. **Context compression is critical**: Full contract text (50K+ tokens) cannot be passed in every handoff; extraction agent must produce structured output, not pass-through raw text
2. **Supervisor adds 30% latency but catches 15% of agent errors**: Worth it for compliance use cases, optional for best-effort tasks
3. **MCP tool failures are the leading cause of pipeline stalls**: Tool circuit breakers and fallback mechanisms are essential
...
- **A2A**: Agent-to-agent (Google/Linux Foundation, 150+ organizations)

### Production Reality Check Numbers

- Only 5.2% of organizations have agents in production (2025)
- 61% stuck in exploration phase
- 60% of multi-agent pilots fail to scale
- Context window explosion is the #1 cost driver
