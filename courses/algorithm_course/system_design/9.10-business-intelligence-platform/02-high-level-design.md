...
flowchart TB
    ...
    subgraph NLP["NLP Pipeline"]
        PARSE[Intent & Entity Parser]
        RESOLVE[Entity → Semantic Field Resolver]
        DISAMBIG[Disambiguation Engine]
    end

    subgraph Semantic["Semantic Grounding"]
        MODEL_LOOKUP[Model Lookup - Fields, Synonyms, Descriptions]
        QUERY_BUILD[Semantic Query Builder]
        VALIDATE[Query Validator - Type Checking]
    end

    subgraph Execution["Normal Query Pipeline"]
        SEM_COMPILE[Semantic Layer Compilation]
        RLS_INJECT[RLS Injection]
    ...
    subgraph Output["Result Presentation"]
        VIZ_SELECT[Auto-Visualization Selector]
        RENDER[Chart Rendering]
        EXPLAIN[Interpretation Explanation]
    end

    NL --> PARSE --> RESOLVE --> DISAMBIG
    DISAMBIG --> MODEL_LOOKUP
    MODEL_LOOKUP --> QUERY_BUILD --> VALIDATE
    VALIDATE --> SEM_COMPILE --> RLS_INJECT --> SQL_GEN --> CACHE_CHECK --> EXEC
    EXEC --> VIZ_SELECT --> RENDER
    RENDER --> EXPLAIN

    classDef input fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef nlp fill:#fff3e0,stroke:#e65100,stroke-width:2px
    ...
    class NL input
    class PARSE,RESOLVE,DISAMBIG nlp
    class MODEL_LOOKUP,QUERY_BUILD,VALIDATE semantic
    class SEM_COMPILE,RLS_INJECT,SQL_GEN,CACHE_CHECK,EXEC exec
    class VIZ_SELECT,RENDER,EXPLAIN output
```

**Key design principle**: NLQ generates a **semantic query** (measures + dimensions + filters), not raw SQL. This ensures the query passes through the same compilation pipeline as all other queries---including RLS injection, fan-out handling, and caching---guaranteeing metric consistency and security.

**Disambiguation strategies:**
- **Field similarity ranking**: Embed semantic model field names, descriptions, and synonyms; rank by cosine similarity to extracted entities
- **Usage-weighted resolution**: When multiple fields match (e.g., "revenue" could be gross or net), prefer the field with higher query frequency
- **Clarification dialog**: When confidence is below threshold, present the user with 2--3 candidate interpretations before executing
...
flowchart LR
    subgraph Source["Source Layer"]
        COL_A[warehouse.orders.revenue]
        COL_B[warehouse.orders.created_at]
        COL_C[warehouse.customers.region]
    end

    subgraph Semantic["Semantic Layer"]
        MEASURE[Measure: Total Revenue - SUM revenue]
        DIM_TIME[Dimension: Order Quarter - QUARTER created_at]
        DIM_GEO[Dimension: Region]
    end

    subgraph Dashboard["Dashboard Layer"]
        W1[Widget: Revenue KPI Card]
        W2[Widget: Revenue by Quarter Chart]
        W3[Widget: Revenue by Region Map]
    end
    ...
    class COL_A,COL_B,COL_C source
    class MEASURE,DIM_TIME,DIM_GEO semantic
    class W1,W2,W3 dash
```

**Impact analysis use case**: Before modifying `warehouse.orders.revenue` column (e.g., renaming, changing calculation), the lineage graph shows that 3 dashboard widgets depend on it through the "Total Revenue" measure. The platform can notify affected dashboard owners, block the model deployment until owners acknowledge, or auto-update downstream references if the change is a simple rename.
