...
|--------|-------------|-------------|
| `cost.warehouse_scanned_bytes` | Bytes scanned at pay-per-query data sources | Per tenant, per data source, per dashboard |
| `cost.compute_seconds` | CPU seconds consumed by query compilation and execution | Per tenant, per query type |
| `cost.extract_cost` | Cost of extract jobs (source scan + transform + storage write) | Per tenant, per data source |
| `cost.cache_storage` | Cache storage consumed per tenant | Per tenant, per tier |
| `cost.network_egress` | Network transfer for federated queries and extract ingestion | Per tenant, per data source |
| `cost.aggregation_build` | Compute cost of pre-aggregation table builds | Per tenant, per aggregation |

### Cost Attribution Dashboard (Internal)

```
┌────────────────────────────────────────────────────────────────┐
│ Cost Attribution - Last 30 Days                                 │
├──────────────────────┬──────────────────────┬──────────────────┤
│ Total Query Cost      │ Top Cost Tenant       │ Cost per Query   │
│ [$145,230]            │ [Acme Corp: $12,400]  │ [$0.012 avg]     │
├──────────────────────┴──────────────────────┴──────────────────┤
│ [Chart: Daily cost trend by category (warehouse, extract,      │
│  compute, cache, network)]                                      │
├────────────────────────────────────────────────────────────────┤
│ [Table: Top 20 most expensive dashboards with owner]            │
│ Dashboard          │ Owner │ Daily Views │ Daily Cost │ $/View  │
│ Exec Summary       │ CFO   │ 5,200       │ $320       │ $0.06   │
│ Sales Pipeline     │ CRO   │ 12,000      │ $890       │ $0.07   │
├────────────────────────────────────────────────────────────────┤
│ [Chart: Cost per tenant tier (Enterprise / Mid-Market / SMB)]   │
└────────────────────────────────────────────────────────────────┘
```
...
```
FUNCTION detect_cost_anomaly(tenant_id, current_period):
    historical_avg = get_rolling_average(tenant_id, window = 30 days)
    historical_stddev = get_rolling_stddev(tenant_id, window = 30 days)

    IF current_period.cost > historical_avg + 3 * historical_stddev:
        // Significant cost spike detected
        top_queries = get_top_cost_queries(tenant_id, current_period, limit = 10)
        new_dashboards = get_new_dashboards(tenant_id, current_period)

        alert(
            severity = P2,
            message = "Tenant {tenant_id} cost spike: ${current_period.cost} vs ${historical_avg} avg",
            enrichment = { top_queries, new_dashboards }
        )
```
