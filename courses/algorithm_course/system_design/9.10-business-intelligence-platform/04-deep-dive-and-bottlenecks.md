...
### Slowest part of the process: Fan-Out and Symmetric Aggregates
...
### Slowest part of the process: Aggregation Staleness vs. Query Freshness
...
### Slowest part of the process: Widget Interaction Cascade
...
### Slowest part of the process: Data Transfer Costs
...
## Deep Dive 6: Visualization Grammar Type Checking

### The Problem

Users create visualizations by mapping data fields to visual channels (axes, color, size). Without validation, they can create misleading or meaningless charts---mapping a categorical field to a continuous axis, using a pie chart for non-additive measures, or encoding too many dimensions on a single chart.

### Visualization Type System

```
FUNCTION validate_visual_encoding(chart_spec, fields):
    errors = []
    warnings = []

    FOR channel IN chart_spec.encoding_channels:
        field = fields[channel.field_ref]

        // Rule 1: Data type → channel compatibility
        IF channel.type == POSITION_CONTINUOUS AND field.data_type == NOMINAL:
            errors.APPEND("Cannot map nominal field '{field.name}' to continuous axis. Use ordinal or quantitative.")

        IF channel.type == SHAPE AND field.data_type == QUANTITATIVE:
            warnings.APPEND("Shape encodes categories, not quantities. Consider using size or color for '{field.name}'.")

        IF channel.type == SIZE AND field.data_type == NOMINAL:
            errors.APPEND("Size channel requires quantitative or ordinal data. '{field.name}' is nominal.")

    // Rule 2: Chart-specific aggregation validation
    IF chart_spec.type == PIE_CHART:
        measure = chart_spec.measure_field
        IF measure.aggregation NOT IN [SUM, COUNT]:
            warnings.APPEND("Pie slices should represent parts of a whole. '{measure.name}' uses {measure.aggregation}, which may not sum to a meaningful total.")

    // Rule 3: Cardinality warnings
    color_field = chart_spec.get_channel(COLOR)
    IF color_field != NULL AND color_field.estimated_cardinality > 12:
        warnings.APPEND("Color channel has {color_field.estimated_cardinality} distinct values. Humans distinguish ~7 colors; consider filtering or grouping.")

    // Rule 4: Dual-axis validation
    IF chart_spec.has_dual_axis:
        IF chart_spec.y1.scale_range / chart_spec.y2.scale_range > 100:
            warnings.APPEND("Dual axes have very different scales. This can be visually misleading.")

    RETURN { errors, warnings }
```

### Slowest part of the process: Auto-Visualization Selection

When the system must automatically choose a chart type (NLQ results, default visualizations), it must map the query's field structure to the most effective visual encoding. The selection rules form a decision tree:

| Fields | Recommended Chart | Fallback |
|--------|-------------------|----------|
| 1 measure | KPI card / single number | Gauge if target exists |
| 1 measure + 1 temporal dimension | Line chart | Area chart |
| 1 measure + 1 nominal dimension (≤12 values) | Bar chart | Horizontal bar if labels are long |
| 1 measure + 1 nominal dimension (>12 values) | Top-N bar chart + "other" | Treemap |
| 1 measure + 2 dimensions (nominal + temporal) | Grouped bar or multi-line | Stacked bar if additive |
| 2 measures + 1 dimension | Scatter plot | Bubble chart if 3rd measure exists |
| 1 measure + geographic dimension | Choropleth map | Symbol map if point data |
| Multiple measures, no dimensions | KPI grid / scorecard | Table |
