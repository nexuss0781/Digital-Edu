...
| Query compilation (semantic → SQL) | < 100ms | Semantic layer compilation must not be the Slowest part of the process |
...
## Cost Model Estimations

### Infrastructure Cost Breakdown (Per 1,000 Tenants)

```
Compute:
  Query executors:        250 cores × $0.04/core-hour = $240K/year
  Semantic compilers:     100 cores × $0.04/core-hour = $96K/year
  Dashboard renderers:    30 cores × $0.04/core-hour  = $29K/year
  Extract workers:        1,800 cores (burst) × $0.02/core-hour = $315K/year
  Total compute:          ~$680K/year

Storage:
  Columnar extract store: 500 TB × $0.02/GB-month = $120K/year
  Query result cache:     100 TB × $0.10/GB-month = $1.2M/year (in-memory)
  Metadata + audit:       10 TB × $0.02/GB-month  = $2.4K/year
  Pre-aggregation:        50 TB × $0.02/GB-month  = $12K/year
  Total storage:          ~$1.34M/year

Network:
  Dashboard payload:      290 MB/s outbound × $0.05/GB = $460K/year
  Extract ingestion:      500 TB/day × $0.01/GB = $1.83M/year
  Internal traffic:       Included in compute
  Total network:          ~$2.29M/year

Total per 1K tenants:     ~$4.3M/year → ~$4,300/tenant/year
```

### Per-Query Cost Attribution

| Query Type | Avg Cost | Components |
|-----------|----------|------------|
| Cached query (L1 hit) | $0.000001 | Cache lookup + network |
| Cached query (L2 hit) | $0.000005 | Distributed cache read + network |
| Live query (warehouse) | $0.005--$0.50 | Warehouse compute (varies by data scanned) |
| Federated query | $0.01--$1.00 | Multi-source compute + network transfer |
| Extract refresh (incremental) | $0.10--$5.00 | Source scan + transform + storage write |
| Pre-aggregation build | $0.50--$50.00 | Full fact table scan + aggregate compute + storage |

### Embedded Analytics Scale Implications

```
Embedded dashboard instances:  10M active
Avg embed views per day:       100M (10× more views than native due to end-user volume)
Queries from embedded:         1B/day (adds 2× to base query volume)
RLS contexts (embedded):       50K unique (one per customer of each embedding tenant)
Cache partitions needed:       50K × avg 2,000 cached queries = 100M additional cache entries
Additional cache storage:      100M × 50 KB = 5 TB
```

This means embedded analytics, though using the same infrastructure, effectively **triples** the query volume and significantly expands cache requirements due to the high RLS context cardinality. Capacity planning must model embedded growth separately from native usage.
...
| ID | Requirement | Description |
|----|-------------|-------------|
| DG-01 | **Data Lineage** | Track field-level lineage from source database column through semantic model to dashboard widget; forward and backward traversal |
| DG-02 | **Impact Analysis** | Before a semantic model change, identify all affected dashboards, alerts, and scheduled reports |
| DG-03 | **Metric Certification** | Mark specific measures as "certified" by data team; visual indicator on dashboards using certified vs. uncertified metrics |
| DG-04 | **Content Lifecycle** | Automatically detect stale dashboards (no views in 90+ days); notify owners; archive after grace period |
| DG-05 | **Cost Attribution** | Attribute warehouse query costs to specific dashboards, tenants, and users for chargeback and optimization |
