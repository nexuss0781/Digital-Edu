...
## Deep Dive Probes: Advanced Topics

### Data Freshness Architecture

**Q: "A CEO opens their executive dashboard at 9 AM and sees revenue numbers from yesterday at 6 PM. They complain the data is stale. How do you fix this?"**

| Level | Answer Quality |
|-------|---------------|
| **Strong** | Identifies the end-to-end freshness pipeline (source pipeline latency + extract latency + cache TTL); proposes freshness SLA stacking analysis; suggests per-dashboard freshness targets; mentions "data as of" timestamps with source-aware staleness calculation; discusses the cost trade-off of more frequent extracts |
| **Acceptable** | Proposes increasing extract frequency; mentions cache invalidation on refresh; discusses trade-off between freshness and performance |
| **Weak** | "Just run the extract more often" without considering source pipeline latency; no mention of cascading freshness dependencies |

### Multi-Tenant Query Isolation

**Q: "Tenant A runs a massive ad-hoc query that scans 500 million rows. How do you prevent this from affecting Tenant B's dashboard load times?"**

Expected strategies:
- Per-tenant connection pool quotas (Tenant A can't consume Tenant B's connection slots)
- Per-tenant query concurrency limits (max 10 concurrent queries for standard tier)
- Query cost estimation at compilation time; reject queries exceeding cost budget
- Separate execution queues: interactive queries (dashboard loads) get priority over ad-hoc exploration
- Compute isolation for enterprise tenants: dedicated query executor pools
- Circuit breaker: if a tenant's queries consistently time out, throttle their query rate temporarily

### NLQ Architecture

**Q: "How does a natural language query like 'Show me revenue by region for last quarter' get executed?"**

| Level | Answer Quality |
|-------|---------------|
| **Strong** | NLQ maps to semantic model (not raw SQL); leverages field descriptions and synonyms for entity resolution; generates a semantic query (measures + dimensions + filters), not SQL; passes through normal RLS and caching pipeline; discusses disambiguation for ambiguous terms |
| **Acceptable** | Describes text-to-SQL with some awareness of the semantic layer; mentions disambiguation |
| **Weak** | "Use an LLM to generate SQL" with no mention of semantic layer grounding, RLS, or metric consistency |

---

## Follow-Up Depth Questions

These questions test whether a candidate can go deeper when probed:

| Initial Topic | Follow-Up | What It Tests |
|--------------|-----------|---------------|
| Semantic layer | "What happens when two views can both serve the 'revenue' measure via different join paths?" | Understanding of join graph ambiguity and canonical path selection |
| Caching | "How do you warm the cache after a data refresh without causing a query storm?" | Staggered warm-up; priority-based refresh; background re-execution |
| Dashboard rendering | "A widget depends on another widget's selection. How do you avoid a loading spinner cascade?" | Optimistic rendering; skeleton states; partial result streaming |
| Embedded analytics | "The embed token expires while the user is actively using the dashboard. What happens?" | Silent token refresh; embed gateway token rotation; WebSocket session handling |
| Pre-aggregation | "An aggregate table has different data than the raw table due to a refresh lag. A user drills from summary to detail and sees inconsistent numbers. How do you handle this?" | Freshness-aware drill-through; same-snapshot isolation; freshness warnings |
| Federation | "A cross-source join needs to transfer 10 GB of data from a warehouse. How do you optimize this?" | Bloom filter pushdown; predicate transfer; materialization for frequent joins; cost estimation |
