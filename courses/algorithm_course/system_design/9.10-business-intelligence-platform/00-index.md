...
| Pattern | How It Applies to BI Platforms |
|---------|-------------------------------|
| **CQRS (Command Query Responsibility Segregation)** | The BI platform is inherently CQRS: writes (model definitions, dashboard configurations) go to the metadata store, while reads (analytical queries) go through the semantic layer → query executor → data source pipeline. Read and write paths scale independently. |
| **Materialized View** | Pre-aggregation tables are materialized views that trade freshness for query performance. The auto-aggregation advisor continuously decides which materialized views to create, refresh, and retire based on query patterns. |
| **Compiler Pipeline** | The semantic layer follows a classic compiler architecture: parse (model DSL) → intermediate representation (join graph + field DAGs) → optimization passes (predicate pushdown, join elimination, aggregate routing) → code generation (SQL dialect adaptation). |
| **Circuit Breaker** | Data source connections use circuit breakers to prevent cascade failures. When a source is unresponsive, the breaker opens and queries fall back to cached results or local extracts. |
| **Reactive Streams** | Dashboard rendering uses reactive stream patterns: widget queries execute in parallel, results stream progressively to the client, and filter changes propagate through the dependency graph with debouncing and deduplication. |
| **Multi-Tier Cache** | Caching spans five tiers (widget, query result, extract, pre-aggregation, compiled model) with different TTLs, invalidation strategies, and RLS-aware partitioning---far more complex than typical web application caching. |
