...
## 11. Deletion Vectors Transform the MoR vs. CoW Trade-off by Eliminating the Delete File I/O Problem

**Category**: Write Strategy

**One-liner**: Inline bitmaps that mark deleted rows provide O(1) per-row filtering without separate file reads, collapsing the cost gap between MoR and CoW.

**Why it matters**: The traditional MoR approach writes a separate delete file (position deletes or equality deletes) for each batch of deletions. At read time, the engine must load each delete file, parse it, and apply it against the corresponding base file — an O(D × B) operation when D delete files span B base files. Deletion vectors replace this with a RoaringBitmap stored inline in the manifest entry or as a tiny sidecar (typically < 2 KB). Each bit represents a row position; set bits indicate deleted rows. The read-time cost drops to a single bitmap check per row — O(1) with no additional file I/O. This effectively eliminates the biggest argument against MoR: read-time merge overhead. With deletion vectors, a table can accept frequent single-row deletes (GDPR erasure, correction updates) without degrading read performance, and without the write amplification of CoW (which would rewrite a 256 MB file to delete one row). Compaction is still needed eventually — to physically reclaim space and re-cluster data — but the urgency drops from "critical to prevent read degradation" to "beneficial for storage efficiency." This shifts the MoR vs. CoW conversation from a binary architectural choice to a nuanced cost-optimization decision where deletion vectors are the default for most update patterns.

---

## 12. The Catalog's Credential Vending Function Makes It the Most Security-Critical Component — Not Just an Availability Dependency

**Category**: Security

**One-liner**: Because the catalog brokers all access to data files via short-lived, scoped credentials, compromising the catalog compromises the entire lakehouse's data, not just its metadata.

**Why it matters**: When discussing the catalog, most architects focus on its role in atomic commits (CAS on the snapshot pointer) and its availability as a dependency for query planning. But the catalog's credential-vending function makes it far more critical than an availability concern. Every query engine, ingestion pipeline, and compaction worker receives its object-storage credentials from the catalog. These credentials are scoped to specific file paths and have short TTLs (typically 15 minutes). This design enforces least-privilege access — an engine receives credentials only for the files it needs for the current query. However, it also means that the catalog holds the keys to all data in the lakehouse. A compromised catalog can vend broadly-scoped credentials to an attacker, granting access to any table's data files. Unlike a traditional database breach (which requires extracting data through the query interface), a catalog breach exposes raw files on object storage — the attacker can download entire Parquet files at storage-bandwidth speed, bypassing all query-level access controls, column masking, and row filters. This security posture demands that the catalog be treated with the same rigor as a secrets management system: hardware-backed key storage, mTLS for all API traffic, rate-limited credential vending with anomaly detection, and an immutable audit log of every credential issued.
