...
- **Cryptocurrency or digital asset lending** — Fiat currency lending only; no crypto collateral or DeFi integration
- **Credit card products** — Focus on MSME-specific lending products; no consumer revolving credit cards
- **Wealth management or advisory** — Lending platform only; no investment advisory or portfolio management
- **Peer-to-peer lending marketplace** — Direct or co-lending model; no P2P platform operation
- **Gold or collateral-backed lending** — Unsecured digital lending; no physical collateral management
- **Microfinance group lending (JLG/SHG)** — Individual MSME lending; no joint liability group management
- **Trade finance or letter of credit** — Domestic working capital only; no international trade instruments
...
| Model inference (p99) | ≤ 200 ms | Credit scoring must not be the Slowest part of the process in the decision pipeline |
...
| KFS generation on approval | 100% of approvals | Per decision |
| Grievance resolution | ≤ 15 days | Per grievance |
| Bureau reporting lag | ≤ 15 days from month-end | Monthly |
...
|---|---|---|---|
| **Fraud vs. speed** | More aggressive fraud checks (2s+ latency, fewer false negatives) | Faster decisions (200ms, higher fraud leakage) | Tiered: fast-path rules (50ms) for 95% of applications; slow-path graph analysis (2s) for flagged cases |
| **Data completeness vs. decision speed** | Wait for all data sources (60s+ latency) | Decide on partial data (30s) | Decide on partial data with wider confidence intervals; route low-confidence decisions to manual review |
| **Model accuracy vs. fairness** | Unconstrained model (Gini 0.44) | Adversarially debiased model (Gini 0.40) | Debiased model at the Pareto frontier knee; accept 4-point Gini loss for fair lending compliance |
| **Consent freshness vs. UX friction** | Frequent consent renewal (every 30 days) | Long consent duration (1 year) | 6-month consent with renewal nudges at 30 days before expiry; balance data freshness with borrower friction |
| **Collection intensity vs. customer experience** | Aggressive multi-channel contact (higher recovery) | Conservative contact (lower complaint rate) | ML-optimized contact: high intensity for high-probability cases, minimal contact for low-probability (saves cost and preserves relationship) |
| **Auto-approve rate vs. manual review quality** | High auto-approve (40%) with higher leakage | Low auto-approve (20%) with more manual reviews | Dynamic: 30% auto-approve baseline; increase for repeat borrowers with clean history; decrease during fraud spikes |
...
```
Year 1 (Launch → Steady State):
  Monthly applications: 50K → 500K
  Active portfolio: 0 → 3M loans (₹6,000 crore outstanding)
  Partners: 5 embedded finance integrations
  Products: Term loan + credit line
  Co-lending: 1 bank partner

Year 2 (Scale):
  Monthly applications: 500K → 1.2M
  Active portfolio: 3M → 10M loans (₹20,000 crore outstanding)
  Partners: 20 embedded finance integrations
  Products: + Invoice financing + supply chain finance
  Co-lending: 3 bank partners + capital market instruments

Year 3 (Platform):
  Monthly applications: 1.2M → 2.5M
  Active portfolio: 10M → 25M loans (₹50,000 crore outstanding)
  Partners: 50 embedded finance integrations
  Products: + UPI credit line + seasonal agriculture finance
  Co-lending: 5+ bank partners + securitization
  Geographic: domestic full coverage + 1 international market pilot
```
