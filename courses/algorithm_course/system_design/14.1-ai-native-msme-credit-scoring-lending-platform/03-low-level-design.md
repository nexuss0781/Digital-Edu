...
```
ModelRegistryEntry:
  model_id:                string          # globally unique model identifier
  model_version:           string          # semantic version (e.g., "4.2.1")
  segment:                 enum[BUREAU_PLUS, THIN_FILE, NEW_TO_CREDIT]
  product:                 enum[TERM_LOAN, CREDIT_LINE, INVOICE_FINANCING]
  role:                    enum[CHAMPION, CHALLENGER, SHADOW, RETIRED]
  artifact:
    artifact_path:         string          # path in model artifact store
    artifact_hash:         string          # SHA-256 hash of model binary
    artifact_size_mb:      float64
    framework:             string          # e.g., "xgboost-2.0", "lightgbm-4.1"
  training:
    training_data_start:   date            # earliest label date in training set
    training_data_end:     date            # latest label date (must be 90+ days old)
    sample_count:          int             # number of training samples
    feature_count:         int             # number of features used
    feature_list:          [string]        # ordered feature names
    hyperparameters:       map[string, any]
  performance:
    gini_coefficient:      float64         # on out-of-time validation set
    ks_statistic:          float64
    auc_roc:               float64
    brier_score:           float64
    vintage_default_rate:  map[string, float64]  # risk_grade → 90-day default rate
  fairness:
    gender_approval_gap:   float64         # percentage points
    geo_variance:          float64
    equalized_odds_gap:    float64
    fairness_certified:    boolean
    certified_by:          string
    certification_date:    datetime
  lifecycle:
    created_at:            datetime
    deployed_at:           datetime
    promoted_at:           datetime        # when promoted to champion
    retired_at:            datetime
    approved_by:           string          # human approver ID
    approval_notes:        string
```

### UPI Credit Line Record

```
UPICreditLine:
  credit_line_id:          string          # globally unique credit line identifier
  borrower_id:             string
  bank_partner_id:         string          # issuing bank
  upi_credential_id:       string          # NPCI credential reference

  limits:
    sanctioned_limit:      float64         # total approved credit line
    available_balance:      float64         # real-time available balance
    utilized_amount:        float64         # currently drawn amount
    min_payment_due:        float64         # minimum payment for current cycle
    total_outstanding:      float64         # including unbilled + billed
  ...
  billing:
    ...
    statement_date:        date
    due_date:              date
    interest_rate_monthly: float64         # monthly interest rate
    transactions_in_cycle: int

  authorization:
    max_per_txn:           float64         # maximum single transaction amount
    daily_limit:           float64         # maximum daily draw
    blocked_mccs:          [string]        # blocked merchant category codes
    velocity_limit_hourly: int             # max transactions per hour

  status:
    line_status:           enum[ACTIVE, FROZEN, SUSPENDED, CLOSED]
    last_transaction_at:   datetime
    next_review_date:      date            # periodic limit review

UPICreditLineTransaction:
  transaction_id:          string
  credit_line_id:          string
  upi_ref_id:             string          # UPI transaction reference
  merchant_vpa:           string
  merchant_mcc:           string          # merchant category code
  amount:                 float64
  authorization_latency_ms: int           # time to authorize
  authorization_result:   enum[APPROVED, DECLINED_LIMIT, DECLINED_FRAUD, DECLINED_MCC]
  timestamp:              datetime
  billing_cycle:          string          # which billing cycle this belongs to
  emi_converted:          boolean         # whether converted to EMI
```

### Stacking Detection Consortium Record

```
ConsortiumQueryRecord:
  query_id:                string
  lender_id:               string          # querying lender (hashed)
  query_timestamp:         datetime
  identifiers:
    pan_hash:              string          # SHA-256 hash of PAN
    phone_hash:            string          # SHA-256 hash of phone number
    device_hash:           string          # SHA-256 hash of device fingerprint
  response:
    applications_7d:       int             # applications at other consortium members
    applications_30d:      int
    sanctions_7d:          int             # sanctioned loans at other members
    sanctions_30d:         int
    total_sanctioned_amount_bucket: enum[UNDER_1L, 1L_5L, 5L_10L, 10L_25L, ABOVE_25L]
    most_recent_sanction_days_ago: int
    risk_flag:             enum[CLEAR, MODERATE_STACKING, HIGH_STACKING, EXTREME_STACKING]
