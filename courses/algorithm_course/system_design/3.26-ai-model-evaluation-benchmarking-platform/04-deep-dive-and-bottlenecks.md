...
| Slowest part of the process | Symptom | Root Cause | Impact |
|------------|---------|------------|--------|
...
## Critical Component 4: Agentic and Multimodal Evaluation Architecture

### The Core Challenge

As AI systems evolve from single-turn text models to multi-step agents and multimodal models (vision-language, audio), evaluation platforms must extend beyond simple input-output scoring to handle trajectory evaluation, tool use verification, and cross-modal faithfulness.

### Agentic Evaluation Pipeline

Agent workflows produce execution trajectories rather than single outputs. Evaluation must score each step and the overall trajectory.

```
FUNCTION evaluate_agent_trajectory(trajectory, task_spec):
    """
    Evaluate a multi-step agent execution trajectory.
    trajectory: [{step, action, tool, args, result, reasoning}, ...]
    task_spec: {goal, constraints, expected_tools, ground_truth}
    """
    ...
    FOR i, step IN ENUMERATE(trajectory):
        ...
        IF step.tool IN task_spec.expected_tools:
            step_score.tool_selection = 1.0
        ELSE:
            step_score.tool_selection = LLM_JUDGE(
                "Was {step.tool} a reasonable choice for: {step.reasoning}?",
                rubric=TOOL_SELECTION_RUBRIC
            )

        // Argument correctness
        step_score.arg_quality = LLM_JUDGE(
            "Are these arguments correct for {step.tool}?",
            context=trajectory[:i],  // Prior context
            rubric=ARG_CORRECTNESS_RUBRIC
        )

        // Recovery quality (only for error steps)
        IF step.result.is_error:
            next_step = trajectory[i+1] IF i+1 < LEN(trajectory) ELSE None
            step_score.recovery = evaluate_recovery(step, next_step)

        step_scores.append(step_score)

    // Trajectory-level scores
    trajectory_score = {
        task_completion: evaluate_final_result(trajectory[-1], task_spec.ground_truth),
        efficiency: LEN(task_spec.expected_tools) / LEN(trajectory),
        token_cost: SUM(step.token_count FOR step IN trajectory),
        step_scores: step_scores,
        overall: WEIGHTED_AVERAGE(step_scores, weights=STEP_WEIGHTS)
    }
...
|--------|----------|-------------|
| **Task Completion Rate** | Did the agent achieve the goal? | Binary or partial credit scoring against ground truth |
| **Tool Selection Accuracy** | Did the agent pick the right tools? | % of steps using expected tools from the optimal path |
| **Step Efficiency** | How many steps vs. optimal path? | Optimal steps / actual steps (1.0 = optimal) |
| **Error Recovery Rate** | Did the agent recover from errors? | % of errors followed by successful correction |
| **Token Efficiency** | How many tokens were consumed? | Total tokens / baseline tokens for equivalent task |
| **Trajectory Coherence** | Was the reasoning chain logical? | LLM-as-Judge scoring of step-to-step reasoning flow |

### LLM-as-Judge Debiasing

Research (2024-2025) identified systematic biases in LLM judges that corrupt evaluation results if left unaddressed.

| Bias | Effect | Debiasing Technique |
|------|--------|-------------------|
| **Position Bias** | Favors first (or last) response in pairwise comparison by 5-15% | Evaluate each pair twice with swapped positions; average scores |
| **Verbosity Bias** | Prefers longer responses regardless of quality | Include length penalty in rubric; normalize scores by response length |
| **Self-Enhancement Bias** | Models rate their own family's outputs higher | Use cross-provider judge panels (e.g., GPT-4 judges Claude outputs, vice versa) |
| **Authority Bias** | Favors responses that cite sources or use confident language | Calibrate on examples where confident responses are wrong |
| **Format Bias** | Prefers markdown-formatted or bulleted responses over prose | Standardize output format before evaluation or instruct judge to ignore formatting |

```
FUNCTION debiased_pairwise_judge(output_a, output_b, rubric, judge_model):
    // Evaluate A-first
    score_ab = LLM_JUDGE(
        prompt=PAIRWISE_PROMPT(first=output_a, second=output_b, rubric=rubric),
        model=judge_model,
        temperature=0
    )

    // Evaluate B-first (position swap)
    score_ba = LLM_JUDGE(
        prompt=PAIRWISE_PROMPT(first=output_b, second=output_a, rubric=rubric),
        model=judge_model,
        temperature=0
    )
    ...
    IF score_ab.winner == "first" AND score_ba.winner == "second":
        // Consistent: A wins regardless of position
        RETURN {winner: "A", confidence: HIGH, bias_detected: false}
    ELSE IF score_ab.winner == "second" AND score_ba.winner == "first":
        // Consistent: B wins regardless of position
    ...
    ELSE:
        ...
        RETURN {winner: "TIE", confidence: LOW, bias_detected: true,
                escalate: true}  // Send to additional judge or human
```

---

### Case Study: Building a Regression Testing Pipeline for a Financial Services LLM

**Context:** A financial services firm deploys an LLM-powered compliance assistant that reviews loan applications against 200+ regulatory rules. Before any model or prompt update, the evaluation platform must verify that no regulatory check regresses.

**Phase 1: Golden Dataset Construction**

The team created a versioned golden dataset of 5,000 loan application scenarios with known correct regulatory outcomes:
- 2,000 clear-approval cases across 15 loan types
- 1,500 clear-rejection cases covering each regulatory rule
- 1,000 edge cases requiring nuanced judgment (identified from production disagreements)
- 500 adversarial cases designed to trigger common LLM failure modes (contradictory information, ambiguous phrasing)

Each case included ground truth labels from 3 regulatory experts with Krippendorff's Alpha > 0.85.

**Phase 2: Tiered Evaluation Architecture**

| Tier | Coverage | Metrics | Gate Criteria |
|------|----------|---------|---------------|
| Tier 1 (Programmatic) | 100% of 5,000 cases | Exact match on regulatory decision, JSON schema validation | > 99% schema compliance |
| Tier 2 (Fast LLM) | 100% of 500 adversarial cases | Binary pass/fail on reasoning quality | > 90% pass rate |
| Tier 3 (Full G-Eval) | 100% of 1,000 edge cases | Multi-criteria scoring (accuracy, completeness, citation quality) | No single rule drops > 5% from baseline |

**Phase 3: Distributional Regression Detection**

The key insight was that aggregate accuracy was insufficient. A model update that improved overall accuracy by 2% simultaneously degraded Fair Housing Act compliance checks by 12%.

The team implemented per-regulation regression testing:
- Each of the 200+ regulatory rules was tested independently
- Bonferroni-corrected significance threshold (p < 0.05/200 = 0.00025) prevented false alarms
- Any single-rule regression exceeding 5% absolute blocked the deployment
- Dashboard showed a heatmap of per-rule score changes across model versions

**Results:**

| Metric | Before Platform | After Platform |
|--------|----------------|----------------|
| Regression escape rate | 15% of deployments had production issues | < 1% regression escapes |
| Deployment velocity | 1 update per quarter (manual review) | 2 updates per month (automated gates) |
| Evaluation cost per deployment | $12,000 (manual expert review) | $850 (automated tiered evaluation) |
| Mean time to detect regression | 14 days (production monitoring) | 0 days (pre-deployment gate) |
| Per-rule regression visibility | None (aggregate only) | Full heatmap across 200+ rules |
...
| LLM-as-Judge | Position debiasing | 10-15% accuracy improvement |
...
| Agent Evaluation | Trajectory decomposition | Enables multi-step quality measurement |
