...
### Emerging Requirements (P1 - 2025-2026)

| Feature | Description | Key Capabilities |
|---------|-------------|------------------|
| **Agentic Evaluation** | Evaluate multi-step agent trajectories | Step-by-step scoring, tool use accuracy, trajectory coherence, task completion rate |
| **LLM Judge Debiasing** | Mitigate systematic biases in LLM-as-Judge | Position randomization, cross-provider panels, calibration against human labels |
| **Evaluator Versioning** | Track evaluation criteria as versioned artifacts | Evaluator config versioning, historical comparability, re-evaluation capability |
| **Distributional Regression Testing** | Detect per-category quality regressions | Per-slice statistical tests, hierarchical testing, FDR correction |
| **Multimodal Evaluation** | Evaluate vision-language and audio-language outputs | Image faithfulness, cross-modal consistency, VHELM/HEIM benchmark support |
| **Evaluator Distillation** | Train custom domain-specific judge models | Distillation pipeline, agreement benchmarking, cost comparison |
| **EU AI Act Compliance Reporting** | Generate required evaluation documentation for high-risk AI | Bias testing reports, red-team summaries, evaluation methodology audit trail |
...
| **Synthetic Data Generation** | Generate test cases automatically | Edge Case (Unusual or extreme situation) generation, adversarial examples |
| **Leaderboard Hosting** | Public model comparison dashboards | Rankings, submissions, auto-evaluation |
| **Federated Evaluation** | Cross-organizational benchmark runs | On-premises data, centralized score aggregation |
| **Continuous Production Evaluation** | Real-time evaluation integrated into production traffic | Streaming evaluation, sampling controls, cost budgets |
