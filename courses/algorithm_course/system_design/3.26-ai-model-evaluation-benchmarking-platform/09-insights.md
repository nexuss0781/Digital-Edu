...
## Insight 6: Multi-Provider LLM Load Balancing Turns Rate Limits from a Slowest part of the process into a Feature
...
## Insight 9: LLM-as-Judge Position Bias Requires Systematic Debiasing

**Category:** Consistency
**One-liner:** LLM judges systematically favor the first or second response in pairwise comparisons, requiring position randomization and averaging to produce unbiased rankings.

**Why it matters:** Research consistently shows that LLM-as-Judge evaluators exhibit position bias -- when comparing two outputs, GPT-4 favors the first position by 5-15% depending on the task. Left uncorrected, this bias silently corrupts A/B test conclusions and benchmark rankings. The debiasing protocol requires evaluating each pair twice (A,B and B,A) and averaging scores. If the two evaluations disagree (e.g., each position wins once), the pair is marked as a tie or escalated to a third evaluation with different framing. Beyond position bias, LLM judges also exhibit verbosity bias (preferring longer responses) and self-enhancement bias (models rate their own family's outputs higher). Mitigating these requires rubric engineering that explicitly penalizes length-padding and multi-judge panels that use models from different providers. The broader architectural lesson is that any automated evaluation system must be calibrated against known biases, just as human annotation systems calibrate against annotator drift.

---

## Insight 10: Agentic Evaluation Requires Trajectory Scoring, Not Just Outcome Measurement

**Category:** System Modeling
**One-liner:** Evaluating AI agents demands scoring the entire trajectory (tool selections, intermediate reasoning, error recovery) rather than just the final output, because two agents can reach the same answer through vastly different quality paths.

**Why it matters:** Traditional evaluation treats model output as a single string to score. Agent evaluation breaks this model entirely. An agent that solves a coding task by correctly identifying the bug, selecting the right tool, and applying a targeted fix scores differently from one that brute-forces through trial-and-error even if both produce a passing test suite. The evaluation platform must decompose agent trajectories into scored steps: tool selection accuracy (did the agent use the right tool?), tool argument correctness (did it pass the right parameters?), recovery quality (did it handle errors gracefully?), and efficiency (how many steps and tokens did it consume?). Frameworks like Inspect AI and agentic RAGAS extensions implement this by recording full execution traces and applying per-step rubrics. The scalability challenge is that trajectory evaluation is 5-10x more expensive than single-turn evaluation because each step requires context from all previous steps, making LLM-as-Judge calls sequentially dependent rather than parallelizable.

---

## Insight 11: Evaluator Versioning Is as Critical as Model Versioning

**Category:** Consistency
**One-liner:** When the evaluation criteria change (new rubric, new judge model, updated prompt), all historical scores become incomparable unless the platform tracks evaluator versions alongside result versions.

**Why it matters:** Teams commonly update their evaluation prompts, switch LLM judge models, or refine scoring rubrics -- and then wonder why their historical trend lines show a discontinuity. The root cause is that the measuring instrument changed, not the model quality. The platform must treat the evaluator configuration (judge model, prompt template, scoring rubric, temperature, metric weights) as a versioned artifact, stored alongside each evaluation result. When displaying trends or comparisons, the system must filter by evaluator version or clearly mark version boundaries. Re-evaluation capabilities (replaying a dataset version through a new evaluator version) allow retroactive comparison. This is analogous to the metrology principle that measurement calibration records must accompany measurement results -- without knowing which ruler was used, the measurement is meaningless.

---

## Insight 12: Regression Testing for LLMs Requires Distributional Comparison, Not Point Estimates

**Category:** Scaling
**One-liner:** Comparing average scores between model versions masks regressions on specific subpopulations; distributional comparison (per-category, per-difficulty) catches targeted quality drops that aggregate metrics hide.

**Why it matters:** A model upgrade might improve overall MMLU accuracy from 82% to 84% while simultaneously degrading performance on medical reasoning from 75% to 60%. If the regression testing engine only checks aggregate scores, this critical failure passes the gate. The platform must decompose evaluation results by category, difficulty level, input length, language, and domain -- then run statistical tests on each slice independently. The challenge is combinatorial: with 57 MMLU categories and 3 difficulty levels, there are 171 slices, each requiring sufficient sample size for statistical power. The solution is hierarchical testing: first test the aggregate, then test major categories, then drill into subcategories only where the parent shows movement. This limits the multiple comparison problem while still catching targeted regressions. Bonferroni correction or false discovery rate (FDR) control manages the family-wise error rate across multiple simultaneous tests.
