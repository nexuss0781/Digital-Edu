...
## Emerging Topics (2025-2026)

### Agentic Evaluation

Multi-step agent workflows require trajectory-level evaluation beyond single-turn output scoring:

| Challenge | Why It's Hard | How to Address |
|-----------|--------------|----------------|
| **Trajectory scoring** | Two agents can reach the same answer through vastly different quality paths | Score each step (tool selection, argument quality, recovery) plus overall outcome |
| **Sequential dependency** | Each step's evaluation requires context from all previous steps | LLM-as-Judge calls are sequentially dependent, 5-10x more expensive than single-turn |
| **Tool use verification** | Agent may use correct but suboptimal tools | Compare against expected tool sequences; partial credit for reasonable alternatives |
| **Non-determinism** | Same agent produces different trajectories on each run | Multiple trajectory evaluations with statistical aggregation |

**Interview Talking Point:**
> "Agent evaluation fundamentally changes the evaluation model from input-output pairs to trajectory scoring. We decompose each trajectory into scored steps -- tool selection accuracy, argument correctness, recovery quality, and efficiency -- then aggregate into an overall trajectory score. The key challenge is that step evaluations are sequentially dependent, making parallelization impossible within a single trajectory."

### LLM-as-Judge Bias Mitigation

Research (2024-2025) has documented systematic biases in LLM judges:

| Bias | Impact | Mitigation |
|------|--------|------------|
| **Position bias** | 5-15% preference for first response in pairwise comparisons | Evaluate pairs twice with swapped positions; average scores |
| **Verbosity bias** | Longer responses scored higher regardless of quality | Normalize by length; penalize padding in rubric |
| **Self-enhancement bias** | Models rate their own family's outputs higher | Cross-provider judge panels |

**Interview Talking Point:**
> "We don't trust a single LLM judge evaluation. For pairwise comparisons, we swap positions and average. For scoring, we use cross-provider panels -- GPT-4 judges Claude outputs and vice versa. We calibrate every judge against a human-labeled gold set and track agreement drift over time."

### Evaluator Model Distillation

Organizations increasingly train smaller, cheaper judge models rather than relying on expensive frontier models:

**Interview Talking Point:**
> "At scale, even GPT-4o-mini is expensive for evaluation. We can distill a domain-specific judge model by training on 10K examples of frontier-model evaluations aligned with human labels. The distilled model runs at 10x lower cost with 90%+ agreement with the frontier judge, but only for the specific evaluation criteria it was trained on."

### Trade-off: Evaluating the Evaluator

| Approach | Pros | Cons |
|----------|------|------|
| **Human agreement rate** | Gold standard for quality | Expensive, slow to measure |
| **Cross-judge consistency** | Cheap, automated | May reflect shared biases |
| **Known-answer accuracy** | Objective, automated | Only works for tasks with clear ground truth |
| **Flip-rate on perturbations** | Tests robustness | Doesn't test accuracy directly |

### Trap Questions: Emerging Topics

**Trap 7: "How do you evaluate an AI agent?"**

**What Interviewer Wants:** Understanding of trajectory evaluation beyond single-turn scoring

**Best Answer:**
> "Agent evaluation requires decomposing the execution trajectory into scored steps rather than just scoring the final output. We measure tool selection accuracy (did it pick the right tool?), argument quality (correct parameters?), recovery from errors, and overall efficiency (steps vs. optimal path). The key architectural challenge is that step evaluations are sequentially dependent -- each step's context includes all prior steps -- so we can't parallelize within a trajectory, making it 5-10x more expensive than single-turn evaluation."

**Trap 8: "Can you trust LLM-as-Judge results?"**

**What Interviewer Wants:** Awareness of systematic biases and calibration requirements

**Best Answer:**
> "Not naively. LLM judges have documented biases -- position bias (5-15% preference for first response), verbosity bias, and self-enhancement bias. We mitigate with position swapping for pairwise comparisons, cross-provider judge panels, and continuous calibration against human-labeled gold sets. We version our evaluator configurations and track agreement drift over time, re-calibrating when agreement drops below 80%."
