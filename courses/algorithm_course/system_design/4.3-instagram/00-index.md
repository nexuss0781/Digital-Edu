...
| Custom Hardware | MSVP video transcoding ASIC | Purpose-built silicon for media-first workloads |
| ML Scale | 1,000+ production models with automated lifecycle | Model Registry + Stability Metric + Launch Pipeline |
| User Agency | "Your Algorithm" controllable ranking (Dec 2025) | Explicit preference channel alongside behavioral signals |
...
| Explore Recommendations | Very High | 65B features, 90M predictions/sec, three-stage ranking funnel |
...
| **Explicit Preferences** | High (new) | "Your Algorithm" tool user-set topic preferences (Dec 2025) |
| **Content Originality** | Medium (new) | Prioritizes original content over reposts/aggregated content |
...
- [ ] Discuss MSVP custom silicon for video transcoding (9x H.264, 50x VP9)
- [ ] Explain the three-stage ranking funnel (Retrieval → ESR → LSR)
- [ ] Describe journey to 1,000+ ML models (Model Registry, Stability Metric, Launch Pipeline)
- [ ] Discuss LLM-based content moderation (semantic reasoning layer)
...
│  • MSVP custom ASIC: H.264 9x, VP9 50x faster at ~10W                 │
│  • Super Resolution (server-side + ExecuTorch on-device)               │
...
│                                                                         │
│  EXPLORE:                                                               │
│  • Three-stage: Retrieval → ESR → LSR (1,500 candidates for Explore)   │
│  • Andromeda: 10,000x model capacity, sublinear inference cost         │
│  • 65B features extracted per request                                  │
│  • 90M predictions/sec, 1,000+ ML models                              │
│                                                                         │
│  RANKING SIGNALS:                                                       │
...
|------------|--------|-------------|
| **MSVP Custom ASIC** | H.264 9x faster, VP9 50x faster at ~10W | First custom video transcoding silicon; eliminates "generalist tax" of general-purpose CPUs for media-first workloads |
| **Journey to 1,000+ ML Models** | Model Registry + Stability Metric + Launch Pipeline | Deployment velocity *increased* as model count grew—proving ML at scale is an infrastructure problem, not a modeling problem |
| **Three-Stage Ranking Funnel** | Retrieval → ESR → LSR with 1,500 Explore candidates | Early-Stage Ranker (ESR) added between retrieval and Late-Stage Ranker (LSR) for cost-efficient candidate filtering |
| **GEM Foundation Model** | +5% ad conversions | Generative Ads Recommendation Model trained at LLM scale; treats ads ranking as a generative task |
| **ExecuTorch 1.0 GA** | 50KB runtime footprint | Powers on-device features: Instagram Cutouts (SqueezeSAM), client-side Super Resolution, on-device content understanding |
| **"Your Algorithm" Tool** (Dec 2025) | User-controllable recommendation preferences | Explicit preference channel alongside implicit behavioral signals; responds to EU AI Act/DSA requirements |
| **LLM-Based Content Moderation** | 5K scam attempts/day caught, 80%+ impersonation reduction | LLaMA models add semantic reasoning layer that catches adversarial content traditional classifiers miss |
| **Diversity-Aware Notification Ranking** | Score(c) = R(c) × D(c) with maximal marginal relevance | Prevents notification fatigue by balancing relevance with diversity across notification types |
| **$72B Capex (2025)** | $115-135B planned 2026; Hyperion data center (2,250 acres, 5GW) | Infrastructure investment at semiconductor-industry scale; MSVP, custom AI training chips, liquid cooling |
...
| [LLM Inference Engine](../3.23-llm-inference-engine/00-index.md) | LLM-based moderation, on-device inference |
| [ML Platform (Uber Michelangelo)](../3.5-uber-michelangelo-ml-platform/00-index.md) | Model registry, ML lifecycle management at scale |
| [TikTok](../4.5-tiktok/00-index.md) | Short-form video ranking, interest-graph vs social-graph |
