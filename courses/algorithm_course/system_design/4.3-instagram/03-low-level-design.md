...
Time Complexity (Speed of the algorithm): O(n * m) where n = candidates, m = features per candidate
Space Complexity (Memory usage of the algorithm): O(n) for storing scored posts
```
...
Time Complexity (Speed of the algorithm): O(log n) for ANN search + O(k) for cluster queries
Space Complexity (Memory usage of the algorithm): O(candidates) ~ O(5000)
```
