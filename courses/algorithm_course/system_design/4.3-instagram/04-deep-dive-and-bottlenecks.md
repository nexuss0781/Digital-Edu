...
```
Step-by-step plan in plain English: ProcessingRetryLogic
...
```
Step-by-step plan in plain English: StoryExpirationService
...
### Three-Stage Ranking Funnel

Instagram evolved from a two-stage to a three-stage ranking funnel with the addition of an Early-Stage Ranker (ESR) between retrieval and the Late-Stage Ranker (LSR):

```
┌─────────────────────────────────────────────────────────────────────────┐
│                  THREE-STAGE RANKING FUNNEL (2025)                       │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  STAGE 1: RETRIEVAL (Light Models, <50ms)                              │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • Andromeda deep retrieval (per-user customized DNN)              │ │
│  │ • Interest-based ANN search (2,000 items)                        │ │
│  │ • Community cluster retrieval (2,500 items)                      │ │
│  │ • Trending/global content (500 items)                            │ │
│  │ Output: ~5,000 candidates (Feed), ~1,500 (Explore)              │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                              ↓                                          │
│  STAGE 2: EARLY-STAGE RANKER (ESR, <30ms)                             │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • Lightweight scoring model (fewer features than LSR)            │ │
│  │ • Filters candidates using cost-efficient inference              │ │
│  │ • Reduces candidate set while preserving recall                  │ │
│  │ • Trade-off: Small quality loss for major compute savings        │ │
│  │ Output: ~500-1,000 candidates                                    │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                              ↓                                          │
│  STAGE 3: LATE-STAGE RANKER (LSR, <200ms)                             │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • Full feature extraction (65B features across all candidates)   │ │
│  │ • Heavy multi-task models predicting:                            │ │
│  │   - Engagement (like, comment, share, save probabilities)        │ │
│  │   - Watch time prediction                                        │ │
│  │   - Quality and integrity scoring                                │ │
│  │ • 1,000+ models applied in ensemble                              │ │
│  │ • Diversity constraints and content-type balance                  │ │
│  │ Output: ~200 ranked posts                                        │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│  WHY THREE STAGES:                                                     │
│  • Two-stage applied heavy LSR to 5,000 candidates (expensive)        │
│  • ESR reduces to ~500-1,000 at 10x lower cost per candidate          │
│  • Net effect: ~60% reduction in ML inference compute for ranking     │
│  • Quality impact: <1% engagement regression (acceptable trade-off)   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### Slowest part of the process 1: Celebrity Uploads
...
### Slowest part of the process 2: Explore Candidate Explosion
...
### Slowest part of the process 3: Stories Cache Invalidation
...
## Deep Dive 4: MSVP Custom Silicon for Video Transcoding

### Why This Is Critical

Video transcoding is Instagram's single largest compute workload. At 95 million uploads per day, with each video requiring multiple encoding passes (H.264 fast path, AV1 quality path, multiple ABR variants), transcoding consumes more CPU cycles than all other services combined. General-purpose CPUs waste transistors on capabilities (speculative execution, branch prediction) that video encoding never uses.
...
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ Dedicated hardware blocks for each encoding stage:               │ │
│  │                                                                   │ │
│  │ ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐          │ │
│  │ │ Motion   │→ │Transform │→ │Quantize  │→ │ Entropy  │          │ │
│  │ │Estimation│  │  (DCT)   │  │          │  │ Coding   │          │ │
│  │ └──────────┘  └──────────┘  └──────────┘  └──────────┘          │ │
│  │                                                                   │ │
│  │ Each block is hardwired for the specific computation:            │ │
│  │ • No instruction fetch/decode overhead                           │ │
│  │ • No cache hierarchy (data flows through pipeline)               │ │
│  │ • No speculative execution waste                                 │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│  PERFORMANCE COMPARISON                                                │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                                                                   │ │
│  │ Metric          │ CPU (x86)    │ MSVP ASIC    │ Improvement      │ │
│  │ ────────────────┼──────────────┼──────────────┼─────────────     │ │
│  │ H.264 encode    │ Baseline     │ 9x faster    │ 9x               │ │
│  │ VP9 encode      │ Baseline     │ 50x faster   │ 50x              │ │
│  │ Power per chip  │ ~200W        │ ~10W         │ 20x efficiency   │ │
│  │ Perf-per-watt   │ Baseline     │ ~180-1000x   │ Transformative   │ │
│  │                                                                   │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│  DEPLOYMENT MODEL                                                      │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • MSVP chips deployed in data center PCIe cards                  │ │
│  │ • Orchestration via standard job scheduler                       │ │
│  │ • Transparent to application code (same encoding API)            │ │
│  │ • CPU fallback for codecs not yet supported on MSVP             │ │
│  │ • Part of $72B capex (2025) infrastructure investment            │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| Aspect | Before MSVP | After MSVP |
|--------|-------------|------------|
| Encoding cost trajectory | Rising (higher quality = more CPU) | Declining (fixed silicon amortizes over volume) |
| Codec adoption speed | Slow (CPU cost limits experimentation) | Fast (ASIC handles new codecs via firmware) |
| Power consumption | Hundreds of MW for encoding fleet | Fraction, freeing power budget for ML |
| Scaling model | Add more CPU servers | Add more MSVP cards (denser, cheaper) |

---

## Case Studies

### Case Study 1: Journey to 1,000+ ML Models

Instagram's ML model count grew from dozens to 1,000+ over several years, each phase introducing new infrastructure challenges:

```
EVOLUTION:

Phase 1: Ad-Hoc (< 50 models)
┌─────────────────────────────────────────────────────────────────────┐
│ • Models managed by individual teams                                │
│ • Manual deployment via code review                                 │
│ • Regressions caught by human observation                          │
│ • Works, but doesn't scale                                          │
└─────────────────────────────────────────────────────────────────────┘
                              ↓
Phase 2: Registry + Monitoring (50-200 models)
┌─────────────────────────────────────────────────────────────────────┐
│ • Model Registry on Configerator: centralized metadata, versioning │
│ • Each model has registered owner, input/output contract           │
│ • Basic monitoring: inference latency, error rates                 │
│ • Problem: regressions still hard to attribute                     │
└─────────────────────────────────────────────────────────────────────┘
                              ↓
Phase 3: Stability Metric + Launch Pipeline (200-1000+ models)
┌─────────────────────────────────────────────────────────────────────┐
│ • Model Stability Metric: multi-dimensional health index           │
│   - Inference latency p50/p99                                      │
│   - Prediction drift (output distribution shift)                   │
│   - Feature freshness (are input features current?)                │
│   - Error rate and exception frequency                             │
│ • Launch Pipeline automation:                                       │
│   - A/B test → statistical significance check → gradual rollout   │
│   - Automatic rollback if stability metric degrades                │
│ • Result: deployment velocity INCREASED as model count grew        │
└─────────────────────────────────────────────────────────────────────┘
```

**Key Lesson:** At scale, ML is fundamentally an infrastructure problem. The models may be excellent, but without registry, health scoring, and automated deployment, 1,000 models is 1,000 potential points of failure.

### Case Study 2: Diversity-Aware Notification Ranking

Instagram's notification system sends hundreds of millions of notifications daily. Naive relevance-based ranking caused notification fatigue: users received clusters of similar notifications (all likes, or all follow suggestions) even when individual notifications were relevant.

```
PROBLEM:
Raw relevance ranking → 10 like notifications in a row → user disables notifications

SOLUTION: Maximal Marginal Relevance (MMR) with Diversity Factor

Score(c) = R(c) × D(c)

Where:
  R(c) = relevance score from notification ranking model
  D(c) = diversity bonus = 1 / (1 + similarity to already-selected notifications)

Step-by-step plan in plain English: NotificationRanking

function rankNotifications(user_id, candidates):
    selected = []

    for i in range(min(len(candidates), MAX_NOTIFICATIONS)):
        bestScore = -1
        bestCandidate = null

        for c in candidates:
            relevance = NotificationRankingModel.score(user_id, c)
            diversity = calculateDiversity(c, selected)
...
function calculateDiversity(candidate, selected):
    if isEmpty(selected):
        return 1.0
    ...
    for s in selected:
        ...
        // Also factor in notification type similarity
        if candidate.type == s.type:
            sim += 0.3  // Same-type penalty
        maxSimilarity = max(maxSimilarity, sim)

    return 1.0 / (1.0 + maxSimilarity)
```

**Key Lesson:** Optimizing individual notification relevance is not the same as optimizing the notification *set*. Diversity-aware ranking treats the notification tray as a portfolio, not a ranked list.
