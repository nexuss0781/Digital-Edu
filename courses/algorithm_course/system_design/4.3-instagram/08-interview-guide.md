...
Modern interviews increasingly probe awareness of the latest developments:

### Custom Silicon for Media Processing

**"Why would a social platform build custom hardware?"**

```
The "Generalist Tax" Argument:

General-purpose CPUs waste transistors on capabilities video encoding
never uses (branch prediction, speculative execution, general-purpose ALUs).

At Instagram's scale (95M uploads/day), this waste compounds into
millions of dollars of unnecessary compute annually.

MSVP (Meta Scalable Video Processor):
• H.264: 9x faster than CPU
• VP9: 50x faster than CPU
• Power: ~10W per chip (vs hundreds of watts for equivalent CPU throughput)
• Performance-per-watt: 180-1000x improvement

This is the same trajectory as TPUs for ML and SmartNICs for networking:
once a workload dominates your compute profile, purpose-built hardware
becomes an economic inevitability.
```

### Scaling ML to 1,000+ Models

**"How do you manage 1,000+ ML models in production?"**

```
Three-pillar approach:

1. Model Registry (centralized metadata)
   • Every model has: owner, version history, input/output contract
   • Built on Configerator (Meta's config management system)

2. Model Stability Metric (automated health scoring)
   • Composite score: inference latency + prediction drift +
     feature freshness + error rate
   • Turns "is this model healthy?" from judgment call to number

3. Launch Pipeline (automated deployment)
   • A/B test → significance check → gradual rollout → full deployment
   • Automatic rollback if stability metric degrades

Key insight: Deployment velocity INCREASED as model count grew,
because automation eliminated the coordination overhead that
would otherwise scale linearly with model count.
```

### User-Controllable Recommendations

**"How do you give users control over what they see?"**

```
"Your Algorithm" Tool (December 2025):

Adds explicit preference channel alongside implicit behavioral signals:
• Users indicate topics they want more or less of
• Preferences injected as features into ranking model
• Soft boosts (not hard filters) to avoid preference lock-in
• Combined with diversity injection to prevent filter bubbles

Architectural significance:
• Bidirectional: improves user trust AND model accuracy
• Regulatory: addresses EU AI Act / DSA requirements
• Signal quality: explicit preference > behavioral inference
  (watching a car crash ≠ wanting more car crashes)

Design trade-off: React too aggressively → filter bubble;
too weakly → tool feels useless. Balance via mixing strategy.
```

### Trap Question: "Why not just use LLMs for all content moderation?"

**Best answer:**
> "LLMs are 10-50x more expensive per item than traditional classifiers. At 95 million uploads per day, running every item through an LLM would be prohibitively expensive. Instagram uses a cascading architecture: hash matching (1ms, catches known violations), ML classifiers (10ms, catches common patterns), and LLM reasoning (100-500ms, catches semantic violations). The LLM stage is applied selectively—only to the ~15% of content where earlier stages have moderate confidence. This keeps cost impact to ~15-20% of the moderation budget while catching violations traditional pipelines would miss entirely."
...
│  • AV1 codec + MSVP custom ASIC (9x H.264, 50x VP9)                   │
│  • Three-stage ranking (retrieval → ESR → LSR)                         │
│  • Multi-tier caching (Client → CDN → Redis → DB)                      │
│  • 1,000+ ML models with Registry + Stability Metric                   │
│  • Async processing with immediate ack                                  │
│  • LLM-based moderation (cascading: hash → ML → LLM)                   │
│                                                                         │
│  STORIES TTL:                                                           │
...
│  • "Real-time ranking" → Three-stage funnel, precompute candidates     │
│  • "LLMs for all moderation" → Cascading pipeline, selective apply     │
│  • "Why custom hardware" → Generalist tax, 9-50x encoding speedup     │
│                                                                         │
