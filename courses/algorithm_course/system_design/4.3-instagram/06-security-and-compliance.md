...
| Scams/Fraud | LLM reasoning (2025) | Auto-remove, 5K+ catches/day |
| Impersonation | LLM + profile analysis (2025) | Account suspension, 80%+ reduction |
| Misinformation | Fact-check partnerships | Label, reduce distribution |
| IP Violation | Hash matching + reports | Remove on DMCA |

### LLM-Based Content Moderation (2025)

Traditional ML classifiers detect surface patterns. LLM-powered moderation adds a semantic reasoning layer that understands context and intent:
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  CASCADING ARCHITECTURE (cost-optimized):                              │
│                                                                         │
│  Stage 1: Hash Matching (~1ms per item)                                │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • PhotoDNA hash matching for known CSAM/terrorism                │ │
│  │ • Near-duplicate detection for known violating content           │ │
│  │ • Cost: Negligible                                               │ │
│  │ • Catches: ~5% of all violations (known content)                 │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                              ↓ (content passes)                         │
│  Stage 2: ML Classifiers (~10ms per item)                              │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • Image classification (nudity, violence, weapons)               │ │
│  │ • Text classification (hate speech, spam patterns)               │ │
│  │ • Behavioral signals (account age, posting velocity)             │ │
│  │ • Cost: Moderate (GPU inference)                                 │ │
│  │ • Catches: ~80% of all violations (common patterns)              │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                              ↓ (moderate confidence, 0.3-0.8 score)    │
│  Stage 3: LLM Reasoning (~100-500ms per item)                         │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │ • LLaMA-family models with moderation-specific fine-tuning       │ │
│  │ • Understands context: "Is this satire or genuine threat?"       │ │
│  │ • Detects adversarial content: misspellings, coded language      │ │
│  │ • Cross-modal reasoning: text + image context together           │ │
│  │ • Cost: 10-50x more expensive per item than Stage 2              │ │
│  │ • Applied selectively: only to moderate-confidence items (~15%)  │ │
│  │ • Catches: 5K+ scam attempts/day, 80%+ impersonation reduction  │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│  COST MANAGEMENT:                                                      │
│  • Stage 3 applied to ~15% of content (moderate confidence from S2)   │
│  • Overall cost impact: ~15-20% increase in moderation budget         │
│  • ROI: Catches violations that would require 10x more human review   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### LLM Moderation Examples

```
SCAM DETECTION (catches adversarial patterns):

Input text: "DM me for fr33 g1ftcard!! Only 100 left!! 🎁🎁"
Traditional classifier: PASS (misspellings bypass keyword filter)
LLM reasoning: "This message uses intentional misspellings to evade
  detection. The pattern (urgency + free offer + DM solicitation)
  is consistent with gift card scam campaigns." → BLOCK

IMPERSONATION DETECTION:

Profile: "Official Apple Support @apple_support_help"
Bio: "We help with all Apple issues. DM for quick assistance."
Traditional classifier: Unclear (legitimate-looking profile)
LLM reasoning: "This profile claims to be an official support channel
  for a major brand but uses an unofficial handle format. The bio
  solicits DMs, a common pattern in impersonation scams targeting
  users seeking customer support." → FLAG FOR REVIEW
```
