...
### Slowest part of the process 1: EMPI Matching at Scale
...
### Slowest part of the process 2: Real-Time Bed State Consistency
...
### Slowest part of the process 3: Integration Hub Throughput
...
- Single-threaded Slowest part of the process
...
### Slowest part of the process 4: AI Model Inference Latency
...
## Deep Dive 5: TEFCA-Based Nationwide Interoperability (2025+)

### Why TEFCA Changes the Interoperability Landscape

The Trusted Exchange Framework and Common Agreement (TEFCA) establishes a nationwide floor for health information exchange. Prior to TEFCA, interoperability was achieved through point-to-point connections or regional HIEs with inconsistent policies. TEFCA introduces Qualified Health Information Networks (QHINs) as standardized on-ramps, fundamentally changing how an HMS connects to external systems.

### TEFCA Architecture Impact on HMS

```
HMS INTEGRATION WITH TEFCA:

Before TEFCA:
┌───────────────────────────────────────────────┐
│ HMS → Point-to-point with each partner         │
│   → 50 connections for 50 partners             │
│   → Each with different trust frameworks       │
│   → Manual BAAs and data sharing agreements    │
└───────────────────────────────────────────────┘

After TEFCA:
┌───────────────────────────────────────────────┐
│ HMS → Single QHIN connection                   │
│   → Access to all TEFCA participants           │
│   → Standardized trust framework (TEFCA SOP)   │
│   → Standardized query/response (XCPD + XCA)   │
│   → Patient matching via national rules         │
└───────────────────────────────────────────────┘
```

### TEFCA Exchange Purposes Relevant to HMS

| Exchange Purpose | HMS Use Case | Protocol |
|-----------------|--------------|----------|
| **Treatment** | Query external records during admission | XCPD + XCA |
| **Individual Access** | Patient portal data export | FHIR APIs |
| **Public Health** | Reportable condition notifications | eICR |
| **Benefits Determination** | Prior authorization queries | FHIR Da Vinci |
| **Operations** | Quality reporting data submission | FHIR Bulk Data |

### EMPI Federation via TEFCA

TEFCA's national patient matching requirements directly impact the EMPI. The HMS must support:

1. **TEFCA patient matching** - Respond to XCPD (Cross-Community Patient Discovery) queries from QHINs using the EMPI's probabilistic matcher
2. **Consent-aware responses** - TEFCA requires respecting patient opt-out preferences (vary by state: opt-in vs. opt-out)
3. **Minimum Necessary** - Return only the data elements required for the stated exchange purpose
4. **Identity proofing at IAL2** - TEFCA requires NIST IAL2 identity verification for patient portal access

### CMS Interoperability Rules (2026 Enforcement)

The CMS-0057-F rule (effective January 2026) mandates that payers implement FHIR-based APIs for prior authorization. This impacts the HMS revenue cycle:

```
PRIOR AUTHORIZATION AUTOMATION (CMS-0057-F):

Before (Manual):
  Surgeon schedules case → Staff calls payer → Fax clinical docs
  → 5-14 day wait → Approval/denial by fax
  Delay impact: OR scheduling delays, patient anxiety

After (FHIR Da Vinci PAS):
  Surgeon schedules case → HMS auto-extracts clinical criteria
  → HMS submits PAS request via FHIR → Payer responds in <72h
  → Real-time status tracking → Auto-updates OR schedule
  Benefit: 70% of prior auth decisions within 24 hours

ARCHITECTURE:
  OR Scheduling → Prior Auth Service → FHIR Client
  → Payer FHIR Endpoint (Da Vinci PAS)
  → Response handler → OR Schedule update + notification
```

---

## Deep Dive 6: Ambient AI for Clinical-Operational Handoff (2025+)

### Why Ambient AI Matters for HMS

Ambient AI clinical documentation (voice-to-note technology deployed by major EMR vendors since 2024) changes the data flow into HMS. Instead of clinicians manually entering data that triggers HMS workflows, ambient AI generates structured clinical notes that the HMS must consume in near-real-time.

### Impact on HMS Workflows

| Workflow | Before Ambient AI | After Ambient AI |
|----------|------------------|-----------------|
| **Discharge planning** | Discharge summary written hours after decision | LOS prediction updated as clinician discusses discharge with patient |
| **Coding** | Coder reviews final note days later | AI coding suggestions generated as notes are finalized in real-time |
| **Bed management** | Discharge order triggers bed release | Predicted discharge captured from ambient note, enabling proactive bed assignment |
| **OR scheduling** | Surgeon manually enters case details | Ambient AI captures surgical plan from clinic visit, pre-populates OR request |

### Integration Architecture

```
AMBIENT AI → HMS PIPELINE:

Clinician speaks → Ambient AI (EMR) → Structured FHIR resources
  → FHIR Subscription notification to HMS
  → HMS processes:
    1. Discharge intent detected → Bed prediction model updated
    2. Procedure mentioned → OR scheduling pre-populated
    3. Diagnosis captured → AI coding pre-staged
    4. Social determinants noted → Discharge planning flagged

LATENCY BUDGET:
  Ambient capture → Note generation: ~30 seconds
  Note → FHIR resource: ~5 seconds
  FHIR → HMS notification: ~2 seconds
  HMS processing: ~500ms
  Total: <40 seconds from spoken word to HMS action
```

### Design Considerations

1. **Confidence filtering** - Ambient AI notes include confidence scores; HMS should only trigger workflows above a configurable threshold (e.g., >0.8 for discharge intent)
2. **Human-in-the-loop** - Clinician must sign the ambient note before HMS takes irreversible actions (bed release, charge capture)
3. **Incremental updates** - Notes may be amended multiple times; HMS must handle FHIR resource updates idempotently
4. **PHI boundaries** - HMS receives structured FHIR resources, not raw audio transcripts
