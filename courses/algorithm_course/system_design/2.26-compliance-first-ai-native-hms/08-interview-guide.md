...
### Trap 7: "Why not just use FHIR for everything and drop HL7v2 support?"

**What Interviewer Wants:** Test your understanding of real-world healthcare interoperability constraints.

**Best Answer:**
> "In an ideal world, yes. In practice, 70-80% of hospital interfaces still run on HL7v2, especially lab (LIS) and radiology (RIS) systems that were installed decades ago and won't be replaced for years. Dropping HL7v2 means no lab results, no radiology reports, no pharmacy integration at most hospitals. The pragmatic approach is FHIR-primary with an HL7v2 translation engine (like Mirth Connect) that converts legacy messages to FHIR resources internally. New integrations use FHIR natively; legacy systems get adapters. TEFCA further reinforces this—it mandates FHIR APIs but acknowledges that internal hospital systems will need translation layers for years."

---

### Trap 8: "How do you prevent AI models from making patient safety decisions without physician oversight?"

**What Interviewer Wants:** Test your understanding of AI governance in healthcare and regulatory boundaries.

**Best Answer:**
> "Three architectural guardrails: 1) **No autonomous clinical actions**—AI models in the HMS are advisory only. Bed predictions inform nurse decisions, OR duration estimates inform schedulers, coding suggestions require coder approval. No AI output directly modifies a treatment plan or patient record. 2) **Confidence thresholds with human escalation**—high-confidence AI outputs are pre-populated but flagged for review; low-confidence outputs go to manual queues. 3) **Audit trail for every AI-influenced decision**—we log the model version, input features, output, confidence score, and the human decision (accept/modify/reject). This satisfies both FDA Software as a Medical Device (SaMD) guidelines and HIPAA's requirement for human accountability. The key insight: HMS AI is operational (beds, scheduling, billing), not clinical (diagnosis, treatment), which keeps us below the FDA SaMD regulatory threshold."
