...
> **Tier B — AI-Augmented:** AI models generate predictions, recommendations, and draft outputs (bed demand forecasts, OR duration estimates, ICD-10 coding suggestions) that clinicians and administrators review before action. No AI output directly alters patient placement, surgical schedules, or billing codes without human confirmation.

| Decision | AI Role | Human Role | Override | Escalation |
|----------|---------|------------|----------|------------|
| Bed demand forecasting | Predicts occupancy 24-72h ahead | Reviews forecast before staffing decisions | Manual override on any forecast | Auto-escalate if predicted occupancy >95% |
| EMPI probabilistic matching | Scores candidate matches, ranks by confidence | Confirms/rejects merge for scores <0.95 | Can force-merge or force-split any record | Escalate to HIM director if duplicate rate >2% |
| OR case duration prediction | Estimates duration from historical + surgeon + procedure | Surgeon confirms or adjusts estimate | Surgeon override always takes precedence | Escalate if prediction error >30% for 5 consecutive cases |
| ICD-10/CPT coding suggestion | Suggests codes from clinical documentation | Certified coder reviews and finalizes | Coder selects final codes; AI is advisory only | Escalate to coding supervisor if denial rate >8% |
| ADT workflow routing | Recommends next step in saga orchestration | Approves transfer/discharge decisions | Clinician can override any routing decision | Escalate to bed manager if patient wait >4 hours |
| Revenue cycle anomaly detection | Flags undercoded or miscoded encounters | Revenue analyst investigates flagged items | Analyst determines final billing action | Escalate to compliance officer if pattern detected |
...
| **ADT Workflow** | Admission processing, discharge planning, transfer coordination | Discharge readiness prediction, workflow Slowest part of the process detection |
...
- [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) - Audit trail patterns for compliance events
- [Configuration Management System](../1.11-configuration-management-system/00-index.md) - Policy-as-code, dynamic configuration
- [Telemedicine Platform](../10.1-telemedicine-platform/00-index.md) - Remote care integration, patient scheduling
...
> **Vendor Freshness Notice (2026-03-21):** This document references specific vendor products and ML frameworks (XGBoost, Prophet, BioBERT, OPA, HAPI FHIR). Verify version compatibility and feature availability against current vendor documentation before adopting in production architectures.
