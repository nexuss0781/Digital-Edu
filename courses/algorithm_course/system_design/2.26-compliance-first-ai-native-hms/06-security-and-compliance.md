...
│   ├─ 1.2 Brute Force (Checking every single possibility) authentication [LOW risk]
...
## 21st Century Cures Act: Information Blocking (2025 Enforcement)

### What Is Information Blocking?

The ONC's Information Blocking Rule (effective April 2021, enforcement strengthened in 2025) makes it illegal for healthcare providers, health IT developers, and HIEs to interfere with the access, exchange, or use of electronic health information (EHI). This directly constrains HMS architecture decisions.

### HMS Design Implications

| Principle | Architectural Impact |
|-----------|---------------------|
| **No restricting data export** | FHIR Bulk Data export must be supported; cannot limit API access to proprietary formats |
| **No unreasonable fees** | API access for patient data cannot be gated behind premium pricing tiers |
| **No limiting interoperability** | Must support USCDI (United States Core Data for Interoperability) data classes |
| **Eight exceptions exist** | Preventing harm, privacy, security, infeasibility, health IT performance, content/manner, fees, licensing |

### USCDI Compliance Mapping

The US Core Data for Interoperability (USCDI v4, 2025) defines the minimum data classes that must be exchangeable. HMS must ensure these are available via FHIR APIs:

```
USCDI DATA CLASSES RELEVANT TO HMS:

Patient Demographics → FHIR Patient (EMPI golden record)
Encounter Information → FHIR Encounter (ADT events)
Procedures → FHIR Procedure (OR cases)
Health Insurance → FHIR Coverage (revenue cycle)
Facility Information → FHIR Location (bed management)
Care Team Members → FHIR CareTeam (ADT workflow participants)
Clinical Notes → consumed from EMR, not generated by HMS

HMS RESPONSIBILITY:
  - Expose FHIR APIs for all owned USCDI data classes
  - Support SMART on FHIR for third-party app access
  - Maintain provenance records for data origin tracking
  - Implement FHIR Bulk Data Access for population-level export
```

### Practical Safeguard: Security Exception

The information blocking rule's security exception allows the HMS to deny data access when there is a legitimate security risk. This is implemented via OPA policies that log the reason for every denial, creating an auditable trail that demonstrates the denial was based on security concerns, not information blocking.
