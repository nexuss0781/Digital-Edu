...
| FR-15 | **Autonomous robot inspection** — Deploy legged and tracked robots for confined-space, hazardous-area, and repetitive inspection tasks; autonomous SLAM-based navigation in GPS-denied environments; onboard LiDAR + RGB-D capture with real-time hazardous atmosphere monitoring (gas, temperature, humidity) | Support fleet of 5-10 robots per large site; SLAM localization accuracy ≤ 5 cm; autonomous mission duration ≥ 2 hours per charge; data registered to BIM coordinates within 3 cm accuracy |
| FR-16 | **Generative AI for clash resolution** — When clash detection identifies high-severity conflicts, automatically generate ranked redesign alternatives that satisfy routing constraints, code clearances, trade sequencing, and cost targets; produce 3D visualizations for engineer review and approval | Generate ≥ 3 resolution alternatives per critical clash within 2 minutes; alternatives must satisfy applicable building code clearances; cost impact estimation within ±15% accuracy |
| FR-17 | **Natural language project queries** — LLM-powered conversational interface allowing project managers, engineers, and field supervisors to query project status, BIM data, cost forecasts, schedule risks, and safety analytics using natural language instead of navigating dashboards | Response latency ≤ 5 seconds for factual queries; support multi-turn conversations with context retention; ground all responses in verified project data with source citations; handle ambiguous queries with clarification prompts |
...
| Robot mission success rate | ≥ 95% autonomous mission completion without human intervention |
| Robot data registration accuracy | ≤ 3 cm to BIM coordinate system |
...
| Inspection robots per site | 5–10 (legged + tracked) |
| NL query concurrency | 5,000+ simultaneous conversations across all projects |
| Generative clash resolutions per day | 500+ across all sites |
...
| Robot inspection mission completion | ≥ 95% | Per mission |
| Robot localization accuracy (SLAM) | ≤ 5 cm RMS | Per mission |
| Robot data BIM registration | ≤ 3 cm | Per data upload |
| NL query response (factual) p95 | ≤ 5 s | Per query |
| NL query response (analytical) p95 | ≤ 15 s | Per query |
| NL query grounding accuracy | ≥ 95% citations correct | Weekly audit |
| Generative clash resolution p95 | ≤ 2 min per clash | Per clash event |
| Clash resolution code compliance | 100% alternatives pass code check | Per resolution set |
...
|---|---|---|
| **BIM model version conflicts** — Multiple disciplines upload conflicting model versions simultaneously; element GUIDs collide or are reused across versions | Progress tracking maps observations to wrong elements; clash detection produces false positives/negatives | Enforce GUID immutability with server-side validation; implement optimistic concurrency control with conflict detection; maintain per-discipline version chains with federation points; alert coordination leads when inter-discipline references break |
| **Sensor failures during critical operations** — Cameras obscured by dust, LiDAR damaged by vibration, IoT sensors lose power during concrete pour monitoring | Gaps in safety coverage; missed progress observations; incomplete temperature/humidity curing data | Redundant camera coverage with overlap zones (each point visible from ≥ 2 cameras); self-diagnostic sensor health monitoring with automated alerts; edge compute detects coverage gaps and flags zones as LOW_CONFIDENCE; for curing sensors, require N+1 redundancy on critical pours |
| **Extreme weather events** — Hurricanes, severe thunderstorms, extreme heat (>45°C), freezing temperatures (<-20°C) affecting site operations and equipment | Edge compute thermal shutdown; camera lens fogging/icing; drone grounding; network infrastructure damage; worker evacuation invalidates all progress assumptions | Weather-triggered operational mode changes: "storm mode" suspends non-safety processing, increases safety alert sensitivity, activates backup power; thermal management for edge hardware (active cooling/heating in enclosures); automated progress recalibration after weather events (re-survey affected zones) |
| **Photogrammetry registration failure** — ICP convergence failure due to insufficient geometric features in open areas, repetitive textures, or major construction changes between captures | Point cloud cannot be aligned to BIM; progress tracking for affected zones produces no results | Multi-level fallback: primary ICP → feature-based registration using GCP targets → manual registration assist; place high-visibility fiducial markers at survey control points; detect registration failure early (residual error > threshold) and request re-capture before daily deadline |
| **Robot navigation failure in dynamic environments** — Unexpected obstacles (material deliveries, temporary shoring, debris) block planned robot paths; robot becomes stuck or trapped | Mission fails; inspection data for confined space is unavailable; robot may require manual rescue | Autonomous recovery behaviors: reverse-path backtracking, alternative route planning against continuously updated occupancy map; deadman switch alerts operator after 5 minutes of zero progress; design robot docking stations at zone boundaries for easy manual retrieval; pre-mission path validation against latest site state |
| **LLM hallucination in project queries** — Natural language interface generates plausible but incorrect answers about cost, schedule, or safety status | Project managers make decisions based on false information; erosion of trust in the system | Mandatory retrieval-augmented generation (RAG) grounding: every factual claim must link to a verified data source; confidence scoring with explicit "I don't have enough data" responses when retrieval fails; human-in-the-loop review for queries involving financial or safety decisions; audit trail for all LLM responses |
| **Subcontractor gaming of performance metrics** — Subcontractors learn which elements are most visible to cameras and prioritize those, leaving hidden work incomplete | Progress tracking overestimates completion; hidden defects discovered during inspection | Randomized inspection targeting that includes low-visibility zones; cross-reference CV-detected progress against inspection results; statistical anomaly detection for subcontractors whose camera-visible completion consistently exceeds overall completion; robot inspection of concealed spaces |
| **Massive model uploads during peak hours** — Multiple project teams upload full BIM models (2M+ elements each) simultaneously, overwhelming the BIM gateway | Clash detection queues back up; cost estimates stale; progress tracking operates against outdated models | Rate limiting with priority queuing (safety-impacting model changes first); incremental model diffing to reduce processing volume; off-peak upload incentives in the UI; auto-scaling BIM processing workers with queue depth triggers |
