...
| **Expiration backlog age** | Oldest unprocessed expired paste | >1 hour indicates cleanup Slowest part of the process |
...
RB-006: Reference Count Drift
├── Symptoms: Orphaned blobs in object storage with no metadata references
├── Immediate: No user impact; flag for reconciliation
├── Investigation: Query metadata store for all content_hash values, compare against object storage inventory
├── Remediation: Delete orphaned blobs (no metadata references); increment counts for under-counted hashes
└── Prevention: Weekly reconciliation job; monitor ref_count decrement failure rate

RB-007: CDN Cache Poisoning
├── Symptoms: Users reporting incorrect content for valid paste URLs
├── Immediate: Purge CDN cache for affected slugs
├── Investigation: Check if origin is serving correct content; review CDN purge logs
├── Remediation: Verify content hash integrity at origin; re-populate CDN cache
└── Prevention: Validate content hash on CDN miss before caching; use Vary header correctly

RB-008: Burn-After-Reading Failure
├── Symptoms: Burn pastes accessible after first read; multiple reads returning content
├── Immediate: Manually delete affected paste metadata
├── Investigation: Check atomic UPDATE success; verify CDN Cache-Control: no-store headers
├── Root cause: CDN caching burn paste content (missing bypass), or database CAS failure
└── Prevention: Integration test for burn flow; CDN rule audit for no-cache paths
```

---

## 6. Observability Architecture Summary

```
Observability Data Flow:

Application → Structured Logs → Log Aggregation → Alerting
           → Metrics (RED/USE) → Time-Series DB → Dashboards
           → Distributed Traces → Trace Backend → Latency Analysis

Key Observability Dimensions:
├── By paste visibility: public / unlisted / private
├── By client type: web / CLI / API / embed
├── By cache layer: CDN hit / app cache hit / DB hit
├── By operation: create / read / delete / expire
├── By auth type: anonymous / API key / session
└── By content status: normal / flagged / quarantined

Health Check Hierarchy:
├── L1: Synthetic paste create + read (every 30s, multi-region)
├── L2: Component health endpoints (every 10s)
├── L3: Dependency connectivity (DB, object store, cache, CDN origin)
└── L4: Business metric canaries (creation rate within expected range)
```
