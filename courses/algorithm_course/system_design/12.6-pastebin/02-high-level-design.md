...
### 5.5 Abuse Detection Pipeline Position

| Decision | Synchronous abuse scanning before content storage |
|---|---|
| **Why** | Prevents malware blobs from ever reaching object storage; avoids the "store-then-scan-then-delete" race where malicious content is briefly accessible |
| **Trade-off** | Adds 30-100ms to write latency; abuse service becomes a single point of failure on the write path |
| **Alternative** | Async scanning after storage (lower write latency but creates an accessibility window for malicious content) |
| **Verdict** | Synchronous for critical patterns (malware signatures, credential dumps); async for lower-confidence classifications (spam scoring, quality signals). Circuit breaker on abuse service: if it's down, allow writes with a "pending review" flag |

### 5.6 Expiration Strategy Architecture

| Decision | Three-tier expiration: lazy deletion on read + TTL indexes + background sweep |
|---|---|
| **Why** | No single mechanism handles correctness (never serve expired content), storage reclamation (free disk), and index maintenance (keep queries fast) simultaneously |
| **Trade-off** | Higher implementation complexity than a single cron approach; three code paths to maintain and test |
| **Alternative** | Database-level TTL (where supported) handles both correctness and cleanup but lacks fine-grained control over batch sizes and timing |
| **Verdict** | Three-tier approach ensures expired content is never served (lazy deletion) while avoiding database contention from bulk cleanup (batched sweep during off-peak hours) |

### 5.7 Database Choices
...
| **Idempotency Key** | Yes | API paste creation uses client-provided idempotency key to prevent duplicate pastes on retry |
| **Backpressure** | Yes | KGS signals low pool capacity to slow down paste creation rather than failing |

---

## 7. Cross-Service Data Flows

### 7.1 Paste Creation Flow (Happy Path)

```
Client → API Gateway → Paste Service
                         ├── 1. Claim key from KGS (sync, <5ms)
                         ├── 2. Compute SHA-256 of content
                         ├── 3. Scan content via Abuse Detection (sync, <100ms)
                         ├── 4. Check content_hash in metadata DB (dedup lookup)
                         │     ├── EXISTS: reuse blob, increment ref_count
                         │     └── NOT EXISTS: PUT content to object storage
                         ├── 5. INSERT metadata row (slug, hash, user, expiry)
                         ├── 6. SET cache entry (metadata + content)
                         └── 7. Return 201 with paste URL
```

### 7.2 Expiration Processing Flow

```
Expiration Service (runs continuously):
  ├── Phase 1: Read expired records from metadata DB
  │   └── SELECT slug, content_hash FROM pastes
  │       WHERE expires_at < NOW() AND is_deleted = FALSE
  │       ORDER BY expires_at ASC LIMIT 1000
  ├── Phase 2: For each expired paste
  │   ├── Mark is_deleted = TRUE in metadata
  │   ├── Decrement ref_count for content_hash
  │   ├── If ref_count = 0: DELETE blob from object storage
  │   ├── EVICT from cache cluster
  │   └── Queue CDN purge for slug URL
  ├── Phase 3: Commit batch
  └── Sleep for 30 seconds, then repeat
```

### 7.3 Burn-After-Reading Flow

```
Client → CDN (MISS — Cache-Control: no-store) → API Gateway → Paste Service
  ├── 1. SELECT metadata WHERE slug = X
  ├── 2. Check burn_after_reading flag
  ├── 3. Atomic UPDATE: SET is_read = TRUE WHERE slug = X AND is_read = FALSE
  │     ├── rows_affected = 1: proceed to serve content
  │     └── rows_affected = 0: return 404 (already read)
  ├── 4. Fetch content from object storage (or cache)
  ├── 5. Return content to client
  └── 6. Async: trigger cleanup (decrement ref_count, delete metadata)
```
