...
├── Impact: High server cost, increased latency, scaling Slowest part of the process
...
│  ├── Burn-after-reading converts GET into CAS mutation   │
│  ├── URL slug is a capability token (security boundary)  │
│  ├── Ref counting drift requires reconciliation job      │
│  ├── CDN TTL is a correctness knob, not just perf       │
│  └── Immutable content makes caching trivially simple    │
│                                                          │
└──────────────────────────────────────────────────────────┘
```
