...
## 5. Derived Throughput Calculations

### 5.1 Write Path Load

```
Paste creation (peak):
  2M pastes/day × 3x business-hour concentration = ~70 writes/sec peak
  Average paste size: 5 KB
  Write throughput: 70 × 5 KB = ~350 KB/sec ingest
  Key claims: 70 claims/sec from KGS pool
  KGS replenishment: batch generate 10,000 keys every ~2.4 minutes
  Content hash lookups: 70/sec (deduplication check)
  Dedup hit rate: ~20% → 56 unique writes/sec to object storage
```
...
```
Read serving (peak):
  10M reads/day × 3x business-hour concentration = ~350 reads/sec peak
  CDN cache hit rate: 80% → 280 reads/sec served from CDN edge
  Application cache hit rate (of CDN misses): 60% → 42 reads from app cache
  Origin database reads: 350 × 0.2 × 0.4 = ~28 reads/sec from metadata DB
  Object storage reads: ~28 fetches/sec (cache miss path)

  Bandwidth:
    CDN egress: 280 × 5 KB = ~1.4 MB/sec
    Origin egress: 70 × 5 KB = ~350 KB/sec
```

### 5.3 Expiration Processing

```
Expiration distribution (estimated):
  10 minutes: 5% of pastes → 100K pastes/day
  1 hour: 10% → 200K/day
  1 day: 25% → 500K/day
  1 week: 15% → 300K/day
  1 month: 10% → 200K/day
  Never expires: 35% → 700K/day (accumulates indefinitely)

  Peak expiration processing:
    Heaviest period: 1-day expirations from previous day's peak
    ~500K pastes expire in 24 hours → ~6 pastes/sec background cleanup
    Each cleanup: metadata delete + ref count decrement + conditional blob delete
    Ref count check: ~6/sec
    Blob deletes (after dedup check): ~4/sec (some shared content preserved)
```

### 5.4 Key Generation Service Sizing

```
KGS pool management:
  Consumption rate: ~23 keys/sec average, ~70 keys/sec peak
  Pool size: 1,000,000 pre-generated keys (comfortable buffer)
  Refill trigger: when pool drops below 200,000 keys (20% threshold)
  Batch generation: 100,000 keys per batch
  Generation rate: ~10,000 keys/sec (including uniqueness verification)
  Time to generate batch: ~10 seconds
  Time to exhaust pool at peak: 1M / 70 = ~4 hours (safe margin)

  Keyspace utilization:
    8-char Base62 = 62^8 = 218 trillion possible slugs
    At 2M/day = 730M/year
    Years to exhaust 1% of keyspace: 218T × 0.01 / 730M = ~3,000 years
```
...
1. **Text-only content**: No binary uploads; maximum 512 KB per paste
2. **Immutable content**: Paste content cannot be edited after creation (metadata like title can be updated)
3. **Anonymous-first**: Majority of pastes created without authentication; rate limiting must work for anonymous users
4. **Expiration is non-negotiable**: The system must guarantee that expired pastes are inaccessible, even if storage cleanup lags
5. **Global access**: Users worldwide; CDN edge distribution is essential for read performance
6. **Bot traffic**: 30-40% of API traffic is automated (CI/CD logs, error reporting tools, scripts)
7. **Abuse is constant**: The platform will be used for malware distribution, credential dumps, and spam unless actively prevented
