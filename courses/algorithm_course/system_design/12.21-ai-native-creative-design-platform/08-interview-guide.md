...
- Latency budget: 5-second SLO decomposed across subtasks; image generation is the Slowest part of the process
...
| Cache design | "Two users enter similar but not identical prompts. Can you serve a cached result?" |
| Model deployment safety | "Your new diffusion model is 15% better at image quality. How do you deploy it safely?" |
| Magic resize depth | "Resize a landscape banner to portrait. What happens to the text?" |

---

## Trap Questions (Additional)

### Trap 4: "Can you cache AI generation results?"

**What it tests:** Understanding of cache key design in generative AI systems.

**Trap:** The candidate says "yes, cache by prompt hash" without considering the sparse key space, or says "no, every generation is unique" without considering semantic similarity.

**Good answer:** "Exact-match caching by prompt hash gives ~5% hit rate because natural language prompts rarely repeat exactly. But I can use embedding-based semantic similarity: encode the prompt with CLIP text encoder, search the cache for entries with cosine similarity > 0.92, and return cached results as 'suggested starting points.' This boosts effective hit rate to 15%+. The cache key should include {prompt_embedding, brand_kit_version, model_version, aspect_ratio} to ensure cached results are brand-consistent and model-appropriate. Cache invalidation on model version changes is critical."

### Trap 5: "Your layout model sometimes produces overlapping elements. How do you fix this?"

**What it tests:** Understanding of constraint-based post-processing and the limits of ML-based layout.

**Good answer:** "The layout model is a neural network—it produces approximate positions that minimize a learned loss function, but it doesn't guarantee hard constraints like zero overlap. I add a deterministic constraint solver as a post-processing step: after the layout model proposes positions, the solver checks for overlap, minimum spacing, and alignment grid compliance. If violations are found, the solver adjusts positions using a priority-weighted displacement algorithm (higher-priority elements stay put; lower-priority elements move). This hybrid approach—neural proposal + deterministic validation—gives the visual quality of learned layouts with the hard guarantees of constraint solving."
...
"I'll scope this as a consumer + enterprise platform with 250M MAU and 50M AI generation requests/day. Key clarification: the AI must produce structured, editable scene graphs—not flat images. This is the core design challenge. Brand enforcement must be at generation time, not post-processing. Real-time collaboration with AI as a concurrent writer. Content safety must be a blocking gate."

**Phase 2 — Estimation (5 min):**
"50M generations/day = ~1,700/sec at peak (3x average). Each generation needs ~3s of GPU time across multiple models. Peak GPU demand: ~5,100 concurrent GPU-seconds. With INT8 quantization and batching (4 concurrent per GPU): ~1,300 GPUs for design generation. Image-only generation adds ~1,700 more. Total fleet: ~3,000-4,000 GPUs. Cost: ~$69M/year GPU alone. This is the dominant cost—every architectural decision is a cost optimization."

**Phase 3 — Architecture (10 min):**
"Three main pillars: generation pipeline, collaboration layer, asset platform. The generation orchestrator decomposes prompts into a DAG: prompt interpretation → layout generation → parallel {image generation + text generation} → brand enforcement → CRDT merge. The collaboration layer uses tree-based CRDT for the scene graph, with AI generation submitting operations through the same merge path as human edits. Content safety is a synchronous gate between image generation and CRDT merge—no unsafe image ever reaches the canvas."

**Phase 4 — Deep Dive (GPU economics, selected by interviewer):**
"Cost optimization is multi-dimensional. INT8 quantization gives 2x throughput with imperceptible quality loss. Progressive generation (4-step preview → async 20-step) saves 30% GPU for rejected generations. Semantic similarity caching (CLIP embedding match at cosine > 0.92) gives 15% effective cache hit rate. Model distillation for simple elements (backgrounds, icons) uses 5x faster 8-step model. Tiered GPU pools: free-tier on older hardware (8s latency), paid on premium (5s). Off-peak scaling reduces fleet 65% during low-traffic hours. Combined, these bring effective cost from $0.015 to $0.005 per generation."
...
|---|---|
| **AI Image Generation (3.20)** | "How does your design platform's image generation differ from a standalone text-to-image service?" |
| **Google Docs (6.5)** | "How does adding AI generation change the CRDT collaboration model compared to a traditional document editor?" |
| **AI Guardrails (3.22)** | "How do you prevent your design platform from generating copyrighted content? How is this different from text guardrails?" |
| **CDN Design (1.15)** | "How do you serve 100M+ generated images per day to 250M users globally? What caching strategy works for AI-generated content?" |
