...
| Component | Specification | Count | Monthly Cost | Annual Cost |
|---|---|---|---|---|
| **Design generation GPUs** | High-end inference GPU, 80 GB VRAM | 1,303 | $1.88M | $22.5M |
| **Image-only generation GPUs** | High-end inference GPU, 80 GB VRAM | 1,737 | $2.50M | $30.0M |
| **Safety + embedding GPUs** | Mid-tier GPU, 24 GB VRAM | 200 | $0.14M | $1.7M |
| **Batch processing GPUs** (off-peak retraining, template generation) | Spot/preemptible instances | 500 | $0.36M | $4.3M |
| **Headroom (30%)** | Mixed | 1,122 | $0.81M | $9.7M |
| **GPU fleet total** | | ~4,862 | **$5.7M** | **$68.2M** |

### Non-GPU Infrastructure

| Component | Monthly Cost | Notes |
|---|---|---|
| **Design document store** | $150K | 750 TB structured storage with strong consistency |
| **Asset object storage** | $500K | Rolling ~44 PB/year with lifecycle management |
| **Generated image cache** | $75K | 750 TB SSD-backed cache for 30-day rolling |
| **Collaboration service** | $200K | In-memory CRDT engines, WebSocket fleet |
| **CDN bandwidth** | $800K | Asset delivery to 250M MAU globally |
| **Stream processing** | $100K | Event ingestion, analytics, safety pipeline |
| **Search infrastructure** | $120K | Template and asset search (embedding + keyword) |
| **Non-GPU total** | **~$1.95M** | **~$23.4M/year** |

### Cost Optimization Levers

| Lever | Savings | Trade-off |
|---|---|---|
| INT8 quantization (applied to diffusion + layout models) | ~$34M/year (2x throughput = halve fleet) | < 1% quality degradation on automated benchmarks; imperceptible to users |
| Generation result caching (prompt+brand+seed hash) | ~$5M/year (12% cache hit rate) | Cache invalidation on model version change; stale results if brand kit updated |
| Progressive generation (4-step preview → async 20-step) | ~$10M/year (30% of previews accepted, skipping full generation) | User sees lower-quality preview initially; depends on UX willingness to accept |
| Model distillation (8-step student for simple elements) | ~$8M/year (backgrounds, icons use 5x faster distilled model) | Slight quality reduction for simple elements; not suitable for hero images |
| Off-peak GPU scaling (reduce fleet 65% during low-traffic) | ~$15M/year (8 hours/day at 35% capacity) | Requires 15-min spin-up time; may miss sudden traffic spikes |
| Tiered GPU pools (free-tier on older GPUs) | ~$8M/year (free-tier uses cheaper hardware) | Free-tier users experience higher latency (8s vs 5s) |
...
|---|---|---|---|
| Design generation ≤ 5s p95 (99.9%) | 43.2 min/month | Alert at 3× burn (>130 min/month) | Scale GPU fleet; disable non-critical generation features; route to cached templates |
| Content safety catch rate ≥ 99.99% | 500 misses/50M daily | Alert at 2× (>1000 misses/day) | Tighten safety threshold (accept higher false positive rate); disable affected model version; human review all borderline |
| Collaboration sync ≤ 100ms p95 (99.95%) | 22 min/month | Alert at 5× burn rate | Reduce CRDT checkpoint frequency; shed presence updates; failover to single-writer mode |
| Platform availability 99.95% | 22 min/month | Alert at 3× burn rate | Activate DR region; graceful degradation (disable AI, keep manual editing) |
| Design document durability 11 nines | 0 loss acceptable | Any document loss is SEV-0 | Restore from backup; full incident review; no user data can be lost |

---

## Capacity Planning Summary

| Dimension | Daily | Monthly | Annual |
|---|---|---|---|
| AI generation requests | 50M | 1.5B | 18B |
| GPU-seconds consumed | 150M | 4.5B | 54B |
| New design documents | 200M | 6B | 72B |
| Asset uploads | 100M | 3B | 36B |
| Net new asset storage | 120 TB | 3.6 PB | 43.2 PB |
| CRDT operations | 345.6B | 10.4T | 124.4T |
| Export renders | 10M | 300M | 3.6B |
| Content safety screenings | 50M | 1.5B | 18B |
