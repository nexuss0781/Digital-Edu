...
### Per-Request Latency Budgets

```
Spray nozzle actuation (edge-only, no cloud):
  Total budget: 15.0 ms
  Image capture + CSI transfer:     1.5 ms  (10.0%)
  Preprocessing (undistort, norm):   1.0 ms  ( 6.7%)
  INT8 model inference:              7.0 ms  (46.7%)
  Nozzle zone mapping:               2.0 ms  (13.3%)
  Solenoid actuation (mechanical):   2.5 ms  (16.7%)
  Buffer/overhead:                   1.0 ms  ( 6.7%)

  At 20 km/h = 5.56 m/s:
    15 ms = 8.3 cm travel
    Nozzle spray cone = 25–50 cm → target within tolerance
    At 24 km/h (max rated speed): 15 ms = 10.0 cm → still within cone

Sensor reading ingestion (edge-to-cloud):
  LoRaWAN uplink:        50–200 ms  (gateway proximity + air time)
  Gateway aggregation:   0–900 sec  (batched every 15 min)
  Cellular backhaul:     100–500 ms (gateway → cloud)
  Broker + ingestion:    50–200 ms  (message processing)
  Total: 15 min (dominated by gateway batch interval)
  For critical alerts (frost): immediate uplink, bypasses batching → ~1 sec

Satellite imagery pipeline (cloud processing):
  Tile download from provider:    5–30 min  (depends on provider API)
  Atmospheric correction:         30 sec/tile
  Cloud masking (GPU):            10 sec/tile
  Vegetation index computation:   0.5 sec/tile
  Field-level clipping + anomaly: 2 sec/field
  Total per tile: ~45 sec processing + download latency
  SLO: < 4 hours from satellite acquisition to field health map
```

### Data Volume Growth Model

```
Daily ingestion by source:
  Satellite imagery:     90 GB/day (raw) → 35 GB/day (products)
  Drone imagery:         40 TB/day (peak) → 10 TB/day (products)
  Soil sensors:          192 GB/day (raw) → 24 GB/day (compressed)
  Spray logs:            130 TB/day (raw) → 13 TB/day (compressed)
  Equipment telemetry:   43 GB/day (raw) → 8 GB/day (compressed)
  Weather data:          28 GB/day (gridded GRIB2) → 7 GB/day (extracted)

  Total peak daily ingestion: ~170 TB/day raw → ~23 TB/day stored
  Annual storage growth: ~3 PB/year (with retention policies applied)

Growth drivers:
  - Acreage growth: linear with customer acquisition
  - Imagery resolution: as commercial satellites improve (3m → 1m),
    data volume grows 9x per resolution doubling
  - Spray log detail: trend toward per-plant-level logging increases
    volume by 5–10x vs. current per-nozzle logging
  - Sensor density: trend from 1 sensor/10 acres to 1 sensor/2 acres
    increases sensor volume 5x
```

### Compute Budget per Agricultural Operation

```
Per-acre compute cost breakdown (target: total < $0.50/acre/year):

  Satellite monitoring (continuous, 365 days):
    Processing: ~$0.02/acre/year (amortized tile processing across all fields)
    Storage: ~$0.01/acre/year (3-year NDVI archive)
    Subtotal: $0.03/acre/year

  Drone analysis (5 flights/year average):
    Processing: ~$0.15/acre/year (GPU ortho-mosaic stitching + inference)
    Upload bandwidth: ~$0.02/acre/year
    Subtotal: $0.17/acre/year

  Soil sensor network (continuous):
    Gateway infrastructure: ~$0.05/acre/year (amortized gateway + cellular)
    Ingestion and storage: ~$0.01/acre/year
    Subtotal: $0.06/acre/year

  Yield prediction (weekly, growing season):
    Simulation + ML: ~$0.03/acre/year (with caching optimization)

  Irrigation optimization (daily, growing season):
    Compute: ~$0.01/acre/year

  Prescription generation (10/year average):
    Compute: ~$0.01/acre/year

  Platform overhead (API, storage, sync):
    ~$0.08/acre/year

  Total: ~$0.39/acre/year (within $0.50 target)

  Revenue model:
    Subscription: $3–8/acre/year
    Value delivered: $15–50/acre/year (herbicide savings + yield improvement)
