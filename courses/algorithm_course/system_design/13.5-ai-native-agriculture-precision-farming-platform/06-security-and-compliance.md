...
## Adversarial ML Defense for Spray Models

### Threat: Adversarial Inputs to Weed Detection

The spray controller's ML model is vulnerable to adversarial attacks that could cause it to misclassify crops as weeds (resulting in crop destruction) or weeds as crops (resulting in weed survival).

```
Attack vectors:
  - Physical adversarial patches: printed patterns placed in fields that
    cause misclassification when captured by boom cameras
  - Model extraction: reverse-engineering the edge model via API probing
    (less relevant here since models are on isolated edge hardware)
  - Training data poisoning: corrupting training labels to degrade future
    model versions (e.g., labeling weed images as crop in training data)

Defenses:
  - Hardware-enforced rate limits: even if the model classifies everything
    as weed, PWM duty cycle caps prevent application rates from exceeding
    the product label maximum—physical harm is bounded
  - Confidence thresholding: detections below 0.60 confidence default to
    no-spray rather than spray, limiting the impact of low-confidence
    adversarial misclassifications
  - Post-spray validation: drone surveys 24–48 hours after spraying detect
    crop damage from false positives; if damage rate exceeds 3%,
    model is automatically suspended
  - Training data provenance: all training images are signed with metadata
    (field_id, rig_id, timestamp, GPS); labels require agronomist review
    with two-signature approval for datasets > 1,000 images
  - Model ensemble on edge: run 2 independent models (different architectures)
    on alternating camera pairs; disagreement triggers conservative no-spray
```

---

## Data Aggregation Privacy Risks

### Competitive Intelligence from Aggregate Data

Individual farm data is protected, but aggregated platform data creates privacy risks:

| Aggregate Data | Intelligence Risk | Mitigation |
|----------------|-------------------|------------|
| Regional yield predictions | Commodity traders front-run grain markets using advance yield estimates | Aggregate predictions delayed by 2 weeks before external release; per-farm opt-out |
| County-level weed pressure maps | Herbicide companies target marketing based on infestation patterns | Minimum aggregation unit: 50 farms per region; k-anonymity enforcement |
| Equipment utilization patterns | Competitors analyze market share and adoption trends | Equipment manufacturer data never shared in aggregate; strict contractual controls |
| Water usage patterns | Regulators may use to enforce drought restrictions retroactively | Water usage shared only with explicit farmer consent; per-farm opt-in for regulatory reporting |

### Data Sharing Consent Architecture

```
Consent model:
  - Granular consent per data category (yield, inputs, equipment, water)
  - Per-purpose consent (research, benchmark, regulatory, commercial)
  - Revocable at any time with 72-hour processing SLO
  - Consent state stored in cryptographically tamper-evident log
  - Default: all sharing OFF; farmer must explicitly opt-in per category

Anonymization pipeline:
  - Spatial perturbation: field locations jittered by 1–5 km before aggregation
  - Temporal aggregation: individual data points rolled up to seasonal averages
  - k-anonymity: no aggregate cell contains fewer than k=50 distinct farms
  - Differential privacy: add calibrated noise to aggregate statistics
    (ε = 1.0 for yield estimates, ε = 0.5 for financial data)
```

---

## Supply Chain Security for Edge Firmware

### Edge Device Software Bill of Materials (SBOM)

The spray controller firmware includes third-party components (ML runtime, camera drivers, GPS libraries) that must be tracked for vulnerability management:

```
SBOM requirements:
  - Every firmware build generates CycloneDX SBOM listing all components,
    versions, and known CVEs at build time
  - Firmware release gated on: zero critical CVEs, zero high CVEs older
    than 30 days, all components from verified sources
  - Automated CVE scanning against NVD database runs daily for all
    deployed firmware versions
  - Emergency OTA patch capability for critical CVEs (within 72 hours
    for internet-connected rigs; within 14 days via LoRaWAN for
    intermittently connected rigs)

Firmware update lifecycle:
  - Development → QA validation → Staged rollout (1% → 10% → 50% → 100%)
  - Every firmware version signed with platform HSM key
  - Rollback path: every rig retains n-1 firmware in secondary boot partition
  - Maximum firmware age policy: rigs running firmware > 6 months old
    restricted from loading new ML models until firmware is updated

Incident response for edge security events:
  - Severity 1 (model corruption, safety interlock bypass):
    Fleet-wide halt via LoRaWAN broadcast; all rigs revert to broadcast spray;
    incident response within 15 minutes
  - Severity 2 (single rig compromise, firmware validation failure):
    Isolate affected rig; remote disable via LoRaWAN command;
    dispatch field technician within 48 hours
  - Severity 3 (CVE in non-critical component, firmware update available):
    Schedule OTA update in next maintenance window (within 14 days)
```
