...
| SLI | Definition | SLO Target | Error Budget (30-day) |
|-----|-----------|------------|----------------------|
| **Spray latency** | p99 camera-to-nozzle actuation time (edge-measured) | ≤ 15 ms | < 0.01% of actuations exceed 15 ms |
| **Imagery freshness** | Max age of most recent cloud-free observation per field | ≤ 10 days | < 5% of fields exceed 15 days (weather-adjusted) |
| **Sensor data completeness** | Percentage of expected sensor readings received within 30 min | ≥ 98% | 2% error budget = ~4,800 missed readings/day per 10M sensors |
| **Prescription timeliness** | Time from anomaly detection to prescription delivery | ≤ 24 hours | < 5% of prescriptions exceed 48 hours |
| **Yield prediction calibration** | Quantile calibration error (measured post-harvest) | ≤ 0.05 | P10 events should occur 10% ± 5% of the time |
| **Edge fleet sync coverage** | Percentage of active rigs synced to latest model within 7 days of release | ≥ 95% | 5% budget allows for remote rigs with limited connectivity |

### Error Budget Policy

```
Spray latency budget exhausted (> 0.01% exceed 15 ms):
  → Investigate hardware thermal throttling on affected rigs
  → Halt model complexity increases until budget recovers
  → No new features that increase inference pipeline latency

Imagery freshness budget exhausted (> 5% fields stale > 15 days):
  → Verify satellite provider data delivery SLAs
  → Increase drone survey dispatch for affected regions
  → Activate SAR gap-filling pipeline for extended cloud cover

Sensor completeness budget exhausted (< 98%):
  → Check LoRaWAN gateway health and coverage maps
  → Investigate regional cellular backhaul outages
  → Dispatch field technicians to gateway clusters with > 5% miss rate

Prescription timeliness budget exhausted (> 5% exceed 48 hours):
  → Identify Slowest part of the process stage (satellite processing, anomaly detection,
    drone dispatch, prescription generation)
  → Increase worker pool for Slowest part of the process stage
  → Review drone dispatch automation thresholds
  → Consider relaxing anomaly detection sensitivity to reduce false triggers

Edge fleet sync budget exhausted (< 95% within 7 days):
  → Map connectivity coverage for under-synced rigs
  → Evaluate LoRaWAN model delivery for rigs with no cellular
  → Consider pre-positioning model updates during dealer service visits
```
