...
### Precision Spray Edge Pipeline

```mermaid
sequenceDiagram
    participant CAM as Camera Array
    participant GPU as Edge GPU
    participant MAP as Nozzle Mapper
    participant SOL as Solenoid Controller
    participant LOG as Spray Logger
    participant GPS as GPS Receiver

    Note over CAM,SOL: Total budget: 15 ms

    CAM->>GPU: Raw frame (CSI-2, 1.5 ms)
    GPU->>GPU: Preprocess: undistort + normalize (1.0 ms)
    GPU->>GPU: INT8 inference: weed/crop/soil (7.0 ms)
    GPU->>MAP: Detection results {bbox, class, confidence}
    MAP->>MAP: Project to ground plane (homography, 2.0 ms)
    MAP->>SOL: Nozzle activation commands
    SOL->>SOL: PWM signal generation + valve actuation (2.5 ms)

    Note over CAM,SOL: 15 ms elapsed — weed passes nozzle

    par Async logging (non-blocking)
        GPS->>LOG: Current position
        GPU->>LOG: Detection metadata
        SOL->>LOG: Actuation record
        LOG->>LOG: Buffer to SSD (store-and-forward)
    end
```
...
flowchart TB
    subgraph Inputs["Decision Inputs"]
        ...
        WF[Weather Forecast 10-day]
        CS[Crop Growth Stage]
        ET[ET₀ Reference]
    end

    subgraph Compute["Optimization Engine"]
        WB[Water Balance Model]
        SC[Schedule Candidates]
        OPT[Multi-Objective Optimizer]
    end

    subgraph Output["Irrigation Commands"]
        SCH[Zone Schedules]
        CTL[Pivot Controller]
        RPT[Water Usage Report]
    end
    ...
    class SM,WF,CS,ET input
    class WB,SC,OPT compute
    class SCH,CTL,RPT output
```
...
**Trade-off:** The shared twin creates a potential single point of failure and a consistency Slowest part of the process. The platform uses eventually consistent replication (services write to the twin asynchronously; readers tolerate up to 5-minute staleness) and partitions the twin by geographic region to distribute load.

### Decision 5: Multi-Scale Pest Detection Pipeline vs. Single-Resolution Approach

**Choice:** A three-tier detection pipeline (satellite → drone → camera trap) instead of a single high-resolution survey approach.

**Rationale:** Each resolution level serves a distinct detection purpose. Satellite imagery (10m) detects regional anomaly patterns across millions of acres daily—it cannot identify pest species but can flag stressed zones requiring investigation. Drone imagery (1–5 cm) surveys flagged zones at plant-level resolution, identifying disease symptoms and weed species across hundreds of acres per flight. Camera traps provide 24-hour species-specific identification for targeted areas. Running drone surveys across all managed acreage would require 100,000+ flights/day (infeasible); satellite pre-screening reduces drone surveys to ~5,000/day by targeting only anomalous zones.

**Trade-off:** The multi-tier approach introduces latency: satellite acquisition → anomaly detection → drone dispatch → drone survey → processing → prescription takes 2–5 days end-to-end. For fast-moving threats (e.g., fungal disease spreading across a field in 72 hours), this pipeline may be too slow. The mitigation is automated drone dispatch when satellite anomalies exceed a severity threshold, without waiting for human confirmation.

### Decision 6: Probabilistic Yield Predictions vs. Point Estimates

**Choice:** Yield predictions expressed as quantile distributions (P10–P90) rather than single point estimates.

**Rationale:** Farm financial decisions (crop insurance coverage levels, forward grain contracts, input purchasing) depend on downside risk, not expected value. A farmer selling grain futures needs a reliable P10 estimate to determine the maximum safe delivery commitment—committing to the P50 means a 50% chance of defaulting. Insurance coverage decisions require the probability of yield falling below specific thresholds. Point estimates, no matter how accurate on average, provide no risk information.

**Trade-off:** Quantile regression models are more complex to train, more expensive to evaluate (5 quantiles × model inference cost), and harder for farmers to interpret without clear UX. The platform addresses interpretation through "traffic light" visualizations: green (P10 above county average), yellow (P10 below average), red (P10 below insurance trigger).

---

## Pest Detection Lifecycle

```mermaid
flowchart TB
    subgraph Monitor["Broad Monitoring (Daily)"]
        SAT[Satellite NDVI Scan]
        ANOM[Anomaly Detection]
    end

    subgraph Survey["Targeted Survey (On-Demand)"]
        DD[Drone Dispatch]
        DS[Drone Survey Flight]
        PI[Plant-Level Identification]
    end

    subgraph Confirm["Species Confirmation (24-48h)"]
        CT[Camera Trap Deploy]
        SI[Species Identification]
    end

    subgraph Act["Action"]
        IPM[IPM Decision Engine]
        RX[Treatment Prescription]
        SP[Spray / Application]
    end

    SAT --> ANOM
    ANOM -->|anomaly detected| DD
    ANOM -->|no anomaly| SAT
    DD --> DS
    DS --> PI
    PI -->|high confidence| IPM
    PI -->|low confidence| CT
    CT --> SI
    SI --> IPM
    IPM --> RX
    RX --> SP
    SP -->|post-treatment survey| DS

    classDef monitor fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef survey fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef confirm fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef act fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class SAT,ANOM monitor
    class DD,DS,PI survey
    class CT,SI confirm
    class IPM,RX,SP act
```
...
```mermaid
flowchart LR
    subgraph Train["Cloud Training"]
        TD[Training Data Curation]
        MT[Model Training]
        QT[Quantization INT8]
        VA[Offline Validation]
    end

    subgraph Deploy["Staged Deployment"]
        S1[1% Fleet Canary]
        S2[10% Regional]
        S3[50% National]
        S4[100% Fleet-wide]
    ...
    subgraph Edge["Edge Runtime"]
        ...
        LV[Local Validation]
        AC[Activate Model]
        MN[Monitor Performance]
    end

    subgraph Guard["Quality Gates"]
        G1{Accuracy ≥ 95%?}
        G2{FPR < 2%?}
        G3{Latency < 8ms?}
    end
    ...
    classDef guard fill:#fffde7,stroke:#f57f17,stroke-width:2px

    class TD,MT,QT,VA train
    class S1,S2,S3,S4 deploy
    class DL,LV,AC,MN edge
...
The platform is event-driven, with domain events propagating state changes across subsystems:

| Event | Producer | Consumers | Payload |
|-------|----------|-----------|---------|
| `satellite.tile.available` | Satellite Ingestor | Imagery Pipeline | tile_id, provider, bands, cloud_estimate, bbox |
| `imagery.field.health_updated` | Crop Monitoring Service | Field Digital Twin, Notification Engine | field_id, ndvi_map, anomalies[], timestamp |
| `drone.flight.uploaded` | Drone Ingestor | Drone Analytics Engine | flight_id, field_id, image_count, resolution |
| `drone.orthomosaic.ready` | Drone Analytics Engine | Pest Detection, Field Digital Twin | flight_id, orthomosaic_url, dsm_url, plant_count |
| `sensor.reading.ingested` | Sensor Ingestion Service | Soil Analysis, Irrigation Optimizer | sensor_id, field_id, h3_cell, calibrated_values |
| `sensor.drift.detected` | Calibration Service | Sensor Fleet Manager, Notification Engine | sensor_id, drift_magnitude, drift_direction, recommended_action |
| `spray.session.completed` | Edge Sync Service | Spray Analytics, Field Digital Twin | session_id, rig_id, field_id, acres, savings_pct, summary |
| `pest.outbreak.detected` | Pest Detection Service | Notification Engine, Prescription Generator | field_id, pest_type, severity, affected_area, confidence |
| `yield.prediction.updated` | Yield Prediction Service | Field Digital Twin, Farmer API | field_id, quantiles, confidence_width, model_version |
| `irrigation.schedule.generated` | Irrigation Optimizer | Pivot Controller, Farmer API | field_id, schedule[], water_budget |
| `prescription.ready` | Prescription Generator | Edge Sync, Notification Engine | prescription_id, field_id, operation_type, format, expires_at |
| `weather.forecast.updated` | Weather Ingestor | Irrigation Optimizer, Yield Prediction, Frost Alert | location_grid, forecast_horizon, updated_fields |
| `equipment.telemetry.batch` | Equipment Telemetry Unit | Field Digital Twin, Maintenance Predictor | equipment_id, field_id, can_bus_data[], gps_track |
| `model.deployed.edge` | Edge Fleet Manager | Observability, Spray Analytics | rig_id, model_version, validation_score, deployed_at |

### Event Consumer Matrix

| Consumer | Events Subscribed | Processing Mode |
|----------|-------------------|-----------------|
| **Field Digital Twin** | imagery.*, sensor.*, spray.*, yield.*, equipment.* | Streaming (< 5 min staleness) |
| **Prescription Generator** | imagery.anomaly, pest.outbreak, sensor.threshold_crossed | On-demand (triggered by events) |
| **Notification Engine** | pest.outbreak, sensor.drift, frost.alert, prescription.ready | Real-time push |
| **Spray Analytics** | spray.session.completed, model.deployed.edge | Batch (daily aggregation) |
| **Yield Prediction Service** | weather.forecast.updated, imagery.field.health_updated | Weekly batch + event-triggered recalc |
| **Irrigation Optimizer** | weather.forecast.updated, sensor.reading.ingested | Near-real-time (30 min cycle) |
| **Edge Sync Service** | prescription.ready, model.deployed.edge | Opportunistic (on connectivity) |
...
|---------|----------|-----------|
| **Edge autonomy** | Spray controller operates with zero cloud dependency; all inference on-device | 15 ms latency budget + 25% farmland has no cellular |
| **Data fusion** | H3 hexagonal grid at resolution 10 as spatial index; temporal versioning | Aligns heterogeneous data sources without resolution mismatch |
| **Connectivity resilience** | Store-and-forward with priority queues; LoRaWAN for sensors; prescription caching | Cellular coverage is intermittent; operations cannot stop |
| **Seasonal elasticity** | Counter-seasonal GPU allocation; spot instances for drone processing; pre-warming | 10x–20x load variation; over-provisioning breaks per-acre economics |
| **Model safety** | Staged rollout (1% → 100%); edge validation against reference dataset; auto-rollback | Bad model update to 50K rigs during spray season = catastrophic crop damage |
| **Data portability** | ADAPT framework compliance; ISOBUS task files; 72-hour export SLO | Farmer data rights are legal requirements; prevents vendor lock-in |
| **Agronomic correctness** | Prescription resolution matched to implement capability; physics model anchors ML | Fine-resolution prescription for coarse implement creates false precision |
| **Fail-safe design** | Solenoid default-open; dual-boot firmware; hardware rate limiters | Controller failure must revert to safe (broadcast spray), not dangerous (no spray) |
