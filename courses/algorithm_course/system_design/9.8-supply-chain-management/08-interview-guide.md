...
3. **Multi-objective optimization is the core challenge**. The system must simultaneously minimize cost, maximize service level, balance workloads, and manage risk. These objectives conflict. The interesting design decisions are about how to manage these trade-offs: Pareto frontiers, configurable weights, and tiered optimization strategies (real-time Practical rule of thumb vs. batch optimal).
...
| **Algorithm Knowledge** | FIFO (First-In-First-Out, like a line at a store) allocation, simple routing | ATP calculation; nearest-warehouse routing | Demand pattern classification; safety stock formula; VRP heuristics | Ensemble forecasting with backtesting; multi-objective optimization; order smoothing for bullwhip mitigation |
...
| Route optimization quality gap | 3--5% Practical rule of thumb vs. optimal | Justifies the three-tier optimization architecture |
...
| Multi-echelon safety stock savings | 15--30% vs. per-node optimization | Justifies the graph-based inventory modeling approach |
| Autonomous exception resolution rate | 60--80% of exceptions | Quantifies the value of AI-driven operations |

---

## Mental Models for Quick Decision-Making

### The "Physical-Digital Boundary" Test

When deciding consistency models, ask: "Does this operation trigger a physical-world action?" If yes (picking, shipping, manufacturing, carrier dispatch), strong consistency is required. If no (dashboard display, forecast update, analytics), eventual consistency is acceptable.

```
PHYSICAL-WORLD ACTIONS (strong consistency required):
  ✓ Inventory allocation → triggers warehouse picking
  ✓ Carrier tendering → triggers truck dispatch
  ✓ Warehouse task assignment → directs human worker
  ✓ Production order release → starts manufacturing

DIGITAL-ONLY OPERATIONS (eventual consistency acceptable):
  ✓ Control tower dashboard refresh
  ✓ Forecast model update
  ✓ Supplier performance score recalculation
  ✓ Analytics report generation
```

### The "Bullwhip Amplification" Test

When designing inter-tier communication, ask: "Does this design propagate actual end-customer demand or derived/batched signals?" Derived signals amplify; raw demand signals dampen.

```
AMPLIFYING patterns (avoid):
  ✗ Retailer batches orders weekly → distributor sees lumpy demand
  ✗ Each tier re-forecasts from orders received, not POS data
  ✗ Shortage response: over-order to build buffer

DAMPENING patterns (prefer):
  ✓ Real-time POS data shared to all tiers via event stream
  ✓ VMI: supplier sees actual consumption, not batched orders
  ✓ Order smoothing algorithms at each tier boundary
  ✓ CPFR: shared forecast across tiers
```

### The "Three-Tier Optimization" Framework

When choosing between real-time and batch optimization, use this decision tree:

```
Is the decision customer-facing with latency expectations?
  → YES: Real-time Practical rule of thumb (< 200ms)
     Examples: order routing, ATP check, carrier rate quote

Does cross-entity optimization create significant value?
  → YES: Near-real-time batch (every 15 min)
     Examples: multi-order consolidation, workload rebalancing

Is the problem NP-hard with large solution space?
  → YES: Overnight full solver (8-hour window)
     Examples: fleet routing, production scheduling, network design
```
...
| Red Flag | Why It's Problematic | What a Strong Answer Looks Like |
|----------|---------------------|-------------------------------|
| "We'll use microservices for everything" | SCM domains have different compute profiles; one-size-fits-all microservices miss the planning/execution/visibility separation | Three-plane architecture with explicit rationale for why each plane scales differently |
| "ML model will handle the forecasting" | Ignores the model management problem at scale (500M SKU-locations) | Discusses tiered model strategy, automatic model selection, accuracy monitoring, and graceful degradation |
| "Eventually consistent inventory for scale" | Physical inventory cannot be eventually consistent without causing order cancellations | Selective consistency: strong for allocation, eventual for everything else, with explicit justification |
| "We'll just add more servers" | Ignores that inventory allocation Slowest part of the process is contention-bound, not compute-bound | Discusses hot-SKU mitigation: inventory sharding, reservation pools, optimistic concurrency |
| "Single database with read replicas" | Ignores polyglot persistence needs: time-series for IoT, graph for network, columnar for analytics | Explains why each data type needs a different storage engine and how they integrate |
| "The dashboard shows all the metrics" | Confuses monitoring with the control tower's cross-domain correlation capability | Describes cascading impact analysis: port delay → affected shipments → inventory risk → at-risk orders |
