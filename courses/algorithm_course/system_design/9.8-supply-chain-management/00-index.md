...
| [8.10 - Expense Management](../8.10-expense-management-system/) | Downstream; supply chain expense tracking, freight cost approval workflows |
| [12.5 - URL Shortener](../12.5-url-shortener/) | Tracking link generation pattern; shipment tracking URLs share similar link resolution architecture |

## Related Patterns

| Pattern | Where It Appears | Why It Matters |
|---------|-----------------|---------------|
| **CQRS (Command Query Responsibility Segregation)** | Order management: write-optimized order state vs. read-optimized visibility views | Orders are written with strong consistency but queried at high volume from dashboards; separate models prevent read load from impacting write path |
| **Event Sourcing** | Order lifecycle, inventory movements, shipment milestones | Every state change stored as immutable event enables replay, audit trail, and temporal queries ("what was inventory at time T?") |
| **Saga Pattern** | Order-to-delivery orchestration across OMS → WMS → TMS | Cross-service transactions without distributed locks; compensating actions on failure (deallocate if warehouse cannot fulfill) |
| **Circuit Breaker** | All external integrations (carrier APIs, supplier portals, customs systems) | Prevents cascade failure when external partner systems are unavailable; enables graceful fallback to cached data |
| **Competing Consumers** | IoT event processing, EDI message processing, tracking event ingestion | Multiple consumer instances process partitioned event streams for horizontal throughput scaling |
| **Materialized View** | Control tower dashboards, ATP cache, forecast summary views | Pre-computed read models avoid expensive joins at query time; updated asynchronously via event stream |
| **Bulkhead Isolation** | Three-plane architecture (planning / execution / visibility) | Resource isolation prevents CPU-intensive planning workloads from starving latency-sensitive order processing |
| **Write-Ahead Log** | Inventory allocation under contention; IoT event ingestion | Append-only logging decouples write acceptance from state update, enabling higher throughput for hot paths |
