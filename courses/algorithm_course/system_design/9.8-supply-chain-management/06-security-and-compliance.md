...
The EU Digital Product Passport regulation (effective 2027+) requires products to carry a machine-readable digital record of their origin, composition, repairability, and environmental footprint across the full lifecycle.

| Requirement | Implementation | Data Source |
|-------------|---------------|-------------|
| **Product composition** | Bill of Materials (BOM) with material traceability | Manufacturing execution data + supplier certifications |
| **Carbon footprint per unit** | Lifecycle assessment (LCA) data aggregated from supply chain | Transport emissions (GLEC) + manufacturing energy + raw material extraction |
| **Recyclability and repairability** | Product design metadata + disassembly instructions | Product engineering + reverse logistics disposition data |
| **Chain of custody** | Immutable provenance record from raw material to finished good | Event-sourced supply chain events with cryptographic verification |
| **QR/RFID access** | Each product unit carries a unique identifier linking to its digital passport | Serialization system + public read API for consumers and regulators |

### Supply Chain Software Bill of Materials (SBOM)

For software-embedded supply chain products (IoT devices, smart packaging, connected fleet systems):

| Control | Implementation |
|---------|---------------|
| **SBOM generation** | Automated SBOM creation for all firmware deployed to IoT sensors and gateway devices |
| **Vulnerability tracking** | Continuous CVE monitoring against SBOM components; automated risk scoring |
| **Patch management** | OTA firmware update pipeline with staged rollout, rollback capability, and compliance verification |
| **Supply chain attestation** | SLSA-compliant build provenance for all software artifacts deployed to supply chain infrastructure |

---

## Zero-Trust Network Architecture for Supply Chain

```
ZERO-TRUST PRINCIPLES APPLIED TO SCM:

1. IDENTITY-FIRST ACCESS:
   - Every request (user, service, device, partner) authenticated before processing
   - No implicit trust from network location (internal ≠ trusted)
   - Service mesh with mTLS for all inter-service communication

2. MICRO-SEGMENTATION:
   - Planning plane cannot directly access execution plane databases
   - Partner collaboration services isolated in separate network segment
   - IoT gateway network segmented from business application network

3. LEAST-PRIVILEGE DATA ACCESS:
   - Supplier sees only their own SKUs, POs, and scorecards
   - Carrier sees only shipments tendered to them
   - Warehouse worker sees only tasks assigned to their zone
   - Cross-tenant data access blocked at the query layer (mandatory tenant_id filter)

4. CONTINUOUS VERIFICATION:
   - Session tokens re-validated every 15 minutes
   - Device certificates checked against revocation list on every connection
   - Anomalous access patterns (unusual time, location, volume) trigger step-up auth
```
