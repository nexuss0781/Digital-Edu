...
## 6. Advanced Discussion Points (2025–2026 Context)

### 6.1 Emerging Topics Interviewers May Raise

| Topic | Key Points to Demonstrate |
|---|---|
| **Federated learning for health models** | Explain how gradient aggregation replaces data centralization; mention differential privacy on gradients; note FDA predetermined change control plans for continuously learning models |
| **Non-invasive CGM integration** | Discuss the new clinical pipeline requirements; 1-minute sampling creates 10x data volume per sensor; bidirectional integration with insulin delivery systems |
| **On-device NPU inference** | Modern wearable SoCs include dedicated neural processing units that reduce inference power by 60%+; enables larger on-device models (500 KB–1 MB INT8) |
| **Digital biomarkers** | Multi-modal patterns (gait + HRV + voice) that correlate with neurological conditions; requires cross-sensor temporal alignment pipelines |
| **BLE 5.4 enhancements** | Higher throughput (400–800 KB/s) reduces sync duration; Periodic Advertising with Responses (PAwR) enables new power-optimized sync patterns |

### 6.2 Distinguishing "Great" Answers

| Question Area | Good Answer | Great Answer |
|---|---|---|
| **Data model** | "Time-series DB for sensor data" | "Time-series partitioned by user_id with continuous aggregation; confidence score as first-class column; tiered retention matching clinical value decay" |
| **Battery optimization** | "Use BLE, it's low power" | "Power budget cascade: 300 mAh → 1.2 mW average → allocate per subsystem → adaptive duty cycling trades data granularity for battery dynamically based on activity state and remaining charge" |
| **Alert accuracy** | "Use ML to detect anomalies" | "Two-tier inference with INT8 on-device for recall, float32 cloud for precision; personalized baselines replace population thresholds; hysteresis prevents single-beat false alarms; confidence-weighted multi-signal fusion" |
| **Privacy** | "Encrypt everything" | "Cryptographic erasure: delete per-user key → all encrypted PHI unrecoverable; consent-gated hybrid where base functionality uses on-device inference only; federated learning enables model improvement without PHI centralization" |
