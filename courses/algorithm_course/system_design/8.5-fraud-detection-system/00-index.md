...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, API design, scoring algorithms (Step-by-step plan in plain English) |
...
| [4.1 - Notification System](../4.1-notification-system/) | Real-time fraud alerts to users, analyst notifications |
| [1.4 - Distributed LRU Cache](../1.4-distributed-lru-cache/) | Feature store caching, model result caching for repeated entities |
| [8.6 - Distributed Ledger Core Banking](../8.6-distributed-ledger-core-banking/) | Transaction audit trails, double-entry integrity, compliance ledgers |
| [8.9 - Buy Now Pay Later](../8.9-buy-now-pay-later/) | Credit risk scoring, installment fraud patterns, synthetic identity detection |
| [12.13 - Bot Detection System](../12.13-bot-detection-system/) | Device fingerprinting, behavioral biometrics, bot-vs-human classification |
...
|---------|---------------------------|
| **Circuit Breaker** | Independent circuit breaker per downstream dependency (feature store, model serving, rules engine); fail-open on scoring path |
| **Dual-Speed Feature Store** | Real-time features (sub-second freshness, in-memory) + batch features (hourly, key-value store); unified retrieval at scoring time |
| **Ensemble Model** | GBT + DNN + anomaly detection with weighted aggregation; defense-in-depth through architectural diversity |
| **Event Sourcing** | Every transaction event, scoring decision, and analyst disposition captured in append-only event store; enables replay and audit |
| **CQRS** | Scoring hot path (command: score transaction) separated from analytics/reporting cold path (query: fraud trends, model performance) |
| **Saga / Compensating Transaction** | Fail-open scoring with async compensation: allowed transactions flagged for post-review; rollback via chargeback process |
| **Canary Deployment** | Shadow mode → 5% canary → gradual rollout for model versions; automatic rollback on metric degradation |
| **Graph Analysis** | Entity resolution graph with community detection; risk propagation from confirmed fraud nodes through relationship edges |

---

## Evolution and Industry Trends (2024-2026)

| Trend | Impact on Fraud Detection Architecture |
|-------|---------------------------------------|
| **Generative AI for Case Automation** | LLM-generated investigation summaries, SAR narrative drafts; analyst productivity 2-3x improvement |
| **Behavioral Biometrics** | Typing cadence, mouse movement, touch pressure as unspoofable features; requires SDK integration and streaming pipeline |
| **Real-Time Graph Queries** | Sub-100ms graph lookups for high-value transactions via pre-materialized subgraph caches |
| **Federated Learning** | Cross-institution model training without sharing raw data; privacy-preserving fraud intelligence sharing |
| **Network Tokenization** | Visa/Mastercard network tokens replace PANs; changes card-based feature engineering approach |
| **Instant Payments (RTP/FedNow)** | Irrevocable real-time payments eliminate chargeback recovery; shifts fraud prevention from post-authorization to pre-authorization |
| **Synthetic Identity Detection** | ML models trained on credit bureau thin-file patterns; graph-based detection of identity fabrication networks |
| **Explainable AI Regulation** | EU AI Act mandates transparency for high-risk AI systems; SHAP/LIME explanations become regulatory requirement, not nice-to-have |
...
- Visa Research --- Network-Level Fraud Detection with Cross-Merchant Intelligence
- Federal Reserve --- FedNow Service: Implications for Fraud Risk Management
- European Banking Authority --- PSD2 Strong Customer Authentication and Fraud Prevention
