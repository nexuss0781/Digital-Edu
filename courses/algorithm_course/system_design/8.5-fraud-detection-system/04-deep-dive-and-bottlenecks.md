...
## 8. Real-Time Stream Processing Challenges

### Out-of-Order Events and Late Arrivals

Transaction events arrive from multiple payment channels (card-present, card-not-present, ACH, wire) with different propagation delays. A velocity counter that uses event-time (when the transaction occurred) rather than processing-time (when the event arrived) must handle out-of-order events correctly.

```
Problem:
  t=0s:  Transaction A at merchant_1 (arrives at t=0.5s)
  t=1s:  Transaction B at merchant_2 (arrives at t=1.2s)
  t=0.8s: Transaction C at merchant_3 (arrives at t=3s — delayed by network)

  If velocity window = "last 5 minutes by event-time":
    At t=1.2s: counter shows 2 transactions (A, B)
    At t=3.0s: Transaction C arrives late — must retroactively update the counter to 3

  This matters because: If the counter had shown 3 at t=1.2s, Transaction B might have
  been blocked. Late arrivals create a permanently incomplete view of velocity.
```
...
| Strategy | Trade-off |
|----------|-----------|
| Processing-time windows | Simpler but velocity accuracy depends on event arrival order |
| Event-time windows with watermarks | Accurate but adds latency equal to watermark delay |
| Hybrid: process-time for scoring, event-time for training | Fast scoring, correct training data |
| Speculative updates with correction | Score immediately, correct features retroactively |

### Exactly-Once Feature Updates

Velocity counters are stateful aggregations. If a stream processor crashes mid-update, the counter may be incremented twice (at-least-once delivery) or not at all (at-most-once). For fraud detection, over-counting is safer than under-counting (slightly more false positives vs. missed fraud):

```
Safe default: At-least-once delivery with idempotent updates
  - Each transaction event carries a unique event_id
  - Feature update: INCREMENT_IF_NOT_SEEN(counter_key, event_id, amount)
  - Deduplication window: 2x maximum expected event delay
  - Deduplication state stored alongside counter state (co-partitioned)
```

### Partition Rebalancing During Scaling

When the stream processor scales out (adding partitions to handle holiday traffic spikes), state must be redistributed without dropping in-flight feature updates. The rebalancing protocol:

```
1. New consumer joins group → coordinator triggers rebalance
2. Affected partitions pause consumption (events buffer in broker)
3. State checkpoints are migrated to new partition owners
4. New owners restore state and resume consumption
5. During migration window (5-15s): feature store serves stale counters

Impact: 5-15s of stale velocity features during rebalance
Mitigation: Circuit breaker on feature freshness; fall back to batch
         features if real-time features are > 10s stale
```

---

## 9. Cross-Merchant Network Intelligence

### The Network Effect in Fraud Detection

```
Single-merchant detection:
  Merchant A sees: device_123 made 3 purchases today → looks normal
  Merchant B sees: device_123 made 2 purchases today → looks normal
  Merchant C sees: device_123 made 4 purchases today → looks normal

Cross-merchant intelligence:
  Network sees: device_123 made 9 purchases across 3 merchants in 1 day
  → Highly suspicious: legitimate users rarely hit 3+ new merchants in a day

Implementation:
  1. Each merchant's scoring events publish device/IP hashes to shared intelligence hub
  2. Hub maintains cross-merchant velocity counters per entity hash
  3. Hub computes reputation scores (fraud rate per entity across all merchants)
  4. Scores fed back as batch features to each merchant's scoring pipeline

Privacy constraint:
  - Only hashed identifiers shared (no PII crosses merchant boundaries)
  - Reputation scores are aggregated (merchant A cannot see merchant B's transactions)
  - Differential privacy noise added to prevent reconstruction attacks
  - Each merchant can opt out of contributing (but still benefits from network)
```
...
| Slowest part of the process | Severity | Impact | Mitigation |
|-----------|----------|--------|------------|
| Feature store cache miss | High | +5-15ms per miss, breaking latency budget | L1/L2/L3 caching hierarchy; proactive warming |
| Model inference cold start | High | 1-3s for first request on new pod | Pre-warming pipeline; health check gates |
| GC pauses in scoring service | Medium | 10-50ms spikes at p99 | Off-heap model data; tuned GC configuration |
| Graph supernode traversal | High | 2-hop queries explode to millions of paths | Degree-capped traversal; pre-materialized subgraphs |
| Stream processor consumer lag | Medium | Stale velocity features; missed rapid-fire fraud | Partition-based scaling; backpressure handling |
| Label delay (chargebacks) | High | 30-90 day feedback delay; models train on stale labels | Analyst dispositions as fast proxy; random sampling |
| Adversarial drift | Critical | Model performance degrades as fraudsters adapt | Feature importance monitoring; continuous feature expansion |
| Point-in-time feature joins | Medium | Training/serving skew if features are not snapshotted | Versioned feature store; temporal join validation |
| Cross-region feature replication | Medium | Stale features in secondary region during failover | Active-active replication; regional feature stores |
| Rule proliferation | Low-Medium | 2,000+ rules slow evaluation; conflicting rules | Rule performance tracking; sunset ineffective rules |
