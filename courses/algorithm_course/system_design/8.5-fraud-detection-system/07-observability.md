...
## Analyst Productivity Analytics

```
Analyst Performance Metrics (per-analyst tracking):

1. Throughput:
   - Cases resolved per day (target: 40-60)
   - Average investigation time per case (target: 3-5 minutes)
   - Cases by disposition type (confirmed_fraud / false_positive / escalated)

2. Quality:
   - Agreement rate with chargeback outcomes (target: > 85%)
   - Overturn rate by supervisor (target: < 5%)
   - Consistency score: inter-analyst agreement on same case types (target: > 80%)

3. Efficiency:
   - Time to first action (target: < 15 min from assignment)
   - Re-open rate (target: < 3%)
   - Escalation appropriateness (% of escalations that result in higher-severity action)

Analyst Skill Routing:
  Route cases to analysts based on expertise:
  ├── New analyst: Simple blocklist hits, clear-cut fraud patterns
  ├── Mid-level: Ambiguous scores (0.6-0.8), multi-factor decisions
  ├── Senior analyst: Fraud ring investigations, high-value cases (> $10K)
  └── Specialist: SAR preparation, synthetic identity, account takeover
```
...
```
Automated fraud pattern clustering and tracking:

FUNCTION identify_emerging_patterns(confirmed_fraud_cases, lookback_days=7):
    -- Cluster confirmed fraud by feature similarity
    fraud_features = [case.top_features for case in confirmed_fraud_cases]

    clusters = DBSCAN(fraud_features, eps=0.3, min_samples=5)

    FOR cluster IN clusters:
        IF cluster.is_new AND cluster.size > EMERGENCE_THRESHOLD:
            pattern = {
                name: AUTO_NAME(cluster.centroid_features),
                first_seen: cluster.earliest_case.timestamp,
                volume: cluster.size,
                avg_amount: MEAN(cluster.cases.amount),
                common_features: cluster.centroid_features,
                affected_merchants: DISTINCT(cluster.cases.merchant_id),
                growth_rate: cluster.size / lookback_days
            }

            ALERT("Emerging fraud pattern detected: " + pattern.name)
            SUGGEST_RULE(pattern.common_features)
            PRIORITIZE_RETRAIN(include_pattern=pattern)

    RETURN clusters

Tracked Pattern Categories:
├── Account Takeover: credential stuffing, SIM swap, phishing
├── Synthetic Identity: fabricated identities, credit building schemes
├── Card Testing: small transactions to validate stolen cards
├── Bust-Out: build history then exploit credit limits
├── Friendly Fraud: legitimate users filing false chargebacks
├── Refund Fraud: return merchandise fraud, receipt manipulation
├── Promotion Abuse: fake accounts for sign-up bonuses
└── Money Mule: accounts used to launder proceeds
```
