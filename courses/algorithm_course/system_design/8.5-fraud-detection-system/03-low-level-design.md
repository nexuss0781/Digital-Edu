...
### Device Fingerprint Similarity Computation

```
FUNCTION compute_device_similarity(current_device, historical_devices):
    -- Compare current device fingerprint against user's known devices
    -- Returns 0.0 (completely different) to 1.0 (exact match)

    IF LEN(historical_devices) == 0:
        RETURN 0.0  -- New user, no history
    ...
    FOR device IN historical_devices:
        ...
        -- Screen resolution (exact match important)
        IF current_device.screen_resolution == device.screen_resolution:
            score += 0.15
        total_weight += 0.15

        -- Timezone (strong signal for geo consistency)
        IF current_device.timezone_offset == device.timezone_offset:
            score += 0.10
        total_weight += 0.10

        -- Browser/app version (fuzzy match — major version only)
        IF MAJOR_VERSION(current_device.user_agent) == MAJOR_VERSION(device.user_agent):
            score += 0.10
        total_weight += 0.10

        -- OS family and version
        IF OS_FAMILY(current_device.user_agent) == OS_FAMILY(device.user_agent):
            score += 0.15
            IF OS_VERSION(current_device.user_agent) == OS_VERSION(device.user_agent):
                score += 0.05
        total_weight += 0.20

        -- Canvas/WebGL hash (strong hardware fingerprint)
        IF current_device.canvas_hash == device.canvas_hash:
            score += 0.25
        total_weight += 0.25

        -- Language and locale
        IF current_device.language == device.language:
            score += 0.05
        total_weight += 0.05

        -- IP subnet similarity (same /16)
        IF SUBNET(current_device.ip, 16) == SUBNET(device.ip, 16):
            score += 0.05
        total_weight += 0.05

        normalized = score / total_weight
        best_match = MAX(best_match, normalized)

    RETURN best_match
```

### Velocity Window Management

```
FUNCTION manage_velocity_windows(entity_id, entity_type, event_timestamp, amount):
    -- Maintain multiple sliding window counters for velocity features
    -- Uses sorted set with timestamp scores for precise window computation

    windows = [
        {name: "1m",  duration_seconds: 60},
        {name: "5m",  duration_seconds: 300},
        {name: "1h",  duration_seconds: 3600},
        {name: "24h", duration_seconds: 86400}
    ]

    key_prefix = entity_type + ":" + entity_id

    FOR window IN windows:
        count_key = key_prefix + ":count:" + window.name
        amount_key = key_prefix + ":amount:" + window.name

        -- Add current event to sorted set (score = timestamp)
        RT_STORE.sorted_set_add(count_key, event_timestamp, event_timestamp)
        RT_STORE.sorted_set_add(amount_key, event_timestamp, amount)

        -- Remove expired entries (older than window duration)
        cutoff = event_timestamp - window.duration_seconds
        RT_STORE.sorted_set_remove_range_by_score(count_key, 0, cutoff)
        RT_STORE.sorted_set_remove_range_by_score(amount_key, 0, cutoff)

        -- Set TTL to auto-expire inactive windows
        RT_STORE.set_ttl(count_key, window.duration_seconds * 2)
        RT_STORE.set_ttl(amount_key, window.duration_seconds * 2)

    -- Return current window values for scoring
    RETURN {
        txn_count_1m:      RT_STORE.sorted_set_count(key_prefix + ":count:1m"),
        txn_count_5m:      RT_STORE.sorted_set_count(key_prefix + ":count:5m"),
        txn_count_1h:      RT_STORE.sorted_set_count(key_prefix + ":count:1h"),
        txn_count_24h:     RT_STORE.sorted_set_count(key_prefix + ":count:24h"),
        txn_amount_sum_1h: RT_STORE.sorted_set_sum(key_prefix + ":amount:1h"),
        txn_amount_sum_24h:RT_STORE.sorted_set_sum(key_prefix + ":amount:24h")
    }
```
...
ModelPerformanceSnapshot {
    ...
    snapshot_date       : DATE
    evaluation_window   : ENUM               -- daily, weekly, monthly

    -- Aggregate Metrics
    auc_roc             : FLOAT
    auc_pr              : FLOAT
    precision_at_95r    : FLOAT              -- Precision when recall = 95%
    recall_at_95p       : FLOAT              -- Recall when precision = 95%
    false_positive_rate : FLOAT
    detection_rate_value: FLOAT              -- By fraud value, not count
    log_loss            : FLOAT

    -- Per-Segment Metrics
    segment_metrics     : JSONB              -- {cnp: {auc: 0.98, fpr: 0.04}, cp: {auc: 0.96, ...}}

    -- Feature Importance
    top_features        : JSONB              -- [{name: "velocity_5m", importance: 0.15}, ...]
    feature_importance_hash : VARCHAR(64)    -- Hash of ranked feature list for drift detection

    -- Drift Indicators
    psi_max             : FLOAT              -- Max PSI across monitored features
    psi_features_above_threshold : INT       -- Count of features with PSI > 0.10
    score_distribution_hash : VARCHAR(64)    -- Hash of binned score histogram

    -- Operational
    total_scored        : BIGINT
    total_labeled       : BIGINT
    label_rate          : FLOAT              -- Fraction of scored transactions with labels

    INDEX idx_model_date ON (model_version, snapshot_date)
    INDEX idx_date ON (snapshot_date)
}
```
