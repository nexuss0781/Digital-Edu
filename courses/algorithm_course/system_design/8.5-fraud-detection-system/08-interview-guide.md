...
### "How would you handle instant/irrevocable payments (RTP, FedNow)?"

**Key points:**
- Traditional card payments have chargeback protection (merchant bears cost); instant payments are irrevocable
- Must shift from post-authorization to pre-authorization fraud prevention
- Tighter thresholds: false negatives are permanent losses with no recovery mechanism
- Real-time graph queries become critical for high-value instant transfers
- Step-up authentication for all transactions above a threshold (e.g., $500)
- Payee-side risk scoring (is the recipient a known mule account?)

### "How would you detect synthetic identity fraud?"

**Key points:**
- Synthetic identities combine real and fabricated data (real SSN + fake name + fake address)
- Traditional identity verification passes because partial data is valid
- Graph analysis reveals: multiple "identities" sharing thin-file attributes (recently issued SSN, no credit history, new address)
- Behavioral patterns: synthetic IDs "credit build" for 3-6 months then execute burst fraud
- Cross-institution signals: same SSN appearing at multiple institutions simultaneously
- ML features: credit file thin-ness, address stability, identity element recombination patterns

### "How would you handle model explainability for regulators?"

**Key points:**
- EU AI Act classifies fraud detection as "high-risk AI" requiring transparency
- GDPR Article 22 gives right to explanation for automated decisions
- SHAP values for per-prediction explanations (which features contributed most)
- Global feature importance for model-level understanding
- Counterfactual explanations: "This transaction would have been allowed if the amount were below $500 and from your usual device"
- Model cards documenting training data, feature set, performance, known limitations
- Decision audit trail: reproduce any historical decision with exact features and model version

---

## Complexity Escalation Guide

| Topic | When to Raise | Time Needed | Signal Level |
|-------|-------------|-------------|--------------|
| **Step-up authentication** | After basic scoring pipeline | 3-4 min | Senior |
| **Cross-merchant intelligence** | After graph analysis discussion | 4-5 min | Staff |
| **Instant payment fraud** | After reliability discussion | 3-4 min | Staff+ |
| **Federated learning** | If privacy concerns raised | 3-4 min | Principal |
| **A/B testing fraud models** | After model deployment | 3-4 min | Staff |
...
```
Must-draw components:
☑ Payment Service → Scoring API (synchronous)
☑ Decision Service orchestrating rules + ML
☑ Rules Engine (blocklists, velocity, geo)
☑ Feature Assembly with RT + Batch stores
☑ Model Serving (ensemble: GBT + DNN + anomaly)
☑ Event Bus → async consumers
☑ Case Management workflow

Should-mention:
☑ Fail-open architecture with circuit breakers
☑ Stream Processor for real-time feature computation
☑ Graph Engine for fraud ring detection (async)
☑ Model canary deployment pipeline
☑ Dynamic thresholds by segment

Depth signals:
☐ Step-up authentication layer
☐ Cross-merchant intelligence hub
☐ Point-in-time feature consistency for training
☐ Adversarial drift detection
☐ Selection bias and random sampling
...
|-----------|--------|-------------|-----------|--------------|-------------|
| **Requirements** | 10% | Skips scoping | Identifies latency and accuracy | Articulates cost asymmetry (FP vs FN) | Quantifies ROI and error budgets |
| **Architecture** | 25% | Monolithic "fraud check" | Separates rules from ML | Hot/cold path separation with fail-open | Cross-merchant intelligence; step-up layer |
| **ML Knowledge** | 20% | "Use a model" | Discusses ensemble rationale | Feature engineering + point-in-time | Adversarial drift; A/B testing challenges |
| **Feature Engineering** | 15% | Ignores features | Knows velocity counters | Dual-speed store with caching tiers | Cross-merchant features; privacy-preserving aggregation |
| **Reliability** | 15% | No failure discussion | Mentions fail-open | Graceful degradation levels per dependency | Chaos engineering; regional failover; feature consistency |
| **Adversarial Thinking** | 15% | Treats as static ML | Mentions model retraining | Discusses drift, feedback loops, probing | Feature rotation; honeypots; continuous expansion |

### Score Interpretation

| Total Score | Recommendation |
|-------------|---------------|
| 1-2 | Not ready: lacks understanding of real-time ML systems and adversarial dynamics |
| 3-4 | Junior/mid: understands basics but misses reliability and feedback loop integrity |
| 5-6 | Senior: strong architecture with fail-open, dual-speed features, and ensemble rationale |
| 7-8 | Staff+: discusses cross-merchant intelligence, adversarial defense, and instant payment implications |

### Common Follow-Up Scenarios

| Scenario | What to Look For |
|----------|-----------------|
| "Fraud rate suddenly drops to zero" | Is it real (good!) or is the model broken? Check chargebacks vs model predictions; verify feature store health |
| "A VIP customer was blocked" | Explain dynamic thresholds, step-up authentication; discuss the UX recovery path and how to prevent recurrence |
| "Fraudsters figured out our rules" | Discuss adversarial drift, feature importance monitoring, rule-vs-ML layering, and why you can't just add more rules |
| "We need to support instant payments" | Explain the loss of chargeback safety net, pre-auth vs post-auth shift, payee-side scoring |
| "An analyst is rubber-stamping cases" | Discuss label quality impact on model accuracy, supervisor review sampling, consistency scoring |
