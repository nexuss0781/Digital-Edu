...
```
Every model decision must be fully reproducible for regulatory audit:

Decision Provenance Chain:
  1. Transaction event (immutable, timestamped)
  2. Feature snapshot at scoring time (versioned)
  3. Model version and weights used
  4. Rule set version active at time of scoring
  5. Threshold configuration active
  6. Decision output and explanations
  7. Analyst disposition (if reviewed)
  8. Chargeback outcome (if disputed)

Storage: All 8 components stored with scoring_id as correlation key
Retention: 7 years (regulatory minimum for financial records)
Access: Compliance team read-only; immutable append-only storage
Query: "Reproduce the exact decision for transaction X from 3 years ago"

FUNCTION reproduce_decision(scoring_id):
    scoring = AUDIT_STORE.get(scoring_id)
    features = FEATURE_SNAPSHOT_STORE.get(scoring.feature_snapshot_id)
    model = MODEL_ARCHIVE.get(scoring.model_version)
    rules = RULE_ARCHIVE.get(scoring.rule_set_version)
    thresholds = THRESHOLD_ARCHIVE.get(scoring.threshold_version)

    reproduced_score = model.predict(features)
    reproduced_rules = rules.evaluate(scoring.transaction, features)
    reproduced_decision = DECIDE(reproduced_score, reproduced_rules, thresholds)

    ASSERT reproduced_decision == scoring.decision
    RETURN {original: scoring, reproduced: reproduced_decision, match: true}
```

---

## Adversarial ML Defense Strategies

| Defense | Mechanism | Effectiveness | Cost |
|---------|-----------|---------------|------|
| **Score obfuscation** | Return allow/block/review, never raw scores | Prevents gradient-based probing | Zero (implementation only) |
| **Rate limiting probes** | Cap scoring API calls per entity per hour | Prevents systematic boundary mapping | Minimal (existing infrastructure) |
| **Ensemble diversity** | GBT + DNN + anomaly with different decision surfaces | Requires evading 3 independent models simultaneously | Moderate (model training cost) |
| **Feature rotation** | Periodically add new features, deprecate leaked ones | Changes the target for adversarial optimization | High (continuous feature engineering) |
| **Honeypot features** | Features that only change when system is being probed | Detects adversarial probing directly | Low (monitoring cost) |
| **Adversarial training** | Include adversarial examples in training data | Model learns to be robust to known evasion tactics | Moderate (training pipeline changes) |
| **Behavioral canaries** | Inject known-fraud patterns at low volume to test model | Validates model still catches baseline attack vectors | Low (automated testing) |
