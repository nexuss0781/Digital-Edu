...
| Component | Scaling Dimension | Scaling Strategy | Slowest part of the process |
|-----------|------------------|-----------------|-----------|
...
### Why Naive A/B Testing Fails for Fraud

```
Problem: Standard A/B testing doesn't work for fraud models.

In a normal A/B test:
  Group A: sees old model → some fraud blocked, some allowed
  Group B: sees new model → some fraud blocked, some allowed

The problem: blocking changes the outcome.
  - If Model A blocks Transaction X → we never learn if it was actually fraud
  - If Model B allows Transaction X → it may result in a chargeback (label)
  - Model B gets MORE labels (chargebacks from allowed transactions)
  - Model A gets FEWER labels (blocked transactions have no ground truth)

This creates an apples-to-oranges comparison where the model that
blocks less appears to "find more fraud" simply because it generated
more labels.
```
...
```
1. Shadow Scoring (Recommended for initial comparison):
   - Both models score every transaction
   - Only production model's decision is used
   - Compare models on score agreement/disagreement
   - Advantage: no impact on users
   - Limitation: can't measure real-world FP impact

2. Interleaved Deployment:
   - For transactions in the "review" zone (score 0.55-0.85):
     - Randomly assign to Model A or Model B for final decision
     - Track chargebacks and FP rates per group
   - For clear allow/block zones: use production model
   - Advantage: measures real-world impact on ambiguous transactions
   - Limitation: only tests the gray zone

3. Counterfactual Logging:
   - Production model makes all decisions
   - Log what the challenger model WOULD have decided
   - After chargeback settlement (30-90 days):
     - Compare: did the challenger correctly identify fraud that production missed?
   - Advantage: no user impact; measures detection improvement
   - Limitation: 30-90 day delay for results

4. Random Holdout:
   - 0.1% of transactions scored by both models
   - Both decisions logged; production decision enforced
   - Analyst reviews all holdout transactions regardless
   - Provides unbiased comparison on a small sample
```
...
### Point-in-Time Correctness Architecture

```
Problem: When the scoring service fetches features, the features must reflect
state BEFORE the current transaction, not after. If Transaction #N updates
a velocity counter and Transaction #N's scoring reads the updated counter,
it sees "future" information.

Solution: Read-before-write ordering

FUNCTION score_with_consistency(transaction):
    -- Step 1: Read features FIRST (reflects pre-transaction state)
    features = FEATURE_STORE.read(transaction.entity_keys)

    -- Step 2: Score with pre-transaction features
    score = MODEL.predict(features)

    -- Step 3: Publish transaction event (AFTER scoring)
    EVENT_BUS.publish(transaction)

    -- Step 4: Stream processor updates features asynchronously
    -- (happens after scoring, so next transaction sees updated features)

    RETURN score

For training data:
    -- Feature store maintains versioned snapshots
    -- Training pipeline joins each transaction with feature snapshot
    -- closest to (but not after) the transaction timestamp
    -- This ensures training features match inference features exactly

    FUNCTION build_training_sample(transaction, label):
        feature_snapshot = FEATURE_STORE.get_snapshot(
            entity_keys = transaction.entity_keys,
            as_of = transaction.timestamp - EPSILON  -- Strictly before
        )
        RETURN {
...
## Cross-Merchant Intelligence Scaling

```
Scaling the Fraud Intelligence Hub:

Tier 1: Small network (< 100 merchants):
  - Single hub instance
  - Batch aggregation (hourly)
  - Simple hash-based reputation scores
  - Latency: features available within 1 hour

Tier 2: Medium network (100-1,000 merchants):
  - Sharded hub (partition by entity hash)
  - Near-real-time aggregation (per-minute)
  - Weighted reputation with decay
  - Latency: features available within 5 minutes

Tier 3: Large network (1,000+ merchants):
  - Distributed hub with regional shards
  - Real-time streaming aggregation
  - ML-based reputation scoring
  - Differential privacy for entity-level signals
  - Latency: features available within 30 seconds

Data volume at scale:
  10M transactions/day × 1,000 merchants = 10B events/day to hub
  Entity deduplication reduces to ~500M unique entities
  Storage: ~500M × 200 bytes = 100 GB (fits in memory cluster)

Network value formula:
  Detection improvement ≈ k × log(merchant_count)
  At 10 merchants: ~15% improvement over single-merchant
  At 100 merchants: ~30% improvement
  At 1,000 merchants: ~45% improvement
  Diminishing returns after ~1,000 merchants for any given fraud vector
```
...
Graph growth trajectory:
  Current: 100M nodes, 1B edges
  +6 months: 160M nodes, 1.8B edges
  +12 months: 280M nodes, 3.5B edges

Scaling strategy:

Phase 1 — Vertical (current → 200M nodes):
  - Single graph database cluster with read replicas
  - Hot subgraphs cached in memory
  - Community detection runs on full graph hourly
  - Sufficient for batch + limited real-time queries

Phase 2 — Sharded (200M → 1B nodes):
  - Partition graph by community membership
  - Each shard holds one or more communities
  - Cross-shard edges maintained in a lightweight edge index
  - Community detection runs per-shard with cross-shard merge

Phase 3 — Tiered (1B+ nodes):
  - Hot tier: recent entities (last 90 days) in memory graph
  - Warm tier: all entities in disk-backed graph
  - Cold tier: historical graph snapshots for forensic queries
  - Query routing: check hot → warm → cold

Graph query optimization:
├── Pre-materialize 2-hop subgraphs for top-10K entities
├── Maintain edge index sorted by risk score for pruned traversal
├── Batch community detection results cached in feature store
└── Real-time queries limited to 2-hop with degree cap = 200
```
