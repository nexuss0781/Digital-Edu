...
| API Category | Per-User Limit | Per-Tenant Limit | Burst | Notes |
|-------------|---------------|-----------------|-------|-------|
| **Requisition CRUD** | 50/hour | 5,000/hour | 2x for 5 min | Higher limits during bulk upload via CSV |
| **Approval actions** | 200/hour | 10,000/hour | 3x for 10 min | Batch approval UI generates bursts |
| **Catalog search** | 300/hour | 50,000/hour | 2x for 5 min | Typeahead generates rapid sequential requests |
| **PO operations** | 50/hour | 5,000/hour | 2x for 5 min | Amendment tracking counts separately |
| **Invoice submission** | 100/hour | 20,000/hour | 5x for 30 min | Vendor batch submissions need higher burst |
| **Budget queries** | 500/hour | 100,000/hour | No burst | High-frequency; dashboard polling |
| **Vendor management** | 30/hour | 1,000/hour | No burst | Low-frequency administrative operations |
| **Report generation** | 10/hour | 200/hour | No burst | CPU-intensive; rate-limited aggressively |
| **Auction bid submission** | 60/min per auction | Unlimited | N/A | Per-auction rate to prevent bid flooding |
...
| Document Type | Minimum Retention | Jurisdiction Notes |
|---------------|-------------------|-------------------|
| Purchase Orders | 7 years | SOX: 7 years from fiscal year close; Germany (HGB): 10 years |
| Invoices | 7 years | SOX; EU VAT Directive: 6-10 years depending on member state |
| Approval decisions | 7 years | Part of financial controls audit trail |
| Vendor contracts | Contract term + 7 years | Retention clock starts at contract expiration |
| RFQ / bid records | 5 years | Government contracts may require 15+ years (FAR 4.703) |
| Budget records | 7 years | Aligned with financial statement retention |
| Audit logs | 7 years minimum | Tamper-evident storage required; litigation hold override |

---

## Integration Latency Requirements

| Integration | Direction | Latency SLO | Protocol |
|-------------|-----------|-------------|----------|
| ERP / GL sync | Outbound (PO postings, budget sync) | < 30s for journal entries; batch for bulk sync | REST API / message queue |
| Vendor PO dispatch | Outbound | < 5 minutes from PO approval | cXML, EDI/AS2, email fallback |
| Invoice OCR processing | Inbound | < 60s per page | REST API (async with callback) |
| Sanctions screening | Outbound | < 10s for real-time; < 4 hours for batch | REST API |
| Credit rating check | Outbound | < 30s | REST API |
| Tax engine calculation | Outbound/Inbound | < 500ms per line item | REST API (synchronous) |
| Catalog punch-out | Round-trip | < 3s for session initiation | cXML / OCI |
