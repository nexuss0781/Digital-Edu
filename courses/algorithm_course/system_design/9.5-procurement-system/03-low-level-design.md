...
TABLE catalog_item:
    id                  UUID PRIMARY KEY
    tenant_id           UUID NOT NULL
    catalog_type        ENUM(INTERNAL, PUNCHOUT)
    ...
    commodity_code      VARCHAR(20)
    unit_of_measure     VARCHAR(10)
    unit_price          DECIMAL(18,4) NULLABLE (punchout items have dynamic pricing)
    currency            VARCHAR(3)
    vendor_id           UUID FOREIGN KEY -> vendor
    contract_id         UUID NULLABLE
    ...
    search_keywords     TEXT (denormalized for search indexing)
    created_at          TIMESTAMP
    updated_at          TIMESTAMP
    ...
    INDEX: (tenant_id, commodity_code)
    FULLTEXT INDEX: (description, search_keywords)
```
...
// Time Complexity (Speed of the algorithm): O(R × C) where R = rules, C = conditions per rule
// Space Complexity (Memory usage of the algorithm): O(S) where S = approval steps
```
...
// Rule that never changes: allocated = available + soft_encumbered + hard_encumbered + actual_spent
```
