...
## Reverse Auction Real-Time Architecture

```mermaid
flowchart TB
    subgraph Bidders["Bidder Clients"]
        B1[Bidder 1 - WebSocket]
        B2[Bidder 2 - WebSocket]
        BN[Bidder N - WebSocket]
    end

    subgraph AuctionCluster["Auction Engine Cluster"]
        LB[WebSocket Load Balancer - Session Affinity]
        AE1[Auction Engine Node 1]
        AE2[Auction Engine Node 2]
        VAL[Bid Validator]
        RANK[Rank Calculator]
        TIMER[Timer Service]
    end

    subgraph Persistence["Auction State"]
        ASTATE[(Auction State DB)]
        STREAM[Event Stream]
    ...
    VAL --> RANK
    RANK --> TIMER
    TIMER -->|"Auto-extend on last-second bid"| RANK
    RANK --> STREAM
    AE1 --> ASTATE
    AE2 --> ASTATE
    ASTATE -->|"Periodic"| SNAPSHOT

    classDef client fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef service fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef data fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class B1,B2,BN client
    class LB,AE1,AE2,VAL,RANK,TIMER service
    class ASTATE,STREAM,SNAPSHOT data
```

### Auction Engine Design Decisions

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Connection protocol** | WebSocket with heartbeat every 10s | HTTP polling adds unacceptable latency for real-time bidding; WebSocket enables server-push of rank updates and timer events |
| **Bid ordering** | Server-assigned sequence numbers with wall-clock timestamp | Client-side timestamps are unreliable; server assigns monotonically increasing sequence IDs within each auction to establish definitive bid ordering |
| **Anti-sniping** | Auto-extend auction by 2 minutes if bid placed in last 60 seconds | Prevents "last-second bid" strategy that undermines fair competition; configurable per auction |
| **Rank visibility** | Bidder sees their own rank (1st, 2nd, 3rd) but not competitor prices | Reveals enough information to drive competitive bidding without exposing exact competitor pricing |
| **State recovery** | Event-sourced auction state with periodic snapshots | If auction engine crashes, replay events from last snapshot to recover exact state; no bid lost |
| **Minimum decrement** | Configurable per auction (e.g., "each bid must be at least 1% lower than current lowest") | Prevents trivial bid improvements that extend auctions without meaningful savings |
