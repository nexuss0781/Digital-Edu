...
| Control | Implementation |
|---------|---------------|
| **Real-time sanctions screening** | Every new vendor and every PO dispatch checked against OFAC SDN, EU Consolidated List, and UN sanctions lists in real time |
| **Dual-use goods detection** | Commodity codes cross-referenced against Export Administration Regulations (EAR) control lists; flagged items require export license verification |
| **Country risk scoring** | Vendor incorporation country and ship-to destinations scored against risk tiers; high-risk jurisdictions trigger enhanced due diligence |
| **Beneficial ownership verification** | For high-risk vendors, ultimate beneficial ownership verified against PEP databases and adverse media screening |
| **Periodic re-screening** | All active vendors re-screened quarterly against updated sanctions lists; newly sanctioned vendors automatically suspended |
...
## Zero Trust Architecture for Procurement

Procurement platforms handle sensitive financial data across multiple trust boundaries (internal users, external vendors, system integrations). Zero trust principles apply at every layer:

| Principle | Implementation |
|-----------|---------------|
| **Never trust, always verify** | Every API call validated against ABAC policy---even internal service-to-service calls carry tenant context and are authorized |
| **Least privilege access** | Users see only documents in their cost center hierarchy; vendors see only their own POs/invoices; AP clerks see only invoices assigned to them |
| **Micro-segmentation** | Services communicate through API gateway; no direct service-to-service calls without mutual TLS and service identity verification |
| **Continuous verification** | Session tokens re-validated every 30 minutes; step-up authentication for sensitive operations (approve > $500K, change vendor bank details) |
| **Assume breach** | Audit trail with tamper-evident hash chain; budget reconciliation detects unauthorized modifications; anomaly detection flags unusual access patterns |

### Supply Chain Attack Prevention

| Vector | Mitigation |
|--------|------------|
| **Compromised vendor portal credentials** | Email OTP with session binding; IP geolocation anomaly detection; rate-limited invoice submission |
| **Malicious invoice attachments** | Sandboxed document processing; attachment scanning before storage; no executable formats accepted |
| **cXML injection** | Schema validation against cXML DTD before processing; input sanitization for all vendor-supplied fields |
| **Punch-out session hijacking** | Signed session tokens with HMAC; time-limited punch-out sessions (30 min max); cart data validated against catalog pricing |
| **Vendor impersonation for bank detail change** | Dual-channel verification (email + phone to registered contacts); 48-hour cooling-off period; second-approver requirement |
