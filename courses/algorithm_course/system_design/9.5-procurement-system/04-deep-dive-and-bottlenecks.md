...
| **Lock contention on hot cost centers** | Multiple concurrent requisitions against same cost center create serialization Slowest part of the process | Pre-allocated budget slices for high-volume cost centers; shard budget counters across N partitions with periodic reconciliation |
...
### Slowest part of the process 1: Approval Queue Latency at Quarter-End
...
- **Approval prioritization**: Sort by business impact (amount × urgency × days-pending) rather than FIFO (First-In-First-Out, like a line at a store). Surface the most critical approvals first.
...
### Slowest part of the process 2: Three-Way Matching Throughput
...
### Slowest part of the process 3: Month-End GL Posting Backlog

**Problem**: When invoices are matched, journal entries must be posted to the General Ledger system. Month-end processing creates a concentrated batch of GL postings (100K+ entries in the last 2 business days) that overwhelms the ERP integration API.

**Mitigation**:
- **Micro-batching**: Instead of posting individual journal entries, aggregate matched invoices into batches of 100 entries and post as a single GL document. Reduces API calls by 100x.
- **Priority queuing**: Prioritize GL postings by amount (large invoices first) and by vendor payment terms (invoices approaching early-payment-discount deadlines first).
- **Asynchronous confirmation**: Post entries to GL asynchronously and reconcile confirmation status in batch. Matching engine does not wait for GL confirmation before marking invoice as matched.
- **Pre-computed journal templates**: For recurring vendors with standard commodity categories, pre-compute the GL account mapping and tax treatment. Eliminates per-invoice account determination logic.

### Slowest part of the process 4: Catalog Search Performance
