...
Evidence Metadata (time-series optimized):
  ...
  Automatic partition Cutting off unnecessary steps for time-range queries
...
Multi-tenant compliance platforms must prevent one organization's evidence collection surge (e.g., during an audit preparation) from degrading service for other organizations:

```
Tenant Resource Controls:
  Evidence Collection:
    - Per-tenant sync job concurrency limit: Starter=5, Business=15, Enterprise=50
    - Per-tenant evidence ingestion rate limit: 1,000 artifacts/minute (burst: 5,000)
    - Large tenant detection: if evidence volume >10x median, auto-throttle to background priority

  Scoring Engine:
    - Per-tenant scoring queue: separate priority queues prevent one tenant's batch sync
      from blocking another tenant's real-time drift detection
    - Score computation timeout: 30 seconds per org; fail-open with cached score

  API Layer:
    - Per-tenant request rate limiting (see SLA tiers)
    - Request prioritization: audit-active tenants get elevated priority
    - Connection pool isolation: Enterprise/Regulated tenants get dedicated connection pools

  AI/ML Inference:
    - Per-tenant inference quota: 500 requests/hour (Starter), 5,000/hour (Enterprise)
    - Batch inference queuing for non-interactive workloads (gap analysis, policy generation)
```

### Evidence Storage Capacity Management

```
Per-Tenant Storage Governance:
  Monitoring:
    - Track per-tenant storage growth rate
    - Alert when tenant approaches tier storage limit (80% threshold)
    - Forecast storage exhaustion date based on 30-day trend

  Enforcement:
    - Starter: 100 GB evidence storage cap
    - Business: 1 TB evidence storage cap
    - Enterprise: 10 TB (soft limit; auto-expand with billing notification)
    - Regulated: Unlimited (dedicated storage partition)

  Optimization:
    - Automatic evidence deduplication (identical configs across syncs)
    - Tiered storage lifecycle (hot → warm → cold based on evidence age)
    - Compression of cold-tier evidence artifacts (lossless, ~60% reduction)
```
