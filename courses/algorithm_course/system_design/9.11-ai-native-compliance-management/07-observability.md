...
The AI/ML models (framework interpreter, gap analyzer, anomaly detector, remediation advisor) require dedicated observability beyond standard application metrics:

| Model | Primary Metric | Secondary Metrics | Alert Threshold |
|-------|---------------|-------------------|-----------------|
| Framework Interpreter | Mapping accuracy (human-validated) | Confidence distribution, false positive rate | Accuracy <85% on weekly validation |
| Gap Analyzer | Gap detection precision | Recall on known gaps, false gap rate | Precision <80% or recall <90% |
| Anomaly Detector | True positive rate | False alarm rate, detection latency | False positive rate >15% |
| Remediation Advisor | Remediation success rate | User acceptance rate, time-to-remediate delta | Acceptance rate <60% |
...
Model Drift Monitoring:
  Input Distribution:
    - Compare current evidence feature distributions against training baseline
    - KL divergence threshold: 0.2 (alert when exceeded)
    - Trigger: new integration types, framework updates, organizational structure changes

  Output Distribution:
    - Track confidence score histograms weekly
    - Alert when mean confidence drops >10% from baseline
    - Alert when UNKNOWN/low-confidence outputs exceed 20% of predictions

  Feedback Loop:
    - Track human override rate per model
    - Human overrides >30% suggest model degradation
    - Overrides feed back as training data for next model version
```

### Evidence Pipeline Health Dashboard

```
Pipeline Health Metrics:
  ├── Evidence Collection
  │   ├── Active integrations: count by type and status
  │   ├── Collection success rate: rolling 24-hour window per integration type
  │   ├── Evidence freshness: age of newest evidence per control per org
  │   ├── Stale evidence alerts: controls where evidence exceeds freshness SLA
  │   └── Integration error breakdown: auth failures, rate limits, schema changes, timeouts
  │
  ├── Evidence Processing
  │   ├── Processing throughput: artifacts/second through each pipeline stage
  │   ├── Processing latency: P50/P95/P99 per stage (normalize, dedup, classify, seal)
  │   ├── Deduplication rate: percentage of no-change heartbeats vs. new evidence
  │   ├── Classification confidence: distribution of ML classifier confidence scores
  │   └── Integrity seal rate: percentage of artifacts successfully sealed within SLA
  │
  ├── Scoring Pipeline
  │   ├── Event-to-score latency: end-to-end from evidence event to score cache update
  │   ├── Debounce effectiveness: ratio of events received to scoring passes triggered
  │   ├── Score stability: standard deviation of score changes per org per hour
  │   ├── Drift detection rate: PASSING→FAILING transitions per hour
  │   └── Framework coverage: percentage of requirements with linked passing controls
  │
  └── Remediation Pipeline
      ├── Open remediations by criticality and age
      ├── SLA compliance: percentage resolved within SLA by control priority
      ├── Ticket sync health: bidirectional sync success rate with external ticketing
      └── Re-evaluation success: percentage of remediated controls that pass on re-check
```

### Compliance Observability for Auditors

The platform provides auditor-facing observability to demonstrate the system's own operational integrity:

```
Auditor-Visible Metrics:
  - Platform uptime history (monthly, rolling 12 months)
  - Evidence collection continuity: gaps in collection schedule per integration
  - Timestamp authority availability: TSA response times and failover events
  - Evidence integrity verification results: weekly hash verification pass rate
  - Audit trail completeness: API write count vs. audit event count reconciliation
  - Encryption key rotation history: last rotation date per tenant

Purpose: These metrics are themselves compliance evidence for the platform's own SOC 2
audit, demonstrating operational rigor to customers and their auditors.
```
