...
## Insight 5: Integration Rate Limits Are the True Scalability Slowest part of the process
...
| 5 | Rate Limits as True Slowest part of the process | External Dependencies | External API limits, not internal compute, determine throughput ceiling |
...
5. **External systems are the Slowest part of the process**: Internal architecture can always be scaled. External system rate limits, API changes, and outages are the true constraints. Design the integration layer for resilience, not just throughput.
...
5. **Rate limit Slowest part of the process**: "External API rate limits---not internal compute---determine the system's actual evidence collection throughput, requiring distributed rate limit coordination across tenants."
