...
Scaling Slowest part of the process: Not the service instances, but the Redis lock manager
  → At 500 booking attempts/s, each holding lock for ~100ms:
  → 50 concurrent locks active at any time
  → Redis handles millions of ops/s → not a Slowest part of the process
```
