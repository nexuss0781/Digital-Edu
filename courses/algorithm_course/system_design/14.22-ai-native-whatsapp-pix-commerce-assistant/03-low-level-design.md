...
MEDClaimRecord {
    claim_id:           UUID
    transaction_id:     UUID                    // Original PIX transaction
    end_to_end_id:      String                  // PIX endToEndId
    claim_type:         ENUM [FRAUD, OPERATIONAL_ERROR]
    status:             ENUM [INITIATED, SENT_TO_RECEIVER_PSP, FUNDS_BLOCKED,
                              ANALYSIS_IN_PROGRESS, RETURN_APPROVED,
                              RETURN_DENIED, FUNDS_RETURNED, CLOSED]

    // Claim details
    claimant_user_id:   UUID
    claimant_amount:    Decimal(18,2)           // Amount being claimed
    claim_reason:       String                  // User-provided reason
    claim_evidence:     List<String>            // Conversation transcript excerpts

    // Timeline (BCB mandated)
    claim_initiated_at: Timestamp
    sent_to_receiver_at: Timestamp (nullable)   // Must be within 80 minutes
    funds_blocked_at:   Timestamp (nullable)
    analysis_deadline:  Timestamp               // 7 days from initiation
    resolution_at:      Timestamp (nullable)

    // Resolution
    amount_returned:    Decimal(18,2) (nullable)// May be partial
    return_end_to_end_id: String (nullable)     // New PIX transfer for return
...
### Open Finance Consent Record

```
OpenFinanceConsent {
    consent_id:         UUID
    user_id:            UUID
    institution_id:     String                  // ISPB of the data-sharing institution
    institution_name:   String

    // Scope
    data_types:         List<ENUM [BALANCE, TRANSACTIONS, CREDIT, INVESTMENTS]>
    permissions:        List<String>            // Specific Open Finance permission codes
    shared_since:       Timestamp
    ...
    // Data access
    last_data_fetch_at: Timestamp (nullable)
    encryption_key_id:  String                  // Per-institution encryption key for field-level encryption
...
### Pix Automático Subscription

```
PixAutomaticoSubscription {
    subscription_id:    UUID
    user_id:            UUID
    merchant_id:        String                  // Merchant CNPJ or PIX key

    // Subscription details
    description:        String                  // "Netflix", "Gym membership"
    amount:             Decimal(18,2)
    currency:           String                  // BRL
    frequency:          ENUM [MONTHLY, WEEKLY, BIWEEKLY, ANNUAL]
    day_of_period:      Integer                 // Day of month (1-28) or day of week (1-7)

    // Status
    status:             ENUM [PENDING_AUTHORIZATION, ACTIVE, PAUSED, CANCELLED, FAILED]
    next_payment_date:  Date
    last_payment_date:  Date (nullable)
    last_payment_status: ENUM [SUCCESS, FAILED, PENDING]

    // Authorization
    authorized_at:      Timestamp (nullable)
    authorized_via:     ENUM [WHATSAPP_CONVERSATION, BANKING_APP, MERCHANT_REDIRECT]
    max_amount:         Decimal(18,2)           // User-set ceiling

    // Conversation context
    originating_conversation_id: UUID           // WhatsApp conversation that set up the subscription
    last_notification_conversation_id: UUID (nullable)
