...
### Slowest part of the process 1: LLM Inference Latency
...
**Impact:** If LLM inference becomes the Slowest part of the process, messages queue up, response times degrade beyond the 1.5s SLA, and users experience the conversational assistant as "slow"—destroying the chat-like UX.

**Mitigation Strategies:**
1. **Tiered extraction**: Simple patterns ("R$50 para Maria") handled by a lightweight regex+Practical rule of thumb engine (10ms); only ambiguous or complex messages escalated to the full LLM
...
4. **Speculative execution**: While the LLM runs, optimistically resolve recipient and check fraud signals in parallel; if the LLM extraction matches the Practical rule of thumb pre-guess, the downstream work is already done

### Slowest part of the process 2: WhatsApp Cloud API Rate Limits
...
### Slowest part of the process 3: Voice Message Processing Throughput
...
| Slowest part of the process | Severity | Resolution Cost | Priority |
|---|---|---|---|
...
| DICT cache invalidation | Low | Risk-aware TTL + pre-settlement re-validation | P3 |
| Template approval latency | Low | 7-day advance submission; version fallback | P3 |

---

## Critical Component 4: Pix Automático Integration (Recurring Payments)

### Why This Is Critical

Pix Automático (launched October 2024) enables automatic recurring PIX payments—subscriptions, utility bills, loan installments—via pre-authorized direct debits. For the WhatsApp assistant, this creates a new conversation flow type: setting up recurring payments through natural language ("cobra Netflix todo dia 5 do mês") that persists beyond the 24-hour conversation window.
...
```
User: "Quero pagar a Netflix automaticamente todo mês"
System: Extract intent → RECURRING_PAYMENT_SETUP
        Extract: merchant="Netflix", frequency=MONTHLY

System: "Para configurar pagamento automático da Netflix:
         - Valor: R$39,90/mês
         - Dia do pagamento: 5 de cada mês
         - Valor máximo autorizado: R$50,00
         [Autorizar] [Cancelar]"

User: [Autorizar]
System: → Auth handoff to banking app
        → User authorizes recurring mandate
        → Mandate registered with BCB via PSP
        → "Pagamento automático Netflix ativado!
            Próximo pagamento: 05/04/2026"
```

**Execution Flow (Monthly):**

```
Day 5 of each month:
  1. PSP receives debit instruction from merchant
  2. System validates: mandate active? amount ≤ max_amount? balance sufficient?
  3. Execute PIX debit automatically
  4. Send WhatsApp notification via template message:
     "Netflix R$39,90 debitado automaticamente via PIX"
  5. If insufficient balance: send alert + retry in 4 hours
```
...
|---|---|---|
| **Beyond 24-hour window** | Subscription notifications happen monthly; require template messages approved by Meta | Pre-approve subscription notification templates; maintain active conversation state beyond 24h via template re-engagement |
| **User mandate management** | User may want to cancel, pause, or modify via WhatsApp | New conversation intents: CANCEL_SUBSCRIPTION, MODIFY_SUBSCRIPTION, LIST_SUBSCRIPTIONS with state machine paths |
| **Balance insufficient** | Auto-debit fails if user has insufficient funds | Retry schedule (4h, 12h, 24h); notify user with option to transfer funds from another account |
| **Amount disputes** | Merchant charges different amount than expected | Compare against user-authorized max_amount; block and notify if exceeded |

---

## Critical Component 5: Open Finance Multi-Bank Aggregation

### Why This Is Critical

BCB's Open Finance Phase 4 (2025-2026) enables consent-based data sharing across institutions. The WhatsApp assistant can access balances and transaction history from multiple banks, transforming from a single-bank payment tool into a multi-bank financial hub.

### How It Works Internally

```
User: "Qual meu saldo?" (What's my balance?)

System flow:
  1. Check user's active Open Finance consents
  2. For each consented institution, fetch balance via Open Finance API
...
User: "Paga R$500 pro João" (Pay R$500 to João)

System: "De qual conta?
         [Nubank R$1.234] [Itaú R$5.678] [Bradesco R$890]"
```
...
| Challenge | Why It's Hard | Solution |
|---|---|---|
| **Per-institution latency variance** | Different banks respond in 200ms to 3s; aggregated query is gated on slowest bank | Parallel fan-out with per-institution timeout (2s); show available balances immediately, late responders populate asynchronously |
| **Consent revocation cascade** | User revokes one bank's consent mid-conversation; cached data must become inaccessible immediately | Per-institution encryption keys; delete key on revocation; cached data becomes cryptographically inaccessible without log modification |
| **Account selection for payment** | User has 3+ accounts; must choose source account before PIX settlement | Intelligent default (highest balance above payment amount) with user override; remember preference per recipient |
| **Stale balance display** | Balance changes between Open Finance query and payment execution; user sees R$1,234 but actual balance is R$1,100 | Display balance with "as of" timestamp; re-verify at settlement time; handle insufficient funds gracefully with alternative account suggestion |

---

## Critical Component 6: DICT Cache Architecture

### Why This Is Critical

The DICT (Diretório de Identificadores de Contas Transacionais) is BCB's centralized PIX key directory that maps PIX keys to bank accounts. Every payment requires a DICT lookup to resolve the recipient's PIX key to their institution and account number. At 3M daily payments + disambiguation queries, the system performs 5M+ DICT lookups per day. Direct DICT queries take 30-50ms via the RSFN financial network, which is acceptable for individual transactions but creates latency pressure at scale and introduces a single-point-of-failure dependency on BCB infrastructure.

### How It Works Internally

```
DICT Cache Architecture:

Local Cache (per-node, in-process):
├── Size: ~2GB per node (top 10M most-queried PIX keys)
├── TTL: 5 minutes
├── Hit rate: ~60% (frequent recipients resolved locally)
└── Eviction: LFU (least frequently used)

Distributed Cache (Redis cluster):
├── Size: ~50GB (full active directory, ~160M registered keys)
├── TTL: 15 minutes (BCB-recommended maximum staleness)
├── Hit rate: ~95% (combined with local cache: 98%)
├── Refresh: background sync every 5 minutes for high-frequency keys
└── Partition: by PIX key hash (even distribution across 16 Redis shards)

DICT Direct Query (fallback):
├── Latency: 30-50ms via RSFN
├── Used for: cache misses, real-time validation before settlement
├── Rate limit: BCB imposes 1,000 queries/second per participant
└── Circuit breaker: open after 5 failures in 10 seconds
```

**Cache Consistency Protocol:**

The DICT cache must balance freshness against availability. A stale cache entry has two failure modes:

| Failure Mode | Impact | Likelihood | Mitigation |
|---|---|---|---|
| **PIX key reassigned** | Payment goes to wrong recipient | Very low (key reassignment requires 7-day cooling period) | Pre-settlement DICT validation; 15-minute cache TTL well within cooling period |
| **PIX key deactivated** | Payment fails at SPI with "key not found" | Low | SPI returns clear error; retry with fresh DICT query; inform user |
| **Account frozen (MED)** | Payment succeeds but funds may be clawed back | Low | DICT metadata includes account flags; check during fraud scoring |
| **New key not yet in cache** | Recipient "not found" despite having a valid PIX key | Medium (for very new keys) | Cache miss triggers direct DICT query; populate cache on success |

### Privacy Architecture for Multi-Bank Data

Each institution's data is encrypted with a per-institution key. Revoking consent for one institution makes that institution's data unreadable without modifying conversation logs:
...
Conversation log entry:
{
  "message": "Seus saldos:",
  "data": {
    "nubank": { "balance": "ENC[key_nubank]{1234.56}", "institution": "Nubank" },
    "itau":   { "balance": "ENC[key_itau]{5678.90}", "institution": "Itaú" },
    "bradesco": { "balance": "ENC[key_bradesco]{890.12}", "institution": "Bradesco" }
  }
}

When user revokes Itaú consent:
  → Delete key_itau from key store
  → Itaú balance data becomes unreadable in all conversation logs
  → No log modification needed (preserves immutability)
```
