...
## MoE-Specific Security Considerations

```
EXPERT PARALLELISM SECURITY (2025-2026)
────────────────────────────────────────

ALL-TO-ALL COMMUNICATION RISKS:
- Token hidden states are sent between GPUs based on routing
- Cross-node all-to-all exposes intermediate representations on the network
- Routing decisions reveal which experts are activated (information leakage)

MITIGATIONS:

1. ENCRYPTED ALL-TO-ALL:
   - TLS or IPsec for cross-node expert communication
   - Hardware-level encryption via NVLink for intra-node
   - Performance impact: 5-10% on cross-node transfers

2. ROUTING PRIVACY:
   - Expert routing patterns can reveal input characteristics
   - Don't expose expert utilization metrics to tenants
   - Aggregate routing statistics for monitoring only

3. EXPERT ISOLATION IN MULTI-TENANT:
   - Shared experts see tokens from all tenants
   - Cannot isolate at expert level (routing is dynamic)
   - Solution: Instance-level tenant isolation for sensitive workloads

DISAGGREGATED SERVING SECURITY:
- KV cache transfers between prefill and decode workers
- Ensure KV data encrypted in transit (RDMA with encryption)
- Verify source identity of KV transfers (prevent cache poisoning)
- Clear KV on decode worker after request completion
```
...
| **MoE** | Encrypted all-to-all communication | Required (cross-node) |
| **MoE** | Routing pattern privacy | Recommended |
| **Disaggregated** | Encrypted KV cache transfers | Required (cross-node) |
| **Disaggregated** | KV transfer source verification | Recommended |
