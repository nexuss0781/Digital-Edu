...
### Blackwell Generation Scaling (B200/GB200)

```
BLACKWELL INFERENCE IMPROVEMENTS (2025-2026)
────────────────────────────────────────────

HARDWARE LEAP:
                    H100 SXM    →    B200        →    GB200 (2×B200)
Memory:             80 GB HBM3       192 GB HBM3e     384 GB HBM3e
Bandwidth:          3.35 TB/s        8 TB/s            16 TB/s
FP8 TFLOPS:         1,979            4,500             9,000
FP4 TFLOPS:         N/A              9,000             18,000
NVLink:             900 GB/s         1.8 TB/s          1.8 TB/s

INFERENCE IMPACT:
─────────────────
Decode throughput:  2.4x from bandwidth alone (8 / 3.35)
KV cache capacity:  2.4x (192 / 80 GB)
Concurrent requests: ~60 (vs ~25) for 70B INT8 at 1K context

NEW CAPABILITIES:
FP4 (NVFP4) quantization:
- 70B model in 17.5 GB (vs 70 GB INT8)
- Leaves 174 GB for KV cache (22x more than H100)
- Quality impact: <1% on most benchmarks with calibration
- Enables 405B models on single GPU (INT4)

DEPLOYMENT IMPLICATIONS:
- Models that required TP=2 on H100 fit on single B200
- 405B in FP8 fits on 2 B200 GPUs (vs 8 H100)
- Decode throughput scales proportionally with bandwidth
- Cost per token drops ~3x at same price point
```
...
| Flash Attention 3 | Hopper/Blackwell optimization | H100/H200/B200 |
| FlashInfer | Variable sequence lengths, MoE | CUDA 12+ |
