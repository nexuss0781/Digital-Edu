...
ALGORITHM allocate_blocks(sequence, num_tokens):
    ...
    Time Complexity (Speed of the algorithm): O(num_blocks)
    Space Complexity (Memory usage of the algorithm): O(1) additional
    """
...
ALGORITHM scheduler_step():
    ...
    Time Complexity (Speed of the algorithm): O(running + waiting)
    """
...
ALGORITHM prefix_cache_lookup(token_ids):
    ...
    Time Complexity (Speed of the algorithm): O(num_tokens)
...
ALGORITHM speculative_decode(context, k=4):
    ...
    Time Complexity (Speed of the algorithm): O(k) draft forwards + O(1) target forward
...
ALGORITHM chunked_prefill_schedule(max_tokens=8192):
    ...
    Time Complexity (Speed of the algorithm): O(running_seqs)
    """
...
ALGORITHM copy_on_write(seq, block_idx):
    ...
    Time Complexity (Speed of the algorithm): O(BLOCK_SIZE) for copy
    """
...
ALGORITHM select_preemption_victim():
    ...
    PROCEDURE:
        ...
        SWITCH preemption_policy:
            CASE FCFS:
                # Preempt most recently admitted (LIFO (Last-In-First-Out, like a stack of plates))
...
### Algorithm 9: MoE Expert Routing and Token Dispatch

```
ALGORITHM moe_expert_dispatch(hidden_states, top_k=2):
    """
    Route tokens to experts in a Mixture-of-Experts layer.
    Handles expert parallelism across GPUs.

    Time Complexity (Speed of the algorithm): O(batch_size × num_experts) for routing
    Communication: All-to-All for token dispatch
    """

    INPUT:
        hidden_states: Tensor[batch_size, hidden_dim]
        top_k: int  # Experts per token (typically 2)
    ...
    PROCEDURE:
        ...
        gate_logits = gate_network(hidden_states)  # [batch, num_experts]
        ...
        top_k_probs = top_k_probs / SUM(top_k_probs, dim=-1)  # Renormalize

        # STEP 3: Build dispatch plan (which tokens → which GPU)
        dispatch_plan = {}  # gpu_id → list of (token_idx, expert_local_id, weight)
        FOR token_idx IN RANGE(batch_size):
            FOR k IN RANGE(top_k):
                expert_id = top_k_indices[token_idx, k]
                target_gpu = expert_placement[expert_id]  # GPU hosting this expert
                local_expert_id = expert_id % experts_per_gpu
                weight = top_k_probs[token_idx, k]
                dispatch_plan[target_gpu].APPEND((token_idx, local_expert_id, weight))

        # STEP 4: All-to-All communication (send tokens to expert GPUs)
        received_tokens = ALL_TO_ALL(hidden_states, dispatch_plan)

        # STEP 5: Execute local experts
        local_outputs = {}
        FOR (token, expert_id, weight) IN received_tokens[MY_GPU]:
            expert_output = local_experts[expert_id].forward(token)
            local_outputs[token] = expert_output * weight

        # STEP 6: All-to-All communication (return results)
        combined = ALL_TO_ALL_REVERSE(local_outputs)

        # STEP 7: Combine weighted expert outputs
        output = ZEROS(batch_size, hidden_dim)
        FOR token_idx, expert_results IN combined:
            output[token_idx] = SUM(expert_results)

        RETURN output
```

### Algorithm 10: Prefix-Aware Load Balancing

```
ALGORITHM prefix_aware_route(request, instances):
    """
    Route request to instance maximizing prefix cache hit probability.
    Uses consistent hashing on prefix tokens.

    Time Complexity (Speed of the algorithm): O(log(instances)) with consistent hash ring
    """
    ...
    PROCEDURE:
        # STEP 1: Extract prefix signature
        prefix_tokens = request.prompt_tokens[:PREFIX_HASH_LENGTH]
        prefix_hash = HASH(prefix_tokens)

        # STEP 2: Look up preferred instance via consistent hash
        preferred = consistent_hash_ring.get_node(prefix_hash)

        # STEP 3: Check if preferred instance is healthy and not overloaded
        IF preferred.is_healthy AND preferred.load < OVERLOAD_THRESHOLD:
            RETURN preferred

        # STEP 4: Fallback to load-based routing
        # Check next K nodes on hash ring (they may also have this prefix)
        FOR candidate IN consistent_hash_ring.get_next_k(prefix_hash, k=3):
            IF candidate.is_healthy AND candidate.load < OVERLOAD_THRESHOLD:
                RETURN candidate

        # STEP 5: Final fallback — least outstanding requests
        RETURN MIN(healthy_instances, key=lambda i: i.outstanding_requests)
```
