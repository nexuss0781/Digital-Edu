...
```
┌─────────────────────────────────────────────────────────────────────┐
│                PAGEDATTENTION KERNEL Step-by-step plan in plain English                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
...
### Edge Case (Unusual or extreme situation) 1: High Temperature Sampling
...
### Edge Case (Unusual or extreme situation) 2: Variable Acceptance in Batched Speculative Decoding
...
### Edge Case (Unusual or extreme situation) 3: Draft Model Quality Degradation
...
### Slowest part of the process 1: Block Allocation Contention
...
### Slowest part of the process 2: Prefix Cache Eviction Storms
...
### Slowest part of the process 3: CUDA Launch Overhead
...
### Slowest part of the process 4: Memory Fragmentation
...
### Slowest part of the process 5: MoE Expert Load Imbalance

**Problem:** Mixture-of-Experts models route tokens unevenly, creating GPU hotspots.

```
SYMPTOM:
- Some GPUs at 95% utilization, others at 20%
- Tail latency spikes from overloaded expert GPUs
- Throughput limited by slowest GPU in the pipeline

CAUSE:
- Router assigns tokens to "popular" experts disproportionately
- Expert parallelism places complete experts on specific GPUs
- Skewed routing creates stragglers that block the batch

SOLUTION: Expert Replication + Dynamic Routing

    DESIGN:
    ───────
    Phase 1: Profile expert utilization during warmup
    Phase 2: Replicate top-K popular experts across multiple GPUs
    Phase 3: Route to least-loaded replica at runtime

    ALGORITHM:
    def route_token_to_expert(token_hidden, expert_id):
        # Get all GPUs hosting this expert
        replicas = expert_placement[expert_id]  # e.g., [GPU2, GPU5]

        # Pick least-loaded replica
        selected_gpu = min(replicas, key=lambda g: g.pending_tokens)

        # All-to-all: send token to selected GPU
        send_token(token_hidden, selected_gpu)

    def rebalance_experts(utilization_stats, interval=60s):
        # Find overloaded experts (>2x average utilization)
        hot_experts = [e for e in experts
                       if utilization_stats[e] > 2 * avg_utilization]

        # Find underloaded GPUs with capacity
        cold_gpus = [g for g in gpus
                     if g.memory_free > expert_size]

        # Replicate hot experts to cold GPUs
        for expert in hot_experts:
            if cold_gpus:
                target = cold_gpus.pop()
                replicate_expert(expert, target)

RESULTS:
- Load imbalance reduced from 4x to 1.3x
- Tail latency reduced 60%
- ~5% memory overhead for replicated experts
```

---

## Deep Dive 4: KV Cache Compression Techniques (2025-2026)
...
│  ────────────────────────────────────────                           │
│  Per token: K[num_heads, head_dim] + V[num_heads, head_dim]         │
│  Example (70B): 64 heads × 128 dim × 2 (K+V) × 2 bytes = 32 KB    │
│  × 80 layers = 2.56 MB per token (without GQA)                     │
│                                                                     │
│  GROUPED QUERY ATTENTION (GQA) COMPRESSION:                         │
│  ───────────────────────────────────────────                        │
│  Share KV heads across query head groups                            │
│  Llama-70B: 8 KV heads instead of 64 → 8x compression              │
│  Per token: 320 KB (with GQA) vs 2.56 MB (without)                 │
│                                                                     │
│  MULTI-HEAD LATENT ATTENTION (MLA) COMPRESSION:                     │
│  ──────────────────────────────────────────────                     │
│  Project KV into low-rank latent vector during training             │
│  Store only latent vector c_t (small) instead of K, V (large)      │
│  Reconstruct K, V on-the-fly using learned up-projections           │
│                                                                     │
│  Standard:  token → K_proj → K cache (large)                       │
│             token → V_proj → V cache (large)                       │
│                                                                     │
│  MLA:       token → Down_proj → c_t (tiny latent)                  │
│             c_t → K_up_proj → K (computed on-the-fly)               │
│             c_t → V_up_proj → V (computed on-the-fly)               │
│                                                                     │
│  DeepSeek-V3 (671B MoE):                                           │
│  Latent dim: 512 (vs 128 × 128 = 16,384 for standard)              │
│  Compression: ~32x per head group                                  │
│  Per-token KV: ~60-100 KB vs ~1.2 MB standard                      │
│                                                                     │
│  TRADE-OFF:                                                         │
│  ──────────                                                         │
│  Extra compute: Up-projection during every attention computation    │
│  But: Decode is memory-bound by 300x, so trading compute for        │
│       memory is overwhelmingly positive                             │
│  Net effect: More concurrent requests, lower memory pressure,       │
│              slightly higher compute per attention layer             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### Runtime KV Cache Compression (Heavy-Hitter Oracle)

```
┌─────────────────────────────────────────────────────────────────────┐
│           RUNTIME KV CACHE EVICTION (H2O / SnapKV)                  │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  OBSERVATION:                                                       │
│  ────────────                                                       │
│  Not all KV entries contribute equally to attention output.          │
│  Attention scores follow a heavy-tailed distribution:               │
│  - ~5% of tokens receive >80% of attention weight                   │
│  - Most tokens contribute negligibly to the output                  │
│                                                                     │
│  H2O (Heavy-Hitter Oracle):                                         │
│  ──────────────────────────                                         │
│  Keep only "heavy hitter" tokens + recent window in KV cache        │
│  Evict low-attention tokens to free memory                          │
│                                                                     │
│  ALGORITHM:                                                         │
│  def h2o_evict(attention_scores, kv_cache, budget):                 │
│      # Accumulate attention scores across recent steps              │
│      cumulative_scores = attention_scores.sum(dim=0)                │
│                                                                     │
│      # Keep top-K heavy hitters + last W recent tokens              │
│      heavy_hitters = topk(cumulative_scores, k=budget - window)     │
│      recent = range(seq_len - window, seq_len)                      │
│      keep = union(heavy_hitters, recent)                            │
│                                                                     │
│      # Evict the rest                                               │
│      evict = set(range(seq_len)) - keep                             │
│      for pos in evict:                                              │
│          kv_cache.mark_evictable(pos)                               │
│                                                                     │
│  SnapKV (2024-2025):                                                │
│  ────────────────────                                               │
│  Observe attention patterns during prefill to predict which          │
│  KV entries will matter during decode                               │
│  Compress KV cache before decode starts                             │
│  Achieves 3-6x KV compression with <1% quality loss                │
│                                                                     │
│  COMPARISON:                                                        │
│  ───────────                                                        │
│  | Technique | Compression | Quality Loss | When Applied |          │
│  |-----------|-------------|--------------|--------------|          │
│  | GQA       | 4-8x        | 0% (trained) | Architecture |          │
│  | MLA       | 10-20x      | 0% (trained) | Architecture |          │
│  | FP8 KV    | 2x          | <0.1%        | Runtime      |          │
│  | H2O       | 3-5x        | <1%          | Runtime      |          │
│  | SnapKV    | 3-6x        | <1%          | Runtime      |          │
│  | Combined  | 20-60x      | 1-2%         | Both         |          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Slowest part of the process | Impact | Solution | Improvement |
|------------|--------|----------|-------------|
...
| MoE expert load imbalance | GPU stragglers, tail latency | Expert replication + dynamic routing | 60% tail latency reduction |
