...
|----|--------|--------|---------------|
| **NFR-1** | Block Allocation Latency | < 1 μs per block | Must not Slowest part of the process iteration (~10ms) |
...
### Example 4: MoE Model Serving (DeepSeek-V3 Class, 671B)

```
Model Characteristics:
- Total parameters: 671B
- Active parameters per token: 37B (top-2 of 256 experts)
- Architecture: MLA (Multi-Head Latent Attention) + MoE
- KV cache per token: ~60-100 KB (MLA compressed, vs ~1.2 MB standard)

Memory Planning (FP8 quantization):
- Total weights: 671B × 1 byte (FP8) = 671 GB
- Minimum GPUs for weights: 671 / 80 = 9 GPUs (H100)
                           671 / 192 = 4 GPUs (B200)
- Expert distribution: 256 experts across 8 GPUs = 32 experts/GPU

Parallelism Configuration (8× H100 node):
- Attention layers: TP=8 (tensor parallel across all GPUs)
- Expert layers: EP=8 (expert parallel, 32 experts per GPU)
- Shared MLP/embedding: TP=8

KV Cache Budget (MLA advantage):
- With MLA: 80 KB per token (vs 1.2 MB standard)
- Remaining memory per GPU: ~5 GB for KV
- Tokens per GPU: 5 GB / 80 KB = 62,500 tokens
- At 2K average context: ~31 concurrent requests per GPU
- Compare: Standard attention would allow only ~4 concurrent

Traffic Pattern:
- 500 requests per minute
- Average prompt: 2,000 tokens, output: 500 tokens

Infrastructure:
- 1 × 8-GPU H100 node (EP=8 for experts, TP=8 for attention)
- OR 1 × 4-GPU B200 node (EP=4, TP=4)
- MLA enables 8x more concurrent requests than standard attention
```

### Example 5: Disaggregated Serving at Scale

```
Traffic Pattern:
- 10,000 requests per minute (high-throughput API)
- Average prompt: 1,500 tokens, output: 200 tokens
- Model: 70B INT8

Disaggregated Architecture:
- Prefill workers: compute-optimized, handle prompt processing
- Decode workers: memory-optimized, handle token generation

Prefill Sizing:
- 1,500 tokens prefill at ~150ms per request
- Each GPU handles ~6 prefills/sec
- Need: 10,000 / 60 = 167 req/sec
- Prefill GPUs: 167 / 6 = 28 GPUs

Decode Sizing:
- 200 tokens at ~50 TPS per request = 4 sec decode time
- Each GPU handles ~25 concurrent decode sequences
- Active decode requests: 167 req/sec × 4 sec = 668 concurrent
- Decode GPUs: 668 / 25 = 27 GPUs

KV Transfer:
- Per request: 1,500 tokens × 320 KB = 480 MB
- Transfer time (RDMA): 480 MB / 50 GB/s = 9.6 ms
- Adds ~10ms to TTFT (acceptable for throughput workload)

Infrastructure:
- 4 × 8-GPU nodes for prefill (32 GPUs)
- 4 × 8-GPU nodes for decode (32 GPUs)
- RDMA fabric between nodes for KV transfer
- Throughput gain: ~40% over combined architecture
```
