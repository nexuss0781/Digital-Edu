...
[Write Step-by-step plan in plain English for key operations]
```
...
Slowest part of the process 1: Lock Contention
...
Slowest part of the process 3: Memory Fragmentation
...
### Trap 6: "How does serving a MoE model differ from a dense model?"

```
BAD ANSWER:
"MoE is the same, just bigger. Use more tensor parallelism."

GOOD ANSWER:
"MoE models are fundamentally different for inference because only a
fraction of parameters activate per token. DeepSeek-V3 has 671B total
parameters but only activates ~37B per token (top-2 of 256 experts).

This changes the serving architecture in three ways:

1. PARALLELISM STRATEGY:
   Dense models use tensor parallelism (shard every layer across GPUs).
   MoE models need expert parallelism: place complete experts on
   specific GPUs and route tokens dynamically via all-to-all
   communication. The attention layers still use TP.

2. MEMORY PROFILE:
   Total weights are large (671B) but active weights are small (37B).
   Decode is less memory-bound because you only read active expert
   weights plus the shared attention layers. But you need enough
   memory to store ALL experts across the GPU pool.

3. LOAD BALANCING:
   Some experts are 'popular' -- they receive disproportionate traffic.
   This creates GPU hotspots. Solutions include expert replication
   (place hot experts on multiple GPUs) and load-aware routing.

The communication pattern also differs: TP uses fixed AllReduce
(predictable), EP uses dynamic all-to-all (variable based on routing).
This makes MoE inference less predictable and harder to optimize."
```
...
│                                                                     │
│  2025-2026 ADDITIONS:                                               │
│  B200: 192 GB HBM3e, 8 TB/s bandwidth, FP4 native                  │
│  GB200: 384 GB, 16 TB/s, NVLink 1.8 TB/s                           │
│  MLA: 10-20x KV compression at model level (DeepSeek-V3)           │
│  Expert Parallelism: all-to-all for MoE, hybrid TP+EP               │
│  Zero-overhead scheduler: overlap CPU scheduling with GPU execution  │
│  NVIDIA Dynamo: disaggregated inference orchestration                │
│                                                                     │
│  INTERVIEW KEYWORDS:                                                │
...
│  MoE, expert parallelism, MLA, disaggregated P/D, prefix-aware      │
│  scheduling, zero-overhead scheduler, FP4/NVFP4, KV compression     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
