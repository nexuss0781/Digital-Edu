...
│      Target: p99 < 1ms                                              │
│      Alert: p99 > 5ms → scheduler Slowest part of the process                        │
│                                                                     │
...
4. CHECK I/O Slowest part of the process:
...
│  └─────────────────────────────┴─────────────────────────────┘      │
│                                                                     │
│  ROW 6: MOE & DISAGGREGATED (if applicable)                         │
│  ┌─────────────────────────────┬─────────────────────────────┐      │
│  │  Expert Utilization Heatmap │  KV Transfer Latency        │      │
│  │  [Heatmap: expert × GPU]    │  [Histogram P/D transfer]   │      │
│  └─────────────────────────────┴─────────────────────────────┘      │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
## MoE-Specific Observability

```
┌─────────────────────────────────────────────────────────────────────┐
│                    MOE INFERENCE METRICS                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  EXPERT ROUTING METRICS:                                            │
│  ───────────────────────                                            │
│  expert_utilization_ratio                                           │
│      Description: Tokens processed / tokens expected (uniform)      │
│      Type: Gauge                                                    │
│      Labels: expert_id, gpu_id                                      │
│      Alert: Any expert > 3x average → routing imbalance             │
│                                                                     │
│  expert_load_imbalance_factor                                       │
│      Description: max(utilization) / mean(utilization)              │
│      Type: Gauge                                                    │
│      Target: < 1.5x                                                 │
│      Alert: > 2.0x sustained → consider expert replication          │
│                                                                     │
│  all_to_all_latency_seconds                                         │
│      Description: Token dispatch communication time                 │
│      Type: Histogram                                                │
...
│      Description: Time to transfer KV from prefill to decode        │
│      Type: Histogram                                                │
│      Labels: transfer_type (nvlink, rdma, pcie)                     │
│      Target: < 10ms for 1K tokens                                   │
│                                                                     │
│  kv_transfer_bytes_total                                             │
│      Description: Total bytes transferred between P/D workers       │
│      Type: Counter                                                  │
│                                                                     │
│  prefill_queue_depth                                                │
│      Description: Requests waiting for prefill                      │
│      Type: Gauge                                                    │
│      Alert: > 50 sustained → scale prefill workers                  │
│                                                                     │
│  decode_queue_depth                                                 │
│      Description: Sequences waiting for decode slot                 │
│      Type: Gauge                                                    │
│      Alert: > 200 sustained → scale decode workers                  │
│                                                                     │
│  prefill_to_decode_ratio                                            │
│      Description: Active prefill workers / active decode workers    │
│      Type: Gauge                                                    │
│      Target: 1:3 to 1:5 (decode needs more instances)               │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
