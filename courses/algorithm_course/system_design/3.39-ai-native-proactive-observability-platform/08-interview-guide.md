...
### Q: "What is compound AI and why does it matter for observability?"

**Good Answer:**
> "Compound AI combines multiple AI approaches: deterministic causal AI for topology-aware dependency tracing, statistical models for anomaly detection, predictive models for SLO forecasting, and agentic LLMs for natural language investigation. Each has strengths — causal AI never hallucinates but can't handle novel patterns, while LLMs are flexible but can hallucinate. The compound approach uses causal AI as ground truth that validates LLM conclusions, giving you both flexibility and reliability."

### Q: "How would you defend against telemetry manipulation attacks?"

**Good Answer:**
> "This is an emerging threat — attackers can inject fake telemetry to trigger false remediations or mask real incidents. Defense has four layers: source authentication via mTLS, schema and rate validation at ingestion, statistical integrity checking by cross-validating against independent signals like eBPF, and AI action guardrails using Policy-as-Code. The key is complementing probabilistic LLM reasoning with deterministic causal AI that traces dependency graphs and can detect when LLM conclusions contradict the topology."

### Q: "How do you manage observability costs at scale?"

**Good Answer:**
> "Observability costs are approaching $34B globally, with traces at 60-70% of spend. We use adaptive telemetry pipelines that reduce volume by 50-80% while preserving incident response: keep 100% of errors and slow requests, intelligently sample normal traffic, filter low-value attributes, and deduplicate repeated log patterns. Hot/Warm/Cold storage tiering further reduces costs — most queries hit 7-day hot data on SSD, while cold data on object storage costs 10x less. OpAMP enables dynamic pipeline reconfiguration during incidents to temporarily increase sampling for affected services."
