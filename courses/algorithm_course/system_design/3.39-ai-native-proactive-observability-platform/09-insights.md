...
**Category:** Caching
**One-liner:** Five optimization layers (query cache, materialized views, projection tables, partition Cutting off unnecessary steps, approximate queries) ensure that dashboard queries hit pre-computed rollups while exploratory queries are cost-estimated and potentially rejected.

**Why it matters:** With billions of events and high-cardinality dimensions, unoptimized queries can scan terabytes of data, consuming more compute than the services being monitored. The five-layer optimization stack addresses different query patterns: query caching (80%+ hit rate for dashboard refreshes), materialized views (pre-aggregated 1min/5min/1hr/1day rollups for time-range queries), projection tables (alternative sort orders for different access patterns), partition Cutting off unnecessary steps (time and service-based partitioning eliminates irrelevant data), and approximate queries (HyperLogLog for cardinality, T-Digest for percentiles, sampling for exploration). The query cost estimator actively rejects or downgrades queries that exceed cost thresholds, suggesting optimizations like using rollup tables or adding service filters.
...
## Insight 15: Compound AI Architectures Combine Deterministic and Probabilistic Reasoning to Eliminate Hallucination Risk

**Category:** System Modeling
**One-liner:** Pairing causal AI (deterministic topology-graph reasoning) with agentic LLMs (flexible natural language investigation) gives observability platforms both the reliability of rule-based systems and the flexibility of AI, with causal AI serving as ground truth that validates or overrides LLM conclusions.

**Why it matters:** LLMs can hallucinate root causes — producing well-formed but incorrect explanations that look plausible. Causal AI traces service dependency graphs deterministically, identifying propagation paths without guessing. When an LLM concludes "the database is the root cause" but causal AI shows the database has no dependency path to the affected service, the system flags a contradiction. This compound approach (used by Dynatrace's Davis AI and emerging across the industry) achieves investigation accuracy above 90% because every probabilistic conclusion is cross-validated against deterministic topology analysis. The industry consensus is that neither approach alone is sufficient — causal AI cannot handle novel patterns, and LLMs cannot be trusted without ground truth validation.

---

## Insight 16: Telemetry Manipulation Attacks Represent an Emerging Threat That Requires Defense-in-Depth

**Category:** Security
**One-liner:** As AI-driven observability systems gain autonomous action capabilities, attackers can subvert them by injecting manipulated telemetry — triggering false remediations, masking real incidents, or causing cardinality explosions that overwhelm the platform.

**Why it matters:** Traditional observability treated telemetry as trusted data. With AI agents that autonomously rollback deployments, scale infrastructure, or open circuit breakers based on telemetry analysis, manipulated telemetry becomes a new attack vector. A 2025 RSA paper demonstrated subverting LLM-driven operations through telemetry injection. Defense requires four layers: source authentication (mTLS for all collectors), telemetry validation (schema validation, cardinality guards, rate limiting), statistical integrity checking (cross-validating against independent signals like eBPF), and AI action guardrails (Policy-as-Code limits on blast radius, forbidden actions, and mandatory human approval for high-impact operations). The key insight is that deterministic causal AI provides a manipulation-resistant cross-check because it reasons from topology graphs, not from potentially-compromised telemetry patterns.

---

## Insight 17: Adaptive Telemetry Pipelines Reduce Observability Costs by 50-80% While Preserving Full Incident Response Capability

**Category:** Traffic Shaping
**One-liner:** Intelligent sampling that keeps 100% of errors and slow requests while sampling normal traffic at 1-10%, combined with attribute filtering and Hot/Warm/Cold tiering, dramatically reduces the $34B global observability cost without sacrificing incident detection or investigation quality.

**Why it matters:** Traces alone account for 60-70% of observability costs. Most telemetry data represents healthy requests that are never investigated — storing it at full resolution is pure waste. Adaptive telemetry pipelines apply four techniques: rule-based sampling (keep 100% errors/slow, sample normal at dynamic rates), attribute filtering (remove debug-level fields), cardinality management (aggregate high-cardinality dimensions for non-investigation use), and log deduplication (merge repeated patterns). The critical design constraint is that during active incidents, sampling must automatically increase to 100% for affected services — OpAMP enables pushing this configuration change to collectors within seconds. Combined with Hot/Warm/Cold storage tiering (7 days SSD, 90 days HDD, 2 years object storage), platforms achieve 60-80% total cost reduction.
