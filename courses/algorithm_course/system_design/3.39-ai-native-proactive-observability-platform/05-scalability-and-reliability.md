...
│  • Slowest part of the process: Kafka throughput, collector CPU                  │
│                                                                 │
│  STORAGE SCALE                                                  │
│  ─────────────                                                  │
...
│  • Slowest part of the process: Query fanout, cross-shard joins                 │
│                                                                 │
│  QUERY SCALE                                                    │
│  ───────────                                                    │
│  • Horizontal: Add ClickHouse replicas, query cache nodes       │
│  • Target: 10K+ queries/sec                                     │
│  • Slowest part of the process: Hot partition skew, cache invalidation           │
│                                                                 │
│  AI INFERENCE SCALE                                             │
│  ──────────────────                                             │
│  • Horizontal: Add GPU nodes for parallel inference             │
│  • Target: 100K anomaly checks/sec                              │
│  • Slowest part of the process: Model loading time, GPU memory                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| **ClickHouse Cold** | 99.999999999% | Object storage with 11 nines durability |
| **PostgreSQL (Actions)** | 99.9999% | Synchronous replication, PITR |
| **Redis Cache** | N/A (cache) | Persistence disabled, rebuild on failure |

---

## OpAMP-Based Fleet Scaling

### Dynamic Collector Fleet Management

With hundreds of OTel collectors deployed across regions, OpAMP enables centralized fleet scaling without manual intervention:

```python
class CollectorFleetManager:
    """
    Manages OTel collector fleet via OpAMP protocol.
    Enables dynamic reconfiguration without restarts.
    """

    def __init__(self, opamp_server: OpAMPServer):
        self.opamp = opamp_server
        self.collectors = {}  # agent_id -> CollectorState

    async def handle_incident_scaling(self, incident: Incident):
        """
        During incidents, dynamically adjust collector behavior
        for affected services.
        """
        affected_services = incident.affected_services

        for agent_id, state in self.collectors.items():
            if state.serves_any(affected_services):
                # Increase sampling for affected services
                new_config = state.config.copy()
                new_config.sampling_rates[incident.service] = 1.0  # 100%
                new_config.batch_timeout = "100ms"  # Faster flush

                await self.opamp.push_config(agent_id, new_config)

    async def canary_rollout(self, new_version: str, canary_pct: float = 0.05):
        """
        Roll out new collector version using canary strategy.
        """
        total = len(self.collectors)
        canary_count = max(1, int(total * canary_pct))

        # Select canary collectors (prefer non-critical regions)
        canary_set = self.select_canary_collectors(canary_count)

        for agent_id in canary_set:
            await self.opamp.push_package(agent_id, new_version)

        # Monitor canary health for 30 minutes
        healthy = await self.monitor_canary_health(canary_set, duration_minutes=30)

        if healthy:
            # Proceed with staged rollout
            await self.staged_rollout(new_version, stages=[0.25, 0.5, 1.0])
        else:
            # Rollback canary
            for agent_id in canary_set:
                await self.opamp.push_package(agent_id, self.current_version)

    async def adaptive_pipeline_config(self):
        """
        Dynamically adjust collector pipelines based on system health.
        """
        system_health = await self.get_system_health()

        if system_health.kafka_lag > self.thresholds.high_lag:
            # Backpressure: reduce collection rates
        ...
        elif system_health.storage_usage > 0.85:
            # Storage pressure: increase filtering
            await self.broadcast_config_update({
                "filter_debug_logs": True,
                "attribute_allowlist": self.essential_attributes
            })
```

### Multi-Region Collector Topology

```
┌─────────────────────────────────────────────────────────────────┐
│                 GLOBAL COLLECTOR TOPOLOGY                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  US-WEST REGION              US-EAST REGION                    │
│  ─────────────────           ─────────────────                  │
│  Collectors: 150              Collectors: 120                   │
│  Throughput: 3M eps           Throughput: 2.5M eps              │
│  Version: v0.95.0             Version: v0.95.0                  │
│           │                            │                        │
│           └────────────┬───────────────┘                        │
│                        │                                        │
│                        ▼                                        │
│  EU-WEST REGION       OpAMP         AP-EAST REGION             │
│  ─────────────────    SERVER        ─────────────────           │
│  Collectors: 80       (Central)     Collectors: 60              │
│  Throughput: 1.5M eps  │            Throughput: 1M eps          │
│  Version: v0.94.0      │            Version: v0.95.0            │
│  (canary rollout)      │                                        │
│           │            │                                        │
│           └────────────┘                                        │
│                                                                 │
│  TOTAL FLEET: 410 collectors                                   │
│  TOTAL THROUGHPUT: 8M events/sec                               │
│  CONFIG PUSH LATENCY: < 5 seconds (via WebSocket)              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**Note:** Given the autonomous nature of this system, all stages require safety-focused evaluation criteria. Stage 5 deployment requires continuous monitoring with automated circuit-breakers that can pause autonomous operation. Sandbox boundaries must be validated at every stage before expanding autonomy scope.
