...
## Compound AI Architecture (2025-2026 Pattern)

Modern observability platforms use **compound AI** — multiple specialized AI models orchestrated together — rather than relying on a single model:

```mermaid
flowchart TB
    subgraph Input["Input Signals"]
        TEL["Telemetry<br/>Stream"]
        CTX["Historical<br/>Context"]
    ...
    subgraph Compound["Compound AI System"]
        ...
        subgraph Deterministic["Deterministic Layer"]
            CAUSAL["Causal AI<br/>(Topology Graph)"]
            RULES["Rule Engine<br/>(Known Patterns)"]
        end

        subgraph Probabilistic["Probabilistic Layer"]
        ...
        subgraph Agentic["Agentic Layer"]
            ...
            RAG["RAG Pipeline<br/>(Runbooks/History)"]
            TOOLS["Tool Calling<br/>(Query/Investigate)"]
        end

        subgraph Predictive["Predictive Layer"]
            FORECAST["Time Series<br/>Forecasting"]
            SLO["SLO Burn<br/>Prediction"]
        end
    end

    subgraph Output["Output"]
        RCA["Root Cause<br/>Analysis"]
        ACTION["Remediation<br/>Proposals"]
        NLQ["Natural Language<br/>Explanations"]
    end

    TEL --> CAUSAL & STAT & IF & FORECAST
    CTX --> RAG & FORECAST
    KB --> RULES & RAG
    CAUSAL --> LLM
    RULES --> LLM
    ...
    RAG --> LLM
    LLM --> TOOLS
    TOOLS --> RCA & ACTION & NLQ

    classDef input fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef deterministic fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef probabilistic fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef agentic fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef predictive fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef output fill:#fce4ec,stroke:#c2185b,stroke-width:2px

    class TEL,CTX,KB input
    class CAUSAL,RULES deterministic
    class STAT,IF probabilistic
    class LLM,RAG,TOOLS agentic
    class FORECAST,SLO predictive
    class RCA,ACTION,NLQ output
```

**Why compound AI matters:**

| AI Type | Strengths | Weaknesses | Role in Observability |
|---------|-----------|------------|----------------------|
| **Causal AI** | Deterministic, no hallucination, topology-aware | Cannot handle novel patterns | Ground truth validation |
| **Statistical** | Well-understood, fast, low resource | Cannot explain "why" | Anomaly detection |
| **LLM/Agentic** | Natural language, flexible reasoning, tool use | Hallucination risk, expensive | Investigation, explanation |
| **Predictive** | Forecast future state, SLO breach prediction | Requires clean historical data | Proactive alerting |
| **Rule-based** | Zero latency, deterministic | Cannot adapt to unknowns | Known pattern matching |

---

## OpAMP Fleet Management

The **Open Agent Management Protocol (OpAMP)** enables centralized management of distributed OTel collectors:
...
│  │                    OpAMP SERVER                            │ │
│  │                                                           │ │
│  │  • Collector inventory and health                         │ │
...
│   │  v0.95.0     │    │  v0.94.0     │    │  v0.95.0     │       │
│   │  Config: A   │    │  Config: B   │    │  Config: A   │       │
│   └─────────────┘    └─────────────┘    └─────────────┘       │
│                                                                 │
│  USE CASES:                                                     │
│  • Push sampling rate changes during incidents                  │
│  • Roll out new collector versions with canary strategy         │
│  • Dynamically add/remove exporters for new backends           │
│  • Collect collector health metrics and logs                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**Why OpAMP matters:** Managing hundreds of OTel collectors across clusters and regions without OpAMP requires manual configuration changes and rolling restarts. OpAMP enables treating the collector fleet as a managed service with dynamic reconfiguration, reducing operational toil by 60-80%.
