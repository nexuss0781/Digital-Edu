...
|---|---|---|---|
| **PRIMARY** | `entry_id` | Unique lookup | Point reads for transaction details |
| **journal_idx** | `journal_id` | Group debit/credit entries of one transaction | Reconciliation: sum(debits) = sum(credits) per journal |
| **wallet_time_idx** | `(wallet_id, created_at DESC)` | Wallet transaction history (most recent first) | Mini-statement: "Show last 10 transactions" |
| **idempotency_idx** | `idempotency_key` (UNIQUE) | Duplicate detection | Every incoming transaction checks this first |
| **type_time_idx** | `(transaction_type, created_at)` | Regulatory reporting by transaction type | Monthly report: "All P2P transfers in March" |
| **counterparty_idx** | `(counterparty_id, created_at)` | Find transactions between two wallets | Fraud investigation: "All transactions between wallet A and B" |
| **country_time_idx** | `(country_code, created_at)` | Country-specific queries and partitioning | Per-country regulatory reports |

**Partitioning strategy:** The ledger is partitioned first by `country_code` (regulatory data isolation) and then by `created_at` (monthly partitions). This dual partitioning ensures: (1) queries never cross country boundaries, (2) old partitions can be archived to cold storage without affecting active partition performance, (3) partition Cutting off unnecessary steps eliminates scanning irrelevant months for historical queries.

### Wallet Indexes

| Index | Columns | Purpose |
|---|---|---|
| **PRIMARY** | `wallet_id` | Internal lookups |
| **msisdn_country_idx** | `(msisdn, country_code)` (UNIQUE) | Phone-number-based lookup (USSD identifies users by MSISDN) |
| **type_country_idx** | `(wallet_type, country_code)` | Query all agents in a country, all merchant wallets, etc. |
| **status_idx** | `(status, last_active_at)` | Find dormant accounts for escheatment processing |

### Agent Index Strategy

| Index | Columns | Purpose |
|---|---|---|
| **SPATIAL** | `location` (GeoPoint) | Find nearest agents for customer routing; find nearby dealers for rebalancing |
| **region_type_idx** | `(region, agent_type)` | Regional agent analytics and float forecasting |
| **parent_idx** | `parent_agent_id` | Navigate the agent hierarchy (retail → dealer → super-agent) |
| **performance_idx** | `(performance_score DESC, status)` | Identify underperforming agents for review |

### Query Optimization

**Hot queries (>1,000 QPS):** Wallet balance by MSISDN served from cache, DB read only on miss. Idempotency check via hash-based lookup in dedicated key-value store (<3ms). USSD session lookup in-memory only, never hits disk.

**Warm queries (100–1,000 QPS):** Agent float position computed from event stream, cached in time-series store. Credit score cached with 24-hour TTL. Transaction history (last 10) indexed on `(wallet_id, created_at DESC)` with LIMIT 10.

**Cold queries (<100 QPS):** Regulatory reporting scans monthly partitions with pre-aggregated materialized views. Fraud investigation uses full-text search across metadata JSON fields via dedicated search index. Trust account reconciliation maintained as a running counter.
...
```
FUNCTION compute_fee(transaction_type, amount, sender_wallet, channel, country):
  // Load fee schedule for country and transaction type
  schedule = fee_schedules[country][transaction_type]

  // Find applicable band (tiered pricing)
  band = schedule.bands.find(b => b.min <= amount AND amount <= b.max)

  IF band.type == "FLAT":
    fee = band.flat_amount
  ELSE IF band.type == "PERCENTAGE":
    fee = ROUND_UP(amount * band.percentage / 100, 1)  // Round up to nearest cent
  ELSE IF band.type == "TIERED":
    fee = band.flat_amount + ROUND_UP((amount - band.threshold) * band.marginal_rate / 100, 1)

  // Apply channel adjustment (incentivize app usage)
  IF channel == "APP":
    fee = MAX(fee - schedule.app_discount, schedule.minimum_fee)

  // Country-specific tax/levy
  IF country == "GH" AND transaction_type IN ["P2P_TRANSFER", "MERCHANT_PAYMENT"]:
    e_levy = ROUND_UP(amount * 0.01, 1)  // Ghana e-levy: 1%
    fee = fee + e_levy

  // Enforce minimum and maximum
  fee = CLAMP(fee, schedule.minimum_fee, schedule.maximum_fee)

  RETURN {
    fee_cents: fee,
    breakdown: {base_fee, channel_discount, tax},
    display_text: format_for_channel(fee, channel, country)
  }
```
...
## Algorithm 4: Hot Wallet Balance Bucketing

```
Algorithm: HotWalletBalanceBucketing(wallet_id, operation, amount)

Problem: A popular merchant or super-agent receives hundreds of concurrent
transactions, all updating the same wallet balance. Optimistic concurrency
control causes excessive retries under contention.

Solution: Split the wallet's balance across N partitions. Each transaction
targets a random partition, reducing contention by N×.

Data Structure:
  WalletPartition {
    wallet_id:      UUID
    partition_id:   Integer (0 to N-1)
    balance_cents:  BigInteger
    version:        BigInteger
  }
  // Total balance = SUM(balance_cents) across all partitions

Steps:
  1. DETERMINE partition count based on wallet type:
     partition_count = PARTITION_TABLE[wallet.wallet_type]
       // PERSONAL: 1 (no bucketing needed)
       // MERCHANT: 10
       // SUPER_AGENT: 20
       // SYSTEM_FEE: 50

  2. FOR CREDIT operations (receiving money):
     target_partition = RANDOM(0, partition_count - 1)
     // Credits can go to any partition — no balance check needed
     UPDATE WalletPartition
       SET balance_cents = balance_cents + amount,
           version = version + 1
       WHERE wallet_id = wallet_id
         AND partition_id = target_partition
         AND version = expected_version

  3. FOR DEBIT operations (sending money):
     // Must ensure sufficient funds. Try random partition first:
     target_partition = RANDOM(0, partition_count - 1)
     IF WalletPartition[target_partition].balance_cents >= amount:
       DEBIT from target_partition
     ELSE:
       // Insufficient funds in random partition — try others
       FOR partition IN partitions ORDER BY balance_cents DESC:
         IF partition.balance_cents >= amount:
           DEBIT from partition
           BREAK
       ELSE:
         // No single partition has enough — consolidate
         total = SUM(all partitions)
         IF total >= amount:
           CONSOLIDATE: move balances to partition 0, then debit
         ELSE:
           REJECT: "Insufficient balance"

  4. PERIODIC REBALANCING (every 5 minutes for hot wallets):
     // Redistribute balance evenly across partitions to prevent
     // debit failures from uneven distribution
     total = SUM(balance_cents for all partitions)
     target_per_partition = total / partition_count
     FOR each partition:
       SET balance_cents = target_per_partition

  Concurrency guarantee:
    Each partition is independently versioned.
    Contention reduced by partition_count factor.
    A 50-partition system_fee wallet handles 50× more concurrent
    updates than an unbucketed wallet.
```

---

## Algorithm 5: Loan Repayment Auto-Sweep

```
Algorithm: AutoSweepRepayment(wallet_id, incoming_amount)

Triggered: After any credit to a wallet that has an outstanding loan

Steps:
  1. CHECK for active loans:
     active_loans = query_loans(borrower_wallet_id = wallet_id,
                                status IN [DISBURSED, PARTIALLY_REPAID, OVERDUE])
     IF active_loans IS EMPTY: RETURN  // No loans — no sweep

  2. COMPUTE sweep amount:
     // Respect minimum balance: never sweep below KES 50 (livelihood protection)
     available_for_sweep = incoming_amount - MIN_BALANCE_AFTER_SWEEP
     IF available_for_sweep <= 0: RETURN

     // Apply sweep percentage (typically 10-20% of incoming)
     loan = active_loans.sort_by(due_date ASC).first()  // Oldest loan first
     sweep_amount = MIN(
       available_for_sweep * loan.sweep_percentage,
       loan.total_due_cents - loan.amount_repaid  // Don't overpay
     )

  3. EXECUTE repayment as ledger transaction:
     journal = create_journal(type = LOAN_REPAY)
     debit(wallet_id, sweep_amount, journal)
     credit(loan.pool_wallet_id, sweep_amount, journal)

  4. UPDATE loan record:
     loan.amount_repaid += sweep_amount
     IF loan.amount_repaid >= loan.total_due_cents:
       loan.status = FULLY_REPAID
       // Trigger credit score improvement event
       emit_event(LOAN_REPAID, {wallet_id, loan_id, days_to_repay})

  5. SEND notification (if sweep_amount > 0):
     SMS to user: "KES {sweep_amount} auto-deducted for loan repayment.
       Remaining: KES {remaining}. Bal: KES {new_balance}."
```
...
|---|---|---|---|---|
| **Optimistic concurrency conflict** | Yes | 5 | 10ms, 25ms, 50ms, 100ms, 200ms | Re-read balance, recompute, retry |
| **Database connection timeout** | Yes | 3 | 100ms, 500ms, 1s | Different connection from pool |
| **Replication lag exceeded** | Yes | 2 | 500ms, 1s | Wait for replica to catch up |
| **Insufficient balance** | No | 0 | N/A | Return error to user immediately |
| **Fraud check block** | No | 0 | N/A | Transaction permanently rejected |
| **Idempotency duplicate** | No | 0 | N/A | Return cached original result |
| **MNO gateway timeout** | Yes (USSD) | 1 | 2s | Retry once; if still no response, SMS fallback |
| **SMS delivery failure** | Yes | 3 | 30s, 60s, 300s | Must eventually deliver (legal receipt) |
| **Partner bank API failure** | Yes | 5 | 1s, 5s, 30s, 60s, 300s | Non-critical path; queue for retry |
| **Float service timeout** | Yes | 2 | 200ms, 500ms | Use cached float value if all retries fail |

### Dead Letter Queue Processing

```
FUNCTION process_dead_letter(failed_event):
  // Events that exhaust all retries are sent to a dead letter queue (DLQ)
  // DLQ events are reviewed by operations team within SLA

  CLASSIFY failed_event:
    CASE "ledger_write_failure":
      PRIORITY: P0  // Financial impact — may have debited without crediting
      ACTION: Manual investigation + corrective journal entry
      SLA: 1 hour

    CASE "sms_delivery_exhausted":
      PRIORITY: P2  // User didn't receive receipt
      ACTION: Retry via alternative SMS provider; fallback to USSD push
      SLA: 4 hours

    CASE "fraud_deep_analysis_failure":
      PRIORITY: P1  // Transaction approved without full fraud check
      ACTION: Run offline fraud analysis; reverse if fraud detected
      SLA: 2 hours

    CASE "float_sync_failure":
      PRIORITY: P2  // Agent float dashboard may show stale data
      ACTION: Force re-derive float from ledger events
      SLA: 8 hours
```
...
| Entity | Record Size | Daily Volume | Annual Volume | Partition Strategy |
|---|---|---|---|---|
| **Wallet** | 512 bytes | 65M static (slow growth) | +15M/year | By country_code |
| **LedgerEntry** | 1 KB | 270M (3 entries × 90M txns) | 98.5 TB | By country_code + created_at (monthly) |
| **USSDSession** | 500 bytes | 50M (ephemeral) | N/A (in-memory, TTL-based) | By session_id hash |
| **AgentProfile** | 1 KB | 300K static | +50K/year | By country_code + region |
| **CreditScore** | 500 bytes | 40M monthly recalc | 240M records/year | By wallet_id |
| **Loan** | 400 bytes | 40K new/day | 14.6M/year | By country_code + status |
| **InsurancePolicy** | 350 bytes | 20K new/day | 7.3M/year | By country_code + product_type |
| **FraudDecision** | 2 KB | 90M/day | 66 TB/year | By country_code + created_at |
| **AuditEvent** | 2 KB | 90M/day | 66 TB/year | By country_code + created_at |
