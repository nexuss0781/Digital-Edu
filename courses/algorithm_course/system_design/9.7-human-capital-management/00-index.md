...
| System | Relationship to HCM |
|--------|---------------------|
| [Payment Gateway (Stripe/Razorpay)](../8.2-stripe-razorpay/00-index.md) | ACH disbursement and direct deposit integration; payroll as a payer to banking networks |
| [Multi-Tenant SaaS Platform](../6.3-multi-tenant-saas-platform-architecture/00-index.md) | Tenant isolation model; shared-infrastructure payroll across thousands of organizations |
| [Calendar/Scheduling System](../6.15-calendar-scheduling-system/00-index.md) | Time-and-attendance scheduling; shift management and recurring schedule patterns |
| [Fraud Detection System](../8.5-fraud-detection-system/00-index.md) | Payroll fraud detection; ghost employee detection; anomaly scoring on pay runs |
...
|---------|-----------------|-------------|
| **Batch Orchestration with Checkpoint Recovery** | Payroll engine | Embarrassingly parallel computation with per-unit checkpointing for resumable execution within hard deadlines |
| **Bi-Temporal Data Management** | Employee master, compensation, employment | Two time dimensions (business effective date + system recorded date) enabling retroactive processing and audit |
| **Event-Driven Domain Integration** | Employee lifecycle events | Asynchronous propagation of HR events across bounded contexts via event bus with per-entity ordering |
| **Data-Driven Rule Engine** | Tax calculation, leave policies, compliance | Externalizing frequently-changing regulatory rules as versioned data rather than application code |
| **CQRS (Command Query Responsibility Segregation)** | Analytics warehouse | Separate read-optimized models for reporting from write-optimized operational stores |
| **Effective Dating** | All mutable HCM entities | Every record change includes when it becomes effective, enabling point-in-time reconstruction |
| **Multi-Graph Hierarchy** | Org structure | Multiple independent DAGs (supervisory, cost center, legal entity) with hierarchy-specific storage strategies |
| **Pre-Computation for Spike Absorption** | Benefits open enrollment | Batch-generate personalized data before the spike window; serve from read cache during the spike |
| **Append-Only Financial Records** | Payroll results, GL entries | Committed financial records are never modified; corrections are additive entries preserving the audit chain |
| **File-Based Distributed Synchronization** | Carrier feeds, ACH files, tax filings | Integration with external systems via periodic file exchange with asynchronous acknowledgment and reconciliation |
