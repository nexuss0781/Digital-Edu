...
| Scenario | Trigger | Peak Load | Duration | Design Implication |
|----------|---------|-----------|----------|--------------------|
| **Payday morning** | Pay stubs become available | 50K concurrent self-service sessions; 250 req/sec | 2-3 hours (morning peak) | Read-optimized pay stub serving; CDN for static assets |
| **Shift change (7 AM)** | 500 locations × 200 workers clock in | 500+ punches/sec sustained for 15 minutes | 15-minute burst | Edge buffering at time clocks; batch ingestion tier |
| **Open enrollment launch** | Annual enrollment window opens | 15-20K concurrent sessions; 400 req/sec | First 3 days heaviest | Pre-computed enrollment packages; horizontal API scaling |
| **Year-end payroll** | Final regular + bonus runs + W-2 generation | 2-3x normal batch compute; 300K+ pay results | 2-day processing window | Reserved compute capacity; staggered run scheduling |
| **Mass termination / RIF** | Reduction in force event | 1,000+ terminations in single batch | Single business day | Lifecycle event fan-out; final pay queue; COBRA notification burst |
| **Annual compensation review** | Merit increase cycle launch | 10K+ managers accessing comp planning simultaneously | 2-week window | Pre-loaded comp worksheets; manager-level data partitioning |

### Integration Volume Benchmarks

| Integration | Volume | Frequency | SLA |
|-------------|--------|-----------|-----|
| ACH/Direct deposit files | 150K payment records per file | Per pay period (biweekly = 26/year) | File delivered to bank 48 hours before pay date |
| Benefits carrier feeds (EDI 834) | 500-80K member records per carrier | Daily deltas + monthly full reconciliation | Delivered within 24 hours of enrollment change |
| Tax agency filings | 150K W-2s; quarterly 941s for each state | Annual (W-2); quarterly (941, SUI) | W-2 by January 31; 941 within 30 days of quarter end |
| Background check requests | 500-2,000 per month | On-demand (new hires) | Initiated within 1 business day of hire |
| State new-hire reporting | All new hires | Within 20 days of hire (varies by state) | Automated weekly batch |
| ACA 1095-C forms | All full-time employees | Annual | Furnished to employees by March 2; filed with IRS by March 31 |

### Regulatory Filing Calendar

| Filing | Jurisdiction | Deadline | Volume | Failure Consequence |
|--------|-------------|----------|--------|---------------------|
| **W-2** | US Federal + State | Jan 31 (employees); Jan 31 (SSA) | 150K per org | $50-$280 penalty per late form; potential IRS audit |
| **Quarterly 941** | US Federal | 30 days after quarter end | 1 per legal entity per quarter | Penalty + interest on unpaid taxes; potential trust fund recovery |
| **State unemployment (SUI)** | Each US state | Quarterly (varies by state) | 1 per state of operation | Loss of FUTA credit; back-payment demands |
| **New hire reporting** | Each US state | Within 20 days of hire | All new hires | $25-$500 penalty per late report per state |
| **ACA 1095-C** | US Federal | March 2 (furnish); March 31 (file) | All full-time employees | $310 penalty per form per employee |
| **EEO-1** | US Federal (EEOC) | Annually (varies) | 1 per establishment | Compliance investigation; government contract risk |
| **Pay equity reporting** | California, Illinois, others | Varies by state | Per establishment | Fines; enforcement action |
| **GDPR data subject requests** | EU member states | 30 days from receipt | Per request | Fines up to 4% of global annual revenue |
| **Payroll tax deposits** | US Federal (IRS) | Semi-weekly or monthly (based on liability) | Per payroll run | 2-15% penalty on late deposits |
| **Workers' compensation audit** | Each US state | Annual | Per state of operation | Premium adjustment; potential penalty |

The regulatory filing calendar illustrates why HCM batch processing must meet hard deadlines: every missed filing carries penalties that scale with the number of affected employees.
A late W-2 filing for 150K employees at $50/form represents $7.5M in potential penalties.
This penalty structure makes filing deadline compliance a P0 system reliability requirement.
