...
| `ess.auth.failed_attempts` | Counter | Failed authentication attempts | > 10 per user in 1 hour (Brute Force (Checking every single possibility)) |
...
| Leave request submission | Every 5 minutes | Submit and approve a leave request for a synthetic employee |
| Carrier feed generation | Daily | Generate and validate a synthetic carrier feed for a test carrier |

---

## Interview Checklist

- Explain which payroll metrics matter most and why (run progress, YTD drift, anomaly detection)
- Describe how you would detect a stalled payroll run vs. a slow one (progress rate derivative, not just percentage)
- Walk through the time punch trace and explain where latency accumulates under load
- Articulate why payroll amounts must be masked in logs but calculation hashes provide debugging capability
- Explain the difference between P0 alerts (payroll stalled, enrollment down) and P2 alerts (timecard approval lagging) in terms of business impact
- Describe how you would observe the open enrollment spike in real-time and make scaling decisions based on dashboard signals
