...
### Race Condition 6: Concurrent Poll Creation and Session End

**Scenario:** The instructor creates a poll at the exact moment another co-host ends the session.

**Problem:** The poll creation completes successfully and is broadcast to participants, but the session end command arrives immediately after. Participants receive both a "new poll" and "session ended" notification. Some clients render the poll before processing the session end, leaving orphaned poll UI.

**Solution:** Session state guards. All engagement operations (poll creation, Q&A, hand raise) check session state before execution. The session end operation acquires an exclusive lock on the session state. If a poll creation arrives while the lock is held, it receives a `409 Conflict` response. The client treats session-end as a terminal state that overrides any pending engagement interactions.

---

## Slowest part of the process Analysis

### Slowest part of the process 1: Hour-Boundary Session Start Thundering Herd
...
### Slowest part of the process 2: SFU Memory Pressure Under High Subscription Count
...
### Slowest part of the process 3: CRDT State Growth for Long Whiteboard Sessions
...
### Slowest part of the process 4: TURN Relay Capacity During Enterprise Firewall Hours

**The Problem:** Enterprise and school networks often block direct UDP connectivity via restrictive firewalls. During business hours (8 AM–5 PM per timezone), 15-25% of participants require TURN relay fallback. TURN relays are bandwidth-intensive—each relay allocates dedicated bandwidth for both upstream and downstream media, doubling the effective bandwidth cost per participant. At peak, the TURN fleet can become the capacity Slowest part of the process before the SFU fleet.

**Impact:** Participants behind restrictive NATs experience join failures (ICE fails all candidates) or are forced into audio-only mode (insufficient relay capacity for video).

**Mitigation Strategy:**

| Strategy | Implementation | Impact |
|---|---|---|
| **ICE lite on SFU** | SFU acts as controlled agent, accepting any valid check without performing its own | Reduces ICE negotiation time; more participants succeed on direct/srflx before falling back to relay |
| **TURN server geo-distribution** | Deploy TURN fleet in same regions as SFU nodes | Minimizes relay hop latency; reduces cross-region relay bandwidth |
| **TCP TURN fallback** | Support TURN over TCP/443 for networks blocking UDP entirely | Enables connectivity through the most restrictive firewalls at the cost of higher latency |
| **TURN pre-allocation** | Pre-allocate TURN relay during waiting room phase | Eliminates allocation delay at session start; relay ready before media flow begins |
| **Corporate network profiling** | Cache ICE candidate success history per network prefix | Skip known-failing candidates; go directly to TURN for previously relayed networks |

### Slowest part of the process 5: Active Speaker Flicker in Panel Discussions

**The Problem:** In panel discussions, seminars, or classroom debates, multiple participants speak in rapid succession or overlapping turns. The active speaker detection algorithm must decide who is speaking at each moment, but with 3+ overlapping audio sources, the algorithm oscillates rapidly between speakers ("flicker"), causing the video UI to rapidly switch highlighted speakers—a visually jarring and disorienting experience.

**Impact:** Gallery view becomes unusable as the active speaker tile constantly changes; simulcast layer upgrades/downgrades cascade through the SFU, consuming bandwidth on wasted keyframe requests.

**Mitigation Strategy:**

| Strategy | Implementation | Impact |
|---|---|---|
| **Hysteresis hold timer** | Require 500ms of sustained dominance before switching active speaker | Prevents sub-second oscillation; speaker must "hold the floor" |
| **Multi-speaker mode** | Support 2-3 concurrent active speakers with proportional tile sizing | Panel discussions don't need to pick one speaker; show all active |
| **Smoothed energy window** | Use 1-second RMS window instead of per-packet levels | Filters out brief interjections (e.g., "uh-huh") that shouldn't trigger switches |
| **Priority weighting** | Instructor audio gets 1.5x weight in speaker detection | Instructor commentary doesn't get overridden by student questions |
| **Rate-limited PLI** | Cap keyframe requests to 1 per 2 seconds per publisher | Prevents cascading keyframe storms from rapid speaker changes |
