...
| Metric | Value | Notes |
|--------|-------|-------|
| Global virtual classroom market | ~$35B (2026) | Growing at 14% CAGR |
| Concurrent sessions (large platform) | 50,000–200,000 | Simultaneous live sessions |
| Peak concurrent participants | 5M–15M | Across all sessions globally |
| Participants per session (typical) | 20–50 (class), 200–500 (lecture) | Webinar mode supports 2,000+ |
| Media bandwidth per participant | 1.5–4 Mbps down, 0.5–2 Mbps up | Varies with simulcast layers |
| Aggregate SFU bandwidth (peak) | 5–20 Tbps | Across all sessions |
| Session start rate (peak hour) | 3,000–5,000 new sessions/minute | At hour boundaries |
| Recording storage (daily) | 50–200 TB/day | Multi-track raw recordings |
| SFU fleet size (peak) | 8,000–60,000 nodes | Depending on year and scale |

---

## Key Trade-Offs in Live Classroom Design

| Trade-Off | Option A | Option B | This System's Choice |
|-----------|----------|----------|---------------------|
| **Media routing** | SFU (low server CPU, high client bandwidth) | MCU (high server CPU, low client bandwidth) | SFU with simulcast—enables per-subscriber quality control and multi-track recording |
| **Whiteboard sync** | CRDT (no central server, automatic convergence) | OT (central server required, intent preservation) | CRDT—relay server optional, not required; survives relay failure |
| **Video codec** | Simulcast VP8/H.264 (universal support) | SVC VP9/AV1 (efficient but limited support) | Simulcast now, SVC roadmap—design SFU to support both |
| **Signaling transport** | WebSocket (native browser, bidirectional) | MQTT (pub/sub model, QoS levels) | WebSocket—no broker dependency; server fan-out manageable at session scale |
| **E2EE** | Always on (maximum privacy) | Configurable per session (features vs privacy) | Configurable—E2EE disables recording and transcription |
| **Recording** | Live mixing (simple, immediate) | Multi-track + post-composition (flexible, delayed) | Multi-track—enables speaker-view, gallery-view, and audio-only renditions |
| **Scaling** | Reactive auto-scaling | Schedule-aware pre-warming | Hybrid—pre-warming for hour boundaries, reactive buffer for unscheduled demand |
...
| Design | Relevance |
|--------|-----------|
| [11.1 - Online Learning Platform](../11.1-online-learning-platform/) | Asynchronous counterpart; session recordings and attendance data feed into LMS gradebook and course analytics |
| [11.3 - Enterprise Communication Platform](../11.3-enterprise-communication-platform/) | Shared WebRTC infrastructure patterns; SFU scaling, signaling protocols, and TURN relay fleet management apply directly |
| [4.7 - Live Streaming Platform](../4.7-live-streaming-platform/) | CDN-based delivery for webinar mode (2000+ participants); HLS/DASH fallback when WebRTC interactive latency is not required |
| [5.3 - Collaborative Document Editing](../5.3-collaborative-document-editing/) | CRDT and OT conflict resolution patterns directly inform whiteboard collaboration engine design |
| [6.5 - Real-Time Collaboration](../6.5-real-time-collaboration/) | Shared CRDT architecture, presence indicators, and cursor synchronization patterns used in whiteboard and shared annotation |
| [10.5 - Telemedicine Platform](../10.5-telemedicine-platform/) | Similar E2EE requirements for sensitive sessions; HIPAA-grade recording security overlaps with FERPA compliance |
| [11.4 - Notification System](../11.4-notification-system/) | Session reminders, recording-ready notifications, and schedule change alerts route through multi-channel notification infrastructure |
| [15.3 - Distributed Tracing Platform](../15.3-distributed-tracing-platform/) | Cross-plane trace propagation (signaling → SFU → recording) requires specialized trace context for real-time media systems |
...
|-------|----------------------|
| **AI-powered engagement analytics** | ML models analyze camera-on rates, facial engagement cues (with consent), and participation patterns to produce real-time engagement heatmaps; requires GPU inference pipeline co-located with SFU for low-latency frame sampling |
| **Real-time AI translation** | Speech-to-text → machine translation → text-to-speech pipeline enables cross-language classrooms with <3-second delay; requires streaming ASR models with language detection and speaker diarization |
| **Spatial audio for breakout rooms** | 3D audio positioning simulates physical classroom seating; participants "hear" others from directional positions, improving conversational naturalness and reducing talk-over in group discussions |
| **AV1/VP9 SVC adoption** | Scalable Video Coding replaces simulcast, reducing publisher bandwidth by 40% (single layered stream vs. 3 independent encodings); SFU must support layer extraction from SVC bitstreams |
| **Edge AI for noise suppression** | Client-side neural noise suppression (RNNoise, DeepFilterNet) eliminates background noise without server-side processing, preserving E2EE compatibility and reducing SFU compute load |
| **Generative AI teaching assistants** | LLM-based assistants answer student questions in real-time during lectures, generate session summaries, and auto-create flashcards from whiteboard content; requires RAG pipeline over course materials |
| **Immersive XR classrooms** | VR/AR headsets render 3D classroom environments with spatial presence; requires ultra-low-latency 6DoF pose tracking and volumetric video streaming at 90fps |
