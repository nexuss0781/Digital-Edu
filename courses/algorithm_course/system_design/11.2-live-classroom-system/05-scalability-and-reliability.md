...
| Component | Trigger | Scale Up | Scale Down | Min/Max Instances |
|-----------|---------|----------|------------|-------------------|
| **SFU nodes** | Schedule-aware: T-10 min forecast; reactive: pool >70% for 5 min | Add 10% of fleet per scaling event | Drain idle nodes during off-peak (>10 min idle, 0 active sessions) | 500 / 60,000 |
| **Signaling servers** | WebSocket connections >4,000 per instance | Add 2 instances per event | Remove when connections <1,000 for 15 min | 50 / 2,000 |
| **TURN relays** | Bandwidth >60% capacity per relay | Add 5 relays per region per event | Remove when <20% utilization for 30 min | 100 / 5,000 |
| **Recording workers** | Queue depth >200 pending compositions | Add 50 workers per event | Scale to zero when queue empty for 30 min | 0 / 5,000 |
| **Whiteboard CRDT relays** | Active whiteboard sessions >50 per relay | Add 10 relays per event | Remove when sessions <10 for 15 min | 30 / 1,000 |
| **Transcription workers** | Audio streams waiting >30s for transcription slot | Add 20 workers per event | Scale to zero when no active transcription sessions | 0 / 2,000 |
...
| Scenario | Configuration | Target | Success Criteria |
|----------|--------------|--------|-----------------|
| **Hour-boundary burst** | 4,000 sessions starting in 60s window | TTFF p99 < 5s | 99% sessions start successfully within 5s |
| **Sustained peak** | 100K concurrent sessions, 30 participants each | MOS p50 > 4.0 | No SFU node above 85% CPU for 30 min |
| **Large lecture** | Single 500-participant session | Glass-to-glass p99 < 200ms | All subscribers receive instructor video; gallery view stable |
| **Breakout room storm** | 5,000 sessions create breakout rooms simultaneously | Transition p99 < 3s | 95% participants transition within 3s |
| **SFU failover under load** | Kill SFU node hosting 12 active sessions | Media gap < 7s | All participants auto-reconnect; no data loss |
| **Recording backlog** | 10,000 sessions end within 10 min | Composition within 2h | 99% recordings available within 2h |

### Load Testing Anti-Patterns for Live Classrooms

- **Synthetic media only:** Real WebRTC clients behave differently than RTP packet generators—test with headless browser fleets running real WebRTC stacks
- **Ignoring ICE variability:** Lab-network tests miss TURN fallback scenarios—include simulated NAT environments in test topology
- **Single-region testing:** Cross-region cascade behavior only manifests in multi-region deployments—test with participants distributed across regions
- **Omitting whiteboard load:** CRDT sync under concurrent drawing generates significant DataChannel traffic—include active whiteboard usage in scenarios

---

## Connection Pool and Resource Management

| Resource | Pool Size | Strategy | Rationale |
|----------|-----------|----------|-----------|
| **SFU UDP sockets** | 10,000 per node (pre-allocated) | Pre-allocated at boot; refilled from standby pool | Avoid socket allocation during session start burst |
| **Signaling WebSocket connections** | 5,000 per instance | Connection-per-instance limit with L4 balancer distribution | Prevent single-instance overload; sticky sessions via session_id hash |
| **Database connections** | 50 per service instance (separate read/write pools) | PgBouncer connection pooling in transaction mode | DB connections are expensive; pool prevents exhaustion |
| **Redis connections** | 100 per service instance | Connection pool with health check pings | Session state lookups are latency-critical; pre-warmed connections |
| **TURN allocations** | 500 per TURN server | Per-session allocation with 6-hour TTL | TURN allocations consume bandwidth and port resources |
| **gRPC channels (signaling → SFU)** | 10 per SFU node | Persistent multiplexed channels | Avoid TCP handshake overhead on every signaling command |
...
|------------|----------------|------------------------|--------------|
| **SFU node kill** | Terminate SFU process mid-session | Participants reconnect within 7s; recording not corrupted | Single session cluster |
| **Network partition (SFU ↔ subscriber)** | Inject 100% packet loss for 5s to random SFU | Affected subscribers switch to TURN; media resumes within 10s | Individual participants |
| **Cascade link degradation** | Inject 10% packet loss on inter-region SFU link | Cross-region video degrades gracefully; audio unaffected | Cross-region sessions |
| **Signaling server crash** | Kill signaling server with 2,000 active WebSockets | Clients reconnect to backup within 3s; in-progress media unaffected | New joins delayed |
| **Redis cache eviction** | Flush session state cache | Session service falls back to database; latency increases but no errors | All active sessions |
| **Recording sink saturation** | Throttle recording agent disk I/O to 50% | Recording chunks buffered in memory; live media unaffected | Recording quality |
| **TURN relay exhaustion** | Set TURN fleet to reject new allocations | Clients that need relay fail ICE; fallback to audio-only | NAT-constrained users |
