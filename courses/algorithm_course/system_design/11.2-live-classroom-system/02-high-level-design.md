...
| **Scalability** | Peer-to-peer friendly, server is optional relay | Central server is Slowest part of the process |
...
**Cascade selection Practical rule of thumb:**
...
### Multi-Track Recording and Composition Flow

```mermaid
flowchart TB
    subgraph LiveSession["Live Session (Real-Time)"]
        SFU[SFU Node<br/>Active Media Routing]
        FORK[Media Fork<br/>Passive RTP Copy]
    end

    subgraph Capture["Capture Layer"]
        REC_AGENT[Recording Agent<br/>Per-Session Process]
        CHUNK_STORE[(Chunk Store<br/>5-Second Segments)]
        MANIFEST[Manifest Builder<br/>Track Timeline + Events]
    end

    subgraph PostSession["Post-Session Composition"]
        QUEUE[Composition Queue<br/>Priority-Ordered]
        WORKER[Composition Worker<br/>Multi-Track Merge]
        RENDER[Multi-Rendition<br/>Encoder]
    end

    subgraph Output["Output Layer"]
        FULL[(Full Recording<br/>Gallery View)]
        SPEAKER[(Speaker View<br/>Active Speaker Focus)]
        AUDIO_ONLY[(Audio-Only<br/>Podcast Format)]
        TRANSCRIPT[(Searchable<br/>Transcript)]
    end

    SFU --> FORK
    FORK -->|"Forked RTP packets"| REC_AGENT
    REC_AGENT --> CHUNK_STORE
    REC_AGENT --> MANIFEST
    ...
    RENDER --> FULL & SPEAKER & AUDIO_ONLY & TRANSCRIPT

    classDef live fill:#fce4ec,stroke:#c62828,stroke-width:2px
    ...
    class SFU,FORK live
    class REC_AGENT,CHUNK_STORE,MANIFEST capture
    class QUEUE,WORKER,RENDER post
    class FULL,SPEAKER,AUDIO_ONLY,TRANSCRIPT output
```

**Key design decisions for recording:**

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Capture method** | Media fork at SFU (passive copy) | Zero impact on live media path; no SDP renegotiation needed |
| **Chunk format** | 5-second segments per track | Enables parallel composition; limits data loss on agent crash to 5 seconds |
| **Composition timing** | Post-session (not live) | Live composition requires real-time transcoding (expensive, fragile); post-session allows retry and multiple renditions |
| **Manifest format** | JSON timeline with track entries and events | Declarative manifest enables different composition layouts from same source tracks |
| **Storage during session** | Local SSD on recording agent → async upload | Avoids network dependency during live capture; upload after session ends |
| **Multi-rendition output** | Gallery view, speaker view, audio-only, transcript | Different use cases: review (gallery), study (speaker), accessibility (audio/transcript) |

### Session Schedule and Pre-Warming Architecture

```mermaid
flowchart LR
    subgraph Input["Schedule Sources"]
        CAL[Calendar Service<br/>Recurring Sessions]
        LMS[LMS Integration<br/>Course Schedule]
        ADHOC[Ad-Hoc Session<br/>Creation API]
    end

    subgraph Planner["Capacity Planner (runs T-10 min)"]
        FORECAST[Demand Forecaster<br/>Schedule + History]
        ALLOCATOR[SFU Pre-Allocator<br/>Region-Aware Bin Packing]
    end

    subgraph PreWarm["Pre-Warming Pipeline"]
        PROVISION[Instance Provisioner<br/>Boot SFU Process]
        TLS_LOAD[TLS/DTLS Cert<br/>Pre-Load]
        UDP_POOL[UDP Socket Pool<br/>Pre-Allocation]
        HEALTH[Readiness Check<br/>Accept Traffic Gate]
    end

    subgraph Ready["Ready Pool"]
        SFU_WARM1[SFU Warm 1<br/>Ready for Session]
        SFU_WARM2[SFU Warm 2<br/>Ready for Session]
        SFU_WARM3[SFU Warm 3<br/>Ready for Session]
    end

    CAL & LMS & ADHOC --> FORECAST
    FORECAST --> ALLOCATOR
    ALLOCATOR --> PROVISION --> TLS_LOAD --> UDP_POOL --> HEALTH
    HEALTH --> SFU_WARM1 & SFU_WARM2 & SFU_WARM3
    ...
    classDef ready fill:#fffde7,stroke:#f57f17,stroke-width:2px

    class CAL,LMS,ADHOC input
    class FORECAST,ALLOCATOR planner
    class PROVISION,TLS_LOAD,UDP_POOL,HEALTH warm
    class SFU_WARM1,SFU_WARM2,SFU_WARM3 ready
```

**Pre-warming timeline:**

| Time Before Hour | Action | Purpose |
|------------------|--------|---------|
| T-10 min | Read schedule, compute required SFU capacity | Demand forecasting |
| T-8 min | Provision new SFU instances if pool insufficient | Instance boot |
| T-5 min | Load TLS/DTLS certificates, pre-allocate UDP socket pools | Crypto initialization |
| T-3 min | Run readiness checks, mark nodes as available | Traffic readiness |
| T-1 min | Pre-assign sessions to warm SFU nodes | Eliminate allocation latency at T-0 |
| T-0 (hour boundary) | Sessions start on pre-assigned, fully warm nodes | Sub-second SFU allocation |
...
## Engagement Tools Architecture

### Poll and Q&A Real-Time Flow
...
sequenceDiagram
    ...
    INS->>SIG: Create poll (question, options, timer)
    SIG->>ENG: Store poll + start countdown
    ENG->>CACHE: Cache poll state (votes: {})
    SIG-->>ALL: Broadcast: poll_started (question, options, timer)

    par Voting Phase (parallel submissions)
        ALL->>SIG: Vote (poll_id, option_index)
        SIG->>ENG: Record vote (deduplicated by participant_id)
        ENG->>CACHE: HINCRBY poll:{id}:votes option_index 1
        ENG-->>ALL: Broadcast: vote_count_updated (live tallies)
    end

    Note over ENG: Timer expires or instructor closes

    ENG->>CACHE: GET final vote counts
    ENG-->>ALL: Broadcast: poll_results (final tallies, correct answer)
    ENG->>ENG: Persist results to analytics pipeline
```

**Design considerations for engagement tools:**

| Tool | Consistency | Delivery | Notes |
|------|-------------|----------|-------|
| **Polls** | Eventual (vote counts can lag by 500ms) | Broadcast to all via signaling channel | Dedup by (participant_id, poll_id); last vote wins for changed answers |
| **Q&A** | Eventual (upvote counts can lag) | Broadcast new questions; upvote counts throttled to 1 update/sec | Sorted by upvote count for instructor view |
| **Hand raise** | Strong (queue ordering must be authoritative) | Ordered queue maintained by server | Position in queue determined by server timestamp; no ties |
| **Reactions** | Best-effort (loss tolerable) | Ephemeral broadcast; no persistence | Fire-and-forget; displayed for 3s and discarded |
| **Attendance check** | Strong (response recorded or not) | Point-in-time check with 60s response window | Presence verification; non-response logged as absent |

---

## Whiteboard Persistence and Recovery Strategy

| Scenario | Strategy | Recovery Time |
|----------|----------|---------------|
| **Client refresh** | Reload from latest server snapshot + replay buffered ops since snapshot | < 2s (snapshot load + replay) |
| **CRDT relay failure** | Clients buffer ops locally; replay on relay recovery or failover | < 5s for relay failover; zero drawing interruption |
| **Late joiner** | Serve latest compacted snapshot (no tombstones); no history replay needed | < 1s (snapshot transfer) |
| **Session end** | Final snapshot persisted to durable storage; CRDT state garbage-collected | N/A (permanent storage) |
| **Cross-session persistence** | Whiteboard state linked to classroom (not session); carries across recurring sessions | Loaded on session start if classroom has prior whiteboard state |
