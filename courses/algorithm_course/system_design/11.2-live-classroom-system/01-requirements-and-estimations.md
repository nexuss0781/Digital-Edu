...
```
Daily Traffic Profile (UTC, global education platform):
...
Notes:
- Asia-Pacific peak: 01:00-07:00 UTC (morning classes)
- Europe peak: 07:00-15:00 UTC
- Americas peak: 13:00-23:00 UTC
- Extreme hour-boundary spikes: 40% of sessions start within 60s of :00
- Academic calendar: 3x traffic during midterm/finals weeks
- Summer trough: 30-40% of peak-semester traffic
```
...
|-------------|-----------|-------------|----------|
| LMS roster sync (LTI 1.3) | Bidirectional | < 5s for roster pull at session start | REST API / LTI Advantage |
| Calendar service | Inbound | < 2s for schedule lookup | REST API / iCal |
| Speech-to-text (transcription) | Outbound (audio stream) | < 2s end-to-end for caption display | gRPC streaming |
| Content moderation | Outbound | < 500ms for chat text; < 2s for image classification | REST API (async) |
| Object storage (recordings) | Outbound | < 100ms per chunk upload | HTTP PUT (multipart) |
| Notification service | Outbound | < 30s for session reminders; < 10s for recording-ready | Message queue |
| Analytics pipeline | Outbound | < 5s for event ingestion | Event stream (async) |
| SSO / Identity provider | Inbound | < 1s for token validation | OIDC / SAML 2.0 |

---

## Regulatory Retention Requirements

| Data Type | Minimum Retention | Jurisdiction Notes |
|-----------|-------------------|-------------------|
| Session recordings | 90 days (contractual minimum) | FERPA: "reasonable period"; institutions typically require 1-3 years |
| Student attendance records | Duration of enrollment + 5 years | FERPA educational records; varies by institution policy |
| Chat transcripts | 90 days | May extend to 1 year under institutional discovery policies |
| Engagement/analytics data | 1 year | Aggregated data retained longer; PII stripped after retention period |
| Security/audit logs | 3 years | SOC 2 requirement; FERPA breach logs retained indefinitely |
| Parental consent records (K-12) | Duration of account + 3 years | COPPA requirement; proof of verifiable parental consent |
| Content moderation actions | 2 years | Required for pattern analysis and compliance audits |
