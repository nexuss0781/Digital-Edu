...
## Zero Trust Architecture for Media Infrastructure

| Principle | Implementation | Scope |
|-----------|---------------|-------|
| **Verify explicitly** | Every SFU connection verified via DTLS fingerprint + JWT session token; no implicit trust from network location | All media and signaling paths |
| **Least privilege** | Students receive audio + video consume only; screen share and recording permissions require explicit grant | Per-participant RBAC |
| **Assume breach** | SFU-to-SFU cascade links encrypted with DTLS-SRTP even within private VPC; internal service communication via mTLS | Inter-service communication |
| **Micro-segmentation** | SFU media ports isolated in dedicated network segment; signaling and API in separate segments; recording pipeline in isolated subnet | Network topology |
| **Continuous validation** | JWT tokens refreshed every 15 minutes via WebSocket keep-alive; stale tokens rejected mid-session | Token lifecycle |
...
|------------|-------------|----------------|
| **GDPR (EU)** | Personal data of EU residents processed and stored within EU | EU participants routed to EU-region SFU; session metadata stored in EU database shard; recordings stored in EU object storage |
| **FERPA (US)** | Education records maintained with appropriate security safeguards | Encryption at rest for all attendance and engagement data; access logging for all record access |
| **PDPA (Singapore)** | Cross-border transfer restrictions for personal data | AP-region deployment with data localization for Singapore institutions |
| **PIPL (China)** | Personal information must remain in China unless security assessment passed | Dedicated China region deployment with separate infrastructure stack |
| **Institutional policy** | Many universities require data to remain within national borders | Configurable per-organization: restrict SFU routing and storage to specified regions |
