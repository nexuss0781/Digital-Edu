...
## Anomaly Detection for Real-Time Media

| Detection Pattern | Signal | Method | Action |
|------------------|--------|--------|--------|
| **Regional quality degradation** | Aggregate MOS drops in single region while others stable | Statistical outlier detection on per-region MOS time series | Alert network team; potential ISP peering issue or regional SFU problem |
| **Participant network cliff** | Individual participant's loss rate jumps from <1% to >10% suddenly | Threshold on loss rate delta over 10s window | Proactively downgrade to audio-only; suggest TURN relay switch |
| **SFU hot-spotting** | Single SFU node serving 3x average sessions | Bin-packing imbalance detection on session-to-node mapping | Migrate sessions off hot node; investigate allocator bug |
| **Cascade congestion** | Inter-region cascade link utilization >80% while intra-region is normal | Bandwidth utilization monitoring on cascade links | Reduce cross-region video quality; increase active-speaker-only forwarding |
| **CRDT state explosion** | Whiteboard state size growing >5 MB/minute | State size time-series derivative monitoring | Trigger emergency compaction; investigate runaway object creation |
| **Recording pipeline stall** | Composition queue depth increasing while worker count stable | Queue depth derivative + worker health cross-correlation | Scale workers; check if storage backend is throttling |
| **Authentication storm** | JWT validation rate spikes 10x above baseline | Rate counter on auth service with anomaly detection | Potential credential stuffing attack; enable CAPTCHA; alert security |
...
|-----|-------------|------------|-------------|
| **Media availability** | % of session-minutes with uninterrupted media (no gap > 5s) | 99.95% monthly | SFU-reported session health status |
| **Join success rate** | % of join attempts that result in first frame within 5s | 99.9% monthly | Client-side TTFF measurement |
| **Audio MOS** | p50 Mean Opinion Score across all active sessions | > 4.0 | RTCP-derived E-model calculation |
| **Signaling latency** | p99 of control operations (mute, hand raise, poll vote) | < 100ms | Server-side timestamp delta |
| **Recording completeness** | % of recorded sessions with full recording available within 2h | 99.99% monthly | Composition pipeline success rate |
| **Breakout transition time** | p95 time from trigger to media flowing in breakout room | < 2s | Orchestrator event timestamps |

### Error Budget Policy

| SLO | Monthly Error Budget | Burn Rate Alert | Action |
|-----|---------------------|-----------------|--------|
| Media availability (99.95%) | 21.6 minutes | > 2x burn rate for 1h | Page media infra team; halt non-critical deployments |
| Join success rate (99.9%) | 0.1% of joins | > 5x burn rate for 15 min | Freeze SFU pool changes; investigate ICE/TURN health |
| Recording completeness (99.99%) | 0.01% of recordings | > 3x burn rate for 30 min | Scale composition workers; check storage availability |
...
```
ALERT RECEIVED
├── Is it media quality? (MOS, jitter, loss)
│   ├── Affects single participant → Check participant network (RTCP reports)
│   ├── Affects single session → Check SFU node health (CPU, memory, network)
│   ├── Affects single region → Check regional network/ISP peering
│   └── Affects platform-wide → Check cascade links, DNS, certificate expiry
│
├── Is it session start failure?
│   ├── SFU allocation failure → Check SFU pool capacity, auto-scaler health
│   ├── Signaling timeout → Check signaling server fleet, Redis connectivity
│   └── ICE failure → Check TURN fleet capacity, STUN server availability
│
├── Is it recording pipeline?
│   ├── Composition backlog growing → Scale workers; check storage write throughput
│   └── Composition failures → Check worker logs; likely storage or format issue
│
└── Is it security?
    ├── Auth spike → Potential credential stuffing; enable CAPTCHA
    └── Content moderation alert → Review flagged content; escalate per policy
```
...
| Dimension | Metric | Optimization Lever |
|-----------|--------|-------------------|
| **SFU compute** | Cost per session-minute by region and session tier | Right-size SFU nodes; consolidate small sessions onto shared nodes |
| **Bandwidth egress** | Cost per GB by region and connection type (direct vs TURN) | Optimize simulcast layer selection; reduce TURN usage through better ICE strategies |
| **TURN relay** | Cost per relay-minute; % of participants using TURN | Improve ICE candidate gathering; use ICE lite on SFU to reduce relay fallback |
| **Recording storage** | Cost per TB by storage tier (hot/warm/cold) | Aggressive tiering; move to cold after 30 days; compress composed recordings |
| **Composition compute** | Cost per recording-hour of composition | Batch composition during off-peak hours; parallelize track processing |
| **Transcription** | Cost per audio-minute transcribed | Transcribe only when requested; cache repeated segments (pre-recorded intros) |
