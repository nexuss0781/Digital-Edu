...
- Show the layer selection Step-by-step plan in plain English
...
|---------|-----------|-------------|
| **"Design a virtual exam proctoring system"** | E2EE mandatory; multi-camera monitoring; lockdown browser integration | Recording becomes surveillance footage; E2EE must still allow server-side proctoring; requires client-side ML for gaze detection |
| **"Design a live coding classroom (pair programming)"** | Code editor is the whiteboard; needs character-level CRDT, syntax highlighting sync | Replace whiteboard CRDT with text CRDT (similar to collaborative editors); add terminal sharing via PTY forwarding |
| **"Design a virtual lab (science demonstrations)"** | High-fidelity screen share; IoT device integration; safety compliance | Screen share at 60fps for real-time experiments; IoT data channels for instrument readings; requires annotation overlay on live video |
| **"Design a corporate training platform"** | Compliance tracking; completion certificates; SCORM integration | Attendance becomes completion verification; adds quiz/assessment engine; LMS integration depth increases |
| **"Design a live tutoring marketplace"** | 1:1 sessions at massive scale; payment integration; tutor matching | SFU optimized for 2-party; horizontal scale is in session count (millions concurrent 1:1) not session size; matchmaking algorithm added |

---

## Red Flags by Experience Level

| Level | Red Flag | What It Reveals |
|-------|----------|-----------------|
| **Junior** | Designs for 10-person meetings and doesn't consider 500-participant lectures | Hasn't scoped the problem correctly |
| **Junior** | Proposes HTTP polling for real-time events | Doesn't understand real-time communication requirements |
| **Junior** | No mention of NAT traversal or TURN | Missing fundamental WebRTC infrastructure |
| **Mid** | Treats SFU as a black box without discussing simulcast or layer selection | Lacks depth in the core media routing problem |
| **Mid** | No separation between media plane and control plane | Will design a coupled system with correlated failure modes |
| **Mid** | Proposes strong consistency for whiteboard | Doesn't understand CRDT trade-offs for real-time collaboration |
| **Senior** | No discussion of hour-boundary thundering herd | Missing the defining capacity challenge for educational platforms |
| **Senior** | Ignores recording architecture entirely | Recording is a significant subsystem, not an afterthought |
| **Staff+** | No cost analysis of simulcast layer selection | Missing the dominant cost driver (60% of total platform cost) |
| **Staff+** | Proposes same SFU strategy for 20-person and 500-person sessions | Doesn't understand session-tier-based scaling |

---

## System Evolution Discussion

| Phase | Timeline | Key Capabilities | Architecture Impact |
|-------|----------|-------------------|-------------------|
| **Phase 1: AI-Augmented Classroom** | 2025–2026 | Real-time transcription, auto-translation, noise suppression, engagement scoring | Add ML inference pipeline co-located with SFU; streaming ASR integration |
| **Phase 2: Adaptive Learning Integration** | 2026–2027 | AI tutoring assistant, personalized content delivery based on engagement signals, auto-generated quizzes from lecture content | RAG pipeline over course materials; real-time LLM inference for Q&A; engagement-to-content feedback loop |
| **Phase 3: Immersive XR Classrooms** | 2027–2028 | VR/AR headset support, spatial audio, 3D whiteboard, volumetric video, gesture recognition | Ultra-low-latency 6DoF pose streaming; 90fps video encoding; 3D CRDT for spatial objects; new SFU media types |
...
| Quality | Indicator |
|---------|-----------|
| **Architectural clarity** | Immediately draws media plane vs control plane separation; doesn't conflate SFU with signaling |
| **Real-time intuition** | Discusses latency budgets in milliseconds, not seconds; understands that retry/queue doesn't apply to media |
| **Education domain awareness** | Mentions FERPA/COPPA without prompting; discusses role-based permissions (instructor vs student) |
| **Cost consciousness** | Connects simulcast layer selection to bandwidth cost; recognizes SFU compute + egress as 60% of total cost |
| **Failure mode depth** | Explains SFU failover with specific timelines (detection: 2s, reconnect: 3-5s); discusses shadow SFU pattern |
| **Scale pattern recognition** | Identifies hour-boundary thundering herd as the defining capacity challenge; proposes schedule-aware pre-warming |
...
| **MCU** | Multipoint Control Unit — decodes, mixes, re-encodes (higher latency, lower bandwidth) |
| **Simulcast** | Publisher sends multiple quality encodings simultaneously (e.g., 720p + 360p + 180p) |
| **SVC** | Scalable Video Coding — single layered encoding with extractable quality levels |
| **ICE** | Interactive Connectivity Establishment — NAT traversal protocol for WebRTC |
| **TURN** | Traversal Using Relays around NAT — relay server for participants behind restrictive NATs |
| **DTLS-SRTP** | Datagram TLS for key exchange + Secure RTP for media encryption |
| **CRDT** | Conflict-Free Replicated Data Type — data structure guaranteeing convergence without coordination |
| **RGA** | Replicated Growable Array — CRDT type for ordered sequences (z-order in whiteboard) |
| **LWW Register** | Last-Writer-Wins Register — CRDT type for single-value properties (position, color) |
| **GCC** | Google Congestion Control — bandwidth estimation algorithm using delay gradients |
| **MOS** | Mean Opinion Score — voice quality metric (1.0–5.0 scale) |
| **TTFF** | Time to First Frame — join latency from click to first decoded video frame |
| **PLI** | Picture Loss Indication — RTCP feedback requesting a new keyframe |
| **NACK** | Negative Acknowledgment — request for retransmission of a lost packet |
