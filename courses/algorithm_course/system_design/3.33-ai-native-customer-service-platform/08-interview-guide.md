...
### Trap 5: "How do you handle a customer switching channels mid-conversation?"

**What they want:** Deep understanding of omnichannel architecture beyond "unified context."

**Best answer:**
"Channel switching is harder than it sounds because it involves identity resolution,
state transfer, and modality adaptation. Here's the architecture:
1. **Identity resolution**: Match the customer across channels (phone number → email → account)
2. **Session federation**: Create a new channel session linked to the existing conversation
3. **Context transfer**: Load full conversation history into new channel's context window
4. **Modality adaptation**: Reformat responses (voice transcript becomes text summary,
   rich buttons become text options for SMS)
5. **Agent handoff**: If human was engaged on original channel, notify them of switch
6. **State machine**: Conversation state persists independent of channel — the channel
   is just a delivery mechanism around a canonical conversation

The key insight is normalizing all channels to a canonical message format at the gateway.
See the Message Normalizer in our architecture — it transforms channel-specific formats
to a common envelope. Adding a new channel means writing one adapter, not touching any
AI pipeline code."

### Trap 6: "How do you measure AI agent quality beyond resolution rate?"

**What they want:** You think about quality holistically, not just the binary "resolved or not."

**Best answer:**
"Resolution rate alone is misleading — an agent could 'resolve' by giving wrong answers
that customers don't push back on. A comprehensive quality framework:
1. **CSAT per AI-resolved conversation**: Direct quality signal (target: 4.0+/5)
2. **Repeat contact rate**: If customer comes back about same issue within 48h, the
   first resolution was likely wrong (target: < 10%)
3. **Human correction rate**: When humans review AI conversations, how often do they
   flag errors (target: < 5%)
4. **Grounding accuracy**: Are AI responses actually supported by knowledge base content?
   Post-hoc automated checking.
5. **Action reversal rate**: How often are AI-executed actions (refunds, cancellations)
   reversed by humans afterward? (target: < 2%)
6. **Customer effort score**: How many messages did the customer need to send? Fewer is better.
7. **Sentiment trajectory**: Did sentiment improve during the conversation? (positive slope = good)

These metrics together paint a much richer picture than 'resolved yes/no.'"

---

## Advanced Discussion: Agent Operating Procedures (2025-2026 Trend)

**When the interviewer asks about the future of AI customer service:**

"The most significant architectural trend is the shift from prompt-engineered agents
to declarative Agent Operating Procedures (AOPs). Instead of writing 'You are a helpful
customer service agent...' in a prompt, you define structured behavior specifications:

```
AOP: handle_subscription_cancellation
  trigger: intent=cancel_subscription, confidence>0.85
  context_required: [subscription_details, billing_history, retention_offers]
  steps:
    1. verify_identity(method=step_up_auth)
    2. fetch_subscription(include=billing_history)
    3. IF eligible_for_retention_offer:
         present_retention_offer(tier=customer.tier)
         IF customer_accepts: apply_offer, END
    4. confirm_cancellation(show=remaining_value, effective_date)
    5. execute_cancellation
    6. send_confirmation(channel=email)
  constraints:
    - never process without identity verification
    - never cancel if subscription is under contract (inform customer)
    - max_retention_offers_per_session: 1
  fallback: escalate_to_retention_team
```

This is significant because:
1. **Testable**: You can unit-test AOPs with mock conversations
2. **Auditable**: Behavior changes are git diffs, not prompt rewording
3. **Non-engineer editable**: Support leads can modify within constrained editors
4. **Deterministic guardrails**: Constraints are enforced by the interpreter, not the LLM"
...
| # | Topic | Connection |
|---|-------|------------|
| 1 | [3.17 AI Agent Orchestration](../3.17-ai-agent-orchestration-platform/00-index.md) | Foundation architecture — how agents orchestrate tools and actions |
| 2 | [3.22 AI Guardrails & Safety](../3.22-ai-guardrails-safety-system/00-index.md) | How to prevent agents from taking harmful autonomous actions |
| 3 | [3.15 RAG System](../3.15-rag-system/00-index.md) | Knowledge retrieval pipeline powering agent answers |
| 4 | [3.34 Personalization Engine](../3.34-ai-native-real-time-personalization-engine/00-index.md) | Adapting service responses to customer preferences |
| 5 | [3.28 AI Memory Management](../3.28-ai-memory-management-system/00-index.md) | Cross-session context — remembering customer history |
| 6 | [1.18 Event Sourcing](../1.18-event-sourcing-system/00-index.md) | Immutable audit trail for all agent actions |
| 7 | [3.24 Multi-Agent Orchestration](../3.24-multi-agent-orchestration-platform/00-index.md) | Specialized agents collaborating on complex cases |
| 8 | [3.25 AI Observability & LLMOps](../3.25-ai-observability-llmops-platform/00-index.md) | Monitoring AI agent quality and cost |
