...
| Option | Pros | Cons |
|--------|------|------|
| **Centralized KB** | Single source of truth, consistent | Slowest part of the process, complex governance |
...
### Decision 7: Omnichannel Message Normalization

| Option | Pros | Cons |
|--------|------|------|
| **Channel-specific pipelines** | Optimized per channel | O(N×M) integration complexity |
| **Canonical envelope format** | Channel-agnostic AI pipeline | Loses channel-specific features |
| **Hybrid normalization** | Canonical core + channel extensions | Extension field complexity |

**Decision:** `Canonical Envelope with Channel Extensions`

**Rationale:**
- All channels normalize into a single canonical format at the gateway boundary
- Core fields: message text, customer identity, thread reference, modality hints
- Channel-specific metadata preserved in an opaque extension field
- Adding a new channel requires only one adapter — zero changes to AI pipeline
- Voice produces ASR transcript + confidence; email extracts body + threading; social preserves reaction semantics
- This reduces integration from O(N×M) to O(N+M) where N=channels and M=AI components
- See [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) for the analogous event normalization pattern

### Integration Points (Cross-System)

| System | Integration | Data Flow |
|--------|-------------|-----------|
| [3.15 RAG System](../3.15-rag-system/00-index.md) | Knowledge retrieval | Query → vector search → ranked chunks → response grounding |
| [3.22 AI Guardrails](../3.22-ai-guardrails-safety-system/00-index.md) | Action safety | Action plan → guardrail validation → approve/deny/modify |
| [3.34 Personalization Engine](../3.34-ai-native-real-time-personalization-engine/00-index.md) | Response personalization | Customer embedding → tone/formality/detail level adaptation |
| [1.18 Event Sourcing](../1.18-event-sourcing-system/00-index.md) | Audit trail | All actions → immutable event log → replay/compliance |
