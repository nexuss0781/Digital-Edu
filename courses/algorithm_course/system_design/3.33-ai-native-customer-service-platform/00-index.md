...
| **Human Override** | Customer service supervisors monitor AI conversations; agents handle escalations with full context transfer | Domain expert |
...
| Company | Platform | Results | Scale/Impact | Use Case |
|---------|----------|---------|--------------|----------|
| **Weight Watchers** | Sierra AI | 70% AI handling, 4.6/5 CSAT | 1M+ members served | Meal choices, membership management |
| **OluKai** | Sierra AI | 50%+ cases during Black Friday | 10x traffic spike handled | Peak season scaling |
| **SiriusXM** | Sierra AI | Complex subscription management | Multi-tier plan changes | Enterprise loyalty |
| **Duolingo** | Decagon | 80%+ deflection rate | 50M+ active learners | Learning platform support |
| **ClassPass** | Decagon | 95% cost reduction | Booking/cancellation automation | Fitness booking support |
| **NG.CASH** | Decagon | 13% → 70% autonomous resolution | 5x resolution improvement | Fintech support |
| **Lightspeed Commerce** | Intercom Fin | 99% participation, 65% autonomous | Multi-country commerce | Commerce platform |
...
|-------|-------------|------------------------|
| **Agentic Action Depth** | Agents evolving from single-action (refund) to multi-step workflows (investigate → diagnose → resolve → follow-up) | Requires stateful workflow engines, not just LLM chains |
| **Voice-First AI** | Sub-300ms voice AI enabling real-time phone conversations without human agents | Edge inference, streaming ASR/TTS, interrupt handling |
| **Agent Operating Procedures** | Declarative behavior specifications replacing prompt engineering for agent configuration | Version-controlled, testable, auditable agent behavior |
| **Proactive Outreach** | AI initiating customer contact based on predicted issues (shipment delay, billing error) | Event-driven triggers, tone-appropriate outbound messaging |
| **Multi-Agent Collaboration** | Specialized agents (billing, technical, account) collaborating on complex cases | Agent handoff protocols, shared context, conflict resolution |
| **On-Premise LLM Deployment** | Regulated industries running fine-tuned models on-premise for data sovereignty | Hybrid architecture, model distillation, edge deployment |
...
| System | Relationship |
|--------|--------------|
| [3.17 AI Agent Orchestration Platform](../3.17-ai-agent-orchestration-platform/00-index.md) | Foundation for agentic architecture — orchestrates intent-action-response cycles |
| [3.24 Multi-Agent Orchestration Platform](../3.24-multi-agent-orchestration-platform/00-index.md) | Advanced multi-agent coordination for specialized domain agents |
| [3.22 AI Guardrails & Safety System](../3.22-ai-guardrails-safety-system/00-index.md) | Safety rails preventing autonomous agents from taking harmful actions |
| [3.25 AI Observability & LLMOps Platform](../3.25-ai-observability-llmops-platform/00-index.md) | Monitoring AI agent performance, hallucination detection, cost tracking |
| [3.15 RAG System](../3.15-rag-system/00-index.md) | Knowledge retrieval powering agent responses and answer grounding |
| [3.28 AI Memory Management System](../3.28-ai-memory-management-system/00-index.md) | Cross-session context enabling customers to resume without repeating |
| [3.34 Real-Time Personalization Engine](../3.34-ai-native-real-time-personalization-engine/00-index.md) | Personalizing service responses based on customer behavior and preferences |
| [1.18 Event Sourcing System](../1.18-event-sourcing-system/00-index.md) | Immutable event log for conversation audit trail and replay |
