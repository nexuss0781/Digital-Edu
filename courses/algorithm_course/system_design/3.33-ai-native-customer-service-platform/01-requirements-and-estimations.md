...
|-----------|---------------|-----------|----------------|------------|
| Conversation text | PII-Containing | 90 days active, 2 years archived | Role-based, per-tenant | AES-256 at rest, TLS 1.3 in transit |
| Voice recordings | Sensitive PII | 30 days default, configurable | Explicit consent + role-based | AES-256 at rest, encrypted streams |
| Customer profiles | PII | Duration of service + 30 days | Service-level isolation | Field-level encryption for SSN, payment |
| Intent/sentiment analytics | Derived Data | 1 year aggregated | Analytics team only | Anonymized after 30 days |
| LLM interaction logs | Contains PII | 14 days for debugging | Engineering on-call only | Redacted PII before storage |
| Action audit trail | Compliance | 7 years (regulatory) | Compliance team + legal | Immutable, tamper-evident |

### Real-Time Data Subject Rights

| Right | SLA | Implementation |
|-------|-----|----------------|
| **Right to Access** | < 72 hours | Export conversation history, profile, actions taken |
| **Right to Erasure** | < 24 hours for active data | Cascade delete: sessions → messages → embeddings → analytics |
| **Right to Rectification** | < 48 hours | Update customer profile, annotate affected conversations |
| **Right to Restrict Processing** | Immediate | Flag account, route all interactions to human agents |
| **Right to Data Portability** | < 72 hours | Export in machine-readable format (JSON) |

### Disaster Scenario Cost Analysis

| Scenario | Probability | Revenue Impact | Recovery Cost | Mitigation Investment |
|----------|-------------|----------------|---------------|----------------------|
| LLM provider outage (4h) | Medium (2x/year) | $180K lost resolution capacity | $25K incident response | $50K multi-provider failover |
| Data breach (PII exposure) | Low (0.1/year) | $5-50M regulatory fines | $2M incident response + remediation | $200K security hardening |
| Voice system failure (2h) | Medium (3x/year) | $90K call overflow to competitors | $15K per incident | $75K redundant voice infrastructure |
| AI safety incident (wrong action) | Low-Medium | $500K-5M brand damage | $100K customer remediation | $150K guardrail system |
| Knowledge base corruption | Low | 40% resolution rate drop | $50K emergency re-indexing | $30K version control + hot-swap |
