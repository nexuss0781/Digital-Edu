...
|------|--------|-------------------|-----------------|
| **LLM Provider Failure** | Block outbound to primary LLM | Failover to secondary provider within 5s | < 10s total |
| **Redis Cluster Node Loss** | Kill 1 of 6 Redis nodes | Cluster rebalances, sessions continue | < 30s |
| **Database Failover** | Promote read replica | Write traffic redirects, < 5s stale reads | < 15s |
| **Kafka Broker Loss** | Kill 1 of 6 brokers | Partition rebalance, no message loss | < 60s |
| **Intent Service Crash** | Kill all intent pods | HPA restarts, fallback to rule-based routing | < 30s |
| **Network Partition** | Block inter-service traffic | Circuit breaker, graceful degradation | Immediate |
| **Voice Pipeline Failure** | Disable ASR service | Redirect to text channel, inform customer | < 10s |

### Load Testing Strategy

```
1. Steady State Test:
   - Target: 1.5x normal peak QPS (1,400 messages/sec)
   - Duration: 4 hours
   - Verify: latency p95 < 2s, error rate < 0.1%, resolution rate stable

2. Spike Test:
   - Ramp: 0 → 10x normal QPS in 30 seconds
   - Simulate: Black Friday morning surge
   - Verify: auto-scaling triggers within 60s, no dropped messages

3. Soak Test:
   - Target: 1.2x peak QPS
   - Duration: 48 hours
   - Verify: no memory leaks, no degradation, connection pool stability

4. Multi-Channel Stress Test:
   - Simulate: simultaneous spike across chat + voice + email
   - Target: 5K concurrent voice + 20K concurrent chat + 10K email queue
   - Verify: channel isolation (voice failure doesn't affect chat)
```

### Conflict Resolution for Multi-Region Writes

```
Problem: Customer starts conversation in US-East, switches device
         and routes to EU-West — two regions have conflicting session state.

Solution: Last-Writer-Wins with Causal Ordering

1. Vector Clocks:
   - Each message carries a vector clock: {region: logical_time}
   - Concurrent writes detected when clocks are incomparable

2. Resolution Strategy:
   FOR conflicting session state:
     IF one region has more recent customer message → use that region
     IF both have customer messages → merge conversation histories
     IF action in progress → coordinate via distributed lock

3. Customer Experience:
   - Customer never sees a conflict
   - Merged context appears as continuous conversation
   - Actions are never duplicated (idempotency keys)
```
