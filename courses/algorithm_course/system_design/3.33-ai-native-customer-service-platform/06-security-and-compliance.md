...
### Adversarial Customer Manipulation

**Threat:** Sophisticated customers discover they can manipulate AI agents through specific conversational patterns to obtain unauthorized benefits (extra refunds, policy exceptions, free upgrades).

**Detection Signals:**
```
1. Pattern Recognition:
   - Repeated refund requests across multiple sessions with different reasons
   - Conversation steering: customer redirects dialogue toward high-value actions
   - Emotional manipulation: sudden sentiment shifts to trigger escalation advantages
   - Social proof injection: "Your competitor always does this for me"

2. Behavioral Anomaly Detection:
   - Customer action-to-conversation ratio significantly above baseline
   - Average refund value > 2σ above customer segment mean
   - Session count increase correlated with successful actions
   - Cross-account patterns (same IP, device fingerprint, payment method)

3. Mitigation:
   - Per-customer action limits (daily/weekly/monthly caps)
   - Anomaly flags that trigger human review before action execution
   - Abuse score computed across sessions (not just current conversation)
   - Policy-aware guardrails: "This customer has received 3 refunds this month"
```

### Conversation Data Leakage via LLM Context

**Threat:** Customer A's conversation data leaks into Customer B's response through shared LLM context or cached prompts.

**Mitigation Architecture:**
```
1. Session Isolation:
   - Each LLM call receives ONLY the current customer's conversation
   - System prompt is tenant-specific (no shared prompt cache across customers)
   - Response cache keyed by: {tenant_id, intent, entity_hash} — never cross-tenant

2. Output Sanitization:
   - Post-generation PII scan on EVERY response before delivery
   - Regex + NER model detects: names, emails, phone numbers, order IDs, addresses
   - If PII detected that doesn't belong to current customer → block and regenerate

3. LLM Provider Controls:
   - Opt-out of training data usage (all major providers)
   - Zero-retention agreements for conversation content
   - On-premise LLM option for regulated industries (healthcare, finance)
```

### Agent Operating Procedure (AOP) Security

**Threat:** Malicious or poorly tested AOP definitions allow agents to take actions beyond intended scope.

**Controls:**
```
1. AOP Validation Pipeline:
   - Static analysis: verify all referenced actions exist in allow-list
   - Constraint verification: monetary limits, rate limits defined
   - Simulation: run AOP against 100 test conversations before deployment
   - Diff review: mandatory code review for AOP changes

2. Runtime Enforcement:
   - AOP interpreter enforces declared constraints (not the LLM)
   - Action calls must match AOP step definitions
   - Undeclared actions are blocked regardless of LLM output
   - Audit log: which AOP, which step, what action, what result
```
