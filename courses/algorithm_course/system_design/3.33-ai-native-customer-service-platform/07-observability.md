...
## Runbook: Resolution Rate Drop

### Symptoms
- `autonomous_resolution_rate` drops below 55% (normal: 65-70%)
- `escalation_rate` increases above 40%
- No change in traffic volume or intent distribution

### Diagnosis Steps

```
1. Check Intent Detection Quality:
   QUERY: avg(intent_confidence_score) over last 1h vs last 7d baseline
   IF confidence dropped > 5%:
     → Check if model was recently updated
     → Check if knowledge base was re-indexed
     → Check for new intent categories with insufficient training data

2. Check Knowledge Base Health:
   QUERY: avg(rag_relevance_score) over last 1h vs last 7d baseline
   IF relevance dropped:
     → Check recent KB updates (new content may have poor chunking)
     → Check embedding model version (was it changed?)
     → Run golden test queries against KB

3. Check Action Success Rate:
   QUERY: action_success_rate by action_type over last 1h
   IF specific action type failing:
     → Check backend API health for that action
     → Check for rate limiting from backend systems
     → Check for schema changes in backend responses

4. Check Customer Mix:
   QUERY: conversation distribution by complexity tier
   IF shift toward complex conversations:
     → Possibly seasonal (holiday, product launch)
     → Not a system issue — adjust thresholds temporarily
```

### Resolution Steps

| Root Cause | Action | Recovery Time |
|------------|--------|---------------|
| Model regression | Rollback to previous model version | 5 minutes |
| KB quality degradation | Rollback KB index to previous version | 2 minutes |
| Backend API failure | Enable circuit breaker, escalate to backend team | 15 minutes |
| New intent category | Add training data, retrain, deploy — OR lower confidence threshold | 2-24 hours |
| Seasonal shift | Adjust escalation thresholds, pre-scale human agents | Immediate |
...
slo_burn_rate:
  resolution_rate_slo:
    target: 0.65  # 65% autonomous resolution
    error_budget_monthly: 0.35  # 35% can escalate

    alerts:
      - name: "resolution_rate_fast_burn"
        windows: [1h, 5m]
        burn_rate: 14.4
        severity: "page"
        description: "At this rate, monthly error budget exhausted in 2 days"

      - name: "resolution_rate_slow_burn"
        windows: [6h, 30m]
        burn_rate: 6.0
        severity: "page"
        description: "At this rate, monthly error budget exhausted in 5 days"

      - name: "resolution_rate_chronic"
        windows: [3d, 6h]
        burn_rate: 1.0
        severity: "ticket"
        description: "Slow degradation — investigate within 24 hours"

  latency_slo:
    target_p95: 2.0  # 2 seconds
    error_budget_monthly: 0.05  # 5% can exceed
  ...
  action_success_slo:
    target: 0.98  # 98% action success
    error_budget_monthly: 0.02

    alerts:
      - name: "action_failure_fast_burn"
        windows: [1h, 5m]
        burn_rate: 14.4
        severity: "page"
        description: "Actions failing at alarming rate — check backend APIs"
```

### Customer Experience SLO

```
SLO: "90% of conversations should resolve in a single session"

Measurement:
  numerator: conversations with resolution_status = "resolved" AND session_count = 1
  denominator: all conversations with resolution_status = "resolved"

Error budget: 10% can require multiple sessions

Burn rate alert:
  IF multi_session_resolution_rate > 15% over 6h window:
    PAGE on-call
    ACTION: check if context persistence is failing (session expiry issue)
```
