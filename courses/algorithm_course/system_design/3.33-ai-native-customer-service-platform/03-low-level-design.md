...
## Algorithm 8: Agent Operating Procedure Interpreter

**Purpose:** Execute declarative AOPs with deterministic constraint enforcement, separating the LLM reasoning from the procedural guardrails.

```
FUNCTION execute_aop(aop: AgentOperatingProcedure, context: ConversationContext):
    // Step 1: Validate trigger conditions
    IF NOT matches_trigger(aop.trigger, context.current_intent):
        RETURN {status: "not_applicable"}

    // Step 2: Fetch required context
    FOR EACH source IN aop.context_required:
        data = fetch_context_data(source, context.customer_id)
        IF data IS NULL AND source.required:
            RETURN {status: "missing_context", source: source.name}
        context.enriched_data[source.name] = data

    // Step 3: Execute steps sequentially with constraint checking
    execution_log = []
    FOR EACH step IN aop.steps:
        // Check constraints before every action
        FOR EACH constraint IN aop.constraints:
            IF violates_constraint(constraint, context, execution_log):
                RETURN {
                    status: "constraint_violated",
                    constraint: constraint.description,
                    fallback: aop.fallback
                }
        ...
        // Execute the step
        IF step.type == "verify":
            result = execute_verification(step, context)
        ELSE IF step.type == "fetch":
            result = execute_data_fetch(step, context)
        ELSE IF step.type == "present":
            result = generate_response(step.template, context)
            YIELD result  // Stream to customer
            customer_response = AWAIT customer_input()
            context.update(customer_response)
        ELSE IF step.type == "execute":
            // Action execution with idempotency
            idempotency_key = hash(aop.id, context.conversation_id, step.id)
            result = execute_action(step.action, context, idempotency_key)
        ELSE IF step.type == "confirm":
            result = generate_confirmation(step, context, execution_log)
            YIELD result
        ...
        // Check if step result triggers early exit
        IF result.status == "customer_declined":
            BREAK
        IF result.status == "error" AND step.critical:
            RETURN execute_fallback(aop.fallback, context, execution_log)

    RETURN {
        status: "completed",
        execution_log: execution_log,
        actions_taken: filter(execution_log, type="execute")
    }
```

**Constraint Enforcement Examples:**
```
// Constraints are evaluated deterministically, NOT by the LLM
FUNCTION violates_constraint(constraint, context, log):
    MATCH constraint.type:
        CASE "monetary_limit":
            total_amount = sum(log.filter(type="execute").map(a → a.amount))
            RETURN total_amount + pending_amount > constraint.limit

        CASE "action_count":
            action_count = count(log.filter(type="execute", action=constraint.action))
            RETURN action_count >= constraint.max_count

        CASE "require_verification":
            RETURN NOT context.identity_verified

        CASE "business_rule":
            RETURN evaluate_rule(constraint.rule, context.enriched_data)
```
