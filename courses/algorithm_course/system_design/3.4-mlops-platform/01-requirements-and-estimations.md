...
### LLMOps Features (P1 - 2025-2026 Convergence)

| Feature | Description | Priority |
|---------|-------------|----------|
| **LoRA Adapter Registry** | Version and manage parameter-efficient fine-tuning adapters as first-class artifacts | P1 |
| **Prompt Versioning** | Track prompt templates with version control, A/B variant metadata, and evaluation scores | P1 |
| **Evaluation Dataset Management** | Version and curate evaluation datasets (golden sets, adversarial sets) alongside models | P1 |
| **Foundation Model Catalog** | Registry of base models (open-weight and proprietary) with license, capability, and cost metadata | P1 |
| **Fine-Tuning Pipeline Templates** | Pre-built pipelines for SFT, RLHF, DPO, and knowledge distillation workflows | P1 |
| **Token-Level Cost Tracking** | Track training and evaluation costs at token granularity for LLM experiments | P1 |
| **Multi-GPU Topology Scheduling** | Schedule distributed fine-tuning across NVLink-connected GPU pairs for optimal NCCL performance | P1 |
...
| **Compound AI System Pipelines** | DAGs with mixed model-retriever-model chains for RAG and agent workflows | P2 |
| **Cross-Cloud Training** | Execute pipeline stages across multiple cloud providers for GPU capacity | P2 |
