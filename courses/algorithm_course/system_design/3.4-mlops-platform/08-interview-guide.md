...
1. STATEFUL COMPONENTS
   ...
   Database primary is a Slowest part of the process until you shard.
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  LLMOPS CONVERGENCE (2025-2026):                                     │
│  • LoRA adapters as first-class registry artifacts                   │
│  • Prompt versioning alongside model versioning                      │
│  • GPU topology-aware scheduling for distributed fine-tuning         │
│  • Evaluation datasets versioned with models                         │
│  • Token-level cost tracking for LLM experiments                     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
## LLMOps-Specific Interview Questions

### Trade-offs

| Decision | Option A | Option B | Recommendation |
|----------|----------|----------|----------------|
| **LoRA vs Full Fine-Tune** | Full fine-tuning (all weights) | LoRA (adapter weights only) | LoRA for most use cases: 100x smaller artifacts, faster training, composable |
| **Adapter as model version vs separate artifact** | Store adapter as new model version | Store adapter as linked artifact to base model | Separate artifact with base model reference: enables adapter composition |
| **Prompt in code vs registry** | Prompts versioned in git with application code | Prompts versioned in model registry | Registry: enables A/B testing, evaluation tracking, and decoupled deployment |
| **Single vs multi-GPU fine-tuning** | Single large GPU (A100 80GB) | Multi-GPU distributed (4×A100 40GB) | Multi-GPU with topology awareness: better availability, enables larger models |

### Trap Questions

| Trap Question | What Interviewer Wants | Best Answer |
|---------------|------------------------|-------------|
| "Why not just use git for model versioning?" | Test artifact management understanding | "Git handles code well but not 100GB+ model binaries. Model registry provides atomic alias updates, stage transitions with governance, and lineage tracking that git lacks. Git LFS helps but does not provide alias semantics or approval workflows." |
| "How does LLMOps change the MLOps platform?" | Test awareness of industry evolution | "LLMOps adds new artifact types (adapters, prompts, eval datasets), requires GPU topology-aware scheduling, and introduces token-level cost tracking. The core patterns (versioning, lineage, governance) remain the same, but the registry schema, scheduler, and cost model all need extension." |
| "Why separate experiment tracking from model registry?" | Test component design reasoning | "Experiment tracking is write-heavy, eventually consistent, and stores ephemeral run data. Model registry is read-heavy, strongly consistent, and stores production-critical metadata. Different access patterns, consistency requirements, and storage backends." |
