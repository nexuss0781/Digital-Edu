...
### Slowest part of the process 1: Metric Write Throughput
...
### Slowest part of the process 2: Large Model Artifact Upload
...
### Slowest part of the process 3: Experiment Search Performance
...
### Slowest part of the process 4: Scheduler State Management

**Problem:** With 1000+ concurrent jobs, scheduler state can become a Slowest part of the process.
...
## Critical Component 4: LLMOps Extension Architecture

### Why Critical

The convergence of LLMOps with traditional MLOps is the defining platform evolution of 2025-2026. Foundation model fine-tuning, adapter management, and prompt versioning introduce artifact types and lifecycle patterns that the traditional model registry was not designed for.

### LLM Artifact Taxonomy

```
ARTIFACT TYPES IN LLMOPS:

┌─────────────────────────────────────────────────────────────────┐
│                    LLM ARTIFACT HIERARCHY                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  BASE MODEL (Foundation)                                        │
│  ───────────────────────────────────────────────────────────── │
│  • Full model weights (7B-70B+ parameters, 15-150GB)           │
│  • Tokenizer configuration + vocabulary                         │
│  • License metadata (Apache 2.0, Llama Community, etc.)        │
│  • Capability benchmarks (MMLU, HumanEval, etc.)               │
│  Versioning: Immutable, referenced by model card               │
│                                                                 │
│  LoRA ADAPTER (Parameter-Efficient Fine-Tune)                   │
│  ───────────────────────────────────────────────────────────── │
│  • Adapter weights (0.1-1% of base model size, 50-500MB)       │
│  • Target modules configuration (q_proj, v_proj, etc.)         │
│  • Base model reference (which foundation model)               │
│  • Training dataset reference + hyperparameters                │
│  Versioning: Per-adapter, linked to base model version         │
│                                                                 │
│  PROMPT TEMPLATE                                                │
│  ───────────────────────────────────────────────────────────── │
│  • System prompt + user prompt template                        │
│  • Variable slots and validation rules                         │
│  • Associated model + adapter combination                      │
│  • Evaluation scores on golden dataset                         │
│  Versioning: Git-style with diff tracking                      │
│                                                                 │
│  EVALUATION DATASET                                             │
│  ───────────────────────────────────────────────────────────── │
│  • Golden test set (curated input-output pairs)                │
│  • Adversarial test set (edge cases, safety probes)            │
│  • Domain-specific benchmarks                                  │
│  Versioning: Immutable snapshots with lineage                  │
│                                                                 │
│  COMPOUND AI CONFIGURATION                                      │
│  ───────────────────────────────────────────────────────────── │
│  • Model + adapter + prompt + retriever configuration           │
│  • RAG pipeline parameters (chunk size, top_k, reranker)       │
│  • Agent tool definitions and routing rules                    │
│  Versioning: Atomic configuration snapshots                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Extended Model Registry for LLMOps

```
ALGORITHM: LLMArtifactRegistration

FUNCTION register_llm_artifact(artifact_type, metadata, artifact_path):
    MATCH artifact_type:
        "base_model":
            // Validate license compatibility
            validate_license(metadata.license, org_policy)

            // Register with capability benchmarks
            entry = {
                "type": "base_model",
                "name": metadata.model_name,
                "parameter_count": metadata.param_count,
                "architecture": metadata.architecture,
                "license": metadata.license,
                "benchmarks": metadata.benchmarks,
                "artifact_path": artifact_path,
                "artifact_size_bytes": get_size(artifact_path),
                "artifact_checksum": sha256(artifact_path)
            }

        "lora_adapter":
            // Validate base model exists
            base = registry.get(metadata.base_model_ref)
            IF base IS NULL:
                ERROR "Base model not found"

            // Validate adapter compatibility
            validate_adapter_compatibility(metadata, base)
            ...
            entry = {
                ...
                "alpha": metadata.lora_alpha,
                "target_modules": metadata.target_modules,
                "training_dataset_ref": metadata.dataset_ref,
                "artifact_path": artifact_path,
                "artifact_checksum": sha256(artifact_path)
            }
        ...
        "prompt_template":
            entry = {
                ...
                "model_adapter_ref": metadata.model_ref,
                "eval_scores": metadata.evaluation_results
            }

    // Store with standard versioning
    version = registry.create_version(entry)

    // Publish event for downstream consumers
    events.publish("artifact.registered", {
...
### GPU Topology-Aware Scheduling for LLM Fine-Tuning

```
ALGORITHM: TopologyAwareGPUScheduling

CHALLENGE:
  Distributed fine-tuning of a 70B model across 8 GPUs requires
  specific GPU interconnect topology for optimal NCCL performance.
  NVLink-connected GPU pairs communicate 10x faster than PCIe.

FUNCTION schedule_distributed_training(job):
    required_gpus = job.gpu_count  // e.g., 8
    model_size_gb = job.model_size_gb  // e.g., 140 (70B × 2 bytes)
    gpu_memory_required = estimate_memory(
        model_size_gb, job.batch_size, job.optimizer
    )

    // Step 1: Find nodes with sufficient GPU count and memory
    candidates = []
    FOR EACH node IN gpu_cluster.nodes:
        available = node.available_gpus(gpu_memory_required)
        IF len(available) >= required_gpus:
            candidates.append(node)

    // Step 2: Score by topology quality
    scored = []
    FOR EACH node IN candidates:
        topology_score = evaluate_topology(
            node, required_gpus
        )
        // Prefer NVLink-connected GPU groups
        // Score: 1.0 = all NVLink, 0.5 = mixed, 0.0 = PCIe only

        cost_score = calculate_cost_score(node)
        // Prefer spot instances for checkpointable jobs

        locality_score = data_locality_score(node, job.dataset)

        total = (topology_score * 0.5) +
                (cost_score * 0.3) +
                (locality_score * 0.2)
    ...
    best_node = max(scored, key=lambda x: x[1])

    // Step 4: Allocate specific GPUs with optimal topology
    gpus = select_optimal_gpu_subset(
        best_node, required_gpus, gpu_memory_required
    )

    RETURN {
        "node": best_node,
        "gpus": gpus,
        "nccl_config": generate_nccl_config(gpus)
    }

FUNCTION evaluate_topology(node, required_gpus):
    // Get NVLink connectivity matrix
    nvlink_matrix = node.gpu_topology.nvlink_connections

    // Find the largest NVLink-connected subgroup
    // that satisfies required_gpus
    groups = find_nvlink_groups(nvlink_matrix)

    FOR group IN sorted(groups, key=len, reverse=True):
        IF len(group) >= required_gpus:
            RETURN 1.0  // Full NVLink connectivity

    // Partial NVLink (some GPUs PCIe-connected)
    max_nvlink = max(len(g) for g in groups)
    RETURN max_nvlink / required_gpus
```

---

## Case Study: Scaling Experiment Tracking for Foundation Model Training

### Problem

A research organization training foundation models ran 500+ distributed training experiments per week, each producing 50,000+ metric data points (per-step loss, per-layer gradient norms, learning rate schedules). The existing MLflow-based tracking system:

- Collapsed under 10M+ metric writes per day
- Dashboard queries took 30+ seconds for cross-run comparison
- Metric storage grew to 5TB in 3 months
- API latency spiked to 5s during peak training hours

### Architecture Evolution

```
PHASE 1: Direct Write Architecture (Failed at Scale)
  SDK → MLflow API → PostgreSQL (metrics table)
  Problem: PostgreSQL not designed for time-series write throughput

PHASE 2: Buffered Write with ClickHouse (Current)
  SDK → Client Buffer (100ms) → Tracking API → Server Buffer → ClickHouse
  Result: 100x write throughput improvement

PHASE 3: Tiered Storage with Pre-Computed Views
  Hot (7 days): ClickHouse SSD
  Warm (90 days): ClickHouse HDD
  Cold (1+ year): Parquet in Object Storage
  Materialized Views: metric_latest, metric_summary
  Result: Dashboard queries from 30s to <500ms
```
...
|--------|--------|-------|-------------|
| Metric write throughput | 5K/s | 500K/s | 100x |
| Dashboard load time | 30s | 400ms | 75x |
| API p99 latency | 5s | 80ms | 62x |
| Storage cost (monthly) | $15K | $4K | 73% reduction |
| Max concurrent experiments | 100 | 5,000+ | 50x |

### Key Lessons

1. **Time-series metrics need time-series databases**: PostgreSQL is excellent for metadata and registry but wrong for high-cardinality metric ingestion
2. **Client-side batching is non-negotiable at scale**: Without SDK-level buffering, the API surface area grows linearly with worker count
3. **Materialized views replace 90% of queries**: Most dashboard interactions need aggregations, not raw data
4. **Tiered storage follows the access pattern**: Recent experiments need fast queries; old experiments need cheap storage
