...
| Requirement | Implementation |
|------------|---------------|
| Cardholder data isolation | Payment card columns excluded via `column.exclude.list`; never captured by CDC |
| Network segmentation | CDC workers processing payment topics in isolated network segment |
| Encryption | TLS 1.3 for all data in transit; AES-256 at rest; field-level tokenization for any residual card data |
| Access logging | All access to payment CDC topics logged with identity, timestamp, and operation |
| Key rotation | Encryption keys rotated every 90 days; old keys retained for decryption of retained events |
...
|-------|------------|-----------|
| Connector creation | Who created, configuration (sanitized), source DB, tables captured | 2 years |
| Connector configuration change | Who changed, diff of old/new config, timestamp | 2 years |
| Offset manipulation | Who reset, old/new offset values, reason | 2 years |
| Schema registration | Who/what registered, subject, version, compatibility result | Indefinite |
| Topic ACL change | Who changed, principal, permission, resource | 2 years |
| Dead-letter event | Source table, event key (redacted), failure reason, timestamp | 90 days |
| Replication slot creation/drop | Slot name, database, who initiated, reason | 2 years |

### Forensic Investigation Support

```
FUNCTION investigate_data_exposure(table, time_range):
    // Determine who had access to this table's data via CDC

    // Step 1: Which connectors captured this table?
    connectors = query_config_history(
        filter = "table.include.list CONTAINS '{table}'",
        time_range = time_range
    )

    // Step 2: Which topics received events?
    topics = [compute_topic_name(c, table) FOR c IN connectors]

    // Step 3: Who consumed from those topics?
    consumers = []
    FOR EACH topic IN topics:
        acls = query_acl_history(resource = topic, time_range = time_range)
        consumer_groups = query_consumer_group_history(topic, time_range)
        consumers.extend(acls + consumer_groups)

    // Step 4: Were PII masking rules applied?
    masking_rules = query_transform_config(connectors, time_range)
    pii_exposure = NOT all(has_pii_masking(rule, table) FOR rule IN masking_rules)

    RETURN {
        connectors, topics, consumers, masking_rules,
        pii_exposure_risk: pii_exposure,
        recommendation: "Review consumer access logs for topic-level data exposure"
    }
```
...
|----------|---------|----------|--------|
| **Network** | CDC workers in private subnet, no public internet | Critical | |
| **Network** | mTLS between all components (workers ↔ brokers ↔ registry) | Critical | |
| **Network** | Source DB firewall allowlist for CDC worker IPs only | Critical | |
| **Authentication** | Dedicated CDC database user with minimal privileges | Critical | |
| **Authentication** | OAuth 2.0 / mTLS for management API | High | |
| **Authorization** | PostgreSQL publication filtering for WAL access | Critical | |
| **Authorization** | Topic-level ACLs for all CDC topics | Critical | |
| **Authorization** | Schema registry RBAC (connectors: write; consumers: read) | High | |
| **Data Protection** | PII masking in transform layer before streaming platform | Critical | |
| **Data Protection** | Column exclusion for passwords, secrets, tokens | Critical | |
| **Data Protection** | Dead-letter topics with same masking rules as source topics | High | |
| **Key Management** | Database credential rotation (90-day maximum) | High | |
| **Key Management** | mTLS certificate rotation (30-day maximum) | High | |
| **Key Management** | Per-topic encryption keys managed by external KMS | Medium | |
| **Monitoring** | Replication slot connection monitoring (unexpected clients) | Critical | |
| **Monitoring** | Configuration change alerting (sensitive field changes) | High | |
| **Monitoring** | Dead-letter topic growth alerting | Medium | |
| **Compliance** | GDPR: tombstone events for right-to-delete | High | |
| **Compliance** | SOC 2: audit logging for all connector operations | High | |
| **Compliance** | HIPAA: PHI in separate topics with field-level encryption | If applicable | |
