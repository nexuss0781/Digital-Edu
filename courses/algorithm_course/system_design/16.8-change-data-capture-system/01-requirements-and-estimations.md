...
### End-to-End Change Propagation Path

| Phase | Component | P50 Latency | P99 Latency | Notes |
|-------|-----------|-------------|-------------|-------|
| 1 | Source DB commit → WAL entry visible | < 1 ms | < 5 ms | Database-internal; depends on fsync settings |
| 2 | WAL entry → connector reads entry | 10-50 ms | 100 ms | Depends on polling interval and WAL throughput |
| 3 | Connector deserialize + schema resolve | 1-5 ms | 20 ms | Schema cache hit: < 1 ms; cache miss: 5-20 ms |
| 4 | Connector serialize (Avro) + publish | 5-20 ms | 50 ms | Batching adds latency; larger batches = higher latency |
| 5 | Streaming platform replicate + ack | 5-15 ms | 30 ms | Depends on replication factor and acks=all configuration |
| 6 | Consumer poll + deserialize | 10-50 ms | 200 ms | Consumer poll interval is the dominant factor |
| 7 | Consumer process + write to sink | 5-100 ms | 500 ms | Varies widely by sink type (cache: 5ms, search: 50ms, warehouse: 100ms) |
| **Total** | **End-to-end** | **36-241 ms** | **905 ms** | **Dominated by consumer poll interval and sink write speed** |

### Warm vs. Cold Path Latency

| Path | Scenario | Expected Latency | Limiting Factor |
|------|----------|-----------------|-----------------|
| **Warm path** | Steady-state streaming; schema cached; consumer polling | 50-200 ms | Consumer poll interval |
| **Cold start** | Connector just started; schema cache empty; consumer offset at latest | 5-30 seconds | Schema registry cold lookup + initial rebalancing |
| **Post-snapshot** | Transitioning from snapshot to streaming; dedup window active | 200 ms - 2 seconds | Snapshot-to-streaming dedup overhead |
| **Recovery** | Connector restarted after crash; resuming from last committed offset | 15-30 seconds | Worker rebalancing + offset lookup + connection re-establishment |

---

## Key Sizing Formulas

### Streaming Platform Storage

```
hot_storage_bytes = events_per_sec × avg_event_bytes × retention_seconds × replication_factor
                  = 100,000 × 1,024 × 604,800 × 3
                  = ~180 TB

broker_count = ceil(hot_storage_bytes / disk_per_broker)
             = ceil(180 TB / 15 TB)
             = 12 brokers
```

### Connector Worker Memory

```
per_connector_memory = schema_cache + offset_cache + serializer_cache + event_buffer
                     = 10 MB + 1 KB + 50 MB + 256 MB
                     ≈ 316 MB per connector

worker_memory = num_connectors_per_worker × per_connector_memory + JVM_overhead
              = 3 × 316 MB + 2 GB
              ≈ 3 GB per worker

// During snapshot: add snapshot_chunk_buffer (default 10K rows × 1 KB = 10 MB)
// During large transactions: buffer may grow to transaction_size (bounded by max_batch_size)
```

### WAL Disk Pressure

```
wal_accumulation_rate = source_write_rate_bytes_per_sec
                      = 50 MB/sec (for an active OLTP database)

max_safe_connector_outage = (wal_disk_free_bytes × 0.8) / wal_accumulation_rate
                          = (500 GB × 0.8) / 50 MB/sec
                          = 8,000 seconds ≈ 2.2 hours

// Alert threshold: wal_disk_free < wal_accumulation_rate × max_acceptable_outage_seconds
// Example: alert when < 50 GB free (enough for ~17 minutes of outage at 50 MB/sec)
```

### Topic Partition Count

```
partitions_per_table = max(
    ceil(peak_events_per_sec_for_table / max_events_per_partition_per_sec),
    consumer_parallelism_target
)

// Example: orders table with 10K events/sec peak, 5K max per partition
partitions = max(ceil(10,000 / 5,000), 6)  // 6 consumer instances
           = max(2, 6) = 6 partitions
```
...
|--------|--------|--------|--------|-----------------|
| Source databases | 50 | 120 | 250 | New microservices, acquisitions |
| Events/sec (average) | 100K | 300K | 700K | More tables monitored, higher write rates |
| Events/sec (peak) | 500K | 1.5M | 3.5M | Major sale events, global expansion |
| Daily event volume | 8.6B | 26B | 60B | Linear with average throughput |
| Streaming platform storage | 180 TB | 540 TB | 1.2 PB | Retention expansion + volume growth |
| Connector workers | 20 | 50 | 100 | 2-3 connectors per worker |
| Schema registry entries | 5K | 15K | 40K | More tables × more schema versions |
| Brokers | 12 | 30 | 60 | Storage and throughput growth |

**Key Scaling Triggers:**
- **Add connector workers** when any worker CPU > 80% sustained 10 min
- **Add brokers** when cluster disk > 75% or any broker CPU > 70%
- **Increase partitions** when single-partition throughput > 50 MB/sec
- **Add schema registry nodes** when lookup latency p99 > 50 ms
...
|-----------|--------|------------|
| Source DB max replication slots | PostgreSQL limits concurrent slots (default: 10) | Consolidate CDC and replication slots; increase `max_replication_slots` |
| WAL segment size | Default 16 MB; affects granularity of WAL retention | Leave at default; tune `max_slot_wal_keep_size` instead |
| Binlog retention (MySQL) | Default 30 hours; CDC must keep up or lose events | Set `binlog_expire_logs_seconds` to match CDC SLA (e.g., 72 hours) |
| Network MTU | Jumbo frames (9000 bytes) improve throughput for large events | Enable jumbo frames between connector workers and brokers |
| Consumer group max poll interval | Default 5 min; exceeded = consumer eviction + rebalance | Tune `max.poll.interval.ms` based on heaviest consumer processing time |
| Schema registry storage | Schemas stored in internal topic with compaction | Monitor compacted topic size; alert if > 1 GB |
