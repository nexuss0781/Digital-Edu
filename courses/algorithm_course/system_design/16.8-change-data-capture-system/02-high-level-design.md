...
| Scalability | Single Slowest part of the process | Independent parallelism per table |
...
| Component | Primary Responsibility | Failure Mode | Failure Impact | Recovery |
|-----------|----------------------|-------------|----------------|----------|
| **Log Capture Engine** | Read WAL/binlog continuously | Connection lost to source DB | Capture pauses; WAL retention begins | Auto-reconnect with exponential backoff |
| **Snapshot Manager** | Full/incremental table reads | Long-running transaction timeout | Snapshot aborts; must restart from last chunk | Resume from persisted chunk progress |
| **Schema Parser** | Detect DDL from WAL entries | Incompatible DDL detected | Connector halts for affected table | Register new schema or route to dead-letter |
| **Offset Tracker** | Persist log position durably | Offset topic unavailable | Cannot commit progress; at-least-once window grows | Retry with backoff; events re-delivered on restart |
| **Event Envelope Builder** | Wrap changes in standard format | Serialization failure | Event cannot be published | Route to dead-letter topic; alert |
| **Filter & Transform** | Apply include/exclude rules | Malformed filter expression | All events pass unfiltered (fail-open) or connector stops (fail-closed) | Configuration fix + restart |
| **Schema Registry** | Version and validate schemas | Registry unavailable | New schemas cannot register; cache serves existing | Circuit breaker; cached fallback |
| **Streaming Platform** | Durable event storage/delivery | Partition leader failure | Brief write stall (< 1s) | Automatic leader election from ISR |
| **Connector Controller** | Distribute tasks across workers | Worker failure detected | Tasks orphaned on dead worker | Rebalance tasks to surviving workers |

---

## Technology Stack Considerations

| Layer | Recommended | Alternatives | Selection Criteria |
|-------|-----------|-------------|-------------------|
| **CDC Engine** | Debezium | Maxwell (MySQL-only), Canal (MySQL-only) | Multi-engine support, community, schema registry integration |
| **Streaming Platform** | Apache Kafka / Redpanda | Apache Pulsar, Amazon Kinesis | Partition model, exactly-once support, ecosystem maturity |
| **Schema Registry** | Confluent Schema Registry | Apicurio, AWS Glue Schema Registry | Avro support, compatibility modes, REST API |
| **Serialization** | Apache Avro | Protobuf, JSON Schema | Dynamic schemas (no recompilation), compact binary, evolution rules |
| **Offset Storage** | Streaming platform internal topic | External database (PostgreSQL) | Co-located with events for transactional commits |
| **Connector Orchestration** | Kafka Connect | Custom worker framework | Task distribution, rebalancing, REST management API |
| **Monitoring** | Prometheus + Grafana | Datadog, New Relic | Metric cardinality, custom dashboards, alert manager |
| **Secrets Management** | External KMS / Vault | Cloud-native secret stores | Rotation support, audit logging, encryption at rest |
