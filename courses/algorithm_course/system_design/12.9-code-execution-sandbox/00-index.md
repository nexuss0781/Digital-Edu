...
| [03 - Low-Level Design](./03-low-level-design.md) | Data models, API contracts, scheduling algorithms, Step-by-step plan in plain English |
...
| **WASM Runtime** | Wasmtime, Wasmer, WasmEdge | Language support, fuel metering, WASI compliance |

### 2025-2026 Technology Shifts

| Shift | Impact | Status |
|---|---|---|
| **WASM/WASI maturation** | Capability-based isolation eliminates kernel syscall surface for supported languages | Production-ready for ~10 languages (Rust, C, C++, Go, AssemblyScript) |
| **AI code assistant volume** | 10-100× more execution requests from AI-generated snippets | Requires dedicated fast-path execution pools |
| **GPU sandbox isolation** | ML workloads need GPU access inside sandboxes | Emerging via MIG partitioning; immature compared to CPU isolation |
| **Supply chain sandboxing** | Package installation (npm install, pip install) needs sandboxing | Harder than code execution — build scripts need network access |

---

## Related Patterns

| Related System | Shared Pattern | Relationship |
|---|---|---|
| [Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) | Queue-based worker pools with competing consumers | Sandbox workers are specialized job executors; scheduling, retry, and dead-letter logic are isomorphic to general job scheduling |
| [CI/CD Pipeline Build System](../2.4-cicd-pipeline-build-system/00-index.md) | Ephemeral build environments with isolation requirements | CI runners solve the same warm-pool, sandboxed-execution, and artifact-capture problems but with longer-running, less adversarial workloads |
| [Gaming Matchmaking System](../12.4-gaming-matchmaking-system/00-index.md) | Tiered warm pools with language/region affinity routing | Both systems pre-allocate scarce resources (sandboxes / game servers) partitioned by a workload dimension, using pool-sizing heuristics from queuing theory |
| [WebRTC Infrastructure](../12.8-webrtc-infrastructure/00-index.md) | Real-time output streaming via WebSocket with bounded buffers | Output streaming from sandboxes mirrors media relay patterns: bounded buffers, backpressure, and per-session lifecycle management |
| [Netflix Metaflow ML Workflow](../3.6-netflix-metaflow-ml-workflow-platform/00-index.md) | Step-level execution with resource-constrained compute environments | Metaflow's @resources decorator and Batch/Kubernetes execution parallels sandbox resource specification and worker dispatch |
| [Rate Limiter / API Gateway](../1.3-distributed-rate-limiter/00-index.md) | Per-user rate limiting and backpressure under adversarial load | Both systems must enforce fairness quotas under adversarial traffic; the rate limiter pattern recurs in submission throttling |
| [Container Orchestration Platform](../2.1-container-orchestration-platform/00-index.md) | Namespace isolation, cgroup enforcement, and container lifecycle management | Sandbox creation is a specialized form of container orchestration with stricter security constraints and shorter lifetimes |
| [Distributed Load Balancer](../1.2-distributed-load-balancer/00-index.md) | Language-affinity routing and weighted worker selection | Scheduler routing submissions to language-specific worker pools mirrors load balancer affinity routing with health-aware backends |
