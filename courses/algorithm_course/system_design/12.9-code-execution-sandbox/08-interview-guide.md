...
**Why it's wrong:** This system is compute-bound, not data-bound. The database stores 8KB per submission (source + results + metadata). Even at 5M submissions/day, that's 39GB/day — trivially handled by a standard database. The Slowest part of the process is the execution worker pool, not the data store.
...
| **WASM** | "WASM is the future of sandboxing" | "WASM/WASI provides capability-based isolation with near-native performance, but language coverage is limited (~10 of 60+ languages). Use as Tier 0 for supported languages (Rust, C, Go via compilation), fall back to nsjail/microVM for unsupported. The isolation model shifts from kernel-enforced to runtime-enforced—different threat surface" |
| **AI code assist** | "Run LLM-generated code like user code" | "LLM-generated code has different risk profiles: higher volume (auto-suggestions run continuously), more predictable resource usage (single-file snippets), but also more unpredictable behavior (hallucinated APIs, infinite loops from malformed logic). Dedicated fast-path pool with tighter time limits (2s wall-clock) and relaxed memory (LLM code rarely memory-bombs)" |

---

## 8. 2025-2026 Emerging Topics

Interviewers increasingly ask about these newer dimensions:

| Topic | Key Points to Cover |
|---|---|
| **WASM/WASI sandboxing** | Capability-based isolation model, near-native performance, limited language support (~10 languages), no kernel syscall surface, complements rather than replaces OS-level isolation |
| **AI code assistant execution** | Continuous execution of LLM-generated snippets (not just user submissions), 10-100× submission volume increase, dedicated fast-path pools, shorter time limits |
| **Supply chain attacks via dependencies** | Sandboxing package installation (npm install, pip install) is harder than sandboxing code execution — network access needed, build scripts run arbitrary code |
| **GPU sandbox isolation** | ML/AI workloads need GPU access inside sandboxes; GPU namespace isolation is immature compared to CPU/memory; MIG (Multi-Instance GPU) provides hardware partitioning |
