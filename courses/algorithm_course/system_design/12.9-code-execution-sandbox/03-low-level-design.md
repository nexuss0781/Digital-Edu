...
## 6. WASM Sandbox Process Model

For Tier 0 (WASM) execution, the process model differs fundamentally from OS-level sandboxing:

```
WASM Execution Pipeline:

  1. Receive source code (Rust/C/Go)
  2. Compile to WASM AOT (cached by content hash)
     ├── Cache hit → skip compilation (< 1ms)
     └── Cache miss → compile via language toolchain → store in cache
  3. Instantiate WASM module in runtime
     ├── Allocate linear memory (bounded, e.g., 256MB max)
     ├── Grant WASI capabilities (stdin, stdout, /workspace)
     └── Set fuel meter (instruction budget)
  4. Execute with fuel metering
     ├── Each WASM instruction consumes 1 fuel unit
     ├── Fuel exhausted → trap (equivalent to TLE)
     └── Memory access out of bounds → trap (equivalent to RE)
  5. Collect output
     ├── stdout captured via WASI fd_write
     └── Exit code from WASI proc_exit
  6. Discard instance (no scrubbing needed)
```
...
|---|---|---|
| Cache key | SHA256(source_code + language + compiler_version) | Content-addressed, immutable |
| Cache size | 50 GB per worker node | Covers ~500K unique programs |
| Eviction | LRU with 7-day TTL | Most re-submissions are within 24 hours |
| Hit rate (expected) | 60-80% for AI assistant workloads | AI snippets often repeat similar patterns |
| Compilation savings | 2-5s per cache hit | Eliminates the most expensive step |

### Fuel Metering vs cgroup CPU Limits

| Aspect | WASM Fuel Metering | cgroups cpu.max |
|---|---|---|
| Granularity | Per-instruction | Per-timeslice (100ms periods) |
| Determinism | Fully deterministic (same fuel = same limit) | Non-deterministic (depends on CPU speed, scheduling) |
| Overhead | 5-10% (fuel check per instruction) | < 1% (kernel-level scheduling) |
| Side-channel resistance | High (no timing variation from other tenants) | Low (CPU contention affects wall-clock) |
| Language support | Only WASM-compilable languages | All languages |
