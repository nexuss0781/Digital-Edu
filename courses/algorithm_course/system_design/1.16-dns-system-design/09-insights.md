...
**Why it matters:** At 1M+ QPS, a single shared cache with locks becomes the Slowest part of the process -- not the network, not disk. Per-thread L1 caches (100K entries, ~10MB) serve hot entries without any locking, while a lock-free hash map at L2 (10M entries) handles the shared tier. This design insight applies broadly: whenever you have an extremely read-heavy workload (99.99:1 ratio), duplicating hot data across threads is cheaper than synchronizing access. The memory overhead is negligible compared to the throughput gain.
...
## Insight 11: SVCB/HTTPS Records Collapse Connection Setup from 4 Round-Trips to 1
**Category:** Edge Computing
**One-liner:** The SVCB and HTTPS resource record types (RFC 9460) bundle connection parameters -- ALPN protocol, port, ECH public key, and IP hints -- into a single DNS response, eliminating the sequential discovery of connection capabilities.

**Why it matters:** Before SVCB/HTTPS records, a browser connecting to a website performed a serial discovery process: DNS A/AAAA query → TCP connect → TLS handshake (discovering HTTP/2 support) → potential HTTPS redirect → another DNS query + connect. Each step adds 30-100ms. HTTPS records short-circuit this by advertising in the DNS response itself that the server supports HTTP/3, which port to use, the ECH (Encrypted Client Hello) configuration for full SNI privacy, and even IP address hints for the CDN edge. The browser can then initiate a QUIC connection directly with the correct parameters in a single round-trip. The architectural significance extends beyond web browsing: SVCB is a generic service binding mechanism. Any protocol that requires connection parameter discovery (gRPC endpoints, MQTT brokers, SIP servers) can use SVCB to bundle service metadata into DNS, transforming DNS from a simple name→IP mapping into a lightweight service registry. Major browsers adopted HTTPS records in 2024-2025, with Cloudflare and other CDNs publishing them by default, making this one of the most impactful DNS protocol changes since DNSSEC.

---

## Insight 12: DNS-over-QUIC Eliminates Head-of-Line Blocking While Preserving UDP's Latency Profile
**Category:** Scaling
**One-liner:** DoQ (RFC 9250) uses QUIC's independent stream multiplexing to resolve multiple DNS queries concurrently without the head-of-line blocking that plagues DoH (HTTP/2) and DoT (TLS over TCP), achieving encrypted resolution at near-UDP speeds.

**Why it matters:** DNS privacy evolved through three generations with different architectural trade-offs. DoT (DNS-over-TLS, 2016) added encryption but introduced TCP's connection overhead and head-of-line blocking -- a single lost packet stalls all queued queries. DoH (DNS-over-HTTPS, 2018) reused HTTP/2 infrastructure, which eased deployment but inherited TCP's head-of-line blocking and added HTTP framing overhead. DoQ (DNS-over-QUIC, 2022, widely deployed 2025+) achieves the best of both: QUIC's 0-RTT resumption eliminates connection setup latency for returning clients, independent streams prevent a single lost packet from blocking other queries, and the protocol is purpose-built for DNS rather than piggybacking on HTTP. In resolver-to-authoritative communication (where latency matters most), DoQ approaches plain UDP latency while providing encryption and authentication. The deployment challenge is middlebox interference -- some enterprise firewalls block UDP/443 (QUIC's port), requiring graceful fallback to DoH. Major public resolvers began offering DoQ in 2024-2025, and recursive resolvers increasingly use DoQ for upstream communication even when serving clients via DoH.
