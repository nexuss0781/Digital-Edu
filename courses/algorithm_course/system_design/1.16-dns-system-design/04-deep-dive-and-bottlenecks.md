...
```
┌────────────────────────────────────────────────────────────────────┐
│ UDP PROCESSING Slowest part of the process                                           │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Problem:                                                            │
│   Single UDP socket cannot handle 1M+ QPS                          │
│   Kernel packet processing becomes Slowest part of the process                      │
│                                                                     │
│ Symptoms:                                                           │
...
```
┌────────────────────────────────────────────────────────────────────┐
│ BGP CONVERGENCE Slowest part of the process                                          │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ Problem:                                                            │
...
### 5. SVCB/HTTPS Record Processing (2025+)

SVCB and HTTPS resource records (RFC 9460) represent a fundamental expansion of DNS from name→IP mapping to name→connection_parameters mapping.

```
HTTPS RECORD STRUCTURE
═══════════════════════════════════════════════════════════════════

HTTPS 1 . alpn="h2,h3" port=443 ech="..." ipv4hint="1.2.3.4" ipv6hint="2001:db8::1"

Components:
├── Priority: 1 (lower = preferred, 0 = alias mode like CNAME)
├── Target: "." (same name) or explicit target
├── SvcParams:
│   ├── alpn: Application-Layer Protocol Negotiation hints
│   │   └── "h2,h3" → client knows HTTP/3 is available before connecting
│   ├── port: Non-standard port if different from default
│   ├── ech: Encrypted Client Hello configuration
│   │   └── Full TLS ECH public key for SNI privacy
│   ├── ipv4hint/ipv6hint: Address hints (avoid extra A/AAAA query)
│   └── no-default-alpn: Suppress default protocols
└── TTL: Typically 300-3600 seconds
```

#### Connection Optimization Impact

```
WITHOUT HTTPS RECORDS:                  WITH HTTPS RECORDS:
┌─────────────────────────────┐        ┌─────────────────────────────┐
│ 1. DNS A query (50ms)       │        │ 1. DNS HTTPS query (50ms)   │
│ 2. TCP connect (30ms)       │        │    → Gets IP + ALPN + ECH   │
│ 3. TLS handshake (30ms)     │        │ 2. QUIC 0-RTT (30ms)       │
│ 4. ALPN negotiation         │        │    → Direct H3 + ECH       │
│ 5. HTTP/1.1 upgrade to H2   │        │ 3. First byte received      │
│ 6. Possible HTTPS redirect  │        │                              │
│ 7. Repeat 1-5 for HTTPS     │        │ Total: ~80ms                │
│                              │        └─────────────────────────────┘
│ Total: ~300ms               │
└─────────────────────────────┘
         Savings: ~220ms (73% reduction)
```

#### Resolver Implementation Considerations

| Challenge | Description | Solution |
|-----------|-------------|----------|
| **Additional queries** | HTTPS record type requires separate query or piggybacking | Section 9 of RFC 9460: resolvers SHOULD query HTTPS alongside A/AAAA |
| **Cache coordination** | HTTPS records cached separately from A/AAAA | Linked TTL management: HTTPS TTL bounds effective A/AAAA lifetime |
| **Fallback** | Not all authoritatives serve HTTPS records | Graceful fallback to A/AAAA if HTTPS query returns NODATA |
| **ECH key rotation** | ECH keys in HTTPS records must stay current | Short TTLs (300s) for ECH-containing records, key rotation coordination |

---

### 6. DNS-over-QUIC (DoQ) Resolver Architecture (2025+)

```
DoQ vs DoH vs DoT ARCHITECTURAL COMPARISON
═══════════════════════════════════════════════════════════════════

DoT (RFC 7858):
┌───────────────────────────────────────────┐
│ DNS Wire Format → TLS 1.3 → TCP          │
│ - 1-RTT or 0-RTT handshake               │
│ - Head-of-line blocking across queries    │
│ - Single TCP connection per server        │
...
DoH (RFC 8484):
┌───────────────────────────────────────────┐
│ DNS Wire Format → HTTP/2 → TLS 1.3 → TCP │
│ - HTTP/2 stream multiplexing              │
│ - Still has TCP head-of-line blocking     │
│ - Shares port 443 (firewall-friendly)     │
│ - HTTP framing overhead (~5%)             │
│ - Overhead: ~5ms per query                │
└───────────────────────────────────────────┘

DoQ (RFC 9250):
┌───────────────────────────────────────────┐
│ DNS Wire Format → QUIC streams            │
│ - Independent stream per query            │
│ - No head-of-line blocking               │
│ - 0-RTT resumption for returning clients  │
│ - Port 853 (UDP)                          │
│ - Overhead: ~1ms per query                │
│ - Approaches plain UDP performance        │
└───────────────────────────────────────────┘
```

| Metric | Plain UDP | DoT | DoH | DoQ |
|--------|-----------|-----|-----|-----|
| **Encryption** | None | TLS 1.3 | TLS 1.3 | QUIC crypto |
| **Connection setup** | 0-RTT | 1-2 RTT | 1-2 RTT | 0-1 RTT |
| **Head-of-line blocking** | N/A (single query) | Yes (TCP) | Yes (TCP) | No (QUIC streams) |
| **Firewall traversal** | High | Low (port 853) | High (port 443) | Medium (UDP 853) |
| **p50 latency overhead** | 0ms | 2-5ms | 5-10ms | 1-2ms |
| **Multi-query efficiency** | Poor (no mux) | Good (pipelining) | Good (streams) | Best (independent streams) |
