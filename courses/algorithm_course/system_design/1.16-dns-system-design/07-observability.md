...
## AI/ML-Enhanced DNS Observability (2025+)

Major DNS providers have adopted ML-based monitoring that goes beyond statistical thresholds:

### ML-Based Anomaly Detection

| Detection Type | Traditional Approach | ML-Enhanced Approach | Improvement |
|---------------|---------------------|---------------------|-------------|
| **DDoS detection** | Fixed QPS thresholds | LSTM time-series prediction of expected traffic | 60% fewer false positives |
| **Tunneling detection** | Entropy analysis only | Random forest on query features (length, frequency, encoding) | Catches encoded tunnels |
| **Domain generation** | Blocklist matching | Character-level neural network for DGA detection | Detects novel DGA families |
| **Cache poisoning** | TXID/port monitoring | Behavioral anomaly (sudden resolution path changes) | Catches sophisticated attacks |

### Predictive Cache Prefetching

```
PREDICTIVE PREFETCHING PIPELINE
═══════════════════════════════════════════════════════════════════

Query Log → Feature Extraction → Time-Series Model → Prefetch Queue
              │                        │                    │
              ├─ Hour of day           ├─ Next 5-min       ├─ Priority by
              ├─ Day of week           │  QPS prediction    │  expected demand
              ├─ Domain popularity     ├─ Domain access     └─ Respect TTL
              ├─ TTL expiry time       │  probability            constraints
              └─ Client segment        └─ Confidence
                                          score

Result: 15-25% improvement in cache hit rate for predictable
        traffic patterns (business hours, recurring cron jobs)
```

### DoQ/DoH Protocol Monitoring

```yaml
# New metrics for encrypted DNS protocols (2025+)
dns_doq_connections_total:
  type: counter
  labels: [status, quic_version]
  description: DoQ connection attempts and outcomes

dns_doq_0rtt_resumption_rate:
  type: gauge
  description: Percentage of DoQ connections using 0-RTT (indicates returning clients)

dns_doh_http_version:
  type: counter
  labels: [version]
  # version: h2, h3
  description: DoH requests by HTTP version

dns_ech_key_freshness:
  type: gauge
  labels: [domain]
  description: Seconds since ECH key was last rotated (alert if stale)

dns_svcb_query_rate:
  type: counter
  labels: [qname, has_ech]
  description: SVCB/HTTPS record queries, tracking ECH adoption
```
