...
│   □ DoQ (DNS-over-QUIC) endpoint available                       │
│   □ ECH keys published via HTTPS records                          │
│   □ Automated DNSSEC key rollover via CDS/CDNSKEY                │
│   □ AI/ML anomaly detection for DDoS patterns                    │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
## Encrypted Client Hello (ECH) and DNS Privacy (2025+)

ECH completes the DNS privacy picture that DoH/DoT/DoQ started. While encrypted DNS prevents observers from seeing which domain you query, the TLS Server Name Indication (SNI) in the subsequent HTTPS connection still reveals the destination in plaintext. ECH encrypts the SNI using a public key distributed via HTTPS DNS records.

```
DNS PRIVACY EVOLUTION
═══════════════════════════════════════════════════════════════════

Plain DNS (pre-2016):       All visible to network observers
├── DNS query:              ✗ Domain visible
├── TLS SNI:                ✗ Domain visible
└── HTTP content:           ✗ Content visible (HTTP) or encrypted (HTTPS)

DoH/DoT (2018+):           DNS encrypted, SNI still visible
├── DNS query:              ✓ Encrypted
├── TLS SNI:                ✗ Domain still visible
└── HTTP content:           ✓ Encrypted

DoH/DoT + ECH (2025+):     Full privacy chain
├── DNS query:              ✓ Encrypted (DoH/DoT/DoQ)
├── TLS SNI:                ✓ Encrypted (ECH key from HTTPS record)
└── HTTP content:           ✓ Encrypted
```

### ECH Distribution via HTTPS Records

| Aspect | Detail |
|--------|--------|
| **Key format** | ECH config published in HTTPS record's `ech` parameter |
| **Key rotation** | Short TTLs (300-600s) with staggered rotation across PoPs |
| **Retry logic** | If ECH fails (stale key), client retries with `ech="GREASE"` to blend in |
| **CDN integration** | CDN publishes ECH config for all domains behind its edge, enabling privacy even for small sites |
| **Architectural impact** | DNS becomes the key distribution channel for TLS privacy -- increasing DNS's criticality |

### Automated DNSSEC Key Management (CDS/CDNSKEY)

Traditional DNSSEC key rollovers require manual DS record updates at the parent zone -- a process that fails silently if forgotten. CDS (Child DS) and CDNSKEY records (RFC 8078) enable the child zone to publish its desired DS record, allowing the parent to update automatically.

```
TRADITIONAL KEY ROLLOVER:          AUTOMATED (CDS/CDNSKEY):
1. Generate new KSK                1. Generate new KSK
2. Publish in zone                  2. Publish in zone + CDS record
3. Wait for propagation             3. Wait for propagation
4. Contact registrar                4. Parent scans for CDS (automated)
5. Update DS at parent              5. Parent updates DS (automated)
6. Verify chain works               6. Verify chain works
7. Remove old key                   7. Remove old key

Risk: Step 4-5 manual,             Risk: Minimal -- automated
      often forgotten                     monitoring catches failures
```
