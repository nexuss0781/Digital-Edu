...
| **Apple iCloud Private Relay** | DoH + proxy chain | DNS privacy via dual-hop architecture |
| **NextDNS** | Customizable filtering + analytics | Per-device DNS policies |
...
| **Transport** | UDP - performance | DoH/DoT/DoQ - privacy | Privacy needs, firewall traversal |
| **Anycast vs GeoDNS** | Anycast - automatic failover | GeoDNS - control over routing | Stickiness requirements |
| **SVCB/HTTPS records** | Traditional A/AAAA - simple | SVCB - connection parameter bundling | Connection optimization needs |
...
| **DoH/DoT/DoQ** | Medium-High | TLS/QUIC termination, protocol multiplexing |
| **Cache Management** | Medium | TTL enforcement, negative caching |
| **SVCB/HTTPS Records** | Medium | Connection parameter bundling, ECH distribution |
...
## 2025-2026 DNS Evolution

| Development | Status | Impact |
|-------------|--------|--------|
| **DNS-over-QUIC (DoQ)** | RFC 9250, major resolvers deploying (2024-2025) | Encrypted resolution at near-UDP speeds, no head-of-line blocking |
| **SVCB/HTTPS Records** | RFC 9460, browser adoption 2024-2025 | Collapses 4 connection round-trips to 1, ECH key distribution |
| **Encrypted Client Hello (ECH)** | Deployed via HTTPS records | Full SNI privacy -- DNS + TLS handshake both encrypted |
| **Automated DNSSEC Key Management** | CDS/CDNSKEY (RFC 8078) gaining adoption | Eliminates manual DS record management, reduces key rollover failures |
| **AI/ML in DNS** | Production at major providers | Anomaly-based DDoS detection, predictive cache prefetching |
| **DNS64/NAT64** | Expanding with IPv6 adoption | Transparent IPv4→IPv6 translation at DNS layer |
| **CoreDNS in Service Mesh** | Default DNS in container orchestration | Plugin-based DNS with service mesh integration |
| **Multi-CDN DNS Steering** | Enterprise adoption accelerating | Real-time performance-based CDN selection via DNS |

**Key 2025-2026 numbers:**
- Public resolver DoH adoption: 30%+ of encrypted DNS queries
- DNSSEC-signed zones: 15%+ of `.com` domains (up from 5% in 2020)
- Root server instances: 1,900+ globally via Anycast
- Cloudflare 1.1.1.1: p50 < 11ms globally, DoQ support live
- HTTPS record adoption: 60%+ of top 1000 websites
...
| [1.4 Distributed LRU Cache](../1.4-distributed-lru-cache/00-index.md) | TTL-based caching, negative caching patterns |
| [1.9 Consistent Hashing Ring](../1.9-consistent-hashing-ring/00-index.md) | Cache sharding patterns applicable to DNS cache |
| [1.10 Service Discovery](../1.10-service-discovery-system/00-index.md) | DNS-based service discovery (SRV, SVCB records) |
| [1.15 CDN](../1.15-content-delivery-network-cdn/00-index.md) | CDN uses DNS for edge routing, HTTPS records |
| [2.8 Edge Computing Platform](../2.8-edge-computing-platform/00-index.md) | Edge DNS resolution, latency-sensitive caching |
| [3.21 LLM Gateway](../3.21-llm-gateway-prompt-management/00-index.md) | Hierarchical resolution parallels virtual key hierarchy |
