...
## Service Mesh DNS Integration (2025+)

Modern container orchestration platforms use DNS as the primary service discovery mechanism, with CoreDNS as the default resolver in most deployments.

### CoreDNS Plugin Architecture

```
CoreDNS REQUEST PIPELINE
═══════════════════════════════════════════════════════════════════

Query: myservice.mynamespace.svc.cluster.local
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ CoreDNS Plugin Chain                                         │
│                                                              │
│ 1. kubernetes        ─ Service/Pod/Headless lookup          │
│ 2. etcd              ─ Custom service records               │
│ 3. cache (30s)       ─ Local response cache                 │
│ 4. loadbalance       ─ Round-robin A record shuffling       │
│ 5. forward           ─ External DNS for non-cluster queries │
│ 6. prometheus        ─ Metrics export                       │
│ 7. errors + log      ─ Observability                        │
└─────────────────────────────────────────────────────────────┘
```

### DNS Challenges at Container Scale

| Challenge | Description | Impact |
|-----------|-------------|--------|
| **Pod churn** | Thousands of pods created/destroyed per minute | DNS records must update within seconds, not minutes |
| **ndots:5 default** | Container DNS config appends 5 search domains per query | 5x amplification: `api.example.com` generates 6 DNS queries |
| **DNS as SPOF** | CoreDNS down = all service-to-service calls fail | Node-local DNS cache (NodeLocal DNSCache) critical |
| **Headless services** | Returns all pod IPs, not load-balanced VIP | Response size grows linearly with pod count |
| **External DNS sync** | external-dns controller syncs Kubernetes services to cloud DNS | Eventual consistency between cluster state and DNS |

### NodeLocal DNSCache Architecture

```
WITHOUT NodeLocal DNSCache:          WITH NodeLocal DNSCache:
┌─────┐   ┌─────┐   ┌─────┐        ┌─────┐   ┌─────┐   ┌─────┐
...
└──┬──┘   └──┬──┘   └──┬──┘        └──┬──┘   └──┬──┘   └──┬──┘
   ...
   └────────┼────────┘               └────────┼────────┘
            │                                  │
    ┌───────▼──────┐                  ┌───────▼──────┐
    │ kube-dns SVC │                  │NodeLocal DNS │ ← Per-node cache
    │ (ClusterIP)  │                  │  (DaemonSet) │   (port 169.254.x)
    └───────┬──────┘                  └───────┬──────┘
...
Problem: Every DNS query                Benefit: Cache hit = local
crosses network to CoreDNS             (~0.5ms vs ~5ms cross-node)
```

### Multi-CDN DNS Steering (2025+)

Enterprises increasingly use DNS-based steering to route traffic across multiple CDN providers in real-time:

| Strategy | Mechanism | Trade-off |
|----------|-----------|-----------|
| **Performance-based** | RUM (Real User Measurements) feed DNS routing decisions | Most accurate, requires client-side data collection |
| **Cost-based** | Route to cheapest CDN until commit threshold, then overflow | Maximizes CDN discounts, may sacrifice some latency |
| **Availability-based** | Health checks per CDN per region, automatic failover | Most reliable, requires comprehensive monitoring |
| **Hybrid** | Weighted combination of performance, cost, availability | Optimal but complex, requires tuning weights per region |
