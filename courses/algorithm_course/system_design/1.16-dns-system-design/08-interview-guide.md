...
**If the interviewer asks about modern DNS features:**

"SVCB and HTTPS records (RFC 9460) are the biggest protocol change in DNS since DNSSEC. They transform DNS from a simple name→IP mapping into a connection parameter registry. A single DNS response can now tell a browser: the IP address, that HTTP/3 is supported, which port to use, and the ECH public key for SNI privacy. This collapses what used to be 4 round-trips (DNS → TCP → TLS → ALPN negotiation) into essentially 1. The architectural significance is that DNS becomes the bootstrap protocol for all connection parameters, not just addressing."

### DNS-over-QUIC (DoQ)

**When asked about encrypted DNS:**

"DoQ (RFC 9250) is the third generation of encrypted DNS, and it's architecturally the cleanest. DoT suffered from TCP head-of-line blocking (one lost packet stalls all queries). DoH reused HTTP/2 infrastructure which eased deployment but added HTTP framing overhead. DoQ uses QUIC's independent streams -- each DNS query gets its own stream, so a lost packet on one query doesn't block others. Plus QUIC's 0-RTT resumption means returning clients skip the handshake entirely. The result is encrypted DNS at near-UDP speeds. The deployment challenge is middlebox interference -- some firewalls block UDP/853."

### Service Mesh DNS

**When asked about containerized environments:**

"In container orchestration, DNS is the service discovery mechanism. CoreDNS serves as the cluster resolver with a plugin architecture (kubernetes plugin for service lookup, forward plugin for external DNS). The key challenges are:
- **ndots amplification**: The default `ndots:5` config means a query for `api.example.com` generates 6 DNS queries (5 search domain attempts + the original). This is the #1 DNS performance issue in containers.
- **NodeLocal DNSCache**: A DaemonSet that runs a DNS cache on every node, reducing cross-network DNS queries from ~5ms to ~0.5ms.
- **Pod churn**: Thousands of pods created/destroyed per minute -- DNS records must update in seconds, not the minutes that traditional DNS expects."

### AI/ML in DNS Operations

**When asked about modern operations:**

"ML is increasingly used for DNS anomaly detection. Traditional threshold-based alerting generates too many false positives because DNS traffic has strong temporal patterns (business hours, timezone effects). LSTM-based time-series models predict expected traffic, and anomalies are detected against the prediction rather than a static threshold. This reduces false positive DDoS alerts by ~60%. Additionally, character-level neural networks can detect Domain Generation Algorithm (DGA) domains in real-time -- these are the random-looking domains used by malware for command-and-control communication."
