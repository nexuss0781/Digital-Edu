...
### Trap 6: "How would you architect Shorts differently from long-form?"

```
WHAT THEY'RE TESTING:
Understanding that short-form video has fundamentally different
architectural requirements, not just "shorter videos"

WRONG ANSWER:
"Same pipeline, just shorter processing time"

RIGHT ANSWER:
"Shorts differ across five architectural dimensions:

1. DISCOVERY MODEL: 100% recommendation-driven
   - No search, no subscriptions as primary entry
   - Recommendation QPS is 10x long-form
   - Must prefetch next 5-10 videos for instant scroll

2. ENGAGEMENT SIGNALS: Completion rate, not watch time
   - 60-second video watched to end = strong signal
   - Swipe-away speed matters (how fast they skip)
   - Replay rate is a positive signal (unusual for long-form)

3. TRANSCODING: Speed over compression
   - Target: <30 seconds time-to-publish
   - Fewer renditions (3-4 vs 8-10)
   - Fast preset with reduced quality analysis
   - Full resolution ladder generated asynchronously

4. CDN: Higher cache efficiency
   - >95% cache hit rate (viral power law more extreme)
   - Entire video fits in L1 hot cache
   - Aggressive prefetching based on recommendation feed

5. VERTICAL VIDEO: Different encoding profiles
   - 9:16 at 1080x1920 = 2x pixels vs standard 1080p
   - Lower bits-per-pixel (mobile-only consumption)
   - AI smart cropping for horizontal→vertical repurposing"
```

### Trap 7: "How does QUIC improve video streaming over TCP?"

```
WHAT THEY'RE TESTING:
Protocol-level understanding of streaming optimization

WRONG ANSWER:
"It's just faster HTTP"

RIGHT ANSWER:
"QUIC solves three specific TCP problems for video:

1. HEAD-OF-LINE BLOCKING ELIMINATION
   TCP: one lost packet blocks ALL multiplexed streams
   QUIC: each stream is independent, loss only affects that stream
   Impact: ABR quality switches don't stall other requests

2. 0-RTT CONNECTION ESTABLISHMENT
   TCP+TLS: 2-3 round trips before first data byte
   QUIC returning client: 0 round trips (cached credentials)
   Impact: 100-300ms faster video start time

3. CONNECTION MIGRATION
   TCP: connection = (src_ip, src_port, dst_ip, dst_port)
   QUIC: connection = connection_id (survives IP changes)
   Impact: seamless playback when phone switches WiFi→cellular

Result: 9-15% fewer rebuffering events, measurably faster
playback start. YouTube serves 80%+ of traffic over QUIC."
```

### Trap 8: "How do you detect deepfake videos at upload scale?"

```
WHAT THEY'RE TESTING:
Understanding of content integrity at scale

WRONG ANSWER:
"Use AI to detect AI-generated content"

RIGHT ANSWER:
"Detection is necessary but insufficient. Multi-layered approach:

1. DETECTION (accuracy varies):
   - Frequency analysis: ~70% detection rate
   - Temporal consistency: ~85% with Video Transformers
   - Physiological signals: effective for face-swaps
   - Challenge: best detectors <60% on latest generators

2. PROVENANCE (proactive, not reactive):
   - C2PA standard: cryptographic creation history
   - SynthID watermarking: imperceptible marks in AI content
   - Creator disclosure policy: must label AI-generated content

3. POLICY ENFORCEMENT:
   - Undisclosed AI content gets labeled by platform
   - Deepfakes of real people without consent: removal
   - Election-related content: expedited human review

Key insight: it's an arms race where detection always lags.
Provenance-based approaches (C2PA, SynthID) are more
sustainable than detection-only strategies because they
establish trust at creation time rather than inferring it."
```
