...
| Stage | Time (10-min 1080p video) | Slowest part of the process |
|-------|---------------------------|------------|
...
### AV1 Hardware Acceleration Deep Dive

The transition from software-only AV1 encoding to hardware-accelerated pipelines fundamentally changes YouTube's codec selection economics.

```
HARDWARE AV1 ENCODING LANDSCAPE (2025):

CUSTOM ASICs (Argos Video Coding Units):
    - Purpose-built for YouTube's encoding workload
    - Each Argos VCU replaces ~30 CPU cores for H.264, ~100 cores for VP9
    - AV1 encode: 60 seconds for 10-min 1080p (vs 600s on CPU)
    - Iterated through multiple generations (2021-2025)
    - Optimized for YouTube's specific quality targets and resolution ladders

DATA CENTER GPU ENCODING:
    - NVIDIA RTX 50-series (Blackwell, 2025): 9th-gen NVENC
      - Dual AV1 encoders per GPU
      - 2x encode density vs RTX 40-series
    - Intel Flex series: 8 simultaneous 1080p60 AV1 streams per card
    - AMD MI300X: Hardware AV1 for cloud transcoding

MOBILE ON-DEVICE ENCODING:
    - Snapdragon 8 Gen 3+: Hardware AV1 encode on mobile
    - Enables client-side AV1 encoding before upload
    - Reduces server-side transcoding load for Shorts

CODEC EFFICIENCY COMPARISON (2025 benchmarks):
| Metric          | H.264  | VP9     | AV1      | H.266/VVC |
|-----------------|--------|---------|----------|-----------|
| Compression     | Base   | 30-35%  | 50%      | 50-55%    |
| Encode speed    | 1x     | 0.5x    | 0.1x SW  | 0.05x SW  |
|                 |        |         | 1x HW    | N/A HW    |
| Royalty-free    | No     | Yes     | Yes      | No        |
| Device support  | 100%   | 90%     | 70%+     | <5%       |
| Browser support | All    | Most    | All maj. | None      |

REVISED CODEC SELECTION (hardware-aware):
    // With Argos VCUs and GPU encoders available
    IF hardware_av1_capacity_available:
        encode_queue = [AV1_ALL_RESOLUTIONS, H264_ALL_RESOLUTIONS]
        // AV1 first when hardware makes it cost-competitive
    ELSE:
        // Fallback to traditional progressive strategy
        encode_queue = [H264_ALL_RESOLUTIONS]
        IF video.resolution >= 720p:
            encode_queue.append(VP9_HD_RESOLUTIONS)
```
...
Neural approaches augment traditional codec pipelines rather than replacing them.

```
NEURAL-ENHANCED ENCODING:

1. PER-SCENE COMPLEXITY OPTIMIZATION
   - Lightweight neural network analyzes scene complexity
   - Dynamically adjusts CRF (Constant Rate Factor) per segment
   - High-complexity scenes (action, particles): lower CRF (higher quality)
   - Low-complexity scenes (talking head, static): higher CRF (smaller size)
   - Result: 5-15% additional bitrate savings on top of codec efficiency

2. NEURAL SUPER-RESOLUTION (MOBILE STREAMS)
   Architecture:
       Client bandwidth: 2 Mbps (insufficient for 1080p)
       Traditional: serve 480p → viewer sees 480p
       Neural SR: serve 480p → on-device NPU upscales → viewer sees ~720p quality

   Requirements:
       - Device NPU (Neural Processing Unit) available
       - Pre-trained super-resolution model (~5 MB)
       - Inference latency: <10ms per frame on modern NPUs
       - Result: 40-60% bandwidth reduction with perceived quality maintenance

3. CONTENT-AWARE ENCODING (PER-TITLE OPTIMIZATION)
   - Analyze video content characteristics before encoding
   - Generate optimal bitrate ladder per video
   - Animation: lower bitrate sufficient (less noise)
   - Sports/action: higher bitrate needed (temporal complexity)
   - Talk shows: much lower bitrate (static background)
   - Saves 20-30% bandwidth vs fixed bitrate ladder
```

### Short-Form Video Architecture (Shorts)

Shorts have fundamentally different architectural requirements from long-form content.

```
SHORTS vs LONG-FORM ARCHITECTURE DIFFERENCES:

| Aspect          | Shorts (≤60s)        | Long-Form (minutes-hours) |
|-----------------|----------------------|---------------------------|
| Renditions      | 3-4 quality levels   | 8-10 quality levels       |
| Storage/video   | 5-20 MB              | 500 MB - 5 GB             |
| Transcode SLA   | <30 seconds          | Minutes to hours          |
| Cache hit rate   | >95% (viral power law)| 98.5% (long-tail)       |
| Discovery       | 100% recommendation  | Search + Sub + Algo       |
| Engagement signal| Completion rate      | Watch time                |
| Rec throughput   | 10x long-form QPS   | Standard                  |
| Prefetch depth  | Next 5-10 videos     | Next 2-3 segments         |

SHORTS TRANSCODING FAST PATH:
    FUNCTION transcode_short(video):
        // Optimized for time-to-publish, not maximum compression
        IF video.duration <= 60s:
            USE fast_preset (reduced quality analysis)
            SKIP detailed scene complexity analysis
            ENCODE AV1_1080p + H264_720p + H264_360p only
            TARGET completion < 30 seconds
            // Full resolution ladder generated async

VERTICAL VIDEO PROCESSING:
    - 9:16 aspect ratio: 1080x1920 = 2x pixels vs standard 1080x608
    - Encode at lower bits-per-pixel (mobile screens are smaller)
    - Smart cropping for horizontal→vertical repurposing:
      - AI tracks region of interest (faces, action, text)
      - Dynamically crops to vertical framing per scene
    - Thumbnail selection via neural aesthetic scoring model

RECOMMENDATION-FIRST ARCHITECTURE:
    // Shorts feed is pure algorithmic discovery
    FUNCTION prefetch_shorts_feed(user_id):
        candidates = generate_candidates(user_id, count=100)
        ranked = rank_candidates(candidates)
        // Pre-fetch video data for top 10 before user reaches them
        FOR i = 0 TO 9:
            prefetch_to_client_cache(ranked[i].video_data)
        RETURN ranked
```
...
| Rank | Slowest part of the process | Impact | Mitigation Strategy |
|------|------------|--------|---------------------|
