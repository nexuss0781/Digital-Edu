...
### Slowest part of the process 1: Burst Scaling Limits
...
### Slowest part of the process 2: Code Package Download Latency
...
### Slowest part of the process 3: VPC Cold Start Penalty
...
### Slowest part of the process 4: Worker Fleet Capacity
...
### Slowest part of the process 5: Event Source Throughput
...
## Deep Dive 5: WebAssembly as a Serverless Runtime

### WASM Execution Model

```
┌─────────────────────────────────────────────────────────────────────┐
│  WASM SERVERLESS EXECUTION                                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
...
│                                                                      │
│  How It Works:                                                       │
│  1. Developer compiles function to .wasm binary (ahead-of-time)     │
│  2. Platform loads pre-compiled module into WASM runtime             │
│  3. Module executes in linear memory sandbox:                       │
│     - Cannot access host memory outside its linear memory           │
│     - Cannot make syscalls except through WASI capabilities         │
│     - No shared mutable state between modules                       │
│  4. WASI capabilities granted per-function:                         │
│     - wasi:filesystem/read (read-only FS access)                    │
│     - wasi:http/outgoing-handler (make HTTP requests)               │
│     - wasi:sockets/tcp (TCP connections)                            │
│                                                                      │
│  Component Model:                                                    │
│  - WASM Components define typed interfaces (WIT)                    │
│  - Functions in different languages compose via interfaces          │
│  - Example: Rust image processor + Python ML classifier             │
│    linked as WASM Components sharing typed data                     │
│                                                                      │
│  Isolation vs V8 vs MicroVM:                                         │
│  ┌─────────────┬───────────────┬──────────────┬──────────────┐     │
│  │ Property    │ MicroVM       │ V8 Isolate   │ WASM Module  │     │
│  ├─────────────┼───────────────┼──────────────┼──────────────┤     │
│  │ Memory      │ Hardware MMU  │ V8 heap      │ Linear memory│     │
│  │ Syscalls    │ Full kernel   │ V8 sandbox   │ WASI caps    │     │
│  │ Languages   │ Any           │ JS/WASM      │ Any→WASM     │     │
│  │ Cold start  │ ~125ms        │ <5ms         │ <1ms         │     │
│  │ Overhead    │ ~5MB/instance │ ~2MB/isolate │ <1MB/module  │     │
│  │ Escape risk │ Minimal       │ V8 bugs      │ WASM spec    │     │
│  └─────────────┴───────────────┴──────────────┴──────────────┘     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### GPU-Attached Serverless Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│  GPU SERVERLESS INVOCATION FLOW                                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Challenge: Model loading dominates cold start                      │
│                                                                      │
│  Traditional GPU Cold Start:                                        │
│  MicroVM Boot    Model Download    GPU Load      Ready              │
│  ├──125ms──┤├────2000ms────┤├──3000ms──┤├──500ms──┤ = 5.6s         │
│                                                                      │
│  Optimized GPU Cold Start:                                          │
│  Snapshot Restore    Attach Cached Model    Ready                   │
│  ├────200ms────┤├──────100ms──────┤├──50ms──┤ = 350ms              │
│                                                                      │
│  Optimization Strategy:                                              │
│  1. Model Pool: Keep hot models in GPU memory across invocations   │
│     - LRU eviction based on invocation frequency                    │
│     - Models persist across function instance lifecycles            │
│                                                                      │
│  2. GPU Memory Snapshot: Capture GPU state after model load         │
│     - Restore snapshot instead of re-loading from storage           │
│     - Reduces 3-15s model load to 200ms memory restore              │
│                                                                      │
│  3. Model Sharding: Split large models across GPU partitions        │
│     - 70B model: 7 partitions × 10GB each                          │
│     - Parallel load across partitions                               │
│                                                                      │
│  4. Quantization: Reduce model precision for smaller footprint      │
│     - FP16 → INT4: 4x size reduction (14GB → 3.5GB)               │
│     - Acceptable quality loss for most inference workloads          │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
