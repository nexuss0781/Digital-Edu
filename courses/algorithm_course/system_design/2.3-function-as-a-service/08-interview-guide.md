...
## Modern Topics to Address

### When Asked "What about WebAssembly for Serverless?"

> "WASM is emerging as a third isolation model. It provides sub-millisecond cold starts (pre-compiled binaries, no JIT warmup), capability-based sandboxing via WASI (each function gets only the system capabilities it requests), and language-agnostic support (Rust, Go, Python, C++ all compile to WASM). The Component Model allows functions in different languages to compose via typed interfaces. Compared to MicroVMs, WASM trades mature ecosystem tooling for dramatically faster startup. Compared to V8, WASM trades the JavaScript ecosystem for language flexibility and stronger isolation. The main limitation today is I/O maturity — WASI's networking and async capabilities are still evolving."

### When Asked "How Would You Handle AI/ML Inference on Serverless?"

> "GPU-attached serverless is the biggest recent evolution. The challenge is that model loading (2-30GB) creates cold starts 10-100x worse than CPU functions. The mitigation stack is: (1) GPU memory model pools — keep hot models resident in GPU memory across invocations, shared by function instances; (2) model quantization — INT4 quantization reduces a 7B parameter model from 14GB to 3.5GB, cutting load time 4x; (3) GPU memory snapshot/restore — capture GPU state after model loading and restore it in ~200ms instead of re-loading from storage; (4) fractional GPU allocation — time-slice a single GPU across tenants so each pays only for the fraction they use. The architectural insight is that the model is the 'runtime' for AI functions, analogous to the JVM for Java functions, and deserves the same warm pool and caching treatment."

### When Asked "Single-Request vs Multi-Concurrency per Instance?"

> "This is a fundamental cost-efficiency trade-off. Single-request-per-instance (Lambda model) means each request gets dedicated resources — great isolation, but 40-80% of CPU time is wasted during I/O waits. Multi-concurrency (Cloud Run model) runs 10-80 concurrent requests per instance, sharing MicroVM overhead and achieving 80%+ CPU utilization. For I/O-bound workloads (most web APIs), multi-concurrency reduces cost by 5-10x. The trade-off is noisy-neighbor risk: a CPU-intensive request can starve others on the same instance. The right answer is making concurrency a per-function configuration: CPU-bound functions use single-request, I/O-bound functions use multi-concurrency."
