...
**Step-by-step plan in plain English to Write:**
...
**Q: "Why not just increase the context window to 1M tokens instead of building a memory system?"**

**Bad:** "Bigger context is always better."

**Good:** "Context window size and memory systems solve different problems. A 1M token context is expensive ($0.01-0.10 per request at that scale), slow to process (attention is O(n²)), and wasteful -- most past turns are irrelevant to the current decision. A memory system with importance scoring retrieves only the 5-10 most relevant entries (<1K tokens), costing 100x less while being more focused. Long context is useful for single-document analysis; memory systems are essential for multi-session agents that accumulate knowledge over weeks."

**Q: "How would you handle an agent that gets stuck in an infinite loop?"**

**Bad:** "Set a max iteration limit."

**Good:** "Loop detection requires three complementary mechanisms: (1) a hard iteration limit (say, 10 turns without progress), (2) state similarity detection -- if the agent's state after turn N is >90% similar to turn N-2, it's cycling, and (3) output diversity monitoring -- if the agent produces the same tool calls with the same arguments twice, break the loop. When detected, the agent should not just stop but escalate: attempt a different approach (re-plan with explicit instruction to avoid the failing path), and if that also loops, pause for human intervention with full context of what was tried."

**Q: "How do you prevent an agent from taking destructive actions?"**

**Bad:** "We review all actions before execution."

**Good:** "Actions are classified into three tiers: (1) read-only actions (file reads, searches) execute immediately with no approval, (2) reversible write actions (file edits, database updates with undo) execute with automatic checkpointing before the action so rollback is possible, and (3) irreversible actions (sending emails, deleting data, making API calls with side effects) require explicit human approval via a pause-and-confirm flow. The classification is defined per-tool in the MCP server's tool metadata, not in the agent's code. The guardrail layer additionally monitors for patterns like rapid successive deletions or actions on resources outside the agent's declared scope."
