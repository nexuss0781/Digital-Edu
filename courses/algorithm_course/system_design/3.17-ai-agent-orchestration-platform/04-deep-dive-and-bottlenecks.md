...
Each section includes implementation details, trade-offs, Slowest part of the process analysis, and mitigation strategies.
...
#### Slowest part of the process 1: Checkpoint Write Latency
...
#### Slowest part of the process 2: State Size Explosion
...
#### Slowest part of the process 3: Recovery Time
...
class ShortTermMemory:
    ...
    def evict_or_summarize():
        ...
        IF strategy == "FIFO (First-In-First-Out, like a line at a store)":
...
#### Slowest part of the process 1: Memory Retrieval Latency
...
Vector search at scale is slow:
  ...
  - Brute Force (Checking every single possibility): O(n × d) = seconds
...
#### Slowest part of the process 2: Context Window Management
...
3. TOOL DEFINITION Cutting off unnecessary steps
...
#### Slowest part of the process: Guardrail Latency
...
### Case Study 1: Autonomous Coding Agent at Production Scale

**Context:** An AI coding assistant handling 500K+ daily sessions where developers request multi-file code changes, debugging, and refactoring. Average session: 15 turns, 8 tool calls per turn (file reads, writes, terminal commands, web searches).

**Architecture Decisions:**
- Graph-based orchestration with conditional routing: plan → edit → test → fix loop with max 5 iterations
- Delta checkpoints after every turn (5ms write), full snapshots every 10 turns (50ms)
- Three-tier memory: conversation context (in-context), project knowledge (vector DB), and learned coding patterns (procedural store)
- Sandboxed code execution via container-per-session with 30-second timeouts
- Planning-execution model split: reasoning model for initial plan, fast model for tool execution

**Results:**
- 94% task completion rate (up from 67% without checkpointing and memory)
- Average session: 45 seconds end-to-end for simple edits, 3-5 minutes for complex multi-file changes
- Checkpoint recovery saved an estimated 2.3M tokens/day from crash-related restarts
- Procedural memory reduced average turns by 30% for repeated task patterns (e.g., "add tests for this function")

**Key Lesson:** The biggest Slowest part of the process was not LLM inference but tool call latency. File system operations in remote containers averaged 200ms per call, and a typical edit-test cycle required 15+ tool calls. Parallelizing independent tool calls (reading 5 files simultaneously instead of sequentially) cut turn latency by 60%.

### Case Study 2: Multi-Agent Research System

**Context:** A research platform where a coordinator agent decomposes complex research questions into sub-tasks, delegates to specialized worker agents (web search, academic paper analysis, data synthesis), and produces structured reports.

**Architecture Decisions:**
- Hierarchical orchestration: coordinator plans and delegates, workers execute independently
- Shared memory bus: workers publish findings to a shared context that the coordinator and other workers can read
- Token budget management: coordinator allocates token budgets to workers based on subtask complexity
- Guardrails: citation verification (every claim must have a source), factuality checks via cross-referencing

**Results:**
- Report quality rated 4.2/5 by domain experts (vs 3.1/5 for single-agent)
- Average research task: 12 worker invocations across 3 agent types
- 15% of reports required human revision (down from 45% for single-agent)
- Cost: $0.30-1.50 per research report (vs $5-15 for human researcher-hours)

**Key Lesson:** The coordinator's planning quality was the single biggest factor in output quality. When the coordinator produced a poor task decomposition, workers executed well but on the wrong subtasks. Solution: added a "plan review" step where the coordinator self-critiques its plan before delegation, reducing poor decompositions from 25% to 8%.
...
Tool discovery via MCP expands blast radius. Every agent deployment MUST define:

| Boundary | Policy |
|----------|--------|
| **Trusted vs Untrusted MCP Servers** | Maintain an allow-list of verified MCP server origins |
| **Read-Only vs Write-Capable Tools** | Default to read-only; write tools require explicit approval |
| **Approval-Required Actions** | High-blast-radius actions (delete, transfer, publish) require human confirmation |
| **Data Residency** | Data sent to remote MCP servers may leave your compliance boundary |
| **Tool Allow-Lists** | Use `allowed_tools` to restrict which tools an agent can invoke |

## Vendor Freshness Notice

> **Last vendor refresh:** 2026-03-21
>
> **Stable architectural lessons:** Tool runtime patterns, approval flows, memory architecture, eval-driven development
>
> **Fast-moving vendor examples:** Specific API names, pricing, model capabilities — verify against current documentation before citing

**Note:** The OpenAI Assistants API was deprecated August 26, 2025 (sunset August 2026). Current guidance recommends the Responses API for all new projects. This doc focuses on stable patterns that transcend specific vendor APIs.
