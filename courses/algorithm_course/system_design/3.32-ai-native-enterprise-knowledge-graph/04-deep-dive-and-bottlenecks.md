...
| Over-retrieval | Slow, irrelevant context | Too many hops, no Cutting off unnecessary steps | Add relevance scoring, prune early |
...
| **Graph Traversal** | High P95 latency (>500ms) | Dense neighborhoods (super-nodes) | Add hop-by-hop Cutting off unnecessary steps, cache hot entities |
...
### Slowest part of the process Prioritization Matrix

```mermaid
quadrantChart
    title Slowest part of the process Priority (Impact vs Effort)
...
### Case Study 1: Microsoft GraphRAG — Local, Global, and DRIFT Search Modes

**Context:** Microsoft Research developed GraphRAG to address the fundamental limitation of vanilla RAG: inability to answer questions requiring synthesis across multiple documents (e.g., "What are the major themes in this dataset?").

**Architecture Choices:**
- **Indexing phase**: LLM extracts entities and relationships from text chunks, then Leiden algorithm detects hierarchical communities. Each community receives an LLM-generated summary
- **Local search**: Identifies entities in the query, traverses K hops, and assembles entity-centric context. Best for specific factual questions ("Who leads Project X?")
- **Global search**: Distributes the query across all community summaries using map-reduce. Best for thematic questions ("What are the main research directions?")
- **DRIFT search** (2025): Dynamic, Iterative, Refined Traversal — starts with global context, identifies gaps, generates follow-up queries, and iterates until coverage exceeds a threshold. Best for complex multi-faceted questions

**Key Lessons:**
- Community summaries provide a compression mechanism that makes global search tractable over large graphs — without them, thematic queries would require traversing the entire graph
- The choice of Leiden over Louvain is deliberate: community quality directly determines summary quality, which is the backbone of global search
- DRIFT search closes the gap between local (too narrow) and global (too broad) by dynamically adapting the search scope

### Case Study 2: Glean — Enterprise Search with Personal Knowledge Layer

**Context:** Glean builds enterprise search that unifies knowledge across 50+ data sources. Their approach layers a personal knowledge graph on top of enterprise knowledge to provide personalized, contextual results.

**Architecture Choices:**
- **Shared enterprise graph**: Entities (people, projects, documents, concepts) and relationships extracted from all connected data sources (Slack, Jira, Confluence, Google Drive, etc.)
- **Personal interaction layer**: Per-user graph tracking search history, document access, collaboration patterns, and implicit expertise signals
- **Retrieval**: Combines semantic search (vector similarity) with graph-based relevance (entity connectivity, personal affinity) and freshness signals
- **Access control**: Query results are filtered by real-time permission checks against each source system's ACLs — the knowledge graph indexes content but respects source permissions

**Key Lessons:**
- The personal layer is lightweight (~5,000 edges per active user) but dramatically improves result relevance by incorporating user context
- Real-time permission filtering against source systems is architecturally expensive (API calls per result) but non-negotiable for enterprise trust — caching source permissions with short TTL (5 min) provides the necessary balance
- The semantic layer (ontology of entity types and relationship types) must be enterprise-customizable — different organizations have different entity taxonomies, and rigid schemas reduce adoption
