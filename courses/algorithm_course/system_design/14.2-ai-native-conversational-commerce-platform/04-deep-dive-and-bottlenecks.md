...
**Edge Case (Unusual or extreme situation):** A merchant running a flash sale might receive 10,000 customer messages in 60 seconds. All these messages hash to different partitions (different customer phones), so they distribute across the queue naturally. The Slowest part of the process is not the queue but the downstream services (catalog search, cart operations) that all serve the same merchant's data. The platform mitigates this with per-tenant connection pooling and circuit breakers that shed load for a tenant approaching its resource limits without affecting other tenants.
...
**Slowest part of the process: Entity extraction in informal chat language:**
...
### Slowest part of the process: Meta Catalog API Rate Limits
...
## Deep Dive 6: Multi-Tenant Resource Isolation — Preventing Noisy Neighbors in Shared Infrastructure

### The Noisy Neighbor Problem in Conversational Commerce

With 100,000 merchants on shared infrastructure, a single merchant running a Diwali flash sale (generating 10,000 customer messages in 60 seconds) can saturate shared resources—database connections, NLP inference GPU slots, cache capacity—degrading response latency for all other merchants. Unlike web applications where noisy-neighbor effects manifest as slower page loads (annoying but tolerable), in conversational commerce, a 5-second response delay in WhatsApp feels like the merchant is ignoring the customer.

### Resource Isolation Architecture

The platform implements isolation at three levels:

**Level 1: Queue-Level Isolation (coarse-grained)**

Each merchant is assigned to a tenant tier based on their subscription plan and traffic volume:

```
Tenant tier assignment:
  Tier 1 (Enterprise): >10,000 messages/day → Dedicated queue partitions
  Tier 2 (Growth): 1,000-10,000 messages/day → Shared partitions with weighted fair queuing
  Tier 3 (Starter): <1,000 messages/day → Shared default partitions

Worker allocation:
  Tier 1: Dedicated worker pool (min 4 workers per merchant)
  Tier 2: Shared worker pool with per-tenant max concurrency (8 concurrent messages)
  Tier 3: Shared worker pool with per-tenant max concurrency (2 concurrent messages)
```

**Level 2: Compute-Level Isolation (fine-grained)**

NLP inference requests carry a tenant_id and are routed through a tenant-fair scheduler:

```
Tenant-fair NLP scheduling:
  maintain per_tenant_queue[tenant_id] = FIFO (First-In-First-Out, like a line at a store) queue of inference requests
  maintain per_tenant_tokens[tenant_id] = token bucket (refill: 50 tokens/sec)

  ON new_inference_request(tenant_id, request):
    per_tenant_queue[tenant_id].enqueue(request)

  GPU_SCHEDULER loop:
    // Round-robin across tenants with available tokens
    FOR EACH tenant IN round_robin(active_tenants):
      IF per_tenant_tokens[tenant].consume(1):
        request = per_tenant_queue[tenant].dequeue()
        IF request != null:
          submit_to_gpu(request)
```

This ensures that a merchant generating 1,000 NLP requests/second cannot monopolize GPU capacity—their excess requests queue in the per-tenant queue while other merchants continue to get fair scheduling.

**Level 3: Database-Level Isolation (connection and query limiting)**

| Resource | Isolation Mechanism | Default Limit | Enterprise Override |
|---|---|---|---|
| Database connections | Per-tenant connection pool | 5 connections | 20 connections |
| Query execution time | Per-tenant query timeout | 2 seconds | 5 seconds |
| Cache memory | Per-tenant cache quota | 100 MB | 1 GB |
| Outbound API rate | Per-number WhatsApp limit (natural isolation) | 80 msg/sec | Same (Meta enforced) |

### Detecting and Mitigating Tenant Burst

The platform monitors per-tenant resource consumption in real-time:

```
Burst detection (per tenant, evaluated every 10 seconds):
  current_rate = messages_processed_last_10s / 10
  baseline_rate = avg_messages_per_second_last_7_days

  IF current_rate > 10 × baseline_rate:
    // Tenant experiencing unusual burst (flash sale, viral message)
    activate_burst_mode(tenant_id):
      1. Allocate additional queue partitions from burst pool
      2. Increase per-tenant worker concurrency limit by 2x
      3. Alert tenant dashboard: "High traffic detected — burst mode active"
      4. Monitor shared resource impact; if other tenants degraded, throttle burst tenant

  IF current_rate > 50 × baseline_rate AND other_tenants_p99 > SLA_THRESHOLD:
    // Burst is degrading other tenants — enforce hard limits
    throttle_tenant(tenant_id):
      1. Reduce per-tenant concurrency to baseline + 50%
      2. Queue excess messages with "experiencing high demand" auto-response
      3. Alert merchant: "Your traffic exceeds plan limits. Upgrade for higher capacity."
```

The key design principle is **optimistic isolation**: the platform allows tenants to burst above their baseline (flash sales are legitimate business events that the platform should support), but enforces hard limits only when the burst measurably degrades other tenants. This balances the needs of the bursting merchant (who wants maximum throughput during their sale) with the needs of other merchants (who expect consistent latency regardless of other tenants' traffic).
