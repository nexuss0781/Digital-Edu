...
Unlike web sessions where state is URL/cookie-driven, conversational commerce state exists entirely within the message stream. The Context Manager maintains a sliding window of conversation state per customer-merchant pair.

```Step-by-step plan in plain English
STRUCTURE ConversationContext:
    conversation_id: STRING       // {tenant_id}:{customer_phone_hash}
    last_message_timestamp: DATETIME
    session_window_expires: DATETIME  // 24-hour WhatsApp window
    language_preference: STRING   // detected from last 3 messages
    active_intent: ENUM           // current commerce flow in progress
    active_product_context: LIST  // last 3 products discussed (for coreference)
    cart_reference: UUID          // pointer to persistent cart
    pending_decision: ENUM        // awaiting_size_selection, awaiting_payment, etc.
    agent_assigned: UUID          // NULL if bot-handled
    sentiment_score: FLOAT        // rolling average from last 5 messages
    escalation_count: INTEGER     // times escalated in this session

FUNCTION update_context(conversation_id, classified_message):
    context = cache.get(conversation_id)
    IF context IS NULL:
        context = load_from_db(conversation_id) OR create_new(conversation_id)

    // Update sliding product context (last 3 products discussed)
    IF classified_message.entities.product_id IS NOT NULL:
        context.active_product_context.prepend(classified_message.entities.product_id)
        context.active_product_context = context.active_product_context[:3]

    // Update language preference (majority vote of last 3 messages)
    context.language_preference = majority_vote(
        [classified_message.language] + get_recent_languages(conversation_id, count=2))

    // Update session window
    IF classified_message.direction == "inbound":
        context.session_window_expires = now() + 24_HOURS
        context.last_message_timestamp = now()

    // Update sentiment
    context.sentiment_score = 0.7 * context.sentiment_score + 0.3 * classified_message.sentiment

    cache.set(conversation_id, context, ttl=25_HOURS)
    RETURN context
```

### Coreference Resolution for Chat Context

```Step-by-step plan in plain English
FUNCTION resolve_coreference(message_text, entities, context):
    // "ye kitne ka hai" → "this" refers to the last product shown
    // "wo wala" → "that one" refers to a previously discussed product
    // "dono" → "both" refers to the last 2 products

    pronouns = detect_pronouns(message_text)  // "ye", "wo", "it", "this", "both"

    FOR EACH pronoun IN pronouns:
        SWITCH pronoun.type:
            CASE "proximal":  // "ye", "this", "it"
                entities.product_id = context.active_product_context[0]
            CASE "distal":    // "wo", "that"
                entities.product_id = context.active_product_context[1]
                IF entities.product_id IS NULL:
                    entities.product_id = context.active_product_context[0]
            CASE "plural":    // "dono", "both", "sab"
                entities.product_ids = context.active_product_context[:2]
            CASE "ordinal":   // "pehle wala" (first one), "doosra" (second one)
                index = extract_ordinal_index(pronoun)
                entities.product_id = context.active_product_context[index]
...
```Step-by-step plan in plain English
FUNCTION onboard_merchant(merchant_request):
    // Step 1: Create tenant
    tenant = create_tenant(merchant_request.business_name, merchant_request.phone)

    // Step 2: WhatsApp Business API setup
    waba_account = provision_waba(merchant_request.phone, merchant_request.business_details)
    register_webhook(waba_account.id, platform_webhook_url)

    // Step 3: Default configuration
    create_default_bot_flows(tenant.id)        // greeting, FAQ, order status
    submit_default_templates(tenant.id)        // order confirmation, shipping, payment
    configure_operating_hours(tenant.id, merchant_request.business_hours)

    // Step 4: Catalog import (if merchant has existing products)
    IF merchant_request.catalog_csv IS NOT NULL:
        schedule_catalog_import(tenant.id, merchant_request.catalog_csv)

    // Step 5: Payment setup
    configure_payment_gateway(tenant.id, merchant_request.upi_id)

    // Step 6: Warm caches
    preload_tenant_config(tenant.id)

    emit_event("MerchantOnboarded", { tenant_id: tenant.id })
    RETURN { tenant, waba_account, dashboard_url }
```
...
| Level | Trigger | Degraded Behavior | Customer Experience |
|-------|---------|-------------------|-------------------|
| 0 (Normal) | All systems healthy | Full AI + commerce + broadcast | Seamless conversational commerce |
| 1 (AI Degraded) | NLP service latency >2s | Route all messages to keyword matching | Slightly less natural responses; commerce still works |
| 2 (Commerce Degraded) | Catalog/order service down | AI responds to queries; cart/order operations queued | "We're processing your request, you'll receive confirmation shortly" |
| 3 (Broadcast Paused) | Quality rating drops to Yellow | Pause all marketing broadcasts | Conversational commerce continues; campaigns delayed |
| 4 (Read-Only) | Database primary failure | Read conversation history; cannot create orders | "We're experiencing issues. Please try again in a few minutes" |
| 5 (Emergency) | Webhook ingestion failure | Static auto-reply to all messages | "We received your message and will respond shortly" |
