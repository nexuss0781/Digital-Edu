...
2. "What's the Slowest part of the process at 10x scale?" → NLP inference (GPU-bound); outbound gateway (WhatsApp API rate limits); database write throughput. NLP scales with model quantization + more GPUs. API limits are hard constraints that require creative solutions (multi-number strategy).
...
### 1. "Just use an LLM for everything — it can handle all customer messages."

**Wrong answer:** "GPT-4 can understand any language and handle any query, so we'll just send all messages to the LLM."

**Correct answer:** "LLMs are unreliable for transactional operations. If a customer says 'add blue kurta to cart,' the LLM might respond 'I've added the blue kurta to your cart!' without actually modifying the cart state — it's generating plausible text, not executing a transaction. Commerce operations (cart updates, order creation, payment initiation) must use deterministic flows with guaranteed state mutations. The LLM handles only open-ended queries (product recommendations, policy questions, complaint resolution) where approximate responses are acceptable and guardrails prevent hallucinated prices or unauthorized discounts."

### 2. "Process webhooks synchronously — it's simpler."

**Wrong answer:** "Parse the webhook, run NLP, execute business logic, send response, then return 200."

**Correct answer:** "WhatsApp retries if we don't acknowledge within 20 seconds. NLP classification alone takes 100-500ms, business logic 200ms, and the outbound API call 300ms. Under load, this chain takes 1-3 seconds — fine in isolation, but queued requests during broadcast reply storms could push acknowledgment past 20 seconds. Repeated failures cause Meta to disable our webhook. The webhook handler must do nothing except validate the signature, deduplicate, enqueue, and respond 200."

### 3. "Cart state can live in the conversation context only."

**Wrong answer:** "Store the cart as part of the conversation state — when the conversation ends, the cart is gone."

**Correct answer:** "Customers return to complete purchases across multiple sessions. A customer browsing kurtas at night may come back the next day to purchase. The cart must persist independently of the conversation context with a 7-day TTL. However, the conversation context must reference the cart so that when the customer says 'checkout' in a new session, the platform loads the saved cart. This separation between session state (24-hour context) and commerce state (7-day cart) is critical."

### 4. "Broadcast to all contacts for maximum reach."

**Wrong answer:** "Send to everyone — wider reach means more sales."

**Correct answer:** "WhatsApp enforces a 2-marketing-message-per-user-per-24-hour limit. More critically, contacts who haven't engaged in months have high block/report rates. Sending to disengaged contacts degrades quality rating, which triggers sending capacity reduction — a negative feedback loop. The broadcast engine must filter by engagement score, send in progressive waves (testing quality signals with engaged contacts first), and implement a quality circuit breaker that pauses campaigns if the rating drops."

### 5. "Multi-tenancy is just adding tenant_id to database tables."

**Wrong answer:** "Add tenant_id to every table and we're done."

**Correct answer:** "The harder multi-tenancy problem is the outbound gateway. Merchant A running a 500K-contact broadcast campaign can saturate the shared outbound API capacity, causing Merchant B's conversational responses to queue for 30+ seconds. The outbound gateway needs weighted fair queuing (conversational messages get 80% of capacity), per-merchant rate limiting, and broadcast deprioritization. Similarly, the NLP inference pool must be tenant-aware — a merchant's viral product post generating 10K simultaneous customer messages shouldn't starve other merchants' inference capacity."
...
### Key Numbers
- Merchants: 100K; Messages/day: 130M; Orders/day: 2M
- Peak throughput: 22,500 msg/sec; Concurrent conversations: 500K
- Webhook acknowledgment: ≤5s; Message processing: ≤2s; Intent classification: ≤500ms
- Broadcast throughput: 1M messages in 60 minutes
- WhatsApp rate limit: 80 msg/sec per phone number
- Template approval: 1-24 hours; Frequency cap: 2 marketing/user/24h

### Critical Trade-Offs (One-Liners)
- Sync vs. async webhook processing → async (20-second timeout constraint)
- LLM-only vs. hybrid AI → hybrid (deterministic for transactions, LLM for open-ended)
- Real-time vs. eventual catalog sync → eventual with priority queue (API rate limits)
- Precision vs. recall in search → recall-first (zero results kills conversations)
- Maximum vs. progressive broadcast → progressive (quality rating protection)

### Red Flags in Candidate Answers
- Processing business logic inside the webhook handler
- Using LLM for cart/order operations without deterministic validation
- Ignoring WhatsApp's template approval and frequency cap constraints
- Designing English-only NLP for the Indian market
- No strategy for quality rating protection during broadcasts
- Missing idempotency beyond webhook deduplication
- No priority differentiation in outbound message queue
