...
### AI Commerce Safety — Financial Guardrails

Beyond prompt injection, the conversational AI requires commerce-specific safety boundaries that prevent the AI from making financially damaging decisions even without malicious intent:

**Discount and pricing guardrails:**

```
Commerce safety validation (applied to every AI-generated action):

  ON ai_action(action_type, parameters):
    MATCH action_type:
      CASE "apply_discount":
        IF parameters.discount_percent > merchant.max_ai_discount (default 10%):
          REJECT action
          RESPOND "I can offer up to {max}% discount. For a larger discount,
                   let me connect you with our team."
        IF parameters.discount_amount > merchant.max_discount_value (default ₹500):
          REJECT action

      CASE "quote_price":
        actual_price = catalog.get_price(parameters.product_id)
        IF parameters.quoted_price != actual_price:
          REJECT action  // AI hallucinated a different price
          USE actual_price instead

      CASE "confirm_availability":
        actual_stock = catalog.get_stock(parameters.product_id)
        IF actual_stock == 0 AND parameters.claimed_available == true:
          REJECT action  // AI claimed product is available when it is not

      CASE "promise_delivery":
        IF parameters.promised_date < merchant.min_delivery_days:
          REJECT action  // AI promised faster delivery than merchant can fulfill
```

**Merchant-configurable policy boundaries:**

| Policy | Default | Merchant Configurable | Enforcement |
|---|---|---|---|
| Maximum AI-authorized discount | 10% | Yes (0-25%) | Hard block — AI cannot exceed |
| Price negotiation allowed | No | Yes/No | If disabled, AI declines all negotiation requests |
| COD allowed | Yes | Yes/No | AI cannot offer COD if merchant disabled it |
| Return window communicated | 7 days | Configurable | AI quotes exact merchant policy, not generic answers |
| Delivery estimate buffer | +2 days | Configurable | AI adds buffer to actual estimate to prevent over-promising |
| Cross-sell in support conversations | No | Yes/No | AI does not upsell during complaint resolution unless explicitly allowed |

These guardrails ensure that even if the LLM generates a plausible-sounding response ("Sure, I can give you 50% off!"), the deterministic validation layer prevents it from being executed. The AI can only operate within the merchant's explicitly configured policy boundaries—making the LLM's creativity useful for conversation quality while keeping financial decisions under deterministic control.
