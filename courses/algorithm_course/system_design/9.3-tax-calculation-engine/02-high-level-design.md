...
**Rate aggregation logic (Step-by-step plan in plain English):**
```
...
### 5.5 Audit Trail and Determination Archive

Every tax calculation produces an immutable **tax determination record** capturing all inputs (addresses, product tax codes, customer identifiers), intermediate results (resolved jurisdictions, applied rates, evaluated exemptions), and the final output (tax amounts per jurisdiction per line item). These records serve as the legal basis for the tax charged and must be retrievable for the full statute of limitations period (typically 7 years).

The archive uses append-only storage with cryptographic chaining---each record includes a hash of the prior record for tamper detection. Records are indexed by tenant, transaction date, and jurisdiction for efficient retrieval during audits. A separate **recalculation index** tracks which rate version and jurisdiction boundary version were used, enabling deterministic reproduction of any historical calculation.

---

## 6. Cross-Border Tax Handling

### VAT Reverse Charge Mechanism

For B2B transactions across international borders, the tax liability shifts from the seller to the buyer via reverse charge. The engine must detect reverse charge scenarios and produce zero-rated invoices with the correct compliance annotations:

```
FUNCTION determine_cross_border_treatment(seller, buyer, destination_country):
    IF seller.country == buyer.country:
        RETURN DOMESTIC    // standard tax rules apply

    // Intra-EU: check if buyer has valid VAT registration
    IF is_eu_member(seller.country) AND is_eu_member(buyer.country):
        IF buyer.vat_id IS NOT NULL AND validate_vat_id(buyer.vat_id):
            RETURN REVERSE_CHARGE    // zero-rated for seller; buyer self-assesses
        ELSE:
            RETURN DESTINATION_VAT    // seller charges destination country VAT (OSS/IOSS)

    // Non-EU cross-border: check for tax treaty provisions
    treaty = tax_treaty_registry.lookup(seller.country, buyer.country)
    IF treaty IS NOT NULL AND treaty.covers_transaction_type(txn_type):
        RETURN treaty.treatment    // EXEMPT, REDUCED, or STANDARD

    // Default: export zero-rated for seller; import duties/taxes at destination
    RETURN EXPORT_ZERO_RATED
```

### Multi-Currency Tax Computation

Cross-border transactions require currency conversion for tax calculation and reporting. The engine uses the exchange rate applicable on the transaction date per the destination jurisdiction's rules:

```
FUNCTION compute_tax_in_reporting_currency(tax_amount, txn_currency, reporting_currency, txn_date, jurisdiction):
    IF txn_currency == reporting_currency:
        RETURN tax_amount

    // Jurisdiction determines which exchange rate source to use
    rate_source = jurisdiction.exchange_rate_authority    // e.g., ECB for EU, central bank rates
    exchange_rate = fx_rate_service.get_rate(
        from = txn_currency, to = reporting_currency,
        date = txn_date, source = rate_source
    )

    converted = ROUND(tax_amount * exchange_rate, jurisdiction.rounding_precision)
    RETURN { converted_amount: converted, exchange_rate, rate_source, rate_date: txn_date }
```

Jurisdictions vary in which exchange rate date to use (transaction date, invoice date, or period-average for returns). The EU allows monthly average rates for VAT returns but requires transaction-date rates for individual invoices. The engine stores both the original-currency and reporting-currency amounts for every tax determination to support audit reconciliation.
