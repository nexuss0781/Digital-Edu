...
| Component | Slowest part of the process | Symptom | Mitigation |
|-----------|-----------|---------|------------|
...
### Slowest part of the process 1: Rate Table Cache Invalidation Stampede
...
### Slowest part of the process 2: Nexus Counter Contention
...
### Slowest part of the process 3: Exemption Certificate Validation
...
### Audit Defense and Recalculation

When a tax authority audits a seller, the engine must reproduce the exact calculation that produced a historical tax amount---using the rates, rules, and jurisdictions that were in effect at the original transaction time (not current rates). Bi-temporal querying makes this deterministic:

```
FUNCTION reproduce_historical_calculation(transaction_id):
    txn = QUERY TaxTransaction WHERE transaction_id
    determination = QUERY TaxDetermination WHERE transaction_id

    // Re-run calculation using the exact rate versions from the original determination
    reproduced = calculate_tax_with_version(
        request = reconstruct_request(txn),
        rate_version = determination.rate_version,
        jurisdiction_version = determination.jurisdiction_version,
        as_of_date = txn.transaction_date
    )

    // Compare reproduced vs original
    delta = diff_tax_results(determination.original_result, reproduced)
    IF delta.total_difference != 0:
        FLAG_FOR_REVIEW(transaction_id, delta, reason = "reproduction_mismatch")

    RETURN {
        original: determination.original_result,
        reproduced,
        delta,
        rate_version_used: determination.rate_version,
        jurisdiction_boundary_version: determination.jurisdiction_version
    }
```

This capability also drives the **recalculation pipeline**: when a rate correction is published (e.g., an encoding error is discovered), the engine identifies all transactions affected by the corrected rate, re-runs them against the corrected rate package, and computes adjustment amounts for the next filing period.
