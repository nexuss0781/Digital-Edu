...
3. **Apply Costing** - The Costing Engine calculates the cost impact. For receipts, a new cost layer is created. For issues, cost layers are consumed per the configured method (FIFO (First-In-First-Out, like a line at a store), LIFO (Last-In-First-Out, like a stack of plates), FEFO, WAC)
...
- **FIFO (First-In-First-Out, like a line at a store) (First In, First Out)** - Consumes the oldest cost layer first. Most common for perishables and general merchandise.
- **LIFO (Last-In-First-Out, like a stack of plates) (Last In, First Out)** - Consumes the newest cost layer first. Used in specific tax jurisdictions.
...
The CQRS read side maintains multiple projections optimized for different query patterns:
...
flowchart LR
    ...
    classDef readmodel fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class ES event
    class P1,P2,P3,P4,P5 projector
    class RM1,RM2,RM3,RM4,RM5 readmodel
```

| Read Model | Staleness Budget | Storage | Primary Consumers |
|---|---|---|---|
| **ATP Cache** | < 1 second | Distributed KV store with TTL | E-commerce product pages, POS systems |
| **Stock Snapshot** | < 5 seconds | Read replica of write DB | WMS mobile app, dashboard, reports |
| **Cost Ledger** | < 30 seconds | Relational DB (dedicated instance) | Finance team, period-end valuation |
| **Location Heat Map** | < 1 minute | Time-series DB | Warehouse layout optimization, slotting |
| **Analytics Aggregate** | < 5 minutes | Columnar analytics store | Business intelligence, demand planning |

Each projector maintains a **checkpoint** — the last event sequence number it has processed. On restart, the projector resumes from its checkpoint, replaying any events it missed. Projectors are idempotent: processing the same event twice produces the same result (using event sequence numbers as idempotency keys).

### Consistency Guarantees by Query Type

| Query | Consistency Model | Rationale |
|---|---|---|
| "Is this product in stock?" (browsing) | Eventually consistent (ATP cache) | Tolerable — reservation validates at checkout |
| "Reserve N units" (checkout) | Strongly consistent (write path) | Must not oversell |
| "What is current stock at location?" (picking) | Strongly consistent (write DB) | Picker relies on accurate location data |
| "Total inventory valuation?" (finance) | Point-in-time consistent (cost ledger) | Must match a specific reporting instant |
| "Movement history for SKU" (audit) | Fully consistent (event log) | Regulatory requirement — no data can be missed |
...
sequenceDiagram
    ...
    participant GW as API Gateway
    participant RMS as Returns Service
    ...
    CS->>GW: Initiate Return (RMA)
    GW->>RMS: Create Return Authorization
    RMS->>RMS: Validate Return Policy
    RMS-->>CS: Return Label + Instructions

    CS->>GW: Ship Return / Bring to Store
    GW->>RMS: Receive Return at Dock
    RMS->>QC: Quality Inspection
    QC->>QC: Determine Disposition

    alt Restock
        QC->>INV: Putaway as Available
        INV->>CE: Reinstate Cost Layer
        INV->>EB: Emit ReturnRestocked
    else Refurbish
        QC->>INV: Move to Refurbish Zone
        INV->>EB: Emit ReturnRefurbishing
    else Scrap / Donate
        QC->>INV: Write Off Inventory
        INV->>CE: Consume Cost Layer (Write-off)
        INV->>EB: Emit ReturnScrapped
    end

    EB->>QS: Update ATP + Stock Levels
```

### Return Disposition Decision Matrix

| Condition | Category | Disposition | Cost Treatment |
|---|---|---|---|
| Unopened, within 30 days | A-grade | Restock at original location | Reinstate original cost layer |
| Opened, undamaged, within 30 days | B-grade | Restock in secondary zone | Reinstate at reduced value (90% of original) |
| Cosmetic damage only | C-grade | Refurbish zone | New cost layer at refurb cost |
| Functional damage | D-grade | Vendor return or scrap | Cost write-off to shrinkage account |
| Past return window | Rejected | Ship back to customer | No inventory impact |
| Recall item | Recall | Quarantine for investigation | Cost write-off to recall expense |

---

## Multi-Warehouse Network Topology

For organizations with multiple fulfillment centers, the system manages a network of interconnected warehouses:
...
Network topology model:
  warehouse_network {
      ...
      warehouse_type     ENUM('DISTRIBUTION_CENTER', 'FULFILLMENT_CENTER',
                              'REGIONAL_HUB', 'STORE', 'CROSS_DOCK')
      latitude DECIMAL(10,6), longitude DECIMAL(10,6)
      timezone           VARCHAR(50)
      carrier_cutoff_times   JSONB       -- per-carrier, per-service-level
      operating_hours        JSONB       -- day-of-week schedules
      max_daily_throughput   INT         -- pick capacity per day
      current_utilization    DECIMAL(5,2)
      supported_channels     TEXT[]      -- ['ECOMMERCE', 'WHOLESALE', 'RETAIL']
  }

  warehouse_lane {
      lane_id            BIGINT PK
      source_warehouse   VARCHAR(10) FK
      dest_warehouse     VARCHAR(10) FK
      transit_days       INT
...
**Network-Level ATP Aggregation:**

When a customer queries product availability, the ATP engine aggregates across the fulfillment network:

```
FUNCTION network_atp(sku_id, customer_location, channel):
    warehouses = GET eligible warehouses for channel
    results = []

    FOR EACH wh IN warehouses:
        local_atp = get_atp(sku_id, wh.warehouse_id, channel)
        IF local_atp > 0:
            transit = estimate_transit(wh, customer_location)
            results.APPEND({
                warehouse_id: wh.warehouse_id,
                available_qty: local_atp,
                delivery_days: transit.days,
                shipping_cost: transit.cost
            })

    -- Sort by fastest delivery, then lowest cost
    SORT results BY delivery_days ASC, shipping_cost ASC
    RETURN results
```
...
| External System | Integration Pattern | Direction | Frequency |
|---|---|---|---|
| **ERP / GL** | Event-driven via message queue | Outbound | Real-time (inventory valuation events) |
| **E-Commerce Platform** | REST API + webhooks | Bidirectional | Real-time (ATP queries, order allocation) |
| **Supplier Systems** | EDI / API (ASN, PO confirmation) | Inbound | Batch (hourly) + real-time (ASN) |
| **Shipping Carriers** | API integration | Outbound | Real-time (label generation, tracking) |
| **POS Systems** | Real-time API | Bidirectional | Real-time (stock check, sale deduction) |
| **Demand Planning** | Batch export | Outbound | Daily (movement history, stock snapshots) |
| **Quality Management** | Event-driven | Bidirectional | Real-time (hold/release events) |
