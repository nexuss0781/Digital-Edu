...
Flash sale traffic follows a distinctive pattern requiring specialized scaling:

```
Flash sale timeline:
  T-2h:    Pre-scale reservation engine to 10x baseline
  T-0:     Sale opens. Reservation rate spikes from 100 TPS to 50,000 TPS
  T+30s:   Bucket exhaustion begins; rebalancer activates
  T+5m:    Popular SKUs sell out; reservation rate drops to 5,000 TPS
  T+30m:   Long-tail SKUs continue; rate stabilizes at 1,000 TPS
  T+2h:    Sale ends; scale-down begins (conservative, over 30 minutes)

Key constraint: ATP cache must be < 1 second stale during the sale.
  Solution: ATP projector runs synchronously for flash-sale SKUs,
  bypassing the normal async projection pipeline. This trades
  projection throughput for freshness during the critical window.
```
