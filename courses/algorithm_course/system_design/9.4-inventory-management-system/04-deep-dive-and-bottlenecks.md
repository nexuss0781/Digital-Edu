...
## 3. FIFO (First-In-First-Out, like a line at a store)/LIFO (Last-In-First-Out, like a stack of plates)/FEFO Cost Layer Management

### The Problem

A single SKU may accumulate hundreds of cost layers from different purchase orders at different prices. When items are sold, the correct cost layer must be consumed according to the costing method (FIFO (First-In-First-Out, like a line at a store), LIFO (Last-In-First-Out, like a stack of plates), FEFO, Weighted Average, or Standard Cost). Incorrect consumption produces wrong COGS, cascading into financial misstatement and audit failures.
...
flowchart LR
    ...
    C --> D{Costing Method}
    D -->|FIFO (First-In-First-Out, like a line at a store)| E[Consume Oldest First]
    D -->|LIFO (Last-In-First-Out, like a stack of plates)| F[Consume Newest First]
...
Each cost layer contains: `layer_id`, `sku_id`, `warehouse_id`, `quantity_remaining`, `unit_cost`, `received_date`, `expiry_date`, `purchase_order_id`. Layer ordering is determined by costing method: **FIFO (First-In-First-Out, like a line at a store)** (`received_date` ascending), **LIFO (Last-In-First-Out, like a stack of plates)** (`received_date` descending), **FEFO** (`expiry_date` ascending---critical for perishables).
...
**Edge Case (Unusual or extreme situation) at zero inventory**: WAC resets on next receipt after stock fully depletes, preventing stale costs from distorting valuation.
