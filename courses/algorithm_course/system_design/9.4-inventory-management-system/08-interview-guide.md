...
"Design an inventory management system that tracks stock across multiple warehouses, supports different costing methods (FIFO (First-In-First-Out, like a line at a store), LIFO (Last-In-First-Out, like a stack of plates), WAC), and provides real-time availability to e-commerce channels. The system should handle flash-sale traffic, support omnichannel retail, and maintain an auditable movement ledger for financial compliance."
...
| 2 | Which costing methods must be supported (FIFO (First-In-First-Out, like a line at a store), LIFO (Last-In-First-Out, like a stack of plates), WAC, FEFO)? | Cost layer data model and consumption logic vary significantly; FEFO adds expiry tracking |
...
For flash-sale scenarios, introduce reservation bucketing: pre-partition 10,000 reservable units into 100 buckets of 100, each assigned to a reservation processor. This eliminates the single-row contention Slowest part of the process.
...
- **FIFO (First-In-First-Out, like a line at a store)**: consume the oldest layer first (dequeue from head)
- **LIFO (Last-In-First-Out, like a stack of plates)**: consume the newest layer first (dequeue from tail)
- **WAC**: maintain a weighted average cost, updated on each receipt
- **FEFO**: consume the layer with the earliest expiry date first

Show Step-by-step plan in plain English for FIFO (First-In-First-Out, like a line at a store) consumption:
...
4. **Cost layer impact** — the adjustment must consume cost layers (using the configured costing method) so that COGS and inventory valuation remain accurate. Five units at FIFO (First-In-First-Out, like a line at a store) cost are written off.
...
|---|---------|---------------------|
| 1 | Using a single database row with `UPDATE quantity = quantity - 1` for reservations | Creates a serialization Slowest part of the process that caps throughput regardless of hardware scale |
...
- Aware that FIFO (First-In-First-Out, like a line at a store) exists as a costing method but does not detail the data model
