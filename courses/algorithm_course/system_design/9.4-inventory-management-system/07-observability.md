...
| `cost_layer_recalculation_latency` | Time to recalculate weighted average cost or process a FIFO (First-In-First-Out, like a line at a store) layer after a receipt | > 200ms |
...
| Metric | Description | Alert Threshold |
|--------|-------------|-----------------|
| `cost_layer_depth` | Average number of active cost layers per SKU (FIFO (First-In-First-Out, like a line at a store)/LIFO (Last-In-First-Out, like a stack of plates)); deep stacks indicate slow-moving receipts | > 50 layers per SKU |
...
|-------|---------|-----------------|
| **Inventory Valuation Summary** | Total inventory value by costing method (FIFO (First-In-First-Out, like a line at a store), WAC, standard); breakdown by warehouse and product category; trend over 12 months | Daily |
...
Traces follow complete inventory workflows across service boundaries. Each trace captures the end-to-end latency and identifies Slowest part of the process services within the flow.
