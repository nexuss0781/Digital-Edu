...
## LLM Fine-Tuning Workflow Architecture (2025+ Pattern)

The emergence of LLM fine-tuning as a primary ML workload introduces a distinct architectural pattern within Metaflow:

```
Typical LLM Fine-Tuning DAG:

  start → data_prep → tokenize → train → evaluate → register → end
                         ↓                   ↓
                    (foreach: shards)   (checkpoint: every 500 steps)

Resource profile:
  data_prep:  @resources(cpu=16, memory=64000)         # CPU-heavy
  tokenize:   @resources(cpu=32, memory=128000)         # CPU + memory
  train:      @resources(gpu=4, memory=256000)          # Multi-GPU, hours-long
  evaluate:   @resources(gpu=1, memory=64000)           # Single GPU
```
...
|---|---|
| **Train step produces 30-100GB checkpoints** | In-task `@checkpoint` decorator with configurable interval (every 500 training steps) |
| **Multi-GPU coordination** | Single step uses PyTorch DDP / DeepSpeed internally; Metaflow manages the outer job |
| **Cost sensitivity** | Spot/preemptible instances with checkpoint-based recovery; Metaflow `@retry` + `@checkpoint` combination |
| **Experiment tracking** | Config system parameterizes hyperparameters; metadata service stores experiment lineage |
| **Model registry integration** | `register` step publishes to external model registry with artifact hash as provenance link |
