...
|----------|-------------|
| **What AI decides alone** | DAG step scheduling and parallelization, automatic checkpointing, data artifact versioning, compute resource allocation for individual steps |
| **What AI recommends** | Workflow retry strategies, resource sizing suggestions based on historical runs, step timeout recommendations |
| **What requires human approval** | Workflow definition and deployment, production schedule creation (Maestro), infrastructure configuration changes, namespace and access policy modifications |
| **Deterministic source of truth** | Metadata Service (run/step/task state in PostgreSQL) and Datastore (content-addressed artifact storage) — the framework orchestrates execution; data scientists define and control all workflow logic |
| **Rollback path** | Step-level resume from last checkpoint; workflow re-run from any previous step; artifact version rollback via content-addressed storage; production schedule deactivation in Maestro |
...
| [04 - Deep Dive & Bottlenecks](./04-deep-dive-and-bottlenecks.md) | Checkpointing, data versioning, Slowest part of the process analysis |
...
| Related System | Shared Pattern | Link |
|---|---|---|
| [MLOps Platform](../3.4-mlops-platform/00-index.md) | Generic MLOps lifecycle concepts; Metaflow implements the workflow orchestration layer of the broader MLOps stack |
| [Uber Michelangelo](../3.5-uber-michelangelo-ml-platform/00-index.md) | Feature store integration and model serving; contrasts Metaflow's workflow-first approach with Michelangelo's feature-first approach |
| [Distributed Job Scheduler](../2.6-distributed-job-scheduler/00-index.md) | DAG execution and topological scheduling patterns; Metaflow's step scheduler mirrors generic job scheduling with ML-specific checkpointing |
| [CI/CD Pipeline](../2.4-cicd-pipeline-build-system/00-index.md) | DAG-based build systems share the same topological execution model; CI/CD pipelines use similar artifact versioning and caching strategies |
| [Event Sourcing System](../1.18-event-sourcing-system/00-index.md) | Metaflow's metadata service is an event log of run/step/task transitions; artifact versioning mirrors event sourcing's immutable event store |
| [Blob Storage System](../1.12-blob-storage-system/00-index.md) | Content-addressed storage patterns; Metaflow's artifact datastore uses the same hash-based deduplication as blob storage systems |
| [Feature Flag Management](../2.7-feature-flag-management/00-index.md) | Configuration propagation across execution environments; the Config system shares patterns with hierarchical feature flag evaluation |
