...
| GPU utilization | < 40% average | Implement GPU sharing / fractional GPUs |
| Foreach task failure rate | > 5% | Investigate worker stability, adjust retry policies |

---

## 2025-2026 Scale Reference Points

| Organization Type | Daily Runs | Active Projects | Avg Foreach Cardinality | Primary Compute |
|---|---|---|---|---|
| **Large tech (Netflix-scale)** | 50K+ | 3,000+ | 500-5,000 | Batch + Kubernetes |
| **Mid-size ML team** | 5K-10K | 200-500 | 100-1,000 | Kubernetes |
| **Startup / small team** | 500-2K | 20-50 | 10-100 | Local + cloud burst |
| **Enterprise (Outerbounds)** | 10K-30K | 500-1,500 | 200-2,000 | Managed Kubernetes |

The emergence of LLM fine-tuning workflows (2024-2025) has shifted compute profiles: single-step GPU-heavy runs producing 30-100GB model artifacts are now the dominant resource consumer, replacing the previous pattern of many small CPU-bound data processing steps.
