...
│  │                                                               │
│  └── Network Slowest part of the process?                                        │
...
## Kubernetes-Native Deployment (2025-2026)

### Cluster Operator Architecture
...
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  RabbitmqCluster CRD → Cluster Operator → Reconciliation Loop   │
│                                                                   │
│  Managed Resources:                                               │
│  ┌─────────────────┐  ┌─────────────────┐  ┌────────────────┐  │
│  │  StatefulSet     │  │  Headless Svc   │  │  ConfigMap     │  │
│  │  (3 replicas)    │  │  (peer discovery)│  │  (rabbitmq.conf│  │
│  │  rabbitmq-0      │  │  rabbitmq-nodes │  │   + advanced)  │  │
│  │  rabbitmq-1      │  │  .svc.cluster   │  │               │  │
│  │  rabbitmq-2      │  │  .local         │  │               │  │
│  └─────────────────┘  └─────────────────┘  └────────────────┘  │
│                                                                   │
│  Automated Operations:                                            │
│  • Peer discovery via DNS SRV (headless Service)                 │
│  • Rolling upgrades (drain → detach → upgrade → rejoin)          │
│  • Quorum queue leader rebalancing post-upgrade                  │
│  • TLS certificate rotation via cert-manager                     │
│  • PVC management for persistent volumes                         │
│                                                                   │
│  Anti-Patterns Prevented:                                         │
│  • Manual join_cluster / forget_cluster_node                     │
│  • Uncoordinated restarts during partition                       │
│  • Orphaned quorum queue members                                 │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

### Operator vs Manual Cluster Management

| Operation | Manual | K8s Operator |
|-----------|--------|-------------|
| **Cluster formation** | rabbitmqctl join_cluster per node | Automatic via headless Service DNS |
| **Rolling upgrade** | Drain → stop → upgrade → start → rejoin (per node) | kubectl apply new image tag (operator handles sequence) |
| **Node failure** | Detect, forget, reseed data | Pod restart + automatic rejoin |
| **Quorum rebalance** | rabbitmq-queues rebalance manually | Automated post-upgrade reconciliation |
| **TLS rotation** | Manual cert replacement + restart | cert-manager integration, zero-downtime |
| **Scaling** | Manual node addition + policy migration | Update replicas in CRD spec |
