...
### Slowest part of the process 1: Single-Threaded Queue

```
┌─────────────────────────────────────────────────────────────────┐
│                    SINGLE QUEUE Slowest part of the process                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Problem:                                                        │
...
### Slowest part of the process 2: Memory Pressure
...
### Slowest part of the process 3: Network Between Cluster Nodes

```
┌─────────────────────────────────────────────────────────────────┐
│                    CLUSTER NETWORK Slowest part of the process                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Problem:                                                        │
...
### Slowest part of the process 4: Disk I/O

```
┌─────────────────────────────────────────────────────────────────┐
│                    DISK I/O Slowest part of the process                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Problem:                                                        │
...
## Critical Component 5: Super Streams and Partitioned Consumption (2025-2026)

### The Problem

Classic RabbitMQ queues provide either strict ordering (single consumer, single queue) or parallelism (competing consumers, no ordering). There is no native mechanism for ordered consumption across multiple consumers by partition key — the primary use case that drives teams to deploy a separate log-based broker alongside their message queue.

### Super Streams Architecture
...
│  (offset: 1042)  (offset: 3201)  (offset: 891)                  │
│                                                                   │
│  Routing: hash(customer_id) % 3 → partition                      │
│  Ordering: Guaranteed within partition                            │
│  Replay: Seek to any offset per partition                        │
│  Consumer groups: Automatic rebalancing on scale                 │
│                                                                   │
│  vs Kafka Streams:                                               │
│  + Complex routing (exchanges) alongside streaming               │
│  + Same cluster for task queues and event streams                │
│  - Lower throughput ceiling (~200K msg/sec/partition)            │
│  - Smaller ecosystem for stream processing                       │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

### When to Use Super Streams vs Quorum Queues

| Dimension | Quorum Queue | Super Stream |
|-----------|-------------|--------------|
| **Consumption model** | Competing consumers (round-robin) | Partitioned (key-based) |
| **Ordering** | Per-queue FIFO (First-In-First-Out, like a line at a store) | Per-partition FIFO (First-In-First-Out, like a line at a store) |
| **Message lifecycle** | Delete after ACK | Retain with offset tracking |
| **Replay** | No | Yes (seek to offset) |
| **Consumer groups** | Not native | Automatic rebalancing |
| **Use case** | Task queues, RPC | Event streaming, ordered processing |
