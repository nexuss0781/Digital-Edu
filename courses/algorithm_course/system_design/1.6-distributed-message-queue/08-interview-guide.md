...
│                                                                  │
│  Per-Queue FIFO (First-In-First-Out, like a line at a store):                                                 │
│  • Single consumer: Strict FIFO (First-In-First-Out, like a line at a store) guaranteed                      │
...
### "Why not just use Kafka instead?"

```
┌─────────────────────────────────────────────────────────────────┐
│  TRAP: "Is there still a reason to use a message queue?"        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Bad Answer:                                                     │
│  "You wouldn't, Kafka does everything" (Wrong — different tools)│
│                                                                  │
│  Good Answer:                                                    │
│  "They solve different problems and the choice depends on the   │
│   workload:                                                      │
│                                                                  │
│  Choose Message Queue When:                                      │
│  • Complex routing (topic patterns, headers, fan-out)            │
│  • True task distribution (message deleted after ACK)            │
│  • Message priorities needed                                     │
│  • Sub-millisecond latency for individual messages              │
│  • Request-reply / RPC patterns                                  │
│  • Dead letter handling is critical                              │
│                                                                  │
│  Choose Log-Based Broker When:                                   │
│  • Event replay / audit trails needed                           │
│  • Very high throughput (>500K msg/sec)                          │
│  • Multiple independent consumer groups                         │
│  • Stream processing pipelines                                   │
│                                                                  │
│  2025-2026 Update:                                               │
│  • Super streams (RabbitMQ 3.13+) now bridge the gap —          │
│    partitioned, offset-tracked consumption within RabbitMQ       │
│  • For moderate streaming needs (<200K msg/sec), super           │
│    streams eliminate the need for a separate Kafka cluster       │
│  • But for high-throughput streaming, Kafka still wins           │
│    on raw throughput and ecosystem (Flink, ksqlDB)"              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| Single queue for high throughput | Queue is single-threaded Slowest part of the process | Shard across multiple queues |
...
6. 2025-2026 AWARENESS
   □ Super streams for partitioned consumption
...
```
AMQP, exchange, binding, routing key, quorum queue, Raft consensus,
publisher confirm, consumer ACK, dead letter exchange, prefetch count,
pause-minority, flow control, backpressure, lazy queue, channel,
virtual host, fan-out exchange, topic trie, competing consumers,
super streams, K8s cluster operator, message compression,
idempotent consumer, at-least-once, exactly-once, deduplication
```
