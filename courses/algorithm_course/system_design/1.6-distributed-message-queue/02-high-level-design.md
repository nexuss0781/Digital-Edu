...
## Message Compression Strategy (2025-2026)

### Application-Level Compression

```
┌─────────────────────────────────────────────────────────────────┐
│                MESSAGE COMPRESSION DECISION                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  Producer Decision Tree:                                         │
│                                                                   │
│  Message size > 1 KB?                                            │
│  ├── No  → Publish uncompressed (compression overhead > benefit) │
│  └── Yes → Content type?                                         │
│       ├── JSON/XML/Text → Compress with LZ4 (speed) or zstd     │
│       │                   (ratio). Set content_encoding header.  │
│       │                   Typical ratio: 5-10x                   │
│       └── Binary/Image  → Skip (already compressed)             │
│                                                                   │
│  Why Application-Level Not Transport-Level:                      │
│  • Transport compression (TLS) is vulnerable to CRIME attacks    │
│  • Application compression is visible, configurable per message  │
│  • Broker stores compressed bytes — saves disk and replication   │
│  • Fan-out: one compressed copy → N reference copies             │
│                                                                   │
│  Headers to Set:                                                  │
│  • content_type: "application/json"                              │
│  • content_encoding: "lz4" (or "zstd", "gzip")                  │
│                                                                   │
│  Impact (10KB JSON messages, 100K msg/sec):                      │
│  • Without compression: 1 GB/sec network, 86 TB/day disk        │
│  • With LZ4 (8x ratio): 125 MB/sec network, 10.8 TB/day disk   │
│  • CPU cost: ~1ms per message (LZ4), ~3ms (zstd)                │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
