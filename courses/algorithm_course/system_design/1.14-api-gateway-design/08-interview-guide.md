...
### Trap 6: "How would you rate-limit an LLM API where a single request can cost $0.50?"

**Why it's a trap:** Tests whether you recognize that uniform request-counting rate limits break down for AI/LLM workloads.

**Good answer:**
"Traditional request-per-second rate limiting assumes roughly uniform request cost, which is invalid for LLM APIs. A 10-token prompt and a 100K-token prompt differ by 1000x in cost.

**Token-budget rate limiting:**
1. **Estimate input tokens** from request body before forwarding (character-count Practical rule of thumb)
2. **Reserve tokens** from a per-consumer token-per-minute budget
3. **Track output tokens** during streaming SSE response
4. **Reconcile** actual usage at stream completion

**Additional considerations:**
- Cost-weighted routing: route to cheaper models when budget is low
- Streaming connection pools: separate from REST pools (30s vs 5-min timeout)
- Token ledger: maintained per consumer, synced to Redis like traditional counters
- Fallback: if token estimation fails, apply a conservative default budget

The key insight is that the rate limiter becomes a cost accountant — it must understand model-specific pricing and track consumption across the full request lifecycle, not just at admission."
