...
| [2.11 Service Mesh](../2.11-service-mesh/00-index.md) | Sidecar proxy pattern (Envoy, mTLS) |
| [1.11 Configuration Management](../1.11-configuration-management-system/00-index.md) | Dynamic config propagation patterns |
| [3.23 LLM Inference Engine](../3.23-llm-inference-engine/00-index.md) | AI gateway: token-budget rate limiting, streaming SSE |

---

## Case Study: AI Gateway Layer at Scale (2025-2026)

**Challenge:** As organizations adopted LLM-powered APIs internally and externally (2024-2026), existing API gateways struggled with three fundamental mismatches: (1) request cost varied 1000x between a simple /chat and a 100K-token completion, breaking uniform rate limiting; (2) streaming SSE responses lasted 10-60 seconds, exhausting connection pools designed for sub-second request/response; (3) prompt content required inspection for PII and injection attacks, adding a new security layer the plugin chain was never designed for.

**Architecture Decisions:**
- **Token-Budget Rate Limiting:** Replaced request-per-second quotas with token-per-minute budgets. The gateway estimates input tokens from the request body, reserves capacity from a per-consumer token ledger, and reconciles actual usage from the streaming response. This required a new accounting plugin that maintains state across the full request lifecycle — a departure from the stateless plugin model.
- **Streaming-Aware Connection Management:** Introduced tiered connection pools: short-lived pools for REST APIs (30s timeout) and long-lived pools for LLM streaming endpoints (5-minute timeout). The circuit breaker was modified to track stream abandonment rate (client disconnects mid-stream) as a health signal alongside traditional error rates.
- **Prompt Security Scanning:** Added a pre-routing plugin that scans request bodies for prompt injection patterns, PII leakage, and policy violations using a lightweight classifier. Requests exceeding a risk threshold are routed to a sandboxed model endpoint with restricted capabilities rather than being rejected outright — preserving availability while containing risk.

**Results:** Token-budget rate limiting reduced LLM cost overruns by 60% compared to request-count limiting. Streaming connection pools eliminated the timeout cascades that occurred when REST and LLM traffic shared pools. Prompt scanning added 3ms p50 latency (within the plugin budget) while blocking 12% of requests that contained policy violations.

**Key Lesson:** AI/LLM APIs violate the gateway's core assumption of request uniformity. Rather than building a separate "AI gateway," the most effective approach extends the existing plugin architecture with cost-aware primitives (token budgets, stream lifecycle hooks, content classifiers) — preserving the gateway's role as the single enforcement point while adapting to fundamentally different traffic patterns.
