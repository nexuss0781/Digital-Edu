...
|---------|-------------|--------|
| **AI/LLM Gateway** | Token-budget rate limiting, prompt security scanning, streaming SSE lifecycle management | Breaks uniform-cost assumption; requires cost-aware plugins |
| **WebAssembly Plugins** | WASM component model replaces Lua for custom plugins; sandboxed, multi-language, near-native speed | Resolves extensibility-vs-security trilemma |
| **eBPF-Based Filtering** | Kernel-space packet rejection for deny-listed IPs and rate-limited consumers before HTTP parsing | 10x throughput for rejectable traffic |
| **HTTP/3 (QUIC)** | UDP transport, 0-RTT resumption, stream-level flow control, connection migration | Dual-stack TCP+UDP gateway required during transition |
| **Zero-Trust Gateway** | Per-request identity verification (SPIFFE/SPIRE), continuous authorization, no implicit trust from network position | Replaces perimeter-based security model |

**AI Gateway Data Flow:**

```
Request → [Token Estimator] → [Prompt Security Scanner] → [Token Budget Check]
                                                               │
                                                    ┌──────────┴──────────┐
                                                    ▼                     ▼
                                              [Allow]               [Reject 429]
