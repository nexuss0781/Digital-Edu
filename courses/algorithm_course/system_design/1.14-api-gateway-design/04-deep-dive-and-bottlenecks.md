...
| Strategy | Time Complexity (Speed of the algorithm) | Memory | Best For |
|----------|-----------------|--------|----------|
...
### 2. Connection Pooling Slowest part of the process
...
### Edge Case (Unusual or extreme situation) 1: Upstream Returns Slowly
...
### Edge Case (Unusual or extreme situation) 2: Large Request/Response Bodies
...
### Edge Case (Unusual or extreme situation) 3: Client Disconnects Mid-Request
...
### Edge Case (Unusual or extreme situation) 4: WebSocket Upgrade with Auth Expiry
...
### Edge Case (Unusual or extreme situation) 5: Circular Redirect Loop
...
### Edge Case (Unusual or extreme situation) 6: LLM Streaming Response with Token Accounting

**Scenario:** An LLM API returns a streaming SSE response over 30 seconds; the gateway must count output tokens for billing but only knows the final count at stream completion.

**Handling:**
```
function handleStreamingLLMResponse(upstream, consumer, context):
    estimatedInputTokens = estimateTokens(context.requestBody)
    tokenLedger.reserve(consumer, estimatedInputTokens)

    outputTokenCount = 0
    stream = upstream.getResponseStream()

    for chunk in stream:
        // Count tokens in each SSE data event
        if chunk.startsWith("data: "):
            parsed = parseJSON(chunk.substring(6))
            if parsed.choices[0].delta.content:
                outputTokenCount += countTokens(parsed.choices[0].delta.content)
    ...
    tokenLedger.reconcile(consumer, estimatedInputTokens, outputTokenCount)

    // Emit billing event
    emitUsageEvent(consumer, estimatedInputTokens, outputTokenCount, context.model)
```

### Edge Case (Unusual or extreme situation) 7: Plugin Throws Exception
