...
### GenAI Observability (2025-2026 Extension)

| Category | Metric | Description | Alert Threshold |
|----------|--------|-------------|-----------------|
| **LLM Latency** | `llm_inference_latency_seconds` | Time-to-first-token and total generation time | TTFT P99 > 500ms |
| **Token Throughput** | `llm_tokens_per_second` | Output token generation rate | < 30 tokens/s |
| **Embedding Quality** | `embedding_similarity_score` | Cosine similarity between expected and actual embeddings | Mean < 0.8 |
| **Prompt Cost** | `llm_prompt_tokens_total` | Total prompt tokens consumed per model | > 2x baseline |
| **Hallucination Rate** | `llm_hallucination_detected_total` | Output failing fact-check validation | > 5% |
| **RAG Retrieval Quality** | `rag_context_relevance_score` | Relevance of retrieved context to query | Mean < 0.7 |
| **Vector Search Latency** | `vector_search_latency_seconds` | ANN index query time | P99 > 10ms |
| **GPU Utilization** | `gpu_utilization_percent` | GPU compute utilization for inference | < 30% (waste) or > 90% (saturated) |
