...
OPTIMIZATION - Partition Cutting off unnecessary steps:
...
### Slowest part of the process 1: Point-in-Time Join Performance
...
1. Partition Cutting off unnecessary steps
...
|----------|--------|-------|-------------|
| Partition Cutting off unnecessary steps | 4 hours | 30 min | 8x |
| + Pre-Aggregation | 30 min | 10 min | 3x |
| + Incremental | 10 min | 2 min | 5x |
| Combined | 4 hours | 2 min | 120x |
```

### Slowest part of the process 2: Online Store Latency
...
### Slowest part of the process 3: Streaming Feature Lag
...
| Hive partition columns | date | date, entity_type | Faster Cutting off unnecessary steps |
...
## Case Study 1: Chronon Adoption at Stripe

### Context
Stripe adopted Chronon to power their ML feature infrastructure for fraud detection, risk scoring, and payment optimization. Their scale: billions of transactions per year with sub-100ms latency requirements for real-time fraud scoring.

### Challenge
Before Chronon, Stripe maintained separate feature computation logic for training (batch Spark jobs) and serving (custom Java services). This led to:
- Feature drift between environments causing false positive spikes in fraud detection
- 6+ weeks to ship a new feature from development to production
- Engineering overhead of maintaining dual codebases for each feature

### Solution
Stripe adopted Chronon's declarative DSL as the single source of truth:
- **Single definition** for each feature, compiled to both batch and streaming
- **Point-in-time correctness** eliminated data leakage in fraud model training
- **Feature development velocity** improved from weeks to days

### Results

| Metric | Before Chronon | After Chronon | Improvement |
|--------|---------------|---------------|-------------|
| Feature shipping time | 6+ weeks | 3-5 days | ~10x faster |
| Train-serve consistency | Manual verification | Guaranteed by construction | Eliminated entire bug class |
| Feature reuse rate | ~15% | ~60% | 4x more reuse |
| ML engineer time on infrastructure | 40% | 15% | Freed for model work |

### Architectural Insight
Stripe's adoption validates that the declarative DSL approach scales beyond Airbnb's specific use case. The key lesson: organizations with strict latency and correctness requirements (financial services, fraud detection) benefit most from construction-guaranteed consistency rather than testing-verified consistency.

---

## Case Study 2: Airbnb AI-Native Search (2025-2026)

### Context
In 2025-2026, Airbnb began transforming its search and discovery experience into an AI-native platform where the app "knows you" — supporting natural language queries like "a snug lakeside cabin for four, needs a hot tub and paths good for walking."

### Challenge
This transformation required BigHead to evolve beyond traditional ML features:
- **Embedding features**: Vector representations of listing descriptions, photos, and user preferences
- **LLM-derived features**: Semantic understanding of user intent from natural language queries
- **Real-time personalization**: Sub-30ms feature assembly combining traditional features with embeddings
- **Scale**: Serving features for all 7M+ listings with natural language understanding

### Solution
BigHead's architecture extended to support the AI-native search:

```
EXTENSION POINTS:
1. Feature DSL → Added embedding type with configurable encoding model
2. Online Store → Extended KV store with vector index (ANN) for similarity
3. Deep Thought → Added GPU inference pods for LLM query understanding
4. Feature API → Batch lookup now supports mixed scalar + vector features

EXAMPLE FEATURE DEFINITION:
    listing_embedding = FeatureGroup(
        name="listing_semantic_embedding",
        keys=["listing_id"],
        sources=[EntitySource(table="listings", columns=["description", "amenities"])],
        features=[
            Feature(
                name="description_embedding",
                expr="encode(description)",
                aggregation=Aggregation.LAST,
                dtype=DataType.VECTOR(768),
                encoder="sentence-transformer-v2"
            )
        ],
        online_enabled=True
    )
...
| Metric | Traditional Search | AI-Native Search | Impact |
|--------|-------------------|-----------------|--------|
| Query understanding accuracy | Keyword matching | 85% intent recognition | Natural language support |
| Feature assembly latency | 8ms (scalar only) | 12ms (scalar + vector) | Acceptable overhead |
| Feature store size | 21 TB (scalar) | 28 TB (scalar + vector) | 33% increase |
| Search satisfaction | Baseline | +15% engagement | Measurable business impact |

### Architectural Insight
The AI-native search case demonstrates that a well-designed feature platform (declarative DSL, train-serve consistency, multi-path architecture) provides a natural extension point for GenAI workloads. Rather than building a separate LLM infrastructure, teams extended the existing platform with new feature types and index structures.
