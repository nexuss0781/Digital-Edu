...
### AI/ML-Specific Security Threats (2025-2026)

| Threat | Attack Vector | Detection | Mitigation |
|--------|--------------|-----------|------------|
| **Adversarial Input** | Crafted inputs designed to fool ML models | Input validation, anomaly detection on feature distributions | Input sanitization, ensemble models, adversarial training |
| **Prompt Injection** | Malicious prompts in LLM-powered features | Prompt guard classifiers, output validation | Input filtering, output sandboxing, prompt hardening |
| **Training Data Poisoning** | Contaminated data injected into training pipelines | Statistical anomaly detection on training data distributions | Data provenance tracking, validation gates in pipelines |
| **Model Inversion** | Reconstructing training data from model outputs | Query pattern analysis, rate limiting | Differential privacy in training, output rounding |
| **Membership Inference** | Determining if specific data was in training set | Rate limiting, query monitoring | Differential privacy, regularization |
