...
| Strategy | Implementation | Benefit |
|----------|----------------|---------|
| **Partitioning** | By date (ds) and entity_type | Query Cutting off unnecessary steps |
...
### GPU and LLM Workload Scaling (2025-2026 Extension)

| Resource | Scaling Strategy | Implementation | Threshold |
|----------|-----------------|----------------|-----------|
| **GPU Pods** | Time-sliced GPU sharing | Multiple small models share a single GPU via MPS | GPU utilization > 60% → add GPU node |
| **LLM Inference** | KV-cache aware batching | Continuous batching with PagedAttention-style memory management | Queue depth > 50 → add replica |
| **Embedding Generation** | Batch GPU pipeline | Async embedding computation with write-behind to vector index | Backlog > 10K items → scale workers |
| **Vector Index** | Sharded ANN index | HNSW index partitioned by listing region, replicated per zone | P99 search > 5ms → add shard |
