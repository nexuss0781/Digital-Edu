...
| FR-13 | Multi-Cluster Discovery | P2 | ServiceExport/ServiceImport across cluster boundaries with locality-aware routing |
| FR-14 | Zero-Trust Identity | P1 | Cryptographic workload identity (SPIFFE SVID) for registration and discovery authorization |
| FR-15 | Kernel-Level Resolution | P2 | eBPF-based endpoint resolution at the socket layer for sub-microsecond discovery |
| FR-16 | xDS Protocol Support | P2 | Control plane compatibility with Envoy xDS for push-based endpoint delivery to proxies and proxyless gRPC clients |
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  SERVICE DISCOVERY MODEL EVOLUTION (2015-2026)                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Gen 1: Client Library (2015-2018)                                  │
│  ├── Eureka + Ribbon, Consul + client SDK                           │
│  ├── Per-language libraries with discovery logic                    │
│  └── Problem: Library inconsistency across languages                │
│                                                                      │
│  Gen 2: Sidecar Proxy (2018-2023)                                   │
│  ├── Envoy sidecar + Istio control plane                            │
│  ├── Language-agnostic, app-transparent                              │
│  └── Problem: Per-pod overhead (50-100MB × N pods)                  │
│                                                                      │
│  Gen 3: Ambient / eBPF (2023-2026)                                  │
│  ├── Ambient mesh: shared per-node ztunnel for L4                   │
│  ├── eBPF: kernel-level discovery with sub-μs overhead              │
│  ├── Proxyless: xDS-native gRPC libraries                           │
│  └── Principle: match overhead to the feature tier needed            │
│                                                                      │
│  Gen 4: AI-Augmented Discovery (Emerging)                           │
│  ├── Predictive health: ML models predict failures before they hit  │
│  ├── Traffic-aware routing: route based on predicted latency        │
│  └── Anomaly detection: auto-quarantine misbehaving instances       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
