...
When service discovery operates at the kernel level via eBPF, traditional application-level metrics are insufficient. eBPF-specific observability requires monitoring the BPF maps and programs themselves.

| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `ebpf.service_map.entries` | Gauge | Total entries in service endpoint BPF map | Near map size limit |
| `ebpf.service_map.update_latency_ns` | Histogram | Time to update endpoint in BPF map | p99 > 10ms |
| `ebpf.connect_redirect.total` | Counter | Total connect() syscalls redirected | - |
| `ebpf.connect_redirect.errors` | Counter | Failed endpoint lookups in BPF | > 0.1% error rate |
| `ebpf.endpoint_sync.lag_ms` | Gauge | Delay from control plane to BPF map update | > 5000ms |
| `ebpf.program.tail_calls` | Counter | BPF program tail calls (complexity indicator) | Approaching limit (33) |

### Multi-Cluster Discovery Metrics

| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `multicluster.endpoint_sync.lag_ms` | Gauge | Cross-cluster endpoint sync delay | > 10s |
| `multicluster.clusters.connected` | Gauge | Number of connected peer clusters | < expected count |
| `multicluster.remote_endpoints.total` | Gauge | Remote cluster endpoints known locally | Drop > 20% |
| `multicluster.cross_cluster_calls.total` | Counter | Calls routed to remote cluster | Unexpected spike |
| `multicluster.locality.local_ratio` | Gauge | Fraction of calls served locally | < 90% |

### Discovery Topology Map

```
┌─────────────────────────────────────────────────────────────────────┐
│  DISCOVERY TOPOLOGY DASHBOARD                                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Service Dependency Graph (auto-discovered from discovery queries)  │
│                                                                      │
│  ┌──────────┐     ┌──────────┐     ┌──────────┐                    │
│  │ api-gw   │────►│ order    │────►│ payment  │                    │
│  │ 3 inst   │     │ 12 inst  │     │ 8 inst   │                    │
│  │ 100% ●   │     │ 92% ●    │     │ 100% ●   │                    │
│  └──────────┘     └────┬─────┘     └──────────┘                    │
│                        │                                            │
│                        ▼                                            │
...
│                   └──────────┘     └──────────┘                    │
│                                                                      │
│  ● = health percentage   Arrows = discovery dependencies           │
│                                                                      │
│  Anomalies Detected:                                                │
│  ⚠ inventory: 1 instance unhealthy for > 10 minutes                │
│  ⚠ order: discovery latency p99 > 30ms (normally 5ms)              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
