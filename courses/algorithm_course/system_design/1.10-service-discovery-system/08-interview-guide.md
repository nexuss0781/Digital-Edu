...
### Trap 6: "Do you still need sidecars for service discovery?"

**What they want:** Awareness of modern alternatives.

**Good answer:**
> "Not necessarily. The service mesh landscape has evolved significantly:
> 1. **Ambient mesh** replaces per-pod sidecars with a shared per-node ztunnel for L4 discovery and mTLS, reducing memory overhead by 10-50x. L7 features use optional waypoint proxies.
> 2. **eBPF-based discovery** moves endpoint resolution into the Linux kernel, achieving sub-microsecond overhead with zero proxy processes.
> 3. **Proxyless gRPC** uses xDS protocol directly in the gRPC library — the application IS the data plane.
>
> The trend is clear: discovery is moving from per-pod userspace proxies toward shared infrastructure (node-level) or kernel-level (eBPF). Sidecars remain valid for L7-heavy workloads, but for the 90%+ of traffic that only needs L4 routing and mTLS, lighter approaches are winning."

### Trap 7: "How would you handle service discovery for 100,000+ instances across 10 regions?"

**What they want:** Extreme scale architecture.

**Good answer:**
> "At this scale, several architectural changes are mandatory:
> 1. **Regional clusters** — each region has its own registry cluster (5-7 nodes), no single global registry
> 2. **Multi-cluster API** — use MCS (Multi-Cluster Services) or ClusterMesh for cross-cluster discovery with locality-aware routing
> 3. **eBPF endpoint maps** — BPF maps on each node hold the global endpoint table, updated by the control plane in < 1 second, with sub-microsecond lookup
> 4. **Hierarchical caching** — node-level BPF maps → pod-level client cache → registry cluster as source of truth
> 5. **Differential sync** — only propagate endpoint changes (delta), not full snapshots; at 100K instances, a full sync would be > 250MB
>
> The key insight: at this scale, you can't have every client talk to the registry — you need the discovery data pushed to the network layer (eBPF maps, ztunnel config) so applications never make explicit discovery calls."
...
| Assuming sidecars are the only mesh option | Ignores eBPF and ambient mesh | Discuss eBPF kernel-level and ambient approaches |
| Ignoring zero-trust identity | Network-based trust is insufficient | Mention SPIFFE/SPIRE workload identity |
| Single-cluster thinking | Modern systems span multiple clusters | Discuss MCS API and ClusterMesh patterns |
