...
### Identity-First Discovery Model

In a zero-trust architecture, service discovery is not just about *where* a service is — it's about *proving who is asking* and *whether they should know*.

```
┌─────────────────────────────────────────────────────────────────────┐
│  ZERO-TRUST DISCOVERY ARCHITECTURE                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Traditional (Network-Based Trust):                                  │
│  "If you're on the network, you can discover anything"              │
│                                                                      │
│  Zero-Trust (Identity-Based Trust):                                  │
│  "Prove your identity → check your authorization → reveal only      │
│   the endpoints you're permitted to access"                          │
│                                                                      │
...
│  │ Example: spiffe://acme.com/ns/prod/sa/order-service          │   │
│  │ Lifetime: 1 hour (auto-rotated by SPIRE agent)              │   │
│  │ Attestation: Node → Pod → Container image hash               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Layer 2: Authorization Policy                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Policy: "order-service CAN discover payment-service"         │   │
│  │ Policy: "order-service CANNOT discover admin-service"        │   │
│  │ Enforcement: Registry filters results based on caller SVID  │   │
│  │ Effect: Services are invisible to unauthorized callers       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Layer 3: Encrypted Transport (mTLS everywhere)                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ All communication encrypted with SVID-based certificates    │   │
│  │ Registry ↔ Service: mTLS                                     │   │
│  │ Service ↔ Service: mTLS                                      │   │
│  │ Registry ↔ Registry: mTLS                                    │   │
│  │ No plaintext communication anywhere                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│  Layer 4: Runtime Attestation                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Hardware-backed identity (TPM 2.0 / secure enclave)          │   │
│  │ Container image signature verification                       │   │
│  │ Runtime integrity monitoring (binary allow-lists)            │   │
│  │ Identity bound to specific node + pod + container image      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Filtered Discovery Responses

```
ALGORITHM ZeroTrustDiscover(caller_identity, service_name):
    // Discovery responses filtered by caller's authorization

    // Step 1: Verify caller identity
    IF NOT VerifySVID(caller_identity):
        RETURN Error(401, "Invalid identity")

    // Step 2: Check discovery authorization
    IF NOT AuthzPolicy.CanDiscover(caller_identity, service_name):
        // Service appears to not exist — information hiding
        RETURN DiscoverResponse{instances: []}

    // Step 3: Return only authorized endpoints
    instances = GetHealthyInstances(service_name)

    // Step 4: Filter by network policy (optional)
    instances = instances.Filter(i =>
        NetworkPolicy.AllowsTraffic(caller_identity, i))

    RETURN DiscoverResponse{instances: instances}
```

### SPIFFE Federation for Cross-Organization Discovery

```mermaid
flowchart TB
    subgraph OrgA["Organization A (Trust Domain: acme.com)"]
        SpireA["SPIRE Server A"]
        SvcA["payment-service<br/>spiffe://acme.com/payment"]
    end

    subgraph OrgB["Organization B (Trust Domain: partner.io)"]
        SpireB["SPIRE Server B"]
        SvcB["order-service<br/>spiffe://partner.io/order"]
    end

    subgraph Federation["SPIFFE Federation"]
        Bundle["Trust Bundle Exchange"]
    end

    SpireA <-->|"Exchange trust bundles"| Bundle
    SpireB <-->|"Exchange trust bundles"| Bundle
    SvcB -->|"mTLS with cross-domain SVID"| SvcA

    classDef orgA fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef orgB fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef fed fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class SpireA,SvcA orgA
    class SpireB,SvcB orgB
    class Bundle fed
```
