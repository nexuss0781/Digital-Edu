...
- Extra network hop
- Load balancer can become Slowest part of the process
...
### Pattern 5: Ambient Mesh (Sidecarless) Discovery

Node-level shared proxy handles discovery without per-pod sidecars.
...
sequenceDiagram
    ...
    participant CP as Control Plane
    participant WP as Waypoint Proxy (Optional)
    participant Server as Payment Service

    Note over Client,Server: Control plane pushes endpoints to ztunnel
    CP->>ZT: Push service endpoints + mTLS certs

    Note over Client,Server: L4 traffic - no waypoint needed
    Client->>ZT: POST http://payment-service/payments
    Note over ZT: Resolves VIP → backend pod IP
    ZT->>Server: mTLS-encrypted L4 forwarding
    Server-->>ZT: Response
    ZT-->>Client: Response

    Note over Client,Server: L7 traffic - waypoint for header routing
    Client->>ZT: POST with header x-canary: true
    ZT->>WP: Forward to waypoint for L7 inspection
    WP->>Server: Route to canary instance
    Server-->>WP: Response
    WP-->>ZT: Response
    ZT-->>Client: Response
```
...
**Implementation Examples:**
- Ambient mesh with ztunnel DaemonSet
- Cilium with eBPF-based node proxy
- Service mesh lite modes

**Pros:**
- 10-50x less memory than sidecar model
- No sidecar injection complexity
- L4 mTLS and discovery for all pods by default

**Cons:**
- L7 features require additional waypoint deployment
- Newer architecture with less production maturity
- Node-level blast radius (ztunnel crash affects all pods on node)

---

### Pattern 6: eBPF Kernel-Level Discovery

In-kernel service resolution via eBPF programs — zero proxy hops.
...
sequenceDiagram
    ...
    Note over App,Server: Control plane updates eBPF maps
    CP->>Kernel: Update endpoint map: payment-svc → [10.0.1.1, 10.0.1.2]

    Note over App,Server: Application connects normally
    App->>Kernel: connect(payment-svc:8080)
    Note over Kernel: eBPF intercepts connect()<br/>Looks up endpoint map<br/>Rewrites dest: 10.0.1.1:8080
    Kernel->>Server: Direct TCP connection (no proxy)
    Server-->>App: Response (direct path)
```

**Implementation Examples:**
- Cilium service mesh (eBPF kube-proxy replacement)
- eBPF-based ClusterIP resolution
- Socket-level load balancing

**Pros:**
- Sub-microsecond discovery overhead
- Zero additional memory per pod or node
- No proxy latency — direct kernel-to-kernel path

**Cons:**
- Requires Linux kernel 5.10+
- Limited L7 inspection capability in eBPF alone
- Debugging requires eBPF-specific tooling (bpftool, cilium monitor)

---

## Comparison: Discovery Patterns

| Aspect | Client-Side | Server-Side | DNS-Based | Sidecar Mesh | Ambient Mesh | eBPF Kernel |
|--------|-------------|-------------|-----------|--------------|--------------|-------------|
| **Client Complexity** | High | Low | Low | Low | Low | Low |
| **Network Hops** | 0 extra | 1 extra | 0 extra | 2 extra | 0-1 extra | 0 extra |
| **Single Point of Failure** | None | Load Balancer | DNS Server | Control Plane | Control Plane | Control Plane |
| **Language Support** | Library per lang | Any | Any | Any | Any | Any |
| **Metadata Support** | Full | Limited | None | Full | Full (L7 via waypoint) | Limited (L4) |
| **Health Integration** | Yes | Yes | Limited | Yes | Yes | Yes |
| **Resource Overhead** | Client memory | LB resources | Minimal | Sidecar per pod | ztunnel per node | eBPF maps (KB) |
| **L7 Features** | Library-dependent | LB-dependent | None | Full | Waypoint required | Limited |
| **Kernel Requirement** | None | None | None | None | None | Linux 5.10+ |
