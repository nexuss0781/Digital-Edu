...
│  │ Endpoints:           │         │ Endpoints:           │           │
│  │  - pod-a1: 10.0.1.1 │         │  - pod-b1: 10.1.1.1 │           │
│  │  - pod-a2: 10.0.1.2 │         │  - pod-b2: 10.1.1.2 │           │
│  └──────────┬──────────┘         └──────────┬──────────┘           │
│             │                                │                      │
│             └────────────┬───────────────────┘                      │
│                          │                                          │
...
│                          │                                          │
│  ┌───────────────────────▼───────────────────────┐                 │
│  │  ServiceImport (in all clusters)               │                 │
│  │  payment-service.clusterset.local              │                 │
│  │  Endpoints: [pod-a1, pod-a2, pod-b1, pod-b2]  │                 │
│  │  Topology: locality-aware routing enabled      │                 │
│  └────────────────────────────────────────────────┘                 │
│                                                                      │
│  Client Query:                                                      │
│  DNS: payment-service.namespace.svc.clusterset.local               │
│  → Returns endpoints from ALL clusters                              │
│  → Topology-aware hints prefer local cluster first                 │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### eBPF-Powered Multi-Cluster (ClusterMesh Pattern)
...
│                                                                      │
│  Architecture:                                                       │
│  - Each cluster runs a mesh control plane agent                     │
│  - Agents share endpoint information via etcd peering               │
│  - eBPF maps on each node contain global endpoint table             │
│  - Service discovery spans up to 255 connected clusters             │
│                                                                      │
...
│                                                                      │
│  Key Innovation:                                                     │
│  - Global services: same ClusterIP across clusters                  │
│  - Shared services: endpoints merged from multiple clusters         │
│  - Affinity: prefer local cluster, failover to remote               │
│  - No gateway required — direct pod-to-pod (with tunnel/native)    │
│                                                                      │
│  Scaling:                                                            │
│  - Up to 255 clusters connected                                     │
│  - 100K+ services across cluster fleet                              │
│  - Endpoint sync latency: < 5 seconds cross-cluster                │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Proxyless gRPC xDS Discovery

```
┌─────────────────────────────────────────────────────────────────────┐
│  PROXYLESS gRPC SERVICE DISCOVERY                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Traditional: App → Sidecar → Backend                               │
│  Proxyless:   App (with xDS) → Backend (direct)                    │
│                                                                      │
│  Architecture:                                                       │
│  ┌─────────────────┐         ┌─────────────────┐                   │
...
│  └────────┬────────┘         └────────┬────────┘                   │
│           │ xDS stream                │ xDS stream                  │
│           ▼                           ▼                             │
│  ┌─────────────────┐         ┌─────────────────┐                   │
│  │ gRPC Client     │         │ gRPC Server     │                   │
│  │ (built-in xDS   │────────►│ (registers      │                   │
│  │  resolver + LB) │ Direct  │  with CP)       │                   │
│  └─────────────────┘ gRPC    └─────────────────┘                   │
│                                                                      │
│  How It Works:                                                       │
│  1. gRPC library bootstraps xDS connection to control plane        │
│  2. Control plane pushes endpoint discovery (EDS) updates          │
│  3. gRPC library performs client-side load balancing               │
│  4. No sidecar proxy — gRPC library IS the data plane              │
│                                                                      │
│  Benefits:                                                           │
│  - Zero proxy overhead for gRPC-heavy workloads                     │
│  - Native integration with gRPC retry, timeout, circuit breaking   │
│  - Control plane compatibility with existing mesh infrastructure   │
│                                                                      │
│  Limitations:                                                        │
│  - gRPC only — HTTP/1.1 and other protocols need a proxy           │
│  - Language support: Go, Java, C++, Python (not all languages)     │
│  - No mTLS without additional library support                      │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
