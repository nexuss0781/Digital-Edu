...
### Multi-Cluster Endpoint Merging

```
ALGORITHM MergeMultiClusterEndpoints(service_name, local_cluster, peer_clusters):
    // Merge endpoints from multiple clusters with locality preference

    INPUT:
        service_name: String
        local_cluster: ClusterID
        peer_clusters: List<ClusterID>

    PROCESS:
        // Step 1: Get local endpoints (always preferred)
        local_endpoints = localRegistry.GetHealthyInstances(service_name)

        // Step 2: Check if local capacity is sufficient
        IF local_endpoints.count >= MIN_LOCAL_INSTANCES:
            // Sufficient local capacity — return local only with metadata
        ...
        // Step 3: Fetch remote endpoints (parallel queries)
        remote_endpoints = []
        PARALLEL FOR EACH peer IN peer_clusters:
            TRY:
                remote = peerRegistry.GetHealthyInstances(peer, service_name)
                remote_endpoints.AddAll(AnnotateWithCluster(remote, peer))
            CATCH Timeout, Error:
                Log.Warn("Failed to fetch from peer cluster", {peer: peer})

        // Step 4: Merge with locality weights
        merged = []
        FOR EACH ep IN local_endpoints:
            ep.priority = 0  // Highest priority
            ep.weight = 100
            merged.Add(ep)

        FOR EACH ep IN remote_endpoints:
            ep.priority = 1  // Lower priority (failover)
            ep.weight = CalculateCrossClusterWeight(ep.cluster, local_cluster)
            merged.Add(ep)

        // Step 5: Sort by priority, then weight
        merged.SortBy(ep => (ep.priority, -ep.weight))

    RETURN MultiClusterResponse{
        endpoints: merged,
        locality: MIXED,
        local_count: local_endpoints.count,
        remote_count: remote_endpoints.count
    }

    ALGORITHM CalculateCrossClusterWeight(remote_cluster, local_cluster):
        // Weight based on inter-cluster latency
        latency = MeasuredLatency(local_cluster, remote_cluster)
        IF latency < 10ms:    RETURN 80   // Same region
        IF latency < 50ms:    RETURN 50   // Adjacent region
        IF latency < 150ms:   RETURN 20   // Cross-continent
        RETURN 10                          // Distant
```

### eBPF Service Map Data Structure

```
┌─────────────────────────────────────────────────────────────────────┐
│  eBPF MAP STRUCTURES FOR KERNEL-LEVEL DISCOVERY                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Map 1: Service Map (LPM Trie or Hash)                              │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ Key: {service_ip: uint32, service_port: uint16}              │  │
│  │ Value: {                                                      │  │
│  │   backend_count: uint16,                                      │  │
│  │   lb_algo: uint8,          // 0=RR, 1=Random, 2=Maglev       │  │
│  │   session_affinity: uint8, // 0=None, 1=ClientIP             │  │
│  │   backend_slot_start: uint32  // Index into backend map       │  │
│  │ }                                                              │  │
│  │                                                                │  │
│  │ Example: 10.96.0.100:80 → {count:3, algo:RR, start:0}       │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Map 2: Backend Map (Array)                                         │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ Key: uint32 (slot index)                                      │  │
│  │ Value: {                                                      │  │
│  │   pod_ip: uint32,                                             │  │
│  │   pod_port: uint16,                                           │  │
│  │   node_id: uint16,        // For locality preference          │  │
│  │   weight: uint16,         // For weighted load balancing      │  │
│  │   flags: uint8            // 0x01=active, 0x02=draining       │  │
│  │ }                                                              │  │
│  │                                                                │  │
│  │ [0] → {ip:10.0.1.1, port:8080, node:1, weight:100, active}  │  │
│  │ [1] → {ip:10.0.1.2, port:8080, node:2, weight:100, active}  │  │
│  │ [2] → {ip:10.0.2.1, port:8080, node:3, weight:50, drain}    │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Map 3: Affinity Map (LRU Hash)                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ Key: {client_ip: uint32, service_ip: uint32}                  │  │
│  │ Value: {backend_slot: uint32, timestamp: uint64}              │  │
│  │ TTL: Configurable (default 10800s = 3 hours)                  │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Total memory: ~1KB per service + ~64 bytes per backend             │
│  For 10,000 services × 10 backends = ~650KB (fits in L2 cache)     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
