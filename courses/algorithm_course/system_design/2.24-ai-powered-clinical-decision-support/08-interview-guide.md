...
| **LLM-augmented explanations** | Safety-stratified architecture; deterministic safety path + advisory LLM layer |
| **Clinical foundation models** | Fine-tuning domain models; medical hallucination detection; regulatory classification |
| **Multimodal input** | Combining structured FHIR data with clinical notes and lab report PDFs |

---

## Advanced Trade-offs (2025-2026 Landscape)

### LLM Integration Trade-offs

| Decision | Option A | Option B | Recommendation |
|----------|----------|----------|----------------|
| **LLM in critical path** | LLM generates explanations inline | LLM augments asynchronously | Async with 1s timeout; serve structured alert immediately |
| | Pros: Richer explanations | Pros: No latency impact | |
| | Cons: Latency risk, hallucination risk | Cons: Explanation may arrive after clinician acts | |
| **LLM for DDI severity** | LLM assesses severity context | Rule-based severity only | Rule-based only for DDI; LLM never overrides safety alerts |
| | Pros: More nuanced assessment | Pros: Deterministic, auditable | |
| | Cons: Hallucination could suppress alerts | Cons: May miss contextual nuances | |
| **Own vs hosted LLM** | Self-hosted clinical LLM | API-based LLM service | Self-hosted for PHI isolation; API for development |
| | Pros: PHI never leaves infrastructure | Pros: Lower ops burden, latest models | |
| | Cons: GPU cost, model update lag | Cons: PHI exposure risk, vendor dependency | |

### Regulatory Trade-offs

| Decision | Conservative | Aggressive | Recommendation |
|----------|-------------|-----------|----------------|
| **FDA classification** | Treat all CDS as Class II device | Claim exemption for rule-based features | Conservative: treat as device, use PCCP for agility |
| **EU AI Act timing** | Full compliance by Q1 2026 | Minimum viable compliance by Aug 2026 | Full compliance early: competitive advantage and safety margin |
| **PCCP scope** | Narrow pre-authorized changes | Broad change boundaries | Moderate: broad enough for monthly model updates, narrow enough for FDA acceptance |

### Trap Questions: LLM-Specific

| Trap Question | What Interviewer Wants | Best Answer |
|---------------|------------------------|-------------|
| "Why not use GPT-4 to replace rule-based DDI entirely?" | Test safety architecture thinking | "LLMs hallucinate. A hallucinated DDI (or worse, a hallucinated 'no interaction') could harm patients. Safety-critical DDI uses deterministic rules and validated knowledge bases. LLMs augment explanations and guideline synthesis only — they never override safety-critical paths." |
| "How do you handle LLM hallucinations about drug interactions?" | Test guardrail design | "Every LLM output is fact-checked against the structured knowledge base. If the LLM mentions a drug interaction not in the KB, or contradicts a KB-confirmed interaction, the output is discarded and a template explanation is served instead. All LLM outputs are labeled as AI-generated." |
| "What happens when the LLM provider changes their model?" | Test version stability | "LLM model version is pinned per encounter (same as ML model pinning). Updates require validation against the clinical test suite before promotion. If using external APIs, we pin to specific model versions and test before upgrading." |
