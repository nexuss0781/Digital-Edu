...
| **Clinical NLP** | Biomedical NLP Pipeline | Free-text symptom extraction, note parsing |
| **LLM Gateway** | LLM Gateway with prompt management | Clinical foundation model access for guideline synthesis |

---

## LLM-Augmented CDS Architecture (2025-2026 Evolution)

The system integrates clinical foundation models as an augmentation layer alongside deterministic rule engines, following a **safety-stratified architecture** where LLMs never override rule-based safety alerts.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│              LLM-AUGMENTED CDS ARCHITECTURE                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  SAFETY-CRITICAL PATH (Deterministic):                                      │
│  ───────────────────────────────────────────────────────────────────────── │
│  • Drug interaction detection → Knowledge graph + Rule engine               │
│  • Contraindication alerts → Structured DDI database                       │
│  • Dosing boundary checks → Validated pharmacokinetic models                │
│  ► NEVER bypassed or overridden by LLM output                              │
│                                                                             │
│  LLM-AUGMENTED PATH (Advisory):                                             │
│  ───────────────────────────────────────────────────────────────────────── │
│  • Clinical guideline synthesis → RAG over guideline corpus                 │
│  • Natural language explanations → LLM generates clinician-facing text     │
│  • Differential diagnosis enrichment → LLM suggests additional workup      │
│  • Treatment protocol summarization → Contextual protocol retrieval         │
│  ► Always labeled as AI-generated; always paired with structured evidence  │
│                                                                             │
│  INTEGRATION PATTERN:                                                       │
│  ───────────────────────────────────────────────────────────────────────── │
│  1. Deterministic engine produces structured alert/recommendation           │
│  2. LLM receives alert + patient context (de-identified)                   │
│  3. LLM generates natural language explanation and context                  │
│  4. Output validated against guardrails (medical fact-checking)             │
│  5. Combined response: structured alert + LLM-augmented explanation        │
│  ► Timeout: If LLM response >1s, serve structured alert without LLM text  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
|-----------|----------------|-----------|
| **Medical fact-checking** | Cross-reference LLM output against knowledge base assertions | Prevent hallucinated drug interactions or contraindications |
| **PHI stripping** | De-identify patient context before LLM inference; re-attach in post-processing | LLM never sees raw PHI |
| **Confidence gating** | Suppress LLM explanation if structured alert confidence < 0.6 | Low-confidence alerts should not receive persuasive LLM text |
| **Latency budget** | LLM has 1s budget; fall back to template-based explanation on timeout | Core alert delivery must not be blocked by LLM |
| **Version pinning** | Pin LLM model version per encounter (same as ML model pinning) | Consistent explanations within a clinical visit |
| **Regulatory labeling** | All LLM-generated text tagged with "AI-generated summary" disclaimer | EU AI Act transparency and FDA labeling requirements |
...
```
CDS ←→ EMR Integration Architecture:

┌──────────────────┐          ┌──────────────────┐
│   EMR/EHR System │          │   CDSS Platform   │
│  (Source of Truth)│          │  (Clinical Intel)  │
├──────────────────┤          ├──────────────────┤
│ Patient Record   │◀─FHIR───▶│ Patient Context   │
│ Medication List  │  R4      │ DDI Engine        │
│ Problem List     │          │ Risk Scoring      │
│ Lab Results      │          │ Diagnosis Engine  │
│ Clinical Notes   │          │ Guideline Engine  │
│ Orders           │          │ Alert Manager     │
├──────────────────┤          ├──────────────────┤
│ CDS Hooks Client │──Hooks──▶│ CDS Hooks Server  │
│ Alert Display    │◀─Cards───│ Card Generator    │
│ Override Capture │──Feed───▶│ Feedback Handler  │
└──────────────────┘          └──────────────────┘

Key principle: EMR owns the patient record.
CDSS is a stateless advisory service that enriches clinical
decisions without duplicating the record.
```

### Integration with Pharmacy OS

```
CDS ←→ Pharmacy Integration:

Shared Resources:
  • Drug knowledge base (DDI pairs, formulary data)
  • RxNorm terminology mapping
  • Override decisions (CDS alert overrides flow to pharmacy review)

Data Flow:
  1. CDS generates DDI alert during prescribing
  2. If overridden, override record flows to Pharmacy OS
  3. Pharmacy OS flags for pharmacist review before dispensing
  4. Pharmacy verification result flows back as feedback to CDS model
```
