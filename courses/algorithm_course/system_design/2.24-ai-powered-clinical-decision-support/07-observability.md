...
| LLM Fact-Check Failure | Review LLM outputs, check guardrail pipeline, disable LLM augmentation if systematic |
...
┌─────────────────────────────────────────────────────────────────┐
│                    MODEL MONITORING LAYERS                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  LAYER 1: REAL-TIME INFERENCE MONITORING                        │
│  ─────────────────────────────────────────────────────────────  │
│  • Prediction latency per model (p50/p95/p99)                  │
│  • Confidence score distribution (histogram)                    │
│  • Feature value distribution vs training baseline              │
│  • Error rate by model version                                  │
│  • GPU utilization and memory consumption                       │
│                                                                 │
│  LAYER 2: DATA DRIFT DETECTION (Hourly)                         │
│  ─────────────────────────────────────────────────────────────  │
│  • Input feature distribution shift (KL divergence)             │
│  • Label distribution change (if labels available)              │
│  • Population-Stability Index per feature                       │
│  • Alert: PSI > 0.2 triggers retraining evaluation             │
│                                                                 │
│  LAYER 3: CLINICAL OUTCOME MONITORING (Daily)                   │
│  ─────────────────────────────────────────────────────────────  │
│  • Override-with-adverse-outcome rate per alert type            │
│  • Diagnosis confirmation rate (30-day follow-up)               │
│  • Risk score calibration vs actual event rate                  │
│  • Subgroup performance disparity across demographics           │
│                                                                 │
│  LAYER 4: LLM AUGMENTATION MONITORING                           │
│  ─────────────────────────────────────────────────────────────  │
│  • Fact-check pass/fail rate per guideline category             │
│  • LLM response latency vs timeout budget                       │
│  • Fallback-to-template rate (LLM timeout or failure)          │
│  • LLM hallucination detection rate (via automated checking)   │
│  • Token usage and cost per CDS request                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
LLM AUGMENTATION METRICS:

cds_llm_requests_total
  Labels: model_version, request_type (guideline_synthesis|explanation)

cds_llm_latency_seconds
  Labels: model_version
  Buckets: 0.1, 0.25, 0.5, 0.75, 1.0, 2.0
  Alert: p99 > 1.0 triggers fallback evaluation

cds_llm_factcheck_results_total
  Labels: result (pass|fail|contradiction|timeout)
  Alert: fail_rate > 2% triggers LLM review

cds_llm_fallback_total
  Labels: reason (timeout|factcheck_fail|model_error|disabled)

cds_llm_token_usage_total
  Labels: model_version, direction (input|output)
  Purpose: Cost tracking and budget alerting

cds_llm_hallucination_detected_total
  Labels: hallucination_type (drug_name|dosage|contraindication|guideline_ref)
  Alert: Any increment triggers immediate investigation
```
