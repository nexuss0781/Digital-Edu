...
│  Gradient Δ   │  │  Gradient Δ   │  │  Gradient Δ   │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       │                 │                 │
...
| Technique | Implementation | Privacy Budget |
|-----------|----------------|---------------|
| **Secure Aggregation** | Cryptographic protocol ensures server sees only aggregate gradients | N/A (cryptographic) |
| **Differential Privacy** | Gaussian noise added to local gradients before transmission | ε = 8 per training round |
| **Gradient Clipping** | L2 norm clipping prevents individual patient influence leakage | Max norm = 1.0 |
| **Minimum Participation** | Require ≥10 hospitals per round for meaningful aggregation | Privacy amplification |

### Heterogeneous Data Handling

```
CHALLENGE: Hospital populations differ significantly
  - Hospital A: Urban academic center, diverse demographics
  - Hospital B: Rural community hospital, older population
  - Hospital C: Pediatric specialty, different drug profiles

SOLUTION: Personalized Federated Learning

1. GLOBAL MODEL: Trained on aggregate gradients (generalization)
2. LOCAL ADAPTATION: Each hospital fine-tunes on local data (personalization)
3. CONTRIBUTION WEIGHTING: Hospitals weighted by data quality and diversity

FUNCTION federated_round(hospitals, global_model):
    local_updates = []
    ...
    FOR EACH hospital IN hospitals:
        ...
        local_model.train(hospital.local_data, epochs=5)

        // Compute gradient delta
        gradient = local_model.weights - global_model.weights

        // Apply differential privacy
        clipped = clip_gradient(gradient, max_norm=1.0)
        noisy = add_gaussian_noise(clipped, sigma=0.01)

        // Encrypt and send
        encrypted = secure_encrypt(noisy)
        local_updates.append(encrypted)

    // Aggregate (server never sees individual gradients)
    aggregated = secure_aggregate(local_updates)

    // Update global model
    global_model.weights += learning_rate * aggregated
...
## Edge Deployment for Low-Latency Clinical Settings

### On-Premise Edge Architecture

For health systems requiring data sovereignty or ultra-low latency:

```
ON-PREMISE EDGE DEPLOYMENT:

┌───────────────────────────────────────────────────┐
│  HOSPITAL DATA CENTER (Edge Node)                  │
├───────────────────────────────────────────────────┤
│                                                   │
│  ┌─────────────┐  ┌─────────────┐                │
│  │ DDI Service  │  │ Risk Scoring │                │
│  │ (Full)       │  │ (Full)       │                │
│  └──────┬──────┘  └──────┬──────┘                │
│         │                │                        │
│  ┌──────▼──────┐  ┌──────▼──────┐                │
│  │ Knowledge   │  │ Risk Models │                │
│  │ Base Replica│  │ (Quantized) │                │
│  └─────────────┘  └─────────────┘                │
│                                                   │
│  ┌─────────────┐  ┌─────────────┐                │
│  │ Local Cache  │  │ Audit Buffer│                │
│  │ (Full DDI)  │  │ (Write-Ahead│                │
│  └─────────────┘  │  Queue)     │                │
│                    └──────┬──────┘                │
│                           │ Async                 │
│                           │ Sync                  │
...
EDGE CAPABILITIES:
  • Full DDI detection (no cloud dependency)
  • Risk scoring with quantized models
  • Local audit buffering with async cloud sync
  • Offline operation for up to 24 hours

CLOUD-ONLY CAPABILITIES:
  • Diagnosis suggestion (GPU-intensive ML)
  • LLM-augmented guideline synthesis
...
**Regulatory constraint:** In clinical decision support, Stage 5 may never be reached for diagnostic or treatment recommendations. FDA CDS guidance (Jan 2026) requires clinician review for all patient-affecting decisions. Regulatory approval (510(k) or De Novo) may be required before advancing beyond Stage 4 for any Software as a Medical Device (SaMD) classification.
