...
flowchart TB
    subgraph Core["Trading Core"]
        ME[Matching Engine<br/>Single-Threaded · Deterministic]
        OB[Order Book<br/>Red-Black Tree]
        OMS[Order Management<br/>Lifecycle · Idempotency]
    end

    subgraph Custody["Custody Layer"]
        HOT[Hot Wallet<br/>2-5% · Automated]
        WARM[Warm Wallet<br/>3-8% · MPC]
        COLD[Cold Wallet<br/>90%+ · Air-Gapped]
    end

    subgraph Chain["Multi-Chain"]
        DEP[Deposit Pipeline<br/>50+ Chains · Reorg Detection]
        WITH[Withdrawal Pipeline<br/>Address Screening · Signing]
    end

    subgraph Risk["Risk & Compliance"]
        MARGIN[Margin Engine<br/>Liquidation Cascades]
        KYC[KYC/AML<br/>FATF Travel Rule]
        POR[Proof of Reserves<br/>Merkle Tree]
    end

    OMS --> ME --> OB
    ME -->|Events| SETTLE[Settlement<br/>Batch Net]
    DEP --> SETTLE
    WITH --> HOT --> WARM --> COLD
    MARGIN --> ME
    KYC --> WITH
    ...
    classDef custody fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef chain fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef risk fill:#fff3e0,stroke:#e65100,stroke-width:2px

    class ME,OB,OMS,SETTLE core
    class HOT,WARM,COLD custody
    class DEP,WITH chain
    class MARGIN,KYC,POR risk
```
...
| [8.8 - Blockchain Network](../8.8-blockchain-network/) | Consensus mechanisms, block propagation, finality models |
| [1.18 - Event Sourcing System](../1.18-event-sourcing-system/) | Core pattern for matching engine state management and replay |
...
|-------|----------------------|
| **MPC Key Sharding v2** | Next-gen MPC protocols with proactive key share refresh; eliminates share staleness risk; enables key rotation without ceremony |
| **Zero-Knowledge Proof of Reserves** | ZK-PoR proves solvency without revealing individual balances or wallet addresses; stronger privacy guarantees |
| **Intent-Based Trading** | Users express trading intent ("buy BTC when ETH/BTC ratio drops below 0.03") rather than placing explicit orders; requires intent solver infrastructure |
| **AI-Powered Market Surveillance** | ML models detect wash trading, spoofing, and layering patterns in real-time; replaces rule-based detection |
| **Modular Blockchain Integration** | EIP-4844 (proto-danksharding), L2 rollups, and data availability layers change deposit/withdrawal patterns; faster finality reduces confirmation requirements |
| **Real-Time Proof of Reserves** | Move from monthly snapshots to continuous on-chain attestation; streaming Merkle tree updates |
| **Cross-Exchange Clearing** | Institutional settlement netting across multiple exchanges; reduces counterparty risk and capital requirements |
| **Regulatory Convergence (MiCA)** | EU MiCA framework becoming the de facto global standard; unified compliance architecture across jurisdictions |
...
|-----------|-------------|
| **Determinism over parallelism** | Matching engine is single-threaded per pair; correctness and auditability trump raw throughput |
| **Event sourcing as truth** | The matching engine event log is the only write path; all downstream systems derive from events |
| **Defense in depth for custody** | Three tiers (hot/warm/cold), MPC threshold signing, geographic distribution of key shares |
| **Fail-safe over fail-open** | Matching engine pauses on unrecoverable error rather than processing potentially corrupt state |
| **Client-driven consistency** | WebSocket subscribers detect their own gaps via sequence numbers and self-heal via snapshot re-sync |
| **Blast radius limitation** | Hot wallet capped at 2-5% of reserves; per-pair engine isolation; sharded balance database |
| **Irreversibility-aware design** | Every aspect of custody, withdrawal, and deposit assumes cryptocurrency transactions cannot be reversed |
...
- Fireblocks --- MPC Key Management and Institutional Custody (2025)
- Elliptic --- Blockchain Analytics and Compliance Monitoring Solutions
- Trail of Bits --- Cryptocurrency Exchange Security Audit Methodologies
