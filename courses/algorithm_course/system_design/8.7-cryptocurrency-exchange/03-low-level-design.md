...
OrderBook {
    ...
    PriceLevel {
        price:      DECIMAL
        orders:     DoublyLinkedList<Order>  // FIFO (First-In-First-Out, like a line at a store) for time priority
...
## Proof of Reserves Data Model

```
ReserveSnapshot {
    snapshot_id:        UUID (primary key)
    snapshot_time:      TIMESTAMP
    block_heights:      JSONB {chain → block_number}  -- per-chain snapshot point
    merkle_root:        VARCHAR(128)
    total_users:        INTEGER
    status:             ENUM(BUILDING, COMPLETE, PUBLISHED, VERIFIED)
    auditor_signature:  VARCHAR(512) NULLABLE
...
ReserveAssetProof {
    ...
    snapshot_id:        UUID (FK → ReserveSnapshot)
    asset:              VARCHAR(20)
    total_user_balance: DECIMAL(36,18)
    total_on_chain:     DECIMAL(36,18)
    reserve_ratio:      DECIMAL(10,6)  -- on_chain / user_balance (must be >= 1.0)
    wallet_addresses:   JSONB [{address, balance, chain, signed_message}]
    status:             ENUM(PENDING, VERIFIED, DISCREPANCY)

    INDEX(snapshot_id, asset)
}

UserMerkleLeaf {
    leaf_id:            BIGINT (primary key)
    snapshot_id:        UUID (FK → ReserveSnapshot)
    user_hash:          VARCHAR(128)  -- hash(user_id + unique_salt)
    balances_hash:      VARCHAR(128)  -- hash of all asset balances
    merkle_path:        JSONB  -- proof path from leaf to root
    leaf_index:         INTEGER

    INDEX(snapshot_id, user_hash)
}
```

---

## OCO (One-Cancels-Other) Order Algorithm

```
FUNCTION submit_oco_order(user_id, pair, oco_request):
    // OCO = two conditional orders; when one fills, the other is cancelled
    // Common use: limit sell (take profit) + stop-limit sell (stop loss)
    ...
    order_a = create_order(
        ...
        quantity: oco_request.quantity,
        oco_group_id: GENERATE_UUID()
    )
    ...
    order_b = create_order(
        ...
        price: oco_request.stop_limit_price,
        quantity: oco_request.quantity,
        oco_group_id: order_a.oco_group_id
    )

    // Balance lock: only lock once (not double)
    // Both orders share the same locked balance
    lock_amount = calculate_required_balance(order_a)
    lock_balance(user_id, asset, lock_amount, oco_group_id)

    SUBMIT order_a to matching engine
    SUBMIT order_b to stop order book

    // When either fills or triggers:
    ON EVENT order_filled(order_id):
        IF order_id == order_a.id:
            CANCEL order_b (the stop order)
        ELSE IF order_id == order_b.id:
            CANCEL order_a (the limit order)
        RELEASE any remaining locked balance

    RETURN {oco_group_id, order_a, order_b}
```
...
```
FUNCTION generate_deposit_address(user_id, chain):
    // BIP-44 derivation path: m/purpose'/coin_type'/account'/change/index
    // purpose = 44 (BIP-44 standard)
    // coin_type = chain-specific (0=BTC, 60=ETH, 501=SOL, etc.)
    // account = 0 (exchange master)
    // change = 0 (external)
    // index = user's sequential address index for this chain

    master_key = load_master_public_key(chain)  // xpub stored securely
    index = get_next_address_index(user_id, chain)

    derivation_path = "m/44'/{coin_type}'/0'/0/{index}"

    IF chain.type == EVM:
        // Ethereum-compatible: secp256k1 → Keccak256 → last 20 bytes
        child_pubkey = derive_child_key(master_key, derivation_path)
        address = "0x" + keccak256(child_pubkey)[12:32].hex()

    ELSE IF chain.type == UTXO:
        // Bitcoin: secp256k1 → RIPEMD160(SHA256(pubkey)) → Base58Check
        child_pubkey = derive_child_key(master_key, derivation_path)
        // Use native SegWit (bech32) for lower fees
        address = bech32_encode("bc", witness_program(child_pubkey))

    ELSE IF chain.type == SOLANA:
        // Solana: Ed25519 → Base58 of raw public key
        child_pubkey = derive_ed25519_key(master_key, derivation_path)
        address = base58_encode(child_pubkey)

    STORE DepositAddress{user_id, chain, address, derivation_path, index}
    RETURN address
```
