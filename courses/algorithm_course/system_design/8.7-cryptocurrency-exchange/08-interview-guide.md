...
- Each price level holds a FIFO (First-In-First-Out, like a line at a store) queue of orders
...
| Aspect | Crypto Exchange (This) | Stock Exchange (8.3-Zerodha) | Digital Wallet (8.4) | Payment Gateway (8.2) | DeFi DEX |
|--------|----------------------|---------------------------|--------------------|--------------------|---------|
| **Matching** | In-house, deterministic, event-sourced | Exchange-provided (NSE/BSE) | N/A | N/A | On-chain AMM (no order book) |
| **Custody** | Self-custody (hot/warm/cold) | Depository (CDSL/NSDL) | Bank escrow | Pass-through (no custody) | User self-custody (wallet) |
| **Settlement** | T+0 (instant on-exchange) | T+1 via clearinghouse | Instant (internal ledger) | T+0 to T+2 via bank rails | Atomic (same transaction) |
| **Hours** | 24/7/365 | Fixed market hours | 24/7 | 24/7 | 24/7 (blockchain dependent) |
| **Regulation** | VASP/MSB license, KYC/AML | SEBI/SEC regulated | Money transmitter | Payment processor | Largely unregulated |
| **Key risk** | Private key theft (irreversible) | Market manipulation | Double-spend on ledger | Chargeback fraud | Smart contract exploit |
...
Focus on: (1) Matching engine architecture (single-threaded, event-sourced, per-pair), (2) Order book data structure (red-black tree), (3) Hot/warm/cold custody with MPC. Skip margin trading, market data distribution, and blockchain integration.

### 30-Minute Version

Add: (4) Deposit/withdrawal pipeline with confirmation tracking and reorg protection, (5) Balance management with optimistic concurrency, (6) Market data fan-out architecture. Mention but don't detail: margin trading, proof of reserves.

### 60-Minute Version

Full depth: all of the above plus (7) Margin trading and liquidation cascade dampening, (8) Proof of reserves with Merkle trees, (9) Multi-chain abstraction (UTXO vs account model), (10) Regulatory compliance (KYC tiers, FATF travel rule), (11) Chaos engineering and disaster recovery.

---

## Additional Trap Questions

### "How do you handle a listing of a new token that goes viral?"

**Weak answer:** "We deploy a new matching engine and start accepting orders."

**Strong answer:** "New token listings are one of our highest-risk events. We pre-provision the matching engine before the listing goes live, load-test it with synthetic orders, and coordinate a staggered rollout. In the first 60 seconds after listing, we apply aggressive rate limiting (e.g., max 10 orders per user per second) and disable market orders entirely—only limit orders are accepted. This prevents bots from sweeping the initial liquidity. We also enforce a minimum resting time of 200ms on orders to prevent spoofing. The listing is staged: VIP traders get access 5 minutes before general availability, which seeds the order book with depth before the rush. We pre-assign the pair to a dedicated high-performance engine instance, not a multiplexed one, because we expect initial volume to be 10-50× the pair's eventual steady state."

### "Why use event sourcing instead of just writing to a database?"

**Weak answer:** "For performance—we avoid database writes."

**Strong answer:** "Event sourcing gives us three critical properties. First, auditability: the event log is an immutable record of every order, fill, and cancel, which regulators can independently replay to verify trade fairness. Second, recovery: if the matching engine crashes, we load the last snapshot and replay events since that snapshot—deterministically reconstructing the exact pre-crash state. This is why the engine must be single-threaded: deterministic replay requires deterministic execution. Third, decoupled consumers: balances, market data, analytics, and compliance monitoring all consume the same event stream independently. If the balance service goes down, it catches up from the log without affecting the matching engine. The trade-off is indefinite storage growth (our event log grows at ~50 GB/day) and the need for every consumer to be idempotent."

### "What prevents an insider from stealing funds?"

**Weak answer:** "We have access controls and background checks."

**Strong answer:** "Defense in depth against insider threats operates at multiple layers. For cold storage, funds require a 3-of-5 multi-party ceremony with geographically distributed key custodians—no single person (or even two people) can move cold wallet funds. For warm wallets, MPC threshold signing requires 2-of-3 key shares held in separate HSMs in different facilities—physical access to two facilities simultaneously is needed. For hot wallets, the policy engine enforces per-transaction and per-hour limits; even a compromised signing service can only drain the hot wallet balance (2-5% of total). Beyond keys: all admin actions write to an immutable audit log, code deployments to the matching engine require two independent reviewers, and withdrawal approval for amounts above threshold requires compliance sign-off separate from the engineering team. Annual penetration testing explicitly includes insider threat scenarios."

---

## Architecture Walkthrough Script

A suggested narrative flow for whiteboard presentation:

```
1. START with the matching engine (center of the board)
   "Everything flows through the matching engine—it's the single
    source of truth, single-threaded, deterministic per trading pair"

2. DRAW the event log below the matching engine
   "Every input and output is persisted to an append-only event log
    before acknowledgment—this gives us zero-loss recovery and audit"

3. ADD consumers branching from the event log
   "Balance updates, market data, settlement, and compliance monitoring
    all derive from this same event stream—no dual writes"

4. DRAW custody layer on the right
   "90%+ of assets sit in air-gapped cold storage with MPC signatures;
    only 2-5% is in the hot wallet for automated withdrawals"

5. ADD blockchain layer below custody
   "Each chain family (EVM, UTXO, Solana) has its own deposit/withdrawal
    microservice—they're too different for a single abstraction"

6. DRAW market data fan-out above the engine
   "Engine events → central processor → regional relays → edge WebSocket
    servers → 2M concurrent subscribers. 50M messages/sec at peak."

7. FINISH with risk/compliance on the left
   "Pre-trade risk checks gate every order; margin engine monitors
    positions every 100ms; KYC/AML screens every withdrawal"
```
