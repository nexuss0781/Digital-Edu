...
```
Monthly infrastructure cost per 10,000 lots:

Compute:
  Gate Service:           6 instances × $400/mo    = $2,400
  Booking Service:        4 instances × $400/mo    = $1,600
  Slot Service:           6 instances × $400/mo    = $2,400
  Payment Service:        3 instances × $400/mo    = $1,200
  Event Processor:        8 instances × $300/mo    = $2,400
  ANPR Service:           4 GPU instances × $800/mo = $3,200
  Other services:         10 instances × $300/mo   = $3,000
  ─────────────────────────────────────────────────────────
  Compute subtotal:                                  $16,200/mo

Storage:
  PostgreSQL (sharded):   5 TB × $200/TB            = $1,000
  Redis Cluster:          3 nodes × $300/mo          = $900
  Time-Series DB:         2 TB × $150/TB             = $300
  Blob Storage (ANPR):    30 TB × $20/TB             = $600
  Archive Storage:        20 TB × $5/TB              = $100
  ─────────────────────────────────────────────────────────
  Storage subtotal:                                   $2,900/mo

Networking:
  IoT Hub (33K events/sec): $3,000/mo
  API Gateway:              $1,500/mo
  Data transfer:            $2,000/mo
  ─────────────────────────────────────────────────────────
  Network subtotal:                                   $6,500/mo

Total cloud infrastructure:                           $25,600/mo
Per lot:                                              $2.56/mo
Per spot:                                             $0.026/mo
```

### Edge Hardware Cost (Per Gate)

```
Gate Controller (industrial SBC):          $300-500
Standby Controller:                        $300-500
ANPR Camera (IP camera with IR):           $500-1,000
Ticket Printer:                            $200-400
Display Board (LED):                       $300-600
UPS (4-hour battery):                      $200-400
Installation & Cabling:                    $500-1,000
─────────────────────────────────────────────────────
Total per gate:                            $2,300-$4,400
Per lot (2 gates avg):                     $4,600-$8,800
```
...
|------------------|-------------|-------------|-------------------|
| **Single gate controller** | 1 gate at 1 lot | ~2s failover to standby controller | Automatic failover; zero user impact if standby healthy |
| **Both gate controllers** | 1 gate at 1 lot | Gate stuck; attendant required | Physical override key; emergency dispatch |
| **Database shard** | ~100 lots | Bookings/payments unavailable for affected lots | Gates switch to offline mode; auto-failover to replica |
| **Redis cluster node** | ~3,000 lots | Availability queries served from DB (slower) | Sentinel promotes replica; bitmap reconstructed in <5s/lot |
| **IoT Hub** | All lots in region | Sensors stop updating; gates use cached data | Multi-AZ failover; gates operate independently |
| **Payment provider** | All lots | Exit gates fail-open; payments deferred | Provider SLA; deferred billing pipeline activates |
| **ANPR service** | All lots with ANPR | Fall back to ticket dispensing for all entries | Service restart; cached model reload |
| **Entire region** | All lots in region | Gates run on edge controllers; no reservations | Multi-region not needed (parking is local); gates self-sufficient |
