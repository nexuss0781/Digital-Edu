...
### Variant 5: Gateless Parking (Modern ANPR-Only)

- Emphasize: No physical barriers; ANPR-based entry/exit; license-plate-as-payment; mobile app billing
- Focus: Computer vision pipeline, missed-plate reconciliation, revenue protection without gates, privacy compliance
- Good for: ML/computer vision + system design hybrid interviews
...
|------|------------|----------|-------------|
| **Data Model** | Full hierarchy (lot → floor → zone → spot) with spot types and state machine | Lot → spot with basic states | Flat model (lot + list of spots) |
| **Availability** | Redis caching for real-time queries | Database query with proper index | No caching strategy |
| **Entry/Exit** | Multiple modes (QR, ticket, ANPR) | Single mode with clear flow | Vague or missing flow |
| **Payment** | Fee calculation with time-based rates | Basic pricing logic | No pricing discussion |

### Senior/Staff-Level (45-min format)

| Area | Exceptional | Adequate | Insufficient |
|------|------------|----------|-------------|
| **Offline Resilience** | Edge-first with cache sync, reconciliation, fail-open policy, conflict resolution | Mentions offline need but no detailed design | Assumes always-online; no edge discussion |
| **Concurrency** | Optimistic locking with version column; explains why it suits parking (low contention) vs alternatives | Mentions locking but no analysis of contention profile | No concurrency control; or uses heavyweight approach without justification |
| **IoT Pipeline** | Sensor debounce, cross-validation with gate events, display board fan-out | Sensor → update flow without noise handling | Assumes sensors are perfectly reliable |
| **Scalability** | Lot-based sharding; explains zero cross-shard transactions; Redis bitmaps | Mentions sharding without clear key choice | Single database; no partitioning strategy |
| **Trade-offs** | Articulates 3+ decisions with options, trade-offs, and context-dependent recommendations | Makes decisions but doesn't discuss alternatives | Presents single solution without trade-off analysis |

---

## Cross-System Comparison Questions

These questions test the candidate's ability to connect parking system patterns to other domains:

### "How does parking spot allocation differ from hotel room allocation?"

**Answer**: Three key differences:
1. **Physical state tracking**: Parking has sensors confirming vehicle presence; hotels rely on check-in/check-out events. A parked car that overstays is physically detectable; a hotel guest who overstays is only known at the front desk.
2. **Allocation granularity**: Hotels typically assign rooms at check-in (not at booking); parking pre-assigns spots at booking. Hotels can upgrade room types easily; parking can't easily reassign a driver mid-stay.
3. **Contention profile**: Hotels have sparse competition (10-500 rooms of each type); parking has even lower contention per spot (hundreds of spots of each type). Both use optimistic approaches, but parking adds sensor-based physical verification.

### "How would you adapt this design for bicycle parking with docking stations?"

**Answer**: Key adaptations:
- **Physical model**: Dock → bay replaces lot → floor → zone → spot (simpler hierarchy)
- **Sensors**: Weight/presence sensors per dock slot (simpler than car sensors)
- **No gates**: Open docking stations; lock/unlock per bay replaces gate open/close
- **Entry mode**: App-based unlock only (no walk-in tickets, no ANPR)
- **Duration**: Minutes-hours vs hours-days changes pricing granularity
- **Rebalancing**: Bikes must be redistributed (station A fills, station B empties)---a logistics problem that parking doesn't have

### "What patterns from this system apply to EV charging networks?"

**Answer**: Strong overlap in:
- **Spot allocation**: Reservation-based with time windows; optimistic locking for charger assignment
- **Real-time availability**: Bitmap-style tracking of charger status (AVAILABLE, IN_USE, FAULTED)
- **Pricing**: Time-based + consumption-based (kWh); peak/off-peak rates
- **IoT pipeline**: Charger telemetry (power draw, battery level, faults) mirrors sensor pipeline
- **Offline resilience**: Chargers must function during network outages (local authorization)
- New challenge: **Load balancing** (electrical capacity constraint) has no parking analog
