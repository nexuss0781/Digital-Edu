...
### Slowest part of the process 1: Sensor Event Volume at Busy Lots
...
### Slowest part of the process 2: Reservation Queries for Date Ranges
...
### Slowest part of the process 3: Payment at Exit During Rush Hour
...
### Slowest part of the process 4: Display Board Update Fan-Out
...
## Deep Dive 4: EV Charging Spot Management

### Extended State Model for EV Spots

EV charging spots have a richer state model than standard parking bays because they track both vehicle presence and charging progress:

```mermaid
%%{init: {'theme': 'neutral'}}%%
stateDiagram-v2
    [*] --> AVAILABLE: Spot initialized

    AVAILABLE --> RESERVED: Booking confirmed
    AVAILABLE --> OCCUPIED_CHARGING: Walk-in plugs in

    RESERVED --> OCCUPIED_CHARGING: Driver arrives, starts charging
    RESERVED --> AVAILABLE: No-show / cancellation

    OCCUPIED_CHARGING --> OCCUPIED_CHARGED: Charging complete (battery full or limit reached)
    OCCUPIED_CHARGING --> AVAILABLE: Vehicle departs during charging

    OCCUPIED_CHARGED --> AVAILABLE: Vehicle departs
    OCCUPIED_CHARGED --> IDLE_FEE: Grace period (30 min) expires, vehicle still present

    IDLE_FEE --> AVAILABLE: Vehicle departs (idle fee applied)

    note right of OCCUPIED_CHARGING
        Charger reports kWh consumed
        and estimated completion time
        via IoT hub.
    end note

    note right of IDLE_FEE
        Idle fee incentivizes turnover.
        Rate: 2× parking hourly rate.
        Notification sent at charging
        complete + at grace period end.
    end note
```

### Charging Load Balancing

A parking structure with 200 EV spots cannot simultaneously deliver maximum power to all chargers---the building's electrical capacity is a hard constraint:

```
FUNCTION balanceChargingLoad(lotId, maxBuildingKW):
    activeChargers = getActiveChargers(lotId)
    totalDemandKW = SUM(activeChargers.requestedKW)

    IF totalDemandKW <= maxBuildingKW:
        // Sufficient capacity: all chargers at full power
        RETURN  // no throttling needed

    // Priority-based allocation:
    // 1. Vehicles departing within 1 hour get priority
    // 2. Vehicles with low battery (<20%) get priority
    // 3. Remaining capacity distributed equally

    urgentChargers = activeChargers.filter(
        WHERE booking.end_time < NOW() + 1 HOUR
           OR battery_pct < 20
    )

    urgentDemand = SUM(urgentChargers.requestedKW)
    remainingCapacity = maxBuildingKW - urgentDemand

    normalChargers = activeChargers - urgentChargers
    perChargerAllocation = remainingCapacity / normalChargers.count

    FOR charger IN normalChargers:
        charger.setMaxKW(perChargerAllocation)

    logLoadBalancingEvent(lotId, totalDemandKW, maxBuildingKW, throttledCount)
```

---

## Deep Dive 5: Gateless Parking Architecture

### The Shift from Physical Gates to ANPR-Only

Modern parking facilities increasingly eliminate physical gate barriers entirely, using ANPR cameras at entry/exit points to track vehicle movements without any physical obstruction:

| Aspect | Traditional Gated | Gateless (ANPR-Only) |
|--------|------------------|---------------------|
| **Entry flow** | Stop → scan/ticket → wait for gate → drive | Drive through at low speed → ANPR captures plate |
| **Exit flow** | Stop → payment → wait for gate → drive | Drive through → billed automatically via registered payment |
| **Throughput** | ~200 vehicles/hour per lane (gate cycle time) | ~600 vehicles/hour per lane (no stopping) |
| **Hardware cost** | Gate barrier $5K + controller $3K per lane | ANPR camera $2K + edge compute $1K per lane |
| **Failure mode** | Gate stuck = physical blockage | Camera failure = missed capture (reconcile via other cameras) |
| **Offline handling** | Local decision engine required | Simpler---capture plate image locally, process later |
| **Security** | Physical barrier prevents unauthorized entry | Relies on enforcement (ANPR-based violation detection) |

### Gateless Revenue Protection

Without physical barriers, revenue collection depends on reliable plate capture and billing:

```
FUNCTION gatelessEntryCapture(cameraId, image, lotId):
    plates = anprEngine.detectAllPlates(image)
    // Multiple detection: front plate + rear plate for redundancy

    FOR plate IN plates:
        normalizedPlate = normalize(plate.text)

        // Check registered user (app-based payment)
        user = lookupRegisteredUser(normalizedPlate)
        IF user IS NOT NULL:
            startSession(lotId, user.id, normalizedPlate, timestamp)
            RETURN  // automatic billing on exit

        // Check permit
        permit = lookupPermit(normalizedPlate, lotId)
        IF permit IS NOT NULL:
            logPermitEntry(lotId, permit.id, timestamp)
            RETURN

    // Unregistered vehicle: capture for pay-by-plate billing
    storeEntryRecord(lotId, plates[0].text, image, timestamp)
    // Driver pays via app, kiosk, or receives invoice by mail

FUNCTION gatelessExitCapture(cameraId, image, lotId):
    plate = anprEngine.recognize(image)
    session = findActiveSession(lotId, plate)

    IF session IS NOT NULL AND session.user IS NOT NULL:
        fee = calculateFee(session)
        chargeStoredPaymentMethod(session.user, fee)
        closeSession(session)
    ELSE:
        // Unregistered: log exit, generate invoice
        closeUnregisteredSession(lotId, plate, timestamp)
        generateMailInvoice(plate)  // via DMV lookup where permitted
```

### Missed Plate Reconciliation

With gateless systems, ~2-3% of plate reads may fail. The system reconciles using multiple cameras and temporal correlation:

| Recovery Method | Approach | Success Rate |
|----------------|----------|-------------|
| **Redundant cameras** | 2+ cameras per entry/exit; combine readings | Recovers 80% of single-camera misses |
| **Temporal correlation** | Match entry image with exit image by vehicle appearance (color, make, model) | Recovers 60% of remaining |
| **Manual review queue** | Failed reads flagged for human operator review | Catches critical misses |
| **Floor cameras** | Cameras on parking floors cross-reference with entry/exit | Fills gaps for long-duration stays |

---

## Slowest part of the process 5: ANPR Processing at Scale

**Problem**: During morning rush, a busy lot may see 300+ vehicles arriving per hour across 4 entry lanes. Each ANPR image requires ML inference (plate detection + OCR), which takes 200-500ms on CPU. Sequential processing cannot keep up.

**Solution**:
- **Edge GPU inference**: Run lightweight ANPR models directly on edge devices (e.g., NVIDIA Jetson) at each camera. Achieves 50ms inference, handling 20 vehicles/second per lane.
- **Pipeline parallelism**: Image capture, preprocessing, detection, and OCR run as concurrent pipeline stages.
- **Fallback to cloud**: If edge inference confidence is low (<85%), forward image to cloud GPU cluster for higher-accuracy model. Cloud has 2-second latency budget.
- **Batch processing during idle hours**: Re-process all captured images with highest-accuracy model overnight for billing reconciliation.
