...
| Design | Relevance |
|--------|-----------|
| [7.1 - Uber/Lyft Ride-Hailing](../7.1-uber-lyft/) | Real-time geospatial systems; IoT integration patterns; surge pricing parallels |
| [7.7 - Hotel Booking System](../7.7-hotel-booking-system/) | Reservation management with time-window allocation; yield management pricing; overbooking strategies |
| [6.6 - Ticketmaster](../6.6-ticketmaster/) | High-contention seat allocation; optimistic locking under concurrent demand |
| [1.8 - Distributed Lock Manager](../1.8-distributed-lock-manager/) | Optimistic/pessimistic locking for slot allocation; version-column pattern |
| [1.5 - Distributed Log-Based Broker](../1.5-distributed-log-based-broker/) | Sensor event ingestion pipeline; ordered event processing |
| [8.2 - Stripe/Razorpay](../8.2-stripe-razorpay/) | Payment processing integration; PCI-DSS compliance patterns; tokenization |
| [1.4 - Distributed LRU Cache](../1.4-distributed-lru-cache/) | Real-time availability caching with Redis; bitmap data structures |
| [10.2 - IoT Platform](../10.2-iot-platform/) | IoT device management at scale; edge computing patterns; telemetry pipelines |
...
|---------|---------------------------|
| **Edge Computing** | Gate controllers make autonomous decisions locally, syncing with cloud asynchronously; critical for offline resilience |
| **Event Sourcing** | Gate events, sensor readings, and booking state changes captured as immutable event log for reconciliation and audit |
| **Bitmap Index** | Redis bitmaps represent per-spot availability with O(1) lookup and BITOP AND for time-range queries |
| **Optimistic Concurrency Control** | Version-column on ParkingSpot prevents double-allocation without distributed locks |
| **CQRS** | Write path (gate events, bookings) separated from read path (availability queries, display boards) with Redis as materialized view |
| **Saga Pattern** | Multi-step booking flow (allocate spot → create booking → process payment → generate QR) with compensation on failure |
| **Circuit Breaker** | Payment service failures trigger fail-open exit gates with deferred billing rather than blocking vehicles |
| **Digital Twin** | Software representation of physical parking infrastructure mirrors real-time state of every bay, gate, and sensor |

---

## Evolution & Industry Trends (2025-2026)

| Trend | Impact on Architecture |
|-------|----------------------|
| **Camera-based spot detection** | Computer vision replacing per-bay ultrasonic sensors; single camera covers 20-40 bays with edge AI inference; reduces hardware cost by 60-70% |
| **Gateless parking** | ANPR-based entry/exit without physical barriers; eliminates gate failure modes but requires robust license plate billing pipeline |
| **EV charging integration** | Parking + charging as unified service; vehicle-to-grid (V2G) during long-term parking; charging load balancing across the facility |
| **Autonomous vehicle readiness** | AV self-parking in narrower bays (no door clearance needed); 40% more capacity per floor; requires AV communication protocol integration |
| **ML-based dynamic pricing** | Demand prediction models adjust rates in real-time based on occupancy trends, events, weather; replaces static peak/off-peak rules |
| **Curb management platforms** | City-wide dynamic curb allocation (parking, loading, rideshare) coordinated through real-time API; replaces static signage |
...
- Computer Vision for Parking Occupancy Detection (IEEE ITSC, 2025)
- EV Charging Infrastructure in Smart Parking Systems (SAE International, 2025)
- Gateless Parking Architecture: ANPR-Only Entry/Exit Systems (ITS World Congress, 2025)
- Dynamic Pricing Optimization for Parking Facilities (Transportation Research, 2025)
