...
### Permit Cache (for ANPR Fast-Path)

```
// Pre-loaded permit data for sub-100ms gate decisions
Key: permit:{lot_id}:{normalized_plate}
Value: JSON {
    permit_id: "uuid",
    vehicle_id: "uuid",
    permit_type: "MONTHLY",
    ...
    end_date: "2026-12-31",
    spot_id: "uuid" (optional, for reserved-spot permits),
    currently_parked: false
}
TTL: Synced with cloud every 30 seconds; no expiry (overwritten on sync)
```
...
### Gate Controller Sync Cycle

```
FUNCTION syncCycle(gateController, cloudEndpoint):
    // Runs every 30 seconds on each gate controller
    // Uses delta sync to minimize bandwidth

    lastSyncTimestamp = gateController.getLastSyncTime()

    // PHASE 1: PULL (cloud → edge)
    pullRequest = {
        lot_id: gateController.lotId,
        since: lastSyncTimestamp,
        request_types: [BOOKINGS, PERMITS, PRICING, SPOT_STATUS]
    }

    pullResponse = cloudEndpoint.pull(pullRequest)

    // Apply updates atomically to local SQLite
    BEGIN LOCAL TRANSACTION
        FOR booking IN pullResponse.bookings:
            localDB.upsertBooking(booking)
        FOR permit IN pullResponse.permits:
            localDB.upsertPermit(permit)
        FOR rule IN pullResponse.pricingRules:
            localDB.upsertPricingRule(rule)
        FOR spot IN pullResponse.spotUpdates:
            localDB.updateSpotStatus(spot)
    COMMIT LOCAL TRANSACTION

    // PHASE 2: PUSH (edge → cloud)
    pendingEvents = localDB.getUnSyncedEvents()

    IF pendingEvents IS NOT EMPTY:
        pushResult = cloudEndpoint.push({
            gate_id: gateController.gateId,
            events: pendingEvents
        })

        IF pushResult.success:
            localDB.markEventsSynced(pendingEvents.ids)

        // Handle conflicts from push
        FOR conflict IN pushResult.conflicts:
            resolveConflict(conflict)

    // PHASE 3: Update sync metadata
    gateController.setLastSyncTime(NOW())
    gateController.setCloudReachable(true)

    // If sync fails (network error), mark offline
    ON ERROR:
        gateController.setCloudReachable(false)
        gateController.incrementFailedSyncCount()
        IF failedSyncCount > 6:  // 3 minutes of failed syncs
            alertLotOperator("Gate {gateId} offline for 3+ minutes")
```

### Conflict Resolution Rules

| Conflict Type | Edge State | Cloud State | Resolution |
|--------------|-----------|-------------|------------|
| **Booking cancelled during offline** | CONFIRMED (cached) | CANCELLED | Accept cloud state; if vehicle already entered, convert to walk-in ticket |
| **Permit revoked during offline** | ACTIVE (cached) | REVOKED | Accept cloud state; vehicle already inside continues (cannot force exit) |
| **Spot marked OUT_OF_SERVICE** | AVAILABLE (cached) | OUT_OF_SERVICE | Accept cloud state; if vehicle directed to spot, alert attendant |
| **Pricing rule changed** | Old rates (cached) | New rates | Accept cloud state; entries during offline billed at rate active at entry time |
| **Duplicate entry events** | Entry logged offline | Entry also logged by backup system | Deduplicate by gate_id + timestamp ± 5s window |

---

## Webhook & Event Notification APIs
...
POST /api/v1/webhooks
  Body: {
    url: "https://operator-system.example.com/parking-events",
    events: [
        "booking.created", "booking.cancelled", "booking.no_show",
        "gate.entry", "gate.exit", "gate.offline", "gate.online",
        "lot.capacity_threshold",  // triggered at 80%, 90%, 95%, 100%
        "payment.completed", "payment.failed", "payment.deferred",
        "sensor.floor_degraded",   // >10% sensors unhealthy
        "alert.revenue_anomaly"
    ],
    lot_ids: ["lot-42", "lot-43"],  // scope to specific lots (or omit for all)
...
```
{
    "event_type": "lot.capacity_threshold",
    "event_id": "uuid",
    ...
    "data": {
        ...
        "available_spots": 25,
        "by_type": {
            "COMPACT": { "total": 200, "available": 5 },
            "REGULAR": { "total": 200, "available": 12 },
            "EV": { "total": 30, "available": 0 },
            "HANDICAPPED": { "total": 20, "available": 3 },
            "MOTORCYCLE": { "total": 50, "available": 5 }
        }
    },
...
|----------|-------|-------|
| **Delivery** | At-least-once | Idempotency key (event_id) for deduplication |
| **Retry policy** | 3 retries with exponential backoff (1s, 5s, 30s) | After 3 failures, event queued for manual review |
| **Ordering** | Best-effort per lot | Events for the same lot delivered in order when possible |
| **Latency** | < 5s from event occurrence | Non-blocking; does not affect gate operations |
| **Signature** | HMAC-SHA256 | Operator verifies payload integrity |
