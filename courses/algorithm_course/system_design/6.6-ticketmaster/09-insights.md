...
## Insight 11: Payment Gateway as the True Slowest part of the process
...
## Insight 12: Queue Position Randomization Defeats Speed-Based Bot Advantage

**Category:** Fairness
**One-liner:** Randomize queue positions during the join window so that speed of arrival provides zero advantage -- neutralizing bots without detecting them.

**Why it matters:** Bot detection is an arms race: every detection technique has a countermeasure (residential proxies defeat IP blocking, headless browsers defeat JavaScript challenges, AI defeats CAPTCHAs). But randomizing positions during the join window makes speed irrelevant -- a bot that joins 0.001s after the queue opens gets the same random position as a human who joins 10 minutes later. This eliminates the bot's primary advantage (speed) without needing to identify it at all. Combined with Verified Fan pre-registration and account-level purchase limits, this makes scalping economically unattractive. The deeper insight: sometimes the best defense against adversarial behavior is designing the system so the adversary's advantage doesn't matter, rather than trying to detect and block them.

---

## Insight 13: Reconciliation Bridges the Redis-PostgreSQL Consistency Gap

**Category:** Consistency
**One-liner:** Redis (ephemeral holds) and PostgreSQL (durable orders) are two sources of truth that must be periodically reconciled to detect and resolve drift.

**Why it matters:** The two-tier architecture (Redis for contention, PostgreSQL for durability) is essential for performance but creates a consistency gap. Redis holds can be lost on failover, TTL drift can cause premature expiry, and app crashes between payment and seat-state-update create orphaned entries. Post-on-sale reconciliation cross-references every Redis SOLD key against every CONFIRMED order in the database, flagging three discrepancy types: orphaned Redis keys (clean up), missing Redis keys (safe -- DB is authoritative), and double-sells (critical incident). This is not a workaround for a bad design -- it's a deliberate architectural pattern. Any system that uses an ephemeral store as a write buffer in front of a durable store needs reconciliation as a first-class component, not an afterthought.
