...
## 6. Ticket Fraud Prevention

### Fraud Vector Analysis

| Fraud Type | Method | Impact | Detection |
|-----------|--------|--------|-----------|
| **Counterfeit tickets** | Forged barcodes/QR codes | Fans denied entry | Rotating barcodes (change every 15s); server-side validation |
| **Duplicate tickets** | Screenshot shared with multiple people | Multiple people claim same seat | Single-scan enforcement: first scan wins; second scan alerts security |
| **Ticket transfer fraud** | Sell ticket, then "transfer" to self before event | Buyer has invalid ticket | Blockchain-anchored transfer chain; original ticket invalidated on transfer |
| **Chargeback fraud** | Purchase tickets, attend event, then dispute charge | Financial loss for platform | Risk scoring at purchase; 3DS authentication; event attendance proof |
| **Account takeover** | Compromise account, transfer tickets | Fan loses tickets | MFA; transfer confirmation via original email; 24h delay on transfers |

### Secure Ticket Lifecycle

```
TICKET ISSUANCE:
    barcode_seed = CSPRNG(256 bits)   // Cryptographically secure random
    ticket_hash = SHA256(ticket_id + barcode_seed + event_id)
    Store: barcode_seed encrypted at rest (AES-256)

TICKET TRANSFER:
    old_ticket.status = INVALIDATED
    old_ticket.barcode_seed = NULL     // Destroy old barcode generation ability
    new_ticket = issue_new_ticket(new_owner, same_seat)
    new_ticket.barcode_seed = CSPRNG(256 bits)  // New seed
    new_ticket.transfer_chain.append(old_ticket.ticket_id)

GATE VALIDATION:
    scanned_code → lookup ticket_id
    verify TOTP(barcode_seed, current_time_step) matches
    verify ticket.status == ACTIVE
    verify NOT already_scanned
    mark_scanned(ticket_id, gate_id, timestamp)
```

---

## 7. Incident Response for On-Sale Events

### Severity Levels

| Severity | Definition | Example | Response Time | Escalation |
|----------|-----------|---------|---------------|-----------|
| **SEV-1** | Double-sell or complete system outage during on-sale | Seat sold to 2 users | Immediate | VP Engineering + Legal |
| **SEV-2** | Partial outage during on-sale | Payment gateway down, queue stalled | <5 min | On-call lead + payment team |
| **SEV-3** | Degraded performance during on-sale | Checkout latency >10s, error rate >2% | <15 min | On-call engineer |
| **SEV-4** | Non-on-sale system issue | Search returning stale results | <1 hour | Normal on-call |

### On-Sale War Room Protocol

```
PRE-ON-SALE (T-30 min):
    - War room channel opened (dedicated communication channel)
    - All team leads confirm systems nominal
    - Pre-scaling verified: Redis, app pool, WS, CDN warmed
    - Bot detection rules confirmed active
    - Payment gateways health-checked
    - Runbooks pulled up and linked

DURING ON-SALE:
    - Designated "caller" narrates metrics every 60 seconds
    - No changes to production unless SEV-1 or SEV-2
    - Queue drain rate adjustments require caller approval
    - Payment gateway switches require 2-person confirmation

POST-ON-SALE (T+1 hour):
    - Reconciliation engine runs automatically
    - Sell-through report generated
    - Any discrepancies flagged to on-call
    - Post-mortem scheduled if any SEV-1/SEV-2 incidents
```
...
| Data Category | Hot Storage | Warm Storage | Cold Archive | Deletion |
|-------------|-------------|-------------|-------------|---------|
| **Active tickets** | Until event + 30 days | N/A | N/A | After event + 30 days → move to orders |
| **Orders / payments** | 1 year | 3 years | 7 years | After 7 years (financial compliance) |
| **User profiles** | Active | N/A | N/A | 90 days after deletion request |
| **Queue entries** | 7 days post-event | N/A | N/A | Purged after 7 days |
| **Bot detection logs** | 90 days | N/A | N/A | Purged after 90 days |
| **Hold records** | 30 days | N/A | N/A | Purged after 30 days |
| **Access logs / audit** | 90 days | 1 year | 7 years | Per compliance schedule |
| **Device fingerprints** | 90 days | N/A | N/A | Hashed on collection; purged after 90 days |

### GDPR Right to Erasure Implementation

```
FUNCTION handle_erasure_request(user_id):
    // Step 1: Verify identity (prevent social engineering)
    verify_identity(user_id, multi_factor=true)

    // Step 2: Check for legal retention obligations
    active_orders = get_orders(user_id, status=['CONFIRMED', 'DISPUTED'])
    IF any(order.created_at > now() - 7_years for order in active_orders):
        // Cannot delete financial records within retention period
        RETURN {status: "PARTIAL", reason: "Financial records retained per legal obligation",
                retained_until: max_retention_date}

    // Step 3: Anonymize (don't delete — maintain referential integrity)
    anonymize_user(user_id, {
        email: "deleted-{hash}@anonymized.example",
        name: "Deleted User",
        phone: NULL,
        ip_addresses: NULL,
    ...
    // Step 4: Delete non-essential data
    delete_queue_entries(user_id)
    delete_hold_records(user_id)
    delete_activity_logs(user_id)

    // Step 5: Propagate to downstream systems
    emit_event("USER_ERASURE", {user_id, timestamp: now()})
    // Search index, analytics pipeline, bot detection will react

    RETURN {status: "COMPLETED", anonymized: true}
```

---

## 9. Supply Chain Security for Event Ecosystem

| Attack Surface | Risk | Mitigation |
|---------------|------|-----------|
| **Payment SDK compromise** | Attackers inject malicious code into client-side payment tokenization | Subresource Integrity (SRI) hashes; CSP headers; SDK loaded from payment provider CDN only |
| **Venue map SVG injection** | Malicious SVG with embedded scripts uploaded by compromised venue admin | SVG sanitization pipeline; no `<script>`, no event handlers, no external references |
| **Third-party analytics** | Analytics scripts exfiltrate form data (card numbers, PII) | CSP restricts data exfiltration; analytics on separate domain; no access to payment frames |
| **API key leakage** | Partner API keys exposed in client apps or public repos | Key rotation policy (90 days); API key scoping per endpoint; anomaly detection on key usage |
| **Insider threat** | Employee accesses ticket inventory to scalp | RBAC with least privilege; all admin actions audit-logged; 4-eyes principle for bulk operations |
