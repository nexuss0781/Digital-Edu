...
>
> - **HITL can become a Slowest part of the process**: If 20% need review, that's 200K items/day. At 2 minutes each, we need ~700 reviewer-hours. We use dynamic thresholds - if queue grows, we temporarily accept slightly lower accuracy to maintain throughput."
...
### Trap 5: "With GPT-4o vision, do you even need OCR anymore?"

**What interviewer wants:** Understanding of VLM trade-offs, not just hype.

**Response:**
> "VLMs are transformative but don't eliminate OCR entirely. The nuance:
> 1. **For the foundation-model fallback path**, yes — VLMs like GPT-4o extract directly from images better than OCR→LLM because they reason over layout visually, eliminating OCR error propagation.
> 2. **For the primary path**, no — specialized models (LayoutLMv3) still need OCR text as input and are 20-50x faster and 100x cheaper.
> 3. **New risk: prompt injection** — document text enters the LLM prompt, so adversarial documents can manipulate extraction. Discriminative models are immune.
> 4. **Cross-validation** — for high-value fields (amounts), we run both VLM and OCR, then compare. Agreement boosts confidence; disagreement flags for HITL.
>
> The 2025-2026 evolution is: OCR stays for the fast path, VLMs replace the foundation fallback, and small VLMs (Florence-2, Phi-3-vision) enable on-premise VLM extraction for privacy-sensitive workloads."

### Trap 6: "How do you prevent prompt injection through documents?"

**What interviewer wants:** Security awareness for LLM-based systems.

**Response:**
> "This is a critical and often overlooked attack vector. When we send a document image to a VLM with 'extract the invoice total', adversarial text in the document becomes part of the prompt. Defenses are layered:
> 1. **Architecture**: The specialized-model primary path (LayoutLMv3) is inherently immune — it's a discriminative model, not a generative one. Only 20% of documents hit the VLM path.
> 2. **Input sanitization**: Strip known injection patterns (e.g., 'ignore previous instructions') before sending to VLM.
> 3. **Structured output**: Use JSON schema mode so the VLM can only produce fields matching the expected schema, not arbitrary text.
> 4. **Statistical validation**: Post-extraction, compare results against historical norms per vendor/document type. An invoice total 10x below average triggers HITL.
> 5. **Audit trail**: Log the raw VLM response before and after validation for forensic analysis."
...
|  VLM direct extraction, ColPali, prompt injection defense,              |
|  structured output, Florence-2, Phi-3-vision, small VLMs,              |
|  OCR cross-validation, active learning flywheel                          |
+-------------------------------------------------------------------------+
```
...
| "How do VLMs change the architecture?" | VLM replaces OCR+extraction for fallback path, OCR stays for fast path, cross-validation for high-value fields |
| "Can you run this fully on-premise?" | Small VLMs (Florence-2) for extraction, Tesseract/DocTR for OCR, self-hosted everything except frontier VLM fallback |
