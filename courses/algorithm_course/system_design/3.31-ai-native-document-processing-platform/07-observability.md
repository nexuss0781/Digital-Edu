...
### VLM-Specific Observability (2025-2026)

```yaml
metrics:
  # VLM extraction path
  vlm_extraction_total:
    type: counter
    labels: [provider, document_type, structured_output]
    description: "VLM direct extraction calls"

  vlm_ocr_agreement_rate:
    type: gauge
    labels: [document_type, field_name]
    description: "Rate of VLM-OCR cross-validation agreement for high-value fields"

  vlm_hallucination_detected_total:
    type: counter
    labels: [provider, document_type]
    description: "Extractions flagged as VLM hallucination (value not in OCR text)"

  prompt_injection_attempts_total:
    type: counter
    labels: [detection_method, document_type]
    description: "Documents flagged for potential prompt injection"

  vlm_structured_output_parse_failures:
    type: counter
    labels: [provider, schema]
    description: "VLM responses that failed JSON schema validation"

  small_vlm_vs_cloud_accuracy:
    type: gauge
    labels: [small_model, cloud_model, document_type]
    description: "Accuracy comparison between self-hosted and cloud VLMs"
```

| Alert | Condition | Severity | Action |
|-------|-----------|----------|--------|
| **VLM-OCR Disagreement Spike** | Agreement rate < 85% for 15 min | Warning | Investigate VLM hallucination trend |
| **Prompt Injection Surge** | > 10 injection flags per hour | Critical | Quarantine flagged documents, alert security |
| **VLM Parse Failure Rate** | > 5% structured output failures | Warning | Check provider schema compatibility |
