...
| Slowest part of the process | Symptom | Impact | Severity |
|------------|---------|--------|----------|
...
| **Object Storage Throughput** | Upload/download delays | Ingestion Slowest part of the process | Low |
| **VLM Hallucination** | Extracted values not in document | Silent data corruption | High |
| **Prompt Injection via Documents** | Adversarial text manipulates LLM | Incorrect extractions, security breach | High |

### Mitigation Strategies

| Slowest part of the process | Mitigation | Implementation |
|------------|------------|----------------|
...
| **VLM Hallucination** | OCR cross-validation, structured output | Dual-path verification for high-value fields |
| **Prompt Injection** | Input sanitization, discriminative primary | Layered defense: sanitize → constrain → validate |
...
## Critical Component 5: VLM-Based Direct Extraction (2025-2026)

### The Challenge

Vision-language models (VLMs) promise to collapse the OCR→classification→extraction pipeline into a single step, but introduce new failure modes: hallucination (reading text that doesn't exist), prompt injection (adversarial text in documents), and inconsistent structured output.

### VLM vs Traditional Pipeline Comparison

| Dimension | Traditional (OCR → LayoutLMv3) | VLM Direct (GPT-4o / Gemini 1.5) |
|-----------|-------------------------------|-----------------------------------|
| **Latency** | 250ms (50ms OCR + 50ms class + 50ms extract + overhead) | 1-3s per page |
| **Accuracy (clean docs)** | 90-95% | 94-98% |
| **Accuracy (poor quality)** | 70-85% (OCR-limited) | 85-93% (visual reasoning) |
| **Error mode** | OCR propagation (systematic) | Hallucination (unpredictable) |
| **Cost per page** | ~$0.001 (self-hosted) | $0.005-0.01 (API) |
| **Privacy** | On-premise possible | Requires API call (or small VLM) |
| **Prompt injection risk** | None (discriminative model) | High (document text enters prompt) |

### VLM Extraction with Cross-Validation

```
ALGORITHM VLMExtractWithValidation(page_image, schema, high_value_fields)
INPUT:
  page_image: Raw document page image
  schema: Expected fields and types
  high_value_fields: Fields requiring cross-validation (e.g., amounts)

OUTPUT:
  extractions: Validated field extractions with confidence

PROCEDURE:
1. // VLM primary extraction using structured output
   vlm_result = call_vlm(
     image = page_image,
     system = "Extract fields per schema. Return JSON only.",
     schema = schema,
     response_format = "json_schema"
   )

2. // For high-value fields, cross-validate with OCR
   FOR each field IN high_value_fields DO
     IF field IN vlm_result THEN
       ocr_text = get_cached_ocr(page_image)  // Use OCR cache if available
       IF ocr_text IS NULL THEN
         ocr_text = perform_ocr(page_image)
       END IF

       // Check if VLM extraction appears in OCR text
       ocr_match = fuzzy_search(vlm_result[field].value, ocr_text)

       IF ocr_match.score > 0.9 THEN
         vlm_result[field].confidence *= 1.1  // Boost confidence
         vlm_result[field].cross_validated = true
       ELSE
         vlm_result[field].confidence *= 0.7  // Reduce confidence
         vlm_result[field].flag = "VLM-OCR mismatch"
       END IF
     END IF
   END FOR

3. // Prompt injection detection
   FOR each field IN vlm_result DO
     IF contains_instruction_patterns(vlm_result[field].value) THEN
       vlm_result[field].confidence = 0.0
       vlm_result[field].flag = "Potential prompt injection"
     END IF
   END FOR
...
### Small VLM Deployment for Privacy-Sensitive Tenants

| Model | Parameters | VRAM | Speed | Accuracy | Deployment |
|-------|-----------|------|-------|----------|------------|
| **Florence-2-Large** | 0.7B | 4 GB | 120ms/page | 91-95% | On-premise GPU |
| **Phi-3-vision** | 4.2B | 8 GB | 300ms/page | 90-94% | On-premise GPU |
| **LLaVA-Next** | 7B | 16 GB | 500ms/page | 92-96% | On-premise GPU |
| **Qwen-VL-Chat** | 9.6B | 20 GB | 600ms/page | 93-96% | On-premise GPU |

These models enable the VLM extraction path for tenants who cannot send documents to external APIs (HIPAA, financial regulations), at the cost of lower accuracy compared to frontier VLMs.
