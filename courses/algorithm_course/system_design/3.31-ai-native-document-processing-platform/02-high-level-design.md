...
- Use foundation models (GPT-4o, Claude Vision) as fallback for low-confidence or novel documents (20%)
...
### Decision 1b: VLM-First vs OCR-First for Foundation Fallback (2025-2026)

| Option | Pros | Cons |
|--------|------|------|
| **OCR → LLM Text Extraction** | Proven, auditable OCR output | Error propagation, two API calls |
| **VLM Direct Image Extraction** | No OCR errors, layout-aware | Higher latency, hallucination risk, prompt injection |
| **Hybrid: VLM Primary + OCR Validation** | Best accuracy, cross-check | Cost of dual processing |

**Decision:** `VLM Primary + OCR Cross-Validation for High-Value Fields`

**Rationale:**
- Vision-language models (GPT-4o, Gemini 1.5) extract directly from images, eliminating OCR error propagation
- For financial fields (amounts, account numbers), cross-validate VLM output against OCR text
- Structured output mode (JSON schema) constrains VLM responses, reducing hallucination
- VLM path inherits prompt injection risk — mitigated by input sanitization + post-extraction validation
- Small VLMs (Florence-2, Phi-3-vision) can serve as on-premise VLM fallback for privacy-sensitive tenants
...
|-----------|------------|-----------|
| **Model Serving** | Triton Inference Server, vLLM | GPU optimization, batching, VLM serving |
| **Model Registry** | MLflow | Version control, deployment tracking |
| **Feature Store** | Feast | Feature management for ML models |
| **Vector Database** | Qdrant / Pinecone | Document + visual embeddings (ColPali) |
| **LLM Gateway** | LiteLLM / Portkey | Multi-provider routing, caching, structured output |
| **Small VLMs** | Florence-2, Phi-3-vision | On-premise VLM extraction for privacy-sensitive tenants |
