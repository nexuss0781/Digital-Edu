...
#### VLM-Specific Prompt Injection Defense (2025-2026)

When using vision-language models for direct image extraction, the document image itself becomes the attack surface — adversarial text can be rendered in small fonts, watermarks, or hidden layers.

| Defense Layer | Technique | Effectiveness |
|---------------|-----------|---------------|
| **Architecture** | Discriminative models (LayoutLMv3) as primary path — inherently immune to injection | High |
| **Input** | Pre-scan OCR text for known injection patterns before VLM call | Medium |
| **Constraint** | Structured output mode (JSON schema) prevents arbitrary text generation | High |
| **Statistical** | Compare extracted values against per-vendor/per-type historical distributions | High |
| **Cross-validation** | VLM extraction must agree with OCR-based extraction for high-value fields | High |
| **Audit** | Log raw VLM responses before and after validation for forensic analysis | Monitoring |

```
ALGORITHM DetectVLMInjectionAttempt(vlm_result, historical_stats, document_type)
INPUT:
  vlm_result: Extraction from VLM
  historical_stats: Per-vendor field value distributions
...
2. FOR each field, value IN vlm_result.extractions DO
     risk_score = 0.0

     // Check for statistical anomalies
     IF field IN historical_stats THEN
       z_score = abs(value - historical_stats[field].mean) / historical_stats[field].stddev
       IF z_score > 3.0 THEN
         risk_score += 0.4
       END IF
     END IF

     // Check for instruction-like content in values
     IF contains_instruction_patterns(str(value)) THEN
       risk_score += 0.5
     END IF

     // Check for impossible values
     IF field.type == "currency" AND value <= 0 THEN
       risk_score += 0.3
     END IF

     IF risk_score > 0.3 THEN
       flagged_fields.append({field, value, risk_score})
     END IF
   END FOR

3. injection_risk = max(f.risk_score for f in flagged_fields) IF flagged_fields ELSE 0.0
   RETURN injection_risk, flagged_fields
```
