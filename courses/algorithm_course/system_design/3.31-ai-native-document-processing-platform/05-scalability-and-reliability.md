...
## VLM Scaling Considerations (2025-2026)

### VLM vs OCR+Specialized Scaling Profile

| Dimension | OCR + Specialized Models | VLM Direct Extraction |
|-----------|-------------------------|----------------------|
| **GPU per page** | 0.05 GPU-seconds | 0.5-2.0 GPU-seconds (self-hosted) |
| **API cost per page** | $0.001 (self-hosted) | $0.005-0.01 (cloud API) |
| **Batch efficiency** | High (32+ per batch) | Low (1-4 images per request) |
| **Scaling unit** | GPU workers (10x batch multiplier) | API rate limit or GPU count |
| **Cold start** | 5-10s model load | 0s (API) or 30-60s (self-hosted) |

### Hybrid Scaling Strategy

At 1M documents/day with 80% specialized / 20% VLM fallback:

| Path | Volume | GPU Requirement | API Cost/Month |
|------|--------|-----------------|----------------|
| **Specialized (fast path)** | 800K docs × 5 pages | 40 GPU-seconds/sec → 20 A10G GPUs | $0 (self-hosted) |
| **VLM fallback (cloud API)** | 200K docs × 5 pages | 0 (API) | $5,000-10,000 |
| **VLM fallback (self-hosted)** | 200K docs × 5 pages | 12 GPU-seconds/sec → 12 A100 GPUs | $0 |
| **Cross-validation OCR** | 200K × 5 × 30% high-value | Included in existing OCR pool | $225 (Textract) |

### VLM Rate Limit Resilience

```
ALGORITHM VLMFallbackWithRateLimit(page_image, schema, provider_pool)
INPUT:
  page_image: Document image
  schema: Extraction schema
  provider_pool: ["gpt-4o", "gemini-1.5", "claude-vision", "florence-2-local"]

OUTPUT:
  extraction_result: Best available extraction

PROCEDURE:
1. FOR each provider IN provider_pool DO
     IF rate_limiter.has_capacity(provider) THEN
       TRY
         result = call_vlm(provider, page_image, schema)
         rate_limiter.record_usage(provider)
         RETURN result
       CATCH RateLimitError, TimeoutError
         rate_limiter.mark_limited(provider, backoff=60s)
         CONTINUE
       END TRY
     END IF
   END FOR

2. // All VLM providers exhausted — fall back to OCR+specialized
   ocr_result = perform_ocr(page_image)
   specialized_result = run_specialized_extraction(ocr_result, schema)
   specialized_result.fallback_reason = "vlm_rate_limited"
