...
**Optimization**: Pre-compute and cache regex results for common patterns. For small value sets (e.g., `status` has only 5 values), regex matching is fast. For high-cardinality labels, regex matching scans many values and is a known performance Slowest part of the process.
...
## WAL (Write-Ahead Log) Implementation Detail

The WAL is the durability backbone of the ingestion pipeline---every acknowledged sample is WAL-durable before the ingester returns success:

### WAL Segment Format

```
WAL SEGMENT FORMAT:
  Each segment: 128 MB (configurable)
  Segment naming: wal/000001, wal/000002, ...

  Record types within a segment:
  ┌─────────────────────────────────────────────────┐
  │ Record Header (5 bytes)                          │
  │   - type (1 byte): SERIES=1, SAMPLES=2,         │
  │     TOMBSTONE=3, EXEMPLAR=4                      │
  │   - length (4 bytes): record payload size        │
  ├─────────────────────────────────────────────────┤
  │ Record Payload (variable)                        │
  │   SERIES record: series_id + labels (new series) │
  │   SAMPLES record: series_id + [(ts, val), ...]   │
  │   TOMBSTONE record: series_id + time_range       │
  │   EXEMPLAR record: series_id + trace_context     │
  ├─────────────────────────────────────────────────┤
  │ CRC32 checksum (4 bytes)                         │
  └─────────────────────────────────────────────────┘

  Segment rotation: when segment reaches 128 MB
  Old segments: kept until checkpoint covers them, then deleted
```

### WAL Checkpoint Algorithm

```
FUNCTION checkpoint_wal(head_block, wal):
    // Checkpoint captures the state needed to reconstruct the head block
    // without replaying the entire WAL from the beginning

    checkpoint = new_checkpoint()

    // Write all active series definitions
    FOR EACH series IN head_block.active_series:
        checkpoint.write_series(series.id, series.labels)

    // Write the current chunk state for each series
    // (last few samples needed to resume encoding)
    FOR EACH series IN head_block.active_series:
        chunk = series.current_chunk
        checkpoint.write_chunk_state(
            series.id,
            chunk.last_timestamp,
            chunk.last_value,
            chunk.last_delta  // for delta-of-delta continuation
        )

    checkpoint.finalize()

    // Delete WAL segments covered by this checkpoint
    FOR EACH segment IN wal.segments WHERE segment.max_time <= checkpoint.time:
        wal.delete_segment(segment)

    // Recovery: replay only WAL segments AFTER checkpoint
    // Reduces replay data by 80-90%
```
...
## Native Histogram Storage Format

Native histograms (exponential histograms in OTLP) store the entire distribution in a single series, replacing the N+2 fixed-bucket model:

### Encoding

```
NATIVE HISTOGRAM SAMPLE FORMAT:
┌──────────────────────────────────────────────────┐
│ Schema (1 byte)                                  │
│   - Resolution exponent (determines bucket width)│
│   - Schema 0: bucket width = 2^(2^0) = 2x       │
│   - Schema 3: bucket width = 2^(2^-3) ≈ 1.09x   │
│   - Schema 8: bucket width ≈ 1.003x (finest)     │
├──────────────────────────────────────────────────┤
│ Zero Threshold + Zero Count (12 bytes)           │
│   - float64 zero_threshold: values below this go │
│     into the zero bucket                         │
│   - uint32 zero_count                            │
├──────────────────────────────────────────────────┤
│ Positive Span + Buckets (variable)               │
│   - []Span: [{offset, length}, ...]              │
│     Defines which bucket indices are populated   │
│   - []int64 delta_counts: delta-encoded bucket   │
│     counts (exploits locality for compression)   │
├──────────────────────────────────────────────────┤
│ Negative Span + Buckets (variable)               │
│   - Same structure for negative values           │
├──────────────────────────────────────────────────┤
│ Sum (8 bytes, float64)                           │
│ Count (8 bytes, uint64)                          │
└──────────────────────────────────────────────────┘

Typical sample size: 200-500 bytes (vs. 16 bytes for a single counter)
But replaces 22 series → 1 series: net cardinality reduction of 95%
```

### Native Histogram Merge

```
FUNCTION merge_native_histograms(hist_a, hist_b):
    IF hist_a.schema != hist_b.schema:
        // Reduce resolution to the coarser schema
        target_schema = MIN(hist_a.schema, hist_b.schema)
        hist_a = reduce_resolution(hist_a, target_schema)
        hist_b = reduce_resolution(hist_b, target_schema)

    merged = NativeHistogram(schema = hist_a.schema)
    merged.zero_count = hist_a.zero_count + hist_b.zero_count
    merged.sum = hist_a.sum + hist_b.sum
    merged.count = hist_a.count + hist_b.count

    // Merge bucket counts by aligned index
    FOR EACH bucket_index IN UNION(hist_a.bucket_indices, hist_b.bucket_indices):
        merged.buckets[bucket_index] = hist_a.get(bucket_index, 0) + hist_b.get(bucket_index, 0)

    RETURN merged

// Key property: fully mergeable across distributed instances
// Unlike fixed-bucket histograms, native histogram merge produces EXACT results
```

---

## Exemplar Storage Design

Exemplars bridge the gap between metrics and traces by attaching trace context to specific metric samples:

### Exemplar Data Model

```
EXEMPLAR:
  metric_series_id: uint64     // series this exemplar is attached to
  labels: {                     // additional context
    "trace_id": "4bf92f3577b34da6a3ce929d0e0e4736",
    "span_id": "00f067aa0ba902b7"
  }
  value: float64               // the observed value (e.g., latency=0.48s)
  timestamp: int64             // when the observation occurred

STORAGE:
  Separate circular buffer per ingester (not in main chunk storage)
  Retention: 15 minutes (exemplars are for real-time drill-down, not historical)
  Max exemplars per series: 10 (ring buffer, oldest evicted)
  Total exemplar storage: 10M series x 10 exemplars x 64 bytes = ~6.4 GB
```

### Exemplar Query Flow

```
FUNCTION query_exemplars(metric_name, label_matchers, start, end):
    // Resolve matching series using the standard inverted index
    series_ids = resolve_series(metric_name, label_matchers)

    // Fetch exemplars from the circular buffer for matching series
    exemplars = []
    FOR EACH series_id IN series_ids:
        series_exemplars = exemplar_buffer.get(series_id, start, end)
        exemplars.extend(series_exemplars)

    // Sort by timestamp, limit to reasonable count for UI
    RETURN SORT(exemplars, BY timestamp DESC).LIMIT(100)
```

---

## eBPF Metric Collection Pipeline

eBPF probes collect kernel-level metrics without application instrumentation:
...
KERNEL SPACE:
  eBPF programs attached to:
    ...
    - tracepoints (static kernel trace points)
    - socket filters (per-connection data)
    - tc (traffic control for packet-level metrics)

  eBPF maps (shared kernel↔user memory):
    - Per-process TCP connection map: {pid, remote_addr} → {bytes_sent, bytes_recv, retransmits}
    - Per-process DNS latency histogram: {pid, domain} → exponential_histogram
    - Per-process file I/O map: {pid, fd} → {read_bytes, write_bytes, latency_sum, count}
    - Per-CPU scheduling map: {pid} → {runqueue_latency_sum, context_switches}

USER SPACE:
  eBPF exporter (runs every 15 seconds):
    1. Read eBPF maps via BPF syscall
    2. Enrich with process metadata (/proc/{pid}/cmdline → service name)
    3. Convert to OTLP metric format
    4. Push to OTel Collector receiver

ENRICHMENT:
  Raw eBPF metric:  tcp_retransmits{pid="12345"} = 42
  Enriched metric:  tcp_retransmits{service="api-server", pod="api-xyz", namespace="prod"} = 42
  Enrichment source: Kubernetes API → pod-to-pid mapping via cgroup hierarchy
```
