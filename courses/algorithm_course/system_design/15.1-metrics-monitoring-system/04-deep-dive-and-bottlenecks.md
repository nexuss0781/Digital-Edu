...
## Critical Component 4: Exemplar Pipeline

### Why This Is Critical

Exemplars bridge metrics and traces---enabling a single click from a latency spike on a dashboard to the exact trace that caused it. Without exemplars, engineers must manually construct trace queries based on metric label dimensions, a process that adds 2-5 minutes to every incident investigation. The challenge is that exemplars have fundamentally different storage characteristics than metrics: they are short-lived (15-minute retention vs. days/months), high-cardinality (one exemplar per interesting observation), and accessed by a different query pattern (lookup by series ID + time range, not by label matchers).

### Exemplar Ingestion and Storage

```
EXEMPLAR INGESTION FLOW:

  1. Application SDK observes a high-latency request (450ms)
  2. SDK attaches current trace context to the metric sample:
     http_request_duration_seconds_bucket{le="0.5"} = 42
       exemplar: {trace_id="abc123", span_id="def456"} value=0.48

  3. Agent batches exemplars alongside metric samples in OTLP payload
  4. Ingester receives batch:
     - Metric samples → WAL → Head Block (standard path)
     - Exemplars → Circular buffer (separate storage)

  5. Circular buffer per series:
     - Max 10 exemplars per series (ring buffer, oldest evicted)
     - Total memory: ~6.4 GB for 10M series x 10 exemplars x 64 bytes

  6. Query path:
     GET /api/v1/query_exemplars?query=http_request_duration_seconds_bucket&start=...&end=...
     → Resolve series IDs from label matchers
     → Scan exemplar circular buffer for matching series + time range
     → Return exemplars with trace context for dashboard rendering
```

### Exemplar Failure Modes

| Failure | Impact | Handling |
|---|---|---|
| Exemplar buffer full (high-traffic series) | Oldest exemplars evicted; drill-down for old data points not available | Acceptable: exemplars are for recent drill-down, not historical analysis |
| Trace backend unavailable | Click-to-trace fails even though exemplar data exists | Display exemplar data inline (trace_id, value) with "trace backend unavailable" message |
| Exemplar ingestion adds latency to hot path | Additional 2-5% overhead on ingestion pipeline | Async exemplar processing: buffer in ingester and process after WAL write |
...
### Race Condition 3: Concurrent Block Registration and Query

**Scenario**: The compactor registers a new compacted block while a query is resolving which blocks to read. The query might miss data if it reads the old blocks that are then deleted, but the new block isn't yet visible.

```
PROBLEM:
  Thread A (Query): lists blocks at time T1, gets [block_1, block_2, block_3]
  Thread B (Compactor): merges block_1 + block_2 → block_4 at time T2
  Thread B: registers block_4, deletes block_1 + block_2
  Thread A: attempts to read block_1 → file not found

SOLUTION:
  Block lifecycle with reference counting:
  1. New block (block_4) registered BEFORE old blocks deleted
  2. Deletion is lazy: old blocks marked "pending_deletion" but not removed
  3. Active queries hold references to blocks they are reading
  4. Blocks are physically deleted only when:
     - Marked pending_deletion AND
     - Reference count drops to zero (no active queries using them)
  5. Query at T1 continues reading block_1 and block_2 successfully
  6. New queries at T3 see block_4 (merged) instead of block_1 + block_2
  7. Once all T1-era queries complete, block_1 and block_2 are physically deleted

  This is equivalent to MVCC (Multi-Version Concurrency Control) for block storage.
```

### Race Condition 4: Alert State Machine Concurrent Updates
...
### Slowest part of the process 1: Inverted Index Memory Pressure
...
### Slowest part of the process 2: Query Fanout During Incidents
...
### Slowest part of the process 3: WAL Replay on Ingester Restart
...
### Slowest part of the process 4: Native Histogram Storage Amplification

**Problem**: While native histograms reduce cardinality by 95%, each native histogram sample is 15-30x larger than a simple counter sample (~200-500 bytes vs. ~16 bytes). For workloads that heavily use histograms (request latency tracking across many services), the per-sample storage cost increase can partially offset the cardinality savings, and the XOR compression that works brilliantly for float64 values is less effective for the structured bucket data in native histograms.

**Mitigation**:
1. **Delta-encoded bucket counts**: Store the difference from the previous sample's bucket counts, which are typically small (exploiting the same regularity assumption as delta-of-delta for timestamps).
2. **Schema-aware compression**: Group native histogram samples by schema level; within a schema, bucket indices are stable, enabling columnar-style compression of the bucket count arrays.
3. **Selective adoption**: Not all metrics benefit from native histograms. Counters and gauges should remain as simple float64 series. Reserve native histograms for distribution metrics (latency, size, duration) where the cardinality-vs-accuracy trade-off matters.
4. **Adaptive resolution**: Dynamically reduce histogram resolution (coarser buckets) for low-traffic series where high percentile accuracy is less critical, saving storage without sacrificing coverage.

---

## Performance Profiling

### Ingestion Hot Path Breakdown

| Phase | p50 Latency | p99 Latency | Slowest part of the process | Optimization |
|---|---|---|---|---|
| HTTP/gRPC deserialization | 0.5 ms | 2 ms | CPU (protobuf decode) | Batch size tuning (1,000-5,000 samples per batch) |
| Tenant auth + rate check | 0.2 ms | 1 ms | Memory (token cache lookup) | In-process JWT validation; shared-nothing token cache |
| Series fingerprint computation | 0.1 ms | 0.5 ms | CPU (hash computation) | xxHash64; cache fingerprint for known label sets |
| Consistent hash → ingester routing | 0.05 ms | 0.2 ms | Memory (ring lookup) | In-memory hash ring with binary search |
| WAL append (fsync) | 1 ms | 10 ms | Disk I/O (sequential write) | NVMe SSD; batch fsync every 100ms; async replication |
| Head block append | 0.1 ms | 0.5 ms | Memory (chunk append) | Lock-free append buffer per series; stripe locks for new series |
| **Total** | **~2 ms** | **~15 ms** | | |

### Query Hot Path Breakdown

| Phase | p50 Latency | p99 Latency | Slowest part of the process | Optimization |
|---|---|---|---|---|
| PromQL parse | 0.1 ms | 0.5 ms | CPU (parser) | AST cache for repeated dashboard queries |
| Series resolution (index lookup) | 1 ms | 10 ms | Memory (posting list intersection) | Roaring bitmaps; start with smallest posting list |
| Chunk fetch (head block) | 0.5 ms | 2 ms | Memory (direct read) | Chunks are contiguous in memory; no deserialization needed |
| Chunk fetch (object storage) | 5 ms | 50 ms | Network (remote read) | Chunk cache (L2); prefetch adjacent chunks |
| Decompression | 0.5 ms | 3 ms | CPU (Gorilla decode) | SIMD-accelerated decode; skip chunks outside time range |
| Aggregation/evaluation | 1 ms | 20 ms | CPU (vectorized computation) | Batch evaluation; SIMD for sum/avg; prealloc result buffers |
| **Total (recent data)** | **~3 ms** | **~35 ms** | | |
| **Total (historical data)** | **~10 ms** | **~100 ms** | | |

### Memory Profile of Ingester Nodes

| Component | Memory (2M series) | Growth Pattern | Optimization |
|---|---|---|---|
| Series map (fingerprint → series ref) | ~400 MB | Linear with active series | Hash map with open addressing; rehash threshold at 75% |
| Head chunks (active append buffers) | ~600 MB | Linear with active series | Chunk pooling; release chunks on series staleness |
| Inverted index (posting lists) | ~300 MB | Linear with series x labels | Roaring bitmap compression; symbol table dedup |
| WAL replay buffer | ~500 MB | Proportional to checkpoint interval | Frequent checkpointing reduces replay buffer |
| Go runtime (GC, goroutines, stack) | ~500 MB | ~25-30% of live data | Tune GOGC; use memory ballast to stabilize GC |
| **Total** | **~2.3 GB** | | **Provision 4 GB** (headroom for GC pressure) |
