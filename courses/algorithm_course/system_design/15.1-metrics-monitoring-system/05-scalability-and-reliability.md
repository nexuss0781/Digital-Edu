...
The disaggregated TSDB architecture enables independent horizontal scaling of each component along its specific Slowest part of the process:
...
- Write throughput limited by compactor upload parallelism (rarely a Slowest part of the process)

#### Index Scaling

The inverted index is the memory-bound Slowest part of the process. Scaling strategies:
...
### Cardinality-Aware Auto-Scaling

Unlike traditional auto-scaling based on CPU/memory, a metrics system must scale based on cardinality:

```
FUNCTION cardinality_aware_autoscaling():
    // Primary scaling signal: active series count per ingester
    series_per_ingester = get_metric("ingester_active_series") / ingester_count

    IF series_per_ingester > MAX_SERIES_PER_INGESTER * 0.8:
        // Scale based on series, not CPU
        desired_ingesters = CEIL(total_active_series / (MAX_SERIES_PER_INGESTER * 0.6))
        scale_up_ingesters(desired_ingesters)

    // Secondary scaling signal: cardinality growth rate
    growth_rate = derivative("total_active_series[1h]")
    IF growth_rate > CARDINALITY_GROWTH_THRESHOLD:
        // Proactive scaling: add capacity BEFORE hitting limits
        projected_series = total_active_series + (growth_rate * 2)  // 2-hour projection
        desired_ingesters = CEIL(projected_series / (MAX_SERIES_PER_INGESTER * 0.6))
        IF desired_ingesters > ingester_count:
            scale_up_ingesters(desired_ingesters)
            alert("CardinalityGrowthHighRate", "Proactive scale-up triggered")

    // Index memory scaling: separate from ingester CPU scaling
    index_memory_usage = get_metric("index_memory_bytes") / get_metric("node_memory_total")
    IF index_memory_usage > 0.7:
        // Either vertical scale (more RAM) or enable index sharding
        IF index_sharding_enabled:
            add_index_shard()
        ELSE:
            alert("IndexMemoryHigh", "Consider enabling index sharding or vertical scaling")
```
...
### Federation Architecture for Cross-Cluster Queries

```
FEDERATION QUERY FLOW:

  Global Query:
    rate(http_requests_total{service="checkout"}[5m]) summed across all clusters

  Execution:
    1. Global query frontend receives query
    2. Identifies all regional clusters that could have matching series
       (from cluster registry: [us-east, us-west, eu-central, ap-southeast])
    3. Fans out query to all 4 regional query frontends IN PARALLEL
    4. Each regional query frontend executes query locally (100-500ms)
    5. Global query frontend merges results:
       - For sum/count: add results from all regions
       - For avg: compute weighted average (sum/count from each region)
       - For max/min: take extreme value across regions
       - For percentiles: CANNOT meaningfully merge fixed-bucket histograms
         → Use DDSketch or native histograms for cross-cluster percentiles

  Latency breakdown:
    Regional query execution: 100-500ms (parallel)
    Cross-region network RTT: 50-200ms (depends on region pair)
    Result merge: 5-10ms
    Total: 200-800ms (dominated by slowest region + network)

  Optimization:
    Recording rules in each region pre-aggregate common cross-cluster queries
    Federated queries on pre-aggregated data: 50-100ms (minimal data transfer)
```
...
%%{init: {'theme': 'neutral'}}%%
flowchart LR
    subgraph Inputs["Growth Drivers"]
        direction TB
        SVC["Service Count<br/>+30%/year"]
        INST["Instrumentation<br/>Density +20%/year"]
        HIST["Native Histogram<br/>Adoption -15%/year<br/>(cardinality reduction)"]
        EBPF["eBPF Metrics<br/>+500 series/host"]
    end

    subgraph Model["Capacity Model"]
        direction TB
        SERIES["Active Series<br/>Projection"]
        INGEST["Ingestion Rate<br/>Projection"]
        STORE["Storage<br/>Projection"]
    end

    subgraph Capacity["Infrastructure"]
        direction TB
        ING_CAP["Ingester Nodes"]
        QUERY_CAP["Query Nodes"]
        STORE_CAP["Object Storage"]
    end

    SVC & INST & HIST & EBPF --> SERIES
    SERIES --> INGEST --> STORE
    STORE --> ING_CAP & QUERY_CAP & STORE_CAP

    classDef input fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef model fill:#fff3e0,stroke:#e65100,stroke-width:2px
    classDef cap fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px

    class SVC,INST,HIST,EBPF input
    class SERIES,INGEST,STORE model
    class ING_CAP,QUERY_CAP,STORE_CAP cap
```
...
|---|---|---|---|
| Ingester memory | 70% of limit | 85% of limit | Scale ingesters or enforce cardinality caps |
| Active series per ingester | 2M | 3M | Add ingester to hash ring |
| WAL disk usage | 70% of allocated | 85% of allocated | Increase checkpoint frequency or provision more disk |
| Query concurrency | 70% of slots used | 90% of slots used | Scale query nodes; begin shedding low-priority queries |
| Compaction lag | 2 hours | 6 hours | Scale compactor; investigate slow blocks |
| Object storage QPS | 70% of rate limit | 90% of rate limit | Request rate limit increase; optimize read patterns |
| Index memory | 70% of node RAM | 85% of node RAM | Shard index; enable tiered indexing |

### Chaos Engineering Experiments

| Experiment | Target | Validates | Success Criteria |
|---|---|---|---|
| **Kill one ingester** | Ingester node | Replication handles single-node failure; no data loss | Queries return complete data from replicas; new writes redistribute within 60s |
| **Saturate ingestion** | Distributor | Backpressure and rate limiting work correctly | Existing series continue; new series rejected cleanly; no OOM |
| **Inject 10M new series** | Cardinality enforcer | Per-tenant cardinality caps prevent explosion | Cap triggers at configured limit; existing metrics unaffected |
| **Kill object storage connectivity** | Storage layer | Circuit breaker activates; cache serves recent data | Dashboards show recent data from head block; historical queries gracefully degraded |
| **Spike query load 50x** | Query engine | Priority scheduling protects alert evaluations | Alert evaluations complete within 2x interval; dashboard queries shed gracefully |
| **Corrupt WAL segment** | WAL recovery | Ingester detects corruption and recovers from replicas | Corrupted segment skipped; data recovered from replica ingesters |

### Load Shedding Strategy

| Priority | Query Type | Shed Order | Action When Shed |
|---|---|---|---|
| **P0 (Never shed)** | Alert rule evaluations | Never | These define incident detection; shedding causes missed alerts |
| **P1 (Shed last)** | Recording rules | Only when P0 threatened | Skip evaluation cycle; dashboards use stale pre-aggregated data |
| **P2 (Shed early)** | Dashboard auto-refresh | When concurrency > 80% | Return cached stale data with staleness indicator |
| **P3 (Shed first)** | Ad-hoc API queries | When concurrency > 60% | Return 503 with Retry-After header; queue for later |

---

## Data Consistency During Failover

### Ingester Replication Consistency

```
WRITE CONSISTENCY DURING INGESTER FAILURE:

Normal operation (3 ingesters, replication_factor=3):
  Write accepted when 2/3 ingesters acknowledge (quorum)
  All 3 have the data within replication lag (typically <1s)

Single ingester failure:
  Writes continue to 2 healthy replicas (quorum still met)
  Failed ingester's WAL preserves pre-failure data
  On recovery: WAL replay reconstructs head block
  Gap between failure and recovery: covered by replicas

Double ingester failure:
  Quorum cannot be met for affected series
  Distributor queues writes with backpressure (30-second buffer)
  If buffer exhausted: reject writes with 503 (data loss for that window)
  Meta-monitoring alerts within 60 seconds
  Manual intervention: emergency ingester deployment

QUERY CONSISTENCY DURING FAILOVER:
  Query engine fetches from all available replicas
  Deduplication: samples with same (series_id, timestamp, value) merged
  Partial results acceptable during failover (flagged to user)
  Alert evaluations use configurable staleness tolerance (default: 5 minutes)
```

### Performance Engineering: Query Optimization Techniques

| Technique | How It Works | Impact |
|---|---|---|
| **Query deduplication** | Identical in-flight queries share a single TSDB execution; all waiting clients receive the same result | During incidents, 200 engineers opening the same dashboard result in ~50 unique queries (4x dedup ratio) instead of 2,000 |
| **Step alignment** | Query time ranges are snapped to step boundaries, creating identical cache keys for the same dashboard viewed by different users | Cache hit rate improves from 30% to 70%+ for shared dashboards |
| **Partial result caching** | Long time-range queries are split into day-aligned sub-queries; each sub-query cached independently | A 7-day query with 6 cached days only computes 1 day |
| **Lazy chunk loading** | Chunks are not decompressed until aggregation requires the values; if only count is needed, chunk metadata suffices | 30-50% reduction in CPU for count/sum queries |
| **Posting list intersection ordering** | Start intersection with the smallest posting list; skip intersection early when result set is empty | 10x speedup for highly selective queries (e.g., status="500" is smaller than method="GET") |
