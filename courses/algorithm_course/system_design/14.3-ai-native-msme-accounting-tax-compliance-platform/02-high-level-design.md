...
| **Filing orchestration** | Pre-computed returns with deadline-aware submission queue | On-demand return generation at filing time (simpler, but creates month-end Slowest part of the process); fully automated filing without human review (faster, but risky if data has errors) | Pre-computation distributes processing load throughout the month; deadline-aware queue ensures filings complete before cutoff; human review step before submission prevents filing errors; cost is maintaining continuously updated return state and handling amendment flows when transactions are modified after pre-computation |
...
|---|---|---|
| **IRP endpoint unavailable** | Cannot generate IRN for new e-invoices | Queue invoices with "pending e-invoice" status; retry with exponential backoff; failover to secondary IRP endpoints; invoice usable internally but not for dispatch to buyer |
| **Bank feed API disruption** | New transactions not ingested in real-time | Fall back to batch statement upload; display "last sync" timestamp; re-sync and deduplicate when API recovers |
| **Categorization ML service slow/down** | Transactions cannot be auto-classified | Queue transactions with "pending categorization" status; apply rule-based fallback for high-confidence patterns (exact counterparty match from history); alert user of uncategorized items |
| **Government GST portal congested** | Filing submissions timeout or fail | Priority queue with deadline-aware scheduling; pre-authenticated session pool; exponential backoff with jitter; transparent status tracking showing queue position |
| **Ledger database partial outage** | Cannot record journal entries | Read replicas serve read queries (balance lookups, reports); writes queue in WAL until primary recovers; hard stop on new transactions if WAL grows beyond safety threshold |
| **OCR extraction service overloaded** | Invoice image processing delayed | Queue with priority (user-initiated single uploads > batch imports); return immediately with "processing" status; webhook notification on completion |
| **Tax rule repository stale** | Incorrect tax computation on new transactions | Health check compares rule repository version against published gazette notifications; alert tax operations team; apply last-known-good rules with "rate verification pending" flag on affected entries |
| **Entity resolution service unavailable** | Reconciliation cannot resolve counterparty names | Skip entity-enhanced matching; fall back to amount-date-reference matching only; flag reconciliation as "partial — entity resolution unavailable" |

---

## ITC Reconciliation Data Flow

Input Tax Credit reconciliation is one of the platform's most complex cross-system flows, involving data from both internal ledger records and government-published GSTR-2B data.

```mermaid
flowchart TB
    subgraph External["Government Portal"]
        G2B["GSTR-2B\n(Inward Supply Data\nfrom Suppliers' GSTR-1)"]
    end

    subgraph Internal["Platform Data"]
        PL["Purchase Ledger\n(Business's Own\nRecorded Purchases)"]
    end

    subgraph Reconcile["ITC Reconciliation Engine"]
        FETCH["Fetch GSTR-2B\n(API / Scheduled Pull)"]
        PARSE["Parse & Normalize\nSupplier Invoices"]
        MATCH["Multi-Attribute\nMatching"]
        CLASSIFY["Mismatch\nClassifier"]
    end

    subgraph Results["Reconciliation Output"]
        OK["✓ Matched\n(ITC Claimable)"]
        AMT["△ Amount Mismatch\n(Supplier vs. Buyer Amount Differs)"]
        MISS["✗ Missing in GSTR-2B\n(Supplier Not Filed)"]
        DUP["⚠ Duplicate Invoice\n(Same Invoice in Multiple Periods)"]
        EXTRA["+ Extra in GSTR-2B\n(Not in Purchase Ledger)"]
    end

    subgraph Action["Recommended Actions"]
        A1["Claim Full ITC"]
        A2["Claim Lower Amount;\nFollow Up with Supplier"]
        A3["Hold ITC Claim;\nNotify Supplier to File"]
        A4["Flag for CA Review"]
    ...
    classDef result fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef action fill:#e0f7fa,stroke:#00695c,stroke-width:2px

    class G2B external
    class PL internal
    class FETCH,PARSE,MATCH,CLASSIFY engine
    class OK,AMT,MISS,DUP,EXTRA result
    class A1,A2,A3,A4,A5 action
```

### ITC Reconciliation Matching Rules

| Match Dimension | Matching Logic | Tolerance |
|---|---|---|
| **GSTIN** | Exact match between supplier GSTIN in GSTR-2B and purchase ledger | None — must be exact 15-digit match |
| **Invoice Number** | Normalized match (strip spaces, convert to uppercase, remove special characters) | Fuzzy — Levenshtein distance ≤ 2 |
| **Invoice Date** | Date match within settlement window | ±3 days (accounting for data entry lag) |
| **Taxable Amount** | Amount match within rounding tolerance | ±₹1 or ±0.5%, whichever is greater |
| **Tax Amount** | Computed from taxable amount × applicable rate; verified against both sides | ±₹1 per tax component (CGST, SGST, IGST) |
| **Place of Supply** | State code match for determining IGST vs CGST+SGST | Must be exact — determines tax type |

---

## Integration Points

| Integration | Protocol | Direction | Purpose | Reliability Concern |
|---|---|---|---|---|
| **Open Banking APIs** | REST / OAuth 2.0 | Inbound | Real-time bank transaction feeds | Bank API uptime varies (95-99.5%); must handle token refresh failures |
| **Invoice Registration Portal (IRP)** | REST + Digital Signature | Outbound | E-invoice IRN generation | Government system; degraded during peak filing; 3-5 second response times |
| **GST Portal (GSTN)** | REST + DSC/EVC | Bidirectional | GSTR filing, GSTR-2B fetch, authentication | Severe congestion during deadline windows; 10-minute session timeout |
| **Payment Gateways** | Webhooks + REST | Inbound | Payment confirmation, settlement reports | Multiple providers with different webhook reliability; must handle duplicates |
| **UPI Settlement** | Batch File / API | Inbound | UPI transaction data for reconciliation | Settlement files arrive in T+1; real-time feed via API has variable latency |
| **TDS/TCS Portal** | REST | Outbound | TDS return filing, certificate download | Separate authentication from GST portal; different deadline calendar |
| **Accounting Standards DB** | Internal API | Internal | Ind AS / IFRS disclosure requirements, reporting templates | Updated with each standards revision; backward-compatible versioning |
| **CA Collaboration** | WebSocket + REST | Bidirectional | Real-time collaboration, journal approval, filing delegation | Must support concurrent edits with conflict resolution |

---

## Filing Orchestration Architecture

The filing orchestrator manages the most complex external interaction pattern in the platform—submitting GST returns to a government portal with strict deadlines, limited throughput, and unpredictable reliability.

```mermaid
flowchart TB
    subgraph Prep["Pre-Filing (Continuous)"]
        PC["Pre-Compute\nReturn Data"]
        VAL["Schema\nValidation"]
        ITC["ITC\nReconciliation"]
    end

    subgraph Queue["Filing Queue"]
        PQ["Priority Queue\n(deadline-aware)"]
        SPOOL["Session\nPool Manager"]
        RATE["Rate\nLimiter"]
    end

    subgraph Submit["Submission Engine"]
        AUTH["Authenticate\n(OTP/DSC)"]
        UPLOAD["Upload\nReturn JSON"]
        SIGN["Digital\nSignature"]
        CONFIRM["Submit &\nGet ACK"]
    ...
    PQ --> SPOOL --> AUTH
    RATE --> AUTH
    AUTH --> UPLOAD --> SIGN --> CONFIRM

    CONFIRM -->|"Success"| STATUS --> NOTIFY
    CONFIRM -->|"Failure / Timeout"| RETRY --> PQ
    ...
    classDef monitor fill:#e0f7fa,stroke:#00695c,stroke-width:2px

    class PC,VAL,ITC prep
    class PQ,SPOOL,RATE queue
    class AUTH,UPLOAD,SIGN,CONFIRM submit
    class STATUS,NOTIFY,RETRY monitor
```

### Filing Priority Calculation

```
Priority = BaseScore × DeadlineUrgency × BusinessTier × RetryPenalty

Where:
  BaseScore = 1.0 (standard filing)
  DeadlineUrgency = 1 / (hours_to_deadline)²
    - 48h before: priority = 1/2304 ≈ 0.0004
    - 6h before:  priority = 1/36 ≈ 0.028
    - 2h before:  priority = 1/4 = 0.25
    - 30min before: priority = 1/0.25 = 4.0
  BusinessTier = {Premium: 1.5, Standard: 1.0, Trial: 0.5}
  RetryPenalty = 0.8^(retry_count)  // Reduce priority on repeated failures to avoid starvation
```
