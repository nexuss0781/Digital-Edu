...
The ledger service is the most critical scaling challenge because it must maintain the double-entry Rule that never changes while handling 300M+ journal entries per day across 5M businesses. A single database instance cannot sustain this write throughput.
...
3. The per-business causal ordering Rule that never changes is maintained by a single shard's write ordering
...
2. **Session pool management:** The government portal requires authentication before filing. Session tokens expire in 10 minutes. The platform maintains a rotating pool of authenticated sessions (100+ concurrent sessions), pre-authenticating new sessions as old ones expire. This eliminates the authentication Slowest part of the process during filing.
...
| Service | Trigger Metric | Scale-Up Threshold | Scale-Down Threshold | Min/Max Instances | Cooldown |
|---|---|---|---|---|---|
| **Transaction Ingestion Gateway** | Ingestion queue depth | >10,000 messages | <1,000 messages | 10 / 200 | 3 min |
| **Categorization ML Workers** | Pending categorization count | >5,000 transactions | <500 transactions | 20 / 500 | 5 min |
| **OCR Extraction Workers** | OCR queue depth | >1,000 images | <100 images | 10 / 300 (GPU) | 5 min |
| **Reconciliation Workers** | Reconciliation queue depth | >500 jobs | <50 jobs | 5 / 100 | 5 min |
| **Filing Submission Workers** | Filing queue depth + deadline proximity | >100 filings AND deadline <24h | <10 filings | 5 / 200 | 2 min |
| **Ledger Write Service** | Write latency p99 | >150ms | <50ms | 20 / 100 | 3 min |
| **Tax Computation Service** | Computation latency p99 | >40ms | <15ms | 10 / 80 | 3 min |
| **Report Generation Workers** | Report queue depth | >200 reports | <20 reports | 5 / 60 | 5 min |

### Predictive Scaling for Filing Deadline Surges

```
FUNCTION PredictFilingDeadlineSurge():
    // GST filing calendar is deterministic — surge windows are known in advance
    deadlines = [
        { "type": "GSTR-1", "day": 11, "frequency": "monthly" },
        { "type": "GSTR-3B", "day": 20, "frequency": "monthly" },
        { "type": "GSTR-9", "month": 12, "day": 31, "frequency": "annual" }
    ]

    FOR EACH deadline IN deadlines:
        hoursToDeadline = ComputeHoursToNextDeadline(deadline)

        IF hoursToDeadline <= 72:
            // Pre-scale filing workers 72 hours before deadline
            targetScale = BaselineWorkers × (72 / hoursToDeadline)^1.5
            targetScale = MIN(targetScale, MAX_FILING_WORKERS)

            // Also pre-scale government portal session pool
            targetSessions = BaselineSessions × (72 / hoursToDeadline)^1.2

            // Pre-authenticate sessions to avoid auth Slowest part of the process during surge
            IF hoursToDeadline <= 24:
                PreAuthenticateSessionPool(targetSessions)

            ScaleFilingWorkers(targetScale)
            ScaleSessionPool(targetSessions)

    // Also scale based on historical pattern for this specific deadline
    historicalPeak = GetHistoricalFilingSurge(currentDeadline, previousMonths=6)
    EnsureCapacity(historicalPeak × 1.3)  // 30% buffer above historical peak
```
...
| Data Type | Hot (Online DB) | Warm (Compressed, Queryable) | Cold (Archived) | Deletion Eligible |
|---|---|---|---|---|
| **Journal entries** | Current FY + 1 year | 1-3 years | 3-8 years | After 8 years (statutory) |
| **Transaction records** | Current FY | 1-3 years | 3-8 years | After 8 years |
| **Invoice structured data** | Current FY + 1 year | 1-3 years | 3-8 years | After 8 years |
| **Invoice images (OCR)** | 90 days | 90 days - 1 year | 1-8 years (reduced resolution) | After 8 years |
| **Audit trail** | Current FY + 1 year | 1-3 years | 3-8 years (immutable archive) | After 8 years (per-business chain deletion) |
| **Tax rule versions** | Current + 2 prior versions | All versions (for audit replay) | N/A | Never (needed for retroactive computation) |
| **Categorization model state** | Current per-business priors | Monthly snapshots | Annual snapshots | After business account closure + 3 years |
| **Filing status and acknowledgments** | Current FY + 1 year | 1-5 years | 5-8 years | After 8 years |
| **Reconciliation history** | 6 months | 6 months - 2 years | 2-8 years | After 8 years |

### Archival Implementation

```mermaid
flowchart LR
    subgraph Hot["Hot Tier (Online)"]
        DB[(Sharded\nLedger DB)]
    end

    subgraph Warm["Warm Tier (Compressed)"]
        PQ[(Columnar\nParquet Store)]
    end
    ...
    subgraph Mgmt["Lifecycle Manager"]
        AGE["Age-Based\nPolicy Engine"]
        COMP["Compression\nWorker"]
        VER["Integrity\nVerifier"]
    end

    DB -->|"FY close + 1 year"| AGE
    AGE --> COMP
    COMP --> PQ
    PQ -->|"3+ years"| OBJ
    VER --> DB
    ...
    classDef hot fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef warm fill:#fffde7,stroke:#f57f17,stroke-width:2px
    classDef cold fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    classDef mgmt fill:#e0f7fa,stroke:#00695c,stroke-width:2px

    class DB hot
    class PQ warm
    class OBJ cold
    class AGE,COMP,VER mgmt
```
...
### Filing Submission Reliability

The filing orchestrator implements a multi-layer reliability strategy to maximize on-time filing rates:

| Layer | Mechanism | Failure Handled |
|---|---|---|
| **Pre-validation** | Validate all return data against government schema 7 days before deadline | Catch data errors early when there's time to fix |
| **Session pool** | Maintain pool of pre-authenticated government portal sessions; refresh before expiry | Avoid authentication Slowest part of the process during peak hours |
| **Retry with jitter** | Exponential backoff (2s, 4s, 8s, 16s, 32s) with ±25% jitter | Government portal transient errors and rate limiting |
| **Circuit breaker** | Trip after 5 consecutive failures; half-open check every 60s | Government portal extended outage |
| **Priority queue** | Dynamic priority based on deadline proximity: priority = 1 / (hours_to_deadline)² | Prevent priority inversion when portal bandwidth is limited |
| **Idempotency** | Every filing submission carries a unique idempotency key; server-side dedup at government portal | Duplicate submission after timeout (submission may have succeeded) |
| **Status webhook** | Register webhook for async filing status updates from government portal | Handle cases where portal accepts filing but delays acknowledgment |
| **Manual escalation** | Alert business owner with portal link and pre-filled data if automated filing cannot complete within 4 hours of deadline | Last-resort manual filing when automation fails |

---

## Ledger Database Sharding Strategy

### Shard Key Selection: Business ID

| Candidate Shard Key | Pros | Cons | Decision |
|---|---|---|---|
| **business_id** | All financial data for one business on same shard; transactions are never cross-shard; reconciliation runs locally | Uneven shard sizes (large businesses have 100x more data than micro) | **Selected** — business-local access pattern dominates |
| **account_id** | Even distribution; trial balance computation per account is local | Journal entries span multiple accounts → cross-shard writes; violates double-entry atomicity | Rejected — breaks atomic journal entry writes |
| **time-based (month)** | Even temporal distribution; old months easily archived | Current month's data for a business spread across shards; reporting requires fan-out | Rejected — reporting access pattern is per-business |

### Hot Shard Mitigation

Large businesses (1% of accounts generating 20% of transaction volume) create hot shards. Mitigations:

| Strategy | Trigger | Action |
|---|---|---|
| **Shard splitting** | Shard exceeds 80% of disk/IOPS capacity | Split by business_id range; redistribute businesses to new shard |
| **Dedicated shard** | Single business exceeds 5% of shard capacity | Move business to dedicated shard with independent scaling |
| **Read replica routing** | Read QPS on shard exceeds threshold | Route report queries to read replicas; writes stay on primary |
