...
| Data Category | Retention Period | Regulatory Driver | Storage Tier |
|---|---|---|---|
| **Journal entries** | 8 years | Income Tax Act (India); Companies Act | Hot current FY → warm 3 years → cold 8 years |
| **Tax returns and acknowledgments** | 8 years | GST Act; Income Tax Act; statute of limitations for reassessment | Hot 1 year → warm 3 years → cold archive |
| **Invoice records (structured)** | 8 years | GST e-invoicing mandate; ITC verification; audit trail | Hot current FY → warm 3 years → cold archive |
| **Invoice images (OCR source)** | 8 years (reduced resolution after 1 year) | Evidentiary requirement for tax audit; original document preservation | Hot 90 days → warm 1 year (full res) → cold 8 years (compressed) |
| **Audit trail (Merkle chain)** | 8 years per business | Statutory audit requirements; digital evidence standards | Per-business chain; hot 1 year → cold immutable archive |
| **Bank transaction records** | 8 years | Financial audit; reconciliation dispute resolution | Hot current FY → warm 2 years → cold archive |
| **User PII (business owner, CA)** | Duration of account + 3 years | DPDP Act (India); GDPR (EU businesses) | Encrypted at rest; anonymized after retention; crypto-shredding on deletion |
| **Categorization model state** | Duration of account + 1 year | ML explainability for audit; model reproducibility | Per-business priors archived with account data |
| **Tax rule versions** | Indefinite | Retroactive computation for audit; bi-temporal queries | Versioned repository; never deleted |
| **Filing status logs** | 8 years | Compliance evidence; penalty dispute resolution | Warm 2 years → cold archive |

### Traffic Patterns

```
Monthly Filing Cycle (India GST):

Day:  01  03  05  07  09  11  13  15  17  19  20  22  24  26  28  30
      |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |
TXN:  ██████████████████████████████████████████████████████████████  (steady)
CAT:  ██████████████████████████████████████████████████████████████  (follows TXN)
RECON:██  ██                      ██                              ████  (month start/end)
FILE: ██          ████████████          ████████████                    (deadline spikes)
IRP:  ██████████████████████████████████████████████████████████████  (steady)

Notes:
- Transaction ingestion: steady throughout month; slight morning peak (9-11 AM)
- Categorization: follows ingestion with 1-2 second delay
- Bank reconciliation: concentrated at month start (previous month) and month end
- GSTR-1 filing surge: days 8-11 (3-day window with 70% in last 48h)
- GSTR-3B filing surge: days 17-20 (3-day window with 70% in last 48h)
- E-invoicing (IRP): steady throughout month; proportional to invoice creation
- Year-end (March/December): all patterns amplified 3-5x
- Quarter-end (June 30, Sept 30): advance tax computation spikes for all businesses
```
...
| Tier | Businesses | Database | Compute | Use Case |
|---|---|---|---|---|
| **Enterprise** | Manufacturing MSMEs with multi-state GST registrations and high transaction volume | Dedicated shard with independent scaling | Dedicated OCR/ML workers; priority filing queue | >₹50 crore turnover; complex compliance needs |
| **Business** | Growing MSMEs with moderate transaction volume | Shared shards with row-level security; read replicas for reporting | Shared compute with guaranteed resource quotas | ₹5-50 crore turnover; standard compliance |
| **Starter** | Micro businesses, freelancers, small shops | Shared infrastructure; densely packed tenants | Shared compute; best-effort | <₹5 crore turnover; basic compliance |
