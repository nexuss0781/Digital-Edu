...
**Double-entry Rule that never changes enforcement:**
...
## Government Portal Security Integration

### Digital Signature and Authentication

| Government Service | Authentication Method | Key Management Concern |
|---|---|---|
| **Invoice Registration Portal (IRP)** | API key + digital signature (DSC Class 2/3) | Digital signature certificates must be stored in HSM; delegated signing requires CA authorization with scoped permissions and expiry |
| **GST Portal (GSTN)** | GSTIN + password + OTP (EVC) or DSC | Platform stores encrypted credentials with per-business encryption keys; session tokens expire in 10 minutes during peak hours |
| **TDS Portal** | TAN + password + DSC | Separate authentication chain from GST; different credential lifecycle management |
| **Income Tax Portal** | PAN + password + Aadhaar OTP | Requires Aadhaar-linked verification for certain operations; sensitive biometric data never stored |

### Government API Security Hardening

| Measure | Implementation |
|---|---|
| **Credential isolation** | Each business's government portal credentials encrypted with a business-specific key derived from a master key in HSM; platform operators cannot access credentials |
| **Session management** | Government portal session tokens stored in ephemeral memory only (never persisted); rotated at 50% of expiry window; old sessions invalidated before new session created |
| **Request signing** | All IRP submissions digitally signed using business's DSC; signature verification receipt stored as non-repudiation evidence |
| **Audit logging** | Every government portal interaction logged: API endpoint, request hash, response status, timestamp; no request/response bodies logged (contain financial data) |
| **Delegation controls** | CA filing on behalf of business requires explicit delegation token with: scope (which returns can be filed), expiry (maximum 30 days), and revocability (business can revoke at any time) |

---

## Compliance Monitoring and Alerting

| Compliance Area | Monitoring Signal | Alert Threshold | Escalation |
|---|---|---|---|
| **Ledger integrity** | Hash chain verification per business | Any broken chain → immediate alert | Operations + statutory auditor notification |
| **Filing deadline adherence** | Filings remaining vs. hours to deadline | >1,000 filings AND <6 hours to deadline | Auto-scale filing workers; notify businesses at risk |
| **ITC claim accuracy** | Mismatch rate between claimed ITC and GSTR-2B | >5% mismatch rate per business | Flag for CA review before filing |
| **E-invoicing compliance** | Invoices dispatched without IRN | Any invoice dispatched without IRN (post-mandate threshold) | Block dispatch; alert business owner |
| **Data retention compliance** | Records approaching retention expiry | 30 days before scheduled deletion | Generate deletion audit report; queue for automated deletion |
| **Access control violations** | Unauthorized access attempts to financial data | Any cross-tenant data access attempt | Immediate session termination; security incident investigation |
| **Categorization model integrity** | Anomalous correction patterns (mass reclassification, unusual category shifts) | >50 corrections in 1 hour OR category shift affecting >10% of historical entries | Freeze corrections; require CA approval for batch |
| **OCR data exfiltration** | Unusual volume of document downloads or OCR result exports | >100 document downloads in 1 hour by single user | Rate limit; require re-authentication; audit log |

---

## Financial Data Encryption Architecture

### Per-Business Encryption Key Hierarchy

```
Master Key (HSM-stored, never exported)
  └── Tenant Key Encryption Key (per-business, derived from master + business_id)
       ├── Ledger Data Key (encrypts journal entries, balances)
       ├── Document Key (encrypts OCR images, invoice PDFs)
       ├── PII Key (encrypts PAN, bank account numbers, contact info)
       └── Credential Key (encrypts government portal credentials)

Key rotation: Tenant keys rotated annually; data re-encrypted with new key
              during scheduled maintenance window (overnight, non-filing period)

Crypto-shredding: When business account is closed and retention expires,
                  deleting the tenant key makes all business data unrecoverable
                  without touching individual records — O(1) deletion
```

### Field-Level Encryption Scope

| Field Type | Encryption | Searchable While Encrypted | Rationale |
|---|---|---|---|
| **PAN (tax ID)** | AES-256-GCM per-tenant key | Yes (deterministic encryption for exact match) | Regulatory requirement; needed for TDS lookup |
| **Bank account number** | AES-256-GCM per-tenant key | No (encrypted at rest, decrypted only for display) | Financial data protection |
| **Financial amounts** | Plaintext (for computation) | Yes (needed for aggregation queries) | Trade-off: encrypted amounts prevent SQL aggregation |
| **GSTIN** | Plaintext | Yes (needed for ITC reconciliation joins) | Not PII; publicly registered identifier |
| **Counterparty names** | TDE (transparent data encryption) | Yes (needed for entity resolution) | Business operational data; not classified as PII under DPDP Act |
| **Government portal credentials** | AES-256-GCM with business-specific key | Never (decrypted only at submission time in memory) | Highest sensitivity; breach would enable unauthorized filing |
