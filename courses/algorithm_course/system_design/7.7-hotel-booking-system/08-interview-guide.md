...
### Variant 1: Single Hotel Reservation System (Simplified)

- **Skip:** Multi-channel, search index, overbooking, channel manager
- **Focus:** Availability calendar, booking flow, payment, cancellation
- **Good for:** Junior/mid-level, 30-minute format

### Variant 2: OTA Platform (Full Scope)

- **Include:** All features from this guide
- **Focus:** Availability race conditions, overbooking, channel sync, search architecture
- **Good for:** Senior/staff-level, 45-minute format

### Variant 3: Revenue Management Platform (Yield Focus)

- **Emphasize:** Dynamic pricing, competitive rate intelligence, demand forecasting, overbooking optimization
- **Focus:** Rate engine complexity, yield management curves, A/B testing pricing strategies
- **Good for:** ML + systems hybrid interviews

### Variant 4: Channel Distribution Platform (Integration Focus)

- **Emphasize:** Multi-channel sync, rate parity enforcement, inbound booking relay, channel API heterogeneity
- **Focus:** Event-driven architecture, circuit breakers, reconciliation, API translation layer
- **Good for:** Integration/platform-focused interviews
...
|------|------------|----------|-------------|
| **Availability Model** | Calendar matrix with atomic multi-date transactions; SERIALIZABLE isolation; sharding by property_id | Date-based availability with transactions | Simple counter per room; no multi-date atomicity |
| **Concurrency** | Atomic conditional UPDATE with WHERE guard; explains why distributed locks are unnecessary | Mentions locking; some concurrency awareness | No race condition handling; assumes sequential processing |
| **Overbooking** | Statistical model based on no-show rates; configurable per property; walk policies with cost analysis | Mentions overbooking as possible; basic approach | "Prevent all overbooking" or no discussion |
| **Search Architecture** | Two-phase (index discovery + availability verification); bloom filter; explains separation of concerns | Search with availability check; some optimization | Single-phase search; no caching strategy |
| **Channel Sync** | Event-driven push < 5s; per-channel circuit breakers; reconciliation job | Mentions channel sync; some latency awareness | Polling-based sync or no channel discussion |

---

## Cross-System Comparison Questions

### "How does hotel booking differ from flight booking?"

**Answer:** Three fundamental differences:
1. **Inventory authority**: Hotel platform IS the inventory authority; flight booking delegates to GDS. This means hotel platforms must solve concurrency directly.
2. **Inventory structure**: Flight = single seat on single flight (one counter). Hotel = room across N nights (multi-row atomic update across date dimension).
3. **Overbooking ownership**: Airlines manage their own overbooking models. Hotel platforms must implement overbooking on behalf of properties, with configurable risk thresholds per property.

### "What patterns from hotel booking apply to workspace/desk booking?"

**Answer:** Strong overlap in:
- **Calendar matrix**: Same property × resource × date model; desk availability per day
- **Hold mechanism**: Hold desk during booking flow; auto-release on timeout
- **Fragmentation**: Same gap problem (1-day gaps between bookings become unbookable)
- **New challenge**: Recurring bookings (every Tuesday for 3 months) are common for desks but rare for hotels
