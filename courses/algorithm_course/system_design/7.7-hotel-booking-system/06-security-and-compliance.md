...
|-----------|-------------|-------------|---------------|
| **PCI-DSS v4.0** | Global | Protect cardholder data; annual compliance assessment | Client-side tokenization; isolated payment network segment; quarterly scans |
| **GDPR** | EU | Right to erasure, data portability, consent management | PII encryption; anonymization pipeline; consent records; data export API |
| **CCPA** | California | Consumer data access/deletion; no sale of personal data | Data access API; deletion workflow; "Do Not Sell" opt-out |
| **PSD2/SCA** | EU | Strong Customer Authentication for payments > €30 | 3D Secure integration; exemptions for low-risk transactions |
| **Consumer Rights Directive** | EU | 14-day cooling-off period does NOT apply to accommodation bookings | System enforces property-specific cancellation policies, not blanket cooling-off |
| **Accessibility (ADA/EAA)** | US/EU | Accessible booking experience; property accessibility information | Accessible UI; structured accessibility amenity data per property |
| **Local Tourism Tax** | City-level | Collect and remit city/tourist taxes | Tax calculation engine; per-city tax rate database; tax line item on receipts |
...
flowchart TB
    ...
    subgraph DMZ["DMZ"]
        GW["API Gateway<br/>WAF + Rate Limiting"]
        CDN["CDN<br/>(Photos + Static)"]
    end

    subgraph App["Application Zone"]
        SVC["Core Services<br/>(Search, Booking, etc.)"]
        Q["Event Bus"]
    end

    subgraph Sensitive["Sensitive Zone"]
        PAY["Payment Service<br/>(PCI Scope)"]
        DB[("Databases")]
        VAULT["Key Management<br/>+ Credential Vault"]
    end

    subgraph Channel["Channel Zone"]
    ...
    classDef channel fill:#e0f7fa,stroke:#00695c,stroke-width:2px

    class WEB,MOB public
    class GW,CDN dmz
    class SVC,Q app
    class PAY,DB,VAULT sensitive
    class CHAN,OTA channel
```
