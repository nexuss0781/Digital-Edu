...
## 8. Availability Verification Pipeline

### Two-Phase Search: Discovery Then Verification

The search-to-availability pipeline is the most resource-intensive operation in the system. At 5,800 searches/sec peak, each requiring availability checks across potentially thousands of properties, naive implementation would overwhelm the availability service.

```
Search Pipeline Optimization:

Phase 1 — Candidate Discovery (Search Index, ~120ms):
  Input: Destination + filters (stars, amenities, price range, property type)
  Operation: Geo-spatial + faceted search on pre-indexed property data
  Output: 2,000-5,000 candidate property IDs
  Cost: Single search index query; no availability data accessed

Phase 2 — Sold-Out Elimination (Bloom Filter, ~2ms):
  Input: Candidate property IDs
  Operation: Check each against bloom filter of sold-out properties
  Output: Eliminate ~30% of candidates (properties with zero inventory for ANY date)
  Cost: In-memory bloom filter; O(k) per property where k = hash functions
  Rebuild frequency: Every 5 minutes from availability service

Phase 3 — Availability Verification (Sharded Service, ~450ms):
  Input: Filtered candidate property IDs (1,400-3,500)
  Operation: For each candidate, check room_date_inventory for ALL requested dates
  Output: Properties with at least one room type available for entire date range
  Cost: Most expensive step; batch queries to sharded availability DB

Phase 4 — Rate Computation + Ranking (~180ms):
  Input: Available properties
  Operation: Compute best rate per property, apply ranking algorithm
  Output: Sorted, paginated results with price and availability details
  Cost: Cache-heavy (70% hit rate on computed rates)

Total pipeline: ~750ms p50, ~1.8s p99
```

### Batch Availability Check Optimization

```
FUNCTION batchCheckAvailability(property_ids, room_type_filter, dates):
    // Group properties by shard for locality
    shard_groups = groupByShard(property_ids)

    results = PARALLEL_MAP(shard_groups, FUNCTION(shard, shard_properties):
        // Single batch query per shard instead of N individual queries
        available = SELECT property_id, room_type_id,
                           MIN(sellable_count) as min_available
                    FROM room_date_inventory
                    WHERE property_id IN shard_properties
                      AND date IN dates
                    GROUP BY property_id, room_type_id
                    HAVING COUNT(*) = LENGTH(dates)     // must have ALL dates
                       AND MIN(sellable_count) > 0      // all dates available
...
```
FUNCTION rebuildSoldOutBloomFilter():
    // Runs every 5 minutes
    // A property is "sold out" if ANY room type has zero availability for ANY date
    // in common search ranges (next 90 days)

    sold_out_properties = SELECT DISTINCT property_id
                          FROM room_date_inventory
                          WHERE date BETWEEN TODAY AND TODAY + 90
                          GROUP BY property_id
                          HAVING MIN(total_inventory + overbooking_limit
                                     - booked_count - held_count) <= 0

    bloom = new BloomFilter(
        expected_elements = sold_out_properties.count,
        false_positive_rate = 0.01   // 1% false positive is acceptable
    )

    FOR property_id IN sold_out_properties:
        bloom.add(property_id)

    // Atomic swap: replace old bloom filter with new one
    CACHE.SET("bloom:sold_out", bloom.serialize(), TTL = 10 minutes)

    // False positives: a property marked "sold out" that isn't →
    // user doesn't see it in results (minor revenue loss, rare)
    // False negatives: impossible (bloom filter property)
```

---

## 9. Concurrency Patterns Under Peak Load

### Hot Property Contention

During peak season, a popular property (e.g., a beachfront resort on New Year's Eve) can receive hundreds of concurrent booking attempts. Even with property-based sharding, all these requests hit the same shard, the same rows.

```
Contention mitigation for hot properties:

1. Connection pooling with queuing:
   - Each shard has a connection pool (max 50 connections)
   - Under contention, requests queue at the connection pool (not at the DB)
   - Queue timeout: 2 seconds → return TEMPORARILY_UNAVAILABLE
   - Client retries with jitter

2. Optimistic concurrency with version column:
   - Each row has a version column
   - UPDATE ... WHERE version = expected_version
   - On version mismatch: retry (up to 3 times with jitter)

3. Serialization failure handling:
   - SERIALIZABLE transactions may throw serialization failures
   - These are expected under contention, not bugs
   - Retry with exponential backoff: 50ms, 100ms, 200ms
   - After 3 failures: return SOLD_OUT (assume contention exhausted inventory)

4. Hot shard isolation:
   - If a single property generates > 20% of a shard's writes:
     Move that property to a dedicated shard
   - Prevents one hot property from degrading service for ~62,000 other properties
```

### Deadlock Prevention

```
Multi-date bookings can create deadlocks if dates are locked in different orders:

  Transaction A: Lock Dec 20, Lock Dec 21 (waiting for Dec 22)
  Transaction B: Lock Dec 22, Lock Dec 21 (waiting for Dec 20)
  → Deadlock

Prevention: ALWAYS lock dates in chronological order.

FUNCTION holdInventoryOrdered(property_id, room_type_id, dates):
    sorted_dates = SORT(dates, ascending)   // Always lock in date order
    BEGIN TRANSACTION:
        FOR date IN sorted_dates:
            UPDATE room_date_inventory
            SET held_count = held_count + 1
            WHERE ... AND sellable > 0
            // Row lock acquired in date order → no deadlock
    COMMIT
```
