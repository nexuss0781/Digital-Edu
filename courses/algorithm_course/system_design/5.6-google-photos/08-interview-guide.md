...
### "How would you add Ask Photos (AI Q&A)?"

```
Architecture layers on top of existing search:

1. Intent Layer: Gemini-based agent classifies query type
   - Factual ("What restaurant did we visit?") → needs visual analysis
   - Retrieval ("Show me beach photos") → routes to existing search
   - Counting ("How many times did we visit NYC?") → aggregation query

2. Retrieval Layer: Agent selects tools from existing infrastructure
   - Face index, label index, vector index, temporal/geo filters
   - Retrieves candidate set (50-100 photos)

3. Analysis Layer: Gemini multimodal with long context
   - Processes retrieved photos visually (reads signs, menus, etc.)
   - Cross-references with metadata (location, time, people)
   - Generates natural language answer with photo evidence

4. Key trade-off: Latency vs accuracy
   - Fast path (< 2s): Route to existing search for simple queries
   - Slow path (3-8s): Full Gemini RAG for complex questions
   - Cache answers for repeated question patterns
```

### "How would you handle a migration from unlimited to paid storage?"

```
Critical business + technical challenge (Google did this in June 2021):

1. Communication: 6-month notice period before enforcement
2. Grandfathering: Photos uploaded before cutoff date don't count
3. Storage accounting service: Track per-photo storage usage
4. Quota enforcement: Soft limit (warnings) → hard limit (block uploads)
5. Compression incentive: Offer free Storage Saver conversion
6. Technical: Background job to recalculate all users' storage
   - 1.5B users × ~6K photos = 9T items to audit
   - Batch job over ~2 weeks, throttled to avoid overloading Spanner
```
...
| "Design a Gemini-powered RAG system" | Ask Photos architecture maps directly to RAG patterns |
| "Design a content moderation system" | CSAM detection, content safety pipeline |
