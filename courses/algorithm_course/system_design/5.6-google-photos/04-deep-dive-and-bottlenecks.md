...
HAC (Hierarchical Agglomerative Clustering):
    ...
    2. Compute pairwise distances using ANN (not Brute Force (Checking every single possibility))
...
│   ├── Clustering into tree-like structures for fast query-time Cutting off unnecessary steps
...
### Slowest part of the process 1: ML Processing Throughput
...
### Slowest part of the process 2: Thumbnail Serving at Scale
...
### Slowest part of the process 3: Storage Cost at Exabyte Scale
...
## Critical Component 4: Memories & Auto-Curation Engine

### Why Is This Critical?

Memories is Google Photos' primary engagement driver — it proactively surfaces past photos to users without requiring search. The system must:
- Select high-quality, emotionally positive photos from potentially millions of candidates
- Avoid sensitive content (breakups, losses, traumatic events)
- Balance novelty (unseen photos) with nostalgia (recognizable moments)
- Generate engaging presentations (slideshows, collages, cinematic animations)
...
---
flowchart TB
    subgraph Triggers["Memory Generation Triggers"]
        DAILY[Daily Batch Job<br/>Run per user at 2 AM local]
        TEMPORAL[Temporal Trigger<br/>"X years ago today"]
        EVENT[Event Detection<br/>"Best of Trip"]
    end

    subgraph Candidate["Candidate Selection"]
        QUERY[Query Photos<br/>by trigger criteria]
        QUALITY[Quality Scoring<br/>NIMA / MUSIQ]
        FACE_Q[Face Quality<br/>eyes open, smiling]
        DEDUP_M[Diversity Filter<br/>avoid near-duplicates]
    end

    subgraph Scoring["Multi-Factor Scoring"]
        AESTHETIC[Aesthetic Score<br/>composition, lighting]
        EMOTIONAL[Emotional Valence<br/>positive sentiment]
        DIVERSITY[Diversity Score<br/>people, locations, scenes]
        RECENCY[Recency Bias<br/>recently unseen preferred]
    end

    subgraph Presentation["Presentation Generation"]
        SELECT[Select Top 10-20<br/>photos for Memory]
        LAYOUT[Layout Engine<br/>slideshow, collage, video]
        MUSIC[Music Matching<br/>mood-based selection]
        CAPTION[Caption Generation<br/>Gemini-powered]
    end

    Triggers --> Candidate --> Scoring --> Presentation
    ...
    classDef present fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px

    class DAILY,TEMPORAL,EVENT trigger
    class QUERY,QUALITY,FACE_Q,DEDUP_M candidate
    class AESTHETIC,EMOTIONAL,DIVERSITY,RECENCY scoring
    class SELECT,LAYOUT,MUSIC,CAPTION present
```
...
- Trained on large-scale image aesthetics dataset
- Evaluates composition, exposure, color harmony, sharpness
- Threshold for Memories inclusion: score > 6.5

**MUSIQ (Multi-Scale Image Quality Transformer):**
- Resolution-agnostic quality prediction (handles any input size)
- Uses multi-scale feature extraction with transformer attention
- Better than NIMA for mobile photos with varying resolutions
- Used as secondary signal alongside NIMA for ensemble scoring

#### Sensitive Content Filtering

```
FUNCTION filterSensitiveContent(candidates):
    filtered = []
    FOR EACH photo IN candidates:
        // Skip photos user has hidden from Memories
        IF photo.id IN user.hiddenFromMemories:
            CONTINUE

        // Skip photos with certain ML labels
        sensitiveLabels = ["hospital", "cemetery", "crash", "weapon"]
        IF photo.labels INTERSECT sensitiveLabels:
            CONTINUE

        // Skip photos during user-specified hidden date ranges
        IF photo.captureTime WITHIN user.hiddenDateRanges:
            CONTINUE

        // Skip photos of people user has hidden
        photoFaces = getFaceClusters(photo)
        IF photoFaces INTERSECT user.hiddenPeople:
            CONTINUE
...
| Failure Mode | Impact | Mitigation |
|-------------|--------|------------|
| Surfacing a painful memory | Emotional distress for user | Sensitive content filters; allow user to hide people, dates, and topics |
| Too-similar photos in Memory | Boring, repetitive experience | Perceptual hash-based near-duplicate removal; diversity scoring |
| Low-quality photos selected | Memory feels unpolished | NIMA/MUSIQ quality threshold; face quality checks (eyes open, in focus) |
| Memory generation too slow | Not ready by morning | Start batch at 2 AM local; 4-hour SLO; pre-compute candidate sets |
| Missing faces in Memory | Key people absent | Ensure face cluster diversity; include photos of people user frequently tags |

---

## Slowest part of the process 4: Generative AI Inference Cost

**Problem:** Features like Magic Eraser, Best Take, Reimagine, and Ask Photos require generative model inference (diffusion models, Gemini multimodal), which is 10-100x more expensive than discriminative models (classification, detection).

**Impact:** At 210 million monthly editors and growing Ask Photos usage, generative AI cost can dominate the per-user economics.

**Mitigation:**

| Strategy | Implementation |
|----------|---------------|
| **Gated access** | Generative features limited to Google One subscribers or Pixel owners |
| **On-device inference** | Magic Eraser runs on-device for Pixel phones (Tensor chip); cloud fallback for others |
| **Model distillation** | Smaller, faster diffusion models for common edits (erase small objects) |
| **Caching edits** | If user re-edits same photo, cache intermediate latent representations |
| **Batch processing** | Group generative requests; process during off-peak hours when possible |
| **Request throttling** | Per-user daily limit on generative AI operations (e.g., 25 Magic Eraser uses/day) |
