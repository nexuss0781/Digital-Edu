...
| SLO | Error Budget (30d) | Fast Burn Alert | Slow Burn Alert |
|-----|-------------------|-----------------|-----------------|
| Upload success 99.95% | 21.6 min downtime | >2% error rate for 5 min (14.4x burn) | >0.2% error rate for 6 hours (1.4x burn) |
| Viewing latency p99 <300ms | 0.01% budget | >1% violations for 5 min | >0.05% violations for 1 hour |
| Search latency p95 <400ms | 0.05% budget | >5% violations for 2 min | >0.5% violations for 30 min |
| Sync convergence p95 <30s | 0.05% budget | >3% violations for 5 min | >0.3% violations for 1 hour |
...
Daily Metrics:
├── Total storage consumed (EB) — trend over 90 days
├── Daily net growth (PB/day) — current vs rolling 30-day average
├── Growth by quality tier:
│   ├── Original: +12 PB/day (60%)
│   ├── Storage Saver: +6 PB/day (30%)
│   └── Express Backup: +2 PB/day (10%)
├── Deduplication savings (PB avoided)
├── Tiered storage distribution:
│   ├── Hot (SSD, <30 days): 600 PB
│   ├── Warm (HDD, 30d-1yr): 8 EB
│   └── Cold (Archive, >1yr): 29 EB
└── Projected capacity exhaustion date per region

Alerts:
├── Any region >85% capacity → 30-day warning
├── Growth rate >2x 30-day average → investigate
└── Erasure coding rebuild queue >10K chunks → degraded durability
```

### ML Pipeline Capacity

```
DASHBOARD: ML Infrastructure Utilization

TPU/GPU Fleet:
├── Total accelerators allocated to Photos pipeline
├── Utilization by model type:
│   ├── Classification: 72% avg, 95% peak
│   ├── Face detection/embedding: 68% avg, 90% peak
│   ├── Visual embedding: 65% avg, 88% peak
│   ├── OCR: 45% avg, 70% peak
│   └── Generative AI (Magic Eraser): 55% avg, 85% peak
├── Queue depth by priority:
│   ├── P0 (interactive): target <100
│   ├── P1 (backup uploads): target <50K
│   └── P2 (reprocessing): target <500K
└── Cost per inference (trend over 90 days)
```
