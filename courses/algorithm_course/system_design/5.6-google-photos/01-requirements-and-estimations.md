...
### Extended Features (Growth Scope)

| # | Feature | Description |
|---|---------|-------------|
| F15 | **Ask Photos (AI Search)** | Gemini-powered natural language Q&A over photo library ("What restaurant did we visit in Paris?") |
| F16 | **Ultra HDR Support** | Ingest, store, and serve Ultra HDR (gain map JPEG) photos with HDR-aware thumbnails |
| F17 | **Locked Folder** | On-device encrypted folder for sensitive photos; excluded from backups and sharing |
| F18 | **Video Highlights** | Auto-generated highlight reels from video clips using scene detection and beat-matching |
| F19 | **Cinematic Photos** | ML-generated 3D depth effect from 2D photos with parallax animation |
| F20 | **Document Scanning** | Auto-detect and crop documents from photos; OCR for text extraction |
...
| Direction | Metric | Value | Calculation |
|-----------|--------|-------|-------------|
| **Ingress** | Daily upload bandwidth | ~20 PB | 4B × 5 MB |
| **Ingress** | Peak upload bandwidth | ~1.85 Tbps | 140K/s × 5 MB × 8 bits / 3 |
| **Egress** | Daily read bandwidth | ~80 PB | Mix of thumbnails + full-res |
| **Egress** | Peak read bandwidth | ~7.4 Tbps | 1.4M/s × ~2 MB × 8 bits / 3 |
| **Internal** | ML pipeline data movement | ~20 PB/day | Images read from blob for ML inference |
| **Internal** | Cross-region replication | ~20 PB/day | Async blob replication to 2+ regions |

### Ask Photos (AI Search) Traffic

| Metric | Value | Calculation |
|--------|-------|-------------|
| Ask Photos daily queries | ~10 million | Early adoption phase (2025) |
| Gemini inference calls/query | 2-3 | Agent model + retrieval + answer model |
| Average context window per query | ~50 photos | Retrieved photo set for answer model |
| Gemini inference latency target (p95) | <3 seconds | End-to-end including retrieval |
| GPU/TPU cost per query | ~$0.01-0.02 | Multi-step Gemini inference |
...
| **Generative AI Compute** | $50M - $100M | Magic Eraser, Ask Photos, Reimagine (growing rapidly) |

> **Key Insight:** Google Photos is a **loss leader** for most free-tier users, designed to drive Google One subscriptions and keep users in the Google ecosystem. The storage cost per free user (~$0.10/year) is subsidized by the ecosystem value. Generative AI features (Magic Eraser, Ask Photos) are increasingly gated behind Google One subscriptions, shifting the cost model from storage-driven to compute-driven revenue.
