...
Each price level: Doubly-linked list of orders at that price (FIFO (First-In-First-Out, like a line at a store) queue)

BUY Side (Bids)                    SELL Side (Asks)
Price    | Orders (FIFO (First-In-First-Out, like a line at a store))           Price    | Orders (FIFO (First-In-First-Out, like a line at a store))
...
### Price-Time Priority (FIFO (First-In-First-Out, like a line at a store)) Matching Algorithm
...
FUNCTION matchOrder(incoming_order):
    IF incoming_order.side == BUY:
        ...
        WHILE incoming_order.remaining_qty > 0
            ...
            FOR each resting_order IN price_level (FIFO (First-In-First-Out, like a line at a store) order):
...
Edge Case (Unusual or extreme situation): if client misses a delta (WebSocket reconnect),
...
## Deep Dive 7: AMO Queue Architecture

### Deferred Execution: Accept Now, Execute Later

After-Market Orders (AMO) represent a temporally decoupled execution model: orders are submitted outside market hours (3:30 PM – 9:15 AM) but executed only when the market opens. This creates a durable queue with specific processing guarantees.

### AMO Lifecycle

```
Phase 1: Submission (Any time outside market hours)
    User submits order via API/app
    OMS validates: instrument exists, valid order type, valid quantity
    OMS does NOT run risk checks (margin can change overnight)
    OMS persists to PostgreSQL: status = AMO_QUEUED
    Response: "Order queued for next market session"

Phase 2: Pre-Market Validation (9:00 AM - 9:14 AM)
    AMO Processor reads all AMO_QUEUED orders
    Sorts by submission_time ASC (FIFO (First-In-First-Out, like a line at a store) priority)

    FOR each amo_order:
        -- Refresh instrument data (circuit limits may have changed)
        instrument = InstrumentMaster.getLatest(amo_order.instrument_token)

        -- Run risk checks with current margin state
        result = RiskEngine.preTradeCheck(amo_order, user_state)

        IF result == APPROVED:
            amo_order.status = VALIDATED_AMO
            amo_order.margin_blocked = result.margin_amount
            -- Margin is now blocked; user cannot withdraw it
        ELSE:
            amo_order.status = REJECTED
            Notify user: "AMO rejected: " + result.reason

    Log: "Validated X of Y AMO orders; Z rejected"

Phase 3: Staged Release (9:15:00 AM)
    -- Exchange imposes per-broker rate limits
    -- Uncontrolled flood = exchange rejections

    batch_size = 1,000
    batch_interval = 100ms  -- 10,000 orders/sec controlled release

    validated_amos = getValidatedAMOs(sorted_by=submission_time)

    FOR each batch OF validated_amos (batch_size):
        FOR each order IN batch:
            FIXGateway.sendNewOrderSingle(order)
            order.status = PENDING
        WAIT(batch_interval)

    -- Concurrent new orders from users are also processed
    -- AMO and live orders share the FIX session pool

Phase 4: Monitoring
    Track: AMO queue drain rate
    Alert if: queue not fully drained by 9:16 AM
    Metric: amo.queue.drain_time (target: < 60 seconds)
```

### Why Not Validate at Submission Time?

```
Between submission (e.g., 8 PM) and market open (9:15 AM):
    - User's margin may change (deposit, withdrawal, other orders)
    - Instrument circuit limits update overnight
    - SPAN margin parameters refresh daily at 5 PM
    - New F&O contracts may be listed/expired
    - Exchange may issue trading halt notices

Pre-market validation at 9:00 AM catches all overnight changes.
Rejecting at submission would require constant re-validation overnight.
```

---

## Deep Dive 8: Market Data Feed Handler — Binary Protocol Parsing

### Exchange Binary Feed Format

```
The exchange (NSE) publishes market data via UDP multicast in a proprietary
binary format. The feed handler must parse this at wire speed.

Packet Structure (NSE Tick-by-Tick):
    ┌──────────────────────────────────────────────┐
    │ Header (16 bytes)                             │
    │   message_length:    2 bytes (uint16)         │
    │   stream_id:         2 bytes (uint16)         │
    │   sequence_number:   8 bytes (uint64)         │
    │   timestamp_ns:      4 bytes (nanoseconds)    │
    ├──────────────────────────────────────────────┤
    │ Payload (variable, per instrument)            │
    │   instrument_token:  4 bytes (uint32)         │
    │   last_price:        4 bytes (int32, ×100)    │
    │   last_quantity:     4 bytes (uint32)         │
    │   total_buy_qty:     4 bytes (uint32)         │
    │   total_sell_qty:    4 bytes (uint32)         │
    │   best_bid_price:    4 bytes (int32, ×100)    │
    │   best_bid_qty:      4 bytes (uint32)         │
    │   best_ask_price:    4 bytes (int32, ×100)    │
    │   best_ask_qty:      4 bytes (uint32)         │
    │   volume:            8 bytes (uint64)         │
    │   open_interest:     4 bytes (uint32)         │
    │   ... (20-level depth if full feed)           │
    └──────────────────────────────────────────────┘

Prices are transmitted as integers (×100): ₹2,452.05 → 245205
This avoids floating-point representation issues.
```

### Feed Handler Processing Pipeline

```
FUNCTION processFeedPacket(raw_bytes):
    -- Step 1: Parse header (zero-copy if possible)
    header = parseHeader(raw_bytes[0:16])

    -- Step 2: Sequence check
    expected_seq = lastSequence[header.stream_id] + 1
    IF header.sequence_number > expected_seq:
        -- Gap detected: request retransmission
        Metrics.increment("feed.sequence_gap")
        requestRetransmission(header.stream_id, expected_seq, header.sequence_number)
    ELSE IF header.sequence_number < expected_seq:
        -- Duplicate: discard
        RETURN
    lastSequence[header.stream_id] = header.sequence_number

    -- Step 3: Parse payload
    tick = parseTick(raw_bytes[16:])
    tick.price = tick.raw_price / 100.0  -- Convert to decimal

    -- Step 4: Delta compression for downstream
    previous = latestQuoteCache[tick.instrument_token]
    delta = computeDelta(previous, tick)

    IF delta.hasChanges():
        -- Step 5: Update in-memory cache
        latestQuoteCache[tick.instrument_token] = tick

        -- Step 6: Publish to downstream (broker DC via leased line)
        CompressedStream.send(delta)  -- LZ4 compressed delta

Processing rate: ~2M packets/sec
Latency budget: < 100μs per packet (parse + compress + send)
CPU: single-threaded per stream (avoid lock contention)
Memory: ~5,000 instruments × 200 bytes = 1 MB (trivial)
```

---

## Key Slowest part of the process Summary
