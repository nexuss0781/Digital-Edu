...
8. **Smart Order Routing Across Exchanges**: The same instrument trades on multiple exchanges (NSE, BSE) at slightly different prices. Smart order routing evaluates liquidity, spread, and latency across exchanges in real-time and routes to the venue offering best execution—a regulatory obligation that adds algorithmic complexity to the order path.

9. **After-Market Order Queue with Priority Staging**: AMO orders are accepted 24/7 but must be validated, margin-blocked, and released in controlled batches at market open. This creates a deferred execution system where order submission and order routing are temporally decoupled by hours—requiring a durable queue with priority ordering.
...
| Era | Model | Key Shift |
|-----|-------|-----------|
| Pre-2010 | Call-and-trade, terminal-based | Broker employees placed orders on behalf of clients |
| 2010–2015 | Web-based trading, discount brokerage | Self-service web platforms; flat-fee pricing disrupted percentage-based commissions |
| 2015–2020 | Mobile-first, API-driven | Mobile apps became primary interface; API access enabled algo trading for retail |
| 2020–2023 | Zero-commission, gamification | Free equity trades, social trading features, options volume explosion |
| 2023–2025 | Regulatory tightening, T+1 settlement | SEBI mandated upfront margins, T+1 settlement, enhanced cyber security frameworks |
| 2025+ | AI-assisted trading, real-time analytics | AI order routing, sentiment-based insights, sub-second settlement exploration |
| 2026+ | Instant settlement, embedded finance | T+0 settlement pilots, brokerage-as-a-service APIs, embedded trading in super-apps |
...
|---------|-----------------|------------|
| **Event-Sourced State Reconstruction** | Position Service, Risk Engine | Positions are derived from the immutable trade event stream—never written directly; crash recovery replays events from the last checkpoint |
| **In-Process Validation Gate** | Risk Engine ↔ OMS co-location | Sub-100μs pre-trade checks force the risk engine into OMS process memory—network hops are architecturally prohibited |
| **Binary Protocol Fan-Out** | Ticker / WebSocket Server | Mode-based binary streaming (LTP 8B / quote 44B / full 184B) with 100ms batching replaces JSON, cutting bandwidth 60–90% |
| **Predictable Spike Pre-Provisioning** | Market open (9:15 AM) | Deterministic scale-up 60 minutes before a known 15× spike beats reactive auto-scaling that cannot provision in < 60s |
| **Co-Located Edge Gateway** | Exchange colo Feed Handler + Order Gateway | Critical path processing placed at the exchange edge reduces latency from milliseconds to microseconds |
| **Write-Ahead Idempotent Pipeline** | Order persistence before exchange submission | WAL guarantees no order loss on crash; FIX sequence numbers + idempotency keys prevent duplicate execution |
| **Hash-Chained Immutable Audit** | Regulatory audit trail | Append-only logs with per-record hash chains satisfy SEBI 7-year tamper-evidence mandates |
| **Temporal Multi-View Data Model** | T+1 settlement portfolio | Three concurrent views—intraday positions, pending settlement, settled holdings—model the same asset across settlement timelines |
...
| [6.8 - Real-Time Collaborative Editor](../6.8-real-time-collaborative-editor/) | WebSocket connection management at scale, binary delta protocols |
| [12.10 - Polling & Voting System](../12.10-polling-voting-system/) | High-throughput write spikes, thundering herd mitigation |
| [8.13 - Cryptocurrency Wallet](../8.13-cryptocurrency-wallet-system/) | Real-time balance tracking, transaction ledger, key management |
