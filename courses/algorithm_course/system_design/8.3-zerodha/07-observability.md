...
## GTT & Background Service Metrics

| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `gtt.active_triggers.count` | Gauge | Total active GTT triggers across all users | > 5M (capacity planning) |
| `gtt.evaluation.rate` | Counter | Trigger conditions evaluated per second | Informational |
| `gtt.trigger.fired.rate` | Counter | Triggers that fired (price condition met) | Informational |
| `gtt.trigger.latency` | Histogram | Time from price condition met to order placed | > 5s |
| `gtt.order.placement.failure` | Counter | Triggered orders that failed risk check | > 1% of fired |
...
| Metric | Type | Description | Alert Threshold |
|--------|------|-------------|-----------------|
| `fix.session.status` | Gauge | 1 = connected, 0 = disconnected | = 0 (P0 alert) |
| `fix.heartbeat.latency` | Histogram | Round-trip heartbeat time to exchange | > 100ms |
| `fix.sequence.gap_count` | Counter | Sequence gaps requiring resend requests | > 0 (alert) |
| `fix.failover.count` | Counter | Number of failovers triggered | > 0 (investigate) |
| `fix.message.rate` | Counter | FIX messages sent/received per second | > 10K/sec per session (capacity) |
| `fix.reject.rate` | Counter | Exchange-rejected FIX messages | > 0.1% (investigate) |
...
```
Order Path Instrumentation:
    Every order carries a trace_id from API gateway to exchange and back.
    Spans are recorded at each hop:
        API Gateway → OMS → Risk → Order Gateway → Exchange → back

    Critical path latency = sum of spans on the hot path
    Cold path latency = async spans (position update, notification, portfolio)

    Trace sampling:
        Market hours: 100% of orders traced (every order matters)
        After hours: 10% sampling for portfolio/holdings requests
        Market data: 0.1% sampling (too voluminous for full tracing)

Metric Collection:
    Push-based: services push metrics every 10 seconds to aggregation layer
    Pull-based: aggregation layer scrapes service health endpoints every 15s
    Resolution: 1-second granularity for order/market data metrics during market hours
    Retention: 15 days high-resolution; 1 year downsampled (1-minute granularity)

Alert Routing:
    P0 (market hours): page on-call + backup + engineering lead
    P0 (after hours): page on-call
    P1: page on-call
    P2: Slack channel alert
    P3: ticket creation

SLA Breach Detection:
    Monitor all SLO targets continuously during market hours:
        - Order placement p99 > 200ms for 30 seconds → P1
        - Risk engine p99 > 200μs for 60 seconds → P1
        - Market data E2E p99 > 10ms for 60 seconds → P1
        - WebSocket error rate > 1% for 30 seconds → P1
        - Position update lag > 500ms for 60 seconds → P1
        - FIX session latency > 1ms for 30 seconds → P1
    Compound alert: if 3+ SLO breaches simultaneously → auto-escalate to P0

War Room Triggers (auto-opens collaboration channel):
    - Any P0 during market hours
    - Exchange connectivity issue affecting any segment
    - Order throughput drops > 50% from rolling 5-minute average
    - Market data feed gap > 5 seconds
    - Settlement reconciliation mismatch detected
```
