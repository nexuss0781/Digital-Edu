...
```
API Key Lifecycle:
    1. User generates api_key via Kite Console (web-only, post 2FA)
    2. api_secret shown ONCE at generation (never stored in plaintext)
    3. api_key is permanent; api_secret can be regenerated (invalidates old)
    4. Daily access_token obtained via login flow:
       POST /session/token { api_key, request_token, checksum }
       checksum = SHA256(api_key + request_token + api_secret)
    5. access_token expires at midnight IST (end of trading day)

Security Controls:
    - api_secret: stored as SHA256 hash; irretrievable after first display
    - access_token: rotated daily; scoped to single calendar day
    - IP allowlisting: optional; restrict API access to specific IPs
    - Rate limiting: 3 req/sec for trading APIs, 1 req/sec for data APIs
    - Webhook notifications: alert on new API key generation, login from new IP
    - API key scoping (planned): read-only vs. trade-enabled keys
```

### FIX Protocol Security

```
Exchange FIX Session Security:
    - CompID authentication: each FIX session identified by unique SenderCompID
    - Sequence integrity: monotonically increasing sequence numbers per session
    - Replay prevention: exchange rejects messages with duplicate sequence numbers
    - Session encryption: application-level encryption on leased line
    - Heartbeat monitoring: exchange drops session after 3 missed heartbeats
    - IP restriction: exchange only accepts connections from registered IPs at colo
    - Message validation: exchange validates all FIX tags and values; rejects malformed

Broker-Side Controls:
    - FIX session credentials rotated quarterly
    - Dedicated process per exchange session (blast radius containment)
    - FIX message audit log: every outbound/inbound message logged with timestamps
    - Automated gap detection: sequence gaps trigger immediate ResendRequest
```
...
|-----------|---------------|----------|
| PAN/Aadhaar | Restricted PII | Encrypted at rest; masked in UI/logs; access-logged |
| Bank account | Restricted PII | Encrypted; never in logs; withdraw requires transaction PIN |
| API secret | Secret | Hashed; shown once; irretrievable |
| Trading password | Secret | bcrypt hash only; never logged |
| Order/trade data | Confidential | Encrypted in transit; audit-logged access |
| Market data | Public (after exchange publishes) | No DLP controls needed |
| Portfolio holdings | Confidential | User-scoped access; encrypted at rest |
| Audit logs | Regulatory | WORM storage; tamper-evident; 7-year retention |
...
|-------|---------|----------|
| **Detection** | Automated alerts (P0/P1); user reports; exchange notifications | Immediate |
| **Triage** | On-call classifies severity; war room if P0 during market hours | < 5 minutes |
| **Containment** | Isolate affected component; switch to backup; rate-limit if DDoS | < 15 minutes |
| **Investigation** | Trace root cause via distributed tracing and audit logs | 1–4 hours |
| **Recovery** | Restore service; replay events if data loss; reconcile with exchange | Hours |
| **Regulatory Notification** | Notify SEBI if: >15 min market-hours outage, data breach, or security incident | Within 6 hours |
| **Post-Mortem** | Root cause analysis; remediation plan; update runbooks; share with SEBI if required | Within 48 hours |
...
| # | Control | Status |
|---|---------|--------|
| 1 | TLS 1.3 for all client connections | Required |
| 2 | mTLS for internal service communication | Required |
| 3 | 2FA mandatory for all user logins | Required (SEBI mandate) |
| 4 | API key daily token rotation | Required |
| 5 | Rate limiting on all endpoints | Required |
| 6 | WAF with OWASP top 10 rules | Required |
| 7 | Parameterized queries (no string concatenation) | Required |
| 8 | Certificate pinning in mobile apps | Required |
| 9 | DDoS mitigation (L3/L4/L7) | Required |
| 10 | Append-only audit log with hash chain | Required (SEBI) |
| 11 | Encryption at rest for all databases | Required |
| 12 | Network segmentation (DMZ/app/data/colo zones) | Required |
| 13 | Employee trade monitoring | Required (insider trading prevention) |
| 14 | Quarterly penetration testing | Required (SEBI cyber framework) |
| 15 | Annual DR drill with documented results | Required (SEBI BCP) |
| 16 | FIX session IP restriction at exchange | Required |
| 17 | Separate bank accounts for client funds | Required (SEBI fund segregation) |
| 18 | WORM storage for audit logs (7-year retention) | Required (SEBI) |
| 19 | Suspicious trading pattern detection (STR) | Required (AML compliance) |
| 20 | API key daily token rotation + rate limiting | Required |
...
Key Hierarchy:
    Master Key (HSM-stored):
        ...
        │   └── Encrypts: PAN, bank account, Aadhaar at rest
        ├── API Secret Signing Key
        │   └── Signs: JWT access tokens, API checksums
        ├── Audit Log Hash Key
        │   └── Seeds: hash chain for tamper-evidence
        └── FIX Session Keys
            └── Per-exchange: session-level encryption keys

Rotation:
    - Master key: annually (with HSM ceremony)
    - Database DEK: quarterly (re-encrypt at rest)
    - API signing key: monthly (new tokens auto-use new key)
    - FIX keys: quarterly (coordinated with exchange)
    - JWT secrets: daily (access tokens expire at midnight)
```
