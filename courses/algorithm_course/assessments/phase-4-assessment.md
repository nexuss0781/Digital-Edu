...
- **Dynamic Programming:** Optimal substructure, Memoization (Saving results to avoid repeating work), Tabulation (Building a table of results from the bottom up)
- **Backtracking:** State space exploration, Cutting off unnecessary steps, constraint satisfaction
...
- Explain Formula that defines a step based on previous steps
...
### Requirements
- Use backtracking with Cutting off unnecessary steps
- Generate all C(n,k) combinations
- Avoid duplicates
- Time: O(C(n,k) * k), Space: O(k) for recursion
- Explain Cutting off unnecessary steps optimization
...
- O(n!) Time Complexity (Speed of the algorithm)
...
- ✓ Correct time and Space Complexity (Memory usage of the algorithm)
...
- Cutting off unnecessary steps for efficiency
...
- Missing Cutting off unnecessary steps opportunities
...
- ❌ Incorrect Formula that defines a step based on previous steps
...
- ❌ Missing Cutting off unnecessary steps conditions
...
2. **Define state:**
   ...
   - `dp[i]` = Best possible solution for subproblem ending at i
   - `dp[i][j]` = Best possible solution considering first i of A, first j of B
...
5. **Order of computation:**
   ...
   - Top-down: Memoization (Saving results to avoid repeating work) handles this
...
4. **Cutting off unnecessary steps:**
   - Early termination
...
5. **Optimization:**
   ...
   - Memoization (Saving results to avoid repeating work) if overlapping subproblems
