...
| **Time Complexity (Speed of the algorithm)** | O(N log k) where N = total elements, k = number of lists |
| **Space Complexity (Memory usage of the algorithm)** | O(k) for the heap |
...
- **Rule that never changes:** Heap always has at most k elements
