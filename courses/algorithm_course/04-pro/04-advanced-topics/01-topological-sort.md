...
| **Time Complexity (Speed of the algorithm)** | O(V + E) - visits each vertex and edge once |
| **Space Complexity (Memory usage of the algorithm)** | O(V) for queue/stack and result |
...
- **Key Rule that never changes**: Process nodes only when all dependencies met
