...
| **Time Complexity (Speed of the algorithm)** | O(log n) per insert, O(1) for median |
| **Space Complexity (Memory usage of the algorithm)** | O(n) for storing all elements |
...
```python
# WRONG: Not maintaining size Rule that never changes
...
### 3. Not Ensuring Ordering Rule that never changes
