...
**Two Pointers** uses two indices (or references) to traverse data structures, typically to reduce Time Complexity (Speed of the algorithm) from O(n²) to O(n).
...
- Optimize Space Complexity (Memory usage of the algorithm)
...
- [ ] Can optimize Brute Force (Checking every single possibility) O(n²) solutions to O(n) using two pointers
