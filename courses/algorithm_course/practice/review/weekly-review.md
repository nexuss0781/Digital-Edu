...
- Signatures: Choose-explore-unchoose, Cutting off unnecessary steps, depth-first exploration
...
- Signatures: Top-down (Memoization (Saving results to avoid repeating work)) vs. bottom-up (Tabulation (Building a table of results from the bottom up))
...
**Reflection Questions:**
- What's the Formula that defines a step based on previous steps?
- Can I reduce Space Complexity (Memory usage of the algorithm)?
...
- [ ] Brainstorm approaches (Brute Force (Checking every single possibility) → optimal)
...
- [ ] Describe Brute Force (Checking every single possibility) approach
- [ ] State time/Space Complexity (Memory usage of the algorithm)
