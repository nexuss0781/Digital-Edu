...
difficulty: medium
category: medium
topics: ["array", "dynamic-programming", "Memoization (Saving results to avoid repeating work)"]
patterns: ["dp-3d", "Memoization (Saving results to avoid repeating work)", "interval-dp"]
...
prerequisites:
  ...
  - Memoization (Saving results to avoid repeating work)/top-down DP
  - 3D DP states
---
# Remove Boxes
...
The key insight: Sometimes it's better to remove boxes in the middle first to merge boxes of the same color, even if they're not currently consecutive. This suggests we need dynamic programming with Memoization (Saving results to avoid repeating work).
</details>
...
|----------|----------------|------------------|-------|
| Greedy (Wrong) | O(n²) | O(1) | Fails to find Best possible solution |
| Brute Force (Checking every single possibility) Recursion | O(n!) | O(n) | Explores all removal orders - too slow |
| 3D DP with Memoization (Saving results to avoid repeating work) | O(n⁴) | O(n³) | Best possible solution; n³ states, O(n) per state |
| Bottom-up 3D DP | O(n⁴) | O(n³) | Same as Memoization (Saving results to avoid repeating work) but iterative |
...
def dp(i, j, k):
    if i > j:
        return 0
    # ... Memoization (Saving results to avoid repeating work) check
...
- [ ] Implement 3D DP with Memoization (Saving results to avoid repeating work)
