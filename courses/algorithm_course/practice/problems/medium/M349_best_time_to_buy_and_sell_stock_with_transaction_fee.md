...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Try all combinations) | O(2^n) | O(n) | Exponential - impractical |
...
def maxProfit(prices, fee):
    if len(prices) < 2:  # Missing Edge Case (Unusual or extreme situation) check
        return 0
    hold = -prices[0]
    notHold = 0
    for price in prices[1:]:
