...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (check all substrings) | O(n²) | O(1) | Check every substring for validity |
    ...
    for i in range(len(s)):
        for j in range(i + 2, len(s) + 1):
            ...
            if is_valid(substr):
                count += 1
    return count
# Problem: Quadratic Time (Time grows very fast with data, like a square), unnecessary validation
```
