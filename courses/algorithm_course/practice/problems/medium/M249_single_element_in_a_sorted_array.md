...
You have a sorted array where every element appears exactly twice, except for one element that appears only once. Find and return that unique element. The constraint is that your solution must run in O(log n) Time Complexity (Speed of the algorithm) and use O(1) extra space, which rules out linear scans and hash tables.
...
| Binary Search (Pattern) | O(log n) | O(1) | Best possible solution using index parity |
