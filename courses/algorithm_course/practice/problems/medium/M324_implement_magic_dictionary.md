...
<details>
<summary>Hint 1: Brute Force (Checking every single possibility) with String Comparison</summary>
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | buildDict: O(n), search: O(n×m) | O(n×m) | n = dict size, m = avg word length |
