...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Try all partitions) | O(2^n) | O(n) | Exponential without Cutting off unnecessary steps |
| Backtracking with Cutting off unnecessary steps | O(n^2) for first two, O(n) validation | O(n) | Much faster with early termination |
