...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) enumeration | O(n log n) | O(n) | Generate all, sort, return k-th |
...
| Iterative generation | O(k) | O(1) | Better than Brute Force (Checking every single possibility), still slow |
