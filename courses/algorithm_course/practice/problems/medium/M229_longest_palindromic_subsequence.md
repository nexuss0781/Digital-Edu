...
This brings Space Complexity (Memory usage of the algorithm) down from O(n²) to O(n) while maintaining O(n²) time.
...
| Recursion + Memoization (Saving results to avoid repeating work) | O(n²) | O(n²) | Easier to understand, same complexity |
