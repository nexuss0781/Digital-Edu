...
The Brute Force (Checking every single possibility) approach is to simulate placing words row by row. However, with up to 20,000 rows, this can be slow. The key optimization is recognizing that the pattern of which word starts each row will eventually repeat. Use Memoization (Saving results to avoid repeating work) or precomputation to avoid recalculating.
</details>
...
| Brute Force (Checking every single possibility) Simulation | O(rows × cols) | O(1) | Too slow for large inputs |
...
- [ ] Day 2: Implement Brute Force (Checking every single possibility) first, then optimize (35 min)
