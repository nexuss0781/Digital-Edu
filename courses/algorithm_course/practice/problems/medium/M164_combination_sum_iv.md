...
|----------|------|-------|-------|
| Recursive Brute Force (Checking every single possibility) | O(n^target) | O(target) | Exponential; tries all sequences |
| Memoization (Saving results to avoid repeating work) (Top-Down DP) | O(target * n) | O(target) | Cache results for each sum |
...
| Find actual sequences | Return all sequences, not just count | Backtracking with Memoization (Saving results to avoid repeating work) |
