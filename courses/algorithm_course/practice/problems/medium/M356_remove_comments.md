...
**Block comments** start with `/*` and continue until the matching `*/` is found, even across multiple lines. All content between these delimiters is ignored. An important Edge Case (Unusual or extreme situation): `"/*//"` starts a block comment (the `/*` takes precedence), and it won't close until a later `*/` appears, because the `//` inside the block comment is just treated as regular comment text, not as a line comment.
...
Edge Case (Unusual or extreme situation): Block comments can span lines, so `in_block` persists across line boundaries.
</details>
