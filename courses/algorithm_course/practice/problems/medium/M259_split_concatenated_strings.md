...
|----------|----------------|------------------|-------|
| Best possible solution | O(n * L²) | O(n * L) | n = number of strings, L = average length |
| Brute Force (Checking every single possibility) | O(n! * n * L²) | O(n * L) | Try all permutations and reversals |
...
- [ ] Handle Edge Case (Unusual or extreme situation): single string
- [ ] Handle Edge Case (Unusual or extreme situation): all strings identical
