...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(n) | Linear search in postorder for each recursive call |
...
1. **Not handling single-child ambiguity**
   ...
   # Correct: Handle Edge Case (Unusual or extreme situation) where subtree is empty
   if len(preorder) == 1:
