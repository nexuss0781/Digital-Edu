...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) with Set | O(log x(bound) * log y(bound)) | O(log x(bound) * log y(bound)) | Number of unique power combinations is limited by logarithmic growth |
