...
Edge Case (Unusual or extreme situation) awareness: `n-1` is always a valid base (representing n as `11` in base n-1), so a solution always exists.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n) | O(1) | Try all bases from 2 to n-1 |
