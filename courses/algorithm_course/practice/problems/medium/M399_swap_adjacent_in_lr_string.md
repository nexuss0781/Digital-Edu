...
You can do this in a single pass with two pointers instead of extracting characters into separate arrays. Skip over 'X' characters in both strings and compare non-X characters directly, checking the position constraints as you go. This reduces Space Complexity (Memory usage of the algorithm) from O(n) to O(1).
</details>
