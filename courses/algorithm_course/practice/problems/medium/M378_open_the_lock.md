...
Important Edge Case (Unusual or extreme situation): if `"0000"` itself is a deadend, you're stuck immediately and should return `-1` before attempting any moves.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DFS | O(10^4 * 8^d) | O(d) | d = depth, explores redundantly |
