...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Prime Check | O(n * sqrt(b)) | O(1) | b = max bits, check primality each time |
...
3. **Forgetting Edge Case (Unusual or extreme situation): 1 is not prime**
   ```python
