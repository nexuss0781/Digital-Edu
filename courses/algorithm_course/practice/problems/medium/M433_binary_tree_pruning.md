...
id: M433
old_id: A281
slug: binary-tree-Cutting off unnecessary steps
title: Binary Tree Cutting off unnecessary steps
difficulty: medium
category: medium
topics: ["tree"]
patterns: []
...
---
# Binary Tree Cutting off unnecessary steps
...
Think of it like Cutting off unnecessary steps a real tree: you start at the leaves and work your way up toward the root, cutting branches only when you're certain they have no valuable fruit (nodes with value 1) anywhere along them. After Cutting off unnecessary steps, you might end up with an entirely empty tree (return null), a smaller tree, or the original tree unchanged if every subtree contains at least one 1.
...
1. **Using pre-order instead of post-order**
   ...
   def prune(node):
       if not node:
           return None
       if node.val == 0:
           return None  # Premature Cutting off unnecessary steps
...
3. **Forgetting to check if children exist after Cutting off unnecessary steps**
   ```python
...
| Delete Leaves With Given Value | Medium | Different Cutting off unnecessary steps condition |
