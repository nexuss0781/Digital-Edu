...
Step-by-step plan in plain English approach:
```
count = 0
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(10^15) | O(1) | Check every number in range - impractical |
| Generate All | O(5^8 × 15) | O(5^8) | Generate for each length, max length 15 |
| **Optimal with Cutting off unnecessary steps** | **O(5^(h/2) × h)** | **O(5^(h/2))** | h = len(high), generate only valid lengths |
