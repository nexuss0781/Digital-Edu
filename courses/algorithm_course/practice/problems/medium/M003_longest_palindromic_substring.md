...
**Time Complexity (Speed of the algorithm):** O(n²) - n centers × O(n) expansion each
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) (check all substrings) | O(n³) | O(1) | Too slow |
...
| **Palindrome partitioning** | Split into all palindromes | Backtracking + Memoization (Saving results to avoid repeating work) |
