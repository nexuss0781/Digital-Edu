...
Use the compare function or sign function to normalize comparisons: cmp(a, b) returns -1, 0, or 1. This makes alternation checking cleaner: new_cmp should be -old_cmp for continuation. Edge Case (Unusual or extreme situation): when arr[i] == arr[i-1], current length becomes 1 since equal elements can't form turbulent subarray.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all subarrays; inefficient |
...
- [ ] Optimal Time Complexity (Speed of the algorithm) achieved (O(n))
- [ ] Clean, readable code
