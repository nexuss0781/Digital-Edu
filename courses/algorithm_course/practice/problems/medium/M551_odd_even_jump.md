...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(n) | For each position, linearly search for next jump |
...
- [ ] Optimal Time Complexity (Speed of the algorithm) achieved (O(n log n))
- [ ] Clean, readable code
