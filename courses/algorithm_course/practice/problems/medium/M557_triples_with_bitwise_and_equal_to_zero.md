...
The Brute Force (Checking every single possibility) O(n³) solution is too slow. However, you can optimize by pre-computing all possible pairwise AND results (nums[i] & nums[j]) in O(n²), then for each pair result, count how many values k satisfy pair_result & nums[k] == 0.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n³) | O(1) | Three nested loops, too slow for n=1000 |
...
1. **Triple counting instead of using pair frequencies**
   ```python
   # Wrong: Brute Force (Checking every single possibility) O(n³) - too slow
   count = 0
