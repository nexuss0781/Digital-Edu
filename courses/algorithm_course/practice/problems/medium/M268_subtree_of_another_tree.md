...
```python
# WRONG: Missing Edge Case (Unusual or extreme situation) when subRoot is None
def isSubtree(root, subRoot):
    if not root:
        return False
