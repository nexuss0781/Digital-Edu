...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all possible subarrays, calculate sums |
...
The optimal approach achieves Linear Time (Time grows directly with data) by combining prefix sums with hash table lookups, eliminating the need to check every subarray pair.
...
- [ ] Solve using Brute Force (Checking every single possibility) (understand O(n²) solution)
- [ ] Implement hash table + prefix sum solution
- [ ] Handle Edge Case (Unusual or extreme situation): empty array result (return 0)
- [ ] Handle Edge Case (Unusual or extreme situation): k = 0 with zeros in array
- [ ] Test with all negative numbers
- [ ] Test with all positive numbers
- [ ] Test with mixed positive/negative
- [ ] Verify Time Complexity (Speed of the algorithm) is O(n)
- [ ] Verify Space Complexity (Memory usage of the algorithm) is O(n)
