...
You only need to track the previous state, so you can use O(1) space instead of O(n) arrays. Use two variables for the previous position's keep and swap values, and update them as you iterate through the arrays. This space optimization doesn't change the Time Complexity (Speed of the algorithm) but reduces memory usage.
</details>
