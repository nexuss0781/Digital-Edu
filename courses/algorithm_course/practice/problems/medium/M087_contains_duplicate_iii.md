...
3. **Brute Force (Checking every single possibility)**: Check all elements in window, O(k) per element
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n × indexDiff) | O(1) | Check all pairs within indexDiff, simple but slow |
