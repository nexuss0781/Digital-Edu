...
The sliding window only cares about requests within the last 3000ms. Older requests can be discarded since they'll never be counted again. This suggests a FIFO (First-In-First-Out, like a line at a store) (First In, First Out) data structure.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n) per ping | O(n) | Store all timestamps, scan entire list each time |
