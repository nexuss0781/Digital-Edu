...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Enumeration | O(n!) | O(n) | Try all partitions of s |
| **Backtracking with Cutting off unnecessary steps** | **O(n^m)** | **O(m+n)** | n=len(s), m=len(pattern), optimal |
...
### Mistake 2: Not Cutting off unnecessary steps search space early
...
def wordPatternMatch(pattern, s):
    def backtrack(p_idx, s_idx, char_to_str, str_to_char):
        ...
        # Cutting off unnecessary steps: not enough string left
...
- [ ] Add Cutting off unnecessary steps optimizations
