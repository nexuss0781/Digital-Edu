...
Handle negative targets by symmetry - just use abs(target). Also, when sum exceeds target with odd difference, you only need at most 2 more steps to make it even (since consecutive integers have opposite parity). This gives O(sqrt(target)) Time Complexity (Speed of the algorithm).
</details>
