...
This is arguably the most famous array problem in computer science education because it has multiple solution approaches (Brute Force (Checking every single possibility) O(n²), divide-and-conquer O(n log n), and Kadane's O(n)) that illustrate fundamental algorithm design principles. The elegant one-pass solution is a perfect example of dynamic programming that doesn't require storing previous results.
...
Interviewers love this problem because it reveals whether you understand the difference between local and global optima, can recognize when greedy decisions work, and know when to throw away partial solutions (restarting when sum becomes negative). The gap between the O(n²) beginner solution and the O(n) Best possible solution is dramatic.
...
| Brute Force (Checking every single possibility) (all subarrays) | O(n²) or O(n³) | O(1) | Too slow |
| Divide and conquer | O(n log n) | O(log n) | Theoretical interest only |
| Dynamic programming (Tabulation (Building a table of results from the bottom up)) | O(n) | O(n) | Unnecessary space |
| Prefix sum + optimization | O(n) | O(n) | More complex, same result |

**Why Kadane's wins:**
- Linear Time (Time grows directly with data): can't do better than reading each element
