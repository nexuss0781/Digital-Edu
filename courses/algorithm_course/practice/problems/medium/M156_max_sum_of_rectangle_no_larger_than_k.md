...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (All Rectangles) | O(m²n²) × O(mn) = O(m³n³) | O(1) | Try all rectangles, sum each |
| Prefix Sum + Brute Force (Checking every single possibility) | O(m²n²) | O(mn) | 2D prefix sum, O(1) rectangle sum |
