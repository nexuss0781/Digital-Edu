...
<details>
<summary>Hint 1: Brute Force (Checking every single possibility) Recognition</summary>
...
<details>
<summary>Hint 3: Stack Rule that never changes</summary>

Key insight: the stack stores indices of days waiting to find their next warmer day.

Rule that never changes: temperatures at indices in the stack are in **decreasing order** from bottom to top.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (nested loop) | O(n²) | O(1) | For each element, scan remaining array; too slow |
...
Monotonic stack is the standard and most intuitive Best possible solution.
