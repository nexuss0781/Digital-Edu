...
prerequisites: ["backtracking", "recursion", "Cutting off unnecessary steps", "partition-problem"]
---
    ...
    def backtrack(index):
        ...
        for i in range(4):
            if sides[i] + matchsticks[index] <= target:
                ...
                # Cutting off unnecessary steps: if this side is empty, no point trying other empty sides
                if sides[i] == 0:
                    break
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (All Permutations) | O(n! × 4^n) | O(n) | Try all ways to assign sticks to sides |
| Backtracking (No Cutting off unnecessary steps) | O(4^n) | O(n) | Each stick can go to any of 4 sides |
| Backtracking (With Cutting off unnecessary steps) | O(4^n) best case | O(n) | Sorting + early termination helps significantly |
...
**Mistake 1: Not Sorting for Early Cutting off unnecessary steps**
...
**Mistake 2: Missing Critical Cutting off unnecessary steps**
