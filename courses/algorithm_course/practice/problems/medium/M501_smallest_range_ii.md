...
After sorting, there's a partition point where all elements to the left are increased by k and all to the right are decreased by k. The Best possible solution involves trying all possible partition points and finding the minimum range.
</details>
...
Edge Case (Unusual or extreme situation): if all elements get the same operation (+k or -k), the range remains unchanged. This happens when the partition is at the start (all -k) or end (all +k). The answer is min of this unchanged range and all partition attempts.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(2^n) | O(1) | Try all 2^n combinations of +k/-k |
