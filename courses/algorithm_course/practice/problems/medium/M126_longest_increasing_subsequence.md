...
An important Edge Case (Unusual or extreme situation) is an array with all identical elements like [7,7,7,7,7], which has a longest increasing subsequence of length 1 (just picking any single element). Another Edge Case (Unusual or extreme situation) is a strictly decreasing array like [5,4,3,2,1], which also has a maximum length of 1. Empty arrays or single-element arrays have well-defined answers too. The challenge is finding this length efficiently - the naive approach of checking all possible subsequences would be exponentially slow.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(2^n) | O(n) | Try include/exclude each element |
| **DP Bottom-Up** | **O(n²)** | **O(n)** | Classic DP solution |
| DP + Binary Search | O(n log n) | O(n) | Optimal Time Complexity (Speed of the algorithm) |
