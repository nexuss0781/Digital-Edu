...
| **Postorder DFS** | **O(n)** | **O(h)** | **Single pass, Best possible solution** |
...
- [ ] Handle Edge Case (Unusual or extreme situation): entire tree is BST
- [ ] Handle Edge Case (Unusual or extreme situation): no valid BST (all single nodes)
- [ ] Test with left-skewed tree
- [ ] Test with right-skewed tree
- [ ] Test with mixed valid/invalid subtrees
- [ ] Verify O(n) Time Complexity (Speed of the algorithm)
