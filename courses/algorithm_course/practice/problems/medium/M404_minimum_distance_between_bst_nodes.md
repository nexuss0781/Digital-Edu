...
In technical interviews, this problem tests whether you understand tree traversal patterns and can exploit structural properties rather than using Brute Force (Checking every single possibility). It's a classic example of how data structure invariants lead to efficient algorithms.
...
You can do this iteratively using a stack to avoid recursion overhead, or use Morris traversal for O(1) Space Complexity (Memory usage of the algorithm). However, the recursive solution is cleaner and the O(h) stack space is usually acceptable. Early termination isn't possible since the minimum could be anywhere in the tree.
</details>
