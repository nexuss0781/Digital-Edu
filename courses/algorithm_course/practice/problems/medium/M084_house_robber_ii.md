...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(2^n) | O(n) | Try all combinations of robbing/not robbing each house |
| DP with 2D Array | O(n) | O(n) | Track state for each position, but uses unnecessary space |
| **DP Optimized (2 passes)** | **O(n)** | **O(1)** | Run linear House Robber twice, Best possible solution |
| Memoization (Saving results to avoid repeating work) with Recursion | O(n) | O(n) | Top-down DP, but uses call stack |

## Common Mistakes

**Mistake 1: Not handling single house Edge Case (Unusual or extreme situation)**
...
```python
# Correct - Handle Edge Case (Unusual or extreme situation) first
def rob(nums):
    if len(nums) == 1:
        return nums[0]
    if len(nums) == 2:
