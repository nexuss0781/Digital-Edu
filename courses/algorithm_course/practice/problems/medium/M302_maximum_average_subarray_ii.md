...
Time Complexity (Speed of the algorithm): O(n log(range)) where range is max(nums) - min(nums).
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all subarrays, TLE for large n |
