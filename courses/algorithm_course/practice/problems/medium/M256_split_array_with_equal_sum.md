...
A Brute Force (Checking every single possibility) approach checking all possible (i,j,k) combinations would be O(n³), but you can optimize to O(n²) by fixing the middle divider j, then using hash sets to efficiently match equal sums from the left and right portions.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (3 loops) | O(n³) | O(1) | Check all (i,j,k) combinations |
...
- [ ] Implement Brute Force (Checking every single possibility) O(n³) solution
