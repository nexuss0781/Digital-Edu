...
| With Memoization (Saving results to avoid repeating work) | O(n * C_n) | O(n² * C_n) | Cache results for each [start, end] pair |

Note: The exact Time Complexity (Speed of the algorithm) is related to the nth Catalan number, which counts unique BST structures. For practical purposes, it's exponential in n.
