...
The board can be in different "states" after each column: fully filled, or partially filled (top/bottom cell protruding). Define dp states not just by the column number, but by the board's current state. The Formula that defines a step based on previous steps depends on how dominoes and trominos can transition between these states.
</details>
...
The Formula that defines a step based on previous steps simplifies to: dp[i] = 2*dp[i-1] + dp[i-3] (with proper base cases). This comes from analyzing all possible tile placements. You can optimize space to O(1) by only keeping track of the last few values instead of the entire array. Remember to use modulo 10^9+7 at each step to prevent overflow.
</details>
