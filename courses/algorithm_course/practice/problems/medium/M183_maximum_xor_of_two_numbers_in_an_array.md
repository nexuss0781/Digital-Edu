...
Build a binary trie where each node represents a bit (0 or 1). Insert all numbers into the trie. For each number, traverse the trie trying to take the opposite path at each bit (to maximize XOR). This gives O(n × 31) = O(n) Time Complexity (Speed of the algorithm).
</details>
