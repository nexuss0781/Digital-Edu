...
Use dynamic programming with base cases D(0) = 1 and D(1) = 0. Build up the solution iteratively from 2 to n using the Formula that defines a step based on previous steps. Since n can be up to 10⁶, use space optimization by only keeping track of the last two values instead of an entire array. Remember to apply modulo 10⁹ + 7 at each step to prevent overflow.
...
There's a closed-form approximation: D(n) ≈ n! / e, where e is Euler's number. The exact formula is: D(n) = n! * Σ((-1)^k / k!) for k from 0 to n. However, the DP approach with the Formula that defines a step based on previous steps is more numerically stable for large n and easier to implement with modular arithmetic.
...
# Correct: uses (i-1) as per Formula that defines a step based on previous steps
def findDerangement(n):
    MOD = 10**9 + 7
    dp0, dp1 = 1, 0
    for i in range(2, n + 1):
...
- [ ] Implement DP solution with Formula that defines a step based on previous steps
...
- [ ] **Review in 1 week**: Derive Formula that defines a step based on previous steps from scratch
