...
|----------|------|-------|-----------|
| **Greedy (range-based)** | **O(n)** | **O(1)** | Best possible solution |
| BFS with queue | O(n) | O(n) | Uses extra space for queue |
| Dynamic Programming | O(n²) | O(n) | Correct but slower |
| DFS + Memoization (Saving results to avoid repeating work) | O(n²) | O(n) | Explores unnecessary paths |
...
### 4. Not handling Edge Case (Unusual or extreme situation)
```python
