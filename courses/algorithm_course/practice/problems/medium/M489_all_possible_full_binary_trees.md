...
Use recursion with Memoization (Saving results to avoid repeating work). For n nodes, try all ways to distribute nodes between left and right subtrees (both must get odd numbers). For each valid split (i nodes left, n-1-i nodes right), combine all possible left subtrees with all possible right subtrees. Base case: n=1 returns a single leaf node.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(2^n) | O(2^n) | Recalculates same subtrees |
| Optimal (Memoized) | O(C_n) | O(C_n) | C_n is nth Catalan number |

Where C_n ≈ 4^n / (n^(3/2) * sqrt(π)) - exponential but with Memoization (Saving results to avoid repeating work) reuse
