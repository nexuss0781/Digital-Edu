...
- **Interview questions**: Tests mathematical reasoning and avoiding Brute Force (Checking every single possibility)

The key skill is recognizing when Brute Force (Checking every single possibility) (generating all possibilities) can be replaced by direct calculation using combinatorial mathematics.
...
**Time Complexity (Speed of the algorithm):** O(k²) due to list removal (can be O(k log k) with better data structure)

**Space Complexity (Memory usage of the algorithm):** O(k) for the result and remaining list
...
- [ ] Can discuss Time Complexity (Speed of the algorithm) improvement
