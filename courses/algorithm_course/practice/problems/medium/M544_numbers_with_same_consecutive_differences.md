...
When k=0, you'll generate duplicates if you add both d+0 and d-0. Handle this Edge Case (Unusual or extreme situation) by only adding one option when k=0. You can use either BFS with a queue or DFS with recursion. BFS is more straightforward as you process level by level (digit by digit).
</details>
