...
<details>
<summary>🎯 Hint 2: The Formula that defines a step based on previous steps</summary>
...
# Fill table using Formula that defines a step based on previous steps
for i in range(1, m):
    for j in range(1, n):
        dp[i][j] = dp[i-1][j] + dp[i][j-1]
...
- Much faster than Brute Force (Checking every single possibility)
...
- [ ] Applies Formula that defines a step based on previous steps correctly
...
- [ ] Comments explaining Formula that defines a step based on previous steps
