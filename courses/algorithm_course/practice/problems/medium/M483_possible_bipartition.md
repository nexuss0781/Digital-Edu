...
Use BFS with a queue or DFS with recursion - both work equally well. Track colors in an array (or dict) where color[i] is -1 (unvisited), 0 (group 0), or 1 (group 1). For each unvisited node, start a new BFS/DFS. The Time Complexity (Speed of the algorithm) is O(V + E) where V is the number of people and E is the number of dislikes.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(2^n) | O(n) | Try all possible group assignments |
