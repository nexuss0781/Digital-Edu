...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n!) | O(n) | Try all permutations of factors |
| **Backtracking** | **O(2^log n)** | **O(log n)** | Each prime factor can be split or kept, depth = number of prime factors |

The Time Complexity (Speed of the algorithm) is hard to express precisely but relates to the number of ways to factorize n.
