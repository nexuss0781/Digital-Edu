...
Use a dictionary to map values to indices for O(1) lookups. The DP state can be stored as a dictionary with keys (i, j) instead of a 2D array to save space. Only compute dp values where arr[i] + arr[j] might continue a sequence, and track the maximum as you go. Time Complexity (Speed of the algorithm): O(n^2), space: O(n^2) in worst case.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n^3 * log n) | O(1) | Try all pairs, extend greedily |
| DP with Hash Map | O(n^2) | O(n^2) | Best possible solution |
