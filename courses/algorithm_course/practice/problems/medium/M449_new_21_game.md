...
Instead of recalculating the sum of dp[i-maxPts] to dp[i-1] for each i, maintain a running window sum. Add the new element and remove the old element as you slide the window. This reduces Time Complexity (Speed of the algorithm) from O(n * maxPts) to O(n + maxPts).
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Recursion) | O(maxPts^k) | O(k) | Exponential, will timeout |
| DP with Window Sum | O(n + maxPts) | O(n) | Best possible solution |
