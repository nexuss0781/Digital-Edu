...
- Achieve `O(n log n)` Time Complexity (Speed of the algorithm)
...
Implement merge sort: recursively divide the array into halves until single elements, then merge sorted halves back together. The merge step compares elements from two sorted arrays and combines them in sorted order. This guarantees O(n log n) Time Complexity (Speed of the algorithm).
</details>
...
| Heap Sort | O(n log n) | O(1) | In-place, good Space Complexity (Memory usage of the algorithm) |
