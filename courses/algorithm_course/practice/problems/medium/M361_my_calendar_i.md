...
Time Complexity (Speed of the algorithm): O(n) per booking, where n is the number of bookings.
...
Time Complexity (Speed of the algorithm): O(log n) per booking with a balanced BST.
...
```python
# Wrong - misses Edge Case (Unusual or extreme situation) when intervals are adjacent
...
- [ ] Compare Time Complexity (Speed of the algorithm) of different approaches
