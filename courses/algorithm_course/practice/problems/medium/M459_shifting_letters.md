...
The naive approach of applying each shift[i] to positions 0..i would be O(n^2) Time Complexity (Speed of the algorithm). Notice that character at position j will be shifted by shifts[j] + shifts[j+1] + ... + shifts[n-1]. This is a suffix sum! Calculate cumulative shifts from right to left.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n^2) | O(1) | Apply each shift to all affected characters |
