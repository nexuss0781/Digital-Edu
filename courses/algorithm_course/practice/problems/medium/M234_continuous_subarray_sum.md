...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all subarrays |
| Prefix Sum Array | O(n²) | O(n) | Still check all pairs |
| Hash Map + Prefix Sum | O(n) | O(min(n, k)) | Best possible solution |
...
- [ ] Handle Edge Case (Unusual or extreme situation): [0,0] with k=1 (Day 1)
