...
Precompute prefix sums to calculate any subarray sum in O(1). This reduces Time Complexity (Speed of the algorithm) from O(n³k) to O(n²k). Consider whether space can be optimized by using only the previous row of the DP table.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(k^n) | O(n) | Exponential, impractical |
