...
The naive approach recalculates the sum from scratch each time. Instead, consider maintaining a running sum. When a new value enters the window and an old value leaves, how can you update the sum in Constant Time (Same time regardless of data size)?
</details>
...
| **Optimal (Queue + Running Sum)** | **O(1) per call** | **O(size)** | Maintain running sum, update in Constant Time (Same time regardless of data size) |
