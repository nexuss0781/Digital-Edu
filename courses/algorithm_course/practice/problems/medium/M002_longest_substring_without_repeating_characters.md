...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) (check all substrings) | O(n³) | O(min(n,m)) | Simple but unusable |
| Brute Force (Checking every single possibility) optimized | O(n²) | O(min(n,m)) | Still too slow |
