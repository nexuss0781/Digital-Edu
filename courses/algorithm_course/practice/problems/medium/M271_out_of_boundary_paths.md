...
prerequisites: ["dynamic-programming", "Memoization (Saving results to avoid repeating work)", "modular-arithmetic"]
---
...
**Approach 1: Top-Down DP with Memoization (Saving results to avoid repeating work)**
```python
...
|----------|----------------|------------------|-------|
| Top-Down DP + Memo | O(m × n × maxMove) | O(m × n × maxMove) | Memoization (Saving results to avoid repeating work) cache stores all states |
...
### Mistake 3: Not using Memoization (Saving results to avoid repeating work)
```python
# WRONG: Plain recursion without caching
def findPaths(m, n, maxMove, startRow, startColumn):
    def dp(row, col, moves_left):
        # ... recursive logic without Memoization (Saving results to avoid repeating work)
        # This recalculates same states millions of times!
```
**Why it's wrong:** Without Memoization (Saving results to avoid repeating work), the same state `(row, col, moves_left)` is computed repeatedly, leading to exponential Time Complexity (Speed of the algorithm). Always cache results in DP problems.
...
- [ ] Solve with top-down DP and Memoization (Saving results to avoid repeating work) (Day 1)
- [ ] Implement bottom-up DP approach (Day 2)
- [ ] Handle modulo arithmetic correctly (Day 2)
- [ ] Optimize Space Complexity (Memory usage of the algorithm) (Day 3)
