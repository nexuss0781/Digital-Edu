...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n * m) | O(1) | For each worker, check all jobs |
...
1. **Not maintaining maximum profit Rule that never changes**
   ```python
