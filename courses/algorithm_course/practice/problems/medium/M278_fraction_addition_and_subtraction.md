...
| Best possible solution | O(n) | O(1) | Linear scan with constant space |

**Detailed Analysis:**
- **Time**: O(n) to parse string + O(log M) for GCD where M is max value
- **Space**: O(1) auxiliary space (excluding output string)
- **Key Insight**: Each fraction operation takes Constant Time (Same time regardless of data size) since values are bounded by constraints
