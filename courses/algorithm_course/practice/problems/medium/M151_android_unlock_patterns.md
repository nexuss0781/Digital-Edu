...
prerequisites: ["backtracking", "depth-first-search", "symmetry", "Memoization (Saving results to avoid repeating work)"]
---
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DFS | O(9!) | O(9) | Try all permutations, check validity |
| **DFS with Symmetry** | **O(9!)** worst case | **O(9)** | Prune invalid paths early, use symmetry to reduce by ~8x |
| Memoization (Saving results to avoid repeating work) | O(9! × 2⁹) | O(2⁹ × 9) | Cache (current_dot, visited_mask, length) |
...
- [ ] Day 7: Add Memoization (Saving results to avoid repeating work) for repeated subproblems
