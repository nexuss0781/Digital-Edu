...
This represents the Best possible solution as no smaller good integer can be achieved.
...
Use a set to store "banned" numbers (those appearing on both sides of the same card) for O(1) lookup. Then iterate through all unique numbers from both arrays to find the minimum that isn't banned. Edge Case (Unusual or extreme situation): if all numbers are banned, return 0.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(n) | Check all possible flipping combinations |
