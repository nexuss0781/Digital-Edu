...
<details>
<summary>Hint 1: Brute Force (Checking every single possibility) to Understand the Problem</summary>

A Brute Force (Checking every single possibility) solution would check every possible subarray in nums1 against every possible subarray in nums2:
...
This gives O(n * m * min(n, m)) Time Complexity (Speed of the algorithm) - too slow for the constraints, but helps us understand the structure.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n² * m²) | O(1) | Check all pairs of subarrays |
