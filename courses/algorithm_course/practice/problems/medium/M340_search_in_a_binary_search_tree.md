...
Both have the same Time Complexity (Speed of the algorithm), but iterative uses O(1) space vs O(h) for recursion.
</details>
...
- [ ] Analyzed time/Space Complexity (Memory usage of the algorithm) for both approaches
