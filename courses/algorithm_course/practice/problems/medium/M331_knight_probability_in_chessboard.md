...
<details>
<summary>Hint 2: Dynamic Programming with Memoization (Saving results to avoid repeating work)</summary>

Use dynamic programming to avoid recalculating the same (row, col, k) states. Create a 3D Memoization (Saving results to avoid repeating work) table `dp[r][c][moves]` representing:
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) Recursion | O(8^k) | O(k) | Exponential branches, impractical for k > 20 |
| Memoization (Saving results to avoid repeating work) (Top-Down DP) | O(n² × k) | O(n² × k) | Cache all states |
| Tabulation (Building a table of results from the bottom up) (Bottom-Up DP) | O(n² × k) | O(n² × k) | Iterative table filling |
...
### Mistake 3: Incorrect Memoization (Saving results to avoid repeating work) Key
```python
# WRONG: Using only position as Memoization (Saving results to avoid repeating work) key
def knightProbability(n, k, row, column):
    memo = {}
...
**Why it's wrong**: The Memoization (Saving results to avoid repeating work) key must include all parameters that define the state: (row, col, moves). Using only (row, col) will give incorrect results because the probability depends on how many moves remain.
