...
patterns: ["dp-2d", "Memoization (Saving results to avoid repeating work)"]
...
| DFS + Memoization (Saving results to avoid repeating work) | O(k * n²) | O(k * n) | Recursion with memo, same complexity |
...
- [ ] Handle Edge Case (Unusual or extreme situation): no flights available
- [ ] Handle Edge Case (Unusual or extreme situation): single city
- [ ] Optimize space to O(n) using rolling arrays
- [ ] **Day 3**: Solve again without hints
- [ ] **Week 1**: Implement DFS + Memoization (Saving results to avoid repeating work) approach
