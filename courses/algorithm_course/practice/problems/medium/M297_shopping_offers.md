...
patterns: ["Memoization (Saving results to avoid repeating work)", "state-space-search"]
estimated_time_minutes: 30
frequency: low
related_problems: ["M322", "M377", "H072"]
prerequisites: ["dynamic-programming", "Memoization (Saving results to avoid repeating work)", "backtracking"]
...
<details>
<summary>Hint 1: State-Space Search with Memoization (Saving results to avoid repeating work)</summary>

Model this as a state-space search where each state is defined by the current needs (remaining items to purchase). For each state, try two options: (1) use each special offer if it doesn't exceed needs, recursively solve for reduced needs, or (2) buy all remaining items individually at full price. The minimum cost across all valid choices is the answer. Use Memoization (Saving results to avoid repeating work) to avoid recomputing the same state.
...
<details>
<summary>Hint 3: Memoization (Saving results to avoid repeating work) Key Design</summary>

Since needs is a list and lists aren't hashable in Python, convert needs to a tuple for use as a dictionary key. The Memoization (Saving results to avoid repeating work) dictionary maps `tuple(needs) -> minimum_cost`. This ensures that if you encounter the same needs configuration again (e.g., [2,1] reached through different offer combinations), you reuse the previously computed result.
...
|----------|----------------|------------------|-------|
| DFS + Memoization (Saving results to avoid repeating work) | O(s * k^n) | O(k^n) | s is special offers, k is max need value, n is items |
| Brute Force (Checking every single possibility) DFS | O(s^m) | O(m) | m is total items needed; exponential without Memoization (Saving results to avoid repeating work) |
...
```python
# Wrong: exponential time without Memoization (Saving results to avoid repeating work)
def dfs(needs):
...
# Correct: use Memoization (Saving results to avoid repeating work)
memo = {}
def dfs(needs):
...
- [ ] Implement DFS with Memoization (Saving results to avoid repeating work)
- [ ] Validate special offers before applying
- [ ] Convert needs list to tuple for Memoization (Saving results to avoid repeating work) key
...
- [ ] Test Edge Case (Unusual or extreme situation): no beneficial special offers
- [ ] Test Edge Case (Unusual or extreme situation): needs=[0,0,...] (return 0)
