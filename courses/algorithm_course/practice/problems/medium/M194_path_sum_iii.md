...
Since paths can start at any node, you need to consider all possible starting points. The Brute Force (Checking every single possibility) approach is to run a DFS from every node to count paths starting at that node. This gives O(n²) time. Can you do better using prefix sums like in the Subarray Sum problem?
</details>
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) DFS | O(n²) | O(h) | Check all paths from each node |
| Prefix sum with HashMap | O(n) | O(n) | Optimal - single pass with memo |
| DP with Memoization (Saving results to avoid repeating work) | O(n²) | O(n²) | Overkill, harder to implement |
...
**Mistake 3: Using Brute Force (Checking every single possibility) without optimization**
```python
...
- [ ] Day 2: Solve using Brute Force (Checking every single possibility), compare performance
