...
<details>
<summary>💡 Hint 1: Building from Previous Ugly Numbers</summary>

Every ugly number (except 1) is formed by multiplying a previous ugly number by 2, 3, or 5. If you have the sequence [1, 2, 3, 4, 5, 6], the next ugly number must be one of {2×4, 3×3, 5×2} = {8, 9, 10}. How can you systematically generate them in order?

</details>

<details>
<summary>🎯 Hint 2: Three Pointer Technique</summary>

Maintain three pointers (i2, i3, i5) that track which ugly number should be multiplied by 2, 3, and 5 next. At each step, the next ugly number is min(ugly[i2]×2, ugly[i3]×3, ugly[i5]×5). After selecting the minimum, advance the corresponding pointer(s). This is like merging three sorted sequences.
...
Note: Multiple pointers can advance simultaneously to avoid duplicates (e.g., 6 = 2×3 = 3×2).
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Check each number) | O(n·log m) | O(n) | Check if each number ≤ m is ugly |
...
```python
# Wrong: May generate duplicates like 6 (2×3 and 3×2)
def nthUglyNumber(n):
    dp = [1]
    p2 = p3 = p5 = 0
    for i in range(1, n):
...
### Mistake 2: Inefficient Brute Force (Checking every single possibility) checking
```python
