...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(m*n*(m+n)) | O(1) | Check each pixel's row and column individually |
| Preprocessing counts | O(m*n) | O(m+n) | Best possible solution; one pass to count, one to validate |
