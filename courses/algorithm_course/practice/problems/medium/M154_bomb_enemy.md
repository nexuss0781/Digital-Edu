...
The Brute Force (Checking every single possibility) approach places a bomb at each empty cell and counts enemies in all four directions, taking O(mn) time per cell for O(m²n²) total. Can you precompute information to avoid recounting enemies for every position?
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(m²n²) | O(1) | Try each cell, scan all 4 directions |
...
**Mistake 1: Brute Force (Checking every single possibility) for Every Cell**
```python
