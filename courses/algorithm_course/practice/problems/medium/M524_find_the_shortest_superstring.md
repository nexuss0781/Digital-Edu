...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (All Permutations) | O(n! × n × m) | O(n × m) | m = avg word length |
...
2. **Missing Edge Case (Unusual or extreme situation) where one word contains another**
   ```python
