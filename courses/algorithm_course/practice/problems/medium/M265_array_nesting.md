...
Space Complexity (Memory usage of the algorithm): O(1) by modifying the array. If you can't modify, use O(n) visited set.
</details>
...
| Brute Force (Checking every single possibility) | O(n²) | O(n) | Try each starting point without marking |
...
- [ ] Handle Edge Case (Unusual or extreme situation): array [0] (self-loop)
- [ ] Handle Edge Case (Unusual or extreme situation): array [1, 0] (cycle of 2)
