...
**Why it's wrong**: This performs a linear search through the entire employee list for every single employee in the hierarchy, resulting in O(n²) Time Complexity (Speed of the algorithm). Always build a hash map first for O(1) lookups.
