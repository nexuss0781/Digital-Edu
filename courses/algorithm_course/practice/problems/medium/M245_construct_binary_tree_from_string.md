...
An important Edge Case (Unusual or extreme situation) is negative numbers: "-4(2)(3)" has root value -4 (negative four), not 4. You'll need to parse multi-digit numbers and handle the negative sign properly. Empty parentheses like "()(3)" are not used in this format - if a parent has only a right child, you'd still represent it with an empty left subtree using "4()(3)", though this problem typically avoids that pattern.
