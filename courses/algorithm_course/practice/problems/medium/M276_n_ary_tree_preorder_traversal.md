...
**Why reverse?** The stack is LIFO (Last-In-First-Out, like a stack of plates) (Last In, First Out). If we push children left-to-right, the rightmost child would be processed first. Reversing ensures left-to-right processing.
...
for child in reversed(node.children):
    stack.append(child)
```
**Why it's wrong:** Stack is LIFO (Last-In-First-Out, like a stack of plates). Without reversing, children are processed in reverse order (right-to-left instead of left-to-right).
