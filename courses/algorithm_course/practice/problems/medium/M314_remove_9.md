...
The naive approach of iterating through all integers and checking each for the digit 9 will timeout for large `n` values (up to 800 million). The key insight involves recognizing a mathematical pattern related to number base systems that allows direct computation in Logarithmic Time (Time grows slowly as data grows).
