...
Time Complexity (Speed of the algorithm): O(n²) - each expansion can go up to n steps.
...
The expand-around-center approach is generally preferred for better Space Complexity (Memory usage of the algorithm), but DP is useful if you need to query specific substrings later.
...
| Brute Force (Checking every single possibility) | O(n³) | O(1) | Check all substrings, too slow |
