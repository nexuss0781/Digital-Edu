...
Use a hash map to store numbers and their frequencies. This allows O(1) add operations. For find(value), iterate through the hash map and check if (value - number) exists. Be careful with the Edge Case (Unusual or extreme situation) where number + number = value (need frequency >= 2).
...
|----------|-----------------|-------|-------|
| List Storage | O(1) / O(n²) | O(n) | Brute Force (Checking every single possibility): check all pairs |
...
- [ ] Can discuss time/Space Complexity (Memory usage of the algorithm) and trade-offs
