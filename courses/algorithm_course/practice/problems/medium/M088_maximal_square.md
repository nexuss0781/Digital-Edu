...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(m²n² × min(m,n)) | O(1) | Check every cell as top-left, validate all squares |
| DP (2D table) | O(mn) | O(mn) | Build table bottom-up |
| **DP (1D optimized)** | **O(mn)** | **O(n)** | Only need previous row, Best possible solution |
