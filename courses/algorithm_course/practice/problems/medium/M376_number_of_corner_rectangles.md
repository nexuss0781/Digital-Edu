...
Time Complexity (Speed of the algorithm): O(m² * n) where m is rows, n is columns.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (check all 4-tuples) | O(m² * n²) | O(1) | Check every possible rectangle |
...
| Optimized Row Pair | O(m² * n) | O(1) | Most straightforward Best possible solution |
...
- [ ] Review after 3 days
- [ ] Review after 1 week
- [ ] Can explain Time Complexity (Speed of the algorithm) tradeoffs
