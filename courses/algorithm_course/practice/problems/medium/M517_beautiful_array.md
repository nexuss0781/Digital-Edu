...
Start with base case [1]. To build size n: take beautiful array for n/2, transform elements as 2*x-1 for odds, then append 2*x for evens. The linear transformation preserves the beautiful property. Use Memoization (Saving results to avoid repeating work) or build iteratively from size 1 upward.
</details>
