...
Edge Case (Unusual or extreme situation): If a string appears multiple times (duplicates), it cannot be uncommon because any of its subsequences will also be subsequences of its duplicate.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (All Subsequences) | O(n × 2^m) | O(2^m) | m = max string length, too slow |
...
- [ ] Handle Edge Case (Unusual or extreme situation): all identical strings (Day 1)
