...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n³) | O(n) | Generate all substrings, count unique chars each |
| Optimized Brute Force (Checking every single possibility) | O(n²) | O(n) | Use sliding window for each start position |
