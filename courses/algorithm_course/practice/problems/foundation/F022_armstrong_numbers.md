...
An **Armstrong number** (also called narcissistic number or pluperfect digital Rule that never changes) is a number that equals the sum of its own digits each raised to the power of the number of digits.
...
| Perfect digit-to-digit Rule that never changes | Each digit raised to itself | 3435 = 3³ + 4⁴ + 3³ + 5⁵ |
