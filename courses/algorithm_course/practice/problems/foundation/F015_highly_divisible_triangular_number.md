...
This problem teaches you that mathematical structure (formulas, multiplicative properties) can dramatically improve algorithm efficiency over Brute Force (Checking every single possibility).
...
**Time Complexity (Speed of the algorithm):** O(√n) per triangular number. For large triangular numbers, this is still slow.
