...
The Brute Force (Checking every single possibility) approach:
...
Use mathematical formulas for Constant Time (Same time regardless of data size) computation:
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) (Loop) | O(n) | O(1) | Simple iteration, slow for large n |
