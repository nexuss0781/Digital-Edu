...
**Time Complexity (Speed of the algorithm)**: O(log n) per step, O(?) steps (bounded by cycle length)
**Space Complexity (Memory usage of the algorithm)**: O(number of unique values seen)
...
**Time Complexity (Speed of the algorithm)**: O(log n) per step, O(cycle length) steps
**Space Complexity (Memory usage of the algorithm)**: O(1) — no hash set needed!
