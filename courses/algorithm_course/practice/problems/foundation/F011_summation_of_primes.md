...
The Time Complexity (Speed of the algorithm) O(n log log n) is remarkably efficient—almost linear—and demonstrates that clever marking strategies can outperform naive testing approaches.
...
1. Could you use the trial division approach from F017? What would the Time Complexity (Speed of the algorithm) be?
...
- Reduces Space Complexity (Memory usage of the algorithm) to O(√n)
...
- [ ] Can explain the O(n log log n) Time Complexity (Speed of the algorithm)
