...
The challenge here is efficiency: a Brute Force (Checking every single possibility) approach checking all possible triplets would take cubic time. However, by sorting the array first and using the two-pointer technique, you can reduce this to Quadratic Time (Time grows very fast with data, like a square). The key insight is that after fixing the first element, the remaining two elements form a Two Sum variant that can be solved with two pointers moving from opposite ends.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (3 Loops) | O(n³) | O(1) | Check all triplet combinations |
