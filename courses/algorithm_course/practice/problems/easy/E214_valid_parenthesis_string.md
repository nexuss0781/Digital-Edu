...
|----------|----------------|------------------|-------|
| Greedy Range Tracking | O(n) | O(1) | Best possible solution |
| Two-Pass Greedy | O(n) | O(1) | Alternative greedy approach |
| Dynamic Programming | O(n³) | O(n²) | Works but slower |
| Recursion with Memoization (Saving results to avoid repeating work) | O(n²) | O(n²) | Better DP approach |
