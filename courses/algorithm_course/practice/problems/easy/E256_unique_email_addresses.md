...
| Brute Force (Checking every single possibility) (Compare All Pairs) | O(n² * m) | O(1) | Inefficient, not recommended |

**Best possible solution**: Hash Set approach provides O(n * m) time where you process each email once.
