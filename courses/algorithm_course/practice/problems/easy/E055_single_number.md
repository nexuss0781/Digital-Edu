...
The challenge here is not just finding the answer, but doing it efficiently. You need to solve this in Linear Time (Time grows directly with data) O(n) and use only constant extra space O(1). This means you cannot use a hash map or set to track which numbers you've seen, as that would use O(n) space.
