...
The solution must achieve Logarithmic Time (Time grows slowly as data grows) Complexity (Speed of the algorithm) `O(log n)`.
...
|----------|----------------|------------------|-------|
| Binary search (iterative) | O(log n) | O(1) | Best possible solution |
...
- [ ] Week 2: Explain loop Rule that never changes to someone
