...
The technique appears in countless interview problems. The pattern is simple: start pointers at opposite ends, move them toward each other based on a comparison. This reduces O(n²) Brute Force (Checking every single possibility) to O(n) by leveraging the sorted property to eliminate impossible pairs.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all pairs |
