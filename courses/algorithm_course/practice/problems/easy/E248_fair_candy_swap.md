...
This gives O(n + m) Time Complexity (Speed of the algorithm) where n and m are array lengths.

## Complexity Analysis

| Approach | Time | Space | Notes |
|----------|------|-------|-------|
| Hash set lookup | O(n + m) | O(m) | Best possible solution, store Bob's sizes in set |
| Brute Force (Checking every single possibility) | O(n × m) | O(1) | Try all pairs, inefficient |
