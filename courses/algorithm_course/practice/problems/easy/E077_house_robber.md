...
- The Best possible solution might not include the first or last house
...
**Mistake 1: Incorrect Formula that defines a step based on previous steps**
...
```python
# Wrong: Inefficient recursive solution without Memoization (Saving results to avoid repeating work)
def rob(nums):
    def helper(i):
        if i < 0:
            return 0
...
- [ ] Day 3: Try recursive approach with Memoization (Saving results to avoid repeating work)
- [ ] Week 1: Solve House Robber II (circular variant)
- [ ] Week 2: Explain the Formula that defines a step based on previous steps clearly
