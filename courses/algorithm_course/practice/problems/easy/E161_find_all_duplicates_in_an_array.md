...
The challenge here is achieving this with strict efficiency requirements: your solution must run in **O(n) time** (Linear Time (Time grows directly with data)) and use only **O(1) extra space** (constant additional memory beyond the output array).
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (nested loops) | O(n^2) | O(1) | Check each element against all others |
