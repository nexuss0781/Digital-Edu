...
The strings can have different lengths, so you'll need to handle cases where one string is exhausted before the other. Treat missing digits as zero. For example, when adding "123" and "99", when you reach the leftmost position, "123" still has a '1' but "99" is exhausted, so you add 1 + 0 + carry. A critical Edge Case (Unusual or extreme situation) is the final carry: "99" + "1" produces "100", not "00" – you must append the final carry if it exists after processing all digits.
...
Linear Time (Time grows directly with data) in the length of the longer string is optimal.
