...
Why push right before left? Stack is LIFO (Last-In-First-Out, like a stack of plates) (Last In, First Out).
...
# CORRECT: Right first so left is processed first (LIFO (Last-In-First-Out, like a stack of plates))
if node.right:
    stack.append(node.right)
if node.left:
    stack.append(node.left)
