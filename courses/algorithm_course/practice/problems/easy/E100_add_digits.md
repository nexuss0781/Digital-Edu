...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Simulation) | O(log n) | O(1) | Iterate until single digit |
...
```python
# CORRECT: Handle the Edge Case (Unusual or extreme situation) where num % 9 == 0
def addDigits(num):
    if num == 0:
        return 0
