...
### Beginner Hint
The Brute Force (Checking every single possibility) approach is to use two nested loops checking every pair (i, j) where i < j and nums[i] > 2 * nums[j]. Count all such pairs. This gives O(n²) Time Complexity (Speed of the algorithm) but helps understand the problem pattern.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Two Loops) | O(n²) | O(1) | Check all pairs directly |
