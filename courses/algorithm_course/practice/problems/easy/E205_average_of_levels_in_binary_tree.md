...
Alternatively, you can solve this with depth-first search (DFS) by tracking the depth parameter and maintaining arrays for sum and count at each level. Both approaches are O(n) time where n is the number of nodes, but they differ in Space Complexity (Memory usage of the algorithm) based on tree shape.
