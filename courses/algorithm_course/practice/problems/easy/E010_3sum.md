...
This problem combines multiple fundamental techniques: **sorting + two pointers + duplicate elimination**. It's a staple interview question that tests your ability to optimize Brute Force (Checking every single possibility) solutions and handle edge cases.
...
4. What's the Time Complexity (Speed of the algorithm) of checking all triplets? Can sorting help reduce this?
...
<details>
<summary>💡 Hint 1: How many triplets are there?</summary>

The Brute Force (Checking every single possibility) approach uses three nested loops to check every possible triplet:
```
for i in range(n):
...
This is O(n³) Time Complexity (Speed of the algorithm). With n up to 3000, that's up to 27 billion operations!
...
<details>
<summary>🎯 Hint 2: Reduce 3Sum to 2Sum</summary>
...
<details>
<summary>📝 Hint 3: Sort + Two Pointers algorithm</summary>
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) | O(n³) | O(1) | Check all triplets; impractical for n=3000 |
...
- Space Complexity (Memory usage of the algorithm) is just sorting overhead (O(log n) to O(n) depending on sort algorithm)
...
- [ ] Space Complexity (Memory usage of the algorithm) is O(log n) to O(n) (sorting)
...
- [ ] Can derive Time Complexity (Speed of the algorithm) from first principles
