...
The key insight is that you don't need to track all possible subsequences. When building a wiggle sequence, at each position you only care whether you're currently looking for the next element to go up or down. This state-based thinking leads to an elegant greedy solution that runs in Linear Time (Time grows directly with data) with constant space.
...
Can you reduce the Space Complexity (Memory usage of the algorithm) of the DP approach to O(1)?
