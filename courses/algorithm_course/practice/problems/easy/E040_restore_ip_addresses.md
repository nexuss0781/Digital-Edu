...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(3^4 × n) | O(n) | Try all 3^4 partitions, validate each |
| **Backtracking** | **O(3^4)** | **O(1)** | **Constant partitions (4 segments), early Cutting off unnecessary steps** |
...
### 3. Inefficient Cutting off unnecessary steps
```python
...
# CORRECT: Early Cutting off unnecessary steps
def backtrack(start, segments, path):
...
- [ ] Can explain Cutting off unnecessary steps strategies
