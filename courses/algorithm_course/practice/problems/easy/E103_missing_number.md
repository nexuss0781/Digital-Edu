...
Both approaches run in Linear Time (Time grows directly with data) with constant space, but XOR is more elegant and avoids potential integer overflow issues that can occur when summing large numbers.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Check Each) | O(n²) | O(1) | Search for each i in array |
