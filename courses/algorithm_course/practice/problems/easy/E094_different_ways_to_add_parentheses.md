...
patterns: ["divide-and-conquer", "Memoization (Saving results to avoid repeating work)"]
...
This is a medium-difficulty problem that frequently appears in interviews to test recursive thinking and understanding of expression evaluation. It also demonstrates the value of Memoization (Saving results to avoid repeating work): without caching results, you'd recompute the same subexpressions many times.
...
<details>
<summary>📝 Hint 3: Implementation with Memoization (Saving results to avoid repeating work)</summary>

Optimal approach with Memoization (Saving results to avoid repeating work):
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Generate All | O(2^n) | O(2^n) | Generate all parenthesizations explicitly |
| **Divide & Conquer** | **O(C(n))** | **O(C(n))** | C(n) is nth Catalan number, roughly O(4^n/n^(3/2)) |
| With Memoization (Saving results to avoid repeating work) | O(C(n)) | O(C(n)) | Avoids redundant subproblem computation |

**Note:** The number of ways to parenthesize n operators is the nth Catalan number.

**Optimal approach:** Divide and conquer with Memoization (Saving results to avoid repeating work) avoids recomputing subexpressions.
...
- [ ] **Day 3:** Add Memoization (Saving results to avoid repeating work) for optimization
