...
This problem can be approached using either breadth-first search (BFS), which naturally processes nodes level by level, or depth-first search (DFS), where you track the current depth as you traverse. An important Edge Case (Unusual or extreme situation) to consider: what should you return if the tree is empty?
...
- [ ] Handle empty tree Edge Case (Unusual or extreme situation)
