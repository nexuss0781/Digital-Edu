...
### Beginner Hint
Use backtracking to try all 2^n combinations of + and - signs. For each element, recursively try both adding and subtracting it. Count solutions that reach the target. This gives correct results but has exponential Time Complexity (Speed of the algorithm).
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Backtracking) | O(2^n) | O(n) | Try all combinations of signs |
| Recursion + Memoization (Saving results to avoid repeating work) | O(n * sum) | O(n * sum) | Cache (index, current_sum) states |
