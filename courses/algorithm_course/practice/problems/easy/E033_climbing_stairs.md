...
This creates a Formula that defines a step based on previous steps, but a naive recursive solution would recalculate the same values repeatedly, leading to exponential Time Complexity (Speed of the algorithm). You need to recognize the pattern and apply an efficient approach.
...
Key insight: This is a classic Formula that defines a step based on previous steps. Can you express ways(n) in terms of ways(n-1) and ways(n-2)?
...
- Memoization (Saving results to avoid repeating work) (Linear Time (Time grows directly with data), linear space)
- Bottom-up DP (Linear Time (Time grows directly with data), linear space)
- Space-optimized (Linear Time (Time grows directly with data), constant space)
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(2ⁿ) | O(n) | Exponential branching, call stack |
| Memoization (Saving results to avoid repeating work) (Top-Down DP) | O(n) | O(n) | Cache results, recursion stack |
| Tabulation (Building a table of results from the bottom up) (Bottom-Up DP) | O(n) | O(n) | Array to store all subproblems |
| **Space-Optimized DP** | **O(n)** | **O(1)** | Only track last two values |
| Matrix Exponentiation | O(log n) | O(1) | Advanced technique |

## Common Mistakes

### 1. Exponential Recursion Without Memoization (Saving results to avoid repeating work)
```python
...
# CORRECT: Add Memoization (Saving results to avoid repeating work)
