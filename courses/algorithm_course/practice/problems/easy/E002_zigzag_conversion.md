...
Reading row by row gives "PINALSIGYAHRPI". Your task is to return the string obtained by reading the zigzag pattern row by row. Edge Case (Unusual or extreme situation): if there's only 1 row, the string stays unchanged.
...
**Edge Case (Unusual or extreme situation):** When numRows = 1, return the original string.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (2D array) | O(n) | O(n) | Create full zigzag grid |
...
### 2. Not handling Edge Case (Unusual or extreme situation) of single row
```python
...
# CORRECT: Handle Edge Case (Unusual or extreme situation) upfront
if numRows == 1:
    return s
```
