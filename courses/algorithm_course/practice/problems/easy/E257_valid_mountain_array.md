...
|----------|----------------|------------------|-------|
| Single Pass (Two Pointers) | O(n) | O(1) | Best possible solution, visit each element once |
...
**Best possible solution**: Single pass with two phases (climb up, climb down) achieves O(n) time with O(1) space.
