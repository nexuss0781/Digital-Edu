...
This problem introduces the fundamental concept of binary search and the "lower bound" pattern - finding the first position where a value could be placed. Your algorithm must achieve O(log n) Time Complexity (Speed of the algorithm) by repeatedly dividing the search space in half, rather than scanning linearly through the array.
...
- **Rule that never changes maintenance**: Preserving sorted order
...
4. What Rule that never changes does binary search maintain about left and right pointers?
...
```python
# Edge Case (Unusual or extreme situation): target smaller than all elements
nums = [1, 3, 5], target = 0  # Should return 0

# Edge Case (Unusual or extreme situation): target larger than all elements
...
**Optimization:**
- [ ] Achieved O(log n) Time Complexity (Speed of the algorithm)
- [ ] Used O(1) space
