...
### Tier 1: Brute Force (Checking every single possibility) Search
For each element in nums1, you need to find where that same value appears in nums2. The simplest approach is to search through nums2 for each element. What's the Time Complexity (Speed of the algorithm) of this approach? How many comparisons do you make in the worst case?

### Tier 2: Hash Map for Fast Lookup
Instead of searching for each element, what if you could look up its position in Constant Time (Same time regardless of data size)? Think about preprocessing nums2 to build a data structure that maps each value to its index. What data structure provides O(1) lookup?
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Nested Loop) | O(n²) | O(n) | For each element, search entire nums2 |
...
- [ ] Solved with Brute Force (Checking every single possibility) approach
- [ ] Optimized to O(n) with hash map
- [ ] Handled Edge Case (Unusual or extreme situation): single element arrays
- [ ] Handled Edge Case (Unusual or extreme situation): all elements are the same
- [ ] Handled Edge Case (Unusual or extreme situation): duplicate values
