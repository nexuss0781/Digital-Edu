...
A critical Edge Case (Unusual or extreme situation): there might be multiple elements tied for the maximum frequency. You need to consider all of them and find which one appears in the smallest span from its first to last occurrence. Also note that the answer is asking for the length of the subarray, not the subarray itself.
...
| Brute Force (Checking every single possibility) subarray | O(n³) | O(n) | Check all subarrays, count frequencies |
...
- Single pass is optimal for Time Complexity (Speed of the algorithm)
