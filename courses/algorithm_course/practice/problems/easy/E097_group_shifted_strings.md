...
An important Edge Case (Unusual or extreme situation) to handle is wraparound. When you compute the difference from 'z' to 'a', it's not -25 but +1 (wrapping around the alphabet). You need to normalize these differences using modulo 26 to ensure "za" and "ab" are recognized as the same pattern.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Compare All Pairs) | O(n² × m) | O(n × m) | n strings, m average length |
...
### Mistake 3: Single-Character Edge Case (Unusual or extreme situation)
