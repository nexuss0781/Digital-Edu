...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (All Subsequences) | O(2^n * n) | O(2^n * n) | Generate all, filter valid ones |
...
| Optimal (Level-wise Cutting off unnecessary steps) | O(2^n) | O(n) | Recursion depth, minimal extra space |
...
### Mistake 3: Not Cutting off unnecessary steps duplicate values at same level
```python
