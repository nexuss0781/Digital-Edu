...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(nm log(nm)) | O(nm) | Generate all pairs and sort |
...
**Why it's wrong:** You're generating all n × m pairs, which defeats the efficiency of the heap approach. This is essentially Brute Force (Checking every single possibility) with extra steps.
