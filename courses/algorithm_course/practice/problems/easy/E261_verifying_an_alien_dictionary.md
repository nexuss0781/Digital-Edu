...
Given an array of `words` and the alien alphabet `order`, return `true` if the words are properly sorted according to the alien alphabet, and `false` otherwise. An important Edge Case (Unusual or extreme situation) to consider: if "apple" appears before "app" in the list, this violates sorting order because "app" should come first (being a prefix).
...
**Best possible solution**: Hash map approach achieves O(n * m) time with O(1) space.
...
- [ ] Correctly handled prefix Edge Case (Unusual or extreme situation)
