...
### Hint 1: Brute Force (Checking every single possibility) - Try All Subsets
...
**Limitations**: Time Complexity (Speed of the algorithm) O(2^n * L) where n is number of strings and L is average string length. Exponential and impractical for n > 20.

### Hint 2: Recursive with Memoization (Saving results to avoid repeating work)
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (All subsets) | O(2^len(strs) * L) | O(len(strs)) | L is average string length, exponential time |
| Recursive with Memoization (Saving results to avoid repeating work) | O(len(strs) * m * n) | O(len(strs) * m * n) | 3D Memoization (Saving results to avoid repeating work) table |
...
- [ ] Add Memoization (Saving results to avoid repeating work) to recursion
