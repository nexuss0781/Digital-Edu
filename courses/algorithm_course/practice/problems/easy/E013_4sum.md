...
<details>
<summary>💡 Hint 1: Reduce to 2Sum</summary>
...
<details>
<summary>🎯 Hint 2: Sort + Nested Loops + Two Pointers</summary>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n⁴) | O(1) | Check all 4-tuples |
| Hash Map (pairs) | O(n²) | O(n²) | Store all pairs, check complements |
| **Sort + Two Pointers** | **O(n³)** | **O(1) or O(n)** | Two nested loops + two pointers |

Space Complexity (Memory usage of the algorithm): O(1) if sorting in-place, O(n) if creating sorted copy
