...
Think about what data structure allows you to check membership in Constant Time (Same time regardless of data size). That's your clue to solving this efficiently in a single pass through the array.
...
The key question is: have we seen this number before? A data structure that efficiently tracks "seen" elements and checks membership in Constant Time (Same time regardless of data size) would be ideal. Think about what structure allows O(1) lookup.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Compare each pair of elements |
...
```python
# Wrong: O(n²) Time Complexity (Speed of the algorithm)
