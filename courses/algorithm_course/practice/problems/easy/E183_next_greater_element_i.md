...
### Beginner Hint
For each element in nums1, find its position in nums2, then scan to the right in nums2 to find the first greater element. Store results in an array. This Brute Force (Checking every single possibility) approach works but has O(m * n) complexity where m and n are the lengths of the arrays.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Nested Loops) | O(m * n) | O(1) | For each nums1 element, scan nums2 |
