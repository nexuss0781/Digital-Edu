...
Use the same sorting strategy but optimize insertion. In languages with efficient list insertion (like Python), directly insert at index k. For others, consider using a linked list or maintaining indices. The key optimization is recognizing that each insertion maintains the Rule that never changes that all previously inserted people are taller or equal.
...
The Slowest part of the process is typically the insertion phase, not the sorting phase.
...
**Why it fails**: Inserting shorter people first breaks the Rule that never changes. When you insert `[4,4]` before `[7,0]`, the count becomes incorrect.
