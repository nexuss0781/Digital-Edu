...
patterns: ["fibonacci", "Memoization (Saving results to avoid repeating work)", "space-optimization"]
...
| Recursion + Memoization (Saving results to avoid repeating work) | O(n) | O(n) | Each subproblem computed once, stored |
...
### Mistake 1: Naive recursion without Memoization (Saving results to avoid repeating work)
```python
# Wrong: Exponential Time Complexity (Speed of the algorithm)
def fib(n):
    if n <= 1:
        return n
...
### Mistake 2: Using global Memoization (Saving results to avoid repeating work) without cleanup
```python
...
```
**Why it's wrong**: While Memoization (Saving results to avoid repeating work) works, using global state can cause issues in testing or when the function is called multiple times with different inputs. Better to encapsulate.
...
**After 1 Day**
- [ ] Implement recursion with Memoization (Saving results to avoid repeating work) (top-down DP)
- [ ] Implement iterative DP with array (bottom-up)
- [ ] Can you explain why Memoization (Saving results to avoid repeating work) helps?
