...
**Time Complexity (Speed of the algorithm):** O(N² × 4 × K) = O(N² × K)
...
|----------|------|-------|-----------|
| **Brute Force (Checking every single possibility) (all directions)** | **O(N² × K)** | **O(1)** | Standard solution; checks all positions |
...
**Why Brute Force (Checking every single possibility) is Fine:**
...
- [ ] O(N² × K) Time Complexity (Speed of the algorithm)
