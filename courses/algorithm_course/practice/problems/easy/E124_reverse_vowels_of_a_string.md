...
Initialize vowels = set('aeiouAEIOU'), convert s to list (for mutability), left=0, right=len(s)-1. While left < right: if s[left] not in vowels, increment left; elif s[right] not in vowels, decrement right; else swap s[left] and s[right], increment left, decrement right. Return ''.join(s). Each character is visited at most once, giving O(n) Time Complexity (Speed of the algorithm).
...
| Brute Force (Checking every single possibility) (find all vowels) | O(n^2) | O(n) | Inefficient repeated searching |
