...
### Hint 1: Brute Force (Checking every single possibility) - Generate All Substrings
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Generate all) | O(n³) | O(n²) | Generate O(n²) substrings, validate each in O(n), store in set |
...
- [ ] Implement Brute Force (Checking every single possibility) with set
