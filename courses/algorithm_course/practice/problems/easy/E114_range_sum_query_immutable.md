...
Since sumRange can be called up to 10,000 times but the array is initialized only once, it makes sense to invest time during construction to make queries faster. What information can you precompute to answer any range sum query in Constant Time (Same time regardless of data size)?
...
| **Prefix Sum** | **O(1) per query** | **O(n)** | Optimal: O(n) preprocessing, Constant Time (Same time regardless of data size) queries |
...
- [ ] Handle Edge Case (Unusual or extreme situation): single element array (5 min)
