...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (sort each window) | O(n * m log m) | O(m) | m = len(p), n = len(s) |
...
for i in range(len(s) - len(p) + 1):
    ...
    if window_freq == p_freq:
        result.append(i)
```
**Why it fails**: O(n * m) Time Complexity (Speed of the algorithm) instead of O(n).
