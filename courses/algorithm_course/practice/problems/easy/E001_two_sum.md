...
The Brute Force (Checking every single possibility) checks every pair of numbers. For each element at index `i`, you check every element at index `j > i`.

**Think about:**

- How many pairs are there in an array of n elements?
- What's the Time Complexity (Speed of the algorithm) of this approach?
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Simple but slow; checks all pairs |
