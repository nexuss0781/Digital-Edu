...
**Time Complexity (Speed of the algorithm):** O(n log n) dominated by sorting

**Space Complexity (Memory usage of the algorithm):** O(n) for sorted array (or O(1) if sorting in-place)
