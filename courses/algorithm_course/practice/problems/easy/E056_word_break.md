...
Time Complexity (Speed of the algorithm): O(n² × m) where m is average word length
Space Complexity (Memory usage of the algorithm): O(n + w × m) where w is number of words
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(2^n) | O(n) | Try all possible segmentations recursively |
