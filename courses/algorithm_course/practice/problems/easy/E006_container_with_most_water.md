...
The Brute Force (Checking every single possibility) approach checks all possible pairs, giving O(n²) time. However, there's a clever O(n) solution using the two-pointer technique.
...
**Core concept**: Sometimes the Best possible solution requires eliminating impossible candidates rather than trying everything. The two-pointer approach proves that local greedy decisions (moving the shorter line) lead to the global optimum.
...
4. What Rule that never changes can we maintain that guarantees we don't miss the maximum?
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Check all pairs; simple but slow |
...
**Optimization:**
- [ ] Achieved O(n) Time Complexity (Speed of the algorithm)
- [ ] O(1) Space Complexity (Memory usage of the algorithm)
...
- [ ] Can discuss tradeoffs with Brute Force (Checking every single possibility)
