...
### Beginner Hint
Think recursively: at each turn, a player chooses left or right. The score difference for the current player is the chosen value minus the best the opponent can achieve on the remaining array. Use Memoization (Saving results to avoid repeating work) to avoid recalculating subproblems.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (Try All) | O(2^n) | O(n) | Recursive tree without Memoization (Saving results to avoid repeating work) |
| Recursion + Memoization (Saving results to avoid repeating work) | O(n²) | O(n²) | Cache results for each subarray |
...
- [ ] Implemented DP solution with Memoization (Saving results to avoid repeating work)
- [ ] All test cases passing
- [ ] Analyzed time and Space Complexity (Memory usage of the algorithm)
