...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) (compare all pairs) | O(n² * k) | O(n * k) | n=strings, k=max length; too slow |
...
```
# WRONG: Might fail on empty string Edge Case (Unusual or extreme situation)
