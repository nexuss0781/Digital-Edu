...
| **Hash Map of Sets** | **O(1) per query** | **O(n * m)** | Optimal: O(n * m) preprocessing, Constant Time (Same time regardless of data size) queries |
...
- [ ] Handle Edge Case (Unusual or extreme situation): words with length <= 2 (5 min)
- [ ] Handle Edge Case (Unusual or extreme situation): duplicate words in dictionary (5 min)
