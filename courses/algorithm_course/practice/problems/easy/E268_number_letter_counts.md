...
| Cache Results | O(N) + Memoization (Saving results to avoid repeating work) | O(N) | Useful if recalculating same numbers |
...
- Constant Time (Same time regardless of data size) per number: O(1) per conversion
...
- [ ] O(N) total Time Complexity (Speed of the algorithm)
