...
This problem introduces square root optimization, a fundamental technique for improving algorithms that involve divisor enumeration, factorization, or primality testing. The insight that divisors occur in pairs (d, n/d) is crucial for reducing Time Complexity (Speed of the algorithm) from O(n) to O(√n)—a massive improvement for large numbers. This same pattern appears in prime checking, finding all factors, computing greatest common divisors, and cryptographic algorithms.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n) | O(1) | Check all numbers from 1 to n-1 |
...
**Initial Practice**
- [ ] Solve with Brute Force (Checking every single possibility) O(n) approach
...
- [ ] Avoid double-counting Edge Case (Unusual or extreme situation)
