...
### Hint 1: Brute Force (Checking every single possibility) - Try All Pattern Lengths
...
**Limitations**: Time Complexity (Speed of the algorithm) O(n²) because for each potential pattern length, you construct and compare strings.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) (All lengths) | O(n²) | O(n) | Check n/2 patterns, each requires O(n) string construction |
...
- [ ] Implement Brute Force (Checking every single possibility) solution
