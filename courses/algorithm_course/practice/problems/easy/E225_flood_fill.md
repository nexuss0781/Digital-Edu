...
Many candidates fail this problem by not handling the same-color Edge Case (Unusual or extreme situation), not properly checking bounds before accessing array elements, or marking cells as visited after changing color (causing revisits). Mastering flood fill gives you a mental template for all connected-component grid problems.
...
- Space Complexity (Memory usage of the algorithm) depends on connectivity of same-colored region
...
- [ ] Handle same-color Edge Case (Unusual or extreme situation)
