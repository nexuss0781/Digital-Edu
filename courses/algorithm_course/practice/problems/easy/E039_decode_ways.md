...
|----------|------|-------|-------|
| Recursive (Brute Force (Checking every single possibility)) | O(2^n) | O(n) | Try all combinations, exponential |
| Recursive + Memoization (Saving results to avoid repeating work) | O(n) | O(n) | Cache subproblem results |
