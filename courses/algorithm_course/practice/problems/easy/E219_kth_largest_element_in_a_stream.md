...
The key challenge here is efficiency. With potentially 10,000 additions, you cannot afford to sort the entire collection each time. Consider that after each addition, you only care about the top k elements, not the entire sorted list. Also, an important Edge Case (Unusual or extreme situation): the initial array might have fewer than k elements, but you're guaranteed that by the time you call `add`, there will be at least k total elements.
...
- [ ] Can prove O(k) Space Complexity (Memory usage of the algorithm)
