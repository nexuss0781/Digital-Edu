...
1. What's the Brute Force (Checking every single possibility) approach? How can you optimize it?
...
<details>
<summary>💡 Hint 1: Brute Force (Checking every single possibility) baseline</summary>
...
- For each window, you multiply k digits: what's the Time Complexity (Speed of the algorithm)?
...
|----------|------|-------|-----------|
| Brute Force (Checking every single possibility) | O(n*k) | O(1) | Simple but recalculates products |
...
- Constant Time (Same time regardless of data size) to slide window (remove left, add right)
