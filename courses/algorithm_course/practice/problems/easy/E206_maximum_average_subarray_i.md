...
This sliding window technique reduces Time Complexity (Speed of the algorithm) from O(n × k) to O(n). Calculate the sum of the first k elements, then slide the window one position at a time, maintaining a running sum. Track the maximum sum throughout, and at the end, divide by k to get the maximum average. Note that you only need to divide once - working with sums during iteration avoids repeated floating-point operations.
...
### Hint 1: Brute Force (Checking every single possibility) Foundation
Think about calculating the sum of every possible k-length subarray. For each starting position, sum k consecutive elements and track the maximum. What's the Time Complexity (Speed of the algorithm)? Consider how many subarrays exist and what operation you perform for each.
...
|----------|----------------|------------------|-------|
| Brute Force (Checking every single possibility) | O(n * k) | O(1) | Calculate sum for each of n-k+1 windows |
...
- [ ] Solve using Brute Force (Checking every single possibility) (understand the problem)
- [ ] Optimize to sliding window approach
- [ ] Handle edge cases (k=1, k=n, single element)
- [ ] Implement without bugs on first try
- [ ] Explain time/Space Complexity (Memory usage of the algorithm) clearly
