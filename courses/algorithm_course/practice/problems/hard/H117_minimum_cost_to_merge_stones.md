...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(k^n) | O(n) | Exponential; try all merge sequences |
| DP with Memoization (Saving results to avoid repeating work) | O(n³ * k) | O(n² * k) | 3D DP table |
