...
Use dynamic programming with Memoization (Saving results to avoid repeating work):
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DFS | O(m^n) | O(n) | m positions per character, n key length - TLE |
| DP with Memoization (Saving results to avoid repeating work) | O(n × m × k) | O(n × m) | n = key length, m = ring length, k = avg occurrences |
