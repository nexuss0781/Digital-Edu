...
The key to O(n) time is the "sequence start" check. By only counting from numbers where num-1 doesn't exist, each number is visited at most twice (once in the outer loop, once during counting), achieving Linear Time (Time grows directly with data) despite nested loops.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (sort) | O(n log n) | O(1) | Sort then scan for consecutive runs |
