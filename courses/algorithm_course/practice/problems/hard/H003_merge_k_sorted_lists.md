...
<details>
<summary>💡 Hint 1: Understanding the Slowest part of the process</summary>

Consider different approaches and their bottlenecks:

**Brute Force (Checking every single possibility): Merge lists one by one**
...
### Approach 1: Brute Force (Checking every single possibility) (Sequential Merging)
```
result = lists[0]
for i = 1 to k-1:
...
| Brute Force (Checking every single possibility) (collect + sort) | O(Nk log(Nk)) | O(Nk) | Loses linked list structure |
...
- O(Nk log k) matches divide-and-conquer Time Complexity (Speed of the algorithm)
...
- [ ] Day 1: Study solution, understand heap Rule that never changes
