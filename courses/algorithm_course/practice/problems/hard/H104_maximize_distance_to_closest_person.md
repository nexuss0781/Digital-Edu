...
Instead of storing all person positions and iterating again, maintain a running maximum while scanning. Track the previous person's index, and for each new person found, calculate the middle gap distance immediately. This reduces Space Complexity (Memory usage of the algorithm) from O(n) to O(1).
