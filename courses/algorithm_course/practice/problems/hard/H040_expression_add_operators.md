...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Backtracking | O(4^n * n) | O(n) | 4 choices at each position, n positions |
| Optimal (with Cutting off unnecessary steps) | O(4^n * n) | O(n) | Same worst case, but prunes invalid branches |
| Memoization (Saving results to avoid repeating work) attempt | Not applicable | - | Subproblems aren't overlapping |
