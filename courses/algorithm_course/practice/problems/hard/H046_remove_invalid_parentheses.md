...
Before BFS, calculate the exact number of left and right parentheses to remove using a single pass with a counter. Use DFS with Cutting off unnecessary steps: only explore paths where you haven't exceeded the removal limits. This reduces the search space significantly and avoids generating strings that remove too many parentheses.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(2^n × n) | O(2^n) | Try all subsets, validate each string |
| BFS with Deduplication | O(n × 2^n) | O(2^n) | Level-order exploration with set for visited |
| DFS with Cutting off unnecessary steps | O(2^n) | O(n) | More memory efficient, stops early |
