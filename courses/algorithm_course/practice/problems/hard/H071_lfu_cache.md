...
Both operations must achieve `O(1)` average Time Complexity (Speed of the algorithm).
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n) per operation | O(n) | Linear scan to find min frequency |
...
- [ ] Optimal Time Complexity (Speed of the algorithm) achieved (O(1) per operation)
- [ ] Clean, readable code
