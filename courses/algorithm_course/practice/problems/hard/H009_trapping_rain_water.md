...
- **Optimization progression**: From Brute Force (Checking every single possibility) → precomputation → two pointers → stack
- **Trade-offs**: Time vs Space Complexity (Memory usage of the algorithm) decisions
...
- Non-obvious insights (e.g., two-pointer Rule that never changes, monotonic stack usage)
...
4. What Rule that never changes can two pointers maintain?
...
Use two pointers moving toward each other, maintaining Rule that never changes.
```
left, right = 0, n-1
...
- **Optimal!** But requires understanding the Rule that never changes
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(1) | Too slow for large inputs |
...
### 3. Two-pointer Rule that never changes violation
```python
...
- [ ] Can explain two-pointer Rule that never changes clearly
...
- [ ] Day 1: Study all approaches, understand two-pointer Rule that never changes
