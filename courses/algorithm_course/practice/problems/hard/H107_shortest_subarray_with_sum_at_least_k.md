...
The deque maintains two invariants: (1) indices are in increasing order, (2) their corresponding prefix sums are in increasing order. This ensures each index is added and removed at most once, giving O(n) Time Complexity (Speed of the algorithm). The key is understanding why we pop from the back: if prefix[j] >= prefix[i] and j < i, then j can never be a better starting point than i for any future endpoint.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n^2) | O(1) | Check all subarrays |
...
1. **Trying to use sliding window with negative numbers**
   ...
   for right in range(len(nums)):
       ...
       while current_sum >= k:
           ...
           left += 1  # May skip Best possible solution with negatives
