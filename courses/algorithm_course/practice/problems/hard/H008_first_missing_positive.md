...
prerequisites: ["array-indexing", "in-place-modification", "Rule that never changes-maintenance"]
...
- **Rule that never changes maintenance**: Each element moves to its "correct" position
...
Total: O(n) time, O(1) space ✓
BEST: Clearest Rule that never changes, easiest to reason about
```

</details>

<details>
<summary>📝 Hint 3: Cyclic sort Step-by-step plan in plain English</summary>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (check 1,2,3...) | O(n²) | O(1) | Check each value linearly |
...
- [ ] Can describe cyclic sort Rule that never changes
