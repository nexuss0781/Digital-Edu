...
Also handle Edge Case (Unusual or extreme situation): if s2 cannot be derived from s1 at all (even once), return 0 immediately.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n1 × |s1| × |s2|) | O(1) | Simulate entire matching - TLE for large n1 |
