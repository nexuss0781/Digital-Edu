...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DP | O(n * k^2) | O(n * k) | Check all previous colors for each color |
...
3. **Not handling case when k=1**
   ...
   # Correct: Handle Edge Case (Unusual or extreme situation)
   if k == 1:
