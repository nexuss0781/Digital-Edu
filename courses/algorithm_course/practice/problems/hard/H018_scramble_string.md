...
Use recursive divide-and-conquer with Memoization (Saving results to avoid repeating work). For each possible split point i (1 to len-1), check two cases: (1) no swap - compare s1[0:i] with s2[0:i] and s1[i:] with s2[i:], (2) swap - compare s1[0:i] with s2[len-i:] and s1[i:] with s2[0:len-i]. Base cases: if strings are equal return True, if sorted strings differ return False (quick Cutting off unnecessary steps). Memoize results keyed by (s1, s2) to avoid recomputation.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) Recursion | O(4^n) | O(n) | Try all split points and swap combinations |
...
1. **Not Cutting off unnecessary steps with character frequency check**
   ```python
   ...
   def isScramble(s1, s2):
       if s1 == s2:
           return True
       if sorted(s1) != sorted(s2):  # Quick Cutting off unnecessary steps!
           return False
       # Now try splits...
   ```
...
3. **Inefficient Memoization (Saving results to avoid repeating work) with string concatenation**
   ```python
   # Wrong: Using expensive string slicing in Memoization (Saving results to avoid repeating work)
   memo = {}
   def helper(s1, s2):
