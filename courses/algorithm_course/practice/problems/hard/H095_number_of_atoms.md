...
Use a single pass through the string with index tracking. When parsing element names, peek ahead for lowercase letters. When parsing numbers, collect all consecutive digits. Maintain stack Rule that never changes: always have at least one map for the base level. Use collections.defaultdict to simplify count merging.
</details>
