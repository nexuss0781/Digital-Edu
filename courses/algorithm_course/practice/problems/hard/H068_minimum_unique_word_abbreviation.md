...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (all abbrevs) | O(2^m * n * m) | O(m) | m = target length, n = dictionary size |
| Optimal (ordered + Cutting off unnecessary steps) | O(2^m * n * m) | O(n * m) | Same worst case but Cutting off unnecessary steps helps in practice |
