...
When checking neighbors of a 0 cell, use a set to track unique island IDs to avoid counting the same island multiple times. Also handle the all-1s grid Edge Case (Unusual or extreme situation) early by checking if any 0 exists before iterating. This avoids unnecessary computation and incorrect results.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n^4) | O(n^2) | Try flipping each 0, run DFS each time |
