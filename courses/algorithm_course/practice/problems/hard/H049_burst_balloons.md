...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (Try All Orders) | O(n!) | O(n) | Generate all permutations, simulate each |
| DFS with Memoization (Saving results to avoid repeating work) | O(n³) | O(n²) | Try each balloon as last burst in each range |
...
2. **Not Adding Virtual Boundaries**
   ```python
   # Wrong: Complex Edge Case (Unusual or extreme situation) handling for first and last balloons
