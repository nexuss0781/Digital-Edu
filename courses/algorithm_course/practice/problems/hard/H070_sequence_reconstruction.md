...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (generate all perms) | O(n!) | O(n) | Check if all perms satisfy constraints |
| Topological Sort (Kahn's) | O(n + e) | O(n + e) | e = total edges from all sequences |
| Optimal (with early termination) | O(n + e) | O(n + e) | Same but better constants from Cutting off unnecessary steps |
