...
When you insert the number n into a permutation of [1..n-1], its position determines how many new inverse pairs are created. If you place n at position i from the right, it creates i inverse pairs with numbers to its right. This observation leads to a DP Formula that defines a step based on previous steps.
</details>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n! * n^2) | O(n) | Generate all permutations, count inversions |
...
1. **Using the naive DP recurrence without optimization**
   ```python
   # Wrong: O(n^2 * k) Time Complexity (Speed of the algorithm) - TLE
