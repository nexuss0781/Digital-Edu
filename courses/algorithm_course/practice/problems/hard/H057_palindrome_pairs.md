...
Your solution must achieve `O(sum of words[i].length)` Time Complexity (Speed of the algorithm).
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n² × k) | O(1) | Check all pairs, k = max word length |
