...
Given two sorted arrays `nums1` and `nums2`, return the median of the combined sorted array. The algorithm must run in **O(log(m+n))** Time Complexity (Speed of the algorithm).
...
- **Edge Case (Unusual or extreme situation) complexity**: Empty arrays, single elements, different lengths
