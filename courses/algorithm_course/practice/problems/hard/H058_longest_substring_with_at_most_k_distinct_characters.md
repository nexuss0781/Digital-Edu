...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n²) | O(k) | Check all substrings |
...
3. **Not handling k = 0 Edge Case (Unusual or extreme situation)**
   ```python
   # Wrong: Assuming k is always positive
   if not s or k <= 0:
       return 0  # Missing k == 0 case

   # Correct: Proper Edge Case (Unusual or extreme situation) handling
   if not s or k == 0:
       return 0
   if k >= len(set(s)):
       return len(s)
