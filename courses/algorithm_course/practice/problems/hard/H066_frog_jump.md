...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DFS | O(3^n) | O(n) | Try all 3 jump options at each step |
| DP with Memoization (Saving results to avoid repeating work) | O(n²) | O(n²) | At most n positions × n possible jump sizes |
| Optimal (BFS with Cutting off unnecessary steps) | O(n²) | O(n²) | Same complexity but better constant factors |
