...
- **Stack Rule that never changes maintenance**: Keeping stack monotonically increasing
- **Area calculation from boundaries**: Computing rectangles from left/right limits
- **Average performance over time**: Understanding why O(n) despite nested operations
...
**Approach 1: Brute Force (Checking every single possibility) - try all rectangles**
```
...
<details>
<summary>📝 Hint 3: Monotonic stack Step-by-step plan in plain English</summary>
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (all rectangles) | O(n³) | O(1) | Find min for each range |
...
- [ ] Can explain monotonic stack Rule that never changes
...
- [ ] Can explain Average performance over time
