...
To avoid redundant work, mark stops as visited in addition to buses. Once you've explored all buses from a particular stop, you never need to process that stop again. This prevents re-exploring the same bus connections multiple times and reduces Time Complexity (Speed of the algorithm) significantly.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) DFS | O(n^n) | O(n) | Try all bus sequences, exponential |
