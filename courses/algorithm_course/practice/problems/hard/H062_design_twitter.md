...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (collect all & sort) | O(n log n) per feed | O(n) | Where n is total tweets from all followees |
...
3. **Not handling self-following Edge Case (Unusual or extreme situation)**
   ```python
