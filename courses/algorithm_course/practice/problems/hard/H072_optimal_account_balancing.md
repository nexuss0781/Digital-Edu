...
Prune early: if you find a credit that exactly matches a debt, pair them immediately and remove both from consideration. Also, when backtracking, you can skip trying to settle a debt with multiple credits if you've already found a perfect match. Use Memoization (Saving results to avoid repeating work) to cache results for specific balance states if you convert the balance array to a hashable key.
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) | O(n! × 2^n) | O(n) | Try all possible transaction sequences |
| Backtracking with Cutting off unnecessary steps | O(2^n) | O(n) | n is number of non-zero balances (≤12) |
| Optimal (with Memoization (Saving results to avoid repeating work)) | O(2^n × n) | O(2^n × n) | Cache states to avoid recomputation |
