...
- **Formula that defines a step based on previous steps**: Three choices at each step (insert/delete/replace)
...
This hints at needing **Memoization (Saving results to avoid repeating work)** or **bottom-up DP**.
...
### Approach 1: Recursive with Memoization (Saving results to avoid repeating work) (Top-Down DP)
```
memo = {}
...
- **Learning:** Start with Memoization (Saving results to avoid repeating work), then 2D DP
...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) recursion | O(3^max(m,n)) | O(max(m,n)) | Exponential, TLE |
| Recursive + Memoization (Saving results to avoid repeating work) | O(mn) | O(mn) + O(m+n) | Memo table + recursion stack |
...
- [ ] Can derive Formula that defines a step based on previous steps from scratch
...
- [ ] Can explain time/Space Complexity (Memory usage of the algorithm)
