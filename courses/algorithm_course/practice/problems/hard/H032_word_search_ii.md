...
|----------|------|-------|-------|
| Brute Force (Checking every single possibility) (DFS per word) | O(w * m * n * 4^L) | O(L) | w=words, m*n=board, L=word length |
...
3. **Not Cutting off unnecessary steps empty Trie branches**
   ```python
