...
| **Time Complexity (Speed of the algorithm)** | O(n log n) typical (merge sort), varies by problem |
| **Space Complexity (Memory usage of the algorithm)** | O(log n) for recursion stack |
...
**Time Complexity (Speed of the algorithm)**: O(n log n)
- **Divide**: O(1) to find midpoint
- **Conquer**: 2 recursive calls on n/2 elements
- **Combine**: O(n) to merge

**Space Complexity (Memory usage of the algorithm)**: O(n) for temporary arrays during merging
...
**Time Complexity (Speed of the algorithm)**: O(log n)
**Space Complexity (Memory usage of the algorithm)**: O(log n) for recursive, O(1) for iterative

## Master Theorem (Simplified)

The Master Theorem helps analyze divide-and-conquer Time Complexity (Speed of the algorithm).
...
| Recurrence | Example | Time Complexity (Speed of the algorithm) |
|------------|---------|----------------|
...
**Key difference**: If subproblems overlap (same subproblem computed multiple times), use DP with Memoization (Saving results to avoid repeating work). If subproblems are independent, use divide-and-conquer.
...
| **Time Complexity (Speed of the algorithm)** | Often O(n log n), analyze with Master Theorem |
| **Space Complexity (Memory usage of the algorithm)** | O(log n) to O(n) depending on algorithm |
