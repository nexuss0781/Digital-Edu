...
| **Time Complexity (Speed of the algorithm)** | Often O(n log n) due to sorting, O(n) for iteration |
| **Space Complexity (Memory usage of the algorithm)** | O(1) to O(n) depending on problem |
...
**First Principle:** Greedy works when the problem has the "greedy choice property": a locally optimal choice is part of some globally Best possible solution. Unlike DP, you don't need to explore all options - one choice leads forward.
...
A greedy algorithm builds a solution by making the **locally optimal choice** at each step, hoping this leads to a globally Best possible solution.
...
A globally Best possible solution can be arrived at by making **locally optimal (greedy) choices**.

In other words: Making the best immediate choice will **not** prevent you from reaching the optimal final solution.

### 2. Optimal Substructure

An Best possible solution contains **optimal solutions to subproblems**.
...
**Top graph**: Greedy choice (+10) leads to Best possible solution (18).
...
**Time Complexity (Speed of the algorithm)**: O(n log n) for sorting, O(n) for selection.
...
Show that if you have an Best possible solution that differs from the greedy solution, you can **exchange** choices to make it more like the greedy solution without making it worse.

**Example (Activity Selection)**: If an Best possible solution picks activity X instead of the greedy choice (earliest ending activity Y), you can replace X with Y without losing any future activities.

### 2. Induction

Prove that after each greedy choice, the remaining problem has an Best possible solution that includes that choice.
...
**Why greedy fails**: The largest coin might not lead to the Best possible solution.
...
| **Time Complexity (Speed of the algorithm)** | Usually O(n log n) due to sorting |
