...
| **Time Complexity (Speed of the algorithm)** | O(n), O(n²), O(n×m) depending on state dimensions |
| **Space Complexity (Memory usage of the algorithm)** | O(n) to O(n×m), often optimizable |
...
**First Principle:** If a problem has optimal substructure (Best possible solution contains optimal solutions to subproblems) and overlapping subproblems (same subproblems recur), we can solve each subproblem once, store it, and reuse—transforming exponential recursion into polynomial iteration.
...
The Best possible solution can be constructed from **optimal solutions of subproblems**.
...
### Top-Down (Memoization (Saving results to avoid repeating work))
...
### Bottom-Up (Tabulation (Building a table of results from the bottom up))
...
5. **Implement**: Top-down (Memoization (Saving results to avoid repeating work)) or bottom-up (Tabulation (Building a table of results from the bottom up))
6. **Optimize space**: Can you reduce space from O(n) to O(1)?

## Time and Space Complexity (Memory usage of the algorithm)
...
| **Memoization (Saving results to avoid repeating work)** | Top-down DP with recursion |
...
| **Approaches** | Top-down (Memoization (Saving results to avoid repeating work)) or bottom-up (Tabulation (Building a table of results from the bottom up)) |
| **State Definition** | Critical step: what does `dp[i]` represent? |
| **Formula that defines a step based on previous steps** | How to compute `dp[i]` from previous states |
| **Time Complexity (Speed of the algorithm)** | Usually O(n), O(n²), or O(n³) |
