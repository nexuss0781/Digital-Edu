...
| **Time Complexity (Speed of the algorithm)** | O(log n) - halving search space each step |
| **Space Complexity (Memory usage of the algorithm)** | O(1) iterative, O(log n) recursive |
...
Binary search is a divide-and-conquer algorithm that eliminates half of the search space in each iteration. This powerful technique reduces Time Complexity (Speed of the algorithm) from O(n) to O(log n) by exploiting sorted or monotonic properties.
...
- **Time Complexity (Speed of the algorithm)**: O(log n) for search
