...
| **Time Complexity (Speed of the algorithm)** | O(n log n) comparison-based, O(n+k) non-comparison |
| **Space Complexity (Memory usage of the algorithm)** | O(1) to O(n) depending on algorithm |
...
- Need stable sort
- Linear Time (Time grows directly with data) required
...
| Small range integers | Counting Sort | Linear Time (Time grows directly with data) O(n+k) |
