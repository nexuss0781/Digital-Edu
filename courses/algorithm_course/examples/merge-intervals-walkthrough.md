...
def merge(intervals):
    ...
    """
    # Edge Case (Unusual or extreme situation): empty input
    if not intervals:
        return []
...
if not intervals:
    return []
```
- Handle empty input Edge Case (Unusual or extreme situation)
...
### Edge Case (Unusual or extreme situation) 1: Empty Array
...
### Edge Case (Unusual or extreme situation) 2: Single Interval
...
### Edge Case (Unusual or extreme situation) 3: No Overlaps
...
>
> Time Complexity (Speed of the algorithm) is O(n log n) from sorting, space is O(n) for the result array."
...
A: "Then we skip the sort step and have O(n) Time Complexity (Speed of the algorithm). We could add a check to see if the array is sorted, but that's O(n) anyway, so we might as well just sort."
